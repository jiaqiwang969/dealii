 ...
 * 这个模块包含在deal.II的异常机制中使用的类。 <h2>Brief
 * overview</h2>
 * 异常有两种不同的使用方式。  <ul>
 * <li>
 ...
 *
 * <li>
 * 动态断言。这些用于检查依赖于外部事物的条件，这些外部事物可能在一次程序运行中与下一次不同，例如，输出文件是否可以被写入。
 ...
 * <tt>deal.II</tt>中的错误处理机制一般有两种使用方式。
 * 第一种是只在调试模式下使用错误检查，对没有经过全面测试的程序很有用。当程序不再显示错误时，可以关闭错误处理，并以此获得更好的性能，因为在库中对错误的检查是相当频繁的（典型的速度提升是4倍！）。这种异常生成模式对于内部一致性检查是最有用的，比如范围检查或函数参数的有效性检查。这种类型的错误通常是编程错误，程序终止时应该有一个尽可能详细的信息，包括产生异常的位置和原因。
 ...
 * @endcode
 *
 * （注意，函数名称和确切的调用序列可能会随着时间的推移而改变，但一般的原则是不变的）。也就是说，如果给定的条件被违反，那么发生异常的文件和行以及条件本身和异常对象的调用序列就会被传递给
 * deal_II_exceptions::internals::issue_error_noreturn()
 ...
 * 如上所述，一旦对<tt>Assert</tt>的调用失败，程序就会终止。然而，有一种情况我们不想这样做，即当一个C++异常被激活时。发生这种情况的通常情况是，有人通过<tt>AssertThrow</tt>机制抛出了一个异常（见下文），在堆栈被解开的同时，导致了上面堆栈帧中其他对象的破坏。如果其他对象引用了被销毁的对象，一些析构器会通过<tt>Assert</tt>引发一个异常。如果我们当时中止程序，我们只会看到一个对象被销毁的消息，而这个对象仍然被某个地方引用，但我们永远不会看到触发这个异常的原始异常。(你可以在调试器中通过在函数<tt>__throw</tt>上设置断点来看到它，但你不能从程序本身看到它。)
 * 在这种情况下，我们使用一个C++标准库函数来检测另一个活动异常的存在，并且不终止程序，以允许抛出的异常传播到某个可以显示其信息的地方。
 * 由于一个失败的断言导致一连串的其他断言是很常见的，我们只打印第一个消息。如果程序被中止了，那就没有问题。如果不是这样（因为一个C++的异常是有效的），则只显示第一条，并显示一条关于被抑制的后续信息的消息。
 *
 ...
 * 如上所述， @p Assert
 * 宏的默认实现会将具体出错的详细信息打印到屏幕上，然后中止程序。终止程序是很有用的，因为它可以很容易地找到出错的地方
 *
 * - 包括我们如何到达那个地方的所有信息
 *
 * - 通过在调试器中运行该程序。
 ...
 * 这一切都很好，但C++不允许在类的析构器内或在当前从调用栈中更高的析构器中调用的函数中抛出异常。为此，有一个单独的宏，
