 ...
 * for
 * loops</a>的支持。基于deal.II的代码通常有许多这样的循环
 *
 * @code
 ...
 * 这些函数有一些变体，为所有单元（不仅仅是活动单元）和个别层次上的单元提供迭代器范围。
 ...
 * &)是`constexpr`，我们可以在编译时用一个数组初始化一个张量。
 *
 * @code
 * constexpr double[2][2] entries = {{1., 0.}, {0., 1.}};
 * constexpr Tensor<2, 2> A(entries);
 * @endcode
 * 这里，A的内容没有存储在堆栈中。相反，它们在编译时被初始化并插入到可执行程序的`.data`部分。程序可以在运行时使用这些值，而无需花费时间进行初始化。初始化张量可以用一行来简化。
 *
 * @code
 ...
 * 宏 @ref
 * DEAL_II_CONSTEXPR
 * 简化为`constexpr`，如果有一个具有C++14能力的编译器。否则，对于旧的编译器，它完全忽略了DEAL_II_CONSTEXPR。因此，在较新的编译器中，用户可以编写
 *
 * @code
 * constexpr double det_A = determinant(A);
 * @endcode
 * 假设`A`是用`constexpr`指定器声明的。这个例子显示了使用`constexpr`的性能收益，因为这里我们在编译时进行了一个具有
 * $O(\text{dim}^3)$ 复杂性的操作，避免了任何运行时成本。
 *
 *
 */
 ...
 *
 * @note
 * 如果使用的编译器确实支持C++20，那么这个命名空间的内容只是从命名空间`std`导入的类和函数。也就是说，我们回到了编译器提供的东西，而不是我们自己的实现。
 *
 *
 */
namespace std_cxx20
