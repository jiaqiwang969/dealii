 ...
 * 
 * 
 * 这些组别在教程程序中都有涉及，在 step-3
 * 中首次概述了它们的组合方式。下面是这个组的分类指南，以及与每个组有关的文档链接。
 ...
 * <li>
 * <b>%Manifold</b>:
 * 矩阵描述了单元格的形状，更广泛地说，描述了要解决方程的领域的几何形状。它们使用微分几何的语言。更多信息可以在
 * @ref manifold  中找到。
 ...
 * feall 模块中描述。
 * <li>  <b>%Quadrature</b>:
 * 与有限元一样，正交对象是在单元格上定义的。它们只描述单元格上正交点的位置，以及其上正交点的权重。
 ...
 * <li>
 * <b>%Mapping</b>。有限元程序的下一步是，人们希望利用有限元的形状函数和正交规则定义的正交点，计算三角形的每个单元上的矩阵和右侧条目或其他数量。为此，有必要将形状函数、正交点和正交权重从单元格映射到三角形的每个单元。这不是由映射和派生类直接完成的，而是由映射和派生类来促进的：它们描述了如何将点从单位空间映射到实空间并返回，以及提供这个导数的梯度和雅各布决定因素。
 * 这些类都在 @ref
 * mapping 模块中描述。
 * <li>
 * <b>%FEValues</b>。下一步是实际取一个有限元，在映射到实数单元时，在正交公式定义的点上评估其形状函数及其梯度。这就是FEValues类和兄弟姐妹的领域：从某种意义上说，它们提供了一个有限元函数空间的点式视图。
 * 这似乎有局限性：在数学分析中，我们总是以单元或单元面的积分来写公式，涉及到有限元形状函数。因此，人们会认为有必要将有限元空间描述为连续空间。然而，在实践中，这是没有必要的：在实际计算中，所有的积分都被使用正交公式的近似值所取代，因此真正需要的是在域内有限数量的给定位置评估形状函数的能力。FEValues类正是提供了这些信息。给定有限元、正交和映射对象，它们计算连续函数空间（相对于离散的，而不是相对于不连续的）对离散的点的限制。
 * 有许多对象可以做到这一点。FEValues用于对单元格进行评估，FEFaceValues用于对单元格的面进行评估，FESubfaceValues用于对单元格的部分面进行评估。所有这些类都在 @ref
 * feaccess 模块中描述。
 * <li>  <b>Linear Systems</b>:
 * 如果知道如何使用FEValues和朋友们评估单个单元上的形状函数的值和梯度，并且知道如何使用DoFHandler迭代器获得单元上自由度的全局数，那么下一步就是使用问题的双线性形式来组合线性系统的系统矩阵（和右手边）。然后我们将从这个线性系统中确定我们问题的解决方案。
 * 要做到这一点，我们需要有存储和管理矩阵和向量条目的类。deal.II为此提供了一整套的类，以及与其他提供类似功能的软件包的接口。这方面的文档可以在 @ref
 * LAC 模块中找到。
 * <li>
 * <b>Linear Solvers</b>:
 * 为了确定一个有限维度的线性方程组的解，人们需要线性求解器。在有限元应用中，它们经常是迭代式的，但有时也可能要使用直接或稀疏的直接求解器。deal.II有相当多的此类求解器。它们被记录在
 * @ref Solvers 模块中。
 * <li>
 ...
 * 我们提供了Doxygen标签文件，供那些想把应用程序的文档直接链接到deal.II在线文档的人使用。该标签文件在<a
