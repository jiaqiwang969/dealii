...
*   @note   In order to run this program, deal.II must be configured to usethe UMFPACK sparse direct solver. Refer to the <a
href="../../readme.html#umfpack">ReadMe</a> for instructions how to do this.
* <a name="Intro"></a><a name="Introduction"></a><h1>Introduction</h1>
* 

* This program shows how to use Runge-Kutta methods to solve a time-dependentproblem. It solves a small variation of the heat equation discussed first in  step-26   but, since the purpose of this program is only to demonstrate usingmore advanced ways to interface with deal.II's time stepping algorithms, onlysolves a simple problem on a uniformly refined mesh.
* 

* <a name="Problemstatement"></a><h3>Problem statement</h3>
* 

* In this example, we solve the one-group time-dependent diffusionapproximation of the neutron transport equation (see   step-28   for thetime-independent multigroup diffusion). This is a model for how neutrons movearound highly scattering media, and consequently it is a variant of thetime-dependent diffusion equation
* 
*  -  which is just a different name for theheat equation discussed in   step-26  , plus some extra terms.We assume that the medium is notfissible and therefore, the neutron flux satisfies the following equation:@f{eqnarray*}
\frac{1}{v}\frac{\partial \phi(x,t)}{\partial t} = \nabla \cdot D(x) \nabla \phi(x,t)
* 
- \Sigma_a(x) \phi(x,t) + S(x,t)
@f}
* augmented by appropriate boundary conditions. Here,   $v$   is the velocity ofneutrons (for simplicity we assume it is equal to 1 which can be achieved bysimply scaling the time variable),   $D$   is the diffusion coefficient,  $\Sigma_a$   is the absorption cross section, and   $S$   is a source. Because we areonly interested in the time dependence, we assume that   $D$   and   $\Sigma_a$   areconstant.
* Since this program only intends to demonstrate how to use advanced timestepping algorithms, we will only look for the solutions of relatively simpleproblems. Specifically, we are looking for a solution on a square domain  $[0,b]\times[0,b]$   of the form@f{eqnarray*}
...
* We impose the following boundary conditions: homogeneous Dirichlet for   $x=0$   and  $x=b$   and homogeneous Neumann conditions for   $y=0$   and   $y=b$  . We choose thesource term so that the corresponding solution isin fact of the form stated above:@f{eqnarray*}
...
* The Runge-Kutta methods implemented in deal.II assume that the equation to besolved can be written as:@f{eqnarray*}
\frac{dy}{dt} = g(t,y).
@f}
* On the other hand, when using finite elements, discretized time derivatives always result in thepresence of a mass matrix on the left hand side. This can easily be seen byconsidering that if the solution vector   $y(t)$   in the equation above is in fact the vectorof nodal coefficients   $U(t)$   for a variable of the form@f{eqnarray*}
  u_h(x,t) = \sum_j U_j(t) \varphi_j(x)
@f}
* with spatial shape functions   $\varphi_j(x)$  , then multiplying an equation ofthe form@f{eqnarray*}
  \frac{\partial u(x,t)}{\partial t} = q(t,u(x,t))
@f}
* by test functions, integrating over   $\Omega$  , substituting   $u\rightarrow u_h$  and restricting the test functions to the   $\varphi_i(x)$   from above, then thisspatially discretized equation has the form@f{eqnarray*}
M\frac{dU}{dt} = f(t,U),
@f}
* where   $M$   is the mass matrix and   $f(t,U)$   is the spatially discretized versionof   $q(t,u(x,t))$   (where   $q$   is typically the place where spatialderivatives appear, but this is not of much concern for the moment given thatwe only consider time derivatives). In other words, this form fits the generalscheme above if we write@f{eqnarray*}
\frac{dy}{dt} = g(t,y) = M^{-1}f(t,y).
@f}
* 
* Runke-Kutta methods are time stepping schemes that approximate   $y(t_n)\approx
y_{n}$   through a particular one-step approach. They are typically written in the form@f{eqnarray*}
y_{n+1} = y_n + \sum_{i=1}^s b_i k_i
@f}
* where for the form of the right hand side above@f{eqnarray*}
k_i = h M^{-1} f\left(t_n+c_ih,y_n+\sum_{j=1}^sa_{ij}k_j\right).
@f}
* Here   $a_{ij}$  ,   $b_i$  , and   $c_i$   are known coefficients that identify whichparticular Runge-Kutta scheme you want to use, and   $h=t_{n+1}-t_n$   is the time stepused. Different time stepping methods of the Runge-Kutta class differ in thenumber of stages   $s$   and the values they use for the coefficients   $a_{ij}$  ,  $b_i$  , and   $c_i$   but are otherwise easy to implement since one can look uptabulated values for these coefficients. (These tables are often calledButcher tableaus.)
* At the time of the writing of this tutorial, the methods implemented indeal.II can be divided in three categories:  <ol>    <li>   Explicit Runge-Kutta; in order for a method to be explicit, it isnecessary that in the formula above defining   $k_i$  ,   $k_i$   does not appearon the right hand side. In other words, these methods have to satisfy  $a_{ii}=0, i=1,\ldots,s$  .  <li>   Embedded (or adaptive) Runge-Kutta; we will discuss their properties below.  <li>   Implicit Runge-Kutta; this class of methods require the solution of apossibly nonlinear system the stages   $k_i$   above, i.e., they have  $a_{ii}\neq 0$   for at least one of the stages   $i=1,\ldots,s$  .  </ol>  Many well known time stepping schemes that one does not typically associatewith the names Runge or Kutta can in fact be written in a way so that they,too, can be expressed in these categories. They oftentimes represent thelowest-order members of these families.
* 

* <a name="ExplicitRungeKuttamethods"></a><h4>Explicit Runge-Kutta methods</h4>
* 

* These methods, only require a function to evaluate   $M^{-1}f(t,y)$   but not(as implicit methods) to solve an equation that involves  $f(t,y)$   for   $y$  . As all explicit time stepping methods, they become unstablewhen the time step chosen is too large.
...
* These methods use both a lower and a higher order method toestimate the error and decide if the time step needs to be shortened or can beincreased. The term "embedded" refers to the fact that the lower-order methoddoes not require additional evaluates of the function   $M^{-1}f(\cdot,\cdot)$  but reuses data that has to be computed for the high order method anyway. Itis, in other words, essentially free, and we get the error estimate as a sideproduct of using the higher order method.
...
* Implicit methods require the solution of (possibly nonlinear) systems of theform   $\alpha y = f(t,y)$  for   $y$   in each (sub-)timestep. Internally, this isdone using a Newton-type method and, consequently, they require that the userprovide functions that can evaluate   $M^{-1}f(t,y)$   and  $\left(I-\tau M^{-1} \frac{\partial f}{\partial y}\right)^{-1}$   or equivalently  $\left(M
* 
- \tau \frac{\partial f}{\partial y}\right)^{-1} M$  .
...
-M h(t,y)
@f}
* for some (given)   $h(t,y)$  . Implicit methods arealways stable, regardless of the time step size, but too large time steps ofcourse affect the <i>accuracy</i> of the solution, even if the numericalsolution remains stable and bounded.
* Methods in this class include backward Euler, implicit midpoint,Crank-Nicolson, and the two stage SDIRK method (short for "singly diagonallyimplicit Runge-Kutta", a term coined to indicate that the diagonal elements  $a_{ii}$   defining the time stepping method are all equal; this propertyallows for the Newton matrix   $I-\tau M^{-1}\frac{\partial f}{\partial y}$   tobe re-used between stages because   $\tau$   is the same every time).
* 

* <a name="Spatiallydiscreteformulation"></a><h3>Spatially discrete formulation</h3>
* 

* By expanding the solution of our model problemas always using shape functions   $\psi_j$   and writing@f{eqnarray*}
\phi_h(x,t) = \sum_j U_j(t) \psi_j(x),
@f}
* we immediately get the spatially discretized version of the diffusion equation as@f{eqnarray*}
  M \frac{dU(t)}{dt}
  =
* 
-{\cal D} U(t)
...
@f}
* See also   step-24   and   step-26   to understand how we arrive here.Boundary terms are not necessary due to the chosen boundary conditions forthe current problem. To use the Runge-Kutta methods, we recast thisas follows:@f{eqnarray*}
f(y) =
* 
-{\cal D}y
* 
- {\cal A}y + {\cal S}.
@f}
* In the code, we will need to be able to evaluate this function   $f(U)$   alongwith its derivative,@f{eqnarray*}
...
* To simplify the problem, the domain is two dimensional and the mesh isuniformly refined (there is no need to adapt the mesh since we use quadraticfinite elements and the exact solution is quadratic). Going from a twodimensional domain to a three dimensional domain is not verychallenging. However if you intend to solve more complex problems where themesh must be adapted (as is done, for example, in   step-26  ), then it isimportant to remember the following issues:
*   <ol>    <li>   You will need to project the solution to the new mesh when the mesh is changed. Of course,     the mesh     used should be the same from the beginning to the end of each time step,     a question that arises because Runge-Kutta methods use multiple     evaluations of the equations within each time step.  <li>   You will need to update the mass matrix and its inverse every time the     mesh is changed.  </ol>  The techniques for these steps are readily available by looking at   step-26  .
* 
...
* 
*  The next piece is the declaration of the main class. Most of the functions in this class are not new and have been explained in previous tutorials. The only interesting functions are   <code>evaluate_diffusion()</code>   and   <code>id_minus_tau_J_inverse()</code>. <code>evaluate_diffusion()</code>   evaluates the diffusion equation,   $M^{-1}(f(t,y))$  , at a given time and a given   $y$  .   <code>id_minus_tau_J_inverse()</code>   evaluates   $\left(I-\tau
...
* 
 @endcode
* 
*   <a name="codeDiffusionsetup_systemcode"></a>  <h4><code>Diffusion::setup_system</code></h4> Now, we create the constraint matrix and the sparsity pattern. Then, we initialize the matrices and the solution vector.
* 
...
*   Next, we evaluate the weak form of the diffusion equation at a given time   $t$   and for a given vector   $y$  . In other words, as outlined in the introduction, we evaluate   $M^{-1}(-{\cal D}y
* 
- {\cal A}y + {\cal
 S})$  . For this, we have to apply the matrix   $-{\cal D}
* 
- {\cal A}$   (previously computed and stored in the variable   <code>system_matrix</code>  ) to   $y$   and then add the source term which we integrate as we usually do. (Integrating up the solution could be done using   VectorTools::create_right_hand_side()   if you wanted to save a few lines of code, or wanted to take advantage of doing the integration in parallel.) The result is then multiplied by   $M^{-1}$  .
* 
...
*   This function is the driver for all the explicit methods. At the top it initializes the time stepping and the solution (by setting it to zero and then ensuring that boundary value and hanging node constraints are respected; of course, with the mesh we use here, hanging node constraints are not in fact an issue). It then calls   <code>evolve_one_time_step</code>   which performs one time step. Time is stored and incremented through a DiscreteTime object.   
*   For explicit methods,   <code>evolve_one_time_step</code>   needs to evaluate   $M^{-1}(f(t,y))$  , i.e, it needs   <code>evaluate_diffusion</code>  . Because   <code>evaluate_diffusion</code>   is a member function, it needs to be bound to   <code>this</code>  . After each evolution step, we again apply the correct boundary values and hanging node constraints.   
...
* 
 @endcode
* 
*   <a name="codeDiffusionimplicit_methodcode"></a>  <h4><code>Diffusion::implicit_method</code></h4> This function is equivalent to   <code>explicit_method</code>   but for implicit methods. When using implicit methods, we need to evaluate   $M^{-1}(f(t,y))$   and   $\left(I-\tau M^{-1} \frac{\partial f(t,y)}{\partial
...
* 
 @endcode
* 
*   <a name="codeDiffusionembedded_explicit_methodcode"></a>  <h4><code>Diffusion::embedded_explicit_method</code></h4> This function is the driver for the embedded explicit methods. It requires more parameters:
* 
...
* 
* 
*  - Automatically adjusting the step size of the last time step to ensure simulation ends precisely at   <code>final_time</code>  . This adjustment is handled inside the DiscreteTime instance.
* 
...
* 
*  The following   <code>main</code>   function is similar to previous examples and need not be commented on.
* 
