...
* deal.II provides a number of high level entry points to impose periodicboundary conditions.The general approach to apply periodic boundary conditions consists ofthree steps (see also the  @ref GlossPeriodicConstraints   "Glossary entry on periodic boundary conditions"):
* 
*  - Create a mesh
* 
...
* The second and third step are necessary for parallel meshes using the  parallel::distributed::Triangulation   classto ensure that cells on opposite sides of the domain but connected by periodicfaces are part of the ghost layer if one of them is stored on the local processor.If the Triangulation is not a   parallel::distributed::Triangulation,  these steps are not necessary.
* The first step consists of collecting matching periodic faces and storing them ina   <code>std::vector</code>   of   GridTools::PeriodicFacePair.   This is done with thefunction   GridTools::collect_periodic_faces()   that can be invoked for examplelike this:
* @code
...
@endcode
* 
* This call loops over all faces of the container dof_handler on the periodicboundaries with boundary indicator   @p b_id1   and   @p b_id2,  respectively. (You can assign these boundary indicators by hand aftercreating the coarse mesh, see  @ref GlossBoundaryIndicator   "Boundary indicator". Alternatively, youcan also let many of the functions in namespace GridGenerator do thisfor if you specify the "colorize" flag; in that case, these functionswill assign different boundary indicators to different parts of theboundary, with the details typically spelled out in the documentationof these functions.)
* Concretely, if   $\text{vertices}_{1/2}$   are the vertices of two faces  $\text{face}_{1/2}$  , then the function call above will match pairs offaces (and dofs) such that the difference between   $\text{vertices}_2$  and   $matrix\cdot \text{vertices}_1+\text{offset}$   vanishes in everycomponent apart from direction and stores the resulting pairs withassociated data in   @p matched_pairs.   (See  GridTools::orthogonal_equality()   for detailed information about thematching process.)
* Consider, for example, the colored unit square   $\Omega=[0,1]^2$   with boundaryindicator 0 on the left, 1 on the right, 2 on the bottom and 3 on the topfaces. (See the documentation of   GridGenerator::hyper_cube()   for thisconvention on how boundary indicators are assigned.) Then,
* @code
GridTools::collect_periodic_faces(dof_handler,
                                   /*b_id1*/  0,
                                   /*b_id2*/  1,
...
* If we instead consider the parallelogram given by the convex hull of  $(0,0)$  ,   $(1,1)$  ,   $(1,2)$  ,   $(0,1)$   we can achieve the constraints  $u(0,y)=u(1,y+1)$   by specifying an   @p offset:  
* @code
GridTools::collect_periodic_faces(dof_handler,
                                   /*b_id1*/  0,
                                   /*b_id2*/  1,
...
GridTools::collect_periodic_faces(dof_handler,
                                  ...
                                  Tensor<1, 2>(1.,1.));
@endcode
* Here, again, the assignment of boundary indicators 0 and 1 stems fromwhat   GridGenerator::parallelogram()   documents.
* The resulting   @p matched_pairs   can be used in  DoFTools::make_periodicity_constraints   for populating an AffineConstraintsobject with periodicity constraints:
* @code
DoFTools::make_periodicity_constraints(matched_pairs, constraints);
@endcode
* 
* Apart from this high level interface there are also variants of  DoFTools::make_periodicity_constraints   available that combine those twosteps (see the variants of   DofTools::make_periodicity_constraints).  
* There is also a low level interface to  DoFTools::make_periodicity_constraints   if more flexibility is needed. Thelow level variant allows to directly specify two faces that shall beconstrained:
* @code
...
make_periodicity_constraints(face_1,
                             ...
                             matrix = <default value>);
@endcode
* Here, we need to specify the orientation of the two faces using  @p face_orientation,     @p face_flip   and   @p face_orientation.   For a closer descriptionhave a look at the documentation of   DoFTools::make_periodicity_constraints.  The remaining parameters are the same as for the high level interface apartfrom the self-explaining   @p component_mask   and   @p affine_constraints.  
* 
...
* On a quarter-circle defined by   $\Omega=\{{\bf x}\in(0,1)^2:\|{\bf x}\|\in (0.5,1)\}$   we aregoing to solve the Stokes problem@f{eqnarray*}
* 
...
@f}
* 
* The mesh will be generated by   GridGenerator::quarter_hyper_shell(),  which also documents how it assigns boundary indicators to its variousboundaries if its `colorize` argument is set to `true`.
* 
...
* 
* 
*  -   <code>StokesProblem<dim>::setup_dofs()</code>  : To populate an AffineConstraints object with periodicity constraints
* 

* 
* 
*  -   <code>StokesProblem<dim>::create_mesh()</code>  : To supply a distributed triangulation with periodicity information.
* 
