...
@endcode
* then the compiler will deduce that the function   <code>make_grid</code>   for  <code>dim==2</code>   wasrequested and will compile the template above into a function with dim replacedby 2 everywhere, i.e. it will compile the function as if it were definedas
* @code
...
* 
* However, it is worth to note that the function  <code>GridGenerator::hyper_cube</code>   depends on the dimension aswell, so in this case, the compiler will call the function  <code>GridGenerator::hyper_cube@<2@></code>   while if dim were 3,it would call   <code>GridGenerator::hyper_cube@<3@></code>   whichmight be (and actually is) a totally unrelated  function.
* 
...
@endcode
* This function has a member variable of type  <code>DoFHandler@<dim@></code>  . Again, the compiler can'tcompile this function until it knows for which dimension. If you callthis function for a specific dimension as above, the compiler willtake the template, replace all occurrences of dim by the dimension forwhich it was called, and compile it. If you call the function severaltimes for different dimensions, it will compile it several times, eachtime calling the right   <code>make_grid</code>   function and reserving the rightamount of memory for the member variable; note that the size of a  <code>DoFHandler</code>   might, and indeed does, depend on the space dimension.
* 

* 
* The deal.II library is built around this conceptof dimension-independent programming, and therefore allows you to program ina way that will not need todistinguish between the space dimensions. It should be noted that inonly a very few places is it necessary to actually compare thedimension using   <code>if</code>s or <code>switch</code>  es. However, since the compilerhas to compile each function for each dimension separately, even thereit knows the value of   <code>dim</code>   at the time of compilation and willtherefore be able to optimize away the   <code>if</code>   statement along with theunused branch.
* 
...
* 
*  Each of these classes is derived from a common, abstract base class Function, which declares the common interface which all functions have to follow. In particular, concrete classes have to overload the   <code>value</code>   function, which takes a point in dim-dimensional space as parameters and returns the value at that point as a   <code>double</code>   variable.
* 

* 
*  The   <code>value</code>   function takes a second argument, which we have here named   <code>component</code>  : This is only meant for vector-valued functions, where you may want to access a certain component of the vector at the point   <code>p</code>  . However, our functions are scalar, so we need not worry about this parameter and we will not use it in the implementation of the functions. Inside the library's header files, the Function base class's declaration of the   <code>value</code>   function has a default value of zero for the component, so we will access the   <code>value</code>   function of the right hand side with only one parameter, namely the point where we want to evaluate the function. A value for the component can then simply be omitted for scalar functions.
* 
...
* 
*  The last thing to note is that a   <code>Point@<dim@></code>   denotes a point in dim-dimensional space, and its individual components (i.e.   $x$  ,   $y$  , ... coordinates) can be accessed using the () operator (in fact, the [] operator will work just as well) with indices starting at zero as usual in C and C++.
* 
...
* 
*  After this introduction, here is the constructor of the   <code>Step4</code>   class. It specifies the desired polynomial degree of the finite elements and associates the DoFHandler to the triangulation just as in the previous example program,   step-3  :
* 
...
* 
 @endcode
* 
*  Next, we again have to loop over all cells and assemble local contributions.  Note, that a cell is a quadrilateral in two space dimensions, but a hexahedron in 3D. In fact, the   <code>active_cell_iterator</code>   data type is something different, depending on the dimension we are in, but to the outside world they look alike and you will probably never see a difference. In any case, the real type is hidden by using `auto`:
* 
...
* 
*  Since the program will run both 2d and 3d versions of the Laplace solver, we use the dimension in the filename to generate distinct filenames for each run (in a better program, one would check whether   <code>dim</code>   can have other values than 2 or 3, but we neglect this here for the sake of brevity).
* 
...
* 
*  And this is the main function. It also looks mostly like in   step-3  , but if you look at the code below, note how we first create a variable of type   <code>Step4@<2@></code>   (forcing the compiler to compile the class template with   <code>dim</code> replaced by <code>2</code>  ) and run a 2d simulation, and then we do the whole thing over in 3d.
* 
...
* 
*  Each of the two blocks is enclosed in braces to make sure that the   <code>laplace_problem_2d</code>   variable goes out of scope (and releases the memory it holds) before we move on to allocate memory for the 3d case. Without the additional braces, the   <code>laplace_problem_2d</code>   variable would only be destroyed at the end of the function, i.e. after running the 3d problem, and would needlessly hog memory while the 3d run could actually use it.
* 
...
* 
* Essentially the possibilities for playing around with the program are the sameas for the previous one, except that they will now also apply to the 3dcase. For inspiration read up on <a href="step_3.html#extensions"
