...
<ol>
  <li> <a href="#Intro" class=bold>Introduction</a><a href="#Intro" class=bold>Introduction</a>
    <ul>
        <li><a href="#Loadbalancing">Load balancing</a><a href="#Loadbalancing">Load balancing</a>
        <li><a href="#hpdecisionindicators">hp-decision indicators</a><a href="#hpdecisionindicators">hp-decision indicators</a>
        <li><a href="#Hybridgeometricmultigrid">Hybrid geometric multigrid</a><a href="#Hybridgeometricmultigrid">Hybrid geometric multigrid</a>
        <li><a href="#Thetestcase">The test case</a><a href="#Thetestcase">The test case</a>
    </ul>
  <li> <a href="#CommProg" class=bold>The commented program</a><a href="#CommProg" class=bold>The commented program</a>
    <ul>
        <li><a href="#Includefiles">Include files</a><a href="#Includefiles">Include files</a>
        <li><a href="#ThecodeSolutioncodeclasstemplate">The <code>Solution</code> class template</a><a href="#ThecodeSolutioncodeclasstemplate">The <code>Solution</code> class template</a>
        <li><a href="#Parameters">Parameters</a><a href="#Parameters">Parameters</a>
        <li><a href="#MatrixfreeLaplaceoperator">Matrix-free Laplace operator</a><a href="#MatrixfreeLaplaceoperator">Matrix-free Laplace operator</a>
        <li><a href="#Solverandpreconditioner">Solver and preconditioner</a><a href="#Solverandpreconditioner">Solver and preconditioner</a>
      <ul>
        <li><a href="#Conjugategradientsolverwithmultigridpreconditioner">Conjugate-gradient solver with multigrid preconditioner</a><a href="#Conjugategradientsolverwithmultigridpreconditioner">Conjugate-gradient solver with multigrid preconditioner</a>
        <li><a href="#Hybridpolynomialgeometricglobalcoarseningmultigridpreconditioner">Hybrid polynomial/geometric-global-coarsening multigrid preconditioner</a><a href="#Hybridpolynomialgeometricglobalcoarseningmultigridpreconditioner">Hybrid polynomial/geometric-global-coarsening multigrid preconditioner</a>
      </ul>
      ...
      <ul>
        ...
        <li><a href="#LaplaceProbleminitialize_grid">LaplaceProblem::initialize_grid</a><a href="#LaplaceProbleminitialize_grid">LaplaceProblem::initialize_grid</a>
        ...
        <li><a href="#LaplaceProblemprint_diagnostics">LaplaceProblem::print_diagnostics</a><a href="#LaplaceProblemprint_diagnostics">LaplaceProblem::print_diagnostics</a>
        <li><a href="#LaplaceProblemsolve_system">LaplaceProblem::solve_system</a><a href="#LaplaceProblemsolve_system">LaplaceProblem::solve_system</a>
        <li><a href="#LaplaceProblemcompute_indicators">LaplaceProblem::compute_indicators</a><a href="#LaplaceProblemcompute_indicators">LaplaceProblem::compute_indicators</a>
        <li><a href="#LaplaceProblemadapt_resolution">LaplaceProblem::adapt_resolution</a><a href="#LaplaceProblemadapt_resolution">LaplaceProblem::adapt_resolution</a>
  ...
  <li value="3"> <a href="#Results" class=bold>Results</a><a href="#Results" class=bold>Results</a>
    <ul>
      ...
      <ul>
        ...
        <li><a href="#Solvewithmatrixbasedmethods">Solve with matrix-based methods</a><a href="#Solvewithmatrixbasedmethods">Solve with matrix-based methods</a>
        <li><a href="#Multigridvariants">Multigrid variants</a><a href="#Multigridvariants">Multigrid variants</a>
    </ul>
    </ul>
...
* 
*   @note   As a prerequisite of this program, you need to have the p4estlibrary and the Trilinos library installed. The installation of deal.IItogether with these additional libraries is described in the <a
...
* One could balance the workload by assigning weights to every cell whichare proportional to the number of degrees of freedom, and balance thesum of all weights between all processes. Assigning individual weightsto each cell can be realized with the class   parallel::CellWeights   whichwe will use later.
* 
...
@f]where   $\delta_{ij}$   is the Kronecker delta, and   $J_K$   is the Jacobian ofthe mapping from   $\hat{K}$   to   $K$  , which (in this tutorial) is assumedto be constant (i.e., the mapping must be affine).
* Hence, combining all these assumptions, the projection matrix forexpressing   $u_\text{hp}(x)$   in the Legendre basis is just   $\det(J_K) \,
\mathbb{I}$  
* 
...
- \sigma k .
@f]
* We will perform this fit on each cell   $K$   to get a local estimate forthe smoothness of the finite element approximation. The decay rate  $\sigma_K$   then acts as the decision indicator for hp-adaptation. For afinite element on a cell   $K$   with a polynomial degree   $p$  , calculatingthe coefficients for   $k \leq (p+1)$   proved to be a reasonable choice toestimate smoothness. You can find a more detailed and dimensionindependent description in   @cite fehling2020  .
* All of the above is already implemented in the   FESeries::Legendre   classand the   SmoothnessEstimator::Legendre   namespace. With the errorestimates and smoothness indicators, we are then left to flag the cellsfor actual refinement and coarsening. Some functions from the  parallel::distributed::GridRefinement   and   hp::Refinement   namespaces willhelp us with that later.
* 

* <a name="Hybridgeometricmultigrid"></a><h3>Hybrid geometric multigrid</h3>
* 

* Finite element matrices are typically very sparse. Additionally,hp-adaptive methods correspond to matrices with highly variable numbersof nonzero entries per row. Some state-of-the-art preconditioners, likethe algebraic multigrid (AMG) ones as used in   step-40  , behave poorly inthese circumstances.
* We will thus rely on a matrix-free hybrid multigrid preconditioner.  step-50   has already demonstrated the superiority of geometric multigridmethods method when combined with the MatrixFree framework. Theapplication on hp-adaptive FEM requires some additional work thoughsince the children of a cell might have different polynomial degrees. Asa remedy, we perform a p-relaxation to linear elements first (similar toMitchell   @cite mitchell2010hpmg  ) and then perform h-relaxation in theusual manner. On the coarsest level, we apply an algebraic multigridsolver. The combination of p-multigrid, h-multigrid, and AMG makes thesolver to a hybrid multigrid solver.
...
* For elliptic equations, each reentrant corner typically invokes asingularity   @cite brenner2008  . We can use this circumstance to put ourhp-decision algorithms to a test: on all cells to be adapted, we wouldprefer a fine grid near the singularity, and a high polynomial degreeotherwise.
...
* The deal.II library offers multiple strategies to decide which type ofadaptation to impose on cells: either adjust the grid resolution orchange the polynomial degree. We only presented the <i>Legendre
coefficient decay</i> strategy in this tutorial, while   step-27  demonstrated the <i>Fourier</i> equivalent of the same idea.
* See the "possibilities for extensions" section of   step-27   for anoverview over these strategies, or the corresponding documentationfor a detailed description.
* There, another strategy is mentioned that has not been shown in anytutorial so far: the strategy based on <i>refinement history</i>. Theusage of this method for parallel distributed applications is moretricky than the others, so we will highlight the challenges that comealong with it. We need information about the final state of refinementflags, and we need to transfer the solution across refined meshes. Forthe former, we need to attach the   hp::Refinement::predict_error()  function to the   Triangulation::Signals::post_p4est_refinement   signal ina way that it will be called <i>after</i> the  hp::Refinement::limit_p_level_difference()   function. At this stage, allrefinement flags and future FE indices are terminally set and a reliableprediction of the error is possible. The predicted error then needs tobe transferred across refined meshes with the aid of  parallel::distributed::CellDataTransfer.  
* Try implementing one of these strategies into this tutorial and observethe subtle changes to the results. You will notice that all strategiesare capable of identifying the singularities near the reentrant cornersand will perform   $h$  -refinement in these regions, while preferring  $p$  -refinement in the bulk domain. A detailed comparison of thesestrategies is presented in   @cite fehling2020   .
* 
...
* To create a system matrix, you can either use the  LaplaceOperator::get_system_matrix()   function, or use an  <code>assemble_system()</code>   function similar to the one of   step-27  .You can then pass the system matrix to the solver as usual.
