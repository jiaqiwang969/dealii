...
<ol>
  <li> <a href="#Intro" class=bold>Introduction</a><a href="#Intro" class=bold>Introduction</a>
    <ul>
        <li><a href="#Whatstheissue"> What's the issue? </a><a href="#Whatstheissue"> What's the issue? </a>
        <li><a href="#Whattodoinstead"> What to do instead? </a><a href="#Whattodoinstead"> What to do instead? </a>
        <li><a href="#DerivationoftheC0IPmethod"> Derivation of the C0IP method </a><a href="#DerivationoftheC0IPmethod"> Derivation of the C0IP method </a>
      <ul>
        <li><a href="#ConvergenceRates">Convergence Rates </a><a href="#ConvergenceRates">Convergence Rates </a>
      </ul>
        <li><a href="#OtherBoundaryConditions">Other Boundary Conditions</a><a href="#OtherBoundaryConditions">Other Boundary Conditions</a>
  ...
  <li> <a href="#CommProg" class=bold>The commented program</a><a href="#CommProg" class=bold>The commented program</a>
    <ul>
      ...
      <ul>
        ...
        <li><a href="#Solvingthelinearsystemandpostprocessing">Solving the linear system and postprocessing</a><a href="#Solvingthelinearsystemandpostprocessing">Solving the linear system and postprocessing</a>
      </ul>
  ...
  <li value="3"> <a href="#Results" class=bold>Results</a><a href="#Results" class=bold>Results</a>
    <ul>
        <li><a href="#TestresultsoniQsub2subiwithigammapp1i">Test results on <i>Q<sub>2</sub></i><a href="#TestresultsoniQsub2subiwithigammapp1i">Test results on <i>Q<sub>2</sub></i> with <i>&gamma; = p(p+1)</i><i>Q<sub>2</sub></i> with <i>&gamma; = p(p+1)</i> </a><i>&gamma; = p(p+1)</i> </a>
        <li><a href="#TestresultsoniQsub3subiwithigammapp1i">Test results on <i>Q<sub>3</sub></i><a href="#TestresultsoniQsub3subiwithigammapp1i">Test results on <i>Q<sub>3</sub></i> with <i>&gamma; = p(p+1)</i><i>Q<sub>3</sub></i> with <i>&gamma; = p(p+1)</i> </a><i>&gamma; = p(p+1)</i> </a>
        <li><a href="#TestresultsoniQsub4subiwithigammapp1i">Test results on <i>Q<sub>4</sub></i><a href="#TestresultsoniQsub4subiwithigammapp1i">Test results on <i>Q<sub>4</sub></i> with <i>&gamma; = p(p+1)</i><i>Q<sub>4</sub></i> with <i>&gamma; = p(p+1)</i> </a><i>&gamma; = p(p+1)</i> </a>
        <li><a href="#TestresultsoniQsub2subiwithigamma1i">Test results on <i>Q<sub>2</sub></i><a href="#TestresultsoniQsub2subiwithigamma1i">Test results on <i>Q<sub>2</sub></i> with <i>&gamma; = 1</i><i>Q<sub>2</sub></i> with <i>&gamma; = 1</i> </a><i>&gamma; = 1</i> </a>
        <li><a href="#TestresultsoniQsub2subiwithigamma2i">Test results on <i>Q<sub>2</sub></i><a href="#TestresultsoniQsub2subiwithigamma2i">Test results on <i>Q<sub>2</sub></i> with <i>&gamma; = 2</i><i>Q<sub>2</sub></i> with <i>&gamma; = 2</i> </a><i>&gamma; = 2</i> </a>
        <li><a href="#Conclusionsforthechoiceofthepenaltyparameter"> Conclusions for the choice of the penalty parameter </a><a href="#Conclusionsforthechoiceofthepenaltyparameter"> Conclusions for the choice of the penalty parameter </a>
        <li><a href="#Possibilitiesforextensions"> Possibilities for extensions </a><a href="#Possibilitiesforextensions"> Possibilities for extensions </a>
      <ul>
        <li><a href="#Derivationforthesimplysupportedplates"> Derivation for the simply supported plates </a><a href="#Derivationforthesimplysupportedplates"> Derivation for the simply supported plates </a>
    </ul>
    </ul>
...
* @f{align*}{
  ...
  &&\forall \mathbf x \in \partial\Omega,
@f}
* corresponds to the edges of the thin structure attached to the top ofa wall of height   $g(\mathbf x)$   in such a way that the bending forcesthat act on the structure are   $h(\mathbf x)$  ; in most physicalsituations, one will have   $h=0$  , corresponding to the structure simplysitting atop the wall.
...
* 
*  -  but because both the secondderivatives of the test functions and of the trial functions yield adelta function, we would try to integrate the product of two deltafunctions. For example, in 1d, where   $\varphi_i$   are the usualpiecewise linear "hat functions", we would get integrals of the sort
* @f{align*}{
  \int_0^L (\Delta \varphi_i) (\Delta \varphi_j)
  =
  \int_0^L
  \frac 1h \left[\delta(x-x_{i-1})
* 
- 2\delta(x-x_i) + \delta(x-x_{i+1})\right]
  \frac 1h \left[\delta(x-x_{j-1})
* 
- 2\delta(x-x_j) + \delta(x-x_{j+1})\right]
@f}
* where   $x_i$   is the node location at which the shape function  $\varphi_i$   is defined, and   $h$   is the mesh size (assumeduniform). The problem is that delta functions in integrals are definedusing the relationship
* @f{align*}{
  \int_0^L \delta(x-\hat x) f(x) \; dx
  =
  f(\hat x).
@f}
* But that only works if (i)   $f(\cdot)$   is actually well defined at  $\hat x$  , and (ii) if it is finite. On the other hand, an integral ofthe form
* @f{align*}{
...
* @f{align*}{
  ...
  &&\forall \mathbf x \in \partial\Omega,
@f}
* then the following trick works (at least if the domain is convex, seebelow): In the same way as we obtained themixed Laplace equation of   step-20   from the regular Laplace equation byintroducing a second variable, we can here introduce a variable  $v=\Delta u$   and can then replace the equations above by thefollowing, "mixed" system:
* @f{align*}{
* 
...
-f(\mathbf x)
  ...
  &&\forall \mathbf x \in \partial\Omega.
@f}
* In other words, we end up with what is in essence a system of twocoupled Laplace equations for   $u,v$  , each with Dirichlet-type boundaryconditions. We know how to solve such problems, and it should not bevery difficult to construct good solvers and preconditioners for thissystem either using the techniques of   step-20   or   step-22  . So thiscase is pretty simple to deal with.
*   @note   It is worth pointing out that this only works for domains whose  boundary has corners if the domain is also convex
* 
*  -  in other words,  if there are no re-entrant corners.  This sounds like a rather random condition, but it makes  sense in view of the following two facts: The solution of the  original biharmonic equation must satisfy   $u\in H^2(\Omega)$  . On the  other hand, the mixed system reformulation above suggests that both    $u$   and   $v$   satisfy   $u,v\in H^1(\Omega)$   because both variables only  solve a Poisson equation. In other words, if we want to ensure that  the solution   $u$   of the mixed problem is also a solution of the  original biharmonic equation, then we need to be able to somehow  guarantee that the solution of   $-\Delta u=v$   is in fact more smooth  than just   $H^1(\Omega)$  . This can be argued as follows: For convex  domains,  <a href="https://en.wikipedia.org/wiki/Elliptic_operator#Elliptic_regularity_theorem">"elliptic
  regularity"</a> implies that if the right hand side   $v\in H^s$  , then    $u\in H^{s+2}$   if the domain is convex and the boundary is smooth  enough. (This could also be guaranteed if the domain boundary is  sufficiently smooth
* 
...
* @f{align*}{
  ...
  &&\forall \mathbf x \in \partial\Omega.
@f}
* The same trick with the mixed system does not work here, because wewould end up with <i>both</i> Dirichlet and Neumann boundary conditions for  $u$  , but none for   $v$  .
* 

* The solution to this conundrum arrived with the Discontinuous Galerkinmethod wave in the 1990s and early 2000s: In much the same way as onecan use <i>discontinuous</i> shape functions for the Laplace equationby penalizing the size of the discontinuity to obtain a scheme for anequation that has one derivative on each shape function, we can use ascheme that uses <i>continuous</i> (but not   $C^1$   continuous) shapefunctions and penalize the jump in the derivative to obtain a schemefor an equation that has two derivatives on each shape function. Inanalogy to the Interior Penalty (IP) method for the Laplace equation,this scheme for the biharmonic equation is typically called the   $C^0$   IP(or C0IP) method, since it uses   $C^0$   (continuous but not continuouslydifferentiable) shape functions with an interior penalty formulation.
* 

* <a name="DerivationoftheC0IPmethod"></a><h3> Derivation of the C0IP method </h3>
* 

* We base this program on the   $C^0$   IP method presented by SusanneBrenner and Li-Yeng Sung in the paper "C  $^0$   Interior Penalty Methodfor Linear Fourth Order Boundary Value Problems on polygonaldomains''   @cite Brenner2005   , where the method isderived for the biharmonic equation with "clamped" boundaryconditions.
* As mentioned, this method relies on the use of   $C^0$   Lagrange finiteelements where the   $C^1$   continuity requirement is relaxed and hasbeen replaced with interior penalty techniques. To derive this method,we consider a   $C^0$   shape function   $v_h$   which vanishes on  $\partial\Omega$  . We introduce notation   $ \mathbb{F} $   as the set ofall faces of   $\mathbb{T}$  ,   $ \mathbb{F}^b $   as the set of boundary faces,and   $ \mathbb{F}^i $   as the set of interior faces for use further down below.Since the higher order derivatives of   $v_h$   have twovalues on each interface   $e\in \mathbb{F}$   (shared by the two cells  $K_{+},K_{-} \in \mathbb{T}$  ), we cope with this discontinuity bydefining the following single-valued functions on   $e$  :
* @f{align*}{
...
@f}
* for   $k =1,2$   (i.e., for the gradient and the matrix of secondderivatives), and where   $\mathbf n$   denotes a unit vector normal to  $e$   pointing from   $K_+$   to   $K_-$  . In theliterature, these functions are referred to as the "jump" and"average" operations, respectively.
* To obtain the   $C^0$   IP approximation   $u_h$  , we left multiply thebiharmonic equation by   $v_h$  , and then integrate over   $\Omega$  . Asexplained above, we can't do the integration by parts on all of  $\Omega$   with these shape functions, but we can do it on each cellindividually since the shape functions are just polynomials on eachcell. Consequently, we start by using the followingintegration-by-parts formula on each mesh cell   $K \in {\mathbb{T}}$  :
* @f{align*}{
...
* Instead, what we do is recognize that  $\nabla\Delta w_h = \text{grad}\,(\text{div}\,\text{grad}\, w_h)$  , and wecan re-sort these operations as  $\nabla\Delta w_h = \text{div}\,(\text{grad}\,\text{grad}\, w_h)$   where wetypically write   $\text{grad}\,\text{grad}\, w_h = D^2 w_h$   to indicatethat this is the "Hessian" matrix of second derivatives. With thisre-ordering, we can now integrate the divergence, rather than thegradient operator, and we get the following instead:
* @f{align*}{
  \int_K v_h (\Delta^2 w_h)
  &= \int_K (\nabla \nabla v_h) : (\nabla \nabla w_h)
     +\int_{\partial K} v_h (\nabla \Delta w_h \cdot \mathbf n)
...
+ \sum_{e \in \mathbb{F}}
  ...
  \jump{\frac{\partial u_h}{\partial \mathbf n}}.
@f}
* Then, after making cancellations that arise, we arrive at the followingC0IP formulation of the biharmonic equation: find   $u_h$   such that   $u_h =
...
@f}
* Here,   $\gamma$   is the penalty parameter which both weakly enforces theboundary condition
* @f{align*}{
\frac{\partial u(\mathbf x)}{\partial \mathbf n} = j(\mathbf x)
@f}
* on the boundary interfaces   $e \in \mathbb{F}^b$  , and also ensures thatin the limit   $h\rightarrow 0$  ,   $u_h$   converges to a   $C^1$   continuousfunction.   $\gamma$   is chosen to be large enough to guarantee thestability of the method. We will discuss our choice in the program below.
* 

* <a name="ConvergenceRates"></a><h4>Convergence Rates </h4>
* On polygonal domains, the weak solution   $u$   to the biharmonic equationlives in   $H^{2 +\alpha}(\Omega)$   where   $\alpha \in(1/2, 2]$   isdetermined by the interior angles at the corners of   $\Omega$  . Forinstance, whenever   $\Omega$   is convex,   $\alpha=1$  ;   $\alpha$   may be lessthan one if the domain has re-entrant corners but  $\alpha$   is close to   $1$   if one of all interior angles is close to  $\pi$  .
* Now suppose that the   $C^0$   IP solution   $u_h$   is approximated by   $C^0$  shape functions with polynomial degree   $p \ge 2$  . Then thediscretization outlined above yields the convergence rates asdiscussed below.
* 

* <b>Convergence in the $C^0$ IP-norm</b>
* Ideally, we would like to measure convergence in the "energy norm"  $\|D^2(u-u_h)\|$  . However, this does not work because, again, thediscrete solution   $u_h$   does not have two (weak) derivatives. Instead,one can define a discrete (  $C^0$   IP) seminorm that is "equivalent" to theenergy norm, as follows:
* @f{align*}{
 |u_h|_{h}^2 :=
...
* Of course, this is true only if the exact solution is sufficientlysmooth. Indeed, if   $f \in H^m(\Omega)$   with   $m \ge 0$  ,  $u \in H^{2+\alpha}(\Omega)$   where   $ 2 < 2+\alpha  \le m+4$  ,then the convergence rate of the   $C^0$   IP method is  $\mathcal{O}(h^{\min\{p-1, \alpha\}})$  . In other words, the optimalconvergence rate can only be expected if the solution is so smooththat   $\alpha\ge p-1$  ; this canonly happen if (i) the domain is convex with a sufficiently smoothboundary, and (ii)   $m\ge p-3$  . In practice, of course, the solution iswhat it is (independent of the polynomial degree we choose), and thelast condition can then equivalently be read as saying that there isdefinitely no point in choosing   $p$   large if   $m$   is not alsolarge. In other words, the only reasonably choices for   $p$   are   $p\le
m+3$   because larger polynomial degrees do not result in higherconvergence orders.
* For the purposes of this program, we're a bit too lazy to actuallyimplement this equivalent seminorm
* 
*  -  though it's not very difficult andwould make for a good exercise. Instead, we'll simply check in theprogram what the "broken"   $H^2$   seminorm
* @f{align*}{
...
@f}
* yields. The convergence rate in this norm can, from a theoreticalperspective, of course not be <i>worse</i> than the one for  $|\cdot|_h$   because it contains only a subset of the necessary terms,but it could at least conceivably be better. It could also be the case thatwe get the optimal convergence rate even though there is a bug in theprogram, and that that bug would only show up in sub-optimal rates forthe additional terms present in   $|\cdot|_h$  . But, one might hopethat if we get the optimal rate in the broken norm and the normsdiscussed below, then the program is indeed correct. The resultssection will demonstrate that we obtain optimal rates in all normsshown.
* 

* <b>Convergence in the $L_2$-norm</b>
* The optimal convergence rate in the   $L_2$  -norm is   $\mathcal{O}(h^{p+1})$  provided   $p \ge 3$  . More details can be found in Theorem 4.6 of  @cite Engel2002   .
* The default in the program below is to choose   $p=2$  . In that case, thetheorem does not apply, and indeed one only gets   $\mathcal{O}(h^2)$  instead of   $\mathcal{O}(h^3)$   as we will show in the results section.
* 

* <b>Convergence in the $H^1$-seminorm</b>
* Given that we expect  $\mathcal{O}(h^{p-1})$   in the best of cases for a norm equivalent tothe   $H^2$   seminorm, and   $\mathcal{O}(h^{p+1})$   for the   $L_2$   norm, onemay ask about what happens in the   $H^1$   seminorm that is intermediateto the two others. A reasonable guess is that one should expect  $\mathcal{O}(h^{p})$  . There is probably a paper somewhere that provesthis, but we also verify that this conjecture is experimentally truebelow.
* 
...
* We remark that the derivation of the   $C^0$   IP method for thebiharmonic equation with other boundary conditions
* 
*  -  for instance,for the first set of boundary conditions namely   $u(\mathbf x) =
g(\mathbf x)$   and   $\Delta u(\mathbf x)= h(\mathbf x)$   on  $\partial\Omega$  
* 
*  -  can be obtained with suitable modifications to  $\mathcal{A}(\cdot,\cdot)$   and   $\mathcal{F}(\cdot)$   described inthe book chapter   @cite Brenner2011   .
* 

* <a name="Thetestcase"></a><h3>The testcase</h3>
* 

* The last step that remains to describe is what this program solvesfor. As always, a trigonometric function is both a good and a badchoice because it does not lie in any polynomial space in which we mayseek the solution while at the same time being smoother than realsolutions typically are (here, it is in   $C^\infty$   while realsolutions are typically only in   $H^3$   or so on convex polygonaldomains, or somewhere between   $H^2$   and   $H^3$   if the domain is notconvex). But, since we don't have the means to describe solutions ofrealistic problems in terms of relatively simple formulas, we just gowith the following, on the unit square for the domain   $\Omega$  :
* @f{align*}{
  u = \sin(\pi x) \sin(\pi y).
@f}
...
* 
*  -  that works in essence like the assembly of the corresponding terms of the Laplace equation, and you have seen how this works in   step-4   or   step-6  , for example
* 
...
*   The following structures then provide the scratch and copy objects that are necessary for this approach. You may look up the WorkStream namespace as well as the   @ref threads   "Parallel computing with multiple processors" module for more information on how they typically work.
* 
...
* 
 @endcode
* 
*   <a name="Solvingthelinearsystemandpostprocessing"></a>  <h4>Solving the linear system and postprocessing</h4>   
...
* 
           ...
           for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
             {
               local_error +=
                 ((exact_hessians[q_point]
* 
- hessians[q_point]).norm_square()
...
* We run the program with right hand side and boundary values asdiscussed in the introduction. These will produce thesolution   $u = \sin(\pi x) \sin(\pi y)$   on the domain   $\Omega = (0,1)^2$  .We test this setup using   $Q_2$  ,   $Q_3$  , and   $Q_4$   elements, which one canchange via the `fe_degree` variable in the `main()` function. With meshrefinement, the   $L_2$   convergence rates,   $H^1$  -seminorm rate,and   $H^2$  -seminorm convergence of   $u$  should then be around 2, 2, 1 for   $Q_2$   (with the   $L_2$   normsub-optimal as discussed in the introduction); 4, 3, 2 for  $Q_3$  ; and 5, 4, 3 for   $Q_4$  , respectively.
* From the literature, it is not immediately clear whatthe penalty parameter   $\gamma$   should be. For example,  @cite Brenner2009   state that it needs to be larger than one, andchoose   $\gamma=5$  . The FEniCS/Dolphin tutorial chooses it as  $\gamma=8$  , seehttps://fenicsproject.org/docs/dolfin/1.6.0/python/demo/documented/biharmonic/python/documentation.html.   @cite Wells2007   uses a value for   $\gamma$   larger than thenumber of edges belonging to an element for Kirchhoff plates (seetheir Section 4.2). This suggests that maybe  $\gamma = 1$  ,   $2$  , are too small; on the other hand, a value  $p(p+1)$   would be reasonable,where   $p$   is the degree of polynomials. The last of these choices isthe one one would expect to work by comparingto the discontinuous Galerkin formulations for the Laplace equation(see, for example, the discussions in   step-39   and   step-74  ),and it will turn out to also work here.But we should check what value of   $\gamma$   is right, and we will do sobelow; changing   $\gamma$   is easy in the two `face_worker` and`boundary_worker` functions defined in `assemble_system()`.
* 

* <a name="TestresultsoniQsub2subiwithigammapp1i"></a><h3>Test results on <i>Q<sub>2</sub></i><i>Q<sub>2</sub></i> with <i>&gamma; = p(p+1)</i><i>&gamma; = p(p+1)</i> </h3>
* 
...
</table>  We can see that the   $L_2$   convergence rates are around 2,  $H^1$  -seminorm convergence rates are around 2,and   $H^2$  -seminorm convergence rates are around 1. The latter twomatch the theoretically expected rates; for the former, we have notheorem but are not surprised that it is sub-optimal given the remarkin the introduction.
* 

* <a name="TestresultsoniQsub3subiwithigammapp1i"></a><h3>Test results on <i>Q<sub>3</sub></i><i>Q<sub>3</sub></i> with <i>&gamma; = p(p+1)</i><i>&gamma; = p(p+1)</i> </h3>
* 
...
</table>  We can see that the   $L_2$   convergence rates are around 4,  $H^1$  -seminorm convergence rates are around 3,and   $H^2$  -seminorm convergence rates are around 2.This, of course, matches our theoretical expectations.
* 

* <a name="TestresultsoniQsub4subiwithigammapp1i"></a><h3>Test results on <i>Q<sub>4</sub></i><i>Q<sub>4</sub></i> with <i>&gamma; = p(p+1)</i><i>&gamma; = p(p+1)</i> </h3>
* 
...
</table>  We can see that the   $L_2$   norm convergence rates are around 5,  $H^1$  -seminorm convergence rates are around 4,and   $H^2$  -seminorm convergence rates are around 3.On the finest mesh, the   $L_2$   norm convergence rateis much smaller than our theoretical expectationsbecause the linear solver becomes the limiting factor dueto round-off. Of course the   $L_2$   error is also very small already inthat case.
* 

* <a name="TestresultsoniQsub2subiwithigamma1i"></a><h3>Test results on <i>Q<sub>2</sub></i><i>Q<sub>2</sub></i> with <i>&gamma; = 1</i><i>&gamma; = 1</i> </h3>
* 

* For comparison with the results above, let us now also consider thecase where we simply choose   $\gamma=1$  :
*   <table align="center" class="doxtable">
  <tr>
   <th>Number of refinements </th><th>  $\|u-u_h^\circ\|_{L_2}$ </th><th>  Conv. rates  </th><th>  $|u-u_h|_{H^1}$ </th><th> Conv. rates </th><th> $|u-u_h|_{H^2}$ </th><th> Conv. rates </th>
  </tr>
...
</table>  Although   $L_2$   norm convergence rates of   $u$   more or lessfollows the theoretical expectations,the   $H^1$  -seminorm and   $H^2$  -seminorm do not seem to converge as expected.Comparing results from   $\gamma = 1$   and   $\gamma = p(p+1)$  , it is clear that  $\gamma = p(p+1)$   is a better penalty.Given that   $\gamma=1$   is already too small for   $Q_2$   elements, it may not be surprising that if one repeated theexperiment with a   $Q_3$   element, the results are even more disappointing: One again only obtains convergencerates of 2, 1, zero
* 
*  -  i.e., no better than for the   $Q_2$   element (although the errors are smaller in magnitude).Maybe surprisingly, however, one obtains more or less the expected convergence orders when using   $Q_4$  elements. Regardless, this uncertainty suggests that   $\gamma=1$   is at best a risky choice, and at worst anunreliable one and that we should choose   $\gamma$   larger.
* 

* <a name="TestresultsoniQsub2subiwithigamma2i"></a><h3>Test results on <i>Q<sub>2</sub></i><i>Q<sub>2</sub></i> with <i>&gamma; = 2</i><i>&gamma; = 2</i> </h3>
* 

* Since   $\gamma=1$   is clearly too small, one might conjecture that  $\gamma=2$   might actually work better. Here is what one obtains inthat case:
*   <table align="center" class="doxtable">
  <tr>
   <th>Number of refinements </th><th>  $\|u-u_h^\circ\|_{L_2}$ </th><th>  Conv. rates  </th><th>  $|u-u_h|_{H^1}$ </th><th> Conv. rates </th><th> $|u-u_h|_{H^2}$ </th><th> Conv. rates </th>
  </tr>
...
</table>  In this case, the convergence rates more or less follow thetheoretical expectations, but, compared to the results from   $\gamma =
p(p+1)$  , are more variable.Again, we could repeat this kind of experiment for   $Q_3$   and   $Q_4$   elements. In both cases, we will find that weobtain roughly the expected convergence rates. Of more interest may then be to compare the absolutesize of the errors. While in the table above, for the   $Q_2$   case, the errors on the finest grid are comparable betweenthe   $\gamma=p(p+1)$   and   $\gamma=2$   case, for   $Q_3$   the errors are substantially larger for   $\gamma=2$   than for  $\gamma=p(p+1)$  . The same is true for the   $Q_4$   case.
* 

* <a name="Conclusionsforthechoiceofthepenaltyparameter"></a><h3> Conclusions for the choice of the penalty parameter </h3>
* 

* The conclusions for which of the "reasonable" choices one should use for the penalty parameteris that   $\gamma=p(p+1)$   yields the expected results. It is, consequently, what the codeuses as currently written.
* 
...
* 
-\sum_{e \in \mathbb{F}} \int_{e} \jump{\frac{\partial v_h}{\partial \mathbf{n}}} (\frac{\partial^2 u_h}{\partial \mathbf{n^2}}).
  @f}
...
-\sum_{K \in \mathbb{T}} \int_{\partial K} (\nabla v_h) \cdot (\frac{\partial \nabla u_h}{\partial \mathbf{n}}) =
* 
-\sum_{e \in \mathbb{F}^i} \int_{e} \jump{\frac{\partial v_h}{\partial \mathbf{n}}} (\frac{\partial^2 u_h}{\partial \mathbf{n^2}})
* 
...
* @f{align*}{
  \sum_{K \in \mathbb{T}} \int_K (D^2 v_h) : (D^2 u_h) \ dx
* 
- \sum_{e \in \mathbb{F}^i} \int_{e} \jump{\frac{\partial v_h}{\partial \mathbf{n}}} (\frac{\partial^2 u_h}{\partial \mathbf{n^2}}) \ ds
