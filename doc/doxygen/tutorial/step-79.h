...
@f]
* Here,   $\boldsymbol{\sigma} = \mathbf{C} : \boldsymbol{\varepsilon}(\mathbf{u})$   is the stresswithin the body that is caused by the external forces   $\mathbf F$  , where we have for simplicity assumedthat the material is linear-elastic and so   $\mathbf{C}$   is the stress-strain tensor and  $\boldsymbol{\varepsilon}(\mathbf{u})=\frac{1}{2} (\nabla \mathbf{u} + (\nabla\mathbf{u})^T)$   is thesmall-deformation strain as a function of the displacement   $\mathbf{u}$  
* 
...
* The SIMP method is based on an idea of allowing the material to exist in alocation with a density   $\rho$   between 0 and 1. A density of 0 suggests thematerial is not there, and it is not a part of the structure, while a density of1 suggests the material is present. Values between 0 and 1 do not reflect adesign we can create in the real-world, but allow us to turn the combinatorialproblem into a continuous one. One then looks at density values   $\rho$  ,with the constraint that   $0 < \rho_{\min} \leq \rho \leq 1$  . The minimum value  $\rho_{\min}$  , typically chosen to be around   $10^{-3}$  , avoids the possibilityof having an infinite strain energy, but is small enough to provide accurateresults.
* The straightforward application of the effect of this "density" on theelasticity of the media would be to simply multiply the stiffness tensor   $\mathbf{C}_0$  of the medium by the given density, that is,   $\mathbf{C} = \rho \mathbf{C}_0$  . However, thisapproach often gives optimal solutions where density values are far from both 0and 1. As one wants to find a real-world solution, meaning the material eitheris present or it is not, a penalty is applied to these in-between values. Asimple and effective way to do this is to multiply the stiffness tensor by thedensity raised to some integer power penalty parameter   $p$  , so that  $\mathbf{C} = \rho^p \mathbf{C}_0$  . This makes density values farther away from 0 or 1 lesseffective. It has been shown that using   $p=3$   is sufficiently high to create'black-and-white' solutions: that is, one gets optimal solutions in whichmaterial is either present or not present at all points.
...
@f]The final constraint, the balance of linear momentum (which we will refer to as the elasticity equation), gives a method for finding   $\boldsymbol{\sigma}$   and   $\boldsymbol{\varepsilon}$   given the density   $\rho$  .
...
@f]In everything we will do below, we will always consider the displacementfield   $\mathbf{u}$   as the only solution variable, rather than considering  $\mathbf{u}$   and   $\boldsymbol{\sigma}$   as solution variables (as is done in mixedformulations).
* Furthermore, we will make the assumption that the material is linear isotropic,in which case the stress-strain tensor can be expressed in terms of the Lam&eacute;parameters   $\lambda,\mu$   such that
* @f{align}
...
@f]Because we are assuming no body forces, this simplifies further to@f[
  \int_\Omega \boldsymbol{\sigma}(\rho) : (\nabla \mathbf{u} + (\nabla \mathbf{u})^T) d\Omega
  = \int_{\partial \Omega} \mathbf{t} \cdot \mathbf{u} d\partial\Omega,
@f]which is the final form of the governing equation that we'll be consideringfrom this point forward.
...
functions
* 
-- in other words, all of these conditions have to be stated in weak
...
vector $\mathbf{x}$ to represent all primal variables
* 
- the filtered and
unfiltered densities, slack variables and displacement
* 
- and use the vector
...
further away from the set of points that satisfy the constraints
* 
-- or the other
...
* 
 ...
 used throughout this tutorial. Specifically, we have alot* of
...
* 
 ...
 the problem description in the introduction
* 
-- a 6-by-1
...
* 
 ...
 direction
* 
-- the bottom left also in the $x$ direction. deal.II
...
* 
 ...
 of its own for visualization purposes
* 
-- in other words,
...
* 
[0.x.1] Equation 3, which has to do with the filter and which is
                      calculated elsewhere./
                     cell_matrix(i, j) +=
                       fe_values.JxW(q_point)
...
- the part with the filter
                      is added later
* 
[0.x.2] Equation 7: Primal feasibility
* 
- the part with the filter
                      is added later/
                     cell_matrix(i, j) +=
...
* 
-- this is clearly not an efficient
 ...
 we simply stick with the direct solver we have here
* 
-- the
...
* 
 ...
 out the maximum step that meets the dual feasibility
* 
-- that
...
* 
                 /* Equation 1: This equation, along with equations
                  ...
                  variables
* 
- the density, displacement, and
                  unfiltered density.
* 
[0.x.3] Equation 1: This equation, along with equations
                  2 and 3, are the variational derivatives of the
                  Lagrangian with respect to the decision
                  variables
* 
...
* 
                 /* Equation 2; the boundary terms will be added further down
                  below.
* 
[0.x.4] Equation 2; the boundary terms will be added further down
                  below./
                 cell_rhs(i) +=
* 
...
* 
                 /* Equation 4; boundary term will again be dealt
                  ...
                  a constraint.
* 
[0.x.5] Equation 4; boundary term will again be dealt
                  with below. This equation being driven to 0
                  ensures that the elasticity equation is met as
                  a constraint./
                 cell_rhs(i) +=
...
* 
[0.x.6] Equation 5: This equation sets the lower slack
                  variable equal to the unfiltered density,
                  giving a minimum density of 0./
...
* 
                 /* Equation 6: This equation sets the upper slack
                  ...
                  density.
* 
[0.x.7] Equation 6: This equation sets the upper slack
                  variable equal to one minus the unfiltered
                  density./
                 cell_rhs(i) += fe_values.JxW(q_point)
                                (upper_slack_multiplier_phi_i
...
* 
                 /* Equation 7: This is the difference between the
                  ...
                  applied correctly.
* 
[0.x.8] Equation 7: This is the difference between the
                  density and the filter applied to the
                  unfiltered density. This being driven to 0 by
                  the Newton steps ensures that the filter is
                  applied correctly./
...
* 
[0.x.9] Equation 8: This along with equation 9 give the
                  requirement that s*z = \alpha for the barrier
                  size alpha, and gives complementary slackness
                  from KKT conditions when \alpha goes to 0./
                 cell_rhs(i) +=
...
* 
 ...
 "exact"
* 
- that is, it has a minimum in the same location
...
* 
 ...
 started the watchdog iterations
* 
-- that place was
