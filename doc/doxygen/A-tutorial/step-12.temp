...
<ol>
  <li> <a href="#Intro" class=bold>Introduction</a><a href="#Intro" class=bold>Introduction</a>
    <ul>
        ...
        <li><a href="#Thetestproblem">The test problem</a><a href="#Thetestproblem">The test problem</a>
    </ul>
  <li> <a href="#CommProg" class=bold>The commented program</a><a href="#CommProg" class=bold>The commented program</a>
    <ul>
        <li><a href="#Equationdata">Equation data</a><a href="#Equationdata">Equation data</a>
        <li><a href="#TheScratchDataandCopyDataclasses">The ScratchData and CopyData classes</a><a href="#TheScratchDataandCopyDataclasses">The ScratchData and CopyData classes</a>
        <li><a href="#TheAdvectionProblemclass">The AdvectionProblem class</a><a href="#TheAdvectionProblemclass">The AdvectionProblem class</a>
      <ul>
        ...
        <li><a href="#Alltherest">All the rest</a><a href="#Alltherest">All the rest</a>
      </ul>
</ol></td><td width="50%" valign="top"><ol>
  <li value="3"> <a href="#Results" class=bold>Results</a><a href="#Results" class=bold>Results</a>
    <ul>
        <li><a href="#Whyusediscontinuouselements">Why use discontinuous elements</a><a href="#Whyusediscontinuouselements">Why use discontinuous elements</a>
...
* <a name="Intro"></a><a name="AnexampleofanadvectionproblemusingtheDiscountinuousGalerkinmethod"></a><h1>An example of an advection problem using the Discountinuous Galerkin method</h1>
* 
...
* The particular concern of this program are the loops of DG methods. These turnout to be especially complex, primarily because for the face terms, we have todistinguish the cases of boundary, regular interior faces and interior faceswith hanging nodes, respectively. The   MeshWorker::mesh_loop()   handles thecomplexity on iterating over cells and faces and allows specifying "workers"for the different cell and face terms. The integration of face terms itself,including on adaptively refined faces, is done using the FEInterfaceValuesclass.
...
@f]the inflow part of the boundary of the domain and   ${\bf n}$   denotesthe unit outward normal to the boundary   $\Gamma$  . This equation is theconservative version of the advection equation already considered in  step-9   of this tutorial.
* 
...
* 
- v^-$  , where the superscripts referto the left ('+') and right ('-') values at the face. The upwind value  $u^{\text{upwind}}$   is defined to be   $u^+$   if   $\beta \cdot n>0$   and   $u^-$   otherwise.
...
* In order to implement this bilinear form, we need to compute the cell terms(first sum) using the usual way to achieve integration on a cell, the interface terms (second sum) usingFEInterfaceValues, and the boundary terms (the other two terms).The summation of all those is done by   MeshWorker::mesh_loop().  
* 
...
* We solve on a sequence of meshes by refining the mesh adaptively by estimatingthe norm of the gradient on each cell. After solving on each mesh, we outputthe solution in vtk format and compute the   $L^\infty$   norm of the solution. Asthe exact solution is either 0 or 1, we can measure the magnitude of theovershoot of the numerical solution with this.
* 
...
* 
 ...
 @endcode
* 
*   <a name="TheScratchDataandCopyDataclasses"></a>  <h3>The ScratchData and CopyData classes</h3>   
...
* 
 ...
 @endcode
* 
*   <a name="TheAdvectionProblemclass"></a>  <h3>The AdvectionProblem class</h3>   
...
* 
 ...
 @endcode
* 
*  We start with the constructor. The 1 in the constructor call of   <code>fe</code>   is the polynomial degree.
* 
...
* 
 @endcode
* 
*  We start by generating the sparsity pattern. To this end, we first fill an intermediate object of type DynamicSparsityPattern with the couplings appearing in the system. After building the pattern, this object is copied to   <code>sparsity_pattern</code>   and can be discarded.
* 
...
* 
 @endcode
* 
*   <a name="Theassemble_systemfunction"></a>  <h4>The assemble_system function</h4>
* 
...
* 
 ...
 @endcode
* 
*  We refine the grid according to a very simple refinement criterion, namely an approximation to the gradient of the solution. As here we consider the DG(1) method (i.e. we use piecewise bilinear shape functions) we could simply compute the gradients on each cell. But we do not want to base our refinement indicator on the gradients on each cell only, but want to base them also on jumps of the discontinuous solution function over faces between neighboring cells. The simplest way of doing that is to compute approximative gradients by difference quotients including the cell under consideration and its neighbors. This is done by the   <code>DerivativeApproximation</code>   class that computes the approximate gradients in a way similar to the   <code>GradientEstimation</code>   described in   step-9   of this tutorial. In fact, the   <code>DerivativeApproximation</code>   class was developed following the   <code>GradientEstimation</code>   class of   step-9  . Relating to the discussion in   step-9  , here we consider   $h^{1+d/2}|\nabla_h u_h|$  . Furthermore we note that we do not consider approximate second derivatives because solutions to the linear advection equation are in general not in   $H^2$   but only in   $H^1$   (or, to be more precise: in   $H^1_\beta$  , i.e., the space of functions whose derivatives in direction   $\beta$   are square integrable).
* 
...
* @code
 ...
 @endcode
* 
*  The   <code>DerivativeApproximation</code>   class computes the gradients to float precision. This is sufficient as they are approximate and serve as refinement indicators only.
* 
...
* 
 ...
 @endcode
* 
*  The following   <code>main</code>   function is similar to previous examples as well, and need not be commented on.
* 
...
* In refinement iteration 5, the image can't be plotted in a reasonable way anymore as a 3d plot. We thus show a color plot with a range of   $[-1,2]$   (thesolution values of the exact solution lie in   $[0,1]$  , of course). In any case,it is clear that the continuous Galerkin solution exhibits oscillatorybehavior that gets worse and worse as the mesh is refined more and more.
* There are a number of strategies to stabilize the cG method, if one wants touse continuous elements for some reason. Discussing these methods is beyondthe scope of this tutorial program; an interested reader could, for example,take a look at   step-31  .
* 
...
* Given that the exact solution is known in this case, one interestingavenue for further extensions would be to confirm the order ofconvergence for this program. In the current case, the solution isnon-smooth, and so we can not expect to get a particularly high orderof convergence, even if we used higher order elements. But even if thesolution <i>is</i> smooth, the equation is not elliptic and so it is notimmediately clear that we should obtain a convergence order thatequals that of the optimal interpolation estimates (i.e. for examplethat we would get   $h^3$   convergence in the   $L^2$   norm by usingquadratic elements).
...
* A different direction is to observe that the solution of transport problemsoften has discontinuities and that therefore a mesh in which we <i>bisect</i>every cell in every coordinate direction may not be optimal. Rather, a betterstrategy would be to only cut cells in the direction parallel to thediscontinuity. This is called <i>anisotropic mesh refinement</i> and is thesubject of   step-30  .
* 
