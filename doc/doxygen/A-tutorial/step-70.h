 ...
 * 本教程依赖于  step-19  ,  step-32  ,  step-60  。 @htmlonly <table
 ...
 * 在这种情况下，一个可行的替代策略是使用非匹配方法（类似于我们在
 * step-60
 * 中所做的），即背景固定网格（可以在时间上进行局部细化以更好地捕捉固体运动）与旋转的独立网格相耦合。
 * 为了保持与 step-60 中使用的相同符号，我们使用 $\Omega$
 ...
 * 其主要原理是两个网格的离散化保持完全独立。在本教程中，这种方法被用来求解由斯托克斯方程描述的粘性流体的运动，该流体被一个刚性的非变形叶轮搅动。
 * 因此， $\Omega$
 * 中求解的方程是蠕动流的斯托克斯方程（即
 * $\text{Re}\rightarrow 0$
 * 中的流动），并且在与叶轮相关的移动嵌入域* $\Gamma$
 * 上应用无滑动边界条件。然而，这个教程可以很容易地扩展到其他方程（例如，纳维-斯托克斯方程，线性弹性方程等）。它可以被看作是
 * step-60
 * 的一个自然扩展，它可以通过MPI使用分布式并行计算架构解决大型问题。
 * 然而，与 step-60 相反， $\Gamma$
 * 上的Dirichlet边界条件是弱加的，而不是通过使用Lagrange乘法器，并且我们集中于处理两个完全分布式三角计算的耦合（这种组合在
 ...
 * 在本教程中，我们对 $\Gamma$
 * 的进一步细节不感兴趣：我们假设嵌入域的尺寸（`dim`）总是比嵌入域的尺寸（`spacedim`）小一个或相等。
 * 我们要解决以下微分问题：给定 $g$ 上的充分无规则函数
 * $\Gamma$ ，找到 $(\textbf{u},p)$ 的解。
 * @f{eqnarray*}
 *
 *
 *
 ...
 * @f}
 * 本教程的主要目的是展示如何使用惩罚方法将速度场条件
 * $\mathbf{u} = \mathbf{g}$ 以弱的方式施加在非匹配的 $\Gamma$
 * 上。关于斯托克斯问题的更广泛的讨论，包括体力、不同的边界条件和解决策略，可以在
 * step-22 中找到。
 * 让我们首先考虑整个领域中的斯托克斯问题  $\Omega$
 * 。我们寻找一个速度场 $\mathbf{u}$ 和一个压力场 $p$
 * ，满足斯托克斯方程的同质边界条件 $\partial\Omega$  。
 * 斯托克斯方程的弱形式是通过首先将其写成矢量形式得到的@f{eqnarray*}
 * \begin{pmatrix}
 *  {-\Delta \textbf{u} + \nabla p}
 *  \\
 *  {-\textrm{div}\;\textbf{u}}
 ...
 * \end{pmatrix},
 * @f} 。
 * 与矢量值测试函数 $\phi = \begin{pmatrix}\textbf{v} \\ q\end{pmatrix}$ 形成左面的点积，并在域 $\Omega$ 上进行积分，得出以下方程组：@f{eqnarray*}
 * (\mathrm v,
 *
 *
 *
 ...
 * @f}
 *
 * $\Gamma$ 上的积分是低维积分。可以证明（见 @cite Freund1995
 * ），存在一个正的常数 $C_1$ ，所以如果 $\beta > C_1$
 * ，边界的弱施加将是一致和稳定的。在 $\Gamma$
 * 上的前两个附加积分（上述方程中的第二行）在通过部分积分后自然出现，当我们不假设
 * $\mathbf{v}$ 在 $\Gamma$ 上为零。
 * 上文等式中的第三行包含两个项，它们是为了确保弱形式的一致性而添加的，还有一个稳定项，它是为了执行边界条件，其误差与近似误差一致。一致性条款和稳定化条款被添加到实际边界数据的右手边
 * $\mathbf{g}$  。 当 $\mathbf{u}$ 满足 $\Gamma$ 上的条件
 * $\mathbf{u}=\mathbf{g}$ 时， $\Gamma$
 * 上所有的一致性和稳定性积分都被抵消，剩下的就是通常的弱形式的斯托克斯流，也就是说，上述公式是一致的。
 ...
 * + \beta (\textbf{v},\textbf{g})_{\Gamma}.
 * @f}
 * 注意第三行和第四行第一项的不同符号。在这种情况下，稳定性和一致性条件成为
 * $\beta > 0$  。在对称情况下， $\beta$ 的值取决于 $h$
 * ，一般选择 $\beta = C h^{-1} $ ， $h$
 * 是衡量被整合面的大小， $C$ 是一个常数，以便 $1 \leq C
 * \leq 10$
 ...
 * @f} 。
 * 其中跳跃项，用 $\lbrack \cdot \rbrack$
 * 表示，是相对于法向量 $\textbf{n}$
 * 的一个固定方向计算的。2的系数出现在 $\beta$
 * 前面，因为我们看到 $\Gamma$
 * 的每一部分两次，一次来自流体内部，一次来自在其中移动的障碍物内部。对于
 * $\Gamma$ 上的所有其他积分，我们对 $\Gamma$
 * 的每一部分都访问了两次，但都是相反的符号，因此得到了跳跃项）。
 * 这里我们注意到，与不连续的Galerkin方法不同，测试和试验函数在
 * $\Gamma$ 上是连续的。此外，如果 $\Gamma$
 * 不与单元边界对齐，所有的跳跃项也是零，因为一般来说，有限元函数空间在每个单元内都是平滑的，如果
 * $\Gamma$
 ...
 *
 * 在 step-60
 * 中，约束条件的施加需要以拉格朗日乘数的形式增加新的变量。本教程程序不存在这种情况。然而，嵌入域上的速度矢量
 ...
 * @f}
 *
 * 这里， $\Gamma$ 上的积分只是部分体积上的积分。因此，
 * $\mathcal{L}^2$
 * 的惩罚包括增加一个体积项，约束流体的速度与 $\Gamma$
 * 内刚体的速度保持一致。在这种情况下， $\beta$
 * 必须被选择得足够大，以确保 $\Gamma$
 * 中的迪里希特边界条件得到充分尊重，但又不能太高，以保持系统矩阵的适当条件化。
 * $\mathcal{H}^1$
 * 的惩罚可以用类似的方式来构建，在惩罚中加入粘性成分，在
 * $\Gamma$ 中阻尼速度梯度。
 * @f{eqnarray*}{
 * (\nabla \textbf{v}, \nabla \textbf{u})_{\Omega}
 *
 ...
 * 是从 $\hat K$ 到 $K$ 的映射，并使用正交公式计算出 $\hat K$
 * 上的积分。比如说。 \f[ \beta (\textbf{v},\textbf{u})_{\Gamma} =
 * \sum_{K\in \Gamma} \int_{\hat K} \hat{\textbf{u}}(\hat x) (\textbf{v} \circ
 * F_{K}) (\hat x) J_K (\hat x) \mathrm{d} \hat x = \sum_{K\in \Gamma}
 * \sum_{i=1}^{n_q}  \big(\hat{\textbf{u}}(\hat x_i)  (\textbf{v} \circ F_{K})
 * (\hat x_i) J_K (\hat x_i) w_i \big) \f]
 * 计算这个和是不容易的，因为我们必须评估 $(v_j \circ
 * F_{K}) (\hat x_i)$ 。一般来说，如果 $\Gamma$ 和 $\Omega$
 * 没有对齐，那么 $y_i = F_{K}(\hat x_i)$ 点相对于 $\Omega$
 * 是完全任意的，除非我们想出办法将 $V_h(\Omega)$
 * 的所有基函数插在 $\Omega$
 ...
 *
 * - 找到 $T$ 内 $y_i$ 的参考坐标。为此，我们需要将参考元素 $\hat T$ 转换为元素 $T$ ： $\hat y_i = G^{-1}_{T} (y_i)$ 的映射 $G_T$ 的逆向。
 *
 * - 评估  $\Omega$  网格在这一点  $\hat y_i$  的基函数  $v_j$  。这也是比较简单的，使用FEValues。
 *
 *  <p align="center"> <img
 ...
 * 尽管这种方法在目前的情况下是可行的，但它并不适合于使用分布式三角形的平行模拟。事实上，由于嵌入域
 * $\Gamma$
 * 单元上的正交点的位置与嵌入三角形的位置不一致，而且
 * $\Gamma$ 是不断移动的，这就需要将代表 $\Gamma$
 * 的三角形完整地存储在所有处理器中。随着处理器数量和
 * $\Gamma$
 ...
 * \f]，如果你跟随上面的讨论，那么你会记得 $\textbf{u}$ 和
 * $\textbf{v}$
 * 是定义在流体网格上的形状函数，唯一定义在实体网格上的东西是。
 ...
 * $J_K(\hat x_i) w_i$
 * （我们通常用`JxW'表示）在每个正交点上保持不变。所以我们唯一需要跟踪的是位置
 * $x_i=F_K(\hat x_i)$ 。
 *
 * - 但这些位置是随着实体域的速度而移动的。
 ...
 * - 在 $\Gamma$ 上的正交点位置创建 Particles::Particle  。
 * - 调用 Particles::ParticleHandler::insert_global_particles() 函数，将粒子按照实体三角形分布在处理器上*。
 *
 * - 将 "JxW "值作为一个 "属性 "附加到每个 Particles::Particle 对象上。
 * 这个结构的生成相对昂贵，但每次模拟必须只生成一次。一旦生成了
 * Particles::ParticleHandler
 * 并将所需信息附加到粒子上，就可以利用粒子在ParticleHandler中被分组的事实，对
 * $\Gamma$ 进行积分，使我们能够。
 *
 * - 在 $\Omega$ 中至少包含一个粒子的所有单元上循环操作
 *
 * - 在给定单元中的所有粒子上进行循环操作
 *
 * - 计算积分并填充全局矩阵。
 * 由于 Particles::ParticleHandler
 * 可以管理粒子从一个处理器到另一个处理器的交换，嵌入式三角剖分可以通过位移粒子来移动或变形。与这种位移相关的唯一约束是，粒子应该被位移的距离不大于一个单元的大小。这是因为这是
 * Particles::ParticleHandler
 * 能够跟踪离开当前单元的粒子现在所处的单元的极限。
 * 一旦整个问题（斯托克斯问题和浸没式边界定位）被集合起来，最终的鞍点问题将由一个迭代求解器来解决，应用于舒尔补体
 * $S$ （其构造例如在 step-22
 ...
 *
 * 然而，由于这个类将作为一个 "const
 * "引用传递给StokesImmersedProblem类，我们必须确保我们仍然可以在这里定义的Function类派生的对象中正确设置时间。为了做到这一点，我们声明
 ...
 * 当条目不统一时，如元组的情况，我们用冒号来分隔各个条目。例如，像`5:
 * 0.1, 0.2`这样的字符串可以用来解析一个类型为
 ...
 * spacedim`，我们必须以一种方式来存储这个向量的元素，以便每个并行进程
 * "拥有
 * "那些对应于它所拥有的粒子坐标的元素。换句话说，我们必须在所有进程中划分0和`n_particles
 ...
 * @endcode
 *
 * 然而，正如错误信息所指出的，触发该异常的代码也将生成指定的文件（"parameters.prm
 * "在本例中），该文件仅仅包含该程序所关心的所有参数的默认值（根据文件名中是否包含字符串
 * "23 "或
 * "3"，为正确的尺寸和辅助尺寸）。通过检查默认参数文件，我们看到以下内容。
 * @code
 ...
 * end
 * @endcode
 *  如果你现在运行该程序，你会在参数`Output
 * directory`（默认为当前目录）指定的目录中得到一个名为`parameters_22.prm`的文件，其中包含上述参数的简短版本（没有注释和文档），记录了运行程序时使用的所有参数。
 * @code
 * subsection Stokes Immersed Problem
 * set Final time                            = 1
 * set Fluid bounding boxes extraction level = 1
 ...
 * 首先创建 "parameters.prm
 * "文件（程序第一次运行时），然后创建
 * "output/parameters_22.prm
 * "文件（每次用现有的输入文件运行程序时），其理由是你可能想把大多数参数保留为默认值，只修改其中的一小部分，同时仍然能够重新产生结果，检查具体模拟使用了哪些参数。一般来说，把用于模拟的参数文件和模拟输出一起保存起来是很好的科学做法，这样你就可以在以后的时间里重复相同的运行。
 ...
 * @endcode
 * 你可以用Q3/Q2
 * Taylor-Hood有限元运行程序，运行101步，使用Nitsche惩罚为
 * "10"，并保留所有其他参数的默认值。输出目录不仅包含这些参数，而且包含模拟中使用的所有参数的记录。你可以在生成的文件
 ...
 * of the fluid
 * flow</a>（包括在模拟过程中特定时间形成的角顶点），以及蓝色色调的示踪粒子。
