...
* 一般来说，有限元方法，特别是deal.II，是为了解决偏微分方程而发明的。
* 
* 换句话说，是为了解决[连续力学](https://en.wikipedia.org/wiki/Continuum_mechanics)问题。另一方面，有时人们想解决的问题是，跟踪单个物体（"粒子"）以及它们的位置如何演变是很有用的。如果这只是导致一组常微分方程，例如，如果你想长期跟踪太阳系中行星的位置，那么deal.II显然不是你合适的工具。另一方面，如果这种演变是由于与偏微分方程的解的相互作用，或者如果有一个网格来确定哪些粒子与其他粒子相互作用（比如在[平滑粒子流体力学（SPH）](https://en.wikipedia.org/wiki/Smoothed-particle_hydrodynamics)方法中），那么deal.II对你就有支持。
...
* 上述方程构成了一组耦合微分方程。让我们再次把它们集中在一起，以明确这一点。
* @f{align*}{
* 
...
@f}
* 由于电势对粒子位置的依赖性很强，我们不想把它作为一个耦合系统来解决，而是采用一种解耦的方法，即先解决每个时间步长的电势，然后再解决粒子位置。这与我们在 step-21 、 step-31 和 step-32 （仅举几例）中的做法精神完全相同，而且都可以在 step-58 中讨论的算子拆分方法的背景下理解。
...
@f]不想让一个粒子移动超过 $\frac 12 h_i$ ，那么意味着我们应该选择时间步长为@f[
  \Delta t
  \le
  \min_i
  \sqrt{ \frac{h_i m}{e \|\nabla V^{(1)}\| }}.
...
* 
*  @note  在这个教程程序中，我们通过手工插入粒子，并在我们根据包括静电问题的解决在内的条件而特别选择的位置插入粒子。但在其他情况下，人们主要是想把粒子作为被动对象来使用，例如，追踪和可视化流体流动问题的流场。在这些情况下，在 Particles::Generators 命名空间中有许多函数可以自动生成粒子。这个命名空间中的一个函数也被用于 step-70 教程程序中，例如。
* 
...
* 
* 按照惯例，我们把所有与程序细节相对应的东西都放到一个自己的命名空间中。在顶部，我们定义了一些常量，对于这些常量，我们宁愿使用符号名称而不是硬编码的数字。
* 
...
* 
* 对于边界指标，让我们从某个随机值101开始列举。这里的原则是使用不常见的数字*。如果以前有`GridGenerator'函数设置的预定义边界指标，它们很可能是由零开始的小整数，但不是在这个相当随机的范围内。使用下面这样的数字可以避免冲突的可能性，同时也减少了在程序中直接拼出这些数字的诱惑（因为你可能永远不会记得哪个是哪个，而如果它们从0开始，你可能会受到诱惑）。
* 
...
* 
 @endcode
* 
* <a name="ThecodeCathodeRaySimulatorcodeclassimplementation"></a> <h3>The <code>CathodeRaySimulator</code> class implementation</h3> * <h3>The <code>CathodeRaySimulator</code> class implementation</h3>。
...
* 
* 那么让我们开始执行。构造函数所做的实际上只是对顶部的所有成员变量进行简单的初始化。唯一值得一提的是`particle_handler'，它被交给一个三角形的引用，粒子将生活在这个三角形上（目前当然还是空的，但是粒子处理程序存储了这个引用，一旦粒子被添加，就会使用它）。
* 
*--这发生在三角结构建立之后）。) 它得到的另一个信息是每个粒子需要存储多少个 "属性"。在这里，我们需要每个粒子记住的是它当前的速度，也就是一个带有`dim`分量的矢量。然而，每个粒子还有其他的内在属性， Particles::ParticleHandler 类会自动并始终确保这些属性是可用的；特别是，这些属性是粒子的当前位置、它所在的单元、它在该单元中的参考位置，以及粒子的ID。  
...
* 接下来的函数负责生成我们要解决的网格。回顾一下领域的样子。  <p align="center">
 ...
 alt="The geometry used in this program"
 width="600">
 </p>  我们把这个几何体细分为 $4\times 2$ 个单元的网格，看起来像这样。  <div class=CodeFragmentInTutorialComment>
...
* 
 @endcode
* 
* 有了这些数组，我们可以转向稍高的更高级别的数据结构。我们创建一个CellData对象的向量，为每个要创建的单元存储相关的顶点以及 @ref GlossMaterialId "材料ID"（在这里我们将简单地设置为0，因为我们在程序中不使用它）。    
...
* 
 @endcode
* 
* 该函数的其余部分循环所有的单元和它们的面，如果一个面在边界上，则决定哪一个边界指标应该应用到它。如果你将代码与上面的几何图形相比较，各种条件应该是有意义的。    
...
* 
* 计算矩阵条目的函数实质上还是复制了  step-6  中的相应函数。
* 
...
* 这个函数唯一有趣的部分是它如何形成线性系统的右手边。回顾一下，PDE的右边是@f[
 ...
 @f]，在这里我们用 $p$ 来索引粒子，以避免与形状函数 $\varphi_i$ 相混淆； $\mathbf x_p$ 是第 $p$ 个粒子的位置。        
...
* 
*--即一个半开放的范围，这在C++函数中很常见。现在知道了粒子的列表，我们查询它们的参考位置（相对于参考单元），评估这些参考位置的形状函数，并根据上述公式计算力（没有任何 FEValues::JxW). ）。         
*  

* 
*  @note  值得指出的是，调用 Particles::ParticleHandler::particles_in_cell() 和 Particles::ParticleHandler::n_particles_in_cell() 函数在有大量粒子的问题上不是很有效率。但是它说明了写这个算法的最简单的方法，所以我们愿意为了说明问题而暂时承担这个代价。我们在下面的<a href="#extensions">"possibilities for extensions" section</a>中更详细地讨论了这个问题，并在 step-70 中使用了一个更好的方法，例如：。
* 
...
* 
* 最后一个与场相关的函数是细化网格的函数。我们将在第一个时间步骤中多次调用它，以获得一个能很好地适应解的结构的网格，特别是解决解中的各种奇异点，这些奇异点是由于再入角和边界条件类型改变的地方造成的。你可能想再参考一下 step-6 以了解更多的细节。
* 
...
* 
* 现在让我们来看看处理粒子的函数。第一个是关于粒子的创建。正如介绍中提到的，如果电场 $\mathbf E=\nabla V$ 超过某个阈值，即如果 $|\mathbf E| \ge E_\text{threshold}$ ，并且如果电场指向域内（即如果 $\mathbf E \cdot \mathbf n < 0$ ），我们希望在阴极的各点创建一个粒子。正如有限元方法中常见的那样，我们在特定的评估点评估场（及其导数）；通常，这些是 "正交点"，因此我们创建了一个 "正交公式"，我们将用它来指定我们想要评估解决方案的点。在这里，我们将简单地采用QMidpoint，意味着我们将只在面的中点检查阈值条件。然后我们用它来初始化一个FEFaceValues类型的对象来评估这些点的解决方案。  
...
* 只有当电场强度超过阈值时，电子才能逃离阴极，关键是电场要指向*域。一旦我们检查了这一点，我们就在这个位置创建一个新的 Particles::Particle 对象，并将其插入到 Particles::ParticleHandler 对象中，并设置一个唯一的ID。                
...
* 
 ...
 @endcode
* 
* <a name="CathodeRaySimulatormove_particles"></a> <h4>CathodeRaySimulator::move_particles</h4> * <h4>CathodeRaySimulator::move_particles</h4>.
...
* 
 @endcode
* 
* 然后我们可以向FEPointEvaluation对象询问这些位置上的解的梯度（即电场 $\mathbf E$ ），并在各个粒子上循环。
* 
...
* 现在已经得到了其中一个粒子位置的电场，我们首先用它来更新速度，然后更新位置。要做到这一点，让我们首先从存储在粒子的属性中得到旧的速度，计算加速度，更新速度，并将这个新速度再次存储在粒子的属性中。回顾一下，这对应于在介绍中讨论的以下一组更新方程中的第一个。
...
* 
* 正如在介绍中详细讨论的那样，我们需要尊重一个时间步长条件，即颗粒在一个时间步长中不能移动超过一个单元。为了确保这一点，我们首先计算每个单元上所有粒子的最大速度，然后用该速度除以单元大小。然后，我们使用介绍中讨论的安全系数，将下一个时间步长计算为所有单元上该数量的最小值，并使用 DiscreteTime::set_desired_time_step_size() 函数将其设置为所需的时间步长。
* 
...
* 
 ...
 @endcode
* 
* 正如介绍中提到的，我们必须以不同的方式对待第一个时间步长，因为在那里，粒子还没有可用，或者还没有我们计算合理步长所需的相关信息。下面的公式是按照介绍中的讨论进行的。
* 
...
* 有了这个，`output_results()`函数就变得相对简单了。我们使用DataOut类，就像我们在以前几乎所有的教程程序中使用的那样，来输出解决方案（"电动势"），我们使用上面定义的后处理程序来输出其梯度（"电场"）。这些都将被写入VTU格式的文件中，同时将当前时间和时间步长与该文件联系起来。
* 
...
* 
 ...
 @endcode
* 
* 现在做时间上的循环。这个步骤的顺序紧紧围绕着介绍中讨论的算法的大纲。正如在DiscreteTime类的文档中详细讨论的那样，当我们将场和粒子信息向前移动一个时间步长时，存储在`time`变量中的时间与这些量的（部分）位置不一致（在DiscreteTime的字典中，这就是 "更新阶段"）。对`time.advance_time()`的调用通过将`time`变量设置为场和粒子已经处于的时间而使一切重新保持一致，一旦我们处于这个 "一致阶段"，我们就可以生成图形输出并将模拟的当前状态的信息写入屏幕。
* 
...
* 时间步数1000 场自由度：4989 仿真中的粒子总数。 44 这个时间步长损失的粒子数：6 通过阳极损失的粒子的分数。0.0601266
...
* 时间步数 2091 场自由度：4989 仿真中的粒子总数。 44 这个时间步长损失的粒子数：0 通过阳极损失的粒子的分数。0.0503338
...
* 该程序已经计算出了通过阳极上的孔离开域的电子的比例。但是还有一些人们可能感兴趣的量。例如，这些粒子的平均速度。从每个粒子的属性中获得其速度并不是很困难，就像我们在`move_particles()`函数中做的那样，并从中计算出统计数字。
* 

*<a name="Abettersynchronizedvisualization"></a><h4> A better-synchronized visualization </h4>
* 

* 如上所述，由于我们为每一个时间步骤创建输出，所以视频的不同帧之间存在不同的时间差。一个更好的方法是在固定的时间间隔内生成一个新的输出文件，不管每个时间点之间有多少时间步长。
* 
...
* 在发布模式下，在写这篇文章时，该程序在作者的一台笔记本电脑上运行大约3.5分钟。这是可以接受的。但是，如果我们想让模拟变成三维的呢？如果我们不希望在任何时候使用最多100个粒子（就像这里使用的参数一样），而是使用100,000个？如果我们需要一个更细的网格？
