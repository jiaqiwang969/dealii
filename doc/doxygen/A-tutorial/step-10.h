 ...
 * 本教程依赖于  step-7  。 @htmlonly <table class="tutorial"
 ...
 *
 * 这是一个相当短的例子，只显示了使用高阶映射的一些方面。我们所说的
 * <em> 映射 </em>
 * 是指单元格（即单元线、正方形或立方体）与现实空间中的单元格之间的转换。在前面所有的例子中，我们都隐含地使用了线性或d-线性映射；你根本不会注意到这一点，因为如果你不做任何特别的事情，这就是发生的情况。然而，如果你的域有弯曲的边界，在有些情况下，边界的片状线性逼近(即由直线段构成)是不够的，你希望你的计算域也是使用弯曲边界的真实域的逼近。如果边界近似使用逐片二次抛物线来近似真实边界，那么我们说这是二次或
 * $Q_2$
 * 近似。如果我们使用立方体多项式的片断图形，那么这就是
 * $Q_3$ 的近似，以此类推。
 *
 *
 * 对于某些微分方程，已知边界的分片线性逼近，即 $Q_1$
 * 映射，如果精确域的边界是弯曲的，则是不充分的。例如，使用
 * $C^1$
 * 元素的双谐波方程，或具有弯曲反射边界的域上的气体动力学的Eulerequations。在这些情况下，有必要使用高阶映射来计算积分。如果我们不使用这样的高阶映射，边界的逼近顺序将主导整个数值方案的收敛顺序，而不考虑域内部离散化的收敛顺序。
 *
 *
 * 我们没有用这些更复杂的例子来证明高阶映射的使用，而只是做了一个简单的计算：用两种不同的方法计算
 * $\pi=3.141592653589793238462643\ldots$ 的值。
 *
 *
 * 第一种方法是使用一个具有单位半径的圆的三角形近似，并在其上积分一个单位大小的常数函数（
 * $f = 1$
 * ）。当然，如果领域是精确的单位圆，那么面积将是 $\pi$
 * ，但由于我们只使用了分片多项式段的近似，我们所积分的面积值并不完全是
 * $\pi$ 。然而，众所周知，当我们细化三角形时， $Q_p$
 * 的映射会以 $h^{p+1}$ 的阶数逼近边界，其中 $h$
 * 是网格大小。我们将检查不同映射下圆的计算面积值及其向
 * $\pi$
 * 网格细化的收敛性。我们还将发现一个收敛行为，这个行为一开始是令人惊讶的，但有一个很好的解释。
 *
 *
 * 第二种方法的工作原理与此类似，但这次不使用三角形单位圆的面积，而是使用其周长。
 * $\pi$ 是周长的一半，因为我们选择半径等于1。
 *
 *
 * @note
 * 本教程实质上展示了如何为积分选择一个特定的映射，通过将一个特定的几何体附加到三角测量中（例如在
 * step-1
 * 中已经做过），然后将一个映射参数传递给FEValues类，该类在deal.II中用于所有积分。我们选择的几何体是一个圆，deal.II已经有一个可以使用的类（SphericalManifold）。如果你想定义你自己的几何体，例如，因为它很复杂，而且不能用deal.II中已有的类来描述，你将需要阅读
 * step-53  。
 *
 ...
 *
 * 现在，由于我们要计算 $\pi$
 * 的值，我们必须与一些东西进行比较。这些是 $\pi$
 * 的前几个数字，我们事先定义好，以便以后使用。由于我们想计算两个数字的差值，而这两个数字是相当精确的，计算出的
 * $\pi$
 * 的近似值的精度在一个双变量可以容纳的数字范围内，所以我们宁可将参考值声明为
 * <code>long double</code> ，并给它增加一些数字。
 *
 *
 * @code
 ...
 * 这些函数中的第一个只是生成一个圆（超球）的三角形，并输出
 * $Q_p$
 * 的不同值的单元格的映射。然后，我们细化一次网格，再做一次。
 *
 *
 * @code
 * template <int dim>
 ...
 * @endcode
 *
 * 所以首先生成一个圆的粗略三角形，并将一个合适的边界描述关联到它。默认情况下，
 * GridGenerator::hyper_ball
 * 将SphericalManifold附加到边界上（内部使用FlatManifold），所以我们只需调用该函数就可以继续。
 *
 *
 * @code
 ...
 *
 * @endcode
 *  然后在当前网格上交替生成 $Q_1$ 、 $Q_2$ 和 $Q_3$
 * 映射的输出，以及（在循环体的末端）对网格进行一次全局细化。
 *
 *
 * @code
 ...
 * @endcode
 * 顺便提一下，对于一个片状线性映射，你可以给MappingQ的构造函数一个
 * <code>1</code>
 * 的值，但也有一个MappingQ1类可以达到同样的效果。历史上，它以比MappingQ更简单的方式做了很多事情，但今天只是后者的一个包装。然而，如果你没有明确指定另一个映射，它仍然是库中许多地方隐含使用的类。
 *
 *
 *
 ...
 *
 * 现在我们进行代码的主要部分，对 $\pi$
 * 进行逼近。圆的面积当然是由 $\pi r^2$
 * 给出的，所以有一个半径为1的圆，面积代表的只是搜索到的数字。面积的数值计算是通过在整个计算域中积分值为1的常数函数来进行的，即通过计算面积
 * $\int_K 1 dx=\int_{\hat K} 1 \ \textrm{det}\ J(\hat x) d\hat x \approx
 * \sum_i \textrm{det} \ J(\hat x_i)w(\hat x_i)$
 * ，其中总和扩展到三角形中所有活动单元上的所有正交点，
 * $w(x_i)$ 是正交点的权重 $x_i$
 * 。每个单元上的积分都是通过数字正交来逼近的，因此我们唯一需要的额外成分是建立一个FEValues对象，提供每个单元的相应`JxW`值。注意`JxW`是指<i>Jacobian
 * determinant times
 * weight</i>的缩写；因为在数值正交中，两个因子总是出现在相同的地方，所以我们只提供合并的数量，而不是两个单独的数量）。我们注意到，在这里我们不会在其最初的目的中使用FEValues对象，即用于计算特定正交点上的特定有限元的基函数值。相反，我们只用它来获得正交点上的`JxW'，而不考虑我们将给FEValues对象的构造器的（假）有限元。给予FEValues对象的实际有限元根本不使用，所以我们可以给任何。
 *
 *
 * @code
 * template <int dim>
 ...
 * @endcode
 *
 * 我们现在创建一个有限元。与其他的例子程序不同，我们实际上不需要用形状函数做任何计算；我们只需要FEValues对象的`JxW`值。因此，我们使用特殊的有限元类FE_Nothing，它的每个单元的自由度正好为零（顾名思义，每个单元的局部基础为空集）。FE_Nothing的一个比较典型的用法见
 * step-46  。
 *
 *
 * @code
 ...
 * @endcode
 *
 * 同样的，我们需要创建一个DoFHandler对象。我们实际上并没有使用它，但是它将为我们提供
 * "active_cell_iterators"，这是重新初始化三角形的每个单元上的FEValues对象所需要的。
 *
 *
 * @code
 ...
 * @endcode
 *
 * 现在我们设置FEValues对象，给构造函数提供Mapping、假有限元和正交对象，以及要求只在正交点提供`JxW`值的更新标志。这告诉FEValues对象在调用
 * <code>reinit</code>
 * 函数时不需要计算其他数量，从而节省计算时间。
 ...
 * @endcode
 *
 * 我们使用一个ConvergenceTable类的对象来存储所有重要的数据，如
 * $\pi$ 的近似值和与 $\pi$
 * 的真实值相比的误差。我们还将使用ConvergenceTable类提供的函数来计算
 * $\pi$ 的近似值的收敛率。
 *
 *
 * @code
 ...
 * @endcode
 *
 * 在这个循环中，我们首先将当前三角形的活动单元的数量添加到表格中。这个函数会自动创建一个上标为
 * "cells "的表格列，如果这个列之前没有被创建的话。
 *
 *
 * @code
 ...
 *
 * - 否则它将在下面的 FEValues::reinit 函数中抛出一个断言。
 *
 *
 * @code
 ...
 * @endcode
 *
 * ...并将得到的面积值和误差存储在表中。我们需要静态转换为双数，因为没有实现add_value(string,
 * long double)函数。请注意，这也涉及到第二个调用，因为
 * <code>std</code> 命名空间中的 <code>fabs</code>
 * 函数在其参数类型上是重载的，所以存在一个获取并返回
 * <code>long double</code>
 * 的版本，而全局命名空间中只有一个这样的函数被声明（获取并返回一个双数）。
 *
 *
 * @code
 ...
 *
 * 我们想计算 "错误
 * "列的收敛率。因此我们需要在调用`evaluate_all_convergence_rates`之前，将其他列从收敛率评估中省略。
 *
 *
 * @code
 ...
 *
 * 下面，第二个函数也是计算 $\pi$
 * 的近似值，但这次是通过域的周长 $2\pi r$
 * 而不是面积。这个函数只是前一个函数的一个变化。因此，我们主要给出不同之处的文件。
 *
 *
 * @code
 * template <int dim>
 ...
 * @endcode
 * 或者使用其他的文件名。第二行确保生成的输出的长宽比实际上是1:1，也就是说，在你的屏幕上，圆形被画成一个圆，而不是椭圆。第三行关闭了图形中的按键，因为这只会打印出现在并不重要的信息（文件名）。同样地，第四行和第五行关闭了刻度线。然后生成具有特定线宽（"lw"，这里设置为4）和线型（"lt"，这里选择线应该用RGB颜色
 * "黑色 "绘制）的图。 下表显示了 $Q_1$ 、 $Q_2$ 和 $Q_3$
 * 映射的三角计算域，原始粗网格（左）和一次均匀细化网格（右）。
 * <div class="twocolumn" style="width: 80%"> <div> <img
 ...
 * </div> </div>
 * 这些图片显示了高阶映射的明显优势：它们在相当粗的网格上也能相当好地接近真实边界。为了进一步证明这一点，这里是使用
 * $Q_2$ 和 $Q_3$
 * 映射的粗网格的右上角四分之一圆的一部分，其中红色虚线标志着实际圆。
 * <div class="twocolumn" style="width: 80%"> <div> <img
 ...
 *
 *
 * 从输出中可以直接观察到，在所有情况下，数值都会迅速收敛到
 * $\pi=3.141592653589793238462643$ 的真实值。请注意，对于 $Q_4$
 * 的映射，我们已经进入了四舍五入误差的状态，收敛率趋于平稳，这已经是相当大的进步了。然而，也请注意，对于
 * $Q_1$ 映射，即使在最细的网格上，精度也比 $Q_3$
 * 映射的粗略网格上要差得多!
 *
 *
 * 输出的最后一列显示了收敛顺序，以网格宽度 $h$
 * 为单位。在介绍中，我们曾说过， $Q_p$
 * 映射的收敛顺序应该是 $h^{p+1}$
 * 。然而，在所示的例子中，顺序是 $h^{2p}$  !
 * 这个最初令人惊讶的事实可以用 $Q_p$
 * 映射的特性来解释。在<i>p</i>阶，它使用的支持点是基于<i>p</i>+1点Gauss-Lobatto正交规则，以这样的方式选择支持点，使正交规则在2<i>p</i>阶收敛。尽管这些点在这里只用于插值<i>p</i>阶多项式，但我们在对积分进行数值计算时得到了一个超级收敛效应，从而得到了观察到的高收敛阶数。这一效应在以下出版物中也有详细讨论。A.
 * Bonito, A. Demlow, and J. Owen:
 * "对拉普拉斯-贝特拉米算子的特征值和特征函数的有限元近似的先验误差估计"，已提交，2018年）。)
 *
