...
* <a name="Foreword"></a><h3>Foreword</h3> * <a name="Foreword"></a><h3>Foreword</h3>。
* 

* 本程序涉及一个非线性椭圆偏微分方程的例子，即[最小表面方程](https://en.wikipedia.org/wiki/Minimal_surface)。你可以想象这个方程的解描述一个肥皂膜所跨越的表面，这个肥皂膜被一个封闭的金属环所包围。我们想象一下，这根线不仅仅是一个平面的环，而且是弯曲的。肥皂膜的表面张力将使该表面变成最小表面。最小表面方程的解描述了这种形状，并以金属丝的垂直位移为边界条件。为了简单起见，我们在这里假设曲面可以写成图形 $u=u(x,y)$ ，尽管很明显，要构造出钢丝弯曲的情况并不难，因为在这种情况下，曲面只能局部地被构造成图形，而不是全局地。
...
* 从上面的强表述开始，我们通过用测试函数 $\varphi$ 乘以PDE的两边，并对两边进行部分积分，得到弱表述。 @f[
...
* 与上述牛顿步骤相对应的矩阵可以重新表述，以更好地显示其结构。稍微改写一下，我们可以得到它的形式@f[
  A_{ij}
  =
  \left(
    \nabla \varphi_i,
...
\frac{\nabla u^n}{|\nabla u^n|}$ 是 $B$ 的特征向量，其特征值为 $\lambda_1=a_n \left(1-\frac{|\nabla u^n|^2}{1+|\nabla u^n|^2}\right) > 0$ ，而所有与 $v_1$ 垂直的向量 $v_2\ldots v_d$ 和彼此都是特征向量，其特征值为 $a_n$ 。因为所有的特征值都是正的，所以 $B$ 是正定的， $A$ 也是。因此，我们可以使用CG方法来解决牛顿步骤。（矩阵 $A$ 是对称和正定的这一事实不应该让人惊讶。它来自于对一个算子的导数，而这个算子又来自于对一个能量函数的导数：最小表面方程只是最小化了一些非二次元的能量。因此，作为标量能量的二阶导数矩阵，牛顿矩阵必须是对称的，因为与 $i$ th和 $j$ th自由度有关的导数显然应该相减。同样，如果能量函数是凸的，那么第二导数的矩阵必须是正定的，上面的直接计算只是重申了这一点。)
...
* 如上所述，牛顿方法是通过计算一个方向 $\delta u^n$ ，然后以步长 $0 < \alpha^n \le 1$ 执行更新 $u^{n+1} = u^{n}+\alpha^n
...
* 总的来说，我们这里的程序在许多方面与 step-6 并无不同。主类的布局本质上是相同的。另一方面， <code>run()</code> 函数中的驱动算法是不同的，工作方式如下。 <ol>   <li>  从函数 $u^{0}\equiv 0$ 开始，以这样的方式修改它，使 $u^0$ 的值沿边界等于正确的边界值 $g$ （这发生在 <code>MinimalSurfaceProblem::set_boundary_values</code> ）。设 $n=0$  .  </li>  
...
x\|<1\}\subset {\mathbb R}^2$ 上找到最小曲面的解决方案，该曲面沿边界达到 $u(x,y)|{\partial\Omega} = g(x,y) \dealcoloneq \sin(2 \pi (x+y))$ 的值。
* 
...
* -  <code>setup_system</code> 函数需要一个参数，表示这是否是第一次被调用。不同的是，第一次我们需要分配自由度并将 $u^n$ 的解向量设置为正确的大小。在接下来的时间里，这个函数是在我们已经完成了这些步骤，作为细化网格的一部分之后被调用的  <code>refine_mesh</code>  。
* 

* 
* 
* - 然后我们还需要新的函数。  <code>set_boundary_values()</code> 负责正确设置解向量的边界值，这在介绍的最后已经讨论过了。  <code>compute_residual()</code> 是一个计算非线性（离散）残差准则的函数。我们用这个函数来监测牛顿迭代的收敛性。该函数以步长 $\alpha^n$ 为参数来计算 $u^n + \alpha^n
 \; \delta u^n$ 的残差。这是人们通常需要的步长控制，尽管我们在这里不会使用这个功能。最后， <code>determine_step_length()</code> 计算每个牛顿迭代中的步长 $\alpha^n$ 。正如介绍中所讨论的，我们在这里使用一个固定的步长，并把实现一个更好的策略作为一个练习。(  step-77 的做法不同。它只是在整个求解过程中使用了一个外部软件包，而一个好的直线搜索策略是该软件包提供的一部分）。)
* 
...
* 
* 在setup-system函数中，我们总是设置有限元方法的变量。与 step-6 有相同的区别，因为在那里我们在每个细化循环中都要从头开始求解PDE，而在这里我们需要把以前的网格的解放到当前的网格上。因此，我们不能只是重置解向量。因此，传递给这个函数的参数表明我们是否可以分布自由度（加上计算约束）并将解向量设置为零，或者这在其他地方已经发生过了（具体来说，在 <code>refine_mesh()</code> ）。
* 
...
* 
 @endcode
* 
* 对于线性系统的组装，我们必须在正交点上获得前一个解的梯度值。有一个标准的方法： FEValues::get_function_gradients 函数接收一个代表定义在DoFHandler上的有限元场的向量，并评估这个场在FEValues对象最后被重新初始化的单元的正交点的梯度。然后将所有正交点的梯度值写入第二个参数中。
* 
...
* 
* 求解函数和以往一样。在求解过程的最后，我们通过设置 $u^{n+1}=u^n+\alpha^n\;\delta u^n$ 来更新当前的解决方案。
* 
...
* 
* 这个函数的第一部分与 step-6 中的内容相同 ... 然而，在细化网格之后，我们必须将旧的解决方案转移到新的解决方案中，这一点我们要借助SolutionTransfer类来完成。这个过程稍微有点复杂，所以让我们详细描述一下。
* 
...
* 然后我们需要一个额外的步骤：比如说，如果你标记了一个比它的邻居多精炼一次的单元，而这个邻居没有被标记为精炼，那么我们最终会在一个单元界面上跳出两个精炼级别。 为了避免这些情况，库将默默地也要对邻居单元进行一次细化。它通过在实际进行细化和粗化之前调用 Triangulation::prepare_coarsening_and_refinement 函数来实现。 这个函数标志着一组额外的单元格进行细化或粗化，以执行像单悬节点规则这样的规则。 调用此函数后，被标记为细化和粗化的单元格正是那些将被实际细化或粗化的单元格。通常情况下，你不需要用手去做这件事 (Triangulation::execute_coarsening_and_refinement 为你做这件事)。然而，我们需要初始化SolutionTransfer类，它需要知道最终将被粗化或细化的单元集，以便存储旧网格的数据并转移到新网格。因此，我们用手调用这个函数。
* 
...
* 
 @endcode
* 
* 一旦我们有了内插的解决方案和所有关于悬挂节点的信息，我们必须确保我们现在的 $u^n$ 实际上具有正确的边界值。正如在介绍的最后所解释的那样，即使细化前的解具有正确的边界值，也不会自动出现这种情况，因此我们必须明确地确保它现在具有。
* 
...
* 正如介绍中所讨论的，如果我们总是采取全步，即计算 $u^{n+1}=u^n+\delta
 u^n$ ，牛顿方法经常不能收敛。相反，我们需要一个阻尼参数（步长）  $\alpha^n$  并设置  $u^{n+1}=u^n+\alpha^n\delta u^n$  。这个函数就是用来计算  $\alpha^n$  的。  
...
* 正如介绍中所描述的，领域是围绕原点的单位盘，其创建方式与 step-6 中所示相同。网格被全局细化了两次，后来又进行了几次自适应循环。  
...
* 
 @endcode
* 
* 牛顿迭代接下来开始。我们一直迭代到上一次迭代结束时计算的残差（准则）小于 $10^{-3}$ ，正如在 "do{ ... } while "循环结束时检查的那样。} while "循环的末尾进行检查。因为我们没有一个合理的值来初始化这个变量，我们只是使用可以表示为 "双数 "的最大值。
* 
