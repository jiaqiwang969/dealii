 ...
 * 教程不仅是指静态的文档，而且你应该玩玩它们。为此，进入
 * <code>examples/step-1</code>
 * 目录（或任何你感兴趣的教程的编号），然后输入
 * @code
 * cmake .
 * make
 * make run
 * @endcode
 * 第一条命令设置了描述该教程程序所依赖的包含文件、如何编译以及如何运行的文件。这个命令应该能找到已安装的deal.II库，这些库是在编译和安装<a
 * href="../../readme.html"
 * target="body">README</a>文件中描述的一切时产生的。
 * @code
 * cmake
 *
 ...
 * @endcode
 * 代替。
 * 上述命令中的第二条将源代码编译成可执行文件，而最后一条将执行它（严格地说，如果可执行文件还不存在，
 * <code>make run</code>
 * 也将编译代码，所以如果你想的话，可以跳过第二条命令）。这就是运行代码和产生输出所需的全部内容，在教程程序的
 * "结果
 * "部分讨论。这个顺序需要在你想玩的所有教程目录中重复进行。
 * 在学习库的时候，你需要用它来玩，看看会发生什么。为此，用你喜欢的编辑器打开
 * <code>examples/step-1/step-1.cc</code>
 * 的源文件，以某种方式修改它，保存它并按上述方法运行。在本程序的结果部分，我们给出了一些可能的修改建议，在那里我们还提供了一些与其他有用信息的链接。
 *
 ...
 * Bangerth's video
 * lectures</a>中讨论并演示了关于deal.II和计算科学。特别是，你可以看到他运行这个和其他程序的步骤，你会对可以用来处理deal.II的工具有一个更好的概念。特别是，第2和第4讲概述了deal.II和任何有限元代码的构建块。
 * 如果你还不熟悉使用Linux和在命令行上运行东西，你可能有兴趣看第2.9和2.91讲。
 * (  @dealiiVideoLectureSeeAlso{2.9,2.91}) These give overviews over the
 * command 行和关于编译程序时发生的事情，分别。
 * 请注意，deal.II正在积极开发，在开发过程中，我们偶尔会对这些视频讲座中仍然引用的函数或类进行重新命名或废弃。
 * 例如，视频讲座5中的 step-1
 * 代码使用了一个HyperShellBoundary类，后来被SphericalManifold类取代。此外，从deal.II
 * 9.0版本开始， GridGenerator::hyper_shell()
 * 现在会自动将SphericalManifold附加到三角结构中。除此以外，其余的讲座材料都是相关的。
 * <a name="Whatthisprogramdoes"></a><h3> What this program does </h3>
 *
 * 在这个第一个例子中，我们实际上并没有做很多事情，而是展示了两个技术：生成三角形对象的语法是什么，以及在所有单元上进行简单循环的一些要素。我们创建了两个网格，一个是有规律的细化的正方形（不是很令人兴奋，但对于某些问题来说是一个常见的起始网格），还有一个是几何学上的尝试：弧形域，它是向内侧边缘细化的。通过这些，你将了解到每一个有限元程序都必须有的三样东西。Triangulation类型的对象；调用GridGenerator函数来生成网格；以及涉及迭代器的所有单元的循环（迭代器是指针的泛化，在C++标准库中经常使用；在deal.II的背景下， @ref
 * Iterators 模块谈到了它们）。
 * 该程序非常小，不需要大量的介绍。 @dealiiVideoLecture{5,6}
 * <a name="Aboutscientificcomputingingeneral"></a><h3> About scientific
 * computing in general </h3> 。
 *
 * 如果你正在阅读这个教程程序，你很可能有兴趣继续在你自己的项目中使用deal.II。因此，你即将开始使用一个大型科学计算库进行编程练习。除非你已经是大规模编程方法的资深用户，否则这对你来说可能是一个新的领域；伴随着所有的新规则，比如你将不得不处理别人写的代码，你可能不得不考虑记录你自己的代码，因为你可能在一年后不记得它到底在做什么（或者因为别人也会使用它），或者想出一些方法来测试你的程序是否做得对。这些都不是我们通常训练数学家、工程师或科学家的东西，但当你开始编写超过几百行的软件时，这些都很重要。请记住。制作软件并不等同于仅仅写代码。
 * 为了使你在这一旅程中生活得更轻松，让我们指出一些值得在你开始任何大规模编程之前浏览的资源。
 *
 * - <a href="https://github.com/dealii/dealii/wiki/Frequently-Asked-Questions">
 * deal.II
 * FAQ</a>有很多关于deal.II特定方面的问题的答案，但也有一些更普遍的问题，如
 * "我如何调试科学计算代码？"或
 * "我能否训练自己写出更少的错误的代码？"。
 *
 ...
 * project</a>也有关于软件开发的资源，特别是用于并行计算的资源。在该网站的
 * "活动
 * "部分有录制的教程和网络研讨会，涵盖了许多有趣的主题。
 *
 * - <a href="http://arxiv.org/abs/1210.0530">Best
 * Practices for Scientific
 * Computing</a>上有一篇文章，介绍了许多方法，通过这些方法可以确保你是一个高效的程序员，写出能工作的程序。
 * 作为一个一般性的建议。如果你希望在未来花几天时间来编写软件，那么请你学习能使你的生活更有效率的工具，特别是调试器和集成开发环境。(
 * @dealiiVideoLectureSeeAlso{7,8,8.01,25})
 * 你会发现，通过提高工作效率，你很快就会把学习这些工具的时间拿回来几倍！上面提到的几个视频讲座展示了如何使用集成开发环境或调试器等工具。
 *
 ...
 * @endcode
 *
 * 还有这个用于 `std::sqrt` 和 `std::fabs` 函数的声明。
 *
 * @code
 * #include <cmath>
 *
 ...
 * <code>dealii::</code>
 * 的前缀，但这很快就会变得繁琐和令人厌烦。相反，我们只是简单地导入整个deal.II命名空间，以供一般使用。
 *
 *
 * @code
 ...
 *
 * 在下面的第一个函数中，我们简单地使用单位平方作为域，并从中产生一个全局精炼的网格。
 *
 *
 * @code
 * void first_grid()
 ...
 * @endcode
 *
 * 在这里和下面的许多情况下，类名后面的字符串"<2>"表示这是一个应在两个空间维度上工作的对象。同样，也有一些三角形类的版本是在一个（"<1>"）和三个（"<3>"）空间维度中工作的。这种工作方式是通过一些模板魔法实现的，我们将在后面的示例程序中详细研究；在那里，我们还将看到如何以一种基本上独立于维度的方式编写程序。
 *
 *
 * 接下来，我们要用一个正方形域的单个单元来填充三角结构。三角形被细化了四次，总共得到
 * $4^4=256$ 个单元。
 *
 *
 * @code
 ...
 * @endcode
 *
 * 现在我们想把网格的图形表示写到一个输出文件中。deal.II的GridOut类可以用多种不同的输出格式来实现；在这里，我们选择可伸缩矢量图（SVG）格式，你可以用你选择的网络浏览器来进行可视化。
 *
 *
 * @code
 ...
 * @endcode
 *
 * 接下来，我们需要对三角形的活动单元进行循环。你可以把三角形看作是一个单元格的集合。如果它是一个数组，你只需要得到一个指针，用操作符`++`从一个元素递增到下一个元素。三角形的单元不是作为一个简单的数组来存储的，但是一个<i>iterator</i>的概念将指针的工作方式概括为任意的对象集合（更多信息见<a
 * href=
 * "http://en.wikipedia.org/wiki/Iterator#C.2B.2B">wikipedia</a>）。通常情况下，C++中的任何容器类型都会返回一个迭代器，指向集合的开始，其方法称为`begin'，而迭代器则指向集合结束后的1，其方法称为`end`。我们可以用操作符`++it`来增加迭代器`it`，用`*it`来解除引用以获得底层数据，并通过比较`it
 * != collection.end()`来检查我们是否已经完成。
 * 第二个重要部分是我们只需要活动单元。活跃单元是那些没有被进一步细化的单元，也是唯一可以被标记为进一步细化的单元。deal.II提供了迭代器类别，允许我们在<i>all</i>单元（包括活跃单元的父单元）或只在活跃单元上迭代。因为我们想要后者，所以我们需要调用
 * Triangulation::active_cell_iterators(). 方法。
 * 把所有这些放在一起，我们可以用 <div
 ...
 *
 * </div>
 * 在一个三角形的所有活动单元上循环，在这个循环的初始化器中，我们用`auto`关键字表示迭代器`it`的类型。`auto`关键字意味着被声明的对象的类型将从上下文中推断出来。当实际的类型名称很长，甚至可能是多余的时候，这个关键字很有用。如果你不确定类型是什么，想查一下结果支持什么操作，你可以去看方法的文档
 * Triangulation::active_cell_iterators().
 * 在这个例子中，`it`的类型是
 * `Triangulation::active_cell_iterator`.  。
 * 虽然`auto'关键字可以让我们不用输入长长的数据类型名称，但我们仍然要输入大量冗余的声明，说明开始和结束迭代器以及如何递增。与其这样，我们不如使用<a
 * href="http://en.cppreference.com/w/cpp/language/range-for">range- based for
 * loops</a>，它将上面显示的所有语法包成一个更短的形式。
 *
 *
 * @code
 ...
 *
 * @note
 * 关于deal.II中使用的迭代器类的更多信息见 @ref Iterators
 * ，关于基于范围的for循环和`auto`关键字的更多信息见 @ref
 * CPP11  。
 * 接下来，我们在单元格的所有顶点上循环。为此，我们查询一个顶点索引的迭代器（在2D中，这是一个数组，包含的元素是`{0,1,2,3}`，但是由于`cell->vertex_indices()`知道单元格所处的维度，因此返回的数组在所有维度上都是正确的，这使得无论我们在2D还是3D中运行这段代码都是正确的，也就是说，它实现了
 * "独立维编程"
 *
 ...
 * @endcode
 *
 * 如果这个单元格位于内边界，那么它的顶点中至少有一个必须位于内环上，因此与中心的径向距离正好是0.5，达到浮点精度。所以我们计算这个距离，如果我们发现一个顶点具有这个属性，我们就标记这个单元，以便以后进行细化。然后我们也可以打破所有顶点的循环，转到下一个单元。
 ...
 * "这样的语句将会失败，除非我们运气特别好。相反，我们需要以一定的容忍度来做这种比较，通常的方法是写成`if
 ...
 * @endcode
 *
 * 最后，在这五次细化迭代之后，我们要再次将得到的网格写到一个文件中，同样是SVG格式。这和上面的工作一样。
 *
 *
 * @code
 ...
 *
 * 运行该程序会产生两个网格的图形（grid-1.svg和grid-2.svg）。你可以用大多数网络浏览器打开这些网格
 *
 * - 在最简单的情况下，只要在你的文件系统资源管理器中打开当前目录，然后点击文件。如果你喜欢在命令行上工作，你可以用该文件调用你的网络浏览器。`firefox grid-1.svg`，`google-chrome grid-1.svg`，或者任何你的浏览器的名字。如果你这样做，这两个网格应该看起来像这样。
 ...
 * alt=""> </td> </tr> </table>
 * 左边那个，嗯，不是很刺激。右边的是&mdash;至少&mdash;非常规的。这些图片对每个单元格的
 * "细化水平
 * "进行了颜色编码。一个粗略的网格单元要经过多少次的细分才能得到指定的单元。在左边的图片中，这是无聊的，因为网格在全球范围内被细化了若干次，也就是说，<i>every</i>单元被细化的次数相同。
 * （虽然第二个网格完全是人为的和捏造的，而且肯定在应用中不是很实用，但令大家惊讶的是，它已经进入了文献：见
 * @cite Mu05 。显然，它至少在某些方面是好的）。)
 *
 ...
 *
 *  这个程序显然没有太多的功能，但特别是
 * <code>second_grid</code>
 * 函数有很多地方你可以发挥它。例如，你可以修改我们决定对哪些单元格进行细化的标准。一个例子是把条件改成这样。
 * @code
 *    for (auto &cell: triangulation.active_cell_iterators())
 *      if (cell->center()[1] > 0)
 *        cell->set_refine_flag ();
 * @endcode
 * 这将细化所有单元格中心的 $y$
 * 坐标大于零的单元格（我们通过取消引用 <code>cell</code>
 * 迭代器调用的 <code>TriaAccessor::center</code>
 * 函数返回一个Point<2>对象；下标 <code>[0]</code> 将得到 $x$
 * 坐标，下标 <code>[1]</code> 得到 $y$
 * 坐标）。通过查看TriaAccessor提供的函数，你也可以使用更复杂的细化标准。
 ...
 * 对整个迭代器概念的更全面的描述可以在 @ref
 * Iterators 文档模块中找到。
 *
 *  <a name="Differentgeometries"></a><h4> Different geometries </h4> 。
 *
 * 另一种可能性是生成完全不同几何形状的网格。虽然对于复杂的几何体来说，没有办法使用从网格生成器获得的网格，但是有很多几何体可以使用GridGenerator命名空间的函数来创建网格。许多这样的几何体（如本例程序中使用的几何体）包含有弯曲面的单元：换句话说，我们希望放置在边界上的新顶点位于一个圆上。deal.II通过Manifold类（以及从它继承的类）处理复杂的几何体；特别是GridGenerator中对应于非笛卡尔网格的函数（如 GridGenerator::hyper_shell  ]或 GridGenerator::truncated_cone) 将一个Manifold对象附加到三角形中应该是曲线的部分（分别为SphericalManifold和CylindricalManifold），并在应该是平面的部分使用另一个Manifold（FlatManifold）。关于这些类的设计理念和接口的描述，请参见Manifold的文档或 @ref manifold "歧管模块"
 * 。看看它们提供了什么，看看它们如何在这样的程序中被使用。
 * 我们还在 step-49
 * 中讨论了其他各种创建和操作网格的方法（并描述了附加流形的过程）。
 *
 *  <a name="Commentsaboutprogramminganddebugging"></a><h4> Comments about
 * programming and debugging </h4> 。
 *
 * 最后，我们对用deal.II修改或编写程序做一个评论。当你开始使用教程程序或你自己的应用程序时，你会发现错误会发生：你的程序会包含一些代码，这些代码要么是立即中止程序，要么是简单地导致错误的结果。无论哪种情况，你都会发现知道如何使用调试器是非常有帮助的：你可能会通过把调试输出放到你的程序中，编译它，然后运行它来应付一段时间，但最终用调试器来发现错误要快得多，方便得多，也可靠得多，因为你不必总是重新编译程序，而且你可以检查变量的值和它们的变化。
 * 与其推迟学习如何使用调试器，直到你真的看不到任何其他方法来发现一个错误，这里是我们将在本程序中提供的一个建议：尽可能快地学习如何使用调试器。(
 * @dealiiVideoLectureSeeAlso{25}) The deal.II Frequently Asked  Questions
 * (FAQ) page linked to from the top-level <a
 * href="http://www.dealii.org/">deal.II webpage</a> also provides a good
 * number of hints on debugging deal.II programs.
 *
