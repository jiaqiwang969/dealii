...
* 
* 一个经常出现的问题是如何用deal.II解决涉及复值函数的问题。对于许多问题，与其直接使用复值有限元，不如将复值函数分成实部和虚部，并使用单独的标量有限元场对其进行离散化，这样做往往更方便。基本上，这相当于把一个单一的复值方程看成是两个实值方程的系统。这个简短的例子演示了如何在deal.II中通过使用 <code>FE_system</code> 对象来堆叠两个代表实部和虚部的有限元场来实现。(相反的方法，保持所有的复值，在另一个教程程序中演示：见 step-58 。)当分成实部和虚部时，这里涉及的方程属于矢量值问题的范畴。对这一主题的顶层概述可以在 @ref vector_valued 模块中找到。
...
* 为了得出这个问题的模型，我们认为超声波是由波浪方程支配的压力波：@f[
	\frac{\partial^2 U}{\partial t^2}	-	c^2 \Delta U = 0
@f]其中 $c$ 是波速（为简单起见，我们假设它是常数）， $U
= U(x,t),\;x \in \Omega,\;t\in\mathrm{R}$  。边界 $\Gamma=\partial\Omega$ 分为两部分 $\Gamma_1$ 和 $\Gamma_2=\Gamma\setminus\Gamma_1$ ，其中 $\Gamma_1$ 代表换能器透镜， $\Gamma_2$ 是吸收边界（也就是说，我们要在 $\Gamma_2$ 上选择边界条件，使其模仿大域的方式）。在 $\Gamma_1$ 上，换能器产生一个恒定频率 ${\omega}>0$ 和恒定振幅（我们在这里选择为1）的波：@f[
U(x,t) = \cos{\omega t}, \qquad x\in \Gamma_1
@f] 。
* 如果没有其他（内部或边界）源，并且由于唯一的源具有频率 $\omega$ ，那么该解决方案允许一个形式为 $U(x,t) = \textrm{Re}\left(u(x)\,e^{i\omega
t})\right)$ 的变量分离。复值函数 $u(x)$ 描述了频率外波的振幅和相位（相对于源）的空间依赖性 ${\omega}$ ，其中振幅是我们感兴趣的量。通过将这种形式的解决方案插入波浪方程，我们看到，对于 $u$ ，我们有@f{eqnarray*}
* 
-\omega^2 u(x)
* 
...
@f}
* 
* 为了在 $\Gamma_2$ 上找到合适的条件来模拟一个吸收边界，考虑一个频率为 ${\omega}$ 的波在 $k\in {\mathrm{R}^2}$ 方向上行驶。为了使 $V$ 能够解决波浪方程， $|k|={\frac{\omega}{c}}$ 必须成立。假设这个波在 $x_0\in\Gamma_2$ 中以直角撞击边界，即 $n=\frac{k}{|k|}$ ， $n$ 表示 $\Omega$ 在 $x_0$ 的外单位法线。然后在 $x_0$ ，这个波满足方程@f[
c (n\cdot\nabla V) + \frac{\partial V}{\partial t} = (i\, c\, |k|
* 
- i\, \omega) V = 0.
@f]。因此，通过强制执行边界条件@f[
c (n\cdot\nabla U) + \frac{\partial U}{\partial t} = 0, \qquad x\in\Gamma_2,
@f]，以直角撞击边界 $\Gamma_2$ 的波将被完全吸收。另一方面，那些没有以直角撞击边界的波场部分并不满足这个条件，将其作为边界条件来执行将产生部分反射，也就是说，只有部分波会通过边界，就像它不在这里一样，而剩下的那部分波将被反射回域中。
* 如果我们愿意接受这一点作为吸收边界的充分近似，我们最终得出以下问题 $u$ ：@f{eqnarray*}
* 
-\omega^2 u
* 
...
@f} 。
* 这是一个亥姆霍兹方程（类似于 step-7 中的方程，但这次有''坏符号''）， $\Gamma_1$ 上有迪里希特数据， $\Gamma_2$ 上有混合边界条件。由于 $\Gamma_2$ 上的条件，我们不能只是单独处理 $u$ 的实部和虚部方程。然而，我们可以把 $u$ 的PDE看作是 $u$ 的实部和虚部的两个PDE系统， $\Gamma_2$ 的边界条件代表系统中两个部分之间的耦合项。这是按以下思路进行的。让 $v=\textrm{Re}\;u,\; w=\textrm{Im}\;u$ ，然后在 $v$ 和 $w$ 方面，我们有以下系统：@f{eqnarray*}
...
@f}
* 
* 对于测试函数 $\phi,\psi$ 与 $\phi|_{\Gamma_1}=\psi|_{\Gamma_1}=0$ ，经过通常的乘法，在 $\Omega$ 上的积分和应用分项积分，我们得到弱表述@f{eqnarray*}
* 
...
\end{array}
\right)
@f] ](不要被这里的右手边为零所迷惑，那是因为我们还没有包括Dirichlet边界数据。)由于非对角线块的交替符号，我们已经可以看到这个系统是非对称的，事实上它甚至是不确定的。当然，没有必要选择空间 $V_h$ 和 $W_h$ 是相同的。然而，我们希望解的实部和虚部具有类似的性质，因此在实施过程中确实会采取 $V_h=W_h$ ，并且对这两个空间使用相同的基函数 $\phi_i = \psi_i$ 。之所以使用不同的符号，只是为了让我们区分 $v$ 和 $w$ 的形状函数，因为这种区分在实施中起着重要作用。
* 

*<a name="Thetestcase"></a><h3>The test case</h3>
* 

* 在计算中，我们将考虑波在单位方阵中的传播，超声波由换能器透镜产生，该透镜的形状像圆的一个部分，中心在 $(0.5, d)$ ，半径略大于 $d$ ；这种形状应导致声波在圆中心的聚焦。改变 $d$ 会改变透镜的 "焦点"，影响 $u$ 强度的空间分布，我们主要关注的是 $|u|=\sqrt{v^2+w^2}$ 的聚焦程度。
* 在下面的程序中，我们将使用实部和虚部分离的公式来实现复值亥姆霍兹方程。我们还将讨论如何生成一个看起来像正方形的域，并带有模拟换能器的轻微隆起（在 <code>UltrasoundProblem<dim>::make_grid()</code> 函数中），以及如何生成图形输出，不仅包含解分量 $v$ 和 $w$ ，还包括直接在输出文件中的幅度 $\sqrt{v^2+w^2}$ （在 <code>UltrasoundProblem<dim>::output_results()</code> ）。最后，我们使用ParameterHandler类来轻松读取参数，如焦距 $d$ 、波速 $c$ 、频率 $\omega$ ，以及在运行时从输入文件中读取其他一些参数，而不是在源代码中固定这些参数，每次我们想改变参数时，都必须重新编译。
* 
...
* 
 @endcode
* 
* 对于线性系统的求解，我们将使用UMFPACK提供的稀疏LU分解（见SparseDirectUMFPACK类），为此需要以下头文件。 请注意，为了编译这个教程程序，deal.II-library需要建立UMFPACK支持，这个支持在默认情况下是启用的。
* 
...
* 
 @endcode
* 
* 作为这个程序开始的最后一步，我们把这个程序中的所有东西都放到它的名字空间中，并且在这个名字空间中，让所有在deal.II名字空间中的东西都是全局可用的，而不需要在所有东西前面加上 <code>dealii</code><code>::</code>  。
* 
...
* 因为有两个值 $v$ 和 $w$ 需要在边界规定，我们必须告诉基类，这是一个有两个分量的向量值函数， <code>vector_value</code> 函数和它的表亲 <code>vector_value_list</code> 必须返回有两个条目的向量。在我们的例子中，这个函数非常简单，它只是对实部 $v$ 返回1，对虚部 $w$ 返回0，而不管它在哪个点被评估。
* 
...
* 
*  <code>declare_parameters</code> 函数声明了我们的ParameterHandler对象能够从输入文件中读取的所有参数，以及它们的类型、范围条件和它们出现在哪个小节。我们将用一对大括号包住所有进入一个部分的条目，以迫使编辑器将它们缩进一级，从而使阅读哪些条目共同构成一个部分变得更简单。
* 
...
* @code
 ...
 @endcode
* 
* 网格和几何的参数包括应用于初始粗略网格的全局细化步数和换能器镜头的焦距 $d$ 。对于细化步数，我们允许在 $[0,\infty)$ 范围内取整数，其中 Patterns::Integer 对象的第二个参数被省略，表示半开区间。 对于焦距，任何大于零的数字都可以接受。
* 
...
* 
 @endcode
* 
* 下一小节专门讨论方程中出现的物理参数，即频率  $\omega$  和波速  $c$  。同样，两者都需要位于半开区间 $[0,\infty)$ 内，通过调用 Patterns::Double 类，仅以左端点为参数来表示。
* 
...
* 
 ...
 @endcode
* 
* 最后但并非最不重要的是，我们希望能够通过配置文件中的条目来改变输出的一些属性，比如文件名和格式，这就是最后一节的目的。
* 
...
* 
 @endcode
* 
* 因为不同的输出格式在生成输出时可能需要不同的参数（例如，postscript输出需要视角角度、线宽、颜色等），如果我们必须为库中支持的每一种可能的输出格式手工声明所有这些参数，那就很麻烦了。相反，每种输出格式都有一个 <code>FormatFlags::declare_parameters</code> 函数，它在自己的小节中声明了该格式的所有特定参数。下面调用 DataOutInterface<1>::declare_parameters 为所有可用的输出格式执行 <code>declare_parameters</code> ，这样就为每种格式创建了一个自己的小节，并为该特定输出格式声明了参数。(上面 <code>@<1@></code> 的调用中，模板参数的实际值在这里并不重要：该函数做了同样的工作，与维度无关，但碰巧是在一个依赖模板参数的类中。)  要想知道哪种输出格式有哪些参数，你可以查阅DataOutBase类的文档，或者干脆在没有参数文件的情况下运行这个程序。然后，它将创建一个文件，将所有声明的参数设置为默认值，这可以方便地作为一个起点，将参数设置为你想要的值。
* 
...
* 
* 这是在ParameterReader类中的主函数。 它从外部被调用，首先声明所有的参数，然后从调用者提供的文件名的输入文件中读取参数。在对这个函数的调用完成后，可以使用 <code>prm</code> 对象来检索从文件中读入的参数值。
* 
...
* 
* 正如介绍中提到的，我们真正追求的量是超声波强度的空间分布，它对应于 $|u|=\sqrt{v^2+w^2}$  。现在我们可以只满足于在输出中拥有 $v$ 和 $w$ ，并使用合适的可视化或后处理工具从我们计算的解决方案中得出 $|u|$ 。然而，也有一种方法可以输出从deal.II的解决方案中得出的数据，我们在这里要利用这个机制。
* 

* 
* 到目前为止，我们一直使用 DataOut::add_data_vector 函数将包含输出数据的向量添加到一个DataOut对象中。 这个函数有一个特殊的版本，除了数据向量之外，还有一个额外的参数，类型为DataPostprocessor。当这个函数用于输出时，在每个要生成输出数据的点上，指定的DataPostprocessor对象的 DataPostprocessor::evaluate_scalar_field() 或 DataPostprocessor::evaluate_vector_field() 函数被调用，从数据向量代表的有限元函数的值、梯度和二阶导数计算输出量（在面相关数据的情况下，法向量也可用）。因此，这使我们可以输出任何可以从解的值及其导数中局部导出的数量。 当然，超声强度 $|u|$ 就是这样一个量，其计算甚至不涉及 $v$ 或 $w$ 的任何导数。
* 

* 
* 在实践中，DataPostprocessor类只提供了这个功能的接口，我们需要从它派生出我们自己的类，以便实现接口所指定的功能。在最一般的情况下，我们必须实现几个成员函数，但是如果输出量是一个单一的标量，那么其中的一些模板代码可以由一个更专业的类来处理，即DataPostprocessorScalar，我们可以从这个类派生出来。这就是 <code>ComputeIntensity</code> 类的作用。
* 
...
* 
 @endcode
* 
* 在构造函数中，我们需要用两个参数调用基类的构造函数。第一个参数表示这个类计算出来的单一标量在输出文件中应该用什么名字来表示。在我们的例子中，后处理程序有 $|u|$ 作为输出，所以我们使用 "Intensity"。  
* 第二个参数是一组标志，表明后处理程序需要哪些数据来计算输出量。 这可以是update_values、update_gradients和update_hessians（如果是脸部数据，也可以是update_normal_vector）的任何一个子集，这些都在UpdateFlags中记录。 当然，导数的计算需要额外的资源，所以这里只应该给出真正需要的数据的标志，就像我们使用FEValues对象时一样。在我们的例子中，只有 $v$ 和 $w$ 的函数值需要用来计算 $|u|$ ，所以我们用update_values标志就可以了。
* 
...
* 计算本身是直截了当的。我们遍历输出向量中的每个条目，并从 $v$ 和 $w$ 的相应值中计算出 $|u|$  。我们通过创建一个复数 $u$ ，然后对结果调用 `std::abs()` 来实现。(我们可能会想调用 `std::norm()`, ，但是在一个历史的怪圈中，C++委员会决定 `std::norm()` 应该返回绝对值的<i>square</i>。
* 
...
* 
* 最后这里是这个程序的主类。 它的成员函数与前面的例子非常相似，特别是  step-4  ，成员变量的列表也没有任何重大的惊喜。传递给构造函数的ParameterHandler对象被存储为一个引用，以便于从该类的所有函数中访问参数。 由于我们正在处理矢量值的有限元，我们使用的FE对象是FESystem类型的。
* 
...
* 
 @endcode
* 
* 构造函数接收ParameterHandler对象并将其存储在一个引用中。它还初始化了DoF-Handler和有限元系统，该系统由两份标量Q1场组成，一份用于 $v$ ，一份用于 $w$  。
* 
...
* @code
 ...
 @endcode
* 
* 首先，我们生成一些日志输出，并启动一个定时器，这样我们就可以计算出这个函数完成后的执行时间。
...
* 
 @endcode
* 
* 接下来，为换能器镜头的位置和焦点定义了两个点，即圆的中心，圆的部分将形成边界的换能器部分。注意，这是程序中唯一一个在二维和三维中略有不同的地方。尽管本教程只涉及二维情况，但要使这个程序在三维中发挥作用，必要的补充是非常少的，所以我们选择包括它们。
* 
...
* 作为初始粗网格，我们采用一个简单的单位方格，每个方向上有5个细分。分区的数量是这样选择的：我们想指定为换能器边界的线段 $[0.4,0.6]$ 是由一个面来跨越的。然后，我们通过所有的单元格，找到换能器所在的面，事实上，这只是X轴上从0.4到0.6的一条边。这是我们希望根据圆环形边界进行细化的地方，所以我们用不同的流形指标来标记这个边缘。由于我们将在换能器上设置迪里希特边界条件，我们也要改变其边界指标。
* 
...
- transducer).norm_square() < 0.01))
 ...
 @endcode
* 
* 对于换能器透镜的圆形部分，使用一个SphericalManifold对象（当然，在二维中只是代表一个圆），其中心计算方法如上。
* 
...
* 
 @endcode
* 
* 现在全局细化被执行。换能器位置附近的单元将根据换能器透镜的圆弧形边界自动细化。
* 
...
* 
 @endcode
* 
* 首先我们从ParameterHandler对象中查询波速和频率，并将它们存储在本地变量中，因为它们将在本函数中频繁使用。
* 
...
* 
 @endcode
* 
* 像往常一样，计算积分时使用普通高斯正交规则。由于我们的双线性形式涉及到 $\Gamma_2$ 上的边界积分，我们还需要一个正交法则来计算面的积分，这些面是 $dim-1$ 维的。
* 
...
* 
 @endcode
* 
* FEValues对象将为我们评估形状函数。 对于涉及到 $\Omega$ 上的积分的双线性形式的部分，我们需要形状函数的值和梯度，当然还有正交加权。 对于涉及边界积分的条款，只需要形状函数值和正交权重。
* 
...
* 
 ...
 @endcode
* 
* 在每个单元上，我们首先需要重置本地贡献矩阵，并请求FEValues对象来计算当前单元的形状函数。
* 
...
* 
 ...
 @endcode
* 
* 在这一点上，重要的是要记住，我们所处理的是一个有两个组成部分的有限元系统。由于我们构造这个FESystem的方式，即作为两个标量有限元场的笛卡尔乘积，每个形状函数只有一个非零分量（用deal.II的行话说，它们是 @ref  GlossPrimitive "原始"）。 因此，每个形状函数可以被看作是介绍中的 $\phi$  's或 $\psi$  's之一，同样，相应的自由度也可以归属于 $\alpha$  或 $\beta$  。然而，当我们遍历当前单元上的所有自由度时，它们并不以任何特定的顺序出现，因此我们无法立即决定索引为 $i$ 和 $j$ 的自由度是属于我们解决方案的实部还是虚部。 另一方面，如果你看一下介绍中的系统矩阵的形式，这个区别是至关重要的，因为它将决定当前一对DoF的贡献将归入系统矩阵的哪个块，因此我们需要从给定的两个形状函数中计算哪个量。 幸运的是，FESystem对象可以为我们提供这些信息，即它有一个函数 FESystem::system_to_component_index, ，为每个局部的DoF索引返回一对整数，其中第一个整数表示该DoF属于系统的哪个组成部分。这对整数中的第二个整数表示该DoF在标量基有限元域中的索引，但这一信息在这里并不相关。如果你想知道更多关于这个函数和原始向量值元素背后的基本方案，可以看看 step-8 或 @ref vector_valued 模块，那里对这些主题有深入的解释。
* 
...
* 如果DoF $i$ 和 $j$ 都属于同一个组件，即它们的形状函数都是 $\phi$ 的或都是 $\psi$ 的，贡献将最终出现在我们系统矩阵的一个对角块中，由于相应的条目是由同一公式计算的，我们不必理会它们是否真的是 $\phi$ 或 $\psi$ 的形状函数。我们可以简单地通过遍历所有正交点并将其贡献相加来计算条目，其中形状函数的值和梯度由我们的FEValues对象提供。
* 
...
* 我们还必须加上边界项的贡献。为此，我们对当前单元格的所有面进行循环，首先看它是否在边界上，其次看它是否有与 $\Gamma_2$ 相关的正确边界指标，即我们有吸收边界条件的那部分边界。
* 
...
* @code
 ...
 @endcode
* 
* 检查形状函数是否在一个面上有支持并不是严格必要的：如果我们不检查它，我们就会简单地在本地单元格矩阵中增加一些刚好为零的项，因为至少有一个形状函数刚好为零。然而，我们可以通过添加上面的检查来节省这项工作。
* 

* 
* 在任何一种情况下，这些DoFs都会对系统矩阵的非对角线块的边界积分做出贡献。为了计算积分，我们在面的所有正交点上进行循环，并将贡献量与面正交规则提供的正交权重相加。 与对角线块上的条目不同，这里的形状函数哪一个是 $\psi$ ，哪一个是 $\phi$ ，确实很重要，因为这将决定该条目的符号。 我们通过一个简单的条件语句来说明这一点，以确定正确的符号。由于我们已经检查了DoF $i$ 和 $j$ 属于不同的组件，这里只需测试其中一个属于哪个组件即可。
* 
...
* 
 @endcode
* 
* 现在我们已经完成了这个单元，必须把它的贡献从本地系统矩阵转移到全球系统矩阵。为此，我们首先得到这个单元斗室的全局指数列表......
* 
...
* 
 ...
 @endcode
* 
* 唯一剩下的是 $\Gamma_1$ 上的迪里希特边界值，其特征是边界指标1。迪里希特值是由我们上面定义的 <code>DirichletBoundaryValues</code> 类提供的。
* 
...
* 
* 正如在介绍中已经提到的，系统矩阵既不是对称的也不是确定的，因此，如何提出一个迭代求解器和预处理器来很好地处理这个矩阵并不明显。 我们选择了另一种方式，用UMFPACK提供的稀疏LU分解来解决线性系统。这通常是二维问题的一个很好的首选，即使对于大量的DoF也能很好地工作。 SparseDirectUMFPACK类提供了UMFPACK的deal.II接口，它非常容易使用，使我们只需3行代码就能解决我们的线性系统。
* 

* 
* 请注意，为了编译这个例子程序，你需要有支持UMFPACK的deal.II库。
* 
...
* 
 @endcode
* 
* 解决线性系统的代码很短：首先，我们分配一个正确类型的对象。下面的 <code>initialize</code> 调用提供了我们想要反转的矩阵给SparseDirectUMFPACK对象，并同时启动了LU分解的过程。因此，这也是这个程序中大部分计算工作发生的地方。
* 
...
* 
 @endcode
* 
* 分解之后，我们可以把 <code>A_direct</code> 当作代表我们系统矩阵的逆矩阵来使用，所以要计算出解决方案，我们只需要与右边的向量相乘即可。
* 
...
* 
 @endcode
* 
* 定义我们的 <code>ComputeIntensity</code> 类的对象和一个DataOut对象。
* 
...
* 
 @endcode
* 
* 接下来我们从ParameterHandler查询输出相关的参数。 DataOut::parse_parameters  调用充当了  <code>ParameterReader::declare_parameters</code>  中  DataOutInterface<1>::declare_parameters  调用的对应方。它从ParameterHandler收集所有与输出格式相关的参数，并相应地设置DataOut对象的相应属性。
* 
...
* 现在我们从ParameterHandler提供的基本名称和DataOut类提供的后缀组合成文件名（默认后缀被设置为正确的类型，与通过parse_parameters()设置在.prm文件中的类型相匹配）。
* 
...
* 
 @endcode
* 
* 解决方案向量 $v$ 和 $w$ 以常规方式添加到DataOut对象中。
* 
...
* 
 @endcode
* 
* <a name="codeUltrasoundProblemruncode"></a> <h4><code>UltrasoundProblem::run</code></h4> <h4><code>UltrasoundProblem::run</code></h4>。
...
* 
* 最后是该程序的 <code>main</code> 功能。它的结构与其他几乎所有的教程程序相同。唯一的例外是，我们定义了ParameterHandler和 <code>ParameterReader</code> 对象，并让后者从一个叫做 <code>step-29.prm</code> 的文本文件中读入参数值。读取的值将被交给UltrasoundProblem类的一个实例。
* 
...
@endcode
* 
* 可以看出，我们设置了 $d=0.3$ ，这相当于换能器镜头的焦点在 $x=0.5$ ， $y=0.3$ 。粗略的网格被细化了5次，结果是160x160个单元，输出结果以vtu格式书写。参数读取器可以理解更多与输出生成有关的参数，但是我们在这里不需要这些参数，因此坚持使用它们的默认值。
* 下面是在调试模式下程序的控制台输出。
* @code
> make run
...
* 前两张图片显示了 $u$ 的实部和虚部，而最后一张显示了强度 $|u|$ 。我们可以清楚地看到，强度集中在透镜的焦点周围（0.5，0.3），焦点在 $x$ 方向相当尖锐，但在 $y$ 方向却比较模糊，这是由聚焦透镜的几何形状、有限光圈和问题的波浪性质造成的。
* 因为丰富多彩的图形总是很有趣，而且为了进一步强调聚焦效果，这里还有一组图像，强调强度在 $x$ -方向上的实际聚焦效果。
*  <table align="center" class="doxtable">
  <tr>
    <td>
...
* 作为最后的说明，程序的结构使我们很容易确定程序的哪些部分可以随着网格的细化而很好地扩展，哪些部分不可以。下面是5、6、7次全局细化的运行时间。
* @code
> make run
[ 66%] Built target step-29
[100%] Run step-29 with Release configuration
...
@endcode
* 
* 每次我们对网格进行一次细化，所以每一步的单元和自由度的数量大约是四倍。可以看出，生成网格、设置自由度、组装线性系统和生成输出的比例相当接近于线性，而求解线性系统的操作，自由度的数量每增加4倍，就需要增加8倍的时间，也就是说，是 ${\cal O}(N^{3/2})$  。这可以解释为（使用最优排序）有限元矩阵的带宽是 $B={\cal O}(N^{(dim-1)/dim})$ ，而使用LU分解解决带状线性系统的努力是 ${\cal O}(BN)$ 。这也解释了为什么程序在三维中也能运行（在改变了 <code>UltrasoundProblem</code> 对象的维度后），但扩展性很差，在完成对具有可观分辨率的网格上的线性系统的求解前需要极大的耐心，尽管程序的其他部分扩展性很好。
* 
...
* 这个程序的一个明显的可能的扩展是在三维空间运行；毕竟，我们周围的世界是三维的，超声束在三维介质中传播。你可以通过简单地改变 <code>main()</code> 中主类的模板参数并运行它来尝试。但这不会让你走得很远：如果你按照参数文件中的设置做5个全局细化步骤，当然不会。你的内存会耗尽，因为网格（包括 $(2^5)^3 \cdot 5^3=2^{15}\cdot 125 \approx 4\cdot 10^6$ 单元），特别是稀疏直接求解器会占用太多的内存。然而，如果你有一点时间，你可以用3个全局细化步骤进行求解：在2011年初，直接求解大约需要半个小时。然而，你会注意到，这个解法是完全错误的：网格大小根本不足以准确地解决解法的波浪，你可以从解法的图中看到这一点。因此，在这种情况下，如果你不想在这个问题上扔一个更大的（可能是%并行的）机器，那么自适应性就是必不可少的。
* 
