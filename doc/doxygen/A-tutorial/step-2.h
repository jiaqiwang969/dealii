 ...
 * 在前面的例子中，我们已经创建了一个网格，现在我们展示如何在这个网格上定义自由度。在这个例子中，我们将使用最低阶的(
 * $Q_1$
 ...
 * x)$ 的集合。当然，空间 $V_h$
 * 有很多基数，但我们将特别选择由有限元函数描述的基数，这些函数传统上是在网格单元上局部定义的。在这种情况下描述
 ...
 *
 * 下一步将是使用这个有限元和网格计算对应于一个特定微分方程的矩阵和右手边。我们将在
 * step-3
 * 程序中保留这一步骤，而是讨论有限元程序的一个实际问题，即有限元矩阵总是非常稀疏的：矩阵中几乎所有的条目都是零。
 ...
 * 次操作。显然，如果矩阵不是稀疏的，这是不可能的（因为那时矩阵中的条目数必须是
 * ${\cal O}(N^s)$ 和一些 $s>1$
 ...
 * "支持
 * "是指它非零的区域。对于有限元方法，形状函数的支持通常是与它所定义的顶点、边或面相邻的单元。)
 * 换句话说，不在同一单元上定义的自由度 $i$ 和 $j$
 * 不会重叠，因此，矩阵条目 $A_{ij}$ 将为零。
 ...
 * 对于大多数应用和算法来说，自由度的确切编号方式并不重要。例如，我们用来解决线性系统的共轭梯度方法并不关心。另一方面，有些算法确实关心：特别是一些预处理程序，如SSOR，如果它们能以特定的顺序走过自由度，就能更好地工作，如果我们能将它们分类，使SSOR能以这种顺序从0到
 * $N$
 * 进行迭代，那就太好了。其他的例子包括计算不完整的LU或Cholesky因式分解，或者如果我们关心矩阵的块结构（见
 * step-20
 * 的例子）。因此，deal.II在命名空间DoFRenumbering中有能以特定方式重新列举自由度的算法。重新编号可以被认为是选择了一个不同的、经过排列的有限元空间的基础。因此，这种重新编号所产生的稀疏模式和矩阵也是简单的行和列的排列组合，与我们没有明确的重新编号所得到的相比。
 ...
 * 没有简单的方法可以直接将与每个顶点相关的自由度数字可视化。然而，这样的信息几乎不会真正重要，因为编号本身或多或少是任意的。有一些更重要的因素，我们将在下文中展示其中的一个。
 ...
 * 首先，我们必须创建一个结构，用来存储非零元素的位置。然后，这可以被一个或多个稀疏矩阵对象使用，这些对象在这个稀疏模式所存储的位置上存储条目的值。存储这些位置的类是SparsityPattern类。然而，事实证明，当我们试图立即填充这个类时，这个类有一些缺点：它的数据结构的设置方式是，我们需要对我们可能希望的每一行的最大条目数有一个估计。在两个空间维度上，通过 DoFHandler::max_couplings_between_dofs()
 * 函数可以得到合理的估计值，但是在三个维度上，该函数几乎总是严重高估真实的数字，导致大量的内存浪费，有时对于使用的机器来说太多，即使未使用的内存可以在计算稀疏模式后立即释放。为了避免这种情况，我们使用了一个中间对象DynamicSparsityPattern，该对象使用了一个不同的%内部数据结构，我们可以随后将其复制到SparsityPattern对象中，而不需要太多的开销。关于这些数据结构的一些更多信息可以在
 * @ref Sparsity
 ...
 *
 * 请记住，为了使矩阵中的一个条目 $(i,j)$
 * 不为零，形状函数i和j的支持需要相交（否则在积分中，积分将到处为零，因为在某个点上，一个或另一个形状函数为零）。然而，形状函数的支撑点只有在彼此相邻的情况下才会相交，因此，为了使非零条目聚集在对角线周围（其中
 * $i$ 等于 $j$
 ...
 * 类也提供了一些其他的算法来重新编号自由度。例如，如果所有的耦合都在矩阵的下三角或上三角部分，那当然是最理想的，因为那样的话，解决线性系统就只需要向前或向后替换。当然，这对于对称稀疏模式来说是无法实现的，但在一些涉及传输方程的特殊情况下，通过列举从流入边界沿流线到流出边界的自由度，这是可能的。毫不奇怪，
 ...
 *
 * 最后，这是主程序。它所做的唯一事情就是分配和创建三角形，然后创建一个
 * <code>DoFHandler</code>
 ...
 *
 * 该程序运行后，产生了两个疏散模式。我们可以通过在网络浏览器中打开
 * <code>.svg</code> 文件来可视化它们。
 * 结果是这样的（每一个点表示一个可能为非零的条目；当然，该条目是否为零取决于所考虑的方程，但矩阵中的指示位置告诉我们，在离散化局部方程，即微分方程时，哪些形状函数可以，哪些不可以耦合）。
 ...
 * 从右图中可以看出，经过编号后，矩阵的主对角线周围的稀疏度模式更加集中。虽然这一点可能不明显，但两张图片中的非零项的数量是相同的，当然。
 *
 *
 ...
 *
 *  就像 step-1
 * 一样，你可能想用程序玩一下，让自己熟悉一下交易.II。例如，在
 * <code>distribute_dofs</code>
 * 函数中，我们使用线性有限元（这就是FE_Q对象的参数
 * "1"）。如果你使用高阶元素，例如立方体或五元体（使用3和5作为各自的参数），探索稀疏模式如何变化。
