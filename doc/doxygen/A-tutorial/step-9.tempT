...
* 
* 在这个例子中，我们的目标是如下。 <ol>   <li>  解决平流方程  $\beta \cdot \nabla u = f$  ；  <li>  展示如果我们有一台多处理器机器，我们如何使用多线程来快速获得结果；  <li>  开发一个简单的细化标准。 </ol> 虽然第二个目标很难在不参考代码的情况下进行一般性描述，但我们将在下文中讨论其他两个目标。多线程的使用将在程序中的相关地方进行详细说明。然而，我们将遵循 @ref threads "多处理器访问共享内存的并行计算 "文档模块中详述的关于WorkStream方法的一般讨论。
* 
...
* 在本例程序中，我们要对平流方程@f[
  \beta \cdot \nabla u = f,
@f]的解进行数值近似，其中 $\beta$ 是描述平流方向和速度的矢量场（如果 $\beta=\beta(\mathbf x)$ ，可能取决于空间变量）， $f$ 是一个源函数， $u$ 是解。该方程描述的物理过程是一个给定的流场 $\beta$ ，另一种物质随着流场被输送，其密度或浓度由 $u$ 给出。该方程不包含这第二种物质在其载体物质中的扩散，但有源定律。
...
@f]和基函数 $\varphi_i,\varphi_j$ 。 然而，这是每一个数值分析师至少会遇到一次的问题（包括作者）：我们在这里扩展了解决方案 $u_h = \sum_i U_i \varphi_i$ ，但如果我们这样做，我们将不得不解决@f[
  U^T A = F^T,
...
* 看看上面提到的双线性形式，我们看到离散的解决方案必须满足一个方程，其中弱形式的左手边有一个域项@f[
  (v_h + \delta \beta\cdot\nabla v_h, \beta \cdot \nabla u_h)_\Omega,
@f]，如果我们把它拆开，则是@f[
  (v_h, \beta \cdot \nabla u_h)_\Omega
  +
  (\delta \beta\cdot\nabla v_h, \beta \cdot \nabla u_h)_\Omega.
@f]的形式，如果我们想看看这个方程的强形式，我们需要整合第二个项。这就产生了下面的公式，为了简单起见，我们暂时忽略边界项：@f[
  (v_h, \beta \cdot \nabla u_h)_\Omega
* 
...
* "Galerkin方法 "是指用测试函数 $v$ 乘以方程（然后对 $\Omega$ 进行积分）来获得弱表述，其中函数 $v$ 与解决方案 $u$ 来自同一空间（尽管可能具有不同的边界值）。但这并不是严格必要的。我们也可以想象从不同的函数集中选择测试函数，只要这个不同的函数集具有与原始函数集 "同样多的维度"，这样我们最终就能得到与自由度一样多的独立方程（在无限维的情况下，所有这些都需要适当的定义）。利用这种可能性的方法（即以不同的方式选择测试函数集和解集）被称为 "Petrov-Galerkin "方法。在目前的情况下，测试函数的形式都是 $v+\beta\cdot\nabla v$ ，其中 $v$ 是来自解集。
* 

* <a name="Whyisthismethodalsocalledstreamlineupwind"></a><h3>Why is this method also called "streamline-upwind"?</h3> 。
* 

* [上风法](https://en.wikipedia.org/wiki/Upwind_scheme)在推导平流方程的稳定方案方面有一定的历史。一般来说，这个想法是，我们不是在 "这里 "看一个函数，而是在 "上游 "或 "上风 "的一小段距离上看它，也就是说，"这里 "的信息最初来自哪里。这可能建议不要考虑 $u(\mathbf x)$ ，而是考虑 $u(\mathbf x
* 
...
@f} 。
* 对于 $d>2$ ，我们对 $\beta$ 和 ${\mathbf x}_0$ 进行扩展，只需将上面所示的最后一个组件再重复一次。
...
* 在之前所有自适应细化的例子中，我们使用了Kelly等人首先开发的误差估计器，该估计器为每个单元 $K$ 分配了以下指标：@f[
  \eta_K =
  \left(
    \frac {h_K}{24}
    \int_{\partial K}
  ...
  \right)^{1/2},
@f]其中 $[\partial n u_h]$ 表示跨越单元 $K$ 的一个面 $\gamma\subset\partial K$ 的法线导数的跳跃。可以看出，这个误差指标使用的是次导数的离散类似物，由单元大小的一个幂加权，这个幂是根据这里假定使用的线性元素调整的：@f[
  \eta_K \approx
  C h \| \nabla^2 u \|_K,
@f]它本身与能量准则中的误差大小有关。
...
* 
- u(K)}{|{\mathbf y}_{KK'}|},
@f]其中 $u(K)$ 和 $u(K')$ 表示在各自单元中心评价的 $u$ 。现在我们将上述近似值乘以 ${\mathbf y}_{KK'}/|{\mathbf y}_{KK'}|$ ，并对 $K$ 的所有邻居 $K'$ 求和：@f[
  \underbrace{
...
* 
- u(K)}{|{\mathbf y}_{KK'}|}.
@f]如果连接 ${\mathbf y}_{KK'}$ 和其邻居的向量 $K$ 横跨整个空间（即大致为： $K$  ] 在所有方向上都有邻居），那么左侧表达式中括号内的项就形成了一个正方形矩阵，我们可以通过反转来获得 $u$  ]在 $K$ 上的近似值：@f[
  \nabla u
  \approx
  Y^{-1}
  \left(
...
* 
 @endcode
* 
* 以下两个文件为多线程程序提供类和信息。在第一个中，声明了我们需要做并行装配的类和函数（即 <code>WorkStream</code> 命名空间）。第二个文件有一个类MultithreadInfo，可以用来查询系统中的处理器数量，这在决定启动多少个并行线程时通常很有用。
* 
...
* @code
 ...
 @endcode
* 
* 语法可能看起来有点奇怪，但很合理。其格式基本如下：使用其中一个宏的名称  <code>DeclExceptionN</code>, where <code>N</code>  表示异常对象应接受的附加参数的数量。在本例中，由于我们想在两个向量的大小不同时抛出异常，我们需要两个参数，所以我们使用  <code>DeclException2</code>  。第一个参数描述了异常的名称，而后面的参数则声明了参数的数据类型。最后一个参数是一连串的输出指令，这些指令将被输送到  <code>std::cerr</code>  对象中，因此出现了奇怪的格式，前面有  <code>@<@<</code>  操作符之类的。注意，我们可以通过使用名称  <code>arg1</code> through <code>argN</code>  来访问在构造时（即在  <code>Assert</code>  调用中）传递给异常的参数，其中  <code>N</code>  是通过使用各自的宏  <code>DeclExceptionN</code>  来定义的参数数量。    
...
* 
 @endcode
* 
* 以下两个函数实现了上述的接口。第一个函数简单地实现了介绍中所描述的函数，而第二个函数使用了同样的技巧来避免调用虚函数，在前面的例子程序中已经介绍过了。注意第二个函数中对参数的正确大小的检查，这种检查应该始终存在于这类函数中；根据我们的经验，许多甚至大多数编程错误都是由不正确的初始化数组、不兼容的函数参数等造成的；像本例中那样使用断言可以消除许多这样的问题。
* 
...
* 
* 下面是这个程序的主类。它和以前的例子中的主类非常相似，所以我们再次只评论其中的区别。
* 
...
* 
 @endcode
* 
* 下一组函数将被用来组装矩阵。然而，与前面的例子不同， <code>assemble_system()</code> 函数不会自己做这些工作，而是将实际的装配工作委托给辅助函数  <code>assemble_local_system()</code>  和  <code>copy_local_to_global()</code>  。其原理是，矩阵组装可以很好地并行化，因为每个单元上的局部贡献的计算完全独立于其他单元，我们只需要在将一个单元的贡献添加到全局矩阵中时进行同步。    
* 我们在这里选择的并行化策略是文档中 @ref threads 模块中详细提到的可能性之一。具体来说，我们将使用那里讨论的WorkStream方法。由于这个模块有很多文档，我们不会在这里重复设计选择的理由（例如，如果你读完上面提到的模块，你会明白 <code>AssemblyScratchData</code> 和 <code>AssemblyCopyData</code> 结构的目的是什么）。相反，我们只讨论具体的实现。    
* 如果你阅读了上面提到的页面，你会发现为了使汇编并行化，我们需要两个数据结构
* 
* - 一个对应于我们在局部集成过程中需要的数据（"scratch data"，即我们只需要作为临时存储的东西），另一个是将局部集成的信息传递给函数，然后将局部贡献添加到全局矩阵的相应元素。其中前者通常包含FEValues和FEFaceValues对象，而后者则有局部矩阵、局部右手边，以及关于哪些自由度生活在我们正在组装局部贡献的单元上的信息。有了这些信息，以下内容应该是相对不言自明的。
* 
...
* 
 @endcode
* 
* FEValues和FEFaceValues是很昂贵的设置对象，所以我们把它们包含在从头开始的对象中，以便尽可能多的数据在单元格之间被重复使用。
* 
...
* 还有两点意见：首先，这个类没有非静态成员函数或变量，所以这不是一个真正的类，而是起到了C++中 <code>namespace</code> 的作用。我们选择类而不是命名空间的原因是，这样我们可以声明一些私有的函数。如果在命名空间的头文件中声明一些函数，并在实现文件中实现这些函数和其他函数，这也可以用命名空间来实现。没有在头文件中声明的函数仍然在名字空间中，但不能从外部调用。然而，由于我们这里只有一个文件，所以在目前的情况下不可能隐藏函数。  
* 第二个意见是，维度模板参数是附在函数上的，而不是附在类本身。这样，你就不必像其他大多数情况下那样自己指定模板参数，而是编译器可以从作为第一个参数传递的DoFHandler对象的尺寸中自行计算出它的值。  
...
* 
 @endcode
* 
* 在下面的函数中，矩阵和右手边被组装起来。正如上面主类的文档所述，它本身并不做这个，而是委托给下面的函数，利用 @ref threads 中讨论的WorkStream概念。  
...
* 现在，这是一个做实际工作的函数。它与以前的例子程序中的 <code>assemble_system</code> 函数没有什么不同，所以我们将再次只评论其中的差异。数学上的东西紧跟我们在介绍中所说的。  
...
* @code
 ...
 @endcode
* 
* 好的，本单元的这个面是在域的边界上。就像我们在以前的例子中和上面使用的通常的FEValues对象一样，我们必须为现在的面重新初始化FEFaceValues对象。
* 
...
* @code
 ...
 @endcode
* 
* 如果这个面是流入边界的一部分，那么使用从FEFaceValues对象中获得的值和介绍中讨论的公式，计算这个面对全局矩阵和右侧的贡献。
* 
...
* 
 @endcode
* 
* 这里是线性求解的程序。由于系统不再像以前的例子那样是对称正定的，我们不能再使用共轭梯度法。相反，我们使用了一个更通用的解算器，不依赖于矩阵的任何特殊属性：GMRES方法。GMRES和共轭梯度法一样，需要一个合适的预处理程序：我们在这里使用雅可比预处理程序，它对这个问题足够好。
* 
...
* 下面的函数根据介绍中描述的数量细化网格。各自的计算是在  <code>GradientEstimation</code>  类中进行的。
* 
...
* 
 @endcode
* 
* 这个函数与第6步中的函数类似，但由于我们使用的是高阶有限元，所以我们以不同的方式保存解决方案。像VisIt和Paraview这样的可视化程序通常只能理解与节点相关的数据：它们不能绘制五度基函数，这导致我们计算的解的图片非常不准确。为了解决这个问题，我们为每个单元保存了多个 <em> 补丁 </em> ：在二维中，我们为每个单元在VTU文件中保存64个双线性 "单元"，在三维中，我们保存512个。最终的结果是，可视化程序将使用立方体基础函数的片状线性插值：这捕捉到了解决方案的细节，并且在大多数屏幕分辨率下，看起来很平滑。我们在一个单独的步骤中保存网格，没有额外的补丁，这样我们就有了细胞面的可视化表示。  
...
* 
* 现在是对 <code>GradientEstimation</code> 类的实现。让我们先为 <code>EstimateScratchData</code> 类定义构造函数，供 <code>estimate_cell()</code> 函数使用。
* 
...
* @code
 ...
 @endcode
* 
* 我们分配一个向量来保存一个单元的所有活动邻居的迭代器。我们保留活动邻居的最大数量，以避免以后的重新分配。请注意这个活动邻居的最大数量是如何计算出来的。
* 
...
* 接下来是对 <code>GradientEstimation</code> 类的实现。第一个函数除了将工作委托给另一个函数外，并没有做太多的工作，但在顶部有一点设置。  
...
* 这里有一个函数，通过计算梯度的有限差分近似值来估计局部误差。该函数首先计算当前单元的活动邻居列表，然后为每个邻居计算介绍中描述的数量。之所以有这样的顺序，是因为在局部细化网格的情况下，要找到一个给定的邻居并不是一蹴而就的事情。原则上，一个优化的实现可以在一个步骤中找到邻域和取决于它们的量，而不是先建立一个邻域列表，然后在第二步中找到它们的贡献，但是我们很乐意将此作为一个练习。正如之前所讨论的，传递给 WorkStream::run 的工作函数在 "scratch "对象上工作，该对象保留了所有的临时对象。这样，我们就不需要在每次为给定单元调用工作的函数内创建和初始化那些昂贵的对象了。这样的参数被作为第二个参数传递。第三个参数将是一个 "copy-data "对象（更多信息见 @ref threads ），但我们在这里实际上没有使用这些对象。由于 WorkStream::run() 坚持传递三个参数，我们声明这个函数有三个参数，但简单地忽略了最后一个参数。  
...
@code
 ...
 @endcode
* 
 </div> 来做到这一点，这并不比下面实现的解决方案好多少：要么例程本身必须带三个参数，要么它必须被带三个参数的东西包裹起来。我们不使用这种方法，因为在开始时添加未使用的参数更简单。  
* 现在说说细节。
* 
...
* 
 @endcode
* 
* 现在，在我们继续之前，我们首先计算一个当前单元的所有活动邻居的列表。我们首先在所有面上进行循环，看看那里的邻居是否是活跃的，如果它与本单元在同一层次或更粗一级，就会出现这种情况（注意，一个邻居只能比本单元粗一次，因为在deal.II中我们只允许在一个面上有一个最大的细化差）。另外，邻居也可能在同一层次上，并被进一步细化；然后我们必须找到它的哪些子单元紧挨着现在的单元，并选择这些子单元（注意，如果一个活动单元的邻居的子单元紧挨着这个活动单元，由于上面引用的一次细化规则，它本身就必须是活动的）。    
...
* 
 @endcode
* 
* 然后检查邻居是否是活动的。如果是，那么它就在同一层或更粗的一层（如果我们不是在1D中），而且我们在任何情况下都对它感兴趣。
* 
...
* @code
 ...
 @endcode
* 
* 要找到与本单元相邻的子单元，如果我们在本单元的左边（n==0），就依次去找它的右边子单元，如果我们在右边（n==1），就去找左边子单元，直到找到一个活动单元。
* 
...
* 由于这使用了一些非微妙的几何直觉，我们可能想检查一下我们是否做对了，也就是说，检查我们找到的单元格的邻居是否确实是我们目前正在处理的单元。像这样的检查通常是有用的，并且经常发现像上面这一行的算法（不由自主地交换 <code>n==1</code> for <code>n==0</code> 或类似的算法是很简单的）和库中的错误（上面的算法所依据的假设可能是错误的，记录错误，或者由于库中的错误而被违反）。原则上，我们可以在程序运行一段时间后删除这样的检查，但是无论如何，留下它来检查库中或上述算法中的变化可能是一件好事。                  
...
* 
 @endcode
* 
* 好的，现在我们有了所有的邻居，让我们开始对它们进行计算。首先，我们做一些预备工作：找出当前单元格的中心和这一点上的解。后者是以正交点的函数值向量的形式得到的，当然，正交点只有一个。同样地，中心的位置是实空间中第一个（也是唯一的）正交点的位置。
* 
...
* @code
 ...
 @endcode
* 
* 然后得到邻接单元的中心和该点的有限元函数的值。注意，为了这个信息我们必须重新初始化相邻单元的 <code>FEValues</code> 对象。
* 
...
* 
 @endcode
* 
* 计算连接两个单元中心的向量 <code>y</code> 。注意，与介绍不同，我们用 <code>y</code> 表示归一化的差分向量，因为这是计算中到处使用的量。
* 
...
* 
 @endcode
* 
* 如果现在，在收集了邻居的所有信息后，我们可以确定当前单元的梯度近似值，那么我们需要经过跨越整个空间的向量 <code>y</code> ，否则我们就没有梯度的所有成分。这是由矩阵的可逆性表示的。    
...
* 
 @endcode
* 
* 这个函数的最后一部分是将我们刚刚计算出来的内容写入输出向量的元素中。这个向量的地址已经存储在Scratch数据对象中，我们所要做的就是知道如何在这个向量中获得正确的元素
* 
...
* 
*  <code>main</code> 函数与前面的例子类似。主要的区别是我们使用MultithreadInfo来设置最大的线程数（更多信息见文档模块 @ref threads  "多处理器访问共享内存的并行计算"）。使用的线程数是环境变量DEAL_II_NUM_THREADS和  <code>set_thread_limit</code>  的参数中的最小值。如果没有给  <code>set_thread_limit</code>  的值，则使用英特尔线程构建块（TBB）库的默认值。如果省略了对  <code>set_thread_limit</code>  的调用，线程的数量将由 TBB 选择，与 DEAL_II_NUM_THREADS无关。
* 
...
@endcode
* 
* 相当多的单元格被用在最精细的层面上，以解决解决方案的特点。下面是第四和第十个网格。 <div class="twocolumn" style="width: 80%">
  <div>
