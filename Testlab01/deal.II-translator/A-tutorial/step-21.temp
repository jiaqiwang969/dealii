...
* In this project, we propose a numerical simulation for two phaseflow problems in porous media. This problem includes oneelliptic equation and one nonlinear, time dependent transportequation. This is therefore also the first time-dependent tutorialprogram (besides the somewhat strange time-dependence of   @ref step_18  "  step-18  ").
* The equations covered here are an extension of the material already covered in  step-20  . In particular, they fall into the class ofvector-valued problems. A toplevel overview of this topic can be found in the  @ref vector_valued   module.
* 
...
* To derive the governing equations, consider two phase flow in areservoir   $\Omega$   under the assumption that the movement of fluids isdominated by viscous effects; i.e. we neglect the effects of gravity,compressibility, and capillary pressure. Porosity will be consideredto be constant. We will denote variables referring to either of the twophases using subscripts   $w$   and   $o$  , short for water and oil. Thederivation of the equations holds for other pairs of fluids as well,however.
* The velocity with which molecules of each of the two phases move isdetermined by Darcy's law that states that the velocity isproportional to the pressure gradient:@f{eqnarray*}
  \mathbf{u}_{j}
  =
* 
...
@f}
* where   $\mathbf{u}_{j}$   is the velocity of phase   $j=o,w$  ,   $K$   is thepermeability tensor,   $k_{rj}$   is the relative permeability of phase  $j$  ,   $p$   is thepressure and   $\mu_{j}$   is the viscosity of phase   $j$  . Finally,   $S$   isthe saturation (volume fraction), i.e. a function with values between0 and 1 indicating the composition of the mixture of fluids. Ingeneral, the coefficients   $K, k_{rj}, \mu$   may be spatially dependentvariables, and we will always treat them as non-constant functions inthe following.
* We combine Darcy's law with the statement of conservation of mass foreach phase,@f[
  \textrm{div}\ \mathbf{u}_{j} = q_j,
@f]with a source term for each phase. By summing over the two phases,we can express the governing equations in terms of theso-called pressure equation:@f{eqnarray*}
* 
- \nabla \cdot (\mathbf{K}\lambda(S) \nabla p)= q.
@f}
* Here,   $q$   is the sum source term, and@f[
  \lambda(S) = \frac{k_{rw}(S)}{\mu_{w}}+\frac{k_{ro}(S)}{\mu_{o}}
@f]is the total mobility.
* So far, this looks like an ordinary stationary, Poisson-like equation that wecan solve right away with the techniques of the first few tutorial programs(take a look at   step-6  , for example, for something verysimilar). However, we have not said anything yet about the saturation, whichof course is going to change as the fluids move around.
* The second part of the equations is the description of thedynamics of the saturation, i.e., how the relative concentration of thetwo fluids changes with time. The saturation equation for the displacingfluid (water) is given by the following conservation law:@f{eqnarray*}
  S_{t} + \nabla \cdot (F(S) \mathbf{u}) = q_{w},
@f}
* which can be rewritten by using the product rule of the divergence operatorin the previous equation:@f{eqnarray*}
  S_{t} + F(S) \left[\nabla \cdot \mathbf{u}\right]
        + \mathbf{u} \cdot \left[ \nabla F(S)\right]
  = S_{t} + F(S) q + \mathbf{u} \cdot \nabla F(S) = q_{w}.
@f}
* Here,   $q=\nabla\cdot \mathbf{u}$   is the total influx introducedabove, and   $q_{w}$   is the flow rate of the displacing fluid (water).These two are related to the fractional flow   $F(S)$   in the following way:@f[
  q_{w} = F(S) q,
@f]where the fractional flow is often parameterized via the (heuristic) expression@f[
  F(S)
  =
  \frac{k_{rw}(S)/\mu_{w}}{k_{rw}(S)/\mu_{w} + k_{ro}(S)/\mu_{o}}.
@f]Putting it all together yields the saturation equation in the following,advected form:@f{eqnarray*}
  S_{t} + \mathbf{u} \cdot \nabla F(S) = 0,
@f}
* where   $\mathbf u$   is the total velocity@f[
  \mathbf{u} =
  \mathbf{u}_{o} + \mathbf{u}_{w} =
* 
-\lambda(S) \mathbf{K}\cdot\nabla p.
@f]Note that the advection equation contains the term   $\mathbf{u} \cdot \nabla
...
= \mathbf{u} F'(S) \cdot \nabla S$   to observe that the <i>actual</i>velocity with which the phase with saturation   $S$   is transported is  $\mathbf u F'(S)$   whereas the other phase is transported at velocity  $\mathbf u (1-F'(S))$  .   $F(S)$   is consequently often referred to as the<i>fractional flow</i>.
* In summary, what we get are the following two equations:@f{eqnarray*}
* 
...
- \nabla \cdot (\mathbf{K}\lambda(S) \nabla p) &=& q
  ...
  \qquad \textrm{in}\ \Omega\times[0,T].
@f}
* Here,   $p=p(\mathbf x, t), S=S(\mathbf x, t)$   are now time dependentfunctions: while at every time instant the flow field is inequilibrium with the pressure (i.e. we neglect dynamicaccelerations), the saturation is transported along with the flow andtherefore changes over time, in turn affected the flow field againthrough the dependence of the first equation on   $S$  .
...
reservoirs</i>, Trans. SPE AIME, 222 (1961), pp. 92-104).In a slightly modified form, this algorithm can bewritten as follows: for each time step, solve@f{eqnarray*}
  \mathbf{u}^{n+1}+\mathbf{K}\lambda(S^n) \nabla p^{n+1}&=&0 \\
  \nabla \cdot\mathbf{u}^{n+1} &=& q^{n+1} \\
  \frac {S^{n+1}-S^n}{\triangle t} + \mathbf{u}^{n+1} \cdot \nabla F(S^n) &=& 0,
@f}
* where   $\triangle t$   is the length of a time step. Note how we solve theimplicit pressure-velocity system that only depends on the previously computedsaturation   $S^n$  , and then do an explicit time step for   $S^{n+1}$   that onlydepends on the previously known   $S^n$   and the just computed  $\mathbf{u}^{n+1}$  . This way, we never have to iterate for the nonlinearitiesof the system as we would have if we used a fully implicit method. (Ina more modern perspective, this should be seen as an "operatorsplitting" method.   step-58   has a long description of the idea behind this.)
* We can then state the problem in weak form as follows, by multiplying eachequation with test functions   $\mathbf v$  ,   $\phi$  , and   $\sigma$   and integratingterms by parts:@f{eqnarray*}
...
@f}
* Note that in the first term, we have to prescribe the pressure   $p^{n+1}$   onthe boundary   $\partial\Omega$   as boundary values for our problem.   $\mathbf n$  denotes the unit outward normal vector to   $\partial K$  , as usual.
* For the saturation equation, we obtain after integrating by parts@f{eqnarray*}
  (S^{n+1}, \sigma)_\Omega
* 
...
-
  ...
  (S^n,\sigma)_\Omega.
@f}
* Using the fact that   $\nabla \cdot \mathbf{u}^{n+1}=q^{n+1}$  , we can rewrite thecell term to get an equation as follows:@f{eqnarray*}
  (S^{n+1}, \sigma)_\Omega
* 
...
* In each time step, we then apply the mixed finite method of   @ref step_20  "  step-20  " to the velocity and pressure. To be well-posed, we chooseRaviart-Thomas spaces   $RT_{k}$   for   $\mathbf{u}$   and discontinuous elements ofclass   $DGQ_{k}$   for   $p$  . For the saturation, we will also choose   $DGQ_{k}$  spaces.
...
\end{array}
\right)
@f]where the individual matrices and vectors are defined as follows usingshape functions   $\mathbf v_i$   (of type Raviart Thomas   $RT_k$  ) forvelocities and   $\phi_i$   (of type   $DGQ_k$  ) for both pressures and saturations:@f{eqnarray*}
M^u(S^n)_{ij} &=&
...
@f}
* 
*   @note   Due to historical accidents, the role of matrices   $B$   and   $B^T$  has been reverted in this program compared to   step-20  . In other words,here   $B$   refers to the divergence and   $B^T$   to the gradient operatorswhen it was the other way around in   step-20  .
* The system above presents a complication: Since the matrix   $H_{ij}$  depends on   $\mathbf u^{n+1}$   implicitly (the velocities are needed todetermine which parts of the boundaries   $\partial K$   of cells areinflux or outflux parts), we can only assemble this matrix after wehave solved for the velocities.
* The solution scheme then involves the following steps:  <ol>      <li>  Solve for the pressure   $p^{n+1}$   using the Schur complement  technique introduced in   step-20  .
*     <li>  Solve for the velocity   $\mathbf u^{n+1}$   as also discussed in    step-20  .
*     <li>  Compute the term   $F_3-\triangle t\; H \mathbf u^{n+1}$  , using  the just computed velocities.
*     <li>  Solve for the saturation   $S^{n+1}$  .  </ol>  
* In this scheme, we never actually build the matrix   $H$  , but rathergenerate the right hand side of the third equation once we are readyto do so.
* In the program, we use a variable   <code>solution</code>   to store thesolution of the present time step. At the end of each step, we copyits content, i.e. all three of its block components, into the variable  <code>old_solution</code>   for use in the next time step.
* 

* <a name="Choosingatimestep"></a><h3>Choosing a time step</h3>
* 

* A general rule of thumb in hyperbolic transport equations like the equation wehave to solve for the saturation equation is that if we use an explicit timestepping scheme, then we should use a time step such that the distance that aparticle can travel within one time step is no larger than the diameter of asingle cell. In other words, here, we should choose@f[
  \triangle t_{n+1} \le \frac h{|\mathbf{u}^{n+1}(\mathbf{x})|}.
@f]Fortunately, we are in a position where we can do that: we only need thetime step when we want to assemble the right hand side of the saturationequation, which is after we have already solved for   $\mathbf{u}^{n+1}$  . All wetherefore have to do after solving for the velocity is to loop over allquadrature points in the domain and determine the maximal magnitude of thevelocity. We can then set the time step for the saturation equation to@f[
...
* Note that we will have similar restrictions on the time step also in  step-23   and   step-24   where we solve the time dependentwave equation, another hyperbolic problem. We will also come back to the issueof time step choice below in the section on <a href="#extensions">possible
...
* For simplicity, this program assumes that there is no source,   $q=0$  , and thatthe heterogeneous porous medium is isotropic   $\mathbf{K}(\mathbf{x}) =
...
@f]Since the pressure and velocity solve a mixed form Poisson equation, theimposed pressure leads to a resulting flow field for the velocity. On theother hand, this flow field determines whether a piece of the boundary is ofinflow or outflow type, which is of relevance because we have to imposeboundary conditions for the saturation on the inflow part of the boundary,@f[
  \Gamma_{in}(t) = \{\mathbf{x}\in\partial\Omega:
                     \mathbf{n} \cdot \mathbf{u}(\mathbf{x},t) < 0\}.
@f]On this inflow boundary, we impose the following saturation values:@f{eqnarray}
...
* 
 @endcode
* 
*  In this program, we use a tensor-valued coefficient. Since it may have a spatial dependence, we consider it a tensor-valued function. The following include file provides the   <code>TensorFunction</code>   class that offers such functionality:
* 
...
* 
 @endcode
* 
*  Additionally, we use the class   <code>DiscreteTime</code>   to perform operations related to time incrementation.
* 
...
*     <ul>     <li>    <code>assemble_rhs_S</code>   assembles the right hand side of the saturation equation. As explained in the introduction, this can't be integrated into   <code>assemble_rhs</code>   since it depends on the velocity that is computed in the first part of the time step.   
*     <li>    <code>get_maximal_velocity</code>   does as its name suggests. This function is used in the computation of the time step size.   
*     <li>    <code>project_back_saturation</code>   resets all saturation degrees of freedom with values less than zero to zero, and all those with saturations greater than one to one.    </ul>     
*   The rest of the class should be pretty much obvious. The   <code>viscosity</code>   variable stores the viscosity   $\mu$   that enters several of the formulas in the nonlinear equations. The variable   <code>time</code>   keeps track of the time information within the simulation.
* 
...
*   The next problem, however, is that we need a way to initialize this variable. Since this variable is initialized at the beginning of the program, we can't use a regular member function for that since there may not be an object of this type around at the time. The C++ standard therefore says that only non-member and static member functions can be used to initialize a static variable. We use the latter possibility by defining a function   <code>get_centers</code>   that computes the list of center points when called.   
...
...
@endcode
* As we can see, the time step is pretty much constant right from the start,which indicates that the velocities in the domain are not strongly dependenton changes in saturation, although they certainly are through the factor  $\lambda(S)$   in the pressure equation.
...
* With all this, here are a few movies that show how the saturation progressesover time. First, this is for the single crack model, as implemented in the  <code>SingleCurvingCrack::KInverse</code>   class:
*   <img src="https://www.dealii.org/images/steps/developer/step-21.centerline.gif" alt="">  
* As can be seen, the water rich fluid snakes its way mostly along thehigh-permeability zone in the middle of the domain, whereas the rest of thedomain is mostly impermeable. This and the next movie are generated using  <code>n_refinement_steps=7</code>  , leading to a   $128\times 128$   mesh with some16,000 cells and about 66,000 unknowns in total.
* 

* The second movie shows the saturation for the random medium model of class  <code>RandomMedium::KInverse</code>  , where we have randomly distributedcenters of high permeability and fluid hops from one of these zones tothe next:
*   <img src="https://www.dealii.org/images/steps/developer/step-21.random2d.gif" alt="">  
* 

* Finally, here is the same situation in three space dimensions, on a mesh with  <code>n_refinement_steps=5</code>  , which produces a mesh of some 32,000 cellsand 167,000 degrees of freedom:
...
* There are a number of areas where this program can be improved. Three of themare listed below. All of them are, in fact, addressed in a tutorial programthat forms the continuation of the current one:   step-43  .
* 

* <a name="Solvers"></a><h4>Solvers</h4>
* 

* At present, the program is not particularly fast: the 2d random mediumcomputation took about a day for the 1,000 or so time steps. The corresponding3d computation took almost two days for 800 time steps. The reason why itisn't faster than this is twofold. First, we rebuild the entire matrix inevery time step, although some parts such as the   $B$  ,   $B^T$  , and   $M^S$   blocksnever change.
* Second, we could do a lot better with the solver andpreconditioners. Presently, we solve the Schur complement   $B^TM^u(S)^{-1}B$  with a CG method, using   $[B^T (\textrm{diag}(M^u(S)))^{-1} B]^{-1}$   as apreconditioner. Applying this preconditioner is expensive, since it involvessolving a linear system each time. This may have been appropriate for   @ref
step_20   "  step-20  ", where we have to solve the entire problem onlyonce. However, here we have to solve it hundreds of times, and in such casesit is worth considering a preconditioner that is more expensive to set up thefirst time, but cheaper to apply later on.
...
\varphi_i,\nabla\varphi_j)$  . We could then form an incomplete or completedecomposition of this non-mixed matrix and use it as a preconditioner of themixed form.
...
* In the introduction we have identified the time step restriction@f[
  \triangle t_{n+1} \le \frac h{|\mathbf{u}^{n+1}(\mathbf{x})|}
@f]that has to hold globally, i.e. for all   $\mathbf x$  . After discretization, wesatisfy it by choosing@f[
  \triangle t_{n+1} = \frac {\min_K h_K}{\max_{\mathbf{x}}|\mathbf{u}^{n+1}(\mathbf{x})|}.
@f]
