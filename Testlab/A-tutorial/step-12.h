 ...
 * - v^-$
 * ，其中上标指的是面的左（'+'）和右（'-'）值。上风值
 * $u^{\text{upwind}}$ 被定义为 $u^+$ ，如果 $\beta \cdot n>0$
 * ，否则为 $u^-$ 。
 * 因此，依赖网格的弱形式为：@f[
 ...
 * @endcode
 *
 * 我们根据一个非常简单的细化标准来细化网格，即对解的梯度的近似。由于这里我们考虑的是DG(1)方法（即我们使用片状双线性形状函数），我们可以简单地计算每个单元的梯度。但是我们并不希望我们的细化指标只建立在每个单元的梯度上，而是希望同时建立在相邻单元之间的不连续解函数的跳跃上。最简单的方法是通过差分商计算近似梯度，包括考虑中的单元和其相邻的单元。这是由
 * <code>DerivativeApproximation</code>
 * 类完成的，它计算近似梯度的方式类似于本教程 step-9
 * 中描述的 <code>GradientEstimation</code> 。事实上，
 * <code>DerivativeApproximation</code> 类是在 step-9 的
 * <code>GradientEstimation</code> 类之后开发的。与  step-9
 * 中的讨论相关，这里我们考虑  $h^{1+d/2}|\nabla_h u_h|$
 * 。此外，我们注意到，我们不考虑近似的二次导数，因为线性平流方程的解一般不在
 * $H^2$ 中，而只在 $H^1$ 中（或者，更准确地说：在
 ...
 * 将元素改为FE_Q而不是FE_DGQ。
 *
 * - 以与 step-6 完全相同的方式增加对悬挂节点约束的处理。
 *
 * - 我们需要一个不同的求解器； step-29 中的直接求解器是一个方便的选择。一个有经验的deal.II用户将能够在10分钟内完成这个工作。
 * 虽然上面显示了2d解，在界面上包含一些小尖峰，但是在网格细化下高度稳定，当使用连续元素时，结果看起来大不相同。
 * <table align="center"> <tr> <td valign="top"> 0 &nbsp; </td> <td
 * valign="top"> <img
 ...
 * alt=""> </td> </tr> </table>
 * 在细化迭代5中，图像不能再以合理的方式绘制成三维图。因此，我们展示了一个范围为
 * $[-1,2]$ 的彩色图（当然，准确解的解值位于 $[0,1]$
 * ）。在任何情况下，很明显，连续Galerkin解表现出振荡的行为，随着网格的细化越来越差。
