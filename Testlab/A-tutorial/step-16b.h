 ...
 * 这是 step-16
 * 的一个变体，唯一的变化是我们使用MeshWorker框架和预制的LocalIntegrator辅助类，而不是手动组装矩阵。
 * 这个框架在实践中如何使用的细节将作为本教程程序的一部分来解释。
 * <a name="Thetestcase"></a><h3>The testcase</h3>
 *
 *  我们在这里解决的问题与 step-16 中的问题是一样的。
 *
 ...
 * 希望有一个类能够提供单元格和边界及内部面的积分函数。这是由下面的类来完成的。在构造函数中，我们告诉循环应该计算单元格积分（"真"），但不应该计算边界和内部面的积分（两个
 * "假"）。因此，我们只需要一个单元格函数，而不需要面的函数。
 *
 *
 * @code
 * template <int dim>
 ...
 *
 *
 * MeshWorker::LocalResults 的基类 MeshWorker::DoFInfo
 * 包含了可以在这个局部积分器中填充的对象。在MeshWorker框架内，有多少对象被创建是由装配器类决定的。在这里，我们测试一下，比如说需要一个矩阵
 * (MeshWorker::LocalResults::n_matrices()).  矩阵是通过
 * MeshWorker::LocalResults::matrix(),
 * 来访问的，它的第一个参数是矩阵的编号。第二个参数只用于对面的积分，当每个测试函数使用两个矩阵时。那么，第二个具有指标
 * "true "的矩阵将以相同的索引存在。
 *
 *
 * MeshWorker::IntegrationInfo
 * 提供了一个或几个FEValues对象，下面这些对象被
 * LocalIntegrators::Laplace::cell_matrix() 或 LocalIntegrators::L2::L2().
 * 使用，因为我们只组装一个PDE，所以也只有一个索引为0的对象。
 *
 *
 * 此外，我们注意到这个积分器的作用是计算多级预处理的矩阵，以及全局系统的矩阵和右手边。由于系统的汇编器需要一个额外的向量，
 * MeshWorker::LocalResults::n_vectors()
 * 是返回一个非零值。相应地，我们在这个函数的结尾处填充了一个右手边的向量。由于LocalResults可以处理多个BlockVector对象，但我们在这里又是在最简单的情况下，我们将信息输入到零号向量的零块中。
 *
 *
 * @code
 * template <int dim>
 ...
 *
 * 这个主类与  step-6
 * 中的类基本相同。就成员函数而言，唯一增加的是
 * <code>assemble_multigrid</code>
 * 函数，它组装了对应于中间层离散运算符的矩阵。
 *
 *
 * @code
 * template <int dim>
 ...
 * @endcode
 *
 * 以下成员是多网格方法的基本数据结构。前两个表示稀疏模式和多级层次结构中各个层次上的矩阵，非常像上面全局网格的对象。
 ...
 * @endcode
 *
 * 多网格约束必须被初始化。他们也需要知道边界值，所以我们也在这里传递
 * <code>dirichlet_boundary</code> 。
 *
 *
 * @code
 ...
 * 函数在每个层次上生成一个初步的压缩稀疏模式（关于这个主题的更多信息，见
 * @ref Sparsity
 * 模块），然后把它复制到我们真正想要的那个层次上。下一步是用这些稀疏模式初始化两种层次矩阵。
 ...
 *
 * 下面的函数在网格的最细层次上装配线性系统。由于我们想在下面的层次装配中重用这里的代码，我们使用局部积分器类LaplaceIntegrator，而把循环留给MeshWorker框架。因此，这个函数首先设置了这个框架所需的对象，即
 *
 *
 *
 ...
 *
 *
 * - 一个 MeshWorker::DoFInfo 对象，它一方面扩展了单元格迭代器的功能，同时也为其基类LocalResults的返回值提供了空间。
 *
 *
 *
 * - 一个汇编器，在这里是指整个系统。这里的 "简单 "是指全局系统没有一个块状结构。
 *
 *
 * - 本地积分器，它实现了实际的形式。
 * 在循环将所有这些组合成一个矩阵和一个右手边之后，还有一件事要做：集合器将矩阵行和列的受限自由度不加处理。因此，我们在对角线上放一个一，使整个系统摆好。1这个值或任何固定的值都有一个好处，那就是它对矩阵的频谱的影响很容易理解。由于相应的特征向量形成了一个不变的子空间，所选择的值不会影响Krylov空间求解器的收敛性。
 *
 *
 * @code
 * template <int dim>
 ...
 *
 * 下一个函数是建立线性算子（矩阵），定义每一层网格的多网格方法。积分的核心与上面的相同，但是下面的循环会遍历所有已有的单元，而不仅仅是活动的单元，而且结果必须输入正确的层次矩阵中。幸运的是，MeshWorker对我们隐藏了大部分的内容，因此这个函数与之前的函数的区别只在于汇编器的设置和循环中不同的迭代器。另外，最后对矩阵的修复也是比较复杂的。
 *
 *
 * @code
 * template <int dim>
 ...
 *
 * 这是在支持多栅求解器（或者说，事实上，我们使用多栅方法的前置条件器）方面明显不同的另一个函数。
 ...
 * 和MGSmootherPrecondition类提供了对这两种平滑器的支持。这里，我们选择应用单一的SOR迭代。为此，我们定义一个适当的别名，然后设置一个平滑器对象。
 ...
 *
 * @endcode
 * 下一个准备步骤是，我们必须将我们的层次和接口矩阵包裹在一个具有所需乘法函数的对象中。我们将为从粗到细的接口对象创建两个对象，反之亦然；多网格算法稍后将使用转置运算器进行后一种操作，允许我们用已经建立的矩阵初始化该运算器的向上和向下版本。
 *
 *
 * @code
 ...
 * step-6
 * 中的函数几乎没有变化，除了一个小的区别：我们以VTK格式生成输出，以使用今天更现代的可视化程序，而不是
 * step-6 编写时的那些。
 *
 *
 * @code
 ...
 *
 *  如 step-16
 * ，在最细的网格上，解决方案看起来是这样的。 <p
 * align="center"> <img
 * src="https://www.dealii.org/images/steps/developer/step-16.solution.png"
 * alt=""> </p> 与 step-16
 * 相比，输出的格式略有不同，但功能相同，显示了相同的收敛特性：<pre>
 * DEAL::Cycle  0DEAL:: 活动单元数：20DEAL::
 * 自由度数：25（按级别：8，25）  DEAL:cg::Starting  ]值0.510691
 * DEAL:cg::Convergence  步骤6值4.59193e-14  DEAL::Cycle  1DEAL::
 * 活动单元数：44DEAL::
 * 自由度数：55（通过级别：8，25，45）  DEAL:cg::Starting
 * ]值0.440678  DEAL:cg::Convergence  第8步值1.99419e-13  DEAL::Cycle
 * 2DEAL::活动单元数：86DEAL::自由度数：105（按级别：8、25、69、49）
 * DEAL:cg::Starting  值0.371855  DEAL:cg::Convergence  ]
 * 第9步值1.13984e-13  DEAL::Cycle  3DEAL:: 活动单元数：170DEAL::
 * 自由度数：200（按级别：8、25、77、174）  DEAL:cg::Starting
 * 值0.318967  DEAL:cg::Convergence  ] 第9步值2.62112e-13  DEAL::Cycle
 * 4DEAL:: 活动单元数：332DEAL:: 自由度数：388（按级别：8, 25,
 * 86, 231, 204）  DEAL:cg::Starting  值0.276534  DEAL:cg::Convergence
 * 第10步值1.69562e-13  DEAL::Cycle  ] 5DEAL:: 活动单元数: 632DEAL::
 * 自由度数: 714 (按级别: 8, 25, 89, 231, 514, 141)  DEAL:cg::Starting
 * 值 0.215300  DEAL:cg::Convergence  步骤10值 6.47463e-13  DEAL::Cycle  ]
 * 6DEAL:: 活动单元的数量: 1202DEAL:: 自由度的数量: 1332
 * (通过级别: 8, 25, 89, 282, 771, 435, 257)  DEAL:cg::Starting  值
 * 0.175848  DEAL:cg::Convergence  步骤 10 值 1.80664e-13  DEAL::Cycle  ]
 * 7DEAL:: 活动单元数：2288DEAL::
 * 自由度数：2511（按级别：8，25，89，318，779，1420，829，30）
 * DEAL:cg::Starting  值0.136724  DEAL:cg::Convergence
 * 第11步值9.73331e-14</pre>。
 *
