 ...
 * 你可能想知道AD和SD是什么，首先。好吧，这个问题很容易回答，但如果没有上下文，就没有很好的洞察力。我们不打算在这个介绍中涉及这个问题，而是将这个问题推迟到第一个介绍性的例子中，在这个例子的展开过程中，我们将列出关键点。作为补充，我们应该提到，这两个框架的核心理论在 @ref
 * auto_symb_diff 模块中已被广泛讨论，所以在此无需重复。
 ...
 *
 * - <b>Complex problems:</b>很可能有些问题恰好有一个非线性，对线性化或手工制定有极大的挑战。 如果有一个工具为你解决这个挑战，那么在大多数情况下，它是稳健的、可靠的和准确的，可以减轻实现某些问题的痛苦。这方面的例子包括 step-15 ，我们解决的非线性PDE的导数并不难推导，但足够繁琐，人们必须注意用手来做，而且实现牛顿步骤的相应有限元公式需要的不仅仅是实现双线性形式一般需要的几行； step-33 （我们实际上使用AD）是一个更极端的例子。
 *
 ...
 * <a name="Thermodynamicprinciples"></a><h4>Thermodynamic principles</h4>。
 *
 * 作为介绍我们将用来建立磁活性聚合物模型的磁机械耦合材料法的前奏，我们将首先对这些构成法则必须遵守的突出的热力学进行非常简明的总结。这里总结的理论基础由Truesdell和Toupin
 * @cite Truesdell1960a  以及Coleman和Noll  @cite Coleman1963a
 * 详细描述，并遵循Holzapfel  @cite Holzapfel2007a  提出的逻辑。
 * 从热力学第一定律开始，并遵循一些技术假设，可以证明动能加内能率与外部资源提供给系统的功率之间的平衡由以下关系给出，该关系等同于左侧（任意）体积 $V$ 中的能量变化率，以及右侧作用在该体积上的力之和：@f[
 ...
 * + R_{0} \right] dV .
 * @f] 这里 $D_{t}$ 代表总时间导数， $\rho_{0}$ 是在拉格朗日参考框架下测量的材料密度， $\mathbf{v}$ 是材料速度， $\mathbf{a}$ 是其加速度， $U^{*}_{0}$ 是每单位参考体积的内能， $\mathbf{P}^{\text{tot}}$ 是总皮拉应力张量， $\dot{\mathbf{F}}$ 是变形梯度张量的时间率， $\boldsymbol{\mathbb{H}}$ 和 $\boldsymbol{\mathbb{B}}$  ]分别是磁场矢量和磁感应强度（或磁通密度）矢量， $\mathbb{E}$ 和 $\mathbb{D}$ 是电场矢量和电位移矢量，以及 $\mathbf{Q}$ 和 $R_{0}$  材料微分算子 $\nabla_{0} (\bullet) \dealcoloneq \frac{d(\bullet)}{d\mathbf{X}}$ ，其中 $\mathbf{X}$ 是材料位置矢量。随着一些条款的重新排列，引用积分体积的任意性 $V$  ]，总的内能密度率 $\dot{E}_{0}$ 可以确定为@f[
 ...
 * 热力学第二定律，也被称为熵不平等原则，告诉我们某些热力学过程是不可逆的。在考虑了总熵和熵的输入率后，可以得出克劳修斯-杜姆不等式。在局部形式下（以及在物质配置中），这读作@f[
 * \theta \dot{\eta}_{0}
 *
 *
 *
 ...
 * 。
 *
 * 当考虑到表现出机械耗散行为的材料时，可以证明这可以通过用代表内部变量的额外参数来增加材料自由能密度函数，从而在耗散不等式中得到体现  @cite Holzapfel1996a  。因此，我们把它写成@f[
 * \mathcal{D}_{\text{int}}
 * = \mathbf{P}^{\text{tot}} : \dot{\mathbf{F}}
 * + \boldsymbol{\mathbb{H}} \cdot \dot{\boldsymbol{\mathbb{B}}}
 *
 ...
 * 对能量密度函数的材料速率进行扩展，并对各种条款进行重排，结果是表达式@f[
 * \mathcal{D}_{\text{int}}
 ...
 * y\right)}{d x} \Big\vert_{y} . @f]
 * ]更具体到耗散不等式中的编码（用非常通用的自由能量密度函数
 * $\psi_{0}$
 * ，其参数化还有待正式确定），如果一个输入变量是另一个变量的函数，它也被保持不变，连锁规则不会进一步传播，而计算总衍生物将意味着明智地使用连锁规则。通过比较以下两个语句可以更好地理解这一点。
 * @f{align*}
 ...
 * @f] 从耗散功率中剩下的条款（即与机械耗散机制有关的条款）来看，如果假定它们是相互独立的，那么对于每个机制`i`，@f[
 * \frac{\partial \psi_{0}}{\partial \mathbf{C}_{v}^{i}} :
 * \dot{\mathbf{C}}_{v}^{i} \leq 0 .
 * @f]这一约束必须通过自由能函数的适当选择，以及仔细考虑的内部变量的演化规律来满足。
 ...
 * Pelteret2018a
 * ，方程式29）。这个函数实际上会使材料在强磁场的存在下变硬。由于它是由一个sigmoid类型的函数控制的，剪切模量将渐进地收敛于指定的饱和剪切模量。还可以证明，
 * $\psi_{0}$
 * 中的最后一项是磁场的储能密度函数（从第一原理推导出来的），由相对渗透率常数缩放。这个定义共同意味着材料是线性磁化的，也就是说，磁化矢量和磁场矢量是一致的。(这在以电流形式表示的磁能中当然不明显，但当磁感应和磁化从
 * $\psi_{0}$ 中导出，并且所有磁场都以 <em> 的电流配置 </em>
 * 表示时，这种关联性就变得很清楚了。)至于磁感应、应力张量和各种材料切线的具体细节，我们将把这些推迟到教程正文中来介绍，那里定义了构成法的完整、无辅助的实施。
 ...
 *
 *
 * - \mathbf{C}_{v} \right] @f]。至于粘性部分 $\psi_{0}^{MVE}$
 ...
 * "挑选
 * "一个有代表性的点，让它在一个恒定的轴向变形（默认为压缩载荷）和一个恒定的轴向施加的磁场中承受谐波剪切变形。我们将记录这时的应力和磁感应强度，并将数据输出到文件中进行后处理。尽管在这个特定的问题上没有必要，但我们也将计算切线。尽管在这个特定的工作中没有直接使用它们，但为了将构成法嵌入无限元模型（这项工作的一个可能的扩展），需要这些二次导数。因此，我们将利用这个机会，用辅助微分框架来检查我们的手工计算的正确性。
 * <a name="Suggestedliterature"></a><h3>Suggested literature</h3>
 *
 * 除了已经提到的 @ref
 * auto_symb_diff 模块外，以下是一些更详细讨论的参考文献
 *
 ...
 * @endcode
 *
 * 我们不会立即揭示这个函数的导数，而是向前声明返回导数的函数，并将其定义推迟到以后。正如函数名称所暗示的，它们分别返回导数
 * $\frac{df(x,y)}{dx}$  。
 *
 *
 * @code
 ...
 * @endcode
 *
 * 我们现在要求助手类给我们的自变量以其可自动区分的表示。这些被称为
 ...
 * @endcode
 *
 * </div> 当然，所有这些也可以用于二阶甚至高阶导数。
 * 所以现在很清楚，通过上述表示方法，"ADNumberType
 * "携带了一些额外的数据，表示可微调函数相对于原始（敏感）自变量的各种导数。因此应该注意到，使用它们会产生计算开销（因为我们在做导数计算时要计算额外的函数），以及存储这些结果的内存开销。因此，规定的微分运算的级数最好保持在最低水平，以限制计算成本。例如，我们可以先自己计算一阶导数，然后用
 * Differentiation::AD::VectorFunction
 * 辅助类来确定依赖函数集合的梯度，这将是原标量函数的二阶导数。
 ...
 * "执行努力"，让我们把它与手工计算的、在几个独立函数中实现的导数进行比较。
 *
 *
 ...
 *   {
 * @endcode
 * 我们需要做的第一步是形成符号变量，代表我们希望对其进行微分的函数参数。同样，这些将是我们问题的独立变量，因此在某种意义上是原始变量，对任何其他变量没有依赖性。我们通过初始化一个符号类型
 ...
 *
 * 使用计算 $f(x,y)$
 * 的模板函数，我们可以将这些独立变量作为参数传递给函数。返回的结果将是另一个符号类型，代表用于计算
 ...
 * 你可能会注意到，我们在构建符号函数`f_sd`时没有说明我们可能要如何使用它。与上面的AD方法相比，我们调用`f(x_sd,
 ...
 * "占位符
 * "和运算的组成。事实上，用于组成函数的各个组件也是占位符。操作的序列被编码到一个树状的数据结构中（概念上类似于[抽象语法树](https://en.wikipedia.org/wiki/Abstract_syntax_tree)）。
 ...
 * 不仅仅取决于两个标量变量，而是取决于一大堆标量*，每个标量都有几个组成部分。在某些情况下，这些是对称*张量，对于这些张量来说，只有一个分量子集实际上是独立的，我们必须考虑计算
 ...
 *
 * @endcode
 * 我们将在一个方法中计算和存储这些值，而不是随意计算和返回动力学变量或其线性化。然后这些缓存的结果将在请求时返回。我们将把为什么要这样做的精确解释推迟到以后的阶段。现在重要的是看到这个函数接受所有的场变量，即磁场矢量
 ...
 *
 *
 * - 总的皮奥拉-基尔霍夫应力张量  $\mathbf{S}^{\text{tot}}$  。
 *
 *
 * @code
 ...
 * - 磁静力切张量 $\mathbb{D}$  。
 *
 * - 总的参考性磁弹性耦合张量  $\mathfrak{P}^{\text{tot}}$  , 和
 *
 ...
 *
 * 在该类的 "保护
 * "部分，我们存储了一个对支配材料响应的构成参数实例的引用。为了方便起见，我们还定义了一些函数来返回各种构成参数（包括明确定义的，以及计算的）。
 * 与材料的弹性响应有关的参数依次是
 *
 *
 *
 ...
 *
 *
 * - 磁导率常数 $\mu_{0}$ （并不是一个真正的材料常数，而是一个普遍的常数，为了简单起见，我们在这里分组）。
 ...
 *
 * 我们将首先考虑一种非耗散性材料，即受磁超弹性构成法则支配的材料，当浸入磁场时表现出刚性。正如介绍中所述，这种材料的储能密度函数可能由@f[
 * \psi_{0} \left( \mathbf{C}, \boldsymbol{\mathbb{H}} \right)
 * = \frac{1}{2} \mu_{e} f_{\mu_{e}} \left( \boldsymbol{\mathbb{H}} \right)
 * \left[ \text{tr}(\mathbf{C})
 *
 ...
 * \boldsymbol{\mathbb{H}}} {\left(h_{e}^{\text{sat}}\right)^{2}} \right) .
 * @f]给出。
 * 现在来谈谈实现这种行为的类。由于我们期望这个类能完全描述一种材料，我们将把它标记为
 ...
 * @endcode
 *
 * 在这个类的`private`部分，我们需要定义一些提取器，这些提取器将帮助我们设置自变量，并在之后得到与因变量相关的计算值。如果这个类是在有限元问题的背景下使用，那么这些提取器中的每一个都（很可能）与解场的一个分量的梯度有关（在这种情况下，位移和磁标势）。正如你现在可能推断的那样，这里
 ...
 * @endcode
 *
 * 由于我们在每个时间步骤中都会重复使用 @p ad_helper
 ...
 * @endcode
 *
 * 现在我们已经完成了初始设置，我们可以检索我们字段的AD对应关系。这些是真正的能量函数的独立变量，并且对用它们进行的计算是
 ...
 * @endcode
 *
 * 事实上，储存的能量密度函数是这个问题的因变量，所以作为
 * "配置 "阶段的最后一步，我们用 @p ad_helper. 注册其定义。
 *
 *
 * @code
 ...
 * @endcode
 *
 * 请注意，对于耦合项来说，提取器参数的顺序尤其重要，因为它决定了定向导数的取值顺序。因此，如果我们在调用`extract_hessian_component()`时颠倒了提取器的顺序，那么我们实际上是在检索
 ...
 *
 * 在该类的 "私有
 * "部分，我们将希望跟踪内部的粘性变形，因此以下两个（实值的、非符号的）成员变量分别持有
 *
 *
 *
 ...
 * - 内部变量在前一个时间步长的值。
 * (我们将这些变量标记为
 * "Q"，以便于识别；在计算的海洋中，不一定容易区分`Cv`或`C_v`与`C`。)
 *
 *
 * @code
 *   private:
 ...
 * "SD
 * "为后缀，以方便区分符号类型或表达式与实值类型或标量。)
 * 这可以在前面做一次（甚至有可能作为 "静态
 * "变量），以尽量减少与创建这些变量相关的开销。为了实现通用编程的终极目标，我们甚至可以用符号来描述构成参数，有可能*允许一个类的实例在不同的输入下重复使用这些值。
 ...
 *
 *
 * - 字段值（其值由调用此 @p Magnetoviscoelastic_Constitutive_Law_SD 实例的外部函数规定），和
 *
 *
 *
 ...
 * 的大部分程序看起来与用于构建自动微分助手的程序非常相似。尽管如此，我们还是要再次详细说明这些步骤，以强调这两个框架的差异。
 ...
 *
 * @endcode
 * 此外，我们定义了磁-粘弹性对自由能密度函数的贡献。实现这一点所需的第一个成分是一个比例函数，它将使粘性剪切模量在磁场的影响下发生变化（增加）（见
 * @cite Pelteret2018a
 * ，公式29）。此后，我们可以计算能量密度函数的耗散分量；其表达式见
 * @cite Pelteret2018a （方程28），这是对 @cite Linder2011a
 * 中制定的能量密度函数（方程46）的直接扩展。
 *
 *
 * @code
 ...
 * @endcode
 *
 * 接下来我们产生一个中间替换图，它将把在表达式中发现的
 * @p Q_t_sd （我们的标识符）的每一个实例，用 @p
 * Q_t_sd_explicit. 中持有的完整表达式来替换它
 *
 *
 * @code
 ...
 * @endcode
 *
 * 我们可以对两个动力学变量进行这种替换，并立即将替换后的结果与场变量进行区分。如果你愿意，这可以分成两步进行，中间的结果储存在一个临时变量中）。同样，如果你忽略了替换所产生的
 ...
 * 我们需要为哪些条目提供数值，以便它能成功地进行计算。这些基本上是作为
 * @p optimizer
 ...
 *
 * </div>
 * 但这些实际上都已经被编码为替换图的键。这样做还意味着我们需要在两个地方（这里和构建替换图时）管理这些符号，这很烦人，而且如果这个材料类被修改或扩展，可能会出现错误。由于我们此时对数值不感兴趣，所以如果替换图中与每个键项相关的数值被填入无效的数据也没有关系。所以我们将简单地创建一个假的替换图，并从中提取符号。请注意，任何传递给
 ...
 *
 * 由于 @p optimizer
 * 的配置是在前面完成的，所以每次我们想计算动能变量或它们的线性化（导数）时，要做的事情非常少。
 *
 *
 * @code
 *   template <int dim>
 ...
 * 在时间上向前移动时，内部变量的 "当前
 * "状态瞬间定义了 "前一个
 * "时间段的状态。因此，我们记录历史变量的值，作为下一个时间步骤的
 * "过去值 "使用。
 *
 *
 * @code
 ...
 * \right)$ 有助于确保所产生的等级4张量，由于 $\mathbf{C}$
 * 的对称性而持有小的对称性，仍然将等级2对称张量映射到等级2对称张量。参见SymmetricTensor类文档和
 * step-44
 * 的介绍，并进一步解释对称性在四阶张量背景下的含义。
 ...
 *  好吧，这很快就升级了
 *
 * - 尽管 $\psi_{0}$ 和 $f_{\mu_e}$ 的定义可能已经给出了一些提示，表明计算动能场和它们的线性化将需要一些努力，但最终的定义可能比最初想象的要复杂一些。了解了我们现在所做的，也许可以说，我们真的不想计算这些函数相对于其参数的一、二次导数
 *
 ...
 * "手动
 * "安排它们的顺序，以最大限度地重用它们。这意味着我们必须自己管理，并决定哪些值必须在其他值之前计算，同时保持代码本身的某种秩序或结构的模样。这很有效，但也许有点乏味。它对类的未来扩展也没有太大的帮助，因为所有这些值都是这个单一方法的局部。
 ...
 *
 * 如前所述，我们将考虑的具有一个耗散机制的磁涡流材料的自由能密度函数被定义为 @f[
 * \psi_{0} \left( \mathbf{C}, \mathbf{C}_{v}, \boldsymbol{\mathbb{H}}
 * \right)
 * = \psi_{0}^{ME} \left( \mathbf{C}, \boldsymbol{\mathbb{H}} \right)
 * + \psi_{0}^{MVE} \left( \mathbf{C}, \mathbf{C}_{v},
 ...
 * 为参数的。根据设计，能量 $\psi_{0}^{ME} \left( \mathbf{C},
 * \boldsymbol{\mathbb{H}} \right)$
 ...
 * @endcode
 *
 * 接下来我们对与速率有关的构成法做同样的实验。如果SymEngine被设置为使用LLVM即时编译器，则默认选择最高性能的选项，该编译器（结合一些积极的编译标志）产生所有可用选项中最快的代码评估路径。作为后备措施，所谓的
 * "lambda
 ...
 * $65 \times$
 * 的时间来计算所需的数量。这看起来确实是一个相当大的开销，但是，正如介绍中提到的，这是否可以接受，完全是主观的，而且取决于具体情况：你是否更看重计算机的时间，而不是人的时间来做必要的手工计算导数，验证其正确性，实现它们，并验证实现的正确性？如果你开发的研究代码只会在相对较少的实验中运行，你可能更看重自己的时间。如果你开发的生产代码将在10,000核的集群上反复运行几个小时，你的考虑可能就不一样了。无论如何，AD方法的一个很好的特点是，当函数和类在标量类型上建模时，有
 ...
 * "并不意味着你必须对这些数字所通过的算法没有争议。有可能不经意间进行的操作，在进行区分时，会返回一个不正确的结果。
 * 所以这绝对是一个人应该注意的事情。
