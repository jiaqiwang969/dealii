...
@f]，出于明显的原因，把这个变量称为<i>velocity</i>。这样我们就可以把原来的波浪方程重新表述如下：@f{eqnarray*}
...
\\
	...
	\textrm{in}\ \Omega.
@f} 。
* 这种表述的好处是，它现在只包含两个变量的第一时间导数，为此，可以简单地写出时间步进方案。请注意，我们一开始并没有为 $v$ 设定边界条件。然而，我们可以在边界上强制执行 $v=\frac{\partial
g}{\partial t}$ 。在数值例子中发现，这实际上是必要的：如果不这样做，解决方案看起来并不特别错误，但如果不强制执行这些边界条件，Crank-Nicolson方案就不能保存能量。
* 有了这个公式，让我们引入以下的时间分解，其中上标 $n$ 表示一个时间步长， $k=t_n-t_{n-1}$ 是当前时间步长：\f{eqnarray*}
  \frac{u^n
* 
- u^{n-1}}{k}
* 
...
* 
* 
- v^{n-1} = 0$ ，这就是众所周知的前向或显式欧拉方法。另一方面，如果我们设定 $\theta=1$ ，那么我们将得到 $\frac{u^n
* 
- u^{n-1}}{k}
* 
...
* 
* 
- v^n = 0$ ，这对应于后向或隐式欧拉方法。这两种方法都是一阶精确的方法。它们实现起来很简单，但实际上并不十分精确。
* 第三种情况是选择 $\theta=\frac 12$ 。然后上面的第一个方程就会变成 $\frac{u^n
* 
- u^{n-1}}{k}
* 
- \frac 12 \left[v^n + v^{n-1}\right] = 0$ 。这种方法被称为Crank-Nicolson方法，它的优点是二阶精确。此外，它还有一个很好的特性，即保留了溶液中的能量（从物理上讲，能量是膜中粒子的动能加上由于局部拉伸而存在的势能的总和；这个量在连续方程中是一个守恒量，但大多数时间步进方案在时间微分后并没有保留它）。由于 $v^n$ 也出现在 $u^n$ 的方程中，Crank-Nicolson方案也是隐含的。
* 在程序中，我们将把 $\theta$ 作为一个参数，这样就可以很容易地玩弄它。结果部分将显示一些比较不同方案的数字证据。
* 上面的方程（称为<i>semidiscretized</i>方程，因为我们只离散了时间，而没有离散空间），可以通过从第一个方程中消除 $v^n$ 和重新排列项来简化一下。然后我们得到\f{eqnarray*}
...
-
  ...
  \right].
\f}。
* 然后习惯性地对 $u^n(x) \approx u^n_h(x) = \sum_i
U_i^n\phi_i^n(x)$ 进行近似，其中 $\phi_i^n(x)$ 是用于离散化 $n$ 第1个时间步长的形状函数， $U_i^n$ 是解决方案的未知结点值。同样地， $v^n(x) \approx
v^n_h(x) = \sum_i V_i^n\phi_i^n(x)$  。最后，我们有前一个时间步骤的解决方案， $u^{n-1}(x) \approx u^{n-1}_h(x) = \sum_i
U_i^{n-1}\phi_i^{n-1}(x)$ 和 $v^{n-1}(x) \approx v^{n-1}_h(x) = \sum_i
V_i^{n-1}\phi_i^{n-1}(x)$  。请注意，由于前一个时间步骤的解决方案在我们到达时间步骤时已经被计算出来了 $n$ ,  $U^{n-1},V^{n-1}$ 是已知的。此外，请注意上一步的解可能是在不同的网格上计算出来的，所以我们必须使用形状函数  $\phi^{n-1}_i(x)$  。
* 如果我们将这些展开式插入上述方程，并使用目前网格中的测试函数进行测试，我们将得到以下线性系统：\f{eqnarray*}
...
@f]，因此，在没有体力和恒定边界值的情况下，我们得到@f[
	E(t) = \frac 12 \int_\Omega \left(\frac{\partial u}{\partial
	t}\right)^2 + (\nabla u)^2 \; dx
@f]是一个守恒量，即一个不会随时间变化的量。我们将在每个时间步长后计算这个量。我们可以直接看到，如果我们用有限元近似代替 $u$ ，用速度的有限元近似代替 $\frac{\partial u}{\partial t}$ ，那么@f[
...
*  <li>  不同的初始和边界条件，右手边。
*  <li>  更复杂的域或更精细的网格。请记住，时间步长需要以网格宽度为界，所以改变网格应该同时改变时间步长。我们将在  step-24  中再来讨论这个问题。
*  <li>  可变系数。在实际介质中，波速往往是可变的。特别是，现实介质中的 "真实 "波浪方程应为@f[
...
-
     \nabla \cdot
     a(x) \nabla u = f,
  @f]，其中 $\rho(x)$ 是材料的密度， $a(x)$ 与刚度系数有关。那么波速就是 $c=\sqrt{a/\rho}$  。
* 要做这样的改变，我们就必须用可变系数来计算质量和拉普拉斯矩阵。幸运的是，这并不难：函数 MatrixCreator::create_laplace_matrix 和 MatrixCreator::create_mass_matrix 有额外的默认参数，可以用来向它们传递非恒定系数函数。因此，所需的变化相对较小。另一方面，必须再次注意确保时间步长在允许范围内。
*  <li>  在代码内的注释中，我们讨论了这样一个事实：由于边界条件的原因，用于求解 $U^n$ 和 $V^n$ 的矩阵需要在每个时间内被重置，尽管实际内容没有变化。可以通过不消除线性系统中的列来避免复制，这可以通过在调用中附加一个 @p false 参数来实现。
* @code
    MatrixTools::apply_boundary_values(boundary_values,
                                       matrix_u,
                                       solution_u,
...
- k^2\theta(1-\theta)(\nabla u^{n-1},\nabla \varphi)
  ...
  \theta (f^n,\varphi) + (1-\theta) (f^{n-1},\varphi)
  \right].
  \f} 现在，注意我们在网格 ${\mathbb T}^n$ 上求解 $u^n$ ，因此，测试函数 $\varphi$ 也必须来自空间 $V_h^n$ 。正如在介绍中所讨论的，像 $(u^{n-1},\varphi)$ 这样的条款要求我们将上一步的解决方案（可能是在不同的网格 ${\mathbb T}^{n-1}$ 上计算的）与当前网格的测试函数进行整合，从而得到一个矩阵 $M^{n,n-1}$ 。这个整合不同网格的形状函数的过程，充其量也是尴尬的。它是可以做到的，但是由于很难保证 ${\mathbb T}^{n-1}$ 和 ${\mathbb T}^{n}$ 最多只相差一个细化级别，我们必须递归匹配两个网格的单元。这样做是可行的，但它会导致冗长的、不完全明显的代码。
* 第二种方法是：每当我们改变网格时，我们只需使用SolutionTransfer类将旧网格上最后一个时间步长的解插到新网格上。换句话说，我们将解决\f{eqnarray*}
  (u^n,\varphi) + k^2\theta^2(\nabla u^n,\nabla \varphi) &=&
  (I^n u^{n-1},\varphi)
* 
- k^2\theta(1-\theta)(\nabla I^n u^{n-1},\nabla \varphi)
