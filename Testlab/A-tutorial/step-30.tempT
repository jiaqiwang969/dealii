...
* 本例专门讨论 <em> 各向异性细化 </em>  ，它扩展了局部细化的可能性。在大多数情况下，这是对 step-12 教程程序的修改，我们使用相同的DG方法来处理线性转场方程。这个程序将涵盖以下主题。 <ol>   <li>   <em>  各向异性的细化  </em>  ：各向异性的细化是什么意思？   <li>   <em>  实现  </em>  ：对代码进行必要的修改，以便与各向异性的细化网格一起工作。   <li>   <em>  跳跃指标  </em>  : 在DG方法中，各向异性细化的一个简单指标。 </ol>  将不讨论离散化本身，也不讨论此处使用的非各向异性细化的实现技术。请参考 step-12 的内容。
...
* 在前面的教程程序中，所有的适应过程都是基于 <em> 各向同性的 </em> 细化单元，它将所有的边切成两半，并在这些分裂的边上形成新的单元（当然，还有一些额外的边、面和顶点）。在交易二中， <em> 各向异性细化 </em> 指的是只拆分部分边而不改变其他边的过程。以简单的方形单元为例，考虑一下。
* @code
* 
...
---*---*
@endcode
* 或者我们可以分割沿Y轴运行的两条边，再次产生两个子单元，但看起来是这样的。
* @code
* 
...
* 自适应局部细化被用来获得精细的网格，这些网格能够很好地适应高效地解决手头的问题。简而言之，产生较大误差的单元的尺寸被缩小，以获得对手头问题的更好的近似解。然而，很多问题都含有各向异性的特征。突出的例子是可压缩粘性流动中的冲击或边界层。一个有效的网格可以用较高长宽比的单元来逼近这些特征，而这些单元是根据上述特征来确定方向的。只使用各向同性的细化，原始网格单元的长宽比会被保留，因为它们会被单元的子代所继承。因此，从各向同性的网格开始，边界层将被细化，以捕捉壁面法线方向上流场的快速变化，从而导致在法线和切线方向上都具有非常小的边长的单元。通常情况下，切线方向的边长要大得多，因此可以使用更少的单元，而不会在近似精度上有明显的损失。各向异性的细化过程可以通过每个细化步骤将母细胞和子细胞的长宽比修改为2的系数。在多次细化的过程中，精细单元的长宽比可以被优化，节省了相当数量的单元和相应的自由度，从而节省了计算资源、内存和CPU时间。
*<a name="Implementation"></a><h3>Implementation</h3>
* 

* 大多数时候，当我们进行有限元计算时，我们一次只考虑一个单元，例如计算单元对全局矩阵的贡献，或者插值边界值。然而，有时我们不得不关注单元在我们的算法中是如何关联的。单元之间的关系有两种形式：邻居关系和母子关系。对于各向异性细化的情况，deal.II对始终保持的单元格关系使用了某些约定（不变量）。例如，一个细化的单元总是正好有 $2^{dim}$ 个孩子。而且（除了1d情况），两个相邻的单元格最多可以相差一个细化级别：它们同样经常被细化，或者其中一个正好再细化一次，在共同面上正好留下一个悬挂的节点。几乎所有的时候，这些不变量只在库的内部实现中才会被关注。然而，在有些情况下，对它们的了解也与应用程序有关。
...
* 另一个重要的方面，也是本教程中最重要的方面，是在组装单元格之间的表面上的跳跃项时对相邻关系的处理。查看 step-12 中assemble_system函数的文档，我们注意到，我们需要决定一个相邻的单元是否更粗、更细或者与我们当前的单元处于同一（细化）水平。这些决定对于各向异性的细化并不适用，因为单元的 <em> 级 </em> 所提供的信息并不足以完全描述各向异性的单元；例如，一个二维单元的终端子女是在 $x$  方向切割的二维单元，其子女在 $y$ 方向上被切割，是在第2层，还是在第1层，因为如果该单元被各向同性地细化一次，就会产生相同的最佳单元集？
...
*  <ul>  
*  <li>   <em>  细化邻居  </em>  ：如果我们在一个活动单元上，想在一个面 $f\subset \partial K$ 上进行整合，第一种可能性是这个面后面的邻居更细化，即有孩子只占据了公共面的一部分。在这种情况下，所考虑的面必须是一个精致的面，这可以通过询问  <code>if (face->has_children())</code>  来确定。如果这是真的，我们需要循环所有的子面，并得到这个子面后面的邻居的子，这样我们就可以用邻居重新生成一个FEFaceValues对象，用我们的单元格和相应的子面重新生成一个FESubfaceValues对象。
* 对于各向同性的细化，这种情况相当简单，因为我们知道在deal.II中各向同性细化的自适应网格的一个不变性是，邻域只能正好相差一个细化级别。然而，对于各向异性细化的网格来说，这并不完全正确，特别是在三维中；在那里，我们感兴趣的 $f$ 另一边的活动单元实际上可能不是我们邻居的孩子，而可能是孙子或甚至更远的后代。幸运的是，这种复杂性被隐藏在库的内部。我们所要做的就是调用 CellAccessor::neighbor_child_on_subface() 函数。尽管如此，在3D中，有两种情况需要特别考虑。   <ul>   <li>  如果邻居被各向异性地细化了一次以上，可能这里需要考虑的不是两个或四个而是三个子面。想象一下我们正在考虑的（三维）邻接单元的（二维）面的以下细化过程：首先该面沿x方向细化，后来只沿y方向细化左侧子面。
* @code
* 
...
* 当一个三角形被细化时，没有被标记为细化的单元仍然可以被细化。这是由于额外的平滑算法，这些算法是必要的或明确要求的。特别是，每条边上最多有一个悬空节点的限制，经常迫使邻近已经比较细化的单元的细化，并被标记为进一步细化。
* 然而，deal.II也实现了一些算法，确保产生的网格比最低限度的平滑，例如确保没有孤立的细化单元被非细化单元包围，因为这些土地上的额外自由度几乎都受到悬挂节点的约束。(参见Triangulation类的文档和它的 Triangulation::MeshSmoothing 成员以获得更多关于网格平滑的信息)。
...
* 我们解决在 step-12 中提出的线性传输方程。域被扩展到覆盖二维的 $[-1,1]\times[0,1]$ ，其中流场 $\beta$ 在域的右半部分描述了围绕原点的逆时针四分之一圆，在域的左半部分与x轴平行。流入边界同样位于 $x=1$ 处，并沿x轴的正向部分，边界条件选择如 step-12 。
* 

* <a name="CommProg"></a> <h1> The commented program</h1> 。
* deal.II包括的文件已经在前面的例子中介绍过了，因此将不再进一步评论。
* 
...
* 
 @endcode
* 
* 同样地，该类的构造函数以及组装对应于单元格内部和边界面的术语的函数与之前相比也没有变化。装配单元间面术语的函数也没有改变，因为它所做的只是对两个FEFaceValuesBase类型的对象（它是FEFaceValues和FESubfaceValues的基类）进行操作。这些对象从何而来，即它们是如何被初始化的，对这个函数来说并不重要：它只是假设这两个对象所代表的面或子面上的正交点对应于物理空间中的相同点。
* 
...
* @code
 ...
 @endcode
* 
* 我们需要知道，哪个邻居的面朝向我们的单元格。使用 @p  neighbor_face_no函数，我们可以得到较粗和非较粗邻居的这些信息。
* 
...
* @code
 ...
 @endcode
* 
* 为了得到当前子面后面的单元格，我们可以使用 @p neighbor_child_on_subface 函数。它照顾到了所有各向异性细化和非标准面的复杂情况。
* 
...
* 
 ...
 @endcode
* 
* 情况（c）。如果这是一个内部面，并且邻居没有进一步细化，我们就会得到这里（或者，如上所述，我们是在1d中，在这种情况下，我们对每个内部面都会得到这里）。然后我们需要决定是否要对当前面进行整合。如果邻居实际上更粗，那么我们就忽略这个面，而是在访问邻近的单元并查看当前面时进行处理（除了在1d中，如上所述，这是不可能的）。
* 
...
* 另一方面，如果邻居是更精细的，那么我们已经在上面(b)的情况下处理过这个面了（1d除外）。所以对于2d和3d，我们只需要决定是要处理来自当前一侧的同一层次的单元格之间的面还是来自相邻一侧的面。 我们通过引入一个平局来做到这一点。我们只取索引较小的单元格（在当前细化级别内）。在1d中，我们取较粗的单元，或者如果它们在同一层次，则取该层次中指数较小的单元。这就导致了一个复杂的条件，希望在上面的描述中可以理解。
* 
...
* 现在，细化标志被设置为那些具有较大误差指标的单元。如果不做任何改变，这些单元将被各向同性地细化。如果给这个函数的 @p anisotropic 标志被设置，我们现在调用set_anisotropic_flags()函数，该函数使用跳转指标将一些细化标志重置为各向异性细化。
* 
...
* 
 @endcode
* 
* 一旦错误指标被评估，并且误差最大的单元被标记为细化，我们要再次循环这些被标记的单元，以决定它们是否需要各向同性的细化或者各向异性的细化更合适。这就是在介绍中解释的各向异性跳跃指标。
* 
...
* @code
 ...
 @endcode
* 
* 我们想在被标记的单元格的面上评估跳跃，所以我们需要一些对象来评估面上的解的值。
* 
...
* 
 @endcode
* 
* 在装配例程中看到的四种不同的相邻关系的情况在这里以同样的方式重复。
* 
...
* @code
 ...
 @endcode
* 
*并整合解的绝对值，即分别从当前单元格和邻近单元格看到的函数值的绝对值。我们知道，前两个面与单元格上的第一个坐标方向正交，后两个面与第二个坐标方向正交，以此类推，所以我们把这些值累积成有 <code>dim</code> 分量的向量。
* 
...
* @code
 ...
 @endcode
* 
* 我们当前的单元和邻居沿着所考虑的面有相同的细化程度。除此之外，我们的做法与上述案例中的一个子单元基本相同。
* 
...
- u_neighbor[x]) JxW[x];
 ...
 @endcode
* 
* 现在，邻居实际上更粗了。这种情况是新的，因为它没有在汇编程序中出现。在这里，我们必须考虑它，但这并不太复杂。我们只需使用 @p  neighbor_of_coarser_neighbor函数，它再次自行处理各向异性的细化和非标准面的方向。
* 
...
* 
 @endcode
* 
* <a name="TheRest"></a> <h3>The Rest</h3> <a name="TheRest"></a>。  
...
* 
 @endcode
* 
* 如果你想在三维中运行程序，只需将下面一行改为  <code>const unsigned int dim = 3;</code>  。
* 
...
* 
* 这个程序的输出包括控制台的输出，包含网格的SVG文件，以及以VTU格式给出的解决方案。
* @code
...
* 另一个有趣的事情是，当然，要看到两个网格序列上的解决方案。在这里，它们在细化周期1和4上，清楚地显示出解决方案确实是由<i>discontinuous</i>件二项式组成的。
