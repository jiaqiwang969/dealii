...
*   $^\dagger$    <em>  Sandia National Laboratories is a multimission laboratorymanaged and operated by National Technology & Engineering Solutions of Sandia,LLC, a wholly owned subsidiary of Honeywell International Inc., for the U.S.Department of Energy's National Nuclear Security Administration under contractDE-NA0003525. This document describes objective technical results and analysis.Any subjective views or opinions that might be expressed in the paper do notnecessarily represent the views of the U.S. Department of Energy or the UnitedStates Government.  </em>  
*   @note   This tutorial step implements a first-order accurate <i>guaranteed
maximum wavespeed method</i> based on a first-order <i>graph viscosity</i>for solving Euler's equations of gas dynamics   @cite GuermondPopov2016  . Assuch it is presented primarily for educational purposes. For actualresearch computations you might want to consider exploring a corresponding<a href="https://github.com/conservation-laws/ryujin">high-performance
implementation of a second-order accurate scheme</a> that uses <i>convex
limiting</i> techniques, and strong stability-preserving (SSP) timeintegration, see   @cite GuermondEtAl2018  (<a href="https://conservation-laws.43-1.org/">website</a>).
*   @dealiiTutorialDOI{10.5281/zenodo.3698223,https://zenodo.org/badge/DOI/10.5281/zenodo.3698223.svg}  
* <a name="Intro"></a><a name="Introduction"></a><h1>Introduction</h1>
* 

* This tutorial presents a first-order scheme for solving compressibleEuler's equations that is based on three ingredients: a<i>collocation</i>-type discretization of Euler's equations in the contextof finite elements; a graph-viscosity stabilization based on a<i>guaranteed</i> upper bound of the local wave speed; and explicittime-stepping. As such, the ideas and techniques presented in this tutorialstep are drastically different from those used in   step-33  , which focuses onthe use of automatic differentiation. From a programming perspective thistutorial will focus on a number of techniques found in large-scalecomputations: hybrid thread-MPI parallelization; efficient local numberingof degrees of freedom; concurrent post-processing and write-out of resultsusing worker threads; as well as checkpointing and restart.
* It should be noted that first-order schemes in the context of hyperbolicconservation laws require prohibitively many degrees of freedom to resolvecertain key features of the simulated fluid, and thus, typically only serveas elementary building blocks in higher-order schemes  @cite GuermondEtAl2018  . However, we hope that the reader still finds thetutorial step to be a good starting point (in particular with respect tothe programming techniques) before jumping into full research codes such asthe second-order scheme discussed in   @cite GuermondEtAl2018  .
* 
...
\rightarrow \mathbb{R}^{(d+2) \times d}$  , and   $d \geq 1$   is the spacedimension. We say that   $\mathbf{u} \in \mathbb{R}^{d+2}$   is the state and  $\mathbb{f}(\mathbf{u}) \in  \mathbb{R}^{(d+2) \times d}$   is the flux ofthe system. In the case of Euler's equations the state is given by  $\textbf{u} = [\rho, \textbf{m},E]^{\top}$  : where   $\rho \in \mathbb{R}^+$  denotes the density,   $\textbf{m} \in \mathbb{R}^d$   is the momentum, and   $E
...
\end{bmatrix},
@f}
* where   $\mathbb{I} \in \mathbb{R}^{d \times d}$   is the identity matrix and  $\otimes$   denotes the tensor product. Here, we have introduced the pressure  $p$   that, in general, is defined by a closed-form equation of state.In this tutorial we limit the discussion to the class of polytropicideal gases for which the pressure is given by
* @f{align*}
p = p(\textbf{u}) := (\gamma
* 
-1) \Big(E
...
\mathbf{u}_t + \text{div} \, \mathbb{f}(\mathbf{u}) = \boldsymbol{0},
@f}
* pose a significant challenge with respect to solution theory. An evidentobservation is that rewriting the equation in variational form and testing withthe solution itself does not lead to an energy estimate because the pairing  $\langle \text{div} \, \mathbb{f}(\mathbf{u}), \mathbf{u}\rangle$   (understood asthe   $L^2(\Omega)$   inner product or duality pairing) is not guaranteed to benon-negative. Notions such as energy-stability or   $L^2(\Omega)$  -stability are(in general) meaningless in this context.
* Historically, the most fruitful step taken in order to deepen theunderstanding of hyperbolic conservation laws was to assume that thesolution is formally defined as   $\mathbf{u} := \lim_{\epsilon \rightarrow
0^+} \mathbf{u}^{\epsilon}$   where   $\mathbf{u}^{\epsilon}$   is the solutionof the parabolic regularization
* @f{align}
\mathbf{u}_t^{\epsilon} + \text{div} \, \mathbb{f}(\mathbf{u}^{\epsilon})
* 
- {\epsilon} \Delta \mathbf{u}^{\epsilon} = 0.
@f}
* Such solutions, which are understood as the solution recovered in thezero-viscosity limit, are often referred to as <i>viscosity solutions</i>.(This is, because physically   $\epsilon$   can be understood as related to the viscosity of thefluid, i.e., a quantity that indicates the amount of friction neighboring gas particles moving atdifferent speeds exert on each other. The Euler equations themselves are derived underthe assumption of no friction, but can physically be expected to describe the limitingcase of vanishing friction or viscosity.)Global existence and uniqueness of such solutions is an open issue.However, we know at least that if such viscosity solutions exists they haveto satisfy the constraint   $\textbf{u}(\mathbf{x},t) \in \mathcal{B}$   forall   $\mathbf{x} \in \Omega$   and   $t \geq 0$   where
* @f{align}
...
* Following   Step-9  ,   Step-12  ,   Step-33  , and   Step-67  , at this point it might looktempting to base a discretization of Euler's equations on a (semi-discrete)variational formulation:
* @f{align*}
...
* Let   $\mathbb{V}_h$   be scalar-valued finite dimensional space spanned by abasis   $\{\phi_i\}_{i \in \mathcal{V}}$   where:   $\phi_i:\Omega \rightarrow
\mathbb{R}$   and   $\mathcal{V}$   is the set of all indices (nonnegativeintegers) identifying each scalar Degree of Freedom (DOF) in the mesh.Therefore a scalar finite element functional   $u_h \in \mathbb{V}_h$   canbe written as   $u_h = \sum_{i \in \mathcal{V}} U_i \phi_i$   with   $U_i \in
\mathbb{R}$  . We introduce the notation for vector-valued approximationspaces   $\pmb{\mathbb{V}}_h := \{\mathbb{V}_h\}^{d+2}$  . Let   $\mathbf{u}_h
...
\textbf{n})$   is far from trivial and we will postpone the precisedefinition in order to focus first on some algorithmic and implementationquestions. We note that
* 
...
* Consider the following pseudo-code, illustrating a possible straightforward strategy for computing the solution   $\textbf{U}^{n+1}$   at a newtime   $t_{n+1} = t_n + \tau_n$   given a known state   $\textbf{U}^{n}$   at time  $t_n$  :
* @f{align*}
...
@f}
* where   $0<c_{\text{cfl}}\le1$   is a chosen constant. This will require tocompute all   $d_{ij}$   in a separate step prior to actually performing aboveupdate. The core principle remains unchanged, though: we do not loop overcells but rather over all edges of the sparsity graph.
*   @note   It is not uncommon to encounter such fully-algebraic schemes (i.e.no bilinear forms, no cell loops, and no quadrature) outside of the finiteelement community in the wider CFD community. There is a rich history ofapplication of this kind of schemes, also called <i>edge-based</i> or<i>graph-based</i> finite element schemes (see for instance  @cite Rainald2008   for a historical overview). However, it is important tohighlight that the algebraic structure of the scheme (presented in thistutorial) and the node-loops are not just a performance gimmick. Actually, thestructure of this scheme was born out of theoretical necessity: the proof ofpointwise stability of the scheme hinges on the specific algebraic structure ofthe scheme. In addition, it is not possible to compute the algebraicviscosities   $d_{ij}$   using cell-loops since they depend nonlinearly oninformation that spans more than one cell (superposition does not hold: addingcontributions from separate cells does not lead to the right result).
* <a name="Stableboundaryconditionsandconservationproperties"></a><h3>Stable boundary conditions and conservation properties.</h3>
* 

* In the example considered in this tutorial step we use three different types ofboundary conditions: essential-like boundary conditions (we prescribe astate at the left boundary of our domain), outflow boundary conditions(also called "do-nothing" boundary conditions) at the right boundary of thedomain, and "reflecting" boundary conditions   $\mathbf{m} \cdot
\boldsymbol{\nu} = 0$   (also called "slip" boundary conditions) at the top,bottom, and surface of the obstacle. We will not discuss much aboutessential and "do-nothing" boundary conditions since their implementationis relatively easy and the reader will be able to pick-up theimplementation directly from the (documented) source code. In this portionof the introduction we will focus only on the "reflecting" boundaryconditions which are somewhat more tricky.
*   @note   At the time of this writing (early 2020) it is not unreasonable to saythat both analysis and implementation of stable boundary conditions forhyperbolic systems of conservation laws is an open issue. For the case ofvariational formulations, stable boundary conditions are those leading to awell-posed (coercive) bilinear form. But for general hyperbolicsystems of conservation laws (and for the algebraic formulation used in thistutorial) coercivity has no applicability and/or meaning as a notion ofstability. In this tutorial step we will use preservation of the invariant setas our main notion of stability which (at the very least) guaranteeswell-posedness of the discrete problem.
...
\mathcal{B}$   for all   $i \in \mathcal{V}$   when using either direct enforcement ofboundary conditions into the approximation space, or weak enforcement using theNitsche penalty method (which is for example widely used in discontinuousGalerkin schemes). In addition, some of these traditional ideas lead to quiterestrictive time step constraints.
* 
...
* If   $\mathbf{u}_t + \text{div} \, \mathbb{f}(\mathbf{u}) = \boldsymbol{0}$  represents Euler's equation with reflecting boundary conditions on the entiretyof the boundary (i.e.   $\partial\Omega^r \equiv \partial\Omega$  ) and weintegrate in space and time   $\int_{\Omega}\int_{t_1}^{t_2}$   we would obtain
* @f{align*}
...
\boldsymbol{(3)}
@f}
* where   $p_i$   is the pressure at the nodes that lie at the boundary. Clearly  $\boldsymbol{(3)}$   is the discrete counterpart of   $\boldsymbol{(2)}$  . Theproof of identity   $\boldsymbol{(3)}$   is omitted, but we briefly mention thatit hinges on the definition of the <i>nodal normal</i>  $\widehat{\boldsymbol{\nu}}_i$   provided in   $\boldsymbol{(1)}$  . We also note thatthis enforcement of reflecting boundary conditions is different from the oneoriginally advanced in   @cite GuermondEtAl2018  .
* 
...
* 
*  The set of include files is quite standard. The most intriguing part is the fact that we will rely solely on deal.II data structures for MPI parallelization, in particular   parallel::distributed::Triangulation   and   LinearAlgebra::distributed::Vector   included through   <code>distributed/tria.h</code>   and   <code>lac/la_parallel_vector.h</code>  . Instead of a Trilinos, or PETSc specific matrix class, we will use a non-distributed   dealii::SparseMatrix   (  <code>lac/sparse_matrix.h</code>  ) to store the local part of the   $\mathbf{c}_{ij}$  ,   $\mathbf{n}_{ij}$   and   $d_{ij}$   matrices.
* 
...
* 
*   @note   A cleaner approach would be to guard access to all data structures by <a
 href="https://en.wikipedia.org/wiki/Mutator_method">getter/setter
 functions</a>. For the sake of brevity, we refrain from that approach, though.
* 
...
* 
 @endcode
* 
*  We start with defining a number of   types::boundary_id   constants used throughout the tutorial step. This allows us to refer to boundary types by a mnemonic (such as   <code>do_nothing</code>  ) rather than a numerical value.
* 
...
*   The class   <code>Discretization</code>   contains all data structures concerning the mesh (triangulation) and discretization (mapping, finite element, quadrature) of the problem. As mentioned, we use the ParameterAcceptor class to automatically populate problem-specific parameters, such as the geometry information (  <code>length</code>  , etc.) or the refinement level (  <code>refinement</code>  ) from a parameter file. This requires us to split the initialization of data structures into two functions: We initialize everything that does not depend on parameters in the constructor, and defer the creation of the mesh to the   <code>setup()</code>   method that can be called once all parameters are read in via   ParameterAcceptor::initialize().  
* 
...
* 
 @endcode
* 
*   <a name="ThecodeOfflineDatacodeclass"></a>  <h4>The <code>OfflineData</code> class</h4>   
*   The class   <code>OfflineData</code>   contains pretty much all components of the discretization that do not evolve in time, in particular, the DoFHandler, SparsityPattern, boundary maps, the lumped mass matrix,   $\mathbf{c}_{ij}$   and   $\mathbf{n}_{ij}$   matrices. Here, the term <i>offline</i> refers to the fact that all the class members of   <code>OfflineData</code>   have well-defined values independent of the current time step. This means that they can be initialized ahead of time (at <i>time step zero</i>) and are not meant to be modified at any later time step. For instance, the sparsity pattern should not change as we advance in time (we are not doing any form of adaptivity in space). Similarly, the entries of the lumped mass matrix should not be modified as we advance in time either.   
*   We also compute and store a   <code>boundary_normal_map</code>   that contains a map from a global index of type   types::global_dof_index   of a boundary degree of freedom to a tuple consisting of a normal vector, the boundary id, and the position associated with the degree of freedom. We have to compute and store this geometric information in this class because we won't have access to geometric (or cell-based) information later on in the algebraic loops over the sparsity pattern.   
*  

* 
*   @note   Even though this class currently does not have any parameters that could be read in from a parameter file we nevertheless derive from ParameterAcceptor and follow the same idiom of providing a   <code>setup()</code> (and <code>assemble()</code>  ) method as for the class Discretization.
* 
...
*   The purpose of the class members   <code>component_names</code>  ,   <code>pressure</code>, and <code>speed_of_sound</code>   is evident from their names. We also provide a function   <code>compute_lambda_max()</code>  , that computes the wave speed estimate mentioned above,   $\lambda_{max}(\mathbf{U},\mathbf{V},\mathbf{n})$  , which is used in the computation of the   $d_{ij}$   matrix.   
*  

* 
*   @note   The   <code>DEAL_II_ALWAYS_INLINE</code>   macro expands to a (compiler specific) pragma that ensures that the corresponding function defined in this class is always inlined, i.e., the function body is put in place for every invocation of the function, and no call (and code indirection) is generated. This is stronger than the   <code>inline</code>   keyword, which is more or less a (mild) suggestion to the compiler that the programmer thinks it would be beneficial to inline the function.   <code>DEAL_II_ALWAYS_INLINE</code>   should only be used rarely and with caution in situations such as this one, where we actually know (due to benchmarking) that inlining the function in question improves performance.   
...
*   It would be desirable to initialize the class in a single shot: initialize/set the parameters and define the class members that depend on these default parameters. However, since we do not know the actual values for the parameters, this would be sort of meaningless and unsafe in general (we would like to have mechanisms to check the consistency of the input parameters). Instead of defining another   <code>setup()</code>   method to be called (by-hand) after the call to   ParameterAcceptor::initialize()   we provide an "implementation" for the class member   <code>parse_parameters_call_back()</code>   which is automatically called when invoking   ParameterAcceptor::initialize()   for every class that inherits from ParameterAceptor.
* 
...
*   With the   <code>OfflineData</code> and <code>ProblemDescription</code>   classes at hand we can now implement the explicit time-stepping scheme that was introduced in the discussion above. The main method of the   <code>%TimeStepping</code>   class is <code>make_one_step(vector_type &U, double t)</code> that takes a reference to a state vector   <code>U</code> and a time point <code>t</code>   (as input arguments) computes the updated solution, stores it in the vector   <code>temp</code>, swaps its contents with the vector <code>U</code>  , and returns the chosen   step-  size   $\tau$  .   
*   The other important method is   <code>prepare()</code>   which primarily sets the proper partition and sparsity pattern for the temporary vector   <code>temp</code> and the matrix <code>dij_matrix</code>   respectively.
* 
...
*   At its core, the Schlieren class implements the class member   <code>compute_schlieren()</code>  . The main purpose of this class member is to compute an auxiliary finite element field   <code>schlieren</code>  , that is defined at each node by \f[ \text{schlieren}[i] = e^{\beta \frac{ |\nabla r_i|
* 
...
- \min_j |\nabla r_j| }{\max_j |\nabla r_j|
* 
- \min_j |\nabla r_j| } }, \f] where   $r$   can in principle be any scalar quantity. In practice though, the density is a natural candidate, viz.   $r \dealcoloneq \rho$  . <a href="https://en.wikipedia.org/wiki/Schlieren">Schlieren</a> postprocessing is a standard method for enhancing the contrast of a visualization inspired by actual experimental X-ray and shadowgraphy techniques of visualization. (See   step-67   for another example where we create a Schlieren plot.)
* 
...
*   Now, all that is left to do is to chain the methods implemented in the   <code>%TimeStepping</code>, <code>InitialValues</code>  , and   <code>SchlierenPostprocessor</code>   classes together. We do this in a separate class   <code>MainLoop</code>   that contains an object of every class and again reads in a number of parameters with the help of the ParameterAcceptor class.
* 
...
* 
 @endcode
* 
*  Note that in the previous constructor we only passed the MPI communicator to the   <code>triangulation</code>   but we still have not initialized the underlying geometry/mesh. As mentioned earlier, we have to postpone this task to the   <code>setup()</code>   function that gets called after the   ParameterAcceptor::initialize()   function has populated all parameter variables with the final values read from the parameter file.   
*   The   <code>setup()</code>   function is the last class member that has to be implemented. It creates the actual triangulation that is a benchmark configuration consisting of a channel with a disk obstacle, see   @cite GuermondEtAl2018  . We construct the geometry by modifying the mesh generated by   GridGenerator::hyper_cube_with_cylindrical_hole().   We refer to   step-49  ,   step-53  , and   step-54   for an overview how to create advanced meshes. We first create 4 temporary (non distributed) coarse triangulations that we stitch together with the   GridGenerator::merge_triangulation()   function. We center the disk at   $(0,0)$   with a diameter of   <code>disk_diameter</code>  . The lower left corner of the channel has coordinates (  <code>-disk_position</code>, <code>-height/2</code>  ) and the upper right corner has (  <code>length-disk_position</code>  ,   <code>height/2</code>  ).
* 
...
* 
 @endcode
* 
*  We have to fix up the left edge that is currently located at   $x=-$    <code>disk_diameter</code>   and has to be shifted to   $x=-$    <code>disk_position</code>  . As a last step the boundary has to be colorized with   <code>Boundaries::do_nothing</code>   on the right,   <code>dirichlet</code> on the left and <code>free_slip</code>   on the upper and lower outer boundaries and the obstacle.
* 
...
* 
*  Not much is done in the constructor of   <code>OfflineData</code>   other than initializing the corresponding class members in the initialization list.
* 
...
* 
 @endcode
* 
*  We have to create the "local" sparsity pattern by hand. We therefore loop over all locally owned and ghosted cells (see   @ref   GlossArtificialCell) and extract the (global)   <code>dof_indices</code>   associated with the cell DOFs and renumber them using   <code>partitioner->global_to_local(index)</code>  .       
*  

* 
*   @note   In the case of a locally owned dof, such renumbering consist of applying a shift (i.e. we subtract an offset) such that now they will become a number in the integer interval   $[0,$    <code>n_locally_owned</code>    $)$  . However, in the case of a ghosted dof (i.e. not locally owned) the situation is quite different, since the global indices associated with ghosted DOFs will not be (in general) a contiguous set of integers.
* 
...
* @code
 ...
 @endcode
* 
*    <code>CopyData</code>   class that will be used to assemble the offline data matrices using WorkStream. It acts as a container: it is just a struct where WorkStream stores the local cell contributions. Note that it also contains a class member   <code>local_boundary_normal_map</code>   used to store the local contributions required to compute the normals at the boundary.
* 
...
* 
 @endcode
* 
*  Next we introduce a number of helper functions that are all concerned about reading and writing matrix and vector entries. They are mainly motivated by providing slightly more efficient code and <a href="https://en.wikipedia.org/wiki/Syntactic_sugar"> syntactic
 sugar</a> for otherwise somewhat tedious code.
* 

* 
*  The first function we introduce,   <code>get_entry()</code>  , will be used to read the value stored at the entry pointed by a SparsityPattern iterator   <code>it</code> of <code>matrix</code>  . The function works around a small deficiency in the SparseMatrix interface: The SparsityPattern is concerned with all index operations of the sparse matrix stored in CRS format. As such the iterator already knows the global index of the corresponding matrix entry in the low-level vector stored in the SparseMatrix object. Due to the lack of an interface in the SparseMatrix for accessing the element directly with a SparsityPattern iterator, we unfortunately have to create a temporary SparseMatrix iterator. We simply hide this in the   <code>get_entry()</code>   function.
* 
...
* 
 @endcode
* 
*  The   <code>set_entry()</code>   helper is the inverse operation of   <code>get_value()</code>  : Given an iterator and a value, it sets the entry pointed to by the iterator in the matrix.
* 
...
* 
 @endcode
* 
*    <code>gather()</code>   (first interface): this first function signature, having three input arguments, will be used to retrieve the individual components   <code>(i,l)</code>   of a matrix. The functionality of   <code>gather_get_entry()</code>   and   <code>gather()</code>   is very much the same, but their context is different: the function   <code>gather()</code>   does not rely on an iterator (that actually knows the value pointed to) but rather on the indices   <code>(i,l)</code>   of the entry in order to retrieve its actual value. We should expect   <code>gather()</code>   to be slightly more expensive than   <code>gather_get_entry()</code>  . The use of   <code>gather()</code>   will be limited to the task of computing the algebraic viscosity   $d_{ij}$   in the particular case that when both   $i$   and   $j$   lie at the boundary.     
*  

* 
*   @note   The reader should be aware that accessing an arbitrary   <code>(i,l)</code>   entry of a matrix (say for instance Trilinos or PETSc matrices) is in general unacceptably expensive. Here is where we might want to keep an eye on complexity: we want this operation to have constant complexity, which is the case of the current implementation using deal.II matrices.
* 
...
* 
 @endcode
* 
*    <code>gather()</code>   (second interface): this second function signature having two input arguments will be used to gather the state at a node   <code>i</code>   and return it as a   <code>Tensor<1,problem_dimension></code>   for our convenience.
* 
...
* 
 @endcode
* 
*    <code>scatter()</code>  : this function has three input arguments, the first one is meant to be a "global object" (say a locally owned or locally relevant vector), the second argument which could be a   <code>Tensor<1,problem_dimension></code>  , and the last argument which represents a index of the global object. This function will be primarily used to write the updated nodal values, stored as   <code>Tensor<1,problem_dimension></code>  , into the global objects.
* 
...
* 
 @endcode
* 
*  We are now in a position to assemble all matrices stored in   <code>OfflineData</code>  : the lumped mass entries   $m_i$  , the vector-valued matrices   $\mathbf{c}_{ij}$   and   $\mathbf{n}_{ij} =
 \frac{\mathbf{c}_{ij}}{|\mathbf{c}_{ij}|}$  , and the boundary normals   $\boldsymbol{\nu}_i$  .   
*   In order to exploit thread parallelization we use the WorkStream approach detailed in the   @ref threads   "Parallel computing with multiple processors" accessing shared memory. As customary this requires definition of
* 
...
* @f{align*}
 ...
 @f}
*    
*   We will compute the numerator of this expression first and store it in   <code>OfflineData<dim>::BoundaryNormalMap</code>  . We will normalize these vectors in a posterior loop.
* 
...
*   A minor caveat here is that the iterators   <code>indices.begin()</code>   and   <code>indices.end()</code>   supplied to   parallel::apply_to_subranges()   have to be random access iterators: internally,   parallel::apply_to_subranges()   will break the range defined by the   <code>indices.begin()</code>   and   <code>indices.end()</code>   iterators into subranges (we want to be able to read any entry in those subranges with constant complexity). In order to provide such iterators we resort to   std_cxx20::ranges::iota_view.       
*   The bulk of the following piece of code is spent defining the "worker"   <code>on_subranges</code>  : i.e. the  operation applied at each row of the sub-range. Given a fixed   <code>row_index</code>   we want to visit every column/entry in such row. In order to execute such columns-loops we use <a href="http://www.cplusplus.com/reference/algorithm/for_each/">
...
* 
 @endcode
* 
*  Finally, we normalize the vectors stored in   <code>OfflineData<dim>::BoundaryNormalMap</code>  . This operation has not been thread parallelized as it would neither illustrate any important concept nor lead to any noticeable speed gain.
* 
...
* 
*  In this section we describe the implementation of the class members of the   <code>ProblemDescription</code>   class. Most of the code here is specific to the compressible Euler's equations with an ideal gas law. If we wanted to re-purpose   step-69   for a different conservation law (say for: instance the shallow water equation) most of the implementation of this class would have to change. But most of the other classes (in particular those defining loop structures) would remain unchanged.   
*   We start by implementing a number of small member functions for computing   <code>momentum</code>, <code>internal_energy</code>  ,   <code>pressure</code>, <code>speed_of_sound</code>  , and the flux   <code>f</code>   of the system. The functionality of each one of these functions is self-explanatory from their names.
* 
...
*   In general, obtaining a sharp guaranteed upper-bound on the maximum wavespeed requires solving a quite expensive scalar nonlinear problem. This is typically done with an iterative solver. In order to simplify the presentation in this example step we decided not to include such an iterative scheme. Instead, we will just use an initial guess as a guess for an upper bound on the maximum wavespeed. More precisely, equations (2.11) (3.7), (3.8) and (4.3) of   @cite GuermondPopov2016b   are enough to define a guaranteed upper bound on the maximum wavespeed. This estimate is returned by a call to the function   <code>lambda_max_two_rarefaction()</code>  . At its core the construction of such an upper bound uses the so-called two-rarefaction approximation for the intermediate pressure   $p^*$  , see for instance Equation (4.46), page 128 in   @cite Toro2009  .   
*   The estimate returned by   <code>lambda_max_two_rarefaction()</code>   is guaranteed to be an upper bound, it is in general quite sharp, and overall sufficient for our purposes. However, for some specific situations (in particular when one of states is close to vacuum conditions) such an estimate will be overly pessimistic. That's why we used a second estimate to avoid this degeneracy that will be invoked by a call to the function   <code>lambda_max_expansion()</code>  . The most important function here is   <code>compute_lambda_max()</code>   which takes the minimum between the estimates returned by   <code>lambda_max_two_rarefaction()</code>   and   <code>lambda_max_expansion()</code>  .   
*   We start again by defining a couple of helper functions:   
*   The first function takes a state   <code>U</code>   and a unit vector   <code>n_ij</code>   and computes the <i>projected</i> 1D state in direction of the unit vector.
* 
...
* 
 @endcode
* 
*  We return the 1D state in <i>primitive</i> variables instead of conserved quantities. The return array consists of density   $\rho$  , velocity   $u$  , pressure   $p$   and local speed of sound   $a$  :
* 
...
* 
 @endcode
* 
*  Next, we need two local wavenumbers that are defined in terms of a primitive state   $[\rho, u, p, a]$   and a given pressure   $p^\ast$     @cite GuermondPopov2016    Eqn. (3.7):
...
* 
 @endcode
* 
*  All that is left to do is to compute the maximum of   $\lambda^-$   and   $\lambda^+$   computed from the left and right primitive state (  @cite GuermondPopov2016   Eqn. (2.11)), where   $p^\ast$   is given by   @cite GuermondPopov2016   Eqn (4.3):
* 
...
* 
*   @note   The constant 5.0 multiplying the maximum of the sound speeds is <i>neither</i> an ad-hoc constant, <i>nor</i> a tuning parameter. It defines an upper bound for any   $\gamma \in [0,5/3]$  . Do not play with it!
* 
...
* 
 @endcode
* 
*  We conclude this section by defining static arrays   <code>component_names</code>   that contain strings describing the component names of our state vector. We have template specializations for dimensions one, two and three, that are used later in DataOut for naming the corresponding components:
* 
...
*   In the constructor we initialize all parameters with default values, declare all parameters for the `ParameterAcceptor` class and connect the   <code>parse_parameters_call_back</code>   slot to the respective signal.   
*   The   <code>parse_parameters_call_back</code>   slot will be invoked from ParameterAceptor after the call to   ParameterAcceptor::initialize().   In that regard, its use is appropriate for situations where the parameters have to be postprocessed (in some sense) or some consistency condition between the parameters has to be checked.
* 
...
* 
 @endcode
* 
*  So far the constructor of   <code>InitialValues</code>   has defined default values for the two private members   <code>initial_direction</code> and <code>initial_1d_state</code>   and added them to the parameter list. But we have not defined an implementation of the only public member that we really care about, which is   <code>initial_state()</code>   (the function that we are going to call to actually evaluate the initial solution at the mesh nodes). At the top of the function, we have to ensure that the provided initial direction is not the zero vector.   
*  

* 
*   @note   As commented, we could have avoided using the method   <code>parse_parameters_call_back </code>   and defined a class member   <code>setup()</code>   in order to define the implementation of   <code>initial_state()</code>  . But for illustrative purposes we want to document a different way here and use the call back signal from ParameterAcceptor.
* 
...
* 
 @endcode
* 
*  Next, we implement the   <code>initial_state</code>   function object with a lambda function computing a uniform flow field. For this we have to translate a given primitive 1d state (density   $\rho$  , velocity   $u$  , and pressure   $p$  ) into a conserved n-dimensional state (density   $\rho$  , momentum   $\mathbf{m}$  , and total energy   $E$  ).
* 
...
* 
*  The constructor of the   <code>%TimeStepping</code>   class does not contain any surprising code:
* 
...
* 
 @endcode
* 
*  In the class member   <code>prepare()</code>   we initialize the temporary vector   <code>temp</code> and the matrix <code>dij_matrix</code>  . The vector   <code>temp</code>   will be used to store the solution update temporarily before its contents is swapped with the old vector.
* 
...
* 
 @endcode
* 
*  It is now time to implement the forward Euler step. Given a (writable reference) to the old state   <code>U</code>   at time   $t$   we update the state   <code>U</code>   in place and return the chosen time-step size. We first declare a number of read-only references to various different variables and data structures. We do this is mainly to have shorter variable names (e.g.,   <code>sparsity</code>   instead of   <code>offline_data->sparsity_pattern</code>  ).
* 
...
* 
-
 \int_{\Omega} \nabla \phi_i \phi_j \, \mathrm{d}\mathbf{x}$   (or equivalently   $\mathbf{c}_{ij} =
* 
- \mathbf{c}_{ji}$  ) provided either   $\mathbf{x}_i$   or   $\mathbf{x}_j$   is a support point located away from the boundary. In this case we can check that   $\lambda_{\text{max}} (\mathbf{U}_i^{n}, \mathbf{U}_j^{n},
...
*   We use again   parallel::apply_to_subranges()   for thread-parallel for loops. Pretty much all the ideas for parallel traversal that we introduced when discussing the assembly of the matrix   <code>norm_matrix</code>   and the normalization of   <code>nij_matrix</code>   above are used here again.     
*   We define again a "worker" function   <code>on_subranges</code>   that computes the viscosity   $d_{ij}$   for a subrange [i1, i2) of column indices:
* 
...
* 
 @endcode
* 
*  For a given column index i we iterate over the columns of the sparsity pattern from   <code>sparsity.begin(i)</code>   to   <code>sparsity.end(i)</code>  :
* 
...
* 
 @endcode
* 
*  <b>Step 2</b>: Compute diagonal entries   $d_{ii}$   and   $\tau_{\text{max}}$  .
* 

* 
*  So far we have computed all off-diagonal entries of the matrix   <code>dij_matrix</code>  . We still have to fill its diagonal entries defined as   $d_{ii}^n =
* 
- \sum_{j \in \mathcal{I}(i)\backslash \{i\}}
 d_{ij}^n$  . We use again   parallel::apply_to_subranges()   for this purpose. While computing the   $d_{ii}$  s we also determine the largest admissible time-step, which is defined as \f[
 \tau_n \dealcoloneq c_{\text{cfl}}\,\min_{i\in\mathcal{V}}
 \left(\frac{m_i}{-2\,d_{ii}^{n}}\right) \, .
 \f] Note that the operation   $\min_{i \in \mathcal{V}}$   is intrinsically global, it operates on all nodes: first we have to take the minimum over all threads (of a given node) and then we have to take the minimum over all MPI processes. In the current implementation:
* 

* 
* 
*  - We store    <code>tau_max</code>   (per node) as <a
 href="http://www.cplusplus.com/reference/atomic/atomic/"><code>std::atomic<double></code></a>. The internal implementation of   <code>std::atomic</code>   will take care of guarding any possible race condition when more than one thread attempts to read and/or write   <code>tau_max</code>   at the same time.
* 
...
* 
 @endcode
* 
*  on_subranges() will be executed on every thread individually. The variable   <code>tau_max_on_subrange</code>   is thus stored thread locally.
* 
...
* 
 @endcode
* 
*    <code>tau_max_on_subrange</code>   contains the largest possible time-step size computed for the (thread local) subrange. At this point we have to synchronize the value over all threads. This is were we use the <a
 href="http://www.cplusplus.com/reference/atomic/atomic/"><code>std::atomic<double></code></a> <i>compare exchange</i> update mechanism:
* 
...
* 
 @endcode
* 
*  This is a good point to verify that the computed   <code>tau_max</code>   is indeed a valid floating point number.
* 
...
*   Here, we compute the correction \f[
 \mathbf{m}_i \dealcoloneq \mathbf{m}_i
* 
- (\boldsymbol{\nu}_i \cdot
 \mathbf{m}_i) \boldsymbol{\nu}_i,
 \f] which removes the normal component of   $\mathbf{m}$  .
* 
...
* 
 @endcode
* 
*  <b>Step 5</b>: We now update the ghost layer over all MPI ranks, swap the temporary vector with the solution vector   <code>U</code>   (that will get returned by reference) and return the chosen time-step size   $\tau_{\text{max}}$  :
* 
...
*   At various intervals we will output the current state   <code>U</code>   of the solution together with a so-called Schlieren plot. The constructor of the   <code>SchlierenPostprocessor</code>   class again contains no surprises. We simply supply default values to and register two parameters:
* 
...
* 
 @endcode
* 
*  Again, the   <code>prepare()</code>   function initializes two temporary the vectors (  <code>r</code> and <code>schlieren</code>  ).
* 
...
* 
 @endcode
* 
*  We now discuss the implementation of the class member   <code>SchlierenPostprocessor<dim>::compute_schlieren()</code>  , which basically takes a component of the state vector   <code>U</code>   and computes the Schlieren indicator for such component (the formula of the Schlieren indicator can be found just before the declaration of the class   <code>SchlierenPostprocessor</code>  ). We start by noting that this formula requires the "nodal gradients"   $\nabla r_j$  . However, nodal values of gradients are not defined for   $\mathcal{C}^0$   finite element functions. More generally, pointwise values of gradients are not defined for   $W^{1,p}(\Omega)$   functions. The simplest technique we can use to recover gradients at nodes is weighted-averaging i.e.   
...
*   The second thing to note is that we have to compute global minimum and maximum   $\max_j |\nabla r_j|$   and   $\min_j |\nabla r_j|$  . Following the same ideas used to compute the time step size in the class member   <code>%TimeStepping\<dim>::%step()</code>   we define   $\max_j |\nabla r_j|$   and   $\min_j |\nabla r_j|$   as atomic doubles in order to resolve any conflicts between threads. As usual, we use   <code>Utilities::MPI::max()</code>   and   <code>Utilities::MPI::min()</code>   to find the global maximum/minimum among all MPI processes.   
...
- \min_j |\nabla r_j| } }
 \, . \f]   
*   This means that we will have to define two workers   <code>on_subranges</code>   for each one of these stages.
* 
...
* 
 @endcode
* 
*  And synchronize   <code>r_i_max</code> and <code>r_i_min</code>   over all MPI processes.
* 
...
* 
 @endcode
* 
*  Second loop: we now have the vector   <code>r</code>   and the scalars   <code>r_i_max</code> and <code>r_i_min</code>   at our disposal. We are thus in a position to actually compute the Schlieren indicator.
* 
...
*   With all classes implemented it is time to create an instance of   <code>Discretization<dim></code>, <code>OfflineData<dim></code>  ,   <code>InitialValues<dim></code>, <code>%TimeStepping\<dim></code>  , and   <code>SchlierenPostprocessor<dim></code>  , and run the forward Euler step in a loop.   
*   In the constructor of   <code>MainLoop<dim></code>   we now initialize an instance of all classes, and declare a number of parameters controlling output. Most notable, we declare a boolean parameter   <code>resume</code>   that will control whether the program attempts to restart from an interrupted computation, or not.
* 
...
* 
 @endcode
* 
*  We start by implementing a helper function   <code>print_head()</code>   in an anonymous namespace that is used to output messages in the terminal with some nice formatting.
* 
...
* 
 @endcode
* 
*  With   <code>print_head</code>   in place it is now time to implement the   <code>MainLoop<dim>::run()</code>   that contains the main loop of our program.
* 
...
* 
 @endcode
* 
*  We will store the current time and state in the variable   <code>t</code> and vector <code>U</code>  :
* 
...
*   By default the boolean   <code>resume</code>   is set to false, i.e. the following code snippet is not run. However, if   <code>resume==true</code>   we indicate that we have indeed an interrupted computation and the program shall restart by reading in an old state consisting of   <code>t</code>  ,   <code>output_cycle</code>, and <code>U</code>   from a checkpoint file. These checkpoint files will be created in the   <code>output()</code>   routine discussed below.
* 
...
* 
 @endcode
* 
*  We use a   <code>boost::archive</code>   to store and read in the contents the checkpointed state.
* 
...
* 
 ...
 @endcode
* 
*    <code>it1</code>   iterates over all components of the state vector   <code>U</code>  . We read in every entry of the component in sequence and update the ghost layer afterwards:
* 
...
* 
 @endcode
* 
*  and then perform a single forward Euler step. Note that the state vector   <code>U</code>   is updated in place and that   <code>time_stepping.make_one_step()</code>   returns the chosen step size.
* 
...
* 
 @endcode
* 
*  The   <code>interpolate_initial_values</code>   takes an initial time "t" as input argument and populates a state vector   <code>U</code>   with the help of the   <code>InitialValues<dim>::initial_state</code>   object.
* 
...
* 
 @endcode
* 
*  The function signature of   <code>InitialValues<dim>::initial_state</code>   is not quite right for   VectorTools::interpolate().   We work around this issue by, first, creating a lambda function that for a given position   <code>x</code>   returns just the value of the   <code>i</code>  th component. This lambda in turn is converted to a   dealii::Function   with the help of the ScalarFunctionFromFunctionObject wrapper.
* 
...
*   Writing out the final vtk files is quite an IO intensive task that can stall the main loop for a while. In order to avoid this we use an <a
 href="https://en.wikipedia.org/wiki/Asynchronous_I/O">asynchronous
 IO</a> strategy by creating a background thread that will perform IO while the main loop is allowed to continue. In order for this to work we have to be mindful of two things:
* 
...
* 
 @endcode
* 
*  If the asynchronous writeback option is set we launch a background thread performing all the slow IO to disc. In that case we have to make sure that the background thread actually finished running. If not, we have to wait to for it to finish. We launch said background thread with <a
 href="https://en.cppreference.com/w/cpp/thread/async"><code>std::async()</code></a> that returns a <a
 href="https://en.cppreference.com/w/cpp/thread/future"><code>std::future</code></a> object. This   <code>std::future</code>   object contains the return value of the function, which is in our case simply   <code>void</code>  .
* 
...
*  Next we create a lambda function for the background thread. We <a
 href="https://en.cppreference.com/w/cpp/language/lambda">capture</a> the   <code>this</code>   pointer as well as most of the arguments of the output function by value so that we have access to them inside the lambda function.
* 
...
* @code
 ...
 @endcode
* 
*  We checkpoint the current state by doing the precise inverse operation to what we discussed for the <a href="Resume">resume
 logic</a>:
* 
...
* 
 @endcode
* 
*  If the asynchronous writeback option is set we launch a new background thread with the help of <a
 href="https://en.cppreference.com/w/cpp/thread/async"><code>std::async</code></a> function. The function returns a <a
 href="https://en.cppreference.com/w/cpp/thread/future"><code>std::future</code></a> object that we can use to query the status of the background thread. At this point we can return from the   <code>output()</code>   function and resume with the time stepping in the main loop
* 
...
@endverbatim
* 
* One thing that becomes evident is the fact that the program spends twothirds of the execution time computing the graph viscosity d_ij and about athird of the execution time in performing the update, where computing theflux   $f(U)$   is the expensive operation. The preset default resolution isabout 37k gridpoints, which amounts to about 148k spatial degrees offreedom in 2D. An animated schlieren plot of the solution looks as follows:
*   <img src="https://www.dealii.org/images/steps/developer/step-69.coarse.gif" alt="" height="300">  
* It is evident that 37k gridpoints for the first-order method is nowherenear the resolution needed to resolve any flow features. For comparison,here is a "reference" computation with a second-order method and about 9.5Mgridpoints (<a href="https://github.com/conservation-laws/ryujin">github
project page</a>):
...
* This can be fixed, but it would exceed what atutorial* is about.Nevertheless, it is worth showing what one can achieve by adding asecond-order scheme. For example, here is a video computed with <a
href=https://conservation-laws.43-1.org/>the following research code</a>that shows (with a different color scheme) a 2d simulation that correspondsto the cases shown above:
* @htmlonly
<p align="center">
...
@endhtmlonly
* 
* This simulation was done with 38 million degrees of freedom(continuous   $Q_1$   finite elements) per component of the solutionvector. The exquisite detail of the solution is remarkable for thesekinds of simulations, including in the sub-sonic region behind theobstacle.
