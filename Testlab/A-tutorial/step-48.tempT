...
* 本程序演示了如何使用MatrixFree类的基于单元的有限元算子实现，该类首次在 step-37 中介绍，用来解决非线性偏微分方程。最后，我们将使用明确的时间步进方法来解决问题，并介绍高斯-洛巴特有限元，在这种情况下非常方便，因为它们的质量矩阵可以准确地由对角线接近，因此是可逆的矩阵。这一特性的两个要素是：首先，根据Gauss-Lobatto正交规则的点分布，对拉格朗日多项式的结点进行分布。其次，正交是用同样的Gauss-Lobatto正交规则完成的。在这个公式中，只要 $i\neq j$ ，积分 $\int_K \varphi_i \varphi_j
...
* MatrixFree类有三个层次的并行化选项：分布式节点集群上的MPI并行化，由线程积木库安排的线程并行化，以及最后通过SIMD数据类型在两个（或更多）单元的批次上工作的矢量化（有时称为交叉元素或外部矢量化）。 正如我们在 step-37 中所讨论的，你将通过使用特定于你的系统的指令集来获得最佳性能，例如，使用cmake变量<tt>-DCMAKE_CXX_FLAGS="-march=native"</tt>。在  step-37  中已经利用了MPI并行化。在这里，我们额外考虑了用TBB进行线程并行化。这相当简单，因为我们需要做的就是告诉MatrixFree对象的初始化，我们想通过变量 MatrixFree::AdditionalData::thread_parallel_scheme. 使用线程并行方案。 在设置过程中，建立了一个类似于 @ref workstream_paper 中描述的依赖图，这允许安排 @p local_apply 函数在单元块上的工作，而没有几个线程访问同一个向量索引。相对于WorkStream循环，一些额外的巧妙技巧也被应用，以避免<a
...
\f}，并在时间区间[-10,10]内解决方程。常数被选择为 $c_1=c_1=0$ 和<i> m=0.5</i>。正如在 step-25 中提到的，在一维中<i>u</i>作为<i>t</i>的函数是正弦-戈登方程的精确解。对于更高的维度，情况并非如此。
* 
...
* 
*  <code>SineGordonOperation</code> 类实现了每个时间步骤中需要的基于单元的操作。这个非线性操作可以在 <code>MatrixFree</code> 类的基础上直接实现，与线性操作在这个实现的有限元算子应用中的处理方式相同。我们对该类应用两个模板参数，一个是尺寸，一个是有限元的度数。这与deal.II中的其他函数不同，其中只有维度是模板参数。这对于向 @p FEEvaluation 中的内循环提供关于循环长度等的信息是必要的，这对于效率是至关重要的。另一方面，这使得将度数作为一个运行时参数来实现更具挑战性。
* 
...
* 
* 这个算子实现了程序的核心操作，即对Sine-Gordon问题的非线性算子进行单元范围的积分。其实现是基于  step-37  中的FEEvaluation类。由于Gauss-Lobatto元素的特殊结构，某些操作变得更加简单，特别是正交点上的形状函数值的评估，这只是单元自由度值的注入。MatrixFree类在初始化时检测正交点的有限元的可能结构，然后由FEEvaluation自动用于选择最合适的数值核。
* 

* 
* 我们必须为时间步进例程评估的非线性函数包括当前时间的函数值 @p current 以及前一个时间步进的值 @p old. 这两个值都在源向量集合 @p src, 中传递给运算器，该集合只是一个指向实际解向量的指针 <tt>std::vector</tt> 。这种将多个源向量收集到一起的结构是必要的，因为 @p MatrixFree 中的单元格循环正好需要一个源向量和一个目的向量，即使我们碰巧使用许多向量，如本例中的两个。请注意，单元格循环接受任何有效的输入和输出类，这不仅包括向量，还包括一般的数据类型。 然而，只有在遇到收集这些向量的 LinearAlgebra::distributed::Vector<Number> 或 <tt>std::vector</tt> 时，它才会在循环的开始和结束时调用由于MPI而交换幽灵数据的函数。在单元格的循环中，我们首先要读入与本地值有关的向量中的值。 然后，我们评估当前求解向量的值和梯度以及正交点的旧向量的值。接下来，我们在正交点的循环中结合方案中的条款。最后，我们将结果与测试函数进行积分，并将结果累积到全局解向量 @p  dst。
* 
...
* 
* 我们定义了一个随时间变化的函数，作为初始值使用。通过改变起始时间可以得到不同的解决方案。这个函数取自 step-25 ，将代表一维中所有时间的分析解，但在这里只是用来设置一些感兴趣的起始解。在  step-25  中给出了可以测试这个程序的收敛性的更详细的选择。
* 
...
* 
* 如同在 step-25 中，这个函数在 <code>dim</code> 维度上设置了一个范围为 $[-15,15]$ 的立方体网格。我们在域的中心位置更多的细化网格，因为解决方案集中在那里。我们首先细化所有中心在半径为11的单元，然后再细化一次半径为6的单元。 这种简单的临时细化可以通过在时间步进过程中使用误差估计器使网格与解相适应来完成，就像在其他例子程序中所做的那样，并使用 parallel::distributed::SolutionTransfer 来将解转移到新的网格中。
* 
...
* 
* 然后，我们继续将初始状态写入文件，并在 <tt>std::vector</tt> 的指针中收集两个起始解，这些指针后来被 SineGordonOperation::apply() 函数所消耗。接下来，根据文件顶部指定的有限元程度，建立一个 <code> SineGordonOperation class </code> 的实例。
* 
...
* 
* 与 step-40 中一样，我们在程序开始时初始化MPI。由于我们一般会将MPI并行化与线程混合在一起，所以我们也将MPI_InitFinalize中控制线程数量的第三个参数设置为无效数字，这意味着TBB库会自动选择线程的数量，通常是根据系统中可用的核心数量。作为一种选择，如果你想设置一个特定的线程数（例如，当需要只使用MPI时），你也可以手动设置这个数字。
* 
...
* 为了证明使用MatrixFree类而不是标准的 <code>deal.II</code> 汇编例程来评估旧的时间步骤的信息的好处，我们研究了一个简单的在非适应性网格上的串行运行的代码。由于许多时间花在评估正弦函数上，我们不仅显示了完整的正弦-戈登方程的数字，还显示了波浪方程（正弦-戈登方程中跳过的正弦项）的数字。我们同时使用二阶和四阶元素。结果总结在以下表格中。
...
* 在这个程序中，有几处可以改进，使其更加有效（除了在 step-25 中讨论的改进边界条件和物理材料）。
...
*  <li>  <b>Higher order time stepping:</b> 虽然实现允许空间部分的任意顺序（通过调整有限元的程度），但时间步进方案是标准的二阶跃迁方案。由于波的传播问题的解通常是非常平滑的，所以误差很可能被时间步进部分所支配。当然，这可以通过使用较小的时间步长（在固定的空间分辨率下）来解决，但使用高阶时间步长也会更有效率。虽然对于一阶系统来说，这样做是很简单的（使用一些高阶的Runge&ndash;Kutta方案，可能结合像<a
  ...
  method</a>那样的自适应时间步长选择），但对于二阶公式来说，这更具挑战性。至少在有限差分界，人们通常用PDE来寻找改善时间误差的空间修正项。
*  </ul>  
* 
