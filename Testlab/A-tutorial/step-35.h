 ...
 * 本教程依赖于  step-22  。 @htmlonly <table class="tutorial"
 ...
 * \nabla \cdot u = 0,
 * @f}
 * 其中 $u$ 表示流速， $p$ 表示压力。这个方程组由初始条件@f[
 * u |_{t=0} = u_0,
 * @f]与 $u_0$ 充分光滑和螺线管，以及合适的边界条件补充而成。例如，一个可接受的边界条件是@f[
 * u|_{\partial\Omega} = u_b.
 * @f]，也可以规定其他边界条件。在我们这里解决的测试案例中，边界被划分为两个不相交的子集 $\partial\Omega = \Gamma_1 \cup \Gamma_2$ ，我们有@f[
 * u|_{\Gamma_1} = u_b,
 * @f]和@f[
 * u\times n|_{\Gamma_2} = 0, \quad p|_{\Gamma_2} = 0 @f]，其中 $n$
 * 是外部单位法线。 $\Gamma_2$
 * 上的边界条件经常被用来模拟外流条件。
 * 在以前的教程中（例如见 step-20 和 step-22
 * ），我们已经看到了如何使用Schur补数方法来解决时间无关的斯托克斯方程。对于时间依赖的情况，在时间离散化之后，我们将得到一个系统，如
 * @f{align*}
 * \frac1\tau u^k
 *
 * - \nu \Delta u^k + \nabla p^k = F^k, \\
 ...
 * @f}
 * 其中 $\tau$
 * 是时间步长。尽管这个系统的结构与斯托克斯系统相似，因此可以用Schur补数的方法来解决，但事实证明，Schur补数的条件数与
 * $\tau^{-2}$
 * 成正比。这使得该系统非常难以解决，并意味着对于纳维-斯托克斯方程来说，这不是一个有用的解决途径。
 * <a name="Projection"></a><a name="Projectionmethods"></a><h3> Projection
 * methods </h3> 。
 *
 * 相反，我们需要想出一个不同的方法来解决与时间有关的纳维-斯托克斯方程。解决这些问题的困难来自于速度和压力通过约束@f[
 * \nabla \cdot u = 0,
 * @f]耦合在一起的事实，对于这个约束，压力是拉格朗日乘数。
 * 让我们简短地描述一下投影方法在半离散情况下的样子。我们的目标是获得一个速度 $\{u^k\}$ 和压力 $\{p^k\}$ 的序列。假设从初始条件和一阶方法的应用中我们已经找到了 $(u^0,p^0,\phi^0=0)$ 和 $(u^1,p^1,\phi^1=p^1-p^0)$ 。那么投影法包括以下步骤。 <ul>   <li>  <b>Step 0</b>。外推法。定义一下。 @f[
 * u^\star = 2u^k
 *
 * - u^{k-1}, \quad p^\sharp = p^k + \frac43 \phi^k
 *
 * - \frac13 \phi^{k-1}.
 * @f]  <li>  <b>Step 1</b>。扩散步骤。我们发现 $u^{k+1}$
 * 可以解决单一线性方程@f[ \frac1{2\tau}\left( 3u^{k+1}
 *
 ...
 * <li>  <b>Step 2</b>。投影。找到解决@f[ \Delta \phi^{k+1} =
 * \frac3{2\tau} \nabla \cdot u^{k+1}, \quad \partial_n \phi^{k+1}|_{\Gamma_1}
 * = 0, \quad \phi^{k+1}|_{\Gamma_2} = 0
 * @f]  <li>  <b>Step 3</b>的 $\phi^{k+1}$ 。压力校正。这里我们有两个选项。     <ul>   <li>  <i>Incremental Method in Standard Form</i>。压力的更新方式是。     @f[
 * p^{k+1} = p^k + \phi^{k+1}.
 * @f]  <li>  <i>Incremental Method in Rotational
 * Form</i>。在这种情况下@f[ p^{k+1} = p^k + \phi^{k+1}
 *
 * - \nu \nabla \cdot u^{k+1}.
 * @f]  </ul>   </ul>  。
 ...
 * u \cdot \nabla u + \frac12 \left( \nabla\cdot u \right) u.
 * @f]取代，这与连续方程一致（因为 $\nabla\cdot u = 0$
 * ，尽管这对离散解来说不是点状的），并且需要它来保证时间步进方案的无条件稳定性。此外，为了使该术语线性化，我们使用
 * $u^{k+1}$ 的二阶外推法 $u^\star$  。   <li>
 * 投影步骤是亥姆霍兹分解@f[ L^2(\Omega)^d = H \oplus \nabla
 * H^1_{\Gamma_2}(\Omega),
 * @f]的实现，其中@f[
 ...
 * \right\}.
 * @f] 事实上，如果我们在 $u^{k+1}$ 上使用这一分解，我们得到@f[
 * u^{k+1} = v^{k+1} + \nabla \left( \frac{2\tau}{3}  \phi^{k+1} \right),
 * @f]与 $v^{k+1}\in H$  。取这个方程的发散，我们得出了投影方程。   <li>  上述两种变体中更准确的是旋转变体。然而，下面的程序同时实现了两种变体。此外，根据作者的经验，如果粘度 $\nu$ 是可变的，应该使用标准形式。 </ul>
 *
 * <p>  van Kan在 <ul>   <li>  J. van Kan, "A second-order accurate pressure-correction scheme for viscous incompressible flow", SIAM Journal on Scientific and Statistical Computing, Vol. 7, no. 3, pp. 870-891, 1986  </ul>  并由Guermond在 <ul>   <li>  J.-L. Guermond, "Un résultat de convergence d'ordre deux en temps pour l'approximation des équations de Navier-Stokes par une technique de projection incrémentale" , ESAIM: Mathematical Modelling and Numerical Analysis, vol. 33, no. 1, pp. 169-189, 1999  </ul>  ]的情况 $\nu = 1$ 。事实证明，这种技术受到运动压力的非物理边界条件的影响，导致收敛率降低。为了防止这种情况，Timmermans等人在 <ul>   <li>  L. Timmermans, P. Minev, and F. Van De Vosse, "An approximate projection scheme for incompressible flow using spectral elements", International Journal for Numerical Methods in Fluids, vol. 22, no. 7, pp.673-688, 1996  </ul>   <ul>   <li>  J.-L. Guermond 和 J. Shen, "On the error estimates for the rotational pressure-correction projection methods", Mathematics of Computation, vol. 73, no. 248, pp.1719-1737, 2004  </ul>  对Stokes问题进行了全面分析。 </p>
 * <a name ="fullydiscrete"></a><a name="TheFullyDiscreteSetting"></a><h3> The
 * Fully Discrete Setting </h3> 。
 * 为了获得该方法的完全离散设置，我们一如既往地需要一个变分公式。鉴于边界条件的性质，这里有一个微妙的问题。当我们将方程乘以一个合适的检验函数时，出现的一个项是@f[
 *
 *
 *
 *
 ...
 * @f]如果我们，比如说，在整个边界上有迪里希特边界条件，那么经过部分积分，我们将得到@f[
 *
 *
 *
 *
 ...
 * \nabla v.
 * @f]这种表述的一个优点是，它完全解耦了速度的各个组成部分。此外，它们都共享同一个系统矩阵。这一点可以在程序中加以利用。
 * 然而，考虑到非标准的边界条件，为了能够将其考虑在内，我们需要使用下面的%同位素@f[
 * \Delta u = \nabla\nabla\cdot u
 *
 * - \nabla\times\nabla\times u,
 * @f]，这样当我们进行分项积分并考虑到边界条件时，就可以得到@f[
 *
 *
 *
 *
 ...
 * alt=""> 与 $H=4.1$ 一起，使几何形状略微不对称。
 * 我们在顶壁、底壁和障碍物上都施加了无滑动的边界条件。在左边，我们有流入的边界条件@f[
 * u =
 * \left( \begin{array}{c} 4 U_m y (H-y)/H^2 \\ 0 \end{array} \right),
 * @f]与 $U_m = 1.5$ ，即流入的边界条件对应于这个配置的Poiseuille流。最后，在右边的垂直壁上，我们施加条件，速度的垂直分量和压力都应该是零。
 *
 ...
 * 这尤其包括辅助变量 $\phi$
 * 的方程表述，为此我们声明一个 <code>enum</code>
 * 。接下来，我们声明一个类，该类将读取和存储我们程序运行所需的所有参数。
 *
 *
 * @code
 ...
 * @endcode
 *
 * 在这个类的构造函数中，我们声明所有的参数。这方面的细节已经在其他地方讨论过了，例如在
 * step-29  。
 *
 *
 * @code
 ...
 * @endcode
 *
 * 接下来的几个结构和函数是用来做各种并行的事情。它们遵循 @ref
 * threads
 * 中提出的方案，使用WorkStream类。正如那里所解释的，这需要我们为每个汇编器声明两个结构，一个是每个任务的数据，一个是scratch数据结构。然后，这些结构被移交给组装本地贡献的函数，并将这些本地贡献复制到全局对象上。
 ...
 *
 * 在构造函数中，我们只是从作为参数传递的
 * <code>Data_Storage</code>
 * 对象中读取所有数据，验证我们读取的数据是否合理，最后，创建三角形并加载初始数据。
 *
 *
 * @code
 * template <int dim>
 ...
 *
 * 在这组方法中，我们初始化了稀疏模式、约束条件（如果有的话）并组装了不依赖于时间步长的矩阵
 * <code>dt</code>
 * 。注意，对于拉普拉斯矩阵和质量矩阵，我们可以使用库中的函数来完成。因为这个函数的昂贵操作
 *
 * - 创建这两个矩阵
 *
 * 是完全独立的，我们原则上可以把它们标记为可以使用
 * Threads::new_task
 * 函数并行工作的任务。我们在这里不会这样做，因为这些函数在内部已经被并行化了，特别是由于当前的函数在每个程序运行中只被调用一次，所以在每个时间步长中不会产生费用。然而，必要的修改将是非常直接的。
 *
 *
 * @code
 * template <int dim>
 ...
 * @endcode
 *
 * 对于梯度算子，我们从初始化稀疏模式和压缩它开始。这里需要注意的是，梯度算子从压力空间作用到速度空间，所以我们必须处理两个不同的有限元空间。为了保持循环的同步，我们使用之前定义的别名，即
 ...
 *
 * 这是一个时间行进函数，从 <code>t_0</code>
 * 开始，使用时间步长为 <code>dt</code>
 * 的投影法进行时间推进，直到 <code>T</code> 。
 * 它的第二个参数 <code>verbose</code>
 * 表示该函数是否应该输出它在任何特定时刻正在做什么的信息：例如，它将说明我们是否正在进行扩散、投影子步骤；更新前置条件器等。我们没有使用像
 ...
 *
 * 扩散步骤的实现。注意，昂贵的操作是函数末尾的扩散解，我们必须对每个速度分量做一次。为了加快进度，我们允许以%并行方式进行，使用
 * Threads::new_task 函数，该函数确保 <code>dim</code>
 * 的求解都得到处理，并被安排到可用的处理器上：如果你的机器有一个以上的处理器核心，并且这个程序的其他部分目前没有使用资源，那么扩散求解将以%并行方式运行。另一方面，如果你的系统只有一个处理器核心，那么以%并行方式运行将是低效的（因为它导致了，例如，缓存拥堵），事情将被顺序执行。
 *
 *
 * @code
 * template <int dim>
 ...
 *
 * 下面几个函数是关于集合平流项的，这是扩散步骤的系统矩阵的一部分，在每个时间步骤中都会发生变化。如上所述，我们将使用WorkStream类和其他设施，在所有单元上以%并行方式运行装配循环，如 @ref
 * threads 的文档模块中所述。
 *
 *
 * @code
 ...
 * <h4><code>NavierStokesProjection::projection_step</code></h4> * <a
 * name="codeNavierStokesProjectionprojection_stepcode"></a> *
 ...
 *
 * 这是投影法的压力更新步骤。它实现了该方法的标准表述，即@f[ p^{n+1} = p^n +
 * \phi^{n+1}, @f]或旋转形式，即@f[ p^{n+1} = p^n +
 * \phi^{n+1}
 *
 * - \frac{1}{Re} \nabla\cdot u^{n+1}. @f] 。
 *
 ...
 * <code>NavierStokesProjection::output_results</code> </h4> * * <a
 * name="codeNavierStokesProjectionoutput_resultscode"></a> <h4>
 ...
 * 我们不会在这里详细说明这个过程，而是参考 step-32
 * ，那里使用了一个类似的程序（并且有记录），为所有变量创建一个联合的DoFHandler对象。
 * 我们还注意到，我们在这里将涡度作为一个单独的函数中的标量来计算，使用
 * $L^2$ 中的量 $\text{curl} u$
 * 投影到用于速度分量的有限元空间。但原则上，我们也可以把速度作为一个点状量来计算，并通过
 * step-29 和 step-33 中讨论的数据后处理机制来实现。
 *
 *
 * @code
 * template <int dim>
 ...
 * @endcode
 *
 * 下面是一个辅助函数，通过将 $\text{curl} u$
 * 项投影到用于速度分量的有限元空间来计算涡度。这个函数只有在我们生成图形输出时才会被调用，所以不是很频繁，因此我们没有像对待其他装配函数那样，麻烦地使用WorkStream概念来并行化它。不过，如果需要的话，这应该不会太复杂。此外，我们在这里的实现只适用于2D，所以如果情况不是这样，我们也会保释。
 *
 *
 * @code
 * template <int dim>
 ...
 * 我们用下面的 <code>parameter-file.prm</code>
 * 运行代码，它可以在与源代码相同的目录中找到。
 * @verbatim
 ...
 * @endverbatim
 *
 * 如果我们把它设置为 <code>true</code>
 * ，我们就可以得到程序正在做什么以及每个迭代过程需要多少步才能收敛等信息。
 * 让我们绘制 $t=1,5,12,20,25$
 * （即时间步骤200、1000、2400、4000和5000）的结果，在左边一栏中我们显示涡度，在右边显示速度场。
 * <table> <tr> <td> <img
 ...
 * alt="" width="400"> </td> </tr> </table>
 * 这些图像很好地显示了障碍物后面的涡流链的发展和延伸，涡度的符号表明这是一个左转或右转的涡流。
 *
 *  <a name="Re500"></a><a name="Re500"></a><h3> Re = 500 </h3> 。
 *
 *  我们可以将参数文件中的雷诺数 $Re$ 改为 $500$
 * 的值。这样做，并在一定程度上减少时间步长，在
 * $t=20,40$ 时产生以下图像。 <table> <tr> <td> <img
 ...
 *
 * 这个方案可以向以下方向扩展。 <ul>   <li>  自适应网格细化。正如我们所看到的，我们在一个单一的固定网格上计算了所有的东西。 使用自适应网格细化可以提高精度，同时不明显增加计算时间。
 * <li>
 * 自适应时间步进。虽然目前显然没有关于可变时间步长的投影方法的理论，但实践表明，它们的表现非常好。
 * <li>
 * 高雷诺%数。正如我们从结果中看到的，增加雷诺数会显著改变离散化方案的行为。使用众所周知的稳定技术，我们可以计算这个问题或许多其他问题中的流动，当雷诺数非常大，计算成本要求空间分辨率，而流动只能得到有限的解决，特别是对于三维湍流。
 * <li>
 * 可变密度的不可压缩流。对于密度可变的不可压缩流的情况，有类似投影的方法。如果不同密度的流体混合，例如淡水和盐水，或者酒精和水，这种流动就会发挥作用。
 * <li>  可压缩纳维-斯托克斯方程。这些方程适用于以下情况：速度高到足以使流体变得可压缩，但还没有快到我们进入一个粘度变得可以忽略不计的制度，纳维-斯托克斯方程需要被气体动力学的双曲欧拉方程所取代。如果速度超过音速的三分之一，可压缩性就开始成为一个因素，所以它对几乎所有的地面交通工具来说都不是一个因素。另一方面，商业喷气式飞机的飞行速度约为音速的85%，机翼上的流动变得明显超音速，在这种情况下，可压缩的纳维尔-斯托克斯方程也不再适用。然而，在这两者之间的范围内有重要的应用，如小型飞机或许多欧洲和东亚国家的快速列车。 </ul>
 *
