 ...
 * ，只要它们满足某些接口，包括不属于库的矢量类型，但可能在应用程序中定义。在这种情况下，应用程序可以通过手工实例化这些模板，如下一节所述。
 *
 *
 *
 * @subsection  Inst2c 创建新实例
 * 选择你的一个源文件来提供所需的实例。假设你想让头文件<tt>XXXX</tt>中定义的类模板<tt>XX.h</tt>以模板参数<tt>long
 * double</tt>进行实例化。那么，你的文件应该包含以下几行
 *
 * @code
