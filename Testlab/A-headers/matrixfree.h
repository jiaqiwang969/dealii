 ...
 * 本模块描述了deal.II中的无矩阵基础设施。以下是deal.II中各主要类群与无矩阵基础设施的交互方式的概要，可点击图表，下面是更详细的描述。
 *
 *
 ...
 * FEEvaluationAccess类的动机是允许根据分量的多少对插值解字段的值和梯度访问进行特殊化。而基类FEEvaluationBase将梯度作为一个
 ...
 * ，返回解场对面的法向导数。这个量被计算为梯度（在实空间）乘以法向量。梯度和法向量的组合是许多（简单的）二阶椭圆方程的典型特征，例如用内部惩罚法对拉普拉斯进行离散化。如果不需要单独的梯度，联合操作大大减少了数据访问，因为每个正交点只需要
 * "正常雅各布 "的 "dim
 * "数据条目，而单独访问时，雅各布的 "dim^2
 * "字段和法线的 "dim "字段是必要的。
 ...
 * FEFaceEvaluation::gather_evaluate() 和
 * FEFaceEvaluation::integrate_scatter().
 * 中自动做出的。为每个积分任务做这个决定似乎效率不高，但最终这是一个单一的`if`语句（条件跳跃），对于现代CPU来说很容易预测，因为决定在一个积分循环中总是相同的。(人们只需支付一定程度上增加的编译时间，因为编译器需要为所有路径生成代码，虽然)。
 ...
 * 所有这些数据结构坚持的主要界面是，集成任务被分解成一个单元或面的范围，人们可以通过一个整数索引来索引。单元积分、内面积分和边界积分的整数范围的信息是由类
 * internal::MatrixFreeFunctions::TaskInfo,
 * 使用数据字段`cell_partition_data`、`face_partition_data`和`boundary_partition_data`提供。这个类还包含了用于使用线程并行调度任务的索引子范围的信息，以及在`{cell,face,boundary}_partition_data`内对索引范围的分组，用于交织单元和面的积分，这样对单元和面的积分的向量条目的访问就会重新使用已经在缓存中的数据。
 ...
 * 中完成的，在这里我们首先假设了一个非常一般的有限元布局，无论是连续的还是不连续的元素，并且我们解决了由于悬挂节点带来的约束。这个初始步骤是在单元的原始排序中完成的。在后面的阶段，这些单元一般会被重新排列，以反映我们在最终循环中通过单元的顺序，我们也会在DoF指数中寻找可以利用的模式，如单元内连续的指数范围。这种重新排序是为了实现与MPI的通信和计算的重叠（如果启用的话），并在单元格上形成更好的具有矢量化的批次组。指数的数据存储在这个最终的顺序中是线性的，安排在
 ...
 * 函数的单个DoFHandler对象内的索引必须在每个MPI进程中是连续的，也就是说，本地范围必须最多包含一个块。除了基本的分区器，该类还提供了一组更严格的索引集，只涉及所有鬼魂索引的一个子集，被添加到向量的鬼魂范围。这些交换模式被设计为与通过
 * internal::MatrixFreeFunctions::ShapeInfo::nodal_at_cell_boundaries
 * 的减少索引访问相结合，例如。
 ...
 * FEEvaluationAccess::get_normal_derivative()),
 * 实空间的正交点，以及参考元素上的正交点。我们使用一个辅助索引数组，指向每个单元的数据起点，即雅各布系数、JxW值和法向量的`data_index_offsets`字段，以及正交点的`quadrature_point_offsets`。这种偏移使HP-adaptivity的字段长度可变，类似于DoFInfo的做法，但它也使我们称之为<i>geometry
 ...
 * 因为所有来自两边的积分都是一次性执行的，所以出现了一个问题，即在子域边界的两个处理器中哪一个被分配到一个面。本模块的作者进行了大量的实验，发现应用于自由度存储的方案，即把所有可能重叠的项目分配给一个处理器，是相当不平衡的，面的数量最多有20%的差异。为了获得更好的性能，在`face_setup_internal.h`中实现了一个平衡方案，将每对处理器之间的所有接口分成两块，一块由一个处理器完成，另一块由另一个处理器完成。尽管这增加了通过MPI发送的消息的数量，但这是值得的，因为负载变得更加平衡。另外，当本地问题的规模为100,000
 ...
 * 除了以适当的方式安排工作外，该循环还执行两个任务。  <ol>   <li>  对`src`和`dst`向量进行数据交换，分别调用`update_ghost_values()`和 `compress(VectorOperation::add)`, 。如果各自的标志 MatrixFree::AdditionalData::overlap_communication_computation 被设置为 "true"（默认），那么交换可以以异步方式进行，与不需要来自远程处理器的数据的单元的工作重叠通信。  <li>  使用相应的标志将`dst`向量归零。在循环内这样做的好处是，循环知道向量中的哪些条目（首先）被单元格和面的循环中的一些子ranges触及。因此，它可以将向量逐个归零，以确保我们不需要两次访问向量条目（一次归零，一次添加贡献）。这似乎是一个微小的优化，但事实上运算符的评估可以非常快，简单地将一个向量归零就可以花费运算符评估时间的20%左右，所以这确实是值得努力的 由于这个参数有一些实验性，DoFInfo类保留了一个静态变量 internal::MatrixFreeFunctions::DoFInfo::chunk_size_zero_vector ，在这里可以调整（如果有人认为其他东西会更好，例如因为未来的计算机看起来和2018年推出这个参数时不同）。  </ol>
 * 最后， MatrixFree::loop
