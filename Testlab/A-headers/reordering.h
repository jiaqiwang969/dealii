 ...
 * deal.II中的三角形有一个特殊的结构，即不仅有单元，还有面，在三维中还有边，它们本身就是对象。面和边都有独特的方向，而且它们相对于相邻的单元也有特定的方向。因此，在两个空间维度中分隔两个单元的线不仅有一个方向，而且相对于与第一条线相邻的两个四边形的其他线，它也必须有一个明确的方向。同样的定义也适用于三维单元和分隔它们的物体（线、四边形）。
 * 例如，在二维空间中，一个四边形由四条线组成，这四条线有一个方向，根据定义是这样的。
 *
 * @verbatim
 * 2-->--3
 * |     |
 ...
 * 0-->--1
 * @endverbatim
 * 现在，两个相邻的单元格必须有一个顶点编号，使得共同边的方向是相同的。例如，以下两个四边形
 *
 * @verbatim
 * 3---4---5
 * |   |   |
 ...
 * 5)</tt>来描述，因为从两个单元格看，中间的线会得到方向<tt>1->4</tt>。
 * 编号<tt>(0 1 3 4)</tt>和<tt>(5 4 2
 * 1)</tt>将是不允许的，因为左边的四边形会给公共线以<tt>1->4</tt>的方向，而右边的会想用<tt>4->1</tt>，导致歧义的产生。
 * 作为附带说明，我们注意到，如果人们采纳了拥有面的方向是有用的这一观点，那么如上所示的一个单元的四个面的方向几乎是必要的。特别是，不可能将它们的方向定为代表（逆）时针的意义，因为那样的话，我们就无法找到以下三个细胞的有效方向。
 *
 * @verbatim
 *     o
 *   /   \
 ...
 * o---o---o
 * @endverbatim
 * (读者被要求尝试找到一个符合要求的线条方向的选择；很快就会发现不可能存在这样的事情，即使我们允许可能存在同时具有顺时针和逆时针方向的线条的单元。)
 ...
 * 本课的目的是为一组给定的单元格找到一个排序，使生成的三角形满足上述的所有要求。为此，我们将首先展示一些例子，说明为什么这是一个困难的问题，然后开发找到这样一个重新排序的算法。请注意，该算法是在一组CellData对象上操作的，这些对象是用来向三角剖分类描述一个网格的。例如，这些对象是由GridIn类在从输入文件读入网格时生成的。
 ...
 * 如前所述，对单元格的顶点列表进行重新排序，从而得到的网格并不是一个微不足道的问题。特别是，只看一个单元的邻域往往是不够的，它不能在不违反上述要求的情况下被添加到其他单元的集合中。我们将展示两个例子，这一点很明显。
 * 第一个这样的例子如下，我们将称之为
 * "最后的四个单元格"，因为这四个单元格靠近每一行三个垂直单元格的右端（在下面的图片中，我们只显示了左边这样一列三个单元格，但我们将指出如果我们延长这个列表会发生什么）。
 *
 * @verbatim
 * 9---10-----11
 * |   |    / |
 ...
 * 0---1------2
 * @endverbatim
 * 假设你对左边边界的单元格的顶点进行了编号，那么就会产生以下的线条方向。
 *
 * @verbatim
 * 9->-10-----11
 * ^   ^    / |
 ...
 * 0->-1------2
 * @endverbatim
 * 例如，这可以通过使用<tt>(0 1 3 4)</tt>, <tt>(3 4 6 7)</tt>,
 * <tt>(6 7 9
 * 10)</tt>这三个单元格的索引来完成）。现在，你将找不到一种方法来给出正确的单元格的指数，而又不对某条线或其他线引入歧义，或者不违反每个单元格内必须有一个顶点，两条线都从这个顶点出发，而相邻的两条线都指向这个顶点。
 * 在这种情况下，解决方案是对左边三个单元格中的一个进行重新编号，例如，通过对左上角单元格的编号<tt>(9
 * 6 10 7)</tt>来恢复顶点7和10之间线条的意义。
 *
 * @verbatim
 * 9->-10-----11
 * v   v    / |
 ...
 * o---o---o---o---o------o
 * @endverbatim
 * 那么如果我们对左边的单元格进行统一排序，就会遇到和上面一样的问题，从而迫使我们恢复一个单元格的排序（就是上面我们可以排序为<tt>（9
 * 6 7
 * 10）</tt>的那个单元格）。然而，由于相反的线必须有相同的方向，这又会迫使我们旋转它左边的单元格，然后再旋转左边的单元格，如此反复，直到我们到达网格的左端。因此，这是一个我们必须追溯到第一列的三个单元的例子，以找到一个一致的排序，如果我们最初是均匀地排序。
 * 作为第二个例子，考虑下面这个简单的网格，其中单元格的编号顺序很重要。
 *
 * @verbatim
 * 3-----2-----o-----o ... o-----7-----6
 * |     |     |     |     |     |     |
 ...
 * 0-----1-----o-----o ... o-----4-----5
 * @endverbatim
 * 我们在这里只指出了相关顶点的编号。假设用户通过顶点指数<tt>0
 * 1 3 2</tt>和<tt>6 7 5
 * 4</tt>赋予单元格0和1。那么，如果我们按照这个方向，为这两个单元格创建线条后的网格会是这样的。
 *
 * @verbatim
 * 3-->--2-----o-----o ... o-----7--<--6
 * |     |     |     |     |     |     |
 ...
 * 0-->--1-----o-----o ... o-----4--<--5
 * @endverbatim
 * 现在，由于相反的线条必须指向同一方向，我们只能将2至N-1单元格添加到1单元格，使所有垂直线都指向下方。然而，我们不能向任何方向添加单元格N，因为它将有两条不指向同一方向的相反线。我们必须旋转0号或1号单元格，以便能够添加所有其他单元格，从而满足交易.II三角形的要求。
 * 这两个例子说明，如果我们在某个方向上添加了一定数量的单元格，而在添加下一个单元格时又不能不引入已经在另一个方向上添加的面，那么只旋转我们未能添加的单元格附近的单元格可能是不够的。可能有必要回到很远的地方，旋转很久以前就已经输入的单元。
 *
 *  <h3>Solution</h3>
 * 从上面的例子可以看出，如果我们遇到一个不能被添加到已经输入的单元格中的单元格，我们通常不能指出哪个单元格是罪魁祸首，必须换个方向输入。此外，即使我们知道哪个单元格，也可能有大量的单元格不再适合于网格，我们也必须找到不同的方向（在上面的第二个例子中，如果我们旋转了单元格1，那么我们也必须旋转单元格1到N-1）。
 ...
 * 0)</tt>，<tt>(3 2 0 1)</tt>，和<tt>(2 0 1 3)</tt>）。)
 * 当添加一个又一个单元格时，我们以深度优先（pre-order）的方式遍历这个树。当我们遇到从根（单元格0）到叶（最后一个单元格）的路径不允许时（也就是说，通过树的路径所编码的单元格的方向不能导致有效的三角形），我们必须追踪并尝试通过树的另一条路径。
 * 当然，在实践中，我们并不是沿着每条路径走到最后一个节点，然后找出一条路径是否导致有效的三角结构，而是使用一个归纳论证：如果对于所有先前添加的单元格，三角结构是有效的，那么我们可以通过检查进入目前的单元格是否会引入任何具有非唯一方向的面，来找出一条穿过树的路径是否可以产生有效的三角结构；如果是这样，那么我们可以停止跟踪这一点以下的所有路径，立即追踪回去。
 ...
 * 0------1---------2
 * @endverbatim
 * 首先选择一个单元（本例中为（0,1,3,4））。单元的一个侧面被任意地定向（3->4）。这个方向的选择然后在网格中传播，跨越边和元素。(0->1),
 * (6->7)和(9->10)。这涉及到边跳和面跳，在网格中给出了一个以圆点表示的路径。
 *
 * @verbatim
 * 9-->--10-------11
 * |  .  |        /|
 ...
 * 0-->--1---------2
 * @endverbatim
 * 一旦一个元素被完全定向，它就不需要再考虑了。这些元素在图中用o来填充。然后我们转到下一个元素。
 *
 * @verbatim
 * 9-->--10->-----11
 * | ooo |  .     /|
 ...
 * 0-->--1-->-------2
 * @endverbatim
 * 很明显，这个算法的运行时间是线性的，因为它只对每个面精确地接触过一次。
 * 刚刚描述的二维情况下的算法在这个类中同时实现了二维和（通用形式）三维的算法。三维情况下，使用片状而不是单元格串来工作。如果网格是可定向的，那么该算法能够在线性时间内完成其工作；如果它是不可定向的，那么它也会在线性时间内中止。
 ...
 * AABB17 中的工作重新实现），我们使用了最初由Wolfgang
 ...
 * 10</tt>来描述单元格5，并假设单元格1、2、3、4的编号是可以在初始旋转中增加5。所有其他的单元格都以通常的方式进行编号，即从左下角开始，逆时针计数。鉴于对单元格的这种描述，算法将从零单元格开始，一个接一个地添加单元格，直到第六个。然后情况会是这样的。
 *
 * @verbatim
 * 13----->---14--<--15
 * | \         |     |
 ...
 * 0-->--1-->--2-->--3
 * @endverbatim
 * 现在来看看第7个单元格，我们看到它的顶部和底部的两条相对的线有不同的方向；因此我们将发现第7个单元格没有任何方向可以在不违反三角形的一致性的情况下添加。根据上述规则，我们回到索引最大的邻居，也就是单元格6，但由于它的底线是向右的，它的顶线也一定是向右的，所以我们无法找到单元格6的方向，从而使7符合三角形。然后，如果我们已经完成了单元格6的所有可能的方向，我们再追踪到6的最大索引的邻居，并且已经被添加了。然而，我们知道0号单元格的方向不可能是重要的，所以我们得出结论，没有任何可能的方法来确定给定单元格的所有线条的方向，使它们满足deal.II三角形的要求。我们知道这是不可能的，所以会导致抛出一个异常。
 * 这个例子的底线是，当我们看了所有可能的单元格6的方向时，我们找不到一个可以添加单元格7的方向，然后决定追踪回单元格0，我们甚至没有尝试转动单元格5，之后添加单元格7会很简单。因此，必须修改上述算法：如果我们未能在任何方向上添加一个单元格，我们只允许追踪回已经添加的那个邻居，其单元格索引最大。如果我们进一步追踪，因为我们已经用尽了所有可能的方向，但可以添加细胞（即我们追踪，因为另一个细胞，更远的地方不能添加，不管我们现在考虑的细胞的方向），那么我们不允许追踪到它的一个邻居，但必须只追踪一个细胞指数。
