 ...
 * 所有与形状函数和对形状函数的访问有关的类。 这涉及到有限元的实际值。关于自由度的编号请参考
 * @ref dofs  的模块。
 * 本模块的类和函数分为几个子组，在上面列出的各自子模块中讨论。此外，FETools类提供了提供有限元素、元素间变换等信息的函数。
 * 从总体上看，这个模块的各个部分与库中的其他各种部分相互作用。
 *
 ...
 * 这里的类描述了有限元空间，如最简单的Q1（双/三线）空间和高阶拉格朗日空间Qp，但也有更专门的空间，如Nedelec或Raviart-Thomas空间。具体的实现是由抽象的FiniteElement基类派生的。
 * 本质上，这些类必须实现的函数提供了查询单元格上某一点的形状函数的值或导数的能力。为了在整合矩阵和右手条目时发挥作用，我们必须有能力将这些形状函数和梯度映射到实际单元上。这是由Mapping基类（见 @ref
 ...
 *
 * @note
 * FE_PolyTensor类提供了对实现矢量值元素的支持。通常情况下，一个新的向量元素应该派生自这个类。
 * <h3>Discontinuous Galerkin</h3>
 * 对于每个符合任何弱可微函数空间的有限元，如<i>H<sup>1</sup></i>或<i>H<sup>curl</sup></i>，我们可以通过简单地将顶点、边或面的所有自由度分配到单元的内部来定义一个类似的DG空间。这要从拓扑学的角度来理解。这种自由度的插值算子仍然会在边界上。 虽然没有这样做，但我们提供了相当多的这些元素，加上那些没有符合要求的对应元素，比如FE_DGP。以下是当前DG元素的列表。  <ul>   <li>  标量。FE_DGP, FE_DGQ  <li>  标量，不同的形状函数。FE_DGPMonomial, FE_DGPNonparametric, FE_DGQArbitraryNodes  <li>  矢量值的。 FE_DGBDM, FE_DGNedelec, FE_DGRaviartThomas  </ul>
 *
 *
 * @note
 * FE_DGVector类支持向量值的DG元素的实现，其方式是只需提供向量多项式空间。由此派生的实际类只需实现一个构造函数和
 ...
 * 本模块中的类用于从单位坐标映射到实际单元格的坐标。最常见的是，人们使用MappingQ1类，它提供了一个Q1（双线/三线）映射（即对通常的Q1元素来说是一个等价的映射）。然而，还有其他一些实现高阶映射的类，以提供曲线元素。这些在
