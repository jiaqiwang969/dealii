 ...
 * 大多数人只创建过一次有限元（以及可能的映射）对象，但实际上从未调用过它们的任何成员函数
 *
 ...
 * 在详细介绍数据和控制流之前，让我们定义一下哪个类负责提供什么样的信息。
 ...
 * $K$
 * 。类似的操作也可以为矩阵（等级为2的张量，而矢量是等级为1的张量）和高阶张量定义。
 ...
 * 有限元类（即从FiniteElement派生的类）负责在参考单元上定义它们的形状函数、导数和许多其他方面，但也负责计算实际单元上的映射值和导数（显然是在映射对象的帮助下）。在目前的讨论中，只有后一个角色是重要的。
 ...
 * 假设用户想计算形状函数的梯度，例如计算上面的积分。那么他们就会通过给出update_gradients标志来初始化一个FEValues对象（从
 * step-3
 ...
 * 在这一点上，FEValues对象已经找出了完整的标志集，表明大家要计算什么来满足用户的请求。下一步，仍然是在构建FEValues对象的过程中，源于这样的认识：许多东西可以预先计算一次，然后在我们每次移动到一个真实的单元时重新使用。一个例子是，为了计算真实单元上形状函数的梯度，我们需要知道参考单元上形状函数的梯度（在参考单元上的正交点），而且这些梯度总是相同的：每次我们访问一个新单元时，这些值都会保持不变，所以每次都重新计算它们是低效的。对于一些映射类所计算的一些信息也可以提出类似的论点。
 * 因此，FEValues对象同时初始化了映射和它所指向的有限元对象，使用正交对象和上一节所述的最后一组更新标志来计算。这个初始化包括预先计算这些类在给定更新标志集后可以预先计算的内容，然后存储这些信息供以后使用。
 * 然后问题来了：在哪里存储这些信息。在实践中，我们不希望将这些信息存储在映射或有限元对象本身，因为这意味着(i)每次只有一个FEValues对象可以使用任何给定的映射或有限元对象，以及(ii)这些对象不能在多线程环境下使用。
 * 相反，该方法是这样的。
 *
 *
 *
 ...
 *
 *
 * - 其次，FEValues还调用 FiniteElement::get_data() （以及FEFaceValues调用 Mapping::get_face_data(), 和FESubfaceValues再次调用 Mapping::get_subface_data()), 的正交对象和最后一组更新标志。这些函数的作用与映射中的对应函数基本相同，而且如此初始化的对象，这次是源自 FiniteElement::InternalDataBase, 的类型，在以后FEValues对象想要从有限元对象获得什么时，总是会被反馈给有限元。
 ...
 * 前面的所有步骤都发生在创建FEValues对象的时候。到此为止，我们所做的都是设置数据结构，但从用户的角度来看，到目前为止还没有任何有用的计算。这只发生在
 * FEValues::reinit()  在具体单元格上被调用时  $K$  。
 * 然后FEValues所做的事情是，按照这个顺序。
 *
 *
 * - FEValues计算出该单元是否是调用 FEValues::reinit() 的前一个单元的翻译或其他类似的简单转换。这个结果存储在 CellSimilarly::Similarity 对象中，然后将被传递给映射和有限元，以潜在地简化一些计算。例如，如果当前单元只是前一个单元的平移，那么就不需要重新计算映射的雅各布矩阵 $J_K$ （或其逆），因为它将与前一个单元相同。
 *
 *
 *
 * - 接下来， FEValues::reinit() 调用 Mapping::fill_fe_values() （显然，FEFaceValues调用 Mapping::fill_fe_face_values() 和FESubfaceValues调用 Mapping::fill_fe_subface_values()).  ] 这个函数的参数包括我们被要求访问的单元格（或面，或子面），以及上面的单元格相似性参数，对我们之前从 Mapping::get_data(), 获得的对象的引用，以及对 internal::FEValues::MappingRelatedData 类型的对象的引用，映射应该把它的结果写入其中。特别是，它需要计算之前由更新标志指定的所有映射相关信息，然后将它们写进输出对象。  在输出对象中，映射需要填充的字段的例子是JxW值的计算、雅各布矩阵及其反值的计算，以及单元格（如果dim小于spacedim）和面的法向量。
 *
 *
 *
 * - 最后， FEValues::reinit() 调用 FiniteElement::fill_fe_values() （显然，FEFaceValues调用 FiniteElement::fill_fe_face_values() 和FESubfaceValues调用 FiniteElement::fill_fe_subface_values()).  ] 这个函数的参数包括我们被要求访问的单元格（或面，或子面），以及上面的单元格相似性参数，对我们之前从 FiniteElement::get_data(), 获得的对象的引用，以及对 internal::FEValues::MappingRelatedData 类型的对象的引用，映射应该将其结果写入其中。
