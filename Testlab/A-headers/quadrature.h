 ...
 * 本模块包含基类正交以及deal.II提供的正交公式。正交公式提供了两个基本数据：单元格[0,1]^d上的正交点的位置，以及每个正交点的权重。
 ...
 *
 *  <h3>Use</h3>
 * 除其他用途外，正交公式在积分矩阵条目和右手边向量的分量时使用。为此，定义在单元格上的正交点必须被映射到实数单元格上的相应位置，而权重必须乘以雅各布系数的行列式。这一步是由派生自Mapping基类的类来完成的，尽管这通常是隐藏的，因为如果没有提供特定的映射，库的许多部分会退回到使用MappingQ1类型的对象。
 * 下一步是评估形状函数和它们在这些位置的梯度。虽然从FiniteElement基类派生出来的类提供了对单元格上形状函数的描述，但在正交点上的实际评估以及将其与从映射中得到的信息结合起来的工作是由FEValues类及其关联者完成的。因此，从本质上讲，FEValues类是对有限元空间（由FiniteElement类定义）的视图，在正交点（由正交类提供）上进行评估，并映射到真实空间（而不是单元空间）的单元内的位置（由Mapping类提供映射）。
 ...
 *
 *  <h3>QAnisotropic</h3>
 * 通常的高维正交公式产生的张量乘积在每个方向上都是相等的，而QAnisotropic类产生的张量乘积在每个方向上可能是不同的公式。
 *
 *  <h3>QProjector</h3>
 * QProjector类本身实际上不是一个正交规则，但它提供了在高维单元表面计算正交公式的函数。
