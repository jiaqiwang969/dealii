 ...
 * This tutorial depends on step-12b.  @htmlonly <table class="tutorial"
 ...
 * penalty method and Nitsche's weak boundary conditions to solve Poisson's
 * equation. We use multigrid methods on locally refined meshes, which are
 * generated using a bulk criterion and a standard error estimator based on
 * cell and face residuals. All operators are implemented using the MeshWorker
 * interface.  Like in step-12, the discretization relies on finite element
 * spaces, which are polynomial inside the mesh cells $K\in \mathbb T_h$, but
 * have no continuity between cells. Since such functions have two values on
 * each interior face $F\in \mathbb F_h^i$, one from each side, we define mean
 * value and jump operators as follows: let <i>K</i><sub>1</sub> and
 * <i>K</i><sub>2</sub> be the two cells sharing a face, and let the traces of
 * functions <i>u<sub>i</sub></i> and the outer normal vectors
 * <b>n</b><i><sub>i</sub></i> be labeled accordingly. Then, on the face, we
 * let
 * @f[
 * \average{ u } = \frac{u_1 + u_2}2
 * @f]
 *  Note, that if such an expression contains a normal vector, the averaging
 * operator turns into a jump. The interior penalty method for the problem
 * @f[
 ...
 * \biggr\}.
 * @f}
 *  Here, $\sigma_F$ is the penalty parameter, which is chosen as follows: for
 * a face <i>F</i> of a cell <i>K</i>, compute the value
 * @f[
 * \sigma_{F,K} = p(p+1) \frac{|F|_{d-1}}{|K|_d},
 * @f]
 * where <i>p</i> is the polynomial degree of the finite element functions and
 * $|\cdot|_d$ and $|\cdot|_{d-1}$ denote the $d$ and $d-1$ dimensional
 * Hausdorff measure of the corresponding object. If the face is at the
 * boundary, choose $\sigma_F = \sigma_{F,K}$. For an interior face, we take
 * the average of the two values at this face.  In our finite element program,
 * we distinguish three different integrals, corresponding to the sums over
 * cells, interior faces and boundary faces above. Since the MeshWorker::loop
 * organizes the sums for us, we only need to implement the integrals over
 * each mesh element. The class MatrixIntegrator below has these three
 * functions for the left hand side of the formula, the class RHSIntegrator
 * for the right.  As we will see below, even the error estimate is of the
 * same structure, since it can be written as
 * @f{align*}
 ...
 * programmed in the same way.  This program is related to step-12b, in that
 * it uses MeshWorker and discontinuous Galerkin methods. While there, we
 * solved an advection problem, here it is a diffusion problem. Here, we also
 * use multigrid preconditioning and a theoretically justified error
 * estimator, see Karakashian and Pascal (2003). The multilevel scheme was
 * discussed in detail in Kanschat (2004). The adaptive iteration and its
 * convergence have been discussed (for triangular meshes) in Hoppe, Kanschat,
 * and Warburton (2009).   <a name="CommProg"></a> <h1> The commented
 * program</h1>  The include files for the linear algebra: A regular
 * SparseMatrix, which in turn will include the necessary files for
 ...
 *
 * @endcode
 *  All classes of the deal.II library are in the namespace dealii. In order
 * to save typing, we tell the compiler to search names in there as well.
 * @code
 * namespace Step39
 * {
 ...
 * the right hand side and the error estimator.    All these classes have the
 * same three functions for integrating over cells, boundary faces and
 * interior faces, respectively. All the information needed for the local
 * integration is provided by MeshWorker::IntegrationInfo<dim>. Note that the
 * signature of the functions cannot be changed, because it is expected by
 * MeshWorker::integration_loop().    The first class defining local
 * integrators is responsible for computing cell and face matrices. It is used
 * to assemble the global matrix as well as the level matrices.
 * @code
 * template <int dim>
 ...
 * the right hand side function is zero, such that only the boundary condition
 * is set here in weak form.
 * @code
 * template <int dim>
 ...
 * boundary, we cannot just use VectorTools::integrate_difference(). Instead,
 * we use the MeshWorker interface to compute the error ourselves.    There
 * are several different ways to define this energy norm, but all of them are
 * equivalent to each other uniformly with mesh size (some not
 ...
 * functions in the quadrature points. Thus, we have to create the vectors for
 * the exact function's values and gradients inside the cell integrator. After
 * that, everything is as before and we just add up the squares of the
 * differences.    Additionally to computing the error in the energy norm, we
 * use the capability of the mesh worker to compute two functionals at the
 * same time and compute the <i>L<sup>2</sup></i>-error in the same loop.
 * Obviously, this one does not have any jump terms and only appears in the
 ...
 *
 * @endcode
 *   <a name="Themainclass"></a> <h3>The main class</h3>    This class does
 * the main job, like in previous examples. For a description of the functions
 * declared here, please refer to the implementation below.
 * @code
 * template <int dim>
 ...
 * refinement edge between levels. They are stored at the index referring to
 * the finer of the two indices, thus there is no such object on level 0.
 * @code
 *       if (level > 0)
 *         {
 ...
 * for quadrature rules.    Since their default update flags are minimal, we
 * add what we need additionally, namely the values and gradients of shape
 ...
 * @endcode
 *  Now, we throw everything into a MeshWorker::loop(), which here traverses
 * all active cells of the mesh, computes cell and face matrices and assembles
 * them into the global matrix. We use the variable <tt>dof_handler</tt> here
 * in order to use the global numbering of degrees of freedom.
 * @code
 ...
 *
 * @endcode
 *  Then, we tell the Meshworker::VectorSelector for cells, that we need the
 * second derivatives of this solution (to compute the Laplacian). Therefore,
 * the Boolean arguments selecting function values and first derivatives a
 * false, only the last one selecting second derivatives is true.
 * @code
 ...
 * function itself. This function is a clone of the estimation function right
 * above.    Since we compute the error in the energy and the
 * <i>L<sup>2</sup></i>-norm, respectively, our block vector needs two blocks
 * here.
 * @code
 * template <int dim>
 ...
 *
 * @endcode
 *  Create graphical output. We produce the filename by collating the name
 * from its various components, including the refinement cycle that we output
 * with two digits.
 * @code
 * template <int dim>
 * void
 ...
 * name="Logfileoutput"></a><h3>Logfile output</h3>  First, the program
 ...
 * iteration steps is constant at approximately 15.  <a
 ...
 * alt=""> Using the perl script <tt>postprocess.pl</tt>, we extract relevant
 * data into <tt>output.dat</tt>, which can be used to plot graphs with
 * <tt>gnuplot</tt>. The graph above for instance was produced using the
 * gnuplot script <tt>plot_errors.gpl</tt> via
 * @code
 * perl postprocess.pl deallog &> output.dat
 * gnuplot plot_errors.gpl
 * @endcode
 *  Reference data can be found in <tt>output.reference.dat</tt>.   <a
