 ...
 * colspan="2"><b><small>Table of contents</small></b></th></tr> <tr><td
 * width="50%" valign="top">
 * <ol>
 ...
 * magneto-mechanical materials</a>
 * <ul>
 ...
 * name="Introduction"></a><h1>Introduction</h1>   The aim of this tutorial
 * is, quite simply, to introduce the fundamentals of both
 * [automatic](https://en.wikipedia.org/wiki/Automatic_differentiation) and
 * [symbolic differentiation](https://en.wikipedia.org/wiki/Computer_algebra)
 * (respectively abbreviated as AD and SD): Ways in which one can, in source
 * code, describe a function $\mathbf f(\mathbf x)$ and automatically also
 * obtain a representation of derivatives $\nabla \mathbf f(\mathbf x)$ (the
 * "Jacobian"), $\nabla^2 \mathbf f(\mathbf x)$ (the "Hessian"), etc., without
 * having to write additional lines of code. Doing this is quite helpful in
 * solving nonlinear or optimization problems where one would like to only
 * describe the nonlinear equation or the objective function in the code,
 * without having to also provide their derivatives (which are necessary for a
 * Newton method for solving a nonlinear problem, or for finding a minimizer).
 * Since AD and SD tools are somewhat independent of finite elements and
 * boundary value problems, this tutorial is going to be different to the
 * others that you may have read beforehand. It will focus specifically on how
 * these frameworks work and the principles and thinking behind them, and will
 * forgo looking at them in the direct context of a finite element simulation.
 ...
 * examples the aim is to build up an understanding of the steps that are
 * required to use the AD and SD tools, the differences between them, and
 * hopefully identify where they could be immediately be used in order to
 * improve or simplify existing code.  It's plausible that you're wondering
 * what AD and SD are, in the first place. Well, that question is easy to
 * answer but without context is not very insightful. So we're not going to
 * cover that in this introduction, but will rather defer this until the first
 * introductory example where we lay out the key points as this example
 * unfolds. To complement this, we should mention that the core theory for
 * both frameworks
 * is extensively discussed in the @ref auto_symb_diff module, so it bears
 * little repeating here.  Since we have to pick *some* sufficiently
 * interesting topic to investigate and identify where AD and SD can be used
 * effectively, the main problem that's implemented in the second half of the
 * tutorial is one of modeling a coupled constitutive law, specifically a
 * magneto-active material (with hysteretic effects). As a means of an
 * introduction to that, later in the introduction some grounding theory for
 * that class of materials will be presented. Naturally, this is not a field
 * (or even a class of materials) that is of interest to a wide audience.
 * Therefore, the author wishes to express up front that this theory and any
 * subsequent derivations mustn't be considered the focus of this tutorial.
 * Instead, keep in mind the complexity of the problem that arises from the
 * relatively innocuous description of the constitutive law, and what we might
 * (in the context of a boundary value problem) need to derive from that. We
 * will perform some computations with these constitutive laws at the level of
 * a representative continuum point (so, remaining in the  realm of continuum
 * mechanics), and will produce some benchmark results around which we can
 * frame a final discussion on the topic of computational performance.  Once
 * we have the foundation upon which we can build further concepts, we will
 * see how AD in particular can be exploited at a finite element (rather than
 * continuum) level: this is a topic that is covered in step-72, as well as
 * step-33. But before then, let's take a moment to think about why we might
 * want to consider using these sorts of tools, and what benefits they can
 * potentially offer you.   <a
 * name="AmotivationWhywouldIusethesetools"></a><h3>A motivation: Why would I
 * use these tools?</h3>   The primary driver for using AD or SD is typically
 * that there is some situation that requires differentiation to be performed,
 * and that doing so is sufficiently challenging to make the prospect of using
 * an external tool to perform that specific task appealing. A broad
 * categorization for the circumstances under which AD or SD can be rendered
 * most useful include (but are probably not limited to) the following: -
 * <b>Rapid prototyping:</b> For a new class of problems where you're trying
 * to implement a solution quickly, and want to remove some of the intricate
 * details (in terms of both the mathematics as well as the organizational
 * structure of the code itself). You might be willing to justify any
 * additional computational cost, which would be offset by an increased
 * agility in restructuring your code or modifying the part of the problem
 * that is introducing some complex nonlinearity with minimal effort. -
 * <b>Complex problems:</b> It could very well be that some problems just
 * happen to have a nonlinearity that is incredibly challenging to linearize
 * or formulate by hand. Having this challenge taken care of for you by a tool
 * that is, for the most part, robust, reliable, and accurate may alleviate
 * some of the pains in implementing certain problems. Examples of this
 * include step-15, where the derivative of the nonlinear PDE we solve is not
 * incredibly difficult to derive, but sufficiently cumbersome that one has to
 * pay attention in doing so by hand, and where implementing the corresponding
 * finite element formulation of the Newton step takes more than just the few
 * lines that it generally takes to implement the bilinear form; step-33
 * (where we actually use AD) is an even more extreme example. -
 * <b>Verification:</b> For materials and simulations that exhibit nonlinear
 * response, an accurate rather than only approximate material tangent (the
 * term mechanical engineers use for the derivative of a material law) can be
 * the difference between convergent and divergent behavior, especially at
 * high external (or coupling) loads. As the complexity of the problem
 * increases, so do the opportunities to introduce subtle (or, perhaps,
 * not-so-subtle) errors that produce predictably negative results.
 * Additionally, there is a lot to be gained by verifying that the
 * implementation is completely correct. For example, certain categories of
 * problems are known to exhibit instabilities, and therefore when you start
 * to lose quadratic convergence in a nonlinear solver (e.g., Newton's method)
 * then this may not be a huge surprise to the investigator. However, it is
 * hard (if not impossible) to distinguish between convergence behavior that
 * is produced as you near an unstable solution and when you simply have an
 * error in the material or finite element linearization, and start to drift
 * off the optimal convergence path due to that. Having a method of verifying
 * the correctness of the implementation of a constitutive law linearization,
 * for example, is perhaps the only meaningful way that you can use to catch
 * such errors, assuming that you've got nobody else to scrutinize your code.
 * Thankfully, with some tactical programming it is quite straight-forward to
 * structure a code for reuse, such that you can use the same classes in
 * production code and directly verify them in, for instance, a unit-test
 * framework.  This tutorial program will have two parts: One where we just
 * introduce the basic ideas of automatic and symbolic differentiation support
 * in deal.II using a simple set of examples; and one where we apply this to a
 * realistic but much more complicated case. For that second half, the next
 * section will provide some background on magneto-mechanical materials -- you
 * can skip this section if all you want to learn about is what AD and SD
 * actually are, but you probably want to read over this section if you are
 * interested in how to apply AD and SD for concrete situations.   <a
 * name="Theoryformagnetomechanicalmaterials"></a><h3>Theory for
 * magneto-mechanical materials</h3>   <a
 * name="Thermodynamicprinciples"></a><h4>Thermodynamic principles</h4>   As a
 * prelude to introducing the coupled magneto-mechanical material law that
 * we'll use to model a magneto-active polymer, we'll start with a very
 * concise summary of the salient thermodynamics to which these constitutive
 * laws must subscribe. The basis for the theory, as summarized here, is
 * described in copious detail by Truesdell and Toupin @cite Truesdell1960a
 * and Coleman and Noll @cite Coleman1963a, and follows the logic laid out by
 * Holzapfel @cite Holzapfel2007a.  Starting from the first law of
 * thermodynamics, and following a few technical assumptions, it can be shown
 * the the balance between the kinetic plus internal energy rates and the
 * power supplied to the system from external sources is given by the
 * following relationship that equates the rate of change of the energy in an
 * (arbitrary) volume $V$ on the left, and the sum of forces acting on that
 * volume on the right:
 * @f[
 ...
 * + R_{0} \right] dV .
 * @f]
 * Here $D_{t}$ represents the total time derivative, $\rho_{0}$ is the
 * material density as measured in the Lagrangian reference frame,
 * $\mathbf{v}$ is the material velocity and $\mathbf{a}$ its acceleration,
 * $U^{*}_{0}$ is the internal energy per unit reference volume,
 * $\mathbf{P}^{\text{tot}}$ is the total Piola stress tensor and
 * $\dot{\mathbf{F}}$ is the time rate of the deformation gradient tensor,
 * $\boldsymbol{\mathbb{H}}$ and $\boldsymbol{\mathbb{B}}$ are, respectively,
 * the magnetic field vector and the magnetic induction (or magnetic flux
 * density) vector, $\mathbb{E}$ and $\mathbb{D}$ are the electric field
 * vector and electric displacement vector, and $\mathbf{Q}$ and $R_{0}$
 * represent the referential thermal flux vector and thermal source. The
 * material differential operator $\nabla_{0} (\bullet) \dealcoloneq
 * \frac{d(\bullet)}{d\mathbf{X}}$ where $\mathbf{X}$ is the material position
 * vector. With some rearrangement of terms, invoking the arbitrariness of the
 * integration volume $V$, the total internal energy density rate
 * $\dot{E}_{0}$ can be identified as
 * @f[
 * \dot{E}_{0}
 * = \mathbf{P}^{\text{tot}} : \dot{\mathbf{F}}
 * + \boldsymbol{\mathbb{H}} \cdot \dot{\boldsymbol{\mathbb{B}}}
 ...
 * mechanical deformation (the first term), and thermal fluxes and sources
 * (the fourth and fifth terms), but also due to the intrinsic energy stored
 * in the magnetic and electric fields themselves (the second and third terms,
 * respectively).  The second law of thermodynamics, known also as the entropy
 * inequality principle, informs us that certain thermodynamic processes are
 * irreversible. After accounting for the total entropy and rate of entropy
 * input, the Clausius-Duhem inequality can be derived. In local form (and in
 * the material configuration), this reads
 * @f[
 * \theta \dot{\eta}_{0}
 * - R_{0}
 * + \nabla_{0} \cdot \mathbf{Q}
 ...
 * \geq 0 .
 * @f]
 * The quantity $\theta$ is the absolute temperature, and $\eta_{0}$
 * represents the entropy per unit reference volume.  Using this to replace
 * $R_{0} - \nabla_{0} \cdot \mathbf{Q}$ in the result stemming from the first
 * law of thermodynamics, we now have the relation
 * @f[
 * \mathbf{P}^{\text{tot}} : \dot{\mathbf{F}}
 * + \boldsymbol{\mathbb{H}} \cdot \dot{\boldsymbol{\mathbb{B}}}
 * + \mathbb{E} \cdot \dot{\mathbb{D}}
 ...
 * \geq 0 .
 * @f]
 * On the basis of Fourier's law, which informs us that heat flows from
 * regions of high temperature to low temperature, the last term is always
 * positive and can be ignored. This renders the local dissipation inequality
 * @f[
 * \mathbf{P}^{\text{tot}} : \dot{\mathbf{F}}
 * + \boldsymbol{\mathbb{H}} \cdot \dot{\boldsymbol{\mathbb{B}}}
 * + \mathbb{E} \cdot \dot{\mathbb{D}}
 ...
 * = E_{0} - \theta \eta_{0} ,
 * @f]
 * from which we may define the free energy density function $\psi^{*}_{0}$
 * with the stated parameterization, exists and is valid. Taking the material
 * rate of this equation and substituting it into the local dissipation
 * inequality results in the generic expression
 * @f[
 * \mathcal{D}_{\text{int}}
 * = \mathbf{P}^{\text{tot}} : \dot{\mathbf{F}}
 * + \boldsymbol{\mathbb{H}} \cdot \dot{\boldsymbol{\mathbb{B}}}
 ...
 * \geq 0 .
 * @f]
 * Under the assumption of isothermal conditions, and that the electric field
 * does not excite the material in a manner that is considered non-negligible,
 * then this dissipation inequality reduces to
 * @f[
 * \mathcal{D}_{\text{int}}
 * = \mathbf{P}^{\text{tot}} : \dot{\mathbf{F}}
 * + \boldsymbol{\mathbb{H}} \cdot \dot{\boldsymbol{\mathbb{B}}}
 ...
 * \geq 0 .
 * @f]
 *  <a name="Constitutivelaws"></a><h4>Constitutive laws</h4>   When
 * considering materials that exhibit mechanically dissipative behavior, it
 * can be shown that this can be captured within the dissipation inequality
 * through the augmentation of the material free energy density function with
 * additional parameters that represent internal variables @cite
 ...
 * \geq 0 .
 * @f]
 * where $\mathbf{F}_{v}^{i} = \mathbf{F}_{v}^{i} \left( t \right)$ represents
 * the internal variable (which acts like a measure of the deformation
 * gradient) associated with the `i`th mechanical dissipative (viscous)
 * mechanism. As can be inferred from its parameterization, each of these
 * internal parameters is considered to evolve in time. Currently the free
 * energy density function $\psi^{*}_{0}$ is parameterized in terms of the
 * magnetic induction $\boldsymbol{\mathbb{B}}$. This is the natural
 * parameterization that comes as a consequence of the considered balance
 * laws. Should such a class of materials to be incorporated within a
 * finite-element model, it would be ascertained that a certain formulation of
 * the magnetic problem, known as the magnetic vector potential formulation,
 * would need to be adopted. This has its own set of challenges, so where
 * possible the more simple magnetic scalar potential formulation may be
 * preferred. In that case, the magnetic problem needs to be parameterized in
 * terms of the magnetic field $\boldsymbol{\mathbb{H}}$. To make this
 * re-parameterization, we execute a final Legendre transformation
 * @f[
 ...
 * indifference in order to express the energy density function in terms of
 * symmetric deformation measures:
 * @f[
 * \psi_{0} \left( \mathbf{C}, \mathbf{C}_{v}^{i}, \boldsymbol{\mathbb{H}} \right)
 * = \tilde{\psi}_{0} \left( \mathbf{F}, \mathbf{F}_{v}^{i}, \boldsymbol{\mathbb{H}} \right) .
 * @f]
 * The upshot of these two transformations (leaving out considerable explicit
 * and hidden details) renders the final expression for the reduced
 * dissipation inequality as
 * @f[
 * \mathcal{D}_{\text{int}}
 ...
 * transfer of the time derivative to the magnetic induction vector.) The
 * stress quantity $\mathbf{S}^{\text{tot}}$ is known as the total
 * Piola-Kirchhoff stress tensor and its energy conjugate $\mathbf{C} =
 * \mathbf{F}^{T} \cdot \mathbf{F}$ is the right Cauchy-Green deformation
 * tensor, and $\mathbf{C}_{v}^{i} = \mathbf{C}_{v}^{i} \left( t \right)$ is
 * the re-parameterized internal variable associated with the `i`th mechanical
 * dissipative (viscous) mechanism.  Expansion of the material rate of the
 * energy density function, and rearrangement of the various terms, results in
 * the expression
 * @f[
 * \mathcal{D}_{\text{int}}
 ...
 * \geq 0 .
 * @f]
 * At this point, its worth noting the use of the [partial
 * derivatives](https://en.wikipedia.org/wiki/Partial_derivative) $\partial
 * \left( \bullet \right)$. This is an important detail that will be
 * fundamental to a certain design choice made within the tutorial. As brief
 * reminder of what this signifies, the partial derivative of a multi-variate
 * function returns the derivative of that function with respect to one of
 * those variables while holding the others constant:
 * @f[
 * \frac{\partial f\left(x, y\right)}{\partial x}
 * = \frac{d f\left(x, y\right)}{d x} \Big\vert_{y} .
 * @f]
 * More specific to what's encoded in the dissipation inequality (with the
 * very general free energy density function $\psi_{0}$ with its
 * parameterization yet to be formalized), if one of the input variables is a
 * function of another, it is also held constant and the chain rule does not
 * propagate any further, while the computing total derivative would imply
 * judicious use of the chain rule. This can be better understood by comparing
 ...
 * @f}
 *  Returning to the thermodynamics of the problem, we next exploit the
 * arbitrariness of the quantities $\dot{\mathbf{C}}$ and
 * $\dot{\boldsymbol{\mathbb{H}}}$, by application of the Coleman-Noll
 * procedure @cite Coleman1963a, @cite Coleman1967a. This leads to the
 * identification of the kinetic conjugate quantities
 * @f[
 * \mathbf{S}^{\text{tot}}
 ...
 * @f]
 * (Again, note the use of the partial derivatives to define the stress and
 * magnetic induction in this generalized setting.) From what terms remain in
 * the dissipative power (namely those related to the mechanical dissipative
 * mechanisms), if they are assumed to be independent of one another then, for
 * each mechanism `i`,
 * @f[
 * \frac{\partial \psi_{0}}{\partial \mathbf{C}_{v}^{i}} : \dot{\mathbf{C}}_{v}^{i}
 * \leq 0 .
 * @f]
 * This constraint must be satisfied through the appropriate choice of free
 * energy function, as well as a carefully considered evolution law for the
 * internal variables.  In the case that there are no dissipative mechanisms
 * to be captured within the constitutive model (e.g., if the material to be
 * modelled is magneto-hyperelastic) then the free energy density function
 * $\psi_{0} = \psi_{0} \left( \mathbf{C}, \boldsymbol{\mathbb{H}} \right)$
 * reduces to a stored energy density function, and the total stress and
 * magnetic induction can be simplified
 * @f{align*}{
 * \mathbf{S}^{\text{tot}}
 ...
 * completeness, the linearization of the stress tensor and magnetic induction
 * are captured within the fourth-order total referential elastic tangent
 * tensor $\mathcal{H}^{\text{tot}} $, the second-order magnetostatic tangent
 * tensor $\mathbb{D}$ and the third-order total referential magnetoelastic
 * coupling tensor $\mathfrak{P}^{\text{tot}}$. Irrespective of the
 * parameterization of $\mathbf{S}^{\text{tot}}$ and
 ...
 * derivative during the calculation of the first derivatives. We'll see later
 * how this affects the choice of AD versus SD for this specific application.
 * For now, we'll simply introduce the two specific materials that are
 * implemented within this tutorial.  <a
 * name="Magnetoelasticconstitutivelaw"></a><h5>Magnetoelastic constitutive
 * law</h5>   The first material that we'll consider is one that is governed
 * by a magneto-hyperelastic constitutive law. This material responds to both
 * deformation as well as immersion in a magnetic field, but exhibits no time-
 * or history-dependent behavior (such as dissipation through viscous damping
 * or magnetic hysteresis, etc.). The *stored energy density function* for
 * such a material is only parameterized in terms of the (current) field
 * variables, but not their time derivatives or past values.  We'll choose the
 * energy density function, which captures both the energy stored in the
 * material due to deformation and magnetization, as well as the energy stored
 * in the magnetic field itself, to be
 * @f[
 ...
 * @f]
 * and for which the variable $d = \text{tr}(\mathbf{I})$ ($\mathbf{I}$ being
 * the rank-2 identity tensor) represents the spatial dimension and
 ...
 * here and the Neohookean material is the scaling of the elastic shear
 ...
 * stated in its current form, but when the magnetic induction and
 * magnetization are derived from $\psi_{0}$ and all magnetic fields are
 * expressed in the <em>current configuration</em> then this correlation
 * becomes clear.) As for the specifics of what the magnetic induction, stress
 * tensor, and the various material tangents look like, we'll defer presenting
 * these to the tutorial body where the full, unassisted implementation of the
 * constitutive law is defined.  <a
 * name="Magnetoviscoelasticconstitutivelaw"></a><h5>Magneto-viscoelastic
 * constitutive law</h5>   The second material that we'll formulate is one for
 * a magneto-viscoelastic material with a single dissipative mechanism `i`.
 ...
 *  - \mathbf{C}_{v} \right]
 * @f]
 * for the internal viscous variable. We've chosen the magnetoelastic part of
 * the energy $\psi_{0}^{ME} \left( \mathbf{C}, \boldsymbol{\mathbb{H}}
 * \right)$ to match that of the first material model that we explored, so
 * this part needs no further explanation. As for the viscous part
 * $\psi_{0}^{MVE}$, this component of the free energy (in conjunction with
 * the evolution law for the viscous deformation tensor) is taken from @cite
 * Linder2011a (with the additional scaling by the viscous saturation function
 * described in @cite Pelteret2018a). It is derived in a thermodynamically
 * consistent framework that, at its core, models the movement of polymer
 * chains on a micro-scale level.  To proceed beyond this point, we'll also
 * need to consider the time discretization of the evolution law. Choosing the
 * implicit first-order backwards difference scheme, then
 * @f[
 * \dot{\mathbf{C}}_{v}
 ...
 * \right]
 * @f]
 * that matches @cite Linder2011a equation 54.  <a
 * name="Rheologicalexperiment"></a><h3>Rheological experiment</h3>   Now that
 * we have shown all of these formulas for the thermodynamics and theory
 * governing magneto-mechanics and constitutive models, let us outline what
 * the program will do with all of this. We wish to do something *meaningful*
 * with the materials laws that we've formulated, and so it makes sense to
 * subject them to some mechanical and magnetic loading conditions that are,
 * in some way, representative of some conditions that might be found either
 * in an application or in a laboratory setting. One way to achieve that aim
 * would be to embed these constitutive laws in a finite element model to
 * simulate a device. In this instance, though, we'll keep things simple (we
 * are focusing on the automatic and symbolic differentiation concepts, after
 * all) and will find a concise way to faithfully replicate an
 * industry-standard rheological experiment using an analytical expression for
 * the loading conditions.  The rheological experiment that we'll reproduce,
 * which idealizes a laboratory experiment that was used to characterize
 * magneto-active polymers, is detailed in @cite Pelteret2018a (as well as
 * @cite Pelteret2019a, in which it is documented along with the real-world
 * experiments). The images below provide a visual description of the problem
 * set up.  <table align="center" class="tutorial" cellspacing="3"
 ...
 * linear, then the displacement at some measurement point $\mathbf{X}$ within
 ...
 * &= \lambda_{3} X_{3}
 * @f}
 * where $R(X_{1}, X_{2})$ and $\Theta(X_{1}, X_{2})$ are the radius at -- and
 * angle of -- the sampling point, $\lambda_{3}$ is the (constant) axial
 * deformation, $\tau(t) = \frac{A}{RH} \sin\left(\omega t\right)$ is the
 * time-dependent torsion angle per unit length that will be prescribed using
 * a sinusoidally repeating oscillation of fixed amplitude $A$. The magnetic
 * field is aligned axially, i.e., in the $X_{3}$ direction.  This summarizes
 * everything that we need to fully characterize the idealized loading at any
 * point within the rheological sample. We'll set up the problem in such a way
 * that we "pick" a representative point with this sample, and subject it to a
 * harmonic shear deformation at a constant axial deformation (by default, a
 * compressive load) and a constant, axially applied magnetic field. We will
 * record the stress and magnetic induction at this point, and will output
 * that data to file for post-processing. Although its not necessary for this
 * particular problem, we will also be computing the tangents as well. Even
 * though they are not directly used in this particular piece of work, these
 * second derivatives are needed to embed the constitutive law within a finite
 * element model (one possible extension to this work). We'll therefore take
 * the opportunity to check our hand calculations for correctness using the
 * assisted differentiation frameworks.  <a
 * name="Suggestedliterature"></a><h3>Suggested literature</h3>
 * In addition
 * to the already mentioned @ref auto_symb_diff module, the following are a
 * few references that discuss in more detail - magneto-mechanics, and some
 * aspects of automated differentiation frameworks: @cite Pao1978a, @cite
 * Pelteret2019a, and - the automation of finite element frameworks using AD
 * and/or SD: @cite Logg2012a, @cite Korelc2016a.
 * <br>
 ...
 * differentiation</h3>    Automatic and symbolic differentiation have some
 * magical and mystical qualities. Although their use in a project can be
 * beneficial for a multitude of reasons, the barrier to understanding how to
 * use these frameworks or how they can be leveraged may exceed the patience
 * of the developer that is trying to (reliably) integrate them into their
 * work.    Although it is the wish of the author to successfully illustrate
 * how these tools can be integrated into workflows for finite element
 * modelling, it might be best to first take a step back and start right from
 * the basics. So to start off with, we'll first have a look at
 * differentiating a "simple" mathematical function using both frameworks, so
 * that the fundamental operations (both their sequence and function) can be
 * firmly established and understood with minimal complication. In the second
 * part of this tutorial we will put these fundamentals into practice and
 * build on them further.    Accompanying the description of the algorithmic
 * steps to use the frameworks will be a simplified view as to what they
 * *might* be doing in the background. This description will be very much one
 * designed to aid
 * understanding, and
 * the reader is encouraged to view the @ref auto_symb_diff module
 * documentation for a far more formal description into how these tools
 * actually work.     <a name="Ananalyticalfunction"></a> <h4>An analytical
 * function</h4>
 * @code
 ...
 * understands the rules, we wouldn't have to bother with doing it ourselves.
 ...
 * <h4>Computing derivatives using automatic differentiation</h4>    To begin,
 * we'll use AD as the tool to automatically compute derivatives for us. We
 * will evaluate the function with the arguments `x` and `y`, and expect the
 * resulting value and all of the derivatives to match to within the given
 * tolerance.
 * @code
 *   void
 ...
 * Differentiation::AD::ScalarFunction class is the appropriate wrapper class
 * to use to do the computations that we require. (As a point of comparison,
 * if the function arguments represented finite element cell
 ...
 * arguments (what we will call "independent variables") the function $f(x,y)$
 * has. Those arguments are `x` and `y`, so obviously there are two of them.
 * @code
 ...
 * component number `0`, and `y` will be component `1` -- a detail that will
 * be used in the next few lines.
 * @code
 ...
 * return derivatives?    In essence, how this *could* be done is the
 * following: This special number can be viewed as a data structure that
 * stores the function value, and the prescribed number of derivatives. For a
 ...
 * mathematical operations, the AD framework *could* overload the operations
 ...
 * the independent variables `x` and `y`) *might* be defined as follows:
 ...
 * }
 * @endcode
 * </div>    All of that could of course also be done for second and even
 * higher order derivatives.    So it is now clear that with the above
 * representation the `ADNumberType` is carrying around some extra data that
 * represents the various derivatives of differentiable functions with respect
 * to the original (sensitive) independent variables. It should therefore be
 * noted that there is computational overhead associated with using them (as
 * we compute extra functions when doing derivative computations) as well as
 ...
 * Differentiation::AD::VectorFunction helper class to determine the gradient
 * of the collection of dependent functions, which would be the second
 * derivatives of the original scalar function.    It is also worth noting
 * that because the chain rule is indiscriminately applied and we only see the
 * beginning and end-points of the calculation `{x,y}` $\rightarrow$ `f(x,y)`,
 * we will only ever be able to query the total derivatives of `f`; the
 * partial derivatives (`a.derivatives[0]` and `a.derivatives[1]` in the above
 * example) are intermediate values and are hidden from us.    Okay, since we
 * now at least have some idea as to exactly what `f_ad` represents and what
 * is encoded within it, let's put all of that to some actual use. To gain
 * access to those hidden derivative results, we register the final result
 * with the helper class. After this point, we can no longer change the value
 * of `f_ad` and have those changes reflected in the results returned by the
 * helper class.
 * @code
 ...
 * second-order derivatives of this trigonometric function.     <a
 * name="Handcalculatedderivativesoftheanalyticalsolution"></a>
 * <h4>Hand-calculated derivatives of the analytical solution</h4>    Since we
 * now know how much "implementation effort" it takes to have the AD framework
 * compute those derivatives for us, let's compare that to the same computed
 * by hand and implemented in several stand-alone functions.    Here are the
 * two first derivatives of $f(x,y) = \cos\left(\frac{y}{x}\right)$:
 ...
 * + y \cos\left(\frac{y}{x}\right))$ (as expected, on the basis of [Schwarz's
 ...
 * affect our results.    The point of this example of course is that we might
 * have chosen a relatively simple function $f(x,y)$ for which we can
 * hand-verify that the derivatives the AD framework computed is correct. But
 * the AD framework didn't care that the function was simple: It could have
 * been a much much more convoluted expression, or could have depended on more
 * than two variables, and it would still have been able to compute the
 * derivatives -- the only difference would have been that *we* wouldn't have
 * been able to come up with the derivatives any more to verify correctness of
 * the AD framework.           <a
 * name="Computingderivativesusingsymbolicdifferentiation"></a> <h4>Computing
 * derivatives using symbolic differentiation</h4>    We'll now repeat the
 * same exercise using symbolic differentiation. The term "symbolic
 * differentiation" is a little bit misleading because differentiation is just
 * one tool that the Computer Algebra System (CAS) (i.e., the symbolic
 * framework) provides. Nevertheless, in the context of finite element
 * modeling and applications it is the most common use of a CAS and will
 * therefore be the one that we'll focus on. Once more, we'll supply the
 * argument values `x` and `y` with which to evaluate our function $f(x,y) =
 * \cos\left(\frac{y}{x}\right)$ and its derivatives, and a tolerance with
 * which to test the correctness of the returned results.
 * @code
 *   void
 ...
 * initializing a symbolic type Differentiation::SD::Expression, which is a
 * wrapper to a set of classes used by the symbolic framework, with a unique
 * identifier. On this occasion it makes sense that this identifier, a
 * `std::string`, be simply `"x"` for the $x$ argument, and likewise `"y"` for
 * the $y$ argument to the dependent function. Like before, we'll suffix
 * symbolic variable names with `sd` so that we can clearly see which
 * variables are symbolic (as opposed to numeric) in nature.
 * @code
 ...
 * @endcode
 * </div> we would see `f(x,y) = cos(y/x)` printed to the console.    You
 * might notice that we've constructed our symbolic function `f_sd` with no
 * context as to how we might want to use it: In contrast to the AD approach
 * shown above, what we were returned from calling `f(x_sd, y_sd)` is not the
 ...
 * it makes sense that we somehow want to associate "x" and "y" with *some*
 ...
 * by $1$; or, more simply: if the function did an operation like `1+2`, a CAS
 * could replace it by `3`), and so forth: The *expression* that a variable
 * represents is obtained from how the function $f$ is implemented, but a CAS
 * can do with it whatever its functionality happens to be.    Specifically,
 ...
 * use the Differentiation::SD::Expression::differentiate() function with the
 * independent variable given as its argument. Each call will cause the CAS to
 * go through the tree of operations that compose `f_sd` and differentiate
 * each node of the expression tree with respect to the given symbolic
 * argument.
 * @code
 ...
 * simply differentiate the first derivatives with respect to the independent
 ...
 * by the CAS, using the statements <div class=CodeFragmentInTutorialComment>
 * @code
 ...
 * derivatives that were presented earlier.    Now that we have formed the
 * symbolic expressions for the function and its derivatives, we want to
 * evaluate them for the numeric values for the main function arguments `x`
 * and `y`. To accomplish this, we construct a *substitution map*, which maps
 * the symbolic values to their numerical counterparts.
 * @code
 ...
 * variable in the step we have already mentioned above: "substitution".
 * Once we pass this substitution map to the CAS, it will substitute each
 * instance of the symbolic variable (or, more generally, sub-expression) with
 * its numerical counterpart and then propagate these results up the operation
 ...
 * complete.    Due to the strongly-typed nature of C++, we need to instruct
 * the CAS to convert its representation of the result into an intrinsic data
 * type (in this case a `double`). This is the "evaluation" step, and through
 * the template type we define the return type of this process. Conveniently,
 ...
 * function</h4>    The function used to drive these initial examples is
 * straightforward. We'll arbitrarily choose some values at which to evaluate
 * the function (although knowing that `x = 0` is not permissible), and then
 * pass these values to the functions that use the AD and SD frameworks.
 * @code
 *   void run()
 *   {
 *     const double x = 1.23;
 ...
 * compute derivatives at continuum points</h3>    Now that we've introduced
 * the principles behind automatic and symbolic differentiation, we'll put
 * them into action by formulating two coupled magneto-mechanical constitutive
 * laws: one that is rate-independent, and another that exhibits
 * rate-dependent behavior.    As you will recall from the introduction, the
 * material constitutive laws we will consider are far more complicated than
 * the simple example above. This is not just because of the form of the
 * function $\psi_{0}$ that we will consider, but in particular because
 * $\psi_{0}$ doesn't just depend on two scalar variables, but instead on a
 * whole bunch of *tensors*, each with several components. In some cases,
 * these are *symmetric* tensors, for which only a subset of components is in
 * fact independent, and one has to think about what it actually means to
 * compute a derivative such as $\frac{\partial\psi_{0}}{\partial \mathbf{C}}$
 * where $\mathbf C$ is a symmetric tensor. How all of this will work will,
 * hopefully, become clear below. It will also become clear that doing this by
 * hand is going to be, at the very best, *exceedingly* tedious* and, at
 * worst, riddled with hard-to-find bugs.
 * @code
 ...
 * purpose of this program of course.    The first four constitutive
 * parameters respectively represent - the elastic shear modulus $\mu_{e}$, -
 * the elastic shear modulus at magnetic saturation $\mu_{e}^{\infty}$, - the
 * saturation magnetic field strength for the elastic shear modulus
 * $h_{e}^{\text{sat}}$, and - the Poisson ratio $\nu$.
 * @code
 ...
 * parameters for - the viscoelastic shear modulus $\mu_{v}$, - the
 * viscoelastic shear modulus at magnetic saturation $\mu_{v}^{\infty}$, - the
 * saturation magnetic field strength for the viscoelastic shear modulus
 * $h_{v}^{\text{sat}}$, and - the characteristic relaxation time $\tau$.
 * @code
 *     double mu_v       = 20.0e3;
 ...
 * class</h4>    Since we'll be formulating two constitutive laws for the same
 * class of materials, it makes sense to define a base class that ensures a
 * unified interface to them.    The class declaration starts with the
 * constructor that will accept the set of constitutive parameters that, in
 * conjunction with the material law itself, dictate the material response.
 * @code
 *   template <int dim>
 ...
 * $\mathbf{C}$, as well as the time discretizer. These, in addition to the @p
 * constitutive_parameters, are all the fundamental quantities that are
 ...
 *
 * @endcode
 *  ... as well as the two kinetic quantities: - the magnetic induction vector
 * $\boldsymbol{\mathbb{B}}$, and - the total Piola-Kirchhoff stress tensor
 * $\mathbf{S}^{\text{tot}}$
 * @code
 ...
 *
 * @endcode
 *  ... and the linearization of the kinetic quantities, which are: - the
 * magnetostatic tangent tensor $\mathbb{D}$, - the total referential
 * magnetoelastic coupling tensor $\mathfrak{P}^{\text{tot}}$, and - the total
 * referential elastic tangent tensor $\mathcal{H}^{\text{tot}}$.
 * @code
 ...
 * parameters (both explicitly defined, as well as calculated).    The
 * parameters related to the elastic response of the material are, in order: -
 * the elastic shear modulus, - the elastic shear modulus at saturation
 * magnetic field, - the saturation magnetic field strength for the elastic
 * shear modulus, - the Poisson ratio, - the Lam&eacute; parameter, and - the
 * bulk modulus.
 * @code
 *   protected:
 *     const ConstitutiveParameters &constitutive_parameters;
 ...
 * order: - the viscoelastic shear modulus, - the viscoelastic shear modulus
 * at magnetic saturation, - the saturation magnetic field strength for the
 * viscoelastic shear modulus, and - the characteristic relaxation time.
 * @code
 ...
 * order: - the relative magnetic permeability, and - the magnetic
 * permeability constant $\mu_{0}$ (not really a material constant, but rather
 * a universal constant that we'll group here for simplicity).    We'll also
 * implement a function that returns the timestep size from the time
 * discretizion.
 * @code
 ...
 * We'll begin by considering a non-dissipative material, namely one that is
 * governed by a magneto-hyperelastic constitutive law that exhibits
 * stiffening when immersed in a magnetic field. As described in the
 * introduction, the stored energy density function for such a material might
 * be given by
 * @f[
 * \psi_{0} \left( \mathbf{C}, \boldsymbol{\mathbb{H}} \right)
 * = \frac{1}{2} \mu_{e} f_{\mu_{e}} \left( \boldsymbol{\mathbb{H}} \right)
 ...
 * {\left(h_{e}^{\text{sat}}\right)^{2}} \right) .
 * @f]
 *    Now on to the class that implements this behavior. Since we expect that
 ...
 * the @p ad_helper. The @p ad_helper returns the derivatives with respect to
 * all field variables at once, so we'll retain the full gradient vector and
 * Hessian matrix. From that, we'll extract the individual entries that we're
 ...
 * libraries work, the @p ad_helper will always returns the derivatives of the
 * energy density function with respect to all field variables simultaneously.
 * For this reason, it does not make sense to compute the derivatives in the
 * functions `get_B()`, `get_S()`, etc. because we'd be doing a lot of extra
 * computations that are then simply discarded. So, the best way to deal with
 * that is to have a single function call that does all of the calculations
 * up-front, and then we extract the stored data as its needed. That's what
 * we'll do in the `update_internal_data()` method. As the material is
 * rate-independent, we can ignore the DiscreteTime argument.
 * @code
 *   template <int dim, Differentiation::AD::NumberTypes ADTypeCode>
 *   void
 ...
 *
 * @endcode
 *  Since we reuse the @p ad_helper data structure at each time step, we need
 * to clear it of all stale information before use.
 * @code
 *     ad_helper.reset();
 *
 * @endcode
 ...
 * they'll be used repeatedly to ensure that we have the correct
 * interpretation of which variable corresponds to which component of `H` or
 * `C`.
 * @code
 ...
 * differentiation)</h4>    The second material law that we'll consider will
 * be one that represents a magneto-viscoelastic material with a single
 * dissipative mechanism. We'll consider the free energy density function for
 * such a material to be defined as
 * @f{align*}{
 * \psi_{0} \left( \mathbf{C}, \mathbf{C}_{v}, \boldsymbol{\mathbb{H}}
 * \right)
 * &= \psi_{0}^{ME} \left( \mathbf{C}, \boldsymbol{\mathbb{H}} \right)
 ...
 * approach. To support this, the class constructor accepts not only the @p
 * constitutive_parameters, but also two additional variables that will be
 * used to initialize a Differentiation::SD::BatchOptimizer. We'll give more
 * context to this later.
 * @code
 *   template <int dim>
 ...
 * Differentiation::SD::BatchOptimizer will return a collection of results all
 * at once. So, in order to do that just once, we'll utilize a similar
 ...
 * non-symbolic) member variables respectively hold - the value of internal
 * variable time step (and, if embedded within a nonlinear solver framework,
 * Newton step), and - the value of internal variable at the previous
 * timestep.    (We've labeled these variables "Q" so that they're easy to
 * identify; in a sea of calculations it is not necessarily easy to
 * distinguish `Cv` or `C_v` from `C`.)
 * @code
 *   private:
 ...
 * same order as they appear in the @p ConstitutiveParameters class). We also
 * store a symbolic expression, @p delta_t_sd, that represents the time step
 * size):
 * @code
 ...
 * Although we'll only compute them once, we require them to retrieve data
 * from the @p optimizer that is declared below. Furthermore, when serializing
 * a material class like this one (not done as a part of this tutorial) we'd
 ...
 * Differentiation::SD::BatchOptimizer class provides a mechanism by which to
 * transform the symbolic expression tree into another code path that, for
 ...
 * this @p optimizer across several materials (with the same energy function,
 * of course) and potentially multiple continuum points (if embedded within a
 * finite element simulation).    As specified by the template parameter, the
 * numerical result will be of type <tt>double</tt>.
 * @code
 ...
 * optimization (acceleration) technique should be applied, as well as which
 * additional steps should be taken by the CAS to help improve performance
 * during evaluation.
 * @code
 *   template <int dim>
 ...
 * function that is calling into this @p
 * Magnetoviscoelastic_Constitutive_Law_SD instance), and - the current and
 * previous internal viscous deformation (with their values stored within this
 * class instance).
 * @code
 *   template <int dim>
 *   Differentiation::SD::types::substitution_map
 ...
 * procedure to configure the @p optimizer looks very similar to that which is
 * used to construct the automatic differentiation helper. Nevertheless, we'll
 * detail these steps again to highlight the differences that underlie the two
 * frameworks.    The function starts with expressions that symbolically
 * encode the determinant of the deformation gradient (as expressed in terms
 * of the right Cauchy-Green deformation tensor, our primary field variable),
 * as well as the inverse of $\mathbf{C}$ itself:
 * @code
 *   template <int dim>
 ...
 * component of the energy density function; its expression is stated in @cite
 * Pelteret2018a (equation 28), which is a straight-forward extension of an
 * energy density function formulated in @cite Linder2011a (equation 46).
 * @code
 ...
 *
 * @endcode
 *  As it stands, to the CAS the variable @p Q_t_sd appears to be independent
 * of @p C_sd. Our tensorial symbolic expression @p Q_t_sd just has an
 * identifier associated with it, and there is nothing that links it to the
 * other tensorial symbolic expression @p C_sd. So any derivatives taken with
 * respect to @p C_sd will ignore this inherent dependence which, as we can
 * see from the evolution law, is in fact $\mathbf{C}_{v} = \mathbf{C}_{v}
 * \left( \mathbf{C}, t \right)$. This means that deriving any function $f =
 * f(\mathbf{C}, \mathbf{Q})$ with respect to  $\mathbf{C}$ will return
 * partial derivatives $\frac{\partial f(\mathbf{C}, \mathbf{Q})}{\partial
 * \mathbf{C}} \Big\vert_{\mathbf{Q}}$ as opposed to the total derivative
 * $\frac{d f(\mathbf{C}, \mathbf{Q}(\mathbf{C}))}{d \mathbf{C}} =
 * \frac{\partial f(\mathbf{C}, \mathbf{Q}(\mathbf{C}))}{\partial \mathbf{C}}
 ...
 * linearization.    But, first, we'll compute the symbolic expressions for
 * the kinetic variables, i.e., the magnetic induction vector and the
 * Piola-Kirchhoff stress tensor. The code that performs the differentiation
 * quite closely mimics the definition stated in the theory.
 * @code
 ...
 * to inform the CAS of the explicit dependency of @p Q_t_sd on @p C_sd, i.e.,
 * state that $\mathbf{C}_{v} = \mathbf{C}_{v} \left( \mathbf{C}, t \right)$.
 * This means that all future differential operations made with respect to @p
 * C_sd will take into account this dependence (i.e., compute total
 * derivatives). In other words, we will transform some expression such that
 * their intrinsic parameterization changes from $f(\mathbf{C}, \mathbf{Q})$
 * to $f(\mathbf{C}, \mathbf{Q}(\mathbf{C}))$.    To do this, we consider the
 * time-discrete evolution law. From that, we have the explicit expression for
 * the internal variable in terms of its history as well as the primary field
 * variable. That is what it described in this expression:
 * @code
 ...
 * instance of @p Q_t_sd (our identifier) found in an expression and replace
 * it with the full expression held in @p Q_t_sd_explicit.
 * @code
 ...
 * functions that the @p optimizer must evaluate. They are, collectively, the
 * independent variables for the problem, the history variables, the time step
 * sie and the constitutive parameters (since we've not hard encoded them in
 * the energy density function).    So what we really want is to provide it a
 ...
 * material classes</h3>    Now that we've seen how the AD and SD frameworks
 * can make light(er) work of defining these constitutive laws, we'll
 * implement the equivalent classes by hand for the purpose of verification
 * and to do some preliminary benchmarking of the frameworks versus a native
 * implementation.    At the expense of the author's sanity, what is
 * documented below (hopefully accurately) are the full definitions for the
 * kinetic variables and their tangents, as well as some intermediate
 * computations. Since the structure and design of the constitutive law
 * classes has been outlined earlier, we'll gloss over it and simply delineate
 * between the various stages of calculations in the `update_internal_data()`
 * method definition. It should be easy enough to link the derivative
 * calculations (with their moderately expressive variable names) to their
 * documented definitions that appear in the class descriptions. We will,
 * however, take the opportunity to present two different paradigms for
 * implementing constitutive law classes. The second will provide more
 * flexibility than the first (thereby making it more easily extensible, in
 * the author's opinion) at the expense of some performance.     <a
 * name="Magnetoelasticconstitutivelawhandderived"></a> <h4>Magnetoelastic
 * constitutive law (hand-derived)</h4>    From the stored energy that, as
 ...
 * @f]
 * for this magnetoelastic material, the first derivatives that correspond to
 * the magnetic induction vector and total Piola-Kirchhoff stress tensor are
 * @f[
 ...
 * \left[ \mathbf{C}^{-1} \cdot \boldsymbol{\mathbb{H}} \right]
 * @f]
 * The use of the symmetry operator $\text{sym} \left( \bullet \right)$ in the
 * one derivation above helps to ensure that the resulting rank-4 tensor,
 * which holds minor symmetries due to the symmetry of $\mathbf{C}$, still
 * maps rank-2 symmetric tensors to rank-2 symmetric tensors. See the
 * SymmetricTensor class documentation and the introduction to step-44 and for
 * further explanation as to what symmetry means in the context of
 * fourth-order tensors.    The linearization of each of the kinematic
 ...
 * \right]
 * @f}
 *    Well, that escalated quickly -- although the the definition of
 * $\psi_{0}$ and $f_{\mu_e}$ might have given some hints that the calculating
 * the kinetic fields and their linearization would take some effort, it is
 * likely that there's a little more complexity to the final definitions that
 * perhaps initially thought. Knowing what we now do, it's probably fair to
 * say that we really do not want to compute first and second derivatives of
 * these functions with respect to their arguments -- regardless of well we
 * did in calculus classes, or how good a programmer we may be.    In the
 * class method definition where these are ultimately implemented, we've
 * composed these calculations slightly differently. Some intermediate steps
 * are also retained to give another perspective of how to systematically
 * compute the derivatives. Additionally, some calculations are decomposed
 * less or further to reuse some of the intermediate values and, hopefully,
 ...
 * \text{sech}^{2}(x)}{dx} = -2 \tanh(x) \text{sech}^{2}(x)$.
 * @code
 ...
 * <h4>Magneto-viscoelastic constitutive law (hand-derived)</h4>    As
 * mentioned before, the free energy density function for the
 * magneto-viscoelastic material with one dissipative mechanism that we'll be
 ...
 * - \mathbf{C}_{v} \right]
 * @f]
 * that itself is parameterized in terms of $\mathbf{C}$. By design, the
 * magnetoelastic part of the energy $\psi_{0}^{ME} \left( \mathbf{C},
 * \boldsymbol{\mathbb{H}} \right)$ is identical to that of the magnetoelastic
 * material presented earlier. So, for the derivatives of the various
 * contributions stemming from this part of the energy, please refer to the
 * previous section. We'll continue to highlight the specific contributions
 * from those terms by superscripting the salient terms with $ME$, while
 * contributions from the magneto-viscoelastic component are superscripted
 * with $MVE$. Furthermore, the magnetic saturation function
 * $f_{\mu_{v}}^{MVE} \left( \boldsymbol{\mathbb{H}} \right)$ for the damping
 * term has the identical form as that of the elastic term (i.e.,
 * $f_{\mu_{e}}^{ME} \left( \boldsymbol{\mathbb{H}} \right)$ ), and so the
 * structure of its derivatives are identical to that seen before; the only
 * change is for the three constitutive parameters that are now associated
 * with the viscous shear modulus $\mu_{v}$ rather than the elastic shear
 * modulus $\mu_{e}$.    For this magneto-viscoelastic material, the first
 * derivatives that correspond to the magnetic induction vector and total
 * Piola-Kirchhoff stress tensor are
 * @f[
 ...
 * \right]
 * @f]
 * will also dictate how the linearization of the internal variable with
 * respect to the field variables is composed.    Observe that in order to
 * attain the *correct* expressions for the magnetic induction vector and
 * total Piola-Kirchhoff stress tensor for this dissipative material, we must
 * adhere strictly to the outcome of applying the Coleman-Noll procedure: we
 * must take *partial derivatives* of the free energy density function with
 * respect to the field variables. (For our non-dissipative magnetoelastic
 * material, taking either partial or total derivatives would have had the
 * same result, so there was no need to draw your attention to this before.)
 ...
 * derivatives of $\psi_{0}^{MVE} \left( \mathbf{C}, \mathbf{C}_{v} \left(
 * \mathbf{C} \right), \boldsymbol{\mathbb{H}} \right)$ with respect to
 * $\mathbf{C}$ -- the dependence of $\mathbf{C}_{v}^{(t)}$ on $\mathbf{C}$ is
 * not to be taken into account. When deciding whether to use AD or SD to
 * perform this task the choice is clear -- only the symbolic framework
 * provides a mechanism to do this; as was mentioned before, AD can only
 * return total derivatives so it is unsuitable for the task.    To wrap
 * things up, we'll present the material tangents for this rate-dependent
 * coupled material. The linearization of both kinetic variables with respect
 * to their arguments are
 * @f[
 ...
 * \right] .
 * @f]
 * Notice that just the last term of $\mathcal{H}^{\text{tot}, MVE}$ contains
 * the tangent of the internal variable. The linearization of this particular
 * evolution law is linear. For an example of a nonlinear evolution law, for
 * which this linearization must be solved for in an iterative manner, see
 ...
 * We'll see shortly precisely how this can be leveraged to make the part of
 * the code where we actually perform calculations clean and easy (well, at
 * least easier) to follow and maintain. But for now, we can say that it will
 * allow us to move the parts of the code where we compute the derivatives of
 ...
 * function, and all of its derivatives:    The kinematic, or field,
 * variables.
 * @code
 ...
 * function, with the required constitutive parameters passed as arguments to
 * each function.
 * @code
 ...
 *
 * @endcode
 *  Get the values for the elastic and viscous saturation function based on
 ...
 * components: - the "Neo-Hookean"-like term, - the rate-dependent term, and -
 ...
 * indirectly) from them.    If the cache does not already store the value
 * that we're looking for, then we quickly calculate it, store it in the cache
 * and return the value just stored in the cache. That way we can return it as
 * a reference and avoid copying the object. The same goes for any values that
 * a compound function might depend on. Said another way, if there is a
 * dependency chain of calculations that come before the one that we're
 * currently interested in doing, then we're guaranteed to resolve the
 * dependencies before we proceed with using any of those values. Although
 * there is a cost to fetching data from the cache, the "resolved dependency"
 * concept might be sufficiently convenient to make it worth looking past the
 * extra cost. If these material laws are embedded within a finite element
 ...
 * parameters</h4>    The @p RheologicalExperimentParameters class is used to
 * drive the numerical experiments that are to be conducted on the coupled
 * materials that we've implemented constitutive laws for.
 * @code
 ...
 *
 * @endcode
 *  The three steady-state loading parameters are respectively - the axial
 * stretch, - the shear strain amplitude, and - the axial magnetic field
 * strength.
 * @code
 ...
 * conditions are - the loading cycle frequency, - the number of load cycles,
 * and - the number of discrete timesteps per cycle.
 * @code
 ...
 *
 * @endcode
 *  The applied deformation (gradient) is computed based on the geometry of
 * the rheometer and the sample, the sampling point, and the experimental
 ...
 *
 * @endcode
 *  We fetch and compute the loading to be applied to the material at this
 * time step...
 * @code
 *         const Tensor<1, dim> H =
 ...
 *           {
 * @endcode
 *  The next thing that we will do is collect some results to post-process.
 * All quantities are in the "current configuration" (rather than the
 * "reference configuration", in which all quantities computed by the
 * constitutive laws are framed).
 * @code
 *             const Tensor<1, dim> h =
 ...
 * CoupledConstitutiveLaws::run() function</h4>    The purpose of this driver
 * function is to read in all of the parameters from file and, based off of
 * that, create a representative instance of each constitutive law and invoke
 * the function that conducts a rheological experiment with it.
 * @code
 ...
 *
 * @endcode
 *   <a name="Themainfunction"></a> <h3>The main() function</h3>    The main
 * function only calls the driver functions for the two sets of examples that
 * are to be executed.
 * @code
 ...
 * name="Introductoryexample"></a><h3>Introductory example</h3>   The first
 * exploratory example produces the following output. It is verified that all
 * three implementations produce identical results.
 * @code
 * > ./step-71
 ...
 * ... all calculations are correct!
 * @endcode
 *  <a name="Constitutivemodelling"></a><h3>Constitutive modelling</h3>   To
 * help summarize the results from the virtual experiment itself, below are
 * some graphs showing the shear stress, plotted against the shear strain, at
 * a select location within the material sample. The plots show the
 * stress-strain curves under three different magnetic loads, and for the last
 ...
 * plots](https://en.wikipedia.org/wiki/Lissajous_curve). The area of the
 * ellipse that the curve takes for viscoelastic materials provides some
 * measure of how much energy is dissipated by the material, and its
 * ellipticity indicates the phase shift of the viscous response with respect
 * to the elastic response.  <table align="center" class="tutorial"
 ...
 * unloading curve that matches the loading curve -- the material is
 * non-dissipative after all. But here it's clearly noticeable how the
 * gradient of the curve increases as the applied magnetic field increases.
 * The tangent at any point along this curve is related to the instantaneous
 * shear modulus and, due to the way that the energy density function was
 * defined, we expect that the shear modulus increases as the magnetic field
 * strength increases. We observe much the same behavior for the
 * magneto-viscoelastic material. The major axis of the ellipse traced by the
 * loading-unloading curve has a slope that increases as a greater magnetic
 * load is applied. At the same time, the more energy is dissipated by the
 * material.  As for the code output, this is what is printed to the console
 * for the part pertaining to the rheological experiment conducted with the
 ...
 * the magneto-viscoelastic material:
 * @code
 * Coupled magneto-viscoelastic constitutive law using symbolic differentiation.
 ...
 * taken to perform the hand- and assisted- calculations and get some idea of
 * the overhead of using the AD and SD frameworks. Here are the timings taken
 * from the magnetoelastic experiment using the AD framework, based on the
 ...
 * circumstance-dependent as to whether or not this is acceptable or not: Do
 * you value computer time more than human time for doing the necessary
 * hand-computations of derivatives, verify their correctness, implement them,
 * and verify the correctness of the implementation? If you develop a research
 * code that will only be run for a relatively small number of experiments,
 * you might value your own time more. If you develop a production code that
 * will be run over and over on 10,000-core clusters for hours, your
 * considerations might be different. In any case, the one nice feature of the
 * AD approach is the "drop in" capability when functions and classes are
 * templated on the scalar type. This means that minimal effort is required to
 * start working with it.  In contrast, the timings for magneto-viscoelastic
 * material as implemented using just-in-time (JIT) compiled symbolic algebra
 * indicate that, at some non-negligible cost during initialization, the
 * calculations themselves are a lot more efficiently executed:
 * @code
 * +---------------------------------------------+------------+------------+
 ...
 * magneto-viscoelastic constitutive law has more terms to calculate when
 * compared to its magnetoelastic counterpart, it still is a whole order of
 * magnitude faster to execute the computations of the kinetic variables and
 * tangents. And when compared to the hand computed variant that uses the
 * caching scheme, the calculation time is nearly equal. So although using the
 * symbolic framework requires a paradigm shift in terms of how one implements
 ...
 * added cost of value caching for the magneto-viscoelastic material
 * implementation is, in fact, about a $6\times$ increase in the time spent in
 * `update_internal_data()` when compared to the implementation using
 * intermediate values for the numerical experiments conducted with this
 * material. Here's a sample output of the timing comparison extracted for the
 ...
 * optimization schemes for the batch optimizer. So let's compare the
 * computational expense associated with the `LLVM` batch optimizer setting
 ...
 * spent in the "Assisted computation" section when compared to the `LLVM`
 ...
 * the evaluation within the native CAS framework itself, with no
 * transformation of the underlying data structures taking place. Only the use
 * of CSE, which caches intermediate results, will provide any "acceleration"
 * in this instance. With that in mind, here are the results from this
 * selection:
 * @code
 * +---------------------------------------------+------------+------------+
 ...
 * quite some time to produce... The `dictionary` substitution method is
 * perhaps only really viable for simple expressions or when the number of
 * calls is sufficiently small.  <a
 * name="SowhichframeworkshouldIuse"></a><h1>So, which framework should I
 * use?</h1>   Perhaps you've been convinced that these tools have some merit,
 * and can be of immediate help or use to you. The obvious question now is
 * which one to use. Focusing specifically at a continuum point level, where
 * you would be using these frameworks to compute derivatives of a
 * constitutive law in particular, we can say the following: - Automatic
 * differentiation probably provides the simplest entry point into the world
 * of assisted differentiation. - Given a sufficiently generic implementation
 * of a constitutive framework, AD can often be used as a drop-in replacement
 * for the intrinsic scalar types and the helper classes can then be leveraged
 * to compute first (and possibly higher order) derivatives with minimal
 * effort. - As a qualification to the above point, being a "drop-in
 * replacement" does not mean that you must not be contentious of what the
 * algorithms that these numbers are being passed through are doing. It is
 * possible to inadvertently perform an operation that would, upon
 * differentiating, return an incorrect result. So this is definitely
 * something that one should be aware of. A concrete example: When computing
 * the eigenvalues of a tensor, if the tensor is diagonal then a short-cut to
 * the result is simply to return the diagonal entries directly (as extracted
 * from the input tensor). This is completely correct in terms of computing
 * the eigenvalues themselves, but not going through the algorithm that would
 * otherwise compute the eigenvalues for a non-diagonal tensor has had an
 * unintended side-effect, namely that the eigenvalues appear (to the AD
 * framework) to be completely decoupled from one another and their
 * cross-sensitivities are not encoded in the returned result. Upon
 * differentiating, many entries of the derivative tensor will be missing. To
 * fix this issue, one has to ensure that the standard eigenvalue solving
 * algorithm is used so that the sensitivities of the returned eigenvalues
 * with respect to one another are encoded in the result. - Computations
 * involving AD number types may be expensive. The expense increases
 ...
 * specific. - AD is restricted to the case where only total derivatives are
 * required. If a differential operation requires a partial derivative with
 * respect to an independent variable then it is not appropriate to use it. -
 * Each AD library has its own quirks (sad to say but, in the author's
 * experience, true), so it may take some trial and error to find the
 * appropriate library and choice of AD number to suit your purposes. The
 * reason for these "quirks" often boils down to the overall philosophy behind
 * the library (data structures, the use of template meta-programming, etc.)
 * as well as the mathematical implementation of the derivative computations
 * (for example, manipulations of results using logarithmic functions to
 * change basis might restrict the domain for the input values -- details all
 * hidden from the user, of course). Furthermore, one library might be able to
 * compute the desired results quicker than another, so some initial
 * exploration might be beneficial in that regard. - Symbolic differentiation
 * (well, the use of a CAS in general) provides the most flexible framework
 * with which to perform assisted computations. - The SD framework can do
 * everything that the AD frameworks can, with the additional benefit of
 * having low-level control over when certain manipulations and operations are
 * performed. - Acceleration of expression evaluation is possible, potentially
 * leading to near-native performance of the SD framework compared to some
 * hand implementations (this comparison being dependent on the overall
 * program design, of course) at the expense of the initial optimization call.
 * - Clever use of the Differentiation::SD::BatchOptimizer could minimize the
 * expense of the costly call that optimizes the dependent expressions. The
 * possibility to serialize the Differentiation::SD::BatchOptimizer that often
 * (but not always) this expensive call can be done once and then reused in a
 * later simulation. - If two or more material laws differ by only their
 * material parameters, for instance, then a single batch optimizer can be
 * shared between them as long as those material parameters are considered to
 * be symbolic. The implication of this is that you can "differentiate once,
 * evaluate in many contexts". - The SD framework may partially be used as a
 * "drop-in replacement" for scalar types, but one (at the very least) has to
 * add some more framework around it to perform the value substitution step,
 * converting symbolic types to their numerical counterparts. - It may not be
 * possible to use SD numbers within some specialized algorithms. For example,
 * if an algorithm has an exit point or code branch based off of some
 * concrete, numerical value that the (symbolic) input argument should take,
 ...
 * represents the result returned by this algorithm, perhaps declaring it to
 * be a [symbolic
 * function](https://dealii.org/developer/doxygen/deal.II/namespaceDifferentiation_1_1SD.html#a876041f6048705c7a8ad0855cdb1bd7a)
 ...
 * calculations as substituted results.). - The biggest drawback to using SD
 * is that using it requires a paradigm shift, and that one has to frame most
 * problems differently in order to take the most advantage of it. (Careful
 * consideration of how the data structures are used and reused is also
 * essential to get it to work effectively.) This may mean that one needs to
 * play around with it a bit and build up an understanding of what the
 * sequence of typical operations is and what specifically each step does in
 * terms of manipulating the underlying data. If one has the time and
 * inclination to do so, then the benefits of using this tool may be
 * substantial.  <a name="Possibilitiesforextension"></a><h1>Possibilities for
 * extension</h1>   There are a few logical ways in which this program could
 * be extended: - Perhaps the most obvious extension would be to implement and
 * test other constitutive models. This could still be within the realm of
 * coupled magneto-mechanical problems, perhaps considering alternatives to
 * the "Neo-Hookean"-type elastic part of the energy functions, changing the
 * constitutive law for the dissipative energy (and its associated evolution
 * law), or including magnetic hysteretic effects or damage models for the
 * composite polymer that these material seek to model. - Of course, the
 * implemented models could be modified or completely replaced with models
 * that are focused on other aspects of physics, such as electro-active
 * polymers, biomechanical materials, elastoplastic media, etc. - Implement a
 * different time-discretization scheme for the viscoelastic evolution law. -
 * Instead of deriving everything directly from an energy density function,
 * use the Differentiation::AD::VectorFunction to directly linearize the
 * kinetic quantities. This would mean that only a once-differentiable
 * auto-differentiable number type would be required, and would certainly
 * improve the performance greatly. Such an approach would also offer the
 * opportunity for dissipative materials, such as the magneto-viscoelastic one
 * consider here, to be implemented in conjunction with AD. This is because
 * the linearization invokes the total derivative of the dependent variables
 * with respect to the field variables, which is exactly what the AD
 * frameworks can provide. - Investigate using other auto-differentiable
 * number types and frameworks (such as ADOL-C). Since each AD library has its
 * own implementation, the choice of which to use could result in performance
 * increases and, in the most unfortunate cases, more stable computations. It
 * can at least be said that for the AD libraries that deal.II supports, the
 * accuracy of results should be largely unaffected by this decision. - Embed
 * one of these constitutive laws within a finite element simulation.  With
 * less effort, one could think about re-writing nonlinear problem solvers
 * such as the one implemented in step-15 using AD or SD approaches to compute
 * the Newton matrix. Indeed, this is done in step-72.   <a
 * name="PlainProg"></a> <h1> The plain program</h1>
@include "step-71.cc"
 */
