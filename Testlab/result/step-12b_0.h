 ...
 * This tutorial depends on step-16, step-7, step-39.  @htmlonly <table
 ...
 * name="Introduction"></a><h1>Introduction</h1>   This is a variant of
 * step-16 with the only change that we are using the MeshWorker framework
 * with the pre-made LocalIntegrator helper classes instead of assembling the
 * face terms using FEInterfaceValues.  The details of this framework on how
 * it is used in practice will be explained as part of this tutorial program.
 * <a name="Thetestcase"></a><h3>The testcase</h3>   The problem we solve here
 * is the same as the one in step-12.   <a name="CommProg"></a> <h1> The
 ...
 * this program, called AdvectionProblem. It is basically the main class of
 * step-6. We do not have an AffineConstraints object, because there are no
 * hanging node constraints in DG discretizations.    Major differences will
 * only come up in the implementation of the assemble functions, since here,
 * we not only need to cover the flux integrals over faces, we also use the
 * MeshWorker interface to simplify the loops involved.
 * @code
 * template <int dim>
 ...
 * cells and much of the setup of operations will be done outside this class,
 * so all we have to provide are these three operations. They will then work
 * on intermediate objects for which first, we here define alias to the info
 ...
 * variables of the current class, so we can mark them as <code>static</code>
 * and simply pass pointers to these functions to the MeshWorker framework.
 * If, however, these functions would want to access member variables (or
 * needed additional arguments beyond the ones specified below), we could use
 * the facilities of lambda functions to provide the MeshWorker framework with
 * objects that act as if they had the required number and types of arguments,
 * but have in fact other arguments already bound.
 * @code
 ...
 * Instead of writing loops over cells and faces, we leave all this to the
 * MeshWorker framework. In order to do so, we just have to define local
 * integration functions and use one of the classes in namespace
 * MeshWorker::Assembler to build the global system.
 * @code
 * template <int dim>
 ...
 * }
 * @endcode
 * <a name="Results"></a><h1>Results</h1>    The output of this program is
 * very similar to step-16 and we are not repeating the output here.  We show
 * the solutions on the initial mesh, the mesh after two and after five
 ...
 * alt="">   Then we show the final grid (after 5 refinement steps) and the
 * solution again, this time with a nicer 3d rendering (obtained using the
 * DataOutBase::write_vtk function and the VTK-based VisIt visualization
 * program) that better shows the sharpness of the jump on the refined mesh
 * and the over- and undershoots of the solution along the interface:  <img
 ...
 * For ideas for further extensions, please see see step-12.   <a
