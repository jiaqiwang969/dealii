 ...
 * This tutorial depends on step-22.  @htmlonly <table class="tutorial"
 ...
 * <li><a href="#ThecodeNavierStokesProjectioncodeclass">The
 * <code>NavierStokesProjection</code> class</a>
 * <ul>
 * <li><a href="#codeNavierStokesProjectionNavierStokesProjectioncode">
 * <code>NavierStokesProjection::NavierStokesProjection</code> </a>
 * <li><a
 * href="#codeNavierStokesProjectioncreate_triangulation_and_dofscode"><code>NavierStokesProjection::create_triangulation_and_dofs</code></a>
 * <li><a href="#codeNavierStokesProjectioninitializecode">
 * <code>NavierStokesProjection::initialize</code> </a>
 * <li><a href="#codeNavierStokesProjectioninitialize__matricescode">
 * <code>NavierStokesProjection::initialize_*_matrices</code> </a>
 * <li><a href="#codeNavierStokesProjectionruncode">
 * <code>NavierStokesProjection::run</code> </a>
 * <li><a
 * href="#codeNavierStokesProjectiondiffusion_stepcode"><code>NavierStokesProjection::diffusion_step</code></a>
 * <li><a href="#codeNavierStokesProjectionassemble_advection_termcode">
 * <code>NavierStokesProjection::assemble_advection_term</code> </a>
 * <li><a
 * href="#codeNavierStokesProjectionprojection_stepcode"><code>NavierStokesProjection::projection_step</code></a>
 * <li><a href="#codeNavierStokesProjectionupdate_pressurecode">
 * <code>NavierStokesProjection::update_pressure</code> </a>
 * <li><a href="#codeNavierStokesProjectionoutput_resultscode">
 * <code>NavierStokesProjection::output_results</code> </a>
 * </ul>
 ...
 * @endhtmlonly
 * <br>
 *  <i> This program grew out of a student project by Abner Salgado at Texas
 * A&M University. Most of the work for this program is by him. </i>  <a
 * name="Intro"></a> <a name="Introduction"></a><h1> Introduction </h1>   <a
 * name="Motivation"></a> <a name="Motivation"></a><h3> Motivation </h3>  The
 * purpose of this program is to show how to effectively solve the
 * incompressible time-dependent Navier-Stokes equations. These equations
 * describe the flow of a viscous incompressible fluid and read
 * @f{align*}
 * u_t + u \cdot \nabla u - \nu \Delta u + \nabla p = f, \\
 * \nabla \cdot u = 0,
 * @f}
 * where $u$ represents the velocity of the flow and $p$ the pressure. This
 * system of equations is supplemented by the initial condition
 * @f[
 * u |_{t=0} = u_0,
 * @f]
 * with $u_0$ sufficiently smooth and solenoidal, and suitable boundary
 * conditions. For instance, an admissible boundary condition, is
 * @f[
 * u|_{\partial\Omega} = u_b.
 * @f]
 * It is possible to prescribe other boundary conditions as well. In the test
 * case that we solve here the boundary is partitioned into two disjoint
 * subsets $\partial\Omega = \Gamma_1 \cup \Gamma_2$ and we have
 * @f[
 * u|_{\Gamma_1} = u_b,
 * @f]
 * and
 ...
 * @f]
 * where $n$ is the outer unit normal. The boundary conditions on $\Gamma_2$
 * are often used to model outflow conditions.  In previous tutorial programs
 * (see for instance step-20 and step-22) we have seen how to solve the
 * time-independent Stokes equations using a Schur complement approach. For
 * the time-dependent case, after time discretization, we would arrive at a
 * system like
 * @f{align*}
 * \frac1\tau u^k - \nu \Delta u^k + \nabla p^k = F^k, \\
 * \nabla \cdot u^k = 0,
 * @f}
 * where $\tau$ is the time-step. Although the structure of this system is
 * similar to the Stokes system and thus it could be solved using a Schur
 * complement approach, it turns out that the condition number of the Schur
 * complement is proportional to $\tau^{-2}$. This makes the system very
 * difficult to solve, and means that for the Navier-Stokes equations, this is
 * not a useful avenue to the solution.  <a name="Projection"></a> <a
 * name="Projectionmethods"></a><h3> Projection methods </h3>   Rather, we
 * need to come up with a different approach to solve the time-dependent
 * Navier-Stokes equations. The difficulty in their solution comes from the
 * fact that the velocity and the pressure are coupled through the constraint
 * @f[
 * \nabla \cdot u = 0,
 * @f]
 * for which the pressure is the Lagrange multiplier. Projection methods aim
 * at decoupling this constraint from the diffusion (Laplace) operator.  Let
 * us shortly describe how the projection methods look like in a semi-discrete
 * setting. The objective is to obtain a sequence of velocities $\{u^k\}$ and
 * pressures $\{p^k\}$. We will also obtain a sequence $\{\phi^k\}$ of
 * auxiliary variables. Suppose that from the initial conditions, and an
 * application of a first order method we have found $(u^0,p^0,\phi^0=0)$ and
 * $(u^1,p^1,\phi^1=p^1-p^0)$. Then the projection method consists of the
 * following steps:
 * <ul>
 ...
 * @f]
 * <li> <b>Step 1</b>: Diffusion step. We find $u^{k+1}$ that solves the
 ...
 * <ul>
 * <li> <i>Incremental Method in Standard Form</i>. The pressure is updated
 * by:
 *    @f[
 *      p^{k+1} = p^k + \phi^{k+1}.
 *    @f]
 ...
 * </ul>
 * </ul>
 *  Without going into details, let us remark a few things about the
 * projection methods that we have just described:
 * <ul>
 * <li> The advection term $u\cdot\nabla u$ is replaced by its <i>skew
 * symmetric form</i>
 * @f[
 *  u \cdot \nabla u + \frac12 \left( \nabla\cdot u \right) u.
 * @f]
 * This is consistent with the continuous equation (because $\nabla\cdot u =
 * 0$, though this is not true pointwise for the discrete solution) and it is
 * needed to guarantee unconditional stability of the time-stepping scheme.
 * Moreover, to linearize the term we use the second order extrapolation
 * $u^\star$ of $u^{k+1}$.
 ...
 * @f]
 * with $v^{k+1}\in H$. Taking the divergence of this equation we arrive at
 * the projection equation.
 * <li> The more accurate of the two variants outlined above is the rotational
 * one. However, the program below implements both variants. Moreover, in the
 * author's experience, the standard form is the one that should be used if,
 * for instance, the viscosity $\nu$ is variable.
 * </ul>
 *   <p> The standard incremental scheme and the rotational incremental scheme
 * were first considered by van Kan in
 * <ul>
 * <li> J. van Kan, "A second-order accurate pressure-correction scheme for
 * viscous incompressible flow", SIAM Journal on Scientific and Statistical
 * Computing, vol. 7, no. 3, pp. 870–891, 1986
 * </ul>
 * and is analyzed by Guermond in
 * <ul>
 * <li> J.-L. Guermond, "Un résultat de convergence d’ordre deux en temps
 * pour l’approximation des équations de Navier–Stokes par une technique
 * de projection incrémentale", ESAIM: Mathematical Modelling and Numerical
 * Analysis, vol. 33, no. 1, pp. 169–189, 1999
 * </ul>
 * for the case $\nu = 1$. It turns out that this technique suffers from
 * unphysical boundary conditions for the kinematic pressure that lead to
 * reduced rates of convergence. To prevent this, Timmermans et al. proposed
 * in
 * <ul>
 * <li> L. Timmermans, P. Minev, and F. Van De Vosse, "An approximate
 * projection scheme for incompressible flow using spectral elements",
 * International Journal for Numerical Methods in Fluids, vol. 22, no. 7, pp.
 * 673–688, 1996
 * </ul>
 * the rotational pressure-correction projection method that uses a divergence
 * correction for the kinematic pressure. A thorough analysis for scheme has
 * first been performed in
 * <ul>
 * <li> J.-L. Guermond and J. Shen, "On the error estimates for the rotational
 * pressure-correction projection methods", Mathematics of Computation, vol.
 * 73, no. 248, pp. 1719–1737, 2004
 * </ul>
 * for the Stokes problem. </p>  <a name ="fullydiscrete"></a> <a
 * name="TheFullyDiscreteSetting"></a><h3> The Fully Discrete Setting </h3>
 * To obtain a fully discrete setting of the method we, as always, need a
 * variational formulation. There is one subtle issue here given the nature of
 * the boundary conditions. When we multiply the equation by a suitable test
 * function one of the term that arises is
 * @f[
 * -\nu \int_\Omega \Delta u \cdot v.
 * @f]
 * If we, say, had Dirichlet boundary conditions on the whole boundary then
 * after integration by parts we would obtain
 * @f[
 * -\nu \int_\Omega \Delta u \cdot v = \nu \int_\Omega \nabla u : \nabla v
 *                                  - \int_{\partial\Omega} \partial_n u \cdot v
 *                                  = \nu \int_\Omega \nabla u : \nabla v.
 * @f]
 * One of the advantages of this formulation is that it fully decouples the
 * components of the velocity. Moreover, they all share the same system
 * matrix. This can be exploited in the program.  However, given the
 * nonstandard boundary conditions, to be able to take them into account we
 * need to use the following %identity
 * @f[
 * \Delta u = \nabla\nabla\cdot u - \nabla\times\nabla\times u,
 * @f]
 * so that when we integrate by parts and take into account the boundary
 * conditions we obtain
 * @f[
 * -\nu \int_\Omega \Delta u \cdot v = \nu \int_\Omega \left[ \nabla \cdot u \nabla \cdot v
 *                                  + \nabla \times u \nabla \times v \right],
 * @f]
 * which is the form that we would have to use. Notice that this couples the
 * components of the velocity. Moreover, to enforce the boundary condition on
 * the pressure, we need to rewrite
 * @f[
 * \int_\Omega \nabla p \cdot v = -\int_\Omega p \nabla \cdot v + \int_{\Gamma_1} p v\cdot n
 *                              + \int_{\Gamma_2} p v\cdot n
 *                             = -\int_\Omega p \nabla \cdot v,
 * @f]
 * where the boundary integral in $\Gamma_1$ equals zero given the boundary
 * conditions for the velocity, and the one in $\Gamma_2$ given the boundary
 * conditions for the pressure.  In the simplified case where the boundary
 * $\Gamma_2$ is %parallel to a coordinate axis, which holds for the testcase
 * that we carry out below, it can actually be shown that
 * @f[
 * \nu \int_\Omega \nabla u : \nabla v = \nu \int_\Omega \left[ \nabla \cdot u \nabla \cdot v
 *                                  + \nabla \times u \nabla \times v \right].
 * @f]
 * This issue is not very often addressed in the literature. For more
 * information the reader can consult, for instance,
 * <ul>
 * <li> J.-L. GUERMOND, L. QUARTAPELLE, On the approximation of the unsteady
 * Navier-Stokes equations by finite element projection methods, Numer. Math.,
 * 80  (1998) 207-238
 * <li> J.-L. GUERMOND, P. MINEV, J. SHEN, Error analysis of
 * pressure-correction schemes for the Navier-Stokes equations with open
 * boundary conditions, SIAM J. Numer. Anal., 43  1 (2005) 239--258.
 * </ul>
 *    <a name = "implementation"></a> <a name="Implementation"></a><h3>
 * Implementation </h3>   Our implementation of the projection methods follows
 * <i>verbatim</i> the description given above. We must note, however, that as
 * opposed to most other problems that have several solution components, we do
 * not use vector-valued finite elements. Instead, we use separate finite
 * elements the components of the velocity and the pressure, respectively, and
 * use different <code>DoFHandler</code>'s for those as well. The main reason
 * for doing this is that, as we see from the description of the scheme, the
 * <code>dim</code> components of the velocity and the pressure are decoupled.
 * As a consequence, the equations for all the velocity components look all
 * the same, have the same system matrix, and can be solved in %parallel.
 * Obviously, this approach has also its disadvantages. For instance, we need
 * to keep several <code>DoFHandler</code>s and iterators synchronized when
 * assembling matrices and right hand sides; obtaining quantities that are
 * inherent to vector-valued functions (e.g. divergences) becomes a little
 * awkward, and others.  <a name ="testcase"></a> <a
 * name="TheTestcase"></a><h3> The Testcase </h3>   The testcase that we use
 * for this program consists of the flow around a square obstacle. The
 * geometry is as follows:  <img
 * src="https://www.dealii.org/images/steps/developer/step-35.geometry.png"
 * alt="">  with $H=4.1$, making the geometry slightly non-symmetric.  We
 * impose no-slip boundary conditions on both the top and bottom walls and the
 * obstacle. On the left side we have the inflow boundary condition
 * @f[
 * u =
 * \left( \begin{array}{c} 4 U_m y (H-y)/H^2 \\ 0 \end{array} \right),
 * @f]
 * with $U_m = 1.5$, i.e. the inflow boundary conditions correspond to
 * Poiseuille flow for this configuration. Finally, on the right vertical wall
 * we impose the condition that the vertical component of the velocity and the
 * pressure should both be zero. The final time $T=10$.   <a
 * name="CommProg"></a> <h1> The commented program</h1>   <a
 * name="Includefiles"></a> <h3>Include files</h3>    We start by including
 * all the necessary deal.II header files and some C++ related ones. Each one
 * of them has been discussed in previous tutorial programs, so we will not
 ...
 *
 * @endcode
 *   <a name="Runtimeparameters"></a> <h3>Run time parameters</h3>    Since
 * our method has several parameters that can be fine-tuned we put them into
 * an external file, so that they can be determined at run-time.    This
 * includes, in particular, the formulation of the equation for the auxiliary
 * variable $\phi$, for which we declare an <code>enum</code>. Next, we
 * declare a class that is going to read and store all the parameters that our
 ...
 *
 * @endcode
 *   <a name="Equationdata"></a> <h3>Equation data</h3>    In the next
 * namespace, we declare the initial and boundary conditions:
 * @code
 ...
 * do, however, want to use an interface for the equation data that is somehow
 * dimension independent. To be able to do that, our functions should be able
 * to know on which spatial component we are currently working, and we should
 * be able to have a common interface to do that. The following class is an
 ...
 *
 * @endcode
 *   <a name="ThecodeNavierStokesProjectioncodeclass"></a> <h3>The
 * <code>NavierStokesProjection</code> class</h3>    Now for the main class of
 * the program. It implements the various versions of the projection method
 * for Navier-Stokes equations. The names for all the methods and member
 * variables should be self-explanatory, taking into account the
 * implementation details given in the introduction.
 * @code
 * template <int dim>
 ...
 * parallel. They
 * follow the scheme laid out in @ref threads, using the WorkStream class. As
 * explained there, this requires us to declare two structures for each of the
 * assemblers, a per-task data and a scratch data structure. These are then
 * handed over to functions that assemble local contributions and that copy
 * these local contributions to the global objects.    One of the things that
 * are specific to this program is that we don't just have a single DoFHandler
 * object that represents both the velocities and the pressure, but we use
 * individual DoFHandler objects for these two kinds of variables. We pay for
 * this optimization when we want to assemble terms that involve both
 * variables, such as the divergence of the velocity and the gradient of the
 * pressure, times the respective test functions. When doing so, we can't just
 * anymore use a single FEValues object, but rather we need two, and they need
 * to be initialized with cell iterators that point to the same cell in the
 * triangulation but different DoFHandlers.    To do this in practice, we
 * declare a "synchronous" iterator -- an object that internally consists of
 * several (in our case two) iterators, and each time the synchronous
 * iteration is moved forward one step, each of the iterators stored
 * internally is moved forward one step as well, thereby always staying in
 * sync. As it so happens, there is a deal.II class that facilitates this sort
 * of thing. (What is important here is to know that two DoFHandler objects
 * built on the same triangulation will walk over the cells of the
 * triangulation in the same order.)
 * @code
 *   using IteratorTuple =
 ...
 *
 * @endcode
 *   <a name="codeNavierStokesProjectionNavierStokesProjectioncode"></a> <h4>
 * <code>NavierStokesProjection::NavierStokesProjection</code> </h4>    In the
 * constructor, we just read all the data from the <code>Data_Storage</code>
 * object that is passed as an argument, verify that the data we read is
 * reasonable and, finally, create the triangulation and load the initial
 * data.
 * @code
 * template <int dim>
 ...
 * @endcode
 *   <a
 * name="codeNavierStokesProjectioncreate_triangulation_and_dofscode"></a>
 ...
 *
 * @endcode
 *   <a name="codeNavierStokesProjectioninitializecode"></a> <h4>
 * <code>NavierStokesProjection::initialize</code> </h4>    This method
 * creates the constant matrices and loads the initial data
 * @code
 * template <int dim>
 ...
 *
 * @endcode
 *   <a name="codeNavierStokesProjectioninitialize__matricescode"></a> <h4>
 * <code>NavierStokesProjection::initialize_*_matrices</code> </h4>    In this
 * set of methods we initialize the sparsity patterns, the constraints (if
 * any) and assemble the matrices that do not depend on the timestep
 ...
 *
 * @endcode
 *   <a name="codeNavierStokesProjectionruncode"></a> <h4>
 * <code>NavierStokesProjection::run</code> </h4>    This is the time marching
 * function, which starting at <code>t_0</code> advances in time using the
 * projection method with time step <code>dt</code> until <code>T</code>.
 ...
 * substep; updating preconditioners etc. Rather than implementing this output
 * using code like <div class=CodeFragmentInTutorialComment>
 * @code
 * if (verbose) std::cout << "something";
 * @endcode
 * </div> we use the ConditionalOStream class to do that for us. That class
 * takes an output stream and a condition that indicates whether the things
 * you pass to it should be passed through to the given output stream, or
 * should just be ignored. This way, above code simply becomes <div
 * class=CodeFragmentInTutorialComment>
 * @code
 * verbose_cout << "something";
 * @endcode
 * </div> and does the right thing in either case.
 * @code
 * template <int dim>
 ...
 * <h4><code>NavierStokesProjection::diffusion_step</code></h4>    The
 * implementation of a diffusion step. Note that the expensive operation is
 * the diffusion solve at the end of the function, which we have to do once
 ...
 *
 * @endcode
 *   <a name="codeNavierStokesProjectionassemble_advection_termcode"></a> <h4>
 * <code>NavierStokesProjection::assemble_advection_term</code> </h4>    The
 * following few functions deal with assembling the advection terms, which is
 * the part of the system matrix for the diffusion step that changes at every
 * time step. As mentioned above, we will run the assembly loop over all cells
 * in %parallel, using the WorkStream class and other
 * facilities as
 * described in the documentation module on @ref threads.
 * @code
 * template <int dim>
 ...
 * <h4><code>NavierStokesProjection::projection_step</code></h4>    This
 * implements the projection step:
 * @code
 * template <int dim>
 ...
 *
 * @endcode
 *   <a name="codeNavierStokesProjectionupdate_pressurecode"></a> <h4>
 * <code>NavierStokesProjection::update_pressure</code> </h4>    This is the
 * pressure update step of the projection method. It implements
 ...
 *
 * @endcode
 *   <a name="codeNavierStokesProjectionoutput_resultscode"></a> <h4>
 * <code>NavierStokesProjection::output_results</code> </h4>    This method
 * plots the current solution. The main difficulty is that we want to create a
 * single output file that contains the data for all velocity components, the
 * pressure, and also the vorticity of the flow. On the other hand, velocities
 * and the pressure live on separate DoFHandler objects, and so can't be
 * written to the same file using a single DataOut object. As a consequence,
 * we have to work a bit harder to get the various pieces of data into a
 * single DoFHandler object, and then use that to drive graphical output.
 ...
 * DoFHandler object for all variables.    Let us also note that we here
 * compute the vorticity as a scalar quantity in a separate function, using
 * the $L^2$ projection of the quantity $\text{curl} u$ onto the finite
 * element space used for the components of the velocity. In principle,
 * however, we could also have computed as a pointwise quantity from the
 * velocity, and do so through the DataPostprocessor mechanism discussed in
 * step-29 and step-33.
 * @code
 * template <int dim>
 ...
 * function looks very much like in all the other tutorial programs, so there
 ...
 * name="Re100"></a> <a name="Re100"></a><h3> Re = 100 </h3>   We run the code
 * with the following <code>parameter-file.prm</code>, which can be found in
 ...
 * set verbose = false
 * @endverbatim
 *  Since the <code>verbose</code> parameter is set to <code>false</code>, we
 * do not get any kind of output besides the number of the time step the
 * program is currently working on. If we were to set it to <code>true</code>
 * we would get information on what the program is doing and how many steps
 * each iterative process had to make to converge, etc.  Let us plot the
 * obtained results for $t=1,5,12,20,25$ (i.e. time steps 200, 1000, 2400,
 * 4000, and 5000), where in the left column we show the vorticity and in the
 * right the velocity field:  <table> <tr> <td> <img
 * src="https://www.dealii.org/images/steps/developer/step-35.Re_100.vorticity.0.9.3.png"
 * alt="" width="400"> </td> <td> <img
 * src="https://www.dealii.org/images/steps/developer/step-35.Re_100.velocity.0.9.3.png"
 * alt="" width="400"> </td> </tr> <tr> <td> <img
 * src="https://www.dealii.org/images/steps/developer/step-35.Re_100.vorticity.1.9.3.png"
 * alt="" width="400"> </td> <td> <img
 * src="https://www.dealii.org/images/steps/developer/step-35.Re_100.velocity.1.9.3.png"
 * alt="" width="400"> </td> </tr> <tr> <td> <img
 * src="https://www.dealii.org/images/steps/developer/step-35.Re_100.vorticity.2.9.3.png"
 * alt="" width="400"> </td> <td> <img
 * src="https://www.dealii.org/images/steps/developer/step-35.Re_100.velocity.2.9.3.png"
 * alt="" width="400"> </td> </tr> <tr> <td> <img
 * src="https://www.dealii.org/images/steps/developer/step-35.Re_100.vorticity.3.9.3.png"
 * alt="" width="400"> </td> <td> <img
 * src="https://www.dealii.org/images/steps/developer/step-35.Re_100.velocity.3.9.3.png"
 * alt="" width="400"> </td> </tr> <tr> <td> <img
 * src="https://www.dealii.org/images/steps/developer/step-35.Re_100.vorticity.4.9.3.png"
 * alt="" width="400"> </td> <td> <img
 * src="https://www.dealii.org/images/steps/developer/step-35.Re_100.velocity.4.9.3.png"
 * alt="" width="400"> </td> </tr> </table>  The images show nicely the
 * development and extension of a vortex chain behind the obstacles, with the
 * sign of the vorticity indicating whether this is a left or right turning
 * vortex.   <a name="Re500"></a> <a name="Re500"></a><h3> Re = 500 </h3>   We
 * can change the Reynolds number, $Re$, in the parameter file to a value of
 * $500$. Doing so, and reducing the time step somewhat as well, yields the
 * following images at times $t=20,40$:  <table> <tr> <td> <img
 * src="https://www.dealii.org/images/steps/developer/step-35.Re_500.vorticity.0.9.3.png"
 * alt="" width="400"> </td> <td> <img
 * src="https://www.dealii.org/images/steps/developer/step-35.Re_500.velocity.0.9.3.png"
 * alt="" width="400"> </td> </tr> <tr> <td> <img
 * src="https://www.dealii.org/images/steps/developer/step-35.Re_500.vorticity.1.9.3.png"
 * alt="" width="400"> </td> <td> <img
 * src="https://www.dealii.org/images/steps/developer/step-35.Re_500.velocity.1.9.3.png"
 * alt="" width="400"> </td> </tr> </table>  For this larger Reynolds number,
 * we observe unphysical oscillations, especially for the vorticity. The
 * discretization scheme has now difficulties in correctly resolving the flow,
 * which should still be laminar and well-organized. These phenomena are
 * typical of discretization schemes that lack robustness in under-resolved
 * scenarios, where under-resolved means that the Reynolds number computed
 * with the mesh size instead of the physical dimensions of the geometry is
 * large. We look at a zoom at the region behind the obstacle, and the mesh
 * size we have there:   <img
 * src="https://www.dealii.org/images/steps/developer/step-35.Re_500.zoom.9.3.png"
 * alt="" width="400">  We can easily test our hypothesis by re-running the
 * simulation with one more mesh refinement set in the parameter file:  <img
 * src="https://www.dealii.org/images/steps/developer/step-35.Re_500.zoom_2.9.3.png"
 * alt="" width="400">  Indeed, the vorticity field now looks much smoother.
 * While we can expect that further refining the mesh will suppress the
 * remaining oscillations as well, one should take measures to obtain a robust
 * scheme in the limit of coarse resolutions, as described below.   <a
 * name="extensions"></a> <a name="Possibilitiesforextensions"></a><h3>
 * Possibilities for extensions </h3>   This program can be extended in the
 * following directions:
 * <ul>
 * <li> Adaptive mesh refinement: As we have seen, we computed everything on a
 * single fixed mesh. Using adaptive mesh refinement can lead to increased
 * accuracy while not significantly increasing the computational time.
 * <li> Adaptive time-stepping: Although there apparently is currently no
 * theory about projection methods with variable time step, practice shows
 * that they perform very well.
 * <li> High Reynolds %numbers: As we can see from the results, increasing the
 * Reynolds number changes significantly the behavior of the discretization
 * scheme. Using well-known stabilization techniques we could be able to
 * compute the flow in this, or many other problems, when the Reynolds number
 * is very large and where computational costs demand spatial resolutions for
 * which the flow is only marginally resolved, especially for 3D turbulent
 * flows.
 * <li> Variable density incompressible flows: There are projection-like
 * methods for the case of incompressible flows with variable density. Such
 * flows play a role if fluids of different density mix, for example fresh
 * water and salt water, or alcohol and water.
 * <li> Compressible Navier-Stokes equations: These equations are relevant for
 * cases where velocities are high enough so that the fluid becomes
 * compressible, but not fast enough that we get into a regime where viscosity
 * becomes negligible and the Navier-Stokes equations need to be replaced by
 * the hyperbolic Euler equations of gas dynamics. Compressibility starts to
 * become a factor if the velocity becomes greater than about one third of the
 * speed of sound, so it is not a factor for almost all terrestrial vehicles.
 * On the other hand, commercial jetliners fly at about 85 per cent of the
 * speed of sound, and flow over the wings becomes significantly supersonic, a
 * regime in which the compressible Navier-Stokes equations are not applicable
 * any more either. There are significant applications for the range in
 * between, however, such as for small aircraft or the fast trains in many
