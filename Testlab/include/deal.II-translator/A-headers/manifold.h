 ...
 * <h3>Overview</h3>
 * 本模块中的类涉及到流形的描述，一个三角形所描述的域就在其中。这种流形描述在一些情况下是必要的。
 * <ul>
 * <li>
 ...
 * 函数将此manifold_id用于三角形的单元、面或边上，这些单元、面或边应由该manifold描述，然后三角形将询问manifold对象在网格细化时应将一个新顶点置于何处。已经有几个类支持最常见的几何形状，例如，CylindricalManifold或PolarManifold，它们分别代表以圆柱坐标或极坐标描述空间时获得的几何形状。默认情况下，所有使用GridGenerator命名空间中的函数生成的弯曲几何体都会将正确的Manifold对象附加到域的弯曲部分。
 * <li>
 ...
 * 在deal.II中，一个流形被看作是一个点的集合，同时还有一个点与点之间的距离概念（在流形上）。新的点通常是通过在流形上提供一个局部坐标系来获得的，识别局部坐标系中的现有点（使用局部地图将其拉回，以获得其局部坐标），通过现有点的加权和找到局部坐标系中的新点，并将该点在实空间中转换回来（使用局部地图将其向前推）。实现这一机制的主要类是ChartManifold类，这也是用户可能为复杂的几何形状而重载的类。
 * 虽然这个过程在大多数感兴趣的情况下是非琐碎的，但对于大多数琐碎的几何体，如圆柱体、球体或壳体，deal.II提供了合理的实现。更复杂的例子可以用
 * step-53 和 step-54 中的技术来描述。
 * 在大的计划中，这个模块的类与库的其他各种部分相互作用。
 *
 ...
 *
 *  <h3>An example</h3>
 * 一个简单的例子，为什么要处理弯曲的几何图形，已经由
 * step-1
 * 提供了，虽然那里没有详细说明。默认情况下，GridGenerator中的函数会在需要时将流形附加到网格上。在下面的每个代码片段中，我们都会调用
 * Triangulation::reset_all_manifolds()
 * 来移除这些流形，并在例子本身中处理所有的流形附件，以使流形选择的影响清晰。
 * 考虑一下那里显示的 <code>second_grid()</code>
 * 函数的这个小变化，我们只是简单地提炼了<i>every</i>单元格几次。
 *
 * @code
 ...
 * 要创建的圆周方向的10个粗单元开始，然后每个单元都被全局细化3次。每次细化都需要一个新的顶点，它被放置在现有顶点的中间，而不考虑我们可能的意图（但在代码中省略了描述）。
 * 这很容易补救。考虑一下这段代码。
 *
 * @code
 * const Point<2> center (1,0);
 ...
 * 这个网格看起来更好，因为它忠实地再现了域的圆形内边界和外边界。然而，仍然可以在切线上发现20个结点。它们是由于每次细化单元时，内部线的新顶点只是被放置在现有线的中间（边界线的处理方式不同，因为我们附加了一个流形对象）。在有10个单元的第一次细化中，我们得到了改进的点，因为根据下面关于混合不同流形的描述，两个外部边界都提供了一个弯曲的描述。换句话说，第一次细化后的新点最终出现在可能处于直线的几何中间的地方，但不在围绕中心的圆上。
 ...
 * 这里，我们在开始时只创建了三个圆周单元，并对它们进行细化，从而得到所示的网格。很明显，尽管第一次细化将新的点放在中间，但我们有长宽比不好的单元。
 ...
 * 这个网格在细化后既没有正确的几何形状，也没有所有单元的正面积，这对于有限元方法的工作是必要的。然而，即使从这样一个不合适的网格开始，我们也可以通过在边界以及内部单元和边上附加一个合适的几何描述来使事情顺利进行，使用的代码与上面相同。
 *
 * @code
 * const Point<2> center (1,0);
 * const double inner_radius = 0.8,
 ...
 * 在这最后一个例子中，我们最终让GridGenerator完成了它的工作，我们保持了默认的流形配置，即每个单元和面都是SphericalManifold。
 * 在这里，即使从一个初始的、选择不当的网格开始，也保留了我们的能力，可以充分地将网格细化为一个对我们有利的网格。这个例子可能是在这里制造的，但它是相关的，例如在 GridGenerator::hyper_shell() 产生的3D的背景下（见这个函数的文档）。它也与 @ref GlossDistorted "关于扭曲细胞的词汇条目 "
 * 中讨论的案例有关。
 ...
 * 一般来说，在deal.II中混合不同流形描述的过程是通过所谓的transfinite插值实现的。它在二维中的公式，例如，在<a
 ...
 * 1}{\displaystyle 8}$
 * ，再次平衡了不同的实体。如果一个单元格的所有周围都是直的，那么这个公式就会减少到八个顶点中每个顶点的明显权重
 ...
 * $k=10$  。
 * 在文献中，也使用了其他的边界描述。在9.0版本之前，deal.II使用了一种叫做拉普拉斯平滑的东西，其中应用于圆周上的节点以获得内部节点的位置的权重是通过解决单位元素上的拉普拉斯方程来确定的。然而，这导致了接近曲线描述的边界层，即从单元到实心单元的映射的高导数的奇异性。
 * 如果从弯曲的边界描述到内部的直线描述的过渡做错了，通常不可能达到高阶收敛率。例如，单个单元内的拉普拉斯平滑会导致从参考单元到实数单元的映射的第四导数出现奇点，将边界处单元的收敛率限制在3（如果在二维测量全局L2误差，则为3.5）。其他更粗糙的策略，比如完全忽略两个不同流形的存在，只是简单地计算直坐标系中高阶映射的附加点，可能导致更差的收敛率。另一方面，目前在deal.II中的实现，在这方面已经得到了广泛的验证，应该表现得很好。
