 ...
 * algebra)</b></dt>。
 *  <dd>  将一个矩阵或向量作为单个块的集合来处理往往是很方便的。例如，在 step-20 （以及其他教程程序）中，我们要考虑全局线性系统 $Ax=b$ 的形式@f{eqnarray*}
 * \left(\begin{array}{cc}
 *  M & B^T \\ B & 0
 * \end{array}\right)
 * \left(\begin{array}{cc}
 ...
 * 每个块的最小尺寸是由底层有限元决定的（对于标量元素，一个块由一个分量组成，但以FE_RaviartThomas为例，一个块由<tt>dim</tt>分量组成）。然而，几个这样的最小块可以随意组合成用户定义的块，并根据应用情况进行组合。例如，对于<b>Q</b><sub>2</sub><sup><i>d</i></sup>-<b>Q</b><sub>1</sub>（Taylor-Hood）Stokes元素，有<i>d</i>+1个组件，原则上每个组件可以形成自己的块。但我们通常更感兴趣的是只有两个块，其中一个由所有的速度矢量分量组成（即这个块将有<i>d</i>分量），另一个只有一个压力分量。
 ...
 *                 FE_Q<dim>(1), 1);
 * @endcode
 * 另一方面，我们可以用以下方法产生一个具有dim+1块的类似对象
 *
 * @code
 *  FESystem<dim> (FE_Q<dim>(2), dim,
 *                 FE_Q<dim>(1), 1);
 * @endcode
 * 除了块的数量之外，这两个对象在所有实际用途上都是一样的。
 * <i>Global degrees of freedom:</i>
 * 虽然我们在上面用矢量值解函数的矢量分量（或者，等同于用矢量值有限元空间）来定义块，但有限元的每个形状函数都是一个或另一个块的组成部分。因此，我们可以将定义在DoFHandler上的所有自由度划分为各个块。由于默认情况下DoFHandler类以一种或多或少的随机方式列举自由度，你首先要调用
 ...
 * ）组成的一样，经常需要为那些不打算在有限元空间的<i>all</i>块上运行的操作选择一组此类块。使用BlockMask类来选择要操作的块。
 * 块掩码的工作方式与构件掩码基本相同，包括BlockMask类与ComponentMask类具有类似的语义。参见 @ref GlossComponentMask "关于组件掩码的词汇表条目 "
 * 以获得更多信息。
 *
 *
 * @note
 * 虽然组件和块为具有多个向量分量的有限元提供了两种交替但同样有效的观点，但事实上，在整个库中，可以传递ComponentMask参数而不是BlockMask参数的地方多得多。幸运的是，一个可以转换为另一个，使用的语法
 ...
 * <b>Creation of
 * block masks:</b>
 * 块掩码通常是通过要求有限元从某些选定的矢量分量中生成块掩码来创建的，使用这样的代码，我们创建一个掩码，只表示斯托克斯元的速度分量（见
 * @ref vector_valued  ）。
 *
 * @code
 ...
 * form</b></dt>。 <dd>
 * 对于二维空间中的二维三角，边界形式是一个定义在面上的矢量。它是单元格表面上坐标向量的图像的向量乘积。它是一个对表面的法线矢量，指向外侧，具有表面元素的长度。
 ...
 * 选项。一个典型的将部分边界指示器设置为其他东西的代码看起来像这样，这里将位于
 * $x=-1$ 的所有面的边界指示器设置为42。
 *
 * @code
 ...
 * @endcode
 * 这调用了 TriaAccessor::set_boundary_id. 函数 在3D中，调用
 * TriaAccessor::set_all_boundary_ids
 * 来代替每个选定的面可能也是合适的。要查询某个特定面或边的边界指标，请使用
 ...
 *
 * @note
 * 边界指标在网格细化时从母面和边继承到子面。关于边界指示器的一些更多信息也在Triangulation类的文档中介绍。
 *
 *
 * @note  对于 parallel::distributed::Triangulation,
 * 类型的平行三角形，仅在开始时设置一次边界指标是不够的。请看
 * parallel::distributed::Triangulation
 * 的类文档中关于这个主题的长篇讨论。  </dd>
 *
 *  <dt class="glossary">  @anchor  GlossCoarseMesh <b>Coarse mesh</b></dt>
 * <dd>  deal.II中的 "粗略网格
 * "是一个仅由未被细化的单元组成的三角形对象，也就是说，一个没有单元是另一个单元的孩子的网格。这通常是deal.II中最初构建三角形的方式，例如，使用命名空间GridGenerator中的（大部分）函数，GridIn类中的函数，或者直接使用函数
 ...
 * ）将在第一层，等等。三角形的粗网格（在上一段的意义上）正好由三角形的零级单元组成。(它们是否处于活动状态(即没有子代)或已被精炼，对这个定义并不重要)。
 ...
 * 模块中也提供了大量的文档。如果你只考虑偏微分方程（而不是它的离散化），那么<i>components</i>的概念是自然的。
 * 对于一个给定的有限元，可以使用
 ...
 * ）来解决方程组时，人们经常希望将一些操作限制在只有某些解变量。例如，在求解斯托克斯方程时，人们可能希望只插值速度分量的边界值而不插值压力。在deal.II中，这通常是通过传递函数a<i>component
 * mask</i>完成的。分量掩码总是被指定为ComponentMask对象，我们可以把它看作一个数组，其条目数与有限元的分量一样多（例如，在Stokes情况下，有
 * <code>dim+1</code>
 * 个分量），每个条目要么为真，要么为假。在这个例子中，我们只想为斯托克斯系统的速度分量插值边界值，那么这个分量掩码将是
 * <code>[true, true, false]</code> in 2d and <code>[true, true, true,
 * false]</code>
 * ，在3D中表示不应为压力变量设置边界值（解决方案的最后一个
 ...
 * @endcode
 * 将导致2d中的掩码 <code>[true, true, false]</code>
 ...
 *
 * @note
 * 并非所有构件掩码都有实际意义。例如，如果你有一个2D的FE_RaviartThomas对象，那么拥有一个
 * <code>[true, false]</code>
 * 形式的分量掩码是没有任何意义的，因为你试图选择一个有限元的单个矢量分量，其中每个形状函数都有
 * $x$ 和 $y$
 ...
 * 1.在你的矩阵和向量的汇编循环的最后。如果你直接写条目或者使用
 * AffineConstraints::distribute_local_to_global.  使用
 * VectorOperation::add. ，就需要这样做。 2.
 * 当你完成了对矩阵/向量中单个元素的设置后，再进行其他操作（向元素添加，其他操作如缩放、求解、读取等）。使用
 * VectorOperation::insert. 。
 * 3.像2.一样，但用于向单个元素加值。使用
 * VectorOperation::add.  。
 ...
 * deal.II</b></dt>。
 * <dd>
 * 在deal.II中，有几个地方我们要求模板中的类型与某个接口相匹配或以某种方式表现：这种约束在C++中被称为
 ...
 * "codimension one"。 这两个参数不一样的使用例子在  step-34  ,
 * step-38  ,  step-54  中显示。  </dd>   <dt class="glossary">  @anchor
 * GlossDoF <b>Degree of freedom</b></dt>。 <dd>  术语
 * "自由度"（通常缩写为
 ...
 * 类似的问题也发生在三个空间维度的二维网格上。我们注意到，如果二维流形不可定向，就不可能找到一致的方向标志；目前deal.II不支持这种流形。
 * </dd>   <dt class=" glossary">  @anchor  GlossDistorted <b>Distorted
 ...
 * 扭曲的细胞可以以两种不同的方式出现。原始的 @ref GlossCoarseMesh "粗略的网格 "
 ...
 * 的异常来提示创建带有变形单元的粗略网格。
 * 如果你不打算在这些单元上装配任何东西，创建带有变形单元（特别是塌陷/针状单元）的网格是合法的。例如，考虑这样一种情况：人们想模拟一种有液体填充的裂缝的弹性材料的行为，如一个储油罐。如果压力变得太大，裂缝就会被关闭
 *
 ...
 * paper</b></dt> </dt <dd>  "分布式计算论文 "是W. Bangerth, C.
 * Burstedde, T. Heister和M. Kronbichler的一篇论文，题为
 * "大规模并行通用有限元代码的算法和数据结构"，描述了deal.II中%并行分布式计算的实现，即计算中不仅有线性系统被分割到不同机器上，例如
 * step-17  ，还有三角计算和DoFHandler对象。实质上，它是
 ...
 * orientation</b></dt>  <dd>
 * 在三角形中，通过应用右手边规则（x,y），可以从面的方向推导出面的法向量。
 *
 * -> 法线）。)
 * 我们注意到，在2D的标准方向中，面0和面2的法线指向单元格内，面1和面3的法线指向外。在3D中，面0、2和4的法线指向单元格内，而面1、3和5的法线指向外面。这些信息同样可以从
 ...
 * @p true,
 * ，那么这个面就有标准方向，否则它的法向量就指向另一个方向。在应用程序中，你需要这些信息的地方其实并不多，但库中有几个地方用到了这个。注意，在2D中，结果总是
 * @p true.
 ...
 * Mola的一篇论文，题目是
 * "在有限元计算中使用精确几何信息"，描述deal.II如何描述域的几何结构。特别是，它讨论了Manifold类所基于的算法基础，以及它需要为网格细化、法向量的计算和其他许多几何学进入有限元计算的地方提供什么样的信息。
 ...
 * vectors</b></dt>  <dd>
 * 在并行计算中，向量一般有两种：没有和有鬼元素。没有鬼魂元素的向量在处理器之间唯一地划分了向量元素：每个向量条目正好有一个处理器拥有它，而且这个处理器是唯一存储这个条目的值的。换句话说，如果零号处理器存储了一个向量的0...49号元素，一号处理器存储了50...99号元素，那么一号处理器访问这个向量的42号元素就不走运了：它没有被存储在这里，也无法评估其值。这将导致一个断言。
 ...
 * @note
 * @ref distributed
 * 文档模块简要介绍了不同种类的向量通常用于何处。
 * </dd>
 *
 *  <dt class=" glossary">  @anchor  hp_paper <b>%hp-paper</b></dt>  <dd>
 * "hp-paper "是W. Bangerth和O. Kayser-Herold的一篇论文，题目是
 * "hp有限元软件的数据结构和要求"，描述了在实现deal.II的hp-framework时所使用的许多算法和数据结构。特别是，它总结了许多使用连续元素的%hp-有限元必须考虑的棘手问题。
 * 本文的全部参考资料如下。
 *
 * @code{.bib}
 * @Article{BK07,
 ...
 * indicator</b></dt>。 <dd>
 * 构成三角网格的每个对象（单元、面、边等），都与一个唯一的编号（类型为
 * types::manifold_id)
 * ，用于识别哪个流形对象在网格细化时负责生成新的点。
 ...
 * 。也可以调用 TriaAccessor::set_all_manifold_ids
 ...
 *
 *
 * @note  网格细化时，流形指标会从父代继承到子代。关于流形指示器的一些更多信息，也在Triangulation类的文档部分以及 @ref manifold  "流形文档模块 "
 * 中介绍。歧管指标在  step-53  和  step-54  中使用。  </dd>
 * @see   @ref manifold  "关于歧管的模块"
 *
 *  <dt class="glossary">  @anchor  GlossMaterialId <b>Material id</b></dt>
 * <dd>  三角形的每个单元都有一个名为 "材料ID
 * "的属性与之相关。它通常用于具有异质系数的问题，以确定一个单元在域的哪一部分，因此，系数应该在这个特定的单元上具有哪个值。在实践中，一个单元的材料ID通常用于识别哪些单元属于域的特定部分，例如，当你有不同的材料（钢铁、混凝土、木材），但都属于同一个域。在组装双线性表格时，我们通常会查询与某一单元相关的材料ID，并使用它来确定（例如，通过表格查询，或一连串的if-else语句）该单元的正确材料系数是多少。
 * 这个材料ID可以在构建三角形时设置（通过CellData数据结构），也可以在以后通过使用单元格迭代器设置。关于这个功能的典型使用，请看
 * step-28
 ...
 * -np 32 ./step-17
 * @endcode
 * （或者在你的集群上使用的批处理提交系统中使用的等价物），MPI系统会启动32份
 * step-17 的可执行文件。每个副本都可以访问
 * <code>MPI_COMM_WORLD</code>
 * 通信器，然后由所有32个处理器组成，每个处理器有自己的等级。这个MPI宇宙中的一个进程子集后来可以同意创建其他通信器，只允许在一个进程子集之间进行通信。
 * </dd>
 *
 ...
 * -np 32 ./step-17
 * @endcode
 * （或者在你的集群上使用的批处理提交系统中使用的等价物）启动32份
 * step-17
 * 的可执行文件。其中一些实际上可能在同一台机器上运行，但一般来说，它们将在不同的机器上运行，不能直接访问对方的内存空间。
 * 每个进程只能立即访问它自己的内存空间中的对象。一个进程不能从其他进程的内存中读取或写入。因此，进程可以通信的唯一方式是互相发送消息。也就是说（正如在
 * step-17
 ...
 * Methods"，它描述了在实现deal.II的多网格框架时所采用的许多算法和数据结构。它是实现
 ...
 *
 *
 * -  $K$ 是单元，在deal.II中这总是一个线段、四边形或六面体。
 *
 *
 *
 ...
 *
 *
 * -  $\Psi$  是一组 "节点函数"，即函数  $\Psi_i : P \rightarrow {\mathbb R}$  。 $P$ 的维度必须等于节点函数的数量。有了这个定义，我们可以定义局部函数空间的基础，即一组 "形状函数" $\varphi_j\in P$ ，要求 $\Psi_i(\varphi_j) = \delta_{ij}$ ，其中 $\delta$ 是克朗克三角。
 ...
 * "时，我们的意思是，如果我们让它解决的问题变大，程序不会变得过于缓慢（或占用过多的内存），如果我们保持问题大小不变但增加工作的处理器（或内核）数量，运行时间和内存消耗将按比例减少。
 ...
 *
 *
 * - 顺序的部分。许多程序都有不能或不能并行化的代码部分，也就是说，一个处理器必须做一定的、固定的工作量，不会因为周围有总共 $P$ 个处理器而减少。在 step-17 中，生成图形输出时就是这种情况：一个处理器为整个问题创建图形输出，也就是说，它需要做 ${\cal O}(N)$ 工作。这意味着这个函数的运行时间为 ${\cal O}(N)$ ，而不考虑 $P$ ，因此整个程序将无法达到 ${\cal O}(N/P)$ 的运行时间，而是有一个可以描述为 $c_1N/P + c_2N$ 的运行时间，其中第一项来自可扩展的操作，如组装线性系统，而后者来自在进程0上生成图形输出。如果 $c_2$ 足够小，那么程序可能看起来对小数量的处理器具有强扩展性，但最终强扩展性将停止。此外，程序也不能弱扩展，因为在以相同的速度增加处理器数量 $P$ 的同时，增加问题的大小 $N$ 并不能保持这一个函数的运行时间不变。
 *
 *
 *
 * - 重复的数据结构。在 step-17 中，每个处理器都存储整个网格。也就是说，每个处理器都要存储一个大小为  ${\cal O}(N)$  的数据结构，而不考虑  $P$  。最终，如果我们使问题的大小足够大，即使我们增加处理器的数量，这也会溢出每个处理器的内存空间。因此，很明显，这种复制的数据结构可以防止程序弱速扩展。  但它也阻止了程序的强扩展，因为为了创建一个大小为 ${\cal O}(N)$ 的对象，至少要写到 ${\cal O}(N)$ 的内存位置，花费了 ${\cal O}(N)$ 的CPU时间。因此，如果我们提供越来越多的处理器，整个算法的一个组成部分不会表现为 ${\cal O}(N/P)$ 。
 *
 *
 *
 * - 通信。仅举一例，如果你想计算一个向量的 $l_2$ 常数，而所有MPI进程都存储了一些条目，那么每个进程都需要计算其自身条目的平方之和（这需要 ${\cal O}(N/P)$ 时间，因此可以完美扩展），但随后每个进程都需要将其部分之和发送到一个进程，将它们全部相加并取平方根。在最好的情况下，发送一个包含单个数字的信息需要恒定的时间，而不考虑进程的总体数量。因此，同样地，每一个做通信的程序都不能强势扩展，因为程序中有些部分的CPU时间要求并不随着你为固定规模分配的处理器数量而减少  $P$  。在现实中，情况实际上更糟糕：参与一个通信步骤的进程越多，一般来说需要的时间就越长，例如，因为要把每个人的贡献加起来的那个进程必须把所有的东西加起来，需要  ${\cal O}(P)$  时间。换句话说，CPU的时间<i>increases</i>与进程的数量有关，因此不仅阻止了程序的强扩展，而且也阻止了弱扩展。实际上，MPI库并不通过将每个消息发送到一个进程，然后将所有的东西加起来来实现 $l_2$ 规范；相反，它们在树上做成对的减少，而不是像 ${\cal O}(P)$ 那样增长运行时间，而是像 ${\cal O}(\log_2 P)$ 那样，以发送更多消息为代价。尽管如此，根本的一点是，当你增加更多的处理器时，运行时间将随着 $P$ 的增长而增长，而不管操作的实际实现方式如何，因此它不能扩展）。)
 ...
 * @f}
 * 因此，你得到的
 * "加速"，即与在单个进程上运行程序相比，你的程序在 $P$
 * 处理器上运行得更快的因素（假设这是可能的）将是
 *
 * @f{align*}
 ...
 * ，这意味着有一个点，即在这个问题上投入更多的处理器不会再有任何明显的回报了。
 ...
 *
 *
 * - 使用 GridTools::collect_periodic_faces() （三角法）收集周期性的面。
 *
 *
 *
 ...
 *
 *
 * - 使用 GridTools::collect_periodic_faces()  (DoFHandler)收集周期性面。
 *
 *
 *
 ...
 * elements</b></dt>  <dd>  如果存在从形状函数数到矢量 @ref  GlossComponent "分量 "
 * 的唯一关系，一个有限元（由其形状函数描述）就是原始的。这意味着，如果一个元素是原始的，那么矢量值元素的每个形状函数正好有一个非零分量。这尤其包括所有标量元素以及通过FESystem类从其他基元（例如标量）元素组装的矢量值元素，如
 ...
 * "指的是将一个对象的状态写入流中，之后再检索的过程。一个典型的用例是将程序的状态保存到磁盘上，以便以后可能的复活，通常是在长期运行的计算的检查点/重启策略的背景下，或者在不是很可靠的计算机上（例如，在非常大的集群上，个别节点偶尔会出现故障，然后导致整个MPI作业的中断）。在这两种情况下，人们希望偶尔保存程序的状态，以便在失败时，可以在那个点重新启动，而不是从头开始运行。
 * deal.II通过实现<a
 ...
 * 类可以使用类似的方法自动划分网格)。
 * 除了常规的子域id，还有第二套密切相关的标志，与每个单元相关联。"水平子域id"。这些标志不仅存在于活动单元，而且事实上存在于网格层次结构中的每个单元。它们的含义完全类似于常规的子域id，但它们是由
 ...
 * <dd>  支持点根据定义是那些  $p_i$
 * 的点，这样对于形状函数  $v_j$  持有  $v_j(p_i) = \delta_{ij}$
 * 。因此，有限元插值可以由支持点中的值唯一地定义。
 * 拉格朗日元素填补了由 FiniteElement::get_unit_support_points(), 访问的矢量，从而使函数 FiniteElement::has_support_points() 返回<tt>真</tt>。当然，这些支持点是在 @ref GlossReferenceCell 的 "参考单元 "
 * 上。
 ...
 *      }
 * @endcode
 * 通常情况下，如果一个算法走过所有的单元，并且需要另一个单元，例如一个邻居，是否已经被处理过，那么这个用户标志就会被使用。同样，它也可以用来标记边界上的面、四边形或线，对它们已经进行了一些操作。后者通常是有用的，因为一个循环，如
 * @code
 *    // in 3d
 ...
 *
 * @note  如果需要在单元格、行或面中存储更多的信息，而不仅仅是一个布尔标志，那么请参考 @ref GlossUserData  "用户数据"
 * 。  </dd>
 *
 * 用户数据的存储和检索方式如下。
 * @code
 ...
 *          }
 * @endcode
 * 同样，有函数 TriaAccessor::set_user_pointer 用于设置指针，
 * TriaAccessor::user_index 和 TriaAccessor::user_pointer
 * 用于检索索引和指针。要清除所有的用户索引或指针，请使用
 ...
 * 另一方面，在有些情况下，我们希望以特定的、可重复的顺序遍历单元，这只取决于网格本身，而不是其创建历史或任何其他看似任意的设计决定。Z顺序
 * "是实现这一目标的方法之一。
 ...
 *
 * ->
 * (同样，如果你只关心活动单元，那么就把0.0、1.0和1.3从这个列表中删除。)
 * 因为单元格的子代顺序是明确定义的（相对于每一层内的单元格顺序），这种
 * "分层
