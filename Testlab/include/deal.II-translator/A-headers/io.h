 ...
 * PathSearch类是输入处理中的一个辅助类。它用于在目录列表中查找文件，与unix系统在
 * <code>PATH</code>
 * 环境变量中列出的目录中查找可执行文件的方式基本相同。
 *
 *
 * @ingroup IO
 *
 ...
 * deal.II生成三种类型的输出：它可以将三角图/网格写成几种网格阅读器（包括deal.II本身的阅读器）可以理解的格式，它可以创建用于数据可视化的输出。最后，它能以图形格式输出矩阵。
 *
 *  <h3>Visualization of data</h3>
 * deal.II通过DataOutBase类支持大量流行的可视化格式，例如OpenDX、gmv或gnuplot程序所使用的格式。支持的格式的完整列表列在DataOutBase类的文档中。
