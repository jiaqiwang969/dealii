 ...
 * deal.II带有许多不同的矩阵类，专门针对矩阵的各种用途。例如，有完整的矩阵、使用不同存储方案的稀疏矩阵、由单个块组成的矩阵，以及作为其他线性代数类接口实现的矩阵。在可能的情况下，所有这些实现都共享一个共同的接口，该接口至少包含编写迭代线性求解器所需的操作（见 @ref
 ...
 * 这些是deal.II提供的实际矩阵类。可以在其中存储数值并检索它们。此外，它们还提供了线性求解器所需的全部接口（见 @ref
 * Solvers  ）。
 ...
 * 这些矩阵是建立在基本矩阵之上的。它们使用 @ref ConceptMatrixType 定义的接口 "MatrixType概念 "
 * 进行特殊操作。
 *
 *
 * @ingroup Matrices
