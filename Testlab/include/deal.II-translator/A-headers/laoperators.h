 ...
 * $(A+k\,B)\,C$  ，其中  $A$  、  $B$  和  $C$
 * 表示（可能不同的）稀疏矩阵对象。为了构建一个LinearOperator
 * <code>op</code>
 * ，在应用于向量时执行上述计算，我们可以这样写。
 *
 * @code
 ...
 * step-20  教程中有一个LinearOperator类的详细使用例子。
 *
 *
 ...
 * result += d;
 * @endcode
 * 这就避免了任何中间存储。作为第二个例子（涉及一个LinearOperator对象），考虑计算一个残差
 * $b-Ax$  。
 *
 *
 * @code
 ...
 * - op_a x;
 * @endcode
 * 存储残差的计算表达式，并引用向量  <code>b</code> and matrix
 * <code>A</code>
 * 。在这一点上，它不执行任何计算。特别是，如果
 * <code>b</code> 或 <code>A</code> 被改变，<b>after</b>创建的
 * <code>residual_expr</code>
 * 的每一个后续表达式的评估都是用新值进行的
 *
 * @code
 * auto residual_expr =  b
 *
 ...
 * @endcode
 * 因此，作为一种保障，如果你想立即计算一个表达式的结果，总是明确地在左边使用一个矢量类型（而不是
 * <code>auto</code> ）。
 *
 * @code
