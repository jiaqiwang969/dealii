 ...
 *
 *
 * - 首先，将积分从实际单元 $K$ 转换到单位/参考单元 $\hat K$ 。例如，对于拉普拉斯方程，我们将@f[
 ...
 * $Q_p$
 * 有限元的形状函数值；但是这一特性对于Raviart-Thomas元素的形状函数不成立，因为它必须随本地单元旋转）。这允许进一步优化底层装配的计算。
 *
 ...
 * （尽管这些名字没有出现在任何公共接口中）。前者包含所有那些在FEValues对象开始与映射或有限元交互时可以预先计算一次的信息，而后者则包含那些对应于需要在每个单元上计算的标志。例如，如果
 ...
 * 如上所述，数据在两个不同的时间被计算：一次是在开始时在参考单元上，另一次是每当我们移动到一个实际单元时。接下来将讨论这些步骤中每一个步骤所涉及的函数。
 *
 *  <h3>Initialization</h3>
 * 在我们访问第一个实际单元之前，计算参考单元上的数据是一个两步的过程。首先，FEValues、FEFaceValues和FESubfaceValues的构造函数分别需要让Mapping和FiniteElement对象建立内部数据结构。这些结构在以下意义上是内部的：FEValues对象要求有限元和映射对象各创建一个 FiniteElement::InternalDataBase 和 Mapping::InternalDataBase 类型的对象；如果实际的有限元和映射类希望存储一些超出这些基类已经提供的数据，实际上可以创建衍生类型的对象。这其中涉及的函数有  <ul>   <li>Mapping::get_data()   <li>Mapping::get_face_data()   <li>Mapping::get_subface_data()   <li>FiniteElement::get_data()   <li>FiniteElement::get_face_data()   <li>FiniteElement::get_subface_data()   </ul>  。
 * 然后FEValues对象接管了这些对象的所有权，并将在FEValues对象的生命周期结束时销毁它们。之后，FEValues对象要求FiniteElement和Mapping对象向这些InternalDataBase对象中填充与参考单元上可以和需要计算的内容有关的数据。这是在这些函数中完成的。  <ul>   <li>FEValues::initialize()   <li>FEFaceValues::initialize()   <li>FESubfaceValues::initialize()   </ul> 。
 *   <h3>Reinitialization for a mesh cell</h3>
 * 一旦初始化结束，我们调用 FEValues::reinit,   FEFaceValues::reinit 或 FESubfaceValues::reinit 来移动到一个具体的单元或面，我们需要计算 "update_each "种类的数据。这是在以下函数中完成的。  <ul>   <li>FEValues::reinit()  调用 Mapping::fill_fe_values(),  然后 FiniteElement::fill_fe_values()   <li>FEFaceValues::reinit()  调用 Mapping::fill_fe_face_values(),  然后 FiniteElement::fill_fe_face_values()   <li>FESubfaceValues::reinit()  调用 Mapping::fill_fe_subface_values(),  然后 FiniteElement::fill_fe_subface_values()   </ul>  。
