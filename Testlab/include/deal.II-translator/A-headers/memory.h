 ...
 * 这一组有一些基本的类和命名空间用于内存处理。Subscriptor和SmartPointer类用于计数内存处理，即每当SmartPointer被设置为指向一个对象时，它就会增加该对象中的一个计数器；当指针被设置为指向其他地方时，它又会减少。这样一来，人们总是知道一个对象还有多少用户。虽然这本身很少有用，但如果一个对象被销毁，而某个地方的指针仍然指向它，它就会被用来产生一个异常，因为在以后的时间里通过该指针的任何访问都会导致访问无效的内存区域。
