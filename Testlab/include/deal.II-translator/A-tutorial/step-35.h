 ...
 * methods </h3> 。
 *
 * 相反，我们需要想出一种不同的方法来解决与时间有关的Navier-Stokesequations。解决这些问题的困难来自于速度和压力是通过约束@f[
 * \nabla \cdot u = 0,
 * @f]耦合在一起的，对于这个约束，压力是拉格朗日乘数。
 ...
 * 为了获得该方法的完全离散设置，我们一如既往地需要一个变分公式。鉴于边界条件的性质，这里有一个微妙的问题。当我们把方程乘以一个合适的检验函数时，出现的一个项是@f[
 *
 *
 *
 *
 ...
 * Threads::new_task
 * 函数并行工作的任务。我们在这里不会这样做，因为这些函数在内部已经被并行化了，特别是由于当前的函数在每个程序运行中只被调用一次，所以在每个时间步长中不会产生成本。然而，必要的修改将是非常直接的。
 *
 * @code
 * template <int dim>
 ...
 * @endcode
 *
 * 对于梯度算子，我们从初始化稀疏模式和压缩它开始。这里需要注意的是，梯度算子是从压力空间作用到速度空间的，所以我们必须处理两个不同的有限元空间。为了保持循环的同步，我们使用之前定义的别名，即
 ...
 *
 * 这是时间行进函数，从 <code>t_0</code> 开始，使用时间步长
 * <code>dt</code> 的投影法在时间上前进，直到 <code>T</code>  。
 ...
 *
 * 扩散步骤的实现。请注意，昂贵的操作是函数末尾的扩散解，我们必须为每个速度分量做一次。为了加快进度，我们允许以%并行方式进行，使用
 * Threads::new_task 函数，该函数确保 <code>dim</code>
 * 求解全部得到处理，并被安排到可用的处理器上：如果你的机器有一个以上的处理器核心，并且这个程序的其他部分目前没有使用资源，那么扩散求解将以%并行方式运行。另一方面，如果你的系统只有一个处理器核心，那么以%并行方式运行将是低效的（因为它导致了，例如，缓存拥堵），事情将被顺序执行。
 *
 * @code
 * template <int dim>
 ...
 *
 * 下面几个函数是关于集合平流项的，这是扩散步骤的系统矩阵的一部分，在每一个时间步骤中都会发生变化。如上所述，我们将使用WorkStream类和 @ref
 * threads
 * 文件模块中描述的其他设施，在所有单元上以%并行方式运行装配循环。
 *
 * @code
 * template <int dim>
 ...
 * ，那里使用了一个类似的程序（并有记录），为所有变量创建一个联合的DoFHandler对象。
 ...
 * name="Re100"></a><a name="Re100"></a><h3> Re = 100 </h3>
 ...
 * 。这样做，并在一定程度上减少时间步长，在 $t=20,40$
 * 时产生以下图像。 <table> <tr> <td> <img
 ...
 * 本程序可向以下方向扩展。 <ul>   <li>  自适应网格细化。正如我们所看到的，我们在一个单一的固定网格上计算了所有的东西。  使用自适应网格细化可以提高精度，同时不明显增加计算时间。
 * <li>
