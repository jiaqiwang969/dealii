...
@f]The fourth-order   <code>SymmetricTensor</code>   returned by identity_tensor() is   $\mathcal{S}$  .
* 

* <a name="Kinematics"></a><h3>Kinematics</h3>
* 

* Let the time domain be denoted   $\mathbb{T} = [0,T_{\textrm{end}}]$  , where   $t \in \mathbb{T}$   and   $T_{\textrm{end}}$   is the total problem duration.Consider a continuum body that occupies the reference configuration   $\Omega_0$   at time   $t=0$  .%Particles in the reference configuration are identified by the position vector   $\mathbf{X}$  .The configuration of the body at a later time   $t>0$   is termed the current configuration, denoted   $\Omega$  , with particles identified by the vector   $\mathbf{x}$  .The nonlinear map between the reference and current configurations, denoted   $\boldsymbol{\varphi}$  , acts as follows:@f[
	\mathbf{x} = \boldsymbol{\varphi}(\mathbf{X},t) \, .
@f]The material description of the displacement of a particle is defined by@f[
	\mathbf{U}(\mathbf{X},t) = \mathbf{x}(\mathbf{X},t)
* 
- \mathbf{X} \, .
@f]
* The deformation gradient   $\mathbf{F}$   is defined as the material gradient of the motion:@f[
	\mathbf{F}(\mathbf{X},t)
...
@f]The determinant of the of the deformation gradient  $J(\mathbf{X},t) \dealcoloneq \textrm{det}\ \mathbf{F}(\mathbf{X},t) > 0$  maps corresponding volume elements in the reference and current configurations, denoted  $\textrm{d}V$   and   $\textrm{d}v$  ,respectively, as@f[
...
* The Green-Lagrange strain tensor is defined by@f[
...
@f]If the assumption of infinitesimal deformations is made, then the second termon the right can be neglected, and   $\boldsymbol{\varepsilon}$   (the linearisedstrain tensor) is the only component of the strain tensor.This assumption is, looking at the setup of the problem, not valid in   step-18  ,making the use of the linearized   $\boldsymbol{\varepsilon}$   as the strainmeasure in that tutorial program questionable.
...
@f]Clearly,   $\textrm{det}\ \mathbf{F} = \textrm{det}\ (J^{1/3}\mathbf{I}) = J$  .
* The spatial velocity field is denoted   $\mathbf{v}(\mathbf{x},t)$  .The derivative of the spatial velocity field with respect to the spatial coordinates gives the spatial velocity gradient   $\mathbf{l}(\mathbf{x},t)$  , that is@f[
	\mathbf{l}(\mathbf{x},t)
		\dealcoloneq \dfrac{\partial \mathbf{v}(\mathbf{x},t)}{\partial \mathbf{x}}
		= \textrm{grad}\ \mathbf{v}(\mathbf{x},t) \, ,
@f]where   $\textrm{grad} \{\bullet \}
...
* Cauchy's stress theorem equates the Cauchy traction   $\mathbf{t}$   acting on an infinitesimal surface element in the current configuration   $\mathrm{d}a$   to the product of the Cauchy stress tensor   $\boldsymbol{\sigma}$   (a spatial quantity)  and the outward unit normal to the surface   $\mathbf{n}$   as@f[
	\mathbf{t}(\mathbf{x},t, \mathbf{n}) = \boldsymbol{\sigma}\mathbf{n} \, .
@f]The Cauchy stress is symmetric.Similarly,  the first Piola-Kirchhoff traction   $\mathbf{T}$   which acts on an infinitesimal surface element in the reference configuration   $\mathrm{d}A$   is the product of the first Piola-Kirchhoff stress tensor   $\mathbf{P}$   (a two-point tensor)  and the outward unit normal to the surface   $\mathbf{N}$   as@f[
	\mathbf{T}(\mathbf{X},t, \mathbf{N}) = \mathbf{P}\mathbf{N} \, .
@f]The Cauchy traction   $\mathbf{t}$   and the first Piola-Kirchhoff traction   $\mathbf{T}$   are related as@f[
	\mathbf{t}\mathrm{d}a = \mathbf{T}\mathrm{d}A \, .
@f]This can be demonstrated using <a href="http://en.wikipedia.org/wiki/Finite_strain_theory">Nanson's formula</a>.
* The first Piola-Kirchhoff stress tensor is related to the Cauchy stress as@f[
	\mathbf{P} = J \boldsymbol{\sigma}\mathbf{F}^{-T} \, .
@f]Further important stress measures are the (spatial) Kirchhoff stress    $\boldsymbol{\tau} = J \boldsymbol{\sigma}$  and the (referential) second Piola-Kirchhoff stress  $\mathbf{S} = {\mathbf{F}}^{-1} \boldsymbol{\tau} {\mathbf{F}}^{-T}$  .
* 
...
* The push-forward and-pull back operations for second-order covariant tensors   $(\bullet)^{\text{cov}}$   are respectively given by:@f[
...
@f]
* The push-forward and pull back operations for second-order contravariant tensors   $(\bullet)^{\text{con}}$   are respectively given by:@f[
...
@f]For example   $\boldsymbol{\tau} = \chi_{*}(\mathbf{S})$  .
* 

* <a name="Hyperelasticmaterials"></a><h3>Hyperelastic materials</h3>
* 

* A hyperelastic material response is governed by a Helmholtz free energy function   $\Psi = \Psi(\mathbf{F}) = \Psi(\mathbf{C}) = \Psi(\mathbf{b})$   which serves as a potential for the stress.For example, if the Helmholtz free energy depends on the right Cauchy-Green tensor   $\mathbf{C}$   then the isotropic hyperelastic response is@f[
	\mathbf{S}
		= 2 \dfrac{\partial \Psi(\mathbf{C})}{\partial \mathbf{C}} \, .
@f]If the Helmholtz free energy depends on the left Cauchy-Green tensor   $\mathbf{b}$   then the isotropic hyperelastic response is@f[
	\boldsymbol{\tau}
		= 2 \dfrac{\partial \Psi(\mathbf{b})}{\partial \mathbf{b}} \mathbf{b}
		=  2 \mathbf{b} \dfrac{\partial \Psi(\mathbf{b})}{\partial \mathbf{b}} \, .
@f]
* Following the multiplicative decomposition of the deformation gradient, the Helmholtz free energy can be decomposed as@f[
	\Psi(\mathbf{b}) = \Psi_{\text{vol}}(J) + \Psi_{\text{iso}}(\overline{\mathbf{b}}) \, .
@f]Similarly, the Kirchhoff stress can be decomposed into volumetric and isochoric parts as   $\boldsymbol{\tau} = \boldsymbol{\tau}_{\text{vol}} + \boldsymbol{\tau}_{\text{iso}}$   where:
* @f{align*}
...
@f}
* where  $p \dealcoloneq \dfrac{\partial \Psi_{\text{vol}}(J)}{\partial J}$   is the pressure response.  $\mathbb{P}$   is the projection tensor which provides the deviatoric operator in the Eulerian setting.The fictitious Kirchhoff stress tensor   $\overline{\boldsymbol{\tau}}$   is defined by@f[
...
* 
*   @note   The pressure response as defined above differs from the widely-used definition of thepressure in solid mechanics as  $p =
* 
...
* The Helmholtz free energy corresponding to a compressible <a href="http://en.wikipedia.org/wiki/Neo-Hookean_solid">neo-Hookean material</a> is given by@f[
    \Psi \equiv
        \underbrace{\kappa [ \mathcal{G}(J) ] }_{\Psi_{\textrm{vol}}(J)}
        + \underbrace{\bigl[c_1 [ \overline{I}_1
* 
- 3] \bigr]}_{\Psi_{\text{iso}}(\overline{\mathbf{b}})} \, ,
@f]where   $\kappa \dealcoloneq \lambda + 2/3 \mu$   is the bulk modulus (  $\lambda$   and   $\mu$   are the Lam&eacute; parameters)and   $\overline{I}_1 \dealcoloneq \textrm{tr}\ \overline{\mathbf{b}}$  .The function   $\mathcal{G}(J)$   is required to be strictly convex and satisfy the condition   $\mathcal{G}(1) = 0$  ,among others, see Holzapfel (2001) for further details.In this work   $\mathcal{G} \dealcoloneq \frac{1}{4} [ J^2
* 
- 1
* 
- 2\textrm{ln}J ]$  .
* Incompressibility imposes the isochoric constraint that   $J=1$   for all motions   $\boldsymbol{\varphi}$  .The Helmholtz free energy corresponding to an incompressible neo-Hookean material is given by@f[
    \Psi \equiv
        \underbrace{\bigl[ c_1 [ I_1
* 
- 3] \bigr] }_{\Psi_{\textrm{iso}}(\mathbf{b})} \, ,
@f]where   $ I_1 \dealcoloneq \textrm{tr}\mathbf{b} $  .Thus, the incompressible response is obtained by removing the volumetric component from the compressible free energy and enforcing   $J=1$  .
* 
...
* The fourth-order elasticity tensor in the material description is defined by@f[
	\mathfrak{C}
		= 2\dfrac{\partial \mathbf{S}(\mathbf{C})}{\partial \mathbf{C}}
		= 4\dfrac{\partial^2 \Psi(\mathbf{C})}{\partial \mathbf{C} \partial \mathbf{C}} \, .
@f]The fourth-order elasticity tensor in the spatial description   $\mathfrak{c}$   is obtained from the push-forward of   $\mathfrak{C}$   as@f[
...
@f]The fourth-order elasticity tensors (for hyperelastic materials) possess both major and minor symmetries.
* The fourth-order spatial elasticity tensor can be written in the following decoupled form:@f[
...
@f}
* where the fictitious elasticity tensor   $\overline{\mathfrak{c}}$   in the spatial description is defined by@f[
	\overline{\mathfrak{c}}
...
* The three-field variational principle used here is given by@f[
...
- \widetilde{J}]
		...
		\bigr] \textrm{d}v
	+ 	\Pi_{\textrm{ext}} \, ,
@f]where the external potential is defined by@f[
	\Pi_{\textrm{ext}}
		=
* 
...
*   @note   Although the variables are all expressed in terms of spatial quantities, the domain of integration is the initial configuration.This approach is called a   <em>   total-Lagrangian formulation   </em>  .The approach given in   step-18  , where the domain of integration is the current configuration, could be called an   <em>   updated Lagrangian formulation   </em>  .The various merits of these two approaches are discussed widely in the literature.It should be noted however that they are equivalent.
* 
...
* The change in a quantity between the known state at   $t_{\textrm{n}-1}$  and the currently unknown state at   $t_{\textrm{n}}$   is denoted  $\varDelta \{ \bullet \} = { \{ \bullet \} }^{\textrm{n}}
* 
- { \{ \bullet \} }^{\textrm{n-1}}$  .The value of a quantity at the current iteration   $\textrm{i}$   is denoted  ${ \{ \bullet \} }^{\textrm{n}}_{\textrm{i}} = { \{ \bullet \} }_{\textrm{i}}$  .The incremental change between iterations   $\textrm{i}$   and   $\textrm{i}+1$   is denoted  $d \{ \bullet \} \dealcoloneq \{ \bullet \}_{\textrm{i}+1}
* 
- \{ \bullet \}_{\textrm{i}}$  .
* Assume that the state of the system is known for some iteration   $\textrm{i}$  .The linearised approximation to nonlinear governing equations to be solved using the  Newton-Raphson method is:Find   $d \mathbf{\Xi}$   such that@f[
...
@f]then set  $\mathbf{\Xi}_{\textrm{i}+1} = \mathbf{\Xi}_{\textrm{i}}
...
href="http://en.wikipedia.org/wiki/Poisson's_ratio">Poisson's ratio</a>), subject to a good choice of the interpolation fieldsfor   $\mathbf{u},~\widetilde{p}$   and   $\widetilde{J}$  .Typically a choice of   $Q_n \times DGPM_{n-1} \times DGPM_{n-1}$   is made.Here   $DGPM$   is the FE_DGPMonomial class.A popular choice is   $Q_1 \times DGPM_0 \times DGPM_0$   which is known as the mean dilatation method (see Hughes (2000) for an intuitive discussion).This code can accommodate a   $Q_n \times DGPM_{n-1} \times DGPM_{n-1}$   formulation.The discontinuous approximationallows   $\widetilde{p}$   and   $\widetilde{J}$   to be condensed outand a classical displacement based method is recovered.
* For fully-incompressible materials   $\nu = 0.5$   and the three-field formulation will still exhibitlocking behavior.This can be overcome by introducing an additional constraint into the free energy of the form  $\int_{\Omega_0} \Lambda [ \widetilde{J}
* 
- 1]~\textrm{d}V$  .Here   $\Lambda$   is a Lagrange multiplier to enforce the isochoric constraint.For further details see Miehe (1994).
* The linearised problem can be written as@f[
...
@f}
* 
* There are no derivatives of the pressure and dilatation (primary) variables present in the formulation.Thus the discontinuous finite element interpolation of the pressure and dilatation yields a blockdiagonal matrix for  $\mathbf{\mathsf{K}}_{\widetilde{p}\widetilde{J}}$  ,  $\mathbf{\mathsf{K}}_{\widetilde{J}\widetilde{p}}$   and  $\mathbf{\mathsf{K}}_{\widetilde{J}\widetilde{J}}$  .Therefore we can easily express the fields   $\widetilde{p}$   and   $\widetilde{J}$   on each cell simplyby inverting a local matrix and multiplying it by the local right handside. We can then insert the result into the remaining equations and recovera classical displacement-based method.In order to condense out the pressure and dilatation contributions at the element level we need the following results:
* @f{align*}
		d \widetilde{\mathbf{\mathsf{p}}}
...
@f]Note that due to the choice of   $\widetilde{p}$   and   $\widetilde{J}$   as discontinuous at the element level, all matrices that need to be inverted are defined at the element level.
...
* 
*  - The modified system matrix is called   ${\mathbf{\mathsf{K}}}_{\textrm{store}}$  .  That is  @f[
...
*  To this end, we calculate the deformation gradient   $\mathbf{F}$   from the displacement gradient   $\textrm{Grad}\ \mathbf{u}$  , i.e.   $\mathbf{F}(\mathbf{u}) = \mathbf{I} + \textrm{Grad}\ \mathbf{u}$   and then let the material model associated with this quadrature point update itself. When computing the deformation gradient, we have to take care with which data types we compare the sum   $\mathbf{I} +
 \textrm{Grad}\ \mathbf{u}$  : Since   $I$   has data type SymmetricTensor, just writing   <code>I + Grad_u_n</code>   would convert the second argument to a symmetric tensor, perform the sum, and then cast the result to a Tensor (i.e., the type of a possibly nonsymmetric tensor). However, since   <code>Grad_u_n</code>   is nonsymmetric in general, the conversion to SymmetricTensor will fail. We can avoid this back and forth by converting   $I$   to Tensor first, and then performing the addition as between nonsymmetric tensors:
* 
...
* 
*  The Solid class is the central class in that it represents the problem at hand. It follows the usual scheme in that all it really has is a constructor, destructor and a   <code>run()</code>   function that dispatches all the work to private functions of this class:
* 
...
* 
 ...
 @endcode
* 
*  In the private section of this class, we first forward declare a number of objects that are used in parallelizing work using the WorkStream object (see the   @ref threads   module for more information on this).     
...
* 
 ...
 @endcode
* 
*  On the other hand, the ScratchData object stores the larger objects such as the shape-function values array (  <code>Nx</code>  ) and a shape function gradient and symmetric gradient vector which we will use during the assembly.
* 
...
*   We choose not to reset any data in the   <code>reset()</code>   function as the matrix extraction and replacement tools will take care of this
* 
...
* 
 ...
 @endcode
* 
*   <a name="Soliddetermine_component_extractors"></a>  <h4>Solid::determine_component_extractors</h4> Next we compute some information from the FE system that describes which local element DOFs are attached to which block component.  This is used later to extract sub-blocks from the global matrix.   
...
* 
 @endcode
* 
*   <a name="Solidsetup_qph"></a>  <h4>Solid::setup_qph</h4> The method used to store quadrature information is already described in   step-18  . Here we implement a similar setup for a SMP machine.   
...
* 
 @endcode
* 
*   <a name="Solidupdate_qph_incremental"></a>  <h4>Solid::update_qph_incremental</h4> As the update of QP information occurs frequently and involves a number of expensive operations, we define a multithreaded approach to distributing the task across a number of CPU cores.   
...
*  Calculate how well the dilatation   $\widetilde{J}$   agrees with   $J
 ...
 \int_{\Omega_0} {[ J
* 
- \widetilde{J}]}^{2}\textrm{d}V \bigr]^{1/2}$  . We also return the ratio of the current volume of the domain to the reference volume. This is of interest for incompressible media where we want to check how well the isochoric constraint has been enforced.
* 
...
* 
 @endcode
* 
*  Furthermore, after the first Newton iteration within a timestep, the constraints remain the same and we do not need to modify or rebuild them so long as we do not clear the   @p constraints   object.
* 
...
* @code
 ...
 @endcode
* 
*  Since the affine constraints were finalized at the previous Newton iteration, they may not be modified directly. So we need to copy them to another temporary object and make modification there. Once we're done, we'll transfer them back to the main   @p constraints   object.
* 
...
* 
 @endcode
* 
*   <a name="Solidsolve_linear_system"></a>  <h4>Solid::solve_linear_system</h4> We now have all of the necessary components to use one of two possible methods to solve the linearised system. The first is to perform static condensation on an element level, which requires some alterations to the tangent matrix and RHS vector. Alternatively, the full block system can be solved by performing condensation on a global level. Below we implement both approaches.
* 
...
* 
 @endcode
* 
*   <a name="Solidoutput_results"></a>  <h4>Solid::output_results</h4> Here we present how the results are written to file to be viewed using ParaView or VisIt. The method is similar to that shown in previous tutorials so will not be discussed in detail.
* 
...
* 
 ...
 @endcode
* 
*   <a name="Mainfunction"></a>  <h3>Main function</h3> Lastly we provide the main driver function which appears no different to the other tutorials.
* 
...
* Combining spatial refinement and a higher-order interpolation scheme results in a high-quality solution.Three grid refinements coupled with a   $Q_2-DGPM_1-DGPM_1$   formulation producesa result that clearly captures the mechanics of the problem.The deformation of the traction surface is well resolved.We can now observe the actual extent of the applied traction, with the maximum force being appliedat the central point of the surface causing the largest compression.Even though very high strains are experienced in the domain,especially at the boundary of the region of applied traction,the solution remains accurate.The pressure field is captured in far greater detail than before.There is a clear distinction and transition between regions of compression and expansion,and the linear approximation of the pressure field allows a refined visualizationof the pressure at the sub-element scale.It should however be noted that the pressure field remains discontinuousand could be smoothed on a continuous grid for the post-processing purposes.
* 
...
* 
*  - Firstly, an additional constraint could be added to the free-energy  function in order to enforce a high degree of incompressibility in  materials. An additional Lagrange multiplier would be introduced,  but this could most easily be dealt with using the principle of  augmented Lagrange multipliers. This is demonstrated in   <em>  Simo and  Taylor (1991)   </em>  .
* 
...
* 
*  - The program has been developed for solving problems on single-node  multicore machines. With a little effort, the program could be  extended to a large-scale computing environment through the use of  Petsc or Trilinos, using a similar technique to that demonstrated in    step-40  . This would mostly involve changes to the setup, assembly,    <code>PointHistory</code>   and linear solver routines.
* 
