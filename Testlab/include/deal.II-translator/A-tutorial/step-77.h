...
-F_k
@f}
* 其中 $F_k$ 是使用当前节点值矢量 $U_k$ 计算的残差矢量， $J_k$ 是其导数（称为 "雅各布"），而 $\delta U_k$ 是对应于上述函数 $\delta u_k$ 的更新矢量。 $J_k,F_k$ 的构造已在 step-15 中作了详尽的讨论，在每次牛顿迭代中解决线性系统的方法也是如此。因此，让我们关注一下非线性求解过程的另一个方面。计算 $F_k$ 是昂贵的，而组装矩阵 $J_k$ 更是如此。我们是否真的需要在每次迭代中都这样做？事实证明，在许多应用中，这实际上是没有必要的。即使我们用近似值 $\tilde J_k$ 代替 $J_k$ ，这些方法通常也会收敛，并且解决了
* @f{align*}{
