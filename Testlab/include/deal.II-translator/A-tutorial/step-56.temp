...
* The purpose of this tutorial is to create an efficient linear solver for theStokes equation and compare it to alternative approaches.  Here, we will useFGMRES with geometric multigrid as a preconditioner velocity block, and wewill show in the results section that this is a fundamentally better approachthan the linear solvers used in   step-22   (including the scheme described in"Possible Extensions").  Fundamentally, this is because only with multigrid itis possible to get   $O(n)$   solve time, where   $n$   is the number of unknowns ofthe linear system. Using the Timer class, we collect some statistics tocompare setup times, solve times, and number of iterations. We also computeerrors to make sure that what we have implemented is correct.
...
@f}
* 
* Note that we are using the deformation tensor instead of   $\Delta u$   (adetailed description of the difference between the two can be found in  step-22  , but in summary, the deformation tensor is more physical aswell as more expensive).
...
@f}
* 
* Our goal is to compare several solution approaches.  While   step-22  solves the linear system using a "Schur complement approach" in twoseparate steps, we instead attack theblock system at once using FMGRES with an efficientpreconditioner, in the spirit of the approach outlined in the "Results"section of   step-22  . The idea is as follows: if we find a blockpreconditioner   $P$   such that the matrix
* @f{eqnarray*}
\left(\begin{array}{cc} A & B^T \\ B & 0 \end{array}\right) P^{-1}
@f}
* 
* is simple, then an iterative solver with that preconditioner willconverge in a few iterations. Notice that we are doing rightpreconditioning here.  Using the Schur complement   $S=BA^{-1}B^T$  ,we find that
* @f{eqnarray*}
...
* The test problem is a "Manufactured Solution" (see   step-7   fordetails), and we choose   $u=(u_1,u_2,u_3)=(2\sin (\pi x),
* 
- \pi y \cos
(\pi x),- \pi z \cos (\pi x))$   and   $p = \sin (\pi x)\cos (\pi y)\sin
(\pi z)$  .We apply Dirichlet boundary conditions for the velocity on the wholeboundary of the domain   $\Omega=[0,1]\times[0,1]\times[0,1]$  .To enforce the boundary conditions we can just use our reference solution.
* If you look up in the deal.II manual what is needed to create a classderived from   <code>Function@<dim@></code>  , you will find that thisclass has numerous   @p virtual   functions, including  Function::value(),     Function::vector_value(),     Function::value_list(),  etc., all of which can be overloaded.  Different parts of deal.IIwill require different ones of these particularfunctions. This can be confusing at first, but luckily the only thingyou actually have to implement is   @p value().    The other virtualfunctions in the Function class have defaultimplementations inside that will call your implementation of   @p value  by default.
* Notice that our reference solution fulfills   $\nabla \cdot u = 0$  . Inaddition, the pressure is chosen to have a mean value of zero.  Forthe "Method of Manufactured Solutions" of   step-7  , we need to find   $\bf
f$   such that:
* @f{align*}
{\bf f} =
* 
...
* Because we do not enforce the meanpressure to be zero for our numerical solution in the linear system,we need to post process the solution after solving. To do this we usethe   VectorTools::compute_mean_value()   function to compute the mean valueof the pressure to subtract it from the pressure.
* 

* <a name="DoFHandlers"></a><h3> DoF Handlers </h3>
* 

* The way we implement geometric multigrid here only executes it on thevelocity variables (i.e., the   $A$   matrix described above) but not thepressure. One could implement this in different ways, including one inwhich one considers all coarse grid operations as acting on   $2\times
2$   block systems where we only consider the top leftblock. Alternatively, we can implement things by really onlyconsidering a linear system on the velocity part of the overall finiteelement discretization. The latter is the way we want to use here.
* To implement this, one would need to be able to ask questions such as"May I have just part of a DoFHandler?". This is not possible at thetime when this program was written, so in order to answer this requestfor our needs, we simply create a separate, second DoFHandler for just thevelocities. We then build linear systems for the multigridpreconditioner based on only this second DoFHandler, and simplytransfer the first block of (overall) vectors into correspondingvectors for the entire second DoFHandler. To make this work, we haveto assure that the <i>order</i> in which the (velocity) degrees of freedom areordered in the two DoFHandler objects is the same. This is in fact thecase by first distributing degrees of freedom on both, and then usingthe same sequence of DoFRenumbering operations on both.
* 

* <a name="DifferencesfromtheStep22tutorial"></a><h3> Differences from the Step 22 tutorial </h3>
* 

* The main difference between   step-56   and   step-22   is that we use blocksolvers instead of the Schur Complement approach used in  step-22  . Details of this approach can be found under the "Block Schurcomplement preconditioner" subsection of the "Possible Extensions"section of   step-22  . For the preconditioner of the velocity block, weborrow a class from <a href="https://aspect.geodynamics.org">ASPECT</a>called   @p BlockSchurPreconditioner   that has the option to solve forthe inverse of   $A$   or just apply one preconditioner sweep for itinstead, which provides us with an expensive and cheap approach,respectively.
* 
...
* 
 @endcode
* 
*   <a name="FunctionsforSolutionandRighthandside"></a>  <h3>Functions for Solution and Righthand side</h3>   
...
* 
 @endcode
* 
*   <a name="ASPECTBlockSchurPreconditioner"></a>  <h3>ASPECT BlockSchurPreconditioner</h3>
* 
...
*   The bool flag   @p do_solve_A   in the constructor allows us to either apply the preconditioner for the velocity block once or use an inner iterative solver for a more accurate approximation instead.   
...
* 
 @endcode
* 
*   <a name="StokesProblemassemble_multigrid"></a>  <h4>StokesProblem::assemble_multigrid</h4>
* 
...
* 
 ...
 @endcode
* 
*   <a name="StokesProblemprocess_solution"></a>  <h4>StokesProblem::process_solution</h4>
* 
...
* We first run the code and confirm that the finite element solution convergeswith the correct rates as predicted by the error analysis of mixed finiteelement problems. Given sufficiently smooth exact solutions   $u$   and   $p$  ,the errors of the Taylor-Hood element   $Q_k \times Q_{k-1}$   should be
* @f[
\| u
* 
-u_h \|_0 + h ( \| u- u_h\|_1 + \|p
...
@f]
* see for example Ern/Guermond "Theory and Practice of Finite Elements", Section4.2.5 p195. This is indeed what we observe, using the   $Q_2 \times Q_1$  element as an example (this is what is done in the code, but is easilychanged in   <code>main()</code>  ):
...
* Let us compare the direct solver approach using UMFPACK to the twomethods in which we choose   $\widetilde {A^{-1}}=A^{-1}$   and  $\widetilde{S^{-1}}=S^{-1}$   by solving linear systems with   $A,S$   usingCG. The preconditioner for CG is then either ILU or GMG.The following table summarizes solver iterations, timings, and virtualmemory (VM) peak usage:
*   <table align="center" class="doxtable">
<tr>
  <th></th>
...
* 2. Because we are using inner solvers for   $A$   and   $S$  , ILU and GMG require thesame number of outer iterations.
* 3. The number of (inner) iterations for   $A$   increases for ILU with refinement, leadingto worse than linear scaling in solve time. In contrast, the number of inneriterations for   $A$   stays constant with GMG leading to nearly perfect scaling insolve time.
...
* The introduction also outlined another option to precondition theoverall system, namely one in which we do not choose   $\widetilde
{A^{-1}}=A^{-1}$   as in the table above, but in which  $\widetilde{A^{-1}}$   is only a single preconditioner application withGMG or ILU, respectively.
* This is in fact implemented in the code: Currently, the boolean  <code>use_expensive</code> in <code>solve()</code>   is set to   @p true.   Theoption mentioned above is obtained by setting it to   @p false.  
* What you will find is that the number of FGMRES iterations staysconstant under refinement if you use GMG this way. This means that theMultigrid is optimal and independent of   $h$  .
* 
