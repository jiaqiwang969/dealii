...
x,t)$   and a potential   $V=V(\mathbf x)$   is a model often used inquantum mechanics and nonlinear optics. If one measures in appropriatequantities (so that   $\hbar=1$  ), then it reads as follows:
* @f{align*}{
* 
...
@f}
* If there is no potential, i.e.   $V(\mathbf x)=0$  , then it can be usedto describe the propagation of light in optical fibers. If   $V(\mathbf
...
*  - The equation is a nice model problem to explain how <b>operator
  splitting methods</b> work. This is because it has terms with  fundamentally different character: on the one hand,   $- \frac 12
...
* At first glance, the equations appear to be parabolic and similar tothe heat equation (see   step-26  ) as there is only a single timederivative and two spatial derivatives. But this is misleading.Indeed, that this is not the correct interpretation ismore easily seen if we assume for a moment that the potential   $V=0$  and   $\kappa=0$  . Then we have the equation
* @f{align*}{
* 
...
- \frac 12 \Delta \psi
  &= 0.
@f}
* If we separate the solution into real and imaginary parts,   $\psi=v+iw$  ,with   $v=\textrm{Re}\;\psi,\; w=\textrm{Im}\;\psi$  ,then we can split the one equation into its real and imaginary partsin the same way as we did in   step-29  :
* @f{align*}{
...
- \frac 12 \Delta w
  &= 0.
@f}
* Not surprisingly, the factor   $i$   in front of the time derivativecouples the real and imaginary parts of the equation. If we want tounderstand this equation further, take the time derivative of one ofthe equations, say
* @f{align*}{
...
* @f{align*}{
  ...
  &= 0.
@f}
* This equation is hyperbolic and similar in character to the waveequation. (This will also be obvious if you look at the videoin the "Results" section of this program.) Furthermore, we couldhave arrived at the same equation for   $v$   as well.Consequently, a better assumption for the NLSE is to think ofit as a hyperbolic, wave-propagation equation than as a diffusionequation such as the heat equation. (You may wonder whether it iscorrect that the operator   $\Delta^2$   appears with a positive signwhereas in the wave equation,   $\Delta$   has a negative sign. This isindeed correct: After multiplying by a test function and integratingby parts, we want to come out with a positive (semi-)definiteform. So, from   $-\Delta u$   we obtain   $+(\nabla v,\nabla u)$  . Likewise,after integrating by parts twice, we obtain from   $+\Delta^2 u$   theform   $+(\Delta v,\Delta u)$  . In both cases do we get the desired positivesign.)
* The real NLSE, of course, also has the terms   $V\psi$   and  $\kappa|\psi|^2\psi$  . However, these are of lower order in the spatialderivatives, and while they are obviously important, they do notchange the character of the equation.
* In any case, the purpose of this discussion is to figure outwhat time stepping scheme might be appropriate for the equation. Theconclusions is that, as a hyperbolic-kind of equation, we need tochoose a time step that satisfies a CFL-type condition. If we were touse an explicit method (which we will not), we would have to investigatethe eigenvalues of the matrix that corresponds to the spatialoperator. If you followed the discussions of the video lectures(  @dealiiVideoLectureSeeAlso{26,27,28})  then you will remember that the pattern is that one needs to make surethat   $k^s \propto h^t$   where   $k$   is the time step,   $h$   the mesh width,and   $s,t$   are the orders of temporal and spatial derivatives.Whether you take the original equation (  $s=1,t=2$  ) or the reformulationfor only the real or imaginary part, the outcome is that we would need tochoose   $k \propto h^2$   if we were to use an explicit time steppingmethod. This is not feasible for the same reasons as in   step-26   forthe heat equation: It would yield impractically small time stepsfor even only modestly refined meshes. Rather, we have to use animplicit time stepping method and can then choose a more balanced  $k \propto h$  . Indeed, we will use the implicit Crank-Nicolsonmethod as we have already done in   step-23   before for the regularwave equation.
* 
...
-i\kappa |\psi|^2 \psi,
  ...
  \psi(0) &= \psi_0,
@f}
* one may be tempted to "formally solve" it by integrating both sidesover a time interval   $[t_{n},t_{n+1}]$   and obtain
* @f{align*}{
  \psi(t_{n+1})
  &=
  \psi(t_n)
...
-i\kappa |\psi(t)|^2 \,\psi(t)
  ...
  dt.
@f}
* The way this equation can now be read is as follows: For each time interval  $[t_{n},t_{n+1}]$  , the change   $\psi(t_{n+1})-\psi(t_{n})$   in thesolution consists of three contributions:
* 
...
-i\kappa |\psi^{(3)}|^2 \,\psi^{(3)},
  ...
  \psi^{(3)}(t_n) &= \psi(t_n).
@f}
* In other words, they are all trajectories   $\psi^{(k)}$   that start at  $\psi(t_n)$   and integrate up the effects of exactly one of the threeterms. The increments resulting from each of these terms over our timeinterval are then   $I^{(1)}=\psi^{(1)}(t_{n+1})-\psi(t_n)$  ,  $I^{(2)}=\psi^{(2)}(t_{n+1})-\psi(t_n)$  , and  $I^{(3)}=\psi^{(3)}(t_{n+1})-\psi(t_n)$  .
...
* @f{align*}{
 ...
 I^{(1)} + I^{(2)} + I^{(3)}.
@f}
* This intuition is indeed correct, though the approximation is notexact: the difference between the exact left hand side and the term  $I^{(1)}+I^{(2)}+I^{(3)}$   (i.e., the difference between the <i>exact</i> incrementfor the exact solution   $\psi(t)$   when moving from   $t_n$   to   $t_{n+1}$  ,and the increment composed of the three parts on the right hand side),is proportional to   $\Delta t=t_{n+1}-t_{n}$  . In other words, thisapproach introduces an error of size   ${\cal O}(\Delta t)$  . Nothing wehave done so far has discretized anything in time or space, so the<i>overall</i> error is going to be   ${\cal O}(\Delta t)$   plus whatevererror we commit when approximating the integrals (the temporaldiscretization error) plus whatever error we commit when approximatingthe spatial dependencies of   $\psi$   (the spatial error).
* Before we continue with discussions about operator splitting, let ustalk about why one would even want to go this way? The answer issimple: For some of the separate equations for the   $\psi^{(k)}$  , wemay have ways to solve them more efficiently than if we throweverything together and try to solve it at once. For example, andparticularly pertinent in the current case: The equation for  $\psi^{(3)}$  , i.e.,
* @f{align*}{
  \frac{d\psi^{(3)}}{dt}
  &=
* 
...
@f}
* This is easy to see if (i) you plug this solution into thedifferential equation, and (ii) realize that the magnitude  $|\psi^{(3)}|$   is constant, i.e., the term   $|\psi(t_n)|^2$   in theexponent is in fact equal to   $|\psi^{(3)}(t)|^2$  . In other words, thesolution of the ODE for   $\psi^{(3)}(t)$   only changes its <i>phase</i>,but the <i>magnitude</i> of the complex-valued function   $\psi^{(3)}(t)$  remains constant. This makes computing   $I^{(3)}$   particularly convenient:we don't actually need to solve any ODE, we can write the solutiondown by hand. Using the operator splitting approach, none of themethods to compute   $I^{(1)},I^{(2)}$   therefore have to deal with the nonlinearterm and all of the associated unpleasantries: we can get away withsolving only <i>linear</i> problems, as long as we allow ourselves theluxury of using an operator splitting approach.
* Secondly, one often uses operator splitting if the different physicaleffects described by the different terms have different timescales. Imagine, for example, a case where we really did have somesort of diffusion equation. Diffusion acts slowly, but if   $\kappa$   islarge, then the "phase rotation" by the term   $-i\kappa
|\psi^{(3)}(t)|^2 \,\psi^{(3)}(t)$   acts quickly. If we treatedeverything together, this would imply having to take rather small timesteps. But with operator splitting, we can take large time steps  $\Delta t=t_{n+1}-t_{n}$   for the diffusion, and (assuming we didn'thave an analytic solution) use an ODE solver with many small timesteps to integrate the "phase rotation" equation for   $\psi^{(3)}$   from  $t_n$   to   $t_{n+1}$  . In other words, operator splitting allows us todecouple slow and fast time scales and treat them differently, withmethods adjusted to each case.
* 

* <a name="OperatorsplittingtheLiesplittingapproach"></a><h3>Operator splitting: the "Lie splitting" approach</h3>
* 

* While the method above allows to compute the three contributions  $I^{(k)}$   in parallel, if we want, the method can be made slightlymore accurate and easy to implement if we don't let the trajectoriesfor the   $\psi^{(k)}$   start all at   $\psi(t_n)$  , but instead let thetrajectory for   $\psi^{(2)}$   start at the <i>end point</i> of thetrajectory for   $\psi^{(1)}$  , namely   $\psi^{(1)}(t_{n+1})$  ; similarly,we will start the trajectory for   $\psi^{(3)}$   start at the end pointof the trajectory for   $\psi^{(2)}$  , namely   $\psi^{(2)}(t_{n+1})$  . Thismethod is then called "Lie splitting" and has the same order of erroras the method above, i.e., the splitting error is   ${\cal O}(\Delta
t)$  .
...
-i\kappa |\psi^{(3)}(t)|^2 \,\psi^{(3)}(t)
  ...
  dt.
@f}
* (Compare this again with the "exact" computation of   $\psi(t_{n+1})$  :It only differs in how we approximate   $\psi(t)$   in each of the three integrals.)In other words, Lie splitting is a lot simpler to implement that theoriginal method outlined above because data handling is so muchsimpler.
* 

* <a name="OperatorsplittingtheStrangsplittingapproach"></a><h3>Operator splitting: the "Strang splitting" approach</h3>
* 

* As mentioned above, Lie splitting is only   ${\cal O}(\Delta t)$  accurate. This is acceptable if we were to use a first order timediscretization, for example using the explicit or implicit Eulermethods to solve the differential equations for   $\psi^{(k)}$  . This isbecause these time integration methods introduce an error proportionalto   $\Delta t$   themselves, and so the splitting error is proportionalto an error that we would introduce anyway, and does not diminish theoverall convergence order.
...
splitting</a> method comes in. It is easier to explain if we had onlytwo parts, and so let us combine the effects of the Laplace operatorand of the potential into one, and the phase rotation into a secondeffect. (Indeed, this is what we will do in the code since solving theequation with the Laplace equation with or without the potential coststhe same
* 
...
\psi^{(2)}(t_{n+1})$  . In other words, we first make one full time stepfor physical effect one, then one full time step for physical effecttwo. The solution at the end of the time step is simply the sum of theincrements due to each of these physical effects separately.
...
t^2)$   accurate. Written in formulas, this yields the followingsequence of steps:
* @f{align*}{
  \frac{d\psi^{(1)}}{dt}
  &=
* 
...
* To do this, we will apply the second order accurate Crank-Nicolsonscheme that we have already used in some of the other time dependentcodes (specifically:   step-23   and   step-26  ). It reads as follows:
* @f{align*}{
* 
...
* If we multiply the whole equation with   $k_{n+1}$   and sort terms withthe unknown   $\psi^{(n+1,2)}$   to the left and those with the known  $\psi^{(n,2)}$   to the right, then we obtain the following (spatial)partial differential equation that needs to be solved in each timestep:
* @f{align*}{
* 
...
* As mentioned above, the previous tutorial program dealing withcomplex-valued solutions (namely,   step-29  ) separated real and imaginaryparts of the solution. It thus reduced everything to realarithmetic. In contrast, we here want to keep thingscomplex-valued.
* The first part of this is that we need to define the discretizedsolution as   $\psi_h^n(\mathbf x)=\sum_j \Psi^n_j \varphi_j(\mathbf
x) \approx \psi(\mathbf x,t_n)$   where the   $\varphi_j$   are the usual shape functions (which arereal valued) but the expansion coefficients   $\Psi^n_j$   at time step  $n$   are now complex-valued. This is easily done in deal.II: We justhave to use   Vector<std::complex<double>>   instead of Vector<double> tostore these coefficients.
* Of more interest is how to build and solve the linearsystem. Obviously, this will only be necessary for the second step ofthe Strang splitting discussed above, with the time discretization ofthe previous subsection. We obtain the fully discrete version throughstraightforward substitution of   $\psi^n$   by   $\psi^n_h$   andmultiplication by a test function:
* @f{align*}{
* 
...
@f}
* Note that all matrices individually are in fact symmetric,real-valued, and at least positive semidefinite, though the same isobviously not true forthe system matrix   $C =
* 
-iM + \frac 14 k_{n+1} A + \frac 12 k_{n+1} W$  and the corresponding matrix  $R =
* 
-iM
* 
- \frac 14 k_{n+1} A
...
* with the matrix   $C =
* 
-iM + \frac 14 k_{n+1} A + \frac 12 k_{n+1}
W$   and a right hand side that is easily computed as the product ofa known matrix and the previous part-step's solution.As usual, this comes down to the question of what properties thematrix   $C$   has. If it is symmetric and positive definite, then we canfor example use the Conjugate Gradient method.
* Unfortunately, the matrix's only useful property is that it is complexsymmetric, i.e.,   $C_{ij}=C_{ji}$  , as is easy to see by recalling that  $M,A,W$   are all symmetric. It is not, however,<a href="https://en.wikipedia.org/wiki/Hermitian_matrix">Hermitian</a>,which would require that   $C_{ij}=\bar C_{ji}$   where the bar indicates complexconjugation.
* Complex symmetry can be exploited for iterative solvers as a quickliterature search indicates. We will here not try to become toosophisticated (and indeed leave this to the <a
...
@f]assuming for the moment that   $\Omega={\mathbb R}^d$  
* 
*  -  which isof course not the case, but we'll ignore the small difference inintegral.
* Thus, we choose   $\alpha_k=\left(R^d\sqrt{\pi^d}\right)^{-1}$   for all, and  $R=0.1$  . This   $R$   is small enough that the difference between theexact (infinite) integral and the integral over   $\Omega$   should not betoo concerning.We choose the four points   $\mathbf x_k$   as   $(\pm 0.3, 0), (0, \pm
0.3)$  
* 
...
*   <a name="Includefiles"></a>  <h3>Include files</h3> The program starts with the usual include files, all of which you should have seen before by now:
* 
...
* 
 @endcode
* 
*   <a name="ThecodeNonlinearSchroedingerEquationcodeclass"></a>  <h3>The <code>NonlinearSchroedingerEquation</code> class</h3>   
...
* 
 @endcode
* 
*   <a name="ImplementationofthecodeNonlinearSchroedingerEquationcodeclass"></a>  <h3>Implementation of the <code>NonlinearSchroedingerEquation</code> class</h3>
* 

* 
*  We start by specifying the implementation of the constructor of the class. There is nothing of surprise to see here except perhaps that we choose quadratic (  $Q_2$  ) Lagrange elements
* 
...
* 
 ...
 @endcode
* 
*   <a name="Settingupdatastructuresandassemblingmatrices"></a>  <h4>Setting up data structures and assembling matrices</h4>
* 
...
* 
 @endcode
* 
*  Next, we assemble the relevant matrices. The way we have written the Crank-Nicolson discretization of the spatial step of the Strang splitting (i.e., the second of the three partial steps in each time step), we were led to the linear system   $\left[
* 
-iM  +  \frac 14 k_{n+1} A + \frac 12 k_{n+1} W \right]
 \Psi^{(n,2)}
 =
 \left[
* 
...
-  \frac 14 k_{n+1} A
* 
- \frac 12 k_{n+1} W \right]
 \Psi^{(n,1)}$  . In other words, there are two matrices in play here
* 
...
* 
 ...
 @endcode
* 
*   <a name="ImplementingtheStrangsplittingsteps"></a>  <h4>Implementing the Strang splitting steps</h4>
* 
...
*   An important realization is that while   $\psi^{(n,0)}(\mathbf x)$   may be a finite element function (i.e., is piecewise polynomial), this may not necessarily be the case for the "rotated" function in which we have updated the phase using the exponential factor (recall that the amplitude of that function remains constant as part of that step). In other words, we couldcompute*   $\psi^{(n,1)}(\mathbf x)$   at every point   $\mathbf x\in\Omega$  , but we can't represent it on a mesh because it is not a piecewise polynomial function. The best we can do in a discrete setting is to compute a projection or interpolation. In other words, we can compute   $\psi_h^{(n,1)}(\mathbf x) = \Pi_h
 ...
 \; \psi_h^{(n,0)}(\mathbf x) \right)$   where   $\Pi_h$   is a projection or interpolation operator. The situation is particularly simple if we choose the interpolation: Then, all we need to compute is the value of the right hand sideat the node points* and use these as nodal values for the vector   $\Psi^{(n,1)}$   of degrees of freedom. This is easily done because evaluating the right hand side at node points for a Lagrange finite element as used here requires us to only look at a single (complex-valued) entry of the node vector. In other words, what we need to do is to compute   $\Psi^{(n,1)}_j = e^{-i\kappa|\Psi^{(n,0)}_j|^2 \tfrac
 12\Delta t} \; \Psi^{(n,0)}_j$   where   $j$   loops over all of the entries of our solution vector. This is what the function below does
* 
...
* 
*  But oftentimes we are not particularly interested in real and imaginary parts of the solution vector, but instead in derived quantities such as the magnitude   $|\psi|$   and phase angle   $\text{arg}(\psi)$   of the solution. In the context of quantum systems such as here, the magnitude itself is not so interesting, but instead it is the "amplitude",   $|\psi|^2$   that is a physical property: it corresponds to the probability density of finding a particle in a particular place of state. The way to put computed quantities into output files for visualization
* 
...
*   (There is also the rather awkward fact that the <a
 href="https://en.cppreference.com/w/cpp/numeric/complex/norm">std::norm()</a> function does not compute what one would naively imagine, namely   $|\psi|$  , but returns   $|\psi|^2$   instead. It's certainly quite confusing to have a standard function mis-named in such a way...)
* 
...
* 
 @endcode
* 
*  The second of these postprocessor classes computes the phase angle of the complex-valued solution at each point. In other words, if we represent   $\psi(\mathbf x,t)=r(\mathbf x,t) e^{i\varphi(\mathbf x,t)}$  , then this class computes   $\varphi(\mathbf x,t)$  . The function <a
 href="https://en.cppreference.com/w/cpp/numeric/complex/arg">std::arg</a> does this for us, and returns the angle as a real number between   $-\pi$   and   $+\pi$  .     
...
*  -  what we need  is a "cyclic color map" that uses the same colors for the two  extremes of the range of the phase. Such color maps exist,  see <a href="https://nicoguaro.github.io/posts/cyclic_colormaps/">this
  blog post of Nicolás Guarín-Zapata</a> or  <a href="https://stackoverflow.com/questions/23712207/cyclic-colormap-without-visual-distortions-for-use-in-phase-angle-plots">this
...
@f]Because   $V$   is large, this means that the phaserotates quite rapidly*.If you focus on the semi-transparent outer part of the domain, you cansee that. If one colors this region in the same way as the inner part ofthe domain, this rapidly flashing outer part may be psychedelic, but is alsodistracting of what's happening on the inside; it's also quite hard toactually see the radiating waves that are easy to see at the beginningof the video.
* 
