 ...
 * step-8
 * 已经做的完全相同的计算，但它以一种不同的方式来做：我们没有使用deal.II的线性代数类，而是在deal.II提供的类之上建立一切，这些类包裹着<a
 * href="http://www.mcs.anl.gov/petsc/"
 * target="_top">PETSc</a>库的线性代数实现。由于PETSc允许将矩阵和向量分布在MPI网络中的几台计算机上，因此产生的代码甚至能够以%并行方式解决该问题。如果你不知道PETSc是什么，那么现在就是快速浏览其主页的好时机。
 ...
 * @endcode
 *
 * 这里是我们在这个例子程序中特别需要的东西，这些东西在
 * step-8 中没有。首先，我们替换掉标准输出
 * <code>std::cout</code> by a new stream <code>pcout</code>
 ...
 * @endcode
 *
 * 此外，我们还需要一些算法来划分我们的网格，以便它们可以有效地分布在MPI网络上。分区算法在
 * <code>GridTools</code>
 ...
 *
 * 成员变量列表的其余部分与  step-8
 * 中的内容基本相同。然而，我们改变了矩阵和向量类型的声明，改为使用并行的PETSc对象。请注意，我们没有使用单独的稀疏模式，因为PETSc将其作为其矩阵数据结构的一部分进行内部管理。
 *
 * @code
 ...
 * step-8
 * 中已经有的相同成员变量外，我们在这里用连接所有进程的全局MPI通信器来初始化我们将使用的MPI通信器变量（在更复杂的应用中，我们可以在这里使用一个只连接所有进程的一个子集的通信器对象），并调用
 * Utilities::MPI
 ...
 * 类来代替三角形对象，我们可以避免这个手动划分的步骤（就像我们在
 * step-18
 * 中做的那样）。该类实质上做了所有常规三角形的工作，但它也会在每次创建或细化网格操作后自动划分网格。
 ...
 * "拥有
 * "哪些单元，但它知道所有单元的一切。同样，DoFHandler对象知道每个单元的一切，特别是每个单元上的自由度，不管它是否是当前进程拥有的。这不能扩展到大型问题，因为如果问题足够大，最终只是在每个进程中存储整个网格以及与之相关的所有内容将变得不可行。另一方面，如果我们将三角形分割成若干部分，使每个进程只存储它
 * "拥有
 ...
 * 中包含的分区信息。IndexSet包含全局大小（<i>total</i>自由度数）的信息，也包含要在本地存储哪些行的子集。
 ...
 *
 * 我们现在组装矩阵和问题的右手边。在我们进行详细讨论之前，有一些事情值得一提。首先，我们将平行组装系统，也就是说，每个进程将负责在属于这个特定进程的单元上组装。请注意，自由度的分割方式是，单元内部和属于同一子域的单元之间的所有自由度都属于
 * <code>owns</code>
 * 该单元的过程。然而，即使如此，我们有时也需要在一个单元上与属于不同过程的邻居集合，在这些情况下，当我们将局部贡献加到全局矩阵或右手向量中时，我们必须将这些条目转移到拥有这些元素的过程中。幸运的是，我们不需要用手去做这件事。PETSc为我们做了这一切，它在本地缓存了这些元素，当我们在这个函数的末尾调用矩阵和向量的
 * <code>compress()</code>
 * 函数时，根据需要将它们发送到其他进程。
 * 第二点是，一旦我们把矩阵和向量的贡献交给了PETSc，那么，a）很难，b）要把它们拿回来修改，效率非常低。这不仅是PETSc的错，也是这个程序的分布式性质的结果：如果一个条目存在于另一个处理器上，那么获取它的成本必然很高。这样做的后果是，我们不应该试图首先将矩阵和右手边组装起来，就像没有悬挂的节点约束和边界值一样，然后在第二步中消除这些约束（例如使用
 ...
 * (注意，我们仍然使用deal.II的全矩阵和向量类型的本地系统，因为这些类型很小，不需要在不同的进程中共享。)
 *
 * @code
 * template <int dim>
 ...
 * 的开头就这样做了），并允许识别一个单元生活在哪个子域。在这个应用中，我们让每个进程恰好处理一个子域，所以我们确定了这些术语
 ...
 * @endcode
 *  下一步是 "压缩
 * "向量和系统矩阵。这意味着每个进程将对矩阵和向量中那些自己不拥有的条目所做的补充发送给拥有这些条目的进程。在收到其他进程的这些加法后，每个进程再把它们加到它已经拥有的值上。这些加法是将生活在几个单元上的形状函数的积分贡献结合起来，就像在串行计算中一样，不同的是这些单元被分配给不同的进程。
 *
 * @code
 ...
 * @endcode
 *
 * 全局矩阵和右手边的向量现在已经形成。我们仍然要应用边界值，方法与我们在
 * step-3 、 step-4 和其他一些程序中的方法相同。 下面调用
 ...
 * 在函数的顶部，我们设置了一个收敛监视器，并为它指定了我们希望解决线性系统的精度。接下来，我们使用PETSc的CG求解器创建一个实际的求解器对象，该求解器也适用于并行（分布式）矢量和矩阵。最后是一个预处理程序；我们选择使用一个块状雅可比预处理程序，它通过计算矩阵的每个对角线块的不完全LU分解来工作。
 * 换句话说，每个MPI进程从其存储的行中计算出一个ILU，丢掉与本地未存储的行指数相对应的列；这就产生了一个方形矩阵块，我们可以从中计算出一个ILU。这意味着如果你只用一个进程来运行程序，那么你将使用一个ILU(0)作为预处理程序，而如果它在许多进程上运行，那么我们将在对角线上有许多块，预处理程序是这些块中每个块的ILU(0)。在每个处理器只有一个自由度的极端情况下，这个预处理程序只是一个雅可比预处理程序，因为对角线矩阵块只由一个条目组成。这样的预处理程序相对容易计算，因为它不需要处理器之间的任何形式的通信，但一般来说，对于大量的处理器来说，它的效率并不高。)
 * 按照这样的设置，我们就可以解决线性系统。
 *
 * @code
 * template <int dim>
 ...
 * "拥有
 * "的自由度的那些元素。然而，在有些情况下，为了计算一个进程中受限自由度的向量项的值，我们需要访问存储在其他进程中的向量项。
 ...
 * @endcode
 *
 * 当然，和以前的讨论一样，如果你想在大量的进程上解决大问题，这样的步骤显然不能很好地扩展，因为现在每个进程都存储了<i>all
 * elements</i>的解向量。(我们将在 step-40
 * 中展示如何更好地做到这一点。)
 ...
 *
 * 使用某种细化指标，可以对网格进行细化。问题基本上与分布悬挂节点约束相同：为了计算误差指标（即使我们只是对当前进程拥有的单元格的指标感兴趣），我们需要访问解向量的更多元素，而不仅仅是当前处理器存储的那些。为了实现这一点，我们基本上做了我们在
 * <code>solve()</code>
 * 中已经做过的事情，即在每个进程中获取<i>complete</i>的解向量副本，并使用它来计算。如上所述，这本身就很昂贵，而且特别没有必要，因为我们刚刚在
 * <code>solve()</code>
 * 中创建并销毁了这样一个向量，但效率不是这个程序的重点，所以让我们选择一个设计，其中每个函数都是尽可能独立的。
 * 一旦我们有了这样一个包含<i>all</i>解向量元素的 "本地化
 * "向量，我们就可以计算属于当前过程的单元的指标。事实上，我们当然可以计算<i>all</i>细化指标，因为我们的Triangulation和DoFHandler对象存储了所有单元的信息，而且我们有一个完整的解向量副本。但是为了展示如何平行操作，让我们演示一下，如果只计算<i>some</i>错误指标，然后与其他进程交换剩余的指标，会如何操作。最终，每个进程都需要一套完整的细化指标，因为每个进程都需要细化他们的网格，并且需要以与所有其他进程完全相同的方式细化它）。
 * 因此，要做到这一切，我们需要。
 *
 *
 ...
 * @endcode
 *
 * 现在，所有进程都计算了自己单元的错误指标，并将其存储在
 * <code>local_error_per_cell</code>
 * 向量的各个元素中。这个向量中不属于本进程的单元的元素为零。然而，由于所有进程都有整个三角形的副本，并需要保持这些副本的同步性，他们需要三角形所有单元的细化指标值。因此，我们需要分配我们的结果。我们通过创建一个分布式向量来做到这一点，每个进程都有自己的份额，并设置它所计算的元素。因此，当你把这个向量看作是一个存在于所有进程中的向量时，那么这个向量的每个元素都被设置过一次。然后，我们可以将这个并行向量分配给每个进程上的一个本地非并行向量，使<i>all</i>错误指标在每个进程上都可用。
 ...
 * <code>compress()</code>
 * 函数时，PETSc为我们做了这项操作。这种低效率可以通过更多的代码来避免，但我们避免这样做，因为它不是程序总运行时间的主要因素。
 ...
 * @endcode
 *
 * 所以现在我们有了这个分布式向量，它包含了所有单元的细化指标。为了使用它，我们需要获得一个本地副本，然后用它来标记要细化或粗化的单元，并实际进行细化和粗化。重要的是要认识到，<i>every</i>进程对它自己的三角图副本做了这件事，并且以完全相同的方式进行。
 *
 * @code
 ...
 * step-8
 * 中的相同，但有两个小区别。在讨论这些之前，让我们说明这个函数的一般工作原理：我们打算让所有的数据都在一个单一的进程中产生，然后写入一个文件。正如本程序的许多其他部分已经讨论过的那样，这不是一个可以扩展的东西。之前，我们认为我们会在三角计算、DoFHandlers和解决方案向量的副本方面遇到麻烦，每个进程都必须存储所有的数据，而且会出现一个点，即每个进程根本没有足够的内存来存储这么多数据。在这里，情况是不同的：不仅是内存，而且运行时间也是一个问题。如果一个进程负责处理<i>all</i>的数据，而其他所有的进程什么都不做，那么这一个函数最终会在程序的整个运行时间中占主导地位。
 ...
 * step-18
 * 的操作方式。另外，我们可以简单地把所有的东西放在一组独立的文件中，让可视化软件读取所有的文件（可能也使用多个处理器），并从所有的文件中创建一个单一的可视化；这就是
 * step-40 、 step-32
 * 以及后来开发的所有其他并行程序所采取的方法。
 ...
 * <code>solve()</code>
 * 函数中创建的向量，但是为了使事情更加自洽，我们在这里只是从分布式解向量中重新创建一个。
 ...
 * @endcode
 *
 * 这样做了，进程0继续设置输出文件，如 step-8
 * ，并将（本地化的）解决方案向量附加到输出对象上。
 *
 * @code
 ...
 *
 * <code>main()</code>
 * 的工作方式与其他例子程序中的大多数主函数相同，即它将工作委托给管理对象的
 * <code>run</code>
 * 函数，只将所有的东西包装成一些代码来捕捉异常。
 *
 * @code
 ...
 * step-8 中看到的情况非常接近。正如那里和 step-22
 * 中所示，我们也可以制作位移场的矢量图，而不是把它绘制成两个独立的标量场。不过，更有趣的是，在这一步看一下网格和分区的情况。
 ...
 *
 *
 * 值得注意的是，如果我们用不同数量的进程来运行同一个程序，我们可能会得到稍微不同的输出：不同的网格，不同的未知数和迭代收敛的次数。其原因是，虽然矩阵和右手边是相同的，不受所用进程数量的影响，但预处理程序不是：它对每个处理器的
 * <em> 矩阵块分别执行ILU(0) </em>
 ...
 * 在内部，在deal.II中，分级和非结构化三角计算中使用的数据结构的并行化是一个困难的问题，我们花了几年时间才实现了这一点。 step-40
