...
* 在这个项目中，我们为多孔介质中的两个相流问题提出了一个数值模拟。这个问题包括一个椭圆方程和一个非线性的、与时间相关的传输方程。因此，这也是第一个时间相关的教程程序（除了 @ref step_18 "  step-18 "有点奇怪的时间相关）。
...
@f} 。
* 其中 $\mathbf{u}_{j}$ 是相 $j=o,w$ 的速度， $K$ 是渗透性张量， $k_{rj}$ 是相 $j$ 的相对渗透性， $p$ 是压力， $\mu_{j}$ 是相 $j$ 的黏度。最后， $S$ 是饱和度（体积分数），即一个数值在0和1之间的函数，表示流体混合物的组成。一般来说，系数 $K, k_{rj}, \mu$ 可能是空间依赖性的变量，在下文中我们总是将其视为非常数函数。
...
@f]是总流动性。
* 到目前为止，这看起来是一个普通的静止的、类似泊松的方程，我们可以用前几个教程程序的技术立即解决（例如，看一下 step-6 ，就可以看到非常类似的东西）。然而，我们还没有提到饱和度的问题，当然，饱和度会随着流体的移动而改变。
...
* 在储层模拟界，通常是通过回到一阶混合配方来解决上述方程。为此，我们重新引入总速度 $\mathbf u$ ，并将方程写成以下形式：@f{eqnarray*}
  \mathbf{u}+\mathbf{K}\lambda(S) \nabla p&=&0 \\
  \nabla \cdot\mathbf{u} &=& q \\
  S_{t} + \mathbf{u} \cdot \nabla F(S) &=& 0.
@f} 。
* 这种提法还有一个好处，即我们不必把运输方程中出现的总速度 $\mathbf u$ 作为压力的函数来表示，而是可以把它作为主要变量。鉴于前两个方程的鞍点结构以及它们与我们在 step-20 中介绍的混合拉普拉斯公式的相似性，我们将再次使用混合离散化，这一点并不奇怪。
...
* 在每个时间步长中，我们将 @ref step_20 "  step-20 "的混合有限方法应用于速度和压力。为了得到良好的解决，我们选择Raviart-Thomas空间 $RT_{k}$ 作为 $\mathbf{u}$ ，选择类 $DGQ_{k}$ 的不连续元素作为 $p$ 。对于饱和度，我们也将选择 $DGQ_{k}$ 空间。
...
* 本程序中使用的线性求解器是对 step-20 中使用的线性求解器的直接扩展（但没有LinearOperator）。从本质上讲，我们只需将一切从两个求解组件扩展到三个。如果我们使用上面提到的离散空间，并将形状函数放到双线性形式中，就会发现在时间步长 $n+1$ 中需要解决以下线性系统：@f[
\left(
\begin{array}{ccc}
M^u(S^{n}) & B^{T}& 0\\
B &    0 & 0\\
...
* 像我们要解决的饱和方程这样的双曲运输方程的一般经验法则是，如果我们使用显式时间步长方案，那么我们应该使用一个时间步长，使粒子在一个时间步长内所能走的距离不大于单个细胞的直径。换句话说，在这里，我们应该选择@f[
  \triangle t_{n+1} \le \frac h{|\mathbf{u}^{n+1}(\mathbf{x})|}.
@f]。幸运的是，我们处于一个可以做到这一点的位置：我们只需要在我们想要组装饱和方程的右侧时，也就是在我们已经解决了 $\mathbf{u}^{n+1}$ 之后，才需要这个时间步长。因此，在求解速度之后，我们所要做的就是在域中的所有正交点上循环，确定速度的最大幅度。然后我们可以将饱和方程的时间步长设置为@f[
...
@f]由于压力和速度解决的是混合形式的泊松方程，施加的压力导致了速度的流场结果。另一方面，这个流场决定了边界的某一部分是流入还是流出，这一点很重要，因为我们必须对边界的流入部分的饱和度施加边界条件，@f[
...
* 最后，回到测试案例的描述，我们将展示在 @ref step_20 " step-20 "的结果部分末尾介绍的两个渗透率函数的计算结果： <ul>  <li> 一个函数，模拟一条蜿蜒穿过域的裂缝。与 step-20 相类似，但考虑到我们这里的几何形状略有不同，我们用以下函数来描述它。  @f[
    k(\mathbf x)
    =
...
*  <li>  一个模拟某种随机介质的函数。这里，我们选择@f{eqnarray*}
  ...
  @f}。
* 其中中心 $\mathbf{x}_i$ 是域内 $N$ 随机选择的位置。这个函数模拟了一个领域，其中有 $N$ 个渗透率较高的中心（例如，岩石已经开裂），嵌入到更原始的、未受干扰的背景岩石矩阵中。请注意，在这里，我们在上方和下方都切断了渗透率函数，以确保有一个有界的条件数。 </ul> 
*

* <a name="CommProg"></a> <h1> The commented program</h1> 。
* 这个程序是对  step-20  的改编，包括了  step-12  的一些DG方法的技术。因此，该程序的很大一部分与  step-20  非常相似，我们将不再对这些部分进行评论。只有新的东西会被更详细地讨论。
*
...
* 
 @endcode
*
* 此外，我们使用类 <code>DiscreteTime</code> 来执行与时间递增有关的操作。
*
...
*
* 这是该程序的主类。它接近于 step-20 的那个，但有一些额外的功能。   
...
*
* 我们使用的线性求解器也完全类似于  step-20  中使用的线性求解器。因此，下面的类是逐字逐句地从那里复制的。请注意，这里的类不仅是从 step-20 中复制的，而且在deal.II中也有重复的类。在这个例子的未来版本中，它们应该被一个有效的方法所取代，虽然。有一个变化：如果线性系统的大小很小，也就是说，当网格很粗时，那么在 <code>src.size()</code> 函数中的求解器收敛之前，设置最大的 <code>vmult()</code> CG迭代次数有时是不够的。(当然，这是数值取舍的结果，因为我们知道，在纸面上，CG方法最多只能在 <code>src.size()</code> 步内收敛)。因此，我们设定最大迭代次数等于线性系统大小的最大值和200。
*
...
*
* 这里是主类的实现。它的大部分内容实际上是从 step-20 中复制过来的，所以我们不会对它进行过多的评论。你应该试着先熟悉一下那个程序，然后这里发生的大部分事情就应该很清楚了。
*
...
*
* 这是组装线性系统的函数，或者至少是除了(1,3)块之外的所有东西，它取决于在这个时间步长中计算的仍然未知的速度（我们在 <code>assemble_rhs_S</code> 中处理这个问题）。它的大部分内容与 step-20 中一样，但这次我们必须处理一些非线性问题。  然而，函数的顶部与平常一样（注意，我们在开始时将矩阵和右手边设置为零&mdash；对于静止问题我们不必这样做，因为在那里我们只使用每个矩阵对象一次，而且在开始时它是空的）。   
...
* 这里是第一个重要的区别。我们必须在正交点上获得前一个时间步长的饱和函数值。为此，我们可以使用 FEValues::get_function_values （之前已经在 step-9 、 step-14 和 step-15 中使用），这个函数接收一个解向量并返回当前单元的正交点的函数值列表。事实上，它返回每个正交点的完整矢量值解，即不仅是饱和度，还有速度和压力。
*
...
* 
 ...
 @endcode
*
* 接下来，我们还得处理压力边界值的问题。这一点，还是和 step-20 中一样。
*
...
* 
 @endcode
*
* 最后，我们必须处理好饱和度方程。我们在这里要做的第一件事是使用介绍中的公式来确定时间步长。知道了我们领域的形状和我们通过有规律的划分单元创建的网格，我们可以很容易地计算每个单元的直径（事实上我们使用的是单元坐标方向上的线性扩展，而不是直径）。请注意，我们将在 step-24 中学习一种更通用的方法，在那里我们使用 GridTools::minimal_cell_diameter 函数。     
...
*
* 在这个函数中，我们简单地遍历所有的饱和自由度，并确保如果它们应该离开物理上的合理范围，那么它们将被重置到区间  $[0,1]$  。要做到这一点，我们只需循环查看解向量的所有饱和分量；这些分量被存储在块2中（块0是速度，块1是压力）。   
...
*
* 这是我们主类的最后一个函数。它的简洁不言自明。只有两点是值得注意的。首先，该函数在开始时将初始值投射到有限元空间； VectorTools::project 函数这样做需要一个表明悬挂节点约束的参数。我们在这个程序中没有（我们在一个均匀细化的网格上计算），但是这个函数当然需要这个参数。所以我们必须创建一个约束对象。在原始状态下，约束对象是没有排序的，在使用前必须进行排序（使用 AffineConstraints::close 函数）。这就是我们在这里所做的，这也是为什么我们不能简单地用一个匿名的临时对象 <code>AffineConstraints<double>()</code> 作为第二个参数来调用 VectorTools::project 函数。   
...
*有了这些，这里有几段电影，显示了饱和度是如何随时间推移而发展的。首先，这是在 <code>SingleCurvingCrack::KInverse</code> 类中实现的单裂缝模型。
...
* 第二部电影显示了 <code>RandomMedium::KInverse</code> 类的随机介质模型的饱和度，其中我们有随机分布的高渗透率中心，流体从这些区域中的一个跳到另一个。
*  <img src="https://www.dealii.org/images/steps/developer/step-21.random2d.gif" alt=""> 
*

* 最后，这里是在三个空间维度上的相同情况，在一个带有 <code>n_refinement_steps=5</code> 的网格上，产生一个大约32000个单元和167000个自由度的网格。
...
* 那么，为什么我们不显示这样的等值面呢？问题在于计算等值面的方式：它们要求被可视化的场是连续的，因此等值面可以通过遵循至少跨越一个单元的轮廓线来生成。然而，我们的饱和场是片状常数和不连续的。如果我们想为一个饱和度 $S=0.5$ 绘制一个等值面，那么在这个领域中，很可能没有一个点是真正达到饱和度的。如果我们必须在这种情况下定义等值线，我们将不得不采取细胞之间的界面，其中相邻的两个细胞的饱和度大于，另一个细胞的饱和度小于0.5。然而，大多数可视化程序似乎并不具备做这种转换的能力。
*

* <a name="extensions"></a><a name="Possibilitiesforextensions"></a><h3>Possibilities for extensions</h3> 。


* 这个项目有许多可以改进的地方。下面列出了其中的三个方面。事实上，所有这些都在构成当前程序的延续的教程程序中得到解决。   step-43  .
*

*<a name="Solvers"></a><h4>Solvers</h4>


* 目前，该程序不是特别快：二维随机介质的计算在1000个左右的时间步数中花费了大约一天的时间。相应的三维计算在800个时间步数上几乎花了两天时间。它没有比这更快的原因有两个方面。首先，我们在每个时间步骤中都要重建整个矩阵，尽管有些部分如 $B$ 、 $B^T$ 和 $M^S$ 块从未改变。
...
* 在引言中，我们确定了时间步长限制@f[
  \triangle t_{n+1} \le \frac h{|\mathbf{u}^{n+1}(\mathbf{x})|}
@f]，该限制必须在全局范围内成立，即对所有 $\mathbf x$ 。离散化后，我们通过选择@f[
  \triangle t_{n+1} = \frac {\min_K h_K}{\max_{\mathbf{x}}|\mathbf{u}^{n+1}(\mathbf{x})|}.
@f]来满足它。
