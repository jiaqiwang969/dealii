 ...
 * 虽然这描述了要解决的问题，但我们仍然要弄清楚如何实现它。基本上，除了额外的均值约束，我们已经解决了这个问题好几次，使用的是迪里希特边界值，我们只需要放弃对迪里希特边界节点的处理。高阶映射的使用也是相当微不足道的，我们将在使用它的各个地方进行解释；在几乎所有可以想象的情况下，你只会把描述映射的对象视为一个黑盒子，你不需要担心，因为它们的唯一用途似乎是被传递到库中深处的地方，那里的函数知道如何处理它们（即在
 ...
 * 对象，以及矩阵的稀疏模式，还有代表边界上自由度平均值为零的约束条件的对象。
 *
 *
 * @code
 * template <int dim>
 ...
 * <code>DoFTools</code>
 * 命名空间有一个函数可以返回一个IndexSet对象，该对象包含所有在边界上的自由度的指数。
 ...
 * @endcode
 *
 * 然后生成一个只有这一个约束的约束对象。首先清除所有以前的内容（这些内容可能来自以前在更粗的网格上的计算），然后添加这一行，将
 ...
 * @endcode
 *
 * 下一个任务是生成一个稀疏模式。在这里，这的确是一个棘手的任务。通常情况下，我们只需调用
 ...
 * 由于这可能非常困难，以至于无法给出合理的答案，只允许分配合理的内存量，因此有一个动态稀疏模式（DynamicSparsityPattern）类，可以帮助我们解决这个问题。它不要求我们事先知道行可以有多少个条目，而是允许任何长度。因此，在你对行的长度没有很好的估计的情况下，它明显更灵活，但是代价是建立这样一个模式也比建立一个你事先有信息的模式要昂贵得多。尽管如此，由于我们在这里没有其他选择，我们将建立这样一个对象，用矩阵的尺寸初始化它，并调用另一个函数
 * <code>DoFTools::make_sparsity_pattern</code>
 ...
 *
 * 这很简单，对吗？
 * 不过，有两点需要说明。首先，这些函数在很多情况下都会用到。也许你想为一个矢量值有限元创建一个拉普拉斯或质量矩阵；或者你想使用默认的Q1映射；或者你想用拉普拉斯算子中的一个系数来装配矩阵。由于这个原因，在
 * <code>MatrixCreator</code> 和 <code>MatrixTools</code>
 * 命名空间中有相当多的这些函数的变体。当你需要这些函数的一个略微不同的版本时，当然值得看一下文档，并检查一些东西是否适合你的需要。
 ...
 * $2$ ，这就弥补了上面的公式计算  <code>gauss_degree</code>  。
 ...
 * @endcode
 *
 * 对于组装右手边，我们不得不使用两个不同的矢量对象，然后把它们加在一起。我们不得不这样做的原因是，
 ...
 * @endcode
 *
 * 最后，评估我们得到的解决方案。正如在介绍中所说的，我们对解决方案的H1半正态感兴趣。在这里，我们在库中也有一个函数可以做到这一点，尽管是以一种稍微不明显的方式：
 ...
 * @endcode
 *
 * 这是主循环，用线性到立方的映射进行计算。请注意，由于我们只需要
 ...
 * 5120 1.253139 0.000175
 * @endcode
 * 正如我们所期望的，每个不同的mappings的收敛顺序显然是与网格大小成二次方的。
 * <em>   </em>
