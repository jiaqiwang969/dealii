 ...
 * </p>
 * 这个网格中的精细层次只包括在精炼单元上定义的自由度，而没有延伸到没有精炼的域的那一部分。虽然这保证了整体工作的增长
 * ${\cal O}(N)$
 * 为最佳多网格复杂度所必需的，但它导致了在定义平滑的位置和对个别层次上定义的算子提出什么边界条件时的问题，如果层次边界不是外部边界。这些问题在上面引用的文章中有详细的讨论。
 ...
 * CG迭代次数：9。9 周期2 活跃单元数：302 自由度数：352
 * (按级别：8, 25, 89, 223, 160) CG迭代次数。10 周期3
 * 活跃单元数：578 自由度数：649 (按级别：8, 25, 89, 231, 494,
 * 66) CG迭代次数。10 第四周期 活跃单元数：1100
 * 自由度数：1218 (按级别：8, 25, 89, 274, 764, 417, 126)
 * CG迭代次数。10 周期5 活跃单元数：2096 自由度数：2317
 * (按级别：8, 25, 89, 304, 779, 1214, 817) CG迭代次数。11
 * 第六周期 活跃单元数：3986 自由度数：4366 (按级别：8, 25,
 * 89, 337, 836, 2270, 897, 1617) CG迭代次数。10 周期7
 * 活动单元数：7574 自由度数：8350 (按级别：8, 25, 89, 337,
 * 1086, 2835, 2268, 1789, 3217) CG迭代次数。11</pre>
 ...
 * 另一个途径是使用代数多网格方法。这里使用的几何多网格方法有时会有点难实现，因为它需要所有这些额外的数据结构，如果程序要在通过MPI耦合的机器上并行运行，例如，就会变得更加困难。在这种情况下，如果我们能使用一个黑盒子去修饰，那就更简单了，它使用某种多网格层次结构以获得良好的性能，但可以自己计算出水平矩阵和类似东西。代数多栅方法正是这样做的，我们将在
 * step-31 中使用它们来解决斯托克斯问题，在 step-32 和
 * step-40
 * 中使用它们来解决平行变分。也就是说，这个例子程序的MPI并行版本可以在
 * step-50 中找到。
 * 最后，人们可能想考虑如何将几何多网格用于其他类型的问题，特别是 @ref vector_valued "矢量值问题"
 * 。这是 step-56
 * 的主题，我们在这里使用了斯托克斯方程的技术。
 *
