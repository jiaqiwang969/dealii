 ...
 * && \text{ when } |\mathbf{x}| \to \infty, \end{aligned} \f]来保证的。
 * 也就是说，身体在我们的坐标系中是静止的，不具有渗透性，而流体在无限远处具有（恒定）速度
 * $\mathbf{v}_\infty$
 * 。另一种观点是，我们的坐标系随着身体移动，而背景流体在无穷远处处于静止状态。注意，我们将法线
 * $\mathbf{n}$ 定义为域 $\Omega$
 ...
 * \mathbf{\tilde{v}}=\nabla\phi,
 * \f]因此，上述欧拉方程的第二部分可以改写为未知数
 * $\phi$ 的同质拉普拉斯方程：\f{align*} \label{laplace} \Delta\phi
 * &= 0 \qquad &&\text{in}\ \mathbb{R}^n\backslash\Omega, \\
 * \mathbf{n}\cdot\nabla\phi &=
 *
 ...
 *
 * -\phi_\infty. \f]
 * 利用这个结果，我们可以使用所谓的单层和双层势能算子，只在边界
 ...
 * 的极限，利用众所周知的单层和双层运算符的特性，我们得到一个关于
 * $\phi$ 的方程，就在 $\Omega$ 边界的 $\Gamma$  。 \f[\label{SD}
 ...
 * 内是已知的，这意味着，如果我们将诺伊曼数据设置为零，那么任何常数
 ...
 * 中。换句话说，我们想验证上面所说的伯努利定律是否真的成立。为了证明这一点，我们用这个方程的左手边等于
 * @f{align*}
 * \mathbf{v}\cdot\nabla\mathbf{v}
 * &=
 ...
 * $\Gamma = \partial \Omega$ 细分为 $M$ 条线段，如果 $n=3$ ，让
 * $M$
 * 条四边形。我们将每个单独的线段或四边形称为<i>element</i>或<i>cell</i>，与周围空间
 ...
 * 这种方法要求在一定数量的同位点上对边界积分方程进行评估，同位点的数量等于系统的未知数。这些点的选择是一个微妙的问题，需要仔细研究。假设这些点暂时是已知的，并称它们为
 * $\mathbf x_i$ 和 $i=0...n\_dofs$ 。
 * 那么问题就变成了：给定基准点 $\mathbf{v}_\infty$ ，在 $V_h$
 * 中找到一个函数 $\phi_h$ ，使以下 $n\_dofs$ 方程得到满足。
 * \f{align*} \alpha(\mathbf{x}_i) \phi_h(\mathbf{x}_i)
 *
 *
 *
 *
 ...
 * 实现是相当直接的。在以前的教程中没有使用过的主要一点是，deal.II中的大多数类不仅在维度上有模板，而且实际上在我们提出微分方程的流形的维度以及这个流形嵌入的空间的维度上也有模板。默认情况下，第二个模板参数等于第一个，这意味着我们要在二维空间的二维区域内求解。在这种情况下，要使用的三角类是
 ...
 * @f}
 * 见，例如J.N. Newman, <i>Marine Hydrodynamics</i>,
 * 1977,pp.127.对于单位速度和半径，并限制 $(x,y,z)$
 * 位于球体表面， $\phi =
 *
 ...
 * @endcode
 *
 * 一旦我们得到了解决方案，我们就计算计算势的 $L^2$
 * 误差以及实体角的近似值的 $L^\infty$
 * 误差。我们使用的网格是平滑曲线的近似值，因此计算出的角的分量或实体角的对角线矩阵
 * $\alpha(\mathbf{x})$  应该一直等于  $\frac 12$
 * 。在这个例程中，我们输出势的误差和计算角度的近似值的误差。注意，后者的误差实际上不是计算角度的误差，而是我们对球体和圆的近似程度的衡量。
 ...
 * @endcode
 *
 * 通常的deal.II类可以通过指定问题的 "cod dimension
 * "来用于边界元素方法。这是通过将Triangulation,
 * FiniteElement和DoFHandler的可选第二模板参数设置为嵌入空间的维度来实现的。在我们的例子中，我们生成了嵌入到2或3维空间的1或2维网格。
 ...
 * @endcode
 *  以下是我们通过参数文件填充的变量。
 * 在这个例子中，我们使用的新对象是 Functions::ParsedFunction
 * 对象和QuadratureSelector对象。 Functions::ParsedFunction
 * 类允许我们通过参数文件轻松快速地定义新的函数对象，自定义的定义可以非常复杂（所有可用的选项见该类的文档）。
 ...
 * 或  step-6
 * ）中的方式基本相同。这里唯一的新成分是ParsedFunction对象，它在构造时需要说明组件的数量。
 ...
 * $x+y$ 或 $x+y+z$
 * 。实际计算出的解在无穷大时的值为零。在这种情况下，这与精确解相吻合，不需要额外的修正，但是你应该意识到，我们任意设置
 * $\phi_\infty$
 ...
 * ，这样问题才有解。如果不满足这个条件，那么就找不到解，求解器也不会收敛。
 *
 * @code
 ...
 * 所有的网格都被认为是有方向性的，因为它们被嵌入到一个高维空间中。参见GridIn和Triangulation的文档，以进一步了解三角结构中单元的方向。在我们的例子中，网格的法线是外在于2D的圆和3D的球体。
 ...
 * @endcode
 *
 * 与有限元方法不同，如果我们使用拼合边界元方法，那么在每个装配循环中，我们只装配指一个自由度（与支撑点
 * $i$
 * 相关的度）和当前单元之间的耦合信息。这是用一个fe.dofs_per_cell元素的向量完成的，然后它将被分配到全局行的矩阵中
 * $i$  。以下对象将持有这些信息。
 *
 * @code
 ...
 * @endcode
 *
 * 指数 $i$ 在拼合点上运行，这些拼合点是 $i$
 * 个基函数的支持点，而 $j$ 在内部积分点上运行。
 *
 *
 ...
 * @endcode
 *
 * 这样做之后，我们就可以开始对所有单元进行积分循环，首先初始化FEValues对象，得到正交点的
 * $\mathbf{\tilde v}$
 ...
 * @endcode
 *
 * 然后我们在当前单元上形成所有自由度的积分（注意，这包括不在当前单元上的自由度，这与通常的有限元积分有偏差）。如果其中一个局部自由度与支持点
 * $i$
 ...
 * @endcode
 *  然后我们进行积分。如果指数 $i$
 * 不是局部自由度之一，我们只需将单层项加到右边，将双层项加到矩阵中。
 *
 * @code
 ...
 * $j$
 * 索引上运行的单元包含support_point[i]。在这种情况下，单层和双层电势都是单数，它们需要特殊处理。
 ...
 * $r$
 * 的逆上定制的，而我们的奇异函数生活在实空间，然而在三维情况下一切都更简单，因为奇异性与变换的行列式呈线性比例。这使得我们可以只建立一次奇异的二维正交规则，并在所有单元中重复使用它们。
 ...
 * }
 * @endcode
 * <a name="Results"></a><h1>Results</h1> 。   我们使用以下
 * <code>parameters.prm</code>
 * 文件（也可以在所有其他源文件所在的目录中找到）运行该程序。
 * @verbatim
 ...
 * 势的近似 $\phi$
 * 的误差主要是由于域的近似造成的。通过使用高阶映射可以获得更好的近似值。
