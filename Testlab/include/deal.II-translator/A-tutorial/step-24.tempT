...
@f]
* 这里 $\rho (\mathbf r) $ 是密度； $C_p (\mathbf r) $ 是比热； $\frac{\partial T}{\partial t}(t,\mathbf r)$ 是传递的微波能量带来的温升； $H(t,\mathbf r)$ 是加热函数，定义为从沉积的微波能量转化为每一时间和体积的热能。
...
r)\delta(t)$ （其中 $a(\mathbf r)$ 是微波能量的吸收强度图， $\delta(t)$ 是狄拉克三角函数），与上面的第一个方程一起，将产生温度 $T(\mathbf r)$ 在时间 $t=0$ 的瞬时跳跃。使用这一假设，并将所有方程放在一起，我们可以将上述内容写成并合并为：@f[
...
* 一般来说，这是一个困难的问题：好的吸波边界条件是非线性的，并且/或者在数值上非常昂贵。因此，我们选择了一个简单的一阶近似吸收边界条件，即@f[
...
* 使用 step-23 中介绍的一般 $\theta$ 方案，这个模型的半简化、弱化版本是：@f{eqnarray*}
...
* 与 step-23 相比，本程序增加了对简单的吸收性边界条件的处理。此外，它还处理了从实际实验测量中获得的数据。为此，我们需要在实验也评估了真实压力场的点上评估该解决方案。我们将看到如何使用 VectorTools::point_value 函数在下面进一步做到这一点。
*
...
\Delta p-\frac{1}{c_0^2} \frac{\partial^2 p}{\partial t^2} = \lambda
a(\mathbf r)\frac{d\delta(t)}{dt}.
@f]为了看看如何将这个单一方程转化为具有初始条件的PDE的通常状态，让我们假设物理上相当合理的介质最初是静止的，即 $p(t,\mathbf
r)=\frac{\partial p(t,\mathbf r)}{\partial t}=0$ 为 $t<0$ 。接下来，让我们形成两边相对于时间的不定积分：@f[
\int^t \Delta p\; dt
* 
...
@f]换言之，使用 $p(-\epsilon,\mathbf r)=0$ ，我们找回了初始条件@f[
  ...
  =
  \lambda a(\mathbf r).
@f]。同时，我们知道，对于每一个 $t>0$ ，delta函数都是零，所以对于 $0<t<T$ ，我们得到了方程@f[
...
* 
 @endcode
*
* 以下是新的内容：首先，我们需要从吸收性边界条件中得出的边界质量矩阵 $B$ 。同样，由于这次我们考虑的是一个现实的介质，我们必须有一个衡量波速的标准 $c_0$ ，它将进入所有与拉普拉斯矩阵（我们仍然定义为 $(\nabla \phi_i,\nabla \phi_j)$ ）有关的公式。
*
...
*
* 像往常一样，我们必须定义我们的初始值、边界条件和右手函数。这次事情有点简单：我们考虑的是一个由初始条件驱动的问题，所以没有右手函数（尽管你可以在 step-23 中查找，看看如何做到这一点）。其次，没有边界条件：域的整个边界由吸收性边界条件组成。这就只剩下初始条件了，那里的事情也很简单，因为对于这个特殊的应用，只规定了压力的非零初始条件，而没有规定速度的非零初始条件（在初始时间是零）。   
...
*
* 下面的系统几乎是我们在 step-23 中已经做过的，但有两个重要的不同之处。首先，我们必须在原点周围创建一个半径为1的圆形（或球形）网格。这并不新鲜：我们之前在 step-6 和 step-10 中已经这样做了，在那里我们还解释了PolarManifold或SphericalManifold对象如何在细化单元时将新点放在同心圆上，我们在这里也会使用。   
...
*
* 以下两个函数，解决压力和速度变量的线性系统，几乎是逐字逐句地从 step-23 中提取的（除了名称从 $u$ 改为 $p$ 的主变量之外）。
*
...
* 该程序将每个时间步骤的图形数据以及在每个检测器位置评估的数值都写入磁盘。我们将它们绘制成图。目前，我们的实验只在二维空间内通过圆形扫描单个检测器来完成。这里的组织样本是 $X-Y$ 平面上的一个薄片（ $Z=0$ ），我们假设其他 $Z$ 方向的信号不会对数据产生影响。因此，我们只需要将我们的实验数据与二维模拟数据进行比较。
*<a name="Oneabsorber"></a><h3> One absorber </h3>。
*

* 这部电影显示了由一个小型吸收器产生的热声波在介质中传播（在我们的模拟中，我们假设介质是矿物油，其声速为1.437  $\frac{mm}{\mu s}$  ）。
...
* 很明显，在角度 $180^\circ$ 时，源的位置更接近探测器。实验数据中可以看到的所有其他信号都是由于组织的其他部分也有弱吸收体，这些吸收体环绕着中心的小强吸收体产生的信号。另一方面，在模拟数据中，我们只模拟了小的强吸收体。
