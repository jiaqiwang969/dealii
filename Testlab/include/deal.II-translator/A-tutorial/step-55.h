 ...
 * 本教程依赖于  step-40  ,  step-22  。 @htmlonly <table
 ...
 *
 * @note
 * 作为这个程序的先决条件，你需要安装PETSc或Trilinos和p4est库。在<a
 * href="../../readme.html"
 * target="body">README</a>文件中描述了deal.II和这些附加库的安装情况。
 * <a name="Intro"></a><a name="Introduction"></a><h1>Introduction</h1> 。
 *
 *  在 step-40
 * 的基础上，本教程展示了如何使用MPI与PETSc或Trilinos的线性代数并行地解决具有多个组成部分的线性PDEs。为此，我们回到了
 * step-22  中讨论的斯托克斯方程。编写本教程的动机是在
 * step-40 （并行拉普拉斯）和 step-32
 * （针对时间相关问题的并行耦合斯托克斯与布西尼斯克）之间提供一个中间步骤（双关）。
 * 本教程的学习成果是。
 *
 ...
 * 我们正在求解满足斯托克斯方程的速度 $\textbf{u}$ 和压力 $p$ ，其读数为@f{eqnarray*}
 *
 *
 *
 *
 ...
 * 确保你阅读（甚至更好：尝试） step-22 中 "可能的扩展
 * "一节中描述的
 * "块状舒尔补强预处理"。就像那里描述的那样，我们将使用Krylovmethod和块状预处理来解决块状系统。
 * 我们的目标是为线性系统构造一个非常简单的（也许是最简单的）最优预处理程序。如果预处理系统的迭代次数与网格大小无关，则该预处理程序被称为
 * "最优 "或 "最优复杂性" $h$
 * 。你可以把这个定义扩展到也要求与所使用的处理器数量无关（我们将在结果部分讨论这个问题），计算域和网格质量，测试案例本身，有限元空间的多项式程度，等等。
 * 为什么恒定的迭代次数被认为是 "最佳
 * "的？假设被分解的PDE给出一个有N个未知数的线性系统。由于有限元离散化产生的矩阵是稀疏的，矩阵-向量乘积可以在O(N)时间内完成。先决条件的应用充其量也只能是O(N)(例如可以用多棱镜方法)。如果解决线性系统所需的迭代次数与
 * $h$
 * 无关（因此也与N无关），那么解决该系统的总成本将是O(N)。不可能超越这个复杂度，因为即使是查看右手边的所有条目也已经需要O(N)的时间。更多信息见
 * @cite elman2005 ，第2.5章（多网格）。
 * 这里描述的预处理程序比 step-22
 * 中描述的更简单，通常需要更多的迭代，从而需要更多的时间来解决。在考虑预处理程序时，最优性不是唯一重要的指标。但是一个最佳的、昂贵的预处理程序通常比一个便宜的、非最佳的预处理程序更受欢迎。这是因为，最终，随着网格尺寸越来越小，线性问题越来越大，前者将最终战胜后者。
 * <a name="Thesolverandpreconditioner"></a><h3>The solver and
 * preconditioner</h3>
 *
 * 我们对线性系统进行预处理@f{eqnarray*}
 * \left(\begin{array}{cc}
 *  A & B^T \\ B & 0
 * \end{array}\right)
 * \left(\begin{array}{c}
 ...
 * \end{array}\right),
 * @f}来预处理。
 * 其中 $S=-BA^{-1} B^T$ 是舒尔补码。 通过对 $P$
 * 的选择，假设我们准确地处理了 $A^{-1}$ 和 $S^{-1}$
 * （这是一种 "理想化
 * "的情况），预处理的线性系统有三个独立于 $h$
 * 的特征值，因此是 "最优 "的。  见 @cite elman2005
 * 第6.2.1节（特别是第292页）。作为比较，使用 step-22
 * 中理想版本的上块三角预处理(也用于 step-56
 * )会使所有特征值都等于1。 我们将使用 $P^{-1}$
 * 中的逆运算的近似值，它（几乎）与 $h$
 * 无关。在这种情况下，我们可以再次证明，特征值与 $h$
 * 无关。对于Krylov方法，我们选择MINRES，它对分析很有吸引力(迭代次数被证明与
 * $h$
 * 无关，见上述书中第6.2.1章的其余部分)，从计算的角度看很好(比GMRES等更简单、更便宜)，而且适用(矩阵和预处理器是对称的)。
 * 对于近似，我们将使用质量矩阵在压力空间的CG解来近似
 * $S^{-1}$ 的作用。请注意，质量矩阵在光谱上等同于 $S$
 * 。我们可以预期CGiterations的数量与 $h$
 * 无关，即使使用像ILU这样简单的预处理程序。
 * 对于速度块 $A$
 * 的近似，我们将执行一个AMGV循环。在实践中，这种选择并不完全独立于
 * $h$
 * ，这可以解释迭代次数的轻微增加。一个可能的解释是，最粗的层次将被精确解决，而最粗矩阵的层次数和大小是不可预测的。
 *
 *  <a name="Thetestcase"></a><h3>The testcase</h3>
 * 我们将根据经典的Kovasznay问题构建一个人造解，见 @cite
 * kovasznay1948laminar
 * 。这里是一个由X速度着色的解决方案的图像，包括速度的流线。
 * <img
 * src="https://www.dealii.org/images/steps/developer/step-55.solution.png"
 * alt="">
 * 但我们必须在这里作弊，因为我们不是在求解非线性的Navier-Stokes方程，而是求解没有对流项的线性Stokes系统。因此，为了重现完全相同的解决方案，我们使用Kovasznay问题的解决方案的制造方法。这将有效地把对流项移到右手边
 * $f$ 。 右手边是用脚本 "reference.py
 * "计算的，我们用精确的解来计算边界条件和误差。
 *
 ...
 * @endcode
 *
 * 这个类暴露了通过函数 InverseMatrix::vmult().
 * 应用一个给定矩阵的逆的动作，在内部，逆不是明确形成的。相反，一个带有CG的线性求解器被执行。该类扩展了
 * step-22
 * 中的InverseMatrix类，增加了一个指定预处理程序的选项，并允许在vmult函数中使用不同的矢量类型。
 *
 * @code
 ...
 * @endcode
 *
 * <a name="Themainprogram"></a> <h3>The main program</h3>。 主类与
 * step-40
 * 非常相似，只是矩阵和向量现在是块状版本，我们为拥有的和相关的DoF存储一个
 * std::vector<IndexSet>
 * ，而不是一个IndexSet。我们正好有两个IndexSets，一个用于所有速度未知数，一个用于所有压力未知数。
 *
 * @code
 * template <int dim>
 * class StokesProblem
 ...
 *
 * Kovasnay流被定义在域[-0.5,
 * 1.5]^2上，我们通过将最小和最大值传递给
 * GridGenerator::hyper_cube. 来创建这个域。
 *
 * @code
 * template <int dim>
 ...
 * @endcode
 *
 * <a name="SystemSetup"></a> <h3>System Setup</h3>。 与 step-40
 * 相比，块状矩阵和向量的构造是新的，与 step-22
 * 这样的串行代码相比是不同的，因为我们需要提供属于我们处理器的行的集合。
 *
 * @code
 * template <int dim>
 ...
 * @endcode
 *
 * 设置边界条件和悬空节点的约束与  step-40
 * 相同。尽管我们没有任何悬空节点，因为我们只进行全局细化，但把这个函数调用放进去仍然是个好主意，以防以后引入自适应细化。
 *
 * @code
 *   {
 ...
 * @endcode
 *
 * 现在我们根据BlockDynamicSparsityPattern来创建系统矩阵。我们知道我们不会有不同速度分量之间的耦合（因为我们使用的是拉普拉斯而不是变形张量），也不会有压力与其测试函数之间的耦合，所以我们用一个表来把这个耦合信息传达给
 ...
 * @endcode
 *
 * 最后，我们以正确的尺寸构建块状向量。带有两个
 * std::vector<IndexSet> 的函数调用将创建一个重影向量。
 *
 * @code
 ...
 * @endcode
 *
 * <a name="Solving"></a> <h3>Solving</h3>
 ...
 *
 * 与 step-56
 * 一样，我们减去平均压力，以便对我们的参考方案进行误差计算，该方案的平均值为零。
 *
 * @code
 ...
 * 正如上面的讨论所预期的那样，迭代次数与处理器的数量无关，只与
 * $h$ 有非常小的关系。 <table> <tr> <th colspan="2">PETSc</th> <th
 ...
 * <td>49</td> <td>49</td> <td>49</td> <td>51</td> <td>51</td> <td>51</td>
 * <td>49</td> <td>49</td> </tr> <tr> <td>1</td> <td>2467</td> <td>52</td>
 * <td>52</td> <td>52</td> <td>52</td> <td>52</td> <td>54</td> <td>54</td>
 * <td>53</td> </tr> <tr> <td>2</td> <td>9539</td> <td>56</td> <td>56</td>
 * <td>56</td> <td>54</td> <td>56</td> <td>56</td> <td>54</td> <td>56</td>
 * </tr> <tr> <td>3</td> <td>37507</td> <td>57</td> <td>57</td> <td>57</td>
 * <td>57</td> <td>57</td> <td>56</td> <td>57</td> <td>56</td> </tr> <tr>
 * <td>4</td> <td>148739</td> <td>58</td> <td>59</td> <td>57</td> <td>59</td>
 * <td>57</td> <td>57</td> <td>57</td> <td>57</td> </tr> <tr> <td>5</td>
 * <td>592387</td> <td>60</td> <td>60</td> <td>59</td> <td>59</td> <td>59</td>
 * <td>59</td> <td>59</td> <td>59</td> </tr> <tr> <td>6</td> <td>2364419</td>
 * <td>62</td> <td>62</td> <td>61</td> <td>61</td> <td>61</td> <td>61</td>
 * <td>61</td> <td>61</td> </tr> </table> <table> <tr> <th
 * colspan="2">Trilinos</th> <th colspan="8">number of processors</th> </tr>
 * <tr> <th>cycle</th> <th>dofs</th> <th>1</th> <th>2</th> <th>4</th>
 * <th>8</th> <th>16</th> <th>32</th> <th>64</th> <th>128</th> </tr> <tr>
 * <td>0</td> <td>659</td> <td>37</td> <td>37</td> <td>37</td> <td>37</td>
 * <td>37</td> <td>37</td> <td>37</td> <td>37</td> </tr> <tr> <td>1</td>
 * <td>2467</td> <td>92</td> <td>89</td> <td>89</td> <td>82</td> <td>86</td>
 * <td>81</td> <td>78</td> <td>78</td> </tr> <tr> <td>2</td> <td>9539</td>
 * <td>102</td> <td>99</td> <td>96</td> <td>95</td> <td>95</td> <td>88</td>
 * <td>83</td> <td>95</td> </tr> <tr> <td>3</td> <td>37507</td> <td>107</td>
 * <td>105</td> <td>104</td> <td>99</td> <td>100</td> <td>96</td> <td>96</td>
 * <td>90</td> </tr> <tr> <td>4</td> <td>148739</td> <td>112</td> <td>112</td>
 * <td>111</td> <td>111</td> <td>127</td> <td>126</td> <td>115</td>
 * <td>117</td> </tr> <tr> <td>5</td> <td>592387</td> <td>116</td>
 * <td>115</td> <td>114</td> <td>112</td> <td>118</td> <td>120</td>
 * <td>131</td> <td>130</td> </tr> <tr> <td>6</td> <td>2364419</td>
 * <td>130</td> <td>126</td> <td>120</td> <td>120</td> <td>121</td>
 * <td>122</td> <td>121</td> <td>123</td> </tr> </table>
 ...
 * iterations</h4> 。   玩弄平滑器、平滑步骤和Trilinos
 * AMG的其他属性，以实现最佳的预处理程序。 <a
 ...
 * 你可以在对流项 $b \cdot \nabla u$ 中规定精确的流解为 $b$
 * 。如果你把右手边设置为零，这应该会得到与原问题相同的解。
 * <a name="Adaptiverefinement"></a><h4>Adaptive refinement</h4> 。
 *
 * 到目前为止，这个教程程序在每一步中都会对网格进行全局细化。将
 * StokesProblem::refine_grid() 中的代码替换为类似的内容
 * @code
