...
*  As in all programs, we start with a list of include files from the library, and as usual they are in the standard order which is   <code>base</code>  
* 
*  -    <code>lac</code>  
* 
*  -    <code>grid</code>  
...
* 
*  As for the program itself, we first define classes that evaluate the solutions of a Laplace equation. In fact, they can evaluate every kind of solution, as long as it is described by a   <code>DoFHandler</code>   object, and a solution vector. We define them here first, even before the classes that actually generate the solution to be evaluated, since we need to declare an abstract base class that the solver classes can refer to.   
*   From an abstract point of view, we declare a pure base class that provides an evaluation operator() which will do the evaluation of the solution (whatever derived classes might consider an   <code>evaluation</code>  ). Since this is the only real function of this base class (except for some bookkeeping machinery), one usually terms such a class that only has an   <code>operator()</code>   a   <code>functor</code>   in C++ terminology, since it is used just like a function object.   
...
*   In the   step-9   example program, we have already seen how such an exception class can be declared, using the   <code>DeclExceptionN</code>   macros. We use this mechanism here again.     
...
*   Also note that we have to prefix   <code>this-@></code>   to access a member variable of the template dependent base class. The reason here, and further down in the program is the same as the one described in the   step-7   example program (look for   <code>two-stage name lookup</code>   there).
* 
...
*   Besides all these theoretical questions, we here have a good reason, which will become clearer to the reader below. Basically, we want to be able to have a family of different Laplace solvers that differ so much that no larger common subset of functionality could be found. We therefore just declare such an abstract base class, taking a pointer to a triangulation in the constructor and storing it henceforth. Since this triangulation will be used throughout all computations, we have to make sure that the triangulation is valid until it is last used. We do this by keeping a   <code>SmartPointer</code>   to this triangulation, as explained in   step-7  .     
*   Note that while the pointer itself is declared constant (i.e. throughout the lifetime of this object, the pointer points to the same object), it is not declared as a pointer to a constant triangulation. In fact, by this we allow that derived classes refine or coarsen the triangulation within the   <code>refine_grid</code>   function.     
*   Finally, we have a function   <code>n_dofs</code>   is only a tool for the driver functions to decide whether we want to go on with mesh refinement or not. It returns the number of degrees of freedom the present simulation has.
* 
...
*   Basically, in a few words, the constructor of this class takes pointers to a triangulation, a finite element, and a function object representing the boundary values. These are either passed down to the base class's constructor, or are stored and used to generate a   <code>DoFHandler</code>   object later. Since finite elements and quadrature formula should match, it is also passed a quadrature object.     
*   The   <code>solve_problem</code>   sets up the data structures for the actual solution, calls the functions to assemble the linear system, and solves it.     
*   The   <code>postprocess</code>   function finally takes an evaluation object and applies it to the computed solution.     
*   The   <code>n_dofs</code>   function finally implements the pure virtual function of the base class.
* 
...
* 
 @endcode
* 
*  Now here comes the constructor of the class. It does not do much except store pointers to the objects given, and generate   <code>DoFHandler</code>   object initialized with the given pointer to a triangulation. This causes the DoF handler to store that pointer, but does not already generate a finite element numbering (we only ask for that in the   <code>solve_problem</code>   function).
* 
...
* 
 ...
 @endcode
* 
*  As stated above, the   <code>postprocess</code>   function takes an evaluation object, and applies it to the computed solution. This function may be called multiply, once for each evaluation of the solution which the user required.
* 
...
*   The actions of the class are rather what you have seen already in previous examples already, so a brief explanation should suffice: the constructor takes the same data as does that of the underlying class (to which it passes all information) except for one function object that denotes the right hand side of the problem. A pointer to this object is stored (again as a   <code>SmartPointer</code>  , in order to make sure that the function object is not deleted as long as it is still used by this class).     
*   The only functional part of this class is the   <code>assemble_rhs</code>   method that does what its name suggests.
* 
...
* 
 @endcode
* 
*  ... as does the   <code>assemble_rhs</code>   function. Since this is explained in several of the previous example programs, we leave it at that.
* 
...
* 
*  By now, all functions of the abstract base class except for the   <code>refine_grid</code>   function have been implemented. We will now have two classes that implement this function for the   <code>PrimalSolver</code>   class, one doing global refinement, one a form of local refinement.     
*   The first, doing global refinement, is rather simple: its main function just calls   <code>triangulation-@>refine_global (1);</code>  , which does all the work.     
*   Note that since the   <code>Base</code>   base class of the   <code>Solver</code>   class is virtual, we have to declare a constructor that initializes the immediate base class as well as the abstract virtual one.     
...
* 
 ...
 @endcode
* 
*   <a name="LocalrefinementbytheKellyerrorindicator"></a>  <h4>Local refinement by the Kelly error indicator</h4>
* 
...
*   For this example, let us choose as exact solution the function   $u(x,y)=exp(x+sin(10y+5x^2))$  . In more than two dimensions, simply repeat the sine-factor with   <code>y</code> replaced by <code>z</code>   and so on. Given this, the following two classes are probably straightforward from the previous examples.
* 
...
* @code
 ...
 @endcode
* 
*  Then give the   <code>alive</code>   indication for this iteration. Note that the   <code>std::flush</code>   is needed to have the text actually appear on the screen, rather than only in some buffer that is only flushed the next time we issue an end-line.
* 
...
* However, we are not yet finished with evaluation the point valuecomputation. In fact, plotting the error  $e=|u(x_0)-u_h(x_0)|$   for the tworefinement criteria yields the following picture:
* 
...
* 
* (  <em>  Very brief answers for the impatient:  </em>   at steps with largererrors, the mesh is not regular at the point of evaluation, i.e. someof the adjacent cells have hanging nodes; this destroys somesuperapproximation effects of which the globally refined mesh canprofit. Answer 2: this quick hack
* @code
