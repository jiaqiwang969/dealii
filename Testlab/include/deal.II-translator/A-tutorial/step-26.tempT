...
@f}
* 从某种意义上说，这个方程比我们在前面的程序 step-23 、 step-24 、 step-25 中讨论的方程（即波浪方程）更简单。这是由于热方程随着时间的推移平滑了解，因此在许多方面更宽容。例如，当使用隐式时间步长方法时，我们实际上可以采取大的时间步长，我们不会因为每隔几步调整网格而引入小的干扰而产生麻烦，等等。
...
* 还有一个类似的问题，即我们将选择一个右手边，在不同的时间在领域的不同部分开启。为了避免在我们突然需要更细的网格的地方，被太粗的网格抓个正着，我们也将在程序中强制执行<i>minimal</i>的网格细化水平。
...
*
* - 错误地处理边界值，例如同样忘记了 $k_n$ 或 $\theta$ 的系数，或忘记了不仅对右手边而且对系统矩阵应用非零边界值。
...
* 下一个部分是这个程序的主类的声明。它沿用了以前的例子中很好的路径。如果你看过 step-6 ，例如，这里唯一值得注意的是，我们需要建立两个矩阵（质量和拉普拉斯矩阵），并保存当前和前一个时间步长的解。然后，我们还需要存储当前时间、时间步长和当前时间步长的编号。最后一个成员变量表示引言中讨论的theta参数，它允许我们在一个程序中处理显式和隐式欧拉方法以及Crank-Nicolson方法和其他通用方法。   
...
* 这个函数是程序的有趣部分。它负责自适应网格细化的工作。这个函数执行的三个任务是：首先找出需要细化/粗化的单元，然后实际进行细化，最终在两个不同的网格之间传输解向量。第一个任务是通过使用成熟的凯利误差估计器来实现的。第二项任务是实际进行再细化。这也只涉及到基本的函数，例如 <code>refine_and_coarsen_fixed_fraction</code> ，它可以细化那些具有最大估计误差的单元，这些误差加起来占60%，并粗化那些具有最小误差的单元，这些单元加起来占40%的误差。请注意，对于像目前这样的问题，即有事发生的区域正在四处移动，我们希望积极地进行粗化，以便我们能够将单元格移动到有必要的地方。   
...
*
* 作为网格细化的一部分，我们需要将旧网格中的解向量转移到新网格中。为此，我们使用了SolutionTransfer类，我们必须准备好需要转移到新网格的解向量（一旦我们完成了细化，我们将失去旧的网格，所以转移必须与细化同时发生）。在我们调用这个函数的时候，我们将刚刚计算出解决方案，所以我们不再需要old_solution变量（它将在网格被细化后被解决方案覆盖，也就是在时间步长结束时；见下文）。换句话说，我们只需要一个求解向量，并将其复制到一个临时对象中，当我们在下面进一步调用  <code>setup_system()</code>  时，它就不会被重置。     
...
* 
 @endcode
*
* 接下来，我们将强迫项添加到来自时间步长的强迫项中，同时建立矩阵 $M+k_n\theta A$ ，我们必须在每个时间步长中进行反转。这些操作的最后一块是消除线性系统中悬挂的节点约束自由度。
*
...
*
* 最后，人们可能只是同意<i>in general</i> <code>goto</code> 语句是个坏主意，但要务实地指出，在某些场合，它们可能有助于避免代码重复和控制流的尴尬。这可能是其中的一个地方，它与Steve McConnell在他关于良好编程实践的优秀书籍 "Code Complete"  @cite CodeComplete 中所采取的立场相吻合（见 step-1 的介绍中提到的这本书），该书花了惊人的10页来讨论 <code>goto</code> 的一般问题。
*
...
* 为了提高你的模拟在时间上的准确性和分辨率，通常会减少时间步长 $k_n$ 。如果你在这个特定的例子中开始玩弄时间步长，你会注意到，如果 $k_n$ 低于某个阈值，解决方案会变成部分负值。这不是我们所期望发生的（在自然界）。
...
* @f{align*}
  ...
  \forall j &\in S_i,
@f}
*所有解 $u^{n}$ 的符号与之前的解 $u^{n-1}$ 保持一致，并因此与初始值 $u^0$ 保持一致。关于正性保持的更多信息，请参见例如<a href="http://bookstore.siam.org/cs14/">Kuzmin, H&auml;m&auml;l&auml;inen</a>。
