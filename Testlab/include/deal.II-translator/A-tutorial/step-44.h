...
* 本报告的目的是为使用deal.II处理非线性固体力学问题提供基础。线性问题在 step-8 中得到了解决。在 step-18 中部分考虑了几何非线性问题的非标准、超弹性类型的形式：使用线性化构成关系的速率形式，问题域随运动变化而变化。然而， Step-18 确实描述了许多关键概念，以便在交易框架内实现弹性。
* 我们从非线性运动学的速成课程开始。为了简单起见，我们将注意力限制在准静态问题上。此后，我们介绍了各种关键的应力测量，并描述了构成模型。在解释用于管理材料的类的结构之前，我们详细描述了三场公式。
...
* 这里实现的三场公式是由Simo等人（1985）开创的，被称为混合雅各布-压力公式，重要的相关贡献包括Simo和Taylor（1991）以及Miehe（1994）。
...
@f]注意 $\mathcal{I} \neq \overline{\mathcal{I}}^T$ 定义。此外，我们用@f[
...
* 让时间域表示为 $\mathbb{T} = [0,T_{\textrm{end}}]$  ，其中 $t \in \mathbb{T}$ 和 $T_{\textrm{end}}$ 是总的问题持续时间。考虑一个连续体，在时间 $t=0$ 占据参考配置 $\Omega_0$ ，参考配置中的%粒子由位置矢量 $\mathbf{X}$ 识别。 称为当前配置，表示为 $\Omega$ ，粒子由矢量 $\mathbf{x}$ 识别。参考配置和当前配置之间的非线性映射，表示为 $\boldsymbol{\varphi}$ ，作用如下：@f[
...
@f] 。
* 就空间和材料坐标而言，变形的两个重要衡量标准是左和右Cauchy-Green张量，分别表示为 $\mathbf{b} \dealcoloneq \mathbf{F}\mathbf{F}^T$ 和 $\mathbf{C} \dealcoloneq \mathbf{F}^T\mathbf{F}$  .它们都是对称的和正定的。
* 格林-拉格朗日应变张量定义为@f[
...
@f]，如果假定变形量为无穷小，那么右边的第二项可以忽略， $\boldsymbol{\varepsilon}$ （线性化应变张量）是应变张量的唯一组成部分。从问题的设置来看，这一假定在 step-18 中是无效的，因此在该教程中使用线性化 $\boldsymbol{\varepsilon}$ 作为应变度量是值得怀疑的。
...
* 考奇应力定理将作用于当前构型中无限小的表面元素 $\mathrm{d}a$ 的考奇牵引力 $\mathbf{t}$ 等同于考奇应力张量 $\boldsymbol{\sigma}$ （一个空间量）与表面的外向单位法线 $\mathbf{n}$ 的乘积，因为@f[
...
* 前推和后拉运算符允许人们在材料和空间设置之间转换各种措施。这里使用的应力措施是忌变的，而应变措施是协变的。
...
* 例如，如果亥姆霍兹自由能取决于右Cauchy-Green张量 $\mathbf{C}$ ，那么各向同性的超弹性反应是@f[
	\mathbf{S}
		= 2 \dfrac{\partial \Psi(\mathbf{C})}{\partial \mathbf{C}} \, .
@f]，如果亥姆霍兹自由能取决于左Cauchy-Green张量 $\mathbf{b}$ ，那么各向同性的超弹性反应是@f[
	\boldsymbol{\tau}
...
@f]给出，其中 $\kappa \dealcoloneq \lambda + 2/3 \mu$ 是体积模量（ $\lambda$ 和 $\mu$ 是Lam&eacute; 参数）和 $\overline{I}_1 \dealcoloneq \textrm{tr}\ \overline{\mathbf{b}}$  .要求函数 $\mathcal{G}(J)$  是严格凸的，满足 $\mathcal{G}(1) = 0$  等条件，进一步细节见 Holzapfel（2001）。在本工作中 $\mathcal{G} \dealcoloneq \frac{1}{4} [ J^2
* 
- 1
* 
- 2\textrm{ln}J ]$  。
...
* 
		...
		&=0 \, ,
@f}
* 对于所有的虚拟位移 $\delta \mathbf{u} \in H^1(\Omega)$ ，受制于 $\delta \mathbf{u} = \mathbf{0}$ 对 $\partial \Omega_{\mathbf{u}}$ 的约束，以及所有的虚拟压力 $\delta \widetilde{p} \in L^2(\Omega)$ 和虚拟膨胀 $\delta \widetilde{J} \in L^2(\Omega)$ 。
* 人们应该注意到，三场公式中的体积基尔霍夫应力 $\boldsymbol{\tau}_{\textrm{vol}} \equiv \widetilde{p} J \mathbf{I}$ 和随后的体积正切的定义与超弹性材料一节中给出的一般形式略有不同，其中 $\boldsymbol{\tau}_{\textrm{vol}} \equiv p J\mathbf{I}$ .这是因为压力 $\widetilde{p}$ 现在是一个主要场，而不是一个构成性的推导量。人们需要仔细区分主要场和那些从构成性关系中获得的场。
*  @note  虽然变量都是用空间量表示的，但积分域是初始配置。这种方法被称为 <em>  总拉格朗日公式 </em>  。 step-18  中给出的方法，其积分域是当前配置，可称为 <em>  更新拉格朗日公式 </em>  。这两种方法的各种优点在文献中被广泛讨论。但应注意，它们是等价的。
* 
...
@f}
* 第一个方程是空间设置中的（准静态）平衡方程。第二个方程是约束 $J(\mathbf{u}) = \widetilde{J}$  .第三个方程是压力的定义 $\widetilde{p}$  。
...
href="http://en.wikipedia.org/wiki/Poisson's_ratio">Poisson's ratio</a>）的情况下，对 $\mathbf{u},~\widetilde{p}$ 和 $\widetilde{J}$ 的插值场进行良好选择。通常选择 $Q_n \times DGPM_{n-1} \times DGPM_{n-1}$ 。 这个代码可以容纳 $Q_n \times DGPM_{n-1} \times DGPM_{n-1}$ 的表述。不连续近似允许 $\widetilde{p}$ 和 $\widetilde{J}$ 被压缩掉，并恢复基于位移的经典方法。
...
* 因此，压力和膨胀的不连续有限元插值产生了 $\mathbf{\mathsf{K}}_{\widetilde{p}\widetilde{J}}$ 、 $\mathbf{\mathsf{K}}_{\widetilde{J}\widetilde{p}}$ 和 $\mathbf{\mathsf{K}}_{\widetilde{J}\widetilde{J}}$ 的块对角矩阵，因此我们可以很容易地表达每个单元上的场 $\widetilde{p}$ 和 $\widetilde{J}$ ，只需反转一个局部矩阵并乘以局部右手。然后，我们可以将结果插入其余的方程中，并恢复一个经典的基于位移的方法。为了在元素层面凝结出压力和膨胀的贡献，我们需要以下结果。
* @f{align*}
		d \widetilde{\mathbf{\mathsf{p}}}
...
* 一个好的面向对象的材料类的设计将有助于本教程扩展到广泛的材料类型。在本教程中，我们只有一个材料类，名为Material_Compressible_Neo_Hook_Three_Field.理想情况下，这个类将派生自HyperelasticMaterial类，该类将派生自基类Material.这里使用的三场性质的表述也使问题复杂化。
...
*为了有一个灵活的方法，决定Material_Compressible_Neo_Hook_Three_Field仍然能够计算并返回体积Kirchhoff应力和切线。为了做到这一点，我们选择将内插的主场 $\widetilde{p}$ 和 $\widetilde{J}$ 存储在与正交点相关的Material_Compressible_Neo_Hook_Three_Field类中。这个决定应该在以后的教程扩展到考虑其他材料时重新讨论。
* 

*<a name="Numericalexample"></a><h3> Numerical example </h3>
* 

* 这里考虑的数值例子是一个在压缩状态下的几乎不可压缩的块体。这个基准问题取自于
* 
...
* 
 @endcode
* 
* 在这两个标题中定义了一些与有限应变弹性有关的操作。第一个将帮助我们计算一些运动量，第二个提供一些标准的张量定义。
* 
...
* 然后我们把所有与这个教程程序有关的东西都塞进一个自己的命名空间，并把所有deal.II的函数和类名都导入其中。
* 
...
* 
* 接下来，我们同时选择求解器和预处理器的设置。 当牛顿增量中出现大的非线性运动时，使用有效的预处理程序对于确保收敛性至关重要。
* 
...
* 
* 使用牛顿-拉弗森方案来解决非线性治理方程组。 我们现在定义牛顿-拉弗森非线性求解器的公差和最大迭代次数。
* 
...
* 
* 最后，我们将上述所有的结构合并到一个容器中，该容器容纳了我们所有的运行时选择。
* 
...
* 
* 正如介绍中所讨论的，新胡克材料是一种超弹性材料。 整个领域被假定为由可压缩的新胡克材料组成。 这个类别定义了这种材料在三场公式中的行为。 可压缩的新胡克材料可以用应变能量函数（SEF）来描述  $ \Psi =
...
* 
 @endcode
* 
* 体积自由能相对于 $\widetilde{J}$ 的二次导数。我们需要在切线中明确地进行以下计算，因此我们将其公开。 我们计算 $\frac{\partial^2
...
* 第一个函数用于创建一个材料对象，并正确地初始化所有的张量。第二个函数根据当前的变形量 $\textrm{Grad}\mathbf{u}_{\textrm{n}}$ 、压力 $\widetilde{p}$ 和扩张 $\widetilde{J}$ 场值更新存储的数值和应力。
* 
...
* 
 @endcode
* 
* 为此，我们从位移梯度 $\textrm{Grad}\ \mathbf{u}$ 中计算出变形梯度 $\mathbf{F}$ ，即 $\mathbf{F}(\mathbf{u}) = \mathbf{I} + \textrm{Grad}\ \mathbf{u}$ ，然后让与该正交点相关的材料模型自行更新。当计算变形梯度时，我们必须注意和 $\mathbf{I} +
...
* 
 @endcode
* 
* 在成员函数方面，这个类为它所代表的正交点存储了一个材料类型的副本，以备在域的不同区域使用不同的材料，以及变形梯度的逆。
* 
...
* 
 @endcode
* 
* 有几个函数使用多线程组装系统和右手边的矩阵。它们中的每一个都是包装函数，一个是在WorkStream模型中对一个单元做工作时执行的，另一个是将对这一个单元做的工作复制到代表它的全局对象中。
* 
...
* 
 @endcode
* 
* 存储收敛的解和右手边向量以及切线矩阵的对象。有一个AffineConstraints对象，用于跟踪约束条件。 我们利用了为块状系统设计的稀疏性模式。
* 
...
* 在解决准静态问题时，时间成为一个加载参数，即我们随着时间线性地增加加载量，使得这两个概念可以互换。我们选择用一个恒定的时间步长来线性增加时间。  
* 我们以预处理的方式启动该功能，设置初始扩张值，然后在开始模拟之前输出初始网格，并以第一次时间（和加载）的增量开始模拟。  
...
* 
 @endcode
* 
* ......解决当前的时间步骤，更新总的解决方案向量 $\mathbf{\Xi}_{\textrm{n}} = \mathbf{\Xi}_{\textrm{n-1}} +
 \varDelta \mathbf{\Xi}$  ......
* 
...
* 
 ...
 @endcode
* 
* 另一方面，ScratchData对象存储了较大的对象，如形状函数值数组（  <code>Nx</code>  ）和形状函数梯度和对称梯度向量，我们将在装配过程中使用。
* 
...
* 然后我们定义结构来组装静态凝结的切线矩阵。回顾一下，我们希望解决一个基于位移的公式。由于 $\widetilde{p}$ 和 $\widetilde{J}$ 字段是元素间不连续的，所以我们在元素层面上进行缩合。 由于这些操作是基于矩阵的，我们需要设置一些矩阵来存储一些切线矩阵子块的局部贡献。 我们把这些放在PerTaskData结构中。  
...
* 
 ...
 @endcode
* 
* 我们希望在这里执行的操作的ScratchData对象是空的，因为我们不需要临时数据，但它仍然需要为目前在deal.II中对TBB的实现进行定义。 所以我们为此创建了一个假结构。
* 
...
* 最后我们定义结构以协助更新正交点信息。与SC装配过程类似，我们不需要PerTaskData对象（因为这里没有什么可存储的），但还是必须定义一个。请注意，这是因为对于我们这里的操作来说
* 
*-更新正交点的数据
* 
...
* 
 ...
 @endcode
* 
* ScratchData对象将被用来存储求解向量的别名，这样我们就不必复制这个大的数据结构。然后我们定义一些向量来提取正交点的解值和梯度。
* 
...
* 
 ...
 @endcode
* 
* <a name="Solidmake_grid"></a> <h4>Solid::make_grid</h4> <h4>Solid::make_grid</h4>.
...
* 由于我们希望将诺伊曼BC应用于顶面的一个补丁，我们必须找到域的这一部分的单元面，并用一个明显的边界ID号来标记它们。 我们要找的面在+y面上，将得到边界ID 6（0到5已经在创建立方体域的六个面时使用了）。
* 
...
* 然后，DOF处理程序被初始化，我们以一种有效的方式对网格重新编号。我们还记录了每块DOF的数量。
* 
...
* @f{align*}
 ...
 @f}
* 我们优化了稀疏模式，以反映这种结构，并防止为右对角块成分创建不必要的数据。
* 
...
* 
 ...
 @endcode
* 
* <a name="Soliddetermine_component_extractors"></a> <h4>Solid::determine_component_extractors</h4> 接下来我们从FE系统中计算出一些信息，这些信息描述了哪些局部元素DOF是连接到哪个块组件的。 这将在后面用于从全局矩阵中提取子块。  
* 从本质上讲，我们所需要的是FESystem对象指出参考单元上的DOF连接到哪个块状部件上。 目前，插值域的设置是这样的：0表示位移DOF，1表示压力DOF，2表示膨胀DOF。
* 
...
* 
 @endcode
* 
* 接下来我们设置初始正交点数据。请注意，当正交点数据被检索时，它将以智能指针的向量形式返回。
* 
...
* 
 @endcode
* 
* <a name="Solidupdate_qph_incremental"></a> <h4>Solid::update_qph_incremental</h4> 由于QP信息的更新经常发生，并涉及一些昂贵的操作，我们定义了一个多线程的方法，将任务分布在若干个CPU核心上。  
...
* 
 ...
 @endcode
* 
* <a name="Solidsolve_nonlinear_timestep"></a> <h4>Solid::solve_nonlinear_timestep</h4> * <h4>Solid::solve_nonlinear_timestep</h4>.
 

* 
* 下一个函数是Newton-Raphson方案的驱动方法。在它的顶部，我们创建一个新的向量来存储当前的牛顿更新步骤，重置错误存储对象并打印求解器头。
* 
...
* 我们现在进行一些牛顿迭代来迭代解决这个非线性问题。 由于问题是完全非线性的，而且我们使用的是完全牛顿方法，所以存储在正切矩阵和右手边向量中的数据是不能重复使用的，必须在每个牛顿步骤中清除。然后，我们最初建立线性系统并检查收敛性（并在第一次迭代中存储这个值）。rhs向量中的无约束DOF持有失衡的力，并共同决定是否达到了平衡解。    
...
* 
 @endcode
* 
* 我们构建线性系统，但暂不求解它（这个步骤应该比装配要昂贵得多）。
...
* 
 @endcode
* 
* 在最后，如果发现我们事实上已经做了比参数文件允许的更多的迭代，我们会引发一个异常，可以在main()函数中捕获。调用<code>AssertThrow(condition, exc_object)</code>本质上等同于<code>if (!cond) throw exc_object;</code>，但是前一种形式在异常对象中填充了某些字段，这些字段标识了异常发生的位置（文件名和行号），以便更简单地识别问题发生的位置。
* 
...
- \widetilde{J}]}^{2}\textrm{d}V \bigr]^{1/2}$ 计算扩张 $\widetilde{J}$ 与 $J
 \dealcoloneq \textrm{det}\ \mathbf{F}$ 的吻合程度。我们还返回域的当前体积与参考体积的比率。这对不可压缩介质很有意义，因为我们要检查等压约束的执行情况。
* 
...
* 
 ...
 @endcode
* 
* <a name="Solidget_error_residual"></a> <h4>Solid::get_error_residual</h4> * <h4>Solid::get_error_residual</h4>.
 

* 
* 确定该问题的真实残差误差。 就是说，确定无约束自由度的残差误差。 注意，要做到这一点，我们需要忽略受约束的自由度，将这些向量分量的残差设置为零。
* 
...
* 
 ...
 @endcode
* 
* <a name="Solidget_error_update"></a> <h4>Solid::get_error_update</h4> <h4>Solid::get_error_update</h4>.
...
* 
* 这个函数提供了总解，它在任何牛顿步骤中都是有效的。这是必须的，因为为了减少计算误差，总解只在时间步长结束时更新。
* 
...
* 
* 因为我们使用TBB进行装配，所以我们只需设置过程所需的数据结构的副本，并将它们和装配函数一起传递给WorkStream对象进行处理。请注意，我们必须确保在任何装配操作发生之前，矩阵和RHS向量被重置。此外，由于我们描述的是一个诺伊曼BC的问题，我们将需要面的法线，因此必须在面的更新标志中指定这个。
* 
...
* 当然，我们仍然要定义我们如何为单个单元组装正切矩阵的贡献。 我们首先需要重置和初始化一些从头开始的数据结构，并检索一些关于这个单元上DOF编号的基本信息。 我们可以预先计算单元的形状函数值和梯度。请注意，形状函数梯度是针对当前配置而定义的。 也就是  $\textrm{grad}\ \boldsymbol{\varphi} = \textrm{Grad}\ \boldsymbol{\varphi}
 \ \mathbf{F}^{-1}$  。
* 
...
* 现在我们建立本地单元刚度矩阵和RHS向量。由于全局和局部系统矩阵是对称的，我们可以利用这一特性，只建立局部矩阵的下半部分，并将其值复制到上半部分。 所以我们只组装一半的 $\mathsf{\mathbf{k}}_{uu}$  ,  $\mathsf{\mathbf{k}}_{\widetilde{p}
 ...
 \widetilde{J}}$ 块，而整个 $\mathsf{\mathbf{k}}_{\widetilde{p} \widetilde{J}}$  ,  $\mathsf{\mathbf{k}}_{u \widetilde{J}} = \mathbf{0}$  ,  $\mathsf{\mathbf{k}}_{u \widetilde{p}}$ 块的构建。    
...
* 我们可以观察到的是，这两个张量都是以 "i "DoF为索引的形状函数梯度收缩的。这意味着，当我们在 "j "DoF上循环时，这个特殊的操作保持不变。出于这个原因，我们可以从内循环中提取这个操作，并节省许多操作，对于每个正交点和DoF索引 "i "和重复索引 "j"，需要用等级4的对称张量来双重收缩等级2的对称张量，以及用等级2的张量来收缩等级1的张量。            
...
* 
 @endcode
* 
* 这就是 $\mathsf{\mathbf{k}}_{uu}$ 贡献。它包括一个材料贡献，和一个几何应力贡献，只沿着局部矩阵对角线添加。
* 
...
* 使用该正交点的面的法线，我们指定参考配置中的牵引力。对于这个问题，在参考构型中施加一个定义的压力。假设施加的牵引力的方向不随领域的变形而变化。牵引力的定义是使用第一个Piola-Kirchhoff应力，即 $\mathbf{t} = \mathbf{P}\mathbf{N} = [p_0 \mathbf{I}]
 \mathbf{N} = p_0 \mathbf{N}$ 我们使用时间变量来线性提升压力负荷。              
...
* @code
 ...
 @endcode
* 
* 因为我们(a)处理的是牛顿迭代方法，(b)使用位移的增量公式，以及(c)将约束条件应用于增量位移场，所以对位移更新的任何非同质约束只应在第2次迭代时指定。由于该迭代后约束条件将得到完全满足，因此不需要做后续的贡献。
* 
...
* 
 ...
 @endcode
* 
* 在牛顿第2次迭代时，我们希望应用代表位移增量边界条件的全套非均质和均质约束。由于一般情况下，每个时间步长的约束条件可能是不同的，我们需要清除约束矩阵并完全重建它。一个例子是，如果一个表面正在加速，在这种情况下，每个时间步长之间的位移变化是不恒定的。
* 
...
* 由于所有的Dirichlet约束在牛顿第2次迭代后完全得到满足，我们要确保不对这些条目做进一步的修改。这意味着我们要将所有非均质的Dirichlet约束转换成均质的。        
...
* 
 @endcode
* 
* <a name="Solidassemble_sc"></a> <h4>Solid::assemble_sc</h4> 解决整个块系统有点问题，因为对 $\mathsf{\mathbf{K}}_{ \widetilde{J} \widetilde{J}}$ 块没有贡献，使其不可逆转（当使用迭代求解器时）。由于压力和扩张变量DOFs是不连续的，我们可以将它们浓缩成一个较小的仅有位移的系统，然后我们将求解并随后进行后处理以检索压力和扩张的解决方案。
* 

* 
* 静态凝结过程可以在全局层面上进行，但我们需要其中一个块的逆。然而，由于压力和扩张变量是不连续的，静态凝结（SC）操作也可以在每个单元的基础上进行，我们可以通过反转局部块产生块对角线 $\mathsf{\mathbf{K}}_{\widetilde{p}\widetilde{J}}$ 块的逆。我们可以再次使用TBB来做这件事，因为每个操作将是相互独立的。  
...
* 我们现在提取与当前单元相关的自由度对全局刚度矩阵的贡献。  $\widetilde{p}$ 和 $\widetilde{J}$ 插值的不连续性质意味着它们在全局水平上没有局部贡献的耦合。而 $\mathbf{u}$ 道夫则不是这种情况。 换句话说， $\mathsf{\mathbf{k}}_{\widetilde{J} \widetilde{p}}$ 、 $\mathsf{\mathbf{k}}_{\widetilde{p} \widetilde{p}}$ 和 $\mathsf{\mathbf{k}}_{\widetilde{J} \widetilde{p}}$ ，当从全局刚度矩阵中提取时是元素贡献。 但 $\mathsf{\mathbf{k}}_{uu}$ 则不是这种情况。    
* 注意：小写的符号用于表示元素刚度矩阵。
* 

* 
* 目前，与当前元素相关的道夫矩阵（有点松散地表示为 $\mathsf{\mathbf{k}}$ ）是这样的。
...
* 与 $\mathsf{\mathbf{k}}_{\textrm{con}} = \bigl[
 ...
 \mathsf{\mathbf{k}}_{\widetilde{p}\widetilde{J}}^{-1}
 $  。    
* 在这一点上，我们需要注意的是，全局数据已经存在于 $\mathsf{\mathbf{K}}_{uu}$ 、 $\mathsf{\mathbf{K}}_{\widetilde{p} \widetilde{J}}$ 和 $\mathsf{\mathbf{K}}_{\widetilde{J} \widetilde{p}}$ 子块中。 因此，如果我们要修改它们，我们必须考虑到已经存在的数据（也就是说，如果需要的话，简单地添加到它或删除它）。 由于copy_local_to_global操作是一个 "+="操作，我们需要考虑到这一点     
...
* 
* -  $ {\mathsf{\mathbf{k}}}_{\textrm{store}}$  : 由于我们无法访问 $\mathsf{\mathbf{k}}_{uu}$ ，但我们知道它的贡献被添加到全局 $\mathsf{\mathbf{K}}_{uu}$ 矩阵，我们只想添加元素明智的静态凝结 $\overline{\overline{\mathsf{\mathbf{k}}}}$  。
* 
...
* -  $\mathsf{\mathbf{k}}^{-1}_{\widetilde{p} \widetilde{J}}$  : 同样， $\mathsf{\mathbf{k}}_{\widetilde{p}
 \widetilde{J}}$ 也存在于子块中。由于复制操作是一个+=操作，我们需要减去现有的 $\mathsf{\mathbf{k}}_{\widetilde{p} \widetilde{J}}$ 子矩阵，此外还需要 "添加 "我们希望用其替换的子矩阵。
* 
...
* 
* -  $\mathsf{\mathbf{k}}^{-1}_{\widetilde{J} \widetilde{p}}$  : 由于全局矩阵是对称的，这个区块与上面的相同，我们可以简单地用 $\mathsf{\mathbf{k}}^{-1}_{\widetilde{p} \widetilde{J}}$ 来代替这个区块。    
...
* 为了得到 $\mathsf{\mathbf{k}}_{\widetilde{p}
 \widetilde{J}}$ 的逆，我们直接将其反转。 这个操作相对便宜，因为 $\mathsf{\mathbf{k}}_{\widetilde{p} \widetilde{J}}$ 是块状对角线的。
* 
...
* 
 @endcode
* 
* <a name="Solidsolve_linear_system"></a> <h4>Solid::solve_linear_system</h4> 我们现在拥有所有必要的组件，可以使用两种可能的方法之一来解决线性化系统。第一种是在元素层面上进行静态凝结，这需要对切线矩阵和RHS向量进行一些改动。另外，也可以通过在全局层面上进行凝结来解决全块系统。下面我们将实现这两种方法。
* 
...
* 
 ...
 @endcode
* 
* 首先，这里是使用切线矩阵的（永久）增量的方法。对于下面的内容，请回忆一下
...
* 
* 在顶部，我们分配了两个临时向量以帮助进行静态凝结，还有一些变量用来存储线性求解器的迭代次数和（希望是收敛的）残差。
* 
...
* 在这个函数的第一步，我们求解增量位移  $d\mathbf{u}$  。 为此，我们进行静态浓缩，使 $\mathsf{\mathbf{K}}_{\textrm{con}}
 = \bigl[ \mathsf{\mathbf{K}}_{uu} +
 \overline{\overline{\mathsf{\mathbf{K}}}}~ \bigr]$ 和把 $\mathsf{\mathbf{K}}^{-1}_{\widetilde{p} \widetilde{J}}$ 放在原来的 $\mathsf{\mathbf{K}}_{\widetilde{p} \widetilde{J}}$ 块中。也就是说，我们做出 $\mathsf{\mathbf{K}}_{\textrm{store}}$  。
* 
...
* 
 @endcode
* 
* 我们默认选择了SSOR预处理程序，因为它似乎为这个问题在单线程机器上提供了最快的求解器收敛特性。 然而，对于不同的问题规模，这可能不是真的。
* 
...
* 
 ...
 @endcode
* 
* 在局部水平上对扩张场和压力场进行人工浓缩，以及随后的后处理，需要花费相当多的精力来实现。简而言之，我们必须产生逆矩阵 $\mathsf{\mathbf{K}}_{\widetilde{p}\widetilde{J}}^{-1}$ ，并将其永久写入全局切线矩阵中。然后我们对 $\mathsf{\mathbf{K}}_{uu}$ 进行永久修改，产生 $\mathsf{\mathbf{K}}_{\textrm{con}}$ 。这涉及到对切线矩阵的局部子块的提取和操作。在对位移进行求解后，对扩张和压力进行求解所需的各个矩阵-向量操作被仔细地执行。将这些众多的步骤与使用LinearOperator类提供的功能进行的更简单、更透明的实现形成对比。
* 
...
* 
 @endcode
* 
* 我们接下来为切线矩阵子块定义一些线性算子 我们将利用系统的对称性，所以并不是所有的块都需要。
* 
...
* 
 @endcode
* 
* 然后我们构造一个LinearOperator，表示（方形块） $\mathsf{\mathbf{K}}_{\widetilde{J}\widetilde{p}}$ 的逆。由于它是对角线的（或者，当使用高阶分解时，几乎是对角线的），所以雅可比预处理器是合适的。
* 
...
* 现在我们可以构建 $\mathsf{\mathbf{K}}_{\widetilde{J}\widetilde{p}}^{-1}$ 的转置和一个线性算子，它代表了浓缩运算 $\overline{\mathsf{\mathbf{K}}}$ 和 $\overline{\overline{\mathsf{\mathbf{K}}}}$ 以及最后的增强矩阵 $\mathsf{\mathbf{K}}_{\textrm{con}}$  。请注意，schur_complement()算子在这里也能派上用场，但为了清楚起见，也为了展示线性求解方案的表述和实现之间的相似性，我们将手动执行这些操作。
* 
...
* 
 @endcode
* 
* 最后，我们定义一个增强刚度矩阵的逆运算，即  $\mathsf{\mathbf{K}}_{\textrm{con}}^{-1}$  。请注意，增强刚度矩阵的预处理与我们使用静态凝结时的情况不同。在这种情况下，预处理程序是基于未修改的 $\mathsf{\mathbf{K}}_{uu}$ ，而在第一种方法中，我们实际上修改了这个子块的条目。然而，由于 $\mathsf{\mathbf{K}}_{\textrm{con}}$ 和 $\mathsf{\mathbf{K}}_{uu}$ 在同一空间操作，它对这个问题仍然足够。
* 
...
* 
 @endcode
* 
* 现在我们可以解决位移场的问题了。我们可以对线性运算进行嵌套，结果立即被写入牛顿更新向量中。很明显，这个实现紧密地模仿了介绍中所说的推导。
* 
...
* 因为我们要处理的是一个大的变形问题，如果能在一个位移网格上显示结果就更好了  与DataOut类相连的MappingQEulerian类提供了一个接口，通过这个接口可以实现这一点，而不需要我们自己物理性地移动Triangulation对象中的网格点。 我们首先需要将解决方案复制到一个临时矢量，然后创建欧拉映射。我们还向DataOut对象指定了多项式的程度，以便在使用高阶多项式时产生更精细的输出数据集。
* 
...
* <a name="Mainfunction"></a> <h3>Main function</h3> 最后，我们提供了主要的驱动函数，看起来与其他教程没有什么不同。
* 
...
* 首先，我们提出了一系列3维结果与文献（见Reese et al (2000)）的比较，以证明该程序按预期工作。
* 我们首先比较了 $Q_1-DGPM_0-DGPM_0$ 和 $Q_2-DGPM_1-DGPM_1$ 方案的网格细化收敛性，如下图所示。 结果与文献中的结果一致。低阶公式通常高估了低水平细化的位移，而高阶插值方案则低估了位移，但程度较轻。这个基准，以及其他一系列未在此显示的基准，使我们相信代码在正常工作。
...
* 
* 使用Timer类，我们可以分辨出代码中哪些部分需要最高的计算费用。对于一个有大量自由度的案例（即高度精细化），Timer的典型输出如下。 由于超过93%的时间花在线性求解器上，很明显，对于大型三维问题，可能有必要投资一个更好的求解器。
* 
...
* 然后我们用ParaView来显示两种情况的结果。第一种是最粗的网格和最低阶插值方法。 第二种是使用 $Q_2-DGPM_1-DGPM_1$ 公式的精细网格。位移的垂直分量、压力 $\widetilde{p}$ 和扩张 $\widetilde{J}$ 场显示如下。
* 

* 对于第一种情况，很明显，粗略的空间离散化加上大的位移导致了低质量的解决方案（加载比为 $p/p_0=80$ ）。另外，元素之间的压力差非常大。元素上的恒定压力场意味着大的压力梯度没有被捕获。 在这种离散性较差的情况下，也不会出现锁定现象。 扩张解场和压力场明显相关，正的扩张表示正压区域，负的表示压缩区域。正如介绍中所讨论的，压缩压力有一个负号，而扩张压力有一个正号。
* 
...
* 结合空间细化和高阶插值方案可以得到高质量的解决方案。三层网格细化加上 $Q_2-DGPM_1-DGPM_1$  我们现在可以观察到施加牵引力的实际范围，最大的力施加在表面的中心点，造成最大的压缩。 在压缩和膨胀区域之间有明显的区别和过渡，压力场的线性近似允许在子元素尺度上对压力进行精细的可视化处理，但是应该注意的是，压力场仍然是不连续的，可以在连续网格上进行平滑处理以达到后处理目的。
* 
...
* 这个简短的结果分析表明，三场公式能够有效地规避高度不可压缩介质的体积锁定。混合公式能够准确地模拟近似不可压缩块在压缩状态下的位移。命令行输出表明，在泊松比为0.4999时，极端压缩下的体积变化小于0.01%。
* 就运行时间而言， $Q_2-DGPM_1-DGPM_1$ 公式往往比 $Q_1-DGPM_0-DGPM_0$ 公式的计算费用更高。 高阶方法计算时间的增加可能是由于高阶元素所需的带宽增加了。如前所述，使用更好的求解器和预处理器可以减轻使用高阶公式的费用。 据观察，对于给定的问题，与单线程的SSOR预处理程序相比，使用多线程的Jacobi预处理程序可以减少72%的计算运行时间（在最坏的情况下是具有大量自由度的高阶公式）。
* 
...
* 最后，下面展示了两个不同层次的网格细化的2维问题的位移解决方案的结果。很明显，由于2维模拟的额外约束，所产生的位移场，虽然在质量上相似，但与3维情况不同。
* 
...
* 这项工作有一些明显的扩展。
* 
* - 首先，可以在自由能函数中加入一个额外的约束条件，以便在材料中强制执行高度的不可压缩性。一个额外的拉格朗日乘数将被引入，但这可以最容易地使用增强的拉格朗日乘数原理来处理。这在  <em>  Simo和Taylor (1991)  </em>  中得到了证明。
* 
