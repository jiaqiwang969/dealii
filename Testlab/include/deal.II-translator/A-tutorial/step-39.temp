...
* This tutorial depends on   step-12  b.
* @htmlonly
...
* 
*  Additionally to computing the error in the energy norm, we use the capability of the mesh worker to compute two functionals at the same time and compute the <i>L<sup>2</sup></i>-error in the same loop. Obviously, this one does not have any jump terms and only appears in the integration on cells.
* 
...
* @code
 ...
 @endcode
* 
*  Now, we throw everything into a   MeshWorker::loop(),   which here traverses all active cells of the mesh, computes cell and face matrices and assembles them into the global matrix. We use the variable <tt>dof_handler</tt> here in order to use the global numbering of degrees of freedom.
* 
...
* 
 ...
 @endcode
* 
*  Here is the other difference to the previous function: we run over all cells, not only the active ones. And we use functions ending on   <code>_mg</code>   since we need the degrees of freedom on each level, not the global numbering.
* 
...
* 
*  Since we compute the error in the energy and the <i>L<sup>2</sup></i>-norm, respectively, our block vector needs two blocks here.
* 
...
*   <img src="https://www.dealii.org/images/steps/developer/step-39-convergence.svg" alt="">  Using the perl script <tt>postprocess.pl</tt>, we extract relevantdata into <tt>output.dat</tt>, which can be used to plot graphs with<tt>gnuplot</tt>. The graph above for instance was produced using the gnuplotscript <tt>plot_errors.gpl</tt> via
* @code
