...
*   <li>   Fixing the mean value over the domain to zero or any other value. This  is allowed on the continuous level, since   $H^1(\Omega)\subset L^1(\Omega)$    by Sobolev's inequality, and thus also on the discrete level since we  there only consider subsets of   $H^1$  .
...
@f]This, luckily, is exactly the form of constraints for which theAffineConstraints class was designed. Note that we have used thisclass in several previous examples for the representation of hanging nodesconstraints, which also have this form: there, the middle vertex shall havethe mean of the values of the adjacent vertices. In general, theAffineConstraints class is designed to handle affine constraintsof the form@f[
  CU = b
@f]where   $C$   denotes a matrix,   $b$   denotes a vector, and   $U$   the vector of nodalvalues. In this case, since   $C$   represents one homogeneous constraint,   $b$   isthe zero vector.
* In this example, the mean value along the boundary allows just such arepresentation, with   $C$   being a matrix with just one row (i.e. there is onlyone constraint). In the implementation, we will create an AffineConstraintsobject, add one constraint (i.e. add another row to the matrix) referring to thefirst boundary node   $i_0$  , and insert the weights with which all the other nodescontribute, which in this example happens to be just   $-1$  .
...
* 
 @endcode
* 
*  Construct such an object, by initializing the variables. Here, we use linear finite elements (the argument to the   <code>fe</code>   variable denotes the polynomial degree), and mappings of given order. Print to screen what we are about to do.
* 
...
* 
 @endcode
* 
*  The first task is to set up the variables for this problem. This includes generating a valid   <code>DoFHandler</code>   object, as well as the sparsity patterns for the matrix, and the object representing the constraints that the mean value of the degrees of freedom on the boundary be zero.
* 
...
* 
 @endcode
* 
*  The next task is to construct the object representing the constraint that the mean value of the degrees of freedom on the boundary shall be zero. For this, we first want a list of those nodes that are actually at the boundary. The   <code>DoFTools</code>   namespace has a function that returns an IndexSet object that contains the indices of all those degrees of freedom that are at the boundary.     
...
* 
 @endcode
* 
*  Then generate a constraints object with just this one constraint. First clear all previous content (which might reside there from the previous computation on a once coarser grid), then add this one line constraining the   <code>first_boundary_dof</code>   to the sum of other boundary DoFs each with weight
* 
...
* 
 @endcode
* 
*  Finally, once we have the full pattern, we can initialize an object of type   <code>SparsityPattern</code>   from it and in turn initialize the matrix with it. Note that this is actually necessary, since the DynamicSparsityPattern is so inefficient compared to the   <code>SparsityPattern</code>   class due to the more flexible data structures it has to use, that we can impossibly base the sparse matrix class on it, but rather need an object of type   <code>SparsityPattern</code>  , which we generate by copying from the intermediate object.     
*   As a further sidenote, you will notice that we do not explicitly have to   <code>compress</code>   the sparsity pattern here. This, of course, is due to the fact that the   <code>copy_from</code>   function generates a compressed object right from the start, to which you cannot add new entries anymore. The   <code>compress</code>   call is therefore implicit in the   <code>copy_from</code>   call.
* 
...
*   The following are the two most commonly used ones: creation of the Laplace matrix and creation of a right hand side vector from body or boundary forces. They take the mapping object, the   <code>DoFHandler</code>   object representing the degrees of freedom and the finite element in use, a quadrature formula to be used, and the output object. The function that creates a right hand side vector also has to take a function object describing the (continuous) right hand side function.     
...
*   Two remarks are in order, though: First, these functions are used in a lot of contexts. Maybe you want to create a Laplace or mass matrix for a vector values finite element; or you want to use the default Q1 mapping; or you want to assembled the matrix with a coefficient in the Laplace operator. For this reason, there are quite a large number of variants of these functions in the   <code>MatrixCreator</code>   and   <code>MatrixTools</code>   namespaces. Whenever you need a slightly different version of these functions than the ones called above, it is certainly worthwhile to take a look at the documentation and to check whether something fits your needs.     
...
* 
-
 1$  , and the order of the boundary approximation using polynomials of degree   $p$   is   $p+1$  , we know that   $2r \geq p$  . Since r has to be an integer and (as mentioned above) has to be at least   $2$  , this makes up for the formula above computing   <code>gauss_degree</code>  .     
...
* @code
 ...
 @endcode
* 
*  For assembling the right hand side, we had to use two different vector objects, and later add them together. The reason we had to do so is that the   <code>VectorTools::create_right_hand_side</code>   and   <code>VectorTools::create_boundary_right_hand_side</code>   functions first clear the output vector, rather than adding up their results to previous contents. This can reasonably be called a design flaw in the library made in its infancy, but unfortunately things are as they are for some time now and it is difficult to change such things that silently break existing code, so we have to live with that.
* 
