...
* 该方程的基本问题是它需要解的四次方。在我们在 step-3 、 step-4 和其他几个教程中处理的拉普拉斯方程的情况下，一个人乘以一个测试函数，进行积分，通过部分积分，最后在测试函数和试验函数上都只有一个导数。
*
...
*
*--换句话说，如果没有重入角的话。  这听起来是一个相当随意的条件，但考虑到以下两个事实，它是有意义的。原始双调方程的解必须满足  $u\in H^2(\Omega)$  。另一方面，上面的混合系统重述表明， $u$ 和 $v$ 都满足 $u,v\in H^1(\Omega)$ ，因为这两个变量只解决一个泊松方程。换句话说，如果我们想保证混合问题的解 $u$ 也是原来的偏谐方程的解，那么我们需要能够以某种方式保证 $-\Delta u=v$ 的解实际上比只是 $H^1(\Omega)$ 更平稳。这一点可以作如下论证。对于凸域，<a href="https://en.wikipedia.org/wiki/Elliptic_operator#Elliptic_regularity_theorem">"elliptic
  regularity"</a>意味着，如果右侧 $v\in H^s$ ，那么 $u\in H^{s+2}$ 如果域是凸的并且边界足够光滑。(如果域的边界足够光滑，也可以保证这一点。
*
...
*在20世纪90年代和21世纪初，这一难题的解决方案随着非连续加尔金法的出现而到来。与使用<i>discontinuous</i>形状函数处理拉普拉斯方程一样，我们可以使用<i>continuous</i>（而不是 $C^1$ 连续）形状函数的方案，通过惩罚不连续的大小来获得每个形状函数上有一个导数的方程的方案，从而获得每个形状函数上有两个导数的方案。与拉普拉斯方程的内部惩罚（IP）方法类似，这个用于双调方程的方案通常被称为 $C^0$ IP（或C0IP）方法，因为它使用 $C^0$ （连续但不可连续微分）形状函数与内部惩罚公式。
*
...
* 如前所述，该方法依赖于使用 $C^0$ Lagrange有限元，其中 $C^1$ 的连续性要求被放宽并被内部惩罚技术所取代。为了推导这个方法，我们考虑一个 $C^0$ 形状函数 $v_h$ ，它在 $\partial\Omega$ 上消失。我们引入符号 $ \mathbb{F} $ 作为 $\mathbb{T}$ 的所有面的集合， $ \mathbb{F}^b $ 作为边界面的集合，而 $ \mathbb{F}^i $ 作为内部面的集合，以便在下面进一步使用。的高阶导数在每个界面 $e\in \mathbb{F}$ 上有两个值（由两个单元 $K_{+},K_{-} \in \mathbb{T}$ 共享），我们通过在 $e$ 上定义以下单值函数来应对这一不连续性。
* @f{align*}{
...
* 在多边形域上，双调方程的弱解 $u$ 存在于 $H^{2 +\alpha}(\Omega)$ 中，其中 $\alpha \in(1/2, 2]$ 是由 $\Omega$ 的角的内角决定的。例如，只要 $\Omega$ 是凸的， $\alpha=1$ ； $\alpha$ 可能小于1，如果域有重心角，但如果所有内角之一接近 $\pi$ ， $\alpha$ 就接近于 $1$  。
* 现在假设 $C^0$ 的IP解 $u_h$ 被 $C^0$ 的形状函数以多项式程度 $p \ge 2$ 所近似。那么，上述的解算就会产生下面所讨论的收敛率。
*

* <b>Convergence in the $C^0$ IP-norm</b>
* 理想情况下，我们希望在 "能量准则" $\|D^2(u-u_h)\|$ 中测量收敛性。然而，这并不可行，因为同样，离散解 $u_h$ 并没有两个（弱）导数。相反，我们可以定义一个离散( $C^0$  IP)半规范，"等同于 "能量规范，如下所示。
* @f{align*}{
 |u_h|_{h}^2 :=
...
@f}
*产生的结果。从理论上看，这个准则的收敛率当然不会比 $|\cdot|_h$ 的收敛率高<i>worse</i>，因为它只包含必要条件的一个子集，但至少可以想象到它会更好。还有一种情况是，即使程序中存在一个错误，我们也能得到最佳收敛率，而这个错误只会在 $|\cdot|_h$ 中出现的额外条款中显示出次优收敛率。但是，人们可能希望，如果我们在破碎规范和下面讨论的规范中得到最优速率，那么这个程序确实是正确的。结果部分将证明我们在所有所述的规范中都得到了最优率。
*

* <b>Convergence in the $L_2$-norm</b>
* 在  $L_2$  -norm中的最佳收敛率是  $\mathcal{O}(h^{p+1})$  提供  $p \ge 3$  。更多细节可以在  @cite Engel2002  的定理4.6中找到。
...
* 鉴于我们在最好的情况下期望 $\mathcal{O}(h^{p-1})$ 的规范等同于 $H^2$ 的半规范，而 $\mathcal{O}(h^{p+1})$ 的规范等同于 $L_2$ 的半规范，我们可能会问在 $H^1$ 的半规范中会发生什么，这与其他两个规范是中间的。一个合理的猜测是，我们应该期待 $\mathcal{O}(h^{p})$ 。可能在某个地方有一篇论文证明了这一点，但是我们也验证了这个猜想在实验中是真实的。
*
...
* 我们注意到， $C^0$ IP方法对具有其他边界条件的双谐波方程的推导
*
* - 例如，对于第一组边界条件，即 $u(\mathbf x) =
g(\mathbf x)$ 和 $\Delta u(\mathbf x)= h(\mathbf x)$ 对 $\partial\Omega$ 的边界条件
*
...
* 剩下的最后一步要描述的是这个程序的求解内容。一如既往，三角函数既是一个好的选择，也是一个坏的选择，因为它不在任何我们可以寻求解决方案的多项式空间中，同时又比实数解决方案通常更平滑（在这里，它在 $C^\infty$ 中，而实数解决方案通常只在 $H^3$ 左右的凸多边形域中，如果域不是凸的，则在 $H^2$ 和 $H^3$ 之间）。但是，由于我们没有办法用相对简单的公式来描述现实问题的解决方案，我们只是用下面的方法，在域 $\Omega$ 的单位平方上。
* @f{align*}{
  u = \sin(\pi x) \sin(\pi y).
@f}
* 因此，我们需要选择以下条件作为边界条件。
...
* 
 ...
 @endcode
*
* 在下面的命名空间中，让我们定义精确的解决方案，我们将与数值计算的解决方案进行比较。它的形式是 $u(x,y) = \sin(\pi x) \sin(\pi y)$ （只实现了2d的情况），该命名空间还包含一个对应于产生该解的右手边的类。
*
...
* 但这还没有解决我们的问题，即当我们在所有单元格和它们的所有面中循环时，必须跟踪我们已经访问过哪些面。为了使这个过程更简单，我们使用了 MeshWorker::mesh_loop() 函数，它为这个任务提供了一个简单的接口：基于WorkStream命名空间文档中概述的想法， MeshWorker::mesh_loop() 需要三个函数对单元、内部面和边界面进行工作。这些函数对中间结果的从头开始工作，然后将其计算结果复制到复制数据对象中，由复制器函数将其复制到全局矩阵和右侧对象中。   
...
* @f{align*}{
 ...
 @f}
*到右边的向量。     
* 我们使用与 step-22 的装配相同的技术来加速该函数。我们不在最里面的循环中调用`fe_values.shape_hessian(i, qpoint)`，而是创建一个变量`hessian_i`，在循环中对`i`进行一次评估，在循环中对`j`重新使用如此评估的值。为了对称，我们对变量`hessian_j`也做了同样的处理，尽管它确实只用了一次，而且我们可以在计算两个项之间标量乘积的指令中留下对`fe_values.shape_hessian(j,qpoint)`的调用。
*
...
* 
 ...
 @endcode
*
* 下一个构建模块是在网格的每个内部面组装惩罚项。正如在 MeshWorker::mesh_loop(), 的文档中所描述的，这个函数收到的参数表示一个单元和它的相邻单元，以及（对于这两个单元中的每一个）我们需要整合的面（和可能的子面）。同样，我们也得到了一个从头开始的对象，以及一个用于放置结果的副本对象。     
...
* 
 @endcode
*
* 第二部分涉及到确定惩罚参数应该是什么。通过观察双线性形式中各种项的单位，很明显，惩罚必须具有 $\frac{\gamma}{h_K}$ 的形式（即，超过长度尺度的一个），但如何选择无维数 $\gamma$ 并不是先验的。从拉普拉斯方程的不连续Galerkin理论来看，人们可能猜想正确的选择是 $\gamma=p(p+1)$ 是正确的选择，其中 $p$ 是所用有限元的多项式程度。我们将在本方案的结果部分更详细地讨论这个选择。       
...
* 
 ...
 @endcode
*
* 积极的是，因为我们现在只处理与面相邻的一个单元（因为我们在边界上），惩罚因子 $\gamma$ 的计算大大简化了。
*
...
* 
 @endcode
*
* 第三块是术语的组合。由于这些条款包含了矩阵的条款和右手边的条款，所以现在稍微有点麻烦了。前者与上面所说的内部面完全相同，如果我们只是适当地定义了跳跃和平均（这就是FEInterfaceValues类所做的）。后者需要我们评估边界条件 $j(\mathbf x)$ ，在当前情况下（我们知道确切的解决方案），我们从 $j(\mathbf x) = \frac{\partial u(\mathbf x)}{\partial
...
* 到此为止，节目基本结束。其余的函数并不太有趣或新颖。第一个函数只是用一个直接求解器来求解线性系统（也见 step-29  ）。
*
...
* 现在也计算一下 $H^2$ 半正态误差的近似值。实际的 $H^2$ 半规范要求我们对解 $u_h$ 的二阶导数进行积分，但是考虑到我们使用的拉格朗日形状函数， $u_h$ 在单元间的界面上当然有结点，因此二阶导数在界面上是单数。因此，我们实际上只对单元的内部进行积分，而忽略了界面的贡献。这并不是*个与问题的能量准则等价的准则，但仍然给了我们一个关于误差收敛速度的概念。     
...
* 我们用介绍中讨论的右手边和边界值运行程序。我们用 $Q_2$ 、 $Q_3$ 和 $Q_4$ 元素来测试这个设置，我们可以通过`main()`函数中的`fe_degree`变量来改变。通过网格精简， $L_2$ 的收敛率、 $H^1$ 的近似值收敛率和 $H^2$ 的近似值收敛率对于 $Q_2$ 应该是2、2、1左右（如简介中讨论的 $L_2$ 的规范次优）；对于 $Q_3$ 是4、3、2；而对于 $Q_4$ 是5、4、3。
* 从文献中，并不清楚惩罚参数 $\gamma$ 应该是什么。例如， @cite Brenner2009 指出它需要大于1，并选择了 $\gamma=5$ 。FEniCS/Dolphin教程选择了它为 $\gamma=8$ ，见https://fenicsproject.org/docs/dolfin/1.6.0/python/demo/documented/biharmonic/python/documentation.html。   @cite Wells2007 使用的 $\gamma$ 值大于Kirchhoff板的元素所属的边数（见第4.2节）。这表明也许 $\gamma = 1$ 、 $2$ 太小了；另一方面， $p(p+1)$ 的值也是合理的，其中 $p$ 是多项式的度。最后一个选择是人们期望通过比较拉普拉斯方程的非连续Galerkin公式来实现的(例如，见 step-39 和 step-74 中的讨论)。但是我们应该检查一下 $\gamma$ 的值是多少，我们将在下面进行；改变 $\gamma$ 在`assemble_system()`定义的两个`face_worker`和`boundary_worker`函数中很容易。
*
...
</table>  我们可以看到， $L_2$  收敛率约为4， $H^1$  -seminorm收敛率约为3,而 $H^2$  -seminorm收敛率约为2.当然，这与我们的理论预期相符。
*
...
</table>  我们可以看到 $L_2$ 规范收敛率约为5， $H^1$  -seminorm收敛率约为4， $H^2$  -seminorm收敛率约为3。在最细的网格上， $L_2$  规范收敛率远小于我们的理论预期，因为线性求解器因舍入而成为限制因素。当然，在这种情况下， $L_2$ 误差也已经非常小了。
*
...
* 为了与上面的结果进行比较，现在让我们也考虑一下我们只是选择 $\gamma=1$ 的情况。
*  <table align="center" class="doxtable">
  <tr>
   <th>Number of refinements </th><th>  $\|u-u_h^\circ\|_{L_2}$ </th><th>  Conv. rates  </th><th>  $|u-u_h|_{H^1}$ </th><th> Conv. rates </th><th> $|u-u_h|_{H^2}$ </th><th> Conv. rates </th>
  </tr>
...
</table>  虽然 $L_2$ 规范的收敛率或多或少符合理论预期，但 $H^1$  -seminorm和 $H^2$  -seminorm似乎并没有像预期那样收敛。鉴于 $\gamma=1$ 对于 $Q_2$ 元素来说已经太小了，如果用 $Q_3$ 元素重复实验，结果更加令人失望，这可能并不令人惊讶。我们又一次只得到了2、1、0的收敛结果。
*
...
* 由于 $\gamma=1$ 显然太小，人们可能猜想 $\gamma=2$ 实际上可能效果更好。下面是在这种情况下得到的结果。
*  <table align="center" class="doxtable">
  <tr>
   <th>Number of refinements </th><th>  $\|u-u_h^\circ\|_{L_2}$ </th><th>  Conv. rates  </th><th>  $|u-u_h|_{H^1}$ </th><th> Conv. rates </th><th> $|u-u_h|_{H^2}$ </th><th> Conv. rates </th>
  </tr>
...
* 我们应该使用哪种 "合理 "的惩罚参数的结论是： $\gamma=p(p+1)$ 产生了预期的结果。因此，这就是目前编写的代码所使用的。
*
...
* 类似于实施中涉及的 "夹持 "边界条件，我们将推导出 $C^0$ 简支板的IP有限元方案。
* @f{align*}{
    \Delta^2 u(\mathbf x) &= f(\mathbf x)
    \qquad \qquad &&\forall \mathbf x \in \Omega,
    u(\mathbf x) &= g(\mathbf x) \qquad \qquad
