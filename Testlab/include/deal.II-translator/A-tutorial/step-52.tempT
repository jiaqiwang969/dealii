...
@f}。
*由适当的边界条件增强。这里， $v$ 是中子的速度（为简单起见，我们假设它等于1，这可以通过简单地缩放时间变量来实现）， $D$ 是扩散系数， $\Sigma_a$ 是吸收截面， $S$ 是一个源。因为我们只对时间依赖性感兴趣，我们假设 $D$ 和 $\Sigma_a$ 是恒定的。
* 由于本程序只打算演示如何使用先进的时隙算法，我们将只寻找相对简单的问题的解决方案。具体来说，我们要在一个正方形域 $[0,b]\times[0,b]$ 上寻找解，其形式为@f{eqnarray*}
...
* 我们施加了以下边界条件： $x=0$ 和 $x=b$ 的同质Dirichlet条件， $y=0$ 和 $y=b$ 的同质Neumann条件。我们选择源项，以便相应的解实际上是上述形式：@f{eqnarray*}
...
@f}的变量的节点系数向量 $U(t)$ ，这一点就很容易看出。
*与空间形状函数 $\varphi_j(x)$ ，然后将形式为@f{eqnarray*}
  \frac{\partial u(x,t)}{\partial t} = q(t,u(x,t))
@f}的方程乘以测试函数。
*用测试函数，对 $\Omega$ 进行积分，代入 $u\rightarrow u_h$ 并将测试函数限制在上面的 $\varphi_i(x)$ ，那么这个空间离散化方程的形式为@f{eqnarray*}
M\frac{dU}{dt} = f(t,U),
@f} 。
...
@f}为
* 参见 step-24 和 step-26 以了解我们是如何到达这里的。由于当前问题所选择的边界条件，边界项是不必要的。为了使用Runge-Kutta方法，我们将其改写如下：@f{eqnarray*}
f(y) =
* 
-{\cal D}y
* 
- {\cal A}y + {\cal S}.
@f} 。
* 在代码中，我们将需要能够评估这个函数 $f(U)$ 及其导数，@f{eqnarray*}
