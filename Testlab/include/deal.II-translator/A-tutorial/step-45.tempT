...
GridTools::collect_periodic_faces(dof_handler,
                                  ...
                                  matched_pairs);
@endcode
* 将产生周期性约束，从而使 $u(0,y)=u(1,y)$ 对所有 $y\in[0,1]$ 。
...
* 由此产生的 @p matched_pairs 可以用在 DoFTools::make_periodicity_constraints 中，用于用周期性约束填充AffineConstraintsobject。
* @code
...
@f} 。
* 边界 $\Gamma_1$ 定义为 $\Gamma_1 \dealcoloneq \{x\in \partial\Omega: \|x\|\in\{0.5,1\}\}$ 。对于边界的其余部分，我们将使用周期性边界条件，即
* @f{align*}
  u_x(0,\nu)&=-u_y(\nu,0)&\nu&\in[0,1]\\
  u_y(0,\nu)&=u_x(\nu,0)&\nu&\in[0,1].
@f}
