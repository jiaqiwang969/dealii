 ...
 * name="Aboutthetutorial"></a><h3> About the tutorial </h3>
 ...
 * tutorial programs </h3>   在关于deal.II和计算科学的<a
 * href="http://www.math.colostate.edu/~bangerth/videos.html">Wolfgang
 * Bangerth's video
 * lectures</a>中也讨论和演示了这个和其他几个教程程序。特别是，你可以看到他运行这个和其他程序的步骤，你会对可以用来处理deal.II的工具有更好的了解。特别是，第2和第4讲概述了deal.II和任何有限元代码的构建块。
 ...
 *
 * 让我们回到 step-1
 * ，即当前的程序。在这第一个例子中，我们实际上并没有做很多事情，但展示了两个技术：生成三角形对象的语法是什么，以及在所有单元格上进行简单循环的一些要素。我们创建了两个网格，一个是有规律的细化的正方形（不是很令人兴奋，但对于某些问题来说是一个常见的起始网格），还有一个是几何学上的尝试：弧形域，它是向内侧边缘细化的。通过这些，你将了解到每一个有限元程序都必须有的三样东西。Triangulation类型的对象；调用GridGenerator函数来生成网格；以及涉及迭代器的所有单元的循环（迭代器是指针的泛化，在C++标准库中经常使用；在deal.II的背景下，
 * @ref Iterators 模块谈到了它们）。
 ...
 * <code>dealii</code>
 * 中，以确保它们不会与你可能想和deal.II一起使用的其他库的符号相冲突。我们可以在使用这些函数和类时，在每个名字前加上
 * <code>dealii::</code>
 ...
 * href=
 * "http://en.wikipedia.org/wiki/Iterator#C.2B.2B">wikipedia</a>）。通常情况下，C++中的任何容器类型都会返回一个迭代器，指向集合的开始，方法称为`begin'，并返回一个迭代器，指向集合结束后的1，方法称为`end`。我们可以用操作符`++it`来增加迭代器`it`，用`*it`来解除引用以获得底层数据，并通过比较`it
 ...
 *
 * </div>
 * 在一个三角形的所有活动单元上循环，在这个循环的初始化器中，我们使用了`auto`关键字作为迭代器`it`的类型。`auto`关键字意味着被声明的对象的类型将从上下文中推断出来。当实际的类型名称很长，甚至可能是多余的时候，这个关键字很有用。如果你不确定类型是什么，想查一下结果支持什么操作，你可以去看方法的文档
 ...
 *
 * @note
 * 关于deal.II中使用的迭代器类的更多信息，见 @ref Iterators
 * ，关于基于范围的for循环和`auto`关键字的更多信息，见
 * @ref CPP11 。
 ...
 * </h3> 。   <a name="Differentadaptiverefinementstrategies"></a><h4>
 * Different adaptive refinement strategies </h4>
 * 这个程序显然没有太多的功能，但特别是
 * <code>second_grid</code>
 * 函数有很多地方你可以发挥它。例如，你可以修改我们决定提炼哪些单元格的标准。一个例子是把条件改成这样。
 * @code
 *    for (auto &cell: triangulation.active_cell_iterators())
 *      if (cell->center()[1] > 0)
 *        cell->set_refine_flag ();
