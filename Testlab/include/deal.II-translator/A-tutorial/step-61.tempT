...
* 本教程程序介绍了泊松方程的 "弱Galerkin "有限元方法的实现。从某种意义上说，考虑这种方法的动机与 step-51 中的动机相同：我们想考虑不连续的形状函数，但需要解决这样一个事实：与通常的连续Galerkin方法相比，所产生的问题有更多的自由度（因为，举例来说，每个顶点携带的自由度与相邻的单元一样多）。我们还必须处理这样一个事实：与连续Galerkin方法不同，<i>every</i>一个单元上的自由度与它的每个面的邻居单元上的所有自由度相耦合。因此，从 "传统 "非连续Galerkin方法得到的矩阵既大又相对密集。
*  step-51 中的混合非连续Galerkin方法（HDG）和本教程中的弱Galerkin（WG）方法都是通过引入额外的自由度来解决耦合问题的，这些自由度的形状函数只存在于单元间的面（即。在网格的 "骨架 "上），因此将相邻单元上的自由度相互 "隔离"：单元自由度只与同一单元上的其他单元自由度以及面自由度耦合，而不与相邻单元上的自由度耦合。
...
* 弱加勒金有限元方法（WGFEMs）使用离散的弱函数来近似标量未知数，使用离散的弱梯度来近似经典梯度。该方法最初是由王俊平和叶修在论文中提出的<a href="https://doi.org/10.1016/j.cam.2012.10.003">
...
J. Comput. Appl. Math., 103-115, 2013</a> 。与连续Galerkin方法相比，弱Galerkin方法满足重要的物理特性，即局部质量守恒和体法通量连续。
*
...
@f}
* 其中 $\Omega \subset \mathbb{R}^n (n=2,3)$ 为有界域。在流体流经多孔介质的情况下， $p$ 为压力， $\mathbf{K}$ 为渗透率张量， $f$ 为源项， $p_D, u_N$ 我们可以引入一个通量， $\mathbf{u} =
* 
-\mathbf{K} \nabla p$ ，对应于达西速度（以我们在 step-20 中的方式），这个变量在下面的考虑中很重要。
* 在这个程序中，我们将考虑一个测试案例，即在单位平方域上的确切压力为 $p = \sin \left( \pi x\right)\sin\left(\pi y \right)$ ，具有同质的Dirichelet边界条件和 $\mathbf{K}$ 的身份矩阵，然后我们将计算压力、速度和流量的 $L_2$ 误差。
*

*<a name="WeakGalerkinscheme"></a><h3> Weak Galerkin scheme </h3>


* 上述泊松方程有一个解 $p$ ，需要满足问题的弱式表述，@f{equation*}
\mathcal{A}\left(p,q \right) = \mathcal{F} \left(q \right),
@f} 。
*对于所有测试函数 $q$ ，其中@f{equation*}
...
@f}。
* 这里，我们以双线性形式进行了部分积分，我们正在评估 $p,p$ 在内部的梯度和 $q$ 在域的边界的值。所有这些都有很好的定义，因为我们假定解在 $H^1$ 中，对它来说，取梯度和评估边界值是有效的操作。
* 弱Galerkin方法的想法是用一个<i>discontinuous function</i> $p_h$ 来近似精确的 $p$ 解。这个函数可能只在单元格之间的界面上是连续的，由于我们也想沿着界面评价这个函数，我们不仅要规定它在单元格内部应该有什么值，而且还要规定它在界面上的值。我们的做法是， $p_h$ 实际上是一个元组， $p_h=(p^\circ,p^\partial)$ ，尽管它实际上只是一个单一的函数，它要么等于 $p^\circ(x)$ ，要么等于 $p^\partial(x)$ ，取决于它是在细胞内部还是在细胞界面上的某一点 $x$ 被评价。
* 然后我们想把这个近似值简单地贴到上面的双线型中。这适用于我们必须在边界上评估测试函数 $q_h$ 的情况（我们只需取其界面部分 $q_h^\partial$ ），但我们必须小心处理梯度，因为它只在细胞内部定义。因此，泊松方程的弱Galerkin方案由@f{equation*}
\mathcal{A}_h\left(p_h,q \right) = \mathcal{F} \left(q_h \right),
@f}定义。
*对于所有离散测试函数 $q_h$ ，其中@f{equation*}
...
@f}
* 关键的一点是，在这里，我们用<i>discrete weak gradient</i>算子 $\nabla_{w,d} p_h$ 代替了梯度 $\nabla p_h$ ，这对于我们特殊定义的近似值 $p_h$ 是有意义的。
* 问题是该算子如何工作。为此，让我们首先说说我们是如何看待压力的离散近似 $p_h$ 的。如上所述，"函数" $p_h$ 实际上由两部分组成：单元内部的值 $p_h^\circ$ 和界面上的 $p_h^\partial$ 。我们必须为这两部分定义离散的（有限维）函数空间；在这个程序中，我们将使用FE_DGQ来表示 $p_h^\circ$ 作为细胞内部的空间（在每个细胞上定义，但一般是沿界面不连续的），并使用FE_FaceQ来表示 $p_h^\partial$ 作为界面上的空间。
* 然后让我们只考虑一个单元（因为上面的积分都是逐个单元定义的，而且弱离散梯度是逐个单元定义的）。从本质上讲，我们可以认为 $\nabla_{w,d} p_h$ 是定义在 $K$ 上的某个函数，它接近于梯度；特别是，如果 $p_h|_K$ 是一个可微函数的限制（对 $K$ 的内部和边界
*
* 这将使它在内部和边界之间是连续的），那么 $\nabla_{w,d} p_h$ 将只是准确的梯度 $\nabla p_h$  。但是，由于 $p_h|_K$ 在 $K$ 的内部和边界之间不是连续的，我们需要一个更一般的定义；此外，我们不能处理任意函数，因此要求 $\nabla_{w,d} p_h$ 也是在一个有限元空间（由于梯度是一个矢量，必须是矢量值，而且由于弱梯度是在每个单元上单独定义的，因此在单元之间也是不连续的）。
* 这样做的方法是以下列方式定义这个弱梯度算子 $\nabla_{w,d}|_K :
DGQ_k(K) \times DGQ_r(\partial K) \rightarrow RT_s(K)$ （其中 $RT_s(K)$ 是单元格 $K$ 上的矢量值Raviart-Thomas空间，阶数为 $s$ ）：@f{equation*}{
...
@f}
*对于所有测试函数 $\mathbf v_h \in RT_s(K)$ .这在本质上是简单地应用了逐部积分公式。换句话说，对于一个给定的 $p_h=(p^\circ_h,p^\partial_h)$ ，我们需要把 $\nabla_{w,d} p_h|_K$ 看作是度数为 $s$ 的Raviart-Thomas函数，对于这个函数，左手边和右手边在所有测试函数中是相等的。
* 那么，需要说明的一个关键点是以下几点。通常的梯度 $\nabla$ 是一个局部*算子，它仅仅根据一个函数在某一点及其（无限小）邻域的值来计算导数，而弱离散梯度 $\nabla_{w,d}$ 却没有这个特性。它取决于它在整个单元上所应用的函数值，包括单元的边界。然而，两者都是线性算子，从上面 $\nabla_{w,d}$ 的定义可以看出，这将允许我们在下面的讨论中通过矩阵来表示 $\nabla_{w,d}$ 。
*  @note  可能值得指出的是，虽然弱离散梯度是Raviart-Thomas空间 $RT_s(K)$ 中每个单元 $K$ 上的一个元素，但它在单元之间是不连续的。另一方面，定义在整个网格上并由FE_RaviartThomas类实现的Raviart-Thomas空间 $RT_s=RT_s({\mathbb T})$ 代表了在单元间界面上具有连续法线分量的函数。这意味着<i>globally</i>,  $\nabla_{w,d} p_h$ 不在 $RT_s$ 中，尽管它在 $K$ 中的每个单元上。  相反，它是在一个 "破碎的 "拉维亚特-托马斯空间中，下面我们将用符号 $DGRT_s$ 来表示。 这里的术语 "破碎 "是指 "把东西打碎 "的过程，而不是指 "没有功能 "的同义词）。因此，人们可能会（理所当然地）争辩说，弱加尔金文献中使用的符号有点误导性，但这往往取决于使用某种符号的环境
*
* - 在目前的背景下，对Raviart-Thomas空间或元素的提及总是被理解为对 "破碎 "空间的提及。
*  @note  deal.II恰好有一个这种破碎的Raviart-Thomas空间的实现。FE_DGRT类。因此，在本教程中，我们将简单地一直使用FE_DGRT类，尽管在所有那些我们必须计算单元格本地矩阵和向量的地方，它没有任何区别。
*

*<a name="Representingtheweakgradient"></a><h3> Representing the weak gradient </h3>


* 由于 $p_h$ 是有限元空间的一个元素，我们可以像往常一样在一个基中展开它，也就是说，我们可以写成@f{equation*}{
  p_h(\mathbf x) = \sum_j P_j \varphi_j(\mathbf x).
@f} 。
* 这里，由于 $p_h$ 有两个分量（内部和界面分量），基函数 $\varphi_j(\mathbf x)$ 也必须如此，我们可以写成 $\varphi_j = (\varphi_j^\circ,\varphi_j^\partial)$ 。如果你遵循了 step-8 、 step-20 和 @ref vector_valued "向量值问题文档模块 "中的描述，就不会奇怪，对于 $j$ 的某些值， $\varphi_j^\circ$ 将为零，而对于 $j$ 的其他值， $\varphi_j^\partial$ 则为零
*
*--也就是说，形状函数将是一种或另一种的。然而，这在这里并不重要。重要的是，我们需要思考如何表示 $\nabla_{w,d} \varphi_j$ ，因为当我们想实现双线性形式@f{equation*}
\mathcal{A}_h\left(p_h,q_h\right)
  = \sum_{K \in \mathbb{T}}
    \int_K \mathbf{K} \nabla_{w,d} p_h \cdot \nabla_{w,d} q_h \;\mathrm{d}x,
@f}时，这显然是会出现在问题中的。
*
* 关键是 $\nabla_{w,d} \varphi_j$ 是已知的 "破碎 "Raviart-Thomas空间 $DGRT_s$ 的成员。这意味着我们可以（在每个单元 $K$ 上分别表示）@f{equation*}
\nabla_{w,d} \varphi_j|_K
  = \sum_k C_{jk}^K \mathbf v_k|_K
@f} 。
*其中的函数 $\mathbf v_k \in DGRT_s$ ，以及 $C^K$ 是一个维度的矩阵
* @f{align*}{
...
@f}
* 弱离散梯度可以被表示为一个矩阵，这并不奇怪：它是一个从一个有限维空间到另一个有限维空间的线性算子。如果选择这两个空间的基数，那么<i>every linear operator</i>当然可以写成一个矩阵，将与算子的域空间的基数有关的扩展系数向量映射到与图像空间的基数有关的扩展系数向量）。)
* 使用这个扩展，我们可以很容易地使用上面的弱离散梯度的定义来定义矩阵的内容：@f{equation*}{
...
@f} 。
* 对于所有测试函数 $\mathbf v_i \in DGRT_s$  。
* 这显然导致了一个线性系统的形式@f{equation*}{
...
@f}也是如此。
* (在最后一步中，我们假设指数 $i,j,k$ 只涵盖那些在单元 $K$ 上活动的自由度，从而确保空间 $RT_s(K)$ 上的质量矩阵是可逆的。)等价地，利用矩阵 $M$ 的对称性，我们有@f{equation*}{
...
* 在解释了弱离散梯度是如何定义的之后，我们现在可以回到如何组装有关方程的线性系统的问题上了。具体来说，利用上面显示的双线性形式 ${\cal A}_h$ 的定义，我们就需要计算对全局矩阵@f{equation*}{
  A^K_{ij} = \int_K \left({\mathbf K} \nabla_{w,d} \varphi_i\right) \cdot \nabla_{w,d} \varphi_j.
@f}的局部贡献元素。
* 如上所述，我们可以根据每个单元的Raviart-Thomas基来展开 $\nabla_{w,d} \varphi_i$ ，同样，对于 $\nabla_{w,d} \varphi_j$ ：@f{equation*}{
  A^K_{ij} = \int_K
    \left(
      {\mathbf K}
...
* 通过重新排列和，可以得到以下表达式：@f{equation*}{
  A^K_{ij} =
    ...
    \mathbf v_l|_K.
@f} 。
* 因此，如果我们有每个单元 $K$ 的矩阵 $C^K$ ，那么我们可以很容易地计算单元 $K$ 对矩阵 $A$ 的贡献 $A^K$ 如下：@f{equation*}{
  A^K_{ij} =
    \sum_k \sum_l C_{ik}^K C_{jl}^K
    H^K_{kl}
    =
...
* 这里，@f{equation*}{
  ...
  \int_K
    ...
    \mathbf v_l|_K,
@f} 是质量矩阵。
* 这实际上只是使用Raviart-Thomas基数的单元 $K$ 的质量矩阵，并通过渗透性张量 $\mathbf K$ 加权。这里的推导表明，弱Galerkin方法实际上只需要我们计算每个单元 $C^K$ 和 $H^K$ 的矩阵，然后再计算 $A^K = C^K H^K (C^K)^T$ ，这很容易计算。下面要显示的代码正是这样做的。
* 在计算出单元格 $A^K$ 对全局矩阵的贡献后，我们要做的就是将这些局部贡献 "分配 "到全局矩阵中。如何做到这一点，首先显示在  step-3  和  step-4  中。在当前的程序中，这将通过调用 AffineConstraints::distribute_local_to_global(). 来促进。
* 一个线性系统当然也需要一个右手边。在这里，除了我们只需要使用单元内部的部分 $\varphi_i^\circ$ 来计算形状函数 $\varphi_i$ 之外，没有任何与计算右手边相关的困难。
*

*<a name="PostprocessingandiLsub2subierrors"></a><h3> Post-processing and <i>L<sub>2</sub></i><i>L<sub>2</sub></i>-errors </h3>


* 前面几节的讨论给了我们一个线性系统，我们可以解决数值压力 $p_h$ 。我们可以用它来计算变量 $\mathbf u =
* 
-{\mathbf K}\nabla p$ 的近似值，如果这就是我们要解决的模型，它对应于介质在多孔介质中的流动速度。这种步骤
*
* - 从离散问题的解决方案中计算一个派生量
*
* - 通常被称为 "后处理"。
...
@f}
* 其中 $C^K$ 是上面的扩展矩阵， $\mathbf{v}_j$ 是单元上 $RT$ 空间的基础函数。
* 不幸的是， $\mathbf{K} \mathbf{v}_j$ 可能不在 $RT$ 空间中（当然，除非 $\mathbf K$ 是常数乘以身份矩阵）。因此，为了在有限元程序中表示它，我们需要把它投影回我们可以处理的有限维空间。在这里，我们将使用 $L_2$ 投影来把它投射回（破碎的） $RT$ 空间。
...
\left( \mathbf{Kv}_j,\mathbf{v}_k \right)_K.$  所以，而不是上面的公式，单元格 $K$ 上的数值速度反而变成@f{equation*}
...
* 在下面的实现中，元素 $
   d_{jk}
$ 的矩阵被称为 <code>cell_matrix_D</code> ，而元素 $
      \left( \mathbf{Kv}_j,\mathbf{v}_k \right)
...
* 使用这个通过 "后处理 "得到的速度，我们可以通过以下公式定义压力、速度和通量的 $L_2$  误差。
* @f{align*}{
\|p-p_h^\circ\|^2
...
@f}
*其中 $| K |$ 是元素的面积， $\gamma$ 是元素的面， $\mathbf{n}$ 是每个面的单位法向量。最后一项是测量网格单元之间界面上速度向量的法向分量的精度。缩放因子 $|K|/|\gamma|$ 的选择是为了随着网格大小的变化而缩小界面集合的长度（或面积）的差异。
* 上面的第一个错误很容易用 VectorTools::integrate_difference. 计算出来，其他的需要更多的工作，在下面的代码中实现。
*

* <a name="CommProg"></a> <h1> The commented program</h1>。
* <a name="Includefiles"></a> <h3>Include files</h3> 本程序基于 step-7 、 step-20 和 step-51 ，所以以下大部分头文件都很熟悉。我们需要以下文件，其中只有导入FE_DGRaviartThomas类的文件（即`deal.II/fe/fe_dg_vector.h`）是真正的新文件；FE_DGRaviartThomas实现了介绍中讨论的 "破碎 "Raviart-Thomas空间。
*
...
* 这是本程序的主要类别。我们将使用弱加勒金（WG）方法求解内部和面上的数值压力，并计算压力的 $L_2$ 误差。在后处理步骤中，我们还将计算速度和通量的 $L_2$  误差。   
...
*
* 接下来，我们定义系数矩阵 $\mathbf{K}$ （这里是身份矩阵），迪里希特边界条件，右手边 $f = 2\pi^2 \sin(\pi x) \sin(\pi y)$ ，以及对应于这些选择的 $K$ 和 $f$ 的精确解，即 $p =
 \sin(\pi x) \sin(\pi y)$  。
*
...
*
* 在这个构造函数中，我们为矢量值函数创建一个有限元空间，这里将包括用于内部和界面压力的函数，  $p^\circ$  和  $p^\partial$  。
*
...
*
* 这个函数比较有趣。正如介绍中所详述的，线性系统的装配要求我们评估形状函数的弱梯度，它是Raviart-Thomas空间中的一个元素。因此，我们需要定义一个Raviart-Thomas有限元对象，并有FEValues对象在正交点评估它。然后我们需要计算每个单元 $K$ 上的矩阵 $C^K$ ，为此我们需要在介绍中提到的矩阵 $M^K$ 和 $G^K$ 。   
* 可能不明显的一点是，在以前所有的教程程序中，我们总是用DoFHandler的单元格迭代器来调用 FEValues::reinit() 。这样就可以调用诸如 FEValuesBase::get_function_values() 这样的函数，在单元格的正交点上提取有限元函数的值（用DoF值的向量表示）。为了使这一操作发挥作用，我们需要知道哪些向量元素对应于给定单元上的自由度
*
*--也就是说，正是DoFHandler类所提供的那种信息和操作。   
* 我们可以为 "破碎的 "Raviart-Thomas空间创建一个DoFHandler对象（使用FE_DGRT类），但我们在这里真的不想这样做。至少在当前函数中，我们不需要任何与这个破碎空间相关的全局定义的自由度，而只需要引用当前单元上的这种空间的形状函数。因此，我们利用这样一个事实，即人们也可以用单元格迭代器来调用 FEValues::reinit() 的Triangulation对象（而不是DoFHandler对象）。在这种情况下，FEValues当然只能为我们提供只引用单元格的信息，而不是这些单元格上列举的自由度。所以我们不能使用 FEValuesBase::get_function_values(), ，但我们可以使用 FEValues::shape_value() 来获得当前单元格上正交点的形状函数值。下面我们要利用的就是这种功能。下面给我们提供Raviart-Thomas函数信息的变量是`fe_values_rt`（和相应的`fe_face_values_rt`）对象。   
...
* @code
 ...
 @endcode
*
* 接下来我们通过使用 FullMatrix::gauss_jordan(). 对这个矩阵进行求逆，它将被用来计算后面的系数矩阵 $C^K$ 。值得回顾的是，`cell_matrix_M`实际上包含了 $M^K$ 的逆*。
 的逆值。
*
...
* 
 @endcode
*
*  @p cell_matrix_C 是 $G^K$ 的转置与质量矩阵的逆向之间的矩阵乘积（该逆向存储在 @p cell_matrix_M): ）。
*
...
* 最后我们可以计算出局部矩阵  $A^K$  。  元素  $A^K_{ij}$  由  $\int_{E} \sum_{k,l} C_{ik} C_{jl}
 (\mathbf{K} \mathbf{v}_k) \cdot \mathbf{v}_l
 \mathrm{d}x$  得到。我们在上一步已经计算了系数 $C$ ，因此在适当地重新排列循环后得到以下结果。
*
...
*
* 在这个函数中，从先前计算的压力解中计算出速度场。速度的定义为 $\mathbf{u}_h = \mathbf{Q}_h \left(
* 


* 
* 
-\mathbf{K}\nabla_{w,d}p_h \right)$ ，这需要我们计算许多与系统矩阵的组装相同的项。还有我们需要组装的矩阵 $E^K,D^K$ （见介绍），但它们实际上只是遵循同一种模式。   
* 在这里计算与我们在`assemble_system()`函数中已经完成的相同的矩阵，当然是在CPU时间上浪费了。同样，我们从那里复制了一些代码到这个函数中，这通常也是一个糟糕的主意。一个更好的实现可能会提供一个函数来封装这些重复的代码。我们也可以考虑使用计算效率和内存效率之间的经典权衡，在装配过程中每个单元只计算一次 $C^K$ 矩阵，把它们存储在某个地方，然后在这里重新使用它们。例如， step-51 就是这样做的，`assemble_system()`函数需要一个参数来决定是否重新计算本地矩阵，还有一个类似的方法
*
...
* 
 @endcode
*
* 这个 <code>cell_matrix_E</code> 的组成部分是 $(\mathbf{K} \mathbf{w}, \mathbf{w})$ 的积分。   <code>cell_matrix_M</code> 是格拉姆矩阵。
*
...
* 
 @endcode
*
* 然后，我们还需要再次计算矩阵 $C$ ，用来评估弱离散梯度。这与组装系统矩阵时使用的代码完全相同，所以我们只需从那里复制它。
*
...
*
* 这一部分是计算压力的 $L_2$ 误差。  我们定义一个向量，用来保存每个单元上的误差的规范。接下来，我们使用 VectorTool::integrate_difference() 来计算每个单元上的 $L_2$ 准则的误差。然而，我们实际上只关心解向量的内部分量的误差（我们甚至不能评估正交点的界面压力，因为这些都位于单元格的内部），因此必须使用一个权重函数，确保解变量的界面分量被忽略。这是通过使用ComponentSelectFunction来实现的，其参数表明我们要选择哪个分量（零分量，即内部压力）以及总共有多少分量（两个）。
*
...
*
* 在这个函数中，我们评估每个单元的速度的 $L_2$ 误差，以及面的通量的 $L_2$ 误差。该函数依赖于之前计算过的`compute_postprocessed_velocity()`函数，该函数根据之前计算过的压力解来计算速度场。   
...
* 
 @endcode
*
* 为了重建通量，我们需要单元和面的大小。由于通量是按面计算的，我们要在每个单元的所有四个面上进行循环。为了计算面的速度，我们从之前计算的`darcy_velocity`中提取正交点的值。然后，我们计算法线方向的速度平方误差。最后，我们通过对面和单元面积的适当缩放来计算单元上的 $L_2$ 通量误差，并将其加入全局误差。
*
...
* 
 @endcode
*
* 将所有单元和面的误差相加后，我们进行平方根计算，得到速度和流量的 $L_2$ 误差。这些我们都输出到屏幕上。
*
...
* 我们有两组结果要输出：内部解和骨架解。我们使用 <code>DataOut</code> 来实现内部结果的可视化。骨架结果的图形输出是通过使用DataOutFaces类完成的。   
*在这两个输出文件中，内部和面的变量都被储存起来。对于界面输出，输出文件只是包含了内部压力对面的插值，但是因为没有确定从两个相邻的单元中得到的是哪一个内部压力变量，所以在界面输出文件中最好是忽略内部压力。相反，对于单元格内部输出文件，当然不可能显示任何界面压力 $p^\partial$ ，因为这些压力只适用于界面，而不是单元格内部。因此，你会看到它们被显示为一个无效的值（比如一个无穷大）。   
* 对于细胞内部的输出，我们还想输出速度变量。这有点棘手，因为它生活在同一个网格上，但使用不同的DoFHandler对象（压力变量生活在`dof_handler`对象上，达西速度生活在`dof_handler_dgrt`对象上）。幸运的是， DataOut::add_data_vector() 函数有一些变化，允许指定一个矢量对应的DoFHandler，因此我们可以在同一个文件中对两个DoFHandler对象的数据进行可视化。
*
...
* 我们运行的程序的右手边将产生解决方案 $p = \sin(\pi x) \sin(\pi y)$ ，并且在域 $\Omega = (0,1)^2$ 中具有同质的迪里希边界条件。此外，我们选择微分算子 $\mathbf{K}$ 中的系数矩阵作为身份矩阵。我们使用 $\mbox{WG}(Q_0,Q_0;RT_{[0]})$ 、 $\mbox{WG}(Q_1,Q_1;RT_{[1]})$ 和 $\mbox{WG}(Q_2,Q_2;RT_{[2]})$ 元素组合来测试这个设置，可以通过使用`main()`WGDarcyEquation`对象的适当构造器参数来选择。然后我们将可视化单元内部和面上的压力值。我们希望看到压力最大值在1左右，最小值在0左右。通过网格细化，压力、速度和通量的收敛率对于 $\mbox{WG}(Q_0,Q_0;RT_{[0]})$ 应该是1左右，对于 $\mbox{WG}(Q_1,Q_1;RT_{[1]})$ 是2，对于 $\mbox{WG}(Q_2,Q_2;RT_{[2]})$ 是3。
*
...
* 下面的数字显示了使用 $\mbox{WG}(Q_0,Q_0;RT_{[0]})$ 元素的内部压力和表面压力。网格分别被细化了2倍（顶部）和4倍（底部）。(这个数字可以在`make_grid()`函数中调整)。当网格较粗时，我们可以看到面压 $p^\partial$ 与相邻两个单元的内压 $p^\circ$ 的值之间的整齐排列。
*  <table align="center">
  <tr>
...
* 从图中我们可以看出，随着网格的细化，最大和最小的压力值正在接近我们预期的数值。由于网格是矩形网格，每个方向的单元数是偶数，所以我们有对称的解决方案。从右边的三维图中，我们可以看到在 $\mbox{WG}(Q_0,Q_0;RT_{[0]})$ 上，压力在单元的内部是一个常数，正如预期的那样。
...
* 我们可以看到， $\mbox{WG}(Q_0,Q_0;RT_{[0]})$ 的收敛率大约为1.当然，这与我们的理论预期相符。
*
...
* 我们可以使用下一个更高的多项式度数来重复上面的实验：下面的数字是使用 $\mbox{WG}(Q_1,Q_1;RT_{[1]})$ 实现的内部压力和表面压力。网格被细化了4次。  与之前使用 $\mbox{WG}(Q_0,Q_0;RT_{[0]})$ 的数字相比，在每个单元上，解决方案不再是恒定的，因为我们现在使用双线性多项式来做近似。因此，在一个内部有4个压力值，每个面有2个压力值。
*  <table align="center">
  <tr>
...
* 与 $\mbox{WG}(Q_0,Q_0;RT_{[0]})$ 组合的相应图像相比，现在的解决方案明显更准确，特别是在界面上非常接近于连续，我们不再能够区分界面压力 $p^\partial$ 和相邻单元的内部压力 $p^\circ$ 。
* <a name="Convergencetableforik1i"></a><h4>Convergence table for <i>k=1</i><i>k=1</i></h4> 。
*

* 以下是我们从使用 $\mbox{WG}(Q_1,Q_1;RT_{[1]})$ 元素组合得到的压力、速度和通量的收敛率。
*  <table align="center" class="doxtable">
  <tr>
   <th>number of refinements </th><th>  $\|p-p_h^\circ\|$  </th><th>  $\|\mathbf{u}-\mathbf{u}_h\|$ </th><th> $\|(\mathbf{u}-\mathbf{u}_h) \cdot \mathbf{n}\|$ </th>
  </tr>
...
* 让我们再提高一个多项式度数。以下是使用 $WG(Q_2,Q_2;RT_{[2]})$ 实现的内部压力和表面压力，网格大小为 $h = 1/32$ （即5个全局网格细化步骤）。在程序中，当生成图形输出时，我们使用`data_out_face.build_patches(fe.degree)`（见文献 DataOut::build_patches()), ，这里意味着我们将每个2d单元内部分为4个子单元，以提供一个更好的二次多项式的可视化。 <table align="center">
  <tr>
...
* 和以前一样，我们可以用 $\mbox{WG}(Q_2,Q_2;RT_{[2]})$ 的组合来生成压力、速度和流量的 $L_2$ 误差的收敛数据。
*  <table align="center" class="doxtable">
  <tr>
   <th>number of refinements </th><th>  $\|p-p_h^\circ\|$  </th><th>  $\|\mathbf{u}-\mathbf{u}_h\|$ </th><th> $\|(\mathbf{u}-\mathbf{u}_h) \cdot \mathbf{n}\|$ </th>
  </tr>
...
* 再一次， $\mbox{WG}(Q_2,Q_2;RT_{[2]})$ 的收敛率符合预期，数值在3左右。
*
