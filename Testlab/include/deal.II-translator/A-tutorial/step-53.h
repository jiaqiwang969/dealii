 ...
 * 在这两种情况下，我们都需要一种方法来提供关于单个单元、其面和边的域的几何信息。这就是Manifold类开始发挥作用的地方。Manifold是一个抽象的基类，它只定义了一个接口，Triangulation和Mapping类可以通过这个接口查询域的几何信息。从概念上讲，Manifold看待世界的方式与数学分支学科几何学看待世界的方式并无二致：域本质上只是一个点的集合，它被赋予了点之间距离的概念，这样我们就可以在其他一些点的
 * "中间 "获得一个点。
 ...
 * 也是可能的，尽管要麻烦很多。我们不会在这里展示这个公式，而只是提供程序中的实现。
 *
 *  <a name="Implementation"></a><h3>Implementation</h3>
 * 在这个程序中，我们需要解决一些问题。在最大范围内，我们需要编写一个实现ChartManifold接口的类。这涉及到一个函数
 ...
 * .不幸的是，这个类并不完全适合，所以我们需要绕过它。问题来自于我们初始化这个类的方式：在其最简单的形式下，它需要一个数值流，它假定这些数值在
 * $x-y$ 平面（或者，这里是 $\phi-\theta$
 * 平面）上形成一个等距的网格。这就是它们在这里的作用，有点像：它们的顺序是纬度第一，经度第二；更尴尬的是，第一列从最大的数值开始往下数，而不是通常的其他方式。
 * 现在，虽然教程程序是为了说明如何用deal.II编码，但它们不一定要满足与生产代码相同的质量标准。在生产代码中，我们会写一个函数来读取数据，并(i)自动确定第一列和第二列的范围，(ii)自动确定每个方向的数据点的数量，(iii)无论数据排列的顺序如何，都要进行插值，如果有必要的话，在读取和提交给
 ...
 * 让我们从这里需要的包含文件开始。很明显，我们需要描述三角形的文件（
 * <code>tria.h</code>
 * ），以及允许我们创建和输出三角形的文件（
 ...
 * @endcode
 *
 * 其余的包含文件与读取地形数据有关。正如介绍中所解释的，我们将从一个文件中读取它，然后使用下面头文件中第一个声明的
 * Functions::InterpolatedUniformGridData
 * 类。因为数据很大，所以我们读取的文件是以gzip压缩数据的形式存储的，我们利用BOOST提供的一些功能，直接从gzip压缩数据中读取。
 *
 * @code
 ...
 * \theta)$
 * ，它是经度和纬度的函数。正如在介绍中所讨论的，我们在这里将使我们的生活更容易一些，不以最普遍的方式来写这个类，而只为我们在这里感兴趣的特殊目的来写：插值从一个非常具体的数据文件中获得的数据，该文件包含了我们知道其范围的世界上的一个特定地区的信息。
 ...
 * $[-6.983333, 11.98333]$
 * ，使用介绍中讨论的切换端点的技巧，以及 $[25, 35.983333]$
 ...
 * 对象的83,600个元素中的第一个来初始化它。请注意，我们在这里调用的所有成员函数都是静态的，因为(i)它们不访问类的任何成员变量，(ii)因为它们是在对象没有完全初始化的时候调用的。
 *
 * @code
 ...
 *
 * @endcode
 *  其他唯一比较有兴趣的函数是 <code>get_data()</code>
 * 函数。它返回一个临时向量，包含所有描述海拔的83,600个数据点，并从文件中读取
 * <code>topography.txt.gz</code>
 * 。因为该文件被gzip压缩了，我们不能直接通过
 * std::ifstream,
 * 类型的对象来读取它，但是在BOOST库中有一些方便的方法（见http://www.boost.org），允许我们从压缩文件中读取，而不必先在磁盘上解压。其结果是，基本上，只是另一个输入流，就所有的实际目的而言，看起来就像我们一直使用的那些输入流。
 ...
 * @endcode
 *
 * 下一个函数是Manifold基类的接口所要求的，它允许克隆AfricaGeometry类。注意，虽然该函数返回一个
 ...
 * @endcode
 *
 * 下面两个函数就定义了对应于地球WGS84参考形状的正向和反向变换。正向转换遵循介绍中的公式。反变换要复杂得多，至少不是直观的。它还存在一个问题，即它返回的角度在函数结束时我们需要将其夹回区间
 * $[0,2\pi]$ ，如果它应该从那里逃出来。
 *
 * @code
 ...
 * 2\times 1$
 * 矩形网格。我们选择这个数目的细分，因为它导致了细胞大致上像立方体，而不是在某个方向上被拉伸。
 ...
 * @endcode
 *
 * 最后一步是在最初的 $1\times 2\times 1$
 * 粗略网格之外细化网格。我们可以在全局范围内细化若干次，但由于本教程程序的目的，我们实际上只对靠近表面的情况感兴趣，我们只是对所有在边界上有一个指标为5的面的单元细化6次。在我们上面使用的
 * GridGenerator::subdivided_hyper_rectangle()
 ...
 * ，我们可以用任何可以读取VTU文件格式的常用可视化程序来进行可视化。如果只看网格本身，实际上很难看到任何不只是看起来像一块完全圆形的球体的东西（尽管如果修改程序，使其确实产生一个球体并同时观察它们，整体球体和WGS
 ...
 * Victoria</a>的巨大平坦处）。 <img
 * src="https://www.dealii.org/images/steps/developer/step-53.topozoom.png"
 * alt="">
 * 这些图片是经过三个小的修改后产生的。 <ol>   <li>  这两张图片中的第一张在顶面增加了第七个网格细化，第二张总共增加了九个。在第二张图片中，水平方向的网格大小约为1.5km，垂直方向的网格大小略低于1km。(这张图也是用一个更有分辨率的数据集制作的；但是，它太大了，不能作为教程的一部分分发)。
 ...
 * 如果你放大上图所示的网格并仔细观察，你会发现在悬空节点处，连接悬空节点的两条小边与相邻单元的大边的位置不完全相同。这可以通过使用不同的表面描述来更清楚地显示出来，在这种描述中，我们放大了垂直地形以增强效果（由Alexander
 * Grayver提供）。 <img
