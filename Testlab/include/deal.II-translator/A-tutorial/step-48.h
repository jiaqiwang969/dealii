...
* 本程序演示了如何使用MatrixFree类的基于单元的有限元算子实现，该类首次在 step-37 中介绍，用于解决非线性偏微分方程。最后，我们将使用明确的时间步进方法来解决问题，并介绍高斯-洛巴特有限元，在这种情况下非常方便，因为它们的质量矩阵可以准确地由对角线接近，因此是可逆的矩阵。这一特性的两个要素是：首先，根据Gauss-Lobatto正交规则的点分布，对拉格朗日多项式的结点进行分布。其次，正交是用同样的Gauss-Lobatto正交规则完成的。在这个公式中，只要 $i\neq j$ ，积分 $\int_K \varphi_i \varphi_j
dx\approx \sum_q \varphi_i \varphi_j \mathrm{det}(J) \big |_{x_q}$ 就变成零，因为在定义拉格朗日多项式的点中，正好有一个函数 $\varphi_j$ 是一，其他都是零。此外，拉格朗日多项式的节点的Gauss-Lobatto分布将节点向元素边界聚集。这就为高阶离散化方法提供了条件良好的多项式基础。事实上，具有等距节点的FE_Q元素的条件数随着度数的增加而呈指数级增长，这破坏了五阶以上的任何好处。因此，高斯-洛巴特点是FE_Q元素的默认分布（但在度数为1和2时，它们等同于等距点）。
*<a name="Problemstatementanddiscretization"></a><h3> Problem statement and discretization </h3>
*

* 作为一个例子，我们选择解决正弦戈登孤子方程\f{eqnarray*}
u_{tt} &=& \Delta u
* 
...
u(x,t_0) &=& u_0(x).
\f} 。
*这在 step-25 中已经介绍过。作为一种简单的显式时间积分方法，我们选择使用方程的二阶形式的跃迁方案。在这种时间步长的情况下，该方案以弱的形式读出
* \f{eqnarray*}
...
* 
- (\nabla v, (\Delta t)^2 \nabla u^n),
\f}其中<i> v</i>表示一个测试函数，索引<i>n</i>代表时间步长。
...
* 使用这个正交规则，对于<i>p</i>阶有限元，我们使用<i>(2p-1)</i>阶精确公式来评估积分。由于在计算质量矩阵时，两个<i>p</i>阶基函数的乘积在每个方向上都给出了一个具有多项式程度<i>2p</i>的函数，所以积分的计算并不精确。  然而，整体收敛性能并不受到正交误差的干扰，在具有亲和元素形状的网格上，L2误差与<i>h<sup>p+1</sup></i>成正比。但是请注意，当积分项不再是多项式时，在变形（非affine）元素形状的波浪方程上，已经有报道<i>O(h<sup>p</sup>)</i>甚至<i>O(h<sup>p-1</sup>)</i>的L2误差的次优收敛率的减序<a href="https://dx.doi.org/10.1002/num.20353">in
literature</a>。
* 除了在使用显式时间步进时我们可以避免用这种类型的元素解决线性系统外，它们还有另外两个优点。当我们使用和-因子化方法来评价有限元算子时（参见 step-37 ），我们必须在正交点评价函数。在Gauss-Lobatto元素的情况下，正交点和无限元素的节点点是重合的，这种操作是微不足道的，因为正交点的函数值是由其一维系数给出的。这样一来，与一般的高斯正交相比，有限元运算估值的算术工作大约减少了2倍。
...
* 在 <code>deal.II</code> 中处理约束的通常方法是使用AffineConstraints类，该类建立了一个稀疏矩阵，存储关于哪些自由度（DoF）被约束以及它们如何被约束的信息。这种格式使用了不必要的大量内存，因为没有那么多不同类型的约束：例如，在悬挂节点的情况下，当在每个单元上使用线性有限元时，大多数约束具有 $x_k = \frac 12 x_i + \frac 12 x_j$ 的形式，其中系数 $\frac 12$ 总是相同的，只有 $i,j,k$ 不同。虽然存储这些多余的信息在一般情况下不是问题，因为它在矩阵和右手边的装配过程中只需要一次，但它在无矩阵方法中成为一个瓶颈，因为每次我们应用算子时都要访问这些信息，而算子评估的其他部分是如此之快。因此，MatrixFree使用一个我们称之为 <code>constraint_pool</code> 的变量来收集不同约束的权重，而不是一个AffineConstraints对象。然后，只需要存储网格中每个约束的标识符而不是所有的权重。此外，约束不是在前后处理步骤中应用的，而是在我们评估有限元运算器时应用的。因此，约束信息被嵌入到变量 <code>indices_local_to_global</code> 中，该变量用于从全局矢量中提取单元信息。如果一个DoF受到约束， <code>indices_local_to_global</code> 变量包含了它所约束的DoF的全局指数。然后，我们还有一个变量 <code>constraint_indicator</code> ，在每个单元中都有被约束的DoF的局部指数，以及约束类型的标识符。幸运的是，你不会在示例程序中看到这些数据结构，因为类 <code>FEEvaluation</code> 会在没有用户交互的情况下处理这些约束。
* 在存在悬空节点的情况下，通过Gauss-Lobatto正交/节点点程序在元素层面获得的对角质量矩阵并不能直接转化为对角的全局质量矩阵，因为遵循行和列的约束也会增加非对角条目。正如<a href="https://dx.doi.org/10.4208/cicp.101214.021015a">Kormann
(2016)</a>中所解释的那样，在一个矢量上插值约束，保持质量矩阵的对角线形状，与方程一致，但误差的大小与正交误差相同。在下面的程序中，我们将简单地把质量矩阵的对角线作为一个矢量进行组合，以实现这种近似。
*

*<a name="Parallelization"></a><h3> Parallelization </h3>


* MatrixFree类有三个层次的并行化选项：分布式节点集群上的MPI并行化，由Threading Building Blocks库调度的线程并行化，以及最后通过SIMD数据类型在两个（或更多）单元的批次上工作的矢量化（有时称为跨元素或外部矢量化）。正如我们在 step-37 中所讨论的，你将通过使用特定于你的系统的指令集来获得最佳性能，例如，使用cmake变量<tt>-DCMAKE_CXX_FLAGS="-march=native"</tt>。在  step-37  中已经利用了MPI并行化。在这里，我们额外考虑了用TBB进行线程并行化。这相当简单，因为我们需要做的就是告诉MatrixFree对象的初始化，我们想通过变量 MatrixFree::AdditionalData::thread_parallel_scheme. 使用线程并行方案。 在设置过程中，建立了一个类似于 @ref workstream_paper 中描述的依赖图，这允许安排 @p local_apply 函数在单元块上的工作，而没有几个线程访问同一个向量索引。相对于WorkStream循环，一些额外的巧妙技巧也被应用，以避免<a
...
\f}，并在时间区间[-10,10]内解方程。常数被选择为 $c_1=c_1=0$ 和<i> m=0.5</i>。正如在 step-25 中提到的，在一维中<i>u</i>作为<i>t</i>的函数是正弦-戈登方程的精确解。对于更高的维度，情况并非如此。
*
...
*
*  <code>SineGordonOperation</code> 类实现了每个时间步长中需要的基于单元的操作。这个非线性操作可以在 <code>MatrixFree</code> 类的基础上直接实现，与线性操作在这个实现的有限元算子应用中的处理方式相同。我们对该类应用两个模板参数，一个是尺寸，一个是有限元的度数。这与deal.II中的其他函数不同，其中只有维度是模板参数。这对于向 @p FEEvaluation 中的内循环提供关于循环长度等的信息是必要的，这对于效率是至关重要的。另一方面，这使得将度数作为一个运行时参数来实现更具挑战性。
*
...
*
* 这个算子实现了程序的核心操作，即对正弦-戈登问题的非线性算子进行单元范围的积分。其实现是基于  step-37  中的FEEvaluation类。由于Gauss-Lobatto元素的特殊结构，某些操作变得更加简单，特别是正交点上的形状函数值的评估，这只是单元自由度值的注入。MatrixFree类在初始化时检测正交点的有限元的可能结构，然后由FEEvaluation自动用于选择最合适的数值核。
*

*
* 我们必须为时间步进例程评估的非线性函数包括当前时间 @p current 的函数值以及上一个时间步进 @p old. 的值，这两个值都在源向量集合 @p src, 中传递给运算器，它只是一个指向实际解向量的 <tt>std::vector</tt> 的指针。这种将多个源向量收集到一起的结构是必要的，因为 @p MatrixFree 中的单元格循环正好需要一个源向量和一个目的向量，即使我们碰巧使用许多向量，如本例中的两个。请注意，单元格循环接受任何有效的输入和输出类，这不仅包括向量，还包括一般的数据类型。  然而，只有在遇到收集这些向量的 LinearAlgebra::distributed::Vector<Number> 或 <tt>std::vector</tt> 时，它才会在循环的开始和结束时调用由于MPI而交换幽灵数据的函数。在单元格的循环中，我们首先要读入与本地值相关的向量中的值。  然后，我们评估当前求解向量的值和梯度以及正交点的旧向量的值。接下来，我们在正交点的循环中结合方案中的条款。最后，我们将结果与测试函数进行积分，并将结果累积到全局解向量 @p  dst。
*
...
* 我们定义了一个随时间变化的函数，作为初始值使用。通过改变起始时间可以得到不同的解。这个函数取自 step-25 ，将代表所有时间的一维分析解，但在这里只是用于设置一些感兴趣的起始解。在  step-25  中给出了可以测试这个程序的收敛性的更详细的选择。
*
...
*
* 与 step-25 中一样，这个函数在 <code>dim</code> 维度上设置了一个范围为 $[-15,15]$ 的立方体网格。我们在域的中心更多的细化网格，因为解决方案都集中在那里。我们首先细化所有中心在半径为11的单元，然后再细化一次半径为6的单元。  这种简单的临时细化可以通过在时间步进过程中使用误差估计器使网格与解相适应来完成，就像在其他例子程序中所做的那样，并使用 parallel::distributed::SolutionTransfer 来将解转移到新的网格中。
*
...
*
* 这个函数打印出解的规范，并将解的向量写到一个文件中。法线是标准的（除了我们需要在所有处理器上累积平行网格的法线，我们通过 VectorTools::compute_global_error() 函数来做），第二项类似于我们在 step-40 或 step-37 中的做法。请注意，我们可以使用与计算过程中使用的相同的向量进行输出。无矩阵框架中的向量总是提供所有本地拥有的单元的全部信息（这也是本地评估中需要的），包括这些单元上的鬼向量条目。这是 VectorTools::integrate_difference() 函数以及DataOut中唯一需要的数据。这时唯一要做的就是确保在我们从矢量中读取数据之前更新其鬼魂值，并在完成后重置鬼魂值。这是一个只存在于 LinearAlgebra::distributed::Vector 类中的特性。另一方面，带有PETSc和Trilinos的分布式向量需要被复制到包括ghost值的特殊向量（见 step-40 中的相关章节 ）。如果我们还想访问幽灵单元上的所有自由度（例如，在计算使用单元边界上的解的跳跃的误差估计器时），我们需要更多的信息并创建一个初始化了本地相关自由度的向量，就像在  step-40  中那样。还请注意，我们需要为输出分配约束条件
*
* - 它们不是在计算过程中填充的（相反，它们是在无矩阵方法中即时插值的  FEEvaluation::read_dof_values()).  。
*
...
*
* 然后我们将初始状态写入文件，并将两个起始解收集到 <tt>std::vector</tt> 的指针中，随后被 SineGordonOperation::apply() 函数所消耗。接下来，根据文件顶部指定的有限元程度，建立一个 <code> SineGordonOperation class </code> 的实例。
*
...
* 注意这个换位是如何实现的。我们只是在两个向量上调用了交换方法，只交换了一些指针，而不需要复制数据，这在显式时间步进方法中是一个相对昂贵的操作。让我们来看看发生了什么。首先，我们交换 <code>old_solution</code> with <code>old_old_solution</code> ，这意味着 <code>old_old_solution</code> 得到 <code>old_solution</code> ，这就是我们所期望的。同样，在下一步中， <code>old_solution</code> gets the content from <code>solution</code> 也是如此。在此之后， <code>solution</code> 持有 <code>old_old_solution</code> ，但在这一步中会被覆盖。
*
...
*
* 与 step-40 中一样，我们在程序开始时初始化MPI。由于我们一般会将MPI并行化与线程混合在一起，所以我们也将MPI_InitFinalize中控制线程数量的第三个参数设置为无效数字，这意味着TBB库会自动选择线程数量，通常为系统中可用的内核数量。作为一种选择，如果你想设置一个特定的线程数（例如，当需要只使用MPI时），你也可以手动设置这个数字。
*
...
* 为了证明使用MatrixFree类而不是标准的 <code>deal.II</code> 汇编例程来评估旧的时间步长的信息的好处，我们研究了一个简单的在非适应性网格上的序列运行的代码。由于许多时间花在评估正弦函数上，我们不仅显示了完整的正弦-戈登方程的数字，也显示了波浪方程（正弦-戈登方程中跳过的正弦项）的数字。我们同时使用二阶和四阶元素。结果总结在以下表格中。
...
* 在这个程序中，有几处地方可以改进，使其更加有效（除了在 step-25 中讨论的改进边界条件和物理材料）。
*  <ul>   <li>  <b>Faster evaluation of sine terms:</b> 从上面的平波方程和正弦-戈登方程的比较中可以明显看出，正弦项的评估在有限元算子应用的总时间中占主导地位。这有几个原因。首先，VectorizedArray场的deal.II正弦计算没有被矢量化（与算子应用的其他部分相反）。这可以通过将正弦计算交给一个具有矢量化正弦计算的库来解决，比如英特尔的数学内核库（MKL）。通过使用MKL中的函数 <code>vdSin</code> ，该程序在二维中使用了一半的计算时间，在三维中使用了40%的时间。另一方面，正弦计算在结构上要比其他本地操作中的加法和乘法等简单算术操作复杂得多。
*  <li>  <b>Higher order time stepping:</b> 虽然实现允许空间部分的任意顺序（通过调整有限元的程度），但时间步进方案是标准的二阶跃迁方案。由于波的传播问题的解通常是非常平滑的，所以误差很可能被时间步进部分所支配。当然，这可以通过使用较小的时间步长（在固定的空间分辨率下）来解决，但使用高阶时间步长也会更有效率。虽然对于一阶系统来说，这样做是很简单的（使用一些高阶的Runge&ndash;Kutta方案，可能与自适应时间步长选择相结合，如<a
  href="http://en.wikipedia.org/wiki/Dormand%E2%80%93Prince_method">Dormand&ndash;Prince
  method</a>），但对于二阶公式来说，这是更大的挑战。至少在有限差分界，人们通常用PDE来寻找改善时间误差的空间修正项。
*  </ul> 
*
