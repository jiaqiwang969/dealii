...
* 一个经常出现的问题是如何用交易解决涉及复值函数的问题。对于许多问题，与其直接使用复值有限元，不如将复值函数分成实部和虚部，并使用单独的标量有限元场来离散其中的每一个。基本上，这相当于把一个单一的复值方程看成是两个实值方程的系统。这个简短的例子演示了如何在deal.II中通过使用 <code>FE_system</code> 对象来堆叠两个代表实部和虚部的有限元场来实现。(相反的方法，保持所有的复值，在另一个教程程序中演示：见 step-58 。)当分成实部和虚部时，这里涉及的方程属于矢量值问题的范畴。关于这一主题的顶层概述可以在 @ref vector_valued 模块中找到。
...
* 在计算中，我们将考虑波在单位方阵中的传播，超声由换能器透镜产生，透镜的形状像圆的一个部分，中心在 $(0.5, d)$ ，半径略大于 $d$ ；这种形状应该导致声波在圆中心的聚焦。改变 $d$ 会改变透镜的 "焦点"，影响 $u$ 强度的空间分布，我们主要关心的是 $|u|=\sqrt{v^2+w^2}$ 的聚焦程度。
* 在下面的程序中，我们将使用实部和虚部分离的公式来实现复值亥姆霍兹方程。我们还将讨论如何生成一个看起来像正方形的域，并带有模拟换能器的轻微隆起（在 <code>UltrasoundProblem<dim>::make_grid()</code> 函数中），以及如何生成图形输出，不仅包含解分量 $v$ 和 $w$ ，还包括直接在输出文件中的量级 $\sqrt{v^2+w^2}$ （在 <code>UltrasoundProblem<dim>::output_results()</code> 中）。最后，我们使用ParameterHandler类来轻松读取参数，如焦距 $d$ 、波速 $c$ 、频率 $\omega$ ，以及在运行时从输入文件中读取其他一些参数，而不是在源代码中固定这些参数，每次我们想改变参数时，都必须重新编译。
*
...
* 
 @endcode
*
* 作为这个程序开始的最后一步，我们把这个程序中的所有东西都放到它的命名空间中，在它的命名空间中，让所有在deal.II命名空间中的东西都是全局可用的，不需要在所有东西前加上 <code>dealii</code><code>::</code> 。
*
...
* 因为有两个值 $v$ 和 $w$ 需要在边界规定，我们必须告诉基类，这是一个有两个分量的向量值函数， <code>vector_value</code> 函数和它的表弟 <code>vector_value_list</code> 必须返回有两个条目的向量。在我们的例子中，这个函数非常简单，它只是对实部 $v$ 返回1，对虚部 $w$ 返回0，而不管它在哪个点被评估。
*
...
* @code
 ...
 @endcode
*
* 网格和几何的参数包括应用于初始粗网格的全局细化步数和换能器镜头的焦距 $d$ 。对于细化步数，我们允许在 $[0,\infty)$ 范围内取整数，其中 Patterns::Integer 对象的第二个参数被省略，表示半开区间。  对于焦距，任何大于零的数字都可以接受。
*
...
* 
 @endcode
*
* 因为不同的输出格式可能需要不同的参数来生成输出（例如，postscript输出需要视角角度、线宽、颜色等），如果我们必须为库中支持的每一种可能的输出格式手工声明所有这些参数，那将是非常麻烦的。相反，每种输出格式都有一个 <code>FormatFlags::declare_parameters</code> 函数，它在自己的小节中声明了该格式的所有特定参数。下面调用 DataOutInterface<1>::declare_parameters 为所有可用的输出格式执行 <code>declare_parameters</code> ，这样就为每种格式创建了一个自己的小节，并为该特定输出格式声明了参数。(上面 <code>@<1@></code> 的调用中，模板参数的实际值在这里并不重要：该函数做了同样的工作，与维度无关，但碰巧是在一个依赖模板参数的类中。)  要想知道哪种输出格式有哪些参数，你可以查阅DataOutBase类的文档，或者干脆在没有参数文件的情况下运行这个程序。然后，它将创建一个文件，将所有声明的参数设置为默认值，这可以方便地作为一个起点，将参数设置为你想要的值。
*
...
*
* 这是在ParameterReader类中的主函数。  它从外部被调用，首先声明所有的参数，然后从输入文件中读取参数，文件名由调用者提供。在对这个函数的调用完成后，可以使用 <code>prm</code> 对象来检索从文件中读入的参数值。
*
...
*
* 正如介绍中提到的，我们真正追求的数量是超声波强度的空间分布，它对应于  $|u|=\sqrt{v^2+w^2}$  。现在我们可以只满足于在输出中拥有 $v$ 和 $w$ ，并使用合适的可视化或后处理工具从我们计算的解决方案中得出 $|u|$ 。然而，也有一种方法可以输出从deal.II的解决方案中得出的数据，我们在这里要利用这个机制。
*
...
*
* 在实践中，DataPostprocessor类只提供了一个接口，我们需要从它派生出我们自己的类，以便实现接口所指定的功能。在最一般的情况下，我们必须实现几个成员函数，但如果输出量是一个单一的标量，那么其中一些模板代码可以由一个更专业的类，即DataPostprocessorScalar来处理，我们可以从该类中派生出来。这就是 <code>ComputeIntensity</code> 类的作用。
*
...
* 
 @endcode
*
* 在构造函数中，我们需要用两个参数调用基类的构造函数。第一个参数表示这个类计算的单一标量在输出文件中应该用什么名字来表示。在我们的例子中，后处理程序有 $|u|$ 作为输出，所以我们使用 "Intensity"。   
...
* 
 ...
 @endcode
*
* 实际的后处理发生在下面这个函数中。它的输入是一个存储函数值的对象（这里是向量值），代表给 DataOut::add_data_vector, 的数据向量在我们产生输出的所有评估点的评估值，以及一些代表导数的张量对象（这里我们不使用，因为 $|u|$ 只是从 $v$ 和 $w$ 计算出来的）。派生量在 <code>computed_quantities</code> 向量中返回。请记住，这个函数只能使用由  <code>get_needed_update_flags</code>  指定的各自更新标志的数据。例如，我们可能不会在这里使用导数，因为我们对  <code>get_needed_update_flags</code>  的实现要求只提供函数值。
*
...
* 计算本身是很简单的。我们遍历输出向量中的每个条目，并从 $v$ 和 $w$ 的相应值中计算出 $|u|$  。我们通过创建一个复数 $u$ ，然后对结果调用 `std::abs()` 来实现。(我们可能会想调用 `std::norm()`, ，但是在一个历史的怪圈中，C++委员会决定 `std::norm()` 应该返回绝对值的<i>square</i>。
*
...
*
* 最后这里是这个程序的主类。  它的成员函数与之前的例子非常相似，特别是 step-4 ，成员变量的列表也没有任何重大的惊喜。传递给构造函数的ParameterHandler对象被存储为一个引用，以便于从该类的所有函数中访问参数。  由于我们正在使用矢量值的有限元，我们使用的FE对象是FESystem类型的。
*
...
* 
 ...
 @endcode
*
* 作为初始粗网格，我们采用一个简单的单位方格，每个方向上有5个细分。分区的数量是这样选择的：我们想指定为换能器边界的线段 $[0.4,0.6]$ 是由一个面来跨越的。然后，我们通过所有的单元格来找到换能器所在的面，事实上，这只是X轴上从0.4到0.6的一条边。这是我们希望根据圆环形边界进行细化的地方，所以我们用不同的流形指标来标记这个边缘。由于我们将在换能器上设置迪里希特边界条件，我们也要改变其边界指标。
*
...
* 
 @endcode
*
* 像往常一样，计算积分时使用普通的高斯正交规则。由于我们的双线性形式涉及到 $\Gamma_2$ 上的边界积分，我们还需要一个正交法则来计算面的积分，这些面是 $dim-1$ 维的。
*
...
* 
 ...
 @endcode
*
* 在这一点上，重要的是要记住，我们所处理的是一个有两个分量的有限元系统。由于我们构造这个FESystem的方式，即作为两个标量有限元场的笛卡尔乘积，每个形状函数只有一个非零分量（用deal.II的行话来说，它们是 @ref  GlossPrimitive "原始"）。  因此，每个形状函数可以被看作是介绍中的 $\phi$  's或 $\psi$  's之一，同样，相应的自由度也可以归属于 $\alpha$  或 $\beta$  。然而，当我们遍历当前单元上的所有自由度时，它们并不以任何特定的顺序出现，因此我们无法立即决定索引为 $i$ 和 $j$ 的自由度是属于我们解决方案的实部还是虚部。  另一方面，如果你看一下介绍中的系统矩阵的形式，这个区别是至关重要的，因为它将决定当前一对DoF的贡献将归入系统矩阵的哪个块，因此我们需要从给定的两个形状函数中计算哪个量。  幸运的是，FESystem对象可以为我们提供这些信息，即它有一个函数 FESystem::system_to_component_index, ，为每个局部的DoF索引返回一对整数，其中第一个整数表示该DoF属于系统的哪个组成部分。这对整数中的第二个整数表示该DoF在标量基有限元域中的索引，但这一信息在这里并不相关。如果你想知道更多关于这个函数和原始向量值元素背后的基本方案，可以看看 step-8 或 @ref vector_valued 模块，那里对这些主题有深入的解释。
*
...
* 如果DoF $i$ 和 $j$ 都属于同一个组件，即它们的形状函数都是 $\phi$ 's或都是 $\psi$ 's，贡献将最终出现在我们系统矩阵的一个对角线块中，由于相应的条目是由同一个公式计算的，我们不必理会它们是否真的是 $\phi$ 或 $\psi$ 形状函数。我们可以简单地通过遍历所有正交点并将其贡献相加来计算条目，其中形状函数的值和梯度由我们的FEValues对象提供。
*
...
* 
 ...
 @endcode
*
* 我们还必须增加由于边界项的贡献。为此，我们对当前单元格的所有面进行循环，首先看它是否在边界上，其次看它是否有与 $\Gamma_2$ 相关的正确边界指标，即我们有吸收边界条件的那部分边界。
*
...
*
* 在任何一种情况下，这些DoF都会对系统矩阵的非对角线块的边界积分做出贡献。为了计算积分，我们在面的所有正交点上进行循环，并将贡献量与面正交规则所提供的正交权重相加。  与对角线块上的条目不同，这里的形状函数哪一个是 $\psi$ ，哪一个是 $\phi$ ，确实很重要，因为这将决定该条目的符号。  我们通过一个简单的条件语句来说明这一点，以确定正确的符号。由于我们已经检查了DoF $i$ 和 $j$ 属于不同的组件，这里只需测试其中一个属于哪个组件即可。
*
...
*
* 在这里，我们以参数文件中指定的格式输出我们的解 $v$ 和 $w$ 以及派生量 $|u|$ 。从 $v$ 和 $w$ 导出 $|u|$ 的大部分工作已经在 <code>ComputeIntensity</code> 类的实现中完成，因此输出程序相当简单，与前面教程中的内容非常相似。
*
...
* 
 @endcode
*
* 定义我们 <code>ComputeIntensity</code> 类的对象和一个DataOut对象。
*
...
* 
 @endcode
*
* 接下来我们从ParameterHandler查询输出相关的参数。 DataOut::parse_parameters  调用充当了  <code>ParameterReader::declare_parameters</code>  中  DataOutInterface<1>::declare_parameters  调用的对应部分。它从ParameterHandler收集所有与输出格式相关的参数，并相应地设置DataOut对象的相应属性。
*
...
* 前两张图片显示了 $u$ 的实部和虚部，而最后一张显示了强度 $|u|$ 。我们可以清楚地看到，强度集中在透镜的焦点周围（0.5，0.3），焦点在 $x$ 方向相当尖锐，但在 $y$ 方向却比较模糊，这是聚焦透镜的几何形状、有限光圈和问题的波浪性质所造成的。
