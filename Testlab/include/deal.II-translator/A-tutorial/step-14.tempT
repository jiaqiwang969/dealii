...
* 我们不会太详细地讨论这些概念的推导，但会在本例程序中实现主要的想法。对于一般概念的彻底介绍，我们可以参考Becker和Rannacher的开创性工作 @cite BR95 ， @cite BR96r ，以及同一作者在Acta Numerica @cite BR01 上的概述文章；前者介绍了拉普拉斯方程一般函数输出的误差估计和适应性的概念，而后者给出了许多概念在大量其他更复杂方程上的应用实例。关于个别类型方程的应用，也可参见贝克尔 @cite Bec95 、 @cite Bec98 、坎沙特 @cite Kan96 、 @cite FK97 、苏特梅尔 @cite Sut96 的出版物。 ] ,  @cite RS97  ,  @cite RS98c  ,  @cite RS99  , Bangerth  @cite BR99b  ,  @cite Ban00w  ,  @cite BR01a  ,  @cite Ban02  , 和 Hartmann  @cite Har02  ,  @cite HH01  ,  @cite HH01b  。所有这些工作，从Becker和Rannacher的原始介绍到对特定方程的个别贡献，后来都在Bangerth和Rannacher的书中进行了总结，其中包括所有这些主题，见 @cite BR03 。
* 
...
* 在刚才列举的所有情况中，我们感兴趣的是解的函数 $J(u)$ 的评估，而不是各地的 $u$ 的值。由于精确的解决方案 $u$ 是不可用的，只有它的数字近似值 $u_h$ ，所以问计算值 $J(u_h)$ 是否在精确值 $J(u)$ 的一定范围内是明智的，也就是说，我们想约束关于这个函数的误差， $J(u)-J(u_h)$ 。
...
@f}。
* 在最后一步，注意当取 $u_h$ 的法向导数时，我们是指从单元格的这一边取的这个量的值（对于通常的拉格朗日元素，导数在边上不是连续的）。然后我们通过将 $K$ 单元的一半边缘积分与相邻单元 $K'$ 交换来重写上述公式，得到@f{eqnarray*}
  J(e)
  &=&
  \sum_K (f+\Delta u_h, z-\varphi_h)_K
* 
...
*  step-14  示例程序在很大程度上建立在 step-13  程序中已经使用的技术上。它对上面解释的双重加权残差估算器的实现是通过派生出第二个类，正确地称为 <code>DualSolver</code>, from the <code>Solver</code> 基类，并有一个类（ <code>WeightedResidual</code> ）再次连接这两个类，控制原始和双重问题的解决，然后使用两者来计算网格细化的误差指示器。
...
* 我们展示了一种可以通过简单而又非常灵活的方式来实现的方法。我们将把属于某个设置的所有东西放到一个类中，并围绕它提供一个小的C++砂浆，这样整个设置（域、系数、右手边等）就可以通过改变 <em> 中的某个 </em> 地方进行交换。
...
* 
* 正如在介绍中提到的，该程序的重要部分只是从 step-13 的例子程序中拿过来的。因此，我们只对那些新的东西进行评论。  
...
* 相反，我们必须评估这个单元上的有限元场，而且是在某一点。如你所知，在某一点上评估有限元场是通过 <code>FEValues</code> 类完成的，所以我们使用该类。问题是： <code>FEValues</code> 对象需要给定一个正交公式，然后可以计算正交点的有限元量值。在这里，我们并不想做正交，我们只是想指定一些点!               
...
* 
 ...
 @endcode
* 
* 现在是实现线性系统类中动作的函数。首先，构造函数将所有的数据元素初始化为正确的大小，并设置了一些额外的数据结构，例如由于悬空节点而产生的约束。由于设置悬空节点和找出矩阵的非零元素是独立的，所以我们以并行方式进行（如果库被配置为使用并发，至少是这样；否则，这些动作是按顺序执行的）。注意，我们只启动一个线程，并在主线程中做第二个动作。由于只生成了一个线程，所以我们在这里不使用 <code>Threads::TaskGroup</code> 类，而是直接使用创建的一个任务对象来等待这个特定任务的退出。这个方法与我们在上面 <code>Solver::assemble_linear_system()</code> 中使用的方法大致相同。    
...
* 
*  <code>PrimalSolver</code> 类除了实现 <code>output_solution</code> 函数外，也基本没有变化。我们在这个程序中保留了 <code>GlobalRefinement</code> and <code>RefinementKelly</code> 类，然后它们可以依赖这个函数的默认实现，这个函数只是输出原始解。实现双重加权误差估计的类将自行重载这个函数，以同时输出双重解。
* 
...
* 接下来用给与构造函数的值来衡量指标向量中的每个条目，在单元格中心进行评估。我们需要将结果写入对应于当前单元的向量条目中，我们可以通过使用 CellAccessor::active_cell_index(). 询问单元在所有活动单元中的索引来获得这个结果（实际上，对于我们在循环中处理的第一个单元，这个索引是0，第二个单元是1，等等，我们也可以使用一个整数计数器来跟踪这个索引；但是使用 CellAccessor::active_cell_index() 使之更加明确）。
* 
...
* 
 ...
 @endcode
* 
* <a name="Equationdata"></a> <h3>Equation data</h3> <h3>Equation data</h3>。  
...
* 这个想法是这样的（现在是时候也简单看看下面的代码了）：我们可以为边界值和右手边生成对象，只需将外层类的名字作为模板参数给一个类，我们在这里称之为 <code>Data::SetUp</code> ，然后它就为内部类创建对象。在这种情况下，为了得到所有描述弧形山脊解决方案的特征，我们将简单地生成一个 <code>Data::SetUp@<Data::CurvedRidge@></code> 的实例，而我们需要知道的关于该解决方案的一切都将是该对象的静态成员变量和函数。  
* 在这种情况下，这种方法可能看起来是多余的，但是一旦某种设定不仅有迪里希特边界值和右手函数的特征，而且还有材料属性、诺伊曼值、不同的边界描述符等，就会变得非常方便。在这种情况下， <code>SetUp</code> 类可能由十几个对象组成，而每个描述符类（如下面的 <code>CurvedRidges</code> 类）都必须提供这些对象。然后，你会很高兴，只需在一个地方改变 <code>SetUp</code> 类的模板参数，而不是在很多地方改变，就能从一组数据改变到另一组。  
* 有了这个不同测试案例的框架，我们就快完成了，但还有一件事：到现在，我们可以通过改变一个模板参数，静态地选择要选择的数据集。为了能够动态地做到这一点，也就是说，在运行时，我们需要一个基类。我们以明显的方式提供这个基类，见下文，用虚拟抽象函数。这迫使我们引入第二个模板参数 <code>dim</code> ，我们需要这个基类（这可以用一些模板魔法来避免，但我们省略），但这就是全部。  
...
* 
 ...
 @endcode
* 
* 如上所述，这个例子的网格是正方形[-1,1]^2，其中有正方形[-1/2,1/2]^2作为孔。我们将粗略的网格创建为4乘以4的单元，中间的四个单元缺失。要了解网格的具体样子，最简单的方法可能是先看一下本教程程序的 "结果 "部分。一般来说，如果你想了解更多关于创建网格的信息，无论是像我们在这里所做的那样从头开始，还是使用其他技术，你都应该看一下  step-49  。    
...
* @code
 ...
 @endcode
* 
* 如果我们找到了这样的单元，然后初始化 <code>FEValues</code> 对象，并整合每个形状函数梯度的x分量，以及总面积/体积的单位函数。
* 
...
* 
* 与上面的 <code>PrimalSolver</code> 类相同，我们现在实现一个 <code>DualSolver</code> 。它具有所有相同的特征，唯一的区别是它不接受表示右手边对象的函数对象，而现在接受一个 <code>DualFunctionalBase</code> 对象，它将集合对偶问题的右手边向量。这个类的其余部分是相当琐碎的。    
...
* 
 ...
 @endcode
* 
* 在私有部分，我们有两个函数是用来调用原始基类和双基类的 <code>solve_problem</code> 函数。这两个函数将被本类的 <code>solve_problem</code> 函数并行调用。
* 
...
* 
 @endcode
* 
* 在这个类的实现中，我们首先有 <code>CellData</code> and <code>FaceData</code> 成员类的构造函数，以及 <code>WeightedResidual</code> 构造函数。它们只是将字段初始化为正确的长度，所以我们不必过多地讨论它们。
* 
...
* 
 @endcode
* 
* 现在，变得更加有趣了： <code>refine_grid()</code> 函数要求误差估计器计算单元的误差指标，然后使用其绝对值进行网格细化。
* 
...
* 然后为了计算数值逼近的对偶解z插值到原始解的有限元空间并从z中减去：使用 <code>interpolate_difference</code> 函数，在对偶解的元素空间中得到(z-I_hz)。
* 
...
* 然后把它全部交给 WorkStream::run() 来并行计算所有单元的估计值。
* 
...
* 
 @endcode
* 
* 每个单元的第一个任务是计算该单元的单元残差贡献，并将其放入 <code>error_indicators</code> 变量中。
* 
...
* @code
 ...
 @endcode
* 
* 如果我们有了这个，那么我们就需要找出相邻单元格的哪个面，也就是说， <code>how-many'th</code> 这个单元格是本面后面单元格的邻居。为此，有一个函数，我们将结果放入一个变量，名称为 <code>neighbor_neighbor</code>  。
* 
...
* @code
 ...
 @endcode
* 
* 然后定义一个相邻单元的缩写，在该单元上初始化 <code>FEFaceValues</code> 对象，并提取该单元上的梯度。
* 
...
* 
 @endcode
* 
* 因为我们允许使用不同的细化标准（全局细化，通过凯利误差指标细化，可能有一个权重，以及使用双重估计器），定义一些枚举值，随后定义一个该类型的变量。它将默认为 <code>dual_weighted_error_estimator</code>  。
* 
...
* 
 @endcode
* 
* 接下来首先设置一个二元函数，然后设置一个评价对象的列表。我们默认选择在一个评价点对值进行评价，由评价和二元函数类命名空间中的 <code>PointValueEvaluation</code> 类代表。你也可以设置 <code>PointXDerivativeEvaluation</code> 类来代替评价点的值的x-derivative。      
...
* 首先，让我们看看程序的实际计算结果。在第七个网格上，原始和双重数值解看起来是这样的（使用的颜色方案旨在唤起科罗拉多州白雪皑皑的山脉，这个程序的原作者现在把它叫做家）。 <table align="center">
  <tr>
    <td width="50%">
...
</table> 显然，左下角的区域对于右上角的点值评估是如此不重要，以至于网格在那里完全没有被细化，尽管解决方案在该单元的内角有奇异点 由于右手边和领域的对称性，解决方案实际上应该像右上角的所有四个角一样，但是涉及到对偶解决方案的网格细化标准选择了不同的细化方式
* 
...
* 
* 从计算的结果中，我们可以生成两个图形：一个是显示误差 $J(u)-J(u_h)$ （将外推值视为正确）在点值中的收敛性，以及我们将计算值 $J(u_h)$ 和估计误差eta相加得到的值（如果误差估计器 $eta$ 是精确的，那么值 $J(u_h)+\eta$ 将等于精确的点值，而且这个量的误差将总是零；然而，由于误差估计器只是一个
* 
* - 好
* 
...
* 
* 检查这些数字，我们看到对于全局细化，误差与 $O(1/(sqrt(N) log(N)))$ 成正比，而对于二重估计器 $O(1/N)$ 。一般来说，我们看到双重加权误差估计器比其他细化指标更好，至少在与那些具有类似规则行为的指标相比时是如此。凯利指标产生的误差较小，但在画面上的跳动相当不规则，误差的符号有时也会改变。因此，它的行为不允许将结果外推到更大的N值。此外，如果信任双重加权误差估计器的误差估计，可以通过将估计的误差加入计算值来改进结果。就可靠性而言，加权估计器因此比凯利指标更好，尽管后者有时会产生更小的误差。
* 
...
* 
* 解决方案看起来与之前的大致相同（确切的解决方案当然是 <em> 与 </em> 相同，只是网格发生了一些变化），但现在的整体解决方案是不同的。评估点周围的特写显示了这一点。 <table align="center">
  <tr>
    <td width="50%">
...
* 
* - .0528223...正负0.0000005。我们通过在更细的网格上计算得到这个参考值，大约有13万个单元。回顾一下，如果在最佳情况下，误差是 $O(1/N)$ ，那么采用10倍于此的网格，我们的结果会多出一个数字。
* 
...
* 在初始阶段，真实误差改变了它的符号，估计误差再次与它相当匹配。还请注意，当使用估计的误差来修正 $J(u_h)$ 的计算值时，误差有了戏剧性的改善。
* 
...
* 
* 如果我们不选择 <code>Exercise_2_3</code> 数据集，而是在主函数中选择 <code>CurvedRidges</code> ，并选择 $(0.5,0.5)$ 作为评估点，那么我们就可以重新进行前面例子程序的计算，以比较在双重加权误差估计器帮助下获得的结果是否比我们之前的结果更好。
* 
