...
* 在本教程中，我们以无矩阵的DG框架为例，对拉普拉斯的内部罚分进行离散化，即与 step-39 教程中使用的方案相同。拉普拉斯的离散化是由以下弱形式给出的
* @f{align*}
...
* 
-\mathbf{n}^-\cdot \nabla u^- + 2 g_\text{N}$ ，用于给定Dirichlet值 $g_\text{D}$ 和Neumann值 $g_\text{N}$  。然后将这些表达式插入上述弱形式中。涉及已知量 $g_\text{D}$ 和 $g_\text{N}$ 的贡献最终被移到右手边，而未知值 $u^-$ 被保留在左手边，对矩阵项的贡献与内部面相似。通过这些操作，可以得到与 step-39 中相同的弱形式。
...
* deal.II的无矩阵框架提供了必要的基础设施来实现上述离散化方程的作用。与我们在 step-37 和 step-48 中使用的 MatrixFree::cell_loop() 相反，我们现在用 MatrixFree::loop() 来构建一个代码，它需要三个函数指针，一个用于单元积分，一个用于内面积分，一个用于边界面积分（类似于 step-39 教程程序中使用的MeshWorker设计）。在这三个函数中，我们在正交点上实现各自的条款。对于向量条目与正交点上的数值和梯度之间的插值，我们使用FEEvaluation类来处理单元贡献，FEFaceEvaluation类来处理面的贡献。这些函数的基本用法已经在 step-37 教程程序中进行了广泛的讨论。
...
* 每个面被精确访问一次的事实也适用于那些用MPI并行化时不同处理器之间的子域边界的面，其中一个单元属于一个处理器，一个属于另一个。 MatrixFree::reinit() 中的设置将面分成两部分，最终只报告 MatrixFree::n_inner_face_batches() 和 MatrixFree::n_boundary_face_batches(), 中分别实际处理的面。请注意，与 step-37 中讨论的单元积分相类似，deal.II在几个面上应用矢量化以使用SIMD，在我们称之为<i>batch of faces</i>的东西上工作，只用一条指令。面的批次与单元的批次是独立的，尽管处理面的积分的时间与处理各自单元的积分的时间保持一致，以增加数据的可定位性。
* 这个程序的另一个新特点是，我们不再从求值和积分步骤中拆分像 FEEvaluation::read_dof_values() 或 FEEvaluation::distribute_local_to_global() 这样的向量访问，而是分别调用组合函数 FEEvaluation::gather_evaluate() 和 FEEvaluation::integrate_scatter(), 。这对面积分很有用，因为根据面的评估内容，并非所有单元格的向量项都必须首先被触及。例如，想想节点元素FE_DGQ的情况，节点点在元素表面。如果我们对面的形状函数值感兴趣，只有 $(k+ 1)^{d-1}$ 个自由度以非微不足道的方式对它们有贡献（用更专业的方式来说，只有 $(k+1)^{d-1}$ 个形状函数在面的支持度为零，对 FiniteElement::has_support_on_face()). 个自由度返回真）。
...
* FE_DGQHermite元素属于FE_DGQ元素家族，即它的形状函数是一维多项式的张量乘积，并且该元素是完全不连续的。与通常的FE_DGQ元素中的节点特征相反，FE_DGQHermite元素是一个基于Hermite-like概念的节点贡献和衍生贡献的混合物。底层多项式类是 Polynomials::HermiteLikeInterpolation ，可以概括为以下几点。对于三次多项式，我们用两个多项式来表示单位区间左端的函数值和第一次导数， $x=0$  ，用两个多项式来表示单位区间右端的函数值和第一次导数， $x=1$  。在相反的两端，形状函数的值和一阶导数都是零，确保四个基础函数中只有两个对各自末端的值和导数有贡献。然而，我们偏离了经典的赫米特内插法，没有严格地为值和一阶导数分配一个自由度，而是允许一阶导数是第一和第二形状函数的线性组合。这样做是为了改善插值的条件。此外，当度数超过三时，我们在元素内部以类似拉格朗日的方式增加节点点，并在 $x=0$ 和 $x=1$ 这两个点上结合双零。这些额外节点的位置是由一些贾科比多项式的零点决定的，在类 Polynomials::HermiteLikeInterpolation. 的描述中解释过。
* 使用这个元素，我们只需要访问 $2(k+1)^{d-1}$ 的自由度来计算一个面的值和导数。检查赫米特属性是否被满足是在 FEFaceEvaluation::gather_evaluate() 和 FEFaceEvaluation::integrate_scatter() 中透明地完成的，它们检查基础的类型并尽可能减少对数据的访问。显然，如果我们将 FEFaceEvaluation::read_dof_values() 与 FEFaceEvaluation::evaluate(), 分开，这将是不可能的，因为我们需要读取的条目数量取决于导数的类型（只有值，一导数，等等），因此必须给`read_dof_values()`。
* 这种优化不仅对计算面积分有用，而且对MPI鬼层交换也有用。在一个天真的交换中，如果另一个处理器负责计算面的贡献，我们就需要将一个单元的所有自由度发送给另一个处理器。由于我们知道在用FEFaceEvaluation进行的评估中只有部分自由度被触及，所以自然只交换相关自由度。 MatrixFree::loop() 函数在与 LinearAlgebra::distributed::Vector. 结合时支持选定的数据交换。为了实现这一点，我们需要使用 MatrixFree::DataAccessOnFaces, 类型的参数告诉循环我们要对脸部进行哪种评估，正如在下面 `LaplaceOperator::vmult()` 的实现中可以看到的。在这种情况下，数据交换的方式如下。矢量中的幽灵层数据仍然假装代表所有的自由度，这样，FEFaceEvaluation可以继续读取数值，就像单元格是本地拥有的一样。数据交换程序负责将数据打包和解包成这种格式。虽然这听起来很复杂，但我们将在下面的结果部分显示，通过与没有指定面的数据访问的基线代码进行性能比较，这确实是值得的。
...
* 按照 step-37 程序的传统，我们再次用共轭梯度解算器中的几何多网格预处理程序解决泊松问题。在这个教程中，我们没有计算对角线并使用基本的Precondition Chebyshev作为平滑器，而是选择了一种不同的策略。我们实现了一个块状Jacobi预处理程序，其中块状指的是一个单元上的所有自由度。我们没有在预处理程序中建立完整的单元矩阵并应用它的LU因子化（或逆）；这种操作将受到严重的内存带宽限制，因此速度相当慢；我们通过一种特殊的技术，即快速对角线化方法，对块的逆进行近似处理。
...
* 在本程序中，我们坚持使用常数系数和笛卡尔网格，尽管基于张量积的近似版本仍然可以用于更普遍的网格，而且算子评估本身当然也是通用的。此外，我们也不关心自适应网格，因为多网格算法需要获取不同细化边上的通量矩阵，如 step-39 中所解释的。然而，我们所做的一件事是仍然将我们的块状Jacobi预处理器包裹在Precondition Chebyshev中。该类使我们不必寻找适当的松弛参数（对于block-Jacobi平滑器来说，二维的松弛参数约为0.7，三维的松弛参数约为0.5），并且通常比普通的Jacobi平滑器提高了一些平滑效率，因为当Chebyshev多项式的度数设定为1或2时，它可以降低解的时间。
...
* 包含文件与 step-37 中的文件基本相同，只是用FE_DGQHermite的有限元类代替了FE_Q。所有对面积分进行无矩阵计算的功能已经包含在`fe_evaluation.h`中。
* 
...
* 
 @endcode
* 
* 与 step-37 一样，为了简单起见，我们在程序顶部将维数和多项式程度收集为常数。与 step-37 不同的是，这次我们选择了一个真正的高阶方法，度数为8，任何不使用和因式分解的实现都会变得非常慢，而使用MatrixFree的实现则提供了与度数为2或3时基本相同的效率。此外，本教程程序中的所有类都是模板化的，所以在运行时从输入文件或命令行参数中选择度数是很容易的，只要在`main()`函数中添加相应度数的实例即可。
* 
...
* 
* 与 step-7 相类似，我们定义了一个分析解，并试图用我们的离散化来重现。由于本教程的目的是展示无矩阵方法，我们选择了一个最简单的可能性，即一个余弦函数，其导数对我们来说足够简单，可以通过分析计算。再往下看，我们在这里选择的波数2.4将与 $x$ 方向的域范围即2.5相匹配，这样我们在 $x = 2.5$ 包括 $6pi$ 或余弦的三个整波旋转处得到一个周期解。第一个函数定义了解和它的梯度，分别用于表达Dirichlet和Neumann边界条件的解析解。此外，一个代表解的负拉普拉斯的类被用来表示右手边（强制）函数，我们用它来匹配离散化版本中的给定分析解（制造解）。
* 
...
* 
* `LaplaceOperator`类与  step-37  中的相应类相似。一个重要的区别是，我们没有从 MatrixFreeOperators::Base 中派生出这个类，因为我们想呈现 MatrixFree::loop() 中的一些额外功能，这些功能在通用类 MatrixFreeOperators::Base. 中是没有的。 我们从Subscriptor类中派生出这个类，以便能够在Chebyshev预处理程序中使用该操作符，因为该预处理程序通过SmartPointer存储底层矩阵。  
...
* 
 @endcode
* 
* 这个函数实现了LaplaceOperator对向量`src`的作用，并将结果存储在向量`dst`中。与 step-37 相比，这个调用有四个新特性。  
* 第一个新特征是上面提到的`adjust_ghost_range_if_necessary`函数，它需要将向量适合于单元和面函数中FEEvaluation和FEFaceEvaluation所期望的布局。  
* 第二个新特征是我们没有像 step-37 中那样实现`vmult_add()`函数（通过虚拟函数 MatrixFreeOperators::Base::vmult_add()), ，而是直接实现`vmult()`功能。由于单元和面的积分都将和到目的向量中，我们当然必须在某处将向量归零。对于DG元素，我们有两个选择&ndash；一个是使用 FEEvaluation::set_dof_values() 而不是下面`apply_cell`函数中的 FEEvaluation::distribute_local_to_global() 。这是因为MatrixFree中的循环布局是这样的：单元积分总是在面积分之前触及给定的向量条目。然而，这实际上只适用于完全不连续的基数，其中每个单元都有自己的自由度，不与邻近的结果共享。另一种设置，即这里选择的设置，是让 MatrixFree::loop() 来处理向量的归零问题。这可以被认为是简单地在代码中调用`dst = 0;`。对于像 `LinearAlgebra::distributed::Vector`, 这样的支持性向量来说，实现起来就比较麻烦了，因为我们的目标是不要一次就把整个向量清零。在足够小的几千个向量项上进行归零操作的好处是，在 FEEvaluation::distribute_local_to_global() 和 FEFaceEvaluation::distribute_local_to_global(). 中再次访问之前，被归零的向量项会保留在缓存中，因为无矩阵运算符的评估真的很快，仅仅归零一个大的向量就会相当于运算符评估时间的25%，我们显然希望避免这种代价。这种将向量归零的选项也适用于 MatrixFree::cell_loop 和连续基数，尽管在 step-37 或 step-48 的教程程序中没有使用它。  
* 第三个新特征是我们提供在单元格、内面和边界面计算的函数的方式。MatrixFree类有一个叫做`loop`的函数，它接收三个函数指针，用于三种情况，允许分开实现不同的东西。正如在 step-37 中所解释的，这些函数指针可以是 `std::function` 对象或类的成员函数。在这种情况下，我们使用成员函数的指针。  
...
* 为了了解数据如何被减少，想想节点元素FE_DGQ的情况，节点点在元素表面，在一个单元的 $(k+1)^d$ 自由度中，只有 $(k+1)^{d-1}$ 自由度对一个面的多项式程度 $k$ 的值有贡献。类似的减少也可以用于内部惩罚方法，该方法对面的数值和一阶导数进行评估。当在一维中使用类似Hermite的基时，最多只有两个基函数对值和导数有贡献。FE_DGQHermite类实现了这一概念的张量乘积，在介绍中已经讨论过。因此，每个面只需交换 $2(k+1)^{d-1}$ 个自由度，一旦 $k$ 个自由度大于4或5个，这显然是一种胜利。请注意，FE_DGQHermite的这种减少的交换在具有弯曲边界的网格上也是有效的，因为导数是在参考元素上取的，而几何体只在内部混合它们。因此，这与试图用连续的Hermite型形状函数获得 $C^1$ 的连续性是不同的，在这种情况下，非笛卡尔的情况大大改变了情况。显然，在非笛卡尔网格上，导数还包括超出法向导数的形状函数的切向导数，但这些也只需要元素表面的函数值。如果元素不提供任何压缩，循环会自动交换受影响单元的所有条目。
* 
...
* 
 @endcode
* 
* 因为拉普拉斯是对称的，所以`Tvmult()`（多网格平滑界面需要的）操作被简单地转发给`vmult()`的情况。
* 
...
* 
 @endcode
* 
* 面的操作实现了与 step-39 相类似的内部惩罚方法的条款，正如介绍中所解释的。我们需要两个评估器对象来完成这个任务，一个用于处理来自内部面的两边之一的单元格的解，另一个用于处理来自另一边的解。面积分的评价器被称为FEFaceEvaluation，并在构造函数的第二个槽中接受一个布尔参数，以指示评价器应属于两边中的哪一边。在FEFaceEvaluation和MatrixFree中，我们称两边中的一边为 "内部"，另一边为 "外部"。`外部'这个名字是指两边的评价器将返回相同的法向量。对于 "内部 "一侧，法向量指向外部，而另一侧则指向内部，并且与该单元的外部法向量相对应。除了新的类名之外，我们再次得到了一系列的项目，与 step-37 中讨论的类似，但在这种情况下是针对内部面的。请注意，MatrixFree的数据结构形成了面的批次，类似于单元积分的单元批次。一批中的所有面涉及不同的单元格编号，但在参考单元格中具有相同的面编号，具有相同的细化配置（无细化或相同的子面）和相同的方向，以保持SIMD操作的简单和高效。  
...
* @code
 ...
 @endcode
* 
* 在给定的一批面孔上，我们首先更新指向当前面孔的指针，然后访问矢量。如上所述，我们将向量的访问与评估结合起来。在面积分的情况下，对于FE_DGQHermite基础的特殊情况，可以减少对向量的数据访问，正如上面解释的数据交换。因为在 $(k+1)^d$ 个单元自由度中只有 $2(k+1)^{d-1}$ 个单元自由度被非零值或形状函数的导数所乘，这种结构可以被用于评估，大大减少数据访问。减少数据访问不仅是有益的，因为它减少了飞行中的数据，从而有助于缓存，而且当从单元格索引列表中相距较远的单元格中收集数值时，对面的数据访问往往比对单元格积分更不规则。
* 
...
-u^- + 2
 ...
 u^+ =
* 
-\mathbf{n}^-\cdot \nabla u^- + 2 g_\text{N}$  。由于这个操作实现了同质部分，即矩阵-向量乘积，我们必须在这里忽略边界函数  $g_\text{D}$  和  $g_\text{N}$  ，并在  `LaplaceProblem::compute_rhs()`.  中把它们加到右手边 注意，由于通过  $u^+$  把解  $u^-$  扩展到外部，我们可以保持所有因子  $0.5$  与内表面函数相同，也可以参见  step-39  中的讨论。  
...
* 
 @endcode
* 
* 接下来的两个语句所集合的左右边界项似乎有一些任意的符号，但这些都是正确的，可以通过查看 step-39 并插入数值来验证
* 
* - 以及1D情况下的法向量的1。
* 
...
* 
 @endcode
* 
* 一旦我们知道拉普拉斯矩阵的比例系数，我们就将这个权重应用于未被缩放的DG拉普拉斯矩阵，并将数组发送到TensorProductMatrixSymmetricSum类中，用于计算介绍中提到的广义特征值问题。
* 
...
* 在DG背景下，用于近似块状Jacobi预处理的vmult函数非常简单。我们只需要读取当前单元格批的值，对张量积矩阵阵列中的给定条目进行逆运算，并将结果写回去。在这个循环中，我们覆盖了`dst`中的内容，而不是首先将条目设置为零。这对于DG方法来说是合法的，因为每个单元都有独立的自由度。此外，我们手动写出所有单元批的循环，而不是通过 MatrixFree::cell_loop(). 我们这样做是因为我们知道我们在这里不需要通过MPI网络进行数据交换，因为所有的计算都是在每个处理器上的本地单元上完成的。
* 
...
* 
 @endcode
* 
* 该设置函数在两个方面与 step-37 不同。第一个方面是我们不需要为不连续的Ansatz空间插值任何约束，而只是将一个假的AffineConstraints对象传入 Matrixfree::reinit(). 。 第二个变化是因为我们需要告诉MatrixFree也初始化面的数据结构。我们通过为内部面和边界面分别设置更新标志来做到这一点。在边界面，我们需要函数值、它们的梯度、JxW值（用于积分）、法向量和正交点（用于边界条件的评估），而对于内部面，我们只需要形状函数值、梯度、JxW值和法向量。只要`mapping_update_flags_inner_faces`或`mapping_update_flags_boundary_faces`中的一个与UpdateFlags的默认值`update_default`不同，MatrixFree中的面数据结构总是被建立的。
* 
...
* 右手边的计算比 step-37 中要复杂一些。现在的单元项包括分析解的负拉普拉斯，`RightHandSide'，为此我们需要首先将VectorizedArray字段的Point，即一批点，通过分别评估VectorizedArray中的所有通道，分割成一个点。请记住，通道的数量取决于硬件；对于不提供矢量化的系统（或deal.II没有本征），它可能是1，但在最近英特尔架构的AVX-512上也可能是8或16。
* 
...
* 其次，我们还需要应用Dirichlet和Neumann边界条件。这个函数是到函数 `LaplaceOperator::apply_boundary()` 函数的缺失部分，一旦在迪里希特边界上的外部解决值 $u^+ =
* 
-u^- + 2 g_\text{D}$ 和 $\mathbf{n}^-\cdot \nabla u^+ =
 \mathbf{n}^-\cdot \nabla u^-$ 以及在诺伊曼边界上的 $u^+=u^-$ 和 $\mathbf{n}^-\cdot \nabla u^+ =
* 
-\mathbf{n}^-\cdot \nabla u^- + 2
 g_\text{N}$ 被插入并以边界函数 $g_\text{D}$ 和 $g_\text{N}$ 展开。有一点需要记住的是，我们把边界条件移到右手边，所以符号与我们在求解部分施加的相反。    
* 我们本可以通过 MatrixFree::loop 部分发出单元格和边界部分，但我们选择手动写出所有面的完整循环，以了解面指数的索引布局在MatrixFree中是如何设置的：内部面和边界面都共享索引范围，所有批次的内部面的数字都比批次的边界单元格低。两种变体的单一索引使我们可以很容易地在两种情况下使用相同的数据结构FEFaceEvaluation，它附着在同一个数据域上，只是位置不同。内部面的批次数（其中一个批次是由于将几个面合并成一个面进行矢量化）由 MatrixFree::n_inner_face_batches(), 给出，而边界面的批次数由 MatrixFree::n_boundary_face_batches(). 给出。
* 
...
* 因为我们手动运行了单元格的循环，而不是使用 MatrixFree::loop(), ，所以我们一定不要忘记与MPI进行数据交换
* 
...
* 
 @endcode
* 
* `solve()`函数几乎是逐字复制自  step-37  。我们设置了相同的多网格成分，即水平转移、平滑器和粗略的网格求解器。唯一不同的是，我们没有使用拉普拉斯的对角线作为用于平滑的切比雪夫迭代的预处理，而是使用我们新解决的类`%PreconditionBlockJacobi`。不过，机制是一样的。
* 
...
* 
 @endcode
* 
* 因为我们已经用分析解解决了一个问题，所以我们想通过计算数值结果与分析解的L2误差来验证我们实现的正确性。
* 
...
* 
 @endcode
* 
* `run()`函数设置了初始网格，然后以常规方式运行多网格程序。作为一个域，我们选择一个矩形，在 $x$ 方向上有周期性的边界条件，在 $y$ 方向上的正面（即索引号为2的面，边界id等于0）有迪里希特条件，在背面以及 $z$ 方向上的两个面有诺伊曼条件（边界id等于1）的三维情况。与 $y$ 和 $z$ 方向相比， $x$ 方向的域的范围有些不同（鉴于 "解决方案 "的定义，我们希望在这里实现周期性的解决方案）。
* 
...
* 与 step-37 中一样，我们以运行时间来评估多网格求解器。 在两个空间维度上，元素的度数为8，可能的输出如下。
* @code
...
* 在介绍和代码中的注释中，多次提到用FEEvaluation和FEFaceEvaluation评估器可以非常有效地处理高阶。现在，我们想通过观察三维多网格求解器对不同多项式阶数的吞吐量来证实这些说法。我们收集的时间如下。我们首先在接近一千万的问题规模下运行一个求解器，如表前四行所示，并记录时间。然后，我们通过记录每秒解决的百万自由度数（MDoFs/s）来规范吞吐量，以便能够比较不同自由度的效率，其计算方法是自由度数除以求解器时间。
...
* 我们清楚地看到每个自由度的效率最初是如何提高的，直到它达到多项式程度的最大值  $k=4$  。这种效果是令人惊讶的，不仅是因为较高的多项式度数通常会产生一个好得多的解决方案，而且特别是当考虑到基于矩阵的方案时，在较高的度数下密度耦合会导致单调的吞吐量下降（在三维中是巨大的， $k=4$ 比 $k=1$ 慢十倍以上！）。对于更高的度数，吞吐量略有下降，这既是由于迭代次数的增加（从 $k=2,3,4$ 的12次到 $k=10$ 的19次），也是由于操作评估的 $\mathcal O(k)$ 的复杂性。尽管如此，对于更高的多项式度数来说，解题时间的效率仍然会更好，因为它们有更好的收敛率（至少对于像这个问题一样简单的问题）。对于 $k=12$ ，我们在100万个DoFs（求解时间小于1秒）的情况下已经达到了舍入精度，而对于 $k=8$ ，我们需要2400万个DoFs和8秒。对于 $k=5$ ，误差约为 $10^{-9}$ ，有5700万个DoFs，因此，尽管需要16秒的时间，但仍然离舍入很远。
...
* 最后，我们来看一下本教程中的一些特殊成分，即FE_DGQHermite基础，特别是对 MatrixFree::DataAccessOnFaces. 的指定。 在下面的表格中，第三行显示了上面的优化求解器，第四行显示了只将 MatrixFree::DataAccessOnFaces  ]设置为 "未指定 "而非最佳 "梯度"，最后一行是用基本的FE_DGQ元素替换FE_DGQHermite，其中MPI交换更加昂贵，操作由 FEFaceEvaluation::gather_evaluate() 和 FEFaceEvaluation::integrate_scatter(). 完成。
*  <table align="center" class="doxtable">
  <tr>
   <th>degree</th>
   <th>1</th>
