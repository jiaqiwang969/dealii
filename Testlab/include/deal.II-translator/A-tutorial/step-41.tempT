...
\qquad \forall i.&
@f}。
* 其中 $B$ 是所选有限元空间上的质量矩阵，上述指数 $i$ 是位于域内部的自由度集合 $\cal S$ 中的所有自由度（我们在周边有迪里希条件）。然而，如果我们在组合产生这个质量矩阵的所有条款时使用一个特殊的正交规则，即一个正交公式，其中正交点只位于定义了形状函数的插值点；由于除了一个形状函数外，所有的形状函数在这些位置都是零，我们得到一个对角线质量矩阵，即
* @f{align*}
  B_{ii} = \int_\Omega \varphi_i(\mathbf x)^2\ \textrm{d}x,
  \qquad
  B_{ij}=0 \ \text{for } i\neq j.
@f}
* 为了定义 $G$ ，我们使用与 $B$ 相同的技术。换句话说，我们定义了
* @f{align*}
...
- G_i) \rbrace,
@f} 。
* 在这个程序中我们选择 $c = 100$ 。这是一种惩罚性参数，取决于问题本身，需要选择足够大的参数；例如，如果我们使用7个全局细化，使用当前程序对 $c = 1$ 没有收敛作用）。)
...
\qquad \forall i.&
@f} 。
* 我们可以通过始终围绕前一个iterate进行线性化（即应用牛顿方法）来迭代求解，但为此我们需要将不可微调的函数 $C(\cdot,\cdot)$ 线性化。也就是说，它是可微的，事实上我们有@f{equation*}
...
\end{pmatrix},
@f} 。
* 在这里，我们以自然的方式将矩阵 $A,B$ 和向量分割开来，其索引属于活动集 ${\mathcal{A}_k}$ 或非活动集 ${\mathcal{F}_k}$ 。
* 我们也可以通过设置 $\delta U^k \dealcoloneq
U^{k+1}
* 
- U^k$ 和 $\delta \Lambda^k \dealcoloneq \Lambda^{k+1}
* 
- \Lambda^k$ 并将所有已知项带到右边来解决我们感兴趣的变量，而不是求解更新 $\delta U, \delta \Lambda$ 。这就得到了@f{equation*}
\begin{pmatrix}
 A_{\mathcal{F}_k\mathcal{F}_k} & A_{\mathcal{F}_k\mathcal{A}_k} & B_{\mathcal{F}_k} & 0\\
 A_{\mathcal{A}_k\mathcal{F}_k} & A_{\mathcal{A}_k\mathcal{A}_k} & 0 & B_{\mathcal{A}_k}\\
...
@f}。
* 这些是上面基本算法描述中概述的方程。
* 我们甚至可以更进一步。很容易看到，我们可以消除第三行和第三列，因为它意味着 $\Lambda_{\mathcal{F}_k} = 0$  :@f{equation*}
\begin{pmatrix}
...
\end{pmatrix}.
@f} 。
* 这表明，事实上我们只需要解决位于活动集上的拉格朗日乘数。通过考虑第二行，我们可以通过@f{equation*}
 \Lambda^k_S = B^{-1}\left(f_{\mathcal{S}}
* 
- A_{\mathcal{S}}U^k_{\mathcal{S}}\right).
@f}恢复全部拉格朗日乘数矢量。
* 由于第三行和 $B_{\mathcal{A}_k}$ 是一个对角线矩阵的事实，我们能够直接计算出 $U^k_{\mathcal{A}_k}=B^{-1}_{\mathcal{A}_k}G_{\mathcal{A}_k}$ 。因此，我们也可以把这个线性系统写成如下：@f{equation*}
\begin{pmatrix}
...
\end{pmatrix}.
@f} 。
* 幸运的是，这种形式很容易得到：我们只需建立通常的拉普拉斯线性系统@f{equation*}
\begin{pmatrix}
