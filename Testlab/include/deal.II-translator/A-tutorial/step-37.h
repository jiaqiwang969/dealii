...
\partial \Omega @f}，其中 $a(\mathbf x)$ 是一个可变系数。下面，我们解释如何为这个问题实现矩阵-向量乘积，而不明确形成矩阵。当然，对于其他方程也可以用类似的方法进行构造。
...
v_\mathrm{cell},
@f}。
*其中<i>u</i><sub>cell</sub>是<i>u</i>在各单元自由度上的值，<i>v</i><sub>cell</sub>=<i>A</i><sub>cell</sub><i>u</i><sub>cell</sub>相应地得到结果。因此一个天真的尝试来实现拉普拉斯的局部作用将使用以下代码。
* @code
...
@endcode
*
* 这里我们忽略了边界条件以及我们可能拥有的任何悬空节点，尽管使用AffineConstraints类来包括这两者都不是很困难。请注意，我们首先以常规方式生成局部矩阵，作为每个局部矩阵条目在所有正交点上的总和。为了形成上述公式中表达的实际乘积，我们提取了单元格相关自由度的 <code>src</code> 的值（<i>P</i>的作用]<sub>cell,loc-glob</sub>的作用），乘以局部矩阵（<i>A</i><sub>cell</sub>的作用），最后将结果加到目的地向量 <code>dst</code> （<i>P</i><sub>cell,loc-glob</sub><sup>T</sup>的作用，在所有元素上加起来）。原则上，这不会比这更难。
...
J_\mathrm{cell}^{-\mathrm T} B_\mathrm{ref\_cell}, @f}，其中<i>B</i><sub>ref_cell</sub>表示参考单元的梯度，<i>J</i><sup>-T</sup><sub>cell</sub>表示从单元到实数单元转换的反转置Jacobian（在转换语言中，由<i>J</i><sup>-T</sup><sub>cell</sub>代表的操作为变数转换）。<i>J</i><sup>-T</sup><sub>cell</sub>是块对角线的，块的大小等于问题的维度。每个对角线块都是雅各布变换，从定义单元到实际单元。
...
J_\mathrm{cell}^{-\mathrm T}$ 。后者将使用更少的内存，因为张量是对称的，在三维中具有六个独立的值，而对于前者，我们需要九个条目用于反转雅各布，一个用于正交权重和雅各布行列式，一个用于系数，总共是11个双数。原因是前者允许通过一个通用的缓存数据框架来实现通用的微分算子，而后者则专门存储拉普拉斯的系数。如果应用需要的话，这种特殊化可能会带来回报，值得考虑。请注意，deal.II的实现足够聪明，可以检测出笛卡尔或仿生几何，在这种情况下，Jacobian在整个单元中是恒定的，不需要为每个单元存储（实际上在不同的单元中也经常是一样的）。
...
B_\mathrm{val,y})u_\mathrm{cell} = B_\mathrm{val,y} U B_\mathrm{grad,x}$ 。这将计算这个乘积的复杂性从 $p^4$ 降低到 $2 p^3$ ，其中<i>p</i>-1是有限元的度数（即，相当于，<i>p</i>是每个坐标方向上的形状函数的数量），或者一般来说 $p^{2d}$  到 $d p^{d+1}$ 。我们之所以用多项式度数来看复杂度，是因为我们希望能够达到高度，并可能增加多项式度数<i>p</i>而不是网格分辨率。像这里所用的中等度数的好算法是独立于维度的多项式度数的线性算法，而不是基于矩阵的方案或通过FEValues的天真评价。在deal.II的实现中使用的技术自20世纪80年代以来在谱元界已经建立起来。
...
* FEEvaluation中用于评估的计算内核是以优化使用计算资源的方式编写的。为了达到这个目的，它们不对双倍数据类型进行操作，而是采用我们称之为VectorizedArray的东西（例如，查看 FEEvaluationBase::get_value, 的转向类型，该类型是指ascal元素的VectorizedArray，以及矢量有限元素的Tensor）。矢量数组是一个双数或浮点数的短数组，其长度取决于所使用的特定计算机系统。例如，基于x86-64的系统支持流式SIMD扩展（SSE），处理器的矢量单元可以通过一条CPU指令处理两个双数（或四个单精度浮点）。较新的处理器（大约从2012年起）支持所谓的高级向量扩展（AVX），有256位操作数，可以分别使用四个双数和八个浮点数。矢量化是一个单指令/多数据（SIMD）的概念，也就是说，一条CPU指令被用来同时处理多个数据值。通常情况下，有限元程序不会明确使用矢量化，因为这个概念的好处只体现在算术密集型操作上。大部分典型的有限元工作负载的内存带宽有限（对稀疏矩阵和向量的操作），额外的计算能力是无用的。
...
* 请注意，矢量化取决于代码运行的CPU，以及代码是为哪种CPU编译的。为了给你的计算机生成最快的FEEvaluation内核，你应该用所谓的<i>native</i>处理器变量编译deal.II。当使用gcc编译器时，可以通过在cmake构建设置中设置变量<tt>CMAKE_CXX_FLAGS</tt>为<tt>"-march=native"</tt>来启用它（在命令行中，指定<tt>-DCMAKE_CXX_FLAGS="-march=native"</tt>，更多信息见deal.II阅读手册）。其他编译器也有类似的选项。我们在本例的run()函数中输出当前的矢量化长度。
*

*<a name="Runningmultigridonlargescaleparallelcomputers"></a><h3>Running multigrid on large-scale parallel computers</h3>


* 如上所述，无矩阵框架中的所有组件都可以很容易地通过MPI使用领域分解进行并行化。由于在deal.II中通过p4est（详见 step-40 ）可以很容易地访问大规模的并行网格，而且基于单元的无矩阵评估循环<i>only</i>需要将网格分解成每个处理器上大小基本相同的块，因此编写一个使用分布式内存的并行程序所需的工作相对较少。当其他使用MPI的教程程序依赖于PETSc或Trilinos时，这个程序使用deal.II自己的并行向量设施。
...
*
* 我们为泊松问题定义了一个可变系数函数。它类似于 step-5 中的函数，但我们使用的是 $a(\mathbf
 x)=\frac{1}{0.05 + 2\|\bf x\|^2}$ 的形式，而不是一个不连续的形式。这只是为了证明这种实现的可能性，而不是在物理上有什么意义。我们定义系数的方式与早期教程程序中的函数相同。有一个新的函数，即一个带有模板参数 @p number. 的 @p value 方法
*
...
*
* 下面这个名为 <code>LaplaceOperator</code> 的类，实现了微分运算符。就所有的实际目的而言，它是一个矩阵，也就是说，你可以向它询问它的大小（成员函数  <code>m(), n()</code>  ），你可以将它应用于一个矢量（ <code>vmult()</code>  函数）。当然，与实数矩阵的区别在于，这个类实际上并不存储矩阵的<i>elements</i>，而只知道如何计算运算符应用于向量时的作用。   
...
* 注意文件 <code>include/deal.II/matrix_free/operators.h</code> 已经包含了通过类 MatrixFreeOperators::LaplaceOperator. 对拉普拉斯的实现。 出于教育目的，在这个教程程序中重新实现了该运算符，解释了其中的成分和概念。   
* 本程序利用了集成在deal.II中的有限元算子应用的数据缓存。这个数据缓存类被称为MatrixFree。它包含局部和全局自由度之间的映射信息（Jacobian）和索引关系。它还包含约束条件，如来自悬挂节点或迪里切特边界条件的约束。此外，它可以在所有单元上以%并行方式发出一个循环，确保只有不共享任何自由度的单元被处理（这使得循环在写入目标向量时是线程安全的）。与 @ref threads 模块中描述的WorkStream类相比，这是一个更先进的策略。当然，为了不破坏线程安全，我们在写入类全局结构时必须小心。   
...
*
*  @note  储存 <code>VectorizedArray<number></code> 类型的值需要注意。在这里，我们使用deal.II表类，它准备以正确的排列方式来保存数据。然而，存储例如一个 <code>std::vector<VectorizedArray<number> ></code> 是不可能用矢量的。数据与内存地址的边界需要一定的对齐（基本上，在AVX情况下，一个32字节长的VectorizedArray需要从一个能被32整除的内存地址开始）。表类（以及它所基于的AlignedVector类）确保这种对齐方式得到尊重，而 std::vector 一般不这样做，这可能会导致一些系统在奇怪的地方出现分段故障，或者其他系统的性能不理想。
*
...
*
* 为了初始化系数，我们直接赋予它上面定义的系数类，然后选择方法 <code>coefficient_function.value</code> 与矢量数（编译器可以从点数据类型中推导出来）。下面将解释FEEvaluation类（及其模板参数）的使用。
*
...
*
* 这里是这个类的主要功能，矩阵-向量乘积的评估（或者，一般来说，一个有限元算子的评估）。这是在一个需要四个参数的函数中完成的，MatrixFree对象，目标和源向量，以及要处理的单元格的范围。MatrixFree类中的方法 <code>cell_loop</code> 将在内部用一些单元格范围来调用这个函数，这些单元格范围是通过检查哪些单元格可以同时工作来获得的，这样写操作就不会引起任何竞赛条件。请注意，循环中使用的单元格范围并不是直接指当前网格中的（活动）单元格数量，而是一个单元格批次的集合。  换句话说，"单元 "可能是一个错误的开始，因为FEEvaluation将几个单元的数据分组在一起。这意味着在正交点的循环中，我们实际上是将几个单元的正交点作为一个块来看待。这样做是为了实现更高的矢量化程度。  这种 "单元 "或 "单元批 "的数量存储在MatrixFree中，可以通过 MatrixFree::n_cell_batches(). 查询。与deal.II单元迭代器相比，在这个类别中，所有的单元都被布置在一个普通的数组中，不直接知道水平或相邻关系，这使得通过无符号整数来索引单元成为可能。   
...
* 
 @endcode
*
* 这个函数实现了 Base::apply_add() 接口的所有单元的循环。这是通过MatrixFree类的 @p cell_loop 来实现的，它接受该类的operator()，参数为MatrixFree, OutVector, InVector, cell_range。当使用MPI并行化工作时（但没有线程），如本教程程序中所做的，单元格循环对应于以下三行代码。   
...
* 下面的函数实现了算子的对角线的计算。计算无矩阵算子评价的矩阵项原来比评价算子更复杂。从根本上说，我们可以通过在<i>all</i>单位向量上应用算子来获得算子的矩阵表示。当然，这将是非常低效的，因为我们需要进行<i>n</i>运算符的评估来检索整个矩阵。此外，这种方法会完全忽视矩阵的稀疏性。然而，在单个单元上，这是一种方法，而且实际上效率并不低，因为单元内的所有自由度之间通常存在着耦合。   
...
* 
 @endcode
*
* 在本地计算循环中，我们通过循环本地矩阵的所有列来计算对角线，并将条目1放在<i>i</i>的槽中，将条目0放在所有其他槽中，也就是说，我们一次在一个单位向量上应用单元微分算子。调用 FEEvaluation::evaluate, 的内部部分是对正交点的循环，而 FEEvalution::integrate, 则与local_apply函数完全相同。之后，我们挑选出本地结果的第<i>i</i>个条目，并将其放入一个临时存储器中（因为我们在下一次循环迭代时覆盖了 FEEvaluation::get_dof_value() 后面数组中的所有条目）。最后，临时存储被写到目标向量中。注意我们是如何使用 FEEvaluation::get_dof_value() 和 FEEvaluation::submit_dof_value() 来读取和写入FEEvaluation用于积分的数据字段，并在另一方面写入全局向量。   
* 鉴于我们只对矩阵的对角线感兴趣，我们干脆扔掉一路走来计算过的本地矩阵的所有其他条目。虽然计算完整的单元格矩阵，然后扔掉除对角线以外的所有东西看起来很浪费，但整合的效率很高，所以计算不会花费太多的时间。请注意，对于多项式程度来说，每个元素的算子评估的复杂度是 $\mathcal
 O((p+1)^{d+1})$ ，所以计算整个矩阵要花费我们 $\mathcal O((p+1)^{2d+1})$ 次操作，与用FEValues计算对角线的复杂度 $\mathcal O((p+1)^{2d})$ 相差不大。由于FEEvaluation也由于矢量化和其他优化而大大加快了速度，所以用这个函数计算对角线实际上是最快的（简单的）变量。(有可能用 $\mathcal
 O((p+1)^{d+1})$ 操作中的和因子化技术来计算对角线，这涉及到特别适应的内核&mdash;但由于这种内核只在特定情况下有用，而对角线的计算通常不在关键路径上，所以它们没有在deal.II中实现。)   
...
*
* 这个类是基于  step-16  中的一个类。然而，我们用我们的无矩阵实现取代了SparseMatrix<double>类，这意味着我们也可以跳过稀疏性模式。请注意，我们定义LaplaceOperator类时，将有限元的度数作为模板参数（该值在文件的顶部定义），并且我们使用浮点数来表示多网格级矩阵。   
...
* 
 @endcode
*
* 接下来，初始化所有层次上的多网格方法的矩阵。数据结构MGConstrainedDoFs保留了受边界条件约束的指数信息，以及不同细化层次之间的边缘指数，如 step-16 教程程序中所述。然后，我们穿过网格的各个层次，在每个层次上构建约束和矩阵。这与原始网格上的系统矩阵的构造密切相关，只是在访问各层信息而不是活动单元时，在命名上略有不同。
*
...
*
* 组装函数非常简单，因为我们要做的就是组装右手边的数据。多亏了FEEvaluation和所有缓存在MatrixFree类中的数据，我们从 MatrixFreeOperators::Base, 中查询，这可以在几行中完成。由于这个调用没有被包装成一个 MatrixFree::cell_loop （这将是一个替代方案），我们一定不要忘记在装配结束时调用compress()，将右手边的所有贡献发送给各自自由度的所有者。
*
...
*
* 解决的过程与 step-16 中类似。我们先从转移的设置开始。对于 LinearAlgebra::distributed::Vector, 来说，有一个非常快速的转移类，叫做MGTransferMatrixFree，它用同样的快速和因子化核在网格层之间进行插值，这也被用于FEEvaluation。
*
...
* 
 @endcode
*
* 作为一个平滑器，本教程程序使用切比雪夫迭代，而不是 step-16 中的SOR。（SOR将很难实现，因为我们没有明确的矩阵元素，而且很难使它在%并行中有效工作）。  平滑器是用我们的水平矩阵和切比雪夫平滑器的强制性附加数据初始化的。我们在这里使用一个相对较高的度数（5），因为矩阵-向量乘积是比较便宜的。我们选择在平滑器中平滑出 $[1.2 \hat{\lambda}_{\max}/15,1.2 \hat{\lambda}_{\max}]$ 的范围，其中 $\hat{\lambda}_{\max}$ 是最大特征值的估计值（系数1.2在PreconditionChebyshev内部应用）。为了计算该特征值，Chebyshev初始化执行了几步没有预处理的CG算法。由于最高的特征值通常是最容易找到的，而且一个粗略的估计就足够了，我们选择10次迭代。最后，我们还设置了切比雪夫方法中的内部预处理类型，这是一个雅可比迭代。这由DiagonalMatrix类来表示，该类得到了我们的LaplaceOperator类所提供的反对角线条目。     
...
* 对于这些接口矩阵的实现，已经有一个预定义的类 MatrixFreeOperators::MGInterfaceOperator ，它将例程 MatrixFreeOperators::Base::vmult_interface_down() 和 MatrixFreeOperators::Base::vmult_interface_up() 包装在一个带有 @p vmult()和 @p Tvmult() 操作（最初是为矩阵编写的，因此期待这些名字）的新类中。请注意，vmult_interface_down是在多网格V周期的限制阶段使用的，而vmult_interface_up是在延长阶段使用。     
...
*
* 这里是数据输出，是 step-5 的简化版本。我们对细化过程中产生的每个网格使用标准的VTU（=压缩的VTK）输出。此外，我们还使用了一种针对速度而不是磁盘使用量进行优化的压缩算法。默认设置（针对磁盘使用进行优化）使得保存输出的时间是运行线性求解器的4倍，而将 DataOutBase::VtkFlags::compression_level 设置为 DataOutBase::VtkFlags::best_speed 则将其降低到仅为线性求解的四分之一。   
...
*
* 除了我们根据 step-40 设置了MPI框架之外，在主函数中没有任何意外。
*
...
* 由于这个例子解决的是与 step-5 相同的问题（除了一个不同的系数），所以对解决方案没有什么可说的。但我们还是展示了一张图片，通过等高线和体积渲染来说明解决方案的大小。
...
@endcode
*
* 由于某个网格上的 $\mathcal Q_4$ 元素对应于一半网格大小上的 $\mathcal Q_2$ 元素，我们可以比较在第四周期使用四度多项式和第五周期使用二次多项式的运行时间，两者都是210万自由度。令人惊讶的是，尽管多用了一次线性迭代， $\mathcal Q_4$ 元素的求解器实际上比四次方的情况略快。高阶多项式同样快，甚至比低阶多项式更快，这是通过和因子化进行无矩阵算子评估的主要优势之一，见<a
...
* 为了了解无矩阵实现的能力，我们通过测量问题初始化的计算时间（分配DoF、设置和装配矩阵、设置多网格结构），以及无矩阵变体和基于稀疏矩阵的变体的实际解法，来比较上述3D例子和基于稀疏矩阵的变体的性能。如上所示，我们将预处理程序建立在浮点数上，而实际的矩阵和向量建立在双数上。测试在英特尔Corei7-5500U笔记本处理器上运行（两个核心，支持<a
...
* 最后，我们注意到，在对上述大规模系统进行测试的同时，deal.II中的多网格算法的改进也被开发出来了。最初的版本包含了基于MGSmootherPrecondition的次优代码，其中一些MPI_Allreduce命令（检查所有向量条目是否为零）在每一级的平滑操作上进行，这只有在65k核以上才变得明显。然而，下面的图片显示，改进已经在较小的规模上得到了回报，这里显示的是对 $\mathcal Q_5$ 元素在多达14,336个核心上的计算。
...
* 如上所述，代码已准备好用于局部自适应h-精简。对于泊松方程，可以采用KellyErrorEstimator类中实现的Kelly误差指标。为了评估误差指标中的跳跃项，每个MPI进程都需要知道本地相关的DoFs.然而 MatrixFree::initialize_dof_vector() 函数只用一些本地相关的自由度初始化向量。向量中提供的鬼魂指数是一个严格的集合，只有那些在单元积分中被触及的指数（包括约束条件的解决）。]，如下图所示。
* @code
IndexSet locally_relevant_dofs;
DoFTools::extract_locally_relevant_dofs(dof_handler, locally_relevant_dofs);
...
* 这个程序只用MPI来并行化。作为一种选择，MatrixFreeloop也可以在混合模式下发布，例如，在一个集群的节点上使用MPI并行化，在一个节点的共享内存区域内通过Intel TBB使用线程。要使用这个方法，需要在主函数的MPI_InitFinalize数据结构中设置线程数，并设置 MatrixFree::AdditionalData::tasks_parallel_scheme topartition_color来实际进行并行循环。这个用例在  step-48  中进行了讨论。
* <a name="InhomogeneousDirichletboundaryconditions"></a><h4> Inhomogeneous Dirichlet boundary conditions </h4> 。
*

*所提出的程序假定了同质的Dirichlet边界条件。如果涉及到非均质条件，情况就比较复杂了。为了了解如何实现这样的设置，让我们首先回顾一下这些条件是如何在数学公式中出现的，以及它们是如何在基于矩阵的变体中实现的。从本质上讲，非均质迪里希特条件将解中的一些节点值设定为给定值，而不是通过变分原理来确定它们，@f{eqnarray*}
...
N_D$ 上的插值边界值。然后我们把这个解的表示插入到弱的形式中，例如上面显示的拉普拉斯，并把已知量移到右边：@f{eqnarray*}
...
* 在deal.II的实现中，右侧的积分 $(\nabla \varphi_i,\nabla \varphi_j)_\Omega$ 已经包含在我们在每个单元上组装的局部矩阵贡献中。当使用 AffineConstraints::distribute_local_to_global() 时，如 step-6 和 step-7 教程程序中首次描述的那样，我们可以通过将本地矩阵的列<i>j</i>和行<i>i</i>相乘来说明同质约束<i>j</i>的贡献。]的局部矩阵根据积分 $(\varphi_i,
...
@endcode
*
* 然后我们可以将向量 @p solution 传递给 @p  LaplaceOperator::vmult_add() 函数，并将新的贡献添加到 @p system_rhs向量中，该向量在 @p LaplaceProblem::assemble_rhs() 函数中被填充。然而，这个想法并不奏效，因为vmult()函数中使用的 FEEvaluation::read_dof_values() 调用假定所有约束条件的值都是同质的（否则算子就不是线性算子，而是仿生算子）。为了同时检索同质性的值，我们可以选择以下两种策略之一。
...
* FEEvaluation类有一个设施，正好解决了这个要求。对于非均质的Dirichlet值，我们确实希望在从向量 @p solution. 中读取数据时跳过对均质（Dirichlet）约束的隐式施加。例如，我们可以扩展 @p  LaplaceProblem::assemble_rhs() 函数来处理非均质的Dirichlet值，如下所示，假设Dirichlet值已被插值到对象 @p constraints: 中
* @code
template <int dim>
void LaplaceProblem<dim>::assemble_rhs()
{
...
* 获得重新使用 @p  LaplaceOperator::apply_add() 函数的第二个替代方法是添加第二个LaplaceOperator，跳过Dirichlet约束。为了做到这一点，我们初始化第二个MatrixFreeobject，它没有任何边界值约束。这个 @p matrix_free 对象然后被传递给一个 @p LaplaceOperator 类实例 @p inhomogeneous_operator，它只用于创建右手边。
* @code
template <int dim>
void LaplaceProblem<dim>::assemble_rhs()
{
