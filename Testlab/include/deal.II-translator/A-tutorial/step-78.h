...
* 布莱克-斯科尔斯方程是一个偏微分方程，有点超出了普通方案。它描述了一个 "欧罗巴 "股票期权的公平价格是多少。在不做过多解释的情况下，股票 "期权 "是一份可以从银行购买的合同，它允许我，但不要求我，在未来的固定时间 $T$ 以固定的价格 $K$ 购买特定的股票。那么，作为这种期权的买方要回答的问题是 "我认为这样的合约值多少钱？"，或者作为卖方 "我需要为这个合约收取多少钱？"，这既是合约在时间 $T$ 前的函数 $t<T$ ，也是股票价格的函数 $S$ 。费舍尔-布莱克和迈伦-斯科尔斯在假设股票价格表现出随机的价格波动，具有一定的 "波动性 "和背景的指数价格增长（可以认为是通货膨胀率，随着时间的推移，所有的货币都会贬值）的情况下，得出了这种期权的公平价格的偏微分方程 $V(S,t)$ 。由于他们的工作，布莱克和斯科尔斯在1997年获得了诺贝尔经济科学奖，这也是第一个处理有人获得诺贝尔奖的问题的教程程序 @cite black1973pricing  。
* 方程如下。
* @f{align*}{
...
* 第二，在截断域之后，我们需要问在这个现在的有限边界上我们应该设置什么边界值。为了解决这个问题，我们使用 "放-叫 "奇偶性  @cite stoll1969relationship  。一个 "拉动期权 "是指我们被允许，但不是必须，在未来的某个时间 $T$ 以价格 $K$ 向某人出售*股票。这说明
* @f{align*}{
...
* 像往常一样，（1）变成了 $-\textbf{M}V^n+\textbf{M}V^{n-1}$ ，（4）变成了 $k_n\left[-r(1-\theta)\textbf{M}V^{n-1}
* 
- \theta r\textbf{M}V^n\right]$ ，其中 $\textbf{M}_{i,j}=\left(\phi_i(S),\phi_j(S)\right)$ ，我们不仅用 $V$ 表示函数 $V(S)$ ，还用离散化后的节点值向量表示，这是一种自由。
* 有趣的部分来自于（2）和（3）。
*
...
* 
 @endcode
*
* 然后照例将这个程序的所有内容放入一个命名空间，并将deal.II命名空间导入我们将要工作的空间。我们还定义了一个标识符，以便在定义 <code>MMS</code> 时可以运行MMS代码。否则，该程序就会解决原来的问题。
*
...
*
* -  <code>maturity_time</code>  ：时间域的上限。这就是期权到期的时间。
*


*
* -  <code>asset_volatility</code>  ：股票价格的波动性./n
*
...
*
* -  <code>strike_price</code>  ：买方在到期时可选择购买股票的约定价格。   
* 本程序与 step-26 之间的一些细微差别是创建了 <code>a_matrix</code> and the <code>b_matrix</code> ，这在介绍中有所描述。然后，我们还需要存储当前时间，时间步长的大小，以及当前时间步长的数量。接下来，我们将把输出存储到 <code>DataOutStack</code> 变量中，因为我们将把每个时间的解分层在上面，形成解流形。然后，我们有一个变量来存储当前的周期和我们在计算解决方案时将运行的周期数。循环是给定一个网格的一个完整的解决方案计算。我们在每个周期之间细化一次网格，以展示我们程序的收敛特性。最后，我们将收敛数据存储到一个收敛表中。   
...
* 下面是创建非常数系数的拉普拉斯矩阵的代码。这与介绍中的矩阵D相对应。这个非恒定系数在 <code>current_coefficient</code> 变量中表示。
*
...
* 这是我们计算收敛和误差数据的地方，以评估程序的有效性。在这里，我们计算 $L^2$ 、 $H^1$ 和 $L^{\infty}$ 的准则。
*
...
* 
 @endcode
*
* 接下来，我们运行主循环，该循环一直运行到超过成熟时间。我们首先计算方程的右边，这在导言中已经描述过。回顾一下，它包含术语 $\left[-\frac{1}{4}k_n\sigma^2\mathbf{D}-k_nr\mathbf{M}+k_n\sigma^2
...
* 
 @endcode
*
* 第二块是计算源项的贡献。这对应于术语  $-k_n\left[\frac{1}{2}F^{n-1}
...
* 
 @endcode
*
* 接下来，我们将强迫项添加到来自时间步长的强迫项中，同时建立矩阵 $\left[\mathbf{M}+
...
*
*走到这一步，这个程序的主函数又没有什么好讨论的了：它看起来就像自 step-6 以来的所有此类函数。
*
