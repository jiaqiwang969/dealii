...
* 这个程序涉及一个非线性椭圆偏微分方程的例子，即[最小表面方程](https://en.wikipedia.org/wiki/Minimal_surface)。你可以想象这个方程的解描述一个肥皂膜所跨越的表面，这个肥皂膜被一个封闭的线环所包围。我们想象一下，这根线不仅仅是一个平面的环，而且是弯曲的。肥皂膜的表面张力将使该表面变成最小表面。最小表面方程的解描述了这种形状，并以金属丝的垂直位移为边界条件。为了简单起见，我们在这里假设曲面可以写成图形 $u=u(x,y)$ ，尽管很明显，要构造出钢丝弯曲的情况并不难，因为在这种情况下，曲面只能局部地被构造成图形，而不能整体地被构造成图形。
...
* 如上所述，我们用牛顿方法解决这个方程，我们从 $(n-1)$ 个近似解中计算出 $n$ 个近似解，并使用阻尼参数 $\alpha^n$ 来获得更好的全局收敛行为。
* @f{align*}
...
- \nabla \cdot \left( \frac{1}{\left(1+|\nabla u^{n}|^{2}\right)^{\frac{1}{2}}}
  ...
  @f]
* 为了解决最小表面方程，我们必须重复解决这个方程，每个牛顿步骤一次。要解决这个问题，我们必须看一下这个问题的边界条件。假设 $u^{n}$ 已经有了正确的边界值，那么牛顿更新 $\delta u^{n}$ 应该有零边界条件，以便在添加两者之后有正确的边界条件。  在第一个牛顿步骤中，我们从解 $u^{0}\equiv 0$ 开始，牛顿更新仍然需要向解 $u^{1}$ 提供正确的边界条件。
*

* 总而言之，我们必须在第一步用边界条件 $\delta
u^{0}=g$ 来解决上面的PDE，在接下来的所有步骤中用 $\delta u^{n}=0$ 来解决。
*  @note  在某种意义上，人们可能会说，如果程序已经实现了 $F(u)$ ，那么还必须实现 $F'(u,\delta)$ 就是重复的。像往常一样，重复会诱发错误，我们希望能避免它。虽然我们没有在这个程序中探讨这个问题，但我们会在下面的<a
  href="#extensions">Possibilities for extensions</a>部分的末尾再来讨论这个问题，特别是在 step-72 中。
*

*<a name="Weakformulationoftheproblem"></a><h3>Weak formulation of the problem</h3>


* 从上面的强公式开始，我们通过将PDE的两边都乘以一个检验函数 $\varphi$ 并对两边进行部分积分，得到弱公式。  @f[
...
-\left(\nabla \varphi , \frac{1}{\left(1+|\nabla u^{n}|^{2}\right)^{\frac{1}{2}}} \nabla u^{n}
   \right).
  @f]这里的解 $\delta u^{n}$ 是 $H^{1}(\Omega)$ 中的一个函数，受制于上面讨论的边界条件。把这个空间缩小到一个有基数 $\left\{
\varphi_{0},\dots , \varphi_{N-1}\right\}$ 的有限维空间，我们可以写出这个解。
* @f[
  \delta u^{n}=\sum_{j=0}^{N-1} \delta U_{j} \varphi_{j}.
@f]
* 使用基函数作为测试函数并定义 $a_{n} \dealcoloneq \frac{1}
{\sqrt{1+|\nabla u^{n}|^{2}}}$ ，我们可以重写弱的公式。
* @f[
...
@f]
* 其中解 $\delta u^{n}$ 是由系数 $\delta U^{n}_{j}$ 给出的。这个线性方程组可以改写为： * @f[
  A^{n}\; \delta U^{n}=b^{n},
@f] 。
* @f[
  A^{n}\; \delta U^{n}=b^{n},
...
* 与上述牛顿步骤相对应的矩阵可以被重新表述，以更好地显示其结构。稍微改写一下，我们可以得到它的形式@f[
  A_{ij}
  =
  \left(
    \nabla \varphi_i,
    ...
    \nabla \varphi_j
  \right),
@f]，其中矩阵 $B$ （在 $d$ 空间维度上大小为 $d \times d$ ）由以下表达式给出：@f[
  B
  =
  a_n \left\{
   \mathbf I
...
-
  ...
  \frac{\nabla u_n}{\sqrt{1+|\nabla u^{n}|^{2}}}
  \right\}.
@f]另一方面， $B$ 也是正定的，这使 $A$ 具有相同的属性。这可以通过注意到向量 $v_1 =
\frac{\nabla u^n}{|\nabla u^n|}$ 是 $B$ 的特征向量，其特征值为 $\lambda_1=a_n \left(1-\frac{|\nabla u^n|^2}{1+|\nabla u^n|^2}\right) > 0$ ，而所有与 $v_1$ 垂直的向量 $v_2\ldots v_d$ 和彼此都是特征向量，其特征值为 $a_n$ 。因为所有的特征值都是正的，所以 $B$ 是正定的， $A$ 也是。因此，我们可以使用CG方法来解决牛顿步骤。（矩阵 $A$ 是对称和正定的这一事实不应该让人惊讶。它来自于对一个算子的导数，而这个算子又来自于对一个能量函数的导数：最小表面方程只是最小化了一些非二次能量。因此，作为标量能量的二阶导数矩阵，牛顿矩阵必须是对称的，因为与 $i$ th和 $j$ th自由度有关的导数显然应该相减。同样，如果能量函数是凸的，那么第二导数的矩阵必须是正定的，上面的直接计算只是重申了这一点。)
* 然而，值得注意的是，在 $\nabla u$ 变大的问题中，正定性会退化。换句话说，如果我们简单地将所有边界值乘以2，那么一阶 $u$ 和 $\nabla u$ 也将被乘以2，但结果是 $B$ 的最小特征值会变小，矩阵会变得更加不稳定。更具体地说，对于 $|\nabla u^n|\rightarrow\infty$ ，我们有 $\lambda_1 \propto a_n \frac{1}{|\nabla u^n|^2}$ 而 $\lambda_2\ldots \lambda_d=a_n$ ；因此， $B$ 的条件数，也就是 $A$ 的条件数的一个乘法因子，会像 ${\cal O}(|\nabla u^n|^2)$ 一样增长）。用目前的程序可以很简单地验证，将目前程序中使用的边界值乘以越来越大的数值，会导致问题最终无法用我们这里使用的简单预设条件的CG方法解决。
*

*<a name="Choiceofsteplengthandglobalization"></a><h3> Choice of step length and globalization </h3>


* 如上所述，牛顿方法是通过计算一个方向 $\delta u^n$ ，然后以步长 $0 < \alpha^n \le 1$ 进行更新 $u^{n+1} = u^{n}+\alpha^n
\delta u^{n}$ 。一般来说，对于强非线性模型，如果我们总是选择 $\alpha^n=1$ ，牛顿方法就不会收敛，除非我们从足够接近非线性问题的解 $u$ 的初始猜测 $u^0$ 开始。在实践中，我们并不总是有这样的初始猜测，因此采取完整的牛顿步骤（即使用 $\alpha=1$ ）往往是不可行的。
* 因此，一个常见的策略是，当迭代 $u^n$ 离解 $u$ 还很远时，在前几步使用较小的步长，随着我们的接近，使用较大的 $\alpha^n$ 值，直到我们最终可以开始使用全步 $\alpha^n=1$ ，因为我们已经足够接近解。当然，问题是如何选择 $\alpha^n$ 。基本上有两种广泛使用的方法：线条搜索法和信任区域法。
* 在这个程序中，我们简单地选择步长等于0.1。这确保了在手头的测试案例中，我们确实得到了收敛，尽管很明显，由于没有最终恢复到全步长，我们放弃了使牛顿方法如此吸引人的快速、二次收敛。显然，如果这个程序是为了解决更多的现实问题，那么这一点最终是必须解决的。我们将在<a href="#Results">results section</a>中进一步评论这个问题，并在 step-77 中使用一个更好的方法。
*

*<a name="Summaryofthealgorithmandtestcase"></a><h3> Summary of the algorithm and testcase </h3>


* 总的来说，我们这里的程序在许多方面与 step-6 并无不同。主类的布局本质上是相同的。另一方面， <code>run()</code> 函数中的驱动算法是不同的，其工作方式如下。 <ol>   <li>  从函数 $u^{0}\equiv 0$ 开始，以这样的方式修改它，使 $u^0$ 的值沿边界等于正确的边界值 $g$ （这发生在 <code>MinimalSurfaceProblem::set_boundary_values</code> ）。设 $n=0$  . </li> 
*  <li>  通过在 $\partial \Omega$ 上解决带有边界条件 $\delta u^{n}=0$ 的系统 $A^{n}\;\delta
  U^{n}=b^{n}$ 来计算牛顿更新。 </li> 
*  <li>  计算一个步长  $\alpha^n$  。在这个程序中，我们总是设置  $\alpha^n=0.1$  。为了便于以后的扩展，这发生在一个自己的函数中，即在  <code>MinimalSurfaceProblem::determine_step_length</code>  中。 (总是选择  $\alpha^n=0.1$  的策略当然不是最优的)
*
* - 我们应该选择一个适用于给定搜索方向的步长
*
* - 但要做到这一点需要做一些工作。最后，我们把这类事情留给外部软件包。   step-77 就是这样做的）。) </li> 
*  <li>  解决方案的新近似值由  $u^{n+1}=u^{n}+\alpha^n \delta u^{n}$  给出。 </li> 
*  <li>  如果 $n$ 是5的倍数，那么细化网格，将解 $u^{n+1}$ 转移到新的网格中，并设置 $u^{n+1}$ 的值，以便沿边界有 $u^{n+1}|_{\partial\Gamma}=g$ （同样在 <code>MinimalSurfaceProblem::set_boundary_values</code> ）。请注意，这并不是自动保证的，即使在网格细化之前我们就已经有了 $u^{n+1}|_{\partial\Gamma}=g$ ，因为网格细化会在网格中增加新的节点，当我们把旧的解决方案带到新的网格时，必须把旧的解决方案插值到新的节点。我们通过插值选择的数值可能接近于精确的边界条件，但一般来说，并不是正确的数值。 </li> 
*  <li>  设置 $n\leftarrow n+1$ 并转到第二步。 </li>   </ol> .
* 我们解决的测试案例选择如下。我们寻求在单位盘 $\Omega=\{\mathbf x: \|\mathbf
x\|<1\}\subset {\mathbb R}^2$ 上找到最小曲面的解决方案，其中该曲面沿边界达到 $u(x,y)|{\partial\Omega} = g(x,y) \dealcoloneq \sin(2 \pi (x+y))$ 的值。
*
...
*
* -  <code>setup_system</code> 函数需要一个参数，表示这是否是第一次调用它。不同的是，第一次我们需要分配自由度并将 $u^n$ 的解向量设置为正确的大小。在接下来的时间里，这个函数是在我们已经完成了这些步骤，作为细化网格的一部分之后被调用的  <code>refine_mesh</code>  。
*


*
* - 然后我们还需要新的函数。   <code>set_boundary_values()</code> 负责正确设置解向量的边界值，正如在介绍的最后所讨论的。   <code>compute_residual()</code> 是一个计算非线性（离散）残差准则的函数。我们用这个函数来监测牛顿迭代的收敛性。该函数以步长 $\alpha^n$ 为参数来计算 $u^n + \alpha^n
 \; \delta u^n$ 的残差。这是人们通常需要的步长控制，尽管我们在这里不会使用这个功能。最后， <code>determine_step_length()</code> 计算每个牛顿迭代中的步长 $\alpha^n$ 。正如介绍中所讨论的，我们在这里使用一个固定的步长，并把实现一个更好的策略作为一个练习。(  step-77 的做法不同。它只是在整个求解过程中使用了一个外部软件包，而一个好的线搜索策略是该软件包提供的一部分)。
*
...
*
* 边界条件的实现与  step-4  中一样。  它被选择为  $g(x,y)=\sin(2 \pi (x+y))$  。
*
...
*
* 在setup-system函数中，我们总是设置有限元方法的变量。与 step-6 有相同的区别，因为在那里我们在每个细化周期中都要从头开始求解PDE，而在这里我们需要将前一个网格的解放到当前的网格中。因此，我们不能只是重置解向量。因此，传递给这个函数的参数表明我们是否可以分布自由度（加上计算约束）并将解向量设置为零，或者这在其他地方已经发生过了（具体来说，在 <code>refine_mesh()</code> ）。
*
...
* 
 @endcode
*
* 对于线性系统的组装，我们必须在正交点上获得先前解的梯度值。有一个标准的方法： FEValues::get_function_gradients 函数接收一个代表定义在DoFHandler上的有限元场的向量，并评估这个场在FEValues对象最后被重新初始化的单元的正交点的梯度。然后将所有正交点的梯度值写入第二个参数中。
*
...
* 
 @endcode
*
* 有了这个，我们就可以对所有的正交点和形状函数进行积分循环。  在刚刚计算了正交点上旧解的梯度之后，我们就可以计算这些点上的系数 $a_{n}$ 。  然后，系统本身的组装看起来与我们一贯的做法相似，除了非线性项之外，将结果从局部对象复制到全局对象中也是如此。
*
...
* 
 @endcode
*
* 最后，我们从系统中移除悬挂的节点，并将零边界值应用于定义牛顿更新的线性系统  $\delta u^n$  。
*
...
*
* 这个函数的第一部分与 step-6 中的相同 ...然而，在细化网格之后，我们必须将旧的解决方案转移到新的解决方案上，这一点我们要借助于SolutionTransfer类来完成。这个过程稍微有点复杂，所以让我们详细描述一下。
*
...
* 
 @endcode
*
* 那么我们就需要一个额外的步骤：比如说，如果你标记了一个比它的邻居多精炼一次的单元，而这个邻居没有被标记为精炼，那么我们就会在一个单元界面上出现两个精炼级别的跳跃。  为了避免这些情况，库将默默地也要对邻居单元进行一次细化。它通过在实际进行细化和粗化之前调用 Triangulation::prepare_coarsening_and_refinement 函数来实现。  这个函数标志着一组额外的单元格进行细化或粗化，以执行像单悬节点规则这样的规则。  调用此函数后，被标记为细化和粗化的单元格正是那些将被实际细化或粗化的单元格。通常情况下，你不需要用手去做这件事 (Triangulation::execute_coarsening_and_refinement 为你做这件事)。然而，我们需要初始化SolutionTransfer类，它需要知道最终将被粗化或细化的单元集，以便存储旧网格的数据并转移到新网格。因此，我们用手调用这个函数。
*
...
* 
 @endcode
*
* 一旦我们有了内插的解决方案和所有关于悬挂节点的信息，我们必须确保我们现在拥有的 $u^n$ 实际上具有正确的边界值。正如在介绍的最后所解释的那样，即使细化前的解具有正确的边界值，也不会自动出现这种情况，因此我们必须明确地确保它现在具有。
*
...
*
* 为了监测收敛性，我们需要一种方法来计算（离散）残差的规范，即介绍中讨论的向量 $\left<F(u^n),\varphi_i\right>$ 与 $F(u)=-\nabla \cdot \left(
 \frac{1}{\sqrt{1+|\nabla u|^{2}}}\nabla u \right)$ 的规范。事实证明，（尽管我们在当前版本的程序中没有使用这个功能）在确定最佳步长时需要计算残差 $\left<F(u^n+\alpha^n\;\delta u^n),\varphi_i\right>$ ，因此这就是我们在这里实现的：该函数将步长 $\alpha^n$ 作为一个参数。原有的功能当然是通过传递一个零作为参数来获得。   
...
* 
 @endcode
*
* 实际的计算与 <code>assemble_system()</code> 中的差不多。我们首先评估 $u^n+\alpha^n\,\delta u^n$ 在正交点的梯度，然后计算系数 $a_n$ ，然后将其全部插入残差公式中。
*
...
* 正如介绍中所讨论的，如果我们总是采取全步，即计算 $u^{n+1}=u^n+\delta
 u^n$ ，牛顿方法经常不收敛。相反，我们需要一个阻尼参数（步长）  $\alpha^n$  并设置  $u^{n+1}=u^n+\alpha^n\delta u^n$  。这个函数就是用来计算  $\alpha^n$  的。   
* 在这里，我们干脆总是返回0.1。这当然是一个次优的选择：理想情况下，人们希望的是，当我们越来越接近解决方案时，步长变为1，这样我们就可以享受牛顿方法的快速二次收敛。我们将在下面的结果部分讨论更好的策略， step-77 也涉及这方面的内容。
*
...
* 正如介绍中所描述的，领域是围绕原点的单位盘，创建方式与 step-6 中所示相同。网格被全局细化了两次，后来又进行了几次自适应循环。   
...
* 
 @endcode
*
* 接下来开始牛顿迭代。我们一直迭代到上一次迭代结束时计算的残差（准则）小于 $10^{-3}$ ，正如在 "do{ ...} while "循环的末尾进行检查。因为我们没有一个合理的值来初始化这个变量，我们只是使用可以表示为 "双数 "的最大值。
*
...
* 在每一个牛顿步骤中，首先要计算系统矩阵和右手边，然后我们存储右手边的准则作为残差，以便在决定是否停止迭代时进行检查。然后我们求解线性系统（该函数也会更新 $u^{n+1}=u^n+\alpha^n\;\delta u^n$ ），并在这个牛顿步骤结束时输出残差的准则。         
...
* 牛顿方法有两个众所周知的特性。
*
* - 它不可能从任意选择的起点收敛。相反，一个起始点必须足够接近解决方案，以保证收敛。然而，我们可以通过使用<i>step length</i> 0<  $\alpha^n\le
  1$  的阻尼迭代来扩大牛顿方法的收敛区域。
*
* - 如果(i)步长选择为 $\alpha^n=1$ ，它表现出快速的二次收敛，并且(ii)事实上它在选择步长时确实收敛了。
* 这两个观察的结果是，一个成功的策略是在初始迭代中选择 $\alpha^n<1$ ，直到迭代足够接近，可以用全步长收敛，在这一点上，我们希望切换到 $\alpha^n=1$  。问题是如何以自动的方式选择 $\alpha^n$ ，以满足这些标准。
* 我们不想在这里回顾关于这个主题的文献，只是简单地提到有两种基本的方法来解决这个问题：回溯线搜索和信任区域方法。前者更广泛地用于部分微分方程，基本上做了以下工作。
*
* - 计算一个搜索方向
*
* - 看看 $u^n + \alpha^n\;\delta u^n$ 与 $\alpha^n=1$ 产生的残差是否比 $u^n$ 单独产生的残差 "大大缩小"。
*
* - 如果是，则取 $\alpha^n=1$ 。
*
* - 如果不是，试试用 $\alpha^n=2/3$ 的残余量是否 "大大减少"。
*
* - 如果是，那么就采取 $\alpha^n=2/3$  .
*
* - 如果不是，试试用 $\alpha^n=(2/3)^2$ 是否 "大大缩小 "了残差。
*
* - 等等。当然，我们可以选择其他因素 $r, r^2, \ldots$ ，而不是上面选择的 $2/3,
(2/3)^2, \ldots$ ，为 $0<r<1$  。很明显，"回溯 "一词的来源是：我们尝试一个长的步骤，但如果这不起作用，我们就尝试一个更短的步骤，而且是越来越短的步骤，等等。函数 <code>determine_step_length()</code> 的编写方式正是为了支持这种使用情况。
* 我们是否接受一个特定的步长 $\alpha^n$ ，取决于我们如何定义 "相当小"。有许多方法可以做到这一点，但在不详细说明的情况下，我们只需提到最常见的方法是使用沃尔夫和阿米约-戈尔德斯坦条件。对于这些，我们可以证明以下几点。
*
* - 总有一个步长 $\alpha^n$ 可以满足条件，也就是说，只要问题是凸的，迭代就不会被卡住。
*
...
* 关于全球化方法的更多细节，包括回溯，可以在  @cite GNS08  和  @cite NW99  中找到。
* 然而，非常值得一提的是，在实践中，高效非线性求解器的实现与高效有限元方法的实现一样复杂。我们不应该试图通过自己实现所有的必要步骤来重新发明车轮。在 LineMinimization::line_search() 函数中已经有了大量的拼图，可以用来实现这个目的。但是，相反，就像在deal.II这样的库中构建有限元求解器一样，我们应该在[SUNDIALS](https://computing.llnl.gov/projects/sundials)这样的库中构建非线性求解器。事实上，deal.II有与SUNDIALS的接口，特别是通过 SUNDIALS::KINSOL 类与它的非线性求解器子包KINSOL的接口。将目前的问题建立在该接口上并不困难。
*
* - 事实上，这就是 step-77 的作用。
*
...
-\nabla \cdot \left( \frac{1}{\sqrt{1+|\nabla u|^{2}}}\nabla u \right)
    = 0
  @f]的非线性问题时，我们使用牛顿迭代，要求我们反复解决线性偏微分方程。
* @f{align*}
    F'(u^{n},\delta u^{n}) &=- F(u^{n})
  @f}
* 这样我们就可以计算出更新
* @f{align*}
    u^{n+1}&=u^{n}+\alpha^n \delta u^{n}
  @f}
*与牛顿步骤的解决方案 $\delta u^{n}$ 。对于这里的问题，我们可以用手计算导数 $F'(u,\delta u)$ ，并得到@f[
  F'(u,\delta u)
  =
* 
...
- \nabla \cdot \left( \frac{1}{\left(1+|\nabla u|^{2}\right)^{\frac{1}{2}}}\nabla
  ...
  \right).
  @f]，但这已经是一个相当大的表达式，无论是操作还是实现都很麻烦。在某种意义上，它也是重复的。如果我们在代码的某个地方实现了 $F(u)$ ，那么 $F'(u,\delta u)$ 就不是一个独立的信息，而是，至少在原则上，计算机应该能够自己推断出来的东西了。也就是说，如果我们只需要实现 $F(u)$ ，而 $F'(u,\delta u)$ 是以某种方式隐含地完成的，那不是很好吗？这实际上是可能的，并以 "自动区分 "的名义运行。   step-71 概括地讨论了这个概念， step-72 说明了如何在实践中应用这个概念来解决我们在这里考虑的问题。
*
