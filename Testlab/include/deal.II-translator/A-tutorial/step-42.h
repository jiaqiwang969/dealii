 ...
 * 本教程依赖于  step-41  ,  step-40  。 @htmlonly <table
 ...
 * 这个例子是 step-41
 * 的扩展，考虑的是三维接触问题，其材料行为是各向同性的硬化。换句话说，它考虑的是，如果把一个刚性的障碍物推到三维体上（接触问题），其变形受弹塑性材料法则（一种只能容纳一定最大应力的材料）支配，随着变形的累积而硬化。为了说明我们打算做什么，在讨论太多细节之前，让我们先看看解决方案是什么样子的（可变形体是一个立方体
 *
 * -实际上只显示了一半的内容
 *
 * - 障碍物对应于一个汉字，将在下面讨论）。)
 ...
 * alt="">
 *
 *  这个问题的描述意味着，与 step-41
 * 相比，我们必须照顾到一个额外的非线性因素：材料行为。由于我们在这里考虑的是一个三维问题，我们还必须考虑到一个事实，即现在接触区是在可变形体的边界，而不是在内部。最后，与
 * step-41
 * 相比，在处理线性系统和不等式约束时，我们还必须处理悬挂节点的问题，因为我们希望使用自适应网格；在后一种情况下，我们必须处理悬挂节点的约束还是不等式的约束更重要的优先次序。
 * 由于在三维空间中很容易达到几百万个自由度，即使使用自适应网格细化，我们决定使用Trilinos和p4est来并行运行我们的代码，在
 * step-40 的框架上进行并行化。其他并行化的指针可以在
 * step-32 中找到。
 *
 ...
 * @f}
 * 这里，这些方程中的第一个定义了应变 $\varepsilon(\mathbf
 * u)=\frac{1}{2}\left(\nabla \mathbf u + \nabla \mathbf u^T\right)$ 和应力
 * $\sigma$ 之间的关系，即四阶顺应性张量 $A$ ； $\varepsilon^p$
 * 提供了应变的塑性成分，以确保应力不超过屈服应力。我们将只考虑各向同性的材料，对于这些材料，
 * $A$ 可以用Lam&eacute;模量 $\lambda$ 和 $\mu$
 * 表示，或者用体模量 $\kappa$ 和 $\mu$
 * 表示。第二个方程是力平衡；我们将不考虑任何体力，因此假设
 * $\mathbf f=0$  。第三行的互补条件意味着如果
 * $\mathcal{F}(\sigma)< 0$ ，则 $\varepsilon^p=0$ ，但如果且仅如果
 * $\mathcal{F}(\sigma) = 0$ ，则 $\varepsilon^p$
 * 可能是一个非零张量，特别是在这种情况下，
 * $\varepsilon^p$ 必须指向 $\partial \mathcal{F}(\sigma)/\partial \sigma$
 * 方向。不等式 $\mathcal{F}(\sigma)\le 0$
 * 是塑性材料只能支持有限的应力；换句话说，如果外力会导致应力
 * $\sigma$ ，而 $\mathcal{F}(\sigma)> 0$ 会导致塑性变形
 * $\varepsilon^p$ ，它们就会发生反应。这种<i>yield
 * function</i>的典型形式是
 * $\mathcal{F}(\sigma)=|\sigma^D|-\sigma_{\text{yield}}$ ，其中 $\tau^D =
 * \tau
 *
 * - \dfrac{1}{3}tr(\tau)I$ 是张量的偏离部分， $|\cdot|$
 * 表示Frobenius规范。 进一步的方程描述了在 $\Gamma_D$
 * 上的固定的零位移，在可能出现接触的表面
 * $\Gamma_C=\partial\Omega\backslash\Gamma_D$
 * 上，障碍物施加的法向力 $\sigma_n=\mathbf n \cdot (\sigma(\mathbf
 * u) \mathbf n)$ 是向内的（障碍物对我们身体没有
 * "拉"），切向分量为零 $\mathbf \sigma_t= \sigma \mathbf n
 *
 * - \mathbf \sigma_n \mathbf n = \sigma \mathbf n
 *
 * - [\mathbf n \cdot(\sigma \mathbf n)]\mathbf n$
 * 最后一个条件也是一个互补条件，它意味着在 $\Gamma_C$
 * 上，只有当身体与障碍物接触时，法向力才可能是非零的；第二部分描述了障碍物和身体的不可穿透性。
 * 大多数材料
 *
 * - 特别是金属
 *
 * - 具有这样的特性：它们在变形后会出现一些硬化。换句话说， $\sigma_{\text{yield}}$ 随着变形而增加。实际上，导致硬化的不是弹性变形，而是塑性成分。有不同的构成规律来描述这些材料行为。最简单的是线性各向同性硬化，由流动函数 $\mathcal{F}(\sigma,\varepsilon^p) = \vert\sigma^D\vert
 *
 * - (\sigma_0 + \gamma^{\text{iso}}|\varepsilon^p|)$ 描述。
 *
 *  <a name="Reformulationasavariationalinequality"></a><h3>Reformulation as a
 * variational inequality</h3>
 * 一般来说，处理不等式是相当笨拙的。在这里，我们必须处理两个问题：塑性和接触问题。正如本页顶部提到的论文中详细描述的那样，至少可以重新表述塑性，使其看起来像一个非线性，然后我们可以用牛顿方法处理。这在数学上略显棘手，因为非线性不只是一些平滑的函数，而是在应力达到屈服应力的地方有结点；然而，对于这样的<i>semismooth</i>函数，可以证明牛顿方法仍然收敛。
 * 在不详细说明的情况下，我们也将摆脱应力作为一个独立变量，而只用位移
 * $\mathbf u$
 * 来工作。最终，这个重构的目标是我们希望最终得到一个对称的、正定的问题
 *
 * - 如一个线性化的弹性问题，其空间可变系数由塑性行为产生。
 *
 * - 需要在每个牛顿步骤中解决。我们希望如此，因为有高效和可扩展的方法来解决这样的线性系统，如用代数多重网格的CG预处理。这与类似于混合拉普拉斯的鞍点问题（见 step-20 ）相反，如果我们继续使用包含位移和应力的混合公式，就会得到鞍点问题，对于这个问题， step-20 已经提示了构建良好的求解器和预处理器是多么困难。
 * 说到这里，让我们简单陈述一下重构后得到的问题（同样，细节可以在论文中找到）。找到一个位移
 * $\mathbf u \in V^+$ ，以便
 * @f{align*}
 ...
 *
 * 在实际代码中，我们将使用缩写
 * $\gamma=\dfrac{\gamma^{\text{iso}}}{2\mu + \gamma^{\text{iso}}}$  。
 * 鉴于这种表述，我们将应用两种技术。
 *
 * - 运行一个牛顿方法来迭代出投影仪中的非线性。
 *
 * - 为接触条件运行一个主动集方法，其方式与我们在  step-41  中所做的基本相同。
 ...
 * 如前所述，我们将通过应用牛顿方法来处理算子 $P_\Pi$
 * 的非线性问题，尽管该算子在严格意义上是不可微的。然而，它满足<i>slant</i>的可微条件，这就足以让牛顿方法发挥作用。由此产生的方法被称为<i>semi-smooth
 * Newton
 * method</i>，听起来令人印象深刻，但实际上只是将牛顿方法应用于具有适当
 * "导数 "的半光滑函数。
 * 在当前情况下，我们将通过在每次迭代中求解 $i$
 * 来运行我们的迭代，即以下方程（仍然是一个不等式，但已线性化）。
 * @f{align*}
 ...
 * @f}
 * 其中等级4的张量 $I_\Pi=I_\Pi(\varepsilon^D(\mathbf u^{i-1}))$
 * 由以下公式给出
 * @f{align}
 * I_\Pi = \begin{cases}
 ...
 * \end{cases}
 * @f}
 * 这个张量是 $P_\Pi(C\cdot)$ 围绕 $\varepsilon^D(\mathbf u^{i-1})$ 的（形式）线性化。对于我们这里考虑的线性各向同性材料，投影仪的体积和剪切成分由@f{gather*}
 ...
 * 请注意，这个问题对应于一个线性弹性接触问题，其中
 * $I_\Pi$ 起到弹性张量 $C=A^{-1}$
 * 的作用。事实上，如果材料在某一点上没有塑性，那么
 * $I_\Pi=C$  。然而，在材料具有塑性的地方， $I_\Pi$
 * 是一个空间变化的函数。在任何情况下，我们必须为牛顿迭代
 * $\tilde {\mathbf u}^{i}$
 * 求解的系统使我们更接近重写问题的目标，使我们能够使用众所周知的求解器和预处理器来处理椭圆系统。
 * 作为对牛顿方法的最后说明，让我们提到，与牛顿方法一样，我们需要通过控制步长使其全球化。换句话说，虽然上面的系统求解的是
 * $\tilde {\mathbf u}^{i}$ ，但最终的迭代结果将是
 * @f{align*}
 * {\mathbf u}^{i} = {\mathbf u}^{i-1} + \alpha_i (\tilde {\mathbf u}^{i}
 *
 * - {\mathbf u}^{i-1})
 * @f}
 * 其中右侧括号中的差值代表了传统的牛顿方向，即 $\delta
 * {\mathbf u}^{i}$ 。我们将使用标准的直线搜索来确定
 * $\alpha^i$ 。
 *
 *  <a name="ActiveSetmethodstosolvethesaddlepointproblem"></a><h3>Active Set
 * methods to solve the saddle point problem</h3>
 * 这个在每个牛顿步骤中要解决的线性化问题基本上与
 * step-41
 * 相似。唯一的区别在于接触区是在边界而不是在域中。但这没有进一步的后果，所以我们参考文献
 * step-41 ，唯一的提示是 $\mathcal{S}$ 包含了接触边界
 * $\Gamma_C$
 * 上的所有顶点的时间。如同那里，我们需要做的是保持自由度的一个子集的固定，导致额外的约束，可以写成鞍点问题。然而，正如论文中所讨论的，通过以适当的方式写这些约束，消除自由度之间的耦合，我们最终得到一组节点，基本上只是有Dirichlet值附加在它们身上。
 *
 *  <a name="Overallalgorithm"></a><h3>Overall algorithm</h3>
 * 上述算法结合了阻尼半光滑牛顿法（我们用于非线性构成法）和半光滑牛顿法，用于接触。它的工作原理如下。 <ol>   <li>  初始化主动集和非主动集 $\mathcal{A}_i$ 和 $\mathcal{F}_i$ ，使得 $\mathcal{S} = \mathcal{A}_i \cup \mathcal{F}_i$ 和 $\mathcal{A}_i \cap
 * \mathcal{F}_i = \emptyset$ 和设置 $i = 1$  。这里， $\mathcal{S}$
 * 是位于可能发生接触的域的表面的所有自由度的集合。起始值
 * $\hat U^0 \dealcoloneq P_{\mathcal{A}_k}(0)$
 * 满足我们的障碍条件，也就是说，我们将初始零位移投射到可行位移集合上。
 * <li>  组建牛顿矩阵  $A_{pq} \dealcoloneq a'(
 * U^{i-1};\varphi_p,\varphi_q)$  和右手边  $F(\hat U^{i-1})$
 * 。这些对应于线性化的牛顿步骤，暂时忽略了接触不等式。
 * <li>  找到满足以下条件的主-偶对 $(\tilde U^i,\Lambda^i)$ ： 1.
 * @f{align*}
 * A\tilde U^i + B\Lambda^i & = F, &\\
 * \left[B^T\tilde U^i\right]_p & = G_p & \forall p\in\mathcal{A}_i,\\
 * \Lambda^i_p & = 0 & \forall p\in\mathcal{F}_i.
 * @f}
 * 如同 step-41 ，我们可以通过从第一个方程中消除 ${\cal
 * A}_i$
 * 中的那些自由度来得到这个问题的解决方案，并得到一个线性系统
 * $\hat {\hat A}(U^{i-1}) \tilde U^i = \hat {\hat H}(U^{i-1})$ 。
 *
 * <li>  通过应用直线搜索和计算 $U^{i-1}$ 和 $\tilde U^i$
 * 的线性组合来抑制 $i>2$ 的牛顿迭代。这需要找到一个
 * $\alpha^i_l \dealcoloneq 2^{-l},(l=0,\ldots,10)$ ，以便@f{gather*}U^i
 * \dealcoloneq \alpha^i_l\bar U^i + (1-\alpha^i_l)U^{i-1}@f}
 * 满足@f{gather*}
 * \vert {\hat R}\left({\mathbf u}^{i}\right) \vert < \vert {\hat R}\left({\mathbf u}^{i-1}\right) \vert.
 * \f}与 ${\hat R}\left({\mathbf u}\right)=\left(P_{Pi}(C\varepsilon(u)),\varepsilon(\varphi^{i}_p\right)$ ，除了(i)我们设置 ${\hat R}\left({\mathbf u}\right)=0$ 的元素 $p\in\mathcal{A}_i$ ，和(ii)对应悬挂节点的元素，我们以通常的方式消除。
 *  <li>  通过@f{gather*}\mathcal{A}_{i+1} \dealcoloneq \lbrace p\in\mathcal{S}:\Lambda^i_p +
 * c\left(\left[B^TU^i\right]_p
 *
 * - G_p\right) > 0\rbrace,@f}定义新的活动和非活动集。
 * @f{gather*}\mathcal{F}_{i+1} \dealcoloneq \lbrace p\in\mathcal{S}:\Lambda^i_p +
 * c\left(\left[B^TU^i\right]_p
 *
 * - G_p\right) \leq 0\rbrace.@f}
 * <li>  项目 $U^i$ ，使其满足接触不等式，@f{gather*}\hat U^i
 * \dealcoloneq P_{\mathcal{A}_{i+1}}(U^i).@f} 。
 ...
 * 在该算法的第3步中，矩阵 $B\in\mathbb{R}^{n\times m}$ 、 $n>m$ 描述了位移和拉格朗日乘数（接触力）的基数的耦合，在我们的情况下它不是二次的，因为 $\Lambda^k$ 只定义在 $\Gamma_C$ 上，即可能发生接触的表面。如本文所示，我们可以选择 $B$ 是一个每行只有一个条目的矩阵，（另见H&uuml;eber, Wohlmuth:A primal-dual activeset strategy for non-linear multibody contact problems, Comput.MECH.Engrg.194, 2005, pp.Engrg.194, 2005, pp.3147-3166）。向量 $G$ 是由间隙 $g$ @f{gather*}G_p = \begin{cases}
 * g_{h,p}, & \text{if}\quad p\in\mathcal{S}\\
 * 0, & \text{if}\quad p\notin\mathcal{S}.
 * \end{cases}@f}的合适近似值 $g_h$ 定义的。]
 *
 *
 *  <a name="Adaptivemeshrefinement"></a><h3>Adaptive mesh refinement</h3>
 * 由于我们的程序是在三维空间中运行的，所以程序所进行的计算是很昂贵的。因此，使用自适应网格细化是在可接受的运行时间内的一个重要步骤。为了使我们的生活更轻松，我们简单地选择已经在deal.II中实现的KellyErrorEstimator。我们把包含位移
 * $u$
 * 的解决向量交给它。正如我们将在结果中看到的，它产生了一个相当合理的接触区和塑性的自适应网格。
 *
 *  <a name="Implementation"></a><h3>Implementation</h3>
 * 本教程基本上是 step-40 和 step-41
 * 的混合体，但我们不使用PETSc，而是让Trilinos库来处理线性代数的并行化问题（就像
 * step-32 中那样）。由于我们试图解决一个类似于 step-41
 * 的问题，我们将使用同样的方法，但现在是并行的。
 * 一个困难是如何处理来自Dirichlet条件的约束，悬挂的节点和接触产生的不平等条件。为此，我们创建了三个AffineConstraints类型的对象，它们描述了各种约束条件，我们将在每次迭代中适当地将它们组合起来。
 * 与 step-41 相比，本程序有几个新的类。
 * <ul>   <li>   <code>ConstitutiveLaw</code>  描述材料的塑性行为。
 * <li>   <code>SphereObstacle</code>
 * 描述了一个球体，它作为障碍物被推入可变形的弹塑性体。
 * 是用这个还是下一个类来描述障碍物，由输入参数文件决定。
 * <li>   <code>ChineseObstacle</code>
 * （和一个辅助类）是一个允许我们从文件中读入障碍物的类。在我们将在结果部分展示的例子中，这个文件将是
 * <code>'obstacle_file.dat'</code>
 * ，并对应于显示力或功率的中文、日文或韩文符号的数据（见http://www.orientaloutpost.com/
 * : "这个词可以用于激励
 *
 ...
 * 除此之外，我们只对以下几个方面进行评论。 <ul>   <li>  程序允许你通过参数文件从两个不同的粗略网格中选择。这些是立方体 $[0,1]^3$ 或半球体，其开放面朝向正方向 $z$ 。
 * <li> 在这两种情况下，我们将假定惯例，即可能与障碍物接触的边界部分具有边界指标一。对于这两种网格，我们假定这是一个自由表面，即身体要么在那里接触，要么没有力作用在它身上。对于半球体，弯曲部分的边界指标为零，我们在那里施加零位移。对于盒子，我们沿底部施加零位移，但允许沿边的垂直位移（尽管没有水平位移）。 </ul>
 *
 ...
 *
 * @endcode
 * 最后，我们包括两个系统头文件，让我们为输出文件创建一个目录。第一个头文件提供了
 * <code>mkdir</code> 的功能，第二个头文件让我们确定在
 * <code>mkdir</code> 失败时发生了什么。
 *
 * @code
 * #include <sys/stat.h>
 * #include <cerrno>
 ...
 *
 * 该类提供了一个构成法的接口，即应变  $\varepsilon(\mathbf
 * u)$  和应力  $\sigma$
 * 之间的关系。在这个例子中，我们使用的是具有线性、各向同性的硬化的弹塑性材料行为。这种材料的特点是杨氏模量
 * $E$  ，泊松比  $\nu$  ，初始屈服应力  $\sigma_0$
 * 和各向同性硬化参数  $\gamma$  。  对于 $\gamma = 0$
 * 我们得到完美的弹塑性行为。
 * 正如描述这个程序的论文中所解释的，第一个牛顿步骤是用完全弹性材料模型来解决的，以避免同时处理两种非线性（塑性和接触）。为此，这个类有一个函数
 * <code>set_sigma_0()</code> ，我们以后用它来简单地设置
 * $\sigma_0$ 为一个非常大的值
 *
 * - 基本上保证了实际的应力不会超过它，从而产生一个弹性材料。当我们准备使用塑性模型时，我们使用相同的函数，将 $\sigma_0$ 设置回其适当的值。  由于这种方法，我们需要将 <code>sigma_0</code> 作为这个类的唯一非静态成员变量。
 *
 * @code
 * template <int dim>
 ...
 * @endcode
 *
 * ConstitutiveLaw类的构造函数为我们的可变形体设置所需的材料参数。弹性各向同性介质的材料参数可以用多种方式定义，如一对
 * $E, \nu$ （弹性模量和泊松数），使用Lam&eacute; 参数
 * $\lambda,mu$ 或其他几种常用的约定。在这里，构造器采用
 * $E,\nu$
 * 形式的材料参数描述，但由于这证明这些不是出现在塑性投影仪方程中的系数，我们立即将它们转换为更合适的体模和剪模的集合
 * $\kappa,\mu$ 。  此外，构造器将 $\sigma_0$
 * （无任何塑性应变的屈服应力）和 $\gamma$
 * （硬化参数）作为参数。在这个构造器中，我们还计算了应力-应变关系的两个主成分及其线性化。
 *
 * @code
 * template <int dim>
 ...
 *
 * 这是构成法的主成分。它计算四阶对称张量，根据上面给出的投影，当在一个特定的应变点上评估时，它将应变与应力联系起来。我们需要这个函数来计算
 * <code>PlasticityContactProblem::residual_nl_system()</code>
 * 中的非线性残差，我们将这个张量与正交点给出的应变相乘。计算遵循引言中列出的公式。在比较那里的公式和下面的实现时，记得
 * $C_\mu : \varepsilon = \tau_D$ 和 $C_\kappa : \varepsilon = \kappa
 ...
 *
 * 该函数返回线性化的应力应变张量，围绕前一个牛顿步骤
 * $u^{i-1}$ 的解进行线性化。  参数 <code>strain_tensor</code>
 * （通常表示为 $\varepsilon(u^{i-1})$
 * ）必须作为参数传递，并作为线性化点。该函数在变量stress_strain_tensor中返回非线性构成法的导数，在stress_strain_tensor_linearized中返回线性化问题的应力-应变张量。
 * 参见 PlasticityContactProblem::assemble_nl_system
 * ，该函数的使用情况。
 *
 * @code
 * template <int dim>
 ...
 *
 * 下面这个类是可以从输入文件中选择的两个障碍物中的第一个。它描述了一个以位置
 * $x=y=0.5, z=z_{\text{surface}}+0.59$ 和半径 $r=0.6$
 * 为中心的球体，其中 $z_{\text{surface}}$
 * 是可变形体的（平）表面的垂直位置。该函数的
 * <code>value</code> 返回给定 $x,y$
 * 值的障碍物位置，如果该点实际位于球体下方，则返回一个大的正值，如果它位于球体的
 * "阴影 "之外，就不可能干扰变形。
 *
 * @code
 *   template <int dim>
 ...
 * classes</h4>。
 *
 * 以下两个类描述了介绍中概述的障碍，即汉字。这两个中的第一个，
 * <code>BitmapFile</code> 负责从一个以pbm
 * ascii格式存储的图片文件中读入数据。这个数据将被双线性插值，从而提供一个描述障碍物的函数。(下面的代码显示了如何通过在给定的数据点之间进行内插来构建一个函数。人们可以使用本教程程序写完后引入的
 * Functions::InterpolatedUniformGridData,
 * ，它正好做了我们这里想要的东西，但看看如何用手做是有启发的）。)
 * 我们从文件中读取的数据将被存储在一个名为 obstacle_data
 * 的双倍数  std::vector  中。
 * 这个向量构成了计算单片双线性函数的基础，作为一个多项式插值。我们将从文件中读取的数据包括零（白色）和一（黑色）。
 * <code>hx,hy</code> 变量表示 $x$ 和 $y$
 * 方向的像素之间的间距。   <code>nx,ny</code>
 * 是这些方向上每个像素的数量。    <code>get_value()</code>
 * 返回图像在给定位置的值，由相邻的像素值插值而成。
 *
 * @code
 *   template <int dim>
 *   class BitmapFile
 ...
 *
 * 以下两个函数返回坐标为 $i,j$
 * 的给定像素的值，我们将其与定义在位置 <code>i*hx,
 * j*hy</code> 的函数值相识别，并在任意坐标 $x,y$
 * 处对两个函数中的第一个返回的点值进行双线性插值。在第二个函数中，对于每个
 * $x,y$ ，我们首先计算离 $x,y$
 * 左下方最近的像素坐标的（整数）位置，然后计算这个像素内的坐标
 * $\xi,\eta$
 * 。我们从下方和上方截断这两种变量，以避免在评估函数时超出其定义的范围而可能发生的舍入误差问题。
 *
 * @code
 *   template <int dim>
 ...
 *
 * 这是本程序的主类，提供描述非线性接触问题所需的所有函数和变量。它接近于
 * step-41
 * ，但有一些额外的功能，如处理悬空节点，牛顿方法，使用Trilinos和p4est进行并行分布式计算。处理悬空节点使生活变得有点复杂，因为我们现在需要另一个AffineConstraints对象。我们为接触情况下的主动集合方法创建一个牛顿方法，并处理构成法的非线性算子。
 * 这个类的总体布局与其他大多数教程程序非常相似。为了使我们的生活更容易一些，这个类从输入文件中读取一组输入参数。这些参数，使用ParameterHandler类，在
 * <code>declare_parameters</code>
 * 函数中声明（该函数是静态的，因此它可以在我们创建当前类型的对象之前被调用），然后一个已经用于读取输入文件的ParameterHandler对象将被传递给该类的构造函数。
 ...
 * @endcode
 *
 * 就成员变量而言，我们先用一些变量来表示这个程序运行的MPI宇宙，我们用一个流来让正好一个处理器产生输出到控制台（见
 * step-17  ），还有一个变量用来为程序的各个部分计时。
 *
 * @code
 ...
 * @endcode
 *
 * 下一组描述网格和有限元空间。特别是，对于这个并行程序，有限元空间有与之相关的变量，表明哪些自由度存在于当前的处理器上（索引集，也见 step-40
 * 和 @ref distributed
 * 文档模块），以及各种约束：那些由悬挂节点，由Dirichlet边界条件，以及由接触节点的活动集施加的约束。在这里定义的三个AffineConstraints变量中，第一个变量只包含悬挂节点的约束，第二个变量也包含与Dirichlet边界条件相关的约束，第三个变量包含这些约束和接触约束。
 * 变量 <code>active_set</code>
 * 包括那些由接触约束的自由度，我们用
 * <code>fraction_of_plastic_q_points_per_cell</code>
 * 来跟踪每个单元上应力等于屈服应力的正交点的分数。后者仅用于创建显示塑性区的图形输出，但不用于任何进一步的计算；该变量是该类的成员变量，因为该信息是作为计算残差的副产品计算的，但仅在很久之后才使用。(注意，该向量是一个长度等于<i>local
 * mesh</i>上活动单元数量的向量；它从未被用来在处理器之间交换信息，因此可以是一个常规的deal.II向量)。
 *
 * @code
 ...
 * @endcode
 *
 * 下一个变量块对应于解决方案和我们需要形成的线性系统。特别是，这包括牛顿矩阵和右手边；对应于残差（即牛顿右手边）但我们没有消除各种约束的向量，该向量用于确定在下一次迭代中需要约束哪些自由度；以及对应于介绍中简要提到并在随文讨论的
 * $B$ 矩阵对角线的向量。
 *
 * @code
 ...
 * @endcode
 *
 * 然后还有各种各样的其他变量，用于识别参数文件所选择的要求我们建立的网格，被推入可变形体的障碍物，网格细化策略，是否将解决方案从一个网格转移到下一个网格，以及要执行多少个网格细化循环。在可能的情况下，我们将这类变量标记为
 * <code>const</code>
 * ，以帮助读者识别哪些变量以后可能会被修改，哪些可能不会被修改（输出目录是一个例外
 *
 ...
 * <h3>Implementation of the <code>PlasticityContactProblem</code>
 * class</h3>。    <a name="PlasticityContactProblemdeclare_parameters"></a>
 ...
 *
 * 下一个区块涉及到构建起始网格。我们将使用下面的辅助函数和
 * <code>make_grid()</code>
 * 的第一个块来构造一个对应于半球形的网格。deal.II有一个函数可以创建这样的网格，但是它的位置和方向都是错误的，所以我们需要在使用它之前对它进行一些移动和旋转。
 * 供以后参考，如 GridGenerator::half_hyper_ball(),
 * 的文件中所述，半球体的平坦表面的边界指标为0，而其余部分的边界指标为1。
 *
 * @code
 ...
 *
 * </div>
 * 换句话说，立方体的边的边界指标是8。底部的边界指标是6，顶部的指标是1。我们通过循环所有面的所有单元并查看单元中心的坐标值来设置这些指标，并在以后评估哪个边界将携带迪里希特边界条件或将受到潜在接触时利用这些指标。(在目前的情况下，网格只包含一个单元，它的所有面都在边界上，所以严格来说，所有单元的循环和查询一个面是否在边界上都是不必要的；我们保留它们只是出于习惯：这种代码可以在许多程序中找到，基本上都是这种形式。)
 *
 * @code
 *   else
 *     {
 ...
 * 在下文中，每一组操作都被放入一个大括号封闭的块中，该块被块顶部声明的变量计时（
 * TimerOutput::Scope
 * 变量的构造器开始计时部分，在块的末端调用的析构器又停止计时）。
 *
 * @code
 * template <int dim>
 ...
 *
 * 这个函数从前面的函数中分离出来，计算与迪里切特型边界条件相关的约束，并通过与来自悬挂节点的约束合并，将其放入
 * <code>constraints_dirichlet_and_hanging_nodes</code> 变量。
 * 正如介绍中所阐述的，我们需要区分两种情况。
 *
 *
 * - 如果域是一个盒子，我们将底部的位移设置为零，并允许沿侧面在Z方向的垂直运动。如 <code>make_grid()</code> 函数所示，前者对应于边界指标6，后者对应于8。
 *
 *
 ...
 *
 * 下一个辅助函数计算（对角线）质量矩阵，用于确定我们在接触算法中使用的主动集合方法的主动集合。这个矩阵是质量矩阵类型的，但是与标准质量矩阵不同，我们可以通过使用正交公式使其成为对角线（甚至在高阶元素的情况下），该公式的正交点与有限元插值点的位置完全一致。我们通过使用QGaussLobatto正交公式来实现这一点，同时用一组从同一正交公式得出的插值点初始化有限元。该函数的其余部分相对简单：我们将得到的矩阵放入给定的参数中；因为我们知道矩阵是对角线的，所以只需在
 * $i$ 而不是 $j$
 * 上有一个循环即可。严格来说，我们甚至可以避免在正交点
 * <code>q_point</code>
 * 处将形状函数的值与自身相乘，因为我们知道形状值是一个恰好有一个一的向量，当与自身相点时产生一。由于这个函数不是时间关键，为了清楚起见，我们添加了这个术语。
 *
 * @code
 * template <int dim>
 ...
 *
 * 以下函数是我们在 <code>solve_newton()</code>
 * 函数中每次牛顿迭代时调用的第一个函数。它的作用是将解决方案投射到可行集上，并更新接触或穿透障碍物的自由度的活动集。
 ...
 * @endcode
 *
 * 在这个函数的最后，我们在处理器之间交换数据，更新
 * <code>solution</code>
 * 变量中那些已经被其他处理器写入的幽灵元素。然后我们将Dirichlet约束和那些来自悬挂节点的约束合并到已经包含活动集的AffineConstraints对象中。我们通过输出主动约束自由度的总数来结束这个函数，对于这个自由度，我们对每个处理器所拥有的主动约束自由度的数量进行求和。这个本地拥有的受限自由度的数量当然是活动集和本地拥有的自由度集的交集的元素数量，我们可以通过在两个IndexSets上使用
 * <code>operator&</code> 得到。
 *
 * @code
 ...
 *
 * 考虑到问题的复杂性，我们在每次牛顿迭代中要解决的线性系统的组合实际上是相当简单的，这可能会让人感到惊讶。下面的函数建立了牛顿的右手边和牛顿矩阵。它看起来相当简单，因为繁重的工作发生在对
 * <code>ConstitutiveLaw::get_linearized_stress_strain_tensors()</code>
 * 的调用中，特别是在
 ...
 * @endcode
 *
 * 在计算了应力-应变张量及其线性化之后，我们现在可以把矩阵和右手边的部分放在一起。在这两者中，我们需要线性化的应力-应变张量乘以
 * $\varphi_i$ 的对称梯度，即术语 $I_\Pi\varepsilon(\varphi_i)$
 * ，所以我们引入这个术语的缩写。回顾一下，该矩阵对应于随附出版物的符号中的双线性形式
 * $A_{ij}=(I_\Pi\varepsilon(\varphi_i),\varepsilon(\varphi_j))$
 * ，而右手边是 $F_i=([I_\Pi-P_\Pi
 * C]\varepsilon(\varphi_i),\varepsilon(\mathbf u))$ ，其中 $u$
 * 是当前的线性化点（通常是最后的解）。这可能表明，如果材料是完全弹性的，右手边将为零（其中
 * $I_\Pi=P_\Pi$
 * ），但这忽略了一个事实，即右手边还将包含由于接触而产生的非均质约束的贡献。
 ...
 * 也就是说，在略微滥用函数名称的情况下，它实际上做得更多。例如，它还计算对应于牛顿残差的向量，但没有消除受限自由度。我们需要这个向量来计算接触力，并最终计算出下一个活动集。同样，通过跟踪我们在每个单元上遇到的显示塑性屈服的正交点的数量，我们也可以计算出
 * <code>fraction_of_plastic_q_points_per_cell</code>
 * 矢量，随后我们可以输出这个矢量来可视化塑性区。在这两种情况下，作为线条搜索的一部分，这些结果是不必要的，因此我们可能会浪费少量的时间来计算它们。同时，无论如何，这些信息是作为我们在这里需要做的事情的自然副产品出现的，而且我们想在每个牛顿步骤结束时收集一次，所以我们不妨在这里做。
 * 这个函数的实际实现应该是相当明显的。
 *
 * @code
 * template <int dim>
 ...
 *
 *
 * - 对于悬空节点，我们必须将 AffineConstraints::set_zero 函数应用于newton_rhs。如果一个求解值为 $x_0$ 的悬空节点有一个与障碍物接触的数值为 $x_1$ 的邻居和一个没有接触的邻居 $x_2$ ，这就有必要。因为前者的更新将是规定的，所以悬挂的节点约束将有一个不均匀性，看起来像  $x_0 = x_1/2 +
 * \text{gap}/2$
 * 。所以右侧的相应条目是无意义的非零值。这些值我们必须设置为零。
 *
 *
 * - 与 step-40 一样，在求解或使用解决方案时，我们需要在有鬼元素和无鬼元素的向量之间进行洗牌。
 * 该函数的其余部分与 step-40 和 step-41
 * 类似，只是我们使用BiCGStab求解器而不是CG。这是由于对于非常小的硬化参数
 * $\gamma$
 * ，线性系统变得几乎是半无限的，尽管仍然是对称的。BiCGStab似乎更容易处理这种线性系统。
 *
 * @code
 * template <int dim>
 ...
 * 除此以外，这个函数的顶部部分应该是相当明显的。我们将变量
 * <code>previous_residual_norm</code>
 * 初始化为可以用双精度数字表示的最负值，这样，比较当前残差是否小于上一步的残差，在第一步中总是会失败。
 *
 * @code
 * template <int dim>
 ...
 * @endcode
 *
 * 在我们计算了当前牛顿步骤的试解 $\tilde{\mathbf u}$
 * 之后，情况就变得有点棘手了。我们处理的是一个高度非线性的问题，所以我们必须用直线搜索的方式来抑制牛顿方法。为了理解我们如何做到这一点，请回顾一下，在我们的公式中，我们在每一个牛顿步骤中计算一个试解，而不是在新旧解之间进行更新。由于解集是一个凸集，我们将使用直线搜索，尝试以前的解和试验解的线性组合，以保证阻尼解再次出现在我们的解集中。我们最多应用5个阻尼步骤。
 ...
 *
 * @endcode
 * 最后一步是检查收敛性。如果活动集在所有处理器上都没有变化，并且残差小于阈值
 * $10^{-10}$  ，那么我们就终止在当前网格上的迭代。
 *
 * @code
 ...
 *
 * 如果你已经在deal.II教程中做到了这一点，下面这个细化网格的函数应该不会再对你构成任何挑战。它对网格进行细化，可以是全局的，也可以是使用Kelly误差估计器的，如果这样要求的话，还可以将前一个网格的解转移到下一个网格。在后一种情况下，我们还需要再次计算活动集和其他数量，为此我们需要由
 * <code>compute_nonlinear_residual()</code> 计算的信息。
 *
 * @code
 * template <int dim>
 ...
 *
 * 在我们到达 <code>run()</code>
 * 之前，剩下的三个函数都与生成输出有关。下面一个是尝试显示变形体的变形配置。为此，这个函数接收一个位移矢量场，通过先前计算的位移来移动网格（局部）的每个顶点。在生成图形输出之前，我们将以当前的位移场调用该函数，在生成图形输出之后，我们将以负的位移场再次调用该函数，以撤销对网格的改变。
 ...
 * @endcode
 *
 * 在函数的其余部分，我们在每个处理器上生成一个VTU文件，以这个处理器的子域ID为索引。在第一个处理器上，我们还创建了一个
 * <code>.pvtu</code>
 * 文件，对VTU文件的<i>all</i>进行索引，这样就可以一次读取整个输出文件集。这些
 * <code>.pvtu</code>
 * 被Paraview用来描述整个并行计算的输出文件。然后我们再为Paraview的竞争者--VisIt可视化程序做同样的事情，创建一个匹配的
 * <code>.visit</code> 文件。
 *
 * @code
 ...
 *
 * 和其他所有的教程程序一样， <code>run()</code>
 * 函数包含了整体逻辑。这里没有太多的内容：本质上，它在所有的网格细化循环中执行循环，在每个循环中，将事情交给
 * <code>solve_newton()</code>
 * 中的牛顿求解器，并调用函数为如此计算出的解决方案创建图形输出。然后输出一些关于运行时间和内存消耗的统计数据，这些数据是在这个网格的计算过程中收集的。
 *
 * @code
 * template <int dim>
 ...
 * 包含该程序的目录还包含一些输入参数文件，可以用来创建各种不同的模拟。例如，用
 * <code>p1_adaptive.prm</code>
 * 参数文件（用球作为障碍物，用盒子作为领域）在16个核心上运行该程序，会产生这样的输出。
 * @code
 ...
 * src="https://www.dealii.org/images/steps/developer/step-42.CellConstitutionLi2.png"
 * alt="" alt="" width="70%"> </td> </tr> </table>
 * 图片显示了自适应细化，以及一个单元在与球接触过程中的塑化程度。请记住，我们考虑的是每个正交点的应力偏差部分的形式，看是否有弹性或塑性行为。蓝颜色意味着该单元只包含弹性正交点，与红色单元相比，所有正交点都是塑性的。
 *
 * - 这里的网格是最细的
 *
 * - 仔细观察可以看到由障碍物造成的凹陷。这就是 <code>move_mesh()</code> 函数的结果。然而，由于我们在这里考虑的障碍物的压痕非常小，所以很难辨别这种效果；我们可以用计算出的位移的倍数来替换网格的顶点。
 ...
 * 。
 * 像往常一样，有多种可能性来扩展这个程序。从算法的角度来看，这个程序在编写时已经尽可能地使用了接触质量、塑性非线性和线性求解器的最佳可用算法。然而，就更多的现实情况而言，人们还想用这个程序做一些事情。 <ul>   <li>  将程序从静态扩展到准静态，也许可以通过选择后向欧拉模式进行时间离散化。一些理论结果可以在Jörg Frohne的博士论文中找到，<i>FEM-Simulation
 * der Umformtechnik metallischer Oberfl&auml;chen im
 * Mikrokosmos</i>，德国锡根大学，2011。
 * <li>
 * 考虑有摩擦的接触问题也是一个有趣的进展。在几乎每个机械过程中，摩擦都有很大的影响。
 * 为了模拟这种情况，我们必须考虑到接触面的切向应力。摩擦也给我们的问题增加了另一个不等式，因为只要切向应力不超过某个极限，物体和障碍物通常会粘在一起，超过这个极限，两个物体就会互相滑动。
 * 如果我们已经模拟了摩擦性接触，下一步要考虑的是接触区的发热。由两个物体之间的摩擦引起的热量会提高可变形物体的温度，并引起一些材料参数的变化。
 * <li>  对于接触和塑性，实施更精确的、与问题相适应的误差估计器可能是有意义的。 </ul>
 *
