 ...
 *
 *
 * 我们没有用这些更复杂的例子来演示高阶映射的使用，而是只做了一个简单的计算：用两种不同的方法计算
 * $\pi=3.141592653589793238462643\ldots$ 的值。
 *
 *
 * 第一种方法使用单位半径的圆的三角形近似，并在其上积分一个单位幅度的常数函数（
 * $f = 1$
 * ）。当然，如果领域是精确的单位圆，那么面积将是 $\pi$
 * ，但由于我们只使用片状多项式段的近似，我们积分的面积值并不完全是
 * $\pi$  。然而，众所周知，当我们细化三角形时， $Q_p$
 * 的映射会以 $h^{p+1}$ 的阶数逼近边界，其中 $h$
 * 是网格大小。我们将检查圆的计算面积值以及它们在不同映射下向
 * $\pi$
 * 收敛的情况。我们还将发现一个收敛行为，这个行为一开始是令人惊讶的，但有一个很好的解释。
 *
 *
 * 第二种方法与此类似，但这次不使用三角形单位圆的面积，而是使用其周长。
 * $\pi$ 是周长的一半的近似值，因为我们选择半径等于1。
 *
 *
 * @note
 * 本教程实质上展示了如何为积分选择一个特定的映射，方法是将一个特定的几何体附加到三角法上（例如在
 * step-1
 ...
 * 的前几个数字，我们事先定义了这些数字，以便以后使用。由于我们想计算两个数字的差值，而这两个数字是相当精确的，计算出的
 * $\pi$
 * 的近似值的精度在一个双变量可以容纳的位数范围内，所以我们宁可将参考值声明为
 * <code>long double</code> ，并给它增加一些位数。
 *
 * @code
 ...
 * @endcode
 *
 * 所以首先生成一个圆的粗略三角剖分，并将一个合适的边界描述与之关联。默认情况下，
 ...
 * <code>1</code>
 * 的值，但也有一个类MappingQ1可以达到同样的效果。历史上，它以比MappingQ更简单的方式做了很多事情，但今天只是后者的一个包装。然而，如果你没有明确指定另一个映射，它仍然是库中许多地方隐含使用的类。
 *
 *
 ...
 * ，其中总和延伸到三角形中所有活动单元上的所有正交点，
 * $w(x_i)$ 是正交点的重量 $x_i$
 * 。每个单元上的积分都是通过数字正交来逼近的，因此我们唯一需要的额外成分是建立一个FEValues对象，提供每个单元的相应`JxW`值。注意`JxW`是指<i>Jacobian
 * determinant times
 * weight</i>的缩写；因为在数值正交中，两个因子总是出现在相同的地方，所以我们只提供合并的数量，而不是两个单独的数量）。我们注意到，在这里我们不会在其最初的目的中使用FEValues对象，即用于计算特定正交点上的特定有限元的基函数值。相反，我们只用它来获得正交点的
 * "JxW"，而不考虑我们将给FEValues对象的构造者的（假）有限元。给予FEValues对象的实际有限元根本不使用，所以我们可以给任何。
 *
 * @code
 * template <int dim>
 ...
 *
 * @endcode
 * 同样地，我们需要创建一个DoFHandler对象。我们实际上并没有使用它，但是它将为我们提供
 ...
 * @endcode
 *
 * 我们使用ConvergenceTable类的一个对象来存储所有重要的数据，如
 * $\pi$ 的近似值和相对于 $\pi$
 * 的真实值的误差。我们还将使用ConvergenceTable类提供的函数来计算
 * $\pi$ 的近似值的收敛率。
 *
 * @code
 ...
 * @endcode
 *
 * ......并将得到的面积值和错误存储在表中。我们需要静态转换为双数，因为没有实现add_value(string,
 ...
 * @endcode
 * 或者使用其他的文件名。第二行确保生成的输出的长宽比实际上是1:1，也就是说，圆在你的屏幕上被画成一个圆，而不是一个椭圆。第三行关闭了图形中的按键，因为这只会打印出现在并不重要的信息（文件名）。同样地，第四行和第五行关闭了刻度线。然后生成具有特定线宽（"lw"，这里设置为4）和线型（"lt"，这里选择线应该用RGB颜色
 ...
 *
 *
 * 从输出结果中可以立即观察到，在所有情况下，数值都会迅速收敛到
 ...
 * Bonito, A. Demlow, and J.
 * Owen:"对拉普拉斯-贝特拉米算子的特征值和特征函数的有限元近似的先验误差估计"，已提交，2018年）。)
 *
