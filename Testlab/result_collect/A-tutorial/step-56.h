 ...
 * 本教程依赖于  step-16  ,  step-22  。 @htmlonly <table
 ...
 * 让 $u \in H_0^1 = \{ u \in H^1(\Omega), u|_{\partial \Omega} = 0 \}$ 和
 ...
 * @f}
 *
 * 是简单的，那么使用该预处理程序的迭代求解器将在几个迭代中得到解决。请注意，我们在这里做的是正确的预处理。
 * 使用舒尔补码 $S=BA^{-1}B^T$ ，我们发现
 * @f{eqnarray*}
 * P^{-1} = \left(\begin{array}{cc} A & B^T \\ 0 &
 * S \end{array}\right)^{-1}
 * @f}
 *
 * *是一个很好的选择。让 $\widetilde{A^{-1}}$ 是 $A^{-1}$ 的近似值， $\widetilde{S^{-1}}$ 是 $S^{-1}$ 的近似值，我们看到@f{eqnarray*}
 * P^{-1} =
 ...
 *
 * 由于 $P$
 * 的目的只是作为一个预处理程序，我们将在上面的方程中使用右边的近似值。
 * 正如在 step-22 、 $-M_p^{-1}=:\widetilde{S^{-1}} \approx S^{-1}$
 * 中所讨论的，其中 $M_p$
 ...
 * <code>Function@<dim@></code>
 * 的类所需的内容，你会发现这个类有许多 @p virtual
 * 函数，包括 Function::value(),   Function::vector_value(),
 * Function::value_list(), 等，所有这些都可以被重载。
 * deal.II的不同部分将需要这些特定函数中的不同部分。这在一开始会让人感到困惑，但幸运的是，你真正需要实现的只有
 * @p value().
 ...
 * @endcode
 *
 * 这个块结构将dim速度分量与压力分量（用于重新排序）分开。注意，我们有2个而不是像
 * step-22
 ...
 * @f]
 * 例如见Ern/Guermond
 * "有限元素的理论与实践"，第4.2.5节第195页。这确实是我们观察到的，以
 * $Q_2 \times Q_1$ 元素为例（这就是代码中的做法，但在
 * <code>main()</code> 中很容易改变）。 <table align="center"
 ...
 * <td>-</td> <td>-</td> </tr> </table> 从表中可以看出。 1.
 * UMFPACK使用了大量的内存，特别是在3D中。而且，UMFPACK的优化并不随问题的大小而扩展。
 * 2. 因为我们对 $A$ 和 $S$
 * 使用内部求解器，ILU和GMG需要相同数量的外部迭代。 3.
 * ILU对 $A$
 * 的（内部）迭代次数随着细化而增加，导致求解时间的线性扩展性差。相比之下，
 * $A$
 * 的内部迭代次数在GMG中保持不变，导致解算时间几乎完美的缩放。
 * GMG需要比ILU多一点的内存来存储水平和界面矩阵。 <a
