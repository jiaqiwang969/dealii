 ...
 * 希望有一个类能够提供在单元格和边界及内部面的积分功能。这是由下面的类来完成的。在构造函数中，我们告诉循环应该计算单元格积分（"真"），但不应该计算边界和内部面的积分（两个
 ...
 * 包含了可以在这个局部集成器中填充的对象。在MeshWorker框架内，有多少对象被创建是由装配器类决定的。在这里，我们测试一下，比如说需要一个矩阵
 ...
 * 使用，因为我们只组装一个PDE，所以这些对象中也只有一个索引为0。
 *
 *
 ...
 *
 *
 * - 一个 MeshWorker::IntegrationInfoBox 对象，它将提供单元格上正交点的所有所需数据。这个对象可以看作是FEValues的扩展，提供更多的有用信息。
 *
 *
 * - 一个 MeshWorker::DoFInfo 对象，它一方面扩展了单元格迭代器的功能，同时也为其基类LocalResults中的返回值提供了空间。
 *
 *
 ...
 * }
 * @endcode
 * <a name="Results"></a><h1>Results</h1> 。   如 step-16
 ...
 * 自由度数：25（通过级别：8，25）  DEAL:cg::Starting 值0.510691
 ...
 * DEAL:cg::Starting  值0.371855  DEAL:cg::Convergence  第9步值1.13984e-13
 * DEAL::Cycle  3DEAL:: 活动单元数：170DEAL::
 ...
 * 6DEAL:: 活动单元数: 1202DEAL:: 自由度数: 1332 (按级别: 8, 25,
 * 89, 282, 771, 435, 257)  DEAL:cg::Starting  值 0.175848
 * DEAL:cg::Convergence  步骤 10 值 1.80664e-13  DEAL::Cycle  7DEAL::
 * 活动单元数：2288DEAL::
