...
@f} 。
*  $K({\mathbf x})$ 被假定为均匀正定，即有 $\alpha>0$ 使 $K(x)$ 的特征值 $\lambda_i({\mathbf x})$ 满足 $\lambda_i({\mathbf x})\ge \alpha$  。使用符号 $p$ 而不是通常的 $u$ 作为解变量将在下一节中变得清晰。
...
u}$ 乘以 $K^{-1}$ ，因为这使得方程组是对称的：其中一个方程有梯度，第二个方程有负发散，这两个方程当然是彼此相邻的，结果是不对称的双线性形式，因此，在 $K$ 是对称张量的共同假设下，是对称的系统矩阵。
* 这个问题的弱式是通过将两个方程与测试函数相乘，并将一些项分次积分而得到的：@f{eqnarray*}
...
* 为了克服这个问题，已经为 ${\mathbf u},p$ 开发了一些不同的有限元对，导致了稳定的离散问题。其中一个对子是用Raviart-Thomas空间 $RT(k)$ 处理速度 ${\mathbf u}$ ，用非连续元素类 $DQ(k)$ 处理压力 $p$  。关于这些空间的细节，我们特别参考Brezzi和Fortin的关于混合有限元方法的书，但许多其他关于有限元理论的书，例如Brenner和Scott的经典书，也说明了相关结果。在任何情况下，在适当选择函数空间的情况下，离散的表述如下。找到 ${\mathbf
u}_h,p_h$ ，以便@f{eqnarray*}
...
-(1,f)_K,
@f}
* 我们当然可以用更明确的形式写成@f{eqnarray*}
  \int_K {\textrm{div}}\ {\mathbf u}_h
 =
  \int_K f.
@f}。
* 应用发散定理的结果是，对于每个单元 $K$ 的选择， ${\mathbf
u}_h$ 必须满足@f{eqnarray*}
...
* 鉴于上述使用标准求解器和预处理器的困难，让我们再看一下矩阵。如果我们对自由度进行排序，使所有的速度变量排在所有的压力变量之前，那么我们可以将线性系统 $Ax=b$ 细分为以下几块：@f{eqnarray*}
  ...
  \end{array}\right),
@f} 。
* 其中 $U,P$ 分别是速度和压力自由度的值， $M$ 是速度空间的质量矩阵， $B^T$ 对应于负发散算子， $B$ 是其转置，对应于梯度。
* 通过区块消除法，我们可以按以下方式对这个系统重新排序（用系统的第一行乘以 $B^TM^{-1}$ ，然后再减去第二行）：@f{eqnarray*}
  B^TM^{-1}B P &=& B^TM^{-1} F
* 
- G, \\
  MU &=& F
...
@endcode
* 这里，三个LinearOperator对象的乘法产生了一个复合对象 <code>op_S</code> whose <code>vmult()</code> ，该函数首先应用 $B$ ，然后是 $M^{-1}$ （即用 $M$ 解方程），最后是 $B^T$ 到任何指定的输入向量。在这个意义上， <code>op_S.vmult()</code> 类似于以下代码。
* @code
...
@endcode
*（ <code>tmp1</code> and <code>tmp2</code> 是两个临时向量）。这种方法背后的关键点是，我们实际上从未创建过矩阵的内积。相反，每当我们要用 <code>op_S</code> 进行矩阵向量乘法时，我们只需按上述顺序运行所有单独的 <code>vmult</code> 操作。
...
@endcode
* 另一方面，手工方法掩盖了这个事实。
* 现在我们要做的就是形成定义 $P$ 和 $U$ 的两个方程的右边，然后分别用舒尔补强矩阵和质量矩阵来解决它们。例如，第一个方程的右手边是 $B^TM^{-1}F-G$ 。这可以用以下方式实现。
* @code
...
@endcode
* 该类允许<a href="https://en.wikipedia.org/wiki/Lazy_evaluation">lazy evaluation</a>涉及向量和线性运算符的表达式。这是通过存储计算表达式来实现的，只有当对象被转换为矢量对象，或者 PackagedOperation::apply() （或 PackagedOperation::apply_add()) 被手动调用时才会执行计算。假设 <code>F</code> and <code>G</code> 是右边的两个向量，我们可以简单地写出。
* @code
    const auto schur_rhs = transpose_operator(op_B) op_M_inv F
* 
- G;
@endcode
* 这里， <code>schur_rhs</code> 是一个打包的操作，<i>records</i>是我们指定的计算。它不会立即创建一个带有实际结果的向量。
* 有了这些先决条件，解决 $P$ 和 $U$ 的问题就是创建另一个求解器和逆向。
* @code
...
@f}*作为预处理程序如何？
* 作为一个预处理程序呢？这意味着每次我们必须做一个预处理步骤时，我们实际上必须用 $\tilde S$ 来解决。起初，这看起来几乎和立即用 $S$ 求解一样昂贵。然而，请注意，在内迭代中，我们不必计算 $M^{-1}$ ，而只是计算其对角线的逆值，这很便宜。
* 值得庆幸的是，LinearOperator框架使得这一点很容易写出来。我们之前已经对 <code>preconditioner_M</code> 矩阵使用了雅可比预处理程序（ $M$ ）。所以剩下的就是写出近似的舒尔补码应该是什么样子。
* @code
    const auto op_aS =
...
@f}。
*对于系数，我们选择单位矩阵 $K_{ij}=\delta_{ij}$ ，以求得简单性。因此，准确的速度满足@f{eqnarray*}
  {\mathbf u} =
  \left(\begin{array}{cc}
