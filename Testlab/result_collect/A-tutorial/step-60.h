 ...
 * 有两种有趣的情况。
 *
 * - 嵌入域 $\Gamma$ 的几何维度`dim`与域 $\Omega$ 相同（`spacedim`），即 $\Gamma$ 的spacedim维度不为零，或
 *
 * - 嵌入域 $\Gamma$ 的内在维度`dim`小于 $\Omega$ （`spacedim`），因此其间隔维度为零；例如，它是一条嵌入二维域的曲线，或嵌入三维域的面。
 * 在这两种情况下，定义限制算子 $\gamma$ 为算子，给定
 * $\Omega$ 上的连续函数，返回其在 $\Gamma$
 * 上的（连续）限制，即：。 \f[ \gamma : C^0(\Omega) \mapsto
 * C^0(\Gamma), \quad \text{ s.t. } \gamma u = u|_{\Gamma} \in C^0(\Gamma),
 * \quad \forall u \in C^0(\Omega). \f] 众所周知，当 $\Gamma$
 * 的内在维度与 $\Omega$ 相同时，算子 $\gamma$ 可以扩展为
 * $H^1(\Omega)$ 上的连续算子，将 $H^1(\Omega)$ 中的函数映射到
 * $H^1(\Gamma)$ 中的函数。 当 $\Gamma$ 的维数比 $\Omega$
 * 少一个时，同样的情况也适用于一个不太规则的范围空间（即
 * $H^{1/2}(\Gamma)$ ），并且 $\Gamma$
 * 没有边界。在这第二种情况下，算子 $\gamma$
 * 也被称为轨迹*算子，它对于嵌入 $\Omega$
 * 的Lipschitz共维度一的曲线和曲面 $\Gamma$
 * 有很好的定义（关于轨迹算子的进一步细节请阅读<a
 * href="https://en.wikipedia.org/wiki/Trace_operator">this wikipedia
 * article</a>）。 二维的情况要复杂一些，一般来说，当
 * $\Gamma$
 * 的维度为零或一的时候，不可能构造一个连续的轨迹算子，甚至不可能从
 * $H^1(\Omega)$ 到 $L^2(\Gamma)$ ，分别在二维和三维中构造。
 * 在本教程中，我们对 $\gamma$
 * 的进一步细节不感兴趣：假设嵌入域的维数(`dim`)总是小于或等于嵌入域
 * $\Omega$ 的维数(`spacedim`)，我们认为扩展 $\gamma$
 * 是当然的。 我们要解决以下微分问题：给定 $\Gamma$
 * 上的一个充分无规则函数 $g$ ，求 $u$ 的解。
 * @f{eqnarray*}{
 *
 ...
 * @f}
 *
 * 这是一个约束问题，我们正在寻找一个谐波函数 $u$
 * ，满足 $\partial\Omega$ 上的同质边界条件，并在约束条件
 * $\gamma u = g$ 下使用拉格朗日乘法器。
 * 这个问题有一个物理解释：谐波函数，即满足拉普拉斯方程的函数，可以被认为是边界值被规定的膜的位移。目前的情况相当于找到一个膜的形状，不仅边界上的位移，而且
 ...
 * 称为 "障碍物问题"
 *
 * --在 step-41 中进行建模）。) 作为第一个例子，我们研究
 * $\partial\Omega$ 上的零迪里切特边界条件。如果我们在
 * $\partial\Omega$
 * 上应用零Neumann边界条件或两者的混合，也适用同样的方程。
 * 变分公式可以通过引入两个无限空间 $V(\Omega)$ 和
 * $Q^*(\Gamma)$ 来得到，分别用于解 $u$ 和拉格朗日乘子
 * $\lambda$ 。 将第一个方程乘以 $v \in V(\Omega)$
 * ，第二个方程乘以 $q \in Q(\Gamma)$
 * ，可能时进行部分积分，并利用 $\partial\Omega$
 * 的边界条件，我们得到以下变量问题。
 * 给定 $\Gamma$ 上的一个足够规则的函数 $g$ ，找到@f{eqnarray*}{
 * (\nabla u, \nabla v)_{\Omega} + (\lambda, \gamma v)_{\Gamma} &=& 0 \qquad \forall v \in V(\Omega) \\
 * (\gamma u, q)_{\Gamma} &=& (g,q)_{\Gamma} \qquad \forall q \in Q(\Gamma),
 * @f}的解 $u$ 。
 *
 * 其中 $(\cdot, \cdot)_{\Omega}$ 和 $(\cdot, \cdot)_{\Gamma}$ 分别代表
 * $L^2$ 在 $\Omega$ 和 $\Gamma$ 中的标量积。
 * 对变量公式的检查告诉我们，空间 $V(\Omega)$
 * 可以被看作是 $H^1_0(\Omega)$ 。空间 $Q(\Gamma)$
 * ，在同维度为零的情况下，应取为 $H^1(\Gamma)$
 * ，而在同维度为一的情况下，应取为 $H^{1/2}(\Gamma)$ 。
 * 因此，函数 $g$ 应该在 $H^1(\Gamma)$
 * （对于同维度零的情况）或 $H^{1/2}(\Gamma)$
 * （对于同维度一的情况）中，这给我们留下了一个拉格朗日乘数
 ...
 * $\Omega$ 上施加的约束。
 * 为了解决更复杂的问题，例如领域 $\Gamma$
 * 与时间有关的问题，第二种方法可能是更可行的解决方案。处理不对齐的网格本身就很复杂：为了说明如何做，我们研究一个简单的问题。
 ...
 * ，定义在 $\Gamma_0$ 上的连续有限维空间上，对于任何点 $x
 * \in \Gamma_0$ ，其坐标 $\psi(x)$ 在 $\Omega$ 中表示。
 *
 * - 作为 $x\in \Gamma_0$ 的位移映射 $\delta \psi(x) = \psi(x)-x$ ，代表任何点 $x$ 的位移矢量，以使 $x$ 形成其实际配置 $\psi(x) = x +\delta\psi(x)$  。
 * 我们定义了嵌入式参考域 $\Gamma_0$
 * `embedded_grid`：在这个三角形上，我们构建了一个有限维空间（`embedded_configuration_dh`），通过FE_Q对象的有限单元系统（`embedded_configuration_fe`）来描述变形或位移。这个有限维度空间仅用于插值用户提供的函数（`embedded_configuration_function`），代表
 * $\psi$ （如果参数`use_displacement`被设置为 @p false) ）或
 * $\delta\psi$ （如果参数`use_displacement`被设置为 @p true).
 * ）。 拉格朗日乘数 $\lambda$ 和用户提供的函数 $g$
 * 是通过另一个有限维空间`embedded_dh`和另一个有限元素`embedded_fe`定义的，使用相同的参考域。为了考虑到域的变形，MappingFEField或MappingQEulerian对象都用`embedded_configuration`向量初始化。
 * 在嵌入空间中，使用FiniteElement
 * `space_fe`在嵌入网格`space_grid`上构建了一个标准的有限维空间`space_dh`，几乎逐字逐句地遵循
 * step-6 中的方法。 我们分别用\f[ V_h(\Omega) = \text{span}
 * \{v_i\}_{i=1}^n \f]和\f[ Q_h(\Gamma) = \text{span} \{q_i\}_{i=1}^m
 * \f]表示空间 $V$ 和 $Q$ 的离散化，其中 $n$
 * 是`space_dh`的尺寸， $m$ 是`embedded_dh`的尺寸。
 * 一旦所有的有限维空间都被定义，上述问题的变分公式就会给我们留下以下的有限维方程组。
 * \f[ \begin{pmatrix} K & C^T \\ C & 0 \end{pmatrix} \begin{pmatrix} u \\
 * \lambda \end{pmatrix} = \begin{pmatrix} 0 \\ G \end{pmatrix} \f] 其中
 * @f{eqnarray*}{
 ...
 * \sum_{i=1}^{n_q}  \big(\hat q_\alpha(\hat x_i)  (v_j \circ F_{K}) (\hat
 * x_i) J_K (\hat x_i) w_i \big) \f]
 * 计算这个和是不容易的，因为我们必须评估 $(v_j \circ
 * F_{K}) (\hat x_i)$ 。一般来说，如果 $\Gamma$ 和 $\Omega$
 * 没有对齐，那么 $F_{K}(\hat x_i)$ 点相对于 $\Omega$
 ...
 * 进行积分的正交点之一。
 *
 * - 找到 $\Omega$ 的单元， $y_i$ 位于其中。我们将称这个单元为 $T$ 。
 *
 ...
 * alt=""> </p> 上面的三个步骤可以通过依次调用来计算。
 *
 * -  GridTools::find_active_cell_around_point(), ，然后是
 *
 * -  Mapping::transform_real_to_unit_cell().  我们然后
 *
 * - 构建一个自定义的正交公式，包含参考单元格中的点，然后
 *
 * - 构建一个FEValues对象，使用给定的正交公式，并以第一步中获得的单元格为初始化。
 * 这就是deal.II函数 VectorTools::point_value()
 * 在评价一个任意点的有限元场（而不仅仅是一个单一的形状函数）时的做法；但在这种情况下，这将是低效的。
 * 一个更好的解决方案是使用一个方便的包装器在一个点的集合上执行前三个步骤。
 * GridTools::compute_point_locations().
 * 如果人们确实对计算完整的耦合矩阵感兴趣，那么可以调用方法
 * NonMatching::create_coupling_mass_matrix(),
 * ，该方法以有效的方式执行上述步骤，重复使用所有可能的数据结构，并将昂贵的步骤集合起来。这就是我们在本教程后面要使用的函数。
 * 我们通过迭代求解器来解决最终的鞍点问题，应用于舒尔补码
 * $S$ （其构造在 step-20
 ...
 * 在默认情况下， $\Gamma$
 * 的同维度为1，本教程程序实现了虚构边界法。事实证明，同样的技术被用于变异沉浸有限元法，上面定义的耦合算子
 * $C$ 在几乎所有的非匹配方法中都是一样的。
 * 嵌入域被假定包括在 $\Omega$ 中，我们把它当作单元方
 * $[0,1]^2$ 。虚域 $\Gamma$
 * 的定义可以通过参数文件进行修改，可以给出从界线区间
 * $[0,1]$ 到 $\Omega$ 中的一条曲线的映射。
 * 如果该曲线是封闭的，那么结果将类似于在边界为 $\Gamma$
 * 的网格上运行相同的问题。程序也可以在非封闭的 $\Gamma$
 * 上运行，尽管在这些情况下，问题的数学表述更加困难，因为
 ...
 *
 *
 * ParameterAcceptor提供了一个全局订阅机制。每当一个从ParameterAcceptor派生出来的类的对象被构造出来，一个指向该派生类型的对象的指针就会被注册，同时在参数文件中也有一个部分条目。这种注册表在调用单一函数
 * ParameterAcceptor::initialize("file.prm")
 * 时被遍历，该函数反过来确保所有存储在全局注册表中的类都声明它们将使用的参数，在声明了这些参数后，它读取`file.prm`的内容来解析实际参数。
 *
 *
 ...
 * @endcode
 *
 * 另一个新的包含文件是包含 GridTools::Cache
 * 类的文件。deal.II的结构和许多现代数值库一样，是按照有向无环图（DAG）组织的。DAG是一个具有拓扑顺序的有向图：每个节点在结构上代表一个对象，并通过一条（或多条）有向边与非根节点相连，从父节点到子节点。这种结构最重要的例子是Triangulation及其
 * Triangulation::cell_iterator
 * 结构。从Triangulation（主节点），我们可以访问每个单元（三角形的子节点）。从单元本身，我们可以访问该单元的所有顶点。在这个简单的例子中，DAG结构可以表示为三种节点类型（三角结构、单元格迭代器和顶点），由从三角结构到单元格迭代器，以及从单元格迭代器到顶点的定向边连接。这有几个优点，但它本质上造成了
 ...
 * @endcode
 *
 * `Parameters`类派生于ParameterAcceptor。这使得我们可以在其构造函数中使用
 ...
 * @endcode
 *
 * 我们这里唯一的非常规函数是`setup_coupling()`方法，用于生成耦合矩阵的稀疏模式
 * $C$  。
 *
 *
 * @code
 ...
 * @endcode
 *
 * </div> 在内部，存储在ParameterAcceptor中的current
 * path*现在被认为是"/first/second/third/"，也就是说，当你指定一个绝对路径时，ParameterAcceptor会将*当前的部分改为当前路径，即改为直到最后*"/"的部分名称的路径。
 * 你现在可以使用相对路径（例如，`ParameterAcceptor("My Other
 * Class")`）而不是绝对路径（例如，`ParameterAcceptor("/first/second/third/My
 * Other
 * Class")`）构建从ParameterAcceptor派生的另一个类，获得。
 ...
 * @endcode
 *
 * <a name="Setup"></a> <h3>Set up</h3>。 函数
 * `DistributedLagrangeProblem::setup_grids_and_dofs()`
 ...
 * @endcode
 *
 * 在这个教程程序中，我们不仅对 $\Omega$
 * 进行全局细化，还允许根据 $\Gamma$
 * 的位置进行局部细化，根据`parameters.delta_refinement`的值，我们用来决定应该对
 * $\Omega$ 进行多少轮局部细化，对应于 $\Gamma$ 的位置。
 * 有了这个映射，现在就可以通过调用方法
 ...
 * @endcode
 *
 * 与其他教程程序中的情况不同，这里我们使用ParameterAcceptor风格的初始化，即首先构建所有对象，然后对静态方法
 ...
 * end
 * @endcode
 * 产生右边的马鞍。 <div class="twocolumn" style="width: 80%"> <div
 ...
 * $\Gamma$ 上的问题的边界元素近似，并构建一个与 $\Gamma$
 * 相关的泊松问题的超星形算子的近似，这正是一个迪里切特到诺依曼的映射。
 ...
 * $C$
 * ，一个进程需要关于真实空间同一区域的两个网格的信息，但是，当使用分布式网格时，这种信息可能无法获得，因为存储在特定处理器上的
 * $\Omega$
 * 三角测量的本地拥有部分可能与存储在同一处理器上的
 * $\Gamma$ 三角测量的本地拥有部分不在同一位置上。
 * 可以实施各种策略来解决这个问题。
 *
 * - 分布两个网格，以便满足这个约束条件。
 *
