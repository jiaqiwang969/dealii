 ...
 * 本教程依赖于  step-16  ,  step-7  ,  step-39  。 @htmlonly <table
 ...
 * name="Introduction"></a><h1>Introduction</h1> 。   这是 step-16
 * 的一个变体，唯一的变化是我们使用了MeshWorker框架和预制的LocalIntegrator辅助类，而不是使用FEInterfaceValues来组装面的术语。
 ...
 * @endcode
 *
 * 这里有新的包含文件，用于使用MeshWorker框架。第一个包含类
 * MeshWorker::DoFInfo,
 * ，它为局部积分器提供了局部和全局自由度之间的映射。在第二个文件中，我们发现一个类型为
 ...
 * @endcode
 *
 * 这是一个神奇的对象，它知道关于数据结构和局部集成的一切。
 * 这是一个在函数 MeshWorker::loop(),
 * 中做工作的对象，它被下面的 MeshWorker::integration_loop()
 * 隐含地调用。在我们提供指针的函数完成局部积分后，
