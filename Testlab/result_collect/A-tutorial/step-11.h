 ...
 * 本教程依赖于  step-10  。 @htmlonly <table class="tutorial"
 ...
 * 我们要考虑的问题是拉普拉斯问题的解，只需满足纽曼边界条件：@f{eqnarray*}
 *
 *
 *
 *
 ...
 * @f} 。
 * 众所周知，如果这个问题要有一个解决方案，那么力需要满足兼容性条件@f[
 * \int_\Omega f\; dx + \int_{\partial\Omega} g\; ds = 0.
 * @f]我们将考虑这样的特殊情况： $\Omega$ 是围绕原点的半径为1的圆，而 $f=-2$ 、 $g=1$ 。这种选择满足了兼容性条件。
 * 兼容性条件允许上述方程的解，但它仍然保留了一个模糊性：因为只有解的导数出现在方程中，解只确定到一个常数。出于这个原因，我们必须为数字解提出另一个条件，以固定这个常数。
 * 对于这一点，有多种可能性。 <ol>   <li>  将离散化的一个节点固定为零或任何其他固定值。  这相当于一个附加条件  $u_h(x_0)=0$  。虽然这是常见的做法，但不一定是个好主意，因为我们知道拉普拉斯方程的解只在  $H^1$  中，它不允许定义点值，因为它不是连续函数的一个子集。因此，即使对离散函数允许固定一个节点，但对连续函数却不允许，在数值解的这一点上，人们常常可以看到由此产生的错误尖峰。
 * <li>
 * 将域上的均值固定为零或任何其他值。这在连续水平上是允许的，因为
 * $H^1(\Omega)\subset L^1(\Omega)$
 * 是由Sobolev不等式决定的，因此在离散水平上也是如此，因为我们只考虑
 * $H^1$ 的子集。
 * <li>  将域的边界上的均值固定为零或任何其他值。这在连续水平上也是允许的，因为  $H^{1/2}(\partial\Omega)\subset L^1(\partial\Omega)$  ，同样由Sobolev的不等式。 </ol>  我们将选择最后一种可能性，因为我们想用它来证明另一种技术。
 * 虽然这描述了要解决的问题，但我们仍然要弄清楚如何实现它。基本上，除了额外的均值约束，我们已经解决了这个问题好几次，使用迪里希特的边界值，我们只需要放弃对迪里希特边界节点的处理。高阶映射的使用也是相当微不足道的，我们将在使用它的各个地方进行解释；在几乎所有可以想象的情况下，你只会把描述映射的对象视为一个黑盒子，你不需要担心，因为它们的唯一用途似乎是被传递到库中深处的地方，那里的函数知道如何处理它们（即在
 * <code>FEValues</code> 类及其后代中）。
 * 这个程序中最棘手的一点是对均值约束的使用。幸运的是，库中有一个知道如何处理这种约束的类，我们已经经常使用它了，没有提到它的通用性。请注意，如果我们假设边界节点沿边界等距分布，那么均值约束@f[
 * \int_{\partial \Omega} u(x) \; ds = 0
 * @f]可以写成@f[
 * \sum_{i\in\partial\Omega_h} u_i = 0,
 * @f]，其中总和应在位于计算域边界的所有自由度指数上运行。让我们用 $i_0$ 表示边界上数字最小的指数（或者任何其他方便选择的指数），那么这个约束也可以用@f[
 * u_{i_0} = \sum_{i\in\partial\Omega_h\backslash i_0}
 *
 * -u_i.
 * @f]来表示，幸运的是，这正是AffineConstraints类所设计的约束形式。请注意，我们在之前的几个例子中使用了这个类来表示悬空节点的约束，它也有这种形式：在这里，中间的顶点应该有相邻顶点的平均值。一般来说，AffineConstraints类被设计用来处理形式为@f[
 * CU = b @f]的affine约束，其中 $C$ 表示一个矩阵， $b$
 * 表示一个向量，而 $U$
 * 是节点值的向量。在这种情况下，由于 $C$
 * 代表一个同质约束， $b$ 是零向量。
 * 在这个例子中，沿边界的均值允许这样的表述， $C$
 * 是一个只有一行的矩阵（即只有一个约束）。在实现中，我们将创建一个AffineConstraints对象，添加一个指向第一个边界节点
 * $i_0$
 * 的约束（即在矩阵中添加另一行），并插入所有其他节点贡献的权重，在这个例子中刚好是
 * $-1$  。
 ...
 *
 * @endcode
 * 然后我们声明一个表示拉普拉斯问题解决方案的类。由于这个例子程序是基于
 * step-5 ，这个类看起来相当相同，唯一的结构区别是函数
 * <code>assemble_system</code> now calls <code>solve</code>
 * 本身，因此被称为 <code>assemble_and_solve</code>
 * ，而且输出函数被删除，因为解函数非常无聊，不值得被查看。
 ...
 * @endcode
 *
 * 构建这样一个对象，通过初始化变量。在这里，我们使用线性有限元（
 * <code>fe</code>
 * 变量的参数表示多项式程度），以及给定阶数的映射。将我们要做的事情打印到屏幕上。
 *
 * @code
 * template <int dim>
 ...
 * @endcode
 *
 * 第一个任务是为这个问题设置变量。这包括生成一个有效的
 * <code>DoFHandler</code>
 * 对象，以及矩阵的稀疏模式，和代表边界上自由度平均值为零的约束条件的对象。
 *
 * @code
 * template <int dim>
 ...
 * @endcode
 *
 * 下一个任务是构建代表约束的对象，即边界上自由度的平均值应为零。为此，我们首先需要一个实际在边界上的节点的列表。
 * <code>DoFTools</code>
 * 命名空间有一个函数可以返回一个IndexSet对象，该对象包含所有在边界上的自由度的索引。
 * 一旦我们有了这个索引集，我们想知道哪个是边界上自由度对应的第一个索引。我们需要这个，因为我们想通过边界上所有其他自由度的值来约束边界上的一个节点。使用IndexSet类很容易得到这个
 * "第一个 "自由度的索引。
 *
 * @code
 ...
 *
 * @endcode
 * 然后生成一个只有这一条约束的约束对象。首先清除所有以前的内容（这些内容可能来自以前在更粗的网格上的计算），然后添加这一行，将
 * <code>first_boundary_dof</code>
 * 约束到其他边界DoF的总和，每个DoF都有权重
 *
 ...
 * @endcode
 *
 * 下一个任务是生成一个稀疏的模式。在这里，这的确是一个棘手的任务。通常情况下，我们只需调用
 * <code>DoFTools::make_sparsity_pattern</code>
 * 并使用悬挂节点约束来浓缩结果。我们在这里没有悬挂节点约束（因为我们在这个例子中只进行全局细化），但是我们在边界上有这个全局约束。在这种情况下，这带来了一个严重的问题：
 * <code>SparsityPattern</code>
 * 类希望我们事先说明每行的最大条目数，可以是所有行的，也可以是每行单独的。在库中有一些函数可以告诉你这个数字，如果你只有悬空的节点约束（即
 * DoFHandler::max_couplings_between_dofs),
 * ，但这对现在的情况来说是怎样的？困难的出现是因为消除约束的自由度需要在矩阵中增加一些条目，而这些条目并不那么容易确定。因此，如果我们在这里给出每行的最大条目数，我们就会有一个问题。
 * 由于这可能非常困难，以至于无法给出合理的答案，只允许分配合理的内存量，所以有一个动态稀疏模式（DynamicSparsityPattern）类，可以帮助我们解决这个问题。它不要求我们事先知道行可以有多少个条目，而是允许任何长度。因此，在你对行的长度没有很好的估计的情况下，它明显更灵活，但是代价是建立这样一个模式也比建立一个你事先有信息的模式要昂贵得多。尽管如此，由于我们在这里没有其他选择，我们将建立这样一个对象，用矩阵的尺寸初始化它，并调用另一个函数
 * <code>DoFTools::make_sparsity_pattern</code>
 * 来获得由于微分算子而产生的稀疏模式，然后用约束对象浓缩它，在稀疏模式中增加那些消除约束所需的位置。
 *
 * @code
 ...
 * @endcode
 *
 * 最后，一旦我们有了完整的模式，我们就可以从中初始化一个
 * <code>SparsityPattern</code>
 * 类型的对象，反过来用它来初始化矩阵。请注意，这实际上是必要的，因为与
 * <code>SparsityPattern</code>
 * 类相比，DynamicSparsityPattern的效率非常低，因为它必须使用更灵活的数据结构，所以我们不可能将稀疏矩阵类建立在它的基础上，而是需要一个
 * <code>SparsityPattern</code>
 * 类型的对象，我们通过复制中间对象产生这个对象。
 * 作为进一步的附带说明，你会注意到我们在这里没有明确地要
 * <code>compress</code> 稀疏模式。当然，这是由于
 * <code>copy_from</code>
 * 函数从一开始就生成了一个压缩的对象，你不能再向其添加新的条目。因此，
 * <code>compress</code> 的调用是隐含在 <code>copy_from</code>
 * 的调用中的。
 *
 * @code
 ...
 * 以下是两个最常用的函数：创建拉普拉斯矩阵和创建来自体或边界力的右手向量。它们需要映射对象、代表自由度和使用中的有限元的
 * <code>DoFHandler</code>
 * 对象、要使用的正交公式以及输出对象。创建右手向量的函数也必须接受一个描述（连续）右手向量函数的函数对象。
 * 让我们看一下矩阵和体力的集成方式。
 *
 * @code
 ...
 *
 * 这很简单，对吗？
 * 不过，有两点需要说明。首先，这些函数在很多情况下都会用到。也许你想为一个矢量值有限元创建一个拉普拉斯或质量矩阵；或者你想使用默认的Q1映射；或者你想用拉普拉斯算子的一个系数来装配矩阵。由于这个原因，在
 * <code>MatrixCreator</code> 和 <code>MatrixTools</code>
 * 命名空间中有相当多的这些函数的变体。每当你需要这些函数的一个与上面调用的略有不同的版本时，当然值得看一下文档，并检查一些东西是否适合你的需要。
 * 第二点是关于我们使用的正交公式：我们想对双线性形状函数进行积分，所以我们知道我们至少要使用二阶高斯正交公式。另一方面，我们希望正交规则至少有边界近似的阶数。因为有
 * $r$ 点的高斯规则的阶数是 $2r
 *
 * - 1$  ，而使用 $p$ 度的多项式的边界逼近的阶数是 $p+1$
 * ，我们知道 $2r \geq p$
 * 。由于r必须是一个整数，并且（如上所述）必须至少是
 * $2$ ，这就弥补了上面计算 <code>gauss_degree</code> 的公式。
 ...
 * @endcode
 *
 * 对于组装右手边，我们不得不使用两个不同的向量对象，然后再把它们加在一起。我们不得不这样做的原因是，
 * <code>VectorTools::create_right_hand_side</code> 和
 * <code>VectorTools::create_boundary_right_hand_side</code>
 * 函数首先清除了输出向量，而不是将它们的结果与之前的内容相加。这可以合理地称为库在起步阶段的设计缺陷，但不幸的是，事情现在已经是这样了，很难改变这种无声地破坏现有代码的事情，所以我们不得不忍受。
 *
 *
 ...
 *
 * @endcode
 * 最后，评估我们得到的解决方案。正如在介绍中所说的，我们对解决方案的H1半规范感兴趣。在这里，我们在库中也有一个函数可以做到这一点，尽管是以一种稍微不明显的方式：
 * <code>VectorTools::integrate_difference</code>
 * 函数整合了一个有限元函数和一个连续函数之间的差值的规范。因此，如果我们想要一个有限元场的规范，我们只需将连续函数设为零。请注意，这个函数，就像库中的许多其他函数一样，至少有两个版本，一个是以映射为参数的（我们在这里使用），另一个是我们在以前的例子中使用的隐含的
 * <code>MappingQ1</code>  。
 * 还要注意的是，我们采用的是高一级的正交公式，以避免超融合效应，即在某些点上，解刚好特别接近精确解（我们不知道这里是否会出现这种情况，但有已知的案例，我们只是想确认一下）。
 *
 * @code
 ...
 *
 * 换句话说，你在可视化工具中看到的东西实际上不是你正在计算的东西。顺带一提，在使用高阶形状函数时也是如此。大多数可视化工具只呈现双线性/三线性的表示。这在
 * DataOut::build_patches().) 中有详细的讨论。
 * 所以我们需要确保高阶表示被写入文件。我们需要考虑两个特别的话题。首先，我们通过
 * DataOutBase::VtkFlags
 * 告诉DataOut对象，我们打算将元素的细分解释为高阶拉格朗日多项式，而不是双线性斑块的集合。最近的可视化程序，如ParaView
 * 5.5版或更新版，然后可以呈现高阶解决方案（更多细节见<a
 * href="https://github.com/dealii/dealii/wiki/Notes-on-visualizing-high-order-output">wiki
 * page</a> ）。其次，我们需要确保映射被传递给
 * DataOut::build_patches()
 * 方法。最后，DataOut类默认只打印<i>boundary</i>单元的曲面，所以我们需要确保通过映射将内部单元也打印成曲面。
 *
 * @code
 * template <int dim>
 ...
 * @endcode
 *
 * 这是主循环，用线性到立方的映射做计算。请注意，由于我们只需要
 * <code>LaplaceProblem@<2@></code>
 * 类型的对象一次，我们甚至不给它命名，而是创建一个未命名的这样的对象，并调用它的
 * <code>run</code> 函数，随后它又立即被销毁。
 *
 * @code
 ...
 * 5120 1.253139 0.000175
 * @endcode
 * 正如我们所期望的，每个不同的映射的收敛顺序显然是对网格大小的二次方。
 * <em>   </em>
 * 有趣的是，双线性映射(即1度)的误差比高阶映射的误差大三倍以上；因此，在这种情况下，使用高阶映射显然是有利的，不是因为它提高了收敛顺序，只是为了减少收敛顺序之前的常数。另一方面，除了在非常粗的网格上，使用立方映射只能进一步提高结果，而且幅度不大。
