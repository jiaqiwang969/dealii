 ...
 * Introduction </h1> 。 本教程是对 step-1
 * 的扩展，演示了几种方法，以获得比那里显示的更多的网格。
 * @note  本教程也可以作为Jupyter Python笔记本，使用deal.II
 * python接口。该笔记本与原始的C++程序在同一目录下可用。
 * 生成复杂的几何图形是一项具有挑战性的任务，尤其是在三空间维度上。我们将讨论几种方法，但这个清单并不详尽。此外，也没有一种方法适合所有的问题。
 * 这个例子程序显示了一些为计算而创建和修改网格的方法，并以与我们在
 * step-1 中相同的方式将它们输出为 <code>.vtu</code>
 ...
 * step-14
 * 中的函数<tt>create_coarse_grid()</tt>中找到一个例子。GridGenerator中的所有函数都是以这种方式实现。
 ...
 * target="_top">cubit</a>。更多信息见GridIn的文档。问题是，deal.II需要的网格只由四边形和六面体组成。
 *
 ...
 * 文件的例子，它描述了一个内部有两个物体被切割出来的盒子。这就是
 * <code>example.geo</code>
 * 在%Gmsh中的样子（显示边界指示器以及下面要讨论的网格）。
 * <img
 * src="https://www.dealii.org/images/steps/developer/step-49.gmsh_picture.png"
 * alt="">
 * @note  这个文件包含 "物理线 "和 "物理面
 * "是很重要的。它们提供了在deal.II中使用的边界指标和材料ID。没有这些物理实体，任何东西都不会被导入到deal.II中。
 * deal.II的GridIn类可以读取由%Gmsh编写的 <code>.msh</code>
 * 格式，该格式包含为 <code>.geo</code> file. You generate the
 * <code>.msh</code>
 * 描述的几何体创建的网格，通过运行以下命令，可以从
 * <code>.geo</code> 中创建网格
 * @code
 * gmsh
 *
 ...
 * alt="">
 * @note
 * %Gmsh有许多其他接口，人们可以通过这些接口来描述几何图形。特别是，它能够与Python和Julia这样的脚本语言对接，但它也可以用C++来编写脚本。这些接口很有用，如果人们不只是想为一个单一的几何体生成网格（在这种情况下，图形界面或者在简单的情况下，手写的".geo
 ...
 * alt=""> output mesh </td> </tr> </table> 同样地，我们可以用公式
 * $(x,y) \mapsto (x,\tanh(2 y)/\tanh(2))$
 * 将一个有规律的细化的正方形转化为y方向的墙体适应的网格。这在本教程的
 * <code>grid_6()</code>  中完成。 <table width="60%" align="center"> <tr>
 * <td align="center"> <img
 ...
 * alt=""> perturbed output mesh </td> </tr> </table>
 ...
 * 个顶点的坐标，它并不只是提供这个顶点的位置，实际上是对存储这些坐标的位置的一个引用。然后我们可以修改存储在那里的值。
 *
 *
 ...
 * @endcode
 *
 * 在第二步，我们将对网格进行两次细化。为了正确做到这一点，我们应该沿着以原点为中心的圆的表面在内部边界上放置新的点。幸运的是，
 * GridGenerator::hyper_cube_with_cylindrical_hole
 * 已经在内部边界上附加了一个Manifold对象，所以我们不需要做任何事情，只需要细化网格（参见<a
 ...
 * (x,\tanh(2y)/\tanh(2))$
 * 将原始网格中的点转换为新的网格。为了使事情更有趣，而不是像前面的例子那样在一个单一的函数中完成，我们在这里创建一个具有
 ...
 *
 * - 最后，你必须用正确的  <code>manifold_id</code>  标记单元格和单元格面。例如，你可以通过以下方式得到一个在直角坐标中带有弯曲单元的环形扇面（但在极坐标中是矩形）。
 * @code
 ...
 * 为了更详细地说明这个过程，让我们考虑一个由Yuhan
 * Zhou创建的例子，作为2013年德克萨斯A&amp;M大学的一个学期项目的一部分。在一个CAD程序中，这个几何体看起来像这样。
 * <img
 ...
 * @endcode
 *
 * 通过这个代码，我们得到一个看起来像这样的网格。 <img
 * src="https://www.dealii.org/images/steps/developer/  step-49  .yuhan.9.png"
 * title="" width="400" height="355">
 * 这一变化修复了边界，但产生了一个新的问题：与圆柱体轴线相邻的单元格被严重扭曲了。我们应该使用直角坐标对这些中心单元进行计算以避免这个问题。沿着中心线的单元格都有一个面接触到线
 * $(0, 3, z)$
 * ，因此，为了实现这一点，我们回头将这些单元格上的
 * <code>manifold_id</code> s覆盖为零（这是默认的）。
 * @code
 * const Tensor<1, 3>           axis({0.0, 0.0, 1.0});
 * const Point<3>               axial_point(0, 3.0, 0.0);
 * const CylindricalManifold<3> cylinder(axis, axial_point);
 ...
 * .yuhan.10.png" alt="" width="400" height="355">。
 * 这给了我们一个很好的网格，每个圆心的单元仍然是笛卡尔式的，而边界周围的单元是沿着圆的。如果我们再细化两次，我们就能真正看到边界拟合网格的良好细节。
 * <img src="https://www.dealii.org/images/steps/developer/  step-49
 ...
 * step-38  中使用。
 * 也可以尝试提取`Triangulation<2,2>`的边界网格。
 *
 *  <!-- 本教程可能的扩展。
 *
 * - 用于收敛研究的非结构化网格数据库
