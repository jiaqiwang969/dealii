 ...
 * name="Backgroundandpurpose"></a><h3>Background and purpose</h3>
 *
 * 在这个示例程序中，我们并不关心如何使用deal.II及其设施的新方法，而是介绍编写模块化和可扩展有限元程序的方法。其主要原因是现代研究软件的规模和复杂性：实现现代误差估计概念和自适应求解方法的应用程序往往变得比较大。例如，在2002年写这个程序时，deal.II的主要作者的三个最大的应用程序，在写这个例子程序时是。 <ol>   <li>  一个用不连续加尔金有限元法解决双曲守恒方程的程序。33,775行代码；  <li>  一个参数估计程序。28,980行代码；  <li>  一个波浪方程求解器：21,020行代码。 </ol>
 * （库的适当
 *
 * - 不包括示例程序和测试套件
 *
 ...
 *
 *
 * 在以前的例子中，你已经看到库本身是如何被分成几个复合体的，每个复合体都建立在底层复合体之上，但相对独立于其他复合体。 <ol>   <li>  三角形类复合物，以及相关的迭代器类； <li>  有限元类； <li>  DoFHandler类复合物，以及相关的迭代器，建立在三角形和有限元类之上； <li>  实现单元格和实单元格之间映射的类； <li>  FEValues类复合物，建立在有限元和映射的之上。 </ol>  除了这些，还有大量的小类，当然还有以下 "工具 "模块。 <ol>   <li>  以各种图形格式输出； <li>  线性代数类。 </ol>  这些复数也可以在deal.II手册网站的首页上以流程图的形式找到。
 *
 *
 ...
 *
 *
 * 在这里，我们将（再次）实现一个拉普拉斯求解器，尽管与以前的例子程序相比有一些不同。 <ol>   <li>  实现数值求解方程过程的类不再负责驱动 "求解-估计误差-修正-再求解 "的过程，而是将其委托给外部函数。这首先允许在更大的范围内将其作为一个构件，在这里拉普拉斯方程的解可能只是其中的一部分（例如，在非线性问题中，拉普拉斯方程可能要在每个非线性步骤中解决）。它还允许围绕该类建立一个框架，允许使用其他方程的求解器（但具有相同的外部接口），以备对不同类型的偏微分方程评估一些技术。 <li>  它将评估计算出的解的过程分割成一组单独的类。原因是，人们通常对偏微分方程的解本身不感兴趣，而是对它的某些方面感兴趣。例如，人们可能希望在弹性计算中计算某一边界的牵引力，或者在某一位置的接收器上计算地震波的信号。有时，人们可能对这些方面中的几个方面感兴趣。由于解的评估是通常不影响解的过程，我们把它拆成一个单独的模块，以便独立于解算器类的开发来开发这种评估过滤器。 <li>  将实现网格细化的类与计算解的类分开。 <li>  将我们要介绍的测试案例的描述与程序的其他部分分开。 <li>  使用WorkStream设施对线性系统的装配进行并行化。这是在 @ref threads "多处理器访问共享内存的并行计算 "文档模块中可以找到的广泛描述。该实现基本上遵循了  step-9  中已经描述的内容。 </ol>
 *
 *
 * 该程序所做的事情并不新鲜。事实上，这更像是以前程序的变种，从以前的例子中拆解出各种部分和功能。读者应该关注的是它们在这个程序中的安排方式，也就是程序中用来实现所需数学方法的目标的软设计技术。然而，我们必须强调，软件设计在某种程度上也是一个主观问题：不同的人有不同的编程背景，对
 * "正确
 * "的编程风格有不同的看法；因此，这个程序只表达了作者认为有用的做法，如果你对所选择的方式感到不舒服，也不一定要采用这种风格来编写成功的数值软件。然而，它应该作为一个案例研究，以启发读者的想法来达到预期目的。
 *
 *
 ...
 * 该程序实际上做什么甚至不是这个程序的重点，程序的结构更重要。然而，用几句话来描述就是：求解给定右手边的拉普拉斯方程，使其解为函数
 * $u(x,t)=\exp(x+\sin(10y+5x^2))$  。计算的目的是得到
 * $x_0=(0.5,0.5)$
 * 点的解的值，并比较我们在两个细化标准下解决这个值的准确性，即全局细化和凯利等人的误差指标细化，我们在以前的例子中已经使用过。
 *
 *
 ...
 * 和所有的程序一样，我们从库中的包含文件列表开始，和往常一样，它们的标准顺序是
 * <code>base</code>  。
 *
 * -  <code>lac</code>
 *
 ...
 * 对象和一个解向量描述的。我们在这里首先定义它们，甚至在实际生成要评估的解的类之前，因为我们需要声明一个抽象的基类，以便解算器类可以引用。
 * 从抽象的角度来看，我们声明一个纯基类，它提供了一个评估算子（），它将对解进行评估（无论派生类如何考虑
 * <code>evaluation</code>
 * ）。由于这是该基类的唯一真正的功能（除了一些记账机制），我们通常把这样一个只有
 ...
 * <code>operator()</code> ，取一个 <code>DoFHandler</code>
 ...
 *
 * 下一件事是实现实际的评估类。正如介绍中所指出的，我们想从解决方案中提取一个点值，所以第一个类在其
 * <code>operator()</code>
 * 中做了这个。实际的点是通过构造函数给这个类的，还有一个表对象，它将把它的发现放入其中。
 ...
 * ExcNotImplemented())</code>的语句，这应该能在异常被触发时很清楚地说明什么地方出现了问题。在这种情况下，我们省略了它（这的确是不好的风格），但是知道这一点在这里并没有什么坏处，因为如果我们要求它给我们顶点的DoF索引，如果没有的话，语句
 * <code>cell-@>vertex_dof_index(vertex,0)</code> 就会失败。
 ...
 * @endcode
 *
 * 最后，我们要确定我们确实已经找到了评估点，因为如果不是这样的话，我们就不能在那里给出一个合理的解的值，反正剩下的计算也是无用的。所以通过
 ...
 * @endcode
 *  请注意，我们在其他例子程序中也使用了 <code>Assert</code>
 * 宏。它与这里使用的 <code>AssertThrow</code>
 * 宏的不同之处在于，它只是中止程序，而不是抛出一个异常，而且它只在调试模式下这样做。它是用来检查作为参数传递给函数的向量大小的正确宏，等等。
 ...
 * <code>DoFHandler</code>
 * 对象和解的向量，我们就有了做这件事的所有需要，所以我们可以在评估类中做这件事。实际上这样做而不是把它放到计算解决方案的类中的原因是，这样我们有更多的灵活性：如果我们选择只输出它的某些方面，或者根本不输出它。在任何情况下，我们都不需要修改求解器类，我们只需要修改其中的一个模块，就可以构建这个程序了。如上所述，这种形式的封装可以帮助我们保持程序的每一部分相当简单，因为接口保持简单，不可能访问隐藏的数据。
 ...
 * <code>Object</code>
 * ，甚至是数字表示。作者承认，他并不特别喜欢在C++中使用这种风格，因为它将风格置于理性之上。此外，它提倡对一切事物使用虚拟函数（例如，在Java中，所有的函数本身就是虚拟的），然而，这在许多应用中被证明是相当低效的，在这些应用中，函数往往只是访问数据，而不是进行计算，因此很快就会返回；这样，虚拟函数的开销就会很大。笔者的意见是，凡是至少有一部分实际实现的代码可以共享的地方，都应该有抽象的基类，从而将其分离到基类中。
 ...
 * <code>DoFHandler</code>
 * 对象，初始化了给定的三角形的指针。这将导致DoF处理程序存储该指针，但并没有生成一个有限元编号（我们只在
 ...
 * 函数接收一个评估对象，并将其应用于计算的解决方案。这个函数可以被多次调用，对用户要求的每一个解的评估都要调用一次。
 *
 * @code
 *   template <int dim>
 ...
 * WorkStream::run
 * 所期望的完全一致：本地装配函数需要接收一个迭代器、一个从头开始的对象和一个拷贝对象；而复制-本地-全球的函数需要接收的正是一个拷贝对象。但是，如果我们想要的东西稍微更通用一些呢？例如，在目前的程序中，copy-local-to-global函数需要知道将本地贡献写入哪个线性系统对象中，也就是说，它还必须接受一个
 ...
 * function</a>，它用固定的值来包装函数调用，包括各个参数。所有属于外层函数签名的参数在lambda函数中被指定为常规的函数参数。固定值使用捕获列表（`[...]`）传递到lambda函数中。可以使用捕获默认值，也可以明确列出所有要绑定到lambda的变量。为了清楚起见，我们决定在这里省略捕获默认值，但是捕获列表同样可以是`[&]`，这意味着所有使用的变量都通过引用复制到lambda中。
 ...
 * 既然这个任务可能已经完成，也可能没有完成，既然我们可能要等待它完成，那么我们不妨试着把其他需要做的事情打包到这个空隙中。因此，在等待右手边的工作之前，我们首先插值了边界值。当然，另一种可能性是在一个单独的任务中也插值边界值，因为这样做与我们到目前为止在这个函数中所做的其他事情无关。请自由地找到正确的语法，为这个插值创建一个任务，并在这个函数的顶部启动它，同时装配右手边。(你会发现这稍微有点复杂，因为
 ...
 * @endcode
 *
 * 现在是实现线性系统类中动作的函数。首先，构造函数将所有的数据元素初始化为正确的大小，并设置了一些额外的数据结构，例如由于悬空节点而产生的约束。由于设置悬空节点和找出矩阵的非零元素是独立的，所以我们以并行方式进行（如果库被配置为使用并发，至少是这样；否则，这些动作是按顺序执行的）。注意，我们只启动一个线程，并在主线程中做第二个动作。由于只生成了一个任务，我们在这里不使用
 ...
 *
 * 到现在，除了 <code>refine_grid</code>
 * 函数外，抽象基类的所有函数都已经实现了。我们现在将有两个类为
 ...
 * 作为精确解。在超过两个维度的情况下，只需用
 * <code>y</code> replaced by <code>z</code>
 * 重复正弦系数，以此类推。鉴于此，从前面的例子来看，以下两类可能是直接的。
 *
 * @code
 * template <int dim>
 ...
 * <code>std::flush</code>
 * 是需要的，以使文本真正出现在屏幕上，而不是只出现在一些缓冲区中，而这些缓冲区只有在我们下次发出端线时才会被刷新。
 *
 * @code
 ...
 * "kelly "和
 * "global"），用一个粗略的网格（这里是无处不在的单位方格）和一个有限元对象（这里也是无处不在的双线性对象）创建一个求解器对象，并使用该求解器来要求在一连串的细化网格上解决问题。
 ...
 * @endcode
 *
 * 创建一个由这个函数的参数指示的解算器对象。如果该名称不被识别，则抛出一个异常!各自的求解器对象被存储在一个
 * `std::unique_ptr` 中，以避免使用后不得不删除指针。
 *
 * @code
 ...
 * $e=|u(x_0)-u_h(x_0)|$ 绘制成图，可以得出以下结果。
 *
 *  <img src="https://www.dealii.org/images/steps/developer/step-13.error.png"
 * alt="">
 *
 *  <em> 和 </em>
 * 这幅图令人不安的是，自适应网格细化不仅没有像人们通常期望的那样比全局细化好，甚至明显更差，因为它的收敛是不规则的，在使用后续网格的值时阻止了所有的外推技术！另一方面，全局细化提供了一个完美的
 * $1/N$ 或 $h^{-2}$
 ...
 *
 *
 * 因此，我们以一个问题来结束对这个例子程序的讨论。 <p
 * align="center"> <strong>  <em>
 * 如果适应性不比全局细化好，那么它有什么问题？ </em>
 * </strong>
 *
 *
 *  <em>  在本例的最后进行练习。 </em>
 ...
 * 在误差较大的步骤中，网格在评估点是不规则的，也就是说，一些相邻的单元有悬空的节点；这破坏了一些超级近似的效果，而全局精炼的网格可以从中受益。答案2：这个快速黑客
 * @code
