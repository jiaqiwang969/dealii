 ...
 * 本教程依赖于  step-7  ,  step-37  。 @htmlonly <table
 ...
 * 其中 $a(\mathbf x)$ 是一个可变系数。 我们选择
 * $\Omega=[0,1]^3$  和  $a(\mathbf x)=\frac{10}{0.05 + 2\|\mathbf x\|^2}$
 * 作为域。由于系数是围绕原点对称的，但域却不是，我们最终会得到一个非对称的解决方案。
 ...
 *
 * -  LinearAlgebra::CUDAWrappers::Vector,  类似于更常见的Vector<Number>，和
 *
 * -  LinearAlgebra::distributed::Vector<Number,   MemorySpace::CUDA>,  这是一个普通的 LinearAlgebra::distributed::Vector ，我们已经指定了要使用的内存空间。
 * 如果没有指定内存空间，默认是 MemorySpace::Host. 。
 * 接下来，我们展示如何使用 LinearAlgebra::CUDAWrappers::Vector:
 * 将数据移入/移出设备。
 * @code
 ...
 * 但在有些情况下，人们希望在若干台机器上的多个MPI进程之间运行并行计算，而每台机器都配备有GPU。在这种情况下，人们希望使用
 * `LinearAlgebra::distributed::Vector<Number,MemorySpace::CUDA>`,
 * ，它是类似的，但`import()`阶段可能涉及MPI通信。
 * @code
 ...
 * @endcode
 * `relevant_rw_vector`是一个存储向量的等价物子集的对象。通常，这些是 @ref GlossLocallyRelevantDof "本地相关的DoFs"
 * ，这意味着它们在不同的MPI进程之间是重叠的。因此，在机器上存储在该向量中的元素可能与在该机器上由GPU存储的元素不一致，需要MPI通信来导入它们。
 * 在所有这些情况下，当导入一个向量时，数值可以被插入（使用
 * VectorOperation::insert) 或添加到向量的先前内容中（使用
 * VectorOperation::add).  ）。
 *
 ...
 *
 * `HelmholtzOperatorQuad`类实现了对每个正交点的Helmholtz算子的评估。它使用了类似于
 ...
 *
 * 与 step-4 或 step-6
 * 等程序不同，我们不必组装整个线性系统，只需组装右手边的向量。这在本质上与我们在
 * step-4
 * 中做的一样，例如，但我们必须注意在将局部贡献复制到全局向量中时使用正确的约束对象。特别是，我们需要确保对应于边界节点的条目被正确地清零了。这对于CG的收敛是必要的。
 * 另一个解决方案是修改上面的`vmult()`函数，我们假装源向量的条目为零，在矩阵-向量乘积中不考虑它们。但这里使用的方法更简单）。)
 ...
 * @endcode
 *
 * 这个solve()函数最后包含了对之前讨论的新类的调用。这里我们不使用任何预处理程序，即通过身份矩阵预处理，只关注
 * CUDAWrappers::MatrixFree
 ...
 * @endcode
 *
 * 人们可以在这里提出两个意见。首先，数值解的规范与精确（但未知）解的规范相对应。其次，随着网格的细化，迭代次数大约增加一倍。这与CG迭代次数随矩阵条件数的平方根增长的预期一致；而且我们知道二阶微分运算的矩阵条件数会像
 * ${\cal O}(h^{-2})$
