 ...
 * 本教程依赖于  step-15  ,  step-37  。 @htmlonly <table
 ...
 * @f}
 * 这个问题也被称为<i>Gelfand
 * problem</i>，是燃烧理论问题的典型例子，例如见 @cite
 ...
 * 0.
 * @f}
 *  选择拉格朗日有限元空间 $V_h \dealcoloneq \bigl\{ v \in
 * C(\overline{\Omega}) : v|_Q \in \mathbb{Q}_p \text{ for all } Q \in
 * \mathcal{T}_h \bigr\} \cap H_0^1(\Omega)$
 * ，它直接包含了同质Dirichlet边界条件，我们可以定义一个基
 * $\{\varphi_i\}_{i=1,\dots,N}$
 * ，因此只需用这些基函数进行测试即可。因此离散的问题如下。找到
 * $u_h\in V_h$ ，使所有 $i=1,\dots,N$ 都成立。
 * @f{align*}{
 * F(u_h)
 * \dealcoloneq
 * \int_\Omega \nabla \varphi_i \cdot \nabla u_h \,\mathrm{d}x
 ...
 * @f}
 * 由于每个有限元函数是基础函数 $\{\varphi_i\}_{i=1,\dots,N}$
 * 的线性组合，我们可以通过 $\mathbb{R}^N$
 * 的一个向量来确定有限元解决方案，该向量由每个自由度（DOF）的未知值组成。因此，我们定义非线性函数
 * $F\colon\mathbb{R}^N\to\mathbb{R}^N$ 代表离散非线性问题。
 * 为了解决这个非线性问题，我们使用牛顿方法。因此，给定一个初始猜测
 * $u_h^0\in V_h$
 * ，它已经满足了Dirichlet边界条件，我们通过连续应用以下方案来确定一连串的牛顿步骤
 * $\bigl( u_h^n \bigr)_n$  。
 * @f{align*}{
 ...
 * @f}
 * 因此，在每个牛顿步骤中，我们必须解决一个线性问题
 * $A\,x = b$ ，其中系统矩阵 $A$ 由雅各布
 * $F'(u_h^n)[\,\cdot\,]\colon\mathbb{R}^N\to\mathbb{R}^N$ 表示，右手边
 * $b$ 由负残差 $-F(u_h^n)$ 表示。在这种情况下，解向量 $x$
 * 是 $n$
 * 第1个牛顿步骤的牛顿更新。请注意，我们假设初始猜测
 * $u_h^0$
 * 已经满足了问题公式中的迪里希特边界条件（事实上这也可能是一个不均匀的迪里希特边界条件），因此牛顿更新
 * $s_h$ 满足不均匀的迪里希特条件。
 * 到目前为止，我们只测试了基函数，然而，我们也可以将
 * $V_h$
 ...
 * 函数的实施中，我们将通过使用成员函数
 * FEValuesBase::get_function_values() 和
 * FEValuesBase::get_function_gradients().
 * 从装配过程中的最后一个牛顿步骤中收集这一信息，然后
 * <code>assemble_system()</code> 函数将看起来像。
 * @code
 * template <int dim>
 ...
 * @endcode
 *
 * 由于我们想在不存储矩阵的情况下解决这个问题，我们需要在使用前告诉无矩阵运算器这些信息。因此，在派生类
 * <code>JacobianOperator</code> 中，我们将实现一个名为
 * <code>evaluate_newton_step</code>
 * 的函数，它将在使用矩阵-向量实现之前处理最后一个牛顿步骤的信息。此外，我们想为线性求解器使用一个几何多网格（GMG）预处理器，因此为了应用多层次算子，我们需要将最后一个牛顿步骤也传递给这些算子。这是一项棘手的任务，因为包含最后一个牛顿步骤的向量必须被内插到所有级别的三角计算中。在代码中，这项任务将由函数
 * MGTransferMatrixFree::interpolate_to_mg(). 完成。
 * 注意，与之前我们设置和使用几何多网格预处理的情况的一个根本区别是，我们可以重复使用MGTransferMatrixFree对象来计算所有牛顿步骤。所以我们可以在这里节省一些工作，定义一个类变量，并使用已经设置好的MGTransferMatrixFree对象
 * <code>mg_transfer</code> ，该对象在 <code>setup_system()</code>
 * 函数中被初始化。
 * @code
 ...
 * @endcode
 *
 * 评估非线性的函数与 step-37 中评估系数函数的函数
 * <code>evaluate_coefficient</code>
 * 的工作方式基本相同。我们的想法是使用一个FEEvaluation对象来评估牛顿步骤，并将所有单元和所有正交点的表达式存储在一个表中。
 * @code
 ...
 * MatrixFreeOperators::Base 类派生出 <code>JacobianOperator</code>
 ...
 * 函数。然而，它并不评估一个函数对象，而是评估一个代表有限元函数的向量，即雅各布系数所需的最后一个牛顿步骤。因此，我们设置了一个FEEvaluation对象，用
 * FEEvaluation::read_dof_values_plain() 和 FEEvaluation::evaluate()
 * 函数评估正交点的有限元函数。我们将有限元函数的评估值直接存储在
 * <code>nonlinear_values</code> 表中。 这样做效果很好，在
 * <code>local_apply()</code>
 * 函数中我们可以使用存储在表中的值来应用矩阵-向量乘积。然而，我们也可以在这个阶段优化雅各布系数的实现。我们可以直接评估非线性函数
 ...
 * <code>vmult()</code>
 * 函数时都会重建矩阵的信息。然而，对于牛顿方案的应用，我们需要组装线性化问题的右手边并计算残差。因此，我们实现了一个额外的函数
 * <code>evaluate_residual()</code> ，后来我们在
 * <code>assemble_rhs()</code> and the <code>compute_residual()</code>
 * 函数中调用该函数。最后，典型的 <code>solve()</code>
 ...
 * $\alpha=1$
 * 是我们能做的最好的。如果我们没有好的初始值，牛顿方法的自适应版本就变得很有趣。请注意，在理论上，牛顿方法是以二次方顺序收敛的，但只有当我们有一个合适的初始值时才会收敛。对于不合适的初始值，牛顿方法甚至在二次方程下也会发散。一个常见的方法是使用阻尼版本
 * $\alpha<1$
 ...
 * 并将三角形在MPI行列中的分布写进输出文件。最后，我们通过调用
 * DataOut::build_patches().
 * 生成解决方案的补丁。然而，由于我们有一个边界弯曲的计算域，我们另外传递
 * <code>mapping</code>
 ...
 * @endcode
 *
 * 我们在下图中显示了二维和三维问题的解决方案。 <div
 * class="twocolumn" style="width: 80%; text-align: center;"> <div> <img src =
 ...
 * 此外，我们还应用了一个非常有用的优化，在此应该提及。在
 * <code>compute_update()</code>
 * 函数中，我们在将牛顿更新的矢量作为输出矢量传递给求解器之前，明确地重置了该矢量。在这种情况下，我们为CG方法使用了一个零的起始值，这比之前的牛顿更新更合适，
 * <code>newton_update</code>
 ...
 * Newton iteration</h4> 除了在 step-15
 * 中已经提到的牛顿迭代的步长控制版本外，我们还可以为牛顿迭代实现一个更灵活的停止准则。例如，我们可以取代对残差的固定公差
 * <code>TOLf</code> and for the Newton updated <code>TOLx</code>
 * ，实现一个具有给定绝对和相对公差的混合误差控制，这样牛顿迭代就可以成功存在，例如。
 * @f{align*}{
 ...
 * methods</h4> 。 类似于 step-50 和提到的 step-75
 * 的可能扩展，你可以说服自己哪个方法更快。 <a
