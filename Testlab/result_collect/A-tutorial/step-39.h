 ...
 *
 * 这里， $\sigma_F$ 是惩罚参数，选择如下：对于单元格<i>K</i>的一个面<i>F</i>，计算值@f[
 * \sigma_{F,K} = p(p+1) \frac{|F|_{d-1}}{|K|_d},
 * @f]，其中<i>p</i>是有限元函数的多项式程度， $|\cdot|_d$ 和 $|\cdot|_{d-1}$ 表示相应对象的 $d$ 和 $d-1$ 维的豪斯多尔夫度。如果该面位于边界，则选择 $\sigma_F = \sigma_{F,K}$  .对于内部面，我们取该面的两个值的平均值。
 * 在我们的有限元程序中，我们区分了三个不同的积分，分别对应于上面的单元格、内部面和边界面的总和。由于
 * MeshWorker::loop
 * 为我们组织了和，我们只需要实现每个网格元素的积分。下面的MatrixIntegrator类有这三个函数用于公式的左边，RHSIntegrator类用于右边。
 ...
 * 这个程序与 step-12
 * b有关，因为它使用MeshWorker和非连续Galerkin方法。在那里，我们解决的是一个平流问题，而这里是一个扩散问题。在这里，我们还使用了多网格预处理和一个理论上合理的误差估计器，见Karakashian和Pascal(2003)。Kanschat(2004)详细讨论了多层次方案。Hoppe,
 * Kanschat和Warburton
 ...
 * 边界和通量项需要一个惩罚参数，这个参数应该根据单元的大小和多项式的度数来调整。在
 ...
 * <a name="Postprocessingofthelogfile"></a><h3>Postprocessing of the
 * logfile</h3> 。
 *
 *  <img
