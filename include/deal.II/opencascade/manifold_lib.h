...
namespace OpenCASCADE
{
   ...
   * OpenCASCADE utilities.     This class makes no assumptions on the shape
   * you pass to it, and the   topological dimension of the Manifold is
   * inferred from the TopoDS_Shape   itself. In debug mode there is a sanity
   * check to make sure that the   surrounding points (the ones used in
   * project_to_manifold()) actually live   on the Manifold, i.e., calling
   * OpenCASCADE::closest_point()   on those   points leaves them untouched.
   * If this is not the case, an   ExcPointNotOnManifold is thrown.     This
   * could happen, for example, if you are trying to use a shape of type
  ...
  public:
     ...
     * tolerance used to compute distances internally.         The
     * TopoDS_Shape can be arbitrary, i.e., a collection of shapes, faces,
     ...
     * exception is thrown.         The projected point is computed using
     * OpenCASCADE normal projection     algorithms.
     *
     */
   ...
   * ExcPointNotOnManifold is thrown.     Notice that this type of Manifold
   * descriptor may fail to give results if   the triangulation to be refined
   * is close to the boundary of the given   TopoDS_Shape, or when the
   * direction you use at construction time does not   intersect the shape. An
   * exception is thrown when this happens.
   *
   */
  template <int dim, int spacedim>
  ...
  public:
     ...
     * tolerance from the given TopoDS_Shape. If this is not the case, an
     * exception is thrown.         The projected point is computed using
     * OpenCASCADE directional     projection algorithms.
     *
     */
    virtual Point<spacedim>
    project_to_manifold(
   ...
   * surrounding points (hence mesh cell) normal.     The direction normal to
   * the mesh is particularly useful because it is the   direction in which
   * the mesh is missing nodes. For instance, during the   refinement of a
   * cell a new node is initially created around the   baricenter of the cell.
   * This location somehow ensures a uniform distance   from the nodes of the
   * old cell. Projecting such cell baricenter onto the   CAD surface in the
   * direction normal to the original cell will then retain   uniform distance
   * from the points of the original cell. Of course, at the   stage of mesh
   * generation, no dof handler nor finite element are defined,   and such
   * direction has to be estimated. For the case in which 8   surrounding
   * points are present, 4 different triangles are identified with   the
   * points assigned, and the normals of such triangles are averaged to
   * obtain the approximation of the normal to the cell.     The case in which
   * 2 surrounding points are present (i.e.:a cell edge is   being refined) is
   * of course more tricky. The average of the CAD surface   normals at the 2
   * surrounding points is first computed, and then projected   onto the plane
   * normal to the segment linking the surrounding points. This   again is an
   * attempt to have the new point with equal distance with   respect to the
   * surrounding points     This class only operates with CAD faces and makes
   * the assumption that the   shape you pass to it contains at least one
   * face. If that is not the case,   an Exception is thrown. In debug mode
   * there is a sanity check to make   sure that the surrounding points (the
   * ones used in project_to_manifold())   actually live on the Manifold,
   * i.e., calling   OpenCASCADE::closest_point()     on those points leaves
   * them untouched. If this is not the case, an   ExcPointNotOnManifold is
   * thrown.       Notice that this type of Manifold descriptor may fail to
   * give results if   the triangulation to be refined is close to the
   * boundary of the given   TopoDS_Shape, or when the normal direction
   * estimated from the surrounding   points does not intersect the shape.  An
   * exception is thrown when this   happens.
   *
   */
  template <int dim, int spacedim>
  ...
  public:
    /**
     * Construct a Manifold object which will project points on the
     * TopoDS_Shape   @p sh,   along a direction which is approximately normal
     * to     the mesh cell.
     *
     */
   ...
   * ChartManifold class.     This class can only work on TopoDS_Edge or
   * TopoDS_Wire objects, and it   only makes sense when spacedim is three. If
   * you use an object of   topological dimension different from one, an
   * exception is throw.     In debug mode there is an additional sanity check
   * to make sure that the   surrounding points actually live on the Manifold,
   * i.e., calling     OpenCASCADE::closest_point()   on those points leaves
   * them untouched. If   this is not the case, an ExcPointNotOnManifold is
   * thrown.
   *
  ...
  public:
    /**
     * The constructor takes an OpenCASCADE TopoDS_Face   @p face   and an
     * optional       @p tolerance.   This class uses the interval OpenCASCADE
     * variables u, v     to describe the manifold.
     *
     */
    ...
    /**
     * Given a   @p chart_point   in the uv coordinate system, this method
     * returns the     Euclidean coordinates associated.
     *
     */
    virtual Point<spacedim>
     ...
     * the uv coordinate system to the Euclidean coordinate     system. In
     * other words, it is a matrix of size
     * $\text{spacedim}\times\text{chartdim}$  .         This function is used
     * in the computations required by the     get_tangent_vector() function.
  ...
  protected:
    /**
     * Return a tuple representing the minimum and maximum values of u     and
     * v.  Precisely, it returns (u_min, u_max, v_min, v_max)
     *
     */
