 ...
 * that relate 到自动和符号微分。
 * 下面我们将非常简要地介绍什么是自动微分和符号微分，这些计算/数字方案有哪些变化，以及它们如何被整合到deal.II的框架中。所有这些方案的目的是在不想手工计算的情况下自动计算函数的导数或其近似值。常见的例子是，在有限元背景下，人们想解决一个非线性问题，要求一些残差
 * $F(u,\nabla u)=0$ ，其中 $F$
 * 是一个复杂的函数，需要进行微分以应用牛顿方法；还有一种情况是，人们得到一个与参数有关的问题
 * ${\cal A}(q,u,\nabla u) = f$  ] 并希望对参数 $q$
 * 形成导数的情况，例如，为了优化 $q$
 * 的输出函数，或为了对 $q$ 的敏感性分析。我们应该把 $q$
 ...
 *
 *
 * @section  auto_diff_1 自动分化 <a
 * href="https://en.wikipedia.org/wiki/Automatic_differentiation">Automatic
 * differentiation </a>
 * （通常也被称为算法微分），是一种数字方法，可以用来
 * "自动
 * "计算函数对一个或多个输入变量的一阶，或许还有高阶导数。尽管这需要一定的计算成本，但使用这种工具的好处可能是巨大的。如果使用得当，通常复杂的函数的导数可以被计算得非常精确。尽管这些框架所能达到的确切精度在很大程度上取决于它们的基础数学公式，但一些实现方式的计算精度与机器精度相当。请注意，这与经典的数值微分（例如，通过在不同的点上对函数进行评估，使用有限差分近似）不同，后者的精度取决于扰动的大小以及所选择的有限差分方案；这些方法的误差要明显大于表述良好的自动微分方法。
 ...
 *
 *
 * - 在复杂的多物理场框架内自动线性化有限元残差加成，以及
 *
 * - 验证用户对基于单元的计算（如残差）和基于连续体点的计算（如非线性构成法的切线）的线性化实现。
 * 有相当多的自动微分数的实现方法。它们主要分为两大类，即
 * <em>  源代码转换  </em>  和  <em>  操作符重载  </em>
 * 。第一种方法是根据一些输入函数生成新的、可编译的代码，当执行时，返回输入函数的导数。第二种方法是利用<tt>C++</tt>运算符定义的能力，为自定义类的类型进行重载。因此，一个代表这样一个可自动微分的数字的类，在对它进行每一次数学运算后，原则上可以评估和跟踪它的值以及它的方向性导数。由于专门实现
 * <em> 源代码转换 </em>
 ...
 * 基于 <em> 运算符重载 </em>
 * 的专门框架的实现通常属于三类之一。在每一类中，一些定制的数据类代表了被评估函数的浮点值和它的导数，由
 *
 *
 *
 * - 利用 <em> 对偶 </em> / <em> 复步 </em> / <em> 超对偶 </em> 公式（偶尔也称为 <em> 无标记 </em> 方法）。
 *
 *
 *
 * - 那些利用 <em> 拍打 </em> 策略的方法，以及
 *
 *
 *
 * - 那些通过 <em> 表达式模板 </em> 使用的编译时优化。
 * 为了对这些不同的实现方式在实践中的表现提供一些初步的见解，我们对这些方法做了以下的一般性总结。
 *
 *
 *
 * - 上面列出的前两种 <em> 无龙头 </em> 方法（对偶数和复步法）使用截断泰勒级数的某种变化，以及对扰动参数定义的特殊选择，使用基于有限差分的方法来计算函数导数。双重 "数构成了在函数值被评估时同时计算的累积方向导数；在复数步法中，虚值有效地起到了这个作用。扰动参数的选择决定了方案的数值质量，例如泰勒方案截断的影响；对偶数在其一阶导数中不包含任何高阶项，而对于复数阶方法，这些存在的高阶项被忽略了。可以证明，这两种方法都不受减法取消误差的影响，而且在其有限差分方案中，它们在数值上对为数值扰动选择的内部 step- 大小不敏感。因此，对偶数方法产生精确的一阶导数，而复数步骤近似法则不能。然而，对偶数的标准实现不能产生精确的二阶导数值。超对偶数对这一想法有不同的看法，数字以类似于四元数的形式表示（即带有额外的非实数成分），导数由泰勒级数所有四个成分的高阶截断来计算。其结果是，通过适当的实现，第一和第二导数都可以准确计算出来。
 *
 *
 *
 * - 使用 <em> 录音 </em> 方法，选择一个指定的代码子区域，对该区域所有用活动（标记）输入变量执行的操作进行跟踪，并记录在一个被称为录音带的数据结构中。在磁带区域结束时，可以通过用不同的输入变量集 "重放 "磁带来重新评估所记录的函数，而不是直接重新计算该函数。假设录音区域代表一个平滑的函数，那么该函数的任意高阶导数可以通过参考跟踪和存储在磁带上的代码路径来计算。   例如，这可以通过对感兴趣点周围的函数进行评估来实现）。有一些策略可以处理这样的情况：在被评估的点上，所录取的函数是不平滑的，或者它不是分析的。此外，我们可能需要考虑分支函数的情况，即磁带不再是连续的，而是在不同的评估路径上分叉，而这是由于最初记录的输入。
 *
 *
 *
 * - 基于<a href="https://en.wikipedia.org/wiki/Expression_templates">expression templates</a>的方法利用了由抽象语法树（AST）构建的计算图（在这种情况下，是<a href="https://en.wikipedia.org/wiki/Directed_acyclic_graph">directed acyclic graph (DAG)</a>），从其输入值中解析出函数输出。   树上最外层的叶子代表独立变量或常数，并由单数运算符转换，由二进制运算符连接（在最简单的情况下）。因此，在编译时，对函数输入进行的操作是已知的，与此相关的导数操作也可以同时使用众所周知的计算运算导数的规则（如加减法下导数的关联性、乘积规则和连锁规则）来定义。这个运算器返回的编译输出类型不需要是通用的，而是可以根据DAG顶点上给该特定运算器的特定输入（可能携带微分历史）进行专业化。通过这种方式，可以为用于评估依赖函数的每个中间结果的非常专门的个别操作生成一套编译时优化的指令。
 * 当然，这些方法中的每一种都有其优点和缺点，对于要解决的特定问题来说，一种可能比另一种更合适。由于上述实施细节（以及其他未讨论的细节）可能对用户是隐藏的，因此了解使用这些
 * "黑盒子
 ...
 *
 *
 * -  <em> 正向模式 </em> （或 <em> 正向累加 </em> ）自动差分，或
 *
 *
 *
 * -  <em> 反向模式 </em> （或 <em> 反向累积 </em> ）自动分化。
 ...
 *
 * @subsection  auto_diff_1_1 支持的自动微分库
 * 我们目前已经验证了以下数字类型和组合的实现。
 *
 *
 *
 *
 *
 * - 录音的ADOL-C（理论上是可微分的，但将实现最多二阶导数的内部驱动）。
 * - 无带子的ADOL-C（一旦可区分）。
 *
 *
 ...
 * @endcode
 *
 * 同样，为了解Sacado数字类型的实现（特别是表达式模板的使用和利用），选择了一些有用的资源，其中包括：1:
 *
 *
 * @code{.bib}
 ...
 * 由于每个自动微分库的接口都有很大的不同，在不久的将来会建立一个统一的内部接口给每个数字。目标将是让一些驱动类（提供核心功能，以后将在下一节介绍）有一个一致的机制与不同的自动区分库互动。具体来说，它们需要能够正确地初始化和最终确定将被解释为公式的因变量和自变量的数据。
 * 实现支持的自动差分数字的接口的文件摘要如下。
 *
 *
 *
 * - ad_drivers.h: 提供作为内部支持的自动差分库接口的驱动的类。这些在内部被用作我们提供的帮助类的中介。
 *
 *
 *
 ...
 *
 *
 * - ad_number_types.h: 引入了一个枚举（称为类型代码），用于驱动类所支持的可自动区分的数字组合。  下面将讨论使用这种有点限制性的机制的理由。
 *
 *
 *
 * - ad_number_traits.h: 声明一些内部的类，这些类将被专门用于每个自动区分库和/或数字类型。这些随后被用来通过NumberTraits和ADNumberTraits类提供一个统一的接口，这些类在整个驱动中被广泛使用。我们还提供了一些机制来轻松查询这些数字的选择属性，即一些类型特征。
 *
 *
 *
 * - adolc_math.h: ADOL-C数学运算的扩展，允许这些数字在整个库中被一致使用。
 *
 *
 *
 ...
 *
 *
 * - adolc_product_types.h: 定义了一些乘积和标量类型，允许与Tensor和SymmetricTensor类一起使用ADOL-C数。
 *
 *
 *
 * - sacado_math.h: 对Sacado数学运算的扩展，允许这些数字在整个库中被一致使用。
 *
 *
 *
 ...
 * 通过对每个支持的数字类型使用类型代码，我们人为地限制了可以在库中使用的自动差分的数字类型。这种设计选择是由于确保每个数字类型被正确初始化，以及所有嵌套（模板化）类型的组合对库中执行的所有操作保持有效并不是一件小事。此外，在库中有一些冗长的函数，为支持的数字类型进行实例化，并有内部检查，只有在使用库所知道的可自动区分的数字时才会满足。这再次确保了所有计算的完整性得到维护。最后，使用一个简单的枚举作为类模板参数，最终使得在生产代码中切换使用的类型变得非常容易，几乎不需要对用户代码进行进一步的修改。
 * @subsubsection  auto_diff_1_3 自动分化库的用户接口
 * deal.II库为我们支持的自动区分库提供了一个统一的接口。到目前为止，已经为以下情况开发了帮助类。
 *
 *
 *
 * - 旨在在正交点层面（或任何一般连续点）操作的类。
 *
 ...
 * differentiation</a>就其设计和使用而言，与自动微分相当不同。任何符号库的基础都是一个计算机代数系统（CAS），它实现了一种语言和一系列的算法来处理符号（或
 ...
 * @p BatchOptimizer
 * 类（本身经常利用SymEngine提供的功能）加速冗长符号表达式的评估。特别是，
 * @p BatchOptimizer
 ...
 *
 *
 * - symengine_math.h: 实现数学运算，使实现符号表达式的类在整个库和用户代码中得到一致使用。  它为标准命名空间中的许多数学函数提供对应的定义。
 *
 *
 *
 * - symengine_number_traits.h: 提供了一些机制来轻松查询符号数的选择属性，即一些类型特征。
 *
 *
 *
 ...
 *
 *
 * - symengine_scalar_operations.h: 定义了可以对标量符号表达式或变量进行的众多操作。  这包括（但不限于）创建标量符号，对标量进行微分，以及在标量表达式中进行符号替换。
 *
 *
 *
 * - symengine_tensor_operations.h: 定义了可以对符号表达式或变量的张量进行的众多操作，也可以使用这些操作。  这包括（但不限于）创建符号的张量，对符号的张量进行微分，对符号的张量进行微分，以及在张量表达式中进行符号替换。
 *
 *
 *
 * - symengine_types.h: 为一些在符号计算背景下常用的类型提供别名。
 *
 * - symengine_utilities.h。提供了一些在符号计算背景下有用的实用函数。
 *
 *
 */
