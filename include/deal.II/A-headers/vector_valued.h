 ...
 *
 * 矢量值问题是偏微分方程系统。这些问题的解变量不是一个标量函数，而是一个矢量值函数或一组函数。这包括，例如。  <ul>   <li>   step-8 、 step-17 和 step-18 中讨论的弹性方程，其解是每一点的向量值位移。    <li>  在 step-20 和 step-21 中讨论的混合拉普拉斯方程及其扩展，其解是每一点的标量压力和矢量速度。    <li>  在 step-22 中讨论的斯托克斯方程及其扩展，以及 step-31 ，其中的解也是每一点的标量压力和矢量速度。    <li>  由实部和虚部组成的复值解，例如在  step-29  中讨论。  </ul>
 ...
 * @f] 这里，<b>D</b>表示由 $\mathbf Du = \tfrac12 (\nabla \mathbf u + (\nabla \mathbf u)^T)$ 定义的对称梯度，冒号表示两个等级为2的张量的双重收缩（Frobenius内部积）。这个双线性形式看起来确实非常像  step-3  中泊松问题的双线性形式。唯一的区别是  <ol>   <li>  我们用对称梯度替换了梯度算子；这实际上不是一个重大的区别，如果你用  $\nabla$  替换  $\mathbf D$  ，这里说的一切都是真的。事实上，让我们这样做来简化讨论。@f[
 ...
 * @f]。 我们看到，这只是拉普拉斯的双线性形式的三份拷贝，一份应用于每个分量（这是与 $\mathbf D$ 的表述更令人兴奋的地方，我们希望推导出一个也适用于该表述的框架）。我们可以通过选择特殊的测试函数使这个弱形式再次成为微分方程组：首先，选择<b>v</b>=（<i>v</i><sub>1</sub>,0,0）<sup>T</sup>，然后<b>v</b>=（0，<i>v</i> ]<sub>2</sub>,0)<sup>T</sup>, 最后<b>v</b>=(0,0,<i>v</i><sub>3</sub>)<sup>T</sup>. 将这些结果写在彼此下面，我们得到系统 @f[
 * \begin{matrix} (\nabla u_1,\nabla v_1) &&& = (f_1, v_1) \\ & (\nabla
 * u_2,\nabla v_2) && = (f_2, v_2) \\ && (\nabla u_3,\nabla v_3) & = (f_3,
 * v_3) \end{matrix} @f] ，我们使用标准内部积符号  $(\mathbf
 * f,\mathbf g) =
 ...
 * 对于这个问题和其他矢量问题，获得弱式或变式的系统方法是，首先将其视为一个问题，其中运算符和解变量以矢量和矩阵形式书写。对于这个例子，这将是如下的：@f{eqnarray*}
 * \left(
 ...
 * 接下来，我们需要考虑测试函数  $V$  。我们要将方程的两边都与它们相乘，然后对 $\Omega$ 进行积分。结果应该是一个标量的相等。我们可以通过选择 $V$ 来实现这一点，它也是以@f{eqnarray*}
 * V =
 * \left(
 ...
 * 一旦我们确定了双线性形式和函数设置，我们就需要找到一种方法来描述矢量值的有限元空间，我们从中提取解和测试函数。这就是FESystem类的作用：它由较简单的空间组成矢量值的有限元空间。在弹性问题的例子中，我们需要同一元素的
 * <code>dim</code> 份，例如
 *
 * @code
 ...
 * 的向量空间，其中每个分量都是FE_Q类型的连续双线性元素。它将有
 * <code>dim</code>
 ...
 * 同一不连续元素的相等拷贝，这并不能真正反映系统的结构。因此，我们倾向于
 *
 * @code
 * FESystem<dim> ldg_equal_element (FESystem<dim>(FE_DGQ<dim>(1), dim), 1,
 *                                  FE_DGQ<dim>(1),                     1);
 * @endcode
 * 在这里，我们有一个由两个元素组成的系统，一个是矢量值，一个是标量值，与
 * <code>rt_element</code>
 * 非常相似。事实上，在许多代码中，这两个可以互换。这个元素也允许我们很容易地切换到速度的低阶近似的LDG方法，即
 *
 * @code
 ...
 * </td><td>2</td> </table> </table
 * 从这个表中可以看出，FES系统反映了 <code>rt_element</code>
 * 和 <code>ldg_equal_element</code>
 * 情况下微分方程系统的很多结构，因为我们有一个矢量值和一个标量变量。另一方面，卷积元素没有这种结构，我们必须在组装系统时以某种方式重构它，如下所述。
 ...
 * @endcode
 *
 * 所以这里是发生了什么。  <ul>   <li>  我们做的第一件事是声明 "提取器"（见FEValuesExtractors命名空间）。这些对象除了存储矢量值有限元的哪些分量构成单一的标量分量或秩1的张量（即我们所说的 "物理矢量"，总是由 <code>dim</code> 分量组成）外，没有什么作用。在这里，我们声明一个对象，表示由 <code>dim</code> 分量组成的速度，从零分量开始，以及压力的提取器，它是位置 <code>dim</code> 的标量分量。
 * <li>
 ...
 *
 * @note
 * 在目前的语境中，当我们谈论一个矢量时（例如在提取上面的速度分量时），我们指的是物理学上使用的这个词：它有
 * <code>spacedim</code>
 * 个分量，在坐标系变换下以特定方式表现出来。例子包括速度场或位移场。这与数学中使用
 * "矢量
 ...
 * 在有些情况下，可以利用所使用的有限元的知识，对矩阵或右手向量的装配进行一些优化。例如，考虑我们在
 ...
 * 中详细解释了可以用它做什么，使用上面的例子。
 *
 *  @anchor  VVBlockSolvers <h3>Block solvers</h3> 。
 * 使用如上所示的技术，对于一个矢量值的问题，组装线性系统，即矩阵和右手边，并不是特别复杂。然而，然后它还必须被解决。这就比较复杂了。直观地说，我们可以只把矩阵作为一个整体来考虑。对于大多数问题，这个矩阵不会是确定的（除了特殊情况，如
 * step-8 和 step-17
 ...
 * 这就是BlockVector、BlockSparseMatrix和类似的类的用处。为了所有的实际目的，那么可以作为常规的向量或稀疏矩阵使用，也就是说，它们提供元素访问，提供常规的向量操作，并实现例如矩阵-向量乘法。换句话说，组装矩阵和右手边的工作方式与非块版本完全相同。也就是说，在内部，它们以
 * "块
 ...
 * @endcode
 *
 * 这里发生的事情是，我们分配一个临时向量，其元素数与解向量的第一块，即速度分量
 * $U$ 一样多。然后我们将这个临时向量设置为等于矩阵的
 * $(0,1)$ 块，即 $B$
 ...
 * @endcode
 *
 * 之后，变量 <code>local_solution_values</code>
 ...
 * @endcode
 * 结果，我们在这里立即得到了速度，而且是正确的数据类型（因为我们已经用提取器描述了有限元的第一个
 * <code>dim</code>
 * 分量属于一起，形成一个张量）。这段代码也更有效率：它需要更少的动态内存分配，因为张量类将其成分作为成员变量而不是在堆上分配，而且我们节省了周期，因为我们甚至不需要费力计算正交点上的压力变量值。另一方面，如果我们只对压力而不是速度感兴趣，那么以下提取标量值的代码就可以做到。
 *
 * @code
 ...
 * @endcode
 *
 * 在类似的情况下，我们有时需要解的梯度或二阶导数，或者个别标量或矢量分量。为了得到解的所有分量，函数
 ...
 * FEValuesViews::Vector::get_function_laplacians.  。   @anchor  VVOutput
 ...
 * @endcode
 * 换句话说，我们在这里创建了一个由 <code>dim+1</code>
 * 元素组成的数组，在这个数组中，我们存储了有限元的哪些元素是向量，哪些是标量；该数组由
 * <code>dim</code> 的副本和
 * DataComponentInterpretation::component_is_scalar
 * 的单一尾部元素填充。然后，该数组被作为一个额外的参数给到
