 ...
 * 函数在deal.II的不同地方使用，例如，描述边界条件、方程中的系数、强制项或精确解。由于方程的封闭式表达式通常很难作为函数参数传递，deal.II使用函数基类来描述这些对象。基本上，这个基类的接口要求派生类实现返回一个或一列特定位置的函数值的能力，以及可能的（如果需要）函数的梯度或二次导数。有了这个，函数对象就可以被像
 ...
 * 对于随时间变化的计算，边界条件和/或右手边的函数也可能随时间变化。由于在一个给定的时间步长，人们通常只对函数的空间依赖性感兴趣，如果必须向所有使用函数对象的方法传递一个时间变量的值，那就很尴尬了。例如，
