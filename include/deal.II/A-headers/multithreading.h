 ...
 * 在有多个处理器（或多核处理器）的机器上，以%并行方式运行计算的几个部分通常是有利的。例如，我们可以有几个线程在%并行运行，每个线程组装三角形的一个子集的单元格矩阵，然后将它们写入全局矩阵。由于组装矩阵通常是一个昂贵的操作，这经常导致在多处理器机器上显著节省计算时间。
 * 通过Threads命名空间的函数和类，deal.II支持在共享内存（SMP）机器上以%并行方式运行的操作。MultithreadInfo类允许查询系统的某些属性，如CPU的数量。这些用于%并行计算的设施将在下文中描述。
 * step-9 、 step-13 、 step-14 、 step-32 、 step-35 和 step-37
 * 的教程程序也展示了它们在实践中的使用，其中从 step-32
 * 开始的程序采用了更现代的做事风格，基本上我们描述<i>what</i>可以用%parallel完成，而旧的教程程序描述<i>how</i>事情必须要用%parallel完成。
 ...
 * 在共享内存机器上并行的传统观点是将程序分解成<i>threads</i>，即以%并行<i>at the same time</i>运行程序的不同部分。 ]（如果你的机器上的线程多于处理器内核，操作系统会在将执行切换到另一个线程之前短暂地轮流运行每个线程，从而模拟线程并发运行）。下面描述了deal.II的线程设施（见 @ref MTThreads  "基于线程的并行性"
 * ），但我们首先想讨论一个通常比线程更合适的抽象。<i>tasks</i>.
 * 任务本质上是一个程序的各个部分。其中一些任务是独立的，而另一些任务则依赖于之前的任务，要先完成。举例来说，考虑一下大多数教程程序的
 ...
 * @endcode
 *
 * 这里，每个操作都需要大量的计算。但要注意的是，并不是所有的都互相依赖：显然我们不能在1之前运行语句2-4，4需要等待语句2和3的完成。但语句2和3是独立的：它们可以以任何顺序运行，或以%并行方式运行。实质上，我们已经确定了四个<i>tasks</i>，其中一些是相互依赖的，而其他是独立的。在目前的例子中，任务被识别为单独的C++语句，但往往它们更普遍地与整个代码块相吻合。
 ...
 * @endcode
 *
 * 但如果你的电脑只有一个处理器核心，或者我们有两个处理器核心，但已经有一个不同的程序部分与上面的代码%并行运行，怎么办？在这种情况下，上面的代码仍然会启动新的线程，但程序不会运行得更快，因为没有额外的计算资源可用；相反，程序会运行得更慢，因为线程必须被创建和销毁，而且操作系统必须将线程安排到超额的计算资源上。
 ...
 * <code>new_task</code> ，对于线程则是类似的关键字
 * <code>new_thread</code>
 * ）来表明函数调用应该在一个单独的任务中运行。前缀的调用将返回一个任务的句柄，我们可以用它来等待任务的完成，也可以用它来查询被调用函数的返回值（除非它是空的，就像这里一样）。
 * 由于C++不支持创建新的关键字，我们必须要有一点创造性。所选择的方式是引入一个函数
 * <code>new_task</code>
 * ，该函数将调用的函数以及调用的参数作为参数。
 * <code>new_task</code>
 * 函数被重载，以适应没有、1个、2个和最多9个参数的函数启动任务。在deal.II中，这些函数生活在Threads命名空间中。因此，我们在上面尝试做的实际代码看起来是这样的。
 *
 * @code
 * Threads::Task<void>
 *   thread
 ...
 * 会看到），就像它是函数的第一个参数。其次，注意我们如何通过调用
 ...
 *   double result = task.return_value();
 * @endcode
 * 在这里，我们让 <code>C::f</code> 计算其返回值为
 * <code>c.f1(i)+c.f2(i)</code>
 * 。如果有足够的CPU资源，那么加法的两部分以及
 * <code>main()</code>
 * 中的其他东西都将以%的速度并行运行。如果没有，那么我们最终会在其中一个需要返回值的地方阻塞，从而释放出必要的CPU资源来运行所有这些生成的任务来完成。
 *
 *  在很多情况下，比如上面概述的 <code>setup_dofs</code>
 * 函数的介绍性例子，人们可以确定几个独立的作业，可以作为任务运行，但要等待所有的任务在某一点上完成。我们可以通过存储所有
 * Threads::new_task() 调用的返回对象，并对其中每一个调用
 * Threads::Task::join()
 * 来做到这一点。一个更简单的方法是将所有这些任务对象放入一个
 * Threads::TaskGroup
 * 对象中，并一次性地等待所有的任务。然后代码会是这样的。
 *
 * @code
 * dof_handler.distribute_dofs (fe);
 *
 ...
 * 缺点是，只有当线程真正在做一些事情时，CPU核心才会被充分利用，这意味着（i）必须有足够的任务可用，以及（ii）这些任务实际上在做一些事情。请注意，这两个条件都必须满足；特别是，这意味着，如果我们已经确定了足够数量的任务，但如果其中一些任务踌躇不前，例如，因为一个任务正在向磁盘写入数据（这个过程中，CPU经常需要等待磁盘完成交易）或正在等待输入，那么CPU内核就没有得到充分利用。其他情况是，任务在其他外部事件上阻塞，例如通过突扰器与其他任务或线程同步。在这种情况下，调度器会让一个任务在一个线程上运行，但并没有注意到这个线程并没有完全利用CPU核心。
 ...
 * 有些循环在数据上执行的机构是完全独立的，因此可以以%并行方式执行。TBB库不是先验地将循环分割成固定数量的块并在任务或线程上执行，而是使用以下概念：循环迭代的范围被分割成一定数量的子范围（例如CPU核数的2或3倍），并平均分配给线程；然后线程执行子范围，如果它们完成了工作，就从其他线程那里偷取整个或部分子范围以保持忙碌。这样一来，即使不是每个循环迭代都需要同样多的工作，或者一些CPU核心因为操作系统中断了其他工作而落后，工作也是平衡的。
 * TBB库的原语有点笨拙，所以deal.II为最经常使用的操作提供了封装例程。最简单的一个类似于
 * std::transform
 ...
 * @endcode
 *
 * 在这个例子中，我们使用了<i>lambda
 * expression</i>来构建一个函数对象，它接受两个参数并返回两个参数的和。当我们想把向量
 * $x$ 和 $y$ 的各个元素相加，并把两者之和写入 $z$
 * 的元素中时，这正是我们需要的。我们在这里得到的函数对象完全为编译器所知，当它展开
 * parallel::transform
 * 所产生的循环时，就像我们以其明显的形式写出的循环。
 *
 * @code
 ...
 * @endcode
 *
 * 还要注意的是，我们已经确保了没有任何一个CPU能够得到整个循环中小于1000次迭代的那一块（除非整个范围更小）。
 *
 ...
 *  }
 * @endcode
 * 在for循环里面，我们计算矩阵的单行与右侧向量
 * <code>src</code> 的点积，并将其写入 <code>dst</code>
 * 向量的相应元素中。通过利用<i>next</i>行的元素跟随当前行<i>immediately</i>的元素，代码变得更加高效，也就是说，在循环体的开始，我们不必重新设置指向每一行的值和列数的指针。
 ...
 * 函数认为传递的函数参数是单数。还要注意的是，我们需要把源向量和目的向量作为（const）引用，以防止
 ...
 * 一个更有效的方法是让TBB将原始范围分割成子范围，然后不是在循环的每个元素上调用目标函数，而是在整个范围上调用。
 * parallel::apply_to_subranges 函数为这个方法提供了便利。
 *
 * @code
 *  void
 ...
 * @endcode
 *
 * 如果我们能把这个操作分成几个子行，每个子行都计算它们的法线平方的部分，把各个子行的结果加在一起，然后取结果的平方根，那就更好了。这就是
 * parallel::accumulate_from_subranges
 * 函数的作用（注意，你必须指定结果类型作为模板参数，而且像往常一样，最后一个参数是可以在单个CPU核上调度的外循环元素的最小数量）。
 *
 * @code
 *  double
 ...
 * @endcode
 *
 * 通过使mutex成为静态变量，它在全局范围内只存在一次（即对所有可能并行运行的任务都存在一次），并且只有一个任务可以进入由mutex上的获取/释放调用所保护的区域。顺便说一句，写这段代码的更好方法是这样的，确保即使在抛出异常的情况下也能释放mutex，而且不需要记住写对
 ...
 * }
 * @endcode
 * 这里，从ScopedLock被创建到代码块结束时被销毁，mutex一直被锁定。
 ...
 * 一节中所解释的。
 * <li>
 * 第二个正确性问题是，即使我们使用mutex锁定全局矩阵和右手边的对象，我们也是以一种或多或少的随机顺序这样做的：虽然任务是按照我们正常遍历单元格的顺序创建的，但不能保证当我们到了要把局部复制到全局贡献的时候，顺序仍然是我们按顺序计算的一样。换句话说，我们可能会把单元格1的贡献加在单元格0的贡献之前。这看起来无害，因为加法是交换和关联的，但事实上，如果用浮点运算，就不是这样了。
 * $a+b+c \neq a+c+b$
 *
 ...
 * ，将每个单元上计算的结果复制到全局对象中。在WorkStream类中实现的技巧是：(i)
 * <code>MyClass::copy_local_to_global</code>
 * 永远不会以%并行方式运行超过一次，所以我们不需要通过互斥来同步执行，(ii)它在单元格上的运行顺序与它们在迭代器范围内出现的顺序完全相同，也就是说，我们以同样的方式将元素加入全局矩阵<i>every
 ...
 * @endcode
 *
 * 这样做的方法是，我们创建一个样本 <code>per_task_data</code>
 * 对象，工作流对象将对每个以%并行方式运行的任务复制一次。对于每个任务，这个对象将首先被传递给以%并行方式运行的
 * <code>MyClass::assemble_on_one_cell</code>
 ...
 * }
 * @endcode
 * 就像对 <code>PerTaskData</code>
 * 结构一样，我们将创建一个样本 <code>ScratchData</code>
 * 对象并将其传递给工作流对象，后者将根据需要多次复制它。为了使其发挥作用，
 ...
 * 函数的两个版本：一个是接收一个对象和两个成员函数的地址，另一个是简单地接收两个函数对象，可以分别用三个和一个参数调用。因此，换句话说，以下两个调用完全相同。
 *
 * @code
 *   WorkStream::run (dof_handler.begin_active(),
 *                    dof_handler.end(),
 ...
 * 类似地，让我们假设 <code>MyClass::assemble_on_one_cell</code>
 ...
 * { ... }
 * @endcode
 * 因为WorkStream期望能够只用三个参数来调用worker函数，第一个是迭代器，第二个和第三个是ScratchData和PerTaskData对象，所以我们需要向它传递以下内容。
 * @code
 *   WorkStream::run (dof_handler.begin_active(),
 *                    dof_handler.end(),
 *                    std::bind(&MyClass<dim>::assemble_on_one_cell,
 ...
 *                    per_task_data);
 * @endcode
 * 在这里，我们将对象、线性化点参数和当前时间参数与函数绑定，然后再将其交给
 * WorkStream::run().   WorkStream::run()
 * 然后将简单地用单元格、Scratch和per任务对象调用函数，这些对象将被填入
 ...
 * 类）。原因是，如果任务A需要等待任务B完成某件事情，那么只有在保证任务B最终能够运行并完成任务的情况下，这才会奏效。现在想象一下，你有2个处理器，任务A1和A2目前正在运行；让我们假设他们已经排好了任务B1和B2的队列，现在正用一个突变器等待这些排队的任务完成（部分）工作。由于机器只有两个处理器，任务调度器只有在A1或A2完成后才会启动B1或B2
 *
 ...
 * Thread::Mutex
 * 来保护对任务可能写入的变量的访问，那么情况就不一样了：这种对mutexes的使用是可以的；任务可能只是不想等待另一个任务来做某事。
 *
 *  @anchor  MTThreads <h3>Thread-based parallelism</h3>。
 * 尽管任务是一种更高层次的描述事物的方式，但也有一些情况不适合用任务来描述（对其中一些情况的讨论见上文 @ref MTHow "调度任务如何工作以及基于任务的编程何时不高效"
 ...
 * @endcode
 *
 * 这里， Threads::new_thread
 * 启动了给定的函数，该函数在一个新的线程上写入输出文件，该线程可以与其他所有的东西%并行运行：在与
 * KellyErrorEstimator::estimate() 函数%并行的情况下，
 * DataOut::write_vtk()
 ...
 * tasks</h3>
 * 如前所述，deal.II没有实现对线程的任务调度，甚至没有启动线程本身。TBB库在决定使用多少个线程方面做得很好，他们不建议明确设置线程的数量。然而，在大型对称多处理（SMP）机器上，特别是那些有资源/任务管理器的机器，或者在对某些部分内存的访问是可能的，但对远处的处理器来说非常昂贵的系统上（例如，非常大的NUMA
 ...
 * 中首先调用的函数之一。
 * 如果你用MPI运行你的程序，那么你可以使用MPI_InitFinalize类的构造函数的可选第三个参数来实现同样的目标。
 *
 *
 * @note
