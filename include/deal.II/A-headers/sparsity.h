 ...
 * 几乎所有的有限元公式都会导致矩阵的
 * "稀疏"，即每行非零元素的数量(i)与矩阵的整体大小相比相对较小，并且(ii)以一个固定的数字为界，如果网格被细化则不会增长。在这种情况下，不存储矩阵的<i>all</i>元素，而只存储那些实际（或可能）为非零的元素会更有效。这需要为每一行存储非零项的列索引（我们称之为
 * "稀疏模式"），以及这些非零项的实际值。在实践中，有时会发生一些非零值实际上是零的情况。稀疏模式和稀疏矩阵只打算为<i>may</i>非零的条目提供空间，而且是在我们还不知道这些条目最终会有什么值的时候这样做；如果一个系数或单元格恰好有特定的值，它们可能会有一个零值）。)
 ...
 * 如上所述，要获得对稀疏模式中每一行的最大条目数的良好估计往往很复杂。因此，任何试图用不好的估计来分配一个普通的SparsityPattern都需要大量的内存，几乎所有的内存都不会被使用，并且在压缩时被取消分配。
