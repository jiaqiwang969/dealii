 ...
 *
 * - b\bigr).
 * @f]。 因此，预处理相当于对残差应用一个线性算子，因此，预处理器<i>P<sup>-1</sup></i>的动作被实现为<tt>vmult()</tt>。deal.II中需要预处理器的模板用 @ref ConceptPreconditionerType "预处理器类型概念 "来表示。在实践中，我们通常可以把任何定义了 <code>vmult()</code> and <code>Tvmult()</code> 的矩阵类对象作为一个预处理程序。本模块中的所有预处理类都实现了这个接口。
 ...
 * @endcode
 *
 * 这个函数接收要预处理的矩阵以及其他必要的参数，并设置预处理器的内部结构。
