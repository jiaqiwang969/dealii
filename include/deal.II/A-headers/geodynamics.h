 ...
 * 包含一组程序，它们共同构成了地球动力学示范套件。这些程序的理念是利用地球动力学的应用来演示高级有限元软件的技术，也就是对固体地球过程的研究。通过这样做，这些程序应该为解决实际地球动力学问题的更专业的专用程序提供一个基础，例如作为研究生或博士后工作的一部分。下面将对这些计划的动机进行更深入的讨论。
 * 目前，地球动力学测试套件包含以下程序。
 * -  step-8  : 弹性
 *
 * -  step-18  : 一个%平行弹性求解器
 ...
 *
 *  <h3>Rationale</h3>
 * 自适应网格细化（AMR）长期以来一直被认为是一项关键技术，它将有助于准确和有效地解决一些地球动力学应用的数值问题。它在地球动力学界已经讨论了好几年，并且自CIG成立以来一直是其任务清单上的一个议题。然而，到目前为止，在这个方向上发生的事情还比较少。直到最近，才有了在地球动力学中使用AMR的尝试。CIG于2007年10月在Boulder举办了一次关于AMR技术的研讨会；George
 ...
 * publications</a>介绍了用deal.II获得的结果，但我们知道只有相对较少的应用程序是用deal.II从头开始构建的；所有其他的应用程序都是从某个教程程序的修改开始的。
 * <li>  <i>Training:</i>
