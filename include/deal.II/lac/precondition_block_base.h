 ...
 * block relaxation methods. This class does the book keeping for
 * preconditioners and relaxation methods based on inverting blocks on the
 * diagonal of a matrix. It allows us to either store all diagonal blocks and
 * their inverses or the same block for each entry, and it keeps track of the
 * choice. Thus, after initializing it and filling the inverse diagonal blocks
 * correctly, a derived class can use inverse() with an integer argument
 * referring to the block number. Additionally, it allows the storage of the
 * original diagonal blocks, not only the inverses. These are for instance
 * used in the intermediate step of the SSOR preconditioner.
 *
 *
 */
template <typename number>
...
public:
  ...
  /**
   * Print some statistics about the inverses to   @p deallog.   Output
   * depends on   #Inversion. It is richest for svd, where we obtain
   * statistics on extremal   singular values and condition numbers.
   *
   */
  void
...
private:
  ...
  /**
   * Storage of the original diagonal blocks.     Used by the blocked SSOR
   * method.
   *
   */
