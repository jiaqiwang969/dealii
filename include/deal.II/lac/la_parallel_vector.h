...
* */
    ...
    /**
     * Implementation of a parallel vector class. The design of this class is     similar to the standard   ::dealii::Vector   class in deal.II, with the     exception that storage is distributed with MPI.         The vector is designed for the following scheme of parallel     partitioning:       <ul>         <li>   The indices held by individual processes (locally owned part) in     the MPI parallelization form a contiguous range       <code>[my_first_index,my_last_index)</code>  .       <li>   Ghost indices residing on arbitrary positions of other processors     are allowed. It is in general more efficient if ghost indices are     clustered, since they are stored as a set of intervals. The     communication pattern of the ghost indices is determined when calling     the function <code>reinit (locally_owned, ghost_indices,     communicator)</code>, and retained until the partitioning is changed.     This allows for efficient parallel communication of indices. In     particular, it stores the communication pattern, rather than having to     compute it again for every communication. For more information on ghost     vectors, see also the       @ref GlossGhostedVector   "glossary entry on vectors with ghost elements".       <li>   Besides the usual global access operator() it is also possible to     access vector entries in the local index space with the function   @p       local_element(). Locally owned indices are placed first, [0,     locally_owned_size()), and then all ghost indices follow after them     contiguously, [locally_owned_size(),     locally_owned_size()+n_ghost_entries()).       </ul>           Functions related to parallel functionality:       <ul>         <li>   The function   <code>compress()</code>   goes through the data     associated with ghost indices and communicates it to the owner process,     which can then add it to the correct position. This can be used e.g.     after having run an assembly routine involving ghosts that fill this     vector. Note that the   @p insert   mode of   @p compress()   does not set the     elements included in ghost entries but simply discards them, assuming     that the owning processor has set them to the desired value already     (See also the       @ref GlossCompress   "glossary entry on compress").       <li>   The   <code>update_ghost_values()</code>   function imports the data     from the owning processor to the ghost indices in order to provide read     access to the data associated with ghosts.       <li>   It is possible to split the above functions into two phases, where     the first initiates the communication and the second one finishes it.     These functions can be used to overlap communication with computations     in other parts of the code.       <li>   Of course, reduction operations (like norms) make use of     collective all-to-all MPI communications.       </ul>           This vector can take two different states with respect to ghost     elements:       <ul>         <li>   After creation and whenever zero_out_ghost_values() is called (or       <code>operator= (0.)</code>  ), the vector does only allow writing into     ghost elements but not reading from ghost elements.       <li>   After a call to update_ghost_values(), the vector does not allow     writing into ghost elements but only reading from them. This is to     avoid undesired ghost data artifacts when calling compress() after     modifying some vector entries. The current status of the ghost entries     (read mode or write mode) can be queried by the method     has_ghost_elements(), which returns   <code>true</code>   exactly when     ghost elements have been updated and   <code>false</code>   otherwise,     irrespective of the actual number of ghost entries in the vector layout     (for that information, use n_ghost_entries() instead).       </ul>           This vector uses the facilities of the class   dealii::Vector<Number>   for     implementing the operations on the local range of the vector. In     particular, it also inherits thread parallelism that splits most     vector-vector operations into smaller chunks if the program uses     multiple threads. This may or may not be desired when working also with     MPI.         <h4>Limitations regarding the vector size</h4>         This vector class is based on two different number types for indexing.     The so-called global index type encodes the overall size of the vector.     Its type is   types::global_dof_index.   The largest possible value is       <code>2^32-1</code>   or approximately 4 billion in case 64 bit integers     are disabled at configuration of deal.II (default case) or       <code>2^64-1</code> or approximately <code>10^19</code>   if 64 bit     integers are enabled (see the glossary entry on       @ref GlobalDoFIndex       for further information).         The second relevant index type is the local index used within one MPI     rank. As opposed to the global index, the implementation assumes 32-bit     unsigned integers unconditionally. In other words, to actually use a     vector with more than four billion entries, you need to use MPI with     more than one rank (which in general is a safe assumption since four     billion entries consume at least 16 GB of memory for floats or 32 GB of     memory for doubles) and enable 64-bit indices. If more than 4 billion     local elements are present, the implementation tries to detect that,     which triggers an exception and aborts the code. Note, however, that     the detection of overflow is tricky and the detection mechanism might     fail in some circumstances. Therefore, it is strongly recommended to     not rely on this class to automatically detect the unsupported case.         <h4>CUDA support</h4>         This vector class supports two different memory spaces: Host and CUDA. By     default, the memory space is Host and all the data are allocated on the     CPU. When the memory space is CUDA, all the data is allocated on the GPU.     The operations on the vector are performed on the chosen memory space.     From the host, there are two methods to access the elements of the Vector     when using the CUDA memory space:       <ul>         <li>   use get_values():
     * @code
     ...
     * @endcode
     * </ul>       The import method is a lot safer and will perform an MPI communication if     necessary. Since an MPI communication may be performed, import needs to     be called on all the processors.
     * @note   By default, all the ranks will try to access the device 0. This
     * is     fine is if you have one rank per node and one gpu per node. If
     * you     have multiple GPUs on one node, we need each process to access
     * a     different GPU. If each node has the same number of GPUs, this can
     * be done     as follows:     <code> int n_devices = 0;
     * cudaGetDeviceCount(&n_devices); int     device_id = my_rank %
     * n_devices;     cudaSetDevice(device_id);     </code>         <h4>MPI-3
     * shared-memory support</h4>         In Host mode, this class allows to
     * use MPI-3 shared-memory features     by providing a separate MPI
     * communicator that consists of processes on     the same shared-memory
     * domain. By calling     `vector.shared_vector_data();`,     users have
     * read-only access to both locally-owned and ghost values of
     * processes combined in the shared-memory communicator (  @p comm_sm   in
     * reinit()).         For this to work, you have to call the constructor
     * or one of the reinit()     functions of this class with a non-default
     * value for the `comm_sm`     argument, where the argument corresponds to
     * a communicator consisting of     all processes on the same
     * shared-memory domain. This kind of communicator     can be created
    ...
    public:
      ...
      /**
       * Create the vector based on the parallel partitioning described in
       * @p         partitioner. The input argument is a shared pointer, which
       * stores the       partitioner data only once and share it between
       * several vectors with       the same layout.
       *
       */
      Vector(
       ...
       * If the flag   @p omit_zeroing_entries   is set to false, the memory
       * will       be initialized with zero, otherwise the memory will be
       * untouched (and       the user must make sure to fill it with
      ...
      /**
       * Initialize the vector. The local range is specified by   @p         locally_owned_set (note that this must be a contiguous interval,       multiple intervals are not possible). The IndexSet   @p ghost_indices         specifies ghost indices, i.e., indices which one might need to read       data from or accumulate data from. It is allowed that the set of       ghost indices also contains the local range, but it does not need to.             This function involves global communication, so it should only be       called once for a given layout. Use the   @p reinit   function with       Vector<Number> argument to create additional vectors with the same       parallel layout.               @see           @ref GlossGhostedVector   "vectors with ghost elements"
       *
       */
      void
       ...
       * @p partitioner.   The input argument is a shared pointer, which
       * stores       the partitioner data only once and share it between
       * several vectors       with the same layout.             The optional
       * argument   @p comm_sm,   which consists of processes on       the
       * same shared-memory domain, allows users have read-only access to
       * both locally-owned and ghost values of processes combined in the
       * shared-memory communicator. See the general documentation of this
       * class       for more information about this argument.
       *
       */
      void
      reinit(
      ...
      /**
       * Initialize vector with   @p local_size   locally-owned and   @p
       * ghost_size         ghost degrees of freedoms.             The
       * optional argument   @p comm_sm,   which consists of processes on
       * the same shared-memory domain, allows users have read-only access to
       * both locally-owned and ghost values of processes combined in the
       * shared-memory communicator. See the general documentation of this
       * class       for more information about this argument.
       * @note   In the created underlying partitioner, the local index range
       * is         translated to global indices in an ascending and
       * one-to-one fashion,         i.e., the indices of process   $p$   sit
       * exactly between the indices of         the processes   $p-1$   and
       * $p+1$  , respectively. Setting the           @p ghost_size   variable
       * to an appropriate value provides memory space         for the ghost
       * data in a vector's memory allocation as and allows         access to
       * it via local_element(). However, the associated global
       ...
       * could       do this operation with a temporary variable and copying
       * over the data       elements, but this function is significantly more
       * efficient since it       only swaps the pointers to the data of the
       * two vectors and therefore       does not need to allocate temporary
       * storage and move data around.             This function is analogous
       * to the   @p swap   function of all C++       standard containers.
       * Also, there is a global function       <tt>swap(u,v)</tt> that simply
       * calls <tt>u.swap(v)</tt>, again in       analogy to standard
       * functions.
       *
       */
      void
       ...
       * @p in_vector,   and copies all the data.             If one of the
       * input vector or the calling vector (to the left of the
      ...
      //@{
      /**
       * This function copies the data that has accumulated in the data buffer       for ghost indices to the owning processor. For the meaning of the       argument   @p operation,   see the entry on         @ref GlossCompress   "Compressing distributed vectors and matrices"
       * in the glossary.             There are four variants for this
       * function. If called with argument   @p           VectorOperation::add
       * adds all the data accumulated in ghost elements       to the
       * respective elements on the owning processor and clears the
       * ghost array afterwards. If called with argument   @p
       * VectorOperation::insert,   a set operation is performed. Since
       * setting       elements in a vector with ghost elements is ambiguous
       * (as one can set       both the element on the ghost site as well as
       * the owning site), this       operation makes the assumption that all
       * data is set correctly on the       owning processor. Upon call of
       * compress(VectorOperation::insert),   all       ghost entries are thus
       * simply zeroed out (using zero_ghost_values()).       In debug mode, a
       * check is performed for whether the data set is       actually
       * consistent between processors, i.e., whenever a non-zero       ghost
       * element is found, it is compared to the value on the owning
       * processor and an exception is thrown if these elements do not agree.
       * If called with   VectorOperation::min   or   VectorOperation::max,
       * the       minimum or maximum on all elements across the processors is
       * set.
       * @note   This vector class has a fixed set of ghost entries attached
       * to       the local representation. As a consequence, all ghost
       * entries are       assumed to be valid and will be exchanged
       * unconditionally according       to the given VectorOperation. Make
       * sure to initialize all ghost       entries with the neutral element
       * of the given VectorOperation or       touch all ghost entries. The
       * neutral element is zero for         VectorOperation::add   and
       * VectorOperation::insert,   `+inf` for         VectorOperation::min,
       * and `-inf` for   VectorOperation::max.   If all       values are
       * initialized with values below zero and compress is called       with
       ...
       * it takes until all data arrives.             Before the data is
       * actually exchanged, the function must be followed       by a call to
       * @p compress_finish().               In case this function is called
       * for more than one vector before   @p         compress_finish() is
       * invoked, it is mandatory to specify a unique       communication
       * channel to each such call, in order to avoid several       messages
       * with the same ID that will corrupt this operation. Any
       ...
       * warning to prevent this situation.             Must follow a call to
       * the   @p compress_start   function.             When the MemorySpace
       * is CUDA and MPI is not CUDA-aware, data changed on       the device
       * after the call to compress_start will be lost.
       *
       */
      void
       ...
       * the time it takes until all data arrives.             Before the data
       * is actually exchanged, the function must be followed       by a call
       * to   @p update_ghost_values_finish().               In case this
       * function is called for more than one vector before   @p
       ...
       * wait for the communication to finish.             Must follow a call
       * to the   @p update_ghost_values_start   function       before reading
       ...
       * locally owned DoFs.             After calling this method, read
       * access to ghost elements of the       vector is forbidden and an
       * exception is thrown. Only write access to       ghost elements is
       * allowed in this state.               @deprecated   Use
       * zero_out_ghost_values() instead.
       *
       */
       ...
       * other.             Since no data exchange is performed, make sure
       * that neither   @p src         nor the calling vector have pending
       * communications in order to obtain       correct results.
       *
       */
      template <typename Number2>
      void
       ...
       * elements       in   @p V   should be added to the current vector or
       * replace the current       elements. The main purpose of this function
       * is to get data from one       memory space, e.g. CUDA, to the other,
       * e.g. the Host.
       * @note   The partitioners of the two distributed vectors need to be
       * the       same as no MPI communication is performed.
       *
       */
      ...
      /**
       * Change the dimension to that of the vector V. The elements of V are
       * not       copied.
       *
       */
      virtual void
      ...
      /**
       * Import all the elements present in the vector's IndexSet from the
       * input       vector   @p V.     VectorOperation::values     @p
       * operation   is used to decide if       the elements in   @p V
       * should be added to the current vector or replace the       current
       * elements. The last parameter can be used if the same
       * communication pattern is used multiple times. This can be used to
       * improve performance.
       * @note   If the MemorySpace is CUDA, the data in the ReadWriteVector
       * will       be moved to the device.
       *
       */
      virtual void
       ...
       * method       only needs to load three vectors,   @p this,     @p V,
       * @p W,   whereas calling       separate methods means to load the
       * calling vector   @p this   twice. Since       most vector operations
       * are memory transfer limited, this reduces the       time by 25\% (or
       * 50\% if   @p W   equals   @p this).               For complex-valued
       * vectors, the scalar product in the second step is       implemented
       * as         $\left<v,w\right>=\sum_i v_i \bar{w_i}$  .
       *
       */
      virtual Number
       ...
       * form disjoint sets that add up to the complete index set. Obviously,
       * if       a vector is created on only one processor, then the result
       * would       satisfy
       * @code
      ...
      /**
       * Sets all elements of the vector to the scalar   @p s.   If the scalar
       * is       zero, also ghost elements are set to zero, otherwise they
       * remain       unchanged.
       *
       */
      virtual Vector<Number, MemorySpace> &
       ...
       * owned locally.               @deprecated   Use locally_owned_size()
       * instead.
       *
       */
      DEAL_II_DEPRECATED
      ...
      /**
       * Make the   @p Vector   class a bit like the <tt>vector<></tt> class
       * of       the C++ standard library by returning iterators to the start
       * and end       of the <i>locally owned</i> elements of this vector.
       * It holds that end()
       *
       *  - begin() == locally_owned_size().
       * @note   For the CUDA memory space, the iterator points to memory on
       * the       device.
       *
       */
      iterator
      ...
      /**
       * Read and write access to the data in the position corresponding to
       * @p         global_index. The index must be either in the local range
       * of the       vector or be specified as a ghost index at construction.
       * Performance: <tt>O(1)</tt> for locally owned elements that represent
       * a contiguous range and <tt>O(log(n<sub>ranges</sub>))</tt> for ghost
       * elements (quite fast, but slower than local_element()).
       *
      ...
      /**
       * Read access to the data field specified by   @p local_index.
       ...
       * local_index.         Locally owned indices can be accessed with
       * indices         <code>[0,locally_owned_size())</code>  , and ghost
       * indices with indices         <code>[locally_owned_size(),
       * locally_owned_size()+n_ghosts]</code>  .             Performance:
       ...
       * @note   For the CUDA memory space, the pointer points to memory on
       * the       device.
       *
       */
      Number *
       ...
       * the corresponding values are returned in the second.             If
       * the current vector is called   @p v,   then this function is the
       * equivalent       to the code
       * @code
       * for (unsigned int i=0; i<indices.size(); ++i)
       *   values[i] = v[indices[i]];
       * @endcode
       * @pre   The sizes of the   @p indices   and   @p values   arrays must
       * be identical.
       ...
       * argument.             If the current vector is called   @p v,   then
       * this function is the equivalent       to the code
       * @code
       ...
       * return   @p false,   whereas the other processors will return   @p
       * true.
       *
       */
      bool
       ...
       * partitioner is the same on all processors.             This method
       * performs global communication, so make sure to use it       only in a
       * context where all processors call it the same number of       times.
       *
       */
      bool
      ...
      /**
       * Attempt to perform an operation between two incompatible vector
       * types.
       * @ingroup Exceptions
       *
       */
    ...
    private:
       ...
       * vectors. The same applies for complex-valued vectors as for       the
       * add_and_dot() function.
       *
       */
      Number
       ...
       * functions give reasonable results also when used       with several
       * threads.
       *
       */
