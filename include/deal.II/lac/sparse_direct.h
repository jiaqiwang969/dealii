 ...
 * is explained in the   step-22   and   step-29   tutorial programs. This
 * matrix class implements the usual interface of preconditioners, that is a
 * function initialize(const SparseMatrix<double>&matrix,const AdditionalData)
 * for initializing and the whole set of vmult() functions common to all
 * matrices. Implemented here are only vmult() and vmult_add(), which perform
 * multiplication with the inverse matrix. Furthermore, this class provides an
 * older interface, consisting of the functions factorize() and solve(). Both
 ...
 * <a href="http://faculty.cse.tamu.edu/davis/welcome.html">Timothy A.
 * Davis</a>.
 *
 *  <h4>Instantiations</h4> There are instantiations of this class for
 * SparseMatrix<double>, SparseMatrix<float>, SparseMatrixEZ<float>,
 * SparseMatrixEZ<double>, BlockSparseMatrix<double>, and
...
public:
   ...
   * large benefit.     In contrast to the other direct solver classes, the
   * initialization method   does nothing. Therefore initialize is not
   * automatically called by this   method, when the initialization step has
   * not been performed yet.     This function copies the contents of the
   * matrix into its own storage; the   matrix can therefore be deleted after
   * this operation, even if subsequent   solves are required.
   *
   */
   ...
   * solution (exact within the range of numerical accuracy of course).     In
   * other words, this function actually multiplies with the exact inverse
   * of the matrix,   $A^{-1}$  .
   *
   */
  void
   ...
   * @param[in,out]   rhs_and_solution A vector that contains the right hand
   * side       $b$   of a linear system   $Ax=b$   upon calling this
   * function, and that     contains the solution   $x$   of the linear system
   * after calling this     function.     @param[in]   transpose If set to
   * true, this function solves the linear       $A^T x = b$   instead of
   * $Ax=b$  .       @pre   You need to call factorize() before this function
   * can be called.
   *
   */
  void
   ...
   * and solution vector.     If the matrix that was previously factorized had
   * complex-valued entries,   then the `rhs_and_solution` vector will, upon
   * return from this function,   simply contain the solution of the linear
   * system   $Ax=b$  . If the matrix   was real-valued, then this is also
   * true, but the solution will simply   be computed by applying the
   * factorized   $A^{-1}$   to both the   real and imaginary parts of the
...
private:
  ...
  /**
   * The arrays in which we store the data for the solver. These are
   * documented   in the descriptions of the umfpack_*_symbolic() and
   * umfpack_*_numeric()   functions, but in short:
   *
   *
   *
   *
