...
extern "C"
{
  ...
  /**
   * Return internal BLACS value in   @p val   based on the input   @p what
   * and   @p icontxt.     The most common use is in retrieving a default
   * system context (  @p what   = 0,   @p icontxt   is ignored)   to be used
   * in BLACS_GRIDINIT or BLACS_GRIDMAP.
  ...
  /**
   * Map the processes sequentially in row-major or column-major order   into
   * the process grid. Input arguments must be the same on every process.
  ...
  /**
   * Receives a message from a process   @prsrc,     @p csrc   into a general
   * rectangular matrix.
  ...
  /**
   * Sends the general rectangular matrix A to the destination   process   @p
   * rdest     @p cdest   in the process grid.
  ...
  /**
   * Compute the Cholesky factorization of an N-by-N real   symmetric positive
   * definite distributed matrix sub( A ) denoting   A(IA:IA+N-1, JA:JA+N-1).
  ...
  /**
   * Compute the inverse of a real symmetric positive definite   distributed
   * matrix sub( A ) = A(IA:IA+N-1,JA:JA+N-1) using the   Cholesky
  ...
  /**
   * PDTRTRI computes the inverse of a upper or lower triangular   distributed
  ...
  /**
   * Estimate the reciprocal of the condition number (in the   l1-norm) of a
   * real symmetric positive definite distributed matrix   using the Cholesky
   * factorization.
  ...
  /**
   * Compute the Least Common Multiple (LCM) of two positive integers   @p M
   * and   @p N.     In fact the routine Compute the greatest common divisor
   * (GCD) and   use the fact that M*N = GCD*LCM.
  ...
  /**
   * Compute the global index of a distributed matrix entry   pointed to by
   * the local index   @p indxloc   of the process indicated by     @p iproc.
   * @param   indxloc The local index of the distributed matrix entry.
   * @param   nb Block size, size of the blocks the distributed matrix is
   * split   into.     @param   iproc The coordinate of the process whose
   * local array row or column   is to be determined     @param   isrcproc
   * The coordinate of the process that possesses the first   row/column of
   * the distributed matrix     @param   nprocs The total number processes
   * over which the distributed matrix   is distributed
   *
   */
  int
   ...
   *                          + beta sub(C),
   * @f}
   * where     $\mathrm{sub}(C)$   denotes C(IC:IC+M-1,JC:JC+N-1),  and,
   ...
   * matrix A by calling the recommended sequence of ScaLAPACK routines. In
   * its   present form, the routine assumes a homogeneous system and makes no
   ...
   * matrix   B. No communication is performed, pdlacpy performs a local copy
   * $\mathrm{sub}(A) \dealcoloneq \mathrm{sub}(B)$  , where
   * $\mathrm{sub}(A)$     denotes   $A(ia:ia+m-1, ja:ja+n-1)$   and
   * $\mathrm{sub}(B)$   denotes     $B(ib:ib+m-1, jb:jb+n-1)$  .
   *
   */
  void
  ...
  /**
   * Copies the content of a general rectangular distributed matrix   @p A
   * to another distributed matrix   @p B     It is not required that the
   * matrices A and B have the same process grid or   block size, e.g. copying
   * a matrix from a one-dimensional to a   two-dimensional process grid
   * @p ictxt   is a context which is at least a union of all processes in
   * context   A and B
   *
   */
  void
  ...
  /**
   * psyevx computes selected eigenvalues and, optionally, eigenvectors    of
   * a real symmetric matrix A. Eigenvalues/vectors can be selected by
...
* */
  ...
  /*   P_GELS solves overdetermined or underdetermined real linear   systems involving an M-by-N matrix A, or its transpose,   using a QR or LQ factorization of A.  It is assumed that A has full rank.  
* */
  void
...
* */
  ...
  /**
   * psyevr computes selected eigenvalues and, optionally, eigenvectors    of
   * a real symmetric matrix A using a parallel implementation of the MRR
...
/* In the following we have template wrappers for the ScaLAPACK routines wrappers for other numeric types can be added in the future
...
psyevr(const char *  jobz,
       ...
       int *         info)
{
  /*   Netlib ScaLAPACK performs floating point tests (e.g. divide-by-zero) within   the call to pdsyevr causing floating point exceptions to be thrown (at   least in debug mode). Therefore, we wrap the calls to pdsyevr into the   following code to suppress the exception.  
* */
#  ifdef DEAL_II_HAVE_FP_EXCEPTIONS
...
psyevr(const char * jobz,
       ...
       int *        info)
{
  /*   Netlib ScaLAPACK performs floating point tests (e.g. divide-by-zero) within   the call to pssyevr causing floating point exceptions to be thrown (at   least in debug mode). Therefore, we wrap the calls to pssyevr into the   following code to suppress the exception.  
* */
#  ifdef DEAL_II_HAVE_FP_EXCEPTIONS
  fenv_t fp_exceptions;
  feholdexcept(&fp_exceptions);
