...
namespace internal
{
  namespace AffineConstraints
  {
     ...
     * the global entries (global_row): are they obtained directly by some
     * local     entry (local_row) or some constraints (constraint_position).
     * This is not     directly used in the user code, but accessed via the
     * GlobalRowsFromLocal.         The actions performed here correspond to
     * reshaping the constraint     information from global degrees of freedom
     * to local ones (i.e.,     cell-related DoFs), and also transforming the
     * constraint information from     compressed row storage (each local dof
     * that is constrained has a list of     constraint entries associated to
     * it) into compressed column storage based     on the cell-related DoFs
     * (we have a list of global degrees of freedom,     and to each we have a
     * list of local rows where the entries come from). To     increase the
     * speed, we additionally store whether an entry is generated     directly
     * from the local degrees of freedom or whether it comes from a
     * constraint.
     *
     */
    struct Distributing
     ...
     * into a   std::vector<std::pair<uint,double>   > and the row length is
     * kept     fixed at row_length. Both the number of rows and the row
     * length can     change is this structure is filled. In that case, the
     * data is     rearranged. This is not directly used in the user code, but
     * accessed     via the GlobalRowsFromLocal.
     *
     */
    template <typename number>
    struct DataCache
     ...
     * contribution (cell) and their origin (direct/constraint). This     is
     * basically a vector consisting of "Distributing" structs     using
     * access via the DataCache. The structure provides some     specialized
     * sort and insert functions.         In case there are no constraints,
     * this is basically a list of pairs     `<uint,uint>` with the first
     * index being the global index and the second     index the local index.
     * The list is sorted with respect to the global     index.         In
     * case there are constraints, a global dof might get a contribution also
     * because it gets data from a constrained dof. This means that a global
     * dof     might also have indirect contributions from a local dof via a
     * constraint,     besides the direct ones.         The actions performed
     * here correspond to reshaping the constraint     information from global
     * degrees of freedom to local ones (i.e.,     cell-related DoFs), and
     * also transforming the constraint information from     compressed row
     * storage (each local dof that is constrained has a list of
     * constraint entries associated to it) into compressed column storage
     * based     on the cell-related DoFs (we have a list of global degrees of
     * freedom,     and to each we have a list of local rows where the entries
     * come from). To     increase the speed, we additionally store whether an
     * entry is generated     directly from the local degrees of freedom or
     * whether it comes from a     constraint.
     *
     */
    template <typename number>
    ...
    public:
      ...
      /**
       * Append an entry that is constrained. This means that there is one
       * less       nontrivial row.
       *
       */
      void
       ...
       * dofs do not contribute directly to the matrix, but are needed in
       * order       to set matrix diagonals and resolve inhomogeneities.
       *
       */
      size_type
      n_constraints() const;
      ...
      /**
       * The local row where constraint number i was detected, to find that
       * row       easily when the GlobalRowsToLocal has been set up.
       *
       */
      size_type
    ...
    private:
      ...
      /**
       * A number that represents the number of rows with       inhomogeneous
       * constraints.
       *
       */
    ...
    /**
     * Scratch data that is used during calls to distribute_local_to_global
     * and     add_entries_local_to_global. In order to avoid frequent memory
     ...
     * private version of scratch data out of a     ThreadLocalStorage, no
     ...
     * where several threads fight for     the data.         Access to the
     * scratch data is only through an accessor class which     handles the
     * access as well as marks the data as used.
     *
     */
    template <typename number>
    struct ScratchData
...
/**
 * This class
 * implements dealing with linear (possibly inhomogeneous) constraints on
 * degrees of freedom. The concept and origin of such constraints is
 * extensively described in the   @ref constraints   module. The class is
 * meant to deal with a limited number of constraints relative to the total
 * number of degrees of freedom, for example a few per cent up to maybe 30 per
 * cent; and with a linear combination of <i>M</i> other degrees of freedom
 * where <i>M</i> is also relatively small (no larger than at most around the
 * average number of entries per row of a linear system). It is   <em>  not
 * </em>   meant to describe full rank linear systems.
 * The algorithms used in the implementation of this class are described in some detail in the   @ref hp_paper   "hp-paper".
 * There is also a significant amount of documentation on how to use this
 * class in the   @ref constraints   module.
 *
 *  <h3>Description of constraints</h3>
 *  Each "line" in objects of this class corresponds to one constrained degree of freedom, with the number of the line being <i>i</i>, entered by using add_line() or add_lines(). The entries in this line are pairs of the form (<i>j</i>,<i>a<sub>ij</sub></i>), which are added by add_entry() or add_entries(). The organization is essentially a SparsityPattern, but with only a few lines containing nonzero elements, and  therefore no data wasted on the others. For each line, which has been added by the mechanism above, an elimination of the constrained degree of freedom of the form @f[
 * x_i = \sum_j a_{ij} x_j + b_i
 * @f] is performed, where <i>b<sub>i</sub></i> is optional and set by set_inhomogeneity(). Thus, if a constraint is formulated for instance as a zero mean value of several degrees of freedom, one of the degrees has to be chosen to be eliminated.
 ...
 * objects is first explained in   step-6  . Objects of the present type are
 * organized in lines (rows), but only those lines are stored where
 * constraints are present. New constraints are added by adding new lines
 * using the add_line() function, and then populating it using the add_entry()
 * function to a given line, or add_entries() to add more than one entry at a
 * time. The right hand side element, if nonzero, can be set using the
 * set_inhomogeneity() function. After all constraints have been added, you
 * need to call close(), which compresses the storage format and sorts the
 * entries.
 *
 *
 * @note
 * Many of the algorithms this class implements are discussed in the   @ref
 * hp_paper  . The algorithms are also related to those shown in <i>M. S.
...
public:
  ...
  enum MergeConflictBehavior
  {
    ...
    /**
     * In an operation   <code>cm1.merge(cm2)</code>, if <code>cm1</code>
     * and       <code>cm2</code>   have constraints on the same degree of
     * freedom, take     the one from   <code>cm1</code>  .
     *
     */
    left_object_wins,
     ...
     * In an operation   <code>cm1.merge(cm2)</code>, if <code>cm1</code>
     * and       <code>cm2</code>   have constraints on the same degree of
     * freedom, take     the one from   <code>cm2</code>  .
     *
     */
    right_object_wins
  };
  ...
  /**
   * Constructor. The
   * supplied IndexSet defines which indices might be   constrained inside
   * this AffineConstraints container. In a calculation   with a DoFHandler
   * object based on   parallel::distributed::Triangulation     or
   * parallel::shared::Triangulation,   one should use the set of locally
   * relevant dofs (see     @ref GlossLocallyRelevantDof  ).     The given
   * IndexSet allows the AffineConstraints container to save   memory by just
   * not caring about degrees of freedom that are not of   importance to the
   * current processor. Alternatively, if no such   IndexSet is provided,
   * internal data structures for <i>all</i> possible   indices will be
   * created, leading to memory consumption on every   processor that is
   * proportional to the <i>overall</i> size of the   problem, not just
   * proportional to the size of the portion of the   overall problem that is
  ...
  /**
   * Copy operator. Like for many other large objects, this operator   is
   * deleted to avoid its inadvertent use in places such as   accidentally
   * declaring a   @p AffineConstraints   object as a   function argument by
   * value, rather than by reference.     However, you can use the copy_from()
   * function to explicitly   copy AffineConstraints objects.
   *
   */
  AffineConstraints &
  ...
  /**
   * Copy the given object to the current one.     This function exists
   * because   @p operator=()   is explicitly   disabled.
   *
   */
  ...
  /**
   * This function copies the content of   @p constraints_in   with DoFs that
   * are   element of the IndexSet   @p filter.   Elements that are not
   * present in the   IndexSet are ignored. All DoFs will be transformed to
   * local index space   of the filter, both the constrained DoFs and the
   * other DoFs these entries   are constrained to. The local index space of
   * the filter is a contiguous   numbering of all (global) DoFs that are
   * elements in the filter.     If, for example, the filter represents the
   * range <tt>[10,20)</tt>, and   the constraints object   @p constraints_in
   * includes the global indices   <tt>{7,13,14}</tt>, the indices
   * <tt>{3,4}</tt> are added to the calling   constraints object (since 13
   * and 14 are elements in the filter and element   13 is the fourth element
   * in the index, and 14 is the fifth).     This function provides an easy
   * way to create a AffineConstraints for   certain vector components in a
   * vector-valued problem from a full   AffineConstraints, i.e. extracting a
   * diagonal subblock from a larger   AffineConstraints. The block is
   * specified by the IndexSet argument.
   *
   */
  void
   ...
   * which   <code>lines[i]</code>   is true.     This function essentially
   * exists to allow adding several constraints of   the form
   * <i>x<sub>i</sub></i>=0 all at once, where the set of indices   <i>i</i>
   * for which these constraints should be added are given by the   argument
   * of this function. On the other hand, just as if the single-   argument
   * add_line() function were called repeatedly, the constraints can   later
   * be modified to include linear dependencies using the add_entry()
  ...
  /**
   * Call the first add_line() function for every index   <code>i</code>
   * that   appears in the argument.     This function essentially exists to
   * allow adding several constraints of   the form <i>x<sub>i</sub></i>=0 all
   * at once, where the set of indices   <i>i</i> for which these constraints
   * should be added are given by the   argument of this function. On the
   * other hand, just as if the single-   argument add_line() function were
   * called repeatedly, the constraints can   later be modified to include
   * linear dependencies using the add_entry()   function as well as
   * inhomogeneities using set_inhomogeneity().
   *
   */
  void
   ...
   * $a_{ij} x_j$   to the constraints for the   $i$  th degree of freedom.
   ...
   * twice.       @param[in]   constrained_dof_index The index   $i$   of the
   * degree of freedom     that is being constrained.     @param[in]   column
   * The index   $j$   of the degree of freedom being entered     into the
   * constraint for degree of freedom   $i$  .     @param[in]   weight The
   * factor   $a_{ij}$   that multiplies   $x_j$  .
   *
   */
  void
   ...
   * words, it adds a constant   $b_i$   to the constraint for degree of
   * freedom     $i$  . For this to work, you need to call add_line() first
   * for the given   degree of freedom.       @param[in]
   * constrained_dof_index The index   $i$   of the degree of freedom     that
   * is being constrained.     @param[in]   value The right hand side value
   * $b_i$   for the constraint on     the degree of freedom   $i$  .
   *
   */
  void
   ...
   * through this function.     Also, zero entries are discarded, since they
   * are not needed.     After closing, no more entries are accepted. If the
   * object was already   closed, then this function returns immediately.
   * This function also resolves chains of constraints. For example, degree of
   * freedom 13 may be constrained to   $u_{13} = \frac{u_3}{2} +
   * \frac{u_7}{2}$     while degree of freedom 7 is itself constrained as
   * $u_{7} = \frac{u_2}{2} + \frac{u_4}{2}$  . Then, the resolution will be
   * that   $u_{13} = \frac{u_3}{2} + \frac{u_2}{4} + \frac{u_4}{4}$  . Note,
   * however, that   cycles in this graph of constraints are not allowed,
   * i.e., for example     $u_4$   may not itself be constrained, directly or
   * indirectly, to   $u_{13}$     again.
   *
   */
  void
  close();
   ...
   * significantly faster.     Using the default value of the second
   * arguments, the constraints in each   of the two objects (the old one
   * represented by this object and the   argument) may not refer to the same
   * degree of freedom, i.e. a degree of   freedom that is constrained in one
   * object may not be constrained in the   second. If this is nevertheless
   * the case, an exception is thrown.   However, this behavior can be changed
   * by providing a different value for   the second argument.     By default,
   * merging two AffineConstraints objects that are initialized   with
   * different IndexSet objects is not allowed.   This behavior can be altered
   * by setting   @p allow_different_local_lines     appropriately.
   * Merging a AffineConstraints that is initialized with an IndexSet   and
   * one that is not initialized with an IndexSet is not yet implemented.
   *
   */
  void
  merge(
  ...
  /**
   * Shift all entries of this matrix down   @p offset   rows and over   @p
   * offset     columns. If this object is initialized with an IndexSet,
   * local_lines are   shifted as well.     This function is useful if you are
   * building block matrices, where all   blocks are built by the same
   * DoFHandler object, i.e. the matrix size is   larger than the number of
   * degrees of freedom. Since several matrix rows   and columns correspond to
   * the same degrees of freedom, you'd generate   several constraint objects,
   * then shift them, and finally merge() them   together again.
   *
   */
  void
   ...
   * entries are accepted or not.     This function may be called also on
   * objects which are empty or already   cleared.
   *
   */
  void
  clear();
   ...
   * constrained one.     Note that if close() was called before, then this
   * function is   significantly faster, since then the constrained degrees of
   * freedom are   sorted and we can do a binary search, while before close()
   ...
   * of exactly one other degree of freedom.     The function returns   @p
   * false   if either the degree of freedom is not   constrained at all, or
   * if it is constrained to more than one other degree   of freedom, or if it
   * is constrained to only one degree of freedom but   with a weight
   * different from one.
   *
   */
  bool
   ...
   * number is no more obvious.     The name indicates that within the system
   * matrix, references to a   constrained node are indirected to the nodes it
   * is constrained to.
   *
   */
  size_type
  ...
  /**
   * Return a pointer to the vector of entries if a line is constrained,   and
   * a zero pointer in case the dof is not constrained.
   *
   */
  ...
  /**
   * Print the constraints represented by the current object to the   given stream.     For each constraint of the form   @f[
   * x_{42} = 0.5 x_2 + 0.25 x_{14} + 2.75
   * @f]   this function will write a sequence of lines that look like this:
   * @code
   * 42 2 : 0.5
   * 42 14 : 0.25
   * 42 : 2.75
   ...
   * nonzero.     A block of lines such as the one above is repeated for each
   ...
   * are constrained to.     The output of this function can be used as input
   * to the 'dot' program   that can convert the graph into a graphical
   * representation in postscript,   png, xfig, and a number of other formats.
   ...
   * system.     Since this function adds new nonzero entries to the sparsity
   * pattern, the   given sparsity pattern must not be compressed. The current
   * object must be   closed. The sparsity pattern is compressed at the end of
   * the function.
   *
   */
  void
   ...
   * matrix that correspond to constrained degrees of freedom.     See the
   * general documentation of this class for more detailed   information.
   *
   */
  void
  ...
  /**
   * The function copies and condenses values from   @p vec_ghosted   into
   * @p     output. In a serial code it is equivalent to calling condense
   * (vec). If   called in parallel,   @p vec_ghosted   is supposed to contain
   * ghost elements   while   @p output   should not.
   *
   */
  template <class VectorType>
  void
   ...
   * applying inhomogeneous constraints.     The current object must be closed
   * to call this function.     See the general documentation of this class
   ...
   * words, this function implements a   [scatter
   * operation](https://en.wikipedia.org/wiki/Gather-scatter_(vector_addressing)).
   * In most   cases, these local contributions will be the result of an
   * integration   over a cell or face of a cell. However, as long as   @p
   * local_vector   and   @p     local_dof_indices have the same number of
   * elements, this function is   happy with whatever it is given.     In
   * contrast to the similar function in the DoFAccessor class, this
   ...
   * necessary.     Note that this function will apply all constraints as if
   * they were   homogeneous. For correctly setting inhomogeneous constraints,
   * use the   similar function with a matrix argument or the function with
   * both matrix   and vector arguments.
   ...
   * races.       @param[in]   local_vector Vector of local contributions.
   * @param[in]   local_dof_indices Local degrees of freedom indices
   * corresponding to the vector of local contributions.     @param[out]
   * global_vector The global vector to which all local   contributions will
   * be added.
   *
   */
   ...
   * the corresponding degree of freedom is constrained.     The fourth
   * argument <tt>local_matrix</tt> is intended to be used in case   one wants
   * to apply inhomogeneous constraints on the vector only. Such a   situation
   * could be where one wants to assemble of a right hand side   vector on a
   * problem with inhomogeneous constraints, but the global matrix   has been
   * assembled previously. A typical example of this is a time   stepping
   * algorithm where the stiffness matrix is assembled once, and the   right
   * hand side updated every time step. Note that, however, the entries   in
   * the columns of the local matrix have to be exactly the same as those
   ...
   * different   index sets to correctly handle inhomogeneities when the local
   * matrix is   computed from a combination of two neighboring elements, for
   * example for an   edge integral term in DG. Note that in the case that
   * these two elements   have different polynomial degree, the local matrix
   * is rectangular.     <tt>local_dof_indices_row</tt> is the set of row
   * indices and   <tt>local_dof_indices_col</tt> is the set of column indices
   * of the local   matrix. <tt>diagonal=false</tt> says whether the two index
   * sets are equal   or not.     If both index sets are equal,
   * <tt>diagonal</tt> must be set to true or we   simply use the previous
   * function. If both index sets are different   (diagonal=false) the
   * <tt>global_vector</tt> is modified to handle   inhomogeneities but no
   * entries from <tt>local_vector</tt> are added. Note   that the edge
   * integrals for inner edged for DG do not contribute any values   to the
   * right hand side.
   *
   */
  template <typename VectorType>
   ...
   * local_vector) corresponding to the degrees of freedom indices given in
   * @p     local_dof_indices and distributes them to the global vector. In
   * other   words, this function implements a   [scatter
   * operation](https://en.wikipedia.org/wiki/Gather-scatter_(vector_addressing)).
   * In most   cases, these local contributions will be the result of an
   * integration   over a cell or face of a cell. However, as long as the
   * entries in   @p     local_dof_indices indicate reasonable global vector
   * entries, this   function is happy with whatever it is given.     If one
   * of the elements of   @p local_dof_indices   belongs to a constrained
   ...
   * is constrained.     Thus, by using this function to distribute local
   * contributions to the   global object, one saves the call to the condense
   * function after the   vectors and matrices are fully assembled. Note that
   * this function   completely ignores inhomogeneous constraints.
   * @note   This function in itself is thread-safe, i.e., it works properly
   * also when several threads call it simultaneously. However, the function
   * call is only thread-safe if the underlying global vector allows for
   * simultaneous access and the access is not to rows with the same global
   ...
   * In most   cases, these local contributions will be the result of an
   * integration   over a cell or face of a cell. However, as long as   @p
   * local_matrix   and   @p     local_dof_indices have the same number of
   * elements, this function is   happy with whatever it is given.     In
   * contrast to the similar function in the DoFAccessor class, this
   * function also takes care of constraints, i.e. if one of the elements of
   * @p local_dof_indices   belongs to a constrained node, then rather than
   ...
   * in such a case.     By using this function to distribute local
   * contributions to the global   object, one saves the call to the condense
   * function after the vectors and   matrices are fully assembled.
   * @note   This function in itself is thread-safe, i.e., it works properly
   * also when several threads call it simultaneously. However, the function
   ...
   * being filled with arbitrary values.     Since the diagonal entries
   * corresponding to eliminated degrees of freedom   are not set, the result
   * may have a zero eigenvalue, if applied to a   square matrix. This has to
   * be considered when solving the resulting   problems. For solving a source
   * problem   $Au=f$  , it is possible to set the   diagonal entry after
   * building the matrix by a piece of code of the form
   * @code
   ...
   * close to machine accuracy, it may be advisable to adjust it.     For
   * solving eigenvalue problems, this will only add one spurious zero
   ...
   * row and column indices. The convention is that row indices are
   * constrained   according to the calling AffineConstraints
   * <code>*this</code>  , whereas   column indices are constrained according
   * to the given AffineConstraints     <code>column_affine_constraints</code>
   * . This function allows to handle the   case where rows and columns of a
   * matrix are represented by different   function spaces with their own
   * enumeration of indices, as e.g. in mixed   finite element problems with
   * separate DoFHandler objects or for flux   matrices between different
   * levels in multigrid methods.     Like the other method with separate
   * slots for row and column indices,   this method does not add diagonal
   * entries to eliminated degrees of   freedom. See there for a more
  ...
  /**
   * This function
   * simultaneously writes elements into matrix and vector,   according to the
   * constraints specified by the calling AffineConstraints.   In other words,
   * it performs the   [scatter
   * operation](https://en.wikipedia.org/wiki/Gather-scatter_(vector_addressing))
  ...
  /**
   * Do a similar operation as the distribute_local_to_global() function that   distributes writing entries into a matrix for constrained degrees of   freedom, except that here we don't write into a matrix but only allocate   sparsity pattern entries.     As explained in the     @ref hp_paper   "hp-paper"
   * and in   step-27  , first allocating a sparsity pattern and later coming
   * back   and allocating additional entries for those matrix entries that
   * will be   written to due to the elimination of constrained degrees of
   * freedom   (using   AffineConstraints::condense()   ), can be a very
   * expensive procedure.   It is cheaper to allocate these entries right away
   * without having to do a   second pass over the sparsity pattern object.
   * This function does exactly   that.     Because the function only
   * allocates entries in a sparsity pattern, all it   needs to know are the
   * degrees of freedom that couple to each other.   Unlike the previous
   * function, no actual values are written, so the second   input argument is
   * not necessary here.     The third argument to this function,
   * keep_constrained_entries determines   whether the function shall allocate
   * entries in the sparsity pattern at   all for entries that will later be
   * set to zero upon condensation of the   matrix. These entries are
   * necessary if the matrix is built unconstrained,   and only later
   * condensed. They are not necessary if the matrix is built   using the
   * distribute_local_to_global() function of this class which   distributes
   * entries right away when copying a local matrix into a global   object.
   * The default of this argument is true, meaning to allocate the few
   * entries that may later be set to zero.     By default, the function adds
   * entries for all pairs of indices given in   the first argument to the
   * sparsity pattern (unless   keep_constrained_entries is false). However,
   * sometimes one would like to   only add a subset of all of these pairs. In
   * that case, the last argument   can be used which specifies a boolean mask
   * which of the pairs of indices   should be considered. If the mask is
   * false for a pair of indices, then no   entry will be added to the
   * sparsity pattern for this pair, irrespective   of whether one or both of
   * the indices correspond to constrained degrees   of freedom.     This
   * function is not typically called from user code, but is used in the
  ...
  /**
   * This function imports values from a global vector (  @p global_vector)
   * by   applying the constraints to a vector of local values, expressed in
   ...
   * function is happy with whatever it is given.     If one of the elements
   * of   @p local_dof_indices   belongs to a constrained   node, then rather
   * than writing the corresponding element of   @p     global_vector into
   * @p local_vector,   the constraints are resolved as the   respective
   * distribute function does, i.e., the local entry is constructed   from the
   * global entries to which this particular degree of freedom is
   * constrained.     In contrast to the similar function get_dof_values in
   * the DoFAccessor   class, this function does not need the constrained
   * values to be correctly   set (i.e., distribute to be called).
   *
   */
  template <typename ForwardIteratorVec,
            typename ForwardIteratorInd,
   ...
   * function will read the values of   $x_1$   and   $x_2$   from the given
   * vector   and set the element   $x_3$   according to this constraints.
   * Similarly, if   the current object stores the constraint   $x_{42}=208$
   * , then this   function will set the 42nd element of the given vector to
   * 208.
  ...
  struct ConstraintLine
  {
    ...
    /**
     * Row numbers and values of the entries in this line.         For the
     * reason why we use a vector instead of a map and the     consequences
     * thereof, the same applies as what is said for
   ...
   * initialize range-based for loops as supported by C++11.       @return   A
   * range object for the half open range <code>[this->begin(),
  ...
  /**
   * Check if the current object is consistent on all processors   in a
   * distributed computation.     This method checks if all processors agree
   * on the constraints for their   local lines as given by   @p
   * locally_active_dofs.   This method is a collective   operation and will
   * return   @p true   only if all processors are consistent.     Please
   * supply the owned DoFs per processor as returned by
   * Utilities::MPI::all_gather(MPI_Comm,
   * DoFHandler::locally_owned_dofs())   as     @p locally_owned_dofs   and
   * the result of     DoFTools::extract_locally_active_dofs()   as     @p
   * locally_active_dofs.   The former is used to determine ownership of the
   * specific DoF, while the latter is used as the set of rows that need to be
   * checked.     If   @p verbose   is set to   @p true,   additional debug
   * information is written   to   std::cout.
   * @note   This method exchanges all constraint information of locally
   * active   lines and is as such slow for large computations and should
   * probably   only be used in debug mode. We do not check all lines returned
   * by   get_local_lines() but only the locally active ones, as we allow
   * processors   to not know about some locally relevant rows.       @return
   * Whether all AffineConstraints objects are consistent. Returns   the same
...
private:
   ...
   * entries takes place when calling the <tt>close()</tt> function.     We
   * could, instead of using a vector, use an associative array, like a map
   ...
   * actually be shorter than the index of the DoF we check for.     This
   * field exists since when adding a new constraint line we have to   figure
   * out whether it already exists. Previously, we would simply walk   the
   * unsorted list of constraint lines until we either hit the end or   found
   * it. This algorithm is O(N) if N is the number of constraints, which
   ...
   * constrained.     To make things worse, traversing the list of existing
   * constraints   requires reads from many different places in memory. Thus,
   * in large 3d   applications, the add_line() function showed up very
   * prominently in the   overall compute time, mainly because it generated a
   * lot of cache misses.   This should also be fixed by using the O(1)
   * algorithm to access the   fields of this array.     The field is useful
   * in a number of other contexts as well, e.g. when one   needs random
   * access to the constraints as in all the functions that apply
  ...
  /**
   * Internal function to calculate the index of line   @p line_n   in the
   * vector   lines_cache using local_lines.
   *
   */
  size_type
...
namespace internal
{
  ...
  {
    /**
     * A "traits" class that can be used to determine whether a given type is
     * a     block matrix type or not. For example,
     * @code
     ...
     * @endcode
     * is true. This is sometimes useful in template contexts where we may want     to do things differently depending on whether a template type denotes a     regular or a block matrix type.           @see         @ref GlossBlockLA   "Block (linear algebra)"
     *
     */
    template <typename MatrixType>
    ...
    /**
     * A class that can be used to determine whether a given type is a block     sparsity pattern type or not. In this, it matches the IsBlockMatrix     class.           @see         @ref GlossBlockLA   "Block (linear algebra)"
     *
     */
    template <typename MatrixType>
    ...
    public:
      /**
       * A statically computable value that indicates whether the template
       * argument to this class is a block sparsity pattern (in fact whether
       * the       type is derived from BlockSparsityPatternBase<T>).
       *
       */
      static const bool value =
        std::is_same<decltype(check(std::declval<MatrixType *>())),
