...
#  endif
  ...
  /**
   * A namespace for classes that provide wrappers for Trilinos' Tpetra
   * vectors.     This namespace provides wrappers for the   Tpetra::Vector
   * class from the   Tpetra package (https://trilinos.github.io/tpetra.html)
   * that is part of   Trilinos.
   *
   */
  namespace TpetraWrappers
  {
     ...
     * compiled with MPI support.         Tpetra uses Kokkos for
     * thread-parallelism and chooses the execution and     memory space
     * automatically depending on Kokkos configuration. The     priority is
     ...
     *
     *
     *  -   Kokkos::Serial           In case Kokkos was configured with CUDA support, this class stores the     values in unified virtual memory space and performs its action on the     GPU. In particular, there is no need for manually synchronizing memory     between host and device.
    ...
    public:
      ...
      /**
       * Change the dimension to that of the vector   @p V.   The elements of
       * @p V   are not       copied.
       *
       */
      virtual void
      ...
      /**
       * Sets all elements of the vector to the scalar   @p s.   This
       * operation is       only allowed if   @p s   is equal to zero.
       *
       */
      virtual Vector &
       ...
       * input vector   @p V.     VectorOperation::values     @p operation
       * is used to decide if       the elements in   @p V   should be added
       * to the current vector or replace the       current elements. The last
       * parameter can be used if the same       communication pattern is used
       * multiple times. This can be used to       improve performance.
       *
       */
      virtual void
      ...
      /**
       * Return the scalar product of two vectors. The vectors need to have
       * the       same layout.
       *
       */
      virtual Number
      ...
      /**
       * Add   @p a   to all components. Note that   @p is   a scalar not a
       * vector.
       *
       */
      virtual void
       ...
       * method only needs to load three vectors,   @p this,     @p V,     @p
       * W,   whereas       calling separate methods means to load the calling
       * vector   @p this         twice. Since most vector operations are
       * memory transfer limited, this       reduces the time by 25\% (or 50\%
       * if   @p W   equals   @p this).               The vectors need to have
       * the same layout.             For complex-valued vectors, the scalar
       * product in the second step is       implemented as
...
/**
 * Declare   dealii::LinearAlgebra::TpetraWrappers::Vector   as distributed
 * vector.
 *
 *
 */
