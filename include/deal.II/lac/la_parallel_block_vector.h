...
* */
    ...
    public:
       ...
       * will get slow when too many       messages/requests are outstanding.
       * Even when messages are small,       say 1 kB only, we should collect
       * enough data with   @p communication_block_size         to cover
       * typical infiniband latencies which are around a few
       ...
       * blocks to the same size <tt>block_size</tt>.             Confer the
       * other constructor further down if you intend to use blocks       of
       * different sizes.
       *
       */
       ...
       * BlockVector with data elements with less accuracy.             Older
       * versions of gcc did not honor the   @p explicit   keyword on
       ...
       * initialized to match.             This operator is only available if
       * deal.II was configured with PETSc.
       *
       */
      BlockVector<Number> &
       ...
       * function assumes that the vectors layouts have already been
       * initialized to match.             This operator is only available if
       * deal.II was configured with       Trilinos.
       *
       */
      BlockVector<Number> &
       ...
       * size <tt>block_size</tt> each.             If the second argument is
       * left at its default value, then the block       vector allocates the
       * specified number of blocks but leaves them at       zero size. You
       * then need to later reinitialize the individual blocks,       and call
       * collect_sizes() to update the block system's knowledge of       its
       * individual block's sizes.             If
       * <tt>omit_zeroing_entries==false</tt>, the vector is filled with
       * zeros.
       *
       */
      void
       ...
       * dimension <tt>block_sizes[i]</tt>.             If the number of
       * blocks is the same as before this function was       called, all
       * vectors remain the same and reinit() is called for each       vector.
       * If <tt>omit_zeroing_entries==false</tt>, the vector is filled with
       * zeros.             Note that you must call this (or the other
       * reinit() functions)       function, rather than calling the reinit()
       * functions of an individual       block, to allow the block vector to
       * update its caches of vector       sizes. If you call reinit() on one
       * of the blocks, then subsequent       actions on this object may yield
       * unpredictable results since they may       be routed to the wrong
       * block.
       *
       */
      void
       ...
       * applies as for the other reinit() function.             The elements
       * of <tt>V</tt> are not copied, i.e.  this function is the       same
       * as calling <tt>reinit (V.size(), omit_zeroing_entries)</tt>.
       ...
       * in the glossary.             There are two variants for this
       ...
       * compress(VectorOperation::insert),   all       ghost entries are
       * therefore simply zeroed out (using       zero_ghost_values()). In
       * debug mode, a check is performed that makes       sure that the data
       * set is actually consistent between processors,       i.e., whenever a
       * non-zero ghost element is found, it is compared to       the value on
       * the owning processor and an exception is thrown if these
       ...
       * around.             Limitation: right now this function only works if
       * both vectors have       the same number of blocks. If needed, the
       * numbers of blocks should be       exchanged, too.             This
       * function is analogous to the swap() function of all C++
      ...
      /**
       * Calculate the scalar product between each block of this vector and
       * @p V         and store the result in a full matrix   @p matrix.
       * This function       computes the result by forming   $A_{ij}=U_i
       * \cdot V_j$   where   $U_i$         and   $V_j$   indicate the   $i$
       * th block (not element!) of   $U$   and the         $j$  th block of
       * $V$  , respectively. If   @p symmetric   is         <code>true</code>
       * , it is assumed that inner product results in a       square
       * symmetric matrix and almost half of the scalar products can be
       * avoided.             Obviously, this function can only be used if all
       * blocks of both vectors       are of the same size.
      ...
      /**
       * Calculate the scalar product between each block of this vector and
       * @p V         using a metric tensor   @p matrix.   This function
       * computes the result of   $ \sum_{ij} A^{ij} U_i \cdot V_j$   where
       * $U_i$         and   $V_j$   indicate the   $i$  th block (not
       * element) of   $U$   and the         $j$  th block of   $V$  ,
       * respectively. If   @p symmetric   is         <code>true</code>  , it
       * is assumed that   $U_i \cdot V_j$   and   $A^{ij}$   are
       * symmetric matrices and almost half of the scalar products can be
       * avoided.             Obviously, this function can only be used if all
       * blocks of both vectors       are of the same size.
       * @note   Internally, a single global reduction will be called to
       * accumulate the scalar product between locally owned degrees of
       * freedom.
       *
       */
      template <typename FullMatrixType>
      ...
      /**
       * Set each block of this vector as follows:         $V^i = s V^i + b
       * \sum_{j} U_j A^{ji}$   where   $V^i$         and   $U_j$   indicate
       * the   $i$  th block (not element) of   $V$   and the         $j$  th
       * block of   $U$  , respectively.             Obviously, this function
       * can only be used if all blocks of both vectors       are of the same
       * size.
       *
       */
      template <typename FullMatrixType>
