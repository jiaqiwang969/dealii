 ...
 * such as SparseILU or SparseMIC. The decomposition is stored as a sparse
 * matrix which is why this class is derived from the SparseMatrix. Since it
 * is not a matrix in the usual sense (the stored entries are not those of a
 * matrix, but of the two factors of the original matrix), the derivation is
 * <tt>protected</tt> rather than <tt>public</tt>.
 *
 *  <h3>Fill-in</h3> Sparse decompositions are frequently used with additional
 * fill-in, i.e., the sparsity structure of the decomposition is denser than
 * that of the matrix to be decomposed. The initialize() function of this
 * class allows this fill-in via the AdditionalData object as long as all
 * entries present in the original matrix are present in the decomposition
 * also, i.e. the sparsity pattern of the decomposition is a superset of the
 * sparsity pattern in the original matrix. Such fill-in can be accomplished
 * by various ways, one of which is the copy- constructor of the
 * SparsityPattern class that allows the addition of side- diagonals to a
 * given sparsity structure.
 *
 *  <h3>Unified use of preconditioners</h3> While objects of this class can
 * not be used directly (this class is only a base class for others
 * implementing actual decompositions), derived classes such as SparseILU and
 * SparseMIC can be used in the usual form as preconditioners. For example,
 * this works:
 *
 * @code
 ...
 *  1/ The matrix diagonal can be strengthened by adding   <code>strengthen_diagonal</code>   times the sum of the absolute row entries of each row to the respective diagonal entries. By default no strengthening is performed.
 *  2/ By default, each initialize() function call creates its own sparsity. For that, it copies the sparsity of   <code>matrix</code>   and adds a specific number of extra off diagonal entries specified by   <code>extra_off_diagonals</code>  .
 *  3/ By setting   <code>use_previous_sparsity=true</code>   the sparsity is not recreated but the sparsity of the previous initialize() call is reused (recycled). This might be useful when several linear problems on the same sparsity need to solved, as for example several Newton iteration steps on the same triangulation. The default is   <code>false</code>  .
 *  4/ It is possible to give a user defined sparsity to   <code>use_this_sparsity</code>  . Then, no sparsity is created but   <code>*use_this_sparsity</code>   is used to store the decomposed matrix. For restrictions on the sparsity see section `Fill-in' above).
 *
 *  <h3>Particular implementations</h3> It is enough to override the
 * initialize() and vmult() methods to implement particular LU decompositions,
 * like the true LU, or the Cholesky decomposition. Additionally, if that
 * decomposition needs fine tuned diagonal strengthening on a per row basis,
 * it may override the get_strengthen_diagonal() method.
 *
 *
 */
...
public:
  ...
  public:
     ...
     * added to the diagonal entries.         Per default, this value is zero,
     * i.e. the diagonal is not strengthened.
     *
     */
    double strengthen_diagonal;

    /**
     * By default, the   <code>initialize(matrix, data)</code>   function
     * creates     its own sparsity. This sparsity has the same
     * SparsityPattern as       <code>matrix</code>   with some extra off
     * diagonals the number of which     is specified by
     * <code>extra_off_diagonals</code>  .         The user can give a
     * SparsityPattern to   <code>use_this_sparsity</code>  .     Then this
     * sparsity is used and the   <code>extra_off_diagonals</code>
     * argument is ignored.
     *
     */
     ...
     * that was used during the previous initialize() call.         This might
     * be useful when several linear problems on the same sparsity     need to
     * solved, as for example several Newton iteration steps on the     same
     * triangulation.
     *
     */
     ...
     * sparsity to be used.         Note that the sparsity structures of
     * <code>*use_this_sparsity</code>       and the matrix passed to the
     * initialize function need not be equal.     Fill-in is allowed, as well
     * as filtering out some elements in the     matrix.
     *
     */
   ...
   * as preconditioner.     For more detail about possible parameters, see the
   * class documentation   and the documentation of the
   * SparseLUDecomposition::AdditionalData   class.     According to the
   * <code>parameters</code>  , this function creates a new   SparsityPattern
   * or keeps the previous sparsity or takes the sparsity   given by the user
   * to   <code>data</code>  . Then, this function performs the   LU
   * decomposition.     After this function is called the preconditioner is
   * ready to be used   (using the   <code>vmult</code>   function of derived
   * classes).
   *
   */
   ...
   * <i>M</i> being this matrix.     Source and destination must not be the
   * same vector.
   *
   */
...
protected:
   ...
   * diagonal entry in row   <code>row</code>   with sum of absolute values of
   * its   elements   <code>rowsum</code>  .
   * @note   The default implementation in SparseLUDecomposition returns
   * <code>strengthen_diagonal</code>  's value. This variable is set to   a
   * nonzero value in several of the derived classes.
   *
   */
  virtual number
