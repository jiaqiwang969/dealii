...
/**
 * The Brezzi-Douglas-Marini element. <h3>Degrees of freedom</h3>
 * @todo   The 3D version exhibits some numerical instabilities, in particular
 * for higher order
 * @todo   Restriction matrices are missing. The matching pressure space for
 * FE_BDM of order <i>k</i> is the element FE_DGP of order <i>k-1</i>. The BDM
 * element of order   @p p   has <i>p+1</i> degrees of freedom on each face.
 * These are implemented as the function values in the <i>p+1</i> Gauss points
 * on each face. Additionally, for order greater or equal 2, we have
 * additional <i>p(p-1)</i>, the number of vector valued polynomials in
...
public:
   ...
   * returns <tt>FE_BDM<dim>(degree)</tt>, with   @p dim   and   @p degree
