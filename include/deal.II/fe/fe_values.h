...
namespace internal
{
  ...
  /**
   * A class whose specialization is used to define what type the curl of a
   * vector valued function corresponds to.     In 1d, the curl is a scalar.
   *
   */
  ...
  /**
   * A class whose specialization is used to define what type the curl of a
   * vector valued function corresponds to.     In 2d, the curl is a scalar.
   *
   */
  template <class NumberType>
  ...
  /**
   * A class whose specialization is used to define what type the curl of a
   * vector valued function corresponds to.     In 3d, the curl is a vector.
   *
   */
  template <class NumberType>
 ...
 * objects using the square bracket operator. There are classes that present
 * views for single scalar components, vector components consisting of
 * <code>dim</code>   elements, and symmetric second order tensor components
 * consisting of   <code>(dim*dim + dim)/2</code>   elements
 * See the
 * description of the   @ref vector_valued   module for examples how to use
 * the features of this namespace.
 *
 *
...
namespace FEValuesViews
{
  /**
   * A class
   * representing a view to a single scalar component of a possibly
   * vector-valued finite element. Views are discussed in the     @ref
   * vector_valued     module.     You get an object of this type if you apply
   * a   FEValuesExtractors::Scalar     to an FEValues, FEFaceValues or
  ...
  public:
    ...
    /**
     * An alias for the data type of the product of a   @p Number   and the
     * third derivatives of the view this class provides. This is the data
     * type     of scalar components of a finite element field whose degrees
     * of     freedom are described by a vector with elements of type   @p
     * Number.
     *
     */
    template <typename Number>
     ...
     * and derivatives of basis functions of the Scalar view and any   @p
     * Number   type.           @deprecated   Use the types defined in the
     * surrounding class instead.
     *
     */
    template <typename Number>
     ...
     * of an FEFaceValues or FESubfaceValues object.           @param
     * q_point Number of the quadrature point at which function is to     be
     * evaluated.           @dealiiRequiresUpdateFlags{update_values}
     *
     */
    value_type
     ...
     * @note   The meaning of the arguments is as documented for the value()
     * function.           @dealiiRequiresUpdateFlags{update_hessians}
     *
     */
    hessian_type
     ...
     * selected scalar component.         The data type stored by the output
     * vector must be what you get when you     multiply the values of shape
     * functions (i.e.,   @p value_type)   times the     type used to store
     * the values of the unknowns   $U_j$   of your finite     element vector
     * $U$   (represented by the   @p fe_function   argument).
     ...
     * freedom located on the current cell from a global vector associated
     * with     a DoFHandler object (as the function above does), this
     * function instead     takes these local nodal values through its first
     * argument. A typical     way to obtain such a vector is by calling code
     * such as
     * @code
     ...
     * cell, without actually creating a corresponding DoFHandler
     *
     *  -  in that     case, all one would compute is a local representation of that     postprocessed function, characterized by its nodal values; this function     then allows the evaluation of that representation at quadrature points.           @param[in]   dof_values A vector of local nodal values. This vector must       have a length equal to number of DoFs on the current cell, and must       be ordered in the same order as degrees of freedom are numbered on       the reference cell.           @param[out]   values A vector of values of the given finite element field,       at the quadrature points on the current object.           @tparam   InputVector The   @p InputVector   type must allow creation       of an ArrayView object from it; this is satisfied by the         `std::vector`   class, among others.
     *
     */
    template <class InputVector>
    void
     ...
     * the <tt>reinit</tt> function of the FEValues object was called.
     * This function is the equivalent of the
     * FEValuesBase::get_function_gradients   function but it only works on
     * the     selected scalar component.         The data type stored by the
     * output vector must be what you get when you     multiply the gradients
     * of shape functions (i.e.,   @p gradient_type)       times the type used
     * to store the values of the unknowns   $U_j$   of your     finite
     * element vector   $U$   (represented by the   @p fe_function
     * argument).           @dealiiRequiresUpdateFlags{update_gradients}
     *
     */
    template <class InputVector>
    void
     ...
     * selected scalar component.         The data type stored by the output
     * vector must be what you get when you     multiply the Hessians of shape
     * functions (i.e.,   @p hessian_type)   times     the type used to store
     * the values of the unknowns   $U_j$   of your finite     element vector
     * $U$   (represented by the   @p fe_function   argument).
    ...
    /**
     * This function relates to get_function_hessians() in the same way     as
     * get_function_values_from_local_dof_values() relates to
     * get_function_values(). See the documentation of
     * get_function_values_from_local_dof_values() for more information.
     *
     */
     ...
     * Laplacians are the trace of the Hessians.         This function is the
     * equivalent of the       FEValuesBase::get_function_laplacians
     * function but it only works on the     selected scalar component.
     * The data type stored by the output vector must be what you get when you
     * multiply the Laplacians of shape functions (i.e.,   @p value_type)
     * times     the type used to store the values of the unknowns   $U_j$
     * of your finite     element vector   $U$   (represented by the   @p
     ...
     * the <tt>reinit</tt> function of the FEValues object was called.
     * This function is the equivalent of the
     * FEValuesBase::get_function_third_derivatives   function but it only
     * works     on the selected scalar component.         The data type
     * stored by the output vector must be what you get when you     multiply
     * the third derivatives of shape functions (i.e.,   @p
    ...
    /**
     * This function relates to get_function_third_derivatives() in the same
  ...
  /**
   * A class representing a view to a set of   <code>spacedim</code>   components   forming a vector part of a vector-valued finite element. Views are   discussed in the     @ref vector_valued     module.     Note that in the current context, a vector is meant in the sense physics   uses it: it has   <code>spacedim</code>   components that behave in specific   ways under coordinate system transformations. Examples include velocity   or displacement fields. This is opposed to how mathematics uses the word   "vector"
   * (and how we use this word in other contexts in the library, for   example
   * in the Vector class), where it really stands for a collection of
   ...
   * gradient of a vector   $d_{k}, 0\le k<\text{dim}$   is defined as
   * $S_{ij} = \frac{\partial d_{i}}{\partial x_j}, 0\le i,j<\text{dim}$  .
   * You get an object of this type if you apply a
   * FEValuesExtractors::Vector     to an FEValues, FEFaceValues or
   * FESubfaceValues object.
   * @ingroup feaccess vector_valued
   *
   */
  ...
  public:
    /**
     * An alias for the data type of values of the view this class
     * represents. Since we deal with a set of   <code>dim</code>
    ...
    /**
     * An alias for the type of gradients of the view this class represents.
     * Here, for a set of   <code>dim</code>   components of the finite
     * element,     the gradient is a   <code>Tensor@<2,spacedim@></code>  .
     ...
     * <code>SymmetricTensor@<2,spacedim@></code>  .         The symmetric
     * gradient of a vector field   $\mathbf v$   is defined as
     ...
     * An alias for the type of third derivatives of the view this class
     * represents. Here, for a set of   <code>dim</code>   components of the
     * finite element, the third derivative is a
     ...
     * An alias for the data type of the product of a   @p Number   and the
     * third derivatives of the view this class provides. This is the data
     * type     of vector components of a finite element field whose degrees
     * of     freedom are described by a vector with elements of type   @p
     * Number.
     *
     */
    ...
    /**
     * A struct that provides the output type for the product of the value
     * and derivatives of basis functions of the Vector view and any   @p
     * Number   type.           @deprecated   Use the types defined in the
     * surrounding class instead.
     *
     */
     ...
     * FEValuesBase), representing a vector-valued variable.         The
     * second argument denotes the index of the first component of the
     * selected vector.
     *
     */
     ...
     * object with   <code>dim</code>   components, the return type is a
     ...
     * selected by this view, for the shape function and quadrature point
     * selected by the arguments.         See the general documentation of
     * this class for how exactly the     gradient of a vector is defined.
     * @note   The meaning of the arguments is as documented for the value()
     * function.           @dealiiRequiresUpdateFlags{update_gradients}
     *
     */
     ...
     * quadrature point selected by the arguments.         The symmetric
     * gradient is defined as   $\frac 12 [(\nabla \phi_i(x_q)) + (\nabla
     * \phi_i(x_q))^T]$  , where   $\phi_i$   represents the
     * <code>dim</code>   components selected from the FEValuesBase object,
     * and       $x_q$   is the location of the   $q$  -th quadrature point.
     * @note   The meaning of the arguments is as documented for the value()
     * function.           @dealiiRequiresUpdateFlags{update_gradients}
     *
     */
    ...
    /**
     * Return the vector curl of the vector components selected by this view,     for the shape function and quadrature point selected by the arguments.     For 1d this function does not make any sense. Thus it is not     implemented for   <code>spacedim=1</code>  .  In 2d the curl is defined as     @f{equation*}{
     * \operatorname{curl}(u) \dealcoloneq \frac{du_2}{dx}
     *
     * -\frac{du_1}{dy},
     * @f}
     *      whereas in 3d it is given by     @f{equation*}{
     ...
     * This function is the equivalent of the
     * FEValuesBase::get_function_values   function but it only works on the
     * selected vector components.         The data type stored by the output
     * vector must be what you get when you     multiply the values of shape
     * functions (i.e.,   @p value_type)   times the     type used to store
     * the values of the unknowns   $U_j$   of your finite     element vector
     * $U$   (represented by the   @p fe_function   argument).
     ...
     * This function is the equivalent of the
     * FEValuesBase::get_function_gradients   function but it only works on
     * the     selected vector components.         The data type stored by the
     * output vector must be what you get when you     multiply the gradients
     * of shape functions (i.e.,   @p gradient_type)       times the type used
     * to store the values of the unknowns   $U_j$   of your     finite
     * element vector   $U$   (represented by the   @p fe_function
     ...
     * FEValuesBase::get_function_symmetric_gradients   in the FEValues
     * classes     but the information can be obtained from
     * FEValuesBase::get_function_gradients,   of course.         The data
     * type stored by the output vector must be what you get when you
     ...
     * FEValuesBase::get_function_divergences   in the FEValues classes but
     * the     information can be obtained from
     * FEValuesBase::get_function_gradients,       of course.         The data
     * type stored by the output vector must be what you get when you
     * multiply the divergences of shape functions (i.e.,   @p
     * divergence_type)       times the type used to store the values of the
     * unknowns   $U_j$   of your     finite element vector   $U$
     ...
     * function of the FEValues object was called.         There is no
     * equivalent function such as       FEValuesBase::get_function_curls   in
     * the FEValues classes but the     information can be obtained from
     * FEValuesBase::get_function_gradients,       of course.         The data
     * type stored by the output vector must be what you get when you
     * multiply the curls of shape functions (i.e.,   @p curl_type)   times
     * the     type used to store the values of the unknowns   $U_j$   of your
     * finite     element vector   $U$   (represented by the   @p fe_function
     * argument).           @dealiiRequiresUpdateFlags{update_gradients}
     *
     */
    template <class InputVector>
    ...
    /**
     * This function relates to get_function_curls() in the same way     as
     * get_function_values_from_local_dof_values() relates to
     * get_function_values(). See the documentation of
     * get_function_values_from_local_dof_values() for more information.
     *
     ...
     * Laplacians are the trace of the Hessians.         This function is the
     * equivalent of the       FEValuesBase::get_function_laplacians
     * function but it only works on the     selected vector components.
     * The data type stored by the output vector must be what you get when you
     * multiply the Laplacians of shape functions (i.e.,   @p laplacian_type)
     * times the type used to store the values of the unknowns   $U_j$   of
     * your     finite element vector   $U$   (represented by the   @p
     * fe_function   argument).
     * @dealiiRequiresUpdateFlags{update_hessians}
     *
     */
  ...
  /**
   * A class
   * representing a view to a set of   <code>(dim*dim + dim)/2</code>
   * components forming a symmetric second-order tensor from a vector-valued
   * finite element. Views are discussed in the     @ref vector_valued
   * module.     This class allows to query the value and divergence of
   * (components of)   shape functions and solutions representing symmetric
   * tensors. The   divergence of a symmetric tensor   $S_{ij}, 0\le
   * i,j<\text{dim}$   is defined   as   $d_i = \sum_j \frac{\partial
   * S_{ij}}{\partial x_j}, 0\le i<\text{dim}$  , which due to the symmetry of
   * the tensor is also   $d_i = \sum_j \frac{\partial S_{ji}}{\partial x_j}$
   * .  In other words, it due to   the symmetry of   $S$   it does not matter
   * whether we apply the nabla   operator by row or by column to get the
   * divergence.     You get an object of this type if you apply a
  ...
  public:
    /**
     * An alias for the data type of values of the view this class
     * represents. Since we deal with a set of   <code>(dim*dim +
     * dim)/2</code>       components (i.e. the unique components of a
     * symmetric second-order     tensor), the value type is a
    ...
    /**
     * An alias for the type of the divergence of the view this class
     * represents. Here, for a set of   <code>(dim*dim + dim)/2</code>
     * unique     components of the finite element representing a symmetric
     * second-order     tensor, the divergence of course is a
     * <code>Tensor@<1,dim@></code>  .         See the general discussion of
     * this class for a definition of the     divergence.
     *
     */
    ...
    /**
     * A struct that provides the output type for the product of the value
     * and derivatives of basis functions of the SymmetricTensor view and any
     * @p Number   type.           @deprecated   Use the types defined in the
     * surrounding class instead.
     *
     */
    template <typename Number>
     ...
     * symmetric tensor of rank 2.           @param   shape_function Number of
     * the shape function to be evaluated.     Note that this number runs from
     * zero to dofs_per_cell, even in the case     of an FEFaceValues or
     * FESubfaceValues object.           @param   q_point Number of the
     * quadrature point at which function is to     be evaluated.
     * @dealiiRequiresUpdateFlags{update_values}
     *
     */
    value_type
     ...
     * view, for the shape function and quadrature point selected by the
     * arguments.         See the general discussion of this class for a
     ...
     * FEValuesBase::get_function_divergences   in the FEValues classes but
     * the     information can be obtained from
     * FEValuesBase::get_function_gradients,       of course.         See the
     * general discussion of this class for a definition of the
     * divergence.         The data type stored by the output vector must be
     * what you get when you     multiply the divergences of shape functions
     * (i.e.,   @p divergence_type)       times the type used to store the
     * values of the unknowns   $U_j$   of your     finite element vector
     * $U$   (represented by the   @p fe_function   argument).
     * @dealiiRequiresUpdateFlags{update_gradients}
     *
     */
  ...
  /**
   * A class
   * representing a view to a set of   <code>dim*dim</code>   components
   * forming a second-order tensor from a vector-valued finite element. Views
   * are discussed in the     @ref vector_valued     module.     This class
   * allows to query the value, gradient and divergence of   (components of)
   ...
   * \frac{\partial T_{ij}}{\partial x_j}, \, 0\le i<\text{dim}$  , whereas
   * its gradient is   $G_{ijk} = \frac{\partial T_{ij}}{\partial x_k}$  .
   * You get an object of this type if you apply a
   * FEValuesExtractors::Tensor     to an FEValues, FEFaceValues or
   * FESubfaceValues object.
   * @ingroup feaccess vector_valued
   *
   */
  ...
  public:
    ...
    /**
     * Data type for taking the gradient of a second order tensor: a third
     * order     tensor.
     *
     */
    ...
    /**
     * A struct that provides the output type for the product of the value
     * and derivatives of basis functions of the Tensor view and any   @p
     * Number   type.           @deprecated   Use the types defined in the
     * surrounding class instead.
     *
     */
     ...
     * second-order tensor valued variable.         The second argument
     * denotes the index of the first component of the     selected symmetric
     * second order tensor.
     *
     */
     ...
     * the shape function and quadrature point selected by the arguments.
     * Here, since the view represents a vector-valued part of the FEValues
     * object with   <code>(dim*dim)</code>   components (the unique
     * components of     a second-order tensor), the return type is a tensor
     * of rank 2.           @param   shape_function Number of the shape
     * function to be evaluated.     Note that this number runs from zero to
     * dofs_per_cell, even in the case     of an FEFaceValues or
     * FESubfaceValues object.           @param   q_point Number of the
     * quadrature point at which function is to     be evaluated.
     * @dealiiRequiresUpdateFlags{update_values}
     *
    ...
    /**
     * Return the gradient (3-rd order tensor) of the vector components
     * selected     by this view, for the shape function and quadrature point
     * selected by the     arguments.         See the general discussion of
     * this class for a definition of the     gradient.
     * @note   The meaning of the arguments is as documented for the value()
     * function.           @dealiiRequiresUpdateFlags{update_gradients}
     *
     */
     ...
     * gradient.         The data type stored by the output vector must be
     * what you get when you     multiply the gradients of shape functions
     * (i.e.,   @p gradient_type)       times the type used to store the
     * values of the unknowns   $U_j$   of your     finite element vector
     * $U$   (represented by the   @p fe_function   argument).
     * @dealiiRequiresUpdateFlags{update_gradients}
     *
...
namespace internal
{
  namespace FEValuesViews
  {
    ...
    /**
     * A class whose specialization is used to define what FEValuesViews
     * object corresponds to the given FEValuesExtractors object.         When
     * using   FEValuesExtractors::Scalar,   the corresponding view is an
     ...
     * A class whose specialization is used to define what FEValuesViews
     * object corresponds to the given FEValuesExtractors object.         When
     * using   FEValuesExtractors::Vector,   the corresponding view is an
     ...
     * A class whose specialization is used to define what FEValuesViews
     * object corresponds to the given FEValuesExtractors object.         When
     * using   FEValuesExtractors::Tensor<rank>,   the corresponding view is
     * an       FEValuesViews::Tensor<rank,   dim, spacedim>.
     *
     */
     ...
     * A class whose specialization is used to define what FEValuesViews
     * object corresponds to the given FEValuesExtractors object.         When
     * using   FEValuesExtractors::SymmetricTensor<rank>,   the corresponding
...
{
  /**
   * A templated alias that associates to a given Extractor class   the
   * corresponding view in FEValuesViews.
   *
   */
  template <int dim, int spacedim, typename Extractor>
 ...
 * cannot be left to the applications programmer. FEValues, FEFaceValues and
 * FESubfaceValues provide only data handling: computations are left to
 * objects of type Mapping and FiniteElement. These provide functions
 * <tt>get_*_data</tt> and <tt>fill_*_values</tt> which are called by the
 * constructor and <tt>reinit</tt> functions of <tt>FEValues*</tt>,
 * respectively. <h3>General usage</h3> Usually, an object of
 * <tt>FEValues*</tt> is used in integration loops over all cells of a
 * triangulation (or faces of cells). To take full advantage of the
 * optimization features, it should be constructed before the loop so that
 ...
 * equals the gradient on the unit cell
 *
 ...
 * the FEValues object above. <h3>Member functions</h3>
 * The functions of this class fall into different categories:   <ul>     <li>   shape_value(), shape_grad(), etc: return one of the values of this object at a time. These functions are inlined, so this is the suggested access to all finite element values. There should be no loss in performance with an optimizing compiler. If the finite element is vector valued, then these functions return the only non-zero component of the requested shape function. However, some finite elements have shape functions that have more than one non-zero component (we call them non-"primitive"), and in this case this set of functions will throw an exception since they cannot generate a useful result. Rather, use the next set of functions.
 * <li>   shape_value_component(), shape_grad_component(), etc: This is the
 * same set of functions as above, except that for vector valued finite
 * elements they return only one vector component. This is useful for elements
 * of which shape functions have more than one non-zero component, since then
 * the above functions cannot be used, and you have to walk over all (or only
 * the non- zero) components of the shape function using this set of
 * functions.
 ...
 * <li>   reinit: initialize the FEValues object for a certain cell. This function is not in the present class but only in the derived classes and has a variable call syntax. See the docs for the derived classes for more information.   </ul>
 *
...
public:
   ...
   * n_quadrature_points, e.g., if a QCollection with different face
   * quadrature   rules has been passed to initialize FEFaceValues.     This
   * is mostly useful to initialize arrays to allocate the maximum amount   of
   * memory that may be used when re-sizing later on to a the current   number
   * of quadrature points given by n_quadrature_points.
   *
   */
   ...
   * derived class was called.     If the shape function is vector-valued,
   * then this returns the only non-   zero component. If the shape function
   * has more than one non-zero   component (i.e. it is not primitive), then
   * throw an exception of type   ExcShapeFunctionNotPrimitive. In that case,
   * use the   shape_value_component() function.       @param   function_no
   * Number of the shape function to be evaluated. Note   that this number
   * runs from zero to dofs_per_cell, even in the case of an   FEFaceValues or
   * FESubfaceValues object.       @param   point_no Number of the quadrature
   ...
   * function cannot be used.       @param   function_no Number of the shape
   * function to be evaluated.       @param   point_no Number of the
   * quadrature point at which function is to be   evaluated.       @param
   ...
   * performance drawback.     If the shape function is vector-valued, then
   * this returns the only non-   zero component. If the shape function has
   * more than one non-zero   component (i.e. it is not primitive), then it
   * will throw an exception of   type ExcShapeFunctionNotPrimitive. In that
   * case, use the   shape_grad_component() function.     The same holds for
   * the arguments of this function as for the   shape_value() function.
   * @param   function_no Number of the shape function to be evaluated.
   * @param   quadrature_point Number of the quadrature point at which
   ...
   * one component. This function is therefore only of greater interest if the
   * shape function is not primitive, but then it is necessary since the other
   * function cannot be used.     The same holds for the arguments of this
   * function as for the   shape_value_component() function.
   * @dealiiRequiresUpdateFlags{update_gradients}
   *
   */
  Tensor<1, spacedim>
   ...
   * there should be no major performance drawback.     If the shape function
   * is vector-valued, then this returns the only non-   zero component. If
   * the shape function has more than one non-zero   component (i.e. it is not
   * primitive), then throw an exception of type
   * ExcShapeFunctionNotPrimitive. In that case, use the
   * shape_hessian_component() function.     The same holds for the arguments
   * of this function as for the   shape_value() function.
   * @dealiiRequiresUpdateFlags{update_hessians}
   *
   */
   ...
   * since the other function cannot be used.     The same holds for the
   * arguments of this function as for the   shape_value_component() function.
   * @dealiiRequiresUpdateFlags{update_hessians}
   *
   */
  Tensor<2, spacedim>
   ...
   * values is returned, there should be no major performance drawback.     If
   * the shape function is vector-valued, then this returns the only non-
   ...
   * ExcShapeFunctionNotPrimitive. In that case, use the
   * shape_3rdderivative_component() function.     The same holds for the
   * arguments of this function as for the   shape_value() function.
   ...
   * it is necessary since the other function cannot be used.     The same
   * holds for the arguments of this function as for the
   ...
   * of the derived class was called, at the quadrature points.     If the
   * present cell is not active then values are interpolated to the   current
   * cell and point values are computed from that.     This function may only
   * be used if the finite element in use is a scalar   one, i.e. has only one
   * vector component.  To get values of multi-   component elements, there is
   * another get_function_values() below,   returning a vector of vectors of
   * results.       @param[in]   fe_function A vector of values that describes
   * (globally) the   finite element function that this function should
   * evaluate at the   quadrature points of the current cell.
   * @param[out]   values The values of the function specified by fe_function
   * at   the quadrature points of the current cell.  The object is assume to
   ...
   * the type used to store the values of the unknowns   $U_j$   of your
   * finite   element vector   $U$   (represented by the   @p fe_function
   * argument). This   happens to be equal to the type of the elements of the
   * solution vector.       @post     <code>values[q]</code>   will contain
   * the value of the field   described by fe_function at the   $q$  th
   * quadrature point.
   ...
   * last initialized.       @dealiiRequiresUpdateFlags{update_values}
   *
   */
  template <class InputVector>
  void
   ...
   * arguments is as explained there.       @post     <code>values[q]</code>
   * is a vector of values of the field described   by fe_function at the
   * $q$  th quadrature point. The size of the vector   accessed by
   * <code>values[q]</code>   equals the number of components of the   finite
   * element, i.e.   <code>values[q](c)</code>   returns the value of the
  ...
  /**
   * Generate function values from an arbitrary vector. This function   does
   * in essence the same as the first function of this name above,   except
   * that it does not make the assumption that the input vector   corresponds
   * to a DoFHandler that describes the unknowns of a finite   element field
   * (and for which we would then assume that   `fe_function.size() ==
   * dof_handler.n_dofs()`). Rather, the nodal   values corresponding to the
   * current cell are elements of an otherwise   arbitrary vector, and these
   * elements are indexed by the second   argument to this function. What the
   * rest of the `fe_function` input   argument corresponds to is of no
   * consequence to this function.     Given this, the function above
   * corresponds to passing `fe_function`   as first argument to the current
   * function, and using the   `local_dof_indices` array that results from the
   * following call as   second argument to the current function:
   * @code
   ...
   * @endcode
   * and then calling the current function with `local_dof_values` as   first
   * argument, and an array with indices `{0,...,fe.dofs_per_cell-1}`   as
   * second argument.     The point of the current function is that one
   * sometimes wants to   evaluate finite element functions at quadrature
   * points with nodal   values that are not stored in a global vector
   *
   ...
   *
   *  -  in that   case, all one would compute is a local representation of that   postprocessed function, characterized by its nodal values; this function   then allows the evaluation of that representation at quadrature points.       @param[in]   fe_function A vector of nodal values. This vector can have     an arbitrary size, as long as all elements index by `indices` can     actually be accessed.       @param[in]   indices A vector of indices into `fe_function`. This vector     must have length equal to the number of degrees of freedom on the     current cell, and must identify elements in `fe_function` in the     order in which degrees of freedom are indexed on the reference cell.       @param[out]   values A vector of values of the given finite element field,     at the quadrature points on the current object.       @dealiiRequiresUpdateFlags{update_values}
   *
   */
  template <class InputVector>
  void
   ...
   * function corresponds to the previous one, just for the vector-valued
   * case.       @dealiiRequiresUpdateFlags{update_values}
   *
   */
  template <class InputVector>
  void
  ...
  /**
   * Generate vector function values from an arbitrary vector. This   function
   * is similar to the previous one, but the `indices`   vector may also be a
   * multiple of the number of dofs per   cell. Then, the vectors in
   * <tt>value</tt> should allow for the same   multiple of the components of
   * the finite element.     Depending on the value of the last argument, the
   * outer vector of   <tt>values</tt> has either the length of the quadrature
   * rule   (<tt>quadrature_points_fastest == false</tt>) or the length of
   * components   to be filled <tt>quadrature_points_fastest == true</tt>. If
   * <tt>p</tt> is   the current quadrature point number and <tt>i</tt> is the
   * vector   component of the solution desired, the access to <tt>values</tt>
   * is   <tt>values[p][i]</tt> if <tt>quadrature_points_fastest ==
   * false</tt>, and   <tt>values[i][p]</tt> otherwise.     Since this
   * function allows for fairly general combinations of argument   sizes, be
   * aware that the checks on the arguments may not detect errors.
   * @dealiiRequiresUpdateFlags{update_values}
   *
   */
  template <class InputVector>
   ...
   * quadrature points of the current cell.       @param[out]   gradients The
   * gradients of the function specified by   fe_function at the quadrature
   * points of the current cell.  The gradients   are computed in real space
   * (as opposed to on the unit cell).  The object   is assume to already have
   * the correct size. The data type stored by this   output vector must be
   * what you get when you multiply the gradients of   shape function times
   * the type used to store the values of the unknowns     $U_j$   of your
   * finite element vector   $U$   (represented by the   @p     fe_function
   * argument).       @post     <code>gradients[q]</code>   will contain the
   * gradient of the field   described by fe_function at the   $q$  th
   * quadrature point.     <code>gradients[q][d]</code>   represents the
   * derivative in coordinate   direction   $d$   at quadrature point   $q$  .
   * @note   The actual data type of the input vector may be either a
   * Vector&lt;T&gt;, BlockVector&lt;T&gt;, or one of the PETSc or Trilinos
   * vector wrapper classes. It represents a global vector of DoF values
   * associated with the DoFHandler object with which this FEValues object was
   * last initialized.       @dealiiRequiresUpdateFlags{update_gradients}
   *
   */
  template <class InputVector>
  void
   ...
   * applied to multi-component (vector-valued) elements. The meaning of the
   * arguments is as explained there.       @post
   * <code>gradients[q]</code>   is a vector of gradients of the field
   * described by fe_function at the   $q$  th quadrature point. The size of
   * the   vector accessed by   <code>gradients[q]</code>   equals the number
   * of   components of the finite element, i.e.
   * <code>gradients[q][c]</code>     returns the gradient of the   $c$  th
   * vector component at the   $q$  th   quadrature point. Consequently,
   * <code>gradients[q][c][d]</code>   is the   derivative in coordinate
   * direction   $d$   of the   $c$  th vector component of   the vector field
   * at quadrature point   $q$   of the current cell.
   * @dealiiRequiresUpdateFlags{update_gradients}
   *
   */
  template <class InputVector>
  ...
  /**
   * This function relates to the first of the get_function_gradients()
   * function   above in the same way as the get_function_values() with
   * similar arguments   relates to the first of the get_function_values()
   ...
   * instead of its value.     This function may only be used if the finite
   * element in use is a scalar   one, i.e. has only one vector component.
   * There is a corresponding   function of the same name for vector-valued
   * finite elements.       @param[in]   fe_function A vector of values that
   * describes (globally) the   finite element function that this function
   * should evaluate at the   quadrature points of the current cell.
   ...
   * fe_function argument).       @post     <code>hessians[q]</code>   will
   * contain the Hessian of the field   described by fe_function at the   $q$
   * th quadrature point.     <code>hessians[q][i][j]</code>   represents the
   * $(i,j)$  th component of the   matrix of second derivatives at quadrature
   * point   $q$  .
   * @note   The actual data type of the input vector may be either a
   * Vector&lt;T&gt;, BlockVector&lt;T&gt;, or one of the PETSc or Trilinos
   * vector wrapper classes. It represents a global vector of DoF values
   * associated with the DoFHandler object with which this FEValues object was
   * last initialized.       @dealiiRequiresUpdateFlags{update_hessians}
   *
   */
  template <class InputVector>
  void
   ...
   * applied to multi-component (vector-valued) elements. The meaning of the
   * arguments is as explained there.       @post     <code>hessians[q]</code>
   * is a vector of Hessians of the field   described by fe_function at the
   * $q$  th quadrature point. The size of the   vector accessed by
   * <code>hessians[q]</code>   equals the number of   components of the
   * finite element, i.e.   <code>hessians[q][c]</code>     returns the
   * Hessian of the   $c$  th vector component at the   $q$  th quadrature
   * point. Consequently,   <code>hessians[q][c][i][j]</code>   is the
   * $(i,j)$  th   component of the matrix of second derivatives of the   $c$
   * th vector   component of the vector field at quadrature point   $q$   of
   * the current   cell.       @dealiiRequiresUpdateFlags{update_hessians}
   *
   */
  template <class InputVector>
  void
  ...
  /**
   * This function relates to the first of the get_function_hessians()
   * function   above in the same way as the get_function_values() with
   * similar arguments   relates to the first of the get_function_values()
   * functions. See there for   more information.
   * @dealiiRequiresUpdateFlags{update_hessians}
   ...
   * element field's second derivatives instead of its value.     This
   * function may only be used if the finite element in use is a scalar   one,
   * i.e. has only one vector component. There is a corresponding   function
   * of the same name for vector-valued finite elements.       @param[in]
   * fe_function A vector of values that describes (globally) the   finite
   * element function that this function should evaluate at the   quadrature
   * points of the current cell.       @param[out]   laplacians The Laplacians
   * of the function specified by   fe_function at the quadrature points of
   * the current cell.  The Laplacians   are computed in real space (as
   * opposed to on the unit cell).  The object   is assume to already have the
   * correct size. The data type stored by this   output vector must be what
   * you get when you multiply the Laplacians of   shape function times the
   * type used to store the values of the unknowns     $U_j$   of your finite
   * element vector   $U$   (represented by the   @p     fe_function
   * argument). This happens to be equal to the type of the   elements of the
   * input vector.       @post     <code>laplacians[q]</code>   will contain
   * the Laplacian of the field   described by fe_function at the   $q$  th
   * quadrature point.       @post   For each component of the output vector,
   * there holds     <code>laplacians[q]=trace(hessians[q])</code>  , where
   ...
   * applied to multi-component (vector-valued) elements. The meaning of the
   * arguments is as explained there.       @post
   * <code>laplacians[q]</code>   is a vector of Laplacians of the field
   * described by fe_function at the   $q$  th quadrature point. The size of
   * the   vector accessed by   <code>laplacians[q]</code>   equals the number
   * of   components of the finite element, i.e.
   * <code>laplacians[q][c]</code>     returns the Laplacian of the   $c$  th
   * vector component at the   $q$  th   quadrature point.       @post   For
   * each component of the output vector, there holds
   ...
   * arguments relates to the first of the get_function_values() functions.
   ...
   * store the values of the unknowns   $U_j$   of your finite element vector
   * $U$     (represented by the   @p fe_function   argument).       @post
   * <code>third_derivatives[q]</code>   will contain the third   derivatives
   * of the field described by fe_function at the   $q$  th quadrature
   ...
   * @post     <code>third_derivatives[q]</code>   is a vector of third
   * derivatives   of the field described by fe_function at the   $q$  th
   * quadrature point. The   size of the vector accessed by
   * <code>third_derivatives[q]</code>   equals   the number of components of
   * the finite element, i.e.     <code>third_derivatives[q][c]</code>
   * returns the third derivative of the     $c$  th vector component at the
   * $q$  th quadrature point. Consequently,
   ...
   * cell.       @dealiiRequiresUpdateFlags{update_3rd_derivatives}
   *
   */
  template <class InputVector>
  void
  ...
  /**
   * This function relates to the first of the
   * get_function_third_derivatives()   function above in the same way as the
   * get_function_values() with similar   arguments relates to the first of
   * the get_function_values() functions. See   there for more information.
   * @dealiiRequiresUpdateFlags{update_3rd_derivatives}
   *
   */
  template <class InputVector>
   ...
   * and `j` taking on all valid indices for cell degrees of freedom, as
  ...
  /**
   * Return an object that can be thought of as an array containing all
   * indices from   @p start_dof_index   (inclusive) to `dofs_per_cell`
   ...
   * `i` taking on all valid indices for cell degrees of freedom, as   defined
   * by the finite element passed to `fe_values`, and `j` taking   on a
   * specified subset of `i`'s range, starting at `i` itself and ending at
   * the number of cell degrees of freedom. In this way, we can construct the
   * upper half and the diagonal of a stiffness matrix contribution (assuming
   * it   is symmetric, and that only one half of it needs to be computed),
   * for   example.
   * @note   If the   @p start_dof_index   is equal to the number of DoFs in
   * the cell,   then the returned index range is empty.
   *
   */
  std_cxx20::ranges::iota_view<unsigned int, unsigned int>
  ...
  /**
   * Return an object that can be thought of as an array containing all
   * indices from zero (inclusive) to   @p end_dof_index   (inclusive). This
   * allows   one to write code using range-based `for` loops of the following
   * kind:
   * @code
   * FEValues<dim>      fe_values (...);
   * FullMatrix<double> cell_matrix (...);
   ...
   * `i` taking on all valid indices for cell degrees of freedom, as   defined
   * by the finite element passed to `fe_values`, and `j` taking   on a
   * specified subset of `i`'s range, starting at zero and ending at   `i`
   * itself. In this way, we can construct the lower half and the   diagonal
   * of a stiffness matrix contribution (assuming it is symmetric, and   that
   * only one half of it needs to be computed), for example.
   * @note   If the   @p end_dof_index   is equal to zero, then the returned
   ...
   * by the quadrature rule passed to `fe_values`.       @see   CPP11
   *
   */
  std_cxx20::ranges::iota_view<unsigned int, unsigned int>
   ...
   * point.     You can think of the quantity returned by this function as the
   * volume or   surface element   $dx, ds$   in the integral that we
   ...
   * jacobian().       @dealiiRequiresUpdateFlags{update_jacobians}
   *
   */
  ...
  /**
   * Return a reference to the array holding the values returned by
   * jacobian_grads().       @dealiiRequiresUpdateFlags{update_jacobian_grads}
   *
   */
   ...
   * object), then this function returns the outward normal vector to   the
   * cell at the <tt>i</tt>th quadrature point of the face.     In contrast,
   * if you call this function for a cell of codimension one   (i.e., when
   * using a `FEValues<dim,spacedim>` object with   `spacedim>dim`), then this
   * function returns the normal vector to the   cell
   *
   *  -  in other words, an approximation to the normal vector to the   manifold in which the triangulation is embedded. There are of   course two normal directions to a manifold in that case, and this   function returns the "up" direction as induced by the numbering of the   vertices.     The length of the vector is normalized to one.       @dealiiRequiresUpdateFlags{update_normal_vectors}
   *
   */
  const Tensor<1, spacedim> &
  ...
  /**
   * Return the normal vectors at all quadrature points represented by   this
   * object. See the normal_vector() function for what the normal   vectors
   * represent.       @dealiiRequiresUpdateFlags{update_normal_vectors}
   *
   */
  ...
  /**
   * Create a
   * view of the current FEValues object that represents a particular   scalar
   * component of the possibly vector-valued finite element. The   concept of
   * views is explained in the documentation of the namespace   FEValuesViews
   * and in particular in the     @ref vector_valued     module.
   *
   */
  ...
  /**
   * Create a
   * view of the current FEValues object that represents a set of
   ...
   * the namespace FEValuesViews and in particular in the     @ref
   ...
   * Create a
   * view of the current FEValues object that represents a set of
   * <code>(dim*dim)</code>   scalar components (i.e. a 2nd order tensor) of
   * the   vector-valued finite element. The concept of views is explained in
   * the   documentation of the namespace FEValuesViews and in particular in
   * the     @ref vector_valued     module.
   *
   */
...
protected:
  /**
   * Objects of the FEValues class need to store an iterator   to the present
   * cell in order to be able to extract the values of the   degrees of
   * freedom on this cell in the get_function_values() and assorted
   ...
   * get_interpolated_dof_values function).     This calls for a common base
   * class of iterator classes, and making the   functions we need here   @p
   * virtual.   On the other hand, this is the only   place in the library
   * where we need this, and introducing a base class of   iterators and
   * making a function virtual penalizes   <em>  all  </em>   users of   the
   * iterators, which are basically intended as very fast accessor
   ...
   * iterator type.     This way, the use of virtual functions is restricted
   * to only this class,   and other users of iterators do not have to bear
  ...
  /**
   * Initialize some update flags. Called from the   @p initialize   functions
   * of   derived classes, which are in turn called from their constructors.
...
/**
 * Finite element evaluated in quadrature points of a cell. This function
 * implements the initialization routines for FEValuesBase, if values in
 * quadrature points of a cell are needed. For further documentation see this
 * class.
 *
 *
 * @ingroup feaccess
...
public:
   ...
   * @note   We require, in contrast to FEFaceValues, that the number of
   * quadrature     rules in the collection is one.
   *
   */
  FEValues(const Mapping<dim, spacedim> &      mapping,
           const FiniteElement<dim, spacedim> &fe,
  ...
  /**
   * Return a reference to this very object.     Though it seems that it is
   * not very useful, this function is there to   provide capability to the
   * hp::FEValues   class, in which case it provides   the FEValues object for
   * the present cell (remember that for hp-finite   elements, the actual FE
   * object used may change from cell to cell, so we   also need different
   * FEValues objects for different cells; once you   reinitialize the
   * hp::FEValues   object for a specific cell, it retrieves   the FEValues
   * object for the FE on that cell and returns it through a   function of the
   * same name as this one; this function here therefore only   provides the
   * same interface so that one can templatize on FEValues and
   * hp::FEValues).
   *
   */
 ...
 * available in the interior of cells is also available here. See FEValuesBase
 *
 *
 * @ingroup feaccess
 *
...
public:
   ...
   * duty of the derived class's constructors.       @p n_faces_or_subfaces
   * is the number of faces or subfaces that this   object is to store. The
   * actual number depends on the derived class, for   FEFaceValues it is
   * <tt>2*dim</tt>, while for the FESubfaceValues class it   is
   * <tt>2*dim*(1<<(dim-1))</tt>, i.e. the number of faces times the number
  ...
  /**
   * Like the function above, but taking a collection of quadrature rules.
   * This   allows to assign each face a different quadrature rule. In the
   * case that   the collection only contains a single face quadrature, this
   * quadrature   rule is use on all faces.
   *
   */
  FEFaceValuesBase(const unsigned int                  dofs_per_cell,
                   const UpdateFlags                   update_flags,
  ...
  /**
   * Boundary form
   * of the transformation of the cell at the <tt>i</tt>th   quadrature point.
   ...
   * surface mapping.       @dealiiRequiresUpdateFlags{update_boundary_forms}
   *
   */
  const std::vector<Tensor<1, spacedim>> &
...
/**
 * Finite element evaluated in quadrature points on a face. This class adds
 * the functionality of FEFaceValuesBase to FEValues; see there for more
 * documentation. Since finite element functions and their derivatives may be
 * discontinuous at cell boundaries, there is no restriction of this function
 * to a mesh face. But, there are limits of these values approaching the face
 * from either of the neighboring cells.
 *
 *
 * @ingroup feaccess
 *
 ...
 * Finite element evaluated in quadrature points on a face. This class adds
 * the functionality of FEFaceValuesBase to FEValues; see there for more
 * documentation. This class is used for faces lying on a refinement edge. In
 * this case, the neighboring cell is refined. To be able to compute
 * differences between interior and exterior function values, the refinement
 * of the neighboring cell must be simulated on this cell. This is achieved by
 * applying a quadrature rule that simulates the refinement. The resulting
 * data fields are split up to reflect the refinement structure of the
 * neighbor: a subface number corresponds to the number of the child of the
 * neighboring face.
 *
 *
 * @ingroup feaccess
...
public:
   ...
   * subface on a given cell of type "iterator into a Triangulation object",
   * and   the given finite element. Since iterators into a triangulation
   * alone only   convey information about the geometry of a cell, but not
   * about degrees of   freedom possibly associated with this cell, you will
   * not be able to call   some functions of this class if they need
   * information about degrees of   freedom. These functions are, above all,
   * the   <tt>get_function_value/gradients/hessians/third_derivatives</tt>
   * functions. If you want to call these functions, you have to call the   @p
   * reinit variants that take iterators into DoFHandler or other DoF handler
   * type objects.
   *
   ...
   * functions. If you want to call these functions, you have to call the   @p
   * reinit variants that take iterators into DoFHandler or other DoF handler
   * type objects.     This does the same thing as the previous function but
   * takes iterators   instead of numbers as arguments.
   * @note     @p face   and   @p subface   must correspond to a face (and a
   * subface of   that face) of   @p cell.
   *
   */
  void
  reinit(const typename Triangulation<dim, spacedim>::cell_iterator &cell,
