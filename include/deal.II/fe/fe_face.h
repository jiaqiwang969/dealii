 ...
 * on equidistant points. Although the name does not give it away, the element
 * is discontinuous at locations where faces of cells meet. In particular,
 * this finite element is the trace space of FE_RaviartThomas on the faces and
 * serves in hybridized methods, e.g. in combination with the FE_DGQ element.
 * Its use is demonstrated in the   step-51   tutorial program.
 *
 *
 ...
 * use this element with FEValues for cell integration, then the values and
 * derivatives of shape functions will have invalid values and will not likely
 * produce anything useful. In order to make the use of this element as part
 * of an FESystem simpler, using a (cell) FEValues object will not fail
...
public:
   ...
   * second is the corresponding index of the other finite element.     The
   * set of such constraints is non-empty only for dim==1.
   *
   */
   ...
   * of freedom on lines.     The set of such constraints is non-empty only
   * for dim==2.
   *
   */
  virtual std::vector<std::pair<unsigned int, unsigned int>>
  ...
  /**
   * Same as hp_vertex_dof_indices(), except that the function treats degrees
   * of freedom on quads.     The set of such constraints is non-empty only
   * for dim==3.
   *
   */
  virtual std::vector<std::pair<unsigned int, unsigned int>>
...
/**
 * A finite element that is a Legendre element of complete polynomials on each
 * face (i.e., it is the face equivalent of what FE_DGP is on cells) and
 * undefined in the interior of the cells. The basis functions on the faces
 * are from   Polynomials::Legendre. Although the name does not give it away,
 * the element is discontinuous at locations where faces of cells meet. The
 * element serves in hybridized methods, e.g. in combination with the FE_DGP
 * element. An example of hybridizes methods can be found in the   step-51
 * tutorial program.
 *
 *
 * @note   Since this element is defined only on faces, only FEFaceValues and
...
/**
 * FE_FaceP in 1D, i.e., with degrees of freedom on the element vertices. See
 * the documentation of the general template for more information.
 *
 *
 */
template <int spacedim>
