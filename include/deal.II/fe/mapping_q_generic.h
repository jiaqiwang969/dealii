...
/**
 * This class implements the functionality for polynomial mappings   $Q_p$
 * of polynomial degree   $p$   that will be used on all cells of the mesh.
 * The MappingQ1 and MappingQ classes specialize this behavior slightly. The
 * class is poorly named. It should really have been called MappingQ because
 * it consistently uses   $Q_p$   mappings on all cells of a triangulation.
 * However, the name MappingQ was already taken when we rewrote the entire
 * class hierarchy for mappings. One might argue that one should always use
 * MappingQGeneric over the existing class MappingQ (which, unless explicitly
 * specified during the construction of the object, only uses mappings of
 * degree   $p$   <i>on cells at the boundary of the domain</i>). On the other
 * hand, there are good reasons to use MappingQ in many situations: in many
 * situations, curved domains are only provided with information about how
 * exactly edges at the boundary are shaped, but we do not know anything about
 * internal edges. Thus, in the absence of other information, we can only
 * assume that internal edges are straight lines, and in that case internal
 ...
 * <h4>Behavior along curved boundaries and with different manifolds</h4> As
 * described above, one often only knows a manifold description of a surface
 * but not the interior of the computational domain. In such a case, a
 ...
 * contain inverted regions close to the boundary. In order to avoid this
 * situation, this class applies an algorithm for making this transition
 * smooth using a so-called transfinite interpolation that is essentially a
 * linear blend between the descriptions along the surrounding entities. In
 * the algorithm that computes additional points, the
 ...
 * quad and thus always result in a well-defined transformation. The
 * interpolation scheme used in this class makes sure that curved descriptions
 * can go over to flat descriptions within a single layer of elements,
 * maintaining the overall optimal convergence rates of the finite element
 * interpolation. However, one does often get better solution qualities if the
 * transition between curved boundaries and flat interior domains is spread
 * over a larger range as the mesh is refined. This is provided by the special
 * manifold TransfiniteInterpolationManifold.
 *
 *
 */
...
public:
  ...
  public:
     ...
     * the mapping.         Which values, derivatives, or higher order
     * derivatives are computed is     determined by which of the member
     * arrays have nonzero sizes. They are     typically set to their
     * appropriate sizes by the initialize() and     initialize_face()
     * functions, which indeed call this function     internally. However, it
     * is possible (and at times useful) to do the     resizing by hand and
     * then call this function directly. An example is in     a Newton
     * iteration where we update the location of a quadrature point     (e.g.,
     * in   MappingQ::transform_real_to_uni_cell())   and need to re-
     ...
     * GeometryInfo::faces_per_cell   entries contain the vectors in the
     ...
     * functions may also be different.         In general, it is
     * $(p+1)^\text{dim}$  , where   $p$   is the polynomial     degree of the
     * mapping.
     *
     */
    const unsigned int n_shape_functions;

    /*     The default line support points. Is used in when the shape function     values are computed.         The number of quadrature points depends on the degree of this     class, and it matches the number of degrees of freedom of an     FE_Q<1>(this->degree).    
* */
    QGaussLobatto<1> line_support_points;

    /**
     * In case the quadrature rule given represents a tensor product     we
     * need to store the evaluations of the 1d polynomials at     the 1d
     * quadrature points. That is what this variable is for.
     *
     */
    ...
    /**
     * In case the quadrature rule given represents a tensor product     we
     * need to store temporary data in this object.
     *
     */
    mutable AlignedVector<VectorizedArray<double>> scratch;

    /**
     * In case the quadrature rule given represents a tensor product     the
     * values at the mapped support points are stored in this object.
     *
     */
    ...
    /**
     * In case the quadrature rule given represents a tensor product     the
     * values at the quadrature points are stored in this object.
     *
     */
    ...
    /**
     * In case the quadrature rule given represents a tensor product     the
     * gradients at the quadrature points are stored in this object.
     *
     */
    ...
    /**
     * In case the quadrature rule given represents a tensor product     the
     * hessians at the quadrature points are stored in this object.
     *
     */
   ...
   * passed in to the current function.       @param[in]   cell The cell where
   * to evaluate the mapping       @param[in]   unit_points The points in
   * reference coordinates where the   transformation (Jacobians, positions)
   * should be computed.       @param[in]   update_flags The kind of
   * information that should be computed.       @param[out]   output_data A
   * struct containing the evaluated quantities such   as the Jacobian
   * resulting from application of the mapping on the given   cell with its
...
protected:
  ...
  /*   The default line support points. These are used when computing the   location in real space of the support points on lines and quads, which   are needed by the Manifold<dim,spacedim> class.     The number of points depends on the degree of this class, and it matches   the number of degrees of freedom of an FE_Q<1>(this->degree).  
* */
  const std::vector<Point<1>> line_support_points;

  /*   The one-dimensional polynomials defined as Lagrange polynomials from the   line support points. These are used for point evaluations and match the   polynomial space of an FE_Q<1>(this->degree).  
* */
  const std::vector<Polynomials::Polynomial<double>> polynomials_1d;

  /*   The numbering from the lexicographic to the hierarchical ordering used   when expanding the tensor product with the mapping support points (which   come in hierarchical numbers).  
* */
...
* */
   ...
   * location of interior support points.     Access into this table is by
   * @p [structdim-1],   i.e., use 0 to access the   support point weights on
   * a line (i.e., the interior points of the   GaussLobatto quadrature), use
   * 1 to access the support point weights from   to perimeter to the interior
   * of a quad, and use 2 to access the support   point weights from the
   * perimeter to the interior of a hex.     The table itself contains as many
   * columns as there are surrounding points   to a particular object (2 for a
   * line,   <code>4 + 4*(degree-1)</code>   for   a quad,   <code>8 +
   * 12*(degree-1) + 6*(degree-1)*(degree-1)</code>   for a   hex) and as many
   * rows as there are strictly interior points.     For the definition of
   * this table see equation (8) of the `mapping'   report.
   *
   */
   ...
   * avoid some of the overhead in transforming data for mappings.     The
   * table has as many rows as there are vertices to the cell (2 in 1D, 4   in
   * 2D, 8 in 3D), and as many rows as there are additional support points
   ...
   * displacement field in addition to the geometry of the cell.     The
   * default implementation of this function is appropriate for most   cases.
   * It takes the locations of support points on the boundary of the   cell
   * from the underlying manifold. Interior support points (ie. support
   ...
   * support points.     The function works its way from the vertices (which
   * it takes from the   given cell) via the support points on the line (for
   * which it calls the   add_line_support_points() function) and the support
   * points on the quad   faces (in 3d, for which it calls the
   * add_quad_support_points() function).   It then adds interior support
   * points that are either computed by   interpolation from the surrounding
   * points using weights for transfinite   interpolation, or if dim<spacedim,
   * it asks the underlying manifold for   the locations of interior points.
   *
   */
  ...
  /**
   * Transform the point   @p p   on the real cell to the corresponding point
   * on   the unit cell   @p cell   by a Newton iteration.
   *
   */
  Point<dim>
   ...
   * vertices of a line are not included.     This function uses the
   * underlying manifold object of the line (or, if   none is set, of the
   * cell) for the location of the requested points. This   function is
   * usually called by compute_mapping_support_points() function.     This
   * function is made virtual in order to allow derived classes to choose
  ...
  /**
   * Append the support points of all shape functions located on bounding
   * faces (quads in 3d) of the given cell to the vector   @p a.   This
   * function   is only defined for <tt>dim=3</tt>. Points located on the
   * vertices or   lines of a quad are not included.     This function uses
   * the underlying manifold object of the quad (or, if   none is set, of the
   * cell) for the location of the requested points. This   function is
   * usually called by compute_mapping_support_points().     This function is
   * made virtual in order to allow derived classes to choose   shape function
   * support points differently than the present class, which   chooses the
   * points as interpolation points on the boundary.
   *
   */
  virtual void
