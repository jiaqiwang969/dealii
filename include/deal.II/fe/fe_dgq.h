 ...
 * equidistant support points. This is a discontinuous finite element based on
 * tensor products of Lagrangian polynomials. The shape functions are
 * Lagrangian interpolants of an equidistant grid of points on the unit cell.
 * The points are numbered in lexicographical order, with <i>x</i> running
 * fastest, then <i>y</i>, then <i>z</i> (if these coordinates are present for
 * a given space dimension at all). For example, these are the node orderings
 * for <tt>FE_DGQ(1)</tt> in 3d:
 * @verbatim
 *       6-------7        6-------7
 *      /|       |       /       /|
 *     / |       |      /       / |
 ...
 *   0---1---2        0---1---2
 * @endverbatim
 * with node 13 being placed in the interior of the hex. Note, however, that
 * these are just the Lagrange interpolation points of the shape functions.
 * Even though they may physically be on the boundary of the cell, they are
 * logically in the interior since there are no continuity requirements for
 * these shape functions across cell boundaries. While discontinuous, when
 * restricted to a single cell the shape functions of this element are exactly
 * the same as those of the FE_Q element where they are shown visually.
 ...
 * polynomial satisfies the   $\delta_{ij}$   property, i.e., where one
 * polynomial is one and all the others are zero.  For higher polynomial
 ...
 * point distribution close to the element boundary.
 *
 *
 */
...
public:
   ...
   * <tt>source.n_dofs_per_cell()</tt>.     These matrices are only available
   * if the source element is also a   @p     FE_DGQ element. Otherwise, an
   * exception of type     FiniteElement<dim>::ExcInterpolationNotImplemented
   * is thrown.
   *
   */
  virtual void
   ...
   * depending on the #restriction_is_additive_flags. This distinguishes
   * interpolation (concatenation) and projection with respect to scalar
   * products (summation).     Row and column indices are related to coarse
   * grid and fine grid spaces,   respectively, consistent with the definition
...
protected:
   ...
   * polynomials.     The degree of these polynomials is
   * <tt>polynomials.size()-1</tt>.
   *
   */
...
private:
  ...
  /**
   * Compute renumbering for rotation of degrees of freedom.     This function
   * rotates a tensor product numbering of degrees of   freedom by 90 degrees.
   * It is used to compute the transfer   matrices of the children by using
   * only the matrix for the first   child.     The direction parameter
   * determines the type of rotation. It is one   character of   @p xXyYzZ.
   * The character determines the axis of rotation,   case determines the
   * direction. Lower case is counter-clockwise seen in   direction of the
   * axis.     Since rotation around the y-axis is not used, it is not
   * implemented   either.
   *
   */
  void
 ...
 * Gauss-Lobatto quadrature (provided through the base class). See the base
 * class documentation in FE_DGQ for details.
 *
 *
 */
template <int dim, int spacedim = dim>
...
public:
   ...
   * Polynomials::Lagrange     interpolation of the support points in the
   * quadrature rule   <tt>points</tt>. The degree of these polynomials is
   * <tt>points.size()-1</tt>.
   *
   */
 ...
 * TensorProductPolynomials, is lexicographic. For instance, the ordering in
 * 2d is   $P_0(x)P_0(y),\ P_1(x)P_0(y),\ \ldots,\ P_n(x)P_0(y),\
 * P_0(x)P_1(y), \ \ldots,\ P_n(x)P_1(y),\ \ldots,\ P_0(x)P_n(y),\ \ldots,\
 * P_n(x)P_n(y)$   when <tt>degree=n</tt> where   $\{P_i\}_{i=0}^{n}$   are
 * the one-dimensional Legendre polynomials defined on   $[0,1]$  . As opposed
 * to the basic FE_DGQ element, these elements are not interpolatory and no
 * support points are defined. See the base class documentation in FE_DGQ for
 * details.
 *
 *
 */
...
public:
  /**
   * Constructor for tensor product polynomials based on
   * Polynomials::Legendre     interpolation.
   *
   */
 ...
 * defined. Note that Hermite polynomials are only available for degrees
 * larger or equal to three, and thus the beneficial properties of
 * Polynomials::HermiteLikeInterpolation   with only two basis functions
 * having a non-trivial value or derivative on a face per dimension is only
 * present for higher degrees. To facilitate usage also for degrees zero to
 * two, a usual Lagrange basis is constructed by this class. See the base
 * class documentation in FE_DGQ for details.
 *
 *
 */
