...
/**
 * Implementation of hierarchical   @p Qp   shape functions that yield the
 * finite element space of continuous, piecewise polynomials of degree   @p p.
 * This class is realized using tensor product polynomials based on a
 * hierarchical basis   Polynomials::Hierarchical   on the interval
 * <tt>[0,1]</tt> which is suitable for building an   @p hp   tensor product
 * finite element if we assume that each element has a single degree. The
 * constructor of this class takes the degree   @p p   of this finite element.
 * This class is not implemented for the codimension one case (<tt>spacedim !=
 * dim</tt>). <h3>Implementation</h3> The constructor creates a
 * TensorProductPolynomials object that includes the tensor product of   @p
 * Hierarchical   polynomials of degree   @p p.   This   @p
 * TensorProductPolynomials object provides all values and derivatives of the
 * shape functions. <h3>Numbering of the degrees of freedom (DoFs)</h3> The
 * original ordering of the shape functions represented by the
 ...
 * in the following: <h4>Q1 elements</h4> The   $Q_1^H$   element is of
 * polynomial degree one and, consequently, is exactly the same as the   $Q_1$
 * element in class FE_Q. In particular, the shape function are defined in the
 * exact same way:
 * <ul>     <li>   1D case:
 * @verbatim
 *    0-------1
 ...
 * @endverbatim
 *
 * The respective coordinate values of the support points of the degrees of freedom are as follows:   <ul>     <li>   Shape function 0: <tt>[0, 0, 0]</tt>;   <li>   Shape function 1: <tt>[1, 0, 0]</tt>;   <li>   Shape function 2: <tt>[0, 1, 0]</tt>;   <li>   Shape function 3: <tt>[1, 1, 0]</tt>;   <li>   Shape function 4: <tt>[0, 0, 1]</tt>;   <li>   Shape function 5: <tt>[1, 0, 1]</tt>;   <li>   Shape function 6: <tt>[0, 1, 1]</tt>;   <li>   Shape function 7: <tt>[1, 1, 1]</tt>;   </ul>     </ul>
 ...
 * The respective coordinate values of the support points of the degrees of freedom are as follows:   <ul>     <li>   Shape function 0: <tt>[0, 0, 0]</tt>;   <li>   Shape function 1: <tt>[1, 0, 0]</tt>;   <li>   Shape function 2: <tt>[0, 1, 0]</tt>;   <li>   Shape function 3: <tt>[1, 1, 0]</tt>;   <li>   Shape function 4: <tt>[0, 0, 1]</tt>;   <li>   Shape function 5: <tt>[1, 0, 1]</tt>;   <li>   Shape function 6: <tt>[0, 1, 1]</tt>;   <li>   Shape function 7: <tt>[1, 1, 1]</tt>;   <li>   Shape function 8: <tt>[0, 1/2, 0]</tt>;   <li>   Shape function 9: <tt>[1, 1/2, 0]</tt>;   <li>   Shape function 10: <tt>[1/2, 0, 0]</tt>;   <li>   Shape function 11: <tt>[1/2, 1, 0]</tt>;   <li>   Shape function 12: <tt>[0, 1/2, 1]</tt>;   <li>   Shape function 13: <tt>[1, 1/2, 1]</tt>;   <li>   Shape function 14: <tt>[1/2, 0, 1]</tt>;   <li>   Shape function 15: <tt>[1/2, 1, 1]</tt>;   <li>   Shape function 16: <tt>[0, 0, 1/2]</tt>;   <li>   Shape function 17: <tt>[1, 0, 1/2]</tt>;   <li>   Shape function 18: <tt>[0, 1, 1/2]</tt>;   <li>   Shape function 19: <tt>[1, 1, 1/2]</tt>;   <li>   Shape function 20: <tt>[0, 1/2, 1/2]</tt>;   <li>   Shape function 21: <tt>[1, 1/2, 1/2]</tt>;   <li>   Shape function 22: <tt>[1/2, 0, 1/2]</tt>;   <li>   Shape function 23: <tt>[1/2, 1, 1/2]</tt>;   <li>   Shape function 24: <tt>[1/2, 1/2, 0]</tt>;   <li>   Shape function 25: <tt>[1/2, 1/2, 1]</tt>;   <li>   Shape function 26: <tt>[1/2, 1/2, 1/2]</tt>;   </ul>     </ul>
 *
...
public:
  ...
  /**
   * For a finite element of degree   @p sub_degree   <   @p degree,   we
   * return a   vector which maps the numbering on an FE of degree   @p
   * sub_degree   into the   numbering on this element.
   *
   */
...
private:
   ...
   * dofs on quads, etc.     The dofs associated with 1d hierarchical
   * polynomials are ordered with the   vertices first (  $phi_0(x)=1-x$   and
   * $phi_1(x)=x$  ) and then the line dofs   (the higher degree polynomials).
   * The 2d and 3d hierarchical polynomials   originate from the 1d
   * hierarchical polynomials by tensor product. In the   following, the
   * resulting numbering of dofs will be denoted by   `fe_q_hierarchical
   * numbering`.     This function constructs a table which fe_q_hierarchical
   * index each   degree of freedom in the hierarchic numbering would have.
   ...
   * consecutive numbered dofs (like for LagrangeEquidistant).     It is
   * assumed that the size of the output argument already matches the
  ...
  /**
   * Initialize the   @p generalized_face_support_points   field of the
   * FiniteElement   class. Called from the constructor.
   *
   */
  void
