 ...
 * mesh cell. Selecting these flags in a restrictive way is crucial for the
 * efficiency of   FEValues::reinit(),     FEFaceValues::reinit()   and
 * FESubfaceValues::reinit().   Therefore, only the flags actually needed
 * should be selected. It is the responsibility of the involved Mapping and
 * FiniteElement to add additional flags according to their own requirements.
 * For instance, most finite elements will add
 * #update_covariant_transformation if #update_gradients is selected.  By
 * default, all flags are off, i.e. no reinitialization will be done. You can
 * select more than one flag by concatenation using the bitwise or
 * operator|(UpdateFlags,UpdateFlags). <h3>Use of these flags flags</h3>
 *
 *
 */
enum UpdateFlags
{
   ...
   * FEValues objects take the quadrature point locations on the   reference
   * cell as an argument of the constructor (via the   Quadrature object). For
   * most finite elements, knowing the   location of quadrature points on the
   * reference cell is all that   is necessary to evaluate shape functions,
   * evaluate the mapping,   and other things. On the other hand, if you want
   * to evaluate a   right hand side function   $f(\mathbf x_q)$   at
   * quadrature point   locations   $\mathbf x_q$   on the real cell, you need
   * to pass this   flag to the FEValues constructor to make sure you can
   * later   access them.     In the context of DataPostprocessor,
   * DataPostprocessorInputs::CommonInputs::evaluation_points   will be
   * updated.
   *
   */
 ...
 * (if dim<spacedim). However, this concept makes it easy to add additional
...
namespace internal
{
  namespace FEValuesImplementation
  {
     ...
     * dealii::FEValues,     dealii::FEFaceValues,   and
     * dealii::FESubfaceValues       objects. Objects of this kind will be
     * given as <i>output</i> argument     when   dealii::FEValues::reinit()
     * calls   Mapping::fill_fe_values()   for a     given cell, face, or
     * subface.         The data herein will then be provided as <i>input</i>
     * argument in the     following call to
    ...
    public:
       ...
       * FiniteElement::get_nonzero_components()   function which tells us
       * which       components are non-zero and thus have a row in the array
