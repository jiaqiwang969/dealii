...
/**
 * Discontinuous finite elements based on monomials. This finite element
 * implements complete polynomial spaces, that is, dim- dimensional
 ...
 * continuous. The basis functions for this element are chosen to be the
 * monomials listed above. Note that this is the main difference to the FE_DGP
 * class that uses a set of polynomials of complete degree   <code>p</code>
 * that form a Legendre basis on the unit square. Thus, there, the mass matrix
 * is diagonal, if the grid cells are parallelograms. The basis here does not
 * have this property; however, it is simpler to compute. On the other hand,
 * this element has the additional disadvantage that the local cell matrices
 * usually have a worse condition number than the ones originating from the
 * FE_DGP element. This class is not implemented for the codimension one case
 * (<tt>spacedim != dim</tt>). <h3>Transformation properties</h3> It is worth
 * noting that under a (bi-, tri-)linear mapping, the space described by this
 * element does not contain   $P(k)$  , even if we use a basis of polynomials
 * of degree   $k$  . Consequently, for example, on meshes with non-affine
 * cells, a linear function can not be exactly represented by elements of type
 * FE_DGP(1) or FE_DGPMonomial(1).
...
public:
   ...
   * dofs_per_face times <tt>source.dofs_per_face</tt>.     Derived elements
   * will have to implement this function. They may only   provide
   * interpolation matrices for certain source finite elements, for   example
   * those from the same family. If they don't implement interpolation   from
   * a given element, then they must throw an exception of type
