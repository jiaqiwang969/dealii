...
namespace parallel
{
  ...
  namespace shared
  {
     ...
     * step-18  .         Different from the
     * parallel::distributed::Triangulation   and
     ...
     * code than the more involved   parallel::distributed::Triangulation
     * class     in which processors only know their own part of the mesh, but
     * nothing     about cells owned by other processors with the exception of
     * a single     layer of ghost cells around their own part of the domain.
     ...
     * all processors if you want to adapt them, regardless of being
     * classified     as locally owned, ghost or artificial.         The class
     * is also useful in cases where compute time and memory
     ...
     * about the entire mesh. An example could be where an application     has
     * to have both volume and surface meshes that can then both     be
     * partitioned independently, but where it is difficult to ensure     that
     * the locally owned set of surface mesh cells is adjacent to the
     ...
     * adjacent cells from processor to processor.         The partitioning of
     * cells between processors is done internally     based on a number of
     * different possibilities. By passing appropriate     flags to the
     * constructor of this class (see the
     * parallel::shared::Triangulation::Settings       enum), it is possible
     * to select different ways of partitioning the mesh,     including ways
     * that are dictated by the application and not by the     desire to
     * minimize the length of the interface between subdomains owned     by
     * processors (as is done by the METIS and Zoltan packages, both of
    ...
    public:
      ...
      enum Settings
      {
         ...
         * @endcode
         *
         * @note   If you plan to use a custom partition with geometric
         * multigrid,         you must manually partition the level cells in
         * addition to the active         cells.
         *
         */
         ...
         * communication.                 Note: This flag should always be set
         * alongside a flag for an         active cell partitioning method.
         *
         */
      ...
      /**
       * Constructor.             The flag   @p allow_artificial_cells   can
       * be used to enable artificial       cells. If enabled, this class will
       * behave similarly       to   parallel::distributed::Triangulation
       * and         parallel::fullydistributed::Triangulation   in the sense
       * that there will       be locally owned cells, a single layer of ghost
       * cells, and       artificial cells. However, one should not forget
       * that in contrast to       those parallel triangulations all cells are
       * duplicated on all       processes, leading in most cases to
       * significantly more artificial       cells.             If artificial
       * cells are disabled, all non-locally owned cells are       considered
       * ghost cells. This might lead to very expensive ghost-value
       * update steps. While in the case of artificial cells, ghost-value
       * updates lead to communication only with the direct process neighbors
       * in       a point-to-point fashion, these degenerate to an operation
       * in which       every process communicates with every other process
       * (an "all-to-all"       communication) if no artificial cells are
       * available. If such       ghost-value updates are the bottleneck in
       * your code, you may want to       consider enabling artificial cells.
       *
       */
      Triangulation(
       ...
       * flags set.             This step is equivalent to the
       * dealii::Triangulation   class with an       addition of calling
       * dealii::GridTools::partition_triangulation()   at       the end.
       *
       */
      virtual void
      ...
      /**
       * Create a triangulation.             This function also partitions
       * triangulation based on the MPI       communicator provided to the
       * constructor.
       *
       */
      virtual void
      ...
      /**
       * Copy   @p other_tria   to this triangulation.             This
       * function also partitions triangulation based on the MPI
       * communicator provided to the constructor.
       * @note   This function can not be used with
       * parallel::distributed::Triangulation,         since it only stores
       * those cells that it owns, one layer of ghost cells       around the
       * ones it locally owns, and a number of artificial cells.
       *
       */
      virtual void
      copy_triangulation(
       ...
       * serialization. Throw away the previous content.             This
       * function first does the same work as in
       * dealii::Triangulation::load,   then partitions the triangulation
       * based on       the MPI communicator provided to the constructor.
       *
       */
      ...
      /**
       * Return a vector of length   Triangulation::n_active_cells()   where
       * each       element stores the subdomain id of the owner of this cell.
       * The       elements of the vector are obviously the same as the
       * subdomain ids       for locally owned and ghost cells, but are also
       * correct for       artificial cells that do not store who the owner of
       * the cell is in       their subdomain_id field.
       *
       */
      ...
      /**
       * Return a vector of length   Triangulation::n_cells(level)   where
       * each       element stores the level subdomain id of the owner of this
       * cell. The       elements of the vector are obviously the same as the
       * level subdomain       ids for locally owned and ghost cells, but are
       * also correct for       artificial cells that do not store who the
       * owner of the cell is in       their level_subdomain_id field.
       *
       */
      const std::vector<types::subdomain_id> &
    ...
    private:
       ...
       * are artificial will have cell->subdomain_id() ==
       * numbers::artificial;               The original partition information
       * is stored to allow using sequential       DoF distribution and
       * partitioning functions with semi-artificial       cells.
       *
       */
       ...
       * partitioning each level.             The original partition
       * information is stored to allow using sequential       DoF
       * distribution and partitioning functions with semi-artificial
       * cells.
       *
       */
  ...
  namespace shared
  {
     ...
     * parallel::shared::Triangulation   objects throughout the library even
     * if     it is disabled.         Since the constructor of this class is
     * deleted, no such objects     can actually be created as this would be
     * pointless given that     MPI is not available.
     *
     */
    template <int dim, int spacedim = dim>
    class Triangulation
...
namespace internal
{
  namespace parallel
  {
    namespace shared
    {
      ...
      public:
        /**
         * Constructor.                 Stores the subdomain ID of all active
         * cells if the provided         Triangulation is of type
         * parallel::shared::Triangulation.                   Replaces them by
         * their true subdomain ID equivalent.
         *
         */
        ...
        /**
         * Destructor.                 Returns the subdomain ID of all active
         * cells on the           parallel::shared::Triangulation   into their
         * previous state.
         *
         */
        ~TemporarilyRestoreSubdomainIds();
