...
namespace GraphColoring
{
  namespace internal
  {
     ...
     * is not computed.       @param   indices1 A set of indices, assumed
     * sorted.       @param   indices2 A set of indices, assumed sorted.
     ...
     * conflict with other elements in the same zone.         The question
     * whether two iterators conflict is determined by a user-     provided
     * function. The meaning of this function is discussed in the
     * documentation of the   GraphColoring::make_graph_coloring()   function.
     * @param[in]   begin The first element of a range of iterators for which
     * a     partitioning is sought.       @param[in]   end The element past
     * the end of the range of iterators.       @param[in]
     * get_conflict_indices A user defined function object     returning a set
     * of indicators that are descriptive of what represents a     conflict.
     * See above for a more thorough discussion.       @return   A set of sets
     * of iterators (where sets are represented by       std::vector   for
     * efficiency). Each element of the outermost set     corresponds to the
     * iterators pointing to objects that are in the same     partition (i.e.,
     * the same zone).
     *
     */
     ...
     *
     *
     *  - If all the vertices are colored, stop. Otherwise, return to 3.           @param[in]   partition The set of iterators that should be colored.       @param[in]   get_conflict_indices A user defined function object     returning a set of indicators that are descriptive of what represents a     conflict. See above for a more thorough discussion.       @param[out]   partition_coloring A set of sets of iterators (where sets     are represented by   std::vector   for efficiency). Each element of the     outermost set corresponds to the iterators pointing to objects that are     in the same partition (have the same color) and consequently do not     conflict. The elements of different sets may conflict.
     *
     */
    template <typename Iterator>
    void
   ...
   * by a user-provided function.     This function can also be considered as
   * a graph coloring: each object   pointed to by an iterator is considered
   * to be a node and there is an edge   between each two nodes that conflict.
   * The graph coloring algorithm then   assigns a color to each node in such
   * a way that two nodes connected by an   edge do not have the same color.
   ...
   * nonempty intersection.     In this generality, computing the graph of
   * conflicts would require   calling a function that determines whether two
   * iterators (or the two   objects they represent) conflict, and calling it
   * for every pair of   iterators, i.e.,   $\frac 12 N (N-1)$   times. This
   * is too expensive in   general. A better approach is to require a
   * user-defined function that   returns for every iterator it is called for
   * a set of indicators of some   kind that characterize a conflict; two
   * iterators are in conflict if their   conflict indicator sets have a
   * nonempty intersection. In the example of   assembling a matrix, the
   * conflict indicator set would contain the indices   of all degrees of
   * freedom on the cell pointed to (in the case of   continuous Galerkin
   * methods) or the union of indices of degree of freedom   on the current
   * cell and all cells adjacent to the faces of the current   cell (in the
   * case of discontinuous Galerkin methods, because there one   computes face
   * integrals coupling the degrees of freedom connected by a   common face
   *
   *  -  see   step-12  ).
   ...
   * hanging nodes.     In other situations, the conflict indicator sets may
   * represent something   different altogether
   *
   *  -  it is up to the caller of this function to   describe what it means for two iterators to conflict. Given this,   computing conflict graph edges can be done significantly more cheaply   than with   ${\cal O}(N^2)$   operations.     In any case, the result of the function will be so that iterators whose   conflict indicator sets have overlap will not be assigned to the same   color.
   * @note
   * The algorithm used in this function is described in a paper by
   ...
   * coloring is sought.     @param[in]   end The element past the end of the
   * range of iterators.     @param[in]   get_conflict_indices A user defined
   * function object returning   a set of indicators that are descriptive of
   * what represents a conflict.   See above for a more thorough discussion.
   ...
   * coloring using graph connections represented by SparsityPattern.   For
   * further details, refer to   SparsityTools::color_sparsity_pattern.
   *
   */
  unsigned int
