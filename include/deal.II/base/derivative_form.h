 ...
 * \mathbf F: {\mathbb R}^{\text{dim}} \rightarrow {\mathbb
 * R}^{\text{spacedim}}$  . Such functions are always used to map the
 * reference dim-dimensional cell into spacedim-dimensional space. For such
 * objects, the first derivative of the function is a linear map from
 * ${\mathbb R}^{\text{dim}}$   to   ${\mathbb R}^{\text{spacedim}}$  , i.e.,
 * it can be represented as a matrix in   ${\mathbb R}^{\text{spacedim}\times
 * \text{dim}}$  . This makes sense since one would represent the first
 * derivative,   $\nabla \mathbf F(\mathbf x)$   with   $\mathbf x\in {\mathbb
 * R}^{\text{dim}}$  , in such a way that the directional derivative in
 ...
 * F(\mathbf x)$   by a vector in   ${\mathbb R}^{\text{dim}}$  , and the
 * result is a difference of function values, which are in   ${\mathbb
 * R}^{\text{spacedim}}$  . Consequently, the matrix must be of size
 * $\text{spacedim}\times\text{dim}$  . Similarly, the second derivative is a
 * bilinear map from    ${\mathbb R}^{\text{dim}} \times  {\mathbb
 * R}^{\text{dim}}$   to   ${\mathbb R}^{\text{spacedim}}$  , which one can
 * think of a rank-3 object of size
 * $\text{spacedim}\times\text{dim}\times\text{dim}$  . In deal.II we
 * represent these derivatives using objects of type DerivativeForm
 * @<1,dim,spacedim,Number@>,   DerivativeForm  @<2,dim,spacedim,Number@>
 * and so on.
 *
 *
 */
...
public:
  ...
  /**
   * Converts a DerivativeForm <order, dim, dim, Number> to Tensor<order+1,
   * dim,   Number>. In particular, if order == 1 and the derivative is the
   * Jacobian of     $\mathbf F(\mathbf x)$  , then Tensor[i] =   $\nabla
   * F_i(\mathbf x)$  .
   *
   */
  ...
  /**
   * Return the transpose of a rectangular DerivativeForm,   viewed as a two
   * dimensional matrix.
   *
   */
   ...
   * $\sqrt{\sum_{ij} |DF_{ij}|^2} = \sqrt{\sum_{ij} |\frac{\partial
   * F_i}{\partial x_j}|^2}$  .
   *
   */
   ...
   * transformation   $\mathbf F$  . That is to say if   $DF$   is square, it
   * computes     $\det(DF)$  , in case DF is not square returns
   * $\sqrt{\det(DF^T \,DF)}$  .
   *
   */
  Number
   ...
   * $\mathbf F$  , then the current function computes the <i>covariant</i>
   * form   of the derivative, namely   $(\nabla \mathbf F) {\mathbf G}^{-1}$
   * , where     $\mathbf G = (\nabla \mathbf F)^{T}(\nabla \mathbf F)$  . If
   * $\nabla \mathbf F$   is a square matrix (i.e.,   $\mathbf F: {\mathbb
   * R}^n \mapsto {\mathbb R}^n$  ), then this function   simplifies to
   * computing   $\nabla {\mathbf F}^{-T}$  .
   *
   */
...
private:
  /**
   * Auxiliary function that computes   $A T^{T}$   where A represents the
   * current   object.
   *
   */
  DerivativeForm<1, dim, spacedim, Number>
 ...
 *
 * - \mathbf F(\mathbf x).
 * @f] The transformation corresponds to @f[
 * [\text{result}]_{i_1,\dots,i_k} = i\sum_{j} \left[\nabla \mathbf F(\mathbf
 * x)\right]_{i_1,\dots,i_k, j} \Delta x_j @f] in index notation and
 * corresponds to   $[\Delta \mathbf x] [\nabla \mathbf F(\mathbf x)]^T$   in
 * matrix notation.
...
/**
 * Similar to the previous apply_transformation(). Each row of the result
 * corresponds to one of the rows of   @p D_X   transformed by   @p grad_F,
