 ...
 * iterators</i>. Of course, the class may also be used to denote other
 * iterator ranges using different kinds of iterators into other containers.
 *
 *  <h3>Class design: Motivation</h3> Informally, the way the C++11 standard
 * describes <a
 ...
 * https://en.cppreference.com/w/cpp/language/range-for .) In other words, the
 * compiler introduces a temporary variable that <i>iterates</i> over the
 * elements of the container or collection, and the original variable
 * <code>v</code>   that appeared in the range-based for loop represents the
 * <i>dereferenced</i> state of these iterators
 *
 *  -  namely, the <i>elements</i> of the collection.
 ...
 *
 *  <h3>Class design: Implementation</h3> To represent the desired semantics
 * as outlined above, this class stores a half-open range of iterators
 * <code>[b,e)</code>   of the given template type. Secondly, the class needs
 * to provide begin() and end() functions in such a way that if you
 * <i>dereference</i> the result of   IteratorRange::begin(),   you get the
 * <code>b</code>   iterator. Furthermore, you must be able to increment the
 * object returned by   IteratorRange::begin()   so that   <code>*(++begin())
 * == b+1</code>  . In other words,   IteratorRange::begin()   must return an
 * iterator that when dereferenced returns an iterator of the template type
 * <code>Iterator</code>  : It is an iterator over iterators in the same sense
 * as if you had a pointer into an array of pointers. This is implemented in
 * the form of the   IteratorRange::IteratorOverIterators   class.
 *
 *
 * @ingroup CPP11
 *
...
public:
   ...
   * it points to the given argument.       @param   iterator An iterator to
   * which this object is supposed to point.
   *
   */
  ...
  /**
   * Comparison operator     @param   i_o_i Another iterator over iterators.
  ...
  /**
   * Implicit conversion operator.       @warning   When you call this
   * conversion operator (i.e., you convert this   iterator-over-iterators to
   * the iterator we are currently pointing to),   you obtain a `const`
   * reference to this underlying iterator. The only   thing you can really do
   * with this result is dereferencing itself: it   presumably points to
   * something useful, but since you don't know where   the pointed to object
   * lives, you shouldn't increment or decrement the   iterator you get from
   * this operator. As a consequence, the returned   iterator is marked as
   * `const`, as this should prevent you from doing   anything other than
   * dereference it.
   *
   */
...
/**
 * Create an object of type IteratorRange given the beginning and end
 * iterator.
 *
 *
 */
