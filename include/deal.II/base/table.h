...
{
  /**
   * @internal   Have a namespace in which we declare some classes that are
   * used   to access the elements of tables using the <tt>operator[]</tt>.
   * These are   quite technical, since they have to do their work recursively
   * (due to the   fact that the number of indices is not known, we have to
   * return an   iterator into the next lower dimension object if we access
   * one object,   until we are on the lowest level and can actually return a
   * reference to   the stored data type itself).  This is so technical that
   * you will not   usually want to look at these classes at all, except
   * possibly for   educational reasons.  None of the classes herein has a
   * interface that you   should use explicitly in your programs (except, of
   * course, through access   to the elements of tables with
   * <tt>operator[]</tt>, which generates   temporary objects of the types of
   * this namespace).
   *
   */
  namespace TableBaseAccessors
  {
     ...
     * emerges.         The value of <tt>P</tt> is also used to determine the
     * stride: this     object stores a pointer indicating the beginning of
     * the range of     objects that it may access. When we apply
     * <tt>operator[]</tt> on this     object, the resulting new accessor may
     * only access a subset of these     elements, and to know which subset we
     * need to know the dimensions of     the table and the present index,
     * which is indicated by <tt>P</tt>.         As stated for the entire
     * namespace, you will not usually have to deal     with these classes
     * directly, and should not try to use their interface     directly as it
     * may change without notice. In fact, since the     constructors are made
     * private, you will not even be able to generate     objects of this
     * class, as they are only thought as temporaries for     access to
     * elements of the table class, not for passing them around as
    ...
    public:
       ...
       * sub-tables to functions as arguments, as in
       * <code>f(table[i])</code>  .             Using this constructor is
       * risky if accessors are stored longer than       the table it points
       * to. Don't do this.
       *
       */
    ...
    /**
     * @internal   Accessor class for tables. This is the specialization for
     * the     last index, which actually allows access to the elements of the
     * table,     rather than recursively returning access objects for further
...
/**
 * A class holding a multi-dimensional array of objects of templated type. If
 * the template parameter indicating the number of dimensions is one, then
 * this class more or less represents a vector; if it is two then it is a
 * matrix; and so on. This class specifically replaces attempts at
 * higher-dimensional arrays like   <tt>std::vector<std::vector<T>></tt>,   or
 * even higher nested constructs. These constructs have the disadvantage that
 * they are hard to initialize, and most importantly that they are very
 * inefficient if all rows of a matrix or higher-dimensional table have the
 * same size (which is the usual case), since then the memory for each row is
 * allocated independently, both wasting time and memory. This can be made
 * more efficient by allocating only one chunk of memory for the entire
 * object, which is what the current class does.
 *
 *  <h3>Comparison with the Tensor class</h3> In some way, this class is
 * similar to the Tensor class, in that it templatizes on the number of
 * dimensions. However, there are two major differences. The first is that the
 * Tensor class stores only numeric values (as <tt>double</tt>s), while the
 * Table class stores arbitrary objects. The second is that the Tensor class
 * has fixed sizes in each dimension, also given as a template argument, while
 * this class can handle arbitrary and different sizes in each dimension. This
 * has two consequences. First, since the size is not known at compile time,
 * it has to do explicit memory allocation. Second, the layout of individual
 * elements is not known at compile time, so access is slower than for the
 * Tensor class where the number of elements are their location is known at
 * compile time and the compiler can optimize with this knowledge (for example
 * when unrolling loops). On the other hand, this class is of course more
 * flexible, for example when you want a two-dimensional table with the number
 * of rows equal to the number of degrees of freedom on a cell, and the number
 * of columns equal to the number of quadrature points. Both numbers may only
 * be known at run-time, so a flexible table is needed here. Furthermore, you
 * may want to store, say, the gradients of shape functions, so the data type
 * is not a single scalar value, but a tensor itself.
 *
 *  <h3>Dealing with large data sets</h3> The Table classes (derived from this
 * class) are frequently used to store large data tables. A modest example is
 * given in   step-53   where we store a   $380 \times 220$   table of
 * geographic elevation data for a region of Africa, and this data requires
 * about 670 kB if memory; however, tables that store three- or
 * more-dimensional data (say, information about the density, pressure, and
 * temperature in the earth interior on a regular grid of `(latitude,
 * longitude, depth)` points) can easily run into hundreds of megabytes or
 * more. These tables are then often provided to classes such as
 * InterpolatedTensorProductGridData or InterpolatedUniformGridData. If you
 * need to load such tables on single-processor (or multi-threaded) jobs, then
 * there is nothing you can do about the size of these tables: The table just
 * has to fit into memory. But, if your program is parallelized via MPI, then
 * a typical first implementation would create a table object on every process
 * and fill it on every MPI process by reading the data from a file. This is
 * inefficient from two perspectives:
 *
 *
 *
 ...
 * TableBase::replicate_across_communicator()   function that is internally
 * based on   AlignedVector::replicate_across_communicator().   This function
 * allows for workflows like the following where we put that MPI process with
 * rank zero in charge of reading the data (but it could have been any other
 * "root rank" as well):
 *
 * @code
 ...
 * available on all non-root processes, either by re-creating a copy of the
 * table in the other processes' memory space or, if possible, by creating
 * copies in shared memory once for all processes located on each of the
 * machines used by the MPI job.
 *
 *
 * @ingroup data
 *
...
public:
   ...
   * The size is adjusted if needed.     We can't use the other, templatized
   * version since if we don't declare   this one, the compiler will happily
   * generate a predefined copy operator   which is not what we want.
   *
   */
  TableBase<N, T> &
   ...
   * is adjusted if needed.     This function requires that the type
   * <tt>T2</tt> is convertible to   <tt>T</tt>.
   *
   */
  template <typename T2>
  TableBase<N, T> &
   ...
   * accommodate these sizes. If   @p omit_default_initialization     is set
   * to   @p false,   all elements of the table are set to a   default
   * constructed object for the element type. Otherwise the   memory is left
   * in an uninitialized or otherwise undefined state.
   *
   */
  void
   ...
   * inserting   std::istream_iterator).   The second argument denotes whether
   * the   elements pointed to are arranged in a way that corresponds to the
   * last   index running fastest or slowest. The default is to use C-style
   * indexing   where the last index runs fastest (as opposed to Fortran-style
   * where the   first index runs fastest when traversing multidimensional
   * arrays. For   example, if you try to fill an object of type Table<2,T>,
   * then calling   this function with the default value for the second
   * argument will result   in the equivalent of doing
   * @code
   * Table<2,T> t;
   ...
   * the given set of iterators.       @param   entries An iterator to a set
   * of elements from which to initialize   this table. It is assumed that
   * iterator can be incremented and   dereferenced a sufficient number of
   * times to fill this table.     @param   C_style_indexing If true, run over
   * elements of the table with the   last index changing fastest as we
   * dereference subsequent elements of the   input range. If false, change
   ...
   * access this shared memory   region instead of keeping their own copy. See
   * the general documentation   of this class for a code example.     The
   * intent of this function is to quickly exchange large arrays from   one
   * process to others, rather than having to compute or create it on   all
   * processes. This is specifically the case for data loaded from   disk
   *
  ...
  /**
   * Swap the contents of this table and the other table   @p v.   One could
   ...
   * need to allocate temporary storage and move data around.     This
   * function is analogous to the   @p swap   function of all C++   standard
...
protected:
  ...
  /**
   * Return a read-write reference to the indicated element.     This function
   * does no bounds checking and is only to be used internally   and in
   ...
   * and in functions already checked.     We return the requested value as a
   * constant reference rather than by   value since this object may hold data
   * types that may be large, and we   don't know here whether copying is
   * expensive or not.
   *
   */
  typename AlignedVector<T>::const_reference
 ...
 * correct number of arguments, etc. Rather than this general template, these
 * functions are implemented in partial specializations of this class, with
 * fixed numbers of dimensions. See there, and in the documentation of the
...
/**
 * A class representing a one-dimensional table, i.e. a vector-like class. The
 * majority of the interface of this class is implemented in the TableBase
 * base class. See there for an outline of the rationale for and interface of
 * this class.
 *
 *
 * @ingroup data
...
public:
   ...
   * set of iterators.     This function is entirely equivalent to creating a
   * table   <code>t</code>     of the given size and then calling
   * @code
   ...
   * object right away so that it can be marked const.     Using this
   * constructor, you can do things like this:
   * @code
   ...
   *                         true);
   * @endcode
   * @param   size The size of this one-dimensional table.     @param
   * entries An iterator to a set of elements from which to initialize   this
   * table. It is assumed that iterator can be incremented and   dereferenced
   * a sufficient number of times to fill this table.     @param
   * C_style_indexing If true, run over elements of the table with the   last
   * index changing fastest as we dereference subsequent elements of the
...
/**
 * A namespace for iterators and accessors for Table<2, T> and TransposeTable.
 * These classes have special accessors (that is, compared to Table<3, T>)
 * since they have a matrix-like structure; i.e., the accessors also provide
 * row and column information and are designed to be compatible with the
 * SparseMatrix and SparsityPattern iterator classes.
 *
 *
 */
namespace MatrixTableIterators
{
   ...
   * description of the split between iterators and accessors.       @tparam
   ...
   * and can modify the provided table object.       @tparam   storage_order
   * The storage scheme of the underlying table, e.g.,     Storage::row_major
   * for Table<2, T>.
   *
   */
  template <typename TableType, bool Constness, Storage storage_order>
  ...
  protected:
    ...
    /**
     * Check that   <code>linear_index</code>   corresponds to an entry that
     * is     actually stored by the table (i.e., assert that
  ...
  public:
    ...
    /**
     * Move assignment operator. This assigns a new value to the table entry
     * at     the current row and column coordinates.
     *
     */
    const Accessor<TableType, false, storage_order> &
    ...
    /**
     * Get a reference to the value of the element represented by     this
     * accessor.
     *
     */
    value_type &
   ...
   * TransposeTable.       @tparam   TableType the type of the %Table, e.g.,
   * Table<2, T> or   TransposeTable.       @tparam   Constness whether or not
   * this is a constant iterator.       @tparam   storage_order The storage
   * scheme of the underlying table, e.g.,     Storage::row_major   for
   * Table<2, T>.
   *
   */
  template <typename TableType, bool Constness, Storage storage_order>
 ...
 * necessarily only numbers). The majority of the interface of this class is
 * implemented in the TableBase base class. See there for an outline of the
 * rationale for and interface of this class. This class also serves as the
 * base class for the FullMatrix class and consequently has a number of
 * functions that are specific to matrices and their needs.
 *
 *
 * @ingroup data
 *
...
public:
   ...
   *                         true);
   * @endcode
   * @param   size1 The size of this table in the first dimension.     @param
   * size2 The size of this table in the second dimension.     @param
   * entries An iterator to a set of elements from which to initialize   this
   * table. It is assumed that iterator can be incremented and   dereferenced
   * a sufficient number of times to fill this table.     @param
   * C_style_indexing If true, run over elements of the table with the   last
   ...
   * this two-dimensional table. Range checks are performed.     This version
   * of the function only allows read access.
   *
   */
   ...
   * Access operator. Generate an object that accesses the requested row of
   * this two-dimensional table. Range checks are performed.     This version
   * of the function allows read-write access.
   *
   */
   ...
   * the same time. Range checks are performed.     This version of the
   * function only allows read access.
   *
   */
  typename AlignedVector<T>::const_reference
   ...
   * Direct access to one element of the table by specifying all indices at
   * the same time. Range checks are performed.     This version of the
   * function allows read-write access.
   *
   */
  typename AlignedVector<T>::reference
...
protected:
  /**
   * Return a read-write reference to the element <tt>(i,j)</tt>.     This
   * function does no bounds checking and is only to be used internally   and
   * in functions already checked.     These functions are mainly here for
   * compatibility with a former   implementation of these table classes for
   * 2d arrays, then called   <tt>vector2d</tt>.
   *
   */
  typename AlignedVector<T>::reference
   ...
   * constant reference rather than by   value since this object may hold data
   * types that may be large, and we   don't know here whether copying is
   * expensive or not.     These functions are mainly here for compatibility
   * with a former   implementation of these table classes for 2d arrays, then
   * called   <tt>vector2d</tt>.
   *
   */
  typename AlignedVector<T>::const_reference
 ...
 * only numbers). The majority of the interface of this class is implemented
 * in the TableBase base class.See there for an outline of the rationale for
 * and interface of this class.
 *
 *
 * @ingroup data
 *
...
public:
   ...
   * whereas calling the current constructor allows sizing and initializing an
   * object right away so that it can be marked const.     Using this
   * constructor, you can do things like this (shown here for a
   ...
   * @endcode
   * @param   size1 The size of this table in the first dimension.     @param
   * size2 The size of this table in the second dimension.     @param   size3
   * The size of this table in the third dimension.     @param   entries An
   * iterator to a set of elements from which to initialize   this table. It
   * is assumed that iterator can be incremented and   dereferenced a
   * sufficient number of times to fill this table.     @param
   * C_style_indexing If true, run over elements of the table with the   last
   * index changing fastest as we dereference subsequent elements of the
   * input range. If false, change the first index fastest.
   *
   ...
   * performed.     This version of the function only allows read access.
   *
   */
   ...
   * Access operator. Generate an object that accesses the requested two-
   * dimensional subobject of this three-dimensional table. Range checks are
   * performed.     This version of the function allows read-write access.
   *
   */
 ...
 * format. This class copies the functions of Table<2,T>, but the element
 * access and the dimensions will be for the transpose ordering of the data
 * field in TableBase.
 *
 *
 * @ingroup data
