...
namespace Utilities
{
  namespace MPI
  {
    ...
    namespace ConsensusAlgorithms
    {
       ...
       * classes.             There are two kinds of messages:
       *
       *
       *
       *
       ...
       *
       *
       *  - recv message: The answer to a send/request message.               @tparam   T1 the type of the elements of the vector to sent         @tparam   T2 the type of the elements of the vector to received
       ...
       * 1) deliver only references to empty vectors (of size 0) the data to
       * be       sent can be inserted to or read from, and 2) communicate
      ...
      public:
        ...
        /**
         * @return   A vector of ranks this process wants to send a request
         * to.
         * @note   This is the only method which has to be implemented since
         * the               payloads of the messages are optional.
         *
         */
        ...
        /**
         * Add to the request to the process with the specified rank a
         * payload.                   @param[in]    other_rank rank of the
         * process           @param[out]   send_buffer data to be sent part of
         * the request         (optional)
         * @note   The buffer is empty. Before using it, you have to set its
         * size.
         *
         */
        virtual void
         ...
         * to         the process with the specified rank is saved in. The
         * most obvious         task is to resize the buffer, since it is
         * empty when the function is         called.
         * @param[in]    other_rank rank of the process           @param[out]
         * recv_buffer data to be sent part of the request         (optional)
         *
         */
        virtual void
        ...
        /**
         * Prepare the buffer where the payload of the answer of the request
         * to         the process with the specified rank is saved in.
         * @param[in]    other_rank rank of the process           @param[in]
         * buffer_recv received payload (optional)           @param[out]
         * request_buffer payload to be sent as part of the request
         * (optional)
         * @note   The request_buffer is empty. Before using it, you have to
         * set               its size.
         *
         */
        virtual void
        ...
        /**
         * Process the payload of the answer of the request to the process
         * with         the specified rank.                   @param[in]
         * other_rank rank of the process           @param[in]   recv_buffer
         * data to be sent part of the request (optional)
         *
         */
        virtual void
       ...
       * problem</a>.             Dynamic-sparse means in this context:
       *
       *
       *
       *
       ...
       * ConsensusAlgorithms::Interface   base class, using only
       * point-to-point       communications and a single IBarrier.
       * @note   This class closely follows   @cite hoefler2010scalable  .
       * Since the             algorithm shown there is not considering
       * payloads, the algorithm             has been modified here in such a
       * way that synchronous sends             (Issend) have been replaced by
       * equivalent Isend/Irecv, where             Irecv receives the answer
       * to a request (with payload).               @tparam   T1 The type of
       * the elements of the vector to be sent.         @tparam   T2 The type
       * of the elements of the vector to be received.
       *
       */
      template <typename T1, typename T2>
      ...
      public:
        /**
         * Constructor.                   @param   process Process to be run
         * during consensus algorithm.           @param   comm MPI
         * Communicator
         *
         */
      ...
      private:
        ...
        /**
         * Start to send all requests via ISend and post IRecvs for the
         * incoming         answer messages.
         *
         */
        void
        start_communication();
        ...
        /**
         * After all rank has received all answers, the MPI data structures
         * can         be freed and the received answers can be processed.
         *
         */
        void
      ...
      /**
       * This class implements a concrete algorithm for the
       * ConsensusAlgorithms::Interface   base class, using a two step
       * approach.       In the first step the source ranks are determined and
       * in the second       step a static sparse data exchange is performed.
       * @note   In contrast to NBX, this class splits the same         task
       * into two distinct steps. In the first step, all processes         are
       * identified who want to send a request to this process. In the
       * second step, the data is exchanged. However, since
       *
       *  - in the         second step
       *
      ...
      private:
#ifdef DEAL_II_WITH_MPI
        /**
         * List of ranks of processes this processes wants to send a request
         * to.
         *
         */
        std::vector<unsigned int> targets;
        ...
        /**
         * List of ranks of processes wanting to send a request to this
         * process.
         *
         */
        ...
        /**
         * The ith request message from another rank has been received:
        ...
        /**
         * After all answers have been exchanged, the MPI data structures can
       ...
       * ConsensusAlgorithms::Interface   implementations depending on the
       * number       of processes in the MPI communicator. For a small number
       * of processes       it uses PEX and for a large number of processes
       * NBX. The threshold       depends if the program is compiled in debug
       * or release mode.               @tparam   T1 The type of the elements
       * of the vector to be sent.         @tparam   T2 The type of the
       * elements of the vector to be received.
       *
       */
      template <typename T1, typename T2>
      ...
      public:
        ...
        /**
         * @copydoc     Interface::run()
         * @note   The function call is delegated to another
         * ConsensusAlgorithms::Interface   implementation.
         *
         */
        virtual std::vector<unsigned int>
        run() override;
       ...
       * using user-provided function wrappers.       The advantage of this
       * class is that users do not have to write their       own
       * implementation but can register lambda functions directly.
       *
       */
      template <typename T1, typename T2>
      ...
      public:
         ...
         * `prepare_buffer_for_answer`.           @param
