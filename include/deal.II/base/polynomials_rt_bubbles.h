 ...
 * enhanced Raviart-Thomas polynomials. Similarly to the classical
 * Raviart-Thomas space, the enhanced Raviart-Thomas polynomials are
 * constructed such that the divergence is in the tensor product polynomial
 * space <i>Q<sub>k-1</sub></i>. This space is of the form <i>V<sub>k</sub> =
 * RT<sub>k-1</sub> + B<sub>k</sub></i>, where <i>B<sub>k</sub></i> is defined
 * as follows:   <dl>   <dt> In 2D:
 *
 * @f{align*}{
 ...
 * \right\},
 * @f}
 * </dl>   where   $0 \le a_1, a_2, a_3 \le k$  .
 *
 *
 * @note   Unlike the classical Raviart-Thomas space, the lowest order for the
 * enhanced space is 1, similarly to the Brezzi-Douglas-Marini (BDM)
 * polynomial space. The total dimension of the space <i>dim(V<sub>k</sub>) =
 * d*(k+1)^d</i>, where <i>d</i> is the space dimension. This allows to
 * associate shape functions with the Gauss-Lobatto quadrature points as shown
 * in the figures below. <table> <tr> <td align="center">
...
public:
   ...
   * RT_bubbles   polynomial at   @p unit_point.       The size of the vectors
   * must either be zero or equal <tt>n()</tt>.  In   the first case, the
   * function will not compute these values.     If you need values or
   * derivatives of all tensor product polynomials then   use this function,
   * rather than using any of the <tt>compute_value</tt>,
...
private:
  ...
  /**
   * Storage for monomials, we need all polynomials from degree zero   to
   * <i>k+1</i>.
   *
   */
