...
namespace internal
{
   ...
   * of type int, unsigned int,   std::uint64_t,   double or   std::string.
   * In   essence, this structure is the same as
   * <code>boost::variant@<int,unsigned
   * int,std::uint64_t,double,std::string@></code>   but we wrap this object
   * in a   structure for which we can write a function that can serialize it.
   * This is   also why the function is not in fact of type   boost::any.
   *
   */
  struct TableEntry
  {
  public:
     ...
     * of   <code>int,unsigned int,std::uint64_t,double,std::string</code>
     * and it     must match the data type of the object originally stored in
     ...
     * either as an integer, an unsigned   integer,std::uint64_t,   or a
 ...
 * may vary from column to column and from row to row. <h3>Usage</h3> The most
 * important function is the templatized function   <code>add_value(const
 * std::string &key, const T value)</code>   that adds a column with the name
 * <tt>key</tt> to the table if this column does not yet exist and adds the
 * given value of type <tt>T</tt> (which must be one of <tt>int</tt>,
 * <tt>unsigned int</tt>, <tt>double</tt>,   <tt>std::string</tt>)   to this
 * column.  After the table is complete there are different possibilities of
 * output, e.g., into a latex file with write_tex() or as text with
 * write_text(). Two (or more) columns may be merged into a "supercolumn" by
 * twice (or multiple) calling add_column_to_supercolumn(), see there.
 * Additionally there is a function to set for each column the precision of
 * the output of numbers, and there are several functions to prescribe the
 * format and the captions the columns are written with in tex mode. A
 * detailed explanation of this class is also given in the   step-13
 * tutorial program.
 *
 *  <h3>Example</h3> This is a simple example demonstrating the usage of this
 * class. The first column includes the numbers   $i=1 \dots n$  , the second
 * $1^2 \dots n^2$  , the third   $\sqrt{1}\dots\sqrt{n}$  , where the second
 * and third columns are merged into one supercolumn with the superkey
 * <tt>squares and roots</tt>. Additionally the first column is aligned to the
 * right (the default was <tt>centered</tt>) and the precision of the square
 * roots are set to be 6 (instead of 4 as default).
 *
 *
 * @code
 * TableHandler table;
 ...
 *
 *
 *  <h3>Dealing with sparse data: auto-fill mode</h3> When generating output,
 * TableHandler expects that all columns have the exact same number of
 * elements in it so that the result is in fact a table. This assumes that in
 * each of the iterations (time steps, nonlinear iterations, etc) you fill
 * every single column. On the other hand, this may not always be what you
 * want to do. For example, it could be that the function that computes the
 * nonlinear residual is only called every few time steps; or, a function
 * computing statistics of the mesh is only called whenever the mesh is in
 * fact refined. In these cases, the add_value() function will be called less
 * often for some columns and the column would therefore have fewer elements;
 * furthermore, these elements would not be aligned with the rows that contain
 * the other data elements that were produced during this iteration. An
 * entirely different scenario is that the table is filled and at a later time
 * we use the data in there to compute the elements of other rows; the
 * ConvergenceTable class does something like this. To support both scenarios,
 * the TableHandler class has a property called <i> auto-fill mode</i>. By
 * default, auto-fill mode is off, but it can be enabled by calling
 * set_auto_fill_mode(). If auto-fill mode is enabled we use the following
 * algorithm:
 *
 *
 *
 *  - When calling   <code>add_value(key, value)</code>  , we count the number of elements in the column corresponding to   <code>key</code>  . Let's call this number   $m$  .
 *
 *
 *
 ...
 * then the padding will enter default values into this column. The algorithm
 * as described will fail if you try to skip adding values for a key if adding
 * an element for this key is the first thing you want to do for a given
 * iteration or time step, since we would then pad to the length of the
 * longest column of the <i>previous</i> iteration or time step. You may have
 * to re-order adding to this column to a different spot in your program,
 ...
 * for example in column 1. In the case above, we have always padded columns
 * <b>above</b> the element that is being added to a column. However, there is
 * also a case where we have to pad <b>below</b>. Namely, if a previous row
 * has been completely filled using   TableHandler::add_value(),   subsequent
 * rows have been filled partially, and we then ask for output via
 * write_text() or write_tex(). In that case, the last few rows that have been
 * filled only partially need to be padded below the last element that has
 * been added to them. As before, we do that by using default constructed
 * objects of the same type as the last element of that column.
 *
 *
 * @ingroup textoutput
 *
...
public:
   ...
   *
   *
   *  -   <code>table_with_headers</code>  : The table is formatted in such a way   that the contents are aligned under the key of each column, i.e. the key   sits atop each column. This is suitable for tables with few columns where   the entire table can be displayed on the screen. Output looks like this:
   * @code
   *   key1 key2 key3
   *   0    0    ""
   *   1    0    ""
   ...
   *
   *
   *  -   <code>simple_table_with_separate_column_description</code>  : This format   is very similar to   <code>table_with_separate_column_description</code>  ,   but it skips aligning the columns with additional white space. This   increases the performance of write_text() for large tables. Example   output:
   * @code
   *   # 1: key1
   *   # 2: key2
   *   # 3: key3
   ...
   *
   *
   *  -   <code>org_mode_table</code>  : Outputs to org-mode (http://orgmode.org/)   table format. It is easy to convert org-mode tables to HTML/LaTeX/csv.   Example output:
   * @code
   * | key1 | key2 | key3 |
   * | 0    | 0    | ""   |
   * | 1    | 0    | ""   |
  ...
  /**
   * Declare the existence of a column in the table by giving it a name.   As
   * discussed in the documentation of the class, this is not usually
   * necessary
   *
  ...
  /**
   * If a row is only partially filled, then set all elements of that   row
   * for which no elements exist in a particular column to the   empty string.
   * This is akin to the 'auto_fill_mode' described in   the introduction, but
   * more general because it allows you to start   writing into a column for a
   * new row without having to know that   that column had been written to in
   * the previous row.     If all columns have been written into in the
   * current row, then   this function doesn't do anything at all. In other
   * words,   conceptually the function "completes" the current row, though
   * its   use case is to "start" a new row.
   *
   */
  void
  start_new_row();
   ...
   * <tt>add_column_to_supercolumn(c2,sc)</tt>.     Concerning the order of
   * the columns, the supercolumn replaces the first   column that is added to
   * the supercolumn. Within the supercolumn the order   of output follows the
   * order the columns are added to the supercolumn.
   *
   */
  void
   ...
   * to the supercolumn.     This function may also be used to break big
   * tables with too many columns   into smaller ones. For example, you can
   * call this function with the first   five columns and then call one of the
   * <tt>write_*</tt> functions, then   call this function with the next five
   * columns and again <tt>write_*</tt>,   and so on.
   *
   */
  void
  ...
  /**
   * Set the caption the supercolumn <tt>superkey</tt> for tex output.   You
   * may want to chose this different from <tt>superkey</tt>, if it   contains
   * formulas or similar constructs.
   *
   */
  void
   ...
   * then the entry of the table is printed as   <code>""</code>  .     The
   * second argument indicates how column keys are to be displayed. See   the
   * description of TextOutputFormat for more information.
   *
   */
  void
...
protected:
  ...
  struct Column
  {
     ...
     * <tt>TableHandler::set_tex_format(...)</tt>   with <tt>"c", "r",
     * "l"</tt>     for centered, right or left.
     *
     */
   ...
   * replaced by its subcolumn keys.     This function implicitly checks the
   * consistency of the data. The result   is returned in
   * <tt>sel_columns</tt>.
   *
   */
  void
  ...
  /**
   * A variable storing the column and supercolumn keys in the order desired
   * by   the user. By default this is the order of adding the columns. This
   * order   may be changed by set_column_order().
   *
   */
  std::vector<std::string> column_order;

  /**
   * A map from the column keys to the columns (not supercolumns).     The
   * field is declared mutable so that the write_text() and write_tex()
  ...
  /**
   * A map from each supercolumn key to the keys of its subcolumns in the
   * right   order.  It is allowed that a supercolumn has got the same key as
   * a   column.     Note that we do not use a <tt>multimap</tt> here since
   * the order of   column keys for each supercolumn key is relevant.
   *
   */
   ...
   * are used in tex output.     By default these are just the supercolumn
   * keys but they may be changed by   <tt>set_tex_supercaptions(...)</tt>.
   *
   */
