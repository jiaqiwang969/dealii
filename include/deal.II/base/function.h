 ...
 * components. The class serves the purpose of representing both scalar and
 * vector valued functions. To this end, we consider scalar functions as a
 * special case of vector valued functions, in the former case only having a
 * single component return vector. Since handling vectors is comparatively
 * expensive, the interface of this class has functions which only ask for a
 * single component of the vector-valued results (this is what you will
 * usually need in case you know that your function is scalar-valued) as well
 * as functions you can ask for an entire vector of results with as many
 * components as the function object represents. Access to function objects
 * therefore is through the following methods:
 *
 * @code
 ...
 * even higher derivatives at one or several points. You will usually only
 * overload those functions you need; the functions returning several values
 * at a time (value_list(), vector_value_list(), and gradient analogs) will
 * call those returning only one value (value(), vector_value(), and gradient
 * analogs), while those ones will throw an exception when called but not
 * overloaded. Conversely, the functions returning all components of the
 * function at one or several points (i.e. vector_value(),
 * vector_value_list()), will   <em>  not  </em>   call the function returning
 * one component at one point repeatedly, once for each point and component.
 * The reason is efficiency: this would amount to too many virtual function
 * calls. If you have vector- valued functions, you should therefore also
 * provide overloads of the virtual functions for all components at a time.
 ...
 * often less than the virtual function call itself. Support for time
 * dependent functions can be found in the base class FunctionTime.
 *
 *  <h3>Functions that return tensors</h3> If the functions you are dealing
 * with have a number of components that are a priori known (for example,
 * <tt>dim</tt> elements), you might consider using the TensorFunction class
 * instead. This is, in particular, true if the objects you return have the
 * properties of a tensor, i.e., they are for example dim-dimensional vectors
 * or dim-by-dim matrices. On the other hand, functions like
 ...
 *
 *  <h3>Functions that return vectors of other data types</h3> Most of the
 * time, your functions will have the form   $f : \Omega \rightarrow {\mathbb
 * R}^{n_\text{components}}$  . However, there are occasions where you want
 * the function to return vectors (or scalars) over a different number field,
 * for example functions that return complex numbers or vectors of complex
 * numbers:   $f : \Omega \rightarrow {\mathbb C}^{n_\text{components}}$  . In
 * such cases, you can choose a value different than the default   @p double
 * for the second template argument of this class: it describes the scalar
 * type to be used for each component of your return values. It defaults to
 * @p double,   but in the example above, it could be set to
 * <code>std::complex@<double@></code>  .   step-58   is an example of this.
 * @tparam   dim The space dimension of the range space within which the
 * domain     $\Omega$   of the function lies. Consequently, the function will
 * be   evaluated at objects of type   @p Point<dim>.     @tparam
 * RangeNumberType The scalar type of the vector space that is the range   (or
 * image) of this function. As discussed above, objects of the current type
 * represent functions from   ${\mathbb R}^\text{dim}$   to
 * $S^{n_\text{components}}$   where   $S$   is the underlying scalar type of
 * the vector space. The type of   $S$   is given by the   @p RangeNumberType
 * template   argument.
 *
 *
 * @ingroup functions
...
public:
  ...
  /**
   * Virtual destructor; absolutely necessary in this case.     This
   * destructor is declared pure virtual, such that objects of this class
   ...
   * because none of this class's functions are abstract.     We circumvent
   * this problem by making the destructor of this class   abstract virtual.
   * This ensures that at least one member function is   abstract, and
   * consequently, no objects of type Function can be created.   However,
   * there is no need for derived classes to explicitly implement a
   ...
   * declared destructor.     Nonetheless, since derived classes want to call
   * the destructor of a base   class, this destructor is implemented (despite
   * it being pure virtual).
   *
   */
   ...
   * array.     By default, this function repeatedly calls value() for each
   ...
   * function has.     By default, this function repeatedly calls
   * vector_value() for each point   separately, to fill the output array.
   *
   */
  virtual void
   ...
   * point.     The default implementation of this function in Function calls
   ...
   * for each point.     The default implementation of this function in
   * Function calls   value_list() for each component. In order to improve
   * performance, this   can be reimplemented in derived classes to speed up
   * performance.
   *
   */
  virtual void
   ...
   * <tt>points</tt> array.     The outer loop over <tt>gradients</tt> is over
   * the points in the list,   the inner loop over the different components of
   * the function.
   *
   */
  virtual void
...
namespace Functions
{
  ...
  public:
    ...
    /**
     * Constructor; takes an   <tt>std::vector<RangeNumberType></tt>   object
     * as an     argument. The number of components is determined by
     * <tt>values.size()</tt>.
     *
     */
    ...
    /**
     * Constructor; uses whatever stores in [begin_ptr,
     * begin_ptr+n_components)     to initialize a new object.
     *
     */
   ...
   * components.     This function is of use when you want to implement
   * homogeneous boundary   conditions, or zero initial conditions.
   * @ingroup functions
   *
   */
  template <int dim, typename RangeNumberType = double>
  ...
  /**
   * A function whose output is also its input. One possible application of
   * this   function is interpolating or projecting a finite element field
   * that   represents spatial coordinates: e.g., one can set up a finite
   * element field   to interpolate the positions of a Triangulation's cells
   * with this function   (via   VectorTools::interpolate()),   which is
...
public:
   ...
   * denotes a half-open interval of components (for example
   ...
   * <tt>ComponentSelectFunction  @<dim,   RangeNumberType  @></tt>   class
   * can only   have same value for all components.
   * @note   We copy the underlying component value data from   @p f   from
   * its   beginning. So the number of components of   @p f   cannot be less
   * than the   calling object.
   *
   */
  virtual void
 ...
 * sure that   <code>component</code>   is in fact zero, as needs be for
 * scalar functions. The class provides an easy way to turn a simple global
 * function into something that has the required Function  @<dim@>   interface
 * for operations like   VectorTools::interpolate_boundary_values()   etc.,
 * and thereby allows for simpler experimenting without having to write all
 * the boiler plate code of declaring a class that is derived from Function
 * and implementing the   Function::value()   function. An example of this is
 * given in the results section of   step-53  . The class gains additional
 * expressive power because the argument it takes does not have to be a
 * pointer to an actual function. Rather, it is a function object, i.e., it
 * can also be the result of a lambda function or some other object that can
 * be called with a single argument. For example, if you need a Function
 * object that returns the norm of a point, you could write it like so:
 *
 * @code
 ...
 * @endcode
 * and then pass the   <code>my_norm_object</code>   around, or you could
 * write it like so:
 *
 * @code
 ...
 * object" (i.e., an object of a class that is derived from the Function base
 * class). To be more concrete, let us consider the following example:
 *
 * @code
 * RangeNumberType
 * one(const Point<2> &p)
 ...
 * function that could, for example, be passed to
 * VectorTools::integrate_difference().   This effect can also be achieved
 * using the ComponentSelectFunction class but is obviously easily extended to
 * functions that are non-constant in their one component.
 *
 *
 * @ingroup functions
 *
...
public:
   ...
   * Given a function object that takes a Point and returns a RangeNumberType
   * value, convert this into an object that matches the Function  @<dim@>
   * interface.       @param   function_object The scalar function that will
   * form one component   of the resulting Function object.     @param
   * n_components The total number of vector components of the   resulting
   * Function object.     @param   selected_component The single component
   * that should be filled by   the first argument.
   *
   */
 ...
 * interface of the Function base class. The difference is that here the
 * Function object generated may be vector valued, and you can specify the
 * gradients of the function. The number of vector components is deduced from
 * the size of the vector in the constructor. To be more concrete, let us
...
public:
  /**
   * Default constructor.     This constructor does not initialize the
   * internal methods. To have a   usable function, you need to call at least
   * the set_function_values()   method. If you need also the gradients of the
   * solution, then you must   also call the set_function_gradients() method.
   *
   */
  ...
  /**
   * Constructor for functions of which you only know the values.     The
   * resulting function will have a number of components equal to the size
   * of the vector   @p values.   A call to the
   * FunctionFromFunctionObject::gradient()     method will trigger an
   * exception, unless you first call the   set_function_gradients() method.
   *
   */
   ...
   * gradients.     The resulting function will have a number of components
   * equal to the size   of the vector   @p values.   If the size of   @p
   * values   and   @p gradients   does not   match, an exception is
   * triggered.
   *
   */
   ...
   * size of the   @p values   parameter does not match the number of
  ...
  /**
   * Reset the function gradients of this object. An assertion is thrown if
   * the   size of the   @p gradients   parameter does not match the number of
   * components of   this object.
   *
   */
  void
 ...
 * Vector for use in, for example, the   VectorTools::interpolate   or the
 * many other functions taking Function objects. It allows the user to place
 * the desired components into an <tt>n_components</tt> long vector starting
 * at the <tt>selected_component</tt> location in that vector and have all
 * other components be 0. For example: Say you created a class called
 *
 * @code
...
public:
   ...
   * <tt>Tensor<1,dim, RangeNumberType></tt> value, convert this into an
   * object   that matches the Function  @<dim@>   interface.     By default,
   * create a Vector object of the same size as   <tt>tensor_function</tt>
   * returns, i.e., with <tt>dim</tt> components.       @param
   * tensor_function The TensorFunction that will form one component of   the
   * resulting Vector Function object.     @param   n_components The total
   * number of vector components of the   resulting TensorFunction object.
