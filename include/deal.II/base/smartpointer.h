...
/**
 * Smart pointers avoid using dangling pointers. They can be used just like a
 * pointer (i.e. using the <tt>*</tt> and <tt>-></tt> operators and through
 * casting) but make sure that the object pointed to is not deleted or moved
 * from in the course of use of the pointer by signaling the pointee its use.
 ...
 * rule and are allowed, too. The second template argument P only serves a
 * single purpose: if a constructor without a debug string is used, then the
 * name of P is used as the debug string. SmartPointer does NOT implement any
 * memory handling! Especially, deleting a SmartPointer does not delete the
 * object. Writing
 *
 * @code
...
public:
   ...
   * The <tt>id</tt> is used in the call to   Subscriptor::subscribe(id)   and
   * by   ~SmartPointer() in the call to   Subscriptor::unsubscribe().
   *
   */
  ...
  /**
   * Dereferencing operator. This operator throws an ExcNotInitialized() if
  ...
  /**
   * Dereferencing operator. This operator throws an ExcNotInitializedi() if
   * the   pointer is a null pointer.
   *
   */
  T *operator->() const;
   ...
   * have to change their subscription counters.     Note that this function
   * (with two arguments) and the respective functions   where one of the
   * arguments is a pointer and the other one is a C-style   pointer are
   * implemented in global namespace.
   *
   */
  template <class Q>
  void
 ...
 * pointer. Note that we indeed need a reference of a pointer, as we want to
