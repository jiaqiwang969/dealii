 ...
 * of: vertices, lines, quads and hexes. The class is rather primitive: it
 * only stores a single integer that represents the dimensionality of the
 * object represented. In other words, this class is useful primarily as a way
 * to pass around an object whose data type explains what it does (unlike just
 * passing around an integer), and for providing symbolic names for these
 * objects such as   GeometryPrimitive::vertex   instead of an integer zero.
 ...
 * refinement flags in the current space dimension. This general template is
 * unused except in some weird template constructs. Actual use is made,
 * however, of the specializations   <code>RefinementPossibilities@<1@></code>
...
{
  /**
   * Possible values for refinement cases in the current dimension.     Note
   * the construction of the values: the lowest bit describes a cut of   the
   * x-axis, the second to lowest bit corresponds to a cut of the y-axis   and
   * the third to lowest bit corresponds to a cut of the z-axis. Thus, the
   ...
   * offered, of course.     In addition, the tag
   * <code>isotropic_refinement</code>   denotes isotropic   refinement in the
   * space dimension selected by the template argument of   this class.     If
   * you choose anisotropic refinement, for example by passing as argument
   * to   CellIterator::set_refine_flag()     one of the flags
  ...
  enum Possibilities
  {
    ...
    /**
     * Perform isotropic refinement. This implies     refining in all
     * coordinate directions. For the     current general template class
     *
...
/**
 * A class that provides possible choices for isotropic and anisotropic
 * refinement flags in the current space dimension. This specialization is
 * used for   <code>dim=1</code>  , where it offers refinement in x-direction.
 *
 *
 * @ingroup aniso
 *
 ...
 * A class that provides possible choices for isotropic and anisotropic
 * refinement flags in the current space dimension. This specialization is
 * used for   <code>dim=2</code>  , where it offers refinement in x- and
 * y-direction separately, as well as isotropic refinement in both directions
 * at the same time.
 *
 *
 * @ingroup aniso
 ...
 * A class that provides possible choices for isotropic and anisotropic
 * refinement flags in the current space dimension. This specialization is
 * used for   <code>dim=3</code>  , where it offers refinement in x-, y- and
 * z-direction separately, as well as combinations of these and isotropic
 * refinement in all directions at the same time.
 *
 *
 * @ingroup aniso
 *
...
public:
   ...
   * if the current value is   <code>isotropic_refinement</code>   then the
   * result   will be   <code>no_refinement</code>  ; etc.
   *
   */
  RefinementCase
  operator~() const;
   ...
   * as argument. For example, if   <code>i=0</code>   then the returned value
   * is     <tt>RefinementPossibilities<dim>::cut_x</tt>.
   *
   */
...
namespace internal
{
   ...
   * <code>RefinementPossibilities@<2@></code>  , but additionally some
   * subface   possibilities a face might be subdivided into which occur
   * through   repeated anisotropic refinement steps performed on one of two
   * neighboring   cells.     This general template is unused except in some
   * weird template constructs.   Actual is made, however, of the
   * specializations     <code>SubfacePossibilities@<1@></code>  ,
   ...
   * A class that provides all possible situations a face (in the current
   * space dimension   @p dim)   might be subdivided into subfaces.     For
   * <code>dim=0</code>   we provide a dummy implementation only.
   * @ingroup aniso
   *
   */
  template <>
  ...
  {
    /**
     * Possible cases of faces being subdivided into subface.         Dummy
     * implementation.
     *
     */
    enum Possibilities
   ...
   * A class that provides all possible situations a face (in the current
   * space dimension   @p dim)   might be subdivided into subfaces.     For
   * <code>dim=1</code>   there are no faces. Thereby, there are no subface
   * possibilities.
   * @ingroup aniso
   *
   */
  ...
  {
    /**
     * Possible cases of faces being subdivided into subface.         In 1d
     * there are no faces, thus no subface possibilities.
     *
     */
    enum Possibilities
    {
  ...
  /**
   * A class that provides all possible situations a face (in the current
   * space dimension   @p dim)   might be subdivided into subfaces.     This
   * specialization is used for   <code>dim=2</code>  , where it offers the
  ...
  {
    /**
     * Possible cases of faces being subdivided into subface.         In 2d
     * there are following possibilities: a face (line) being refined     (
     * <code>case_x</code>) or not refined (<code>case_no</code>  ).
     *
     */
    enum Possibilities
   ...
   * A class that provides all possible situations a face (in the current
   * space dimension   @p dim)   might be subdivided into subfaces.     This
   * specialization is used for dim=3, where it offers following
   ...
   * (  <code>case_y</code>), and in both directions (<code>case_x</code>
   * which   corresponds to (  <code>case_isotropic</code>  ). Additionally,
   * it offers the   possibilities a face can have through repeated
   * anisotropic refinement   steps performed on one of the two neighboring
   * cells.  It might be   possible for example, that a face (quad) is refined
   * with     <code>cut_x</code>   and afterwards the left child is again
   * refined with     <code>cut_y</code>  , so that there are three active
   * subfaces. Note,   however, that only refinement cases are allowed such
   * that each line on a   face between two hexes has not more than one
   * hanging node. Furthermore,   it is not allowed that two neighboring hexes
   * are refined such that one of   the hexes refines the common face with
   * <code>cut_x</code>   and the other   hex refines that face with
   * <code>cut_y</code>  . In fact,
   ...
   * contained in a single face of neighboring cells.     The following
   * drawings explain the SubfacePossibilities and give the   corresponding
   * subface numbers:
   * @code
   *
   *
  ...
  {
    /**
     * Possible cases of faces being subdivided into subface.         See
     * documentation to the SubfacePossibilities<3> for more details on
  ...
  public:
     ...
     * SubfaceCase::case_x:   ... </code>, which can be written as
     * <code>switch       (static_cast@<std::uint8_t@>(subface_case)</code>.
     * Another application     is to use an object of the current type as an
     * index into an array;     however, this use is deprecated as it assumes
     * a certain mapping from     the symbolic flags defined in the
     * SubfacePossibilities base class to     actual numerical values (the
     * array indices).
     *
     */
    operator std::uint8_t() const;
 ...
 * superseded by the ReferenceCell class
 *
 ...
 * present, e.g. about faces. This class contains as static members
 * information on vertices and faces of a   @p dim-dimensional   grid cell.
 * The interface is the same for all dimensions. If a value is of no use in a
 * low dimensional cell, it is (correctly) set to zero, e.g.
 * #max_children_per_face in 1d. This information should always replace
 * hard-coded numbers of vertices, neighbors and so on, since it can be used
...
struct GeometryInfo<0>
{
   ...
   * isotropically refined cell.     If a cell is refined anisotropically, the
   * actual number of children may   be less than the value given here.
   *
   */
   ...
   * indices from zero to `faces_per_cell`. This allows to write code   using
   * range-based for loops of the following kind:
   * @code
   ...
   * taking on all valid indices.     Of course, since this class is for the
   * case `dim==0`, the   returned object is actually an empty array.
   *
   */
   ...
   * of an isotropically refined face.     If a cell is refined
   * anisotropically, the actual number of children may   be less than the
   * value given here.
   *
   */
   ...
   * taking on all valid indices.     Of course, since this class is for the
   * case `dim==0`, the   returned object is a array with just one entry:
   * zero. That's   because an of dimension zero is really just a single
   * point,   corresponding to a vertex itself.
   *
   */
  ...
  /**
   * Map face vertex number to cell vertex number, i.e. give the cell vertex   number of the <tt>vertex</tt>th vertex of face <tt>face</tt>, e.g.     <tt>GeometryInfo<2>::face_to_cell_vertices(3,0)=2</tt>,   see the image   under point N4 in the 2d section of this class's documentation.     Through the <tt>face_orientation</tt>, <tt>face_flip</tt> and   <tt>face_rotation</tt> arguments this function handles faces oriented in   the standard and non-standard orientation. <tt>face_orientation</tt>   defaults to <tt>true</tt>, <tt>face_flip</tt> and <tt>face_rotation</tt>   default to <tt>false</tt> (standard orientation). In 2d only   <tt>face_flip</tt> is considered. See this     @ref GlossFaceOrientation   "glossary"
   * article for more information.     As the children of a cell are ordered
   * according to the vertices of the   cell, this call is passed down to the
   * child_cell_on_face() function.   Hence this function is simply a wrapper
   * of child_cell_on_face() giving it   a suggestive name.     Of course,
   * since this class is for the case `dim==0`, this function   is not
   * implemented.
   *
   */
  static unsigned int
   ...
   * <tt>GeometryInfo<3>::face_to_cell_lines(5,0)=4</tt>.       Through the
   * <tt>face_orientation</tt>, <tt>face_flip</tt> and
   ...
   * Of course, since this class is for the case `dim==0`, this function   is
   * not implemented.
   *
   */
  static unsigned int
...
/**
 * This class provides dimension independent information to all topological structures that make up the unit, or   @ref GlossReferenceCell   "reference cell".
 * This class has been superseded by the ReferenceCell class
 *
 *  -  see there for more information.
 *
 ...
 * in 3d as well without changes. The most frequently used parts of the class
 * are its static members like vertices_per_cell, faces_per_cell, etc.
 * However, the class also offers information about more abstract questions
 * like the orientation of faces, etc. The following documentation gives a
 * textual description of many of these concepts.
 *
 *  <h3>Implementation conventions for two spatial dimensions</h3> From
 * version 5.2 onwards deal.II is based on a numbering scheme that uses a
 * lexicographic ordering (with x running fastest) wherever possible, hence
 * trying to adopt a kind of 'canonical' ordering. The ordering of vertices
 * and faces (lines) in 2d is defined by
 *
 *
 *
 ...
 * grid; however, it is automatically preserved upon refinement. Further we
 * define that child lines have the same direction as their parent, i.e. that
 * <tt>line->child(0)->vertex(0)==line->vertex(0)</tt> and
 ...
 * of the old line. Similarly we define, that the four children of a quad are
 * adjacent to the vertex with the same number of the old quad. Note that
 * information about several of these conventions can be extracted at run- or
 * compile-time from the member functions and variables of the present class.
 *
 *  <h4>Coordinate systems</h4> When explicit coordinates are required for
 * points in a cell (e.g for quadrature formulae or the point of definition of
 * trial functions), we define the following coordinate system for the unit
 * cell:
 *
 * @verbatim
 * y^   2-----3
 ...
 * <tt>(1,1)</tt>. The   GeometryInfo<dim>::unit_cell_vertex()   function can
 * be used to query this information at run-time.
 *
 *  <h3>Implementation conventions for three spatial dimensions</h3> By
 * convention, we will use the following numbering conventions for vertices,
 * lines and faces of hexahedra in three space dimensions. Before giving these
 ...
 * recover the whole cube by moving the two parts together into one. Note
 * again that information about several of the following conventions can be
 * extracted at run- or compile-time from the member functions and variables
 * of the present class. <h4>Vertices</h4> The ordering of vertices in 3d is
 * defined by the same rules as in the 2d case. In particular, the following
 * is still true:
 *
 *
 *
 ...
 * face to the top. Again, the   GeometryInfo<dim>::unit_cell_vertex()
 * function can be used to query this information at run-time.
 *
 *  <h4>Lines</h4> Here, the same holds as for the vertices:
 *
 *
 *
 * - Line ordering in 3d:   <ul>     <li>  first the lines of face (z=0) in 2d line ordering,   <li>  then the lines of face (z=1) in 2d line ordering,   <li>  finally the lines in z direction in lexicographic ordering   </ul>
 *
 * @verbatim
 *
 *
 ...
 * cells that need not be already sorted. <h4>Faces</h4> The numbering of
 * faces in 3d is defined by a rule analogous to 2d:
 *
 *
 *
 ...
 * @endverbatim
 *
 * The   <em>  standard  </em>   direction of the faces is such, that the induced 2d local coordinate system (x,y) implies (right hand rule) a normal in face normal direction, see N2a).  In the following we show the local coordinate system and the numbering of face lines:   <ul>     <li>   Faces 0 and 1:
 * @verbatim
 *        Face 0           Face 1
 *
 *
 ...
 * @endverbatim
 * </ul>
 * The face line numbers (0,1,2,3) correspond to following cell line numbers.   <ul>     <li>   Face 0: lines 8, 10, 0, 4;   <li>   Face 1: lines 9, 11, 1, 5;   <li>   Face 2: lines 2, 6, 8, 9;   <li>   Face 3: lines 3, 7, 10, 11;   <li>   Face 4: lines 0, 1, 2, 3;   <li>   Face 5: lines 4, 5, 6, 7;   </ul>   You can get these numbers using the   GeometryInfo<3>::face_to_cell_lines()   function.
 * The face normals can be deduced from the face orientation by applying the
 * right hand side rule (x,y
 *
 ...
 * @endverbatim
 *
 * Taking into account the orientation of the faces, the following children are adjacent to the respective faces:   <ul>     <li>   Face 0: children 0, 2, 4, 6;   <li>   Face 1: children 1, 3, 5, 7;   <li>   Face 2: children 0, 4, 1, 5;   <li>   Face 3: children 2, 6, 3, 7;   <li>   Face 4: children 0, 1, 2, 3;   <li>   Face 5: children 4, 5, 6, 7.   </ul>   You can get these numbers using the   GeometryInfo<3>::child_cell_on_face()   function. As each child is adjacent to the vertex with the same number these numbers are also given by the   GeometryInfo<3>::face_to_cell_vertices()   function.
 ...
 * argument. The information which child cell is at which position of which
 * face is most often used when computing jump terms across faces with hanging
 * nodes, using objects of type FESubfaceValues. Sitting on one cell, you
 * would look at a face and figure out which child of the neighbor is sitting
 * on a given subface between the present and the neighboring cell. To avoid
 * having to query the standard orientation of the faces of the two cells
 * every time in such cases, you should use a function call like
 ...
 * argument to   <tt>GeometryInfo<3>::child_cell_on_face</tt>). For
 * anisotropic refinement, the child cells can not be numbered according to
 ...
 * <tt>GeometryInfo<3>::child_cell_on_face</tt>   function. <h4>Coordinate
 * systems</h4> We define the following coordinate system for the explicit
 * coordinates of the vertices of the unit cell:
 *
 * @verbatim
 *                     6-------7        6-------7
 *                    /|       |       /       /|
 ...
 * @endverbatim
 *
 * By the convention laid down as above, the vertices have the following coordinates (lexicographic, with x running fastest):   <ul>     <li>   Vertex 0: <tt>(0,0,0)</tt>;   <li>   Vertex 1: <tt>(1,0,0)</tt>;   <li>   Vertex 2: <tt>(0,1,0)</tt>;   <li>   Vertex 3: <tt>(1,1,0)</tt>;   <li>   Vertex 4: <tt>(0,0,1)</tt>;   <li>   Vertex 5: <tt>(1,0,1)</tt>;   <li>   Vertex 6: <tt>(0,1,1)</tt>;   <li>   Vertex 7: <tt>(1,1,1)</tt>.   </ul>
 *
 *
 *
 *
 * @note
 * Instantiations for this template are provided for dimensions 1,2,3,4, and
 * there is a specialization for dim=0 (see the section on   @ref
...
struct GeometryInfo
{
   ...
   * of an isotropically refined cell.     If a cell is refined
   * anisotropically, the actual number of children may   be less than the
   * value given here.
   *
   */
   ...
   * @endcode
   * Here, we are looping over all faces of all cells, with `face_index`
   * taking on all valid indices for faces (zero and one in 1d, zero   through
   * three in 2d, and zero through 5 in 3d).       @see   CPP11
   *
   */
   ...
   * @endcode
   * Here, we are looping over all vertices of all cells, with `vertex_index`
   * taking on all valid indices.       @see   CPP11
   *
   */
  static std_cxx20::ranges::iota_view<unsigned int, unsigned int>
  vertex_indices();
  ...
  /**
   * Number of lines of a cell.     The formula to compute this makes use of
   * the fact that when going from   one dimension to the next, the object of
   * the lower dimension is copied   once (thus twice the old number of lines)
   * and then a new line is inserted   between each vertex of the old object
  ...
  /**
   * Number of quadrilaterals of a cell.     This number is computed
   * recursively just as the previous one, with the   exception that new quads
   * result from connecting an original line and its   copy.
   *
   */
   ...
   * faces to which the vertex belongs.     The order of the faces for each
   * vertex is such that the first listed face   bounds the reference cell in
   * <i>x</i> direction, the second in <i>y</i>   direction, and so on.
   *
   */
   ...
   * this   function returns the ratio between the area of the   @p subface_no
   * th   subface and the area(=1) of the face.     E.g. for
   * internal::SubfaceCase::cut_xy   the ratio is 1/4 for each of the
   * subfaces.
   *
   */
  static double
  ...
  /**
   * Given the SubfaceCase   @p face_refinement_case   of the   @p face_no
   * th face,   return the smallest RefinementCase of the cell, which
   * corresponds to that   refinement of the face.
   *
   */
   ...
   * mother cell.     For example, in 2D the layout of a cell is as follows:
   * @verbatim
   * .      3
   * .   2-->--3
   * .   |     |
   ...
   * entry.     In the case of anisotropically refined cells and faces, the
   * @p     RefinementCase of the face, <tt>face_ref_case</tt>, might have an
   ...
   * <tt>GeometryInfo<2>::line_to_cell_vertices(3,0)=2</tt>.       The order
   * of the lines, as well as their direction (which in turn   determines
   * which is the first and which the second vertex on a line) is   the
   * canonical one in deal.II, as described in the general documentation   of
   * this class.     For <tt>dim=2</tt> this call is simply passed down to the
  ...
  /**
   * Map the vertex index   @p vertex   of a face in standard orientation to
   * one   of a face with arbitrary   @p face_orientation,     @p face_flip
   * and   @p     face_rotation. The values of these three flags default to
   * <tt>true</tt>,   <tt>false</tt> and <tt>false</tt>, respectively. this
   * combination   describes a face in standard orientation.     This function
   * is only implemented in 3D.
   *
   */
  static unsigned int
   ...
   * face_orientation,   @p face_flip   and   @p face_rotation   to a face in
   * standard   orientation. The values of these three flags default to
   * <tt>true</tt>,   <tt>false</tt> and <tt>false</tt>, respectively. this
   * combination   describes a face in standard orientation.     This function
   * is only implemented in 3D.
   *
  ...
  /**
   * Map the line index   @p line   of a face in standard orientation to one
   * of a   face with arbitrary   @p face_orientation,     @p face_flip   and
   * @p     face_rotation. The values of these three flags default to
   * <tt>true</tt>,   <tt>false</tt> and <tt>false</tt>, respectively. this
   * combination   describes a face in standard orientation.     This function
   * is only implemented in 3D.
   *
  ...
  /**
   * Map the vertex index   @p vertex   of a line in standard orientation to
   * one of a   face with arbitrary   @p line_orientation.   The value of this
   * flag default to   <tt>true</tt>.
   *
   */
  static unsigned int
  ...
  /**
   * Map the line index   @p line   of a face with arbitrary   @p
   * face_orientation,       @p face_flip   and   @p face_rotation   to a face
   * in standard orientation. The   values of these three flags default to
   * <tt>true</tt>, <tt>false</tt> and   <tt>false</tt>, respectively. this
   * combination describes a face in   standard orientation.     This function
   * is only implemented in 3D.
   *
  ...
  /**
   * Return the position of the   @p ith   vertex on the unit cell. The order
   * of   vertices is the canonical one in deal.II, as described in the
  ...
  /**
   * Given a point   @p p   in unit coordinates, return the number of the
   * child   cell in which it would lie in. If the point lies on the interface
   * of two   children, return any one of their indices. The result is always
   * less than     GeometryInfo<dimension>::max_children_per_cell.       The
   * order of child cells is described the general documentation of this
   * class.
   *
   */
  static unsigned int
   ...
   * only up to numerical roundoff.     The tolerance parameter may be less
   * than zero, indicating that the point   should be safely inside the cell.
   *
   */
  static bool
  ...
  /**
   * Compute the value of the   $i$  -th   $d$  -linear (i.e.
   * (bi-,tri-)linear) shape   function at location   $\xi$  .
   *
   */
  static double
  ...
  /**
   * Compute the gradient of the   $i$  -th   $d$  -linear (i.e.
   * (bi-,tri-)linear)   shape function at location   $\xi$  .
   *
   */
  static Tensor<1, dim>
   ...
   * @p dim,   there are   @p dim   vectors with   @p spacedim   components
   * each, and   the alternating form is a tensor of rank spacedim-dim that
   * corresponds to   the wedge product of the   @p dim   unit vectors, and it
   * corresponds to the   volume and normal vectors of the mapping from
   * reference element to the   element described by the vertices.     For
   * example, if dim==spacedim==2, then the alternating form is a scalar
   * (because spacedim-dim=0) and its value equals   $\mathbf v_1\wedge
   * \mathbf v_2=\mathbf v_1^\perp \cdot\mathbf v_2$  , where   $\mathbf
   * v_1^\perp$   is a   vector that is rotated to the right by 90 degrees
   * from   $\mathbf v_1$  . If   dim==spacedim==3, then the result is again a
   * scalar with value   $\mathbf v_1\wedge \mathbf v_2 \wedge \mathbf v_3 =
   * (\mathbf v_1\times \mathbf v_2)\cdot \mathbf v_3$  , where   $\mathbf
   * v_1, \mathbf v_2, \mathbf v_3$   are   the images of the unit vectors at
   * a vertex of the unit dim-dimensional   cell under transformation to the
   * dim-dimensional cell in spacedim-   dimensional space. In both cases,
   * i.e. for dim==2 or 3, the result   happens to equal the determinant of
   * the Jacobian of the mapping from   reference cell to cell in real space.
   * Note that it is the actual   determinant, not its absolute value as often
   * used in transforming   integrals from one coordinate system to another.
   * In particular, if the   object specified by the vertices is a
   * parallelogram (i.e. a linear   transformation of the reference cell) then
   * the computed values are the   same at all vertices and equal the (signed)
   * area of the cell; similarly,   for parallel-epipeds, it is the volume of
   * the cell.     Likewise, if we have dim==spacedim-1 (e.g. we have a quad
   * in 3d space, or   a line in 2d), then the alternating product denotes the
   * normal vector   (i.e. a rank-1 tensor, since spacedim-dim=1) to the
   * object at each   vertex, where the normal vector's magnitude denotes the
   * area element of   the transformation from the reference object to the
   * object given by the   vertices. In particular, if again the mapping from
   * reference object to   the object under consideration here is linear (not
   * bi- or trilinear),   then the returned vectors are all %parallel,
   * perpendicular to the mapped   object described by the vertices, and have
   * a magnitude equal to the   area/volume of the mapped object. If dim=1,
   * spacedim=2, then the returned   value is   $\mathbf v_1^\perp$  , where
   * $\mathbf v_1$   is the image of the   sole unit vector of a line mapped
   * to the line in 2d given by the   vertices; if dim=2, spacedim=3, then the
   * returned values are   $\mathbf v_1 \wedge \mathbf v_2=\mathbf v_1 \times
   * \mathbf v_2$   where   $\mathbf
   * v_1,\mathbf v_2$   are the two three-dimensional vectors that are   tangential to the quad mapped into three-dimensional space.     This function is used in order to determine how distorted a cell is (see   the entry on     @ref GlossDistorted   "distorted cells"
   * in the glossary).
   *
   */
  ...
  /**
   * Unit normal vector (Point<dim>) of a face of the reference cell.     Note that this is only the   <em>  standard orientation  </em>   of faces. At   least in 3d, actual faces of cells in a triangulation can also have the   opposite orientation, depending on a flag that one can query from the   cell it belongs to. For more information, see the     @ref GlossFaceOrientation   "glossary"
   * entry on face orientation.
   *
   */
