...
* 本教程程序介绍了泊松方程的 "弱加勒金 "有限元方法的实现。从某种意义上说，考虑这种方法的动机与 step-51 中的出发点相同：我们想考虑不连续的形状函数，但需要解决这样一个事实：与通常的连续Galerkin方法相比，所产生的问题有更多的自由度（因为，例如，每个顶点携带的自由度与相邻的单元一样多）。 我们还必须处理这样一个事实：与连续Galerkin方法不同，<i>every</i>一个单元上的自由度与它的每个面的邻居单元上的所有自由度相耦合。因此，从 "传统 "非连续Galerkin方法得到的矩阵既大又相对密集。
...
* 弱加尔金有限元方法（WGFEMs）使用离散弱函数来近似标量未知数，使用离散弱梯度来近似经典梯度。 与连续Galerkin方法相比，弱Galerkin方法满足重要的物理特性，即局部质量守恒和体法通量连续。
* 
...
@f}
* 其中 $\Omega \subset \mathbb{R}^n (n=2,3)$ 为有界域。在流体流经多孔介质的情况下， $p$ 为压力， $\mathbf{K}$ 为渗透性张量， $f$ 为源项， $p_D, u_N$  我们可以引入一个通量， $\mathbf{u} =
* 
...
* 上面的泊松方程有一个解 $p$ ，需要满足问题的弱式表述，@f{equation*}
\mathcal{A}\left(p,q \right) = \mathcal{F} \left(q \right),
@f} 。
*对于所有的测试函数 $q$ ，其中@f{equation*}
...
* 弱Galerkin方法的理念是用一个<i>discontinuous function</i> $p_h$ 来近似精确的 $p$ 解。这个函数可能只在单元格之间的界面上是连续的，由于我们也想沿着界面评价这个函数，我们不仅要规定它在单元格内部应该有什么值，而且还要规定它在界面上的值。我们的做法是， $p_h$ 实际上是一个元组， $p_h=(p^\circ,p^\partial)$ ，尽管它实际上只是一个单一的函数，它要么等于 $p^\circ(x)$ ，要么等于 $p^\partial(x)$ ，取决于它是在细胞内部还是在细胞界面上的某一点 $x$ 被评价。
...
* 问题是该算子如何工作。为此，让我们首先说说我们是如何看待压力的离散近似 $p_h$ 的。如上所述，"函数" $p_h$ 实际上由两部分组成：单元内部的值 $p_h^\circ$ 和界面上的 $p_h^\partial$ 。我们必须为这两部分定义离散的（有限维）函数空间；在这个程序中，我们将用FE_DGQ来表示 $p_h^\circ$ 作为细胞内部的空间（在每个细胞上定义，但一般是沿界面不连续的），而用FE_FaceQ表示 $p_h^\partial$ 作为界面上的空间。
...
* 由于 $p_h$ 是有限元空间的一个元素，我们可以像往常一样用基来展开它，也就是说，我们可以写成@f{equation*}{
...
* 在解释了弱离散梯度是如何定义的之后，我们现在可以回到如何组装相关方程的线性系统的问题上了。具体来说，利用上面显示的双线性形式 ${\cal A}_h$ 的定义，我们就需要计算对全局矩阵的局部贡献的元素，@f{equation*}{
...
\left( \mathbf{Kv}_j,\mathbf{v}_k \right)_K.$  所以，而不是上面的公式，单元格 $K$ 上的数字速度反而变成@f{equation*}
...
@f}
*其中 $| K |$ 是元素的面积， $\gamma$ 是元素的面， $\mathbf{n}$ 是每个面的单位法向量。最后一项是测量网格单元之间界面上速度向量的法向分量的精度。缩放因子 $|K|/|\gamma|$ 的选择是为了随着网格尺寸的变化而缩小界面集合的长度（或面积）的差异。
* 上面的第一个错误很容易用 VectorTools::integrate_difference. 计算出来，其他的需要更多的工作，在下面的代码中实现。
* 

* <a name="CommProg"></a> <h1> The commented program</h1>.
* <a name="Includefiles"></a> <h3>Include files</h3> 这个程序是基于 step-7 、 step-20 和 step-51 ，所以下面的头文件大部分是熟悉的。我们需要以下文件，其中只有导入FE_DGRaviartThomas类的文件（即`deal.II/fe/fe_dg_vector.h`）是真正的新文件；FE_DGRaviartThomas实现了介绍中讨论的 "破碎 "Raviart-Thomas空间。
* 
...
* 
 ...
 @endcode
* 
* 我们的第一步，像往常一样，是把所有与本教程程序有关的东西放入自己的命名空间。
* 
...
* 
* 这是本程序的主要类别。我们将使用弱加勒金（WG）方法求解内部和面上的数值压力，并计算出压力的 $L_2$ 误差。在后处理步骤中，我们还将计算速度和通量的 $L_2$ -误差。  
...
* 
 @endcode
* 
* 实现精确压力解决方案的类有一个奇怪之处，即我们把它作为一个有两个分量的向量值来实现。(我们在构造函数中说它有两个分量，在这里我们调用了基础函数类的构造函数)。在`value()`函数中，我们不测试`component`参数的值，这意味着我们为向量值函数的两个分量返回相同的值。我们这样做是因为我们将本程序中使用的有限元描述为一个包含内部和界面压力的矢量值系统，当我们计算误差时，我们希望使用相同的压力解来测试这两个分量。
* 
...
* 
* 在这个构造函数中，我们为矢量值函数创建了一个有限元空间，这里将包括用于内部和界面压力的函数，  $p^\circ$  和  $p^\partial$  。
* 
...
* 
 @endcode
* 
*  @p cell_matrix_C 是 $G^K$ 的转置和质量矩阵的逆之间的矩阵乘积（这个逆被存储在 @p cell_matrix_M): 中 
* 
...
* 
 @endcode
* 
* 接下来，我们计算右手边的 $\int_{K} f q \mathrm{d}x$  。
* 
...
* 
 @endcode
* 
* 最后一步是将局部矩阵的成分分配到系统矩阵中，并将单元右方的成分转移到系统右方。
* 
...
* 
* 在这个函数中，根据之前计算的压力解来计算速度场。速度定义为 $\mathbf{u}_h = \mathbf{Q}_h \left(
* 


* 
* 
-\mathbf{K}\nabla_{w,d}p_h \right)$ ，这需要我们计算许多与系统矩阵的组装相同的项。还有一些矩阵 $E^K,D^K$ 我们需要组装（见介绍），但它们实际上只是遵循同样的模式。  
* 在这里计算与我们在`assemble_system()`函数中已经完成的相同的矩阵，当然是在CPU时间上浪费了。同样，我们从那里复制了一些代码到这个函数中，这通常也是一个糟糕的主意。一个更好的实现可能会提供一个函数来封装这些重复的代码。我们也可以考虑使用计算效率和内存效率之间的经典权衡，在装配过程中每个单元只计算一次 $C^K$ 矩阵，把它们存储在某个地方，然后在这里重新使用。例如， step-51 就是这样做的，`assemble_system()`函数需要一个参数来决定是否重新计算本地矩阵，还有一个类似的方法
* 
...
* 我们现在可以计算局部速度未知数（相对于我们要把 $-\mathbf K \nabla_{w,d} p_h$ 项投射到的Raviart-Thomas空间而言）。
* 
...
* 
* 在这个函数中，我们对每个单元的速度评估 $L_2$ 误差，对面的通量评估 $L_2$ 误差。该函数依赖于之前计算过的`compute_postprocessed_velocity()`函数，该函数根据之前计算过的压力解来计算速度场。  
...
* 
 @endcode
* 
* 将所有单元和面的误差相加后，我们进行平方根计算，得到速度和通量的 $L_2$ 误差。这些我们都输出到屏幕上。
* 
...
* 
* 我们有两组结果要输出：内部解和骨架解。我们使用 <code>DataOut</code> 来可视化内部结果。骨架结果的图形输出是通过使用DataOutFaces类完成的。  
...
* 以下数字显示了使用 $\mbox{WG}(Q_0,Q_0;RT_{[0]})$ 元素的内部压力和表面压力。网格分别细化了2倍（顶部）和4倍（底部）。(这个数字可以在`make_grid()`函数中调整)。当网格较粗时，我们可以看到面压 $p^\partial$ 与相邻两个单元的内压 $p^\circ$ 的值之间的整齐排列。
*  <table align="center">
  <tr>
...
* 我们可以用下一个更高的多项式度数重复上面的实验：下面的数字是用 $\mbox{WG}(Q_1,Q_1;RT_{[1]})$ 实现的内部压力和表面压力。网格被细化了4次。 与之前使用 $\mbox{WG}(Q_0,Q_0;RT_{[0]})$ 的数字相比，在每个单元上，解决方案不再是恒定的，因为我们现在使用双线性多项式来做近似。因此，在一个内部有4个压力值，每个面有2个压力值。
*  <table align="center">
  <tr>
...
* 以下是我们通过使用 $\mbox{WG}(Q_1,Q_1;RT_{[1]})$ 元素组合得到的压力、速度和通量的收敛率。
*  <table align="center" class="doxtable">
  <tr>
   <th>number of refinements </th><th>  $\|p-p_h^\circ\|$  </th><th>  $\|\mathbf{u}-\mathbf{u}_h\|$ </th><th> $\|(\mathbf{u}-\mathbf{u}_h) \cdot \mathbf{n}\|$ </th>
  </tr>
...
* 让我们再高一个多项式度。下面是使用 $WG(Q_2,Q_2;RT_{[2]})$ 实现的内部压力和面压，网格大小为 $h = 1/32$ （即5个全局网格细化步骤）。在程序中，当生成图形输出时，我们使用`data_out_face.build_patches(fe.degree)`（见文献 DataOut::build_patches()), ，这里意味着我们将每个2d单元内部分为4个子单元，以提供一个更好的二次多项式的可视化。 <table align="center">
  <tr>
...
* 再一次， $\mbox{WG}(Q_2,Q_2;RT_{[2]})$ 的收敛率是预期的，其值在3左右。
* 
