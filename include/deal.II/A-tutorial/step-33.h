 ...
 * 本教程依赖于  step-12  ,  step-71  。 @htmlonly <table
 ...
 * \right),
 * @f} 。
 * *我们将选择重力的影响作为特定的右手边强制力，由@f{eqnarray*}
 * \mathbf G(\mathbf w)
 * =
 * \left(
 * \begin{array}{c}
 ...
 * \right),
 * @f}描述。
 * *其中 $\mathbf g=(g_1,g_2,g_3)^T$ 表示重力矢量。这样，整个方程组为：@f{eqnarray*}
 ...
 * 我们使用一个时间步进方案来替代上述方程中的时间导数。为简单起见，我们将
 * $ \mathbf{B}({\mathbf{w}_{n}})(\mathbf z) $ 定义为时间步长 $n$
 * 的空间残差。
 * @f{eqnarray*}
 ...
 * @f}
 *
 * 在每个时间步骤，我们的完全离散化是指应用于任何测试函数 $\mathbf z$ 的残差等于零：@f{eqnarray*}
 ...
 * &=& 0
 * @f} 。
 * 其中 $ \theta \in [0,1] $ 和 $\mathbf{w}_i = \sum_k \mathbf{W}_i^k
 * \mathbf{\phi}_k$  。选择 $\theta=0$
 * 的结果是显式（正向）欧拉方案， $\theta=1$
 * 是稳定的隐式（反向）欧拉方案，而 $\theta=\frac 12$
 * 是克拉克-尼克尔森方案。 在下面的实现中，我们为函数
 * $\mathbf H$ 选择Lax-Friedrichs通量，即
 ...
 * @f} 来解决。
 * *直到 $|R(\mathbf{W}^k_{n+1})|$ （残差）足够小。通过用有限元空间的节点基础而不是所有的 $\mathbf z$ 进行测试，我们得出 $\delta \mathbf W$ 的线性系统：@f{eqnarray*}
 * \mathbf R'(\mathbf{W}^k_{n+1})\delta \mathbf{W}^k_{n+1} & = &
 *
 * -
 * \mathbf R(\mathbf{W}^{k}_{n+1}).
 * @f} 。
 * 这个线性系统，一般来说，既不是对称的，也没有任何特定的确定性属性。我们将使用直接求解器或Trilinos的GMRES实现来解决它。从<a
 * href="#Results">results shown
 ...
 * 的元素。这两个向量很容易有几万个元素或更多。
 * 然而，需要注意的是，并非 $\mathbf R$
 ...
 * ，确保代码的正确性和维护代码变得非常简单。
 *
 * - 雅各布矩阵 $\mathbf R'$ 基本上是由计算残差 $\mathbf
 * R$ 的相同代码来计算的。
 ...
 * @endcode
 *
 * 输出是 $c(a,b)=2a+\cos(ab)$ 在 $a=1,b=2$ 的导数 $\frac{\partial
 * c(a,b)}{\partial a}, \frac{\partial c(a,b)}{\partial b}$  。
 * 应该注意的是，Sacado提供了比本程序中使用的小子集更多的自动差分能力。
 * 然而，理解上面的例子就足以理解Sacado在这个欧拉流程序中的使用。
 * <a name="Trilinossolvers"></a><h3> Trilinos solvers </h3> 。
 * 该程序使用Aztec迭代求解器或Amesossparse直接求解器，两者均由Trilinos软件包提供。
 * 这个软件包本身就是为了用于并行程序而设计的，然而，它也可以像这里一样方便地用于串行程序。
 * Epetra软件包是基本的矢量/矩阵库，解算器就是在此基础上建立的。
 ...
 * 根据作者的经验，对于随时间变化的问题的适应性，如果不注意，细化很容易导致模拟的戛然而止，因为时间步长的限制，如果网格在领域的任何部分变得太细的话。
 * 在这个例子中，细化的数量是有限的，用户可以指定网格中任何地方的最大细化程度。
 * 这样一来，细化就不会使仿真速度减慢到停顿。
 * 当然，这纯粹是一种启发式的策略，如果作者的顾问听说了，作者可能会被永远放逐出有限元误差估计界。
 ...
 * <li>  处理时间步进、外部非线性和内部线性求解、组装线性系统以及驱动这一切的顶层逻辑的 <code>ConservationLaw</code> 类。 </ul>
 * 采用这种方法的原因是它将程序中的各种关注点分开：
 * <code>ConservationLaw</code>
 * 是以这样一种方式编写的，即相对简单地将其适应于不同的方程组。我们只需为其他双曲方程重新实现
 * <code>EulerEquations</code>
 * 类的成员，或者用额外的方程来增强现有的方程（例如通过移动额外的变量，或者通过增加化学成分等）。然而，这些修改不会影响时间步进，也不会影响非线性求解器，如果做得正确的话，因此
 * <code>ConservationLaw</code> 中的任何内容都不必修改。
 * 同样，如果我们想改进线性或非线性求解器，或改进时间步进方案（正如在<a
 * href="#Results">results
 * section</a>的末尾所暗示的那样），那么这根本不需要修改
 * <code>EulerEquations</code> 。
 *
 ...
 * 的方式相同，我们也需要有一种方法来描述右侧的强迫项。正如介绍中提到的，我们在这里只考虑重力，这导致了具体的形式
 * $\mathbf G(\mathbf w) = \left( g_1\rho, g_2\rho, g_3\rho, 0, \rho \mathbf g
 * \cdot \mathbf v \right)^T$
 ...
 * <code>const</code>
 * 的参数。然而，在下面的实现中，它却成为了参数，而且为了使代码能够编译，它必须成为参数。原因是我们在
 * <code>Wminus</code> 类型为 <code>Table@<2,Sacado::Fad::DFad@<double@>
 * @></code>
 * 的地方调用这个函数，这是一个2d表，其指数分别代表正交点和向量分量。我们用
 * <code>Wminus[q]</code>
 * 作为最后一个参数来调用这个函数；对2d表进行下标会产生一个代表1d向量的临时访问器对象，这正是我们在这里想要的。问题是，根据C++
 * 1998和2003标准，临时访问器对象不能被绑定到一个函数的非静态引用参数上，就像我们在这里希望的那样（这个问题将在下一个标准中以rvalue引用的形式得到解决）。
 ...
 * 放入我们的输出文件中。
 * 此外，我们还想增加生成Schlieren图的可能性。Schlieren图是一种将冲击和其他尖锐界面可视化的方法。"schlieren
 * "这个词是一个德语单词，可以翻译成 "条纹"。
 *
 * 然而，用一个例子来解释可能更简单：比如说，当你把高浓度的酒精或透明的盐水溶液倒入水中时，你会看到schlieren；这两种物质的颜色相同，但它们的折射率不同，因此在它们完全混合之前，光线会沿着弯曲的光线穿过混合物，如果你看它，会导致亮度变化。这就是
 ...
 * 函数也会调用基类的相应函数。
 * 请注意，这个类也处理输入文件中指定的初始和边界条件的声明。为此，在这两种情况下，都有像
 ...
 * 的数组，对于解向量的每个分量，表明它是流入、流出还是其他类型的边界，其次是一个FunctionParser对象，一次性描述这个边界ID的解向量的所有分量。
 * <code>BoundaryCondition</code>
 * 结构需要一个构造器，因为我们需要在构造时告诉函数解析器对象它要描述多少个向量分量。因此，这个初始化不能等到我们在后面的
 * <code>AllParameters::parse_parameters()</code>
 * 中实际设置FunctionParser对象所代表的公式。
 ...
 * \mathbf{B}(\mathbf{w}_{n}) (\mathbf{z}_i)_K $ ，其中
 ...
 *
 *
 * - (\mathbf{G}(\mathbf {w}), \mathbf{z}_i)_K $ 为 $\mathbf{w} =
 * \mathbf{w}^k_{n+1}$ 和 $\mathbf{w} = \mathbf{w}_{n}$ ， $\mathbf{z}_i$
 * 为 $i$ 第1个向量值的测试函数。此外，标量积
 ...
 * ）和前一个时间步长的解 $W_{n}$ （变量 <code>W_old</code>
 ...
 * @endcode
 *
 * 在所有这些声明之后，让我们实际计算一些东西。首先，
 * <code>W</code>, <code>W_old</code>, <code>grad_W</code> 和
 * <code>grad_W_old</code> 的值，我们可以通过使用公式
 * $W(x_q)=\sum_i \mathbf W_i \Phi_i(x_q)$
 * 从局部DoF值计算出来，其中 $\mathbf W_i$
 * 是解向量（局部部分）的第 $i$ 个条目，而 $\Phi_i(x_q)$
 * 是在正交点 $x_q$ 评估的 $i$
 * 个矢量值的形状函数值。梯度可以用类似的方法来计算。
 * 理想情况下，我们可以通过调用类似
 * FEValues::get_function_values 和 FEValues::get_function_gradients,
 ...
 * @endcode
 *
 * 接下来，为了计算单元贡献，我们需要在所有正交点评估
 * $\mathbf{F}({\mathbf w}^k_{n+1})$  ,  $\mathbf{G}({\mathbf w}^k_{n+1})$
 * 和  $\mathbf{F}({\mathbf w}_n)$  ,  $\mathbf{G}({\mathbf w}_n)$
 ...
 * 实际上只有一个非零分量，我们可以将其简化一下（关于这个主题的更多内容可以在
 * @ref  矢量值模块中找到）。它将由下面的变量
 * <code>component_i</code>
 * 表示。有了这个，残差项可以重新写成@f{eqnarray*} R_i &=&
 ...
 * (\mathbf{z}_i)_{\text{component\_i}} \right)_K , @f}。
 * 其中积分可以理解为通过对正交点求和来评估。
 * 我们最初对残差的所有贡献进行正意义上的求和，这样我们就不需要对雅各布项进行负数。
 ...
 * w}{\partial t} \approx \mathbf w^n + \delta t \; \frac{\mathbf w^n-\mathbf
 * w^{n-1}}{\delta t} = 2 \mathbf w^n
 *
 * - \mathbf w^{n-1}$
 ...
 * " alt="" height="300">
 * 正如我们所看到的，当沉重的流体质量撞到左下角时，会发生一些振荡，导致迭代的发散。解决这个问题的一个懒办法是添加更多的粘性。如果我们将扩散功率设置为
 * $\eta = 1.5$ 而不是 $2.0$
 * ，模拟将能够度过这一危机。那么，结果就会是这样的。
 *
 *  <img
