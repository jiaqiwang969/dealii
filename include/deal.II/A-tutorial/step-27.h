...
* 现在再来谈谈如何使用 $hp$ 的deal.II的功能的细节。我们要处理的第一个方面是，现在我们不再只有一个用于所有单元的单一有限元，而是有许多不同的单元可以选择使用。为此，deal.II引入了<i>finite element
...
* 我们要考虑的下一个任务是如何处理我们要使用的无限元素对象的列表。在以前的教程程序中，从 step-2 开始，我们看到DoFHandler类负责在网格(由三角形对象描述)和有限元之间建立联系，为网格的每个顶点、面、边和单元分配正确的自由度数量。
...
index</i>，以表明这是活跃在该单元上的有限元，而集合中的所有其他元素在该单元上是不活跃的。其大致内容如下。
* @code
...
* 自适应有限元方法的核心部分之一是，我们用一个指标来检查计算出的解（后验），这个指标告诉我们哪些单元的误差最大，然后对它们进行细化。在其他许多教程程序中，我们使用KellyErrorEstimator类来获得一个单元的误差大小指示，尽管我们也在一些程序中讨论了更复杂的策略，最重要的是在 step-14 中。
* 在任何情况下，只要决定只是 "细化这个单元 "或 "不细化这个单元"，实际的细化步骤就不是特别有挑战性的。然而，在这里，我们有一个能够进行hp细化的代码，也就是说，每当我们检测到某一单元的误差过大，我们突然有两个选择：我们可以通过将其分割成几个小单元来细化该单元，或者我们可以增加用于该单元的形状函数的多项式程度。我们如何知道哪种策略更有前途？在撰写本文时，回答这个问题是 $hp$ 有限元研究的核心问题。
...
* 我们的方法很简单：对于一个函数 $u({\bf x})$ 来说，它必须满足 $K$ 单元上的Sobolev空间 $H^s(K)$ 的条件@f[
   \int_K |\nabla^s u({\bf x})|^2 \; d{\bf x} < \infty.
@f]。从单元格到单元格 $K$ 的映射足够规则，上述条件当然相当于@f[
...
\ldots$  。正如我们证明上面的序列 $a_l$ 通过在整条线上的积分来替代总和而实现的一样，我们可以用在 $d$ -维空间上的积分来替代我们的 $d$ -维总和。现在我们必须注意，在距离 $|{\bf k}|$ 和 $|{\bf k}|+d|{\bf k}|$ 之间，在一个常数以内，有 $|{\bf k}|^{d-1}$ 个模式，就像我们可以把体积元素 $dx\;dy$ 转化为 $2\pi r\; dr$ 一样。因此，不再是 $|{\bf k}|^{2s}|\hat
U_{\bf k}|^2$ 必须衰变为 ${\cal O}(|{\bf k}|^{-1-\epsilon})$ ，而实际上是 $|{\bf k}|^{2s}|\hat U_{\bf k}|^2 |{\bf k}|^{d-1}$ 。指数的比较产生了一个结果。
...
* 那么，我们该如何估计 $u({\bf x})$ 在 $K$ 单元上的局部平滑度？显然，第一步是计算我们解决方案的傅里叶系数。傅里叶级数是无限级数，我们通过只计算级数的前几项来简化我们的任务，例如， $|{\bf k}|\le 2\pi N$ 有一个截止点 $N$ 。让我们顺便说一下，我们希望选择 $N$ 足够大，以便我们至少能捕捉到那些变化最大的形状函数的变化。另一方面，我们也不应该把 $N$ 选得太大：显然，一个有限元函数，作为一个多项式，在任何给定的单元格上都在 $C^\infty$ 中，所以系数必须在某一点上呈指数衰减；由于我们想估计这个多项式所逼近的函数的平稳性，而不是多项式本身，我们需要为 $N$ 选择一个合理的截断。无论如何，计算这个数列并不特别困难：从定义@f[
   \hat U_{\bf k}
   = \int_{\hat K} e^{i {\bf k}\cdot \hat{\bf x}} \hat u(\hat{\bf x}) d\hat{\bf x}
@f]我们可以看出，我们可以将系数 $\hat U_{\bf k}$ 计算为@f[
   \hat U_{\bf k}
   =
   \sum_{i=0}^{\textrm{dofs per cell}}
   \left[\int_{\hat K} e^{i {\bf k}\cdot \hat{\bf x}} \hat \varphi_i(\hat{\bf x})
   d\hat{\bf x} \right] u_i,
@f]，其中 $u_i$ 是这个单元格上 $i$ 的自由度的值。换句话说，我们可以把它写成@f[
   \hat U_{\bf k}
   = {\cal F}_{{\bf k},j} u_j,
@f]与矩阵@f[
   {\cal F}_{{\bf k},j}
...
* 有人可能会问，我们只计算解的<i>reference cell</i>（而不是实数单元）的傅里叶变换，这是否是一个问题？毕竟，在变换过程中，我们将解决方案拉伸了一个系数 $\frac 1h$ ，从而使傅里叶频率移动了一个系数 $h$ 。这是一个特别值得关注的问题，因为我们可能会有网格大小 $h$ 的相邻单元，如果其中一个单元比另一个单元更精细，那么这两个单元就会相差2倍。正如我们在下面的结果部分所看到的那样，估计的平滑度应该是一个或多或少的连续函数，但是在网格尺寸跳跃的地方却表现出跳跃性，这也是引起关注的原因。因此，我们似乎很自然地要问，我们是否需要对这种转换进行补偿。
...
* 
- \mu \ln h) + \mu \ln (|{\bf k}|).
@f]。换句话说，这和原来的最小二乘法问题将产生相同的最佳拟合指数 $\mu$ ，尽管在一种情况下偏移量是 $\beta$ ，在另一种情况下是 $\beta-\mu \ln h$  。然而，由于我们对偏移量不感兴趣，而只对指数感兴趣，所以无论我们是否为了考虑网格大小的影响而对傅里叶频率进行缩放，都没有关系，在两种情况下估计的光滑度指数都是一样的。
* 
...
*  $hp$ 方法的一个问题是，形状函数的高多项式程度与大量的受限自由度一起导致某些行中有大量的非零条目的矩阵。同时，由于有些地方我们使用低多项式度，因此矩阵行的非零项相对较少。因此，为这些矩阵分配稀疏性模式是一个挑战：我们不能简单地从带宽的估计开始组装一个稀疏性模式，而不使用大量的额外内存。
...
* 
-\Delta u = f
@f]，用 $f=(x+1)(y+1)$ ，以及 $u$ 的零迪里切特边界值。我们在域 $[-1,1]^2\backslash[-\frac 12,\frac 12]^2$ 上这样做，即一个中间有一个正方形孔的正方形。
* 当然，与 step-14 的区别在于我们使用 $hp$ 的有限元素进行求解。这个测试案例是有意义的，因为它在洞的四角有re-entrant角，在这些角上解有singularities。因此，我们期望解在域的内部是平滑的，而在奇点附近是粗糙的。我们希望我们的细化和光滑度指标能够看到这种行为，并在远离奇点的地方细化网格，同时增加多项式度数。正如我们将在结果部分看到的那样，情况确实如此。
* 
...
* 
* 这个程序的主类看起来非常像前几个教程程序中已经使用的主类，例如  step-6  中的那个。主要的区别是我们将refine_grid和output_results函数合并为一个，因为我们还想输出一些用于决定如何细化网格的量（特别是估计的解决方案的平滑度）。  
...
* 
* 这是一个从每个单元的局部贡献中集合全局矩阵和右手向量的函数。它的主要工作与之前的许多教程程序中描述的一样。重要的差异是<i>hp</i>有限元方法所需要的。特别是，我们需要使用FEValues对象的集合（通过 hp::FEValues 类实现），并且在将局部贡献复制到全局对象时，我们必须消除受限自由度。这两点在本程序的介绍中都有详细解释。  
...
* 
* 在解决了线性系统之后，我们要对解决方案进行后处理。在这里，我们所做的是估计误差，估计解的局部光滑度，如介绍中所述，然后写出图形输出，最后根据之前计算的指标细化 $h$ 和 $p$ 中的网格。我们在同一个函数中完成这一切，因为我们希望估计的误差和平滑度指标不仅用于细化，而且还包括在图形输出中。
* 
...
* 上述函数只决定了多项式程度是否会通过未来的FE指数发生变化，但并不操作 $h$  -细化标志。因此，对于被标记为两种细化类别的单元格，我们更喜欢 $p$  -而不是 $h$  -细化。下面的函数调用确保只有 $p$  -或 $h$  -细化中的一个被施加，而不是同时施加。
* 
...
* 
 @endcode
* 
* 对于网格自适应细化，我们通过调用 Triangulation::prepare_coarsening_and_refinement(). 将相邻单元的细化水平差限制为1来确保2:1的网格平衡。 我们希望对相邻单元的p水平实现类似的效果：未来有限元的水平差不允许超过指定的差。通过其默认参数，调用 hp::Refinement::limit_p_level_difference() 可以确保它们的级差被限制为1。这不一定会减少域中的悬挂节点的数量，但可以确保高阶多项式不会被限制在面的低得多的多项式上，例如，五阶多项式到二阶多项式。
* 
...
* 
 @endcode
* 
* 在这个过程的最后，我们再细化网格。在这个过程中，正在进行分割的单元的子单元会继承其母单元的有限元指数。此外，未来的有限元指数将变成活动的，因此新的有限元将在下一次调用 DoFHandler::distribute_dofs(). 后被分配给单元。
* 
...
* 
 ...
 @endcode
* 
* <a name="LaplaceProblemcreate_coarse_grid"></a> <h4>LaplaceProblem::create_coarse_grid</h4> * * * <a name="LaplaceProblemcreate_coarse_grid"></a> <h4>LaplaceProblem::create_coarse_grid</h4>。
...
* 
* 主函数又是逐字逐句的：把创建和运行一个主类的对象包装成一个 <code>try</code> 块，并捕捉任何抛出的异常，从而在出现问题时产生有意义的输出。
* 
...
* 在本节中，我们讨论了运行当前教程程序所产生的一些结果。更多的结果，特别是扩展到三维计算和确定程序的各个部分需要多少计算时间，将在 @ref hp_paper "hp-paper "中给出。
* 当运行时，这是该程序产生的结果。
* @code
> make run
...
* 本教程只演示了决定 $h$  -和 $p$  -适应性的一种特殊策略。事实上，还有许多自动决定适应类型的方法，其中一些已经在deal.II中实现：  <ul>   <li>  <i>Fourier coefficient decay:</i> 这是本教程中目前实现的策略。关于这一策略的更多信息，请参见 SmoothnessEstimator::Fourier 命名空间的一般文档。 </li>  
*  <li>  <i>Legendre coefficient decay:</i> 这个策略与目前的策略非常相似，但使用了Legendre级数展开，而不是傅里叶级数：这个策略使用Legendre多项式而不是正弦波作为基础函数。当然，由于我们在每个单元上使用有限维度的多项式来近似解，因此解在Legendre多项式中的扩展也是有限的，因此，当我们谈论这个扩展的 "衰减 "时，我们只能考虑这个扩展的有限多个非零系数，而不是用渐进的方式来思考。 但是，如果我们有足够多的这些系数，我们当然可以把这些系数的衰减看作是精确解的系数衰减的特征（一般来说，精确解不是多项式的，所以会有一个无限的Legendre扩展），考虑我们拥有的系数应该可以揭示一些关于精确解的特性。
...
* 本教程中介绍的所有功能都已经适用于顺序和平行应用。可以不费吹灰之力就改成 parallel::shared::Triangulation 或 parallel::distributed::Triangulation 类。如果你急于尝试，我们建议先阅读 step-18 （前者）和 step-40 （后者），以了解有关该主题的进一步背景信息，然后再回到本教程，尝试你新获得的技能。
