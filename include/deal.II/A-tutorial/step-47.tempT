...
* @f{align*}{
  ...
  &&\forall \mathbf x \in \partial\Omega,
@f}
* 对应于薄结构的边缘连接到高度为 $g(\mathbf x)$ 的墙顶，这样作用在结构上的弯曲力为 $h(\mathbf x)$ ；在大多数物理情况下，我们会有 $h=0$ ，对应于结构只是坐在墙顶。
* 在边界值的第二种可能情况下，我们将有
* @f{align*}{
  u(\mathbf x) &= g(\mathbf x) \qquad \qquad
  &&\forall \mathbf x \in \partial\Omega, \\
  ...
  &&\forall \mathbf x \in \partial\Omega.
@f}
* 这相当于一个 "夹紧 "的结构，非零的 $j(\mathbf x)$ 意味着与水平面有一定的角度。
...
* 该方程的基本问题是，它需要解的四次方。在我们在 step-3 、 step-4 和其他几个教程中处理的拉普拉斯方程的情况下，一个人乘以一个测试函数，进行积分，通过部分积分，最后在测试函数和试验函数上都只得到一个导数
* 
...
* 
* 但由于测试函数和试验函数的二次导数都产生了德尔塔函数，我们将尝试整合两个德尔塔函数的乘积。例如，在1d中， $\varphi_i$ 是通常的线性 "帽子函数"，我们会得到这样的积分
* @f{align*}{
...
@f}
* 其中 $x_i$ 是定义形状函数 $\varphi_i$ 的节点位置， $h$ 是网格大小（假定为均匀的）。问题是，积分中的delta函数是用以下关系定义的
* @f{align*}{
  \int_0^L \delta(x-\hat x) f(x) \; dx
  =
  f(\hat x).
@f}
* 但这只有在以下情况下才有效：(i)  $f(\cdot)$ 实际上在 $\hat x$ 处定义良好，以及(ii)它是有限的。另一方面，一个积分的形式为
* @f{align*}{
...
* @f{align*}{
  ...
  &&\forall \mathbf x \in \partial\Omega,
@f}
* 那么下面的窍门就会起作用（至少如果域是凸的，见下文）。就像我们通过引入第二个变量从常规拉普拉斯方程得到 step-20 的混合拉普拉斯方程一样，我们可以在这里引入一个变量 $v=\Delta u$ ，然后可以用下面的 "混合 "系统代替上面的方程。
* @f{align*}{
* 
...
-f(\mathbf x)
  ...
  &&\forall \mathbf x \in \partial\Omega.
@f}
* 换句话说，我们最终得到的实质上是 $u,v$ 的两个耦合拉普拉斯方程系统，每个方程都有迪里希勒型边界条件。我们知道如何解决这样的问题，使用 step-20 或 step-22 的技术为这个系统构造良好的求解器和预处理器应该不是很困难。因此，这个案例的处理非常简单。
*  @note  值得指出的是，这只适用于边界有角的域，如果该域也是凸形的话
* 
*--换句话说，如果没有重入角的话。 这听起来是一个相当随意的条件，但考虑到以下两个事实，它是有意义的。原始双调方程的解必须满足  $u\in H^2(\Omega)$  。另一方面，上面的混合系统重述表明， $u$ 和 $v$ 都满足 $u,v\in H^1(\Omega)$ ，因为这两个变量只解决一个泊松方程。换句话说，如果我们想保证混合问题的解 $u$ 也是原来的偏谐方程的解，那么我们需要能够以某种方式保证 $-\Delta u=v$ 的解实际上比只是 $H^1(\Omega)$ 更平滑。这一点可以作如下论证。对于凸域，<a href="https://en.wikipedia.org/wiki/Elliptic_operator#Elliptic_regularity_theorem">"elliptic
  regularity"</a>意味着，如果右侧 $v\in H^s$ ，那么 $u\in H^{s+2}$ 如果域是凸的并且边界足够光滑。(如果域的边界足够光滑，这也可以保证
* 
* - 但边界没有角的域在现实生活中并不十分实用）。)  我们知道 $v\in H^1$ ，因为它解决了方程 $-\Delta v=f$ ，但我们仍然留下了边界凸性的条件；我们可以证明多边形、凸形域在这种情况下足以保证 $u\in H^2$ （光滑有界的凸形域将导致 $u\in H^3$ ，但我们不需要这么多规则性）。另一方面，如果域不是凸的，我们不能保证混合系统的解在 $H^2$ 中，因此可能得到一个不能等于原始双调方程的解。
...
* @f{align*}{
  ...
  &&\forall \mathbf x \in \partial\Omega.
@f}
* 混合系统的相同技巧在这里不起作用，因为我们最终会对 $u$ 设定迪里切特和诺伊曼边界条件，但对 $v$ 则没有。
* 

* 在20世纪90年代和21世纪初，这一难题的解决方案随着非连续加尔金法的出现而到来。与使用<i>discontinuous</i>形状函数处理拉普拉斯方程一样，我们可以使用<i>continuous</i>（而不是 $C^1$ 连续）形状函数的方案，通过惩罚不连续的大小来获得每个形状函数上有一个导数的方程的方案，从而获得一个每个形状函数上有两个导数的方案。与拉普拉斯方程的内部惩罚（IP）方法类似，这个用于双调方程的方案通常被称为 $C^0$ IP（或C0IP）方法，因为它使用 $C^0$ （连续但不可连续微分的）形状函数和内部惩罚公式。
* 

*<a name="DerivationoftheC0IPmethod"></a><h3> Derivation of the C0IP method </h3>。
* 

* 我们以SusanneBrenner和Li-Yeng Sung在 "C  $^0$  Interior Penalty Methodfor Linear Four Order Boundary Value Problems on polygonaldomains" @cite Brenner2005 一文中提出的 $C^0$ IP方法为基础，该方法是针对具有 "钳制 "边界条件的双谐波方程提出的。
* 如前所述，该方法依赖于使用 $C^0$ Lagrange有限元，其中 $C^1$ 的连续性要求被放宽并被内部惩罚技术取代。为了推导这个方法，我们考虑一个 $C^0$ 形状函数 $v_h$ ，它在 $\partial\Omega$ 上消失。我们引入符号 $ \mathbb{F} $ 作为 $\mathbb{T}$ 的所有面的集合， $ \mathbb{F}^b $ 作为边界面的集合，而 $ \mathbb{F}^i $ 作为内部面的集合，以便在下面进一步使用。 的高阶导数在每个界面 $e\in \mathbb{F}$ 上有两个值（由两个单元 $K_{+},K_{-} \in \mathbb{T}$ 共享），我们通过在 $e$ 上定义以下单值函数来应对这一不连续性。
* @f{align*}{
...
@f}
*用于 $k =1,2$ （即用于梯度和次导数矩阵），其中 $\mathbf n$ 表示指向 $e$ 的单位向量法线，从 $K_+$ 指向 $K_-$ 。在文献中，这些函数分别被称为 "跳跃 "和 "平均 "操作。
* 为了得到 $C^0$ 的IP近似值 $u_h$ ，我们用 $v_h$ 乘以双谐波方程，然后对 $\Omega$ 进行积分。如上所述，我们不能用这些形状函数对 $\Omega$ 的所有部分进行积分，但我们可以对每个单元单独进行积分，因为形状函数只是每个单元的多项式。因此，我们首先在每个网格单元 $K \in {\mathbb{T}}$ 上使用下面的分部分项积分公式。
* @f{align*}{
...
@f}
* 在这一点上，我们有两个选择。我们可以对域项的 $\nabla\Delta w_h$ 再进行一次积分，得到
* @f{align*}{
  \int_K v_h (\Delta^2 w_h)
...
* 相反，我们所做的是承认 $\nabla\Delta w_h = \text{grad}\,(\text{div}\,\text{grad}\, w_h)$ ，我们可以将这些操作重新排序为 $\nabla\Delta w_h = \text{div}\,(\text{grad}\,\text{grad}\, w_h)$ ，通常写成 $\text{grad}\,\text{grad}\, w_h = D^2 w_h$ ，以表明这是第二导数的 "黑森 "矩阵。通过这样的排序，我们现在可以整合发散，而不是梯度算子，我们得到以下结果。
* @f{align*}{
  \int_K v_h (\Delta^2 w_h)
  &= \int_K (\nabla \nabla v_h) : (\nabla \nabla w_h)
     +\int_{\partial K} v_h (\nabla \Delta w_h \cdot \mathbf n)
...
@f}
* 这里，冒号表示对其左边和右边的矩阵指数的双重收缩，即两个张量之间的标量乘积。两个向量 $a \otimes b$ 的外积产生了矩阵 $(a \otimes b)_{ij} = a_i b_j$ 。
* 然后，我们对所有单元格 $K \in  \mathbb{T}$ 进行求和，并考虑到这意味着每个内部面在求和中出现两次。因此，如果我们把所有的东西分成细胞内部的积分之和和细胞界面的单独之和，我们就可以使用上面定义的跳跃和平均运算符。还有两个步骤。首先，由于我们的形状函数是连续的，形状函数的梯度可能是不连续的，但是连续性保证了实际上只有梯度的法向分量在各面是不连续的，而切向分量是连续的。第二，当网格大小为零时，所产生的离散公式是不稳定的，为了得到一个稳定的公式，收敛到正确的解，我们需要添加以下条款。
* @f{align*}{
* 
...
+ \sum_{e \in \mathbb{F}}
  ...
  \jump{\frac{\partial u_h}{\partial \mathbf n}}.
@f}
* 然后，在对出现的情况进行抵消后，我们得到以下双调子方程的C0IP表述：找到 $u_h$ ，使 $u_h =
g$ 在 $\partial \Omega$ 上，并且
* @f{align*}{
...
@f}
* 这里， $\gamma$ 是惩罚参数，它既弱化了边界条件的执行力
* @f{align*}{
\frac{\partial u(\mathbf x)}{\partial \mathbf n} = j(\mathbf x)
@f}
*在边界界面 $e \in \mathbb{F}^b$ 上，也确保在极限 $h\rightarrow 0$ 中， $u_h$ 收敛为 $C^1$ 连续函数。  $\gamma$ 被选择为足够大，以保证方法的稳定性。我们将在下面的程序中讨论我们的选择。
* 

*<a name="ConvergenceRates"></a><h4>Convergence Rates </h4>。
* 在多边形域上，双调方程的弱解 $u$ 存在于 $H^{2 +\alpha}(\Omega)$ 中，其中 $\alpha \in(1/2, 2]$ 是由 $\Omega$ 的角的内角决定的。例如，只要 $\Omega$ 是凸的， $\alpha=1$ ； $\alpha$ 可能小于1，如果域有重心的角，但如果所有内角之一接近 $\pi$ ， $\alpha$ 就接近于 $1$  。
* 现在假设 $C^0$ 的IP解 $u_h$ 被 $C^0$ 的形状函数以多项式程度 $p \ge 2$ 所近似。那么，上述的解算就会产生下面所讨论的收敛率。
* 

* <b>Convergence in the $C^0$ IP-norm</b>
* 理想情况下，我们希望在 "能量准则 "中测量收敛性  $\|D^2(u-u_h)\|$  。然而，这并不可行，因为同样，离散解 $u_h$ 并没有两个（弱）导数。相反，我们可以定义一个离散( $C^0$  IP)半规范，"等同于 "能量规范，如下所示。
* @f{align*}{
 |u_h|_{h}^2 :=
...
* 当然，只有在精确解足够光滑的情况下，这才是真的。事实上，如果 $f \in H^m(\Omega)$ 中有 $m \ge 0$ ， $u \in H^{2+\alpha}(\Omega)$ 中有 $ 2 < 2+\alpha  \le m+4$ ，那么 $C^0$ IP方法的收敛率为 $\mathcal{O}(h^{\min\{p-1, \alpha\}})$ 。换句话说，只有在解非常平滑的情况下，才能期望达到最佳收敛率 $\alpha\ge p-1$ ；如果(i)域是凸的，有足够平滑的边界，以及(ii) $m\ge p-3$ ，这种情况就会发生。当然，在实践中，解决方案是什么就是什么（与我们选择的多项式程度无关），那么最后一个条件可以等同于说，如果 $m$ 也不大，那么选择 $p$ 大就肯定没有意义。换句话说， $p$ 的唯一合理选择是 $p\le
m+3$ ，因为更大的多项式度数不会导致更高的收敛顺序。
* 就本程序而言，我们有点懒得去实际实现这个等价的语义准则。
* 
* 尽管这并不难，而且会成为一个很好的练习。相反，我们将在程序中简单地检查一下 "破碎的" $H^2$ 语义规范是什么？
* @f{align*}{
...
@f}
*产生的结果。从理论上看，这个准则的收敛率当然不会比 $|\cdot|_h$ 的收敛率高<i>worse</i>，因为它只包含必要条件的一个子集，但至少可以想象到它会更好。还有一种情况是，即使程序中存在一个错误，我们也能得到最佳收敛率，而这个错误只会在 $|\cdot|_h$ 中出现的额外条款中显示出次优的收敛率。但是，人们可能希望，如果我们在破碎规范和下面讨论的规范中得到最优速率，那么这个程序确实是正确的。结果部分将证明我们在所有所述的规范中都得到了最优率。
* 

* <b>Convergence in the $L_2$-norm</b>
* 在 $L_2$ 规范中的最佳收敛率是 $\mathcal{O}(h^{p+1})$ 提供的 $p \ge 3$ 。更多细节可以在  @cite Engel2002  的定理4.6中找到。
* 在下面的程序中，默认是选择 $p=2$ 。在这种情况下，该定理并不适用，事实上，人们只能得到 $\mathcal{O}(h^2)$ 而不是 $\mathcal{O}(h^3)$ ，我们将在结果部分展示。
* 

* <b>Convergence in the $H^1$-seminorm</b>
* 鉴于我们在最好的情况下期望 $\mathcal{O}(h^{p-1})$ 等同于 $H^2$ 半规则的规范，以及 $L_2$ 规范的 $\mathcal{O}(h^{p+1})$ ，我们可能会问在 $H^1$ 半规则中会发生什么，这是与其他两个规范的中间位置。一个合理的猜测是，我们应该期待 $\mathcal{O}(h^{p})$ 。可能在某个地方有一篇论文证明了这一点，但是我们也验证了这个猜想在实验中是真实的。
* 
...
* 我们注意到， $C^0$ IP方法对具有其他边界条件的双谐波方程的推导是这样的
* 
* - 例如，对于第一组边界条件，即 $u(\mathbf x) =
g(\mathbf x)$ 和 $\Delta u(\mathbf x)= h(\mathbf x)$ 的 $\partial\Omega$ 的边界条件 
* 
* 可以通过对书中 $\mathcal{A}(\cdot,\cdot)$ 和 $\mathcal{F}(\cdot)$ 的适当修改得到。
* 

* <a name="Thetestcase"></a><h3>The testcase</h3> 。
* 

* 最后一步要描述的是这个程序解决的问题。一如既往，三角函数既是一个好的选择，也是一个坏的选择，因为它不在任何我们可以寻求解的多项式空间中，同时又比实数解通常更平滑（在这里，它在 $C^\infty$ 中，而实数解通常只在 $H^3$ 左右的凸多边形域中，如果域不是凸的，则在 $H^2$ 和 $H^3$ 之间）。但是，由于我们没有办法用相对简单的公式来描述现实问题的解决方案，我们只是用下面的方法，在域 $\Omega$ 的单位平方上。
* @f{align*}{
  u = \sin(\pi x) \sin(\pi y).
@f}
* 因此，我们需要选择以下条件作为边界条件。
...
@f}
* 该程序有类 `ExactSolution::Solution` 和 `ExactSolution::RightHandSide` ，对这些信息进行编码。
* 
...
* 
* 前面几个include文件已经在前面的例子中使用过了，所以我们在这里不再解释它们的含义。该程序的主要结构与例如 step-4 的结构非常相似，因此我们包含了许多相同的头文件。
* 
...
* 
* - 这在本质上就像拉普拉斯方程相应项的装配，你已经在 step-4 或 step-6 中看到了这是如何工作的，比如说
* 
...
* 但这还没有解决我们的问题，即当我们在所有单元格和它们的所有面中循环时，必须跟踪我们已经访问过哪些面。为了使这个过程更简单，我们使用了 MeshWorker::mesh_loop() 函数，它为这个任务提供了一个简单的接口：基于WorkStream命名空间文档中概述的想法， MeshWorker::mesh_loop() 需要三个函数对单元、内部面和边界面进行工作。这些函数在scratch对象上工作以获得中间结果，然后将其计算结果复制到copy数据对象中，由copyer函数将其复制到全局矩阵和右侧对象中。  
* 下面的结构就提供了这种方法所需的从头开始和复制对象。你可以查阅WorkStream命名空间以及 @ref threads "多处理器的并行计算 "模块，以了解更多关于它们通常如何工作的信息。
* 
...
* @f{align*}{
 ...
 @f}
* 到右手边的向量。    
* 我们使用与组装 step-22 时相同的技术来加速该函数。我们不在最里面的循环中调用`fe_values.shape_hessian(i, qpoint)`，而是创建一个变量`hessian_i`，在循环中对`i`进行一次评估，在循环中对`j`重新使用如此评估的值。为了对称，我们对变量`hessian_j`也做了同样的处理，尽管它确实只用了一次，而且我们可以在计算两个项之间标量乘积的指令中留下对`fe_values.shape_hessian(j,qpoint)`的调用。
* 
...
* 
 ...
 @endcode
* 
* 下一个构建模块是在网格的每个内部面组装惩罚项。正如在 MeshWorker::mesh_loop(), 的文档中所描述的，这个函数收到的参数表示一个单元和它的相邻单元，以及（对于这两个单元中的每一个）我们需要整合的面（和可能的子面）。同样，我们也得到了一个Scratch对象，以及一个用于放置结果的copy对象。    
* 这个函数本身有三个部分。在顶部，我们初始化FEInterfaceValues对象，并创建一个新的 `CopyData::FaceData` 对象来存储我们的输入。这将被推到`copy_data.face_data`变量的末尾。我们需要这样做，因为我们对一个给定单元进行积分的面（或子面）的数量因单元而异，而且这些矩阵的大小也不同，取决于面或子面相邻的自由度。正如 MeshWorker::mesh_loop(), 文档中所讨论的，每次访问一个新的单元时，复制对象都会被重置，所以我们推到`copy_data.face_data()`末尾的内容实际上就是后来的`copier`函数在复制每个单元的贡献到全局矩阵和右侧对象时所能看到的全部内容。
* 
...
* 
 @endcode
* 
* 第二部分涉及到确定惩罚参数应该是什么。通过观察双线性形式中各种项的单位，很明显，惩罚必须具有 $\frac{\gamma}{h_K}$ 的形式（即一个超过长度尺度的惩罚），但应该如何选择无维数 $\gamma$ 并不是先验的。从拉普拉斯方程的不连续Galerkin理论来看，人们可能猜想正确的选择是 $\gamma=p(p+1)$ 是正确的选择，其中 $p$ 是所用有限元的多项式程度。我们将在本程序的结果部分更详细地讨论这个选择。      
* 在上面的公式中， $h_K$ 是单元格 $K$ 的大小。但这也不是很直接的事情。如果使用高度拉伸的单元格，那么一个更复杂的理论说， $h$ 应该被单元格 $K$ 的直径取代，该直径是有关边缘方向的法线。 事实证明，在deal.II中有一个函数用于此。其次，当从一个面的两个不同侧面看时， $h_K$ 可能是不同的。      
...
* 
 ...
 @endcode
* 
* 积极地，因为我们现在只处理与面相邻的一个单元（因为我们在边界上），惩罚因子 $\gamma$ 的计算大大简化了。
* 
...
* 
 @endcode
* 
* 第三块是条款的组合。由于这些条款包含了矩阵和右手边的条款，所以现在稍微有点麻烦了。前者与上面所说的内部面完全相同，如果我们只是适当地定义跳跃和平均（这就是FEInterfaceValues类的作用）。后者需要我们评估边界条件 $j(\mathbf x)$ ，在当前情况下（我们知道确切的解决方案），我们从 $j(\mathbf x) = \frac{\partial u(\mathbf x)}{\partial
 {\mathbf n}}$ 中计算出来。然后，要添加到右侧向量的项是  $\frac{\gamma}{h_e}\int_e
...
* 
 ...
 @endcode
* 
* 在设置了所有这些之后，剩下的就是创建一个scratch和copy数据对象，并调用 MeshWorker::mesh_loop() 函数，然后翻阅所有的单元格和面，调用它们各自的worker，然后是copyer函数，将东西放入全局矩阵和右侧。作为一个额外的好处， MeshWorker::mesh_loop() 以并行方式完成所有这些工作，使用你的机器恰好有多少个处理器核心。
* 
...
* 到此为止，演出基本上结束了。其余的函数并不太有趣或新颖。第一个函数只是用一个直接求解器来求解线性系统（也见 step-29  ）。
* 
...
* 下一个函数评估了计算出的解和精确解之间的误差（在这里是已知的，因为我们选择了右手边和边界值的方式，所以我们知道相应的解）。在下面的前两个代码块中，我们计算了 $L_2$ 准则和 $H^1$ 半准则的误差。
* 
...
* 
 @endcode
* 
* 现在也计算一下 $H^2$ 半规范误差的近似值。实际的 $H^2$ 半规范要求我们对解 $u_h$ 的二阶导数进行积分，但考虑到我们使用的拉格朗日形状函数， $u_h$ 当然在单元间的界面上有结点，因此二阶导数在界面上是奇异的。因此，我们实际上只对单元的内部进行积分，而忽略了界面的贡献。这不是*一个与问题的能量准则等价的准则，但仍然给了我们一个关于误差收敛速度的概念。    
* 我们注意到，我们可以通过定义一个等同于能量准则的准则来解决这个问题。这将涉及到不仅要像我们下面所做的那样将细胞内部的积分相加，而且还要为 $u_h$ 的导数在界面上的跳跃添加惩罚项，并对这两种项进行适当的缩放。我们将把这个问题留给以后的工作。
* 
...
* 我们在运行程序时，使用介绍中讨论过的右手和边界值。我们用 $Q_2$ ,  $Q_3$ , 和 $Q_4$ 元素来测试这个设置，我们可以通过`main()`函数中的`fe_degree`变量来改变这些元素。通过网格精简， $L_2$ 的收敛率、 $H^1$ 的近似值收敛率和 $H^2$ 的近似值收敛率对于 $Q_2$ 应该是2、2、1左右（如介绍中所讨论的 $L_2$ 的规范次优）；对于 $Q_3$ 是4、3、2；而对于 $Q_4$ 是5、4、3。
* 从文献中，并不清楚惩罚参数 $\gamma$ 应该是什么。例如， @cite Brenner2009 指出它需要大于1，并选择了 $\gamma=5$ 。FEniCS/Dolphin教程选择了它为 $\gamma=8$ ，见https://fenicsproject.org/docs/dolfin/1.6.0/python/demo/documented/biharmonic/python/documentation.html。  @cite Wells2007 使用的 $\gamma$ 值大于Kirchhoff板的元素所属的边数（见第4.2节）。这表明也许 $\gamma = 1$ 、 $2$ 太小了；另一方面， $p(p+1)$ 的值也是合理的，其中 $p$ 是多项式的度。最后一个选择是人们期望通过比较拉普拉斯方程的不连续Galerkin公式来实现的(例如，见 step-39 和 step-74 中的讨论)。 但是我们应该检查一下 $\gamma$ 的值是多少，我们将在下面进行；改变 $\gamma$ 在`assemble_system()`定义的两个`face_worker`和`boundary_worker`函数中很容易。
* 
...
</table>  我们可以看到， $L_2$ 的收敛率约为2， $H^1$  -seminorm收敛率约为2，而 $H^2$  -seminorm收敛率约为1。
* 
...
</table>  我们可以看到， $L_2$ 的收敛率约为4， $H^1$  -seminorm收敛率约为3， $H^2$  -seminorm收敛率约为2.当然，这与我们的理论预期相符。
* 
...
</table>  我们可以看到 $L_2$ 规范收敛率约为5， $H^1$  -seminorm收敛率约为4， $H^2$  -seminorm收敛率约为3。在最细的网格上， $L_2$  规范收敛率比我们的理论预期小很多，因为线性求解器由于舍弃而成为限制因素。当然，在这种情况下， $L_2$ 误差也已经非常小了。
* 
...
</table>  虽然 $L_2$ 的规范收敛率或多或少符合理论预期，但 $H^1$  -seminorm和 $H^2$  -seminorm似乎并没有像预期那样收敛。 鉴于 $\gamma=1$ 对于 $Q_2$ 元素来说已经太小了，如果用 $Q_3$ 元素重复实验，结果更加令人失望，这可能并不令人惊讶。我们又一次只得到了2、1、0的收敛结果。
* 
* 然而，也许令人惊讶的是，当使用 $Q_4$ 元素时，人们获得了或多或少的预期收敛顺序。无论如何，这种不确定性表明 $\gamma=1$ 充其量是一个有风险的选择，在最坏的情况下是一个不可靠的选择，我们应该选择 $\gamma$ 更大。
* 
...
* 由于 $\gamma=1$ 显然太小，人们可能猜想 $\gamma=2$ 实际上可能效果更好。以下是在这种情况下得到的结果。
*  <table align="center" class="doxtable">
  <tr>
   <th>Number of refinements </th><th>  $\|u-u_h^\circ\|_{L_2}$ </th><th>  Conv. rates  </th><th>  $|u-u_h|_{H^1}$ </th><th> Conv. rates </th><th> $|u-u_h|_{H^2}$ </th><th> Conv. rates </th>
  </tr>
...
p(p+1)$ 的结果相比，变化更大。同样，我们可以对 $Q_3$ 和 $Q_4$ 元素重复这种实验。在这两种情况下，我们都会发现我们获得了大致上的预期收敛率。那么，更有趣的是比较误差的绝对大小。在上表中，对于 $Q_2$ 情况，最细网格上的误差在 $\gamma=p(p+1)$ 和 $\gamma=2$ 情况之间是可比的，而对于 $Q_3$ ， $\gamma=2$ 的误差比 $\gamma=p(p+1)$ 大很多。对于 $Q_4$ 的情况也是如此。
* 

* <a name="Conclusionsforthechoiceofthepenaltyparameter"></a><h3> Conclusions for the choice of the penalty parameter </h3> 。
* 

* 关于应该使用哪种 "合理 "的惩罚参数选择的结论是， $\gamma=p(p+1)$ 会产生预期的结果。因此，这就是目前编写的代码所使用的。
* 
...
* 
* - 从更多的理论角度来看，上面的收敛结果只使用了 "破碎的" $H^2$ 半规范 $|\cdot|^\circ_{H^2}$ ，而不是 "等效的 "规范 $|\cdot|_h$  。这足以让我们相信，这个程序并没有从根本上被破坏。然而，测量我们有理论结果的实际规范的误差可能是有趣的。例如，使用FEInterfaceValues类与 MeshWorker::mesh_loop() 结合，实现这一补充应该不会太困难，其精神与我们用于装配线性系统的精神相同。
* 

* <a name="Derivationforthesimplysupportedplates"></a> <h4> Derivation for the simply supported plates </h4>。
* 

* 类似于实施中解决的 "夹持 "边界条件，我们将推导出 $C^0$ IP有限元方案，用于简单支撑的板。
* @f{align*}{
    \Delta^2 u(\mathbf x) &= f(\mathbf x)
    \qquad \qquad &&\forall \mathbf x \in \Omega,
    u(\mathbf x) &= g(\mathbf x) \qquad \qquad
    ...
    &&\forall \mathbf x \in \partial\Omega.
  @f}
* 我们用测试函数 $v_h$ 乘以偏谐方程，并对 $ K $ 进行积分，得到。
* @f{align*}{
...
-\int_{\partial K} (\nabla v_h) \cdot (\frac{\partial \nabla u_h}{\partial \mathbf{n}}).
  @f}
* 
* 在所有单元格 $K \in  \mathbb{T}$ 上求和，因为 $\Delta u_h$ 的法线方向在两个单元格和 $v_h = 0$ 共享的每条内边上指向相反方向。
* @f{align*}{
...
- \sum_{e \in \mathbb{F}^i} \int_{e} \jump{\frac{\partial v_h}{\partial \mathbf{n}}} (\frac{\partial^2 u_h}{\partial \mathbf{n^2}}) \ ds
  ...
  @f}
* 
*为了使离散问题对称化和稳定化，我们加入了对称化和稳定化项。 我们最终得到双调方程的 $C^0$ IP有限元方案：找到 $u_h$ ，使 $u_h =g$ 对 $\partial \Omega$ 和 
* @f{align*}{
  \mathcal{A}(v_h,u_h)&=\mathcal{F}(v_h) \quad \text{holds for all test functions } v_h,
  @f}
*其中 
