...
* 该计划是一个旨在模拟热声断层成像的项目的一部分。在热声断层扫描中，脉冲电磁能量被送入生物问题。组织吸收部分能量，组织中吸收能量最多的部分通过热弹性膨胀产生热声波。在成像方面，人们利用不同种类的组织，最重要的是健康和病变组织，吸收不同数量的能量，因此以不同的速度膨胀。实验设置是测量这些源在组织表面产生的压力波的振幅，并试图重建源的分布，这表明吸收器的分布，从而表明不同种类的组织。这个项目的一部分是将模拟数据与实际测量进行比较，因此必须解决 "前向问题"，即描述压力波在组织中传播的波浪方程。因此，本程序是 @ref
...
* 忽略热扩散，某一位置的温度可以最好地表示为
* @f[
\rho C_p \frac{\partial}{\partial t}T(t,\mathbf r) = H(t,\mathbf r)
@f]
* 这里 $\rho (\mathbf r) $ 是密度； $C_p (\mathbf r) $ 是比热； $\frac{\partial T}{\partial t}(t,\mathbf r)$ 是传递的微波能量带来的温升； $H(t,\mathbf r)$ 是加热函数，定义为由沉积的微波能量转化的每一时间和容积的热能。
* 让我们假设组织具有异质的介电特性，但具有同质的声学特性。声学同质介质中的基本声学生成方程可以描述如下：如果 $u$ 是矢量值的位移，那么组织肯定会对压力的变化做出加速反应：@f[
\rho \frac{\partial^2}{\partial t^2}u(t,\mathbf r) =
* 
-\nabla p(t,\mathbf r).
@f]此外，它由于压力过大而收缩，并根据温度的变化而膨胀：@f[
\nabla \cdot u(t,\mathbf r) =
* 
-\frac{p(t,\mathbf r)}{\rho c_0^2}+\beta T(t,\mathbf r) .
@f]这里， $\beta$ 是一个热膨胀的系数。
* 现在让我们假设，加热只发生在一个比波在组织中传播短得多的时间尺度上（即加热组织的微波脉冲的时间长度比波穿过该领域的时间短得多）。在这种情况下，加热速率 $H(t,\mathbf r)$ 可以写成 $H(t,\mathbf r) = a(\mathbf
r)\delta(t)$ （其中 $a(\mathbf r)$ 是对微波能量的吸收强度图， $\delta(t)$ 是狄拉克三角函数），与上述第一个方程式一起，将产生温度的瞬时跳跃 $T(\mathbf r)$  ] 在时间 $t=0$ 的瞬间跳动。利用这一假设，并将所有方程放在一起，我们可以将上述方程写成并合并为：@f[
\Delta p-\frac{1}{c_0^2} \frac{\partial^2 p}{\partial t^2} = \lambda
a(\mathbf r)\frac{d\delta(t)}{dt}
@f] 其中 $\lambda =
* 
- \frac{\beta}{C_p}$  。
* 这个有点奇怪的方程，右边是狄拉克三角函数的导数，可以改写成一个初值问题如下：@f{eqnarray*}
...
* 在逆向问题中，人们希望恢复的是初始条件 $b(\mathbf r) = c_0^2 \lambda a(\mathbf r)$ ，因为它是一张吸收微波能量的地图，因此可能是辨别健康和病变组织的指标。
...
* 一般来说，这是一个困难的问题：好的吸波边界条件是非线性的，而且/或者在数值上非常昂贵。因此，我们选择了一个简单的一阶近似吸收边界条件，即@f[
\frac{\partial\bar{p}}{\partial\mathbf n} =
* 
-\frac{1}{c_0} \frac{\partial\bar{p}}{\partial t}
@f]这里， $\frac{\partial\bar{p}}{\partial\mathbf n}$ 是边界的法向导数。应该指出的是，这不是一个特别好的边界条件，但它是为数不多的可以合理简单实现的边界条件之一。
* 

* <a name="Weakformanddiscretization"></a><h3>Weak form and discretization</h3> 。
* 

* 与 step-23 一样，首先引入第二个变量，定义为压力势的导数：@f[
...
@f} 。
* 注意，我们在这里引入了一个右手边 $f(t,\mathbf r)$ ，以显示如何在一般情况下推导出这些公式，尽管在应用于热声问题时 $f=0$ 。
* 使用 step-23 中介绍的一般 $\theta$ 方案，该模型的半简化、弱化版本是：@f{eqnarray*}
...
@f]被纳入弱形式。
* 由此，我们通过引入有限数量的形状函数得到离散模型，并得到@f{eqnarray*}
...
@f} 。
* 这里的矩阵 $M$ 和 $A$ 与 step-23 一样，边界质量矩阵@f[
	B_{ij} = \left(\varphi_i,\varphi_j\right)_{\partial\Omega}
@f]是使用吸收性边界条件的结果。
* 以上两个方程可以用矩阵形式重写，压力及其导数为未知矢量：@f[
\left(\begin{array}{cc}
 M         &
* 
...
\left(\begin{array}{l}
 ...
 (-c_{0}^{2}k (1-\theta)A+c_0 B)\bar{p}^{n-1} +Mv^{n-1}
                \end{array}\right)
@f] 。
* 通过简单的转换，就可以得到压力势及其导数的两个方程，就像在前面的教程程序中一样：@f{eqnarray*}
...
* 与 step-23 相比，本程序增加了对简单吸收边界条件的处理。此外，它还处理了从实际实验测量中获得的数据。为此，我们需要在实验也评估了真实压力场的点上评估解决方案。我们将看到如何使用 VectorTools::point_value 函数在下面进一步做到这一点。
* 
...
* 在推导波浪方程的初值问题时，我们最初发现该方程有一个狄拉克三角函数的导数作为右手边：@f[
\Delta p-\frac{1}{c_0^2} \frac{\partial^2 p}{\partial t^2} = \lambda
a(\mathbf r)\frac{d\delta(t)}{dt}.
@f]为了看看如何将这个单一的方程转化为具有初始条件的PDE的通常状态，让我们假设物理上相当合理的介质最初是静止的，即 $p(t,\mathbf
r)=\frac{\partial p(t,\mathbf r)}{\partial t}=0$ 为 $t<0$ 。接下来，让我们形成两边相对于时间的不确定积分：@f[
\int^t \Delta p\; dt
* 
...
=
\lambda a(\mathbf r).
@f]换言之，使用 $p(-\epsilon,\mathbf r)=0$  ]，我们找回了初始条件@f[
  \frac{1}{c_0^2} p(0,\mathbf r)
  =
  \lambda a(\mathbf r).
@f]。同时，我们知道对于每一个 $t>0$ ，delta函数都是零，所以对于 $0<t<T$ ，我们得到了方程@f[
\Delta p-\frac{1}{c_0^2} \frac{\partial^2 p}{\partial t^2} = 0.
@f]因此，我们从原来有点奇怪的方程中得到了波浪方程的表示和一个初始条件。
...
@f]形式的积分，其中我们使用 $\delta(\pm \epsilon)=0$ 并插入 $\varphi(t)=1$ ，我们看到，事实上@f[
...
-
                       \frac{\partial p(-\epsilon)}{\partial t}
		       \rightarrow 0.
@f]。然而，我们假设 $\frac{\partial p(-\epsilon)}{\partial t}=0$ .因此，我们得到作为第二个初始条件，@f[
...
* 
 @endcode
* 
* 这是唯一的新内容。我们将需要一个定义在GridTools命名空间的库函数来计算最小的单元格直径。
* 
...
* 
 @endcode
* 
* 以下是新的内容：首先，我们需要那个从吸收边界条件出来的边界质量矩阵 $B$ 。同样，由于这次我们考虑的是一个现实的介质，我们必须有一个衡量波速的标准 $c_0$ ，它将进入所有与拉普拉斯矩阵（我们仍然定义为 $(\nabla \phi_i,\nabla \phi_j)$ ）有关的公式。
* 
...
* 
* 像往常一样，我们必须定义我们的初始值、边界条件和右手函数。这次事情比较简单：我们考虑的是一个由初始条件驱动的问题，所以没有右手函数（尽管你可以在 step-23 中查找，看看如何做到这一点）。其次，没有边界条件：域的整个边界由吸收性边界条件组成。这就只剩下初始条件了，那里的事情也很简单，因为对于这个特殊的应用，只规定了压力的非零初始条件，而没有规定速度的非零初始条件（在初始时间是零）。  
...
* 
* 让我们再从构造函数开始。设置成员变量是很直接的。我们使用矿物油的声波速度（单位为毫米/微秒，是实验性生物医学成像中的常用单位），因为我们想和输出的许多实验都是在这里进行的。再次使用Crank-Nicolson方案，即theta被设定为0.5。随后选择时间步长以满足 $k =
 \frac hc$ ：这里我们将其初始化为一个无效的数字。
* 
...
* @code
 ...
 @endcode
* 
* 构造函数中的第二个任务是初始化存放探测器位置的数组。这个程序的结果与实验进行了比较，其中探测器间距的步长为2.25度，对应于160个探测器位置。扫描圆的半径被选择为中心和边界之间的一半，以避免不完善的边界条件的剩余反射破坏我们的数值结果。    
...
* 我们必须确保的一点是，时间步长满足 step-23 的介绍中讨论的CFL条件。在那个程序中，我们通过设置一个与网格宽度相匹配的时间步长来确保这一点，但是这很容易出错，因为如果我们再细化一次网格，我们也必须确保时间步长有所改变。在这里，我们自动做到了这一点：我们向一个库函数询问任何单元的最小直径。然后我们设置 $k=\frac h{c_0}$  。唯一的问题是： $h$ 到底是什么？问题是，对于波浪方程来说，在这个问题上确实没有好的理论。众所周知，对于由矩形组成的均匀细化网格， $h$ 是最小边长。但对于一般四边形的网格来说，确切的关系似乎是未知的，也就是说，不知道单元的什么属性与CFL条件有关。问题是，CFL条件来自于对拉普拉斯矩阵最小特征值的了解，而这只能对简单结构的网格进行分析计算。  
* 所有这些的结果是，我们并不十分确定我们应该对 $h$ 采取什么具体措施。函数 GridTools::minimal_cell_diameter 计算了所有单元的最小直径。如果单元格都是正方形或立方体，那么最小边长就是最小直径除以 <code>std::sqrt(dim)</code>  。我们简单地将此概括为非均匀网格的情况，没有理论上的理由。  
...
* 
* 以下两个函数，解决压力和速度变量的线性系统，几乎是逐字逐句地从 step-23 中提取的（除了主变量的名字从 $u$ 改为 $p$ ）。
* 
...
* 
* 这个做大部分工作的函数几乎又和 step-23 中一样，尽管我们通过使用介绍中提到的向量G1和G2使事情变得更清楚。与程序的整体内存消耗相比，引入几个临时向量并没有什么坏处。  
...
* 
* 剩下的就是程序的主要功能了。这里没有什么是以前的几个程序中没有显示过的。
* 
...
* 该程序将每个时间步骤的图形数据以及在每个检测器位置的评估值都写入磁盘。我们将它们绘制成图。目前，我们的实验只在二维空间内通过圆形扫描单个检测器完成。这里的组织样本是 $X-Y$ 平面上的一个薄片（ $Z=0$ ），我们假设其他 $Z$ 方向的信号不会对数据产生影响。因此，我们只需要将我们的实验数据与二维模拟数据进行比较。
*<a name="Oneabsorber"></a><h3> One absorber </h3>。
* 

* 这部电影显示了由一个小吸收器产生的热声波在介质中传播（在我们的模拟中，我们假设介质是矿物油，它的声速为1.437  $\frac{mm}{\mu s}$ ）。
...
* 接下来，让我们比较一下实验和计算的结果。可视化使用了地震学中长期使用的一种技术，即把每个探测器的数据都画在一个图中。这样做的方法是将每个检测器的信号与前一个检测器相比偏移一点。例如，这里是前四个探测器的图（从下到上，时间从左到右为微秒），使用程序中使用的源设置，与目前只有一个源的情况相比，使事情更有趣。
...
* 很明显，在角度 $180^\circ$ 时，源的位置更接近探测器。实验数据中可以看到的所有其他信号都是由于组织的其他部分也有弱吸收体，这些吸收体围绕着中心的小强吸收体产生的信号。另一方面，在模拟数据中，我们只模拟了小的强吸收体。
...
* 实验数据和我们的模拟数据在以下两张图中进行了比较。 <table width="100%">
<tr>
<td>
...
* 请注意，在实验数据中，第一个信号（即最左边的暗线）来自组织边界的吸收，因此在来自内部的任何信号之前，首先到达探测器。这个信号在痕迹的末端也是微弱可见的，大约在30 $\mu s$ ，这表明信号穿过整个组织到达另一侧的探测器，在所有来自内部的信号到达它们之后。
...
* 左图和右图之间的一个重要区别是，右图的曲线看起来没有那么多的 "棱角"。角度来自于这样一个事实：虽然连续方程中的波在各个方向上的移动速度相同，但离散化后的情况并非如此：在那里，对角线上的波与平行于网格线的波的移动速度略有不同。这种各向异性导致的波前并不是完全的圆形（在堆积图中会产生正弦信号），而是在某些方向上凸出。更糟糕的是，我们使用的圆形网格（例如见 step-6 的粗略网格视图）也不是各向同性的。最终的结果是，除非网格足够细，否则信号的前沿不是正弦波的。右图在这方面要好得多，尽管仍然可以看到尾部虚假波的形式的伪影。
* 
