...
* 这个程序最后是关于deal.II的主要特征之一：使用自适应（局部）细化网格。这个程序仍然基于 step-4 和 step-5 ，而且，正如你将看到的，它实际上不需要太多的代码来实现自适应。事实上，虽然我们做了大量的解释，但自适应网格可以被添加到一个现有的程序中，只需十几行的额外代码。该程序显示了这些行是什么，以及自适应网格细化（AMR）的另一个重要成分：一个标准，可以用来确定是否有必要细化一个单元，因为它的误差很大，是否可以粗化该单元，因为它的误差特别小，或者我们是否应该保持该单元的原状。我们将在下文中讨论所有这些问题。
* 
...
@f}
* (因为 $h_K\le h_\text{max}$ ，如果你只是把网格大小从总和中拉出来，这个公式马上就暗示了前面的公式。)这个公式所暗示的是，没有必要把<i>largest</i>单元做小，而是单元真的只需要做小<i>where $\| \nabla^{p+1} u \|_K$ is large</i>！换句话说。例如，如果我们使用一个线性元素 $p=1$ ，那么即使网格很粗，那些几乎是线性的地方（如 $\nabla^2 u$ 小）也会得到很好的解决。只有那些二阶导数大的地方才会被大的元素解决得很差，因此这就是我们应该把网格变小的地方。
...
* 悬挂节点约束的实践比我们上面概述的理论要简单得多。在现实中，你实际上只需要在 step-4 这样的程序中增加半打额外的代码，就可以使它与具有悬空节点的自适应网格一起工作。有趣的是，这与你要解决的方程完全无关。这些约束的代数性质与方程无关，只取决于有限元的选择。因此，处理这些约束的代码完全包含在deal.II库本身中，你不需要担心细节问题。
* 你需要使其工作的步骤基本上是这样的。
* 
...
* 事实证明，我们可以把迪里切特边界条件看作是对自由度的另一种约束。这的确是一个特别简单的约束。如果 $j$ 是边界上的一个自由度，其位置为 $\mathbf x_j$ ，那么在 $\partial\Omega$ 上施加边界条件 $u=g$ 就会产生约束 $U_j=g({\mathbf x}_j)$ 。
...
* 
* 由于用于局部细化网格的概念非常重要，我们在这个例子中没有展示很多其他材料。最重要的例外是，我们展示了如何使用双二次元，而不是之前所有例子中使用的双线性元素。事实上，使用高阶元素只需替换程序中的三行，即初始化本程序主类构造函数中的 <code>fe</code> 成员变量，以及在两处使用适当的正交公式。该程序的其他部分没有变化。
...
* 我们不会像前面的例子那样从文件中读取网格，而是使用库中的一个函数生成网格。然而，我们要在每一步中写出局部精炼的网格（只是网格，而不是解决方案），所以我们需要以下的include文件，而不是 <code>grid_in.h</code>  。
* 
...
* 
* 主类又是几乎没有变化的。然而，我们增加了两项内容：我们增加了 <code>refine_grid</code> 函数，该函数用于自适应地细化网格（而不是之前例子中的全局细化），以及一个变量，该变量将保持约束。
* 
...
* 
* 这个类的构造函数与之前的基本相同，但这次我们想使用二次元。为此，我们只需用所需的多项式度数（这里是 <code>2</code> ）替换构造函数的参数（在之前的所有例子中是 <code>1</code> ）。
* 
...
* 
* 下一个函数设置了所有描述线性有限元问题的变量，如DoFHandler、矩阵和向量。与我们在 step-5 中所做的不同的是，我们现在还必须处理悬挂节点约束。这些约束几乎完全由库来处理，也就是说，你只需要知道它们的存在以及如何获得它们，但你不必知道它们是如何形成的，也不必知道对它们到底做了什么。
* 

* 
* 在函数的开头，你会发现所有与 step-5 中相同的东西：设置自由度（这次我们有二次元，但从用户代码的角度来看，与线性的没有区别
* 
* - 或任何其他程度，对于这个问题
* 
...
* 
 ...
 @endcode
* 
* 现在我们已经准备好用指标0（整个边界）来插值边界值，并将得到的约束存储在我们的 <code>constraints</code> 对象中。请注意，我们并不像在前面的步骤中那样，在装配后应用边界条件：相反，我们将所有的约束条件放在AffineConstraints对象中的我们的函数空间。我们可以以任何顺序向AffineConstraints对象添加约束：如果两个约束发生冲突，那么约束矩阵要么中止，要么通过Assert宏抛出一个异常。
* 
...
* 
 @endcode
* 
* 在所有的约束被添加之后，它们需要被排序和重新排列，以更有效地执行一些动作。这种后处理是用 <code>close()</code> 函数完成的，之后就不能再添加任何约束了。
* 
...
* 
 @endcode
* 
* 现在我们首先建立我们的压缩稀疏模式，就像我们在以前的例子中做的那样。尽管如此，我们并没有立即将其复制到最终的稀疏度模式中。 请注意，我们调用了make_sparsity_pattern的一个变体，它把AffineConstraints对象作为第三个参数。我们通过将参数 <code>keep_constrained_dofs</code> 设置为false（换句话说，我们永远不会写入对应于受限自由度的矩阵条目），让该例程知道我们永远不会写入 <code>constraints</code> 所给的位置。如果我们要在组装后压缩约束，我们就必须通过 <code>true</code> 来代替，因为这样我们就会先写进这些位置，然后在压缩过程中再将它们设置为零。
* 
...
* 
* 构成局部贡献的其余代码保持不变。然而，值得注意的是，在引擎盖下，有几处地方与以前不同。首先，变量 <code>dofs_per_cell</code> 和 <code>quadrature_formula.size()</code> 的返回值现在各为9，而以前是4。引入这样的变量作为缩写是一个很好的策略，可以使代码与不同的元素一起工作，而不需要改变太多的代码。其次， <code>fe_values</code> 对象当然也需要做其他事情，因为现在的形状函数是二次的，而不是线性的，在每个坐标变量中。不过，这也是完全由库来处理的事情。
* 
...
* @code
 ...
 @endcode
* 
* 现在我们已经完成了线性系统的组装。约束矩阵照顾到了应用边界条件，也消除了悬挂的节点约束。受约束的节点仍然在线性系统中（在矩阵的对角线上有一个非零条目，选择的方式是使矩阵具有良好的条件，并且这一行的所有其他条目都被设置为零），但是计算出来的值是无效的（也就是说， <code>system_rhs</code> 中的相应条目目前是没有意义的）。我们在 <code>solve</code> 函数的最后为这些节点计算出正确的值。
* 
...
* 
* 我们继续逐步改进。解决线性系统的函数再次使用了SSOR预处理程序，而且除了我们必须加入悬空节点约束外，再次没有改变。如上所述，通过对矩阵的行和列进行特殊处理，从AffineConstraints对象中删除了对应于悬挂节点约束和边界值的自由度。这样一来，这些自由度的值在求解线性系统后就有了错误的、但定义明确的值。然后我们要做的就是利用约束条件给它们分配它们应该有的值。这个过程被称为 <code>distributing</code> 约束，从无约束的节点的值中计算出约束节点的值，只需要一个额外的函数调用，你可以在这个函数的末尾找到。
* 
...
* 
* 估算器的工作方式是将描述自由度的 <code>DoFHandler</code> 对象和每个自由度的数值向量作为输入，为三角测量的每个活动单元计算一个指标值（即每个活动单元一个数值）。为此，它需要两个额外的信息：一个面部正交公式，即 <code>dim-1</code> 维物体上的正交公式。我们再次使用3点高斯法则，这个选择与本程序中的双二次方有限元形状函数是一致和合适的。当然，什么是合适的正交规则取决于对误差估计器评估解场的方式的了解。如上所述，梯度的跃迁是在每个面上进行积分的，对于本例中使用的二次元元素来说，这将是每个面上的二次元函数。然而，事实上，它是梯度跳跃的平方，正如该类文件中所解释的那样，这是一个二次函数，对于它来说，3点高斯公式就足够了，因为它可以精确地整合5阶以下的多项式）。)
* 

* 
* 其次，该函数需要一个边界指示器的列表，用于那些我们施加了 $\partial_n u(\mathbf x) = h(\mathbf x)$ 类诺伊曼值的边界，以及每个此类边界的函数 $h(\mathbf
 x)$ 。这些信息通过一个从边界指标到描述诺伊曼边界值的函数对象的映射来表示。在本例程序中，我们不使用诺伊曼边界值，所以这个映射是空的，实际上是在函数调用期望得到相应函数参数的地方使用映射的默认构造器构造的。
* 

* 
* 输出是一个所有活动单元的值的向量。虽然非常精确地计算一个解的自由度的<b>value</b>可能是有意义的，但通常没有必要特别精确地计算一个单元上对应于解的<b>error
...
* 
 ...
 @endcode
* 
* <a name="Step6output_results"></a> <h4>Step6::output_results</h4> * <h4>Step6::output_results</h4>.
...
* 
* 我们已经在 step-1 中看到了如何实现对网格本身的输出。在这里，我们改变一些东西。  <ol>   <li>  我们使用两种不同的格式。gnuplot和VTU。 </li>   <li>  我们将周期号嵌入到输出文件名中。 </li>   <li>  对于gnuplot输出，我们设置了一个 GridOutFlags::Gnuplot 对象，以提供一些额外的可视化参数，使边缘看起来是弯曲的。这在  step-10  中有进一步的详细解释。 </li>   </ol> 。
* 
...
* 
*  <code>main()</code> 之前的最后一个函数又是该类的主要驱动， <code>run()</code>  。它与  step-5  的函数类似，只是我们在程序中再次生成一个文件，而不是从磁盘中读取，我们自适应地而不是全局地细化网格，并且我们在本函数中输出最终网格上的解。
* 
...
* @code
 ...
 @endcode
* 
* ...如果失败了，尽量收集更多的信息。具体来说，如果被抛出的异常是一个从C++标准类 <code>exception</code>, then we can use the <code>what</code> 派生出来的类的对象，可以通过成员函数得到一个字符串，这个字符串描述了异常被抛出的原因。  
* deal.II的异常类都是从标准类派生出来的，特别是 <code>exc.what()</code> 函数将返回与使用 <code>Assert</code> 宏抛出异常时产生的字符串大致相同的字符串。在前面的例子中，你已经看到了这种异常的输出，然后你知道它包含了异常发生的文件和行号，以及其他一些信息。这也是下面的语句会打印的内容。  
...
* 
 ...
 @endcode
* 
* 如果在某处抛出的异常不是从标准 <code>exception</code> 类派生出来的对象，那么我们根本就不能做什么。我们就简单地打印一个错误信息并退出。
* 
...
* 
* 如果一个人解决相当大的问题（比我们这里的问题大得多），有一件事总是值得一试的，那就是尝试不同的求解器或预处理器。在目前的情况下，线性系统是对称的和正定的，这使得CG算法几乎成了解题的典型选择。然而，我们在 <code>solve()</code> 函数中使用的SSOR预处理器是有待商榷的。
...
O}(N^{3/2})$ 的方式增长（对于几个最小的网格，CPU时间非常小，所以没有记录）。请注意，即使是简单的方法，Jacobi对于这个问题也是最快的。
...
* 如果你看一下上面的网格，你会发现尽管领域是一个圆盘，而且系数的跳动是沿着一个圆的，但是构成网格的单元并不能很好地跟踪这个几何形状。原因在 step-1 中已经提示过了，在没有其他信息的情况下，三角剖分类只看到一堆粗大的网格单元，但当然不知道它们在一起时可能代表什么样的几何。出于这个原因，我们需要告诉Triangulation当一个单元被细化时应该做什么：边缘中点和单元中点的新顶点应该位于哪里，以便子单元比父单元更好地代表所需的几何图形。
...
@endcode
* 
* 在上面的代码中，我们已经对位于边界的面做了这个处理：由于我们使用了 GridGenerator::hyper_ball, ，它将SphericalManifold附加到域的边界上，所以这就自动发生了。为了使网格<i>interior</i>也能追踪到一个圆形域，我们需要更努力一些。首先，回顾一下我们的粗略网格是由一个中心的正方形单元和周围的四个单元组成的。现在首先考虑一下，如果我们将球形网格对象不仅连接到四个外部面，而且连接到周边的四个单元以及它们的所有面，会发生什么。我们可以通过添加下面的片段来实现这一点（测试一个单元格的中心是否大于单元格直径的一个小倍数，比如说十分之一，只对网格中心的方形失效）。
* @code
GridGenerator::hyper_ball(triangulation);
* 
...
* 创建好的网格，特别是使它们符合你想要的几何形状，本身就是一个复杂的话题。你可以在  step-49  ,  step-53  , 和  step-54  中找到更多关于这个问题的教程。  step-65 展示了另一种不太需要手工操作的方法来实现对这里的问题的网格井拟合。关于弯曲域的信息也可以在 @ref manifold 的文档模块 "流形描述 "中找到。
...
* 
*-它应该与系数的跳动保持一致。但是我们也可以在一个更普遍的环境中提出这样的问题。给出一些具有光滑解和光滑系数的方程，我们能说一个好的网格是什么样子的吗？这个问题的答案在直觉上比数学上更容易表述。一个好的网格所包含的单元大体上都像正方形（或立方体，在三维空间）。一个不好的网格会包含一些在某些方向上非常细长的单元，或者，更广泛地说，有一些单元的边缘既短又长。有很多方法可以给每个单元分配一个数字质量指数，以衡量该单元是 "好 "还是 "坏"；其中一些方法经常被选择，因为它们便宜且容易计算，而另一些方法则是基于收敛性证明的内容。前者的一个例子是一个单元格的最长边与最短边的比率。在理想的情况下，这个比率应该是1；不好的单元格的数值远远大于1。后者的例子是考虑从参考单元 $\hat K=[0,1]^d$ 到实际单元 $K$ 的映射的梯度（"雅各布"）；这个梯度是一个矩阵，进入误差估计的一个数量是参考单元上所有点的最大比率，即这个矩阵的最大和最小的特征值。同样不难看出，如果单元格 $K$ 是 $\hat K$ 的仿生图像，这个比率是恒定的，对于正方形和立方体来说，这个比率是一个。
...
* 
-\Delta u = f
@f]在光滑有界的凸域上的解是已知的，其本身就是光滑的。平滑性的确切程度，即解所处的函数空间，取决于域的边界到底有多平滑，以及右手边的平滑程度。特别是如果右手边满足 $f\in C^\infty(\Omega)$ ，那么 $u \in C^\infty(\Omega_i)$ ，其中 $\Omega_i$ 是 $\Omega$ 的任何紧凑子集（ $\Omega$ 是一个开放域，所以紧凑子集需要与 $\partial\Omega$ 保持一个正距离），解的一些规律性可能会在边界上丢失，但一般来说，解是比右手边多一倍的可微积分。
