...
namespace TriaAccessorExceptions
{
  ...
  DeclExceptionMsg(ExcCellNotUsed,
                   ...
                   "currently unused.)");
  /**
   * The cell is not an     @ref GlossActive   "active"
  ...
  /**
   * You are trying to get the periodic neighbor for a face, which does not   have a periodic neighbor. For more information on this, refer to     @ref GlossPeriodicConstraints   "entry for periodic boundaries".
   * @ingroup Exceptions
   *
   */
 ...
 * classes. This class offers only the basic functionality required by the
 * iterators (stores the necessary data members, offers comparison operators
 * and the like), but has no functionality to actually dereference data. This
 * is done in the derived classes. In the implementation, the behavior of this
 * class differs between the cases where <tt>structdim==dim</tt> (cells of a
 * mesh) and <tt>structdim&lt;dim</tt> (faces and edges). For the latter,
 * #present_level is always equal to zero and the constructors may not receive
 * a positive value there. For cells, any level is possible, but only those
 * within the range of the levels of the Triangulation are reasonable.
 * Furthermore, the function objects() returns either the container with all
 * cells on the same level or the container with all objects of this dimension
 * (<tt>structdim&lt;dim</tt>).
 * Some internals
 * of this class are discussed in   @ref IteratorAccessorInternals  .
 *
 *
 * @ingroup grid
 *
...
public:
   ...
   * object pointed to   by   @p b   to the object pointed to by   @p a.
   * However, the result of   dereferencing an iterator is not an object but
   * an accessor; consequently,   this operation is not useful for iterators
   * on triangulations.   Consequently, this operator is declared as deleted
   * and can not be used.
   *
   */
  void
...
protected:
   ...
   * anything, since the iterator will return itself.     This method is
   * protected, since it is only to be called from the iterator   class.
   *
   */
  void
   ...
   * them into a   `std::map`.       If #structure_dimension is less than
   * #dimension, we simply compare the   index of such an object because faces
   * and edges do not have levels. If   #structure_dimension equals
   * #dimension, we compare the level first, and   the index only if levels
   * are equal.
   *
   */
  bool
...
protected:
   ...
   */
  /**
   * This operator advances the iterator to the next element.     For   @p
   * dim=1   only: The next element is next on this level if there are   more.
   * If the present element is the last on this level, the first on the   next
   * level is accessed.
   *
   */
  void
  ...
  /**
   * This operator moves the iterator to the previous element.     For   @p
   * dim=1   only: The previous element is previous on this level if
...
public:
   ...
   * level.     Within a Triangulation object, cells are uniquely identified
   * by a pair     <code>(level, index)</code>   where the former is the
   * cell's refinement   level and the latter is the index of the cell within
   * this refinement   level (the latter being what this function returns).
   * Consequently, there   may be multiple cells on different refinement
   * levels but with the same   index within their level. Contrary to this, if
   * the current object   corresponds to a face or edge, then the object is
   * uniquely identified   solely by its index as faces and edges do not have
   * a refinement level.
   * @note   The indices objects returned by this function are not a
   * contiguous   set of numbers on each level: going from cell to cell, some
   * of the   indices in a level may be unused.
   * @note   If the triangulation is actually of type
   * parallel::distributed::Triangulation   then the indices are relatively
   * only   to that part of the distributed triangulation that is stored on
   * the   current processor. In other words, cells living in the partitions
   * of the   triangulation stored on different processors may have the same
   * index even   if they refer to the same cell, and the may have different
   * indices even   if they do refer to the same cell (e.g., if a cell is
   * owned by one   processor but is a ghost cell on another).
   *
   */
  int
  index() const;
  ...
  /**
   * Return a reference to the triangulation which the object pointed to by
 ...
 * on quad iterators that is dimension independent
 *
 *  -  i.e., also compiles in 1d
 *
 *  -  because quad iterators (via the current class) exist and are syntactically correct. You can not expect, however, to ever create an actual object of one of these iterators in 1d, meaning you need to expect to wrap the code block in which you use quad iterators into something like   <code>if (dim@>1)</code>
 *
...
public:
  ...
  /**
   * Constructor.  This class is used for iterators that do not make   sense
   * in a given dimension, for example quads for 1d meshes. Consequently,
   ...
   * sense in a given dimension, for example quads for 1d meshes.
   * Consequently,   while the creation of such objects is syntactically
   * valid, they make no   semantic sense, and we generate an exception when
   * such an object is   actually generated.
   *
   */
 ...
 * represents objects of dimension   <code>structdim</code>   (i.e. 1 for
 * lines, 2 for quads, 3 for hexes) in a triangulation of dimensionality
 ...
 * manifold embedded in a higher dimensional space). There is a specialization
 * of this class for the case where   @p structdim   equals zero, i.e., for
...
public:
  ...
  /**
   * The copy constructor is not deleted but copied constructed elements
   * should   not be modified, also the comments to the copy assignment
   * operator.
   *
   */
  ...
  /**
   * Test for the element being used or not.  The return value is   @p true
   * for   all iterators that are either normal iterators or active iterators,
   * only   raw iterators can return   @p false.   Since raw iterators are
   * only used in   the interiors of the library, you will not usually need
   * this function.
   *
   */
  bool
  ...
  /**
   * Pointer to the   @p ith   vertex bounding this object. Throw an exception
   * if     <code>dim=1</code>  .
   *
   */
   ...
   * documentation of the GeometryInfo class.     Note that the returned value
   * is only the index of the geometrical vertex.   It has nothing to do with
   * possible degrees of freedom associated with it.   For this, see the   @p
   ...
   * vertex remains useful
   *
   *  -  for example, avoiding inverted or otherwise   distorted (see also     @ref GlossDistorted   "this glossary entry").
   ...
   * @note   The behavior described above is relevant if you have a     parallel::distributed::Triangulation   object. There, refining a mesh   always involves a re-partitioning. In other words, vertices of locally   owned cells (see     @ref GlossLocallyOwnedCell   "this glossary entry")
  ...
  /**
   * Line index of the   @p ith   line bounding this object.     Implemented
   * only for <tt>structdim>1</tt>, otherwise an exception   generated.
   *
   */
  unsigned int
  ...
  /**
   * Quad index of the   @p ith   quad bounding this object.     Implemented
   * only for <tt>structdim>2</tt>, otherwise an exception   generated.
   *
   */
  unsigned int
  ...
  /**
   * Return whether the face with index   @p face   has its normal pointing in
   * the   standard direction (  @p true)   or whether it is the opposite (
   * @p false).     Which is the standard direction is documented with the
   * GeometryInfo   class. In 1d and 2d, this is always   @p true,   but in 3d
   * it may be   different, see the respective discussion in the documentation
   * of the   GeometryInfo class.     This function is really only for
   * internal use in the library unless you   absolutely know what this is all
   * about.
   *
   */
  bool
  ...
  /**
   * Return whether the face with index   @p face   is rotated by 180 degrees
   * (  @p     true) or not (  @p false).   In 1d and 2d, this is always   @p
   * false,   but in   3d it may be different, see the respective discussion
   * in the   documentation of the GeometryInfo class.     This function is
   * really only for internal use in the library unless you   absolutely know
  ...
  /**
   * Return whether the face with index   @p face   is rotated by 90 degrees (
   * @p     true) or not (  @p false).   In 1d and 2d, this is always   @p
   * false,   but in   3d it may be different, see the respective discussion
   * in the   documentation of the GeometryInfo class.     This function is
   * really only for internal use in the library unless you   absolutely know
   ...
   * direction.   @p true   indicates, that the line is oriented from vertex 0
   * to   vertex 1, whereas it is the other way around otherwise. In 1d and
   * 2d,   this is always   @p true,   but in 3d it may be different, see the
   * respective   discussion in the documentation of the GeometryInfo class.
   * This function is really only for internal use in the library unless you
   * absolutely know what this is all about.     This function queries
   ...
   * If the present cell is not refined, one is returned.     If one considers
   * a triangulation as a forest where the root of each tree   are the coarse
   * mesh cells and nodes have descendants (the children of a   cell), then
   * this function returns the number of terminal nodes in the   sub-tree
   * originating from the current object; consequently, if the   current
   * object is not further refined, the answer is one.
   *
   */
  unsigned int
   ...
   * branch of children of this object.     For example, if this object is
   * refined, and one of its children is   refined exactly one more time, then
   * <tt>max_refinement_depth</tt> should   return 2.     If this object is
   * not refined (i.e. it is active), then the return value   is zero.
   *
   */
  unsigned int
  ...
  /**
   * Index of the   @p ith   child. The level of the child is one higher than
   * that   of the present cell, if the children of a cell are accessed. The
   * children   of faces have no level. If the child does not exist,
   *
   *  -  is returned.
   *
   */
  ...
  /**
   * Index of the   @p ith   isotropic child. See the isotropic_child()
   * function   for a definition of this concept.  If the child does not
   * exist,
   *
   *  -  is   returned.
   *
  ...
  /**
   * Return the boundary indicator of this object.     If the return value is the special value     numbers::internal_face_boundary_id,   then this object is in the interior   of the domain.       @see       @ref GlossBoundaryIndicator   "Glossary entry on boundary indicators"
   *
   */
  types::boundary_id
  boundary_id() const;
   ...
   * the boundary_id() function.     This function only sets the boundary
   * object of the current object itself,   not the indicators of the ones
   * that bound it. For example, in 3d, if this   function is called on a
   * face, then the boundary indicator of the 4 edges   that bound the face
   * remain unchanged. If you want to set the boundary   indicators of face
   * and edges at the same time, use the   set_all_boundary_ids() function.
   * You can see the result of not using the   correct function in the results
   * section of   step-49  .       @warning   You should never set the
   * boundary indicator of an interior face   (a face not at the boundary of
   * the domain), or set the boundary   indicator of an exterior face to
   * numbers::internal_face_boundary_id   (this   value is reserved for
   * another purpose). Algorithms may not work or   produce very confusing
   * results if boundary cells have a boundary   indicator of
   * numbers::internal_face_boundary_id   or if interior cells have   boundary
   ...
   * set to the given value.     This function is useful if you set boundary
   * indicators of faces in 3d (in   2d, the function does the same as
   * set_boundary_id()) and you do so   because you want a curved boundary
   * object to represent the part of the   boundary that corresponds to the
   * current face. In that case, the   Triangulation class needs to figure out
   * where to put new vertices upon   mesh refinement, and higher order
   * Mapping objects also need to figure out   where new interpolation points
   * for a curved boundary approximation should   be. In either case, the two
   * classes first determine where interpolation   points on the edges of a
   * boundary face should be, asking the boundary   object, before asking the
   * boundary object for the interpolation points   corresponding to the
   * interior of the boundary face. For this to work   properly, it is not
   * sufficient to have set the boundary indicator for the   face alone, but
   * you also need to set the boundary indicators of the edges   that bound
   * the face. This function does all of this at once. You can see   the
   * result of not using the correct function in the results section of
   * step-49  .
   * @ingroup boundary         @see       @ref GlossBoundaryIndicator   "Glossary entry on boundary indicators"
   *
   */
  ...
  /**
   * Return a
   * constant reference to the manifold object used for this object.     As
   * explained in the     @ref manifold     module, the process involved in
   * finding the appropriate manifold   description involves querying both the
   * manifold or boundary   indicators. See there for more information.
   *
   */
  ...
  /**
   * Return the manifold indicator of this object.     If the return value is the special value   numbers::flat_manifold_id,   then   this object is associated with a standard Cartesian Manifold Description.       @see       @ref GlossManifoldIndicator   "Glossary entry on manifold indicators"
   *
   */
  types::manifold_id
  manifold_id() const;
   ...
   * <tt>manifold_id()</tt> function.     Note that it only sets the manifold
   * object of the current object itself,   not the indicators of the ones
   * that bound it, nor of its children. For   example, in 3d, if this
   * function is called on a face, then the manifold   indicator of the 4
   * edges that bound the face remain unchanged. If you   want to set the
   * manifold indicators of face, edges and all children at   the same time,
   * use the set_all_manifold_ids() function.
  ...
  /**
   * Read the
   * user flag. See     @ref GlossUserFlags     for more information.
   *
   */
  bool
  user_flag_set() const;
  ...
  /**
   * Set the
   * user flag for this and all descendants. See     @ref GlossUserFlags
  ...
  /**
   * Reset the
   * user data to zero, independent if pointer or index. See     @ref
   ...
   * @note
   * User pointers and user indices are mutually exclusive. Therefore,   you
   * can only use one of them, unless you call
   * Triangulation::clear_user_data()   in between.     See     @ref
   * GlossUserData     for more information.
   *
   */
  void
  ...
  /**
   * Reset the
   * user pointer to a   @p nullptr   pointer. See     @ref GlossUserData
   * for more information.
   *
   */
  void
   ...
   * a=static_cast<A*>(cell->user_pointer());</tt>.     See     @ref
   * GlossUserData     for more information.
   *
   */
  void *
  ...
  /**
   * Clear the
   * user pointer of this object and all of its descendants. The   same holds
   * as said for the recursively_set_user_pointer() function. See     @ref
   * GlossUserData     for more information.
   *
   */
  void
   ...
   * User pointers and user indices are mutually exclusive. Therefore,   you
   * can only use one of them, unless you call
   * Triangulation::clear_user_data()   in between. See     @ref GlossUserData
   * for more information.
   *
   */
  void
  ...
  /**
   * Reset the
   * user index to 0. See     @ref GlossUserData     for more information.
   *
   */
  void
  ...
  /**
   * Set the user index of this object and all its children.     Note that the
   * user index is not inherited under mesh refinement, so after   mesh
   * refinement there might be cells or faces that don't have the   expected
   * user indices. In this case, simply loop over all the elements of   the
   * coarsest level that has this information, and use this function to
  ...
  /**
   * Clear the
   * user index of this object and all of its descendants. The same   holds as
   * said for the recursively_set_user_index() function.     See     @ref
   * GlossUserData     for more information.
   *
   */
  void
  ...
  /**
   * Diameter of the object.     The diameter of an object is computed to be
   * the largest diagonal of the   current object. If this object is a
   * quadrilateral, then there are two   such diagonal, and if it is a
   * hexahedron, then there are four diagonals   that connect "opposite"
   * points. For triangles and tetrahedra, the function   simply returns the
   * length of the longest edge.     The situation is more difficult for
   * wedges and pyramids: For wedges, we   return the length of the longest
   * diagonal of the three quadrilateral faces   or the longest edge length of
   * the two triangular faces. For pyramids,   the same principle is applied.
   * In all of these cases, this definition of "diameter" is   not necessarily
   * the true diameter in the sense of the largest distance   between points
   * inside the object. Indeed, one can often construct objects   for which it
   * is not, though these are generally quite deformed compared to   the
   * reference shape. Furthermore, for objects that may use higher order
   * mappings, one may have bulging faces that also create trouble for
   * computing an exact representation of the diameter of the object. That
   * said,   the definition used above is completely sufficient for most
   * computations.
   *
   */
  double
  ...
  /**
   * Return a pair of Point and double corresponding to the center and   the
   * radius of a reasonably small enclosing ball of the object.     The
   * function implements Ritter's O(n) algorithm to get a reasonably   small
   * enclosing ball around the vertices of the object.   The initial guess for
   * the enclosing ball is taken to be the ball   which contains the largest
   * diagonal of the object as its diameter.   Starting from such an initial
   * guess, the algorithm tests whether all   the vertices of the object
   * (except the vertices of the largest diagonal)   are geometrically within
   * the ball.   If any vertex (v) is found to be geometrically outside the
   * ball,   a new iterate of the ball is constructed by shifting its center
   * and   increasing the radius so as to geometrically enclose both the
   * previous   ball and the vertex (v). The algorithm terminates when all the
   * vertices   are geometrically inside the ball.     If a vertex (v) is
   * geometrically inside a particular iterate of the ball,   then it will
   * continue to be so in the subsequent iterates of   the ball (this is true
   * \a by \a construction).
  ...
  /**
   * Return the smallest bounding box that encloses the object.     Notice
   * that this method is not aware of any mapping you may be using to   do
   * your computations. If you are using a mapping object that modifies the
   ...
   * meaning and enumeration of the local axes.     Note that the "length" of
   * an object can be interpreted in a variety of   ways. Here, we choose it
   * as the maximal length of any of the edges of the   object that are
   * parallel to the chosen axis on the reference cell.
   *
   */
  double
   ...
   * object.     Notice that the argument   @p coordinates   are the
   * coordinates on the     <em>  reference cell  </em>  , given in reference
   * coordinates. In other words,   the argument provides a weighting between
   * the different vertices. For   example, for lines, calling this function
   * with argument Point<1>(.5), is   equivalent to asking the line for its
   * center.
   *
   */
  Point<spacedim>
   ...
   * the unit cell by inversion of an affine approximation of the   $d$
   * -linear   function from the reference   $d$  -dimensional cell.     The
   * affine approximation of the unit to real cell mapping is found by a
   ...
   * for Cartesian and affine (parallelogram) meshes in 2D/3D.     For exact
   * transformations to the unit cell, use
   ...
   * lie within the manifold, even if the manifold is curved.     When the
   * object uses a different manifold description as its surrounding,   like
   * when part of the bounding objects of this TriaAccessor use a   non-flat
   * manifold description but the object itself is flat, the result   given by
   * the   TriaAccessor::center()   function may not be accurate enough,
   * even when parameter   @p respect_manifold   is set to true. If you find
   * this   to be case, than you can further refine the computation of the
   * center by   setting to true the second additional parameter   @p
   ...
   * objects. For a 2D object, it puts a weight of   <code>1/2</code>   on
   * each of   the four surrounding lines and a weight   <code>-1/4</code>
   * on the four   vertices. This corresponds to a linear interpolation
   * between the   descriptions of the four faces, subtracting the
   * contribution of the   vertices that is added twice when coming through
   * both lines adjacent to   the vertex. In 3D, the weights for faces are
   * <code>1/2</code>  , the   weights for lines are   <code>-1/4</code>  ,
   * and the weights for vertices are     <code>1/8</code>  . For further
   * information, also confer to the   TransfiniteInterpolationManifold class
   * that is able to not only apply   this beneficial description to a single
   * cell but all children of a coarse   cell.
   *
   */
  Point<spacedim>
  ...
  /**
   * Return the barycenter (also called centroid)   of the object. The barycenter for an object   $K$     of dimension   $d$   in   $D$   space dimensions is given by the   $D$  -dimensional   vector   $\mathbf x_K$   defined by   @f[
   * \mathbf x_K = \frac{1}{|K|} \int_K \mathbf x \; \textrm{d}x
   * @f]   where the measure of the object is given by   @f[
   * |K| = \int_K \mathbf 1 \; \textrm{d}x. @f]   This function assumes that
   * $K$   is mapped by a   $d$  -linear function from   the reference   $d$
   * -dimensional cell. Then the integrals above can be   pulled back to the
   * reference cell and evaluated exactly (if through   lengthy and, compared
   * to the center() function, expensive computations).
   *
   */
  Point<spacedim>
  barycenter() const;
   ...
   * object occupies. For a one-dimensional object, return its length.     The
   * function only computes the measure of cells, faces or edges assumed   to
   * be represented by (bi-/tri-)linear mappings. In other words, it only
...
private:
  ...
  /**
   * Set the indices of those objects that bound the current   object. For
   * example, if the current object represents a cell,   then the argument
   * denotes the indices of the faces that bound the   cell. If the current
   * object represents a line, the argument   denotes the indices of the
   * vertices that bound it. And so on.
   *
   */
  void
   ...
   * return.     It is only possible to set the line_orientation of faces in
   * 3d (i.e.     <code>structdim==2 && dim==3</code>  ).
   *
   */
  void
  ...
  /**
   * Set whether the quad with index   @p face   has its normal pointing in
   * the   standard direction (  @p true)   or whether it is the opposite (
   * @p false).     Which is the standard direction is documented with the
   * GeometryInfo   class.     This function is only for internal use in the
   * library. Setting this flag   to any other value than the one that the
   * triangulation has already set is   bound to bring you disaster.
   *
   */
  void
  ...
  /**
   * Set the flag indicating, what   <code>face_rotation()</code>   will
   * return.     It is only possible to set the face_orientation of cells in
   * 3d (i.e.     <code>structdim==3 && dim==3</code>  ).
   *
   */
  void
   ...
   * 2 children lines (isotropic refinement).     You should know quite
   * exactly what you are doing if you touch this   function. It is
   * exclusively for internal use in the library.
   *
   */
  void
   ...
   * RefinementCase<dim>::no_refinement.       You should know quite exactly
   * what you are doing if you touch this   function. It is exclusively for
 ...
 * spacedim></code> for the case that   @p structdim   is zero. This class
 * represents vertices in a triangulation of dimensionality   <code>dim</code>
 * (i.e. 1 for a triangulation of lines, 2 for a triangulation of quads, and 3
 * for a triangulation of hexes) that is embedded in a space of dimensionality
 ...
 * a higher dimensional space). There is a further specialization of this
 * class for the case that   @p dim   equals one, i.e., for vertices of a
 * one-dimensional triangulation, since in that case vertices are also faces.
 *
 *
 * @ingroup Accessors
 *
...
public:
   ...
   * compatibility with the other accessor classes.   @p index   can be used
   * to   set the global index of the vertex we point to.
   *
   */
  ...
  /**
   * Return the global index of i-th vertex of the current object. If   @p i
   * is   zero, this returns the index of the current point to which this
   * object   refers. Otherwise, it throws an exception.     Note that the
   * returned value is only the index of the geometrical vertex.   It has
   * nothing to do with possible degrees of freedom associated with it.   For
   * this, see the   @p DoFAccessor::vertex_dof_index   functions.
   * @note   Despite the name, the index returned here is only global in the
   * sense that it is specific to a particular Triangulation object or, in the
   * case the triangulation is actually of type
   * parallel::distributed::Triangulation,   specific to that part of the
  ...
  /**
   * Pointer to the   @p ith   line bounding this object. Will point to an
   * invalid   object.
   *
   */
  typename dealii::internal::TriangulationImplementation::
  ...
  /**
   * Line index of the   @p ith   line bounding this object. Throws an
   * exception.
   *
   */
  static unsigned int
  ...
  /**
   * Quad index of the   @p ith   quad bounding this object. Throws an
   * exception.
   *
   */
  static unsigned int
   ...
   * Length of an object in the direction of the given axis, specified in the
   * local coordinate system. See the documentation of GeometryInfo for the
   * meaning and enumeration of the local axes.     This function always
   * returns zero.
   *
   */
  double
   ...
   * respect_manifold and   @p interpolate_from_surrounding   are not used.
   * They   are there to provide the same interface as
...
protected:
   ...
   * Comparison operator for accessors. This operator is used when comparing
   * iterators into objects of a triangulation, for example when putting
   * them into a   `std::map`.       This operator simply compares the global
   * index of the vertex the   current object points to.
   *
   */
  bool
...
/**
 * This class is a specialization of <code>TriaAccessor<structdim, dim,
 * spacedim></code> for the case that   @p structdim   is zero and   @p dim
 * is one. This class represents vertices in a one-dimensional triangulation
 * that is embedded in a space of dimensionality   <code>spacedim</code>
 * (for   <code>spacedim==dim==1</code>   the triangulation represents a
 * domain in   ${\mathbb R}^\text{dim}$  , for   <code>spacedim@>dim==1</code>
 * the triangulation is of a manifold embedded in a higher dimensional space).
 * The current specialization of the TriaAccessor<0,dim,spacedim> class for
 * vertices of a one-dimensional triangulation exists since in the   @p dim
...
public:
  ...
  /**
   * Constructor.     Since there is no mapping from vertices to cells, an
   * accessor object for   a point has no way to figure out whether it is at
   * the boundary of the   domain or not. Consequently, the second argument
   ...
   * refers. Otherwise, it throws an exception.     Note that the returned
   * value is only the index of the geometrical vertex.   It has nothing to do
   * with possible degrees of freedom associated with it.   For this, see the
   * @p DoFAccessor::vertex_dof_index   functions.
   * @note   Despite the name, the index returned here is only global in the
   * sense that it is specific to a particular Triangulation object or, in the
   * case the triangulation is actually of type
   * parallel::distributed::Triangulation,   specific to that part of the
  ...
  /**
   * Return the manifold indicator of this object.       @see       @ref GlossManifoldIndicator   "Glossary entry on manifold indicators"
   *
   */
  types::manifold_id
  manifold_id() const;
   ...
   * <tt>boundary_id()</tt> function.       @warning   You should never set
   * the boundary indicator of an interior face   (a face not at the boundary
   * of the domain), or set the boundary   indicator of an exterior face to
   * numbers::internal_face_boundary_id   (this   value is reserved for
   * another purpose). Algorithms may not work or   produce very confusing
   * results if boundary cells have a boundary   indicator of
   * numbers::internal_face_boundary_id   or if interior cells have   boundary
 ...
 * dimension, etc. The following refers to any dimension: This class allows
 * access to a <tt>cell</tt>, which is a line in 1D and a quad in 2D. Cells
 * have more functionality than lines or quads by themselves, for example they
 * can be flagged for refinement, they have neighbors, they have the
 * possibility to check whether they are at the boundary etc. This class
...
public:
  ...
  /**
   * Return a pointer to the   @p ith   child. Overloaded version which
   * returns a   more reasonable iterator class.
   *
   */
   ...
   * given face and subface number.     To succeed, the present cell must not
   * be further refined, and the   neighbor on the given face must be further
   * refined exactly once; the   returned cell is then a child of that
   * neighbor.     The function may not be called in 1d, since there we have
   * no subfaces.   The implementation of this function is rather
   * straightforward in 2d, by   first determining which face of the neighbor
   * cell the present cell is   bordering on (this is what the   @p
   * neighbor_of_neighbor   function does),   and then asking   @p
   * GeometryInfo::child_cell_on_subface   for the index of   the child.
   ...
   * @p face_flip   and   @p face_rotation   for both this and the neighbor
   * cell to   figure out which cell we want to have.     This can lead to
   * surprising results: if we are sitting on a cell and are   asking for a
   * cell behind subface <tt>sf</tt>, then this means that we are
   ...
   * @p true,   and reversed otherwise.     Similarly, effects of
   * <tt>face_flip()==true</tt> and   <tt>face_rotation()==true()</tt>, both
   * of which indicate a non-standard   face have to be considered.
   ...
   * neighbor's child is behind subface   @p subface   concerning the
   * intrinsic   ordering of the given face.
   *
   */
  TriaIterator<CellAccessor<dim, spacedim>>
  ...
  /**
   Return an iterator to the neighboring cell on the other side of the face   with number   @p face_no.   If the neighbor does not exist,   i.e., if the face with number   @p face_no   of the current object is at the boundary, then   an invalid iterator is returned.     Consequently, the index   @p face_no   must be less than n_faces().     The neighbor of a cell has at most the same level as this cell. For   example, consider the following situation:     @image html limit_level_difference_at_vertices.png ""     Here, if you are on the top right cell and you ask for its left neighbor   (which is, according to the conventions spelled out in the GeometryInfo   class, its <i>zeroth</i> neighbor), then you will get the mother cell of   the four small cells at the top left. In other words, the cell you get as   neighbor has the same refinement level as the one you're on right now   (the top right one) and it may have children.     On the other hand, if you were at the top right cell of the four small   cells at the top left, and you asked for the right neighbor (which is   associated with index   <code>face_no=1</code>  ), then you would get the large   cell at the top right which in this case has a lower refinement level and   no children of its own.
   *
   */
  TriaIterator<CellAccessor<dim, spacedim>>
  ...
  /**
   * Return the cell index of the neighboring cell on the other side of the
   * face   with index   @p face_no.   If the neighbor does not exist, this
   * function returns
   *
  ...
  /**
   * Return the level of the neighboring cell on the other side of the face
   * with   number   @p face_no.   If the neighbor does not exist, this
   * function returns
   *
   ...
   * <tt>cell->neighbor(face_no)</tt>, i.e. return   @p other_face_no   such
   * that   <tt>cell->neighbor(face_no)->neighbor(other_face_no)==cell</tt>.
   * This   function is the right one if you want to know how to get back from
   * a   neighbor to the present cell.     Note that this operation is only
   * useful if the neighbor is not coarser   than the present cell. If the
   * neighbor is coarser this function throws an   exception. Use the   @p
   * neighbor_of_coarser_neighbor   function in that case.
   *
   */
  unsigned int
   ...
   * not depend on the levels of the cells.     Note, that in an anisotropic
   * setting, a cell can only be coarser than   another one at a given face,
   * not on a general basis. The face of the   finer cell is contained in the
   * corresponding face of the coarser cell,   the finer face is either a
   * child or a grandchild of the coarser face.
   *
   */
  bool
   ...
   * function   for the case of a coarser neighbor. It returns a pair of
   * numbers, face_no   and subface_no, with the following property, if the
   * neighbor is not   refined:
   * <tt>cell->neighbor(neighbor)->neighbor_child_on_subface(face_no,
   ...
   * the   step-30   tutorial program.     This function is impossible for
   * <tt>dim==1</tt>.
   *
   */
  ...
  /**
   * This function is a generalization of the   @p neighbor_of_neighbor   and
   * the     @p neighbor_of_coarser_neighbor   functions. It checks whether
   * the neighbor   is coarser or not and calls the respective function. In
   * both cases, only   the face_no is returned.
   *
   */
  unsigned int
  ...
  /**
   * For a cell with its   @c   ith face at a periodic boundary,   see     @ref GlossPeriodicConstraints   "the entry for periodic boundaries",
   * this function returns an iterator to the cell on the other side   of the
   * periodic boundary. If there is no periodic boundary at the   @c   ith
   * face, an exception will be thrown.   In order to avoid running into an
   * exception, check the result of   has_periodic_neighbor() for the   @c
   * ith face prior to using this function.   The behavior of
   * periodic_neighbor() is similar to neighbor(), in   the sense that the
   * returned cell has at most the same level of refinement   as the current
   * cell. On distributed meshes, by calling
   * Triangulation::add_periodicity(),     we can make sure that the element
  ...
  /**
   * For a cell whose   @c   ith face is not at a boundary, this function
   * returns   the same result as neighbor(). If the   @c   ith face is at a
   * periodic boundary   this function returns the same result as
   * periodic_neighbor(). If neither of   the aforementioned conditions are
   * met, i.e. the   @c   ith face is on a   nonperiodic boundary, an
  ...
  /**
   * Return an iterator to the periodic neighbor of the cell at a given   face
   * and subface number. The general guidelines for using this function   is
   * similar to the function neighbor_child_on_subface(). The   implementation
   * of this function is consistent with
   * periodic_neighbor_of_coarser_periodic_neighbor(). For instance,   assume
   * that we are sitting on a cell named   @c   cell1, whose neighbor behind
   * the   @c   ith face is one level coarser. Let us name this coarser
   * neighbor     @c   cell2. Then, by calling
   * periodic_neighbor_of_coarser_periodic_neighbor(), from   @c   cell1, we
   * get   a   @c   face_num and a   @c   subface_num. Now, if we call
   ...
   * periodic_neighbor_of_periodic_neighbor()   for those cells which have a
   * coarser periodic neighbor. The returned   pair of numbers can be used in
   * periodic_neighbor_child_on_subface()   to get back to the current cell.
   * In other words, the following   assertion should be true, for a cell with
   * coarser periodic neighbor:
  ...
  /**
   * This function returns the index of the periodic neighbor at the     @c
   * ith face of the current cell. If there is no periodic neighbor   at the
   * given face, the returned value is
   *
   *  - .
   *
   */
  ...
  /**
   * This function returns the level of the periodic neighbor at the     @c
   * ith face of the current cell. If there is no periodic neighbor   at the
   * given face, the returned value is
   *
   *  - .
   ...
   * returns the face number of that periodic neighbor such that the   current
   * cell is the periodic neighbor of that neighbor. In other words   the
   * following assertion holds for those cells which have a periodic
   * neighbor with the same or a higher level of refinement as the current
   * cell:     @c  {cell->periodic_neighbor(i)->
   ...
   * returns the face number of the periodic neighbor, which is connected   to
   * the   @c   ith face of this cell.
   *
   */
  unsigned int
   ...
   * implementation allows this function to work in the case of   anisotropic
   * refinement.
   *
   */
  bool
  ...
  /**
   * Return whether the   @p ith   vertex or face (depending on the dimension)
   * is   part of the boundary. This is true, if the   @p ith   neighbor does
   * not   exist.
   *
   */
  bool
   ...
   * all faces are in the interior of the domain. For the latter case, the
   * @p     has_boundary_lines function is the right one to ask.
   *
   */
  bool
  at_boundary() const;
  ...
  /**
   * This is a slight variation to the   @p at_boundary   function: for 1 and
   * 2   dimensions, it is equivalent, for three dimensions it returns whether
   * at   least one of the 12 lines of the hexahedron is at a boundary. This,
   * of   course, includes the case where a whole face is at the boundary, but
   ...
   * this cell for isotropic refinement.     If you choose anisotropic
   * refinement, for example by passing as argument   one of the flags
   * RefinementCase::cut_x,     RefinementCase::cut_y,
   ...
   * edges and faces. On the other hand, this process is independent of   how
   * the cell is oriented within the   <em>  global  </em>   coordinate
   * system,   and you should not assume any particular orientation of the
   * cell's   local coordinate system within the global coordinate system of
   ...
   * refinement case   @p face_refinement_case   at face <tt>face_no</tt>,
   * taking   into account orientation, flip and rotation of the face. Return,
   * whether   the refinement flag had to be modified. This function is only
  ...
  /**
   * Set the material id of this cell and all its children (and grand-   children, and so on) to the given value.     See the     @ref GlossMaterialId   "glossary"
   * for more information.
   *
   */
  void
  ...
  /**
   * Return the subdomain id of this cell.     See the     @ref GlossSubdomainId   "glossary"
   * for more information.
  ...
  /**
   * Set the subdomain id of this cell.     See the     @ref GlossSubdomainId   "glossary"
   ...
   * hierarchy of recursively refined cells that make up the mesh. In   other
   * words, the level subdomain id is a property that is also defined   for
   * non-active cells if a multigrid hierarchy is used.
   *
   */
  types::subdomain_id
   ...
   * for more information. This function should not be called if you use a
   * parallel::distributed::Triangulation   object since there the subdomain
   * id   is implicitly defined by which processor you're on.
   *
   */
  void
  ...
  /**
   * Return a globally unique cell index for the current cell,   assuming it
   * is not artificial. The value is identical to   active_cell_index() if the
   * cell is part of a serial   triangulation.     In the context of parallel
   * triangulations, locally-owned cells   are enumerated contiguously within
   * each subdomain of the   mesh. This ensures that the index returned by
   * this function can   be used as the index into vectors with a total of
   * Triangulation::n_globally_active_cells()   entries, and for which   every
   * process stores a contiguous part.  If such a cell-data   vector has been
   * set up with
   * parallel::TriangulationBase::global_active_cell_index_partitioner(),
   * the index returned by this function can then be used to access   the
   * correct vector entry.
   *
   */
  ...
  /**
   * Return the
   * orientation of this cell.     For the meaning of this flag, see     @ref
   * GlossDirectionFlag  .
   *
   */
  bool
   ...
   * functions in GridRefinement, and for generating cell-wise output.     The
   * function throws an exception if the current cell is not active.
   * @note
   * If the triangulation this function is called on is of type
   * parallel::distributed::Triangulation,   then active cells may be locally
   * owned, ghost cells, or artificial (see     @ref GlossLocallyOwnedCell  ,
   * @ref GlossGhostCell  ,   and     @ref GlossArtificialCell  ).   This
   * function counts over all of them, including ghost and artificial   active
   * cells. This implies that the index returned by this function   uniquely
   * identifies a cell within the triangulation on a single   processor, but
   * does not uniquely identify the cell among the (parts of   the)
   * triangulation that is shared among processors. If you would like to
  ...
  /**
   * Return whether this cell is owned by the current processor or is owned by   another processor. The function always returns true if applied to an   object of type   dealii::Triangulation,   but may yield false if the   triangulation is of type   parallel::distributed::Triangulation.       See the     @ref GlossGhostCell   "glossary"
   * and the     @ref distributed     module for more information.       @post
   * The returned value is equal to <code>!is_ghost() &&
   * !is_artificial()</code>.
   * @note   Whether a cell is a ghost cell, artificial, or is locally owned
   * or   is a property that only pertains to cells that are active.
   * Consequently,   you can only call this function if the cell it refers to
   * has no children.
   *
   */
  bool
  ...
  /**
   * Test whether the point   @p p   is inside this cell. Points on the
   * boundary   are counted as being inside the cell.     Note that this
   * function assumes that the mapping between unit cell and   real cell is
   * (bi-, tri-)linear, i.e. that faces in 2d and edges in 3d are   straight
   * lines. If you have higher order transformations, results may be
  ...
  /**
   * Set the neighbor   @p i   of this cell to the cell pointed to by   @p
   * pointer.       This function shouldn't really be public (but needs to for
   * various   reasons in order not to make a long list of functions friends):
   * it   modifies internal data structures and may leave things. Do not use
   * it   from application codes.
   *
   */
  void
...
protected:
   ...
   * <code>invalid_unsigned_int</code>  .     This function is not for public
   * use. Use the function   neighbor_of_neighbor() instead which throws an
   * exception if called for a   coarser neighbor. If neighbor is indeed
   * coarser (you get to know this by   e.g. the neighbor_is_coarser()
   * function) then the   neighbor_of_coarser_neighbor() function should be
   * call. If you'd like to   know only the   <code>face_no</code>   which is
   * required to get back from the   neighbor to the present cell then simply
   * use the neighbor_face_no()   function which can be used for coarser as
   * well as non-coarser neighbors.
   *
   */
  unsigned int
