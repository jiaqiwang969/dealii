 ...
 * returned by `cell->id()` (i.e.,   CellAccessor::id())   where `cell` is
 * assumed to be a cell iterator.
 * This class stores the index of the coarse cell from which a cell is descendant (or, more specifically, the entry on   @ref GlossCoarseCellId   "coarse cell IDs"),
 ...
 * moving from one cell to its children). The important point about this class
 * is that an object of the current class uniquely identifies a cell in
 * triangulation, and it even does so in the context of objects of type
 * parallel::distributed::Triangulation   where the local portion of a mesh
 * may not store all cells. For example, the CellId computed for a ghost cell
 * on one processor will be exactly the same as the CellId computed for the
 * very same cell on the processor that actually owns the cell, although the
 * level and index of the iterators pointing to that cell <i>within the
 ...
 *
 *
 * @note   How this data is internally represented is not of importance (and
...
public:
  ...
  /**
   * Construct a CellId object with a given   @p coarse_cell_id   and vector
   * of   child indices.   @p child_indices   is   interpreted identical to
   * the member variable with the same name, namely   each entry denotes which
   * child to pick from one refinement level to the   next, starting with the
   * coarse cell, until we get to the cell represented   by the current
   * object. Therefore, each entry should be a number between 0   and the
   * number of children of a cell in the current space dimension (i.e.,
   ...
   * (i.e.,     GeometryInfo<dim>::max_children_per_cell).   The array     @p
   * child_indices   must have at least   @p n_child_indices   valid entries.
   *
   */
  CellId(const types::coarse_cell_id coarse_cell_id,
  ...
  /**
   * Return a human-readable string representation of this CellId.     The
   * string returned by this function consists of only ASCII characters   and
   * will look, for example, like this: `"0_3:006"`. Itcan* be   interpreted
   * by humans as saying "This cell originates from the zeroth   coarse mesh
   * cell, lives on refinement level 3, and the path from the   coarse mesh
   * cell to its children and grand children is given by 006".   But it is
   * notmeant* to be interpreted in any meaningful way: It's just   a way of
   * representing the internal state of the current object using   only ASCII
   ...
   * ordering are unspecified except that the operation provides a   total
   * ordering among all cells.
   *
   */
  bool
  operator<(const CellId &other) const;
   ...
   * from one refinement level to the next, starting with the coarse cell,
   * until   we get to the cell represented by the current object.     The
   * number of elements in this container corresponds to (level-1) of the
   * current cell.
   *
   */
...
private:
  /**
   * The number of the coarse cell within whose tree the cell   represented by
   * the current object is located.
   *
   */
   ...
   * refinement level to the next, starting with the coarse cell,   until we
   * get to the cell represented by the current object.   Only the first
   * n_child_indices entries are used, but we use a statically   allocated
   * array instead of a vector of size n_child_indices to speed up   creation
   * of this object. If the given dimensions ever become a limitation   the
