...
{
  /**
   * Indices of the vertices of this cell. These indices correspond   to
   * entries in the vector of vertex locations passed to
  ...
  /**
   * Material or boundary indicator of this cell.   This field is a union that
   * stores <i>either</i> a boundary or   a material id, depending on whether
   * the current object is used   to describe a cell (in a vector of CellData
   * objects) or a   face or edge (as part of a SubCellData object).
   *
   */
  union
  {
    ...
    /**
     * The boundary id of a face or edge being described. See the
     * documentation     of the CellData class for examples of how to use this
     * field.         This variable can only be used if the current object is
     * used to     describe a face or edge, i.e., if   @p structdim   is less
     * than the dimension       @p dim   of a triangulation. In this case, the
     * CellData object this     variable belongs to will be part of a
     * SubCellData object.
     *
     */
...
/**
 * The SubCellData class is used to describe information about faces and edges
 * at the boundary of a mesh when creating a triangulation via
 * Triangulation::create_triangulation().   It contains member variables that
 * describe boundary edges and boundary quads. The class has no template
 * argument and is used both in the description of boundary edges in 2d (in
 * which case the contents of the   @p boundary_quads   member variable are
 * ignored), as well as in the description of boundary edges and faces in 3d
 * (in which case both the   @p boundary_lines   and   @p boundary_quads
 * members may be used). It is also used as the argument to
 * Triangulation::create_triangulation()   in 1d, where the contents of
 * objects of the current type are simply ignored.
 * Each entry in the   @p boundary_lines   and   @p boundary_quads   vectors
 * then needs to correspond to an edge or quad of the cells that are described
 * by the vector of CellData objects passed to
 * Triangulation::create_triangulation().   I.e., the vertex indices stored in
 * each entry need to correspond to an edge or face of the triangulation that
 * has the same set of vertex indices, and in the same order. For these
 * boundary edges or quads, one can then set either or both the
 * CellData::boundary_id   and   CellData::manifold_id. There are also use
 * cases where one may want to set the manifold id of an <i>interior</i> edge
 * or face. Such faces, identified by their vertex indices, may also appear in
 * the   @p boundary_lines   and   @p boundary_quads   vectors (despite the
 * names of these member variables). However, it is then obviously not allowed
 * to set a boundary id (because the object is not actually part of the
 * boundary). As a consequence, to be valid, the   CellData::boundary_id   of
 * interior edges or faces needs to equal
...
struct SubCellData
{
   ...
   * information for edges of 2d or 3d triangulations.     This vector may not
   * be used in the creation of 1d triangulations.
   *
   */
   ...
   * information for quads of 3d triangulations.     This vector may not be
   * used in the creation of 1d or 2d triangulations.
   *
   */
   ...
   * whether   both   @p boundary_lines   and   @p boundary_quads   are empty
   * vectors   when   @p dim   equals one, and whether the   @p boundary_quads
...
{
  /**
   * Configuration flags for Triangulations.   Settings can be combined using
   * bitwise OR.
   *
   */
  enum Settings
   ...
   * Description and used during construction of a   Triangulation. This
   * struct stores   the cell id, the subdomain_id and the level_subdomain_id
   * as well as   information related to manifold_id and boundary_id.
   * @note   Similarly to   dealii::CellData,   this structure stores
   * information   about a cell. However, in contrast to   dealii::CellData,
   * it also stores   a unique id, partitioning information, and information
   * related to cell   faces and edges.
   *
   */
  template <int dim>
  struct CellData
  ...
  struct Description
  {
    ...
    /**
     * List that
     * for each locally-relevant coarse cell provides the     corresponding
     * global       @ref GlossCoarseCellId  .
     *
     */
     ...
     * to the communicator inside of the Triangulation     and an assert is
  ...
  /**
   * A namespace for   TriangulationDescription::Description   utility
   * functions.
  ...
  namespace Utilities
  {
     ...
     * partitioned triangulation `tria` and a specified process.     The input
     * triangulation can be either     a serial triangulation of type
     * dealii::Triangulation   which has been     colored (subdomain_id and/or
     * level_subdomain_id has been set) or a     distributed triangulation of
     * type       dealii::parallel::distributed::Triangulation,   where the
     * partitioning is     adopted unaltered.           @param   tria
     * Partitioned input triangulation.       @param   comm MPI_Communicator
     * to be used. In the case       of
     * dealii::parallel::distributed::Triangulation,   the communicators have
     * to match.       @param   settings See the description of the Settings
     * enumerator.       @param   my_rank_in Construct Description for the
     * specified rank (only       working for serial triangulations that have
     * been partitioned by       functions like
     * GridToold::partition_triangulation()).         @return   Description to
     * be used to set up a Triangulation.
     * @note   If construct_multigrid_hierarchy is set in the settings, the
     * source       triangulation has to be setup with
    ...
    /**
     * Similar to the above function but the owner of active cells are
     * provided     by a cell vector (see also
     * parallel::TriangulationBase::global_active_cell_index_partitioner()
     * and       CellAccessor::global_active_cell_index()).   This function
     * allows to     repartition distributed Triangulation objects.
     * @note   The communicator is extracted from the vector   @p partition.
     * @note   The triangulation   @p tria   can be set up on a
     * subcommunicator of the       communicator of   @p partitioner.   All
     * processes that are not part of that       subcommunicator need to set
     * up the local triangulation with the       special-purpose communicator
     * MPI_COMM_NULL.
     ...
     * provided   `std::function`   objects). Internally only selected
     * processes (     every n-th/each root of a group of size group_size)
     * create a serial     triangulation and the
     * TriangulationDescription::Description   for all     processes in its
     * group, which is communicated.
     * @note   A reasonable group size is the size of a NUMA domain or the
     * size of a compute node.           @param   serial_grid_generator A
     * function which creates a serial triangulation.       @param
     * serial_grid_partitioner A function which can partition a serial
     ...
     * argument the group size.       @param   comm MPI communicator.
     * @param   group_size The size of each group.       @param   smoothing
     * Mesh smoothing type.       @param   setting See the description of the
     * Settings enumerator.       @return   Description to be used to set up a
     * Triangulation.
     * @note   If construct_multigrid_hierarchy is set in the settings, the
