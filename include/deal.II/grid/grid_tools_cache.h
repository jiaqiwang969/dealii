...
namespace GridTools
{
   ...
   * Triangulation.     This class attaches a signal to the Triangulation
   * passed at construction   time to keep track about refinement changes, and
   * allows the user to query   some of the data structures constructed using
   * functions in the GridTools   namespace which are computed only once, and
   * then cached inside this class   for faster access whenever the
   * triangulation has not changed.     Notice that this class only notices if
   * the underlying Triangulation has   changed due to a
   * Triangulation::Signals::any_change()   signal being   triggered.     If
   * the triangulation changes for other reasons, for example because you
  ...
  public:
    /**
     * Constructor.         If you provide the optional `mapping` argument,
     * then this is used     whenever a mapping is required.           @param
     * tria The triangulation for which to store information       @param
     ...
     * Notice that no work is performed when you call this function. The
     * actual     data structures are computed the next time you call the
     * corresponding     `get_*` method.           @param   flags What to mark
     * for update
     *
     */
    void
     ...
     * parallel::distributed::Triangulation   objects, this function will
     * return     also the bounding boxes of ghost and artificial cells.
     *
     */
    const RTree<
    ...
    /**
     * Return the cached RTree object of bounding boxes containing locally
     * owned     active cells, constructed using the active cell iterators of
     * the stored     triangulation.         In contrast to the previous
     * function, this function builds the RTree     using only the locally
     * owned cells, i.e., not including ghost or     artificial cells. The two
     * functions return the same result in serial     computations.
     *
     */
    const RTree<
      std::pair<BoundingBox<spacedim>,
    ...
    /**
     * Returns the vector of set of integer containing the subdomain id     to
     * which each vertex is connected to. This feature is used extensively
  ...
  private:
    ...
    /**
     * A collection of rtree objects covering the whole mesh.         Each
     * entry of the map is constructed from the function
    ...
    /**
     * Store an RTree object, containing the used vertices of the
     * triangulation.
     *
     */
    ...
    /**
     * Store an RTree object, containing the bounding boxes of the cells of
     * the     triangulation.
     *
     */
    mutable RTree<
    ...
    /**
     * Store an RTree object, containing the bounding boxes of the locally
     * owned     cells of the triangulation.
     *
     */
    mutable RTree<
      std::pair<BoundingBox<spacedim>,
    ...
    /**
     * Store an   std::vector   of   std::set   of integer containing the id
     * of all     subdomain to which a vertex is connected to.
     *
     */
