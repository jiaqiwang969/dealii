...
namespace GridTools
{
   ...
   * triangulation. We compute the measure using the integral   $\sum_K \int_K
   * 1
   * \; dx$   where   $K$   are the cells of the given triangulation. The
   * integral   is approximated via quadrature for which we need the mapping
   * argument.     If the triangulation is a dim-dimensional one embedded in a
   * higher   dimensional space of dimension spacedim, then the value returned
   * is the   dim-dimensional measure. For example, for a two-dimensional
   * triangulation   in three-dimensional space, the value returned is the
   * area of the surface   so described. (This obviously makes sense since the
   * spacedim-dimensional   measure of a dim-dimensional triangulation would
   * always be zero if dim   @<     spacedim.     This function also works for
   * objects of type     parallel::distributed::Triangulation,   in which case
   * the function is a   collective operation.       @param   tria The
   * triangulation.     @param   mapping An optional argument used to denote
   * the mapping that   should be used when describing whether cells are
   * bounded by straight or   curved faces. The default is to use a   $Q_1$
   * mapping, which corresponds to   straight lines bounding the cells.
   ...
   * Notice that, even if you pass a non-trivial mapping, the returned value
   * is   computed only using information on the vertices of the
   * triangulation,   possibly transformed by the mapping. While this is
   * accurate most of the   times, it may fail to give the correct result when
   * the triangulation   contains very distorted cells.
   *
   */
  template <int dim, int spacedim>
  double
   ...
   * triangulation.     Notice that, even if you pass a non-trivial mapping to
   * this function, the   returned value is computed only using information on
   * the vertices of the   triangulation, possibly transformed by the mapping.
   * While this is accurate   most of the times, it may fail to give the
   * correct result when the   triangulation contains very distorted cells.
   *
   */
  template <int dim, int spacedim>
  double
   ...
   * up by naming its vertices from the list.       @deprecated   Use the more
   * general function which takes an ArrayView instead.
   *
   */
  template <int dim>
  ...
  /**
   * Given a list of vertices (typically obtained using
   * Triangulation::get_vertices())   as the first, and a list of vertex
   * indices   that characterize a single cell as the second argument, return
   * the   measure (area, volume) of this cell. If this is a real cell, then
   * you can   get the same result using   <code>cell-@>measure()</code>  ,
   * but this   function also works for cells that do not exist except that
   * you make it   up by naming its vertices from the list.     The parameter
   * @p vertex_indices   is expected to have
   * GeometryInfo<dim>::vertices_per_cell   entries. A   std::vector   is
   * implicitly   convertible to an ArrayView, so it can be passed directly.
   * See the   ArrayView class for more information.
   ...
   * p_\text{unit} + b $   by a least squares fit of this affine function to
   * the     $2^\text{dim}$   vertices representing a quadrilateral or
   * hexahedral cell   in `spacedim` dimensions. The result is returned as a
   * pair with the   matrix <i>A</i> as the first argument and the vector
   * <i>b</i> describing   distance of the plane to the origin.     For any
   * valid mesh cell whose geometry is not degenerate, this operation
   ...
   * and affine (parallelogram) meshes in 2D/3D.     This approximation is
   ...
   * @p mapping.   The size of the vector that is returned equals the number
   * of   active cells. The vector contains zero for non locally-owned cells.
   * The   aspect ratio of a cell is defined as the ratio of the maximum to
   * minimum   singular value of the Jacobian, taking the maximum over all
   * quadrature   points of a quadrature rule specified via   @p quadrature.
   * For example, for   the special case of rectangular elements in 2d with
   * dimensions   $a$   and   $b$     (  $a \geq b$  ), this function returns
   * the usual aspect ratio definition     $a/b$  . The above definition using
   * singular values is a generalization to   arbitrarily deformed elements.
   * This function is intended to be used for     $d=2,3$   space dimensions,
   * but it can also be used for   $d=1$   returning a   value of 1.
   * @note   Inverted elements do not throw an exception. Instead, a value of
   * inf   is written into the vector in case of inverted elements.
   * @note   Make sure to use enough quadrature points for a precise
   * calculation   of the aspect ratio in case of deformed elements.
   ...
   * are locally owned and placed at index
   * CellAccessor::active_cell_index(),     respectively. All other values are
   * set to 0.
   *
   */
  template <int dim>
  ...
  /**
   * Compute the smallest box containing the entire triangulation.     If the
   * input triangulation is a   `parallel::distributed::Triangulation`,
   * then each processor will compute a bounding box enclosing all locally
   * owned, ghost, and artificial cells. In the case of a domain without
   * curved   boundaries, these bounding boxes will all agree between
   * processors because   the union of the areas occupied by artificial and
   * ghost cells equals the   union of the areas occupied by the cells that
   * other processors own.   However, if the domain has curved boundaries,
   * this is no longer the case.   The bounding box returned may be
   * appropriate for the current processor,   but different from the bounding
   * boxes computed on other processors.
   *
   */
  template <int dim, int spacedim>
  ...
  /**
   * Return the point on the geometrical object   @p object   closest to the
   * given   point   @p trial_point.   For example, if   @p object   is a
   * one-dimensional line   or edge, then the returned point will be a point
   * on the geodesic that   connects the vertices as the manifold associated
   * with the object sees it   (i.e., the geometric line may be curved if it
   * lives in a higher   dimensional space). If the iterator points to a
   * quadrilateral in a higher   dimensional space, then the returned point
   * lies within the convex hull of   the vertices of the quad as seen by the
   * associated manifold.
   ...
   * specialized function   Manifold::project_to_manifold()   may perform
   * better.
   *
   */
   ...
   * information about faces and lines.     This function is useful in cases
   * where one needs to deconstruct a   Triangulation or manipulate the
   * numbering of the vertices in some way: an   example is
   ...
   * function is called by all   <tt>GridIn::read_*</tt>   functions to
   * eliminate   vertices that are listed in the input files but are not used
   * by the cells   in the input file. While these vertices should not be in
   * the input from   the beginning, they sometimes are, most often when some
   * cells have been   removed by hand without wanting to update the vertex
   * lists, as they might   be lengthy.     This function is called by all
   * <tt>GridIn::read_*</tt>   functions as the   triangulation class requires
   * them to be called with used vertices only.   This is so, since the
   * vertices are copied verbatim by that class, so we   have to eliminate
   * unused vertices beforehand.     Not implemented for the codimension one
   * case.
   *
   */
  template <int dim, int spacedim>
   ...
   * interior of the mesh.     This function is called by some
   * <tt>GridIn::read_*</tt>   functions. Only   the vertices with indices in
   * @p considered_vertices   are tested for   equality. This speeds up the
   * algorithm, which is, for worst-case hyper   cube geometries
   * $O(N^{3/2})$   in 2D and   $O(N^{5/3})$   in 3D: quite slow.   However,
   * if you wish to consider all vertices, simply pass an empty   vector. In
   * that case, the function fills   @p considered_vertices   with all
   * vertices.     Two vertices are considered equal if their difference in
   * each coordinate   direction is less than   @p tol.
   *
   */
  template <int dim, int spacedim>
  void
   ...
   * is the inverse of the orientation required by deal.II.     In 2d and 3d
   * this function checks whether all cells have negative or   positive
   * measure/volume. In the former case, all cells are inverted. It   does
   * nothing in 1d.     The inversion of cells might also work when only a
   * subset of all cells   have negative volume. However, grids consisting of
   * a mixture of negative   and positively oriented cells are very likely to
   * be broken. Therefore, an   exception is thrown, in case cells are not
   * uniformly oriented.
   * @note   This function should be called before
   * GridTools::consistently_order_cells().         @param   all_vertices The
   * vertices of the mesh.     @param   cells The array of CellData objects
   * that describe the mesh's topology.
   *
   */
  template <int dim, int spacedim>
  void
  ...
  /**
   * Given a vector of CellData objects describing a mesh, reorder their   vertices so that all lines are consistently oriented.     The expectations on orientation and a discussion of this function are   available in the   @ref reordering   "reordering module".
   * @param   cells The array of CellData objects that describe the mesh's
   * topology.
   *
   */
  template <int dim>
   ...
   * function object provided as first argument to all its vertices.     The
   * transformation given as argument is used to transform each vertex.   Its
   * respective type has to offer a function-like syntax, i.e. the   predicate
   * is either an object of a type that has an <tt>operator()</tt>,   or it is
   * a pointer to the function. In either case, argument and return   value
   * have to be of type <tt>Point  @<spacedim@></tt>.
   ...
   * current function has no way of asserting a positive determinant     of
   * the Jacobian, but if you happen to use such a transformation,     the
   * result will be a triangulation in which cells have a negative     volume.
   * @note   If you are using a   parallel::distributed::Triangulation   you
   * will   have hanging nodes in your local Triangulation even if your
   * "global" mesh   has no hanging nodes. This will cause issues with wrong
   * positioning of   hanging nodes in ghost cells if you call the current
   * functions: The   vertices of all locally owned cells will be correct, but
   * the vertices of   some ghost cells may not. This means that computations
   * like   KellyErrorEstimator may give wrong answers.
   * @note   This function is in general not compatible with manifolds
   * attached   to the triangulation. For example, in order to refine the grid
   * (using   manifolds) after the grid transformation, you have to make sure
   * that   the original manifold is still valid for the transformed geometry.
   * This   does not hold in general, and it is necessary to clear the
   * manifold and   attach a new one for the transformed geometry in these
   * cases.   If you want to perform refinements according to the original
  ...
  /**
   * Rotate all vertices of the given   @p triangulation   in
   * counter-clockwise   direction around the axis with the given index.
   * Otherwise like the   function above.       @param[in]   angle Angle in
   * radians to rotate the Triangulation by.     @param[in]   axis Index of
   * the coordinate axis to rotate around, keeping   that coordinate fixed
   * (0=x axis, 1=y axis, 2=z axis).     @param[in,out]   triangulation The
   * Triangulation object to rotate.
   ...
   * mapped to the corresponding points in the   @p new_points   map.     The
   * unknown displacement field   $u_d(\mathbf x)$   in direction   $d$   is
   * obtained from the minimization problem \f[ \min\, \int \frac{1}{2}
   * c(\mathbf x) \mathbf \nabla u_d(\mathbf x) \cdot \mathbf \nabla
   * u_d(\mathbf x) \,\rm d x
   * \f]   subject to prescribed constraints. The minimizer is obtained by
   * solving the   Laplace equation of the dim components of a displacement
   * field that maps   the current   domain into one described by   @p
   * new_points   . Linear finite elements with   four Gaussian quadrature
   * points in each direction are used. The difference   between the vertex
   * positions specified in   @p new_points   and their current   value in
   * @p tria   therefore represents the prescribed values of this
   * displacement field at the boundary of the domain, or more precisely at
   * all   of those locations for which   @p new_points   provides values
   * (which may be   at part of the boundary, or even in the interior of the
   * domain). The   function then evaluates this displacement field at each
   * unconstrained   vertex and uses it to place the mapped vertex where the
   ...
   * domain to the new one.       @param[in]   new_points The locations where
   * a subset of the existing   vertices are to be placed. Typically, this
   * would be a map from the vertex   indices of all nodes on the boundary to
   * their new locations, thus   completely specifying the geometry of the
   * mapped domain. However, it may   also include interior points if
   * necessary and it does not need to include   all boundary vertices
   * (although you then lose control over the exact   shape of the mapped
   * domain).       @param[in,out]   tria The Triangulation object. This
   * object is changed in-   place, i.e., the previous locations of vertices
   * are overwritten.       @param[in]   coefficient An optional coefficient
   * for the Laplace problem.   Larger values make cells less prone to
   * deformation (effectively   increasing their stiffness). The coefficient
   * is evaluated in the   coordinate system of the old, undeformed
   * configuration of the   triangulation as input, i.e., before the
   * transformation is applied.   Should this function be provided, sensible
   * results can only be expected   if all coefficients are positive.
   * @param[in]   solve_for_absolute_positions If set to   <code>true</code>
   * , the   minimization problem is formulated with respect to the final
   * vertex   positions as opposed to their displacement. The two formulations
   * are   equivalent for   the homogeneous problem (default value of   @p
   * coefficient),   but they   result in very different mesh motion
   * otherwise. Since in most cases one   will be using a non-constant
   * coefficient in displacement formulation, the   default value of this
   ...
   * orientation of the triangulation, the factor must be positive.     This
   * function uses the transform() function above, so the requirements on
   ...
   * then   boundary vertices are not moved.       @p seed   is used for the
   * initialization of the random engine. Its   default value initializes the
   * engine with the same state as in   previous versions of deal.II.
   *
   */
  template <int dim, int spacedim>
  void
  ...
  /**
   * Analyze the boundary cells of a mesh, and if one cell is found at   a
   * corner position (with dim adjacent faces on the boundary), and its
   * dim-dimensional angle fraction exceeds   @p limit_angle_fraction,
   * refine globally once, and replace the children of such cell   with
   * children where the corner is no longer offending the given angle
   * fraction.     If no boundary cells exist with two adjacent faces on the
   * boundary, then   the triangulation is left untouched. If instead we do
   * have cells with dim   adjacent faces on the boundary, then the fraction
   * between the   dim-dimensional   solid angle and dim*pi/2 is checked
   * against the parameter   @p limit_angle_fraction.     If it is higher, the
   * grid is refined once, and the children of the   offending cell are
   * replaced with some cells that instead respect the limit.   After this
   * process the triangulation is flattened, and all Manifold objects   are
   * restored as they were in the original triangulation.     An example is
   * given by the following mesh, obtained by attaching a   SphericalManifold
   ...
   * </p>       The four cells that were originally the corners of a square
   * will give you   some troubles during computations, as the jacobian of the
   * transformation   from the reference cell to those cells will go to zero,
   * affecting the error   constants of the finite element estimates.
   * Those cells have a corner with an angle that is very close to 180
   * degrees,   i.e., an angle fraction very close to one.     The same code,
   * adding a call to regularize_corner_cells:
   * @code
   * const SphericalManifold<dim> m0;
   * Triangulation<dim> tria;
   * GridGenerator::hyper_cube(tria,-1,1);
   ...
   * the Mapping:       <p ALIGN="center">
   @image html regularize_mesh_02.png
   * </p>       This mesh is very similar to the one obtained by
   * GridGenerator::hyper_ball.     However, using
   * GridTools::regularize_corner_cells   one has the freedom to   choose when
   * to apply the regularization, i.e., one could in principle first   refine
   * a few times, and then call the regularize_corner_cells function:
   * @code
   * const SphericalManifold<dim> m0;
   * Triangulation<dim> tria;
   * GridGenerator::hyper_cube(tria,-1,1);
   ...
   * @endcode
   * This generates the following mesh:       <p ALIGN="center">
   @image html regularize_mesh_03.png
   * </p>       The function is currently implemented only for dim = 2 and
   * will throw an exception if called with dim = 3.       @param[in,out]
   * tria Triangulation to regularize.       @param[in]   limit_angle_fraction
   * Maximum ratio of angle or solid   angle that is allowed for a corner
   * element in the mesh.
   *
   */
  template <int dim, int spacedim>
   ...
   * find_active_cell_around_point() on each element of   @p points,   and
   * return     @p cells,   reference positions   @p qpoints,   and a mapping
   * @p maps   from local   to global indices into   @p points   .
   * @param[in]   cache The triangulation's   GridTools::Cache   .
   * @param[in]   points The point's vector.     @param[in]   cell_hint
   * (optional) A cell iterator for a cell which likely   contains the first
   * point of   @p points.         @return   A tuple containing the following
   * information:
   *
   *
   *
   ...
   *
   *
   *  -   @p cells   : A vector of all the cells containing at least one of     the   @p points   .
   *
   *
   *
   *
   ...
   *
   *
   *  -   @p qpoints   : A vector of vectors of points.   @p qpoints[i]   contains     the reference positions of all points that fall within the cell   @p cells[i]   .
   *
   *
   *
   *
   ...
   *
   *
   *  -   @p indices   : A vector of vectors of integers, containing the mapping between     local numbering in   @p qpoints   , and global index in   @p points   .     If   @p points[a]   and   @p points[b]   are the only two points that fall in   @p cells[c],     then   @p qpoints[c][0]   and   @p qpoints[c][1]   are the reference positions of     @p points[a]   and   @p points[b]   in   @p cells[c],   and   @p indices[c][0]   = a,     @p indices[c][1]   = b. The function     Mapping::transform_unit_to_real(qpoints[c][0])     returns   @p points[a].       The algorithm builds an rtree of   @p points   to sort them spatially, before   attempting to call find_active_cell_around_point().
   ...
   * artificial cell of a   parallel::TriangulationBase,   the point is
   * silently   ignored. If you want to infer for which points the search
   * failed, use the   function compute_point_locations_try_all() that also
   * returns a vector of   indices indicating the points for which the search
   * failed.
   * @note   The actual return type of this function, i.e., the type
   * referenced   above as   @p return_type,   is
   * @code
   * std::tuple<
   * std::vector<
...
#  endif
   ...
   * find_active_cell_around_point() failed.       @return   A tuple
   * containing four elements; the first three   are documented in
   * GridTools::compute_point_locations().     The last element of the   @p
   * return_type   contains the   indices of points which are neither found
   * inside the mesh   nor lie in artificial cells. The   @p return_type
   * equals the   following tuple type:
   * @code
   * std::tuple<
   *   std::vector<
   *      typename Triangulation<dim,spacedim>::active_cell_iterator>,
...
#  endif
   ...
   * process, find the points lying on the locally   owned part of the mesh
   * and compute the quadrature rules for them.   Distributed compute point
   * locations is a function similar to     GridTools::compute_point_locations
   * but working for     parallel::TriangulationBase   objects and, unlike its
   * serial version, also   for a distributed triangulation (see
   * parallel::distributed::Triangulation).         @param[in]   cache a
   * GridTools::Cache   object     @param[in]   local_points the array of
   * points owned by the current process.   Every process can have a different
   * array of points which can be empty and   not contained within the locally
   * owned part of the triangulation     @param[in]   global_bboxes a vector
   * of vectors of bounding boxes; it   describes the locally owned part of
   * the mesh for each process. The bounding   boxes describing which part of
   * the mesh is locally owned by process with   rank rk are contained in
   * global_bboxes[rk]. The local description can be   obtained from
   * GridTools::compute_mesh_predicate_bounding_box;   then the   global one
   * can be obtained using either     GridTools::exchange_local_bounding_boxes
   * or   Utilities::MPI::all_gather       @param[in]   tolerance Tolerance in
   * terms of unit cell coordinates. Depending     on the problem, it might be
   * necessary to adjust the tolerance in order     to be able to identify a
   * cell. Floating     point arithmetic implies that a point will, in
   * general, not lie exactly     on a vertex, edge, or face. In either case,
   * it is not predictable which     of the cells adjacent to a vertex or an
   * edge/face this function returns.     Consequently, algorithms that call
   * this function need to take into     account that the returned cell will
   * only contain the point approximately.     @return   A tuple containing
   * the quadrature information     The elements of the output tuple are:
   *
   *
   *
   *
   ...
   *
   *
   *  - qpoints : a vector of vector of points; containing in   @p qpoints[i]       the reference positions of all points that fall within the cell   @p cells[i]   .
   *
   *
   *
   *
   ...
   *
   *
   *  - points : a vector of vector of points.   @p points[i][j]   is the point in the    real space corresponding.    to   @p qpoints[i][j]   . Notice   @p points   are the points lying on the locally    owned part of the mesh; thus these can be either copies of   @p local_points      or points received from other processes i.e. local_points for other   processes
   *
   *
   *
   *
   ...
   *
   *
   *  - owners : a vector of vectors;   @p owners[i][j]   contains the rank of    the process owning the point[i][j] (previous element of the tuple).     The function uses the triangulation's mpi communicator: for this reason it   throws an assert error if the Triangulation is not derived from     parallel::TriangulationBase   .     In a serial execution the first three elements of the tuple are the same   as in   GridTools::compute_point_locations   .     Note: this function is a collective operation.
   * @note   The actual return type of this function, i.e., the type
   * referenced   above as   @p return_type,   is
   * @code
   * std::tuple<
...
#  endif
  ...
  namespace internal
  {
     ...
     * GridTools::internal::distributed_compute_point_locations().   It
     * provides     information to perform
     * GridTools::distributed_compute_point_locations()       and to set up
     * the communication pattern within
    ...
    struct DistributedComputePointLocationsInternal
    {
       ...
       * position, 4) real position, 5) permutation index within a send
       * buffer.
       *
       */
      ...
      /**
       * Information of each received data value. The elements of the tuple
       * are       as follows: 0) rank of sender, 1) local index, 2)
       * enumeration index.
       * @note   The vector is sorted according to 1), 0), 2).
       * @note   To each point multiple data values might be associated to.
       * This         might be the case if a point coincides with a geometric
       * entity (e.g.,         vertex) that is shared by multiple cells.
       *
       */
    ...
    /**
     * A function that fills DistributedComputePointLocationsInternal.     If
     * the input argument   @p perform_handshake   is set to false only
     * the fields needed by
     * GridTools::internal::distributed_compute_point_locations()   are
     * filled.     If the input argument is set to true additional data
     * structures are     set up to be able to setup the communication pattern
     * within       Utilities::MPI::RemotePointEvaluation::reinit().
     *
     */
    template <int dim, int spacedim>
   ...
   *
   * -> Point<spacedim>` containing the used   vertices of the given
   * `container`. The key of the returned map (i.e.,   the first element of
   * the pair above) is the global index in the   triangulation, whereas the
   * value of each pair is the physical   location of the corresponding
   * vertex. The used vertices are obtained by   looping over all cells,   and
   * querying for each cell where its vertices are through the (optional)
   * `mapping` argument.     In serial Triangulation objects and
   * parallel::shared::Triangulation     objects, the size of the returned map
   * equals   Triangulation::n_used_vertices()   (not
   * Triangulation::n_vertices()).     Note that in
   * parallel::distributed::Triangulation   objects, only vertices in
   * locally owned cells and ghost cells are returned, as for all other
   * vertices   their real location might not be known (e.g. for distributed
   * computations   using MappingQEulerian).     If you use the default
   ...
   * @endcode
   * Notice that the above is not satisfied for mappings that change the   location of vertices, like MappingQEulerian.       @ref ConceptMeshType   "MeshType concept".
   * @param   container The container to extract vertices from.     @param
   ...
   * map of vertices passed as the first argument.       @param   vertices A
   * map of index->vertex, as returned by
   * GridTools::extract_used_vertices().       @param   p The target point.
   * @return   The index of the vertex that is closest to the target point
   * `p`.
   *
   */
  ...
  /**
   * Find and return the index of the used vertex (or marked vertex) in a   given mesh that is located closest to a given point.     This function uses the locations of vertices as stored in the   triangulation. This is usually sufficient, unless you are using a Mapping   that moves the vertices around (for example, MappingQEulerian). In this   case, you should call the function with the same name and with an   additional Mapping argument.       @param   mesh A variable of a type that satisfies the requirements of the     @ref ConceptMeshType   "MeshType concept".
   * @param   p The point for which we want to find the closest vertex.
   * @param   marked_vertices An array of bools indicating which   vertices of
   * @p mesh   will be considered within the search   as the potentially
   * closest vertex. On receiving a non-empty     @p marked_vertices,   the
   * function will   only search among   @p marked_vertices   for the closest
   * vertex.   The size of this array should be equal to the value returned by
   * Triangulation::n_vertices()   for the triangulation underlying the given
   * mesh   (as opposed to the value returned by
   * Triangulation::n_used_vertices()).       @return   The index of the
  ...
  /**
   * Find and return a vector of iterators to active cells that surround a   given vertex with index   @p vertex_index.       For locally refined grids, the vertex itself might not be a vertex of all   adjacent cells that are returned. However, it will always be either a   vertex of a cell or be a hanging node located on a face or an edge of it.       @param   container A variable of a type that satisfies the requirements of   the     @ref ConceptMeshType   "MeshType concept".
   * @param   vertex_index The index of the vertex for which we try to find
   * adjacent cells.     @return   A vector of cells that lie adjacent to the
   * given vertex.
   * @note   It isn't entirely clear at this time whether the function does
   * the   right thing with anisotropically refined meshes. It needs to be
...
#  endif
  ...
  /**
   * Find an active non-artificial cell that surrounds a given point   @p p.   The return type   is a pair of an iterator to the active cell along with the unit cell   coordinates of the point.     The algorithm used in this function proceeds by first looking for the   vertex located closest to the given point, see     GridTools::find_closest_vertex().   Secondly, all adjacent cells to this   vertex are found in the mesh, see     GridTools::find_cells_adjacent_to_vertex().   Lastly, for each of these   cells, the function tests whether the point is inside. This check is   performed using the given   @p mapping   argument to determine whether cells   have straight or curved boundaries.     If a point lies on the boundary of two or more cells, then the algorithm   tries to identify the cell that is of highest refinement level.     If the point requested does not lie in a locally-owned or ghost cell,   then this function will return the (invalid) MeshType<dim,   spacedim>::end()     iterator. This case can be handled similarly to the various   `std::find()`     and   `std::lower_bound()`   functions.       @param   mapping The mapping used to determine whether the given point is     inside a given cell.     @param   mesh A variable of a type that satisfies the requirements of the       @ref ConceptMeshType   "MeshType concept".
   ...
   * entry in the vertex array should be considered     (and the others must
   * be ignored) as the potentially     closest vertex to the specified point.
   * On specifying a non-default       @p marked_vertices,
   * find_closest_vertex() would     only search among   @p marked_vertices
   * for the closest vertex.     The size of this array should be equal to
   * n_vertices() of the     triangulation (as opposed to n_used_vertices() ).
   * The motivation of using       @p marked_vertices   is to cut down the
   * search space of vertices if one has     a priori knowledge of a
   * collection of vertices that the point of interest     may be close to.
   ...
   * to be able to identify a cell. Floating     point arithmetic implies that
   * a point will, in general, not lie exactly     on a vertex, edge, or face.
   * In either case, it is not predictable which     of the cells adjacent to
   * a vertex or an edge/face this function returns.     Consequently,
   * algorithms that call this function need to take into     account that the
   * returned cell will only contain the point approximately.       @return
   * A pair of an iterators into the mesh that points to the   surrounding
   * cell, and of the unit cell coordinates of that point. This   local
   * position might be located slightly outside an actual unit cell,   due to
   * numerical roundoff. Therefore, the point returned by this function
   * should be projected onto the unit cell, using
   * GeometryInfo::project_to_unit_cell().    This is not automatically
   * performed   by the algorithm. The returned cell can be a locally-owned
   * cell or a   ghost cell (but not an artificial cell). The returned cell
   * might be a   ghost cell even if the given point is a vertex of a locally
   * owned cell.   The reason behind is that this is the only way to guarantee
   * that all   processors that participate in a parallel triangulation will
   * agree which   cell contains a point. For example, if two processors come
   * together   at one vertex and the function is called with this vertex,
   * then one   processor will return a locally owned cell and the other one a
   * ghost cell.
   *
   */
  template <int dim, template <int, int> class MeshType, int spacedim>
...
#  endif
  ...
  /**
   * A version of the above function that assumes straight boundaries and   as
   * a consequence simply calls the above function using MappingQ1 for   the
   * mapping argument.       @return   An iterator into the mesh that points
...
#  endif
  ...
  /**
   * Another version where we use that mapping on a given   cell that
   * corresponds to the active finite element index of that cell.   This is
   * obviously only useful for hp-problems, since the active finite   element
   * index for all other DoF handlers is always zero.
   *
   */
  template <int dim, int spacedim>
  ...
  /**
   * Finding an active non-artificial cell around a point can be very
   * expensive   in terms of computational costs. This function aims at
   * providing a fast   version of the above functions by using a space-tree
   * to speed up the   geometry search.       @param   cache Object with
   * information about the space-tree of a triangulation,   see
   * GridTools::Cache.       @param   p The point for which we want to find
   * the surrounding cell.     @param   cell_hint Gives a hint for the
   * geometry search, which is beneficial   if a-priori knowledge is available
   * regarding the cell on which the point   may likely be located. A typical
   * use case would be that this search has   to be done for an array of
   * points that are close to each other and where   the adjacent cell of the
   * previous point is a good hint for the next point   in the array.
   * @param   marked_vertices See above.     @param   tolerance See above.
   ...
   * GridTools::vertex_to_cell_map()),   a map of vertex_to_cell_centers
   * (obtained   through   GridTools::vertex_to_cell_centers_directions()),
   * and   optionally an RTree constructed from the used vertices of the
   * Triangulation.
...
#  endif
   ...
   * non-artificial cells around a point for a given tolerance level
   * `tolerance`   in terms of unit coordinates. Given a first cell with
   * reference coordinates   as parameter     @p first_cell,   e.g. obtained
   * by one of the functions above, all   corresponding neighboring cells with
   * points in unit coordinates are also   identified.     This function is
   * useful e.g. for discontinuous function spaces where, for   the case the
   * given point `p` lies on a vertex, edge or face, several   cells might
   * hold independent values of the solution that get combined in   some way
   * in a user code.     This function is used as follows
   * @code
...
#  endif
  ...
  /**
   * Return a list of all descendants of the given cell that are active. For   example, if the current cell is once refined but none of its children are   any further refined, then the returned list will contain all its   children.     If the current cell is already active, then the returned list is empty   (because the cell has no children that may be active).       @tparam   MeshType A type that satisfies the requirements of the     @ref ConceptMeshType   "MeshType concept".
   * @param   cell An iterator pointing to a cell of the mesh.     @return   A
   * list of active descendants of the given cell
   * @note   Since in C++ the MeshType template argument can not be deduced
   * from   a function call, you will have to specify it after the function
   * name, as   for example in
   * @code
  ...
  /**
   * Extract the active cells around a given cell   @p cell   and return them in   the vector   @p active_neighbors.   These neighbors are specifically the   <i>face</i> neighbors of a cell or, if that neighbor is further   refined, its active children that border on that face. On the other   hand, the neighbors returned do not include cells that lie, for   example, diagonally opposite to a vertex but are not face neighbors   themselves. (In 3d, it also does not include cells that are   adjacent to one of the edges of the current cell, but are not   face neighbors.)       @tparam   MeshType A type that satisfies the requirements of the     @ref ConceptMeshType   "MeshType concept".
   ...
   * predicate returns   @p true.       An example of a custom predicate is
   * one that checks for a given material   id
   * @code
   * template <int dim>
   * bool
   ...
   * IteratorFilters. For example, it is possible to extract a layer   of
   * cells around all of those cells with a given material id,
   * @code
   * GridTools::compute_active_cell_halo_layer(
   * tria, IteratorFilters::MaterialIdEqualTo(1, true));
   * @endcode
   * or around all cells with one of a set of active FE indices for a
   * DoFHandler   with hp-capabilities
   * @code
   * GridTools::compute_active_cell_halo_layer(
   * hp_dof_handler, IteratorFilters::ActiveFEIndexEqualTo({1,2}, true));
   * @endcode
   * Note that in the last two examples we ensure that the predicate returns   true only for locally owned cells. This means that the halo layer will   not contain any artificial cells.       @tparam   MeshType A type that satisfies the requirements of the     @ref ConceptMeshType   "MeshType concept".
   * @param[in]   mesh A mesh (i.e. objects of type Triangulation or
   * DoFHandler).     @param[in]   predicate A function  (or object of a type
   * with an operator())   defining the subdomain around which the halo layer
   * is to be extracted. It   is a function that takes in an active cell and
   * returns a boolean.     @return   A list of active cells sharing at least
   * one common vertex with   the predicated subdomain.
   *
   */
  template <class MeshType>
  std::vector<typename MeshType::active_cell_iterator>
  ...
  /**
   * Extract and return the cell layer around a subdomain (set of   cells) on
   * a specified level of the   @p mesh   (i.e. those cells on   that level
   * that share a common set of vertices with the subdomain   but are not a
   * part of it). Here, the "subdomain" consists of exactly   all of those
   * cells for which the   @p predicate   returns   @p true.
   *
   */
  template <class MeshType>
   ...
   * @param[in]   mesh A mesh (i.e. objects of type Triangulation or
   * DoFHandler).     @return   A list of ghost cells
   *
   */
  template <class MeshType>
  std::vector<typename MeshType::active_cell_iterator>
  ...
  /**
   * Extract and return a set of ghost cells which are within a     @p layer_thickness   around all locally owned cells.   This is most relevant for   parallel::shared::Triangulation     where it will return a subset of all ghost cells on a process, but for     parallel::distributed::Triangulation   this will return all the ghost cells.   All the cells for the   parallel::shared::Triangulation   class that   are not owned by the current processor can be considered as ghost cells;   in particular, they do not only form a single layer of cells around the   locally owned ones.       @tparam   MeshType A type that satisfies the requirements of the     @ref ConceptMeshType   "MeshType concept".
   * @param   mesh A mesh (i.e. objects of type Triangulation or DoFHandler).
   * @param   layer_thickness specifies the geometric distance within   which
   * the function searches for active cells from the locally owned cells.
   * @return   A subset of ghost cells within a given geometric distance of
   * @p     layer_thickness from the locally owned cells of a current process.
   ...
   * Here, the "subdomain" consists of exactly all of those   active cells for
   * which the   @p predicate   returns   @p true.       For a description of
   * how   @p predicate   works,   see compute_active_cell_halo_layer().
   * @note   This function was written before the BoundingBox class was
   * invented.     Consequently, it returns a pair of points, rather than a
   * BoundingBox   object as one may expect. However, BoundingBox has a
   * conversion constructor   from pairs of points, so the result of this
   * function can still be assigned   to a BoundingBox object.
   *
   */
  template <class MeshType>
   ...
   * the given predicate is true, are completely enclosed in at least one of
   * the   bounding boxes. Notice the cover is only guaranteed to contain all
   * these   active cells but it's not necessarily exact i.e. it can include a
   * bigger   area than their union.     For each cell at a given refinement
   * level containing active cells for which   @p predicate   is true,   the
   * function creates a bounding box of its children for which   @p predicate
   * is true.     This results in a cover of all active cells for which   @p
   * predicate   is true; the parameters     @p allow_merge   and   @p
   * max_boxes   are used to reduce the number of cells at a computational
   * cost and   covering a bigger n-dimensional volume.     The parameters to
   * control the algorithm are:
   *
   *
   *
   *
   ...
   *
   *
   *  -   @p predicate   : the property of the cells to enclose e.g.   IteratorFilters::LocallyOwnedCell   .    The predicate is tested only on active cells.
   *
   *
   *
   *
   ...
   *
   *
   *  -   @p refinement_level   : it defines the level at which the initial bounding box are created. The refinement    should be set to a coarse refinement level. A bounding box is created for   each active cell at coarser    level than   @p refinement_level;   if   @p refinement_level   is higher than the number of levels of the    triangulation an exception is thrown.
   *
   *
   *
   *
   ...
   *
   *
   *  -   @p allow_merge   : This flag allows for box merging and, by default, is false. The algorithm has a cost of    O(N^2) where N is the number of the bounding boxes created from the   refinement level; for this reason, if    the flag is set to true, make sure to choose wisely a coarse enough   @p refinement_level.
   *
   *
   *
   *
   ...
   *
   *
   *  -   @p max_boxes   : the maximum number of bounding boxes to compute. If more are created the smaller ones are    merged with neighbors. By default after merging the boxes which can be   expressed as a single one no more boxes are merged. See the     BoundingBox::get_neighbor_type   () function for details.    Notice only neighboring cells are merged (see the   @p get_neighbor_type    function in bounding box class): if    the target number of bounding boxes max_boxes can't be reached by merging   neighbors an exception is thrown     The following image describes an example of the algorithm with   @p refinement_level   = 2,   @p allow_merge   = true   and   @p max_boxes   = 1. The cells with the property predicate are in red, the area of a bounding box is   slightly orange.     @image html bounding_box_predicate.png
   *
   *
   *
   *
   ...
   *
   *
   *  - 3. Because   @p allow_merge   = true the number of bounding boxes is reduced while not changing the cover.    If   @p max_boxes   was left as default or bigger than 1 these two boxes would be returned.
   *
   *
   *
   *
   ...
   *
   *
   *  - 4. Because   @p max_boxes   = 1 the smallest bounding box is merged to the bigger one.   Notice it is important to choose the parameters wisely. For instance,   @p allow_merge   = false and     @p refinement_level   = 1 returns the very same bounding box but with a   fraction of the computational cost.     This function does not take into account the curvature of cells and thus it   is not suited for handling curved geometry: the mapping is assumed to be   linear.
   *
   */
  template <class MeshType>
  ...
  /**
   * Given an array of points, use the global bounding box description
   * obtained   using   GridTools::compute_mesh_predicate_bounding_box   to
   * guess, for each of   them, which process might own it.       @param[in]
   * global_bboxes Vector of bounding boxes describing the portion of    mesh
   * with a property for each process.     @param[in]   points Array of points
   * to test.       @return   A tuple containing the following information:
   *
   *
   *
   *
   ...
   *
   *
   *  - A map from the index   <code>unsigned int</code>   of the point in   @p points       to the rank of the owner.
   *
   *
   *
   *
   ...
   *
   *
   *  - A map from the index   <code>unsigned int</code>   of the point in   @p points       to the ranks of the guessed owners.
   * @note   The actual return type of this function, i.e., the type
   * referenced   above as   @p return_type,   is
   * @code
...
#  endif
  ...
  /**
   * Given a covering rtree (see   GridTools::Cache::get_covering_rtree()),
   * and an   array of points, find a superset of processes which,
   * individually,   may own the cell containing the points.     For further
   * details see   GridTools::guess_point_owner;   here only   different
   * input/output types are reported:       @param[in]   covering_rtree RTRee
   * which enables us to identify which   process(es) in a parallel
   * computation may own the cell that   surrounds a given point.
   * @param[in]   points A vector of points to consider.       @return   A
   ...
   *
   *
   *  - A map from the index   <code>unsigned int</code>   of the point in   @p points       to the rank of the owner; these are points for which a single possible     owner was found.
   *
   *
   *
   *
   ...
   *
   *
   *  - A map from the index   <code>unsigned int</code>   of the point in   @p points       to the ranks of the guessed owners; these are points for which multiple     possible owners were found.
   * @note   The actual return type of this function, i.e., the type
   * referenced   above as   @p return_type,   is
   * @code
...
#  endif
   ...
   * the output of   GridTools::vertex_to_cell_map()   (which is expected as
   * input   parameter for this function). Each tensor represents a geometric
   * vector   from the vertex to the respective cell center.     An assertion
   * will be thrown if the size of the input vector is not equal to   the
   * number of vertices of the triangulation.     result[v][c] is a unit
   * Tensor for vertex index v, indicating the direction   of the center of
   * the c-th cell with respect to the vertex v.
   *
   */
  template <int dim, int spacedim>
   ...
   * the given location   @p position.   The location of the vertices is
   * extracted   from the (optional)   @p mapping   argument, to guarantee
   * that the correct   answer is returned when the underlying mapping
   * modifies the position of the   vertices.
   *
   */
  template <int dim, int spacedim>
  unsigned int
   ...
   * active cell do not have a global index on this processor.     The key of
   * the map is the local index of the vertex and the value is the   global
   * index. The indices need to be recomputed after refinement or   coarsening
   ...
   * relative to the highest elongation.       @param[in]   cell an iterator
   * pointing to the cell.       @return    A   std::pair<unsigned   int,
   * double> such that the   @p first   value   is the dimension of the
   * highest elongation and the   @p second   value is the   ratio among the
   * dimensions of the   @p cell.
   *
   */
  template <int dim, int spacedim>
   ...
   * sparsity pattern are also set.     The rows and columns refer to the
   * cells as they are traversed in their   natural order using cell
   * iterators.
   *
   */
  template <int dim, int spacedim>
  ...
  /**
   * Produce a sparsity pattern for a given level mesh in which nonzero
   * entries   indicate that two cells are connected via a common vertex. The
   * diagonal   entries of the sparsity pattern are also set.     The rows and
   * columns refer to the cells as they are traversed in their   natural order
   ...
   * domain. After calling this function, the subdomain ids of all active
   * cells   will have values   between zero and   @p n_partitions-1.   You
   * can access the subdomain id of a cell by using   <tt>cell-
   * @>subdomain_id()</tt>.       Use the third argument to select between
   * partitioning algorithms provided   by METIS or ZOLTAN. METIS is the
   * default partitioner.     If deal.II was not installed with ZOLTAN or
   * METIS, this function will   generate an error   when corresponding
   * partition method is chosen, unless   @p n_partitions   is one.   I.e.,
   * you can write a program so that it runs in the single-processor
   * single-partition case without packages installed, and only requires them
   * installed when multiple partitions are required.
   * @note   If the   @p cell_weight   signal has been attached to the   @p
   ...
   * it takes into consideration a specific set of   @p cell_weights,   which
   * allow the   partitioner to balance the graph while taking into
   * consideration the   computational effort expended on each cell.
   * @note   If the   @p cell_weights   vector is empty, then no weighting is
   * taken   into consideration. If not then the size of this vector must
   * equal to the   number of active cells in the triangulation.
   *
   */
  template <int dim, int spacedim>
  void
   ...
   * identified by the   <code>cell-@>subdomain_id()</code>   flag.     The
   * difference to the previous function is the second argument, a   sparsity
   * pattern that represents the connectivity pattern between cells.     While
   * the function above builds it directly from the triangulation by
   ...
   * $(i,j)$  , then this means that cells   $i$   and   $j$   (in the order
   * in which   they are traversed by active cell iterators) are to be
   * considered   connected; partitioning algorithm will then try to partition
   * the domain in   such a way that (i) the subdomains are of roughly equal
   * size, and (ii) a   minimal number of connections are broken.     This
   * function is mainly useful in cases where connections between cells
   ...
   * neighborhood of cells (e.g. when solving integral equations).     In
   * addition, this function may be useful in cases where the default
   ...
   * the neighborship graph, and partitioning algorithm   will not usually cut
   * important connections in this case. However, if there   are vertices in
   * the mesh where many cells (many more than the common 4 or 6   in 2d and
   * 3d, respectively) come together, then there will be a significant
   ...
   * case like this, partitioning algorithm may sometimes make bad decisions
   * and   you may want to build your own connectivity graph.
   * @note   If the   @p cell_weight   signal has been attached to the   @p
   * triangulation,     then this will be used and passed to the partitioner.
   *
   */
  ...
  /**
   * Partitions the cells of a multigrid hierarchy by assigning level
   * subdomain   ids using the "youngest child" rule, that is, each cell in
   * the hierarchy is   owned by the processor who owns its left most child in
   * the forest, and   active cells have the same subdomain id and level
   * subdomain id. You can   access the level subdomain id of a cell by using
   * <tt>cell-  @>level_subdomain_id()</tt>.       Note: This function assumes
   * that the active cells have already been   partitioned.
   *
   */
  template <int dim, int spacedim>
  void
  ...
  /**
   * This function allows to ask for the owning subdomain of cells identified
   * by   CellId objects that do not have to exist on the current process.
   ...
   * function.     This function returns the association of each cell with one
   * subdomain. If   you are looking for the association of each   @em   DoF
   * with a subdomain, use   the
   * <tt>DoFTools::get_subdomain_association</tt>   function.
   *
   */
  template <int dim, int spacedim>
  void
  ...
  /**
   * Count how many cells are uniquely associated with the given   @p
   * subdomain     index.     This function may return zero if there are no
   * cells with the given   @p     subdomain index. This can happen, for
   * example, if you try to partition a   coarse mesh into more partitions
   * (one for each processor) than there are   cells in the mesh.     This
   * function returns the number of cells associated with one subdomain.   If
   * you are looking for the association of   @em   DoFs with this subdomain,
  ...
  /**
   * For a triangulation, return a mask that represents which of its vertices   are "owned"
   * by the current process in the same way as we talk about   locally owned
   * cells or degrees of freedom (see     @ref GlossLocallyOwnedCell     and
   ...
   * triangulation are owned by the owner of   this partition; for vertices
   * that lie on the boundary between two or more   partitions, the owner is
   * the processor with the least subdomain_id among   all adjacent
   * subdomains.     For sequential triangulations (as opposed to, for
   * example,     parallel::distributed::Triangulation),   every user vertex
   * is of course   owned by the current processor, i.e., the function returns
   ...
   * returns.       @param   triangulation The triangulation of which the
   * function evaluates   which vertices are locally owned.     @return   The
   * subset of vertices, as described above. The length of the   returned
   * array equals Triangulation.n_vertices() and may, consequently,   be
   * larger than   Triangulation::n_used_vertices().
   *
   */
  template <int dim, int spacedim>
  std::vector<bool>
   ...
   * vertices.     The two meshes may have different refinement histories
  ...
  /**
   * Given a triangulation and a list of cells whose children have become   distorted as a result of mesh refinement, try to fix these cells up by   moving the center node around.     The function returns a list of cells with distorted children that   couldn't be fixed up for whatever reason. The returned list is therefore   a subset of the input argument.     For a definition of the concept of distorted cells, see the     @ref GlossDistorted   "glossary entry".
  ...
  fix_up_distorted_child_cells(
   ...
   /*@}*/ 
  /**
   * @name   Extracting and creating patches of cells     These functions extract and create patches of cells surrounding a single   cell, and creating triangulation out of them.
   *
   */
   /*@{*/ 
   ...
   * following the function name.     @param[in]   cell An iterator pointing
   * to a cell of the mesh.     @return   A list of active cells that form the
   * patch around the given cell
   ...
   * @note   In the context of a parallel distributed computation, it only
   * makes   sense to call this function on locally owned cells. This is
   * because the   neighbors of locally owned cells are either locally owned
   * themselves, or   ghost cells. For both, we know that these are in fact
   * the real cells of   the complete, parallel triangulation. We can also
   * query the degrees of   freedom on these.
   *
   */
  template <class MeshType>
  std::vector<typename MeshType::active_cell_iterator>
  ...
  /**
   * This function
   * takes a vector of active cells (hereafter named   @p     patch_cells) as
   * input argument, and returns a vector of their parent   cells with the
   * coarsest common level of refinement. In other words, find   that set of
   * cells living at the same refinement level so that all cells   in the
   * input vector are children of the cells in the set, or are in the   set
   * itself.       @tparam   Container In C++, the compiler can not determine
   * the type of     <code>Container</code>   from the function call. You need
   * to specify it as   an explicit template argument following the function
   * name. This type has   to satisfy the requirements of a mesh container
   * (see     @ref ConceptMeshType  ).       @param[in]   patch_cells A vector
   * of active cells for which this function   finds the parents at the
   * coarsest common level. This vector of cells   typically results from
   ...
   * GridTools::build_triangulation_from_patch.       DoFHandler's built on
   * top of Triangulation or     parallel:distributed::Triangulation   are
   * supported and handled   appropriately.     The result is the patch of
   * cells representing the support of the basis   element associated to the
   * degree of freedom.  For instance using an FE_Q   finite element, we
   * obtain the standard patch of cells touching the degree   of freedom and
   * then add other cells that take care of possible hanging node
   ...
   * located.       @param[in]   dof_handler The DoFHandler which could be
   * built on a   Triangulation or a   parallel::distributed::Triangulation
   * with a finite   element that has degrees of freedom that are logically
   * associated to a   vertex, line, quad, or hex.     @return   A map from
   * the global_dof_index of   degrees of freedom on locally relevant cells to
   * vectors containing     DoFHandler::active_cell_iterators   of cells in
   * the support of the basis   function at that degree of freedom.
   *
   */
  template <int dim, int spacedim>
  std::map<
  ...
  struct PeriodicFacePair
  {
    ...
    /**
     * A   @p dim     $\times$     @p dim   rotation matrix that describes how vector     valued DoFs of the first face should be modified prior to constraining     to the DoFs of the second face.         The rotation matrix is used in   DoFTools::make_periodicity_constraints()       by applying the rotation to all vector valued blocks listed in the     parameter   @p first_vector_components   of the finite element space. For     more details see   DoFTools::make_periodicity_constraints()   and the     glossary       @ref GlossPeriodicConstraints   "glossary entry on periodic conditions".
     *
     */
  ...
  /**
   * An orthogonal equality test for faces.       @p face1   and   @p face2
   * are considered equal, if a one to one matching   between its vertices can
   * be achieved via an orthogonal equality relation.     Here, two vertices
   * <tt>v_1</tt> and <tt>v_2</tt> are considered equal, if     $M\cdot v_1 +
   * offset
   *
   * - v_2$   is parallel to the unit vector in unit   direction   @p
   * direction.   If the parameter   @p matrix   is a reference to a
   * spacedim x spacedim matrix,   $M$   is set to   @p matrix,   otherwise
   * $M$   is the   identity matrix.     If the matching was successful, the
   * _relative_ orientation of   @p face1     with respect to   @p face2   is
   * returned in the bitset   @p orientation,   where
   * @code
   * orientation[0]
   *
   * -> face_orientation
   ...
   * <tt>face_orientation</tt>: <tt>true</tt> if   @p face1   and   @p face2
   * have   the same orientation. Otherwise, the vertex indices of   @p face1
   * match the   vertex indices of   @p face2   in the following manner:
   * @code
   * face1:           face2:
   *
   * 1
   ...
   * - 1
   * @endcode
   * and any combination of that... More information on the topic can be found   in the     @ref GlossFaceOrientation   "glossary"
   * article.
   *
   */
   ...
   * of the given   @p mesh   (a Triangulation or DoFHandler) and add them to
   ...
   * b_id2.     This function tries to match all faces belonging to the first
   * boundary   with faces belonging to the second boundary with the help of
   * orthogonal_equality().     The bitset that is returned inside of
   * PeriodicFacePair encodes the   _relative_ orientation of the first face
   * with respect to the second face,   see the documentation of
   * orthogonal_equality() for further details.     The   @p direction
   * refers to the space direction in which periodicity is   enforced. When
   * matching periodic faces this vector component is ignored.     The   @p
   * offset   is a vector tangential to the faces that is added to the
   ...
   * Optionally, a   $dim\times dim$   rotation   @p matrix   can be specified
   * that   describes how vector valued DoFs of the first face should be
   * modified   prior to constraining to the DoFs of the second face. The   @p
   * matrix   is   used in two places. First,   @p matrix   will be supplied
   * to   orthogonal_equality() and used for matching faces: Two vertices
   * $v_1$   and     $v_2$   match if   $\text{matrix}\cdot v_1 +
   * \text{offset}
   *
   ...
   * @note   Because elements will be added to   @p matched_pairs   (and
   * existing   entries will be preserved), it is possible to call this
   * function several   times with different boundary ids to generate a vector
   * with all periodic   pairs.
   * @note   Since the periodic face pairs are found on the coarsest mesh
   * level,   it is necessary to ensure that the coarsest level faces have the
   * correct   boundary indicators set. In general, this means that one must
   * first set   all boundary indicators on the coarse grid before performing
   * any global   or local grid refinement.
   *
   */
  template <typename MeshType>
  void
  ...
  /**
   * This compatibility version of collect_periodic_faces() only works on   grids with cells in     @ref GlossFaceOrientation   "standard orientation".
   ...
   * local face index   <code>2*direction</code>   and boundary indicator   @p
   * b_id     and, similarly, a 'right' boundary consisting of all face with
   * local face   index   <code>2*direction+1</code>   and boundary indicator
   * @p b_id.   Faces   with coordinates only differing in the   @p direction
   * component are   identified.     This function will collect periodic face
   * pairs on the coarsest mesh level   and add them to   @p matched_pairs
   * leaving the original contents intact.     See above function for further
   * details.
   * @note   This version of collect_periodic_faces() will not work on   meshes with cells not in     @ref GlossFaceOrientation   "standard orientation".
   *
   */
  template <typename MeshType>
  void
   ...
   * these indicators to child cells, faces, and edges.     The optional
   * parameter   @p reset_boundary_ids,   indicates whether this   function
  ...
  /**
   * Map the given boundary ids to the given manifold ids on faces and   edges
   * at the boundary.     This function copies the boundary ids of the
   * boundary faces and   edges that are present in the parameter   @p
   * src_boundary_ids   to   the corresponding manifold id in   @p
   * dst_manifold_ids,   of the same   faces and edges.     If the optional
   * parameter   @p reset_boundary_ids   is non empty,   each boundary id in
   * @p src_boundary_ids,   is replaced with the   corresponding boundary id
   * in   @p reset_boundary_ids.       An exception is thrown if the size of
   * the input vectors do not   match. If a boundary id indicated in   @p
   * src_boundary_ids   is not   present in the triangulation, it is simply
   ...
   * underlying manifold.     When reading a Triangulation from a supported
   * input format, typical   information that can be stored in a file are
   * boundary conditions for   boundary faces (which we store in the
   * boundary_id of the faces), material   types for cells (which we store in
   * the material_id of the cells) and in   some cases subdomain ids for cells
   * (which we store in the subdomain_id of   the cell).     If you read one
   * of these grids into a Triangulation, you might still want   to use the
   * material_id specified in the input file as a manifold_id   description.
   * In this case you can associate a Manifold object to internal   cells, and
   * this object will be used by the Triangulation to query   Manifold objects
   * for new points. This function iterates over active cells   and copies the
   * material_ids to the manifold_ids.     The optional parameter   @p
   * compute_face_ids,   indicates whether this   function should also set the
   * manifold_ids of the faces (both for internal   faces and for faces on the
   * boundary). If set to true, then each face will   get a manifold_id equal
   * to the minimum of the surrounding manifold_ids,   ensuring that a unique
   * manifold id is selected for each face of the   Triangulation. By default,
   * face manifold_ids are not computed.
   * @ingroup manifold
   *
   */
  template <int dim, int spacedim>
   ...
   * interior and on the boundary) to the value returned by the     @p
   * disambiguation_function   method, called with the set of   manifold
   * indicators of the cells that share the same face or edge.     By default,
   * the   @p disambiguation_function   returns     numbers::flat_manifold_id
   * when the set has size greater than one (i.e.,   when it is not possible
   * to decide what manifold indicator a face or edge   should have according
   * to the manifold indicators of the adjacent cells)   and it returns the
   * manifold indicator contained in the set when it has   dimension one
   * (i.e., when all adjacent cells and faces have the same   manifold
   * indicator).     The parameter   @p overwrite_only_flat_manifold_ids
   * allows you to specify   what to do when a face or an edge already has a
   * manifold indicator   different from   numbers::flat_manifold_id.   If the
   * flag is   @p true,   the edge   or face will maintain its original
   * manifold indicator.   If it is   @p false,   then also the manifold
   * indicator of these faces and edges   is set according to the return value
   * of the   @p disambiguation_function.
   *
   */
  template <int dim, int spacedim>
  void
   ...
   * <code>std_cxx17::optional@<DataType@></code>   to indicate that nothing
   * has     to be sent for this cell.     @param   unpack The function that
   * will be called for each ghost cell     for which data was sent, i.e., for
   * which the   @p pack   function     on the sending side returned a
   * non-empty   std_cxx17::optional   object.     The   @p unpack   function
   * is then called with the data sent by the     processor that owns that
   * cell.     @param   cell_filter Only cells are communicated where this
   * filter function returns     the value `true`. In the default case, the
   * function returns true on all   cells and thus, all relevant cells are
   * communicated.     <h4> An example </h4>     Here is an example that shows
   * how this function is to be used   in a concrete context. It is taken from
   * the code that makes   sure that the   @p active_fe_index   (a single
   * unsigned integer) is   transported from locally owned cells where one can
   * set it in   DoFHandler objects with hp-capabilities, to the corresponding
   * ghost cells   on other processors to ensure that one can query the right
   * value also on   those processors:
   * @code
   ...
   *                                                  unpack);
   * @endcode
   * You will notice that the   @p pack   lambda function returns an `unsigned
   * int`,   not a   `std_cxx17::optional<unsigned   int>`. The former
   * converts   automatically to the latter, implying that data will always be
   * transported   to the other processor.     (In reality, the   @p unpack
   * function needs to be a bit more   complicated because it is not allowed
   * to call     DoFAccessor::set_active_fe_index()   on ghost cells. Rather,
   * the     @p unpack   function directly accesses internal data structures.
   ...
   * processes.     In addition to the parameters of
   * exchange_cell_data_to_ghosts(), this   function allows to provide a   @p
   * cell_filter   function, which can be used to only   communicate marked
   * cells. In the default case, all relevant cells are   communicated.
   *
   */
  template <typename DataType, typename MeshType>
  void
...
* */
  ...
  /**
   * In this
   * collective operation each process provides a vector   of bounding boxes
   * and a communicator.   All these vectors are gathered on each of the
   * processes,   organized in a search tree which, and then returned.     The
   * idea is that the vector of bounding boxes describes a   relevant property
   * of the computations on each process   individually, which could also be
   * of use to other processes. An   example would be if the input vector of
   * bounding boxes   corresponded to a covering of the locally owned
   * partition of a   mesh (see     @ref GlossLocallyOwnedCell  )   of a
   * parallel::distributed::Triangulation   object. While these may   overlap
   * the bounding boxes of other processes, finding which   process owns the
   * cell that encloses a given point is vastly   easier if the process trying
   * to figure this out has a list of   bounding boxes for each of the other
   * processes at hand.     The returned search tree object is an r-tree with
   * packing   algorithm, which is provided by boost library. See
   * https://www.boost.org/doc/libs/1_67_0/libs/geometry/doc/html/geometry/spatial_indexes/introduction.html
   * for more information.     In the returned tree, each node contains a pair
   * of elements:   the first being a bounding box,   the second being the
   * rank of the process whose local description   contains the bounding box.
   ...
   * coincide due to periodicity.     Coinciding vertices are put into a
   * group, e.g.: [1, 25, 51], which is   labeled by an arbitrary element from
   * it, e.g.: "1". All coinciding vertices   store the label to its group, so
   * that they can quickly access all the   coinciding vertices in that group:
   * e.g.: 51
   *
   * ->  "1"
   *
   * -> [1, 25, 51]       @param[in]   tria Triangulation.     @param[out]
   * coinciding_vertex_groups A map of equivalence classes (of
   * coinciding vertices) labeled by an arbitrary element from them.
   * Vertices not coinciding are ignored.     @param[out]
   * vertex_to_coinciding_vertex_group Map of a vertex to the label
   ...
   * subdomains are adjacent to that vertex.       @param[in]   tria
   * Triangulation.
   *
   */
  template <int dim, int spacedim>
  ...
  /**
   * A structure that allows the transfer of cell data of type   @p T   from
   * one processor   to another. It corresponds to a packed buffer that stores
   * a vector of   CellId and a vector of type   @p T.       This class
   * facilitates the transfer by providing the save/load functions   that are
   * able to pack up the vector of CellId's and the associated   data of type
   * @p T   into a stream.     Type   @p T   is assumed to be serializable by
   * <code>boost::serialization</code>   (for   example   <code>unsigned
   * int</code> or <code>std::vector@<double@></code>  ).
   *
   */
...
#  endif
   ...
   * for creating data structures (vectors of Point and CellData) to   create
   * a linear/bilinear surface mesh on the iso line/contour of a   scalar
   * field.     To improve the approximation of the iso line/contour and the
   * resulting   linear surface mesh, one increases the number of subdivision
   * so that the   algorithm is not run on a cell but on subcells with vertex
   * values having   been interpolated from the cell values.
  ...
  public:
    ...
    /**
     * Process all locally-owned cells and fill   @p vertices   and   @p cells
    ...
    /**
     * Process the provided cell and fill   @p vertices   and   @p cells   for
     * all cells     that are cut.
  ...
  private:
     ...
     * @note   Subcells with saddle points are ignored. Please increase the
     * number       of subdivisions in this case.
     *
     */
    void
    ...
    /**
     * Number of subdivisions each cell is subdivided into in each direction
...
namespace GridTools
{
  ...
  namespace internal
  {
    ...
    {
      /**
       * The method   GridTools::project_to_object   requires taking
       * derivatives       along the surface of a simplex. In general these
       * cannot be       approximated with finite differences but special
       * differences of the       form                 df/dx_i
       *
       *  - df/dx_j               <em>  can  </em>   be approximated. This   <code>struct</code>   just stores       the two derivatives approximated by the stencil (in the case of the       example above   <code>i</code> and <code>j</code>  ).
       *
       */
       ...
       * three-point centered scheme. This is used below in a 1D Newton
       * method.
       *
       */
      template <typename F>
       ...
       *
       *  - df/dx_j             where   <code>i</code> and <code>j</code>   are specified by   @p         cross_derivative. The derivative approximation is at   @p center   with a       step size of   @p step   and function   @p f.
       *
       */
      ...
      /**
       * The optimization algorithm used in   GridTools::project_to_object
       * is       essentially a gradient descent method. This function
       * computes entries       in the gradient of the objective function; see
       * the description in the       comments inside
       * GridTools::project_to_object   for more information.
       *
       */
