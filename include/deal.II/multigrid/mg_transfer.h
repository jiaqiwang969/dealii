...
public:
   ...
   * for a globally refined mesh only the finest level in   @p dst   is filled
   * as a   plain copy of   @p src.   All the other level objects are left
   * untouched.
   *
   */
  ...
  /**
   * Transfer from multi-level vector to normal vector.     Copies data from
   * active portions of an MGVector into the respective   positions of a
   * <tt>Vector<number></tt>. In order to keep the result   consistent,
   ...
   * other.     By default, if this function is not called, block vectors have
   * as many   blocks as the finite element has vector components. However,
   * this can be   changed by calling this function with an array that
   * describes how vector   components are to be grouped into blocks. The
   * meaning of the argument is   the same as the one given to the
   * DoFTools::count_dofs_per_component     function.
   *
   */
  void
...
protected:
   ...
   * index pairs locally owned     The data is organized as follows: one
   * vector per level. Each element of   these vectors contains first the
   * global index, then the level index.
   *
   */
  std::vector<
   ...
   * level degree of freedom is not.     Organization of the data is like for
   * @p copy_indices_mine.
   *
   */
  std::vector<
   ...
   * global degree of freedom is not.     Organization of the data is like for
   * @p copy_indices_mine.
   *
   */
  std::vector<
...
public:
   ...
   * Transfer from a vector on the global grid to vectors defined on each of
   * the levels separately for the active degrees of freedom. In particular,
   * for   a globally refined mesh only the finest level in   @p dst   is
   * filled as a   plain copy of   @p src.   All the other level objects are
   * left untouched.
   *
   */
...
protected:
  /**
   * Internal function to perform transfer of residuals or solutions   basesd
   * on the flag   @p solution_transfer.
   *
   */
  template <int dim, typename Number2, int spacedim>
  void
  ...
  /**
   * Mapping for the copy_to_mg() and copy_from_mg() functions. Here only
   * index pairs locally owned is stored.     The data is organized as
   * follows: one table per level. This table has two   rows. The first row
   * contains the global index, the second one the level   index.
   *
   */
 ...
 * level, but requires additional memory. See MGTransferBase to find out which
 * of the transfer classes is best for your needs.
 *
 *
 */
template <typename VectorType>
...
public:
  ...
  /**
   * Actually build the information required for the transfer operations.
   * Needs   to be called before prolongate() or restrict_and_add() can be
   * used.
   *
   */
  template <int dim, int spacedim>
   ...
   * element. The previous content of <tt>dst</tt> is overwritten.       @arg
   * src is a vector with as many elements as there are degrees of   freedom
   * on the coarser level involved.       @arg   dst has as many elements as
   ...
   * <tt>from_level-1</tt> using the transpose operation of the   @p
   * prolongate     method. If the region covered by cells on level
   * <tt>from_level</tt> is   smaller than that of level <tt>from_level-1</tt>
   * (local refinement), then   some degrees of freedom in <tt>dst</tt> are
   * active and will not be   altered. For the other degrees of freedom, the
   * result of the restriction   is added.       @arg   src is a vector with
   * as many elements as there are degrees of   freedom on the finer level
   * involved.       @arg   dst has as many elements as there are degrees of
