...
namespace MeshWorker
{
   ...
   * problems, and the evaluation of finite element fields.     This class is
   * a drop in ScratchData to use with the   WorkStream::run()     function
   * and with the   MeshWorker::mesh_loop   function().     The ScratchData
   * class has three main goals:
   *
   *
   *
   *
   ...
   * the   ScratchData::reinit()   function that takes an active cell
   * iterator,   then subsequent calls to methods that internally need an
   * FEValuesBase   object will use the internal FEValues object initialized
   * with the given   cell to perform their calculations. On the other hand,
   * if you have called   the   ScratchData::reinit()   method that also takes
   * a face index, all   subsequent calls to methods that need an FEValuesBase
   * object, will use an   internally stored FEFaceValues object, initialized
   * with the cell and face   index passed to the   ScratchData::reinit()
   * function. The same applies for   the   ScratchData::reinit()   method
   * that takes three arguments: the cell, the   face index, and the subface
   * index.     The user code should be structured without interleaving work
   * on cells and   work on faces.     Consider, for example, the following
   * snippet of code:
   * @code
  ...
  public:
    /**
     * Create an empty ScratchData object. A SmartPointer pointing to       @p
     * mapping   and   @p fe   is stored internally. Make sure they live
     * longer     than this class instance.         The constructor does not
     * initialize any of the internal FEValues objects.     These are
     * initialized the first time one of the reinit() functions is     called,
     * using the arguments passed here.           @param   mapping The mapping
     * to use in the internal FEValues objects       @param   fe The finite
     * element       @param   quadrature The cell quadrature       @param
     * update_flags UpdateFlags for the current cell FEValues and     neighbor
     * cell FEValues       @param   face_quadrature Face quadrature, used for
     * FEFaceValues and     FESubfaceValues for both the current cell and the
     * neighbor cell       @param   face_update_flags UpdateFlags used for
     * FEFaceValues and     FESubfaceValues for both the current cell and the
     * neighbor cell
     *
     */
     ...
     * different flags for neighbor cells and faces.           @param
     * mapping The mapping to use in the internal FEValues objects
     * @param   fe The finite element       @param   quadrature The cell
     * quadrature       @param   update_flags UpdateFlags for the current cell
     * FEValues       @param   neighbor_update_flags UpdateFlags for the
     * neighbor cell FEValues       @param   face_quadrature Face quadrature,
     * used for FEFaceValues and     FESubfaceValues for both the current cell
     * and the neighbor cell       @param   face_update_flags UpdateFlags used
     * for FEFaceValues and     FESubfaceValues for the current cell
     * @param   neighbor_face_update_flags UpdateFlags used for FEFaceValues
     * and     FESubfaceValues for the neighbor cell
     *
     */
    ScratchData(
      const Mapping<dim, spacedim> &      mapping,
     ...
     * @param   fe The finite element       @param   quadrature The cell
     * quadrature       @param   update_flags UpdateFlags for the current cell
     * FEValues and     neighbor cell FEValues       @param   face_quadrature
     * Face quadrature, used for FEFaceValues and     FESubfaceValues for both
     * the current cell and the neighbor cell       @param   face_update_flags
     * UpdateFlags used for FEFaceValues and     FESubfaceValues for both the
     * current cell and the neighbor cell
     *
     */
    ScratchData(
      const FiniteElement<dim, spacedim> &fe,
     ...
     * a reference to it.         After calling this function,
     * get_current_fe_values() will return the     same object of this method,
     * as an FEValuesBase reference.
     *
     */
    ...
    /**
     * Initialize the internal FEFaceValues to use the given   @p face_no   on
     * the given       @p cell,   and return a reference to it.         After
     * calling this function, get_current_fe_values() will return the     same
     * object of this method, as an FEValuesBase reference.
     *
     */
    ...
    /**
     * Initialize the internal FESubfaceValues to use the given   @p
     * subface_no,       on   @p face_no,   on the given   @p cell,   and
     * return a reference to it.         After calling this function,
     * get_current_fe_values() will return the     same object of this method,
     * as an FEValuesBase reference.         If   @p subface_no   is
     * numbers::invalid_unsigned_int,   the reinit() function     that takes
     * only the   @p cell   and the   @p face_no   is called.
     *
     */
     ...
     * return a reference to it.         After calling this function,
     * get_local_dof_indices(),     get_quadrature_points(),
     * get_normal_vectors(), and get_JxW_values() will     be forwarded to the
     * local FEInterfaceValues object. The methods     get_current_fe_values()
     * will return the FEValuesBase associated to the     current cell, while
     * get_neighbor_fe_values() will be associated with the     neighbor cell.
     * The method get_local_dof_indices() will return the     same result of
     * FEInterfaceValues::get_interface_dof_to_dof_indices(),       while the
     * get_neighbor_dof_indices() will return the local dof indices     of the
     * neighbor cell.
     *
     */
    ...
    /**
     * Get the currently initialized FEValues.         This function will
     * return the internal FEValues if the     reinit(cell) function was
     * called last. If the reinit(cell, face_no)     function was called, then
     * this function returns the internal     FEFaceValues, and if the
     * reinit(cell, face_no, subface_no) function was     called (with a valid
     * @p subface_no   argument), it returns the internal     FESubfaceValues
     * object.
     *
     */
    ...
    /**
     * Initialize the internal neighbor FEValues to use the given   @p cell,
     * and     return a reference to it.         After calling this function,
     * get_current_neighbor_fe_values() will return     the same object of
     * this method, as an FEValuesBase reference.
     *
     */
    const FEValues<dim, spacedim> &
    reinit_neighbor(
     ...
     * Initialize the internal FEFaceValues to use the given   @p face_no   on
     * the     given   @p cell,   and return a reference to it.         After
     * calling this function, get_current_neighbor_fe_values() will return
     ...
     * return a reference to it.         After calling this function,
     * get_current_neighbor_fe_values() will return     the same object of
     * this method, as an FEValuesBase reference.         If   @p subface_no
     * is   numbers::invalid_unsigned_int,   the reinit() function     that
     * takes only the   @p cell   and the   @p face_no   is called.
     *
     */
    const FEFaceValuesBase<dim, spacedim> &
    reinit_neighbor(
    ...
    /**
     * Get the currently initialized neighbor FEValues.         This function
     * will return the neighbor FEValues if the     reinit_neighbor(cell)
     * function was called last. If the     reinit_neighbor(cell, face_no)
     * function was called, then this function     returns the internal
     * neighbor FEFaceValues, and if the     reinit_neighbor(cell, face_no,
     * subface_no) function was     called (with a valid   @p subface_no
     ...
     * cell.         Before you call this function, you have to make sure you
     * have previously     called one of the reinit() functions.         At
     * every call of this function, a new vector of dof values is generated
     ...
     * If you give a unique   @p global_vector_name,   then for each cell you
     * are     guaranteed to get a unique vector of independent dofs of the
     * same type     as the dummy variable. If you use an automatic
     * differentiation number     type (like   Sacado::Fad::DFad<double>,
     ...
     * to perform automatic differentiation.         You can access the
     * extracted local dof values by calling the method
     * get_local_dof_values() with the same   @p global_vector_name   argument
     * you passed here.         Notice that using this initialization strategy
     * renders the use of this     ScratchData object incompatible with the AD
     * helper classes (since they     do their own data management). In
     * particular, it is necessary for the     user to manage all of the AD
     * data themselves (both before and after this     call).
     *
     */
     ...
     * information through this method.         Both the argument   @p
     * global_vector_name   and the type of the   @p dummy       variable
     * should match the ones you passed to the     extract_local_dof_values()
     * function.
     *
     */
    ...
    /**
     * For the solution vector identified by   @p global_vector_name,
     * compute     the values of the function at the quadrature points, and
     * return a     vector with the correct type deduced by the Extractor you
     * passed as the       @p variable   argument.         Before you can call
     * this method, you need to call the     extract_local_dof_values() method
     * at least once, passing the same       @p global_vector_name   string,
     * and the same type for the variable   @p dummy.           If you have
     * not previously called the extract_local_dof_values() method,     this
     * function will throw an exception.         For this function to work
     * properly, the underlying FEValues,     FEFaceValues, or FESubfaceValues
     * object for which you called one of the     reinit() functions, must
     * have computed the information you are     requesting. To do so, the
     * update_values flag must be an element of the     list of UpdateFlags
     * that you passed to the constructor of this object.     See "The
     * interplay of UpdateFlags, Mapping, and FiniteElement" in     the
     * documentation of the FEValues class for more information.
     *
     */
    ...
    /**
     * For the solution vector identified by   @p global_vector_name,
     * compute     the gradients of the function at the quadrature points, and
     * return a     vector with the correct type deduced by the Extractor you
     * passed as the       @p variable   argument.         Before you can call
     * this method, you need to call the     extract_local_dof_values() method
     * at least once, passing the same       @p global_vector_name   string,
     ...
     * object for which you called one of the     reinit() functions, must
     * have computed the information you are     requesting. To do so, the
     * update_gradients flag must be an element of the     list of UpdateFlags
     * that you passed to the constructor of this object.     See "The
     * interplay of UpdateFlags, Mapping, and FiniteElement" in     the
     * documentation of the FEValues class for more information.
     *
    ...
    /**
     * For the solution vector identified by   @p global_vector_name,
     * compute     the symmetric gradients of the function at the quadrature
     * points, and     return a vector with the correct type deduced by the
     * Extractor you passed     as the       @p variable   argument.
     * Before you can call this method, you need to call the
     * extract_local_dof_values() method at least once, passing the same
     * @p global_vector_name   string, and the same type for the variable   @p
     * dummy.           If you have not previously called the
     * extract_local_dof_values() method,     this function will throw an
     * exception.         For this function to work properly, the underlying
     * FEValues,     FEFaceValues, or FESubfaceValues object for which you
     * called one of the     reinit() functions, must have computed the
     * information you are     requesting. To do so, the update_gradients flag
     * must be an element of the     list of UpdateFlags that you passed to
     * the constructor of this object.     See "The interplay of UpdateFlags,
     * Mapping, and FiniteElement" in     the documentation of the FEValues
    ...
    /**
     * For the solution vector identified by   @p global_vector_name,
     * compute     the divergences of the function at the quadrature points,
     * and return a     vector with the correct type deduced by the Extractor
     * you passed as the       @p variable   argument.         Before you can
     * call this method, you need to call the     extract_local_dof_values()
     * method at least once, passing the same       @p global_vector_name
     * string, and the same type for the variable   @p dummy.           If you
     * have not previously called the extract_local_dof_values() method,
     * this function will throw an exception.         For this function to
     * work properly, the underlying FEValues,     FEFaceValues, or
     * FESubfaceValues object for which you called one of the     reinit()
     * functions, must have computed the information you are     requesting.
     * To do so, the update_gradients flag must be an element of the     list
     * of UpdateFlags that you passed to the constructor of this object.
    ...
    /**
     * For the solution vector identified by   @p global_vector_name,
     * compute     the curls of the function at the quadrature points, and
     * return a     vector with the correct type deduced by the Extractor you
     * passed as the       @p variable   argument.         Before you can call
     * this method, you need to call the     extract_local_dof_values() method
     * at least once, passing the same       @p global_vector_name   string,
    ...
    /**
     * For the solution vector identified by   @p global_vector_name,
     * compute     the hessians of the function at the quadrature points, and
     * return a     vector with the correct type deduced by the Extractor you
     * passed as the       @p variable   argument.         Before you can call
     * this method, you need to call the     extract_local_dof_values() method
     * at least once, passing the same       @p global_vector_name   string,
     ...
     * object for which you called one of the     reinit() functions, must
     * have computed the information you are     requesting. To do so, the
     * update_hessians flag must be an element of the     list of UpdateFlags
     * that you passed to the constructor of this object.     See "The
     * interplay of UpdateFlags, Mapping, and FiniteElement" in     the
     * documentation of the FEValues class for more information.
     *
    ...
    /**
     * For the solution vector identified by   @p global_vector_name,
     * compute     the Laplacians of the function at the quadrature points,
     * and return a     vector with the correct type deduced by the Extractor
     * you passed as the       @p variable   argument.         Before you can
     * call this method, you need to call the     extract_local_dof_values()
     * method at least once, passing the same       @p global_vector_name
     ...
     * FESubfaceValues object for which you called one of the     reinit()
     * functions, must have computed the information you are     requesting.
     * To do so, the update_hessians flag must be an element of the     list
     * of UpdateFlags that you passed to the constructor of this object.
     * See "The interplay of UpdateFlags, Mapping, and FiniteElement" in
     * the documentation of the FEValues class for more information.
     *
    ...
    /**
     * For the solution vector identified by   @p global_vector_name,
     * compute     the third_derivatives of the function at the quadrature
     * points, and     return a vector with the correct type deduced by the
     * Extractor you passed     as the   @p variable   argument.
     * Before you can call this method, you need to call the
     * extract_local_dof_values() method at least once, passing the same
     ...
     * FEValues,     FEFaceValues, or FESubfaceValues object for which you
     * called one of the     reinit() functions, must have computed the
     * information you are     requesting. To do so, the
     * update_3rd_derivatives flag must be an     element of the list of
     * UpdateFlags that you passed to the constructor of     this object. See
     * "The interplay of UpdateFlags, Mapping, and     FiniteElement" in the
     * documentation of the FEValues for more information.
     *
     */
    template <typename Extractor, typename Number = double>
    const std::vector<typename FEValuesViews::View<dim, spacedim, Extractor>::
  ...
  private:
    ...
    /**
     * UpdateFlags to use when initializing the neighbor cell FEValues
     * objects.
     *
     */
