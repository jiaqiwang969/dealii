...
namespace LineMinimization
{
   ...
   * $f(x\_low)$   and   $f(x\_hi)$   and the gradient   $g(x\_low)$  , return
   * the local   minimizer of the quadratic interpolation function.     The
   * return type is optional to fit with similar functions that may   not have
   * a solution for given parameters.
   *
   */
  ...
  /**
   * Given   $x\_low$   and   $x\_hi$   together with values of function
   * $f(x\_low)$   and   $f(x\_hi)$   and its gradients (  $g(x\_low)*g(x\_hi)
   * < 0$  ) at   those points, return the local minimizer of the cubic
   * interpolation   function (that is, the location where the cubic
   * interpolation function   attains its minimum value).     The return type
   * is optional as the real-valued solution might not exist.
   *
   */
  template <typename NumberType>
  std_cxx17::optional<NumberType>
   ...
   * $f\_low=f(x\_low)$  ,   $f\_hi=f(x\_hi)$   and   $f\_rec(x\_rec)$     and
   * has derivatve   $g\_low$   at   $x\_low$  .     The return type is
   * optional as the real-valued solution might not exist.
   *
   */
  template <typename NumberType>
  std_cxx17::optional<NumberType>
   ...
   * @p f_hi   , and   @p f_rec[0]   at three points   @p x_low   ,   @p x_hi
   * , and     @p x_rec[0]   as well as the derivatives at two points   @p
   * g_low   and   @p g_hi.     The returned point should be within the bounds
   * @p bounds   .     This function will first try to perform a cubic_fit().
   * If its unsuccessful,   or if the minimum is not within the provided   @p
   * bounds,   a quadratic_fit()   will be performed. The function will
   * fallback to a bisection method if   quadratic_fit() fails as well.
   *
   */
  template <typename NumberType>
  NumberType
  ...
  /**
   * Perform a line search in   $(0,max]$   with strong Wolfe conditions   \f[
   * f(\alpha) \le f(0) + \alpha \mu f'(0) \\ |f'(\alpha)| \le \eta |f'(0)|
   * \f]   using the one dimensional function   @p func   in conjunction with
   * a function   @p interpolate     to choose a new point from the interval
   * based on the function values and   derivatives at its ends.   The
   * parameter   @p a1   is a trial estimate of the first step.
   * Interpolation can be done using either poly_fit() or
   * poly_fit_three_points(), or any other function that has a similar
   * signature.     The function implements Algorithms 2.6.2 and 2.6.4 on
   * pages 34-35 in
   * @code{.bib}
   * @book{Fletcher2013,
   ...
   * }
   * @endcode
   * It consists of a bracketing phase and a zoom phase, where   @p
   * interpolate   is used.     Two examples of use might be as follows:   In
   * the first example, we wish to find the minimum of the function     $100
   * x^4 + (1-x)^2$  . To find the approximate solution using line search
   ...
   * the load-step size is too large. The following illustrates the basic
   * steps   necessary to utilize the scheme within the context of a global
   * nonlinear   solver:
   * @code
   ...
   * }
   * @endcode
   * @param   func A one dimensional function which returns value and
   * derivative   at the given point.     @param   f0 The function value at
   * the origin.     @param   g0 The function derivative at the origin.
   * @param   interpolate A function which determines how interpolation is
   * done   during the zoom phase. It takes values and derivatives at the
   * current   interval/bracket (  $f\_low$  ,   $f\_hi$  ) as well as up to 5
   * values and   derivatives at previous steps. The returned value is to be
   * provided within   the given bounds.     @param   a1 Initial trial step
   * for the bracketing phase.     @param   eta A parameter in the second
   * Wolfe condition (curvature condition).     @param   mu A parameter in the
   * first Wolfe condition (sufficient decrease).     @param   a_max The
   * maximum allowed step size.     @param   max_evaluations The maximum
   * allowed number of function evaluations.     @param   debug_output A flag
   * to output extra debug information into the     <code>deallog</code>
   * static object.     @return   The function returns the step size and the
   * number of times function     @p func   was called.
   *
   */
  template <typename NumberType>
