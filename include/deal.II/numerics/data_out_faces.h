 ...
 * by the class   DataOutBase::Patch()),   one for each face for which output
 * is to be generated. These patches can then be written in several graphical
 * data formats by the functions of the underlying classes. <h3>Interface</h3>
 ...
 *
 *  <h3>Extending this class</h3> The sequence of faces to generate patches
 * from is generated in the same way as in the DataOut class; see there for a
 * description of the respective interface. The functions generating the
 * sequence of faces which shall be used to generate output, are called
 * first_face() and next_face(). Since we need to initialize objects of type
 * FEValues with the faces generated from these functions, it is not
 * sufficient that they only return face iterators. Rather, we need a pair of
 * cell and the number of the face, as the values of finite element fields
 * need not necessarily be unique on a face (think of discontinuous finite
 * elements, where the value of the finite element field depend on the
 * direction from which you approach a face, thus it is necessary to use a
 * pair of cell and face, rather than only a face iterator). Therefore, this
 * class defines an   @p alias   which creates a type   @p FaceDescriptor
 * that is an abbreviation for a pair of cell iterator and face number. The
 * functions   @p first_face   and   @p next_face   operate on objects of this
 * type. Extending this class might, for example, be useful if you only want
 * output from certain portions of the boundary, e.g. as indicated by the
 * boundary indicator of the respective faces. However, it is also conceivable
 * that one generates patches not from boundary faces, but from interior faces
 * that are selected due to other criteria; one application might be to use
 * only those faces where one component of the solution attains a certain
 * value, in order to display the values of other solution components on these
 * faces. Other applications certainly exist, for which the author is not
 * imaginative enough.
...
public:
   ...
   * n_subdivisions See   DataOut::build_patches()   for an extensive
   ...
   * of a curved boundary by using more subdivisions.     Even for non-curved
   * cells the mapping argument can be used for the   Eulerian mappings (see
   * class MappingQ1Eulerian) where a mapping is used   not only to determine
   * the position of points interior to a cell, but also   of the vertices.
   ...
   * deformation of each vertex.       @todo   The   @p mapping   argument
   * should be replaced by a     hp::MappingCollection   in case of a
   ...
   * or, if the   @p surface_only   option was set in the destructor (as is
   * the   default), the first such face that is located on the boundary.
   ...
   * return value.     The default implementation returns the next face of a
   * (locally owned)   active cell, or the next such on the boundary
   * (depending on whether the     @p surface_only   option was provided to
   * the constructor).     This function traverses the mesh active cell by
   * active cell (restricted to   locally owned cells), and then through all
   * faces of the cell. As a result,   interior faces are output twice, a
   * feature that is useful for   discontinuous Galerkin methods or if a
   * DataPostprocessor is used that   might produce results that are
   * discontinuous between cells).     This function can be overloaded in a
   * derived class to select a   different set of faces. Note that the default
   * implementation assumes that   the given   @p face   is active, which is
   * guaranteed as long as first_face()   is also used from the default
   * implementation. Overloading only one of the   two functions should be
   * done with care.
   *
   */
  virtual FaceDescriptor
...
{
  /**
   * @deprecated   Use   dealii::DataOutFaces   without the DoFHandlerType
   * template   instead.
   *
   */
