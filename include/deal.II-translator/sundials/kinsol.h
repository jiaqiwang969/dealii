...
{
  /**
   * Interface to SUNDIALS' nonlinear solver (KINSOL).     KINSOL is a solver
   * for nonlinear algebraic systems in residual form   $F(u) = 0$   or fixed
   * point form   $G(u) = u$  , where   $u$   is a vector which we will
   * assume to be in   ${\mathbb R}^n$   or   ${\mathbb C}^n$  , but that may
   * also have   a block structure and may be distributed in parallel
   * computations; the   functions   $F$   and   $G$   satisfy   $F,G:{\mathbb
   * R}^N \to{\mathbb R}^N$   or     $F,G:{\mathbb C}^N \to{\mathbb C}^N$  .
   * It includes a Newton-Krylov solver   as well as Picard and fixed point
   * solvers, both of which can be   accelerated with Anderson acceleration.
   * KINSOL is based on the previous   Fortran package NKSOL of Brown and
   * Saad. An example of using KINSOL   can be found in the   step-77
   * tutorial program.     KINSOL's Newton solver employs the inexact Newton
   * method. As this solver   is intended mainly for large systems, the user
   * is required to provide   their own solver function.     At the highest
   * level, KINSOL implements the following iteration   scheme:
   *
   *
   *
   *
   ...
   *
   *
   *  -   $\|\lambda \delta_{n} \|_{D_u,\infty} \leq $  tolerance* with outdated     Jacobian information.     KINSOL allows changes to the above strategy through optional solver   inputs. The user can disable the initial Jacobian information evaluation   or change the default value of the number of nonlinear iterations after   which a Jacobian information update is enforced.     To address the case of ill-conditioned nonlinear systems, KINSOL allows   prescribing scaling factors both for the solution vector and for the   residual vector. For scaling to be used, the user may supply the function   get_solution_scaling(), that returns values   $D_u$  , which are diagonal   elements of the scaling matrix such that   $D_u u_n$   has all components   roughly the same magnitude when   $u_n$   is close to a solution, and   get_function_scaling(), that supply values   $D_F$  , which are diagonal   scaling matrix elements such that   $D_F F$   has all components roughly the   same magnitude when   $u_n$   isnot* too close to a solution.     When scaling values are provided for the solution vector, these values are   automatically incorporated into the calculation of the perturbations used   for the default difference quotient approximations for Jacobian   information if the user does not supply a Jacobian solver through the   solve_jacobian_system() function.     Two methods of applying a computed step   $\delta_n$   to the previously   computed solution vector are implemented. The first and simplest is the   standard Newton strategy which applies the update with a constant     $\lambda$   always set to 1. The other method is a global strategy, which   attempts to use the direction implied by   $\delta_n$   in the most efficient   way for furthering convergence of the nonlinear problem. This technique is   implemented in the second strategy, called Linesearch. This option employs   both the   $\alpha$   and   $\beta$   conditions of the Goldstein-Armijo   linesearch algorithm given inJ. E. Dennis and R. B. Schnabel. "Numerical   Methods for Unconstrained Optimization and Nonlinear Equations." SIAM,   Philadelphia, 1996.*, where   $\lambda$   is chosen to guarantee a sufficient   decrease in   $F$   relative to the step length as well as a minimum step   length relative to the initial rate of decrease of   $F$  . One property of the   algorithm is that the full Newton step tends to be taken close to the   solution.     The basic fixed-point iteration scheme implemented in KINSOL is given by:
   *
   *
   *
   *
   ...
   *
   *
   *  - Test for convergence     At each stage in the iteration process, function   $G$   is applied to the   current iterate to produce a new iterate,   $u_{n+1}$  . A test for   convergence is made before the iteration continues.     For Picard iteration, as implemented in KINSOL, we consider a special form   of the nonlinear function   $F$  , such that   $F(u) = Lu
   *
   * - N(u)$  , where   $L$   is   a constant nonsingular matrix and   $N$
   * is (in general) nonlinear.     Then the fixed-point function   $G$   is
   * defined as   $G(u) = u
   *
   * - L^{-1}F(u)$  .   Within each iteration, the Picard step is computed
   * then added to   $u_n$   to   produce the new iterate. Next, the nonlinear
   * residual function is   evaluated at the new iterate, and convergence is
   * checked. The Picard and   fixed point methods can be significantly
   * accelerated using Anderson's   method.     The user has to provide the
   ...
   *
   *
   *  - iteration_function;     Specifying residual() allows the user to use Newton and Picard strategies   (i.e.,   $F(u)=0$   will be solved), while specifying iteration_function(), a   fixed point iteration will be used (i.e.,   $G(u)=u$   will be solved).     If the use of a Newton or Picard method is desired, then the user should   also supply
   *
   *
   *
   *
  ...
  public:
    ...
    public:
      /**
       * KINSOL solution strategy. KINSOL includes a Newton-Krylov solver
       * (both       local and global) as well as Picard and fixed point
       * solvers.
       *
       */
      ...
      /**
       * Initialization parameters for KINSOL.             Global parameters:
       * @param   strategy Solution strategy         @param
       * maximum_non_linear_iterations Maximum number of nonlinear
       * iterations         @param   function_tolerance %Function norm
       * stopping tolerance         @param   step_tolerance Scaled step
       * stopping tolerance             Newton parameters:
       * @param   no_init_setup No initial matrix setup         @param
       ...
       * Newton step         @param   dq_relative_error Relative error for
       * different quotient       computation             Line search
       * parameters:               @param   maximum_beta_failures Maximum
       * number of beta-condition failures             Fixed point and Picard
       * parameters:               @param   anderson_subspace_size Anderson
       * acceleration subspace size
       *
       */
      ...
      /**
       * The solution strategy to use. If you choose
       * SolutionStrategy::newton         or   SolutionStrategy::linesearch,
       * you have to provide also the function       residual(). If you choose
       * SolutionStrategy::picard   or         SolutionStrategy::fixed_point,
       * you have to provide also the function       iteration_function().
       *
       */
      ...
      /**
       * A scalar used as a stopping tolerance on the scaled       maximum
       * norm of the system function   $F(u)$   or   $G(u)$  .             If
       * set to zero, default values provided by KINSOL will be used.
       *
       */
      double function_tolerance;

      /**
       * A scalar used as a stopping tolerance on the minimum       scaled
       * step length.             If set to zero, default values provided by
       * KINSOL will be used.
       *
       */
      ...
      /**
       * Whether an initial call to the preconditioner or Jacobian       setup
       * function should be made or not.             A call to this function
       * is useful when solving a sequence of problems,       in which the
       * final preconditioner or Jacobian value from one problem       is to
       * be used initially for the next problem.
       *
       */
      bool no_init_setup;
       ...
       * difference quotient approximation to the Jacobian matrix when the
       * user       does not supply a solve_jacobian_system_matrix() function.
       * If set to zero, default values provided by KINSOL will be used.
       *
       */
      ...
      /**
       * The maximum number of beta-condition failures in the       linesearch
       * algorithm. Only used if
      ...
      /**
       * The size of the subspace used with Anderson acceleration       in
       * conjunction with Picard or fixed-point iteration.             If you
       * set this to 0, no acceleration is used.
       *
       */
     ...
     * parameters.           @param   data KINSOL configuration data
     ...
     * SolutionStrategy::newton   or   SolutionStrategy::linesearch
     * strategies     were selected.         This function should return:
     *
     *
     *
     *
    ...
    /**
     * A function object that users should supply and that is intended to
     * compute the iteration function   $G(u)$   for the fixed point and
     * Picard     iteration. This function is only used if the
     * SolutionStrategy::fixed_point   or   SolutionStrategy::picard
     * strategies     were selected.         This function should return:
     *
     *
     *
     ...
     * solve_jacobian_system().         The job of setup_jacobian() is to
     * prepare the linear solver for     subsequent calls to
     * solve_jacobian_system(), in the solution of linear     systems   $Ax =
     * b$  . The exact nature of this system depends on the
     * SolutionStrategy that has been selected.         In the cases strategy
     * =   SolutionStrategy::newton   or       SolutionStrategy::linesearch,
     * $A$   is the Jacobian   $J = \partial
     * F/\partial u$  . If strategy =   SolutionStrategy::picard,     $A$   is
     * the     approximate Jacobian matrix   $L$  . If strategy =
     * SolutionStrategy::fixed_point,   then linear systems do not arise, and
     * this     function is never called.         The setup_jacobian()
     * function may call a user-supplied function, or a     function within
     * the linear solver module, to compute Jacobian-related     data that is
     * required by the linear solver. It may also preprocess that     data as
     ...
     * The point of this function is that     setup_jacobian() function is not
     * called at every Newton iteration,     but only as frequently as the
     * solver determines that it is appropriate     to perform the setup task.
     * In this way, Jacobian-related data generated     by setup_jacobian() is
     * expected to be used over a number of Newton     iterations. KINSOL
     * determines itself when it is beneficial to regenerate     the Jacobian
     * and associated information (such as preconditioners     computed for
     * the Jacobian), thereby saving the effort to regenerate     the Jacobian
     * matrix and a preconditioner for it whenever possible.           @param
     * current_u Current value of   $u$         @param   current_f Current
     * value of   $F(u)$   or   $G(u)$           This function should return:
     *
     *
     *
     *
     ...
     * `solve_with_jacobian` callback described below.         A function
     * object that users may supply and that is intended to solve     a linear
     * system with the Jacobian matrix. This function will be called by
     * KINSOL (possibly several times) after setup_jacobian() has been called
     * at     least once. KINSOL tries to do its best to call setup_jacobian()
     * the     minimum number of times. If convergence can be achieved without
     * updating     the Jacobian, then KINSOL does not call setup_jacobian()
     * again. If, on     the contrary, internal KINSOL convergence tests fail,
     * then KINSOL calls     setup_jacobian() again with updated vectors and
     ...
     * not specify a `solve_jacobian_system` or `solve_with_jacobian`
     * function, then only a fixed point iteration strategy can be used.
     * Notice     that this may not converge, or may converge very slowly.
     ...
     * (or in the `setup_jacobian` callback above).             Arguments to
     * the function are:           @param[in]   ycur The current   $y$
     * vector for the current KINSOL     internal step. In the documentation
     * above, this   $y$   vector is generally     denoted by   $u$  .
     * @param[in]   fcur The current value of the implicit right-hand side at
     * `ycur`,   $f_I (t_n, ypred)$  .       @param[in]   rhs The system right
     * hand side to solve for       @param[out]   dst The solution of
     * $J^{-1} src$           This function should return:
     *
     *
     *
     *
     ...
     *
     *
     *  - <0: Unrecoverable error the computation will be aborted and an     assertion will be thrown.           @warning   Starting with SUNDIALS 4.1, SUNDIALS no longer provides the       `ycur` and `fcur` variables
     *
     ...
     * to aprevious* iterate), then you will also have to set the
     ...
     * a linear system with the Jacobian matrix. This function will be called
     * by     KINSOL (possibly several times) after setup_jacobian() has been
     * called at     least once. KINSOL tries to do its best to call
     * setup_jacobian() the     minimum number of times. If convergence can be
     * achieved without updating     the Jacobian, then KINSOL does not call
     * setup_jacobian() again. If, on     the contrary, internal KINSOL
     * convergence tests fail, then KINSOL calls     setup_jacobian() again
     * with updated vectors and coefficients so that     successive calls to
     * solve_jacobian_systems() lead to better convergence     in the Newton
     * process.         If you do not specify a `solve_with_jacobian`
     * function, then only a     fixed point iteration strategy can be used.
     * Notice that this may not     converge, or may converge very slowly.
     * A call to this function should store in `dst` the result of   $J^{-1}$
     * applied to `rhs`, i.e., `J*dst = rhs`. It is the user's responsibility
     ...
     * to this callback is also provided with a tolerance to the linear
     * solver,     indicating that it is not necessary to solve the linear
     * system with     the Jacobian matrix exactly, but only to a tolerance
     * that KINSOL will     adapt over time.         Arguments to the function
     * are:           @param[in]   rhs The system right hand side to solve
     * for.       @param[out]   dst The solution of   $J^{-1} src$  .
     * @param[in]   tolerance The tolerance with which to solve the linear
     * system       of equations.         This function should return:
     *
     *
     *
     *
     ...
     * A function object that users may supply and that is intended to return
     * a     vector whose components are the weights used by KINSOL to compute
     * the     vector norm of the function evaluation away from the solution.
     * The     implementation of this function is optional, and it is used
     * only if     implemented.         The point of this function and the
     * scaling vector it returns is similar     to the one discussed above for
     * `get_solution_scaling`, except that it is     for a vector that scales
     * the components of the function   $F(U)$  , rather     than the
     * components of   $U$  , when computing norms. As above, if no
