 ...
 * []</tt>.
 * Usually, the two grids will be refined differently. Then, the value returned for an iterator on the source grid will be either:   <ul>     <li>   The same cell on the destination grid, if it exists there;   <li>   The most refined cell of the destination grid from which the pendant of the source cell could be obtained by refinement. This cell is always active and has a refinement level less than that of the source cell.   </ul>   Keys for this map are all cells on the source grid, whether active or not.
 ...
 * @tparam   MeshType This class may be used with any class that satisfies the   @ref ConceptMeshType   "MeshType concept".
 * The extension to other classes offering iterator functions and some minor
 * additional requirements is simple. Note that this class could in principle
 * be based on the C++   <tt>std::map<Key,Value></tt>   data type. Instead, it
 * uses another data format which is more effective both in terms of computing
 * time for access as well as with regard to memory consumption.
 *
 *  <h3>Usage</h3> In practice, use of this class is as follows:
 *
 * @code
...
private:
  ...
  /**
   * Set the value of the key   @p src_cell   to   @p dst_cell.   Do so as
   * well for   all the children and their children of   @p src_cell.   This
   * function is used   for cells which are more refined on   @p src_grid
   * than on   @p dst_grid;   then   all values of the hierarchy of cells and
   * their children point to one cell   on the   @p dst_grid.
   *
   */
  void
