...
namespace internal
{
  ...
  namespace TriangulationImplementation
  {
     ...
     * stores the numbers of lines.         In the old days, whenever one
     * wanted to access one of these numbers,     one had to perform a loop
     * over all lines, e.g., and count the elements     until we hit the end
     * iterator. This is time consuming and since access     to the number of
     * lines etc is a rather frequent operation, this was not     an optimal
     * solution.
     *
     */
    template <>
     ...
     * class.         In the old days, whenever one wanted to access one of
     * these numbers,     one had to perform a loop over all lines, e.g., and
     * count the elements     until we hit the end . This is time consuming
...
/**
 * A triangulation is a collection of cells that, jointly, cover the domain on
 * which one typically wants to solve a partial differential equation. This
 * domain, and the mesh that covers it, represents a   @p dim
 *
 *  - imensional manifold and lives in   @p spacedim   spatial dimensions, where   @p dim   and   @p spacedim   are the template arguments of this class. (If   @p spacedim   is not specified, it takes the default value `spacedim=dim`.)
 * Thus, for example, an object of type   @p Triangulation<1,1>   (or simply
 * @p   Triangulation<1> since   @p spacedim==dim   by default) is used to
 * represent and handle the usual one-dimensional triangulation used in the
 * finite element method (so, segments on a straight line). On the other hand,
 * objects such as   @p Triangulation<1,2>   or   @p Triangulation<2,3>
 * (that are associated with curves in 2D or surfaces in 3D) are the ones one
 * wants to use in the boundary element method. The name of the class is
 * mostly hierarchical and is not meant to imply that a Triangulation can only
 * consist of triangles. Instead, triangulations consist of line segments in
 * 1d (i.e., if `dim==1`), and of three-dimensional cells (if `dim==3`).
 * Moreover, historically, deal.II only supported quadrilaterals (cells with
 * four vertices: deformed rectangles) in 2d and hexahedra (cells with six
 * sides and eight vertices that are deformed boxes), neither of which are
 * triangles. In other words, the term "triangulation" in the deal.II language
 * is synonymous with "mesh" and is to be understood separate from its
 * linguistic origin. This class is written to be as independent of the
 * dimension as possible (thus the complex construction of the
 * dealii::internal::TriangulationImplementation::TriaLevel   classes) to
 * allow code-sharing, to allow reducing the need to mirror changes in the
 * code for one dimension to the code for other dimensions. Nonetheless, some
 * of the functions are dependent of the dimension and there only exist
 * specialized versions for distinct dimensions.
 * This class satisfies the   @ref ConceptMeshType   "MeshType concept"
 * requirements. <h3>Structure and iterators</h3> The actual data structure of
 * a Triangulation object is rather complex and quite inconvenient if one
 * attempted to operate on it directly, since data is spread over quite a lot
 * of arrays and other places. However, there are ways powerful enough to work
 * on these data structures without knowing their exact relations. deal.II
 * uses class local alias (see below) to make things as easy and dimension
 * independent as possible. The Triangulation class provides iterators which
 * enable looping over all cells without knowing the exact representation used
 * to describe them. For more information see the documentation of
 * <tt>TriaIterator</tt>. Their names are alias imported from the Iterators
 * class (thus making them local types to this class) and are as follows:
 * <ul>     <li>   <tt>cell_iterator</tt>: loop over all cells used in the Triangulation   <li>   <tt>active_cell_iterator</tt>: loop over all active cells   </ul>
 ...
 * and therefore lines have no faces. The Triangulation class offers functions
 * like begin_active() which gives you an iterator to the first active cell.
 * There are quite a lot of functions returning iterators. Take a look at the
 * class doc to get an overview. Usage of these iterators is similar to usage
 * of standard container iterators. Some examples taken from the Triangulation
 * source code follow (notice that in the last two examples the template
 * parameter   @p spacedim   has been omitted, so it takes the default value
 * <code>dim</code>  ).
 * <ul>     <li>     <em>  Counting the number of cells on a specific level  </em>
 * @code
 *    template <int dim, int spacedim>
 *    unsigned int
 ...
 * </ul>
 *
 *  <h3>Usage</h3> Usage of a Triangulation is mainly done through the use of
 * iterators. An example probably shows best how to use it:
 *
 * @code
 * int main ()
 * {
 ...
 * consists of a vector storing the indices of the vertices of this cell in
 * the vertex list. To see how this works, you can take a look at the
 * GridIn<dim>::read_*   functions. The appropriate function to be called is
 * create_triangulation(). Creating the hierarchical information needed for
 * this library from cells storing only vertex information can be quite a
 * complex task.  For example in 2D, we have to create lines between vertices
 * (but only once, though there are two cells which link these two vertices)
 * and we have to create neighborhood information. Grids being read in should
 * therefore not be too large, reading refined grids would be inefficient
 * (although there is technically no problem in reading grids with several
 * 10.000 or 100.000 cells; the library can handle this without much
 * problems). Apart from the performance aspect, refined grids do not lend too
 * well to multigrid algorithms, since solving on the coarsest level is
 * expensive. It is wiser in any case to read in a grid as coarse as possible
 * and then do the needed refinement steps. It is your duty to guarantee that
 * cells have the correct orientation. To guarantee this, in the input vector
 * keeping the cell list, the vertex indices for each cell have to be in a
 * defined order, see the documentation of GeometryInfo<dim>. In one
 * dimension, the first vertex index must refer to that vertex with the lower
 * coordinate value. In 2D and 3D, the corresponding conditions are not easy
 * to verify and no full attempt to do so is made. If you violate this
 * condition, you may end up with matrix entries having the wrong sign
 * (clockwise vertex numbering, which results in a negative area element) of
 * with wrong matrix elements (twisted quadrilaterals, i.e. two vertices
 * interchanged; this results in a wrong area element). There are more subtle
 * conditions which must be imposed upon the vertex numbering within cells.
 * They do not only hold for the data read from an UCD or any other input
 * file, but also for the data passed to create_triangulation(). See the
 * documentation for the GridIn class for more details on this, and above all
 * to the GridReordering class that explains many of the problems and an
 * algorithm to reorder cells such that they satisfy the conditions outlined
 * above.
 ...
 *
 *
 * <h3>Refinement and coarsening of a triangulation</h3> Refinement of a
 * triangulation may be done through several ways. The most low-level way is
 * directly through iterators: let   @p i   be an iterator to an active cell
 * (i.e. the cell pointed to has no children), then the function call
 * <tt>i->set_refine_flag()</tt> marks the respective cell for refinement.
 * Marking non-active cells results in an error. After all the cells you
 * wanted to mark for refinement, call execute_coarsening_and_refinement() to
 * actually perform the refinement. This function itself first calls the   @p
 * prepare_coarsening_and_refinement   function to regularize the resulting
 * triangulation: since a face between two adjacent cells may only be
 * subdivided once (i.e. the levels of two adjacent cells may differ by one at
 * most; it is not possible to have a cell refined twice while the neighboring
 * one is not refined), some additional cells are flagged for refinement to
 * smooth the grid. This enlarges the number of resulting cells but makes the
 * grid more regular, thus leading to better approximation properties and,
 * above all, making the handling of data structures and algorithms much
 * easier. To be honest, this is mostly an algorithmic step than one needed by
 * the finite element method. To coarsen a grid, the same way as above is
 * possible by using <tt>i->set_coarsen_flag</tt> and calling
 * execute_coarsening_and_refinement(). The reason for first coarsening, then
 * refining is that the refinement usually adds some additional cells to keep
 * the triangulation regular and thus satisfies all refinement requests, while
 * the coarsening does not delete cells not requested for; therefore the
 * refinement will often revert some effects of coarsening while the opposite
 * is not true. The stated order of coarsening before refinement will thus
 * normally lead to a result closer to the intended one. Marking cells for
 * refinement 'by hand' through iterators is one way to produce a new grid,
 * especially if you know what kind of grid you are looking for, e.g. if you
 * want to have a grid successively refined towards the boundary or always at
 * the center (see the example programs, they do exactly these things). There
 * are more advanced functions, however, which are more suitable for automatic
 * generation of hierarchical grids in the context of a posteriori error
 * estimation and adaptive finite elements. These functions can be found in
 * the GridRefinement class.
 *
 *  <h3>Smoothing of a triangulation</h3> Some degradation of approximation
 * properties has been observed for grids which are too unstructured.
 * Therefore, prepare_coarsening_and_refinement() which is automatically
 * called by execute_coarsening_and_refinement() can do some smoothing of the
 * triangulation. Note that mesh smoothing is only done for two or more space
 * dimensions, no smoothing is available at present for one spatial dimension.
 * In the following, let <tt>execute_*</tt> stand for
 * execute_coarsening_and_refinement(). For the purpose of smoothing, the
 * Triangulation constructor takes an argument specifying whether a smoothing
 * step shall be performed on the grid each time <tt>execute_*</tt> is called.
 * The default is that such a step not be done, since this results in
 * additional cells being produced, which may not be necessary in all cases.
 * If switched on, calling <tt>execute_*</tt> results in flagging additional
 * cells for refinement to avoid vertices as the ones mentioned. The
 * algorithms for both regularization and smoothing of triangulations are
 * described below in the section on technical issues. The reason why this
 * parameter must be given to the constructor rather than to
 ...
 * in some refinement steps would need to be refined twice. The parameter
 * taken by the constructor is an integer which may be composed bitwise by the
 * constants defined in the enum #MeshSmoothing (see there for the
 * possibilities).
 *
 *
 ...
 * Each cell, face or edge stores information denoting the material or the part of the boundary that an object belongs to. The material id of a cell is typically used to identify which cells belong to a particular part of the domain, e.g., when you have different materials (steel, concrete, wood) that are all part of the same domain. One would then usually query the material id associated with a cell during assembly of the bilinear form, and use it to determine (e.g., by table lookup, or a sequence of if-else statements) what the correct material coefficients would be for that cell. See also   @ref GlossMaterialId   "this glossary entry".
 ...
 * typical use of this functionality, see the   step-28   tutorial program.
 * The functions of the GridGenerator namespace typically set the material ID
 * of all cells to zero. When reading a triangulation through the GridIn
 * class, different input file formats have different conventions, but
 * typically either explicitly specify the material id, or if they don't, then
 * GridIn simply sets them to zero. Because the material of a cell is intended
 * to pertain to a particular region of the domain, material ids are inherited
 * by child cells from their parent upon mesh refinement. Boundary indicators
 * on lower dimensional objects (these have no material id) indicate the
 * number of a boundary component. The weak formulation of the partial
 * differential equation may have different boundary conditions on different
 * parts of the boundary. The boundary indicator can be used in creating the
 * matrix or the right hand side vector to indicate these different parts of
 * the model (this use is like the material id of cells). Boundary indicators
 * may be in the range from zero to   numbers::internal_face_boundary_id-1.
 * The value   numbers::internal_face_boundary_id   is reserved to denote
 * interior lines (in 2D) and interior lines and quads (in 3D), which do not
 * have a boundary indicator. This way, a program can easily determine,
 * whether such an object is at the boundary or not. Material indicators may
 * be in the range from zero to   numbers::invalid_material_id-1. Lines in two
 * dimensions and quads in three dimensions inherit their boundary indicator
 * to their children upon refinement. You should therefore make sure that if
 * you have different boundary parts, the different parts are separated by a
 * vertex (in 2D) or a line (in 3D) such that each boundary line or quad has a
 * unique boundary indicator. By default (unless otherwise specified during
 * creation of a triangulation), all parts of the boundary have boundary
 * indicator zero. As a historical wart, this isn't true for 1d meshes,
 * however: For these, leftmost vertices have boundary indicator zero while
 * rightmost vertices have boundary indicator one. In either case, the
 * boundary indicator of a face can be changed using a call of the kind
 * <code>cell-@>face(1)-@>set_boundary_id(42);</code>  .
 * @see     @ref GlossBoundaryIndicator   "Glossary entry on boundary indicators"
 *
 *  <h3>History of a triangulation</h3> It is possible to reconstruct a grid
 * from its refinement history, which can be stored and loaded through the
 * @p save_refine_flags   and   @p   load_refine_flags functions. Normally,
 ...
 * @endcode
 *
 * The same scheme is employed for coarsening and the coarsening flags. You
 * may write other information to the output file between different sets of
 * refinement information, as long as you read it upon re-creation of the
 ...
 * There is another set of user data, which can be either an <tt>unsigned int</tt> or a <tt>void</tt>, for each line, quad, etc. You can access these through the functions listed under <tt>User data</tt> in the accessor classes. Again, see   @ref GlossUserData   "the glossary for more information".
 * The value of these user indices or pointers is   @p nullptr   by default.
 * Note that the pointers are not inherited to children upon refinement.
 * Still, after a remeshing they are available on all cells, where they were
 * set on the previous mesh. The usual warning about the missing type safety
 * of   @p void   pointers are obviously in place here; responsibility for
 * correctness of types etc lies entirely with the user of the pointer.
 *
 *
 * @note   User pointers and user indices are stored in the same place. In
 * order to avoid unwanted conversions, Triangulation checks which one of them
 * is in use and does not allow access to the other one, until
 * clear_user_data() has been called.
 *
 *  <h3>Describing curved geometries</h3>
 * deal.II implements
 * all geometries (curved and otherwise) with classes inheriting from
 * Manifold; see the documentation of Manifold,   step-49  , or the   @ref
 * manifold   module for examples and a complete description of the
 * algorithms. By default, all cells in a Triangulation have a flat geometry,
 * meaning that all lines in the Triangulation are assumed to be straight. If
 * a cell has a manifold_id that is not equal to   numbers::flat_manifold_id
 * then the Triangulation uses the associated Manifold object for computations
 * on that cell (e.g., cell refinement). Here is a quick example, taken from
 * the implementation of   GridGenerator::hyper_ball(),   that sets up a polar
 * grid:
 *
 *
 * @code
 ...
 * more information.
 * You should take note of one caveat: if you have concave boundaries, you must make sure that a new boundary vertex does not lie too much inside the cell which is to be refined. The reason is that the center vertex is placed at the point which is a weighted average of the vertices of the original cell, new face midpoints, and (in 3D) new line midpoints. Therefore if your new boundary vertex is too near the center of the old quadrilateral or hexahedron, the distance to the midpoint vertex will become too small, thus generating distorted cells. This issue is discussed extensively in   @ref GlossDistorted   "distorted cells".
 * <h3>Getting notice when a triangulation changes</h3> There are cases where
 * one object would like to know whenever a triangulation is being refined,
 * copied, or modified in a number of other ways. This could of course be
 * achieved if, in your user code, you tell every such object whenever you are
 * about to refine the triangulation, but this will get tedious and is error
 * prone. The Triangulation class implements a more elegant way to achieve
 * this: signals. In essence, a signal is an object (a member of the
 * Triangulation class) that another object can connect to. A connection is in
 * essence that the connecting object passes a function object taking a
 * certain number and kind of arguments. Whenever the owner of the signal
 * wants to indicate a certain kind of event, it 'triggers' the signal, which
 * in turn means that all connections of the signal are triggered: in other
 * word, the function objects are executed and can take the action that is
 * necessary. As a simple example, the following code will print something to
 * the output every time the triangulation has just been refined:
 * @code
 *   void f()
 *   {
 ...
 *   }
 * @endcode
 * This code will produce output twice, once for each refinement cycle. A more
 * interesting application would be the following, akin to what the FEValues
 * class does. This class stores a pointer to a triangulation and also an
 * iterator to the cell last handled (so that it can compare the current cell
 * with the previous one and, for example, decide that there is no need to
 * re-compute the Jacobian matrix if the new cell is a simple translation of
 * the previous one). However, whenever the triangulation is modified, the
 * iterator to the previously handled cell needs to be invalidated since it
 * now no longer points to any useful cell (or, at the very least, points to
 * something that may not necessarily resemble the cells previously handled).
 * The code would look something like this (the real code has some more error
 * checking and has to handle the case that subsequent cells might actually
 * belong to different triangulation, but that is of no concern to us here):
 *
 * @code
 * template <int dim>
 * class FEValues
 ...
 * that otherwise takes no arguments) to the   <code>this</code>   pointer of
 * the FEValues object. Note how here there is no need for the code that owns
 * the triangulation and the FEValues object to inform the latter if the
 * former is refined. (In practice, the function would want to connect to some
 * of the other signals that the triangulation offers as well, in particular
 * to creation and deletion signals.) The Triangulation class has a variety of
 * signals that indicate different actions by which the triangulation can
 * modify itself and potentially require follow-up action elsewhere. Please
 * refer to   Triangulation::Signals   for details. <h3>Serializing (loading
 * or storing) triangulations</h3> Like many other classes in deal.II, the
 * Triangulation class can stream its contents to an archive using BOOST's
 * serialization facilities. The data so stored can later be retrieved again
 * from the archive to restore the contents of this object. This facility is
 * frequently used to save the state of a program to disk for possible later
 * resurrection, often in the context of checkpoint/restart strategies for
 * long running computations or on computers that aren't very reliable (e.g.
 * on very large clusters where individual nodes occasionally fail and then
 * bring down an entire MPI job). For technical reasons, writing and restoring
 * a Triangulation object is not trivial. The primary reason is that unlike
 * many other objects, triangulations rely on many other objects to which they
 * store pointers or with which they interface; for example, triangulations
 * store pointers to objects describing boundaries and manifolds, and they
 * have signals that store pointers to other objects so they can be notified
 * of changes in the triangulation (see the section on signals in this
 * introduction). Since these objects are owned by the user space (for example
 * the user can create a custom manifold object), they may not be
 * serializable. So in cases like this,   boost::serialize   can store a
 * reference to an object instead of the pointer, but the reference will never
 * be satisfied at write time because the object pointed to is not serialized.
 * Clearly, at load time,   boost::serialize   will not know where to let the
 * pointer point to because it never gets to re-create the object originally
 * pointed to. For these reasons, saving a triangulation to an archive does
 * not store all information, but only certain parts. More specifically, the
 ...
 * On the other hand, since these are objects that are usually set in user
 * code, they can typically easily be set again in that part of your code in
 * which you re-load triangulations. In a sense, this approach to
 * serialization means that re-loading a triangulation is more akin to calling
 * the   Triangulation::create_triangulation()   function and filling it with
 * some additional content, as that function also does not touch the signals
 * and Manifold objects that belong to this triangulation. In keeping with
 * this analogy, the   Triangulation::load()   function also triggers the same
 * kinds of signal as   Triangulation::create_triangulation().
 *
 *  <h3>Technical details</h3> <h4>%Algorithms for mesh regularization and
 * smoothing upon refinement</h4> We chose an inductive point of view: since
 * upon creation of the triangulation all cells are on the same level, all
 * regularity assumptions regarding the maximum difference in level of cells
 * sharing a common face, edge or vertex hold. Since we use the regularization
 * and smoothing in each step of the mesh history, when coming to the point of
 * refining it further the assumptions also hold. The regularization and
 * smoothing is done in the   @p   prepare_coarsening_and_refinement function,
 * which is called by   @p   execute_coarsening_and_refinement at the very
 * beginning.  It decides which additional cells to flag for refinement by
 * looking at the old grid and the refinement flags for each cell.
 * <ul>     <li>     <em>  Regularization:  </em>   The algorithm walks over all cells checking whether the present cell is flagged for refinement and a neighbor of the present cell is refined once less than the present one. If so, flag the neighbor for refinement. Because of the induction above, there may be no neighbor with level two less than the present one.
 ...
 * <li>     @p eliminate_unrefined_islands:   For each cell we count the
 * number of neighbors which are refined or flagged for refinement. If this
 * exceeds the number of neighbors which are not refined and not flagged for
 * refinement, then the current cell is flagged for refinement. Since this may
 * lead to cells on the same level which also will need refinement, we will
 * need additional loops of regularization and smoothing over all cells until
 ...
 * flagged for refinement) the refine flag is cleared. For a description of
 * the distinction between the two versions of the flag see above in the
 * section about mesh smoothing in the general part of this classes
 * description.
 * The same applies as above: several loops may be necessary.   </ul>     </ul>
 ...
 * refinement. This makes readable programming easier. All the described
 * algorithms apply only for more than one space dimension, since for one
 * dimension no restrictions apply. It may be necessary to apply some
 * smoothing for multigrid algorithms, but this has to be decided upon later.
 *
 *  <h3>Warning</h3> It seems impossible to preserve   @p constness   of a
 * triangulation through iterator usage. Thus, if you declare pointers to a
 * @p const   triangulation object, you should be well aware that you might
 * involuntarily alter the data stored in the triangulation.
 *
 *
 * @ingroup grid aniso
 *
...
public:
  ...
  enum MeshSmoothing
  {
     ...
     * Triangulation::execute_coarsening_and_refinement()   the first time.
     * The     easiest way to achieve this is by calling global_refine(1)
     * straight     after creation of the triangulation.  It follows that if
     * at least one     of the children of a cell is or will be refined than
     * all children need     to be refined. If the #patch_level_1 flag is set,
     * than the flags     #eliminate_unrefined_islands,
    ...
    patch_level_1 = 0x4,
    /**
     * Each       @ref GlossCoarseMesh   "coarse grid"
     * cell is refined at least once,     i.e., the triangulation     might
     * have active cells on level 1 but not on level 0. This flag     ensures
     * that a mesh which has coarsest_level_1 has still     coarsest_level_1
     * after coarsening and refinement. It is, however, the     user's
     * responsibility to ensure that the mesh has coarsest_level_1     before
     * calling execute_coarsening_and_refinement the first time. The
     ...
     * level 1 may not be coarsened.         The main use of this flag is to
     * ensure that each cell has at least one     neighbor in each coordinate
     * direction (i.e. each cell has at least a     left or right, and at
     * least an upper or lower neighbor in 2d). This is     a necessary
     * precondition for some algorithms that compute finite     differences
     * between cells. The DerivativeApproximation class is one of     these
     * algorithms that require that a triangulation is coarsest_level_1
     ...
     * refined.         However, in many cases it is sufficient to refine the
     * coarser of the     two original cells in an anisotropic way to avoid
     * the case of multiple     hanging vertices on a single edge. Doing only
     * the minimal anisotropic     refinement can save cells and degrees of
     * freedom. By specifying this     flag, the library can produce these
     * anisotropic refinements.         The flag is not included by default
     * since it may lead to     anisotropically refined meshes even though no
     * cell has ever been     refined anisotropically explicitly by a user
     * command. This surprising     fact may lead to programs that do the
     * wrong thing since they are not     written for the additional cases
     * that can happen with anisotropic     meshes, see the discussion in the
     ...
     * but flagged for coarsening).         The reason for the change in
     * definition of an island is, that this     option would be a bit
     * dangerous, since if you consider a chain of     refined cells (e.g.
     * along a kink in the solution), the cells at the two     ends would be
     * coarsened, after which the next outermost cells would     need to be
     * coarsened. Therefore, only one loop of flagging cells like     this
     * could be done to avoid eating up the whole chain of refined cells
     * (`chain reaction'...).         This algorithm also takes into account
     * cells which are not actually     refined but are flagged for
     * refinement. If necessary, it takes away the     refinement flag.
  ...
  /**
   * An alias that is used to identify cell iterators. The concept of   iterators is discussed at length in the     @ref Iterators   "iterators documentation module".
  ...
  /**
   * The same as above to allow the usage of the "MeshType concept" also   on
  ...
  /**
   * An alias that is used to identify iterators that point to faces.   The concept of iterators is discussed at length in the     @ref Iterators   "iterators documentation module".
   * The current alias identifies faces in a triangulation. The   TriaIterator
   * class works like a pointer to objects that when   you dereference it
   * yields an object of type TriaAccessor, i.e.,   class that can be used to
   * query geometric properties of faces   such as their vertices, their area,
   * etc.
   * @ingroup Iterators
   *
   */
   ...
   * least one active cell.     Other than the "active" qualification, this
   * alias is identical to the     @p face_iterator   alias. In particular,
   * dereferencing either yields   the same kind of object.
   * @ingroup Iterators
   *
   */
  ...
  /**
   * An alias that defines an iterator type to iterate over   vertices of a mesh.  The concept of iterators is discussed at   length in the     @ref Iterators   "iterators documentation module".
   * @ingroup Iterators
   *
   */
   ...
   * above since all vertices in a mesh are active (i.e., are a vertex of   an
   * active cell).
   * @ingroup Iterators
   *
   */
  ...
  /**
   * An alias that defines an iterator over the (one-dimensional) lines   of a
   * mesh. In one-dimensional meshes, these are the cells of the mesh,
  ...
  /**
   * An alias that allows iterating over the <i>active</i> lines, i.e.,   that
   * subset of lines that have no children. In one-dimensional meshes,   these
   * are the cells of the mesh, whereas in two-dimensional   meshes the lines
   * are the faces of cells.     In two- or three-dimensional meshes, lines
   * without children (i.e.,   the active lines) are part of at least one
   * active cell. Each such line may   additionally be a child of a line of a
   * coarser cell adjacent to a cell   that is active. (This coarser neighbor
   * would then also be active.)
   * @ingroup Iterators
   *
   */
  ...
  /**
   * An alias that defines an iterator over the (two-dimensional) quads   of a
   * mesh. In two-dimensional meshes, these are the cells of the mesh,
  ...
  /**
   * An alias that allows iterating over the <i>active</i> quads, i.e.,   that
   * subset of quads that have no children. In two-dimensional meshes,   these
   * are the cells of the mesh, whereas in three-dimensional   meshes the
   * quads are the faces of cells.     In three-dimensional meshes, quads
   * without children (i.e.,   the active quads) are faces of at least one
   * active cell. Each such quad may   additionally be a child of a quad face
   * of a coarser cell adjacent to a cell   that is active. (This coarser
   * neighbor would then also be active.)
   * @ingroup Iterators
   *
   */
  ...
  /**
   * An alias that defines an iterator over the (three-dimensional) hexes   of
   * a mesh. This iterator only makes sense in three-dimensional meshes,
   ...
   * This iterator only makes sense in three-dimensional meshes,   where hexes
   * are the cells of the mesh. Consequently, in these   three-dimensional
   * meshes, this iterator is equivalent to the     @p active_cell_iterator
   * alias.
   * @ingroup Iterators
   *
   */
  ...
  /**
   * A structure that is used as an exception object by the   create_triangulation() function to indicate which cells among the coarse   mesh cells are inverted or severely distorted (see the entry on     @ref GlossDistorted   "distorted cells"
   * in the glossary).     Objects of this kind are thrown by the
   * create_triangulation() and   execute_coarsening_and_refinement()
   * functions, and they can be caught in   user code if this condition is to
   * be ignored. Note, however, that such   exceptions are only produced if
   * the necessity for this check was   indicated when calling the constructor
   * of the Triangulation class.     A cell is called <i>deformed</i> if the
   * determinant of the Jacobian of   the mapping from reference cell to real
   * cell is negative at least at one   vertex. This computation is done using
   * the     GeometryInfo::jacobian_determinants_at_vertices   function.
   *
   */
  ...
  /**
   * Create an empty triangulation. Do not create any cells.       @param   smooth_grid Determines the level of smoothness of the mesh size   function that should be enforced upon mesh refinement.       @param   check_for_distorted_cells Determines whether the triangulation   should check whether any of the cells that are created by   create_triangulation() or execute_coarsening_and_refinement() are   distorted (see     @ref GlossDistorted   "distorted cells").
  ...
  /**
   * Copy constructor.     You should really use the   @p copy_triangulation
   * function, so this   constructor is deleted. The reason for this is   that
   * we may want to use triangulation objects in collections. However,   C++
   * containers require that the objects stored in them are copyable, so   we
   * need to provide a copy constructor. On the other hand, copying
  ...
  /**
   * Move constructor.     Create a new triangulation by stealing the internal
   * data of another   triangulation.
   *
   */
  ...
  /**
   * Return MPI communicator used by this triangulation. In the case of   a
  ...
  /**
   * Assign a manifold object to a certain part of the triangulation. If   an
   * object with manifold number   @p number   is refined, this object is used
   * to find the location of new vertices (see the results section of
   * step-49     for a more in-depth discussion of this, with examples).  It
   * is also used   for non-linear (i.e.: non-Q1) transformations of cells to
   * the unit cell   in shape function calculations.     A copy of   @p
   * manifold_object   is created using   Manifold<dim,   spacedim>::clone()
   * and stored internally.     It is possible to remove or replace a Manifold
   * object during the   lifetime of a non-empty triangulation. Usually, this
   * is done before the   first refinement and is dangerous afterwards.
   * Removal of a manifold   object is done by reset_manifold(). This
   * operation then replaces the   manifold object given before by a straight
   * manifold approximation.
   * @ingroup manifold         @see       @ref GlossManifoldIndicator   "Glossary entry on manifold indicators"
   *
   */
  void
  ...
  /**
   * Reset those parts of the triangulation with the given     @p
   * manifold_number   to use a FlatManifold object. This is the   default
   * state of a non-curved triangulation, and undoes   assignment of a
   * different Manifold object by the function
  ...
  /**
   * Reset all parts of the triangulation, regardless of their   manifold_id,
   * to use a FlatManifold object. This undoes assignment   of all Manifold
   * objects by the function     Triangulation::set_manifold().
   * @ingroup manifold         @see       @ref GlossManifoldIndicator   "Glossary entry on manifold indicators"
   *
   */
  void
   ...
   * @note   If no manifold could be found, the default flat manifold is
   * returned.
   * @ingroup manifold         @see       @ref GlossManifoldIndicator   "Glossary entry on manifold indicators"
   *
   */
   ...
   * equal one).
   * @ingroup boundary         @see       @ref GlossBoundaryIndicator   "Glossary entry on boundary indicators"
   *
   */
  ...
  /**
   * Copy   @p other_tria   to this triangulation. This operation is not
   * cheap, so   you should be careful with using this. We do not implement
   * this function   as a copy constructor, since it makes it easier to
   * maintain collections   of triangulations if you can assign them values
   * later on.     Keep in mind that this function also copies the pointer to
   * the boundary   descriptor previously set by the   @p set_manifold
   * function. You must   therefore also guarantee that the Manifold objects
   * describing the boundary   have a lifetime at least as long as the copied
   * triangulation.     This triangulation must be empty beforehand.     The
   * function is made   @p virtual   since some derived classes might want to
   * disable or extend the functionality of this function.
   * @note   Calling this function triggers the 'copy' signal on other_tria,
   * i.e.   the triangulation being copied <i>from</i>.  It also triggers the
   ...
   * @note   This function triggers the "create" signal after doing its work.
   * See   the section on signals in the general documentation of this class.
   * For   example as a consequence of this, all DoFHandler objects connected
   * to   this triangulation will be reinitialized via   DoFHandler::reinit().
   ...
   * @note   The namespace   TriangulationDescription::Utilities   contains
   * functions     to create   TriangulationDescription::Description.
  ...
  /**
   * Revert or
   * flip the direction_flags of a dim<spacedim triangulation, see     @ref
   * GlossDirectionFlag  .     This function throws an exception if dim equals
   * spacedim.
   *
   */
  void
  ...
  /**
   * Refine all cells   @p times   times. In other words, in each one of   the
   * @p times   iterations, loop over all cells and refine each cell
   * uniformly into   $2^\text{dim}$   children. In practice, this   function
   * repeats the following operations   @p times   times: call
   * set_all_refine_flags() followed by   execute_coarsening_and_refinement().
   * The end result is that the   number of cells increases by a factor of
   * $(2^\text{dim})^\text{times}=2^{\text{dim} \times \text{times}}$  .
   * The execute_coarsening_and_refinement() function called in this   loop
   * may throw an exception if it creates cells that are   distorted (see its
   * documentation for an explanation). This   exception will be propagated
   * through this function if that   happens, and you may not get the actual
   * number of refinement   steps in that case.
   * @note   This function triggers the pre- and post-refinement signals
   * before   and after doing each individual refinement cycle (i.e. more than
   * once if   `times > 1`) . See the section on signals in the general
  ...
  /**
   * Coarsen all cells the given number of times.     In each of one of the
   * @p times   iterations, all cells will be marked for   coarsening. If an
   * active cell is already on the coarsest level, it will   be ignored.
   * @note   This function triggers the pre- and post-refinement signals
   * before   and after doing each individual coarsening cycle (i.e. more than
   * once if   `times > 1`) . See the section on signals in the general
   * documentation of   this class.
   *
   */
  ...
  /**
   * Execute both refinement and coarsening of the triangulation.     The
   * function resets all refinement and coarsening flags to false. It uses
   * the user flags for internal purposes. They will therefore be overwritten
   * by undefined content.     To allow user programs to fix up these cells if
   * that is desired, this   function after completing all other work may
   * throw an exception of type   DistortedCellList that contains a list of
   * those cells that have been   refined and have at least one child that is
   * distorted. The function does   not create such an exception if no cells
   * have created distorted children.   Note that for the check for distorted
   * cells to happen, the     <code>check_for_distorted_cells</code>   flag
   * has to be specified upon   creation of a triangulation object.     See
   * the general docs for more information.
   * @note   This function triggers the pre- and post-refinement signals
   * before   and after doing its work. See the section on signals in the
   * general   documentation of this class.
   * @note   If the boundary description is sufficiently irregular, it can   happen that some of the children produced by mesh refinement are   distorted (see the extensive discussion on     @ref GlossDistorted   "distorted cells").
   ...
   * smoothing.     Regarding the refinement process it fixes the closure of
   * the refinement   in <tt>dim>=2</tt> (make sure that no two cells are
   * adjacent with a   refinement level differing with more than one), etc.
   * It performs some   mesh smoothing if the according flag was given to the
   * constructor of this   class.  The function returns whether additional
   * cells have been flagged   for refinement.     See the general doc of this
   * class for more information on smoothing upon   refinement.     Regarding
   * the coarsening part, flagging and deflagging cells in   preparation of
   * the actual coarsening step are done. This includes   deleting coarsen
   * flags from cells which may not be deleted (e.g. because   one neighbor is
   * more refined than the cell), doing some smoothing, etc.     The effect is
   * that only those cells are flagged for coarsening which will   actually be
   * coarsened. This includes the fact that all flagged cells   belong to
   * parent cells of which all children are flagged.     The function returns
   * whether some cells' flagging has been changed in the   process.     This
   * function uses the user flags, so store them if you still need them
   * afterwards.
   *
   */
  virtual bool
...
* */
  ...
  /**
   * A structure that has   boost::signal   objects for a number of actions
   * that a   triangulation can do to itself. Please refer to the "Getting
   * notice when   a triangulation changes" section in the general
   * documentation of the   Triangulation class for more information and
   * examples.     For documentation on signals, see
  ...
  struct Signals
  {
     ...
     * itself called by other functions such as
     * Triangulation::refine_global()       ). At the time this signal is
     * triggered, the triangulation is still     unchanged.
     *
     */
     ...
     * GridTools::partition_triangulation_zorder()   functions. At the time
     ...
     * <code>cell_iterator->vertex(v) = xxxx</code>   cannot be detected by
     * this     method.
     *
     */
     ...
     * @note   This signal is triggered with the immediate parent cell of a
     * set     of active cells as argument. The children of this parent cell
     * will     subsequently be coarsened away.
     *
     */
     ...
     * @note   The signal parameter   @p cell   corresponds to the immediate
     * parent     cell of a set of newly created active cells.
     *
     */
    boost::signals2::signal<void(
      const typename Triangulation<dim, spacedim>::cell_iterator &cell)>
     ...
     * function     is called and in the destructor of the triangulation. This
     * signal is     also triggered when loading a triangulation from an
     * archive via       Triangulation::load()   as the previous content of
     * the triangulation is     first destroyed.         The signal is
     * triggered before the data structures of the     triangulation are
     * destroyed. In other words, the functions     attached to this signal
     * get a last look at the triangulation,     for example to save
     * information stored as part of the     triangulation.
     *
     */
     ...
     * summed to calculate the final weight.         This function is used in
     * step-68  .
     *
     */
     ...
     * Triangulation::refine_global()       ). At the time this signal is
     * triggered, the triangulation is still     unchanged. This signal     is
     * different from the pre_refinement signal, because in the parallel
     ...
     * parallel::distributed::Triangulation::execute_coarsening_and_refinement()
     * function. At the time this signal is triggered, the p4est oracle has
     * been     refined and the cell relations have been updated. The
     * triangulation is     unchanged otherwise, and the p4est oracle has not
     * yet been repartitioned.
     *
     */
     ...
     * This signal is triggered at the end of execution of the
     * parallel::distributed::Triangulation::execute_coarsening_and_refinement()
     * function when the triangulation has reached its final state. This
     * signal     is different from the post_refinement signal, because in the
     * parallel     distributed case the post_refinement signal is triggered
     * multiple times     without a way to distinguish the last signal call.
     *
     */
    ...
    /**
     * This signal is triggered at the beginning of execution of the
     * parallel::distributed::Triangulation::repartition()   function. At the
     * time     this signal is triggered, the triangulation is still
     * unchanged.
     * @note   The   parallel::distributed::Triangulation::repartition()
     * function is     also called by
     * parallel::distributed::Triangulation::load().   Thus, the
    ...
    /**
     * This signal is triggered at the end of execution of the
     * parallel::distributed::Triangulation::repartition()       function when
     * the triangulation has reached its final state.
     *
     */
    ...
    /**
     * This signal is triggered at the beginning of execution of the
     * parallel::distributed::Triangulation::save()       function. At the
     * time this signal is triggered, the triangulation     is still
     * unchanged.
     *
     */
    ...
    /**
     * This signal is triggered at the end of execution of the
     * parallel::distributed::Triangulation::save()       function when the
     * triangulation has reached its final state.
     *
     */
    ...
    /**
     * This signal is triggered at the beginning of execution of the
     * parallel::distributed::Triangulation::load()       function. At the
     * time this signal is triggered, the triangulation     is still
     * unchanged.
     *
     */
    ...
    /**
     * This signal is triggered at the end of execution of the
     * parallel::distributed::Triangulation::load()       function when the
     * triangulation has reached its final state.
     *
     */
...
* */
  ...
  /**
   * Clear all
   * user flags.  See also     @ref GlossUserFlags  .
   *
   */
  void
  clear_user_flags();
  ...
  /**
   * Save all
   * user flags. See the general documentation for this class and the
   * documentation for the   @p save_refine_flags   for more details.  See
   * also     @ref GlossUserFlags  .
   *
   */
  void
  ...
  /**
   * Same as
   * above, but store the flags to a bitvector rather than to a file.   The
   * output vector is resized if necessary.  See also     @ref GlossUserFlags
   * .
   *
   */
  void
  ...
  /**
   * Read the
   * information stored by   @p save_user_flags.    See also     @ref
   * GlossUserFlags  .
   *
   */
  void
  ...
  /**
   * Clear all
   * user flags on lines.  See also     @ref GlossUserFlags  .
   *
   */
  void
  ...
  /**
   * Load the
   * user flags located on lines.  See also     @ref GlossUserFlags  .
   *
   */
  void
  ...
  /**
   * Clear all
   * user flags on quads.  See also     @ref GlossUserFlags  .
   *
   */
  void
  ...
  /**
   * Load the
   * user flags located on quads.  See also     @ref GlossUserFlags  .
   *
   */
  void
  ...
  /**
   * Save the
   * user flags on hexs.  See also     @ref GlossUserFlags  .
   *
   */
  void
  ...
  /**
   * Load the
   * user flags located on hexs.  See also     @ref GlossUserFlags  .
   *
   */
  void
  ...
  /**
   * Clear all
   * user pointers and indices and allow the use of both for next   access.
  ...
  /**
   * Save all
   * user indices. The output vector is resized if necessary. See   also
   * @ref GlossUserData  .
   *
   */
  void
  ...
  /**
   * Read the
   * information stored by save_user_indices().  See also     @ref
   * GlossUserData  .
   *
   */
  void
  ...
  /**
   * Save all
   * user pointers. The output vector is resized if necessary.  See   also
   * @ref GlossUserData  .
   *
   */
  void
  ...
  /**
   * Read the
   * information stored by save_user_pointers().  See also     @ref
   * GlossUserData  .
   *
   */
  void
  ...
  /**
   * Save the
   * user indices on lines. The output vector is resized if   necessary.  See
  ...
  /**
   * Load the
   * user indices located on lines.  See also     @ref GlossUserData  .
   *
   */
  void
  ...
  /**
   * Save the
   * user indices on quads. The output vector is resized if   necessary.  See
   * also     @ref GlossUserData  .
   *
   */
  void
  ...
  /**
   * Load the
   * user indices located on quads.  See also     @ref GlossUserData  .
   *
   */
  void
  ...
  /**
   * Save the
   * user indices on hexes. The output vector is resized if   necessary.  See
   * also     @ref GlossUserData  .
   *
   */
  void
  ...
  /**
   * Load the
   * user indices located on hexs.  See also     @ref GlossUserData  .
   *
   */
  void
  ...
  /**
   * Load the
   * user pointers located on lines.  See also     @ref GlossUserData  .
   *
   */
  void
  ...
  /**
   * Save the
   * user pointers on quads. The output vector is resized if   necessary.  See
   * also     @ref GlossUserData  .
   *
   */
  void
  ...
  /**
   * Load the
   * user pointers located on quads.  See also     @ref GlossUserData  .
   *
   */
  void
  ...
  /**
   * Save the
   * user pointers on hexes. The output vector is resized if   necessary.  See
   * also     @ref GlossUserData  .
   *
   */
  void
  ...
  /**
   * Load the
   * user pointers located on hexs.  See also     @ref GlossUserData  .
   *
   */
  void
...
* */
   ...
   * @note   The given   @p level   argument needs to correspond to a level of
   * the     triangulation, i.e., should be less than the value returned by
   * n_levels(). On the other hand, for parallel computations using     a
   * parallel::distributed::Triangulation   object, it is often convenient
   ...
   * have cells at one of the higher levels. In those cases, the       @p
   * level   argument is accepted if it is less than what the
   * n_global_levels() function returns. If the given   @p level   is
   * between the values returned by n_levels() and n_global_levels(),     then
   * no cells exist in the local portion of the triangulation     at this
   * level, and the function simply returns what end() would     return.
   *
   */
  cell_iterator
  ...
  /**
   * Iterator to the first active cell on level   @p level.   If the given
   * level   does not contain any active cells (i.e., all cells on this level
   * are   further refined, then this function returns
  ...
  /**
   * Return an iterator to a cell of this Triangulation object constructed
   * from   an independent CellId object.     If the given argument
   * corresponds to a valid cell in this triangulation,   this operation will
   * always succeed for sequential triangulations where the   current
   * processor stores all cells that are part of the triangulation. On   the
   * other hand, if this is a parallel triangulation, then the current
   ...
   * documentation of active_cell_iterators().       @return   The half open
   * range   <code>[this->begin(), this->end())</code>
   * @ingroup CPP11
   *
   */
   ...
   * in the documentation of active_cell_iterators().       @param[in]   level
   * A given level in the refinement hierarchy of this   triangulation.
   * @return   The half open range <code>[this->begin(level),
   * this->end(level))</code>       @pre   level must be less than
   * this->n_levels().
   * @ingroup CPP11
   *
   */
   ...
   * documentation of active_cell_iterators().       @param[in]   level A
   * given level in the refinement hierarchy of this   triangulation.
...
* */
  ...
  /**
   * Return the total number of active quads, active or not on level   @p
   * level.
   *
   */
  unsigned int
  ...
  /**
   * Return the total number of used cells, active or not, on level   @p
   * level.     Maps to <tt>n_lines(level)</tt> in one space dimension and so
   * on.
   *
   */
  unsigned int
   ...
   * derived classes (e.g., in   parallel::distributed::Triangulation)   where
   * it   may return a value greater than the number of active cells reported
   * by   the triangulation object on the current processor.
   *
   */
   ...
   * equals n_lines(), in 3D it equals n_quads(), while in 1D it equals   the
   ...
   * more levels in this data structure than one may think
   *
   ...
   * n_levels() over all processors for a
   * parallel::distributed::Triangulation     and therefore can be larger than
   * n_levels().
   *
   */
  ...
  /**
   * Return true if the triangulation has hanging nodes.     The function is
   * made virtual since the result can be interpreted in   different ways,
   * depending on whether the triangulation lives only on a   single
   * processor, or may be distributed as done in the
   ...
   * etc, which is how many cells meet if the triangulation is refined.     In
   * one space dimension, two is returned.
   *
   */
  unsigned int
  ...
  /**
   * Return a reference to the current object.     This doesn't seem to be very useful but allows to write code that can   access the underlying triangulation for anything that satisfies the     @ref ConceptMeshType   "MeshType concept"
...
* */
   ...
   * n_raw_lines(), in 3d it equals n_raw_quads(), while in 1D it equals   the
   * number of vertices.
   * @note   This function really exports internal information about the
   * triangulation. It shouldn't be used in applications. The function is only
   * part of the public interface of this class because it is used in some of
...
* */
  ...
  /**
   * Determine an estimate for the memory consumption (in bytes) of this
   * object.     This function is made virtual, since a triangulation object
   * might be   accessed through a pointer to this base class, even if the
   ...
   * boundaries.     The vector can be filled by the function
   * GridTools::collect_periodic_faces.       For more information on periodic
   * boundary conditions see     GridTools::collect_periodic_faces,
  ...
  /**
   * Indicate if the triangulation only consists of hypercube-like cells,
   * i.e.,   lines, quadrilaterals, or hexahedra.
   *
   */
  bool
...
protected:
   ...
   * bool flags are packed but the data is written as ASCII text.     The
   * flags are stored in a binary format: for each   @p true,   a   @p 1   bit
   * is   stored, a   @p 0   bit otherwise.  The bits are stored as
   * <tt>unsigned   char</tt>, thus avoiding endianness. They are written to
   * @p out   in plain   text, thus amounting to 3.6 bits in the output per
   * bits in the input on   the average. Other information (magic numbers and
   * number of elements of   the input vector) is stored as plain text as
   * well. The format should   therefore be interplatform compatible.
   *
   */
  static void
  ...
  /**
   * Re-read a vector of bools previously written by   @p write_bool_vector
...
private:
   ...
   * been coarsened away in previous mesh refinement cycles.     Since users
   * should never have to access these internal properties of how   we store
   * data, these iterator types are made private.
   *
   */
...
* */
   ...
   * level   has no lines, a past-the-end iterator is returned.  If lines are
   * no   cells, i.e. for   @p dim>1   no   @p level   argument must be given.
   * The same   applies for all the other functions above, of course.
   *
   */
  raw_line_iterator
...
* */
  ...
  /**
   * Iterator to the first hex, used or not, on level   @p level.   If a level
   * has   no hexes, a past-the-end iterator is returned.
   * @note   The given   @p level   argument needs to correspond to a level of
   * the     triangulation, i.e., should be less than the value returned by
   * n_levels(). On the other hand, for parallel computations using     a
   * parallel::distributed::Triangulation   object, it is often convenient
...
* */
   ...
   * subscriptions.     Make sure, you know what you do, when calling this
   * function, as its use   is reasonable in very rare cases, only. For
   * example, when the   subscriptions were for the initially empty
   * Triangulation and the   Triangulation object wants to release its memory
   * before throwing an   assertion due to input errors (e.g. in the
  ...
  /**
   * Translate the unique id of a coarse cell to its index. See the glossary   entry on     @ref GlossCoarseCellId   "coarse cell IDs"
   * for more information.
   * @note   For serial and shared triangulation both id and index are the
   * same.         For distributed triangulations setting both might differ,
   * since the         id might correspond to a global id and the index to a
   * local id.       @param   coarse_cell_id Unique id of the coarse cell.
  ...
  /**
   * Translate the index of coarse cell to its unique id. See the glossary   entry on     @ref GlossCoarseCellId   "coarse cell IDs"
   * for more information.
   * @note   See the note of the method
   * coarse_cell_id_to_coarse_cell_index().       @param   coarse_cell_index
   * Index of the coarse cell.     @return   Id of the coarse cell.
   *
   */
   ...
   * is no such space for faces.     The field is declared as a pointer for a
   * rather mundane reason: all other   fields of this class that can be
   * modified by the TriaAccessor hierarchy   are pointers, and so these
   * accessor classes store a const pointer to the   triangulation. We could
   * no longer do so for TriaAccessor<0,1,spacedim> if   this field (that can
   * be modified by   TriaAccessor::set_boundary_id)   were   not a pointer.
   *
   */
   ...
   * programs.     The field is declared as a pointer for a rather mundane
   * reason: all other   fields of this class that can be modified by the
   * TriaAccessor hierarchy   are pointers, and so these accessor classes
   * store a const pointer to the   triangulation. We could no longer do so
   * for TriaAccessor<0,1,spacedim> if   this field (that can be modified by
   * TriaAccessor::set_manifold_id)   were   not a pointer.
   *
   */
