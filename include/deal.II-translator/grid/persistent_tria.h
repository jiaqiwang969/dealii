 ...
 * cell numbers to the geometrical cells. Basically, this is a drop-in
 * replacement for the triangulation. Since it is derived from the
 * Triangulation class, it shares all the functionality, but it overrides some
 * virtual functions and adds some functions, too. The main change to the base
 * class is that it overrides the   @p   execute_coarsening_and_refinement
 * function, where the new version first stores all refinement and coarsening
 * flags and only then calls the respective function of the base class. The
 * stored flags may later be used to restore the grid just as it was before.
 * Some other functions have been extended slightly as well, see their
 * documentation for more information. We note that since the triangulation is
 * created in exactly the same state as it was before, other objects working
 * on it should result in the same state as well. This holds in particular for
 * the DoFHandler object, which will assign the same degrees of freedom to the
 * original cells and the ones after reconstruction of the triangulation. You
 * can therefore safely use data vectors computed on the original grid on the
 * reconstructed grid as well.
 *
 ...
 * lifetime of the object of this class. Basically, usage looks like this:
 *
 * @code
 ...
 * a triangulation; it only becomes one after   @p restore   is first called.
 * Note also that the   @p execute_coarsening_and_refinement   stores all
 * necessary flags for later reconstruction using the   @p restore   function.
...
public:
   ...
   * time.     The coarse grid must persist until the end of this object,
   * since it will   be used upon reconstruction of the grid.
   *
   */
   ...
   * called on it before.     Repeatedly calls the <tt>restore(unsigned
   * int)</tt> function in a loop   over all refinement steps.
   *
   */
  void
  restore();
  ...
  /**
   * Overload this function to use   @p tria   as a new coarse grid. The
   * present   triangulation and all refinement and coarsening flags storing
   * its history   are deleted, and the state of the underlying triangulation
   * is reset to be   empty, until   @p restore_grid   is called the next
   * time.     The coarse grid must persist until the end of this object,
   * since it will   be used upon reconstruction of the grid.
   *
   */
  ...
  /**
   * An overload of the respective function of the base class.     Throw an
   * error, since this function is not useful in the context of this   class.
   *
   */
  DEAL_II_DEPRECATED
  virtual void
