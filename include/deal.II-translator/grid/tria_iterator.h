 ...
 *
 *  In addition to the standard interface, an iterator of this class provides
 * a <tt>-  @></tt>   operator, i.e. you can write statements like
 *
 * @code
 ...
 * @endcode
 *
 * Note the usage of <tt>++cell</tt> instead of <tt>cell++</tt> since this
 * does not involve temporaries and copying. It is recommended to use a fixed
 * value <tt>end</tt> inside the loop instead of <tt>tria.end()</tt>, since
 * the creation and copying of these iterators is rather expensive compared to
 * normal pointers. The objects pointed to are accessors, derived from
 ...
 * structures as they are a collection of functions providing access to the
 * data stored in Triangulation or DoFHandler objects. Using these accessors,
 * the structure of these classes is hidden from the application program.
 * <h3>Which iterator to use when</h3>
 * @attention   Application programs will rarely use TriaRawIterator, but
 * rather one of the derived classes TriaIterator or TriaActiveIterator.
 * <ul>     <li>   TriaRawIterator objects point to lines, cells, etc in the lists whether they are used or not (in the vectors, also <i>dead</i> objects are stored, since deletion in vectors is expensive and we also do not want to destroy the ordering induced by the numbering in the vectors). Therefore not all raw iterators point to valid objects.
 * <li>   The derived class TriaIterator selects the valid cells, that is,
 * cells used somewhere in the triangulation hierarchy.
 * <li>   TriaActiveIterator objects which only loop over active cells.   </ul>
 * <h3>Purpose</h3> Iterators are not much slower than operating directly on
 * the data structures, since they perform the loops that you had to handcode
 * yourself anyway. Most iterator and accessor functions are inlined. The main
 * functionality of iterators, resides in the <tt>++</tt> and <tt>\--</tt>
 * operators. These move the iterator forward or backward just as if it were a
 * pointer into an array. Here, this operation is not so easy, since it may
 * include skipping some elements and the transition between the triangulation
 * levels. This is completely hidden from the user, though you can still
 * create an iterator pointing to an arbitrary element.  Actually, the
 * operation of moving iterators back and forth is not done in the iterator
 * classes, but rather in the accessor classes. Since these are passed as
 * template arguments, you can write your own versions here to add more
 * functionality. Furthermore, the iterators described here satisfy the
 * requirement of input and bidirectional iterators as stated by the C++
 * standard. It is therefore possible to use the functions from the algorithm
 * section of the C++ standard, e.g.,   <em>  count_if  </em>   (see the
 * documentation for Triangulation for an example) and several others.
 * <h3>Implementation</h3> The iterator class itself does not have much
 * functionality. It only becomes useful when assigned an Accessor (the second
 * template parameter), which really does the access to data. An Accessor has
 * to fulfill some requirements:
 * <ul>     <li>   It must have two members named <tt>present_level</tt> and <tt>present_index</tt> storing the address of the element in the triangulation presently pointed to. These data have to be accessible by all triangulation iterators listed above.
 ...
 * <li>   For the TriaIterator and the TriaActiveIterator class, it must have
 * a member function <tt>bool used()</tt>, for the latter a member function
 * <tt>bool active()</tt>.
 * <li>   It must have void operators <tt>++</tt> and <tt>\--</tt>.
 * <li>   It must declare a local alias <tt>AccessorData</tt> which states the data type the accessor expects to get passed as fourth constructor argument. By declaring a local data type, the respective iterator class may type-safely enforce that data type to be one of its own constructor argument types. If an accessor class does not need additional data, this type shall be <tt>void</tt>.   </ul>
 ...
 * add functionality. The accessors provided by the library consist of two
 * groups, determined by whether they access the data of Triangulation objects
 * or DoFHandler objects. They are derived from TriaAccessor and DoFAccessor,
 * respectively. Each group also has specialized accessors for cells (as
 * opposed to faces and lines) that offer more functionality such as accessing
 * neighbors.
 ...
 * and implemented.
 *
 *  <h3>Past-the-end iterators</h3> There is a representation of
 * past-the-end-pointers, denoted by special values of the member variables
 * @p present_level   and   @p present_index:   If <tt>present_level>=0</tt>
 * and <tt>present_index>=0</tt>, then the object is valid (there is no check
 * whether the triangulation really has that many levels or that many cells on
 * the present level when we investigate the state of an iterator; however, in
 * many places where an iterator is dereferenced we make this check); if
 * <tt>present_level==-1</tt> and <tt>present_index==-1</tt>, then the
 * iterator points past the end; in all other cases, the iterator is
 * considered invalid. You can check this by calling the <tt>state()</tt>
 * function. An iterator is also invalid, if the pointer pointing to the
 * Triangulation object is invalid or zero. Finally, an iterator is invalid,
 * if the element pointed to by   @p   present_level and   @p present_index
 * is not used, i.e. if the   @p used   flag is set to false. The last two
 * checks are not made in <tt>state()</tt> since both cases should only occur
 * upon uninitialized construction through   @p memcpy   and the like (the
 * parent triangulation can only be set upon construction). If an iterator is
 * constructed empty through the empty constructor, <tt>present_level==-2</tt>
 * and <tt>present_index==-2</tt>. Thus, the iterator is invalid anyway,
 * regardless of the state of the triangulation pointer and the state of the
 * element pointed to. Past-the-end iterators may also be used to compare an
 * iterator with the <i>before-the-start</i> value, when running backwards.
 * There is no distinction between the iterators pointing past the two ends of
 * a vector. By defining only one value to be past-the-end and making all
 * other values invalid provides a second track of security: if we should have
 * forgotten a check in the library when an iterator is incremented or
 * decremented, we automatically convert the iterator from the allowed state
 * "past-the-end" to the disallowed state "invalid" which increases the chance
 * that some time earlier than for past-the-end iterators an exception is
 * raised.
 * @ref Triangulation
 *
 * @ingroup grid
...
public:
  ...
  /**
   * Default constructor. This constructor creates an iterator pointing   to
   * an invalid object. The iterator is consequently not usable.
   *
   */
  TriaRawIterator();
   ...
   * be convertible.     Through this constructor, it is also possible to
   * construct objects for   derived iterators:
   * @code
   ...
   * path from the   @p OtherAccessor   class to the   @p Accessor   class of
   * this   object. One such path would be derived class to base class, which
   * for   example may be used to get a   Triangulation::raw_cell_iterator
   * from a     DoFHandler::raw_cell_iterator,   since the DoFAccessor class
   * is derived   from the TriaAccessorBase class.
   *
   */
  template <typename OtherAccessor>
   ...
   * like <tt>(*i).index ();</tt>     This function has to be specialized
   * explicitly for the different   @p     Pointees, to allow an
   ...
   * to point to <tt>tria->quads.cells[index]</tt>.     You must not
   ...
   * is thus like <tt>i->index ();</tt>     There is a   @p const   and a non-
   * @p const   version.
   *
   */
  const Accessor *operator->() const;
   ...
   * of its state.       @warning   This function should not be used in
   * application programs. It is   only intended for limited purposes inside
   * the library and it makes   debugging much harder.
   *
   */
  const Accessor &
  access_any() const;
  ...
  /**
   * Ordering relation for iterators.     This relation attempts a total ordering of cells.     The relation is defined as follows:     For objects of   <tt>Accessor::structure_dimension   <     Accessor::dimension</tt>,   we simply compare the index of such an object.   The ordering is lexicographic according to the following hierarchy (in   the sense, that the next test is only applied if the previous was   inconclusive):       <ol>       <li>   The past-the-end iterator is always ordered last. Two past-the-end   iterators rank the same, thus false is returned in that case.  </li>         <li>   The level of the cell.  </li>       <li>   The index of a cell inside the level.  </li>       </ol>
   * @note   The ordering is not consistent between different processor in a
   * parallel::distributed::Triangulation   because we rely on index(), which
   ...
   * previously pointed to.     Since this operation involves a temporary and
   * a copy operation and since   an   @p iterator   is quite a large object
   * for a pointer, use the prefix   operator <tt>++iterator</tt> whenever
   * possible, especially in the header   of for loops (<tt>for (;
   * iterator!=end; ++iterator)</tt>) since there you   normally never need
   * the returned value.
   *
   */
  TriaRawIterator
  ...
  /**
   * Prefix   @p \--   operator:   @p \--iterator.   This operator moves the
   * iterator to   the previous element and returns a reference to
   * <tt>*this</tt>.
   *
   */
  TriaRawIterator &
  ...
  /**
   * Postfix   @p \--   operator:   @p iterator\--.   This operator moves the
   * iterator   to the previous element, but returns an iterator to the
   * element   previously pointed to.     The same applies as for the postfix
   * operator++: If possible, avoid it by   using the prefix operator form to
   * avoid the use of a temporary variable.
   *
   */
  TriaRawIterator
  operator--(int);
...
/**
 * This specialization of TriaRawIterator provides access only to the   <em>
 * used  </em>   lines, quads, cells, etc.
 *
 *
 * @ingroup grid
 *
...
public:
   ...
   * used just by looking at its type).       @pre   The argument passed to
   * this constructor must either be     (i) a past-the-end iterator; or (ii)
   * it must point to     a used object. All other cases will result in an
   * exception.
   *
   */
  ...
  /**
   * Constructor, initialized with the triangulation, the level and   index of
   * the object pointed to. The last parameter is of a type declared   by the
   * accessor class.       @pre   The argument passed to this constructor must
   * either be     (i) a past-the-end iterator; or (ii) it must point to     a
   * used object. All other cases will result in an exception.
   *
   */
  TriaIterator(
    const Triangulation<Accessor::dimension, Accessor::space_dimension> *parent,
   ...
   * path from the   @p OtherAccessor   class to the   @p Accessor   class of
   * this   object. One such path would be derived class to base class, which
   * for   example may be used to get a   Triangulation::cell_iterator   from
   * a     DoFHandler::cell_iterator,   since the DoFAccessor class is derived
   ...
   * operation and since an   @p active_iterator   is quite a large object for
   * a   pointer, use the prefix operator <tt>++i</tt> whenever possible,
  ...
  /**
   * Prefix   @p \--   operator:   @p \--i.   This operator advances the
   * iterator to the   previous used element and returns a reference to
   * <tt>*this</tt>.
   *
   */
  TriaIterator<Accessor> &
  ...
  /**
   * Declare some aliases which are standard for iterators and are used   by
   * algorithms to enquire about the specifics of the iterators they   work
   * on.
   *
   */
...
/**
 * This specialization of TriaIterator provides access only to the   <em>
 * active  </em>   lines, quads, cells, etc. An active cell is a cell which is
...
public:
   ...
   * pointing to a potentially non-active object (or at least from which   it
   * is not apparent from the type alone that it is active).       @pre   The
   * argument passed to this constructor must either be     (i) a past-the-end
   * iterator; or (ii) it must point to     an active object. All other cases
   ...
   * Constructor, initialized with the triangulation, the level and   index of
   * the object pointed to. The last parameter is of a type declared   by the
   * accessor class used by the current iterator.       @pre   The argument
   * passed to this constructor must either be     (i) a past-the-end
   * iterator; or (ii) it must point to     an active object. All other cases
   * will result in an exception.
   *
   */
   ...
   * path from the   @p OtherAccessor   class to the   @p Accessor   class of
   * this   object. One such path would be derived class to base class, which
   * for   example may be used to get a   Triangulation::active_cell_iterator
   * from a     DoFHandler::active_cell_iterator,   since the DoFAccessor
   * class is derived   from the TriaAccessorBase class.
   *
   */
  template <typename OtherAccessor>
   ...
   * <tt>TriaIterator<OtherAccessor></tt>.       @pre   The argument passed to
   * this constructor must either be     (i) a past-the-end iterator; or (ii)
   * it must point to     an active object. All other cases will result in an
   * exception.
   *
   */
  template <typename OtherAccessor>
  ...
  /**
   * Prefix   @p \--   operator:   @p \--i.   This operator advances the
   * iterator to the   previous active element and returns a reference to
   * <tt>*this</tt>.
   *
   */
  TriaActiveIterator<Accessor> &
  ...
  /**
   * Declare some alias which are standard for iterators and are used   by
   * algorithms to enquire about the specifics of the iterators they   work
   * on.
   *
   */
...
/**
 * Print the address to which this iterator points to   @p out.   The address
 * is given by the pair <tt>(level,index)</tt>, where   @p index   is an index
