...
namespace GridOutFlags
{
  ...
  struct Msh
  {
     ...
     * triangulation.         It is not necessary if you only want to write
     * the triangulation to view     or print it.         Default:   @p false.
     *
     */
    bool write_faces;
    /**
     ...
     * same boundary indicators for the different parts of the boundary of the
     * triangulation.         It is not necessary if you only want to write
     * the triangulation to view     or print it.         This is used only if
     * <tt>dim==3</tt>, and ignored in all other cases.         Default:   @p
     * false.
     *
     */
    bool write_lines;
  ...
  struct Ucd
  {
     ...
     * written using this flag.         Default:   <code>false</code>  .
     *
     */
    bool write_preamble;
     ...
     * same boundary indicators for the different parts of the boundary of the
     * triangulation.         It is not necessary if you only want to write
     * the triangulation to view     or print it.         This directive is
     * ignored if <tt>dim!=3</tt>.         Default:   @p false.
     *
     */
    bool write_lines;
  ...
  struct Gnuplot
  {
     ...
     * significantly, however.         Default:   @p false.
     *
     */
    bool write_cell_numbers;

    /**
     * Number of points,   <em>  excluding  </em>   the vertices, to plot on
     * curved     lines. Since GNUPLOT can only plot straight lines, setting
     * this number     to a value greater than zero (4 or 5 is usually enough
     * for refined     grids) makes the plot look curved even though it is
     * not.
     *
     */
   ...
   * classes.     By default, the size of the picture is scaled such that the
   * width equals   300 units.
   * @ingroup output
   *
   */
  struct EpsFlagsBase
  {
     ...
     * that the given   @p size   equals the width or the height of the
     * resulting     picture.
     *
     */
    enum SizeType
     ...
     * is specified by the flag   @p size_type.           Default is 300.
     *
     */
    unsigned int size;
    ...
    /**
     * Should lines
     * with a set   @p user_flag   be drawn in a different color     (red)?
     * See       @ref GlossUserFlags       for information about user flags.
     *
     */
     ...
     * the vertices of the face.         This number is only used if the
     * mapping used is not simply the standard       $Q_1$   mapping (i.e., an
     * object of kind MappingQGeneric(1)) that may     describe edges of cells
     * as curved and that will then be approximated     using line segments
     * with a number of intermediate points as described     by the current
     * variable.
     *
     */
   ...
   * common to all dimensions are listed in the base class.     This class
   * does not actually exist, we only here declare the general   template and
   * declare explicit specializations below.
   * @ingroup output
   *
   */
  template <int dim>
  ...
  struct Eps<2> : public EpsFlagsBase
  {
     ...
     * middle of each cell. The default value is to not do this.         The
     * format of the cell number written is <tt>level.index</tt>, or
     ...
     * <tt>level.index</tt>, or simply   @p index.   If   @p true,   the first
     * format     is taken. Default is   @p true.           The flag has
     * obviously no effect if   @p write_cell_numbers   is   @p false.
     *
     */
  ...
  struct Eps<3> : public EpsFlagsBase
  {
     ...
     * negative y-axis.         Default is the Gnuplot-default of 30.
     *
     */
    double turn_angle;
  ...
  struct XFig
  {
     ...
     * material or boundary id.         Depth of the object is 900-level, if
     * this value is true.
     *
     */
    bool level_depth;
    ...
    /**
     * Thickness of border lines of polygons. Default is 1.         Set this
     * to zero to avoid border lines for very fine meshes.
     *
     */
    int line_thickness;
  ...
  /**
   Flags controlling SVG output.     The figure below is a zoomed-in illustration of what the SVG flags are   capable of producing. These exact settings are the same as those used   in the test   <code>tests/grid/grid_out_svg_02.cc</code>   with the addition   of the flag   <code>svg_flags.label_boundary_id = true;</code>   .       @image html svg_flags.png
   * @ingroup output
   *
   */
  struct Svg
  {
     ...
     * corresponding boundary edge. Defaults to false.         Note: Depending
     * on the choice of image viewer, the boundary id     labels may not
     * appear to be centered in the circle.
     *
     */
 ...
 * different formats. See the enum   GridOut::OutputFormat   for a list of
 * formats and the corresponding output function names. Usage is simple:
 * either you use the direct form
 *
 * @code
 ...
 * ParameterHandler class. Note that here, we have created an unnamed object
 * of type GridOut and called one of its <tt>write_*</tt> functions. This
 * looks like as if the respective function could really be made   @p static.
 * This was not done in order to allow for parameters to be passed to the
 * different output functions in a way compatible with the scheme of allowing
 * the right output format to be selected at run-time through the generic   @p
 * write   function. In order to explain this, consider each function had one
 * or more additional parameters giving the details of output, for example
 * position of the spectator for 3d meshed, line thicknesses, etc. While this
 * would allow each output function any flexibility it needs, it would not
 * allow us to use the generic function   @p write   which is given a
 * parameter determining the output format, since it is impractical to give it
 * a list of parameters for each and every output format supported which it
 * may then pass on to the respective output function. Rather, we have chosen
 * to let each object of this class GridOut have a set of parameters for each
 * supported output format. These are collected in structures
 * GridOutFlags::Eps(),     GridOutFlags::Gnuplot(),   etc declared in the
 * GridOutFlags namespace, and you can set your preferred flags like this:
 *
 * @code
 * GridOut grid_out;
 ...
 * initial values. The advantage of this approach is that it is possible to
 * change the flags of one or more output formats according to your needs and
 * later use the generic   @p write   function; the actual output function
 * then called will use the flags as set before. Note that some of the
 * structures describing the flags of the different output formats are empty
 * since the respective format does not support any flags. The structure and
 * the   @p set_flags   function are provided anyway. Note also that some of
 * the structures may differ between the dimensions supported by this class;
 * they then have a template parameter, as usual.
 *
 *
 * @ingroup grid
 *
...
public:
  ...
  /**
   * Write triangulation in OpenDX format.     Cells or faces are written
   * together with their level and their material   id or boundary indicator,
   * resp.     Not implemented for the codimension one case.
   *
   */
  template <int dim, int spacedim>
  void
  ...
  /**
   * Write the triangulation in the gnuplot format.     In GNUPLOT format,
   * each cell is written as a sequence of its confining   lines. Apart from
   * the coordinates of the lines' end points, the level and   the material of
   * the cell are appended to each line of output. Therefore,   if you let
   * GNUPLOT draw a 2d grid as a 3d plot, you will see more refined   cells
   * being raised against cells with less refinement.  Also, if you draw   a
   * cut through a 3d grid, you can extrude the refinement level in the
   * direction orthogonal to the cut plane. The same can be done with the
   * material id, which is plotted after the level.     A more useful
   * application of this feature is the following: if you use   the GNUPLOT
   ...
   * @p mapping   is a pointer to a mapping used for the transformation of
   * cells   at the boundary. If zero, then use standard Q1 mapping.     Names
   * and values of additional flags controlling the output can be found   in
   * the documentation of the   GridOutFlags::Gnuplot   class, which also
  ...
  /**
   * Write the triangulation in the msh format.     Msh is the format used by
   * %Gmsh and it is described in the %Gmsh user's   guide. Besides the usual
   * output of the grid only, you can decide through   additional flags (see
   * below, and the documentation of the     GridOutFlags::Msh()   class)
   * whether boundary faces with non-zero boundary   indicator shall be
   * written to the file explicitly. This is useful, if you   want to re-read
   * the grid later on, since <tt>deal.II</tt> sets the   boundary indicator
   * to zero by default; therefore, to obtain the same   triangulation as
   * before, you have to specify faces with differing   boundary indicators
   * explicitly, which is done by this flag.     Names and values of further
   * flags controlling the output can be found in   the documentation of the
   * GridOutFlags::Msh()   class.     Works also in the codimension one case.
   *
   */
  template <int dim, int spacedim>
  void
  ...
  /**
   * Write the triangulation in any format supported by gmsh API.     Gmsh API
   * allows writing its output in several formats through their C++   API.
   * This function translates a Triangulation object into a gmsh collection
   * of entities and calls the   gmsh::write()   method with the filename
   * passed as   argument. This method generates a different entity for each
   * unique pair of   non default manifold id and boundary id, and writes a
   * gmsh physical group   for each unique combination, allowing you to read
   * back the triangulation   using the   GridIn::read_msh()   method that
   * takes a string as argument.     In particular, all cell objects with non
   * default boundary id or non   default manifold id are grouped in a unique
   * physical tag, whose name   contains the boundary and manifold indicators.
   * The names are constructed   using   Patterns::Tools::to_value()   applied
   * to a   `std::map<std::string,   int>`   where the keys are either
   * `MaterialID`, `BoundaryID`, or `ManifoldID`,   i.e., a cell with material
   * id 1, and manifold id 3 would be grouped in a   physical tag (whose
   * numbering is not specified), named   `MaterialID:1, ManifoldID:3`.
  ...
  /**
   * Write the triangulation in the ucd format.     UCD (unstructured cell
   * data) is the format used by AVS and some other   programs. It is
   * described in the AVS developer's guide. Besides the usual   output of the
   * grid only, you can decide through additional flags (see   below, and the
   * documentation of the   GridOutFlags::Ucd()   class) whether   boundary
   * faces with non-zero boundary indicator shall be written to the   file
   * explicitly. This is useful, if you want to re-read the grid later   on,
   * since <tt>deal.II</tt> sets the boundary indicator to zero by   default;
   * therefore, to obtain the same triangulation as before, you have   to
   * specify faces with differing boundary indicators explicitly, which is
   * done by this flag.     Names and values of further flags controlling the
   * output can be found in   the documentation of the   GridOutFlags::Ucd()
   * class.     Works also for the codimension one case.
   *
   */
  template <int dim, int spacedim>
  void
  ...
  /**
   * Write the triangulation in the encapsulated postscript format.     In
   * this format, each line of the triangulation is written separately. We
   ...
   * of the   GridOutFlags::Eps()   class).     The bounding box is close to
   * the triangulation on all four sides, without   an extra frame. The line
   * width is chosen to be 0.5 by default, but can be   changed. The line
   * width is to be compared with the extension of the   picture, of which the
   * default is 300.     The flag   @p color_lines_on_user_flag   allows to
   * draw lines with the   @p     user_flag set to be drawn in red. The colors
   * black and red are defined as     @p b   and   @p r   in the preamble of
   * the output file and can be changed there   according to need.       @p
   * mapping   is a pointer to a mapping used for the transformation of cells
   * at the boundary. If zero, then use standard Q1 mapping.     Names and
   * values of additional flags controlling the output can be found   in the
   * documentation of the   GridOutFlags::Eps()   class. Especially the
   * viewpoint for three dimensional grids is of importance here.     Not
  ...
  /**
   * Write two-dimensional XFig-file.     This function writes all grid cells
   * as polygons and optionally boundary   lines. Several parameters can be
   * adjusted by the XFigFlags control   object.     If levels are coded to
   * depth, the complete grid hierarchy is plotted with   fine cells before
   * their parents. This way, levels can be switched on and   off in xfig by
   * selecting levels.     Polygons are either at depth 900-level or at 900+
   * @p material_id,     depending on the flag   @p level_depth.
   * Accordingly, boundary edges are at   depth 800-level or at 800+  @p
   * boundary_id.   Therefore, boundary edges are   always in front of cells.
  ...
  /**
   * Write the triangulation in the SVG format.     SVG (Scalable Vector
   * Graphics) is an XML-based vector image format   developed and maintained
   * by the World Wide Web Consortium (W3C). This   function conforms to the
   * latest specification SVG 1.1, released on August   16, 2011.     The
   * cells of the triangulation are written as polygons with additional
   ...
   * version of MathGL>=2.0.0 is required.     To get a handle on the
   * resultant MathGL script within a graphical   environment an interpreter
   * is needed. A suggestion to start with is     <code>mglview</code>, which
   * is bundled with MathGL. <code>mglview</code>     can interpret and
   * display small-to-medium MathGL scripts in a graphical   window and
   * enables conversion to other formats such as EPS, PNG, JPG,   SVG, as well
   * as view/display animations. Some minor editing, such as   modifying the
   * lighting or alpha channels, can also be done.
   ...
   * dimension.     In three dimensions, this function writes a file that
   * contains
   *
   *
   *
  ...
  /**
   * Write triangulation in VTU format.     Due to the way this function
   * writes data to the output stream,   the resulting output files correspond
   * to a faithful representation   of the mesh in that all cells are visible
   * for visualization. In general,   the data is not in a format that allows
   * reading this file in again   through the GridIn class. This is because
   * every vertex of the mesh is   duplicated as many times as there are
   * adjacent cells. In other words,   every cell has its own, separate set of
   * vertices that are at the   same location as the vertices of other cells,
   * but are separately   numbered.     In order to create a file that can be
   * read with the GridIn class,   the flag
   * GridOutFlags::Vtu::serialize_triangulation   must be set to true. In
  ...
  /**
   * Return the   @p OutputFormat   value corresponding to the given string.
   * If   the string does not match any known format, an exception is thrown.
   ...
   * implemented.     To get a list of presently available format names, e.g.
   * to give it to the   ParameterHandler class, use the function
...
private:
  ...
  /**
   * Write the grid information about faces to   @p out.   Only those faces
   * are   printed which are on the boundary and which have a boundary
   * indicator not   equal to zero, since the latter is the default for
   * boundary faces.     Since, in %Gmsh, geometric elements are continuously
   * numbered, this   function requires a parameter   @p next_element_index
   * providing the next   geometric element number. This index should have a
   * numerical value equal   to one more than the index previously used to
   * write a geometric element   to   @p out.         @returns   The next
   * unused geometric element index.       @warning     @p next_element_index
   * should be (at least) one larger than the   current number of
   * triangulation elements (lines, cells, faces) that have   been written to
   * @p out.   %Gmsh will not load the saved file correctly if   there are
   * repeated indices.     This function unfortunately can not be included in
   * the regular   @p     write_msh function, since it needs special treatment
   * for the case   <tt>dim==1</tt>, in which case the face iterators are
   * <tt>void*</tt>'s   and lack the member functions which are called. We
   * would not actually   call these functions, but the compiler would
   * complain anyway when   compiling the function for <tt>dim==1</tt>. Bad
   * luck.
   *
   */
  template <int dim, int spacedim>
  ...
  /**
   * Write the grid information about lines to   @p out.   Only those lines
   * are   printed which are on the boundary and which have a boundary
   * indicator not   equal to zero, since the latter is the default for
   * boundary faces.     Since, in %Gmsh, geometric elements are continuously
   * numbered, this   function requires a parameter   @p next_element_index
   ...
   * repeated indices.     This function unfortunately can not be included in
   * the regular   @p     write_msh function, since it needs special treatment
   * for the case   <tt>dim==1</tt> and <tt>dim==2</tt>, in which case the
   * edge iterators are   <tt>void*</tt>'s and lack the member functions which
   * are called. We would   not actually call these functions, but the
   ...
   * are   printed which are on the boundary and which have a boundary
   * indicator not   equal to zero, since the latter is the default for
   * boundary faces.     Since (in the UCD format) geometric elements are
   * continuously numbered,   this function requires a parameter   @p
   * next_element_index   providing the   next geometric element number. This
   * index should have a numerical value   equal to one more than the index
   * previously used to write a geometric   element to   @p out.
   * @returns   The next unused geometric element index.       @warning     @p
   * next_element_index   should be (at least) one larger than the   current
   * number of triangulation elements (lines, cells, faces) that have   been
   * written to   @p out.   Visualization programs may not load the saved
   * file correctly if there are repeated indices.     This function
   * unfortunately can not be included in the regular   @p     write_ucd
   * function, since it needs special treatment for the case
   ...
   * are   printed which are on the boundary and which have a boundary
   * indicator not   equal to zero, since the latter is the default for
   * boundary lines.     Since (in the UCD format) geometric elements are
   * continuously numbered,   this function requires a parameter   @p
   * next_element_index   providing the   next geometric element number. This
   * index should have a numerical value   equal to one more than the index
   * previously used to write a geometric   element to   @p out.
   ...
   * This function always returns an empty list in one dimension.     The
   * reason for this function is the same as for write_ucd_faces(). See
