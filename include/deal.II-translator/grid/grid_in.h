 ...
 * available <a href="https://github.com/martemyev/tethex">here</a>. The mesh
 * you read will form the coarsest level of a   @p Triangulation   object. As
 * such, it must not contain hanging nodes or other forms of adaptive
 * refinement, or strange things will happen if the mesh represented by the
 * input file does in fact have them. This is due to the fact that most mesh
 * description formats do not store neighborship information between cells, so
 * the grid reading functions have to regenerate it. They do so by checking
 * whether two cells have a common face. If there are hanging nodes in a
 * triangulation, adjacent cells have no common (complete) face, so the grid
 * reader concludes that the adjacent cells have no neighbors along these
 ...
 * take a look at the PersistentTriangulation class. To read grid data, the
 * triangulation to be filled has to be empty. Upon calling the functions of
 * this class, the input file may contain only lines in one dimension; lines
 * and quads in two dimensions; and lines, quads, and hexes in three
 * dimensions. All other cell types (e.g. triangles in two dimensions,
 * triangles or tetrahedra in 3d) are rejected.  (Here, the "dimension" refers
 * to the dimensionality of the mesh; it may be embedded in a higher
 * dimensional space, such as a mesh on the two-dimensional surface of the
 * sphere embedded in 3d, or a 1d mesh that discretizes a line in 3d.) The
 * result will be a triangulation that consists of the cells described in the
 * input file, and to the degree possible with material indicators and
 * boundary indicators correctly set as described in the input file.
 *
 *
 * @note   You can not expect vertex and cell numbers in the triangulation to
 * match those in the input file. (This is already clear based on the fact
 * that we number cells and vertices separately, whereas this is not the case
 * for some input file formats; some formats also do not require consecutive
 * numbering, or start numbering at indices other than zero.)
 *
 *  <h3>Supported input formats</h3>
 * At present, the following input formats are supported:   <ul>     <li>     @p UCD   (unstructured cell data) format: this format is used for grid input as well as data output. If there are data vectors in the input file, they are ignored, as we are only interested in the grid in this class. The UCD format requires the vertices to be in following ordering: in 2d
 *
 * @verbatim
 *    3-----2
 *    |     |
 ...
 * documentation of the format in the   @p BAMG   manual is very incomplete,
 * so we don't actually parse many of the fields of the output since we don't
 * know their meaning, but the data that is read is enough to build up the
 * mesh as intended by the mesh generator. This format can be read by the
 ...
 * %Gmsh   mesh generator (see http://gmsh.info/). The documentation in the
 * @p %Gmsh   manual explains how to generate meshes compatible with the
 * deal.II library (i.e. quads rather than triangles). In order to use this
 * format, %Gmsh has to output the file in the old format 1.0. This is done
 * adding the line "Mesh.MshFileVersion = 1" to the input file.
 * <li>   <tt>%Gmsh 2.0 mesh</tt> format: this is a variant of the above
 * format. The read_msh() function automatically determines whether an input
 * file is version 1 or version 2.
 * <li>   <tt>Tecplot</tt> format: this format is used by   @p TECPLOT   and
 * often serves as a basis for data exchange between different applications.
 * Note, that currently only the ASCII format is supported, binary data cannot
 * be read.
 * <li>   <tt>UNV</tt> format: this format is generated by the Salome mesh generator, see http://www.salome-platform.org/ . The sections of the format that the   GridIn::read_unv   function supports are documented here:   <ul>     <li>   section 2411: http://www.sdrl.uc.edu/sdrl/referenceinfo/universalfileformats/file-format-storehouse/universal-dataset-number-2411   <li>   section 2412: http://www.sdrl.uc.edu/sdrl/referenceinfo/universalfileformats/file-format-storehouse/universal-dataset-number-2412   <li>   section 2467: http://www.sdrl.uc.edu/sdrl/referenceinfo/universalfileformats/file-format-storehouse/universal-dataset-number-2467   <li>   all sections of this format, even if they may not be supported in our reader, can be found here: http://www.sdrl.uc.edu/sdrl/referenceinfo/universalfileformats/file-format-storehouse   </ul>   Note that Salome, let's say in 2D, can only make a quad mesh on an object that has exactly 4 edges (or 4 pieces of the boundary). That means, that if you have a more complicated object and would like to mesh it with quads, you will need to decompose the object into >= 2 separate objects. Then 1) each of these separate objects is meshed, 2) the appropriate groups of cells and/or faces associated with each of these separate objects are created, 3) a compound mesh is built up, and 4) all numbers that might be associated with some of the internal faces of this compound mesh are removed.
 ...
 * http://www.cacr.caltech.edu/~slombey/asci/vtk/vtk_formats.simple.html The
 * VTK format requires the vertices to be in following ordering: in 2d
 *
 * @verbatim
 *    3-----2
 *    |     |
 ...
 *
 *
 * <li>   <tt>Cubit</tt> format: deal.II doesn't directly support importing
 * from Cubit at this time. However, Cubit can export in UCD format using a
 * simple plug-in, and the resulting UCD file can then be read by this class.
 * The plug-in script can be found on the deal.II wiki page under <a
 * href="https://github.com/dealii/dealii/wiki/Mesh-Input-And-Output">Mesh
 * Input and Output</a>. Alternatively, Cubit can generate ABAQUS files that
 * can be read in via the read_abaqus() function. This may be a better option
 * for geometries with complex boundary condition surfaces and multiple
 * materials
 *
 *  - information which is currently not easily obtained through Cubit's python interface.
 * </ul>
 * <h3>Structure of input grid data. The GridReordering class</h3> It is your
 * duty to use a correct numbering of vertices in the cell list, i.e. for
 * lines in 1d, you have to first give the vertex with the lower coordinate
 * value, then that with the higher coordinate value. For quadrilaterals in
 * two dimensions, the vertex indices in the   @p quad   list have to be such
 * that the vertices are numbered in counter-clockwise sense. In two
 * dimensions, another difficulty occurs, which has to do with the sense of a
 * quadrilateral. A quad consists of four lines which have a direction, which
 ...
 * implemented. There are more ambiguous cases, where the triangulation may
 * not know what to do at all without the use of sophisticated algorithms.
 * Furthermore, similar problems exist in three space dimensions, where faces
 * and lines have orientations that need to be taken care of. For this reason,
 * the <tt>read_*</tt> functions of this class that read in grids in various
 * input formats call the GridReordering class to bring the order of vertices
 * that define the cells into an ordering that satisfies the requirements of
 * the Triangulation class. Be sure to read the documentation of that class if
 * you experience unexpected problems when reading grids through this class.
 *
 *  <h3>Dealing with distorted mesh cells</h3>
 * For each of the mesh reading functions, the last call is always to   Triangulation::create_triangulation().   That function checks whether all the cells it creates as part of the coarse mesh are distorted or not (where distortion here means that the Jacobian of the mapping from the reference cell to the real cell has a non-positive determinant, i.e. the cell is pinched or twisted; see the entry on   @ref GlossDistorted   "distorted cells"
...
public:
   ...
   * VTK_QUAD (9), VTK_TRIANGLE (5), and VTK_LINE (3).     Depending on the
   * template dimension, only some of the above are accepted.     In
   * particular, in three dimensions, this function expects the file to
   * contain
   *
   *
   *
   ...
   * GridOutFlags::Vtu::serialize_triangulation   set to true.     Notice that
   * this function does not support reading in arbitrary vtu files,   but only
   * files that were written by deal.II itself, using the function
   * GridOut::write_vtu   and setting
   * GridOutFlags::Vtu::serialize_triangulation     to true.     When this
   * flag is set to true, the generated vtu file contains the   triangulation
   * in a xml section which is ignored by general vtu readers.   If this
   * section is absent, an exception is thrown.
   *
   */
  void
   ...
   * generator. Numerical data is ignored.     Note the comments on generating
   * this file format in the general   documentation of this class.
   *
   */
  void
  ...
  /**
   * Read grid data from an ucd file. Numerical data is ignored.   It is not
   * possible to use a ucd file to set both boundary_id and   manifold_id for
   * the same cell. Yet it is possible to use   the flag
   * apply_all_indicators_to_manifolds to decide if   the indicators in the
   * file refer to manifolds (flag set to true)   or boundaries (flag set to
   * false). If the flag is set, the   indicators are used for cells as
   * manifold id, too.
   *
   */
  void
   ...
   * the flag apply_all_indicators_to_manifolds to decide if   the indicators
   * in the file refer to manifolds (flag set to true)   or boundaries (flag
   * set to false).
   ...
   * file format. The %Gmsh formats are documented at   http://www.gmsh.info/.
   * @note   The input function of deal.II does not distinguish between
   * newline   and other whitespace. Therefore, deal.II will be able to read
   * files in a   slightly more general format than %Gmsh.
   * @ingroup simplex
   *
   */
  void
  ...
  /**
   * Read grid data using Gmsh API. Any file supported by Gmsh can be passed
   * as   argument. The format is deduced from the filename extension.
   ...
   * interpreted using the function   Patterns::Tools::to_value()   applied to
   * a     `std::map<std::string,   int>`. The keys can be either `MaterialID`
   * (if the   physical group refers to object of dimension `dim`),
   * `BoundaryID` (if the   group refers to objects of dimension < `dim`), or
   * `ManifoldID`.     From the Gmsh documentation, the formats of the
   * physical tags follows the   following conventions:
   * @code
  ...
  /**
   * Read in a file supported by Assimp, and generate a Triangulation   out of
   * it.  If you specify a   @p mesh_index,   only the mesh with   the given
   * index will be extracted, otherwise all meshes which are   present in the
   * file will be used to generate the Triangulation.     This function can
   * only be used to read two-dimensional meshes (possibly   embedded in three
   * dimensions). This is the standard for graphical software   such as
   * blender, or 3D studio max, and that is what the original Assimp   library
   * was built for. We "bend" it to deal.II to support complex   co-dimension
   * one meshes and complex two-dimensional meshes.     If   @p
   * remove_duplicates   is set to true (the default), then   duplicated
   * vertices will be removed if their distance is lower   than   @p tol.
   ...
   * `ignore_unsupported_element_types`, all the other element types are
   * simply   ignored by this algorithm. If your mesh contains a mixture of
   * triangles   and quadrilaterals, for example, only the quadrilaterals will
   * be   extracted. The resulting mesh (as represented in the Triangulation
   * object)   may not make any sense if you are mixing compatible and
   * incompatible   element types. If `ignore_unsupported_element_types` is
   * set to `false`,   then an exception is thrown when an unsupported type is
   * encountered.       @param   filename The file to read from     @param
   * mesh_index Index of the mesh within the file     @param
   * remove_duplicates Remove duplicated vertices     @param   tol Tolerance
   * to use when removing vertices     @param
   * ignore_unsupported_element_types Don't throw exceptions if we
   ...
   * versions of read_exodusii that may provide additional output data, but
   * for   now it has a single field.
   *
   */
  struct ExodusIIData
  {
    /**
     * A vector containing a mapping from deal.II boundary ids (or manifold
     * ids)     to the provided ExodusII sideset ids.
     *
     */
  ...
  /**
   * Read in a mesh stored in the ExodusII file format.     ExodusII is a feature-rich file format that supports many more features   (like node sets, finite element fields, quality assurance data, and more)   than most other grid formats supported by this class. Many of these   features do not have equivalent representations in deal.II and are   therefore not supported (for example, deal.II does not assign degrees of   freedom directly to nodes, so data stored in a nodal format is not loaded   by this function). At the current time only the following information is   extracted from the input file:       <ol>         <li>  Block ids: the block id of an element is loaded as its material       id.  </li>         <li>  Elements and vertices: the core geometric information stored in the       ExodusII file populates the attached Triangulation object. Higher-order       elements are automatically truncated to lower-order elements since       deal.II does not support this feature (e.g., there is no equivalent to       the   <code>QUAD9</code>   element in deal.II since all quadrilaterals have       four vertices and additional geometric information is either stored in       a Manifold or something like MappingQEulerian).  </li>         <li>  Sideset ids: these are interpreted as boundary ids or manifold ids       (see the note on the output value below). An error will occur if you       attempt to read an ExodusII file that assigns a sideset id to an       internal face boundary id.  </li>       </ol>       Sideset ids are not translated for Triangulations with nonzero codimension   since those Triangulations do not support the setting of boundary ids.       @param   filename The name of the file to read from.       @param   apply_all_indicators_to_manifolds Boolean determining if the sideset   ids should be interpreted as manifold ids or boundary ids. The default   value is <tt>false</tt>, i.e., treat all sideset ids as boundary ids. If   your mesh sets sideset ids on internal faces then it will be necessary to   set this argument to   <code>true</code>   and then do some postprocessing to   set the boundary ids correctly.       @return   This function returns a struct containing some extra data stored by   the ExodusII file that cannot be loaded into a Triangulation
   *
   ...
   *
   *  - however, a boundary cell   face in deal.II has exactly one boundary id. All boundary faces that are   not in a sideset are given the (default) boundary id of   $0$  . This function   then groups sidesets together into unique sets and gives each one a   boundary id. For example: Consider a single-quadrilateral mesh whose left   side has no sideset id, right side has sideset ids   $0$   and   $1$  , and whose   bottom and top sides have sideset ids of   $0$  . The left face will have a   boundary id of   $0$  , the top and bottom faces boundary ids of   $1$  , and the   right face a boundary id of   $2$  . Hence the vector returned by this function   in that case will be   $\{\{\}, \{0\}, \{0, 1\}\}$  .
   *
   */
  ExodusIIData
...
protected:
   ...
   * the GridReordering class).     The output of this function consists of
   * vectors for each line bounding   the cells indicating the direction it
   * has with respect to the orientation   of this cell, and the cell number.
   * The whole output is in a form such   that it can be read in by Gnuplot
   * and generate the full plot without   further ado by the user.
   *
   */
  static void
...
private:
   ...
   * parameters from a tecplot header, contained in the string   @p header.
   * The   other variables are output variables, their value has no influence
