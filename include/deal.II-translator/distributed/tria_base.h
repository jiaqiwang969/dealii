...
namespace parallel
{
   ...
   * parallel::shared::Triangulation.       It is, consequently, a class that
   * can be used to test whether a   pointer of reference to a triangulation
   * object refers to a   sequential triangulation, or whether the
   * triangulation is in fact   parallel. In other words, one could write a
   * function like this:
   * @code
  ...
  public:
    ...
    /**
     * Return the
     * number of active cells in the triangulation that are locally     owned,
     * i.e. that have a subdomain_id equal to     locally_owned_subdomain().
     * Note that there may be more active cells in     the triangulation
     * stored on the present processor, such as for example     ghost cells,
     * or cells further away from the locally owned block of     cells but
     * that are needed to ensure that the triangulation that stores     this
     * processor's set of active cells still remains balanced with     respect
     * to the 2:1 size ratio of adjacent cells.         As a consequence of
     * the remark above, the result of this function is     always smaller or
     * equal to the result of the function with the same     name in the
     * ::Triangulation base class, which includes the active ghost     and
     * artificial cells (see also       @ref GlossArtificialCell       and
     * @ref GlossGhostCell  ).
     *
     */
    unsigned int
     ...
     * The returned sets are symmetric, that is if   @p i   is contained in
     * the     list of processor   @p j,   then   @p j   will also be
     * contained in the list of     processor   @p i.
     *
     */
     ...
     * ghost cells.         The returned sets are symmetric, that is if   @p i
     * is contained in the     list of processor   @p j,   then   @p j   will
     * also be contained in the list of     processor   @p i.
    ...
    /**
     * Return partitioner for the global indices of the cells on the given
     * @p       level of the triangulation.
     *
     */
    const std::weak_ptr<const Utilities::MPI::Partitioner>
     ...
     * subdomains are adjacent to that vertex.           @deprecated   Use
     * GridTools::compute_vertices_with_ghost_neighbors()       instead of
     ...
     * @note   This function involves a global communication gathering all
     * current       IDs from all processes.
     *
     */
     ...
     * between MPI processes.         All the vertices that have been moved
     * and might be in the ghost layer     of a process have to be reported in
     * the   @p vertex_locally_moved       argument. This ensures that that
     * part of the information that has to     be send between processes is
     * actually sent. Additionally, it is quite     important that vertices on
     * the boundary between processes are     reported on exactly one process
     * (e.g. the one with the highest id).     Otherwise we could expect
     * undesirable results if multiple processes     move a vertex
     * differently. A typical strategy is to let processor   $i$       move
     * those vertices that are adjacent to cells whose owners include
     * processor   $i$   but no other processor   $j$   with   $j<i$  ; in
     * other words,     for vertices at the boundary of a subdomain, the
     * processor with the     lowest subdomain id "owns" a vertex.
     ...
     * this function again.           @param   vertex_locally_moved A bitmap
     * indicating which vertices have     been moved. The size of this array
     * must be equal to       Triangulation::n_vertices()   and must be a
     * subset of those vertices     flagged by
     * GridTools::get_locally_owned_vertices().             @see   This
     * function is used, for example, in       GridTools::distort_random().
     *
     */
    void
  ...
  public:
     ...
     * then be transferred to the new owning processor of the cell (or     its
     * parent/children) when the mesh is re-partitioned; (ii) when
     * serializing a computation to a file, it is necessary to attach     data
     * to cells so that it can be saved (  @a   e.g. in
     ...
     * with a different subdivision of cells among the processors.         The
     * way this function works is that it allows any number of interest
     ...
     * where each   parallel::distributed::SolutionTransfer   object that
     * works     on the current Triangulation object then needs to register
     * its intent.     Each of these parties registers a callback function
     * (the first     argument here,   @p pack_callback)   that will be called
     * whenever the     triangulation's execute_coarsening_and_refinement() or
     * save()     functions are called.         The current function then
     * returns an integer handle that corresponds     to the number of data
     * set that the callback provided here will attach.     While this number
     * could be given a precise meaning, this is     not important: You will
     * never actually have to do anything with     this number except return
     * it to the notify_ready_to_unpack() function.     In other words, each
     * interested party (i.e., the caller of the current     function) needs
     * to store their respective returned handle for later use     when
     * unpacking data in the callback provided to
     * notify_ready_to_unpack().         Whenever   @p pack_callback   is then
     * called by     execute_coarsening_and_refinement() or load() on a given
     * cell, it     receives a number of arguments. In particular, the first
     * argument passed to the callback indicates the cell for which     it is
     * supposed to attach data. This is always an active cell.         The
     * second, CellStatus, argument provided to the callback function     will
     * tell you if the given cell will be coarsened, refined, or will
     * persist as is. (This status may be different than the refinement     or
     * coarsening flags set on that cell, to accommodate things such as
     * the "one hanging node per edge" rule.). These flags need to be     read
     * in context with the p4est quadrant they belong to, as their
     * relations are gathered in local_cell_relations.         Specifically,
     * the values for this argument mean the following:
     *
     *
     *
     *
     ...
     * @note   If this function is used for serialization of data       using
     * save() and load(), then the cell status argument with which       the
     * callback is called will always be `CELL_PERSIST`.         The callback
     * function is expected to return a memory chunk of the     format
     * `std::vector<char>`,   representing the packed data on a     certain
     * cell.         The second parameter   @p returns_variable_size_data
     * indicates whether     the returned size of the memory region from the
     * callback function     varies by cell (<tt>=true</tt>) or stays constant
     * on each one     throughout the whole domain (<tt>=false</tt>).
     ...
     * callbacks have been called, and you will have to re-register       them
     * with a triangulation if you want them to be active for       another
     ...
     * processors, across mesh refinement, or serialization of data to     a
     * file are ready to receive that data back. The important part about
     ...
     * completely available again on the current processor, but only     after
     * a DoFHandler has been reinitialized and distributed     degrees of
     * freedom. In other words, there is typically a significant     amount of
     * set up that needs to happen in user space before the classes     that
     * can receive data attached to cell are ready to actually do so.     When
     * they are, they use the current function to tell the triangulation
     * object that now is the time when they are ready by calling the
     * current function.         The supplied callback function is then called
     * for each newly locally     owned cell. The first argument to the
     * callback is an iterator that     designates the cell; the second
     * argument indicates the status of the     cell in question; and the
     * third argument localizes a memory area by     two iterators that
     * contains the data that was previously saved from     the callback
     * provided to register_data_attach().         The CellStatus will
     * indicate if the cell was refined, coarsened, or     persisted
     * unchanged. The   @p cell_iterator   argument to the callback     will
     * then either be an active,     locally owned cell (if the cell was not
     * refined), or the immediate     parent if it was refined during
     * execute_coarsening_and_refinement().     Therefore, contrary to during
     * register_data_attach(), you can now     access the children if the
     * status is `CELL_REFINE` but no longer for     callbacks with status
     * `CELL_COARSEN`.         The first argument to this function, `handle`,
     * corresponds to     the return value of register_data_attach(). (The
     * precise     meaning of what the numeric value of this handle is
     * supposed     to represent is neither important, nor should you try to
     * use     it for anything other than transmit information between a
  ...
  protected:
    /**
     * Save additional
     * cell-attached data into the given file. The first     arguments are
     * used to determine the offsets where to write buffers to.         Called
     * by   @ref save  .
     *
     */
    void
    ...
    /**
     * Load additional
     * cell-attached data from the given file, if any was saved.     The first
     * arguments are used to determine the offsets where to read     buffers
     * from.         Called by   @ref load  .
     *
     */
    void
     ...
     * parallel::distributed::SolutionTransfer.           Relations will be
     * stored in the private member local_cell_relations. For     an extensive
     * description of CellStatus, see the documentation for the     member
    ...
    /**
     * This class in the private scope of
     * parallel::DistributedTriangulationBase       is dedicated to the data
     * transfer across repartitioned meshes     and to/from the file system.
    ...
    public:
       ...
       * cell       in   @p cell_relations.               All registered
       * callback functions in   @p pack_callbacks_fixed   will write
       * into the fixed size buffer, whereas each entry of   @p
       * pack_callbacks_variable         will write its data into the variable
       * size buffer.
       *
       */
      void
      ...
      /**
       * Unpack the CellStatus information on each entry of         @p
       * cell_relations.               Data has to be previously transferred
       * with execute_transfer()       or read from the file system via
       * load().
       *
       */
      void
       ...
       * @p cell_relations   with the provided   @p unpack_callback
       * function.             The parameter   @p handle   corresponds to the
       * position where the         @p unpack_callback   function is allowed
       * to read from the memory. Its       value needs to be in accordance
       * with the corresponding pack_callback       function that has been
       * registered previously.             Data has to be previously
       * transferred with execute_transfer()       or read from the file
       * system via load().
       *
       */
      void
      ...
      /**
       * Transfer data to file system.             The data will be written in
       * a separate file, whose name       consists of the stem   @p filename
       * and an attached identifier       <tt>_fixed.data</tt> for fixed size
       * data and <tt>_variable.data</tt>       for variable size data.
       * All processors write into these files simultaneously via MPIIO.
       * Each processor's position to write to will be determined       from
       * the provided input parameters.             Data has to be previously
       * packed with pack_data().
       *
       */
      void
      ...
      /**
       * Transfer data from file system.             The data will be read
       * from separate file, whose name       consists of the stem   @p
       * filename   and an attached identifier       <tt>_fixed.data</tt> for
       * fixed size data and <tt>_variable.data</tt>       for variable size
       * data.       The   @p n_attached_deserialize_fixed   and   @p
       * n_attached_deserialize_variable         parameters are required to
       * gather the memory offsets for each       callback.             All
       * processors read from these files simultaneously via MPIIO.       Each
       * processor's position to read from will be determined       from the
       * provided input arguments.             After loading, unpack_data()
       * needs to be called to finally       distribute data across the
       ...
       * values to their default state.             Frees memory completely.
       *
       */
      void
      clear();
       ...
       * each cell has exactly the same size.             The last entry of
       * this container corresponds to the data size       packed per cell in
       * the fixed size buffer (which can be accessed       calling
       * <tt>sizes_fixed_cumulative.back()</tt>).
       *
       */
