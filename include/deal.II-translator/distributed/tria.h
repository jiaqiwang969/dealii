...
namespace parallel
{
  namespace distributed
  {
     ...
     * <h3> Interaction with boundary description </h3>         Refining and
     * coarsening a distributed triangulation is a complicated     process
     * because cells may have to be migrated from one processor to
     ...
     * detail in the       @ref distributed_paper  .     Unfortunately, in
     * this process, some information can get lost relating     to flags that
     * are set by user code and that are inherited from mother     to child
     * cell but that are not moved along with a cell if that cell is
     * migrated from one processor to another.         An example are boundary
     * indicators. Assume, for example, that you start     with a single cell
     * that is refined once globally, yielding four     children. If you have
     * four processors, each one owns one cell. Assume     now that processor
     * 1 sets the boundary indicators of the external     boundaries of the
     * cell it owns to 42. Since processor 0 does not own     this cell, it
     * doesn't set the boundary indicators of its ghost cell     copy of this
     * cell. Now, assume we do several mesh refinement cycles and     end up
     * with a configuration where this processor suddenly finds itself     as
     * the owner of this cell. If boundary indicator 42 means that we need
     ...
     * indicator along this cell's boundary to 42.         The way to avoid
     * this dilemma is to make sure that things like setting     boundary
     * indicators or material ids is done immediately every time a
     ...
     * dealii::Triangulation::execute_coarsening_and_refinement   multiple
     * times     and this function needs to know about boundaries. In other
     * words, it is     <i>not</i> enough to just set boundary indicators on
     ...
     * indicators and that will be called by the   dealii::Triangulation
     * class.     The triangulation does not provide a pointer to itself to
     * the function     being called, nor any other information, so the trick
     * is to get this     information into the function. C++ provides a nice
     * mechanism for this     that is best explained using an example:
     * @code
     * #include <functional>
     *
     * template <int dim>
     ...
     * coarse     grid as argument.         This approach can be generalized.
     * In the example above, we have used a     global function that will be
     * called. However, sometimes it is necessary     that this function is in
     * fact a member function of the class that     generates the mesh, for
     * example because it needs to access run-time     parameters. This can be
     * achieved as follows: assuming the       <code>set_boundary_ids()</code>
     * function has been declared as a (non-     static, but possibly private)
     * member function of the       <code>MyClass</code>   class, then the
     * following will work:
     * @code
     * #include <functional>
     *
     ...
     * }
     * @endcode
     * The lambda function above again is an object that can     be called
     * like a global function with no arguments, and this object in     turn
     * calls the current object's member function
     * <code>set_boundary_ids</code>   with a reference to the triangulation
     * to     work on. Note that     because the
     * <code>create_coarse_mesh</code>   function is declared as
     * <code>const</code>  , it is necessary that the
     * <code>set_boundary_ids</code>   function is also declared
     * <code>const</code>  .         <b>Note:</b>For reasons that have to do
     * with the way the       parallel::distributed::Triangulation   is
     * implemented, functions that     have been attached to the
     * post-refinement signal of the triangulation     are called more than
     * once, sometimes several times, every time the     triangulation is
     * actually refined.
     * @ingroup distributed
     *
     */
    ...
    public:
      ...
      enum Settings
      {
         ...
         * every time a repartitioning in p4est happens. This can be a bit
         * more         expensive, but guarantees the same memory layout and
         * therefore cell         ordering in the deal.II mesh. As assembly is
         * done in the deal.II         cell ordering, this flag is required to
         * get reproducible behavior         after snapshot/resume.
         *
         */
        ...
        /**
         * Setting this flag will disable automatic repartitioning of the
         * cells         after a refinement cycle. It can be executed manually
         * by calling         repartition().
         *
         */
      ...
      /**
       * Constructor.               @param   mpi_communicator The MPI
       * communicator to be used for       the triangulation.
       * @param   smooth_grid Degree and kind of mesh smoothing to be applied
       * to       the mesh. See the   dealii::Triangulation   class for a
       * description of       the kinds of smoothing operations that can be
       * applied.               @param   settings See the description of the
       * Settings enumerator.       Providing
       * <code>construct_multigrid_hierarchy</code>   enforces
       * <code>Triangulation::limit_level_difference_at_vertices</code>
       * for smooth_grid.
       * @note   This class does not currently support the
       * <code>check_for_distorted_cells</code>   argument provided by the
       * base       class.
      ...
      /**
       * Transfer data across forests.             Besides the actual   @p
       * parallel_forest,   which has been already refined       and
       * repartitioned, this function also needs information about its
       ...
       * @p previous_global_first_quadrant.               Data has to be
       * previously packed with
       ...
       * flags set.             Since the current processor only has control
       * over those cells it owns       (i.e. the ones for which <code>cell-
       * @>subdomain_id()   ==       this-
       * @>locally_owned_subdomain()</code>),   refinement and coarsening
       ...
       * will       be largely ignored: the decision to refine the global mesh
       * will only       be affected by flags set on locally owned cells.
       * @note   This function by default partitions the mesh in such a way
       * that       the number of cells on all processors is roughly equal. If
       * you want       to set weights for partitioning, e.g. because some
       * cells are more       expensive to compute than others, you can use
       * the signal cell_weight       as documented in the
       * dealii::Triangulation   class. This function will       check whether
       * a function is connected to the signal and if so use it.       If you
       * prefer to repartition the mesh yourself at user-defined
       ...
       * @p no_automatic_repartitioning   is set in the constructor. Setting
       * the       flag and then calling repartition() gives the same result.
       ...
       * @note   If no function is connected to the cell_weight signal
       * described       in the   dealii::Triangulation   class, this function
       * will balance the       number of cells on each processor. If one or
       * more functions are       connected, it will calculate the sum of the
       * weights and balance the       weights across processors. The only
       * requirement on the weights is       that every cell's weight is
       * positive and that the sum over all       weights on all processors
       * can be formed using a 64-bit integer.       Beyond that, it is your
       * choice how you want to interpret the weights.       A common approach
       * is to consider the weights proportional to the cost       of doing
       * computations on a cell, e.g., by summing the time for       assembly
       * and solving. In practice, determining this cost is of course
       ...
       * calling this function.             You do not need to load with the
       * same number of MPI processes that       you saved with. Rather, if a
       * mesh is loaded with a different number       of MPI processes than
       * used at the time of saving, the mesh is       repartitioned
       * appropriately. Cell-based data that was saved with
       * DistributedTriangulationBase::DataTransfer::register_data_attach()
       * can       be read in with
       * DistributedTriangulationBase::DataTransfer::notify_ready_to_unpack()
       * after calling load().             If you use p4est version > 0.3.4.2
       * the   @p autopartition   flag tells       p4est to ignore the
       * partitioning that the triangulation had when it       was saved and
       * make it uniform upon loading. If   @p autopartition   is       set to
       * false, the triangulation is only repartitioned if needed (i.e.
       ...
       * p4est_t or p8est_t depending on   @p dim).                 @warning
       * If you modify the p4est object, internal data structures       can
       * become inconsistent.
       *
       */
      const typename dealii::internal::p4est::types<dim>::forest *
       ...
       * conditions. As a result, each pair of faces will differ by at most
       * one       refinement level and ghost neighbors will be available
       * across these       faces.             The vector can be filled by the
       * function         GridTools::collect_periodic_faces.               For
       * more information on periodic boundary conditions see
    ...
    private:
       ...
       * vector local_cell_relations.             The vector contains an
       * active cell iterator for every locally owned       p4est quadrant, as
       * well as a CellStatus flag to describe their       relation.
       * The stored vector will be ordered by the occurrence of quadrants in
       * the corresponding local sc_array of the parallel_forest. p4est
       * requires       this specific ordering for its transfer functions.
       * Therefore, the size       of this vector will be equal to the number
       * of locally owned quadrants       in the parallel_forest object.
       * These relations will be established for example in the mesh
       * refinement       process: after adapting the parallel_forest, but
       * before applying these       changes to this triangulation, we will
       * record how cells will change in       the refinement process. With
       * this information, we can prepare all       buffers for data transfer
       * accordingly.
       *
       */
      virtual void
       ...
       * SparsityTools::reorder_hierarchical()   to ensure that the part of
       * the       forest stored by p4est is located on geometrically close
       * coarse grid       cells.
       *
       */
       ...
       * and copy it to p4est data structures.             This function
       * exists in 2d and 3d variants.
       *
       */
      void
       ...
       * be equal to either n_active_cells() or
       * n_locally_owned_active_cells(),       because the triangulation is
       * not updated yet. The weights are sorted       in the order that p4est
       * will encounter them while iterating over       them.
       *
       */
       ...
       * conditions) are marked by true.             Used by
    ...
    public:
      ...
      /**
       * This method, which is only implemented for dim = 2 or 3,       needs
       * a stub because it is used in dof_handler_policy.cc
       *
       */
...
namespace parallel
{
  namespace distributed
  {
     ...
     * even if it is disabled.         Since the constructor of this class is
     * deleted, no such objects     can actually be created as this would be
     * pointless given that     p4est is not available.
     *
     */
    template <int dim, int spacedim = dim>
    class Triangulation
...
namespace parallel
{
  namespace distributed
  {
     ...
     * active cells to match the p4est oracle.         The modification only
     * happens on   parallel::distributed::Triangulation       objects, and
     * persists for the lifetime of an instantiation of this     class.
     ...
     * be refined.         The use of this class is demonstrated in   step-75
     * .
     *
     */
    template <int dim, int spacedim = dim>
    ...
    public:
      /**
       * Constructor.             Stores the refine and coarsen flags of all
       * active cells if the       provided Triangulation is of type
       * parallel::distributed::Triangulation.               Adjusts them to
       * be consistent with the p4est oracle.
       *
       */
      TemporarilyMatchRefineFlags(dealii::Triangulation<dim, spacedim> &tria);

      /**
       * Destructor.             Returns the refine and coarsen flags of all
       * active cells on the         parallel::distributed::Triangulation
