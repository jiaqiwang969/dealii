...
namespace parallel
{
  namespace distributed
  {
     ...
     * indicators) while refining and/or coarsening a distributed
     * triangulation     and handle the necessary communication.         This
     * class therefore does for cell-related information what
     * parallel::distributed::SolutionTransfer   does for the values of
     * degrees of     freedom defined on a
     * parallel::distributed::Triangulation.           This class has been
     * designed to operate on any kind of datatype that is     serializable. A
     * non-distributed container (like Vector or   `std::vector`)       has to
     * be provided, which holds the cell-wise data in the same order as
     * active cells are traversed. In other words, each entry corresponds to
     * the     cell with the same index   CellAccessor::active_cell_index(),
     * and the     container has to be of size
     * Triangulation::n_active_cells().           <h3>Transferring cell-wise
     * data</h3>         The following code snippet demonstrates how to
     * transfer cell-related data     across refinement/coarsening of the
     ...
     *
     * @endcode
     * <h3>Use for serialization</h3>         This class can be used to
     * serialize and later deserialize a distributed     mesh with attached
     * data to separate files.         For serialization, the following code
     * snippet saves not only the     triangulation itself, but also the
     * cell-wise data attached:
     * @code
     * Vector<double> data_to_transfer(triangulation.n_active_cells());
     * //[fill data_to_transfer with cell-wise values...]
     *
     ...
     * interface     with the aim of serialization, the calls to the
     * corresponding     prepare_for_serialization() and deserialize()
     * functions need to happen in     the same order, respectively. Classes
     * relying on this interface are e.g.
     * parallel::distributed::CellDataTransfer,
     * parallel::distributed::SolutionTransfer,   and
     * Particles::ParticleHandler.
     * @note   See the documentation of
     * parallel::distributed::SolutionTransfer   for     matching code
     * snippets for both transfer and serialization.
     * @ingroup distributed
     *
     */
    ...
    public:
      /**
       * Constructor.               @param[in]   triangulation The
       * triangulation on which all operations will         happen. At the
       * time when this constructor is called, the refinement         in
       * question has not happened yet.         @param[in]
       * transfer_variable_size_data Specify whether your VectorType
       ...
       * the VectorType container is a container itself.         @param[in]
       * refinement_strategy %Function deciding how data will be
       * stored on refined cells from its parent cell.         @param[in]
       * coarsening_strategy %Function deciding which data to store         on
       ...
       * It registers the data transfer of   @p in   on the underlying
       * triangulation.         @p in   includes data to be interpolated onto
       * the new (refined and/or       coarsened) grid. See documentation of
       * this class for more information       on how to use this
       * functionality.             This function can be called only once for
       * the specified container       until data transfer has been completed.
       * If multiple vectors shall be       transferred via this class, use
       * the function below.
       *
       */
      void
      ...
      /**
       * Prepare the serialization of the given vector.             The
       * serialization is done by   Triangulation::save().   See documentation
      ...
      /**
       * Execute the deserialization of the stored information.       This
       * needs to be done after calling   Triangulation::load().
       *
       */
      void
    ...
    private:
      ...
      /**
       * %Function deciding on how to process data from children to be stored
       * on       the parent cell.
       *
       */
      const std::function<value_type(
      ...
      /**
       * The handle that triangulation has assigned to this object       with
       * which we can access our memory offset and our pack function.
       *
       */
      unsigned int handle;

      /**
       * Registers the pack_callback() function to the triangulation       and
       * stores the returning handle.
       *
       */
      void
