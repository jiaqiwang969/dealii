...
namespace parallel
{
  namespace distributed
  {
     ...
     * locally relevant DoF values (these values required for the
     ...
     * LinearAlgebra::distributed::Vector   allows writing into ghost
     * elements.     For a ghosted vector the interpolation step can be
     * accomplished via
     * @code
     ...
     * serialize and later deserialize a distributed     mesh with solution
     * vectors to a file. If you use more than one     DoFHandler and
     * therefore more than one SolutionTransfer object, they     need to be
     * serialized and deserialized in the same order.         If vector has
     * the locally relevant DoFs, serialization works as     follows:
     * @code
     ...
     * with     many different FiniteElement objects, each cell's data has to
     * be     processed with its corresponding `future_fe_index`. Further, if
     * refinement is involved, data will be packed on the parent cell with its
     * `future_fe_index` and unpacked later with the same index on its
     * children.     If cells get coarsened into one, data will be packed on
     * the children with     the least dominant finite element of their common
     * subspace, and unpacked     on the parent with this particular finite
     * element (consult       hp::FECollection::find_dominated_fe_extended()
     * for more information).         Transferring a solution across
     * refinement works exactly like in the     non-hp-case. However, when
     * considering serialization, we also have to     store the active FE
     * indices in an additional step. A code snippet     demonstrating
     * serialization with the       parallel::distributed::SolutionTransfer
     * class with DoFHandler objects     with hp-capabilities is provided in
     * the following. Here VectorType is     your favorite vector type, e.g.
     * PETScWrappers::MPI::Vector,         TrilinosWrappers::MPI::Vector,   or
     * corresponding block vectors.         If vector has the locally relevant
     * DoFs, serialization works as follows:
     * @code
     ...
     * sol_trans.deserialize(distributed_vector);
     * @endcode
     * <h3>Interaction with hanging nodes</h3>         In essence, this class
     * implements the same steps as does       dealii::SolutionTransfer
     * (though the implementation is entirely     separate). Consequently, the
     * same issue with hanging nodes and     coarsening can happen with this
     * class as happens with       dealii::SolutionTransfer.   See there for
    ...
    public:
      /**
       * Constructor.               @param[in]   dof The DoFHandler on which
       * all operations will happen.         At the time when this constructor
       ...
       * vectors in   @p all_in   in each cell that'll be coarsened.   @p
       * all_in         includes all vectors that are to be interpolated onto
       * the new       (refined and/or coarsened) grid.
       *
       */
      void
      ...
      /**
       * Same as the previous function but for only one discrete function to
       * be       interpolated.
       *
       */
      void
       ...
       * done by   Triangulation::save().   The given vector needs all
       * information       on the locally active DoFs (it must be ghosted).
       ...
       * done after calling   Triangulation::load().   The given vector must
       * be a       fully distributed vector without ghost elements. See
