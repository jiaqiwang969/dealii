...
namespace parallel
{
  ...
  {
    /**
     * A distributed triangulation with a distributed coarse grid.         The
     * motivation for   parallel::fullydistributed::Triangulation   has its
     ...
     *
     *
     *  - storing the coarse-grid information on every process is too expensive       from a memory point of view (as done by         parallel::distributed::Triangulation).   Normally, a process only needs a       small section of the global triangulation, i.e., a small section of the       coarse grid such that a partitioning of the coarse grid is indeed       essential. The cells stored on each process consist of the         @ref GlossLocallyOwnedCell   "locally owned cells" and the         @ref GlossGhostCell   "ghost cells".
     *
     *
     *
     *
     ...
     *  - on the finest level
     *
     *  - among all       processes by simply partitioning a space-filling curve might not lead       to an optimal result for triangulations that originate from large       coarse grids: e.g. partitions that belong to the same process might       be discontinuous, leading to increased communication (within a       node and beyond). Graph-based partitioning algorithms might be a sound       alternative to the space filling curve used by         parallel::distributed::Triangulation.           To be able to construct a fully partitioned triangulation that     distributes the coarse grid and gives flexibility regarding partitioning,     the following ingredients are required:
     *
     *
     *
     *
     ...
     *
     *
     *  - information about which cell should be refined as well as information       regarding the subdomain_id, the level_subdomain_id, manifold_id,       and boundary_id of each cell.         The ingredients listed above are bundled in the struct       TriangulationDescription::Description.   The user has to fill this     data structure
     *
     *  - in a pre-processing step
     *
     *  - before actually creating the     triangulation. Predefined functions to create       TriangulationDescription::Description   can be found in the namespace       TriangulationDescription::Utilities.           Once the   TriangulationDescription::Description   `construction_data` has     been constructed, the triangulation `tria` can be created by calling     `tria.create_triangulation(construction_data);`.
     ...
     * refine afterwards.
     * @note   Currently only simple periodicity conditions (i.e. without
     * offsets           and rotation matrices
     *
     *  - see also the documentation of             GridTools::collect_periodic_faces())   are supported.
     *
     */
    ...
    public:
       ...
       * communicator to be used for the
       * triangulation.
       *
       */
       ...
       * an assertion. Instead, use             the other
       * create_triangulation() function to create the
       * triangulation.
       *
       */
      virtual void
       ...
       * @param   other_tria The triangulation to be copied. It can be a
       * serial              Triangulation or a
       * parallel::distributed::Triangulation.   Both              can have
       ...
       * copy_triangulation.               @param   partitioner A partitioning
       * function, which takes as input argument                          a
       * reference to the triangulation to be partitioned
       * and the number of partitions to be created.
       * The function needs to set subdomain                          ids for
       * each active cell of the given triangulation,
       * with values between zero (inclusive)                          and the
       * second argument to the function (exclusive).         @param
       * settings See the description of the Settings enumerator.
       * @note   As a default,   GridTools::partition_triangulation_zorder()
       * is used             as partitioner and data structures on multigrid
       * levels are not             set up.
       *
       */
      void
      set_partitioner(
       ...
       * system. See the SolutionTransfer class on how to store solution
       * vectors       into this file. Additional cell-based data can be saved
      ...
      /**
       * Load the triangulation saved with save() back in. The mesh       must
       * be empty before calling this function.             You need to load
       * with the same number of MPI processes that       you saved with,
       * hence autopartition is disabled.             Cell-based data that was
       * saved with register_data_attach() can be read       in with
    ...
    private:
      ...
      /**
       * Go through all active cells that are locally owned and record how
       * they       will change in the private member vector
       * local_cell_relations.             As no adaptive mesh refinement is
       * supported at the moment for this       class, all cells will be
       * flagged with the CellStatus CELL_PERSIST.       These relations will
       * currently only be used for serialization.             The stored
       * vector will have a size equal to the number of locally owned
      ...
      /**
       * Boolean indicating that the function create_triangulation() was
       * called       for internal usage.
       *
       */
