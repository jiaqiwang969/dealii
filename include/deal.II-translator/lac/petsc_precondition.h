...
#    endif
   ...
   * and preconditioner classes.     Note that derived classes only provide
   * interfaces to the relevant   functionality of PETSc. PETSc does not
   * implement all preconditioners for   all matrix types. In particular, some
   * preconditioners are not going to   work for parallel jobs, such as for
   * example the ILU preconditioner.
  ...
  /**
   * A class
   * that implements the interface to use the PETSc Jacobi   preconditioner.
   * See the comment in the base class     @ref PreconditionBase     for when
   * this preconditioner may or may not work.
   * @ingroup PETScWrappers
   *
   */
  ...
  /**
   * A class that implements the interface to use the PETSc Block Jacobi   preconditioner. PETSc defines the term "block Jacobi"
   * as a preconditioner   in which it looks at a number of diagonal blocks of
   * the matrix and then   defines a preconditioner in which the
   * preconditioner matrix has the same   block structure as only these
   * diagonal blocks, and each diagonal block   of the preconditioner is an
   * approximation of the inverse of the   corresponding block of the original
   * matrix.   The blocking structure of the matrix is determined by the
   * association of degrees of freedom to the individual processors in an
   * MPI-parallel job. If you use this preconditioner on a sequential job (or
   * an   MPI job with only one process) then the entire matrix is the only
   * block.     By default, PETSc uses an ILU(0) decomposition of each
   * diagonal block of   the matrix for preconditioning. This can be changed,
   * as is explained in   the relevant section of the PETSc manual, but is not
   * implemented here.     See the comment in the base class     @ref
   * PreconditionBase     for when this preconditioner may or may not work.
   * @ingroup PETScWrappers
   *
   */
  ...
  /**
   * A class
   * that implements the interface to use the PETSc Eisenstat
   * preconditioner, which implements SSOR on the diagonal block owned by
   * each processor.     See the comment in the base class     @ref
   * PreconditionBase     for when this preconditioner may or may not work.
   * @ingroup PETScWrappers
   *
   */
   ...
   * class is typically used with SolverPreOnly to get a direct   solver.
   * Alternatively, you can use   PreconditionBase::vmult()   directly.
   ...
   * configured with HYPRE (e.g. with \--download-hypre=1).     The
   * preconditioner does support parallel distributed computations. See
   ...
   * reduce the cost of applying the preconditioner.     ParaSails solves
   * symmetric positive definite (SPD) problems using a   factorized SPD
   * preconditioner and can also solve general (nonsymmetric   and/or
   * indefinite) problems with a nonfactorized preconditioner. The   problem
   * type has to be set in   @p AdditionalData.       The preconditioner does
  ...
  public:
    ...
    struct AdditionalData
    {
      ...
      /**
       * This parameter specifies the type of problem to solve:         <ul>           <li>     @p 0:   nonsymmetric and/or indefinite problem, and nonsymmetric       preconditioner         <li>     @p 1:   SPD problem, and SPD (factored) preconditioner         <li>     @p 2:   nonsymmetric, definite problem, and SPD (factored)       preconditioner         </ul>         Default is <tt>symmetric = 1</tt>.
       *
       */
  ...
  /**
   * Alias for backwards-compatibility.     @deprecated   Use
   * PETScWrappers::PreconditionBase   instead.
   *
   */
