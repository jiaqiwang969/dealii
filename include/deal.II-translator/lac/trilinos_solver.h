...
#    endif
   ...
   * user guide</a>.     This solver class can also be used as a standalone
   * class, where the   respective Krylov method is set via the flag
   * <tt>solver_name</tt>. This   can be done at runtime (e.g., when parsing
   * the solver from a   ParameterList) and is similar to the deal.II class
   * SolverSelector.
  ...
  public:
     ...
     * @p preconditioner   are an operator.     This function can be used when
     * both <tt>A</tt> and the   @p preconditioner       are LinearOperators
     * derived from a TrilinosPayload.     Depending on the information
     * provided by derived classes and the object     passed as a
     * preconditioner, one of the linear solvers and preconditioners     of
     * Trilinos is chosen.
     *
     */
     ...
     * from a TrilinosPayload.     Depending on the information provided by
     * derived classes and the object     passed as a preconditioner, one of
     * the linear solvers and preconditioners     of Trilinos is chosen.
     *
     */
    void
    solve(const Epetra_Operator &   A,
    ...
    /**
     * Solve the linear system <tt>Ax=b</tt> where both <tt>A</tt> and its
     * @p preconditioner   are an operator, and the vectors   @p x   and   @p
     * b   are     native Trilinos vector types.     This function can be used
     * when both <tt>A</tt> and the   @p preconditioner       are
     * LinearOperators derived from a TrilinosPayload.     Depending on the
     * information provided by derived classes and the object     passed as a
     * preconditioner, one of the linear solvers and preconditioners     of
     * Trilinos is chosen.
     *
  ...
  public:
     ...
     * give a vector memory object.         The last argument takes a
   ...
   * as third-party solvers Amesos_Superludist or Amesos_Mumps.     For
   * instructions on how to install Trilinos for use with direct solvers
  ...
  public:
    ...
    struct AdditionalData
    {
      ...
      /**
       * Set the solver type (for third party solver support of Trilinos       Amesos package). Possibilities are:         <ul>           <li>    "Amesos_Lapack"   </li>           <li>    "Amesos_Scalapack"   </li>           <li>    "Amesos_Klu"   </li>           <li>    "Amesos_Umfpack"   </li>           <li>    "Amesos_Pardiso"   </li>           <li>    "Amesos_Taucs"   </li>           <li>    "Amesos_Superlu"   </li>           <li>    "Amesos_Superludist"   </li>           <li>    "Amesos_Dscpack"   </li>           <li>    "Amesos_Mumps"   </li>           </ul>         Note that the availability of these solvers in deal.II depends on       which solvers were set when configuring Trilinos.
       *
       */
     ...
     * initialize(). Note the matrix is not refactorized during     this call.
     *
     */
    void
