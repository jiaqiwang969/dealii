 ...
 * <tt>MatrixType</tt> consisting of invertible blocks of   @p blocksize   on
 * the diagonal and provides the inversion of the diagonal blocks of the
 * matrix. It is not necessary for this class that the matrix be block
 * diagonal; rather, it applies to matrices of arbitrary structure with the
 * minimal property of having invertible blocks on the diagonal. Still the
 * matrix must have access to single matrix entries. Therefore,
 * BlockMatrixArray and similar classes are not a possible matrix class
 * template arguments. The block matrix structure used by this class is given,
 * e.g., for the DG method for the transport equation. For a downstream
 * numbering the matrices even have got a block lower left matrix structure,
 * i.e. the matrices are empty above the diagonal blocks.
 *
 *
 ...
 * step-22  . For all matrices that are empty above and below the diagonal
 * blocks (i.e. for all block diagonal matrices) the   @p BlockJacobi
 * preconditioner is a direct solver. For all matrices that are empty only
 * above the diagonal blocks (e.g. the matrices one gets by the DG method with
 * downstream numbering)   @p BlockSOR   is a direct solver. This first
 * implementation of the   @p PreconditionBlock   assumes the matrix has
 * blocks each of the same block size. Varying block sizes within the matrix
 * must still be implemented if needed. The first template parameter denotes
 * the type of number representation in the sparse matrix, the second denotes
 * the type of number representation in which the inverted diagonal block
 * matrices are stored within this class by <tt>invert_diagblocks()</tt>. If
 * you don't want to use the block inversion as an exact solver, but rather as
 * a preconditioner, you may probably want to store the inverted blocks with
 * less accuracy than the original matrix; for example, <tt>number==double,
 * inverse_type=float</tt> might be a viable choice.
...
public:
   ...
   * diagonal blocks may be computed.     Additionally, a relaxation parameter
   * for derived classes may be provided.
   *
   */
  void
...
protected:
   ...
   * meaning of these vectors see PreconditionBlockSOR.     In a second step,
   * the inverses of the diagonal blocks may be computed.   Make sure you use
   * invert_permuted_diagblocks() to yield consistent data.     Additionally,
   ...
   * depending on the size of the vector.     If the size of the permutation
   * vectors is equal to the dimension of the   linear system, it is assumed
   * that rows are permuted individually. In this   case, set_permutation()
   * must be called before initialize(), since the   diagonal blocks are built
   * from the permuted entries of the matrix.     If the size of the
   * permutation vector is not equal to the dimension of   the system, the
   * diagonal blocks are computed from the unpermuted entries.   Instead, the
   * relaxation methods step() and Tstep() are executed applying   the blocks
   * in the order given by the permutation vector. They will throw   an
   * exception if length of this vector is not equal to the number of
   * blocks.
   ...
   * vmult() and Tvmult().
   * @note   It is safe to call set_permutation() before initialize(), while
   * the   other order is only admissible for block permutation.
   *
   */
  void
...
public:
  ...
  /**
   * Perform one block relaxation step in forward numbering.     Depending on
   * the arguments   @p dst   and   @p pref,   this performs an SOR step
   * (both reference the same vector) of a Jacobi step (both different
   * vectors). For the Jacobi step, the calling function must copy   @p dst
   * to     @p pref   after this.
  ...
  /**
   * Perform one block relaxation step in backward numbering.     Depending on
   * the arguments   @p dst   and   @p pref,   this performs an SOR step
   * (both reference the same vector) of a Jacobi step (both different
   * vectors). For the Jacobi step, the calling function must copy   @p dst
   * to     @p pref   after this.
...
/**
 * Block Jacobi preconditioning. See PreconditionBlock for requirements on the matrix. This class satisfies the   @ref ConceptRelaxationType   "relaxation concept".
 *
 *
 * @note
...
public:
  ...
  /**
   * Execute block Jacobi preconditioning.     This function will
   * automatically use the inverse matrices if they exist,   if not then
   * BlockJacobi will need much time inverting the diagonal block   matrices
   * in each preconditioning step.
   *
   */
  template <typename number2>
  void
  ...
  /**
   * Execute block Jacobi preconditioning, adding to   @p dst.       This
   * function will automatically use the inverse matrices if they exist,   if
   * not then BlockJacobi will need much time inverting the diagonal block
...
private:
  /**
   * Actual implementation of the preconditioner.     Depending on   @p
   * adding,   the result of preconditioning is added to the   destination
   * vector.
   *
   */
  template <typename number2>
...
/**
 * Block SOR preconditioning. This class satisfies the   @ref ConceptRelaxationType   "relaxation concept".
 * The functions   @p vmult   and   @p Tvmult   execute a (transposed)
 * block-SOR step, based on the blocks in PreconditionBlock. The elements
 * outside the diagonal blocks may be distributed arbitrarily. See
 * PreconditionBlock for requirements on the matrix. The blocks used in this
 * class must be contiguous and non-overlapping. An overlapping Schwarz
 * relaxation method can be found in RelaxationBlockSOR; that class does not
 * offer preconditioning, though. <h3>Permutations</h3> Optionally, the
 * entries of the source vector can be treated in the order of the indices in
 * the permutation vector set by #set_permutation (or the opposite order for
 * Tvmult()). The inverse permutation is used for storing elements back into
 * this vector. This functionality is automatically enabled after a call to
...
public:
  ...
  /**
   * Execute block SOR preconditioning.     This function will automatically
   * use the inverse matrices if they exist,   if not then BlockSOR will waste
   * much time inverting the diagonal block   matrices in each preconditioning
   * step.     For matrices which are empty above the diagonal blocks BlockSOR
   * is a   direct solver.
   *
   */
  template <typename number2>
  ...
  /**
   * Execute block SOR preconditioning.     Warning: this function performs
   * normal   @p vmult   without adding. The   reason for its existence is
   * that BlockMatrixArray requires the adding   version by default. On the
   * other hand, adding requires an additional   auxiliary vector, which is
   * not desirable.       @see   vmult
   *
   */
  template <typename number2>
  void
  ...
  /**
   * Backward application of vmult().     In the current implementation, this
   * is not the transpose of vmult(). It   is a transposed Gauss-Seidel
   * algorithm applied to the whole matrix, but   the diagonal blocks being
   * inverted are not transposed. Therefore, it is   the transposed, if the
  ...
  /**
   * Execute backward block SOR preconditioning.     Warning: this function
   * performs normal   @p vmult   without adding. The   reason for its
   * existence is that BlockMatrixArray requires the adding   version by
   * default. On the other hand, adding requires an additional   auxiliary
   * vector, which is not desirable.       @see   vmult
   *
   */
  template <typename number2>
  void
...
protected:
   ...
   * vmult_add().     If a #permutation is set by set_permutation(), it will
   * automatically be   honored by this function.     The parameter   @p
   * adding   does not have any function, yet.
   *
   */
  template <typename number2>
  void
   ...
   * Tvmult_add().     If a #permutation is set by set_permutation(), it will
   * automatically be   honored by this function.     The parameter   @p
   * adding   does not have any function, yet.
   *
   */
...
/**
 * Block SSOR preconditioning. This class satisfies the   @ref ConceptRelaxationType   "relaxation concept".
 * The functions   @p vmult   and   @p Tvmult   execute a block-SSOR step,
 * based on the implementation in PreconditionBlockSOR.  This class requires
 * storage of the diagonal blocks and their inverses. See PreconditionBlock
 * for requirements on the matrix. The blocks used in this class must be
 * contiguous and non-overlapping. An overlapping Schwarz relaxation method
 * can be found in RelaxationBlockSSOR; that class does not offer
 * preconditioning, though.
 *
 *
 * @note
 * Instantiations for this template are provided for <tt>  @<float@>   and
...
public:
  ...
  /**
   * Execute block SSOR preconditioning.     This function will automatically
   * use the inverse matrices if they exist,   if not then BlockSOR will waste
   * much time inverting the diagonal block   matrices in each preconditioning
   * step.
   *
