...
namespace PETScWrappers
{
  ...
  namespace MPI
  {
     ...
     * <h3>Parallel communication model</h3>         The parallel
     * functionality of PETSc is built on top of the Message     Passing
     * Interface (MPI). MPI's communication model is built on     collective
     * communications: if one process wants something from another,     that
     * other process has to be willing to accept this communication. A
     ...
     * which this vector is shared have to call the   @p l2_norm   function.
     * If     you don't do this, but instead only call the   @p l2_norm
     * function on one     process, then the following happens: This one
     * process will call one of     the collective MPI functions and wait for
     * all the other processes to     join in on this. Since the other
     * processes don't call this function,     you will either get a time-out
     * on the first process, or, worse, by the     time the next a call to a
     * PETSc function generates an MPI message on     the other processes, you
     * will get a cryptic message that only a subset     of processes
     * attempted a communication. These bugs can be very hard to     figure
     * out, unless you are well-acquainted with the communication model     of
     * MPI, and know which functions may generate MPI messages.         One
     * particular case, where an MPI message may be generated unexpectedly
     * is discussed below.             <h3>Accessing individual elements of a
     * vector</h3>         PETSc does allow read access to individual elements
     * of a vector, but in     the distributed case only to elements that are
     * stored locally. We     implement this through calls like
     * <tt>d=vec(i)</tt>. However, if you     access an element outside the
     * locally stored range, an exception is     generated.         In
     * contrast to read access, PETSc (and the respective deal.II wrapper
     ...
     * synchronization with compress() happens in between.         In order to
     * make sure these calls to compress() happen at the     appropriate time,
     * the deal.II wrappers keep a state variable that store     which is the
     * presently allowed operation: additions or writes. If it     encounters
     * an operation of the opposite kind, it calls compress() and     flips
     * the state. This can sometimes lead to very confusing behavior, in
    ...
    public:
      ...
      /**
       * Constructor. Set dimension to   @p n   and initialize all elements
       * with       zero.               @arg   locally_owned_size denotes the
       * size of the chunk that shall be       stored on the present process.
       * @arg   communicator denotes the MPI communicator over which the
       * different parts of the vector shall communicate             The
       * constructor is made explicit to avoid accidents like this:
       * <tt>v=0;</tt>. Presumably, the user wants to set every element of the
       * vector to zero, but instead, what happens is this call:
       * <tt>v=Vector  @<number@>(0);</tt>,   i.e. the vector is replaced by
       * one       of length zero.
       *
       */
       ...
       * the given vector, and copies all elements.               @arg
       * locally_owned_size denotes the size of the chunk that shall be
       * stored on the present process.               @arg   communicator
       * denotes the MPI communicator over which the       different parts of
       * the vector shall communicate
       *
       */
       ...
       * @arg   local_size denotes the size of the chunk that shall be stored
       * on       the present process.               @arg   communicator
       * denotes the MPI communicator over which the       different parts of
       * the vector shall communicate               @deprecated   The use of
       * objects that are explicitly of type VectorBase       is deprecated:
       * use   PETScWrappers::MPI::Vector   instead.
       *
       */
      DEAL_II_DEPRECATED
      explicit Vector(const MPI_Comm &  communicator,
       ...
       * IndexSet::is_ascending_and_one_to_one().    In particular, the DoFs
       * in         @p local   need to be contiguous, meaning you can only
       * create vectors       from a DoFHandler with several finite element
       * components if they are       not reordered by component (use a
       * PETScWrappers::BlockVector         otherwise).  The global size of
       * the vector is determined by       local.size(). The global indices in
       * @p ghost   are supplied as ghost       indices so that they can be
       * read locally.             Note that the   @p ghost   IndexSet may be
       * empty and that any indices       already contained in   @p local
       * are ignored during construction. That       way, the ghost parameter
       * can equal the set of locally relevant       degrees of freedom, see
       * step-32  .
       * @note   This operation always creates a ghosted vector, which is considered       read-only.               @see           @ref GlossGhostedVector   "vectors with ghost elements"
       *
       */
       ...
       * IndexSet.             Note that   @p local   must be ascending and
       * 1:1, see         IndexSet::is_ascending_and_one_to_one().    In
       * particular, the DoFs in         @p local   need to be contiguous,
       * meaning you can only create vectors       from a DoFHandler with
       * several finite element components if they are       not reordered by
       * component (use a   PETScWrappers::BlockVector         otherwise).
       *
       */
       ...
       * vector wrapper class) into this object.             Contrary to the
       * case of sequential vectors, this operators requires       that the
       * present vector already has the correct size, since we need       to
       * have a partition and a communicator present which we otherwise
       ...
       * same amount of memory is used               @p locally_owned_size
       * denotes how many of the   @p N   values shall be       stored locally
       * on the present process. for less data.               @p communicator
       * denotes the MPI communicator henceforth to be used       for this
       * vector.             If   @p omit_zeroing_entries   is false, the
       * vector is filled by zeros.       Otherwise, the elements are left an
       * unspecified state.
       *
       */
      void
      ...
      /**
       * Change the dimension to that of the vector   @p v,   and also take
       * over       the partitioning into local sizes as well as the MPI
       * communicator.       The same applies as for the other   @p reinit
       * function.             The elements of   @p v   are not copied, i.e.
       * this function is the same       as calling <tt>reinit(v.size(),
       * v.locally_owned_size(),       omit_zeroing_entries)</tt>.
       *
       */
      void
      ...
      /**
       * Reinit as a vector with ghost elements. See the constructor with       same signature for more details.               @see           @ref GlossGhostedVector   "vectors with ghost elements"
       *
       */
      void
      ...
      /**
       * Reinit as a vector without ghost elements. See constructor with same       signature for more details.               @see           @ref GlossGhostedVector   "vectors with ghost elements"
       *
       */
      void
       ...
       * @note   This function overloads the one in the base class to make
       * this       a collective operation.
       *
       */
      bool
      all_zero() const;
    ...
    protected:
      /**
       * Create a vector of length   @p n.   For this class, we create a
       * parallel       vector.   @p n   denotes the total size of the vector
       * to be created.   @p         locally_owned_size denotes how many of
       * these elements shall be stored       locally.
       *
       */
      virtual void
