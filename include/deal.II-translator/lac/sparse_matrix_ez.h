...
/**
 * Sparse matrix without sparsity pattern. Instead of using a pre-assembled
 * sparsity pattern, this matrix builds the pattern on the fly. Filling the
 * matrix may consume more time than for   @p SparseMatrix,   since large
 * memory movements may be involved when new matrix elements are inserted
 * somewhere in the middle of the matrix and no currently unused memory
 * locations are available for the row into which the new entry is to be
 * inserted. To help optimize things, an expected row-length may be provided
 * to the constructor, as well as an increment size for rows. This class uses
 * a storage structure that, similar to the usual sparse matrix format, only
 * stores non-zero elements. These are stored in a single data array for the
 * entire matrix, and are ordered by row and, within each row, by column
 * number. A separate array describes where in the long data array each row
 * starts and how long it is. Due to this structure, gaps may occur between
 * rows. Whenever a new entry must be created, an attempt is made to use the
 * gap in its row. If no gap is left, the row must be extended and all
 * subsequent rows must be shifted backwards. This is a very expensive
 * operation and explains the inefficiency of this data structure and why it
 * is useful to pre-allocate a sparsity pattern as the SparseMatrix class
 * does. This is where the optimization parameters, provided to the
 * constructor or to the reinit() functions come in.   @p default_row_length
 * is the number of entries that will be allocated for each row on
 * initialization (the actual length of the rows is still zero). This means,
 * that   @p default_row_length   entries can be added to this row without
 * shifting other rows. If fewer entries are added, the additional memory will
 * of course be wasted. If the space for a row is not sufficient, then it is
 * enlarged by   @p default_increment   entries. This way, subsequent rows are
 * not shifted by single entries very often. Finally, the   @p default_reserve
 * allocates extra space at the end of the data array. This space is used
 * whenever any row must be enlarged. It is important because otherwise not
 * only the following rows must be moved, but in fact <i>all</i> rows after
 * allocating sufficiently much space for the entire data array. Suggested
 * settings:   @p default_row_length   should be the length of a typical row,
 * for instance the size of the stencil in regular parts of the grid. Then,
 * @p default_increment   may be the expected amount of entries added to the
 * row by having one hanging node. This way, a good compromise between memory
 * consumption and speed should be achieved.   @p default_reserve   should
 * then be an estimate for the number of hanging nodes times   @p
 * default_increment. Letting   @p default_increment   be zero causes an
 * exception whenever a row overflows. If the rows are expected to be filled
 * more or less from first to last, using a   @p default_row_length   of zero
 * may not be such a bad idea.
 *
 *
 * @note   The name of the class makes sense by pronouncing it the American
 * way,   where "EZ" is pronounced the same way as the word "easy".
 *
 *
 */
template <typename number>
...
public:
   ...
   * be called for empty objects.     If you really want to copy a whole
   * matrix, you can do so by using the   @p     copy_from function.
   *
   */
   ...
   * The optional parameters   @p default_row_length   and   @p
   * default_increment     allow for preallocating memory. Providing these
   * properly is essential for   an efficient assembling of the matrix.
   *
   */
  ...
  /**
   * Print statistics. If   @p full   is   @p true,   prints a histogram of
   * all   existing row lengths and allocated row lengths. Otherwise, just the
  ...
  /**
   * Compute numbers of entries.     In the first three arguments, this
   * function returns the number of entries   used, allocated and reserved by
   * this matrix.     If the final argument is true, the number of entries in
   * each line is   printed as well.
   *
   */
  void
  ...
  //@{
  /**
   * Set the element <tt>(i,j)</tt> to   @p value.       If <tt>value</tt> is
   * not a finite number an exception is thrown.     The optional parameter
   * <tt>elide_zero_values</tt> can be used to specify   whether zero values
   * should be added anyway or these should be filtered   away and only
   * non-zero data is added. The default value is <tt>true</tt>,   i.e., zero
   * values won't be added into the matrix.     If this function sets the
   * value of an element that does not yet exist,   then it allocates an entry
   * for it. (Unless `elide_zero_values` is `true`   as mentioned above.)
   * @note   You may need to insert zero elements if you want to keep a
  ...
  /**
   * Add   @p value   to the element <tt>(i,j)</tt>.     If this function adds
   * to the value of an element that does not yet exist,   then it allocates
   * an entry for it.     The function filters out zeroes automatically, i.e.,
   * it does not create   new entries when adding zero to a matrix element for
   * which no entry   currently exists.
   *
   */
  void
   ...
   * the data type of this matrix.     The optional parameter
   * <tt>elide_zero_values</tt> can be used to specify   whether zero values
   * should be added anyway or these should be filtered   away and only
   * non-zero data is added. The default value is <tt>true</tt>,   i.e., zero
   * values won't be added into the matrix.     The function returns a
   * reference to   @p this.
   *
   */
  ...
  /**
   * Add   @p matrix   scaled by   @p factor   to this matrix.     The source
   * matrix may be a matrix of arbitrary type, as long as its data   type is
   * convertible to the data type of this matrix and it has the   standard
   * @p const_iterator.
   *
   */
  template <typename MatrixType>
   ...
   * does not exist in the matrix.     In case you want a function that
   * returns zero instead (for entries that   are not in the sparsity pattern
   * of the matrix), use the   @p el   function.
   *
   */
  number
   ...
   * this   matrix. This function does the same as   @p vmult   but takes the
   * transposed   matrix.
   *
   */
  template <typename somenumber>
  ...
  /**
   * Adding Matrix-vector multiplication. Add   $M^T*src$   to   $dst$   with
   * $M$     being this matrix. This function does the same as   @p vmult_add
   ...
   * @p     src vector by the inverse of the respective diagonal element and
  ...
  /**
   * Apply SOR preconditioning matrix to   @p src.   The result of this method
   * is     $dst = (om D
   *
   * - L)^{-1} src$  .
   *
  ...
  /**
   * Apply transpose SOR preconditioning matrix to   @p src.   The result of
   * this   method is   $dst = (om D
   *
   * - U)^{-1} src$  .
   *
   */
  ...
  /**
   * Add the matrix   @p A   conjugated by   @p B,   that is,   $B A B^T$   to
   * this   object. If the parameter   @p transpose   is true, compute   $B^T
   * A B$  .     This function requires that   @p B   has a   @p
   * const_iterator   traversing all   matrix entries and that   @p A   has a
   * function <tt>el(i,j)</tt> for access   to a specific entry.
   *
   */
  ...
  /**
   * Iterator starting at the first entry of row   @p r.   If this row is
   * empty,   the result is <tt>end(r)</tt>, which does NOT point into row
   * @p r.
   *
   */
  const_iterator
   ...
   * flexible setting of the output format:   @p     precision and   @p
   * scientific   are used to determine the number format,   where   @p
   * scientific   =   @p false   means fixed point notation.  A zero entry
   * for   @p width   makes the function compute a width, but it may be
   ...
   * readable output, even integers.     This function may produce   @em
   * large amounts of output if applied to a   large matrix!
   *
   */
  void
  ...
  /**
   * Write the data of this object in binary mode to a file.     Note that
   * this binary format is platform dependent.
   *
   */
  void
  ...
  /**
   * Read data that has previously been written by   @p block_write.       The
   * object is resized on this operation, and all previous contents are
   * lost.     A primitive form of error checking is performed which will
   * recognize the   bluntest attempts to interpret some data as a vector
   * stored bitwise to a   file, but not more.
   *
   */
  void
...
private:
  ...
  /**
   * Version of   @p vmult   which only performs its actions on the region
   * defined   by <tt>[begin_row,end_row)</tt>. This function is called by
   * @p vmult   in   the case of enabled multithreading.
   *
   */
  template <typename somenumber>
  void
  ...
  /**
   * Version of   @p matrix_norm_square   which only performs its actions on
   * the   region defined by <tt>[begin_row,end_row)</tt>. This function is
   * called   by   @p matrix_norm_square   in the case of enabled
   * multithreading.
   *
   */
  template <typename somenumber>
  ...
  /**
   * Version of   @p matrix_scalar_product   which only performs its actions
   * on   the region defined by <tt>[begin_row,end_row)</tt>. This function is
   * called by   @p matrix_scalar_product   in the case of enabled
   * multithreading.
   *
   */
  template <typename somenumber>
