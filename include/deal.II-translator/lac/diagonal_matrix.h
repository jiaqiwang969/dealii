 ...
 * VectorType::scale,   so the template vector class needs to provide a   @p
 * scale()   method. When using this class with
 * ConstraintsMatrix::distribute_local_to_global(),   the underlying vector
 * needs to provide write access to all entries referenced by cells in an
 * assembly process. This means that this class also needs access to ghost
 * entries that are owned by other processors than the calling one. In
 * practice this requires initialization of the vector as follows
 *
 * @code
...
public:
   ...
   * where `n` is the size of the vector, and with diagonal entries equal to
   * the   elements of   @p vec.
   *
   */
  ...
  /**
   * Clear content of this object and reset to the state of default
   * constructor.
   *
   */
  void
   ...
   * <i>i==j</i> due to the matrix storage.     If the vector representing the
   * diagonal is distributed with MPI, not all   of the indices <i>i</i> might
   * actually be accessible. Refer to the method
   ...
   * other entries are ignored and no exception is thrown.     This function
   * is for a consistent interface with the other matrix   classes in deal.II
   * and can be used in     AffineConstraints::distribute_local_to_global   to
   * get exactly the same   diagonal as when assembling into a sparse matrix.
   *
   */
  ...
  /**
   * Add value to the element (i,j).     Due to the storage of this matrix,
   * entries are only added to the diagonal   of the matrix. All other entries
   * are ignored and no exception is thrown.
   *
   */
  void
   ...
   * @p diagonal   member of this class.     This is a part of the interface
   * required   by linear_operator().
   *
   */
  void
