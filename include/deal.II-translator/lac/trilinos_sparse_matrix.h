...
namespace TrilinosWrappers
{
  ...
  namespace SparseMatrixIterators
  {
     ...
     * For a regular   dealii::SparseMatrix,   we would use an accessor for
     * the     sparsity pattern. For Trilinos matrices, this does not seem so
     * simple,     therefore, we write a little base class here.
     *
     */
    class AccessorBase
    {
    ...
    protected:
       ...
       * likely that we will access them sequentially anyway.             In
       * order to make copying of iterators/accessor of acceptable
     ...
     * of the matrix.         The general template is not implemented, only
     * the specializations for     the two possible values of the second
     * template argument. Therefore, the     interface listed here only serves
     * as a template provided since doxygen     does not link the
     * specializations.
     *
     */
     ...
     * PETSc matrices.         Note that Trilinos stores the elements within
     * each row in ascending     order. This is opposed to the deal.II sparse
     * matrix style where the     diagonal element (if it exists) is stored
     * before all other values, and     the PETSc sparse matrices, where one
     * can't guarantee a certain order of     the elements.
  ...
  public:
     ...
     * #n columns.         The number of columns entries per row is specified
     * as the maximum     number of entries argument.
     *
     */
    ...
    /**
     * Generate a matrix that is completely stored locally, having #m rows and
     * #n columns.         The vector <tt>n_entries_per_row</tt> specifies the
     ...
     * rows.         This is a collective operation that needs to be called on
     * all     processors in order to avoid a dead lock.
     *
     */
     ...
     * distributed) Trilinos sparsity pattern.         This is a collective
     * operation that needs to be called on all     processors in order to
     * avoid a dead lock.         If you want to write to the matrix from
     * several threads and use MPI,     you need to use this reinit method
     * with a sparsity pattern that has     been created with explicitly
     * stating writeable rows. In all other     cases, you cannot mix MPI with
     ...
     * this.         This is a collective operation that needs to be called on
     * all     processors in order to avoid a dead lock.
     *
     */
    void
     ...
     * deal.II matrix can be filtered away).         The optional parameter
     * <tt>copy_values</tt> decides whether only the     sparsity structure of
     * the input matrix should be used or the matrix     entries should be
     * copied, too.         This is a collective operation that needs to be
     * called on all     processors in order to avoid a deadlock.
     ...
     * %parallel partitioning. The parameter   @p n_max_entries_per_row   sets
     * the     number of nonzero entries in each row that will be allocated.
     * Note that     this number does not need to be exact, and it is even
     * allowed that the     actual matrix structure has more nonzero entries
     * than specified in the     constructor. However it is still advantageous
     * to provide good estimates     here since this will considerably
     * increase the performance of the     matrix setup. However, there is no
     * effect in the performance of matrix-     vector products, since
     * Trilinos reorganizes the matrix memory prior to     use (in the
     * compress() step).
     *
     */
     ...
     * The parameter   @p n_max_entries_per_row   defines how much memory will
     * be     allocated for each row. This number does not need to be
     * accurate, as     the structure is reorganized in the compress() call.
     *
     */
     ...
     * to the deal.II own object. The optional argument   @p exchange_data
     * can     be used for reinitialization with a sparsity pattern that is
     * not fully     constructed. This feature is only implemented for input
     * sparsity     patterns of type DynamicSparsityPattern. If the flag is
     * not set, each     processor just sets the elements in the sparsity
     * pattern that belong to     its rows.         This is a collective
     * operation that needs to be called on all     processors in order to
     * avoid a dead lock.
     *
     */
     ...
     * DynamicSparsityPattern.         This is a collective operation that
     * needs to be called on all     processors in order to avoid a dead lock.
     *
     */
    template <typename SparsityPatternType>
     ...
     * generating it.         The optional parameter <tt>copy_values</tt>
     * decides whether only the     sparsity structure of the input matrix
     * should be used or the matrix     entries should be copied, too.
     ...
     * matrices.         The optional parameter <tt>copy_values</tt> decides
     * whether only the     sparsity structure of the input matrix should be
     * used or the matrix     entries should be copied, too.         This is a
     * collective operation that needs to be called on all     processors in
     ...
     * the default constructor.         This is a collective operation that
     * needs to be called on all     processors in order to avoid a dead lock.
     *
     */
    void
    ...
    /**
     * Set the element (<i>i,j</i>) to   @p value.           This function is
     * able to insert new elements into the matrix as long as     compress()
     * has not been called, so the sparsity pattern will be     extended. When
     * compress() is called for the first time (or in case the     matrix is
     * initialized from a sparsity pattern), no new elements can be     added
     * and an insertion of elements at positions which have not been
     * initialized will throw an exception.         For the case that the
     * matrix is constructed without a sparsity pattern     and new matrix
     * entries are added on demand, please note the following     behavior
     * imposed by the underlying Epetra_FECrsMatrix data structure:     If the
     * same matrix entry is inserted more than once, the matrix entries
     ...
     * In     the case you cannot make sure that matrix entries are only set
     * once,     initialize the matrix with a sparsity pattern to fix the
     ...
     * situation in FE calculations.         This function is able to insert
     * new elements into the matrix as long as     compress() has not been
     * called, so the sparsity pattern will be     extended. After compress()
     * has been called for the first time or the     matrix has been
     * initialized from a sparsity pattern, extending the     sparsity pattern
     * is no longer possible and an insertion of elements at     positions
     * which have not been initialized will throw an exception.         The
     * optional parameter <tt>elide_zero_values</tt> can be used to
     ...
     * values are inserted/replaced.         For the case that the matrix is
     * constructed without a sparsity pattern     and new matrix entries are
     * added on demand, please note the following     behavior imposed by the
     * underlying Epetra_FECrsMatrix data structure:     If the same matrix
     * entry is inserted more than once, the matrix entries     will be added
     * upon calling compress() (since Epetra does not track     values to the
     * same entry before the final compress() is called), even     if
     * VectorOperation::insert   is specified as argument to compress(). In
     ...
     * in columns given by col_indices into the sparse matrix.         This
     * function is able to insert new elements into the matrix as long as
     * compress() has not been called, so the sparsity pattern will be
     * extended. After compress() has been called for the first time or the
     * matrix has been initialized from a sparsity pattern, extending the
     * sparsity pattern is no longer possible and an insertion of elements at
    ...
    /**
     * Add   @p value   to the element (<i>i,j</i>).         Just as the
     * respective call in deal.II SparseMatrix<Number> class (but     in
     * contrast to the situation for PETSc based matrices), this function
     ...
     * the usual situation in FE calculations.         Just as the respective
     * call in deal.II SparseMatrix<Number> class (but     in contrast to the
     * situation for PETSc based matrices), this function     throws an
     * exception if an entry does not exist in the sparsity pattern.
     * The optional parameter <tt>elide_zero_values</tt> can be used to
     ...
     * only sets the entries to zero.         This operation is used in
     * eliminating constraints (e.g. due to hanging     nodes) and makes sure
     * that we can write this modification to the matrix     without having to
     * read entries (such as the locations of non-zero     elements) from it
     * &mdash; without this operation, removing constraints     on %parallel
     * matrices is a rather complicated procedure.         The second
     * parameter can be used to set the diagonal entry of this row     to a
     * value different from zero. The default is to set it to zero.
     ...
     * for the matrix object and the one for the vector object do not
     * coincide.     This case is not implemented. Calling it will result in a
     * runtime error.     Despite looking complicated, the return type is just
     * `void`.
     *
     */
    template <typename VectorType>
     ...
     * takes the transposed matrix.         Source and destination must not be
     * the same vector.         This function can be called with several types
     * of vector objects,     see the discussion about   @p VectorType   in
     * vmult().         This function will be called when the underlying
     * number type for the     matrix object and the one for the vector object
     * are the same.     Despite looking complicated, the return type is just
     * `void`.
     *
     */
    template <typename VectorType>
     ...
     * with <i>M</i> being this matrix.         Source and destination must
     * not be the same vector.         This function can be called with
     * several types of vector objects,     see the discussion about   @p
     * VectorType   in vmult().
     *
     */
    template <typename VectorType>
     ...
     * as vmult_add() but takes the transposed matrix.         Source and
     * destination must not be the same vector.         This function can be
     * called with several types of vector objects,     see the discussion
     * about   @p VectorType   in vmult().
     *
     */
    template <typename VectorType>
    void
     ...
     * useful,     e.g. in the finite element context, where the   $L_2$
     * norm of a function     equals the matrix norm with respect to the mass
     * matrix of the vector     representing the nodal values of the finite
     * element function.         Obviously, the matrix needs to be quadratic
     * for this operation.         The implementation of this function is not
     * as efficient as the one in     the   @p SparseMatrix   class used in
     * deal.II (i.e. the original one, not     the Trilinos wrapper class)
     * since Trilinos doesn't support this     operation and needs a temporary
     * vector.         The vector has to be initialized with the same IndexSet
     * the matrix     was initialized with.         In case of a localized
     * Vector, this function will only work when     running on one processor,
     * since the matrix object is inherently     distributed. Otherwise, an
    ...
    /**
     * Compute the matrix scalar product   $\left(u,Mv\right)$  .         The
     * implementation of this function is not as efficient as the one in
     * the   @p SparseMatrix   class used in deal.II (i.e. the original one,
     * not     the Trilinos wrapper class) since Trilinos doesn't support this
     * operation and needs a temporary vector.         The vector   @p u   has
     * to be initialized with the same IndexSet that     was used for the row
     * indices of the matrix and the vector   @p v   has     to be initialized
     * with the same IndexSet that was used for the     column indices of the
     * matrix.         In case of a localized Vector, this function will only
     * work when     running on one processor, since the matrix object is
     * inherently     distributed. Otherwise, an exception will be thrown.
     ...
     * The vectors   @p dst   and   @p b   have to be initialized with the
     * same     IndexSet that was used for the row indices of the matrix and
     * the vector       @p x   has to be initialized with the same IndexSet
     * that was used for the     column indices of the matrix.         In case
     * of a localized Vector, this function will only work when     running on
     * one processor, since the matrix object is inherently     distributed.
     ...
     * function.         The content as well as the sparsity pattern of the
     * matrix C will be     changed by this function, so make sure that the
     * sparsity pattern is not     used somewhere else in your program. This
     * is an expensive operation, so     think twice before you use this
     * function.
     *
     */
    void
     ...
     * entries.         This function assumes that the calling matrix
     * <tt>A</tt> and <tt>B</tt>     have compatible sizes. The size of
     * <tt>C</tt> will be set within this     function.         The content as
     * well as the sparsity pattern of the matrix C will be     changed by
     * this function, so make sure that the sparsity pattern is not     used
     * somewhere else in your program. This is an expensive operation, so
     ...
     * that     is compatible to the l1-norm for vectors, i.e.    $|Mv|_1 \leq
     * |M|_1 |v|_1$  .  (cf. Haemmerlin-Hoffmann: Numerische Mathematik)
     *
     */
    TrilinosScalar
    l1_norm() const;
     ...
     * j} |M_{ij}|$  , (max. sum of rows).  This is the natural matrix norm
     * that     is compatible to the linfty-norm of vectors, i.e.
     * $|Mv|_\infty \leq |M|_\infty |v|_\infty$  .  (cf. Haemmerlin-Hoffmann:
     ...
     * iterate over them.         When you iterate over the elements of a
     * parallel matrix, you will only     be able to access the locally owned
     * rows. (You can access the other     rows as well, but they will look
     * empty.) In that case, you probably     want to call the begin()
     * function that takes the row as an argument to     limit the range of
     ...
     * that case if neither row   @p r   nor any of the following rows contain
     * any     nonzero entries.         The elements accessed by iterators
     * within each row are ordered in the     way in which Trilinos stores
     * them, though the implementation guarantees     that all elements of one
     * row are accessed before the elements of the     next row. If your
     * algorithm relies on visiting elements within one row,     you will need
     * to consult with the Trilinos documentation on the order     in which it
     * stores data. It is, however, generally not a good and long-     term
     * stable idea to rely on the order in which receive elements if you
     * iterate over them.
     ...
     * ,     or past the end of the entire sparsity pattern if none of the
     * rows     after   @p r   contain any entries at all.         Note that
     * the end iterator is not necessarily dereferenceable. This is     in
  ...
  namespace internal
  {
    ...
    namespace LinearOperatorImplementation
    {
       ...
       * wrapping a Trilinos solver, which calls Epetra_Operator functions, as
       * a       LinearOperator.
       * @note   The   TrilinosWrappers::SparseMatrix   or
       * TrilinosWrappers::PreconditionBase   that this payload wraps is
       * passed by       reference to the <tt>vmult</tt> and <tt>Tvmult</tt>
       * functions. This       object is not thread-safe when the transpose
       * flag is set on it or the       Trilinos object to which it refers.
       * See the docuemtation for the
      ...
      public:
        ...
        /**
         * Definition for the vector type for the domain space of the
         * operator.
         *
         */
        ...
        /**
         * Default constructor
         * @note   By design, the resulting object is inoperable since there
         * is         insufficient information with which to construct the
        ...
        /**
         * Constructor for a preconditioner based on an exemplary
         * preconditioner
         *
         */
        TrilinosPayload(
        ...
        /**
         * Composite copy constructor                 This is required for
         * PackagedOperations as it sets up the domain and         range maps,
         * and composite <tt>vmult</tt> and <tt>Tvmult</tt>         operations
         ...
         * functions, namely <tt>inv_vmult</tt> and <tt>inv_Tvmult</tt>, both
         * of         which wrap inverse operations. The <tt>vmult</tt> and
         * <tt>Tvmult</tt>         operations retain the standard
         * definitions inherited from   @p op.
         * @note   This function is enabled only if the solver and
         * preconditioner         derive from the respective TrilinosWrappers
         * base classes.         The C++ compiler will therefore only consider
         * this function if the         following criterion are satisfied:
         * 1. the   @p Solver   derives from   TrilinosWrappers::SolverBase,
         * and         2. the   @p Preconditioner   derives from
         ...
         * Invoking this factory function will configure two additional
         * functions, namely <tt>inv_vmult</tt> and <tt>inv_Tvmult</tt>, both
         * of         which         are disabled because the   @p Solver   or
         * @p Preconditioner   are not         compatible with
         * Epetra_MultiVector.         The <tt>vmult</tt> and <tt>Tvmult</tt>
         * operations retain the standard         definitions inherited from
         * @p op.
         * @note   The C++ compiler will only consider this function if the
         * following criterion are satisfied:         1. the   @p Solver
         * does not derive from   TrilinosWrappers::SolverBase,   and
        ...
        /**
         * Return an IndexSet that defines the partitioning of the domain
         * space         of this matrix, i.e., the partitioning of the vectors
         * this matrix has         to be multiplied with / operate on.
         *
         */
        IndexSet
        locally_owned_domain_indices() const;
        ...
        /**
         * Sets an internal flag so that all operations performed by the
         * matrix,         i.e., multiplications, are done in transposed
         * order.
         * @note   This does not reshape the matrix to transposed form
         * directly,         so care should be taken when using this flag.
         *
         */
        void
        transpose();
         ...
         * @note   This is not called by a LinearOperator, but rather by
         * Trilinos         functions that expect this to mimic the action of
         * the LinearOperator.
         *
         */
         ...
         * Sets an internal flag so that all operations performed by the
         * matrix,         i.e., multiplications, are done in transposed
         * order.                 This overloads the same function from the
         * Trilinos class         Epetra_Operator.
         * @note   This does not reshape the matrix to transposed form
         * directly,         so care should be taken when using this flag.
         * When the flag is set to         true (either here or directly on
         * the underlying Trilinos object         itself), this object is no
         * longer thread-safe. In essence, it is not         possible ensure
         * that the transposed state of the LinearOperator and         the
         * underlying Trilinos object remain synchronized throughout all
        ...
        /**
         * Apply the vmult operation on a vector   @p X   (of internally
         * defined         type VectorType) and store the result in the vector
         * @p Y.                   This overloads the same function from the
         * Trilinos class         Epetra_Operator.
         * @note   The intended operation depends on the status of the
         * internal         transpose flag. If this flag is set to true, the
         * result will be         the equivalent of performing a Tvmult
         * operation.
         *
         */
        virtual int
        ...
        /**
         * Apply the vmult inverse operation on a vector   @p X   (of
         * internally         defined type VectorType) and store the result in
         * the vector   @p Y.                   In practise, this function is
         * only called from a Trilinos solver if         the wrapped object is
         * to act as a preconditioner.                 This overloads the same
         * function from the Trilinos class         Epetra_Operator.
        ...
        /**
         * Return a label to describe this class.                 This
         * overloads the same function from the Trilinos class
         * Epetra_Operator.
         *
         */
        virtual const char *
         ...
         * this object.                 This overloads the same function from
         * the Trilinos class         Epetra_Operator.
         *
         */
        ...
        /**
         * Return the partitioning of the domain space of this matrix, i.e.,
         * the         partitioning of the vectors this matrix has to be
         * multiplied with.                 This overloads the same function
         * from the Trilinos class         Epetra_Operator.
         *
         */
        ...
        /**
         * Return the partitioning of the range space of this matrix, i.e.,
         * the         partitioning of the vectors that are result from
         * matrix-vector         products.                 This overloads the
         * same function from the Trilinos class         Epetra_Operator.
         *
         */
        virtual const Epetra_Map &
      ...
      private:
        ...
        /**
         * Internal communication pattern in case the matrix needs to be
...
#    endif
         ...
         * case, this always returns a negetive result.                 This
         * overloads the same function from the Trilinos class
         * Epetra_Operator.
         *
         */
        ...
        /**
         * Return the infinity norm of this operator.         Throws an error
         * since, in general, we cannot compute this value.
         * This overloads the same function from the Trilinos class
         * Epetra_Operator.
         *
         */
