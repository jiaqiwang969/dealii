...
public:
   ...
   * usage with   reinit(BlockSparsityPattern). The number of blocks per row
   * and column are   then determined by that function.
   *
   */
   ...
   * later on by calling the reinit() function.     This constructor
   * initializes all sub-matrices with the sub-sparsity   pattern within the
   * argument.     You have to make sure that the lifetime of the sparsity
   * structure is at   least as long as that of this matrix or as long as
   * reinit() is not called   with a new sparsity structure.
   *
   */
   ...
   * previously tied to.     This calls   SparseMatrix::clear   on all
   * sub-matrices and then resets this   object to have no blocks at all.
   *
   */
  void
  clear();
   ...
   * reserved.     Basically, this function only calls
   * SparseMatrix::reinit()   of the sub-   matrices with the block sparsity
   * patterns of the parameter.     You have to make sure that the lifetime of
   * the sparsity structure is at   least as long as that of this matrix or as
   * long as reinit(const   SparsityPattern &) is not called with a new
   * sparsity structure.     The elements of the matrix are set to zero by
   * this function.
   *
   */
  virtual void
  ...
  /**
   * Apply the Jacobi preconditioner to a simple vector.     The matrix must
   * be a single square block for this.
   *
   */
