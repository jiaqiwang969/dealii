...
namespace LinearAlgebra
{
  ...
  namespace CUDAWrappers
  {
     ...
     * @note   Only float and double are supported.           @see
     * CUDAWrappers
     * @ingroup Vectors
     *
     */
    ...
    public:
       ...
       * with       zero.             The constructor is made explicit to
       * avoid accident like this:       <tt>v=0;</tt>. Presumably, the user
       * wants to set every elements of       the vector to zero, but instead,
       * what happens is this call:       <tt>v=Vector  @<Number@>(0);</tt>,
       ...
       * functions.             This function is virtual in order to allow for
       ...
       * elements int   @p V   should be added to the current vector or
       * replace       the current elements. The last parameter is not used.
       * It is only used       for distributed vectors. This is the function
       * that should be used to       copy a vector to the GPU.
       *
       */
      virtual void
      ...
      /**
       * Add   @p to   all components. Note that   @p a   is a scalar not a
       * vector.
       *
       */
      virtual void
       ...
       * if   @p W   equals   @p this).               For complex-valued
