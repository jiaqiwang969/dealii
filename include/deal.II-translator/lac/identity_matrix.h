 ...
 * given size, i.e. a matrix with entries   $A_{ij}=\delta_{ij}$  . While it
 * has the most important ingredients of a matrix, in particular that one can
 * ask for its size and perform matrix-vector products with it, a matrix of
 * this type is really only useful in two contexts: preconditioning and
 * initializing other matrices. <h4>Initialization</h4> The main usefulness of
 * this class lies in its ability to initialize other matrix, like this:
 *
 * @code
 ...
 * matrices.
 *
 *  <h4>Preconditioning</h4> No preconditioning at all is equivalent to
 * preconditioning with preconditioning with the identity matrix. deal.II has
 * a specialized class for this purpose, PreconditionIdentity, than can be
 * used in a context as shown in the documentation of that class. The present
 * class can be used in much the same way, although without any additional
 * benefit:
 *
 * @code
