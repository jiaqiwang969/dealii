...
namespace TrilinosWrappers
{
  namespace MPI
  {
    ...
    public:
       ...
       * the internal structure of the block components.             Since
       * Trilinos only works on doubles, this function is limited to
       ...
       * the flag   @p vector_writable   is set to   @p false,   the vector
       * only       allows read access to the joint set of   @p
       * parallel_partitioning   and         @p ghost_entries.   The effect of
       * the reinit method is then equivalent       to calling the other
       * reinit method with an index set containing both       the locally
       * owned entries and the ghost entries.             If the flag   @p
       * vector_writable   is set to true, this creates an       alternative
       * storage scheme for ghost elements that allows multiple       threads
       * to write into the vector (for the other reinit methods, only
      ...
      /**
       * Return if this Vector contains ghost elements.               @see           @ref GlossGhostedVector   "vectors with ghost elements"
       *
       */
      bool
...
#  endif
     ...
     * exchanges the data of the two vectors.           @relatesalso
...
/**
 * Declare   dealii::TrilinosWrappers::MPI::BlockVector   as distributed
 * vector.
 *
 *
 */
