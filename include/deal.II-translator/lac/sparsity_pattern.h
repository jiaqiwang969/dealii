...
namespace SparsityPatternIterators
{
   ...
   * matrices.     Note that this class only allows read access to elements,
   * providing their   row and column number (or alternatively the index
   * within the complete   sparsity pattern). It does not allow modifying the
   ...
   * the   SparsityPatternBase class. As a consequence, some operations are
   ...
   * is expensive   to access the row index (this requires   $O(\log(N))$
   * operations for a   matrix with   $N$   row). As a consequence, when you
   * design algorithms that   use these iterators, it is common practice to
   * not loop over <i>all</i>   elements of a sparsity pattern at once, but to
   * have an outer loop over   all rows and within this loop iterate over the
   * elements of this row. This   way, you only ever need to dereference the
   * iterator to obtain the column   indices whereas the (expensive) lookup of
   * the row index can be avoided by   using the loop index instead.
   *
   */
...
/**
 * A class that can store which elements of a matrix are nonzero (or, in fact, <i>may</i> be nonzero) and for which we have to allocate memory to store their values. This class is an example of the "static"
 * type of sparsity patters (see   @ref Sparsity  ). It uses the <a
 * href="https://en.wikipedia.org/wiki/Sparse_matrix">compressed row storage
 * (CSR)</a> format to store data, and is used as the basis for the derived
 * SparsityPattern class and SparseMatrix class. The elements of a
 * SparsityPatternBase, corresponding to the places where SparseMatrix objects
 * can store nonzero entries, are stored row-by-row. The ordering of non-zero
 * elements within each row (i.e. increasing column index order) depends on
 * the derived classes.
 *
 *
 */
...
public:
  ...
  /**
   * Typedef an iterator class that allows to walk over all nonzero elements
   * of a sparsity pattern.     Since the iterator does not allow to modify
   * the sparsity pattern, this   type is the same as that for   @p
   * const_iterator.
   *
   */
   ...
   * index.     Indices with this invalid value are used to insert new entries
   * to the   sparsity pattern using the add() member function, and are
   * removed when   calling compress().     You should not assume that the
   * variable declared here has a certain   value. The initialization is given
   * here only to enable the compiler to   perform some optimizations, but the
   * actual value of the variable may   change over time.
   *
   */
  static const size_type invalid_entry = numbers::invalid_size_type;

  /**
   * @name   Construction and Initialization     Constructors, destructor, functions initializing, copying and filling an   object.
   *
   */
  // @{
  /**
   ...
   * rows and   @p n   columns, with at most   @p max_per_row     nonzero
   * entries per row.     This function simply maps its operations to the
  ...
  /**
   * Reallocate memory for a matrix of size   @p m   times   @p n.   The
   * number of   entries for each row is taken from the array   @p row_lengths
   * which   has to give this number of each row   $i=1\ldots m$  .     If
   * <tt>m*n==0</tt> all memory is freed, resulting in a total
  ...
  /**
   * Same as above, but with an ArrayView argument instead.     The derived
   * classes are responsible for implementation of this function.
   *
   */
  virtual void
  reinit(const size_type                      m,
   ...
   * pattern does not   represent a quadratic matrix.
   *
   */
  void
  symmetrize();
   ...
   * non-compressed sparsity patterns.     If the entry already exists,
   * nothing bad happens.
   *
   */
  void
   ...
   * pattern.     The order in which elements are accessed depends on the
   * storage scheme   implemented by derived classes.
   *
   */
  iterator
  begin() const;
   ...
   * also that the iterator may not be dereferenceable in   that case.     The
   * order in which elements are accessed depends on the storage scheme
   ...
   * $(i,j)$     represents a nonzero entry of the matrix. Consequently, the
   * maximum   bandwidth a   $n\times m$   matrix can have is
   * $\max\{n-1,m-1\}$  , a diagonal   matrix has bandwidth 0, and there are
   * at most   $2*q+1$   entries per row if   the bandwidth is   $q$  . The
   * returned quantity is sometimes called "half   bandwidth" in the
   * literature.
   *
   */
  size_type
   ...
   * entries should happen to be zero, it is counted anyway.     This function
   * may only be called if the matrix struct is compressed. It   does not make
   * too much sense otherwise anyway.
   *
   */
  std::size_t
   ...
   * i.e. <tt>column_number(row,0)==row</tt>.     If the sparsity pattern is
   * already compressed, then (except for the   diagonal element), the entries
   * are sorted by columns, i.e.   <tt>column_number(row,i)</tt> <tt><</tt>
  ...
  /**
   * The index of a global matrix entry in its row.     This function is
   * analogous to operator(), but it computes the index not   with respect to
   * the total field, but only with respect to the row   <tt>j</tt>.
   *
   */
  size_type
   ...
   * value is the pair composed of row and column index.     This function may
   * only be called if the sparsity pattern is closed. The   global index must
   * then be between zero and n_nonzero_elements().     If <tt>N</tt> is the
   * number of rows of this matrix, then the complexity   of this function is
   * <i>log(N)</i>.
   *
   */
...
/**
 * This class stores a sparsity pattern in the <a
 * href="https://en.wikipedia.org/wiki/Sparse_matrix">compressed row storage
 * (CSR)</a> format to store data, and is used as the basis for the
 * SparseMatrix class. The elements of a SparsityPattern, corresponding to the
 * places where SparseMatrix objects can store nonzero entries, are stored
 * row-by-row. Within each row, elements are generally stored left-to-right in
 * increasing column index order; the exception to this rule is that if the
 * matrix is square (n_rows() == n_columns()), then the diagonal entry is
 ...
 * object (using   SparsityPattern::begin   and   SparsityPattern::end)   you
 * will find that the elements are not sorted by column index within each row
 * whenever the matrix is square (the first item will be the diagonal,
 * followed by the other entries sorted by column index).
 * While this
 * class forms the basis upon which SparseMatrix objects base their storage
 * format, and thus plays a central role in setting up linear systems, it is
 * rarely set up directly due to the way it stores its information. Rather,
 * one typically goes through an intermediate format first, see for example
 * the   step-2   tutorial program as well as the documentation module   @ref
 * Sparsity  . You can iterate over entries in the pattern using begin(),
 * end(), begin(row), and end(row). These functions return an iterator of type
 * SparsityPatternIterators::Iterator.   When dereferencing an iterator   @p
 * it,   you have access to the member functions in
...
public:
  ...
  /**
   * @name   Construction and setup     Constructors, destructor, functions initializing, copying and filling an   object.
   *
   */
  // @{
  /**
   ...
   * <tt>v</tt> a   std::vector   of SparsityPattern objects.     Usually, it
   * is sufficient to use the explicit keyword to disallow   unwanted
   * temporaries, but this does not work for   <tt>std::vector</tt>s.
  ...
  /**
   * Initialize a rectangular pattern of size <tt>m x n</tt>.       @param[in]
   * m The number of rows.     @param[in]   n The number of columns.
   ...
   * <tt>max_per_row</tt> nonzero entries per row.     This constructor
   * automatically enables optimized storage of diagonal   elements. To avoid
   * this, use the constructor taking row and column   numbers separately.
   *
   */
  SparsityPattern(const size_type m, const unsigned int max_per_row);

  /**
   * Initialize a quadratic pattern of size <tt>m x m</tt>.       @param[in]
   * m The number of rows and columns.     @param[in]   row_lengths Maximum
  ...
  /**
   * Make a copy with extra off-diagonals.     This constructs objects
   * intended for the application of the ILU(n)-method   or other incomplete
   * decompositions.  Therefore, additional to the   original entry structure,
   * space for <tt>extra_off_diagonals</tt> side-   diagonals is provided on
   * both sides of the main diagonal.     <tt>max_per_row</tt> is the maximum
   * number of nonzero elements per row   which this structure is to hold. It
   * is assumed that this number is   sufficiently large to accommodate both
   * the elements in <tt>original</tt>   as well as the new off-diagonal
   * elements created by this constructor. You   will usually want to give the
   * same number as you gave for   <tt>original</tt> plus the number of side
   * diagonals times two. You may   however give a larger value if you wish to
   * add further nonzero entries   for the decomposition based on other
   * criteria than their being on side-   diagonals.     This function
   * requires that <tt>original</tt> refers to a quadratic   matrix structure.
   * It must be compressed. The matrix structure is not   compressed after
  ...
  /**
   * Reallocate memory for a matrix of size   @p m   times   @p n.   The
   * number of   entries for each row is taken from the ArrayView   @p
   * row_lengths   which   has to give this number of each row   $i=0\ldots
   * m-1$  .
   *
   */
  virtual void
   ...
   * quadratic matrices, which is always the first entry of each row.     The
   * memory which is no more needed is released.     SparseMatrix objects
   * require the SparsityPattern objects they are   initialized with to be
   * compressed, to reduce memory requirements.
   *
   */
  void
  compress();
   ...
   * entries that will form the matrix sparsity pattern.     The first two
   * parameters determine the size of the matrix. For the two   last ones,
   * note that a sparse matrix can be described by a sequence of   rows, each
   * of which is represented by a sequence of pairs of column   indices and
   * values. In the present context, the begin() and end()   parameters
   * designate iterators (of forward iterator type) into a   container, one
   * representing one row. The distance between begin() and   end() should
   * therefore be equal to n_rows(). These iterators may be   iterators of
   * <tt>std::vector</tt>,     <tt>std::list</tt>,   pointers into a   C-style
   * array, or any other iterator satisfying the requirements of a   forward
   * iterator. The objects pointed to by these iterators (i.e. what we   get
   * after applying <tt>operator*</tt> or <tt>operator-></tt> to one of
   ...
   * magic.     While the order of the outer iterators denotes the different
   * rows of the   matrix, the order of the inner iterator denoting the
   * columns does not   matter, as they are sorted internal to this function
   * anyway.     Since that all sounds very complicated, consider the
   * following example   code, which may be used to fill a sparsity pattern:
   * @code
   ...
   * each of the two   <tt>std::vector</tt>   occurrences by
   * <tt>std::list</tt>,     and the inner one by   <tt>std::set</tt>   as
   * well.     Another example would be as follows, where we initialize a
   * whole matrix,   not only a sparsity pattern:
   * @code
   ...
   * take as column index. As previously, the outer   <tt>std::vector</tt>
   * could   be replaced by   <tt>std::list</tt>,   and the inner
   * <tt>std::map<unsigned     int,double></tt> could be replaced by
   * <tt>std::vector<std::pair<unsigned     int,double> ></tt>, or a list or
   * set of such pairs, as they all return   iterators that point to such
   * pairs.
   *
   */
   ...
   * matrix entry pattern for this object.     Previous content of this object
   * is lost, and the sparsity pattern is in   compressed mode afterwards.
   * Once you have built a sparsity pattern with this function, you   probably
   * want to attach a SparseMatrix object to it. The original   `matrix`
   * object can then be copied into this SparseMatrix object   using the
   * version of   SparseMatrix::copy_from()   that takes a   FullMatrix object
   * as argument. Through this procedure, you can   convert a FullMatrix into
   * a SparseMatrix.
   *
   */
  template <typename number>
   ...
   * may only be called for non-compressed sparsity patterns.     If some of
   * the entries already exist, nothing bad happens.
   *
   */
  template <typename ForwardIterator>
  void
   ...
   * explicitly or not.     This function mainly serves the purpose of
   * describing the current class   in cases where several kinds of sparsity
   * patterns can be passed as   template arguments.
   *
   */
  bool
   ...
   * return   SparsityPattern::invalid_entry.       This function is usually
   * called by the   SparseMatrix::operator()().   It may   only be called for
   * compressed sparsity patterns, since in this case   searching whether the
   * entry exists can be done quite fast with a binary   sort algorithm
   * because the column numbers are sorted.     If <tt>m</tt> is the number of
   * entries in <tt>row</tt>, then the   complexity of this function is
   * <i>log(m)</i> if the sparsity pattern is   compressed.
   ...
   * computers using a different operating system or number format.     The
   ...
   * previous contents are   lost.     A primitive form of error checking is
   * performed which will recognize the   bluntest attempts to interpret some
   * data as a vector stored bitwise to a   file, but not more.
   *
   */
  void
