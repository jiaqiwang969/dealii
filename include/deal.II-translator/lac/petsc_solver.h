...
#    endif
   ...
   * set certain parameters for individual solvers.     Optionally, the user
   * can create a solver derived from the SolverBase   class and can set the
   * default arguments necessary to solve the linear   system of equations
   * with SolverControl. These default options can be   overridden by
   * specifying command line arguments of the form   @p
   *
   *  - sp_*.   For example,   @p
   *
   *  - sp_monitor_true_residual prints out true residual norm   (unpreconditioned) at each iteration and   @p
   *
   *  - sp_view provides   information about the linear solver and the preconditioner used in the   current context. The type of the solver can also be changed during   runtime by specifying   @p
   *
   *  - sp_type{richardson, cg, gmres, fgmres, ..} to   dynamically test the optimal solver along with a suitable preconditioner   set using   @p
   *
   *  - c_type{jacobi, bjacobi, ilu, lu, ..}. There are several   other command line options available to modify the behavior of the PETSc   linear solver and can be obtained from the <a
   * href="http://www.mcs.anl.gov/petsc">documentation and manual pages</a>.
   * @note   Repeated calls to solve() on a solver object with a
   * Preconditioner   must be used with care. The preconditioner is
   * initialized in the first   call to solve() and subsequent calls reuse the
   * solver and preconditioner   object. This is done for performance reasons.
   * The solver and   preconditioner can be reset by calling reset().     One
   * of the gotchas of PETSc is that
   *
   ...
   * that the communicator   @em   must match that of the matrix and all
   * vectors   in the linear system which we want to solve. Aggravating the
   * situation is   the fact that the default argument to the solver classes,
   * @p     PETSC_COMM_SELF, is the appropriate argument for the sequential
   * case   (which is why it is the default argument), so this error only
   * shows up in   parallel mode.
  ...
  public:
     ...
     * over which parallel computations are to happen.         Note that the
     * communicator used here must match the communicator used     in the
     * system matrix, solution, and right hand side object of the solve     to
     * be done with this solver. Otherwise, PETSc will generate hard to
  ...
  private:
     ...
     * expensive, such as in the case of an ILU for example.         The
     * actual declaration of this class is complicated by the fact that
     * PETSc changed its solver interface completely and incompatibly between
     * versions 2.1.6 and 2.2.0 :-(         Objects of this type are
     * explicitly created, but are destroyed when the     surrounding solver
     * object goes out of scope, or when we assign a new     value to the
     * pointer to this object. The respectiveDestroy functions     are
     * therefore written into the destructor of this object, even though
...
#    endif
  ...
  public:
     ...
     * default,   @p PETSC_COMM_SELF   is used here, but you can change this.
     * Note     that for single processor (non-MPI) versions, this parameter
     * does not     have any effect.         The last argument takes a
     * structure with additional, solver dependent     flags for tuning.
   ...
   * object have no particular meaning here.     MUMPS allows to make use of
   * symmetry in this matrix. In this class this   is made possible by the
   * set_symmetric_mode() function. If your matrix is   symmetric, you can use
