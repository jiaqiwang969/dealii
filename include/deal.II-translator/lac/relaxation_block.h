 ...
 * relaxation methods and smoothers. This class uses the infrastructure
 * provided by PreconditionBlockBase. It adds functions to initialize with a
 * block list and to do the relaxation step. The actual relaxation method with
 * the interface expected by SolverRelaxation and MGSmootherRelaxation is in
 * the derived classes. This class allows for more general relaxation methods
 * than PreconditionBlock, since the index sets may be arbitrary and
 * overlapping, while there only contiguous, disjoint sets of equal size are
 * allowed. As a drawback, this class cannot be used as a preconditioner,
 * since its implementation relies on a straight forward implementation of the
 * Gauss- Seidel process. Parallel computations require you to specify an
 * initialized ghost vector in   AdditionalData::temp_ghost_vector.
 *
 *
...
public:
  ...
  public:
     ...
     * setting this true if your blocks are not equal, in     particular if
     * their sizes differ.
     *
     */
    bool same_diagonal;
     ...
     * Setting this parameter to a value greater than zero takes precedence
     * over     threshold, i.e. kernel_size must be zero if you want to use
     * threshold.     This parameter is used in the call to
     ...
     * kernel_size must be zero if you want to use threshold.     This
    ...
    /**
     * The order in which blocks should be traversed. This vector can initiate     several modes of execution:           <ol>             <li>  If the length of the vector is zero, then the relaxation method     will be executed from first to last block.  </li>             <li>   If the length is one, then the inner vector must have the same     size as the number of blocks. The relaxation method is applied in the     order given in this vector.  </li>             <li>   If the outer vector has length greater one, then the relaxation     method is applied several times, each time in the order given by the     inner vector of the corresponding index. This mode can for instance be     used for ADI methods and similar direction sweeps.  </li>             </ol>
     *
     */
     ...
     * point to an initialized vector that contains all indices     that
     * appear in the   @p block_list   sa ghost values. Typically, this the
   ...
   * inverses of the diagonal blocks may be computed.     Note that
   * AdditionalData, different from other preconditioners, defines   quite
   * large objects, and that therefore the object is not copied, but   rather
   * a pointer is stored. Thus, the lifetime of
...
protected:
  /**
   * Perform one block relaxation step.     Depending on the arguments   @p
   * dst   and   @p pref,   this performs an SOR step   (both reference the
   * same vector) or a Jacobi step (both are different   vectors). For the
   * Jacobi step, the calling function must copy   @p dst   to     @p prev
   * after this.
   *
   */
  void
 ...
 * This class implements the step() and Tstep() functions expected by the   @ref ConceptRelaxationType   "relaxation concept".
