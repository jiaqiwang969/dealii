...
namespace GinkgoWrappers
{
   ...
   * The various derived classes only take   the additional data that is
   * specific to them and solve the given linear   system. The entire
   * collection of solvers that Ginkgo implements is   available at <a Ginkgo
   * href="https://ginkgo-project.github.io/ginkgo/doc/develop/">
   * documentation and manual pages</a>.
  ...
  public:
    /**
     * Constructor.         The   @p exec_type   defines the paradigm where
     * the solution is computed.     It is a string and the choices are "omp"
     * , "reference" or "cuda".     The respective strings create the
     * respective executors as given below.         Ginkgo currently supports
     * three different executor types:         +    OmpExecutor specifies that
     * the data should be stored and the     associated operations executed on
     * an OpenMP-supporting device (e.g. host     CPU);     ```     auto omp =
     * gko::create<gko::OmpExecutor>();       ```     +    CudaExecutor
     * specifies that the data should be stored and the          operations
     * executed on the NVIDIA GPU accelerator;     ```
     ...
     * which can be used to debug the library.     ```     auto ref =
     * gko::create<gko::ReferenceExecutor>();       ```         The following
     * code snippet demonstrates the using of the OpenMP executor     to
     * create a solver which would use the OpenMP paradigm to the solve the
     * system on the CPU.         ```     auto omp =
     * gko::create<gko::OmpExecutor>();       using cg =   gko::solver::Cg<>;
     * auto solver_gen =           cg::build()                .with_criteria(
     * gko::stop::Iteration::build().with_max_iters(20u).on(omp),
     * gko::stop::ResidualNormReduction<>::build()
     * .with_reduction_factor(1e-6)                      .on(omp))
     * .on(omp);     auto solver = solver_gen->generate(system_matrix);
     * solver->apply(lend(rhs), lend(solution));     ```             The   @p
     * solver_control   object is the same as for other     deal.II iterative
     * solvers.
     *
     */
    ...
    /**
     * Initialize the matrix and copy over its data to Ginkgo's data
     * structures.
     *
     */
    void
  ...
  protected:
    ...
    /**
     * The residual criterion object that controls the reduction of the
     * residual     based on the tolerance set in the solver_control member.
     *
     */
    ...
    /**
     * The Ginkgo combined factory object is used to create a combined
     * stopping     criterion to be passed to the solver.
     *
     */
     ...
     * `gko::OmpExecutor`,     `gko::CudaExecutor`   and
     * `gko::ReferenceExecutor`       and more details can be found in
  ...
  private:
    /**
     * Initialize the Ginkgo logger object with event masks. Refer to     <a
    ...
    /**
     * Ginkgo matrix data structure. First template parameter is for storing
     * the     array of the non-zeros of the matrix. The second is for the row
     * pointers     and the column indices.           @todo   Templatize based
     * on Matrix type.
     *
     */
  ...
  public:
    ...
    /**
     * Constructor.           @param[in,out]   solver_control The solver
     * control object is then used to     set the parameters and setup the CG
     * solver from the CG factory which     solves the linear system.
  ...
  /**
   * An implementation of the solver interface using the Ginkgo Bicgstab
   * solver.
   * @ingroup GinkgoWrappers
   *
   */
  ...
  public:
    ...
    /**
     * Constructor.           @param[in,out]   solver_control The solver
     * control object is then used to     set the parameters and setup the
     * Bicgstab solver from the Bicgstab     factory which solves the linear
     * system.           @param[in]   exec_type The execution paradigm for the
     * Bicgstab solver.           @param[in]   data The additional data
     ...
     * Bicgstab solver from the Bicgstab     factory which solves the linear
     * system.           @param[in]   exec_type The execution paradigm for the
     * Bicgstab solver.           @param[in]   preconditioner The
     * preconditioner for the solver.           @param[in]   data The
     * additional data required by the solver.
     *
     */
    SolverBicgstab(SolverControl &                           solver_control,
                   const std::string &                       exec_type,
  ...
  public:
    ...
    /**
     * Constructor.           @param[in,out]   solver_control The solver
     * control object is then used to     set the parameters and setup the CGS
     * solver from the CGS factory which     solves the linear system.
   ...
   * subspace method which is suitable for symmetric positive definite
   * methods.     Though this method performs very well for symmetric positive
   * definite   matrices, it is in general not suitable for general matrices.
   ...
   * vectors spanning the Krylov subspace. This increases the computational
   * cost   of every Krylov solver iteration but allows for non-constant
   * preconditioners.
   * @ingroup GinkgoWrappers
   *
   */
  ...
  public:
    ...
    /**
     * Constructor.           @param[in,out]   solver_control The solver
     * control object is then used to     set the parameters and setup the FCG
     * solver from the FCG factory which     solves the linear system.
  ...
  public:
     ...
     * Constructor.           @param[in,out]   solver_control The solver
     * control object is then used to     set the parameters and setup the
     * GMRES solver from the GMRES factory     which solves the linear system.
  ...
  public:
    ...
    /**
     * Constructor.           @param[in,out]   solver_control The solver
     * control object is then used to     set the parameters and setup the IR
     * solver from the IR factory which     solves the linear system.
