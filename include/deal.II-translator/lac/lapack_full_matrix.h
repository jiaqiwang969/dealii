...
public:
   ...
   * @p size.       In order to avoid the implicit conversion of integers and
   * other types to   a matrix, this constructor is declared
   * <tt>explicit</tt>.     By default, no memory is allocated.
   *
   */
  ...
  /**
   * Set a particular entry of the matrix to a   @p value.     Thus, calling
   * <code>A.set(1,2,3.141);</code>   is entirely equivalent to the
   * operation   <code>A(1,2) = 3.141;</code>  . This function exists for
   * compatibility with the various sparse matrix objects.       @param   i
   * The row index of the element to be set.     @param   j The column index
   * of the element to be set.     @param   value The value to be written into
   * the element.
   *
   */
  void
  ...
  /**
   * Perform a rank-1 update of a symmetric matrix     $ \mathbf A \leftarrow
   * \mathbf A + a \, \mathbf v \mathbf v^T $  .     This function also works
   * for Cholesky factorization.   In that case, updating (  $a>0$  ) is
   * performed via Givens rotations, whereas downdating (  $a<0$  ) via
   * hyperbolic   rotations. Note that the latter case might lead to a
   * negative definite   matrix in which case the error will be thrown
   * (because Cholesky   factorizations are only valid for symmetric and
   ...
   * rotation</a>     @p csr   (a triplet of cosine, sine and radius, see
   * Utilities::LinearAlgebra::givens_rotation()   for the definition of the
   * rotation matrix   $\mathbf G$  )   to this matrix in the plane spanned by
   * the   @p i'th   and   @p k'th   unit vectors.   If   @p left   is
   * <code>true</code>  , the rotation is applied from left     $\mathbf A
   * \leftarrow \mathbf G \mathbf A$     and only rows   @p i   and   @p k
   * are affected.   Otherwise, transpose of the rotation matrix is applied
   * from right     $\mathbf A \leftarrow \mathbf A \mathbf G^T$     and only
   * columns   @p i   and   @p k   are affected.
   *
   */
  void
  ...
  /**
   * Same as above but will preserve the values of matrix upon resizing.   The
   * original values   of the matrix are kept on increasing the size   \f[
   * \mathbf A \rightarrow \left( \begin{array}{cc} \mathbf A & \mathbf 0 \\
   * \mathbf 0 & \mathbf 0 \end{array} \right) \f]   Whereas if the new size
   * is smaller, the matrix will contain the upper left   block of the
   * original one \f[ \left( \begin{array}{cc} \mathbf A_{11} & \mathbf A_{12}
   * \\ \mathbf A_{21} & \mathbf A_{22} \end{array} \right) \rightarrow
   * \mathbf A_{11} \f]
   *
   */
  void
  ...
  /**
   * Remove row   @p row   and column   @p col   from the matrix.   \f[ \left(
   ...
   * <tt>src</tt> is copied into   <tt>this</tt>. The upper left corner of the
   * block being copied is   <tt>(src_offset_i,src_offset_j)</tt>.  The upper
   * left corner of the   copied block is
   * <tt>(dst_offset_i,dst_offset_j)</tt>.  The size of the   rectangular
   * block being copied is the maximum size possible, determined   either by
   * the size of <tt>this</tt> or <tt>src</tt>.     The final two arguments
   * allow to enter a multiple of the source or its   transpose.
   *
   */
  template <typename MatrixType>
  void
  ...
  /**
   * Matrix-vector-multiplication.     Depending on previous transformations recorded in #state, the result of   this function is one of     <ul>       <li>   If #state is   LAPACKSupport::matrix   or   LAPACKSupport::inverse_matrix,     this will be a regular matrix vector product using LAPACK gemv().     <li>   If #state is   LAPACKSupport::svd   or   LAPACKSupport::inverse_svd,   this   function first multiplies with the right transformation matrix, then with   the diagonal matrix of singular values or their reciprocal values, and   finally with the left transformation matrix.     </ul>       The optional parameter   @p adding   determines, whether the result is   stored in the vector     $\mathbf w = \mathbf A \cdot \mathbf v$     or added to it     $\mathbf w \mathrel{+}= \mathbf A \cdot \mathbf v$  .
   * @note   Source and destination must not be the same vector.
   * @note   The template with   @p number2   only exists for compile-time
   * compatibility with FullMatrix. Only the case   @p number2   =   @p number
   * is   implemented due to limitations in the underlying LAPACK interface.
   * All   other variants throw an error upon invocation.
   *
   */
  template <typename number2>
  void
  ...
  /**
   * Adding Matrix-vector-multiplication   $\mathbf w \mathrel{+}= \mathbf A
   * \cdot \mathbf v$  .     See the documentation of vmult() for details on
   * the implementation.
   *
   */
  template <typename number2>
  ...
  /**
   * Transpose matrix-vector-multiplication.     The optional parameter   @p
   * adding   determines, whether the result is   stored in the vector
   * $\mathbf w = \mathbf A^T \cdot \mathbf v$     or added to it     $\mathbf
   * w \mathrel{+}= \mathbf A^T \cdot \mathbf v$  .     See the documentation
   * of vmult() for details on the implementation.
   *
   */
  template <typename number2>
  void
   ...
   * \mathbf A^T \cdot \mathbf v$  .     See the documentation of vmult() for
   ...
   * determines, whether the result is   stored in the matrix     $\mathbf C
   * = \mathbf A \cdot \mathbf B$     or added to it     $\mathbf C
   * \mathrel{+}= \mathbf A \cdot \mathbf B$  .
   * @note   It is assumed that   @p A   and   @p B   have compatible sizes
   * and that     @p C   already has the right size.       @p This   function
  ...
  /**
   * Matrix-matrix-multiplication using transpose of <tt>this</tt>.     The
   * optional parameter   @p adding   determines, whether the result is
   * stored in the matrix     $\mathbf C = \mathbf A^T \cdot \mathbf B$     or
   * added to it     $\mathbf C \mathrel{+}= \mathbf A^T \cdot \mathbf B$  .
   * @note   It is assumed that   @p A   and   @p B   have compatible sizes
   ...
   * diagonal vector   @p V.       If the   <code>adding=false</code>   then
   * the result is stored in the matrix     $\mathbf C = \mathbf A^T \cdot
   * \rm{diag}(\mathbf V) \cdot \mathbf B$     otherwise it is added
   * $\mathbf C \mathrel{+}= \mathbf A^T \cdot \rm{diag}(\mathbf V) \cdot
   * \mathbf B$  .
   * @note   It is assumed that   @p A,     @p B   and   @p V   have
   * compatible sizes and that     @p C   already has the right size.
  ...
  /**
   * Matrix-matrix-multiplication using transpose of   @p B.       The
   * optional parameter   @p adding   determines, whether the result is
   * stored in the matrix     $\mathbf C = \mathbf A \cdot \mathbf B^T$     or
   * added to it     $\mathbf C \mathrel{+}= \mathbf A \cdot \mathbf B^T$  .
   * @note   It is assumed that   @p A   and   @p B   have compatible sizes
   * and that     @p C   already has the right size.
   * @note   This function uses the BLAS function Xgemm.
   *
  ...
  /**
   * Matrix-matrix-multiplication using transpose of <tt>this</tt> and     @p
   * B.       The optional parameter   @p adding   determines, whether the
   * result is   stored in the matrix     $\mathbf C = \mathbf A^T \cdot
   * \mathbf B^T$     or added to it     $\mathbf C \mathrel{+}= \mathbf A^T
   * \cdot \mathbf B^T$  .
   * @note   It is assumed that   @p A   and   @p B   have compatible sizes
   * and that     @p C   already has the right size.
   * @note   This function uses the BLAS function Xgemm.
  ...
  /**
   * Performs out-place transposition.   Matrix   @p B   should be
   * appropriately sized.
   * @note   for complex number types, conjugate transpose will be performed.
   * @note   If deal.II is configured with Intel-MKL, `mkl_?omatcopy` will be
   * used,   otherwise transposition is done element by element.
   *
   */
  void
  ...
  /**
   * Scale rows of this matrix by   @p V   . This is equivalent to
   * premultiplication   with a diagonal matrix   $\mathbf A\leftarrow {\rm
   * diag}(\mathbf V)\mathbf A$  .
   *
   */
  void
  ...
  /**
   * Estimate the reciprocal of the condition number   $1/k(\mathbf A)$   in
   * $L_1$     norm (  $1/(||\mathbf A||_1 \, ||\mathbf A^{-1}||_1)$  ) of a
   * symmetric   positive definite matrix using Cholesky factorization. This
   * function can   only be called if the matrix is already factorized.
   * @note   The condition number   $k(\mathbf A)$   can be used to estimate
   * the numerical   error related to the matrix inversion or the solution of
   * the   system of linear algebraic equations as     <code>error =
   * std::numeric_limits<Number>::epsilon k</code>  .   Alternatively one can
   ...
   * @param[in]   l1_norm Is the   $L_1$   norm of the matrix before calling
   * Cholesky   factorization. It can be obtained by calling l1_norm().
   *
   */
  number
  ...
  /**
   * Estimate the reciprocal of the condition number   $1/k(\mathbf A)$   in
   * $L_1$     norm for triangular matrices. The matrix has to have the
  ...
  /**
   * Compute the determinant of a matrix. As it requires the LU factorization
   * of   the matrix, this function can only be called after
  ...
  /**
   * Invert the matrix by first computing an LU/Cholesky factorization with
   * the   LAPACK function Xgetrf/Xpotrf and then building the actual inverse
   * using   Xgetri/Xpotri.
   *
   */
  void
  ...
  /**
   * Solve the linear system with right hand side   @p v   and put the
   * solution   back to   @p v.   The matrix should be either triangular or
   * LU/Cholesky   factorization should be previously computed.     The flag
   * transposed indicates whether the solution of the transposed   system is
   * to be performed.
   *
   */
  void
  ...
  /**
   * Same as above but for multiple right hand sides (as many as there   are
   * columns in the matrix   @p B).
   *
   */
  void
   ...
   * itself will be   LAPACKSupport::unusable   after this operation.     The
   * optional arguments allow to compute left and right eigenvectors as
   * well.     Note that the function does not return the computed eigenvalues
   * right   away since that involves copying data around between the output
   * arrays of   the LAPACK functions and any return array. This is often
   * unnecessary   since one may not be interested in all eigenvalues at once,
   * but for   example only the extreme ones. In that case, it is cheaper to
   * just have   this function compute the eigenvalues and have a separate
   ...
   * eigenvalues in the interval   $(\rm{lower\_bound}, \rm{upper\_bound}]$
   * are   computed with the absolute tolerance   $\rm abs\_accuracy$  . An
   * approximate   eigenvalue is accepted as converged when it is determined
   * to lie in an   interval   $[a,b]$   of width less than or equal to
   * $\rm{abs\_accuracy} + eps \rm{max}(|a|,|b|)$  , where   $eps$   is the
   * machine precision.  If     $\rm{abs\_accuracy}$   is less than or equal
   * to zero, then     $eps\,|\mathbf{T}|_1$   will be used in its place,
   * where   $|\mathbf{T}|_1$   is   the 1-norm of the tridiagonal matrix
   * obtained by reducing   $\mathbf A$   to   tridiagonal form. Eigenvalues
   * will be computed most accurately when     $\rm{abs\_accuracy}$   is set
   * to twice the underflow threshold, not zero.   After this routine has been
   * called, all eigenvalues in   $(\rm{lower\_bound}, \rm{upper\_bound}]$
   * will be stored in eigenvalues and the corresponding   eigenvectors will
   * be stored in the columns of eigenvectors, whose dimension   is set
   * accordingly.
   ...
   *
   *
   *  - itype = 3:   $\mathbf B \cdot \mathbf A \cdot \mathbf x=\lambda \mathbf x$       where   $\mathbf A$   is this matrix.   $\mathbf A$     and   $\mathbf B$   are assumed to be symmetric, and   $\mathbf B$   has to be   positive definite. Only eigenvalues in the interval   $(\rm{lower\_bound},
   * \rm{upper\_bound}]$   are computed with the absolute tolerance
   * $\rm{abs\_accuracy}$  .  An approximate eigenvalue is accepted as
   * converged   when it is determined to lie in an interval   $[a,b]$   of
   * width less than or   equal to   $\rm{abs\_accuracy} + eps \rm{max}(
   * |a|,|b| )$  , where   $eps$   is   the machine precision. If
   * $\rm{abs\_accuracy}$   is less than or equal to   zero, then   $eps \,
   * |\mathbf{T}|_1$   will be used in its place, where     $|\mathbf{T}|_1$
   * is the 1-norm of the tridiagonal matrix obtained by   reducing   $\mathbf
   * A$   to tridiagonal form. Eigenvalues will be computed most   accurately
   * when   $\rm{abs\_accuracy}$   is set to twice the underflow   threshold,
   * not zero. After this routine has been called, all eigenvalues in
   * $(\rm{lower\_bound}, \rm{upper\_bound}]$   will be stored in eigenvalues
   * and   the corresponding eigenvectors will be stored in eigenvectors,
   * whose   dimension is set accordingly.
   ...
   * function Xgesdd.     Requires that the #state is   LAPACKSupport::matrix,
   * fills the data members   #wr, #svd_u, and #svd_vt, and leaves the object
   * in the #state     LAPACKSupport::svd.       The singular value
   * decomposition factorizes the provided matrix (A) into   three parts: U,
   * sigma, and the transpose of V (V^T), such that A = U sigma   V^T. Sigma
   * is a MxN matrix which contains the singular values of A on   the diagonal
   * while all the other elements are zero. U is a MxM orthogonal   matrix
   * containing the left singular vectors corresponding to the singular
   * values of A. V is a NxN orthonal matrix containing the right singular
   * vectors corresponding the singular values of A.     Note that the
   * variable #svd_vt contains the tranpose of V and can be   accessed by
   * get_svd_vt(), while U is accessed with get_svd_u().
   *
   */
  void
  compute_svd();
   ...
   * LAPACKSupport::inverse_svd.       For a singular value decomposition, the
   * inverse is simply computed by   replacing all singular values by their
   * reciprocal values. If the matrix   does not have maximal rank, singular
   * values 0 are not touched, thus   computing the minimal norm right inverse
   * of the matrix.     The parameter   @p threshold   determines, when a
   * singular value should   be considered zero. It is the ratio of the
   * smallest to the largest   nonzero singular value   $s_{max}$  . Thus, the
   * inverses of all   singular values less than    $s_{max}/\rm{threshold}$
   * will   be set to zero.
   *
   */
  void
  ...
  /**
   * Retrieve the matrix #svd_u after compute_svd() or compute_inverse_svd()
   * was   called.
   *
   */
  ...
  /**
   * Print the matrix and allow formatting of entries.     The parameters
   * allow for a flexible setting of the output format:       @param   out
   * This specifies the stream to write to.       @param   precision denotes
   * the number of trailing digits.       @param   scientific is used to
   * determine the number format, where     <code>scientific = false</code>
   * means fixed point notation.       @param   width denotes the with of each
   * column. A zero entry for     @p width   makes the function compute a
   ...
   * denominator to get nicer numbers.       @param   threshold all entries
   * with absolute value smaller than   this are considered zero.
   * @note   The entries stored resemble a matrix only if the state is either
   * LAPACKSupport::matrix   or   LAPACK::inverse_matrix.   Otherwise, calling
...
private:
   ...
   * factorization.     Also used as the scratch array IWORK for LAPACK
