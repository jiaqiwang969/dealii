...
/**
 * A class
 * that represents a vector of numerical elements. As for the other classes,
 * in the   @ref Vectors   group, this class has a substantial number of
 * member functions. These include:
 *
 *
 *
 ...
 * intends to implement not simply an array that allows access to its
 * elements, but indeed a vector that is a member of the mathematical concept
...
public:
   ...
   * copies all elements.     We would like to make this constructor explicit,
   * but standard containers   insist on using it implicitly.
  ...
  /**
   * Copy constructor taking a vector of another data type.     This
   * constructor will fail to compile if   there is no conversion path from
   * @p OtherNumber   to   @p Number.   You may   lose accuracy when copying
   * to a vector with data elements with   less accuracy.
   *
   */
   ...
   * This   constructor can be used to initialize a vector using a
   * brace-enclosed   list of numbers, such as in the following example:
   * @code
   * Vector<double> v({1,2,3});
   * @endcode
   * This creates a vector of size 3, whose (double precision) elements have
   * values 1.0, 2.0, and 3.0.     This constructor will fail to compile if
   * there is no conversion path from   @p OtherNumber   to   @p Number.   You
   * may   lose accuracy when copying to a vector with data elements with
   * less accuracy.
   *
   */
  template <typename OtherNumber>
   ...
   * configuration time.     Note that due to the communication model used in
   * MPI, this operation can   only succeed if all processes do it at the same
   * time when   <code>v</code>     is a distributed vector: It is not
   * possible for only one process to   obtain a copy of a parallel vector
   * while the other jobs do something   else.
   *
   */
   ...
   * only succeed if all processes that have knowledge of   @p v     (i.e.
   * those given by   <code>v.get_mpi_communicator()</code>  ) do it at   the
   * same time. This means that unless you use a split MPI communicator   then
   * it is not normally possible for only one or a subset of processes   to
   ...
   * zero.     The constructor is made explicit to avoid accidents like this:
   * <tt>v=0;</tt>. Presumably, the user wants to set every element of the
   * vector to zero, but instead, what happens is this call:   <tt>v=Vector
   * @<number@>(0);</tt>,   i.e. the vector is replaced by one of   length
   * zero.
   *
   */
   ...
   * iterators. This function is there in analogy to the   @p std::vector
   * class.
   *
   */
   ...
   * vector classes.     For the parallel vector wrapper class, this function
   * compresses the   underlying representation of the vector, i.e. flushes
   ...
   * anything else can be done on   it.     However, for the implementation of
   * this class, it is immaterial and thus   an empty function.
   *
   */
  void
   ...
   * this   vector remains unchanged if possible, to make things faster; this
   * may   waste some memory, so keep this in mind.  However, if <tt>N==0</tt>
   * all   memory is freed, i.e. if you want to resize the vector and release
   * the   memory not needed, you have to first call <tt>reinit(0)</tt> and
   * then   <tt>reinit(N)</tt>. This cited behavior is analogous to that of
   * the   standard library containers.     If   @p omit_zeroing_entries   is
   * false, the vector is filled by zeros.   Otherwise, the elements are left
   * an unspecified state.     This function is virtual in order to allow for
   * derived classes to handle   memory separately.
   *
   */
  virtual void
  ...
  /**
   * Same as above, but will preserve the values of vector upon resizing.   If
   * we new size is bigger, we will have   \f[ \mathbf V \rightarrow \left(
   * \begin{array}{c} \mathbf V   \\ \mathbf 0 \end{array} \right) \f]
   * whereas if the desired size is smaller, then   \f[ \left(
   * \begin{array}{c} \mathbf V_1   \\ \mathbf V_2 \end{array} \right)
   * \rightarrow \mathbf V_1 \f]
   *
   */
  void
   ...
   * Utilities::LinearAlgebra::givens_rotation())     to the vector in the
   * plane spanned by the   @p i'th   and   @p k'th   unit vectors.
   *
   */
  void
   ...
   * for   the other   @p reinit   function.     The elements of   @p V   are
   * not copied, i.e.  this function is the same as   calling <tt>reinit
   * (V.size(), omit_zeroing_entries)</tt>.
   *
   */
   ...
   * functions.     This function is virtual in order to allow for derived
   * classes to handle   memory separately.
   *
   */
  virtual void
  ...
  /**
   * Set all components of the vector to the given number   @p s.       Since
   * the semantics of assigning a scalar to a vector are not immediately
   ...
   * the internal data of the vector   @p v   and resets   @p v   to the state
   * it would   have after being newly default-constructed.
   *
   */
  Vector<Number> &
   ...
   * underlying type of   @p this   vector, so the return type and the
   * accuracy   with which it the result is computed depend on the order of
   * the arguments   of this vector.     For complex vectors, the scalar
   * product is implemented as     $\left<v,w\right>=\sum_i v_i \bar{w_i}$  .
   ...
   * the elements.       @dealiiOperationIsMultithreaded   The algorithm uses
   * pairwise summation   with the same order of summation in every run, which
   * gives fully   repeatable results from one run to another.
   *
   */
  real_type
  l2_norm() const;
   ...
   * whereas calling   separate methods means to load the calling vector   @p
   * this   twice. Since   most vector operations are memory transfer limited,
   * this reduces the time   by 25\% (or 50\% if   @p W   equals   @p this).
   ...
   * the   C++ standard library by returning iterators to the start and end of
  ...
  /**
   * Access the value of the   @p ith   component.     Exactly the same as
   * operator().
   *
   */
  ...
  /**
   * Access the   @p ith   component as a writeable reference.     Exactly the
   * same as operator().
   *
   */
   ...
   * function allows getting a whole set of elements at once. The   indices of
   * the elements to be read are stated in the first argument, the
   * corresponding values are returned in the second.     If the current
   * vector is called   @p v,   then this function is the equivalent   to the
   * code
   * @code
  ...
  /**
   * Instead of getting individual elements of a vector via operator(),   this
   * function allows getting a whole set of elements at once. In   contrast to
   * the previous function, this function obtains the   indices of the
   * elements by dereferencing all elements of the iterator   range provided
   * by the first two arguments, and puts the vector   values into memory
   * locations obtained by dereferencing a range   of iterators starting at
   * the location pointed to by the third   argument.     If the current
   * vector is called   @p v,   then this function is the equivalent   to the
   * code
   * @code
   ...
   * starting at   @p values_begin   as there are iterators between       @p
   * indices_begin   and   @p indices_end.
   *
   */
   ...
   * and not a   vector.       @dealiiOperationIsMultithreaded
   *
   */
  void
  add(const Number s);
  ...
  /**
   * Assignment <tt>*this = a*u</tt>.       @dealiiOperationIsMultithreaded
   *
   */
  void
   ...
   * parallel vector classes (e.g.,   LinearAlgebra::distributed::Vector
   * class).
   *
   */
  void
   ...
   * check   whether refinement indicators are really all positive (or zero).
  ...
  /**
   * This function exists for compatibility with the   @p     parallel vector
   * classes (e.g.,   LinearAlgebra::distributed::Vector   class).   Always
   * returns false since this implementation is serial.
   *
   */
  bool
