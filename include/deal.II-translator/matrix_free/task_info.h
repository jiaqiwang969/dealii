...
namespace internal
{
  ...
  namespace MatrixFreeFunctions
  {
    ...
    struct TaskInfo
    {
       ...
       * controlled by the input arguments.               @param
       * cells_with_comm A list of cells that need to exchange data
       ...
       * with communication have the ghost data ready.               @param
       * dofs_per_cell Gives an expected value for the number of degrees
       * of freedom on a cell, which is used to determine the block size for
       * interleaving cell and face integrals.               @param
       * categories_are_hp Defines whether
       ...
       * variant.               @param   cell_vectorization_categories This
       * set of categories defines       the cells that should be grouped
       * together inside the lanes of a       vectorized array. This can be
       * the polynomial degree in an hp-element       or a user-provided
       * grouping.               @param   cell_vectorization_categories_strict
       * Defines whether the       categories defined by the previous
       * variables should be separated       strictly or whether it is allowed
       * to insert lower categories into the       next high one(s).
       * @param   parent_relation This data field is used to specify which
       * cells       have the same parent cell. Cells with the same ancestor
       * are grouped       together into the same batch(es) with vectorization
       * across cells.               @param   renumbering When leaving this
       * function, the vector contains a       new numbering of the cells that
       * aligns with the grouping stored in       this class.
       * @param   incompletely_filled_vectorization Given the vectorized
       * layout       of this class, some cell batches might have components
       * in the       vectorized array (SIMD lanes) that are not used and do
       * not carray       valid data. This array indicates the cell batches
       * where this occurs       according to the renumbering returned by this
       * function.
       *
       */
      void
      ...
      /**
       * First step in the block creation for the task-parallel blocking
       * setup.               @param   boundary_cells A list of cells that
       * need to exchange data prior       to performing computations. These
       * will be given a certain id in the       partitioning.
       * @param   renumbering When leaving this function, the vector contains
       * a       new numbering of the cells that aligns with the grouping
       * stored in       this class (before actually creating the tasks).
       * @param   incompletely_filled_vectorization Given the vectorized
       * layout       of this class, some cell batches might have components
       * in the       vectorized array (SIMD lanes) that are not used and do
       * not carray       valid data. This array indicates the cell batches
       ...
       * when used in parallel.             The strategy is based on a
       * two-level approach. The outer level is       subdivided into
       * partitions similar to the type of neighbors in       Cuthill-McKee,
       * and the inner level is subdivided via colors (for       chunks within
       * the same color, can work independently). One task is
       * represented by a chunk of cells. The cell chunks are formed before
       * subdivision into partitions and colors.               @param
       * connectivity (in/out) Determines whether cells `i` and `j` are
       * conflicting, expressed by an entry in position (i,j).
       * @param   renumbering (in/out) At output, the element j of this
       * variable       gives the original number of the cell that is
       * reordered to place j by       the ordering due to the thread graph.
       * @param   irregular_cells (in/out) Informs the current function
       * whether       some SIMD lanes in VectorizedArray would not be filled
       * for a given       cell batch index.               @param   hp_bool
       * Defines whether we are in hp-mode or not
       *
       */
      void
       ...
       * two-level approach. The outer level is       subdivided into
       * partitions similar to the type of neighbors in       Cuthill-McKee,
       * and the inner level is again subdivided into Cuthill-
       ...
       * levels of partitions.               @param   cell_active_fe_index The
       * active FE index corresponding to the       individual indices in the
       * list of all cell indices, in order to be       able to not place
       * cells with different indices into the same cell       batch with
       * vectorization.               @param   connectivity (in/out)
       * Determines whether cells `i` and `j` are       conflicting, expressed
       * by an entry in position (i,j).               @param   renumbering
       * (in/out) At output, the element j of this variable       gives the
       * original number of the cell that is reordered to place j by       the
       * ordering due to the thread graph.               @param
       * irregular_cells (in/out) Informs the current function whether
       * some SIMD lanes in VectorizedArray would not be filled for a given
       * cell batch index.               @param   hp_bool Defines whether we
       * are in hp-mode or not
       *
       */
      void
       ...
       * depending on the setting in the data structure.               @param
       * cell_active_fe_index The active FE index corresponding to the
       ...
       * batch with vectorization.               @param   connectivity
       * (in/out) Determines whether cells `i` and `j` are       conflicting,
       * expressed by an entry in position (i,j).               @param
       * renumbering (in/out) At output, the element j of this variable
       * gives the original number of the cell that is reordered to place j by
       * the ordering due to the thread graph.               @param
       * irregular_cells (in/out) Informs the current function whether
       * some SIMD lanes in VectorizedArray would not be filled for a given
       * cell batch index.               @param   hp_bool Defines whether we
       * are in hp-mode or not
      ...
      /**
       * This function creates partitions according to the provided
       * connectivity       graph.               @param   connectivity
       * Connectivity between (blocks of cells)               @param
       * cluster_size The number of cells in each partition should be a
       * multiple of cluster_size (for blocking later on)               @param
       * cell_partition Saves of each (block of cells) to which
       * partition the block belongs               @param   partition_list
       * partition_list[j] gives the old number of the       block that should
       * be renumbered to j due to the partitioning               @param
       * partition_size Vector pointing to start of each partition (on
       * output)               @param   partition number of partitions created
       *
       */
      void
       ...
       * field   @p partition_row_index   stores the distance from one
       * 'vector' to       the next within the linear storage of all data to
       * the two-level       partitioning.
       *
       */
