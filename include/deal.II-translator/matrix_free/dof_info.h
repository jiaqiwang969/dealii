...
namespace internal
{
  namespace MatrixFreeFunctions
  {
     ...
     * AffineConstraints object.         This class only stores index
     * relations. The weights for hanging node     constraints are stored in a
     * different field. This is because a     different field allows for the
     * same compressed weight data on different     DoFHandlers for
     * vector-valued problems. There, the indices might be     constrained
     * differently on different components (e.g. Dirichlet     conditions only
     * on selected components), whereas the weights from     hanging nodes are
     * the same and need to be stored only once. The     combination will be
     * handled in the MatrixFree class.
     * @ingroup matrixfree
     *
     */
    struct DoFInfo
    {
       ...
       * zero inside the   MatrixFree::loop()   call. The goal is to only
       * clear a       part of the vector at a time to keep the values that
       * are zeroed in       caches, saving one global vector access for the
       * case where this is       applied rather than `vector = 0.;`.
       * We set the granularity to 64
       *
       ...
       * temporary numbering employed by the   @p read_dof_indices   function.
       * The       numbers are localized with respect to the MPI process, and
       * ghosts       start at the end of the locally owned range. This way,
       * we get direct       access to all vector entries.
       *
       */
      void
       ...
       * cells       together and interprets them as one cell only, as is
       ...
       * transposition in   IndexStorageVariants::contiguous.   Currently,
       * these       more advanced features are not implemented, so there is
       * only limited       value of this function.
       *
       */
      void
      ...
      /**
       * Fills the array that defines how to zero selected ranges in the
       * result       vector within the cell loop, filling the two member
       * variables   @p         vector_zero_range_list_index and   @p
       * vector_zero_range_list.               The intent of this pattern is
       * to zero the vector entries in close       temporal proximity to the
       * first access and thus keeping the vector       entries in cache.
       *
       */
      template <int length>
      void
       ...
       * functions in   FEEvaluationBase::read_write_operation()   on which
       * array       to get the data from. One example of more efficient
       * storage is the       enum value   IndexStorageVariants::contiguous,
       * which means that one can       get the indices to all degrees of
       * freedom of a cell by reading only       the first index for each
       * cell, whereas all subsequent indices are       merely an offset from
       * the first index.
       *
       */
      enum class IndexStorageVariants : unsigned char
      {
        ...
        contiguous,
        /**
         * This value indicates that the indices with a cell are contiguous
         * and         interleaved for vectorization, i.e., the first DoF
         * index on a cell         to the four or eight cells in the
         * vectorization batch come first,         than the second DoF index,
         * and so on. Furthermore, the interleaving         between cells
         * implies that only the batches for vectorization can be
         * accessed efficiently, whereas there is a strided access for getting
         * only some of the entries.                 The two additional
         * categories `interleaved_contiguous_strided` and
         ...
         * adjacent sides will break with the interleaved storage. We then
         * have         to make a strided access as described in the next
         * category. The last         category
         * `interleaved_contiguous_mixed_strides` appears in the ghost
         ...
         * interleave the indices between cells.                 For a
         * cell/face of this index type, the data access in
        ...
        /**
         * Similar to interleaved_contiguous storage, but for the case when
         * the         interleaved indices are only contiguous within the
         * degrees of         freedom, but not also over the components of a
         * vectorized array.         This happens typically on faces with DG
         * where the cells have         `interleaved_contiguous` storage but
         * the faces' numbering is not the         same as the cell's
         * numbering. For a         cell/face of this index type, the data
         * access in FEEvaluationBase         is directed to the array
         * `dof_indices_contiguous` with the index
         * `cell_index*n_vectorization*n_components`.
         *
         */
        interleaved_contiguous_strided,
        /**
         * Similar to interleaved_contiguous_separate storage, but for the
         * case         when the interleaved indices are not `n_vectorization
         * apart`. This         happens typically within the ghost layer of DG
         * where the remote         owner has applied an interleaved storage
         * and the current processor         only sees some of the cells. For
         * a         cell/face of this index type, the data access in
         * FEEvaluationBase         is directed to the array
         * `dof_indices_contiguous` with the index
       ...
       * dof_indices and   @p constraint_indicator   fields. These two fields
       * are       always accessed together, so it is simpler to keep just one
       * variable       for them. This also obviates keeping two rowstart
       * vectors in sync.
       *
       */
       ...
       * This       array also includes the indirect contributions from
       * constraints,       which are described by the   @p
       * constraint_indicator   field. Because of       variable lengths of
       * rows, this would be a vector of a vector.       However, we use one
       * contiguous memory region and store the rowstart       in the variable
       * @p row_starts.
       *
       */
       ...
       * dof_indices used according to the description in
       * IndexStorageVariants.             The three arrays given here address
       * the types for the faces decorated       as interior (0), the faces
       * decorated with as exterior (1), and the       cells (2) according to
       * CellOrFaceAccess.
       *
       */
       ...
       * dof_indices used according to the description in
       * IndexStorageVariants.             The three arrays given here address
       * the types for the faces decorated       as minus (0), the faces
       * decorated with as plus (1), and the cells       (2).
       *
       */
       ...
       * but this field allows for faster access.             The three arrays
       * given here address the types for the faces decorated       as
       * interior (0), the faces decorated with as exterior (1), and the
       * cells (2) according to CellOrFaceAccess.
       *
       */
       ...
       * ghost indices to the full       vector partitioner stored in   @p
       * vector_partitioner.   These       partitioners are used in
       * specialized loops that only import parts of       the ghosted region
       * for reducing the amount of communication. There       are five
       * variants of the partitioner initialized:
       *
       *
       *
       *
       ...
       * which are included in   @p dof_indices.   Because of variable lengths
       * of       rows, this would be a vector of a vector. However, we use
       * one       contiguous memory region and store the rowstart in the
       ...
       * directly.             The outer vector goes through the various FE
       * indices in the hp-case,       similarly to the   @p dofs_per_cell
       * variable.
       *
       */
       ...
       * calling   @p assign_ghosts.   Then, all information is collected by
       * the       partitioner.
       *
       */
