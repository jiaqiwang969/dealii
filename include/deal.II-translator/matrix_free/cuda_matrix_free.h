...
#  endif
   ...
   * products or residual computations on the same mesh.     This class does
   * not implement any operations involving finite element basis   functions,
   * i.e., regarding the operation performed on the cells. For these
   * operations, the class FEEvaluation is designed to use the data collected
   * in   this class.     This class implements a loop over all cells
   * (cell_loop()). This loop is   scheduled in such a way that cells that
   * share degrees of freedom   are not worked on simultaneously, which
   * implies that it is possible to   write to vectors in parallel without
   * having to explicitly synchronize   access to these vectors and matrices.
   * This class does not implement any   shape values, all it does is to cache
   * the respective data. To implement   finite element operations, use the
   * class   CUDAWrappers::FEEvalutation.       This class traverse the cells
   * in a different order than the usual   Triangulation class in deal.II.
...
#  endif
       ...
       * This       class can cache data needed for gradient computations
       * (inverse       Jacobians), Jacobian determinants (JxW), quadrature
       * points as well as       data for Hessians (derivative of Jacobians).
       * By default, only data for       gradients and Jacobian determinants
       * times quadrature weights, JxW, are       cached. If quadrature points
       * of second derivatives are needed, they       must be specified by
       * this field.
       *
       */
     ...
     * on     each element in parallel.   @p func   is a functor which is
     * applied on each color.           @p func   needs to define     \code
     * __device__ void operator()(       const unsigned int
     * cell,       const typename   CUDAWrappers::MatrixFree<dim,
     * Number>::Datagpu_data,           CUDAWrappers::SharedData<dim,
     * Number>                     shared_data,       const Number
     * src,       Number
     * dst) const;       static const unsigned int n_dofs_1d;       static
     * const unsigned int n_local_dofs;       static const unsigned int
     * n_q_points;     \endcode
     *
     */
    // clang-format on
     ...
     * cell_loop()     but it uses a simpler functor.           @p func
     * needs to define     \code      __device__ void operator()(        const
     * unsigned int                                          cell,
     * const typename   CUDAWrappers::MatrixFree<dim,
     * Number>::Datagpu_data);       static const unsigned int n_dofs_1d;
     * static const unsigned int n_local_dofs;     static const unsigned int
     * n_q_points;     \endcode
     *
     */
    ...
    /**
     * Copy the values of the constrained entries from   @p src   to   @p dst.
    ...
    /**
     * Set the entries in   @p dst   corresponding to constrained values to
     * @p val.       The main purpose of this function is to set the
     * constrained entries of     the source vector used in cell_loop() to
     * zero.
     *
     */
    template <typename VectorType>
     ...
     * information     about these objects, you can query them with the
     * respective access     functions. If you just want to initialize a
     * (parallel) vector, you should     usually prefer this data structure as
     * the data exchange information can     be reused from one vector to
     * another.
     *
     */
  ...
  private:
    ...
    /**
     * Helper function. Copy the values of the constrained entries of   @p src
     * to       @p dst.   This function is used when MPI is not used.
     *
     */
    template <typename VectorType>
    void
    ...
    /**
     * Helper function. Copy the values of the constrained entries of   @p src
     * to       @p dst.   This function is used when MPI is used.
     *
     */
    void
    ...
    /**
     * Helper function. Set the constrained entries of   @p dst   to   @p val.
     ...
     * in_constrained_values-operations.
     *
     */
   ...
   * only unique for a given MPI process.       @relates
  ...
  /**
   * Copy   @p data   from the device to the device.   @p update_flags
  ...
  /**
   * This function is the host version of local_q_point_id().       @relates
   * CUDAWrappers::MatrixFree
   *
   */
  template <int dim, typename Number>
   ...
   * host   using copy_mf_data_to_host().       @relates
   * CUDAWrappers::MatrixFree
   *
   */
  template <int dim, typename Number>
