...
{
  /**
   * This class implements a wrapper around   ArborX::BVH.   BVH stands for
   * Bounding   Volume Hierarchy.     From
   * [Wikipedia](https://en.wikipedia.org/wiki/Bounding_Volume_Hierarchy):
   * <blockquote>   A bounding volume hierarchy (BVH) is a tree structure on a
   * set of geometric   objects. All geometric objects are wrapped in bounding
   * volumes that form   the leaf nodes of the tree. These nodes are then
   * grouped as small sets and   enclosed within larger bounding volumes.
   * These, in turn, are also grouped   and enclosed within other larger
   * bounding volumes in a recursive fashion,   eventually resulting in a tree
   * structure with a single bounding volume at   the top of the tree.
   * Bounding volume hierarchies are used to support   several operations on
   * sets of geometric objects efficiently, such as in   collision detection
   * and ray tracing.   </blockquote>     Because ArborX uses Kokkos, Kokkos
   * needs to be initialized and finalized   before using this class.
   *
   */
  class BVH
  {
  public:
    /**
     * Constructor. Use a vector of BoundingBox   @p bounding_boxes   as
     * primitives.
     *
     */
    ...
    /**
     * Return the indices of those BoundingBox objects that satisfy the   @p
     * queries.       Because   @p queries   can contain multiple queries, the
     * function returns a pair     of indices and offsets.         Below is an
     * example piece of code that does the following: Let us assume     that
     * we have a set of bounding boxes for objects
     *
     ...
     * @endcode
     * The elements of `bvh_bounding_boxes` that intersect the `j`th
     * BoundingBox     of `query_bounding_boxes` are given by:
     * @code
     ...
     * boxes individual points lie in
     *
