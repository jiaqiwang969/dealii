...
/**
 * This class facilitates the storage of any general data. It offers a
 * mechanism to store any amount of data, of any type, which is then made
 * accessible by an identifier string. When using this class, please cite
 *
 *
 * @code{.bib}
...
public:
  ...
  /**
   * Print the contents of the internal cache to the   @p stream.       Each
   * key and value pair in the   @p any_data   map are printed on an
  ...
  /**
   * Clear all data stored in this class instance.     When you call this
   * function, it destroys all objects you asked to be stored   as copies, and
   * it forgets about the references to data you asked to store   by
   * reference. As a consequence, you are now free to destroy the objects to
   * which references were stored at whatever time you want
   *
   ...
   * copies of any   @p Type   stored by   @p data   only remains valid while
   * the   GeneralDataStorage instance in which it is stored is alive.
   * Furthermore, as elucidated in the last example, the copy of the   class
   * instance (owned by GeneralDataStorage) that is being pointed to   is no
   * longer alive when the reset() function is called, or when it is   removed
   * via a call to remove_object_with_name().
   * @code
   * GeneralDataStorage data;
   * double* ptr_to_some_number = null_ptr;
   *
  ...
  /**
   * Store internally a copy of the given object. The copied object is   owned
   * by this class, and is accessible via reference through the
   * get_object_with_name() method.     This function ensures that no   @p
   * entry   with the given   @p name   is   already stored by this class
   * instance.
   *
   */
   ...
   * Store internally a copy of the given object. The copied object is   owned
   * by this class, and is accessible via reference through the
   * get_object_with_name() method.     This function does not perform any
   * checks to ensure that the   @p entry     with the given   @p name   is
   * already stored by this class instance. If the     @p name   does in fact
   * point to existing data, then this is overwritten.
   *
   */
  template <typename Type>
  void
  ...
  /**
   * Add a reference to an already existing object. The object is not   owned
   * by this class, and the user has to guarantee that the   referenced object
   * lives longer than this class instance. The stored   reference is
   * accessible through the get_object_with_name() method.     This function
   * ensures that no   @p entry   with the given   @p name   is   already
   * stored by this class instance.
   *
   */
  template <typename Type>
  void
   ...
   * lives longer than this class instance. The stored   reference is
   * accessible through the get_object_with_name() method.     This function
   * does not perform any checks to ensure that the   @p entry     with the
   * given   @p name   is already stored by this class instance. If the     @p
   * name   does in fact point to existing data, then this is overwritten.
   *
   */
  template <typename Type>
  void
   ...
   * not exist, then the input   @p arguments   will be used to construct an
   * object   of the given   @p Type   and a reference to this new object then
   * be returned.     A copy of an object of type   @p Type   , which is owned
   * by this class   instance, is generated by calling its constructor with
   * the given set of   arguments. For this function, the   @p arguments   are
   * passed as   <tt>lvalue</tt> references.
   *
   */
   ...
   * not exist, then the input   @p arguments   will be used to construct an
   * object   of the given   @p Type   and a reference to this new object then
   * be returned.     Same as above for a single argument.
   *
   */
   ...
   * be returned.     A copy of an object of type   @p Type   , which is owned
   * by this class   instance, is generated by calling its constructor with
   * the given set of   arguments. In contrast to the previous function of the
   * same name, for   this function the   @p arguments   are passed as
   * <tt>rvalue</tt> references.
   *
   */
  template <typename Type, typename Arg, typename... Args>
  Type &
   ...
   * stored in this class, or if the object with the given name is   neither
   * of the exact specified   @p Type   nor a pointer to the   @p Type.
   *
   */
  template <typename Type>
  Type &
  ...
  /**
   * Return a constant reference to the object with the given name.     This
   * function throws an exception if either an object with the given name   is
   * not stored in this class, or if the object with the given name is
   * neither of the exact specified   @p Type   nor a pointer to the   @p
   * Type.
   *
   */
  template <typename Type>
  ...
  /**
   * An entry with this name does not exist in the internal   boost::any
   * map.
   *
   */
