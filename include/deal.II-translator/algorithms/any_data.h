...
public:
  ...
  /**
   * @brief Access to stored data object by name.       Find the object with
   * given name, try to convert it to <tt>type</tt> and   return it. This
   * function throws an exception if either the name does not   exist or if
   * the conversion fails. If such an exception is not desired,   use
   * try_read() instead.
   *
   */
  template <typename type>
  ...
  /**
   * @brief Read-only access to stored data object by name.       Find the
   * object with given name, try to convert it to <tt>type</tt> and   return
   * it. This function throws an exception if either the name does not   exist
   * or if the conversion fails. If such an exception is not desired,   use
   * try_read() instead.
   *
   */
  template <typename type>
  ...
  /**
   * @brief Dedicated read only access by name.       For a constant object,
   * this function equals entry(). For a non-const   object, it forces read
   * only access to the data. In particular, it throws   an exception if the
   * object is not found or cannot be converted to type.   If such an
   * exception is not desired, use try_read() instead.       @warning   Do not
   * use this function for stored objects which are pointers.   Use read_ptr()
   * instead!
   *
   */
  template <typename type>
  ...
  /**
   * @brief Dedicated read only access by name for pointer data.       If the
   * stored data object is a pointer to a constant object, the logic of
  ...
  /**
   * @brief Dedicated read only access by name without exceptions.       This
   * function tries to find the name in the list and return a pointer to   the
   * associated object. If either the name is not found or the object   cannot
   * be converted to the return type, a null pointer is returned.
   *
   */
  template <typename type>
  const type *
  ...
  /**
   * @brief Find index of a named object       Try to find the object and
   * return its index in the list. Throw an   exception if the object has not
   * been found.
   *
   */
  unsigned int
  ...
  /**
   * @brief Try to find index of a named object       Try to find the object
   * and return its index in the list. returns
