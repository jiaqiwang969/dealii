...
/**
 * Namespace containing numerical algorithms in a unified form. All
 * algorithmic classes in this namespace are derived from either Operator or
 * OutputOperator, depending on whether they return a value or not. See the
...
{
  /**
   * @todo   Update this documentation and the one of Operator     The
   * abstract base class of all algorithms in this library. An operator is
   * an object with an operator(), which transforms a set of named vectors
   * into another set of named vectors.     Furthermore, an operator can be
   * notified of parameter changes by the   calling routine. The outer
   * iteration can notify() the Operator of an   Event, which could be for
   * instance a change of mesh, a different time   step size or too slow
   * convergence of Newton's method, which would then   trigger reassembling
   * of a matrix or similar things.     <h3>Usage for nested iterations</h3>
  ...
  public:
    ...
    /**
     * The copy constructor is deleted since objects of this class     should
     * not be copied.
     *
     */
    ...
    /**
     * Set the stream   @p os   to which data is written. If no stream is
     * selected     with this function, data goes to   @p deallog.
     *
     */
    void
