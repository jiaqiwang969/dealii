...
{
  /**
   * Control class for timestepping schemes. Its main task is determining the   size of the next time step and the according point in the time interval.   Additionally, it controls writing the solution to a file.     The size of the next time step is determined as follows:     <ol>       <li>   According to the strategy, the step size is tentatively added to the   current time.     <li>   If the resulting time exceeds the final time of the interval, the   step size is reduced in order to meet this time.     <li>   If the resulting time is below the final time by just a fraction of   the step size, the step size is increased in order to meet this time.     <li>   The resulting step size is used from the current time.     </ol>       The variable   @p print_step   can be used to control the amount of output   generated by the timestepping scheme.
  ...
  public:
    ...
    /**
     * Read the control parameters from a parameter handler.         This
     * function also calls restart() to reset all other internal
     * parameters of this class to their appropriate values based on     the
     ...
     * stepping strategy.           @param[in]   step The size of the first
     * step, which may be overwritten by     the time stepping strategy.
     *
     */
    void
  ...
  private:
    ...
    /**
     * The size of the current time step. This may differ from   @p step_val,
     * if     we aim at   @p final_val.
     *
     */
