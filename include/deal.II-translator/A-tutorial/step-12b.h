 ...
 * This tutorial depends on   step-16  ,   step-7  ,   step-39  . @htmlonly
 ...
 * insteadof assembling the face terms using FEInterfaceValues. The details of
 * this framework on how it is used in practice will be explainedas part of
 * this tutorial program. <a name="Thetestcase"></a><h3>The testcase</h3>
 *
 ...
 * program, called AdvectionProblem. It is basically the main class of
 * step-6  . We do not have an AffineConstraints object, because there are no
 * hanging node constraints in DG discretizations.
 *
 *
 ...
 * faces, we also use the MeshWorker interface to simplify the loops involved.
 *
 *
 * @code
 * template <int dim>
 ...
 * different degree the whole program stays the same, only replace 1 in the
 * constructor by the desired polynomial degree.
 *
 *
 * @code
 *   FE_DGQ<dim>     fe;
 ...
 * @endcode
 *
 * Finally, we have to provide functions that assemble the cell, boundary, and
 * inner face terms. Within the MeshWorker framework, the loop over all cells
 * and much of the setup of operations will be done outside this class, so all
 * we have to provide are these three operations. They will then work on
 * intermediate objects for which first, we here define alias to the info
 * objects handed to the local integration functions in order to make our life
 * easier below.
 *
 *
 * @code
 ...
 * @endcode
 *
 * The following three functions are then the ones that get called inside the
 * generic loop over all cells and faces. They are the ones doing the actual
 * integration. In our code below, these functions do not access member
 * variables of the current class, so we can mark them as
 * <code>static</code>   and simply pass pointers to these functions to the
 * MeshWorker framework. If, however, these functions would want to access
 * member variables (or needed additional arguments beyond the ones specified
 * below), we could use the facilities of lambda functions to provide the
 ...
 * @endcode
 *
 * This is the magic object, which knows everything about the data structures
 * and local integration.  This is the object doing the work in the function
 * MeshWorker::loop(),   which is implicitly called by
 ...
 * with number of points one higher than the polynomial degree used. Since the
 * quadratures for cells, boundary and interior faces can be selected
 ...
 * @endcode
 *
 * These are the types of values we need for integrating our system. They are
 * added to the flags used on cells, boundary and interior faces, as well as
 * interior neighbor faces, which is forced by the four   @p true   values.
 *
 *
 * @code
 ...
 * and face. Now, we need an object which receives the integrated (local) data
 * and forwards them to the assembler.
 *
 *
 * @code
 ...
 * first argument, which is an active iterator). As noted in the discussion
 * when declaring the local integration functions in the class declaration,
 * the arguments expected by the assembling integrator class are not actually
 * function pointers. Rather, they are objects that can be called like
 * functions with a certain number of arguments. Consequently, we could also
 * pass objects with appropriate operator() implementations here, or lambda
 * functions if the local integrators were, for example, non-static member
 * functions.
 *
 *
 * @code
 ...
 * that these objects can handle much more complex structures, thus the access
 * here looks more complicated than might seem necessary.
 *
 *
 * @code
 ...
 * @endcode
 *
 * For quadrature points, weights, etc., we use the FEValuesBase object of the
 * first argument.
 *
 *
 * @code
 ...
 * refer to trial and test functions, respectively. The %numbers indicate the
 * cells provided by info1 and info2. By convention, the two matrices in each
 * info object refer to the test functions on the respective cell. The first
 * matrix contains the interior couplings of that cell, while the second
 * contains the couplings between cells.
 *
 *
 * @code
 ...
 * @endcode
 *
 * Here, following the previous functions, we would have the local right hand
 * side vectors. Fortunately, the interface terms only involve the solution
 * and the right hand side does not receive any contributions.
 *
 *
 *
 *
 ...
 * @endcode
 *
 * We additionally assemble the term   $(\beta\cdot n u,\hat v)_{\partial
 * \kappa_+}$  ,
 *
 *
 * @code
 ...
 * @endcode
 *
 * And this is another new one:   $(\beta\cdot n \hat u,\hat v)_{\partial
 * \kappa_-}$  :
 *
 *
 * @code
 ...
 *
 *
 * The output of this program is very similar to   step-16   and we are not
 * repeating the output here. We show the solutions on the initial mesh, the
 * mesh after twoand after five adaptive refinement steps. <img
 ...
 * alt="">
 *
 *  Then we show the final grid (after 5 refinement steps) and the solution
 * again,this time with a nicer 3d rendering (obtained using the
 * DataOutBase::write_vtk  function and the VTK-based VisIt visualization
 * program) that better shows thesharpness of the jump on the refined mesh and
 * the over- and undershoots of thesolution along the interface: <img
 * src="https://www.dealii.org/images/steps/developer/step-12.grid-5.png"
 * alt="">    <img
 * src="https://www.dealii.org/images/steps/developer/step-12.3d-solution.png"
 * alt="">
 *
 *  And finally we show a plot of a 3d computation. <img
