...
* This example is based on the Laplace equation in 2d and deals with thequestion what happens if a membrane is deflected by some external force but isalso constrained by an obstacle. In other words, think of a elastic membraneclamped at the boundary to a rectangular frame (we choose   $\Omega =
...
* Intuitively, gravity acts downward and so   $f(\mathbf x)$   is a negativefunction (we choose   $f=-10$   in this program). The first condition then meansthat the total force acting on the membrane is gravity plus somethingpositive: namely the upward force that the obstacle exerts on the membrane atthose places where the two of them are in contact. How big is this additionalforce? We don't know yet (and neither do we know "where" it actually acts) butit must be so that the membrane doesn't penetrate the obstacle.
* The fourth equality above together with the last inequality forms the obstaclecondition which has to hold at every point of the whole domain. The latter ofthese two means that the membrane must be above the obstacle   $g(\mathbf x)$  everywhere. The second to last equation, often called the "complementaritycondition" says that where the membrane is not in contact with the obstacle(i.e., those   $\mathbf x$   where   $u(\mathbf x)
* 
...
\neq 0$  , i.e., there can be additional forces (though there don't have to be:it is possible for the membrane to just touch, not press against, theobstacle).
* 

* <a name="Derivationofthevariationalinequality"></a><h3>Derivation of the variational inequality</h3>
* 

* An obvious way to obtain the variational formulation of the obstacle problem is to consider the total potential energy:@f{equation*}
 E(u) \dealcoloneq \dfrac{1}{2}\int\limits_{\Omega} \nabla u \cdot \nabla u
* 
- \int\limits_{\Omega} fu.
@f}
* We have to find a solution   $u\in G$   of the following minimization problem:@f{equation*}
 E(u)\leq E(v)\quad \forall v\in G,
@f}
* with the convex set of admissible displacements:@f{equation*}
...
G$  . Then the function@f{equation*}
 F(\varepsilon) \dealcoloneq E(u+\varepsilon(v-u)),\quad\varepsilon\in\left[0,1\right],
@f}
* takes its minimum at   $\varepsilon = 0$   (because   $u$   is a minimizer of theenergy functional   $E(\cdot)$  ), so that   $F'(0)\geq 0$   for any choiceof   $v$  . Note that  $u+\varepsilon(v-u) = (1-\varepsilon)u+\varepsilon v\in G$   because of theconvexity of   $G$  . If we compute   $F'(\varepsilon)\vert_{\varepsilon=0}$   ityields the variational formulation we are searching for:
* <i>Find a function $u\in G$ with</i>@f{equation*}
 \left(\nabla u, \nabla(v-u)\right) \geq \left(f,v-u\right) \quad \forall v\in G.
@f}
* 
* This is the typical form of variational inequalities, where not just   $v$  appears in the bilinear form but in fact   $v-u$  . The reason is this: if   $u$   isnot constrained, then we can find test functions   $v$   in   $G$   so that   $v-u$   can haveany sign. By choosing test functions   $v_1,v_2$   so that   $v_1-u =
* 
-(v_2-u)$   itfollows that the inequality can only hold for both   $v_1$   and   $v_2$   if the twosides are in fact equal, i.e., we obtain a variational equality.
...
* The variational inequality above is awkward to work with. We would thereforelike to reformulate it as an equivalent saddle point problem. We introduce aLagrange multiplier   $\lambda$   and the convex cone   $K\subset V'$  ,   $V'$  dual space of   $V$  ,   $K \dealcoloneq \{\mu\in V': \langle\mu,v\rangle\geq 0,\quad \forall
v\in V, v \le 0 \}$   ofLagrange multipliers, where   $\langle\cdot,\cdot\rangle$   denotes the dualitypairing between   $V'$   and   $V$  . Intuitively,   $K$   is the cone of all "non-positivefunctions", except that   $K\subset (H_0^1)'$   and so contains other objectsbesides regular functions as well.This yields:
...
@f}
* In other words, we can consider   $\lambda$   as the negative of the additional, positive force that theobstacle exerts on the membrane. The inequality in the second line of thestatement above only appears to have the wrong sign because we have  $\mu-\lambda<0$   at points where   $\lambda=0$  , given the definition of   $K$  .
* The existence and uniqueness of   $(u,\lambda)\in V\times K$   of this saddlepoint problem has been stated in Glowinski, Lions and Tr&eacute;moli&egrave;res: Numerical Analysis of VariationalInequalities, North-Holland, 1981.
* 
...
- G_i) \rbrace,
@f}
* with some   $c>0$  . (In this program we choose   $c = 100$  . It is a kind of apenalty parameter which depends on the problem itself and needs to be chosenlarge enough; for example there is no convergence for   $c = 1$   using thecurrent program if we use 7 global refinements.)
* After some head-scratching one can then convince oneself that the inequalitiesabove can equivalently be rewritten as@f{equation*}
 C([BU]_i,\Lambda_i) = 0, \qquad \forall i\in{\cal S}.
@f}
* The primal-dual active set strategy we will use here is an iterative scheme which is based onthis condition to predict the next active and inactive sets   $\mathcal{A}_k$   and  $\mathcal{F}_k$   (that is, those complementary sets of indices   $i$   for which  $U_i$   is either equal to or not equal to the value of the obstacle  $B^{-1}G$  ). For a more in depth treatment of this approach, see Hintermueller, Ito, Kunisch: The primal-dual active setstrategy as a semismooth newton method, SIAM J. OPTIM., 2003, Vol. 13, No. 3,pp. 865-888.
...
* 1. Initialize   $\mathcal{A}_k$   and   $\mathcal{F}_k$  , such that   $\mathcal{S}=\mathcal{A}_k\cup\mathcal{F}_k$   and   $\mathcal{A}_k\cap\mathcal{F}_k=\emptyset$   and set   $k=1$  .2. Find the primal-dual pair   $(U^k,\Lambda^k)$   that satisfies
...
* @f{align*}
 ...
 @f}
*  Note that the second and third conditions imply that exactly   $|S|$   unknowns are fixed, with the first condition yielding the remaining   $|S|$   equations necessary to determine both   $U$   and   $\Lambda$  .3. Define the new active and inactive sets by @f{equation*}
 \begin{split}
...
- G_i)\geq 0\rbrace.
 \end{split}
 @f}
* 4. If   $\mathcal{A}_{k+1}=\mathcal{A}_k$   (and then, obviously, also   $\mathcal{F}_{k+1}=\mathcal{F}_k$  ) then stop, else set   $k=k+1$   and go to step (2).
* The method is called "primal-dual" because it uses both primal (thedisplacement   $U$  ) as well as dual variables (the Lagrange multiplier  $\Lambda$  ) to determine the next active set.
* At the end of this section, let us add two observations. First,for any primal-dual pair   $(U^k,\Lambda^k)$   that satisfies thesecondition, we can distinguish the following cases:
* 1.   $\Lambda^k_i + c([BU^k]_i
* 
- G_i) < 0$   (i active):    <br>    Then either   $[BU^k]_i<G_i$   and   $\Lambda^k_i=0$   (penetration) or   $\Lambda^k_i<0$   and   $[BU^k]_i=G_i$   (pressing load).2.   $\Lambda^k_i + c([BU^k]_i
* 
- G_i)\geq 0$   (i inactive):    <br>    Then either   $[BU^k]_i\geq G_i$   and   $\Lambda^k_i=0$   (no contact) or   $\Lambda^k_i\geq0$   and   $[BU^k]_i=G_i$   (unpressing load).
* Second, the method above appears intuitively correct and useful but a bit adhoc. However, it can be derived in a concisely in the following way. To thisend, note that we'd like to solve the nonlinear system@f{eqnarray*}
 &A U + B\Lambda = F,&\\
 &C([BU-G]_i, \Lambda_i) = 0,
\qquad \forall i.&
@f}
* We can iteratively solve this by always linearizing around the previousiterate (i.e., applying a Newton method), but for this we need to linearizethe function   $C(\cdot,\cdot)$   that is not differentiable. That said, it isslantly differentiable, and in fact we have@f{equation*}
...
- G_i)\geq 0.
                                  \end{cases}
@f}
* This suggest a semismooth Newton step of the form@f{equation*}
 \begin{pmatrix}
...
\end{pmatrix},
@f}
* where we have split matrices   $A,B$   as well as vectors in the natural way intorows and columns whose indices belong to either the active set  ${\mathcal{A}_k}$   or the inactive set   ${\mathcal{F}_k}$  .
* Rather than solving for updates   $\delta U, \delta \Lambda$  , we can also solvefor the variables we are interested in right away by setting   $\delta U^k \dealcoloneq
U^{k+1}
* 
- U^k$   and   $\delta \Lambda^k \dealcoloneq \Lambda^{k+1}
* 
- \Lambda^k$   andbringing all known terms to the right hand side. This yields@f{equation*}
\begin{pmatrix}
 A_{\mathcal{F}_k\mathcal{F}_k} & A_{\mathcal{F}_k\mathcal{A}_k} & B_{\mathcal{F}_k} & 0\\
 A_{\mathcal{A}_k\mathcal{F}_k} & A_{\mathcal{A}_k\mathcal{A}_k} & 0 & B_{\mathcal{A}_k}\\
...
* We could even drive this a bit further.It's easy to see that we can eliminate the third row and the third columnbecause it implies   $\Lambda_{\mathcal{F}_k} = 0$  :@f{equation*}
\begin{pmatrix}
...
\end{pmatrix}.
@f}
* This shows that one in fact only needs to solve for the Lagrange multiplierslocated on the active set. By considering the second row one would then recoverthe full Lagrange multiplier vector through@f{equation*}
 \Lambda^k_S = B^{-1}\left(f_{\mathcal{S}}
* 
- A_{\mathcal{S}}U^k_{\mathcal{S}}\right).
@f}
* Because of the third row and the fact that   $B_{\mathcal{A}_k}$   is a diagonal matrix we are ableto calculate   $U^k_{\mathcal{A}_k}=B^{-1}_{\mathcal{A}_k}G_{\mathcal{A}_k}$   directly. We can therefore also write thelinear system as follows:@f{equation*}
\begin{pmatrix}
...
\end{pmatrix}.
@f}
* Fortunately, this form is easy to arrive at: we simply build the usual Laplacelinear system@f{equation*}
\begin{pmatrix}
...
\end{pmatrix},
@f}
* and then let the AffineConstraints class eliminate all constrained degrees offreedom, namely   $U^k_{\mathcal{A}_k}=B^{-1}_{\mathcal{A}_k}G_{\mathcal{A}_k}$  ,in the same way as if the dofs in   $\mathcal{A}_k$   were Dirichlet data. Theresult linear system (the second to last one above) is symmetric and positivedefinite and we solve it with a CG-methodand the AMG preconditioner from Trilinos.
* 

* <a name="Implementation"></a><h3>Implementation</h3>
* 

* This tutorial is quite similar to   step-4  . The general structure of the programfollows   step-4   with minor differences:
* 
*  - We need two new methods,   <code>assemble_mass_matrix_diagonal</code>   and    <code>update_solution_and_constraints</code>  .
* 
...
* You may want to read up on   step-4   if you want to understand thecurrent program.
* 
...
* 
*  This class supplies all function and variables needed to describe the obstacle problem. It is close to what we had to do in   step-4  , and so relatively simple. The only real new components are the update_solution_and_constraints function that computes the active set and a number of variables that are necessary to describe the original (unconstrained) form of the linear system (  <code>complete_system_matrix</code>   and   <code>complete_system_rhs</code>  ) as well as the active set itself and the diagonal of the mass matrix   $B$   used in scaling Lagrange multipliers in the active set formulation. The rest is as in   step-4  :
* 
...
* 
*  In the following, we define classes that describe the right hand side function, the Dirichlet boundary values, and the height of the obstacle as a function of   $\mathbf x$  . In all three cases, we derive these classes from Function  @<dim@>,   although in the case of   <code>RightHandSide</code>   and   <code>Obstacle</code>   this is more out of convention than necessity since we never pass such objects to the library. In any case, the definition of the right hand side and boundary values classes is obvious given our choice of   $f=-10$  ,   $u|_{\partial\Omega}=0$  :
* 
