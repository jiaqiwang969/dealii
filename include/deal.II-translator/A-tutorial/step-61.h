...
* Weak Galerkin Finite Element Methods (WGFEMs) use discrete weak functionsto approximate scalar unknowns, and discrete weak gradients toapproximate classical gradients.The method was originally introduced by Junping Wang and Xiu Yein the paper<a href="https://doi.org/10.1016/j.cam.2012.10.003">
...
@f}
* where   $\Omega \subset \mathbb{R}^n (n=2,3)$   is a bounded domain.In the context of the flow of a fluid through a porous medium,  $p$   is the pressure,   $\mathbf{K}$   is a permeability tensor,  $f$   is the source term, and  $p_D, u_N$   represent Dirichlet and Neumann boundary conditions.We can introduce a flux,   $\mathbf{u} =
* 
-\mathbf{K} \nabla p$  , that correspondsto the Darcy velocity (in the way we did in   step-20  ) and this variable willbe important in the considerations below.
* In this program, we will consider a test case where the exact pressureis   $p = \sin \left( \pi x\right)\sin\left(\pi y \right)$   on the unit square domain,with homogeneous Dirichelet boundary conditions and   $\mathbf{K}$   the identity matrix.Then we will calculate   $L_2$   errors of pressure, velocity, and flux.
* 

* <a name="WeakGalerkinscheme"></a><h3> Weak Galerkin scheme </h3>
* 

* The Poisson equation above has a solution   $p$   that needs to satisfy the weakformulation of the problem,@f{equation*}
\mathcal{A}\left(p,q \right) = \mathcal{F} \left(q \right),
@f}
* for all test functions   $q$  , where@f{equation*}
...
@f}
* The key point is that here, we have replaced the gradient   $\nabla p_h$   by the<i>discrete weak gradient</i> operator  $\nabla_{w,d} p_h$   that makes sense for our peculiarly defined approximation   $p_h$  .
* The question is then how that operator works. For this, let us first say how wethink of the discrete approximation   $p_h$   of the pressure. As mentioned above,the "function"   $p_h$   actually consists of two parts: the values   $p_h^\circ$   inthe interior of cells, and   $p_h^\partial$   on the interfaces. We have to definediscrete (finite-dimensional) function spaces for both of these; in thisprogram, we will use FE_DGQ for   $p_h^\circ$   as the space in the interior ofcells (defined on each cell, but in general discontinuous along interfaces),and FE_FaceQ for   $p_h^\partial$   as the space on the interfaces.
* Then let us consider just a single cell (because the integrals above are alldefined cell-wise, and because the weak discrete gradient is defined cell-by-cell).The restriction of   $p_h$   to cell   $K$  ,   $p_h|_K$   then consistsof the pair   $(p_h^\circ|_K,p_h^\partial|_{\partial K})$  . In essence, we canthink of   $\nabla_{w,d} p_h$   of some function defined on   $K$   that approximatesthe gradient; in particular, if   $p_h|_K$   was the restriction of a differentiablefunction (to the interior and boundary of   $K$  
* 
*  -  which would make it continuousbetween the interior and boundary), then  $\nabla_{w,d} p_h$   would simply be the exact gradient   $\nabla p_h$  . But, since  $p_h|_K$   is not continuous between interior and boundary of   $K$  , we need a moregeneral definition; furthermore, we can not deal with arbitrary functions, andso require that   $\nabla_{w,d} p_h$   is also in a finite element space (which, sincethe gradient is a vector, has to be vector-valued, and because the weak gradientis defined on each cell separately, will also be discontinuous between cells).
* The way this is done is to define this weak gradient operator   $\nabla_{w,d}|_K :
DGQ_k(K) \times DGQ_r(\partial K) \rightarrow RT_s(K)$   (where   $RT_s(K)$   is thevector-valued Raviart-Thomas space of order   $s$   on cell   $K$  ) in the following way:@f{equation*}{
...
*   @note   deal.II happens to have an implementation of this broken Raviart-Thomas  space: The FE_DGRT class. As a consequence, in this tutorial we will simply  always use the FE_DGRT class, even though in all of those places where  we have to compute cell-local matrices and vectors, it makes no difference.
* 

* <a name="Representingtheweakgradient"></a><h3> Representing the weak gradient </h3>
* 

* Since   $p_h$   is an element of a finite element space, we can expand it in a basisas we always do, i.e., we can write@f{equation*}{
  p_h(\mathbf x) = \sum_j P_j \varphi_j(\mathbf x).
@f}
* Here, since   $p_h$   has two components (the interior and the interface components),the same must hold true for the basis functions   $\varphi_j(\mathbf x)$  , which wecan write as   $\varphi_j = (\varphi_j^\circ,\varphi_j^\partial)$  . If you'vefollowed the descriptions in   step-8  ,   step-20  , and the  @ref vector_valued   "documentation module on vector-valued problems",it will be no surprise that for some values of   $j$  ,   $\varphi_j^\circ$   will bezero, whereas for other values of   $j$  ,   $\varphi_j^\partial$   will be zero
* 
*  -  i.e.,shape functions will be of either one or the other kind. That is not important,here, however. What is important is that we need to wonder how we can represent  $\nabla_{w,d} \varphi_j$   because that is clearly what will appear in theproblem when we want to implement the bilinear form@f{equation*}
\mathcal{A}_h\left(p_h,q_h\right)
  = \sum_{K \in \mathbb{T}}
    \int_K \mathbf{K} \nabla_{w,d} p_h \cdot \nabla_{w,d} q_h \;\mathrm{d}x,
@f}
* 
* The key point is that   $\nabla_{w,d} \varphi_j$   is known to be a member of the"broken" Raviart-Thomas space   $DGRT_s$  . What this means is that we canrepresent (on each cell   $K$   separately)@f{equation*}
\nabla_{w,d} \varphi_j|_K
  = \sum_k C_{jk}^K \mathbf v_k|_K
@f}
* where the functions   $\mathbf v_k \in DGRT_s$  , and where   $C^K$   is a matrix ofdimension
* @f{align*}{
...
@f}
* (That the weak discrete gradient can be represented as a matrix should not comeas a surprise: It is a linear operator from one finite dimensionalspace to another finite dimensional space. If one chooses basesfor both of these spaces, then <i>every linear operator</i> canof course be written as a matrix mapping the vector of expansion coefficientswith regards to the basis of the domain space of the operator, tothe vector of expansion coefficients with regards to the basis in the imagespace.)
* Using this expansion, we can easily use the definition of the weakdiscrete gradient above to define what the matrix is going to be:@f{equation*}{
...
* This clearly leads to a linear system of the form@f{equation*}{
...
-\int_K (\nabla \cdot \mathbf v_i) \varphi_j^\circ
             +\int_{\partial K} (\mathbf v_i \cdot \mathbf n) \varphi_j^\partial,
@f}
* and consequently@f{equation*}{
  \left(C^K\right)^T = \left(M^K\right)^{-1} G^K.
@f}
* (In this last step, we have assumed that the indices   $i,j,k$   only rangeover those degrees of freedom active on cell   $K$  ,thereby ensuring that the mass matrix on the space   $RT_s(K)$   is invertible.)Equivalently, using the symmetry of the matrix   $M$  , we have that@f{equation*}{
  C^K = \left(G^K\right)^{T} \left(M^K\right)^{-1}.
@f}
* Also worth pointing out is that thematrices   $C^K$   and   $G^K$   are of course not square but rectangular.
* 

* <a name="Assemblingthelinearsystem"></a><h3> Assembling the linear system </h3>
* 

* Having explained how the weak discrete gradient is defined, we can nowcome back to the question of how the linear system for the equation in questionshould be assembled. Specifically, using the definition of the bilinearform   ${\cal A}_h$   shown above, we then need to compute the elements of thelocal contribution to the global matrix,@f{equation*}{
  A^K_{ij} = \int_K \left({\mathbf K} \nabla_{w,d} \varphi_i\right) \cdot \nabla_{w,d} \varphi_j.
@f}
* As explained above, we can expand   $\nabla_{w,d} \varphi_i$   in terms of theRaviart-Thomas basis on each cell, and similarly for   $\nabla_{w,d} \varphi_j$  :@f{equation*}{
  A^K_{ij} = \int_K
    \left(
      {\mathbf K}
...
@f}
* By re-arranging sums, this yields the following expression:@f{equation*}{
  A^K_{ij} =
    ...
    \mathbf v_l|_K.
@f}
* So, if we have the matrix   $C^K$   for each cell   $K$  , then we can easily computethe contribution   $A^K$   for cell   $K$   to the matrix   $A$   as follows:@f{equation*}{
  A^K_{ij} =
    \sum_k \sum_l C_{ik}^K C_{jl}^K
    H^K_{kl}
    =
...
* Here,@f{equation*}{
  ...
  \int_K
    ...
    \mathbf v_l|_K,
@f}
* which is really just the mass matrix on cell   $K$   using the Raviart-Thomasbasis and weighting by the permeability tensor   $\mathbf K$  . The derivationhere then shows that the weak Galerkin method really just requires usto compute these   $C^K$   and   $H^K$   matrices on each cell   $K$  , and then  $A^K = C^K H^K (C^K)^T$  , which is easily computed. The code to be shownbelow does exactly this.
* Having so computed the contribution   $A^K$   of cell   $K$   to the globalmatrix, all we have to do is to "distribute" these local contributionsinto the global matrix. How this is done is first shown in   step-3   and  step-4  . In the current program, this will be facilitated by calling  AffineConstraints::distribute_local_to_global().  
* A linear system of course also needs a right hand side. There is no difficultyassociated with computing the right hand side here other than the factthat we only need to use the cell-interior part   $\varphi_i^\circ$   foreach shape function   $\varphi_i$  .
* 

* <a name="PostprocessingandiLsub2subierrors"></a><h3> Post-processing and <i>L<sub>2</sub></i><i>L<sub>2</sub></i>-errors </h3>
* 

* The discussions in the previous sections have given us a linearsystem that we can solve for the numerical pressure   $p_h$  . We can usethis to compute an approximation to the variable   $\mathbf u =
* 
-{\mathbf K}\nabla p$  that corresponds to the velocity with which the medium flows in a porousmedium if this is the model we are trying to solve. This kind ofstep
* 
*  -  computing a derived quantity from the solution of the discreteproblem
* 
*  -  is typically called "post-processing".
* Here, instead of using the exact gradient of   $p_h$  , let us insteaduse the discrete weak gradient of   $p_h$   to calculate the velocity on each element.As discussed above,on each element the gradient of the numerical pressure   $\nabla p$   can beapproximated by discrete weak gradients    $ \nabla_{w,d}\phi_i$  :@f{equation*}
\nabla_{w,d} p_h
= \nabla_{w,d} \left(\sum_{i} P_i \phi_i\right)
= \sum_{i} P_i \nabla_{w,d}\phi_i.
@f}
* 
* On cell   $K$  ,the numerical velocity   $ \mathbf{u}_h =
* 
...
* Unfortunately,   $\mathbf{K} \mathbf{v}_j$   may not be in the   $RT$   space(unless, of course, if   $\mathbf K$   is constant times the identity matrix).So, in order to represent it in a finite element program, we need toproject it back into a finite dimensional space we can work with. Here,we will use the   $L_2$  -projection to project it back to the (broken)   $RT$  space.
...
\left( \mathbf{Kv}_j,\mathbf{v}_k \right)_K.$  So, rather than the formula shown above, the numerical velocity on cell   $K$  instead becomes@f{equation*}
\mathbf{u}_h = \mathbf{Q}_h \left(
* 
-\mathbf{K}\nabla_{w,d}p_h \right) =
* 
-\sum_i \sum_j P_i B^K_{ij}\mathbf{Q}_h \left( \mathbf{K}\mathbf{v}_j \right),
@f}
* and we have the following system to solve for the coefficients   $d_{jk}$  :@f{equation*}
 \sum_j
...
* In the implementation below, the matrix with elements  $
   d_{jk}
$  is called   <code>cell_matrix_D</code>  ,whereas the matrix with elements  $
      \left( \mathbf{Kv}_j,\mathbf{v}_k \right)
$  is called   <code>cell_matrix_E</code>  .
* Then the elementwise velocity is@f{equation*}
\mathbf{u}_h =
* 
-\sum_{i} \sum_{j}P_ic_{ij}\sum_{k}d_{jk}\mathbf{v}_k =
...
* Using this velocity obtained by "postprocessing" the solution, we candefine the   $L_2$  -errors of pressure, velocity, and fluxby the following formulas:
* @f{align*}{
\|p-p_h^\circ\|^2
...
\|(\mathbf{u}-\mathbf{u}_h) \cdot \mathbf{n}\|^2
  &= \sum_{K \in \mathbb{T}} \sum_{\gamma \subset \partial K}
    \frac{|K|}{|\gamma|} \|\mathbf{u} \cdot \mathbf{n}
* 
- \mathbf{u}_h \cdot \mathbf{n}\|_{L_2(\gamma)}^2,
@f}
* where   $| K |$   is the area of the element,  $\gamma$   are faces of the element,  $\mathbf{n}$   are unit normal vectors of each face. The last of thesenorms measures the accuracy of the normal component of the velocityvectors over the interfaces between the cells of the mesh. The scalingfactor   $|K|/|\gamma|$   is chosen so as to scale out the difference inthe length (or area) of the collection of interfaces as the mesh sizechanges.
* The first of these errors above is easily computed using  VectorTools::integrate_difference.   The others require a bit more workand are implemented in the code below.
* 

*  <a name="CommProg"></a> <h1> The commented program</h1>
*   <a name="Includefiles"></a>  <h3>Include files</h3> This program is based on   step-7  ,   step-20   and   step-51  , so most of the following header files are familiar. We need the following, of which only the one that imports the FE_DGRaviartThomas class (namely, `deal.II/fe/fe_dg_vector.h`) is really new; the FE_DGRaviartThomas implements the "broken" Raviart-Thomas space discussed in the introduction:
* 
...
* 
 @endcode
* 
*   <a name="TheWGDarcyEquationclasstemplate"></a>  <h3>The WGDarcyEquation class template</h3>
* 
...
* 
   ...
   Tensor<1, dim> ExactVelocity<dim>::value(const Point<dim> &p) const
   {
     ...
     return_value[0] =
* 
-numbers::PI std::cos(numbers::PI p[0])
                       std::sin(numbers::PI p[1]);
     return_value[1] =
* 
-numbers::PI std::sin(numbers::PI p[0])
...
* 
 @endcode
* 
*   <a name="WGDarcyEquationclassimplementation"></a>  <h3>WGDarcyEquation class implementation</h3>
* 

* 
*   <a name="WGDarcyEquationWGDarcyEquation"></a>  <h4>WGDarcyEquation::WGDarcyEquation</h4>
* 
...
* 
 @endcode
* 
*   <a name="WGDarcyEquationmake_grid"></a>  <h4>WGDarcyEquation::make_grid</h4>
* 
...
* 
 @endcode
* 
*   <a name="WGDarcyEquationsetup_system"></a>  <h4>WGDarcyEquation::setup_system</h4>
* 
...
* 
 ...
 @endcode
* 
*  In the bilinear form, there is no integration term over faces between two neighboring cells, so we can just use   <code>DoFTools::make_sparsity_pattern</code>   to calculate the sparse matrix.
* 
...
* 
 @endcode
* 
*   <a name="WGDarcyEquationassemble_system"></a>  <h4>WGDarcyEquation::assemble_system</h4>
* 
...
* @code
   ...
   void WGDarcyEquation<dim>::assemble_system()
   {
     ...
     const QGauss<dim
* 
- 1> face_quadrature_formula(fe_dgrt.degree + 1);
* 
...
* 
 @endcode
* 
*  We need   <code>FEValuesExtractors</code>   to access the   @p interior   and   @p face   component of the shape functions.
* 
...
* @code
 ...
 @endcode
* 
*  Next we take the inverse of this matrix by using   FullMatrix::gauss_jordan().   It will be used to calculate the coefficient matrix   $C^K$   later. It is worth recalling later that `cell_matrix_M` actually contains theinverse*
...
* 
 @endcode
* 
*    @p cell_matrix_C   is then the matrix product between the transpose of   $G^K$   and the inverse of the mass matrix (where this inverse is stored in   @p cell_matrix_M):  
* 
...
* 
 @endcode
* 
*   <a name="WGDarcyEquationdimsolve"></a>  <h4>WGDarcyEquation<dim>::solve</h4>
* 
...
* 
 ...
 @endcode
* 
*   <a name="WGDarcyEquationdimcompute_postprocessed_velocity"></a>  <h4>WGDarcyEquation<dim>::compute_postprocessed_velocity</h4>
* 
...
* 
 @endcode
* 
*  The component of this   <code>cell_matrix_E</code>   is the integral of   $(\mathbf{K} \mathbf{w}, \mathbf{w})$  .   <code>cell_matrix_M</code>   is the Gram matrix.
* 
...
* 
 @endcode
* 
*   <a name="WGDarcyEquationdimcompute_pressure_error"></a>  <h4>WGDarcyEquation<dim>::compute_pressure_error</h4>
* 
...
* 
 @endcode
* 
*   <a name="WGDarcyEquationdimcompute_velocity_error"></a>  <h4>WGDarcyEquation<dim>::compute_velocity_error</h4>
* 
...
* 
 ...
 @endcode
* 
*   <a name="WGDarcyEquationoutput_results"></a>  <h4>WGDarcyEquation::output_results</h4>
* 

* 
*  We have two sets of results to output: the interior solution and the skeleton solution. We use   <code>DataOut</code>   to visualize interior results. The graphical output for the skeleton results is done by using the DataOutFaces class.   
...
* 
 ...
 @endcode
* 
*   <a name="WGDarcyEquationrun"></a>  <h4>WGDarcyEquation::run</h4>
* 
...
* We run the program with a right hand side that will produce thesolution   $p = \sin(\pi x) \sin(\pi y)$   and with homogeneous Dirichletboundary conditions in the domain   $\Omega = (0,1)^2$  . In addition, wechoose the coefficient matrix in the differential operator  $\mathbf{K}$   as the identity matrix. We test this setup using  $\mbox{WG}(Q_0,Q_0;RT_{[0]})$  ,   $\mbox{WG}(Q_1,Q_1;RT_{[1]})$   and  $\mbox{WG}(Q_2,Q_2;RT_{[2]})$   element combinations, which one canselect by using the appropriate constructor argument for the`WGDarcyEquation` object in `main()`. We will then visualize pressurevalues in interiors of cells and on faces. We want to see that thepressure maximum is around 1 and the minimum is around 0. With meshrefinement, the convergence rates of pressure, velocity and fluxshould then be around 1 for   $\mbox{WG}(Q_0,Q_0;RT_{[0]})$   , 2 for  $\mbox{WG}(Q_1,Q_1;RT_{[1]})$  , and 3 for  $\mbox{WG}(Q_2,Q_2;RT_{[2]})$  .
* 

* <a name="TestresultsoniWGQsub0subQsub0subRTsub0subi"></a><h3>Test results on <i>WG(Q<sub>0</sub>,Q<sub>0</sub>;RT<sub>[0]</sub>)</i><i>WG(Q<sub>0</sub>,Q<sub>0</sub>;RT<sub>[0]</sub>)</i></h3>
* 

* The following figures show interior pressures and face pressures using the  $\mbox{WG}(Q_0,Q_0;RT_{[0]})$   element. The mesh is refined 2 times (top)and 4 times (bottom), respectively. (This number can be adjusted in the`make_grid()` function.) When the mesh is coarse, one can seethe face pressures   $p^\partial$   neatly between the values of the interiorpressures   $p^\circ$   on the two adjacent cells.
*   <table align="center">
  <tr>
...
* <a name="TestresultsoniWGQsub1subQsub1subRTsub1subi"></a><h3>Test results on <i>WG(Q<sub>1</sub>,Q<sub>1</sub>;RT<sub>[1]</sub>)</i><i>WG(Q<sub>1</sub>,Q<sub>1</sub>;RT<sub>[1]</sub>)</i></h3>
* 
...
* Compared to the corresponding image for the   $\mbox{WG}(Q_0,Q_0;RT_{[0]})$  combination, the solution is now substantially more accurate and, inparticular so close to being continuous at the interfaces that we canno longer distinguish the interface pressures   $p^\partial$   from theinterior pressures   $p^\circ$   on the adjacent cells.
* <a name="Convergencetableforik1i"></a><h4>Convergence table for <i>k=1</i><i>k=1</i></h4>
* 

* The following are the convergence rates of pressure, velocity, and fluxwe obtain from using the   $\mbox{WG}(Q_1,Q_1;RT_{[1]})$   element combination:
*   <table align="center" class="doxtable">
  <tr>
   <th>number of refinements </th><th>  $\|p-p_h^\circ\|$  </th><th>  $\|\mathbf{u}-\mathbf{u}_h\|$ </th><th> $\|(\mathbf{u}-\mathbf{u}_h) \cdot \mathbf{n}\|$ </th>
  </tr>
...
* 
* <a name="TestresultsoniWGQsub2subQsub2subRTsub2subi"></a><h3>Test results on <i>WG(Q<sub>2</sub>,Q<sub>2</sub>;RT<sub>[2]</sub>)</i><i>WG(Q<sub>2</sub>,Q<sub>2</sub>;RT<sub>[2]</sub>)</i></h3>
* 

* Let us go one polynomial degree higher.The following are interior pressures and face pressures implemented using  $WG(Q_2,Q_2;RT_{[2]})$  , with mesh size   $h = 1/32$   (i.e., 5 global meshrefinement steps). In the program, we use`data_out_face.build_patches(fe.degree)` when generating graphical output(see the documentation of   DataOut::build_patches()),   which here implies thatwe divide each 2d cell interior into 4 subcells in order to provide a bettervisualization of the quadratic polynomials.  <table align="center">
  <tr>
...
* <a name="Convergencetableforik2i"></a><h4>Convergence table for <i>k=2</i><i>k=2</i></h4>
* 

* As before, we can generate convergence data for the  $L_2$   errors of pressure, velocity, and fluxusing the   $\mbox{WG}(Q_2,Q_2;RT_{[2]})$   combination:
*   <table align="center" class="doxtable">
  <tr>
   <th>number of refinements </th><th>  $\|p-p_h^\circ\|$  </th><th>  $\|\mathbf{u}-\mathbf{u}_h\|$ </th><th> $\|(\mathbf{u}-\mathbf{u}_h) \cdot \mathbf{n}\|$ </th>
  </tr>
...
* Once more, the convergence rates of   $\mbox{WG}(Q_2,Q_2;RT_{[2]})$   isas expected, with values around 3.
* 
