...
\f}where the fluid density   $\rho$   and the acceleration   $\mathbf{g}$   dueto external forces are given and the velocity   $\mathbf{v}$   and thepressure   $p$   are the unknowns. Here   $\Omega$   is a closed boundedregion representing the body around which the fluid moves.
...
* For the purpose of this tutorial program, we will consider only stationaryflow without external forces:\f{align*}
  (\mathbf{v}\cdot\nabla)\mathbf{v}
  &=
* 
...
* Uniqueness of the solution of the Euler equations is ensured by adding theboundary conditions\f[
...
\mathbf{v}_\infty=0$  ) and we have boundary conditions\f[
...
\mathbf{v}=0$   in   $\mathbb{R}^n\backslash\Omega$  , we can represent thevelocity, and consequently also the perturbation velocity, as thegradient of a scalar function:\f[
  \mathbf{\tilde{v}}=\nabla\phi,
\f]and so the second part of Euler equations above can be rewrittenas the homogeneous Laplace equation for the unknown   $\phi$  :\f{align*}
\label{laplace}
\Delta\phi &= 0 \qquad &&\text{in}\ \mathbb{R}^n\backslash\Omega,
	   \\
	   \mathbf{n}\cdot\nabla\phi &=
* 
-\mathbf{n}\cdot\mathbf{v}_\infty
	   && \text{on}\ \partial\Omega
\f}while the momentum equation reduces to Bernoulli's equation that expresses thepressure   $p$   as a function of the potential   $\phi$  :\f[
...
\f]
* where the derivative is done in the variable   $\mathbf{y}$  . By usingthe usual Green identities, our problem can be written on the boundary  $\partial\Omega = \Gamma$   only. We recall the general definition ofthe second Green %identity:
* \f[\label{green}
  \int_{\omega}
...
\f]
* where   $\tilde{\mathbf{n}}$   is the normal to the surface of   $\omega$   pointingoutwards from the domain of integration   $\omega$  .
...
\f]
* In our program the normals are defined as <i>outer</i> to the domain  $\Omega$  , that is, they are in fact <i>inner</i> to the integrationdomain, and some care is required in defining the various integralswith the correct signs for the normals, i.e. replacing   $\tilde{\mathbf{n}}$  by   $-\mathbf{n}$  .
* If we substitute   $u$   and   $v$   in the Green %identity with the solution  $\phi$   and with the fundamental solution of the Laplace equationrespectively, as long as   $\mathbf{x}$   is chosen in the region  $\mathbb{R}^n\backslash\Omega$  , we obtain:\f[
  \phi(\mathbf{x})
* 
-
...
\f]
* where the normals are now pointing <i>inward</i> the domain ofintegration.
* Notice that in the above equation, we also have the integrals on theportion of the boundary at   $\Gamma_\infty$  . Using the boundaryconditions of our problem, we have that   $\nabla \phi$   is zero atinfinity (which simplifies the integral on   $\Gamma_\infty$   on theright hand side).
* The integral on   $\Gamma_\infty$   that appears on the left hand side canbe treated by observing that   $\nabla\phi=0$   implies that   $\phi$   atinfinity is necessarily constant. We define its value to be  $\phi_\infty$  .  It is an easy exercise to prove that
* \f[
* 
...
-\phi_\infty.
\f]
* Using this result, we can reduce the above equation only on theboundary   $\Gamma$   using the so-called Single and Double LayerPotential operators:
* \f[\label{integral}
  \phi(\mathbf{x})
* 
...
-\left(S \frac{\partial \phi}{\partial n_y}\right)(\mathbf{x})
  \qquad \forall\mathbf{x}\in \mathbb{R}^n\backslash\Omega.
\f]
* (The name of these operators comes from the fact that they describe theelectric potential in   $\mathbb{R}^n$   due to a single thin sheet of chargesalong a surface, and due to a double sheet of charges and anti-charges alongthe surface, respectively.)
* In our case, we know the Neumann values of   $\phi$   on the boundary:  $\mathbf{n}\cdot\nabla\phi =
* 
-\mathbf{n}\cdot\mathbf{v}_\infty$  .Consequently,\f[
  \phi(\mathbf{x})
* 
- (D\phi)(\mathbf{x}) = \phi_\infty +
   \left(S[\mathbf{n}\cdot\mathbf{v}_\infty]\right)(\mathbf{x})
   \qquad \forall\mathbf{x} \in \mathbb{R}^n\backslash\Omega.
\f]If we take the limit for   $\mathbf{x}$   tending to   $\Gamma$   ofthe above equation, using well known properties of the single and double layeroperators, we obtain an equation for   $\phi$   just on the boundary   $\Gamma$   of  $\Omega$  :
* \f[\label{SD}
...
\f]
* which is the Boundary Integral Equation (BIE) we were looking for,where the quantity   $\alpha(\mathbf{x})$   is the fraction of angle orsolid angle by which the point   $\mathbf{x}$   sees the domain ofintegration   $\mathbb{R}^n\backslash\Omega$  .
* In particular, at points   $\mathbf{x}$   where the boundary  $\partial\Omega$   is differentiable (i.e. smooth) we have  $\alpha(\mathbf{x})=\frac 12$  , but the value may be smaller or largerat points where the boundary has a corner or an edge.
...
\f]for two dimensional flows and\f[
  \alpha(\mathbf{x}) \phi(\mathbf{x})
...
\f]for three dimensional flows, where the normal derivatives of the fundamentalsolutions have been written in a form that makes computation easier. In eithercase,   $\phi$   is the solution of an integral equation posed entirely on theboundary since both   $\mathbf{x},\mathbf{y}\in\partial\Omega$  .
* Notice that the fraction of angle (in 2d) or solid angle (in 3d)  $\alpha(\mathbf{x})$   by which the point   $\mathbf{x}$   sees the domain  $\Omega$   can be defined using the double layer potential itself:\f[
...
\right]
@f}
* The integral on   $\Gamma_\infty$   is unity, see above, so division by the constant   $\phi_\infty$   gives us the explicitexpression above for   $\alpha(\mathbf{x})$  .
* While this example program is really only focused on the solution of theboundary integral equation, in a realistic setup one would still need to solvefor the velocities. To this end, note that we have just computed  $\phi(\mathbf{x})$   for all   $\mathbf{x}\in\partial\Omega$  . In the next step, wecan compute (analytically, if we want) the solution   $\phi(\mathbf{x})$   in allof   $\mathbb{R}^n\backslash\Omega$  . To this end, recall that we had\f[
  \phi(\mathbf{x})
  =
  \phi_\infty +
  (D\phi)(\mathbf{x})
  ...
  \left(S[\mathbf{n}\cdot\mathbf{v}_\infty]\right)(\mathbf{x})
  \qquad \forall\mathbf{x}\in \mathbb{R}^n\backslash\Omega.
\f]where now we have everything that is on the right hand side (  $S$   and   $D$   areintegrals we can evaluate, the normal velocity on the boundary is given, and  $\phi$   on the boundary we have just computed). Finally, we can then recoverthe velocity as   $\mathbf{\tilde v}=\nabla \phi$  .
* Notice that the evaluation of the above formula for   $\mathbf{x} \in
\Omega$   should yield zero as a result, since the integration of theDirac delta   $\delta(\mathbf{x})$   in the domain  $\mathbb{R}^n\backslash\Omega$   is always zero by definition.
* As a final test, let us verify that this velocity indeed satisfies themomentum balance equation for a stationary flow field, i.e., whether  $\mathbf{v}\cdot\nabla\mathbf{v} =
* 
-\frac 1\rho \nabla p$   where  $\mathbf{v}=\mathbf{\tilde
v}+\mathbf{v}_\infty=\nabla\phi+\mathbf{v}_\infty$   for some (unknown) pressure  $p$   and a given constant   $\rho$  . In other words, we would like to verify thatBernoulli's law as stated above indeed holds. To show this, we use thatthe left hand side of this equation equates to
* @f{align*}
  \mathbf{v}\cdot\nabla\mathbf{v}
  &=
  ...
  [(\nabla\phi+\mathbf{v}_\infty)\cdot\nabla] (\nabla\phi)
@f}
* where we have used that   $\mathbf{v}_\infty$   is constant. We would like towrite this expression as the gradient of something (remember that   $\rho$   is aconstant). The next step is moreconvenient if we consider the components of the equation individually(summation over indices that appear twice is implied):
* @f{align*}
  [\mathbf{v}\cdot\nabla\mathbf{v}]_i
  &=
...
-
  \partial_i [v_{\infty,j}] \partial_j \phi
@f}
* Again, the last term disappears because   $\mathbf{v}_\infty$   is constant and wecan merge the first and third term into one:
* @f{align*}
  [\mathbf{v}\cdot\nabla\mathbf{v}]_i
  &=
...
\partial_i [(\partial_j\phi)(\partial_j\phi)]$   which is in the desired gradientform. As a consequence, we can now finally state that
* @f{align*}
  [\mathbf{v}\cdot\nabla\mathbf{v}]_i
  &=
  \partial_i (\partial_j [(\partial_j\phi) \phi + v_{\infty,j} \partial_j \phi])
...
-\frac 12 \rho \mathbf{v}^2
  .
@f]This is exactly Bernoulli's law mentioned above.
* 
...
- \int_{\Gamma_y} \frac{ \partial G(\mathbf{y}-\mathbf{x}_i)}{\partial\mathbf{n}_y }
    ...
    ,
\f}
* where the quantity   $\alpha(\mathbf{x}_i)$   is the fraction of (solid)angle by which the point   $\mathbf{x}_i$   sees the domain   $\Omega$  , asexplained above, and we set   $\phi_\infty$   to be zero.  If the supportpoints   $\mathbf{x}_i$   are chosen appropriately, then the problem canbe written as the following linear system:
* \f[
...
* In two dimensions it is not necessary to compute the diagonal elements  $\mathbf{N}_{ii}$   of the system matrix, since, even if the denominatorgoes to zero when   $\mathbf{x}=\mathbf{y}$  , the numerator is alwayszero because   $\mathbf{n}_y$   and   $(\mathbf{y}-\mathbf{x})$   areorthogonal (on our polygonal approximation of the boundary of   $\Omega$  ), andthe only singular integral arises in the computationof   $\mathbf{b}_i$   on the i-th element of   $\mathcal{T}_h$  :\f[
  \frac{1}{\pi}
  \int_{K_i}
...
* The resulting matrix   $\mathbf{A}+\mathbf{N}$   is full. Depending on itssize, it might be convenient to use a direct solver or an iterativeone. For the purpose of this example code, we chose to use only aniterative solver, without providing any preconditioner.
...
* The implementation is rather straight forward. The main point that hasn't beenused in any of the previous tutorial programs is that most classes in deal.IIare not only templated on the dimension, but in fact on the dimension of themanifold on which we pose the differential equation as well as the dimensionof the space into which this manifold is embedded. By default, the secondtemplate argument equals the first, meaning for example that we want to solveon a two-dimensional region of two-dimensional space. The triangulation classto use in this case would be   <code>Triangulation@<2@></code>  , which is anequivalent way of writing   <code>Triangulation@<2,2@></code>  .
* However, this doesn't have to be so: in the current example, we will forexample want to solve on the surface of a sphere, which is a two-dimensionalmanifold embedded in a three-dimensional space. Consequently, the right classwill be   <code>Triangulation@<2,3@></code>  , and correspondingly we will use  <code>DoFHandler@<2,3@></code>   as the DoF handler class and  <code>FE_Q@<2,3@></code>   for finite elements.
* Some further details on what one can do with things that live oncurved manifolds can be found in the report<a target="_top"
...
Library</i><i>Tools
for the Solution of PDEs Defined on Curved Manifolds with the deal.II
Library</i> by A. DeSimone, L. Heltai, C. Manigrasso</a>. In addition, the  step-38   tutorial program extends what we show here to cases where the equationposed on the manifold is not an integral operator but in fact involvesderivatives.
* 
...
@f}
* see, e.g. J. N. Newman, <i>Marine Hydrodynamics</i>, 1977,pp. 127. For unit speed and radius, and restricting   $(x,y,z)$   to lieon the surface of the sphere,  $\phi =
* 
-x/2$  . In the test problem,the flow is   $(1,1,1)$  , so the appropriate exact solution on thesurface of the sphere is the superposition of the above solution withthe analogous solution along the   $y$   and   $z$   axes, or   $\phi =
\frac{1}{2}(x + y + z)$  .
* 
...
* 
 ...
 @endcode
* 
*  The call to   Triangulation::set_manifold   copies the manifold (via   Manifold::clone()),   so we do not need to worry about invalid pointers to   <code>manifold</code>  :
* 
...
*   We use this second option, through the   @p factor_out_singularity   parameter of both QGaussLogR and QGaussOneOverR.   
...
* We ran the program using the following   <code>parameters.prm</code>   file (whichcan also be found in the directory in which all the other source files are):
* @verbatim
...
@endverbatim
* 
* As we can see from the convergence table in 2d, if we choosequadrature formulas which are accurate enough, then the error weobtain for   $\alpha(\mathbf{x})$   should be exactly the inverse of thenumber of elements. The approximation of the circle with N segments ofequal size generates a regular polygon with N faces, whose angles areexactly   $\pi-\frac {2\pi}{N}$  , therefore the error we commit should beexactly   $\frac 12
* 
- (\frac 12
* 
...
* The error in the approximation of the potential   $\phi$   is largely dueto approximation of the domain. A much better approximation could beobtained by using higher order mappings.
...
* This is the first tutorial program that considers solving equations defined onsurfaces embedded in higher dimensional spaces. But the equation discussedhere was relatively simple because it only involved an integral operator, notderivatives which are more difficult to define on the surface. The   step-38  tutorial program considers such problems and provides the necessary tools.
* From a practical perspective, the Boundary Element Method (BEM) usedhere suffers from two bottlenecks. The first is that assembling thematrix has a cost that isquadratic* in the number of unknowns, thatis   ${\cal O}(N^2)$   where   $N$   is the total number of unknowns. This canbe seen by looking at the `assemble_system()` function, which has thisstructure:
* @code
    for (const auto &cell : dof_handler.active_cell_iterators())
      {
        ...
* 
        for (unsigned int i = 0; i < dof_handler.n_dofs(); ++i)
          ...
@endcode
* Here, the first loop walks over all cells (one factor of   $N$  ) whereasthe inner loop contributes another factor of   $N$  .
* This has to be contrasted with the finite element method forlocal*
differential operators: There, we loop over all cells (one factor of  $N$  ) and on each cell do an amount of work that is independent of howmany cells or unknowns there are. This clearly presents abottleneck.
* The second bottleneck is that the system matrix is dense (i.e., is oftype FullMatrix) because every degree of freedom couples with everyother degree of freedom. As pointed out above, justcomputing* thismatrix with its   $N^2$   nonzero entries necessarily requires at least  ${\cal O}(N^2)$   operations, but it's worth pointing out that it alsocosts this many operations to just do one matrix-vector product. Ifthe GMRES method used to solve the linear system requires a number ofiterations that grows with the size of the problem, as is typicallythe case, then solving the linear system will require a number ofoperations that grows even faster than just   ${\cal O}(N^2)$  .
* "Real" boundary element methods address these issues by strategiesthat determine which entries of the matrix will be small and canconsequently be neglected (at the cost of introducing an additionalerror, of course). This is possible by recognizing that the matrixentries decay with the (physical) distance between the locations wheredegrees of freedom   $i$   and   $j$   are defined. This can be exploited inmethods such as the Fast Multipole Method (FMM) that control whichmatrix entries must be stored and computed to achieve a certainaccuracy, and
* 
*  -  if done right
* 
