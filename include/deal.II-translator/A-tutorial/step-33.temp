...
href="http://trilinos.org">Trilinos</a> linear solvers (these can be foundin Trilinos in the Aztec/Amesos packages) and an automaticdifferentiation package, Sacado, also part of Trilinos. deal.II mustbe configured to use Trilinos. Refer to the <a
href="../../readme.html#trilinos">ReadMe</a> file for instructions how todo this.
*   @note   While this program demonstrates the use of automatic differentiationwell, it does not express the state of the art in Euler equation solvers.There are much faster and more accurate method for this equation, andyou should take a look at   step-67   and   step-69   to see how this equationcan be solved more efficiently.
* 
...
E)^{\top}$   consisting of   $\rho$   the fluid density,   ${\mathbf v}=(v_1,\ldots v_d)^T$   theflow velocity (and thus   $\rho\mathbf v$   being the linear momentumdensity), and  $E$   the energy density of the gas. We interpret the equations above as  $\partial_t \mathbf{w}_i + \nabla \cdot \mathbf{F}_i(\mathbf{w}) = \mathbf
...
(\gamma
* 
-1)(E-\frac{1}{2} \rho |\mathbf v|^2)$  . For the constituentsof air (mainly nitrogen and oxygen) and other diatomic gases, the ratio ofspecific heats is   $\gamma=1.4$  .
* This problem obviously falls into the class of vector-valuedproblems. A general overview of how to deal with these problems indeal.II can be found in the   @ref vector_valued   module.
...
* We use a time stepping scheme to substitute the time derivative in theabove equations. For simplicity, we define   $ \mathbf{B}({\mathbf{w}_{n}})(\mathbf z) $   as the spatial residual at time step   $n$   :
* @f{eqnarray*}
...
* In the implementation below, we choose the Lax-Friedrichs flux for thefunction   $\mathbf H$  , i.e.    $\mathbf{H}(\mathbf{a},\mathbf{b},\mathbf{n}) =
\frac{1}{2}(\mathbf{F}(\mathbf{a})\cdot \mathbf{n} +
\mathbf{F}(\mathbf{b})\cdot \mathbf{n} + \alpha (\mathbf{a}
* 
- \mathbf{b}))$  ,where   $\alpha$   is either a fixed number specified in the input file, or where  $\alpha$   is a mesh dependent value. In the latter case, it is chosen as  $\frac{h}{2\delta T}$   with   $h$   the diameter of the face to which the flux isapplied, and   $\delta T$   the current time step.
...
@f}
* This linear system is, in general, neither symmetric nor has anyparticular definiteness properties. We will either use a direct solveror Trilinos' GMRES implementation to solve it. As will become apparent fromthe <a href="#Results">results shown below</a>, this fully implicit iterationconverges very rapidly (typically in 3 steps) and with the quadraticconvergence order expected from a Newton method.
* 

* <a name="Automaticdifferentiation"></a><h3> Automatic differentiation </h3>
* 

* Since computing the Jacobian matrix   $\mathbf R'(\mathbf W^k)$   is aterrible beast, we use an automatic differentiation package, Sacado,to do this.  Sacado is a package within the <a
...
* One can imagine that for the full Jacobian matrix as a whole,this could be prohibitively expensive: the number of independent variables arethe   $\mathbf W^k$  , the dependent variables the elements of the vector   $\mathbf
...
* <a name="Trilinossolvers"></a><h3> Trilinos solvers </h3>
...
*     <li>  A namespace that deals with everything that has to do with run-time  parameters.
...
* Similarly, if we wanted to improve on the linear or nonlinear solvers, or onthe time stepping scheme (as hinted at the end of the <a
href="#Results">results section</a>), then this would not require changes inthe   <code>EulerEquations</code>   at all.
* 
...
* 
 ...
 @endcode
* 
*   <a name="Transformationsbetweenvariables"></a>  <h4>Transformations between variables</h4>
* 
...
* 
 ...
 @endcode
* 
*   <a name="EulerEquationscompute_flux_matrix"></a>  <h4>EulerEquations::compute_flux_matrix</h4>
* 
...
* @code
 ...
 @endcode
* 
*  First compute the pressure that appears in the flux matrix, and then compute the first   <code>dim</code>   columns of the matrix that correspond to the momentum terms:
* 
...
* 
 ...
 @endcode
* 
*   <a name="EulerEquationscompute_normal_flux"></a>  <h4>EulerEquations::compute_normal_flux</h4>
* 
...
* 
 @endcode
* 
*   <a name="EulerEquationscompute_forcing_vector"></a>  <h4>EulerEquations::compute_forcing_vector</h4>
* 
...
* 
 ...
 @endcode
* 
*   <a name="Dealingwithboundaryconditions"></a>  <h4>Dealing with boundary conditions</h4>
* 
...
*   There is a little snag that makes this function unpleasant from a C++ language viewpoint: The output vector   <code>Wminus</code>   will of course be modified, so it shouldn't be a   <code>const</code>   argument. Yet it is in the implementation below, and needs to be in order to allow the code to compile. The reason is that we call this function at a place where   <code>Wminus</code>   is of type   <code>Table@<2,Sacado::Fad::DFad@<double@> @></code>  , this being 2d table with indices representing the quadrature point and the vector component, respectively. We call this function with   <code>Wminus[q]</code>   as last argument; subscripting a 2d table yields a temporary accessor object representing a 1d vector, just what we want here. The problem is that a temporary accessor object can't be bound to a non-const reference argument of a function, as we would like here, according to the C++ 1998 and 2003 standards (something that will be fixed with the next standard in the form of rvalue references).  We get away with making the output argument here a constant because it is the <i>accessor</i> object that's constant, not the table it points to: that one can still be written to. The hack is unpleasant nevertheless because it restricts the kind of data types that may be used as template argument to this function: a regular vector isn't going to do because that one can not be written to when marked   <code>const</code>  . With no good solution around at the moment, we'll go with the pragmatic, even if not pretty, solution shown here:
* 
...
* 
 @endcode
* 
*   <a name="EulerEquationsPostprocessor"></a>  <h4>EulerEquations::Postprocessor</h4>
* 
...
* 
 ...
 @endcode
* 
*   <a name="Runtimeparameterhandling"></a>  <h3>Run time parameter handling</h3>
* 
...
*   We will split the run-time parameters into a few separate structures, which we will all put into a namespace   <code>Parameters</code>  . Of these classes, there are a few that group the parameters for individual groups, such as for solvers, mesh refinement, or output. Each of these classes have functions   <code>declare_parameters()</code>   and   <code>parse_parameters()</code>   that declare parameter subsections and entries in a ParameterHandler object, and retrieve actual parameter values from such an object, respectively. These classes declare all their parameters in subsections of the ParameterHandler.   
...
*   The meaning of each parameter is also briefly described in the third argument of the   ParameterHandler::declare_entry   call in   <code>declare_parameters()</code>  .
* 
...
*   Since this class is derived from all the ones above, the   <code>declare_parameters()</code> and <code>parse_parameters()</code>   functions call the respective functions of the base classes as well.     
*   Note that this class also handles the declaration of initial and boundary conditions specified in the input file. To this end, in both cases, there are entries like "w_0 value" which represent an expression in terms of   $x,y,z$   that describe the initial or boundary condition as a formula that will later be parsed by the FunctionParser class. Similar expressions exist for "w_1", "w_2", etc, denoting the   <code>dim+2</code>   conserved variables of the Euler system. Similarly, we allow up to   <code>max_n_boundaries</code>   boundary indicators to be used in the input file, and each of these boundary indicators can be associated with an inflow, outflow, or pressure boundary condition, with homogeneous boundary conditions being specified for each component and each boundary indicator separately.     
...
*   The   <code>BoundaryCondition</code>   structure requires a constructor since we need to tell the function parser object at construction time how many vector components it is to describe. This initialization can therefore not wait till we actually set the formulas the FunctionParser object represents later in   <code>AllParameters::parse_parameters()</code>       
*   For the same reason of having to tell Function objects their vector size at construction time, we have to have a constructor of the   <code>AllParameters</code>   class that at least initializes the other FunctionParser object, i.e. the one describing initial conditions.
* 
...
* 
 @endcode
* 
*  The first few member variables are also rather standard. Note that we define a mapping object to be used throughout the program when assembling terms (we will hand it to every FEValues and FEFaceValues object); the mapping we use is just the standard   $Q_1$   mapping
* 
...
* 
 @endcode
* 
*  Next come a number of data vectors that correspond to the solution of the previous time step (  <code>old_solution</code>  ), the best guess of the current solution (  <code>current_solution</code>  ; we say <i>guess</i> because the Newton iteration to compute it may not have converged yet, whereas   <code>old_solution</code>   refers to the fully converged final result of the previous time step), and a predictor for the solution at the next time step, computed by extrapolating the current and previous solution one time step into the future:
* 
...
* 
 ...
 @endcode
* 
*   <a name="ConservationLawConservationLaw"></a>  <h4>ConservationLaw::ConservationLaw</h4>   
...
* 
 @endcode
* 
*   <a name="ConservationLawsetup_system"></a>  <h4>ConservationLaw::setup_system</h4>   
...
* 
 ...
 @endcode
* 
*   <a name="ConservationLawassemble_system"></a>  <h4>ConservationLaw::assemble_system</h4>   
...
*   At the top of the function we do the usual housekeeping: allocate FEValues, FEFaceValues, and FESubfaceValues objects necessary to do the integrations on cells, faces, and subfaces (in case of adjoining cells on different refinement levels). Note that we don't need all information (like values, gradients, or real locations of quadrature points) for all of these objects, so we only let the FEValues classes whatever is actually necessary by specifying the minimal set of UpdateFlags. For example, when using a FEFaceValues object for the neighboring cell we only need the shape values: Given a specific face, the quadrature points and   <code>JxW</code>   values are the same as for the current cells, and the normal vectors are known to be the negative of the normal vectors of the current cell.
* 
...
* 
 @endcode
* 
*  Then loop over all the faces of this cell.  If a face is part of the external boundary, then assemble boundary conditions there (the fifth argument to   <code>assemble_face_terms</code>   indicates whether we are working on an external or internal face; if it is an external face, the fourth argument denoting the degrees of freedom indices of the neighbor is ignored, so we pass an empty vector):
* 
...
* 
 ...
 @endcode
* 
*   <a name="ConservationLawassemble_cell_term"></a>  <h4>ConservationLaw::assemble_cell_term</h4>   
...
*   In order to mark the variables as independent, the following does the trick, marking   <code>independent_local_dof_values[i]</code>   as the   $i$  th independent variable out of a total of   <code>dofs_per_cell</code>  :
* 
...
*   We initially sum all contributions of the residual in the positive sense, so that we don't need to negative the Jacobian entries.  Then, when we sum into the   <code>right_hand_side</code>   vector, we negate this residual.
* 
...
* 
 ...
 @endcode
* 
*   <a name="ConservationLawassemble_face_term"></a>  <h4>ConservationLaw::assemble_face_term</h4>   
...
*  Next, we need to define the values of the conservative variables   ${\mathbf W}$   on this side of the face (  $ {\mathbf W}^+$  ) and on the opposite side (  ${\mathbf W}^-$  ), for both   ${\mathbf W} =
 {\mathbf W}^k_{n+1}$   and    ${\mathbf W} = {\mathbf W}_n$  . The "this side" values can be computed in exactly the same way as in the previous function, but note that the   <code>fe_v</code>   variable now is of type FEFaceValues or FESubfaceValues:
* 
...
* 
 ...
 @endcode
* 
*   <a name="ConservationLawcompute_refinement_indicators"></a>  <h4>ConservationLaw::compute_refinement_indicators</h4>
* 

* 
*  This function is real simple: We don't pretend that we know here what a good refinement indicator would be. Rather, we assume that the   <code>EulerEquation</code>   class would know about this, and so we simply defer to the respective function we've implemented there:
* 
...
* 
 @endcode
* 
*   <a name="ConservationLawrefine_grid"></a>  <h4>ConservationLaw::refine_grid</h4>
* 
...
* 
 ...
 @endcode
* 
*   <a name="ConservationLawoutput_results"></a>  <h4>ConservationLaw::output_results</h4>
* 
...
*   First, we update the time and produce graphical output if so desired. Then we update a predictor for the solution at the next time step by approximating   $\mathbf w^{n+1}\approx \mathbf w^n +
 ...
 \mathbf w^n
* 
- \mathbf w^{n-1}$   to try and make adaptivity work better.  The idea is to try and refine ahead of a front, rather than stepping into a coarse set of elements and smearing the old_solution.  This simple time extrapolator does the job. With this, we then refine the mesh if so desired by the user, and finally continue on with the next time step:
* 
...
* We run the problem with the mesh   <code>slide.inp</code>   (this file is in thesame directory as the source code for this program) and the following inputdeck (available as   <code>input.prm</code>   in the same directory):
* @verbatim
...
@endverbatim
* 
* This output reports the progress of the Newton iterations and the timestepping. Note that our implementation of the Newton iteration indeed showsthe expected quadratic convergence order: the norm of the nonlinear residualin each step is roughly the norm of the previous step squared. This leads tothe very rapid convergence we can see here. This holds untiltimes up to   $t=1.9$   at which time the nonlinear iteration reports alack of convergence:
* @verbatim
...
* 
T=1.88
...
* By replacing the definitions of flux matrices and numerical fluxes inthis class, as well as the various other parts defined there, itshould be possible to apply the   <code>ConservationLaw</code>   class toother hyperbolic conservation laws as well.
* 
