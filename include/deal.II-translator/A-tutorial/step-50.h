 ...
 * This tutorial depends on   step-16  ,   step-37  . @htmlonly <table
 ...
 * @note   As a prerequisite of this program, you need to have both p4est and
 * either the PETScor Trilinos library installed. The installation of deal.II
 * together with these additionallibraries is described in the <a
 * href="../../readme.html" target="body">README</a> file.
 *
 *  <a name="Intro"></a><a name="Introduction"></a><h1>Introduction</h1>
 *
 *
 * This example shows the usage of the multilevel functions in deal.II
 * onparallel, distributedmeshes and gives a comparison between geometric and
 * algebraic multigrid methods.The algebraic multigrid (AMG) preconditioner is
 * the same used in   step-40  . Two geometricmultigrid (GMG) preconditioners
 * are considered: a matrix-based version similar to thatin   step-16   (but
 * for parallel computations) and a matrix-free versiondiscussed in   step-37
 * . The goal is to find out which approach leads tothe best solver for large
 * parallel computations. This tutorial is based on one of the numerical
 * examples in  @cite clevenger_par_gmg  . Please see that publication for a
 * detailed backgroundon the multigrid implementation in deal.II. We will
 * summarize some of theresults in the following text. Algebraic multigrid
 * methods are obviously the easiest to implementwith deal.II since classes
 * such as   TrilinosWrappers::PreconditionAMG  and
 * PETScWrappers::PreconditionBoomerAMG   are, in essence, black
 * boxpreconditioners that require only a couple of lines to set up even
 * forparallel computations. On the other hand, geometric multigrid
 * methodsrequire changes throughout a code base
 *
 *  -  not very many, but one hasto know what one is doing.
 * What the results of this program will showis that algebraic and geometric
 * multigrid methods are roughlycomparable in performance <i>when using
 * matrix-based formulations</i>,and that matrix-free geometric multigrid
 * methods are vastly better forthe problem under consideration here. A
 * secondary conclusion will bethat matrix-based geometric multigrid methods
 * really don't scale wellstrongly when the number of unknowns per processor
 * becomes smaller than20,000 or so.
 *
 ...
 * @f}
 * on the domain   $\Omega = [-1,1]^\text{dim} \setminus [0,1]^\text{dim}$
 * (an L-shaped domainfor 2D and a Fichera corner for 3D) with   $\epsilon =
 * 1$   if   $\min(x,y,z)>-\frac{1}{2}$   and  $\epsilon = 100$   otherwise.
 * In other words,   $\epsilon$   is small along the edgesor faces of the
 * domain that run into the reentrant corner, as will be visiblein the figure
 * below. The boundary conditions are   $u=0$   on the whole boundary andthe
 * right-hand side is   $f=1$  . We use continuous   $Q_2$   elements for
 * thediscrete finite element space   $V_h$  , and use aresidual-based,
 * cell-wise a posteriori error estimator  $e(K) = e_{\text{cell}}(K) +
 * e_{\text{face}}(K)$   from   @cite karakashian2003posteriori   with
 * @f{align*}
 * e_{\text{cell}}(K) &= h^2 \| f + \epsilon \triangle u \|_K^2, \\
 * e_{\text{face}}(K) &= \sum_F h_F \| \jump{ \epsilon \nabla u \cdot n } \|_F^2,
 * @f}
 * to adaptively refine the mesh. (This is a generalization of the Kellyerror
 * estimator used in the KellyErrorEstimator class that drives meshrefinement
 * in most of the other tutorial programs.)The following figure visualizes the
 * solution and refinement for 2D:  <img width="400px"
 * src="https://www.dealii.org/images/steps/developer/step-50-2d-solution.png"
 * alt="">  In 3D, the solution looks similar (see below). On the left you can
 * see the solution and on the right we show a slice for   $x$   close to
 * thecenter of the domain showing the adaptively refined mesh.  <table
 * width="60%" align="center"> <tr> <td align="center"> <img width="400px"
 ...
 * alt=""> </td> </tr> </table>  Both in 2D and 3D you can see the adaptive
 * refinement picking up the corner singularity and the inner singularity
 * where the viscosity jumps, while the interface along the line that
 * separates the two viscosities is (correctly) not refined as it is resolved
 * adequately.This is because the kink in the solution that results from the
 * jumpin the coefficient is aligned with cell interfaces.
 *
 *  <a name="Workloadimbalanceforgeometricmultigridmethods"></a><h3>Workload
 * imbalance for geometric multigrid methods</h3>
 *
 *  As mentioned above, the purpose of this program is to demonstrate theuse
 * of algebraic and geometric multigrid methods for this problem, andto do so
 * for parallel computations. An important component of makingalgorithms scale
 * to large parallel machines is ensuring that everyprocessor has the same
 * amount of work to do. (More precisely, whatmatters is that there are no
 * small fraction of processors that havesubstantially more work than the rest
 * since, if that were so, a largefraction of processors will sit idle waiting
 * for the small fraction tofinish. Conversely, a small fraction of processors
 * havingsubstantially <i>less</i> work is not a problem because the
 * majorityof processors continues to be productive and only the small
 * fractionsits idle once finished with their work.) For the active mesh, we
 * use the   parallel::distributed::Triangulation   class as donein   step-40
 * which uses functionality in the external library<a
 * href="http://www.p4est.org/">p4est</a> for the distribution of the active
 * cellsamong processors. For the non-active cells in the multilevel
 * hierarchy, deal.IIimplements what we will refer to as the "first-child
 * rule" where, for each cellin the hierarchy, we recursively assign the
 * parent of a cell to the owner of thefirst child cell. The following figures
 * give an example of such a distribution. Herethe left image represents the
 * active cells for a sample 2D mesh partitioned using aspace-filling curve
 * (which is what p4est uses to partition cells);the center image gives the
 * tree representationof the active mesh; and the right image gives the
 * multilevel hierarchy of cells. Thecolors and numbers represent the
 * different processors. The circular nodes in the treeare the non-active
 * cells which are distributed using the "first-child rule". <img
 * width="800px"
 * src="https://www.dealii.org/images/steps/developer/step-50-workload-example.png"
 * alt=""> Included among the output to screen in this example is a value
 * "Partition efficiency"given by one over   MGTools::workload_imbalance().
 * This value, which will be denotedby   $\mathbb{E}$  ,  quantifies the
 * overhead produced by not having a perfect work balanceon each level of the
 * multigrid hierarchy. This imbalance is evident from theexample above: while
 * level   $\ell=2$   is about as well balanced as is possiblewith four cells
 * among three processors, the coarselevel   $\ell=0$   has work for only one
 * processor, and level   $\ell=1$   has workfor only two processors of which
 * one has three times as much work asthe other.
 * For defining   $\mathbb{E}$  , it is important to note that, as we are using local smoothingto define the multigrid hierarchy (see the   @ref mg_paper   "multigrid paper"
 * for a description oflocal smoothing), the refinement level of a cell
 * corresponds to that cell's multigridlevel. Now, let   $N_{\ell}$   be the
 * number of cells on level   $\ell$  (both active and non-active cells) and
 * $N_{\ell,p}$   be the subset owned by process  $p$  . We will also denote
 * by   $P$   the total number of processors.Assuming that the workload for
 * any one processor is proportional to the numberof cells owned by that
 * processor, the optimal workload per processor is given by
 * @f{align*}
 * W_{\text{opt}} = \frac1{P}\sum_{\ell} N_{\ell} = \sum_{\ell}\left(\frac1{P}\sum_{p}N_{\ell,p}\right).
 * @f}
 * Next, assuming a synchronization of work on each level (i.e., on each level
 * of a V-cycle,work must be completed by all processors before moving on to
 * the next level), thelimiting effort on each level is given by
 * @f{align*}
 ...
 * W = \sum_{\ell} W_\ell.
 * @f}
 * Then we define   $\mathbb{E}$   as a ratio of the optimal partition to the
 * parallelcomplexity of the current partition
 * @f{align*}
 ...
 * @f}
 * The value   MGTools::workload_imbalance()$=   1/\mathbb{E}  $ then
 * represents the factor increase in timings we expect for GMG methods
 * (vmults, assembly, etc.) due to the imbalance of the mesh partition
 * compared to a perfectly load-balanced workload. We will report on these in
 * the results section below for a sequence of meshes, and compare with the
 * observed slow-downs as we go to larger and larger processor numbers (where,
 * typically, the load imbalance becomes larger as well).
 *
 ...
 * timing. In summary, the value of $  \mathbb{E}  $ is highly dependent on
 * the degree of local mesh refinement used and has an optimal value $
 * \mathbb{E} \approx 1  $ for globally refined meshes. Typically for
 * adaptively refined meshes, the number of processors used to distribute a
 * single mesh has a negative impact on $  \mathbb{E}  $ but only up to a
 ...
 * impact on $  \mathbb{E}  $. Finally, $  1/\mathbb{E}  $ was shown to give
 * an accurate representation of the slowdown in parallel scaling expected for
 ...
 * \mathbb{E}  $ is the partition efficiency from the introduction (also equal
 * to 1.0/workload imbalance), "Setup" is a combination of setup, setup
 ...
 * 117$ in 3d. But if the coarse meshconsists of hundreds or thousands of
 * cells, this approach will nolonger work and might start to dominate the
 * overall run-time of each V-cyle.A common approach is then to solve the
 * coarse mesh problem using analgebraic multigrid preconditioner; this would
 * then, however, requireassembling the coarse matrix (even for the
 * matrix-free version) asinput to the AMG implementation.
 *
