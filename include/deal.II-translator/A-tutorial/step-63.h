 ...
 * This tutorial depends on   step-16  . @htmlonly <table class="tutorial"
 ...
 * multigrid(GMG) preconditioner. The basics of this preconditioner are
 * discussed in   step-16  ;here we discuss the necessary changes needed for a
 * non-symmetricPDE. Additionally, we introduce the idea of block smoothing
 * (as compared topoint smoothing in   step-16  ), and examine the effects of
 * DoF renumbering foradditive and multiplicative smoothers. <a
 * name="Equation"></a><h3>Equation</h3> The advection-diffusion equation is
 * given by
 * @f{align*}{
 *
 ...
 * direction</i>, and   $f$   is a source. A few notes: 1. If
 * $\boldsymbol{\beta}=\boldsymbol{0}$  , this is the Laplace equation solved
 * in   step-16  (and many other places). 2. If   $\varepsilon=0$   then this
 * is the stationary advection equation solved in  step-9  . 3. One can define
 * a dimensionless number for this problem, called the<i>Peclet number</i>:
 * $\mathcal{P} \dealcoloneq \frac{\|\boldsymbol{\beta}\| L}{\varepsilon}$  ,
 * where   $L$   is the length scale of the domain. Itcharacterizes the kind
 * of equation we areconsidering: If   $\mathcal{P}>1$  , we say the problem
 * is<i>advection-dominated</i>, else if   $\mathcal{P}<1$   we will say the
 * problem is<i>diffusion-dominated</i>. For the discussion in this tutorial
 * we will be concerned withadvection-dominated flow. This is the complicated
 * case: We know thatfor diffusion-dominated problems, the standard Galerkin
 * method worksjust fine, and we also know that simple multigrid methods such
 * asthose defined in   step-16   are very efficient. On the other hand,
 * foradvection-dominated problems, the standard Galerkin approach leads
 * tooscillatory and unstable discretizations, and simple solvers are oftennot
 * very efficient. This tutorial program is therefore intended toaddress both
 * of these issues.
 *
 *  <a name="Streamlinediffusion"></a><h4>Streamline diffusion</h4>
 *
 *  Using the standard Galerkin finite element method, for suitable
 * testfunctions   $v_h$  , a discrete weak form of the PDE would read
 * @f{align*}{
 * a(u_h,v_h) = F(v_h)
 * @f}
 * where
 ...
 * @f}
 *
 * Unfortunately, one typically gets oscillatory solutions with thisapproach.
 * Indeed, the following error estimate can be shown for thisformulation:
 * @f{align*}{
 * \|\nabla (u-u_h)\| \leq (1+\mathcal{P}) \inf_{v_h} \|\nabla (u-v_h)\|.
 * @f}
 * The infimum on the right can be estimated as follows if the exactsolution
 ...
 * \|\nabla^k u)\|
 * @f}
 * where   $k$   is the polynomial degree of the finite elements used. As
 * aconsequence, we obtain the estimate
 * @f{align*}{
 * \|\nabla (u-u_h)\|
 * \leq (1+\mathcal{P}) C h^k
 * \|\nabla^k u)\|.
 * @f}
 * In other words, the numerical solution will converge. On the other
 * hand,given the definition of   $\mathcal{P}$   above, we have to expect
 * poornumerical solutions with a large error when   $\varepsilon \ll
 * \|\boldsymbol{\beta}\| L$  , i.e., if the problem has only a smallamount of
 * diffusion. To combat this, we will consider the new weak form
 * @f{align*}{
 * a(u_h,\,v_h) + \sum_K (-\varepsilon \Delta u_h +
 * \boldsymbol{\beta}\cdot \nabla u_h-f,\,\delta_K
 * \boldsymbol{\beta}\cdot \nabla v_h)_K = F(v_h)
 * @f}
 * where the sum is done over all cells   $K$   with the inner product
 * takenfor each cell, and   $\delta_K$   is a cell-wise constantstabilization
 * parameter defined in  @cite john2006discontinuity  . Essentially, adding in
 * thediscrete strong form residual enhances the coercivity of the
 * bilinearform   $a(\cdot,\cdot)$   which increases the stability of the
 * discretesolution. This method is commonly referred to as <i>streamline
 * diffusion</i> or <i>SUPG</i> (streamline upwind/Petrov-Galerkin).
 *
 *  <a name="Smoothers"></a><h3>Smoothers</h3>
 *
 *  One of the goals of this tutorial is to expand from using a
 * simple(point-wise) Gauss-Seidel (SOR) smoother that is used in   step-16
 * (class PreconditionSOR) on each level of the multigrid hierarchy.The term
 * "point-wise" is traditionally used in solvers to indicate that onesolves at
 * one "grid point" at a time; for scalar problems, this meansto use a solver
 * that updates one unknown of the linearsystem at a time, keeping all of the
 * others fixed; one would theniterate over all unknowns in the problem and,
 * once done, start over againfrom the first unknown until these "sweeps"
 * converge. Jacobi,Gauss-Seidel, and SOR iterations can all be interpreted in
 * this way.In the context of multigrid, one does not think of these methods
 * as"solvers", but as "smoothers". As such, one is not interested inactually
 * solving the linear system. It is enough to remove the high-frequencypart of
 * the residual for the multigrid method to work, because that
 * allowsrestricting the solution to a coarser mesh.  Therefore, one only does
 * a few,fixed number of "sweeps" over all unknowns. In the code in
 * thistutorial this is controlled by the "Smoothing steps" parameter. But
 * these methods are known to converge rather slowly when used assolvers.
 * While as multigrid smoothers, they are surprisingly good,they can also be
 * improved upon. In particular, we consider"cell-based" smoothers here as
 * well. These methods solve for allunknowns on a cell at once, keeping all
 * other unknowns fixed; theythen move on to the next cell, and so on and so
 * forth. One can thinkof them as "block" versions of Jacobi, Gauss-Seidel, or
 * SOR, butbecause degrees of freedom are shared among multiple cells,
 * theseblocks overlap and the methods are in factbest be explained within the
 * framework of additive and multiplicativeSchwarz methods. In contrast to
 * step-16  , our test problem contains an advectiveterm. Especially with a
 * small diffusion constant   $\varepsilon$  , information istransported along
 * streamlines in the given advection direction. This meansthat smoothers are
 * likely to be more effective if they allow information totravel in
 * downstream direction within a single smootherapplication. If we want to
 * solve one unknown (or block of unknowns) ata time in the order in which
 * these unknowns (or blocks) areenumerated, then this information propagation
 * propertyrequires reordering degrees of freedom or cells (for the cell-based
 * smoothers)accordingly so that the ones further upstream are treated
 * earlier(have lower indices) and those further downstream are treated
 * later(have larger indices). The influence of the ordering will be visiblein
 * the results section. Let us now briefly define the smoothers used in this
 * tutorial.For a more detailed introduction, we refer to  @cite
 * KanschatNotesIterative   and the books   @cite smith2004domain   and
 * @cite toselli2006domain  .A Schwarzpreconditioner requires a decomposition
 * @f{align*}{
 * V = \sum_{j=1}^J V_j
 * @f}
 * of our finite element space   $V$  . Each subproblem   $V_j$   also has a
 * Ritzprojection   $P_j: V \rightarrow V_j$   based on the bilinear form
 * $a(\cdot,\cdot)$  . This projection induces a local operator   $A_j$   for
 * eachsubproblem   $V_j$  . If   $\Pi_j:V\rightarrow V_j$   is the orthogonal
 * projector onto  $V_j$  , one can show   $A_jP_j=\Pi_j^TA$  . With this we
 * can define an <i>additive Schwarz preconditioner</i> for theoperator   $A$
 * as
 * @f{align*}{
 * B^{-1} = \sum_{j=1}^J P_j A^{-1} = \sum_{j=1}^J A_j^{-1} \Pi_j^T.
 * @f}
 * In other words, we project our solution into each subproblem, apply
 * theinverse of the subproblem   $A_j$  , and sum the contributions up over
 * all   $j$  . Note that one can interpret the point-wise (one unknown at a
 * time)Jacobi method as an additiveSchwarz method by defining a subproblem
 * $V_j$   for each degree offreedom. Then,   $A_j^{-1}$   becomes a
 * multiplication with the inverse of adiagonal entry of   $A$  . For the
 * "Block Jacobi" method used in this tutorial, we define a subproblem  $V_j$
 * for each cell of the mesh on the current level. Note that we use
 * acontinuous finite element, so these blocks are overlapping, as degrees
 * offreedom on an interface between two cells belong to both subproblems.
 * Thelogic for the Schwarz operator operating on the subproblems (in deal.II
 * theyare called "blocks") is implemented in the class RelaxationBlock. The
 * "BlockJacobi" method is implemented in the class RelaxationBlockJacobi.
 * Manyaspects of the class (for example how the blocks are defined and how to
 * invertthe local subproblems   $A_j$  ) can be configured in the smoother
 * data, see  RelaxationBlock::AdditionalData   and
 * DoFTools::make_cell_patches()   for details. So far, we discussed additive
 * smoothers where the updates can be appliedindependently and there is no
 * information flowing within a single smootherapplication. A
 * <i>multiplicative Schwarz preconditioner</i> addresses thisand is defined
 * by
 * @f{align*}{
 * B^{-1} = \left( I- \prod_{j=1}^J \left(I-P_j\right) \right) A^{-1}.
 * @f}
 * In contrast to above, the updates on the subproblems   $V_j$   are
 * appliedsequentially. This means that the update obtained when inverting
 * thesubproblem   $A_j$   is immediately used in   $A_{j+1}$  . This
 * becomesvisible when writing out the project:
 * @f{align*}{
 * B^{-1}
 * =
 * \left(
 ...
 * @f}
 *
 * When defining the sub-spaces   $V_j$   as whole blocks of degrees
 * offreedom, this method is implemented in the class RelaxationBlockSOR and
 * used when youselect "Block SOR" in this tutorial. The class
 * RelaxationBlockSOR is alsoderived from RelaxationBlock. As such, both
 * additive and multiplicativeSchwarz methods are implemented in a unified
 * framework. Finally, let us note that the standard Gauss-Seidel (or SOR)
 * method can beseen as a multiplicative Schwarz method with a subproblem for
 * each DoF.
 *
 *  <a name="Testproblem"></a><h3>Test problem</h3>
 *
 *  We will be considering the following test problem:   $\Omega =
 * [-1,\,1]\times[-1,\,1]\backslash B_{0.3}(0)$  , i.e., a squarewith a circle
 * of radius 0.3 centered at theorigin removed. In addition, we use
 * $\varepsilon=0.005$  ,   $\boldsymbol{\beta} =
 ...
 * @f}
 *
 * The following figures depict the solutions with (left) and without(right)
 * streamline diffusion. Without streamline diffusion we see largeoscillations
 * around the boundary layer, demonstrating the instabilityof the standard
 * Galerkin finite element method for this problem. <table width="60%"
 ...
 * @endcode
 *
 * We will be using   MeshWorker::mesh_loop   functionality for assembling
 * matrices:
 *
 *
 * @code
 ...
 *
 *
 * As always, we will be putting everything related to this program into a
 * namespace of its own.
 *
 *
 * Since we will be using the MeshWorker framework, the first step is to
 * define the following structures needed by the assemble_cell() function used
 * by   MeshWorker::mesh_loop():   `ScratchData` contains an FEValues object
 * which is needed for assembling a cell's local contribution, while
 * `CopyData` contains the output from a cell's local contribution and
 * necessary information to copy that to the global system. (Their purpose is
 * also explained in the documentation of the WorkStream class.)
 *
 *
 * @code
 * namespace Step63
 ...
 *
 *
 * The second step is to define the classes that deal with run-time parameters
 * to be read from an input file. We will use ParameterHandler to pass in
 * parameters at runtime. The structure `Settings` parses and stores the
 * parameters to be queried throughout the program.
 *
 *
 * @code
 * struct Settings
 ...
 * @endcode
 *
 * <a name="Cellpermutations"></a>  <h3>Cell permutations</h3> The ordering in
 * which cells and degrees of freedom are traversed will play a role in the
 * speed of convergence for multiplicative methods. Here we define functions
 * which return a specific ordering of cells to be used by the block
 * smoothers. For each type of cell ordering, we define a function for the
 * active mesh and one for a level mesh (i.e., for the cells at one level of a
 * multigrid hierarchy). While the only reordering necessary for solving the
 * system will be on the level meshes, we include the active reordering for
 * visualization purposes in output_results(). For the two downstream ordering
 * functions, we first create an array with all of the relevant cells that we
 * then sort in downstream direction using a "comparator" object. The output
 * of the functions is then simply an array of the indices of the cells in the
 * just computed order.
 *
 *
 * @code
 ...
 * @endcode
 *
 * The functions that produce a random ordering are similar in spirit in that
 * they first put information about all cells into an array. But then, instead
 * of sorting them, they shuffle the elements randomly using the facilities
 * C++ offers to generate random numbers. The way this is done is by iterating
 * over all elements of the array, drawing a random number for another element
 * before that, and then exchanging these elements. The result is a random
 * shuffle of the elements of the array.
 *
 *
 * @code
 * template <int dim>
 ...
 *
 *
 * The problem solved in this tutorial is an adaptation of Ex. 3.1.3 found on
 * pg. 118 of <a
 ...
 * Incompressible Fluid Dynamics by Elman, Silvester, and Wathen</a>. The main
 * difference being that we add a hole in the center of our domain with zero
 * Dirichlet boundary conditions. For a complete description, we need classes
 * that implement the zero right-hand side first (we could of course have just
 * used   Functions::ZeroFunction):
 *
 *
 * @code
 * template <int dim>
 ...
 *
 *
 * The streamline diffusion method has a stabilization constant that we need
 * to be able to compute. The choice of how this parameter is computed is
 * taken from <a
 ...
 * step-16  . The major difference is that, since we are defining our
 * multigrid smoother at runtime, we choose to define a function
 * `create_smoother()` and a class object `mg_smoother` which is a
 * `std::unique_ptr`   to a smoother that is derived from MGSmoother. Note
 * that for smoothers derived from RelaxationBlock, we must include a
 * `smoother_data` object for each level. This will contain information about
 * the cell ordering and the method of inverting cell matrices.
 *
 *
 *
 *
 ...
 *
 *
 * Here we first set up the DoFHandler, AffineConstraints, and SparsityPattern
 * objects for both active and multigrid level meshes. We could renumber the
 * active DoFs with the DoFRenumbering class, but the smoothers only act on
 * multigrid levels and as such, this would not matter for the computations.
 * Instead, we will renumber the DoFs on each multigrid level below.
 *
 *
 * @code
 * template <int dim>
 ...
 * @endcode
 *
 * Having enumerated the global degrees of freedom as well as (in the last
 * line above) the level degrees of freedom, let us renumber the level degrees
 * of freedom to get a better smoother as explained in the introduction.  The
 * first block below renumbers DoFs on each level in downstream or upstream
 * direction if needed. This is only necessary for point smoothers (SOR and
 * Jacobi) as the block smoothers operate on cells (see `create_smoother()`).
 * The blocks below then also implement random numbering.
 *
 *
 * @code
 ...
 * @endcode
 *
 * The rest of the function just sets up data structures. The last lines of
 * the code below is unlike the other GMG tutorials, as it sets up both the
 * interface in and out matrices. We need this since our problem is
 * non-symmetric.
 *
 *
 * @code
 ...
 * for either an active or a level cell (whatever it is passed as its first
 * argument), and only assembles a right-hand side if called with an active
 * cell.
 *
 *
 *
 ...
 * @endcode
 *
 * If we are using streamline diffusion we must add its contribution to both
 * the cell matrix and the cell right-hand side. If we are not using
 * streamline diffusion, setting   $\delta=0$   negates this contribution
 * below and we are left with the standard, Galerkin finite element assembly.
 *
 *
 * @code
 ...
 * @endcode
 *
 * The assembly of the local matrix has two parts. First the Galerkin
 * contribution:
 *
 *
 * @code
 ...
 *
 *
 * Here we employ   MeshWorker::mesh_loop()   to go over cells and assemble
 * the system_matrix, system_rhs, and all mg_matrices for us.
 *
 *
 *
 *
 ...
 * @endcode
 *
 * Unlike the constraints for the active level, we choose to create constraint
 * objects for each multigrid level local to this function since they are
 ...
 * `interface_in` dof pair. Note: For `interface_in`, we load the transpose of
 * the interface entries, i.e., the entry for dof pair   $(j,i)$   is stored
 * in `interface_in(i,j)`. This is an optimization for the symmetric case
 * which allows only one matrix to be used when setting the edge_matrices in
 * solve(). Here, however, since our problem is non-symmetric, we must store
 ...
 * error, and reflects values necessary to keep the iteration counts in the
 * GMRES solve constant (or as close as possible) as we refine the mesh. The
 * two values given for both "Jacobi" and "SOR" in the `.prm` files are for
 ...
 * preconditioner. This requires the setup of the transfer between levels, the
 * coarse matrix solver, and the smoother. This setup follows almost
 ...
 * use an appropriate Krylov subspace method. We choose here to use GMRES
 * since it offers the guarantee of residual reduction in each iteration. The
 * major disavantage of GMRES is that, for each iteration, the number of
 * stored temporary vectors increases by one, and one also needs to compute a
 * scalar product with all previously stored vectors. This is rather
 * expensive. This requirement is relaxed by using the restarted GMRES method
 * which puts a cap on the number of vectors we are required to store at any
 * one time (here we restart after 50 temporary vectors, or 48 iterations).
 * This then has the disadvantage that we lose information we have gathered
 * throughout the iteration and therefore we could see slower convergence. As
 * a consequence, where to restart is a question of balancing memory
 * consumption, CPU effort, and convergence speed. However, the goal of this
 * tutorial is to have very low iteration counts by using a powerful GMG
 * preconditioner, so we have picked the restart length such that all of the
 * results shown below converge prior to restart happening, and thus we have a
 * standard GMRES method. If the user is interested, another suitable method
 ...
 *
 *
 * The final function of interest generates graphical output. Here we output
 * the solution and cell ordering in a .vtu format.
 *
 *
 * At the top of the function, we generate an index for each cell to visualize
 * the ordering used by the smoothers. Note that we do this only for the
 * active cells instead of the levels, where the smoothers are actually used.
 * For the point smoothers we renumber DoFs instead of cells, so this is only
 * an approximation of what happens in reality. Finally, the random ordering
 * is not the random ordering we actually use (see `create_smoother()` for
 * that). The (integer) ordering of cells is then copied into a (floating
 ...
 * @endcode
 *
 * The remainder of the function is then straightforward, given previous
 * tutorial programs:
 *
 *
 * @code
 ...
 *
 *
 * As in most tutorials, this function creates/refines the mesh and calls the
 * various functions defined above to set up, assemble, solve, and output the
 * results.
 *
 *
 * In cycle zero, we generate the mesh for the on the square
 * <code>[-1,1]^dim</code>   with a hole of radius 3/10 units centered at the
 * origin. For objects with `manifold_id` equal to one (namely, the faces
 * adjacent to the hole), we assign a spherical manifold.
 *
 *
 *
 *
 ...
 *
 *
 * Finally, the main function is like most tutorials. The only interesting bit
 * is that we require the user to pass a `.prm` file as a sole command line
 * argument. If no parameter file is given, the program will output the
 * contents of a sample parameter file with all default values to the screen
 * that the user can then copy and paste into their own `.prm` file.
 *
 *
 *
 *
 ...
 *
 *  The major advantage for GMG is that it is an   $\mathcal{O}(n)$
 * method,that is, the complexity of the problem increases linearly with
 * theproblem size. To show then that the linear solver presented in
 * thistutorial is in fact   $\mathcal{O}(n)$  , all one needs to do is show
 * thatthe iteration counts for the GMRES solve stay roughly constant as
 * werefine the mesh. Each of the following tables gives the GMRES iteration
 * counts to reduce theinitial residual by a factor of   $10^8$  . We selected
 * a sufficient number of smoothing steps(based on the method) to get
 * iteration numbers independent of mesh size. Ascan be seen from the tables
 * below, the method is indeed   $\mathcal{O}(n)$  . <a
 * name="DoFCellRenumbering"></a><h4> DoF/Cell Renumbering </h4>
 *
 *  The point-wise smoothers ("Jacobi" and "SOR") get applied in the order
 * theDoFs are numbered on each level. We can influence this using
 * theDoFRenumbering namespace. The block smoothers are applied based on
 * theordering we set in `setup_smoother()`. We can visualize this numbering.
 * Thefollowing pictures show the cell numbering of the active cells in
 * downstream,random, and upstream numbering (left to right): <img
 * src="https://www.dealii.org/images/steps/developer/step-63-cell-order.png"
 * alt=""> Let us start with the additive smoothers. The following table
 * showsthe number of iterations necessary to obtain convergence from GMRES:
 ...
 * <td>20</th> <th></th> <td>16</th> <td>16</th> <td>16</th> </tr> </table> We
 * see that renumbering theDoFs/cells has no effect on convergence speed. This
 * is because thesesmoothers compute operations on each DoF (point-smoother)
 * or cell(block-smoother) independently and add up the results. Since we
 * candefine these smoothers as an application of a sum of matrices, andmatrix
 * addition is commutative, the order at which we sum thedifferent components
 * will not affect the end result. On the other hand, the situation is
 * different for multiplicative smoothers: <table align="center"
 ...
 * Here, we can speed upconvergence by renumbering the DoFs/cells in the
 * advection direction,and similarly, we can slow down convergence if we do
 * the renumberingin the opposite direction. This is because
 * advection-dominatedproblems have a directional flow of information (in the
 * advectiondirection) which, given the right renumbering of
 * DoFs/cells,multiplicative methods are able to capture. This feature of
 * multiplicative methods is, however, dependent on thevalue of
 * $\varepsilon$  . As we increase   $\varepsilon$   and the problembecomes
 * more diffusion-dominated, we have a more uniform propagationof information
 * over the mesh and there is a diminished advantage forrenumbering in the
 * advection direction. On the opposite end, in theextreme case of
 * $\varepsilon=0$   (advection-only), we have a 1st-orderPDE and
 * multiplicative methods with the right renumbering becomeeffective solvers:
 * A correct downstream numbering may lead to methodsthat require only a
 * single iteration because information can bepropagated from the inflow
 * boundary downstream, with no informationtransport in the opposite
 * direction. (Note, however, that in the caseof   $\varepsilon=0$  , special
 * care must be taken for the boundaryconditions in this case).
 *
 *  <a name="Pointvsblocksmoothers"></a><h4> %Point vs. block smoothers </h4>
 *
 *  We will limit the results to runs using the downstreamrenumbering. Here is
 * a cross comparison of all four smoothers for both  $Q_1$   and   $Q_3$
 * elements: <table align="center" class="doxtable"> <tr> <th></th> <td></th>
 * <th colspan="1">$Q_1$</th> <th colspan="4">Smoother (smoothing steps)</th>
 ...
 * <td></th> </tr> </table> We see that for   $Q_1$  , both multiplicative
 * smoothers require a smallercombination of smoothing steps and iteration
 * counts than eitheradditive smoother. However, when we increase the degree
 * to a   $Q_3$  element, there is a clear advantage for the block smoothers
 * in termsof the number of smoothing steps and iterations required tosolve.
 * Specifically, the block SOR smoother gives constant iterationcounts over
 * the degree, and the block Jacobi smoother only sees abouta 38% increase in
 * iterations compared to 75% and 183% for Jacobi andSOR respectively. <a
 * name="Cost"></a><h3> Cost </h3>
 *
 *  Iteration counts do not tell the full story in the optimality of a
 * onesmoother over another. Obviously we must examine the cost of
 * aniteration. Block smoothers here are at a disadvantage as they arehaving
 * to construct and invert a cell matrix for each cell. Here is acomparison of
 * solve times for a   $Q_3$   element with 74,496 DoFs: <table align="center"
 ...
 * </tr> </table> The smoother that requires the most iterations (Jacobi)
 * actually takesthe shortest time (roughly 2/3 the time of the next
 * fastestmethod). This is because all that is required to apply a
 * Jacobismoothing step is multiplication by a diagonal matrix which is
 * verycheap. On the other hand, while SOR requires over 3x more
 * iterations(each with 3x more smoothing steps) than block SOR, the times
 * areroughly equivalent, implying that a smoothing step of block SOR
 * isroughly 9x slower than a smoothing step of SOR. Lastly, block Jacobiis
 * almost 6x more expensive than block SOR, which intuitively makessense from
 * the fact that 1 step of each method has the same cost(inverting the cell
 * matrices and either adding or multiply themtogether), and block Jacobi has
 * 3 times the number of smoothing steps periteration with 2 times the
 * iterations.
 *
 *  <a name="Additionalpoints"></a><h3> Additional points </h3>
 *
 *  There are a few more important points to mention:
 * <ol>    <li>   For a mesh distributed in parallel, multiplicative methods cannotbe executed over the entire domain. This is because they operate onecell at a time, and downstream cells can only be handled once upstreamcells have already been done. This is fine on a single processor: Theprocessor just goes through the list of cells one after theother. However, in parallel, it would imply that some processors areidle because upstream processors have not finished doing the work oncells upstream from the ones owned by the current processor. Once theupstream processors are done, the downstream ones can start, but bythat time the upstream processors have no work left. In other words,most of the time during these smoother steps, most processors are infact idle. This is not how one obtains good parallel scalability!
 * One can use a hybrid method wherea multiplicative smoother is applied on
 * each subdomain, but as youincrease the number of subdomains, the method
 * approaches the behaviorof an additive method. This is a major disadvantage
 * to these methods.  </li>
 * <li>   Current research into block smoothers suggest that soon we will
 * beable to compute the inverse of the cell matrices much cheaper thanwhat is
 * currently being done inside deal.II. This research is based onthe fast
 * diagonalization method (dating back to the 1960s) and hasbeen used in the
 * spectral community for around 20 years (see, e.g., <a
 ...
 * and Fischer</a>). There are currently efforts to generalize thesemethods to DG and make them more robust. Also, it seems that oneshould be able to take advantage of matrix-free implementations andthe fact that, in the interior of the domain, cell matrices tend tolook very similar, allowing fewer matrix inverse computations.  </li>    </ol>
 * Combining 1. and 2. gives a good reason for expecting that a methodlike
 * block Jacobi could become very powerful in the future, eventhough currently
 * for these examples it is quite slow.
 *
 *  <a name="Possibilitiesforextensions"></a><h3> Possibilities for extensions
 * </h3>
 *
 *  <a name="ConstantiterationsforQsub5sub"></a><h4> Constant iterations for
 * Q<sub>5</sub> </h4>
 *
 *  Change the number of smoothing steps and the smoother relaxationparameter
 * (set in   <code>Smoother::AdditionalData()</code>   inside
 * <code>create_smoother()</code>  , only necessary for point smoothers)
 * sothat we maintain a constant number of iterations for a   $Q_5$   element.
 * <a name="Effectivenessofrenumberingforchangingepsilon"></a><h4>
 * Effectiveness of renumbering for changing epsilon </h4>
 *
 *  Increase/decrease the parameter "Epsilon" in the `.prm` files of
 * themultiplicative methods and observe for which values renumbering nolonger
 * influences convergence speed. <a name="Meshadaptivity"></a><h4> Mesh
 * adaptivity </h4>
 *
 *  The code is set up to work correctly with an adaptively refined mesh
 * (theinterface matrices are created and set). Devise a suitable
 * refinementcriterium or try the KellyErrorEstimator class.
 *
