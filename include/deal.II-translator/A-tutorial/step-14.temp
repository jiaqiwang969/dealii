...
* We will not discuss the derivation of these concepts in too great detail, butwill implement the main ideas in the present example program. For a thoroughintroduction into the general idea, we refer to the seminal work of Becker andRannacher   @cite BR95  ,   @cite BR96r  , and the overview article of the same authors inActa Numerica   @cite BR01  ; the first introduces the concept of errorestimation and adaptivity for general functional output for the Laplaceequation, while the second gives many examples of applications of theseconcepts to a large number of other, more complicated equations. Forapplications to individual types of equations, see also the publications byBecker   @cite Bec95  ,   @cite Bec98  , Kanschat   @cite Kan96  ,   @cite FK97  , Suttmeier  @cite Sut96  ,   @cite RS97  ,   @cite RS98c  ,   @cite RS99  , Bangerth   @cite BR99b  ,  @cite Ban00w  ,   @cite BR01a  ,   @cite Ban02  , and Hartmann   @cite Har02  ,   @cite HH01  ,  @cite HH01b  . All of these works, from the original introduction by Becker andRannacher to individual contributions to particular equations, have later beensummarized in a book by Bangerth and Rannacher that covers all of these topics,see   @cite BR03  .
* 
...
* In all the cases just listed, it is the evaluation of a functional   $J(u)$   ofthe solution which we are interested in, rather than the values of   $u$  everywhere. Since the exact solution   $u$   is not available, but only itsnumerical approximation   $u_h$  , it is sensible to ask whether the computedvalue   $J(u_h)$   is within certain limits of the exact value   $J(u)$  , i.e. wewant to bound the error with respect to this functional,   $J(u)-J(u_h)$  .
* For simplicity of exposition, we henceforth assume that both the quantity ofinterest   $J$  , as well as the equation are linear, and we will in particularshow the derivation for the Laplace equation with homogeneous Dirichletboundary conditions, although the concept is much more general. For thisgeneral case, we refer to the references listed above.  The goal is to obtainbounds on the error,   $J(e)=J(u)-J(u_h)$  . For this, let us denote by   $z$   thesolution of a dual problem, defined as follows:@f[
  a(\varphi,z) = J(\varphi) \qquad \forall \varphi,
@f]where   $a(\cdot,\cdot)$   is the bilinear form associated with the differentialequation, and the test functions are chosen from the corresponding solutionspace. Then, taking as special test function   $\varphi=e$   the error, we havethat@f[
  J(e) = a(e,z)
@f]and we can, by Galerkin orthogonality, rewrite this as@f[
  J(e) = a(e,z-\varphi_h)
@f]where   $\varphi_h$   can be chosen from the discrete test space inwhatever way we find convenient.
* Concretely, for Laplace's equation, the error identity reads@f[
  J(e) = (\nabla e, \nabla(z-\varphi_h)).
@f]Because we want to use this formula not only to compute error, butalso to refine the mesh, we need to rewrite the expression above as asum over cells where each cell's contribution can then be used as anerror indicator for this cell.Thus, we split the scalar products into terms for each cell, andintegrate by parts on each of them:@f{eqnarray*}
  J(e)
  &=&
  \sum_K (\nabla (u-u_h), \nabla (z-\varphi_h))_K
  \\
...
@f}
* Next we use that   $-\Delta u=f$  , and that for solutions of the Laplaceequation, the solution is smooth enough that   $\partial_n u$   iscontinuous almost everywhere
* 
*  -  so the terms involving   $\partial_n u$   on onecell cancels with that on its neighbor, where the normal vector has theopposite sign. (The same is not true for   $\partial_n u_h$  , though.)At the boundary of the domain, where there is no neighbor cellwith which this term could cancel, the weight   $z-\varphi_h$   can be chosen aszero, and the whole term disappears.
* Thus, we have@f{eqnarray*}
  J(e)
  &=&
...
@f}
* Using that for the normal vectors on adjacent cells we have   $n'=-n$  , we define the jump of thenormal derivative by@f[
  [\partial_n u_h] \dealcoloneq \partial_n u_h|_K + \partial_{n'} u_h|_{K'}
  =
  \partial_n u_h|_K
* 
- \partial_n u_h|_{K'},
@f]and get the final form after setting the discrete function   $\varphi_h$  , whichis by now still arbitrary, to the point interpolation of the dual solution,  $\varphi_h=I_h z$  :@f{eqnarray*}
  J(e)
  &=&
...
@f}
* 
* With this, we have obtained an exact representation of the error of the finiteelement discretization with respect to arbitrary (linear) functionals  $J(\cdot)$  . Its structure is a weighted form of a residual estimator, as both  $f+\Delta u_h$   and   $[\partial_n u_h]$   are cell and edge residuals that vanishon the exact solution, and   $z-I_h z$   are weights indicating how important theresiduals on a certain cell is for the evaluation of the given functional.Furthermore, it is a cell-wise quantity, so we can use it as a mesh refinementcriterion. The question, is: how to evaluate it? After all, the evaluationrequires knowledge of the dual solution   $z$  , which carries the informationabout the quantity we want to know to best accuracy.
* In some, very special cases, this dual solution is known. For example, if thefunctional   $J(\cdot)$   is the point evaluation,   $J(\varphi)=\varphi(x_0)$  , thenthe dual solution has to satisfy@f[
* 
...
* 
-\Delta z = \delta(x-x_0),
@f]with the Dirac delta function on the right hand side, and the dual solution isthe Green's function with respect to the point   $x_0$  . For simple geometries,this function is analytically known, and we could insert it into the errorrepresentation formula.
* However, we do not want to restrict ourselves to such special cases. Rather,we will compute the dual solution numerically, and approximate   $z$   by somenumerically obtained   $\tilde z$  . We note that it is not sufficient to computethis approximation   $\tilde z$   using the same method as used for the primalsolution   $u_h$  , since then   $\tilde z-I_h \tilde z=0$  , and the overall errorestimate would be zero. Rather, the approximation   $\tilde z$   has to be from alarger space than the primal finite element space. There are various ways toobtain such an approximation (see the cited literature), and we will choose tocompute it with a higher order finite element space. While this is certainlynot the most efficient way, it is simple since we already have all we need todo that in place, and it also allows for simple experimenting. For moreefficient methods, again refer to the given literature, in particular  @cite BR95  ,   @cite BR03  .
* With this, we end the discussion of the mathematical side of this program andturn to the actual implementation.
* 

* 
*   @note   There are two steps above that do not seem necessary if all youcare about is computing the error: namely, (i) the subtraction of  $\phi_h$   from   $z$  , and (ii) splitting the integral into a sum of cellsand integrating by parts on each. Indeed, neither of these two stepschange   $J(e)$   at all, as we only ever consider identities above untilthe substitution of   $z$   by   $\tilde z$  . In other words, if you careonly about <i>estimating the global error</i>   $J(e)$  , then these stepsare not necessary. On the other hand, if you want to use the errorestimate also as a refinement criterion for each cell of the mesh,then it is necessary to (i) break the estimate into a sum of cells,and (ii) massage the formulas in such a way that each cell'scontributions have something to do with the local error. (While thecontortions above do not change the value of the <i>sum</i>   $J(e)$  ,they change the values we compute for each cell   $K$  .) To this end, wewant to write everything in the form "residual times dual weight"where a "residual" is something that goes to zero as the approximationbecomes   $u_h$   better and better. For example, the quantity   $\partial_n
u_h$   is not a residual, since it simply converges to the (normalcomponent of) the gradient of the exact solution. On the other hand,  $[\partial_n u_h]$   is a residual because it converges to   $[\partial_n
...
* We demonstrate a way how this can be achieved in a simple, yet very flexibleway. We will put everything that belongs to a certain setup into one class,and provide a little C++ mortar around it, so that entire setups (domains,coefficients, right hand sides, etc.) can be exchanged by only changingsomething in   <em>  one  </em>   place.
...
*   Given the interface of the   <code>PointValueEvaluation</code>   class, the declaration of this class provides little surprise, and neither does the constructor:
* 
...
*   Thus: initialize the   <code>FEValues</code>   object on this cell,
* 
...
*   However, we also want to look at the grids generated. This again can be done with one such class. Its structure is analog to the   <code>SolutionOutput</code>   class of the previous example program, so we do not discuss it here in more detail. Furthermore, everything that is used here has already been used in previous example programs.
* 
...
* 
*  Likewise, the   <code>Solver</code>   class is entirely unchanged and will thus not be discussed.
* 
...
*   Adding new testcases is now simple, you don't have to touch the framework classes, only a structure like the   <code>CurvedRidges</code>   one is needed.
* 
...
* 
*  Based on the above description, the   <code>SetUpBase</code>   class then looks as follows. To allow using the   <code>SmartPointer</code>   class with this class, we derived from the   <code>Subscriptor</code>   class.
* 
...
* 
*  The class that is used to describe the boundary values and right hand side of the   <code>curved ridge</code>   problem already used in the   step-13   example program is then like so:
* 
...
* 
 @endcode
* 
*  Initialize a   <code>FEValues</code>   object with a quadrature formula, have abbreviations for the number of quadrature points and shape functions...
* 
...
* @code
 ...
 @endcode
* 
*  If we have found such a cell, then initialize the   <code>FEValues</code>   object and integrate the x-component of the gradient of each shape function, as well as the unit function for the total area/volume.
* 
...
* 
 ...
 @endcode
* 
*  In the private section, we have two functions that are used to call the   <code>solve_problem</code>   functions of the primal and dual base classes. These two functions will be called in parallel by the   <code>solve_problem</code>   function of this class.
* 
...
* 
 @endcode
* 
*  In the implementation of this class, we first have the constructors of the   <code>CellData</code> and <code>FaceData</code>   member classes, and the   <code>WeightedResidual</code>   constructor. They only initialize fields to their correct lengths, so we do not have to discuss them in too much detail:
* 
...
* 
 @endcode
* 
*  Now, it is becoming more interesting: the   <code>refine_grid()</code>   function asks the error estimator to compute the cell-wise error indicators, then uses their absolute values for mesh refinement.
* 
...
* 
 @endcode
* 
*  Add the data vectors for which we want output. Add them both, the   <code>DataOut</code>   functions can handle as many data vectors as you wish to write to output:
* 
...
*   First, let's do that for the primal solution: it is cell-wise interpolated into the finite element space in which we have solved the dual problem: But, again as in the   <code>WeightedResidual::output_solution</code>   function we first need to create an AffineConstraints object including the hanging node constraints, but this time of the dual finite element space.
* 
...
* 
 @endcode
* 
*  Then for computing the interpolation of the numerically approximated dual solution z into the finite element space of the primal solution and subtracting it from z: use the   <code>interpolate_difference</code>   function, that gives (z-I_hz) in the element space of the dual solution.
* 
...
* 
 @endcode
* 
*  First task on each cell is to compute the cell residual contributions of this cell, and put them into the   <code>error_indicators</code>   variable:
* 
...
* 
 @endcode
* 
*  The first step is to get the values of the gradients at the quadrature points of the finite element field on the present cell. For this, initialize the   <code>FEFaceValues</code>   object corresponding to this side of the face, and extract the gradients using that object.
* 
...
* @code
 ...
 @endcode
* 
*  Then define an abbreviation for the neighbor cell, initialize the   <code>FEFaceValues</code>   object on that cell, and extract the gradients on that cell:
* 
...
* @code
 ...
 @endcode
* 
*  Start with some checks again: get an iterator pointing to the cell behind the present subface and check whether its face is a subface of the one we are considering. If that were not the case, then there would be either a bug in the   <code>neighbor_neighbor</code>   function called above, or
* 
*  -  worse
* 
...
* 
 @endcode
* 
*  Since we allow to use different refinement criteria (global refinement, refinement by the Kelly error indicator, possibly with a weight, and using the dual estimator), define a number of enumeration values, and subsequently a variable of that type. It will default to   <code>dual_weighted_error_estimator</code>  .
* 
...
* 
 @endcode
* 
*  Next to last, a function that is used as a weight to the   <code>RefinementWeightedKelly</code>   class. The default value of this pointer is zero, but you have to set it to some other value if you want to use the   <code>weighted_kelly_indicator</code>   refinement criterion.
* 
...
* @code
 ...
 @endcode
* 
*  Here, we could as well have used   <code>global_refinement</code>   or   <code>weighted_kelly_indicator</code>  . Note that the information given about dual finite elements, dual functional, etc is only important for the given choice of refinement criterion, and is ignored otherwise.
* 
...
* 
 @endcode
* 
*  Then set the description of the test case, i.e. domain, boundary values, and right hand side. These are prepackaged in classes. We take here the description of   <code>Exercise_2_3</code>  , but you can also use   <code>CurvedRidges@<dim@></code>  :
* 
...
* 
 @endcode
* 
*  Next set first a dual functional, then a list of evaluation objects. We choose as default the evaluation of the value at an evaluation point, represented by the classes   <code>PointValueEvaluation</code>   in the namespaces of evaluation and dual functional classes. You can also set the   <code>PointXDerivativeEvaluation</code>   classes for the x-derivative instead of the value at the evaluation point.       
...
* First let's look what the program actually computed. On the seventhgrid, primal and dual numerical solutions look like this (using acolor scheme intended to evoke the snow-capped mountains ofColorado that the original author of this program now callshome):  <table align="center">
  <tr>
    <td width="50%">
...
* 
* From the computed results, we can generate two graphs: one that showsthe convergence of the error   $J(u)-J(u_h)$   (taking theextrapolated value as correct) in the point value, and the value thatwe get by adding up computed value   $J(u_h)$   and estimatederror eta (if the error estimator   $eta$   were exact, then the value  $J(u_h)+\eta$   would equal the exact point value, and the errorin this quantity would always be zero; however, since the errorestimator is only a
* 
*  - good
* 
...
* 
* Besides evaluating the values of the solution at a certain point, theprogram also offers the possibility to evaluate the x-derivatives at acertain point, and also to tailor mesh refinement for this. To let theprogram compute these quantities, simply replace the two occurrences of  <code>PointValueEvaluation</code>   in the main function by  <code>PointXDerivativeEvaluation</code>  , and let the program run:
* @code
Refinement cycle: 0
   Number of degrees of freedom=72
...
* 
* The solution looks roughly the same as before (the exact solution ofcourse   <em>  is  </em>   the same, only the grid changed a little), but thedual solution is now different. A close-up around the point ofevaluation shows this:  <table align="center">
  <tr>
    <td width="50%">
...
</table>  This time, the grids in refinement cycles 0, 5, 6, 7, 8, and 9 looklike this:
...
* Note the asymmetry of the grids compared with those we obtained forthe point evaluation. This is due to the fact that the domain and the primalsolution may be symmetric about the diagonal, but the   $x$  -derivative isnot, and the latter enters the refinement criterion.
* 
...
* 
*  - .0528223... plus or minus0.0000005. We get this reference value by computing on finer grid aftersome more mesh refinements, with approximately 130,000 cells.Recall that if the error is   $O(1/N)$   in the optimal case, thentaking a mesh with ten times more cells gives us one additional digitin the result.
* 
...
* After an initial phase where the true error changes its sign, theestimated error matches it quite well, again. Also note the dramaticimprovement in the error when using the estimated error to correct thecomputed value of   $J(u_h)$  .
* 
...
* 
* If instead of the   <code>Exercise_2_3</code>   data set, we choose  <code>CurvedRidges</code>   in the main function, and choose   $(0.5,0.5)$  as the evaluation point, then we can redo thecomputations of the previous example program, to compare whether theresults obtained with the help of the dual weighted error estimatorare better than those we had previously.
* 
