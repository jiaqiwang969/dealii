 ...
 *
 *  This is a variant of   step-16   with the only change that we are using
 * theMeshWorker framework with the pre-made LocalIntegrator helper classes
 * insteadof manually assembling the matrices. The details of this framework
 * on how it is used in practice will be explainedas part of this tutorial
 * program. <a name="Thetestcase"></a><h3>The testcase</h3>
 *
 ...
 * functions for integration on cells and boundary and interior faces. This is
 * done by the following class. In the constructor, we tell the loop that cell
 ...
 * (MeshWorker::LocalResults::n_matrices()).   The matrices are accessed
 * through   MeshWorker::LocalResults::matrix(),   which takes the number of
 * the matrix as its first argument. The second argument is only used for
 * integrals over faces when there are two matrices for each test function
 * used. Then, a second matrix with indicator 'true' would exist with the same
 * index.
 *
 *
 * MeshWorker::IntegrationInfo   provides one or several FEValues objects,
 * which below are used by   LocalIntegrators::Laplace::cell_matrix()   or
 ...
 * the boundary values as well, so we pass the
 ...
 * with these sparsity patterns. It may be worth pointing out that the
 * interface matrices only have entries for degrees of freedom that sit at or
 * next to the interface between coarser and finer levels of the mesh. They
 * are therefore even sparser than the matrices on the individual levels of
 * our multigrid hierarchy. If we were more concerned about memory usage (and
 * possibly the speed with which we can multiply with these matrices), we
 * should use separate and different sparsity patterns for these two kinds of
 * matrices.
 *
 *
 * @code
 ...
 *
 *
 *  - a   MeshWorker::IntegrationInfoBox   object, which will provide all the required data in quadrature points on the cell. This object can be seen as an extension of FEValues, providing a lot more useful information,
 *
 *
 *
 ...
 * functions are almost unchanged from those in   step-6  , with the exception
 * of one minor difference: we generate output in VTK format, to use the more
 * modern visualization programs available today compared to those that were
 ...
 *
 *  As in   step-16  , the solution looks like this on the finest mesh: <p
 * align="center"> <img
 ...
 * to   step-16   but isfunctionally the same and shows the same convergence
 * properties:<pre>  DEAL::Cycle   0DEAL::   Number of active cells:
 ...
 * DEAL:cg::Convergence   step 11 value 9.73331e-14</pre>
 *
* <a name="PlainProg"></a><h1> The plain program</h1>  @include "step-16b.cc"
 *
 */
