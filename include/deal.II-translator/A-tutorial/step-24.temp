...
@f]
* Here   $\rho (\mathbf r) $   is the density;   $C_p (\mathbf r) $   is the specificheat;   $\frac{\partial T}{\partial t}(t,\mathbf r)$   is the temperature rise dueto the delivered microwave energy; and   $H(t,\mathbf r)$   is the heatingfunction defined as the thermal energy per time and volume transformed fromdeposited microwave energy.
* Let us assume that tissues have heterogeneous dielectric properties buthomogeneous acoustic properties. The basic acoustic generation equation in anacoustically homogeneous medium can be described as follows: if   $u$   is thevector-valued displacement, then tissue certainly reacts to changes inpressure by acceleration:@f[
\rho \frac{\partial^2}{\partial t^2}u(t,\mathbf r) =
* 
-\nabla p(t,\mathbf r).
@f]Furthermore, it contracts due to excess pressure and expands based on changes in temperature:@f[
\nabla \cdot u(t,\mathbf r) =
* 
-\frac{p(t,\mathbf r)}{\rho c_0^2}+\beta T(t,\mathbf r) .
@f]Here,   $\beta$   is a thermoexpansion coefficient.
...
* In the inverse problem, it is the initial condition   $b(\mathbf r) = c_0^2 \lambda a(\mathbf r)$   thatone would like to recover, since it is a map of absorption strengths formicrowave energy, and therefore presumably an indicator to discern healthyfrom diseased tissue.
...
* In general, this is a hard problem: Good absorbing boundary conditions arenonlinear and/or numerically very expensive. We therefore opt for a simplefirst order approximation to absorbing boundary conditions that reads@f[
\frac{\partial\bar{p}}{\partial\mathbf n} =
* 
-\frac{1}{c_0} \frac{\partial\bar{p}}{\partial t}
@f]Here,   $\frac{\partial\bar{p}}{\partial\mathbf n}$   is the normal derivative atthe boundary. It should be noted that this is not a particularly good boundarycondition, but it is one of the very few that are reasonably simple to implement.
* 

* <a name="Weakformanddiscretization"></a><h3>Weak form and discretization</h3>
* 

* As in   step-23  , one first introduces a second variable, which isdefined as the derivative of the pressure potential:@f[
v = \frac{\partial\bar{p}}{\partial t}
@f]
* With the second variable, one then transforms the forward problem intotwo separate equations:@f{eqnarray*}
\bar{p}_{t}
* 
- v & = & 0 \\
\Delta\bar{p}
...
- \frac{1}{c_0^2}\,v_{t} & = & f
@f}
* with initial conditions:@f{eqnarray*}
\bar{p}(0,\mathbf r) & = & b(r) \\
v(0,\mathbf r)=\bar{p}_t(0,\mathbf r) & = & 0.
@f}
* Note that we have introduced a right hand side   $f(t,\mathbf r)$   here to showhow to derive these formulas in the general case, although in the applicationto the thermoacoustic problem   $f=0$  .
* The semi-discretized, weak version of this model, using the general   $\theta$   schemeintroduced in   step-23   is then:@f{eqnarray*}
...
@f}
* where   $\phi$   is an arbitrary test function, and where we have used theabsorbing boundary condition to integrate by parts:absorbing boundary conditions are incorporated into the weak form by using@f[
\int_\Omega\varphi \, \Delta p\; dx =
* 
-\int_\Omega\nabla \varphi \cdot \nabla p dx +
\int_{\partial\Omega}\varphi \frac{\partial p}{\partial {\mathbf n}}ds.
@f]
* From this we obtain the discrete model by introducing a finite number of shapefunctions, and get@f{eqnarray*}
...
@f}
* The matrices   $M$   and   $A$   are here as in   step-23  , and theboundary mass matrix@f[
	B_{ij} = \left(\varphi_i,\varphi_j\right)_{\partial\Omega}
@f]results from the use of absorbing boundary conditions.
* Above two equations can be rewritten in a matrix form with the pressure and its derivative asan unknown vector:@f[
\left(\begin{array}{cc}
 M         &
* 
...
\left(\begin{array}{l}
 ...
 (-c_{0}^{2}k (1-\theta)A+c_0 B)\bar{p}^{n-1} +Mv^{n-1}
                \end{array}\right)
@f]
* By simple transformations, one then obtains two equations forthe pressure potential and its derivative, just as in the previous tutorial program:@f{eqnarray*}
...
* Compared to   step-23  , this programs adds the treatment of asimple absorbing boundary conditions. In addition, it deals with data obtainedfrom actual experimental measurements. To this end, we need to evaluate thesolution at points at which the experiment also evaluates a real pressurefield. We will see how to do that using the   VectorTools::point_value   functionfurther down below.
* 
...
* In the derivation of the initial value problem for the wave equation, weinitially found that the equation had the derivative of a Dirac delta functionas a right hand side:@f[
\Delta p-\frac{1}{c_0^2} \frac{\partial^2 p}{\partial t^2} = \lambda
a(\mathbf r)\frac{d\delta(t)}{dt}.
@f]In order to see how to transform this single equation into the usual statementof a PDE with initial conditions, let us make the assumption that thephysically quite reasonable medium is at rest initially, i.e.   $p(t,\mathbf
r)=\frac{\partial p(t,\mathbf r)}{\partial t}=0$   for   $t<0$  . Next, let us formthe indefinite integral with respect to time of both sides:@f[
\int^t \Delta p\; dt
* 
...
@f]This immediately leads to the statement@f[
P(t,\mathbf r)
* 
- \frac{1}{c_0^2} \frac{\partial p}{\partial t}
=
\lambda a(\mathbf r) \delta(t),
@f]where   $P(t,\mathbf r)$   is such that   $\frac{dP(t,\mathbf r)}{dt}=\Delta
p$  . Next, we form the (definite) integral over time from   $t=-\epsilon$   to  $t=+\epsilon$   to find@f[
...
@f]If we use the property of the delta function that   $\int_{-\epsilon}^{\epsilon}
\delta(t)\; dt = 1$  , and assume that   $P$   is a continuous function in time, we findas we let   $\epsilon$   go to zero that@f[
* 
...
=
\lambda a(\mathbf r).
@f]In other words, using that   $p(-\epsilon,\mathbf r)=0$  , we retrieve the initialcondition@f[
  \frac{1}{c_0^2} p(0,\mathbf r)
  =
  \lambda a(\mathbf r).
@f]At the same time, we know that for every   $t>0$   the delta function is zero, sofor   $0<t<T$   we get the equation@f[
\Delta p-\frac{1}{c_0^2} \frac{\partial^2 p}{\partial t^2} = 0.
@f]Consequently, we have obtained a representation of the wave equation and oneinitial condition from the original somewhat strange equation.
* Finally, because we here have an equation with two time derivatives, we stillneed a second initial condition. To this end, let us go back to the equation@f[
\Delta p-\frac{1}{c_0^2} \frac{\partial^2 p}{\partial t^2} = \lambda
a(\mathbf r)\frac{d\delta(t)}{dt}.
@f]and integrate it in time from   $t=-\epsilon$   to   $t=+\epsilon$  . This leads to@f[
P(\epsilon)-P(-\epsilon)
* 
...
@f]Using integration by parts of the form@f[
...
@f]where we use that   $\delta(\pm \epsilon)=0$   and inserting   $\varphi(t)=1$  , wesee that in fact@f[
  \int_{-\epsilon}^{\epsilon}\frac{d\delta(t)}{dt} \; dt
  =
  0.
@f]
* Now, let   $\epsilon\rightarrow 0$  . Assuming that   $P$   is a continuous function intime, we see that@f[
...
-
                       \frac{\partial p(-\epsilon)}{\partial t}
		       \rightarrow 0.
@f]However, we have assumed that   $\frac{\partial p(-\epsilon)}{\partial t}=0$  .Consequently, we obtain as the second initial condition that@f[
  \frac{\partial p(0)}{\partial t} = 0,
@f]completing the system of equations.
* 
...
*   The upshot of all this is that we're not quite sure what exactly we should take for   $h$  . The function   GridTools::minimal_cell_diameter   computes the minimal diameter of all cells. If the cells were all squares or cubes, then the minimal edge length would be the minimal diameter divided by   <code>std::sqrt(dim)</code>  . We simply generalize this, without theoretical justification, to the case of non-uniform meshes.   
...
* This movie shows the thermoacoustic waves generated by a single small absorberpropagating in the medium (in our simulation, we assume the medium is mineraloil, which has a acoustic speed of 1.437   $\frac{mm}{\mu s}$  ):
*   <img src="https://www.dealii.org/images/steps/developer/step-24.one_movie.gif" alt="">  
* For a single absorber, we of course have to change the  <code>InitialValuesP</code>   class accordingly.
...
* Experimental data and our simulated data are compared in the following twofigures:  <table width="100%">
<tr>
<td>
