 ...
 * <ul>
 * <li><a href="#Motivation"> Motivation </a><a href="#Motivation"> Motivation
 * </a>
 * <li><a href="#Projectionmethods"> Projection methods </a><a
 * href="#Projectionmethods"> Projection methods </a>
 * <li><a href="#TheFullyDiscreteSetting"> The Fully Discrete Setting </a><a
 * href="#TheFullyDiscreteSetting"> The Fully Discrete Setting </a>
 ...
 * <code>NavierStokesProjection</code> class</a><a
 * href="#ThecodeNavierStokesProjectioncodeclass">The
 * <code>NavierStokesProjection</code> class</a>
 * <ul>
 * <li><a href="#codeNavierStokesProjectionNavierStokesProjectioncode">
 * <code>NavierStokesProjection::NavierStokesProjection</code> </a><a
 * href="#codeNavierStokesProjectionNavierStokesProjectioncode">
 * <code>NavierStokesProjection::NavierStokesProjection</code> </a>
 * <li><a
 * href="#codeNavierStokesProjectioncreate_triangulation_and_dofscode"><code>NavierStokesProjection::create_triangulation_and_dofs</code></a><a
 ...
 * <code>NavierStokesProjection::initialize</code> </a><a
 * href="#codeNavierStokesProjectioninitializecode">
 * <code>NavierStokesProjection::initialize</code> </a>
 * <li><a href="#codeNavierStokesProjectioninitialize__matricescode">
 * <code>NavierStokesProjection::initialize_*_matrices</code> </a><a
 * href="#codeNavierStokesProjectioninitialize__matricescode">
 * <code>NavierStokesProjection::initialize_*_matrices</code> </a>
 * <li><a href="#codeNavierStokesProjectionruncode">
 * <code>NavierStokesProjection::run</code> </a><a
 * href="#codeNavierStokesProjectionruncode">
 * <code>NavierStokesProjection::run</code> </a>
 * <li><a
 * href="#codeNavierStokesProjectiondiffusion_stepcode"><code>NavierStokesProjection::diffusion_step</code></a><a
 ...
 * <code>NavierStokesProjection::assemble_advection_term</code> </a><a
 * href="#codeNavierStokesProjectionassemble_advection_termcode">
 * <code>NavierStokesProjection::assemble_advection_term</code> </a>
 * <li><a
 * href="#codeNavierStokesProjectionprojection_stepcode"><code>NavierStokesProjection::projection_step</code></a><a
 ...
 * <code>NavierStokesProjection::update_pressure</code> </a><a
 * href="#codeNavierStokesProjectionupdate_pressurecode">
 * <code>NavierStokesProjection::update_pressure</code> </a>
 * <li><a href="#codeNavierStokesProjectionoutput_resultscode">
 * <code>NavierStokesProjection::output_results</code> </a><a
 * href="#codeNavierStokesProjectionoutput_resultscode">
 ...
 * <ul>
 * <li><a href="#Re100"> Re = 100 </a><a href="#Re100"> Re = 100 </a>
 * <li><a href="#Re500"> Re = 500 </a><a href="#Re500"> Re = 500 </a>
 ...
 *
 *  <a name="Motivation"></a><a name="Motivation"></a><h3> Motivation </h3>
 * The purpose of this program is to show how to effectively solve the
 * incompressible time-dependentNavier-Stokes equations. These equations
 ...
 * \nabla \cdot u = 0,
 * @f}
 * where   $u$   represents the velocity of the flow and   $p$   the pressure. This system of equations is supplemented bythe initial condition@f[
 * u |_{t=0} = u_0,
 * @f]with   $u_0$   sufficiently smooth and solenoidal, and suitable boundary conditions. For instance, an admissible boundarycondition, is@f[
 * u|_{\partial\Omega} = u_b.
 * @f]It is possible to prescribe other boundary conditions as well. In the test case that we solve here the boundaryis partitioned into two disjoint subsets   $\partial\Omega = \Gamma_1 \cup \Gamma_2$   and we have@f[
 * u|_{\Gamma_1} = u_b,
 * @f]and@f[
 * u\times n|_{\Gamma_2} = 0, \quad p|_{\Gamma_2} = 0 @f]where   $n$   is the
 * outer unit normal. The boundary conditions on   $\Gamma_2$   are oftenused
 * to model outflow conditions. In previous tutorial programs (see for
 * instance   step-20   and  step-22  ) we have seenhow to solve the
 * time-independent Stokes equations using a Schur complement approach. For
 * thetime-dependent case, after time discretization, we would arrive at a
 * system like
 * @f{align*}
 * \frac1\tau u^k
 *
 ...
 * similar to the Stokes system and thusit could be solved using a Schur
 * complement approach, it turns out that the condition number of theSchur
 * complement is proportional to   $\tau^{-2}$  . This makes the system
 * verydifficult to solve, and means that for the Navier-Stokes equations,
 * this isnot a useful avenue to the solution. <a name="Projection"></a><a
 * name="Projectionmethods"></a><h3> Projection methods </h3>
 *
 * Rather, we need to come up with a different approach to solve the time-dependent Navier-Stokesequations. The difficulty in their solution comes from the fact that the velocity and the pressure are coupledthrough the constraint@f[
 * \nabla \cdot u = 0,
 * @f]for which the pressure is the Lagrange multiplier.Projection methods aim at decoupling this constraint from the diffusion (Laplace) operator.
 * Let us shortly describe how the projection methods look like in a semi-discrete setting. The objective is toobtain a sequence of velocities   $\{u^k\}$   and pressures   $\{p^k\}$  . We willalso obtain a sequence   $\{\phi^k\}$   of auxiliary variables.Suppose that from the initial conditions, and an application of a first order method we have found  $(u^0,p^0,\phi^0=0)$   and   $(u^1,p^1,\phi^1=p^1-p^0)$  . Then the projection method consists of the following steps:  <ul>      <li>   <b>Step 0</b>: Extrapolation. Define:  @f[
 * u^\star = 2u^k
 *
 * - u^{k-1}, \quad p^\sharp = p^k + \frac43 \phi^k
 *
 * - \frac13 \phi^{k-1}.
 * @f]    <li>   <b>Step 1</b>: Diffusion step. We find   $u^{k+1}$   that
 * solves the single  linear equation  @f[ \frac1{2\tau}\left( 3u^{k+1}
 *
 * - 4u^k + u^{k-1} \right) + u^\star \cdot\nabla u^{k+1} + \frac12 \left(
 ...
 *
 *
 * -\nu \Delta u^{k+1} + \nabla p^\sharp = f^{k+1}, \quad u^{k+1}|_{\Gamma_1}
 * = u_b, \quad u^{k+1} \times n|_{\Gamma_2} = 0. @f]
 * <li>   <b>Step 2</b>: Projection. Find   $\phi^{k+1}$   that solves  @f[
 * \Delta \phi^{k+1} = \frac3{2\tau} \nabla \cdot u^{k+1}, \quad \partial_n
 * \phi^{k+1}|_{\Gamma_1} = 0, \quad \phi^{k+1}|_{\Gamma_2} = 0
 * @f]    <li>   <b>Step 3</b>: Pressure correction. Here we have two options:      <ul>          <li>   <i>Incremental Method in Standard Form</i>. The pressure is updated by:      @f[
 * p^{k+1} = p^k + \phi^{k+1}.
 * @f]        <li>   <i>Incremental Method in Rotational Form</i>. In this
 * case      @f[ p^{k+1} = p^k + \phi^{k+1}
 *
 * - \nu \nabla \cdot u^{k+1}.
 * @f]      </ul>    </ul>
 * Without going into details, let us remark a few things about the projection methods that we have just described:  <ul>      <li>   The advection term   $u\cdot\nabla u$   is replaced by its <i>skew symmetric form</i>  @f[
 * u \cdot \nabla u + \frac12 \left( \nabla\cdot u \right) u.
 * @f]  This is consistent with the continuous equation (because
 * $\nabla\cdot u = 0$  ,  though this is not true pointwise for the discrete
 * solution) and it is needed to  guarantee unconditional stability of the
 * time-stepping scheme. Moreover, to linearize the term we use the second
 * order extrapolation   $u^\star$   of    $u^{k+1}$  .    <li>   The
 * projection step is a realization of the Helmholtz decomposition  @f[
 * L^2(\Omega)^d = H \oplus \nabla H^1_{\Gamma_2}(\Omega),
 * @f]  where  @f[
 * H = \left\{ v \in L^2(\Omega)^d:\  \nabla\cdot v =0, \  v\cdot
 * n|_{\Gamma_1} = 0 \right\},
 * @f]  and  @f[
 * H^1_{\Gamma_2}(\Omega) = \left\{ q \in H^1(\Omega):\ q|_{\Gamma_2} = 0
 * \right\}.
 * @f]  Indeed, if we use this decomposition on   $u^{k+1}$   we obtain  @f[
 * u^{k+1} = v^{k+1} + \nabla \left( \frac{2\tau}{3}  \phi^{k+1} \right),
 * @f]  with   $v^{k+1}\in H$  . Taking the divergence of this equation we arrive at the projection equation.    <li>   The more accurate of the two variants outlined above is the rotational  one. However, the program below implements both variants. Moreover, in the author's experience,  the standard form is the one that should be used if, for instance, the viscosity   $\nu$   is variable.  </ul>
 *
 * <p>  The standard incremental scheme and the rotational incremental scheme were first considered by van Kan in  <ul>      <li>   J. van Kan, "A second-order accurate pressure-correction scheme for viscous incompressible flow",       SIAM Journal on Scientific and Statistical Computing, vol. 7, no. 3, pp. 870–891, 1986  </ul>  and is analyzed by Guermond in  <ul>      <li>   J.-L. Guermond, "Un résultat de convergence d’ordre deux en temps pour                        l’approximation des équations de Navier–Stokes par une technique de projection incrémentale",       ESAIM: Mathematical Modelling and Numerical Analysis, vol. 33, no. 1, pp. 169–189, 1999  </ul>  for the case   $\nu = 1$  .It turns out that this technique suffers from unphysical boundary conditions for the kinematic pressure thatlead to reduced rates of convergence. To prevent this, Timmermans et al. proposed in  <ul>      <li>   L. Timmermans, P. Minev, and F. Van De Vosse,       "An approximate projection scheme for incompressible flow using spectral elements",       International Journal for Numerical Methods in Fluids, vol. 22, no. 7, pp. 673–688, 1996  </ul>  the rotational pressure-correction projection method that uses a divergence correction for the kinematic pressure.A thorough analysis for scheme has first been performed in  <ul>      <li>   J.-L. Guermond and J. Shen, "On the error estimates for the rotational pressure-correction projection methods",       Mathematics of Computation, vol. 73, no. 248, pp. 1719–1737, 2004  </ul>  for the Stokes problem.  </p>
 * <a name ="fullydiscrete"></a><a name="TheFullyDiscreteSetting"></a><h3> The
 * Fully Discrete Setting </h3>
 * To obtain a fully discrete setting of the method we, as always, need a variational formulation. There is onesubtle issue here given the nature of the boundary conditions. When we multiply the equation by a suitable testfunction one of the term that arises is@f[
 *
 *
 *
 *
 ...
 * @f]If we, say, had Dirichlet boundary conditions on the whole boundary then after integration by parts we wouldobtain@f[
 *
 *
 *
 *
 ...
 * \nabla v. @f]One of the advantages of this formulation is that it fully
 * decouples the components of the velocity. Moreover,they all share the same
 * system matrix. This can be exploited in the program.
 * However, given the nonstandard boundary conditions, to be able to take them into account we need to usethe following %identity@f[
 * \Delta u = \nabla\nabla\cdot u
 *
 * - \nabla\times\nabla\times u,
 * @f]so that when we integrate by parts and take into account the boundary conditions we obtain@f[
 *
 *
 *
 *
 ...
 * -\nu \int_\Omega \Delta u \cdot v = \nu \int_\Omega \left[ \nabla \cdot u
 * \nabla \cdot v + \nabla \times u \nabla \times v \right],
 * @f]which is the form that we would have to use. Notice that this couples the components of the velocity.Moreover, to enforce the boundary condition on the pressure, we need to rewrite@f[
 * \int_\Omega \nabla p \cdot v =
 *
 * -\int_\Omega p \nabla \cdot v + \int_{\Gamma_1} p v\cdot n +
 * \int_{\Gamma_2} p v\cdot n =
 *
 * -\int_\Omega p \nabla \cdot v, @f]where the boundary integral in
 * $\Gamma_1$   equals zero given the boundary conditions for the velocity,and
 * the one in   $\Gamma_2$   given the boundary conditions for the pressure.
 * In the simplified case where the boundary   $\Gamma_2$   is %parallel to a coordinate axis, which holds forthe testcase that we carry out below, it can actually be shown that@f[
 * \nu \int_\Omega \nabla u : \nabla v = \nu \int_\Omega \left[ \nabla \cdot u \nabla \cdot v
 *                                  + \nabla \times u \nabla \times v \right].
 * @f]This issue is not very often addressed in the literature. For more information the reader can consult, forinstance,  <ul>      <li>   J.-L. GUERMOND, L. QUARTAPELLE, On the approximation of the unsteady Navier-Stokes equations by  finite element projection methods, Numer. Math., 80  (1998) 207-238    <li>   J.-L. GUERMOND, P. MINEV, J. SHEN, Error analysis of pressure-correction schemes for the  Navier-Stokes equations with open boundary conditions, SIAM J. Numer. Anal., 43  1 (2005) 239--258.  </ul>
 *
 *
 * <a name = "implementation"></a><a name="Implementation"></a><h3>
 * Implementation </h3>
 *
 *  Our implementation of the projection methods follows <i>verbatim</i> the
 * description given above. We must note,however, that as opposed to most
 * other problems that have several solution components, we do not
 * usevector-valued finite elements. Instead, we use separate finite elements
 * the components of the velocityand the pressure, respectively, and use
 * different   <code>DoFHandler</code>  's for those as well. The mainreason
 * for doing this is that, as we see from the description of the scheme, the
 * <code>dim</code>   componentsof the velocity and the pressure are
 * decoupled. As a consequence, the equations for all the velocity
 * componentslook all the same, have the same system matrix, and can be solved
 * in %parallel. Obviously, this approachhas also its disadvantages. For
 * instance, we need to keep several   <code>DoFHandler</code>  s and
 * iteratorssynchronized when assembling matrices and right hand sides;
 * obtaining quantities that are inherent tovector-valued functions (e.g.
 * divergences) becomes a little awkward, and others. <a name
 * ="testcase"></a><a name="TheTestcase"></a><h3> The Testcase </h3>
 *
 *  The testcase that we use for this program consists of the flow around a
 * square obstacle. The geometry isas follows: <img
 * src="https://www.dealii.org/images/steps/developer/step-35.geometry.png"
 * alt=""> with   $H=4.1$  , making the geometry slightly non-symmetric.
 * We impose no-slip boundary conditions on both the top and bottom walls and the obstacle. On the left side wehave the inflow boundary condition@f[
 * u =
 * \left( \begin{array}{c} 4 U_m y (H-y)/H^2 \\ 0 \end{array} \right),
 * @f]with   $U_m = 1.5$  , i.e. the inflow boundary conditions correspond to Poiseuille flow for this configuration.Finally, on the right vertical wall we impose the condition that the vertical component of the velocityand the pressure should both be zero.The final time   $T=10$  .
 *
 ...
 * @endcode
 *
 * <a name="Runtimeparameters"></a>  <h3>Run time parameters</h3> Since our
 * method has several parameters that can be fine-tuned we put them into an
 * external file, so that they can be determined at run-time. This includes,
 * in particular, the formulation of the equation for the auxiliary variable
 * $\phi$  , for which we declare an   <code>enum</code>  . Next, we declare a
 * class that is going to read and store all the parameters that our program
 * needs to run.
 *
 *
 * @code
 ...
 * @endcode
 *
 * In the constructor of this class we declare all the parameters. The details
 * of how this works have been discussed elsewhere, for example in   step-29
 * .
 *
 *
 * @code
 ...
 * <code>NavierStokesProjection</code> class</h3>
 *
 *
 ...
 *
 * The next
 * few structures and functions are for doing various things in parallel. They
 * follow the scheme laid out in   @ref threads  , using the WorkStream class.
 * As explained there, this requires us to declare two structures for each of
 * the assemblers, a per-task data and a scratch data structure. These are
 * then handed over to functions that assemble local contributions and that
 * copy these local contributions to the global objects. One of the things
 * that are specific to this program is that we don't just have a single
 * DoFHandler object that represents both the velocities and the pressure, but
 * we use individual DoFHandler objects for these two kinds of variables. We
 * pay for this optimization when we want to assemble terms that involve both
 ...
 * triangulation but different DoFHandlers. To do this in practice, we declare
 ...
 * @endcode
 *
 * The same general layout also applies to the following classes and functions
 * implementing the assembly of the advection term:
 *
 *
 * @code
 ...
 * <code>NavierStokesProjection::NavierStokesProjection</code> </h4>
 *
 *
 * In the constructor, we just read all the data from the
 * <code>Data_Storage</code>   object that is passed as an argument, verify
 * that the data we read is reasonable and, finally, create the triangulation
 ...
 * <code>NavierStokesProjection::initialize</code> </h4>
 *
 *
 ...
 * <code>NavierStokesProjection::initialize_*_matrices</code> </h4>
 *
 *
 ...
 * <code>NavierStokesProjection::run</code> </h4>
 *
 *
 ...
 * <code>dt</code>   until   <code>T</code>  . Its second parameter,
 * <code>verbose</code>   indicates whether the function should output
 * information what it is doing at any given moment: for example, it will say
 * whether we are working on the diffusion, projection substep; updating
 * preconditioners etc. Rather than implementing this output using code like
 ...
 *
 *   const auto n_steps = static_cast<unsigned int>((T
 *
 * - t_0) / dt);
 ...
 * this in %parallel, using the   Threads::new_task   function which makes
 ...
 * <code>NavierStokesProjection::assemble_advection_term</code> </h4>
 *
 *
 * The following
 * few functions deal with assembling the advection terms, which is the part
 * of the system matrix for the diffusion step that changes at every time
 * step. As mentioned above, we will run the assembly loop over all cells in
 * %parallel, using the WorkStream class and other facilities as described in
 * the documentation module on   @ref threads  .
 *
 *
 * @code
 * template <int dim>
 ...
 * <code>NavierStokesProjection::update_pressure</code> </h4>
 *
 *
 *  This is the pressure update step of the projection method. It implements the standard formulation of the method, that is @f[ p^{n+1} = p^n +
 ...
 * <code>NavierStokesProjection::output_results</code> </h4>
 *
 *
 ...
 * output. We will not elaborate on this process here, but rather refer to
 * step-32  , where a similar procedure is used (and is documented) to create
 * a joint DoFHandler object for all variables. Let us also note that we here
 * compute the vorticity as a scalar quantity in a separate function, using
 * the   $L^2$   projection of the quantity   $\text{curl} u$   onto the
 * finite element space used for the components of the velocity. In principle,
 ...
 *
 *  <a name="Re100"></a><a name="Re100"></a><h3> Re = 100 </h3>
 *
 *  We run the code with the following   <code>parameter-file.prm</code>  ,
 * which can be found in thesame directory as the source:
 * @verbatim
 ...
 * ,we do not get any kind of output besides the number of the time stepthe
 * program is currently working on.If we were to set it to   <code>true</code>
 * we would get information on what the program is doing andhow many steps
 * each iterative process had to make to converge, etc. Let us plot the
 * obtained results for   $t=1,5,12,20,25$   (i.e. time steps200, 1000, 2400,
 * 4000, and 5000), where in the left column we show thevorticity and in the
 ...
 * development and extension of a vortex chainbehind the obstacles, with the
 * sign of the vorticity indicatingwhether this is a left or right turning
 * vortex.
 *
 *  <a name="Re500"></a><a name="Re500"></a><h3> Re = 500 </h3>
 *
 *  We can change the Reynolds number,   $Re$  , in the parameter file to
 * avalue of   $500$  . Doing so, and reducing the time step somewhat as
 * well,yields the following images at times   $t=20,40$  : <table> <tr> <td>
 * <img
 ...
 * we observe unphysical oscillations, especiallyfor the vorticity. The
 * discretization scheme has now difficulties in correctlyresolving the flow,
 * which should still be laminar and well-organized.These phenomena are
 * typical of discretization schemes that lack robustnessin under-resolved
 * scenarios, where under-resolved means that the Reynoldsnumber computed with
 * the mesh size instead of the physical dimensions ofthe geometry is large.
 * We look at a zoom at the region behind the obstacle, andthe mesh size we
 * have there:
 *
 *  <img
 ...
 * simulation with one moremesh refinement set in the parameter file: <img
 ...
 * While we can expect thatfurther refining the mesh will suppress the
 * remaining oscillations as well,one should take measures to obtain a robust
 * scheme in the limit of coarseresolutions, as described below.
 *
 *  <a name="extensions"></a><a name="Possibilitiesforextensions"></a><h3>
 * Possibilities for extensions </h3>
 *
 * This program can be extended in the following directions:  <ul>      <li>   Adaptive mesh refinement: As we have seen, we computed everything on a single fixed mesh.  Using adaptive mesh refinement can lead to increased accuracy while not significantly increasing the  computational time.
 ...
 * <li>   High Reynolds %numbers: As we can see from the results, increasing
 * the Reynolds number changes significantly  the behavior of the
 * discretization scheme. Using well-known stabilization techniques we could
 * be able to  compute the flow in this, or many other problems, when the
 * Reynolds number is very large and where computational  costs demand spatial
 * resolutions for which the flow is only marginally resolved, especially for
 * 3D turbulent  flows.
