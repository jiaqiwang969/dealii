...
<ol>
  <li> <a href="#Intro" class=bold>Introduction</a><a href="#Intro" class=bold>Introduction</a>
    <ul>
        ...
        <li><a href="#Projectionmethods"> Projection methods </a><a href="#Projectionmethods"> Projection methods </a>
        <li><a href="#TheFullyDiscreteSetting"> The Fully Discrete Setting </a><a href="#TheFullyDiscreteSetting"> The Fully Discrete Setting </a>
  ...
  <li> <a href="#CommProg" class=bold>The commented program</a><a href="#CommProg" class=bold>The commented program</a>
    <ul>
        ...
        <li><a href="#ThecodeNavierStokesProjectioncodeclass">The <code>NavierStokesProjection</code> class</a><a href="#ThecodeNavierStokesProjectioncodeclass">The <code>NavierStokesProjection</code> class</a>
      <ul>
        <li><a href="#codeNavierStokesProjectionNavierStokesProjectioncode"> <code>NavierStokesProjection::NavierStokesProjection</code> </a><a href="#codeNavierStokesProjectionNavierStokesProjectioncode"> <code>NavierStokesProjection::NavierStokesProjection</code> </a>
        <li><a href="#codeNavierStokesProjectioncreate_triangulation_and_dofscode"><code>NavierStokesProjection::create_triangulation_and_dofs</code></a><a href="#codeNavierStokesProjectioncreate_triangulation_and_dofscode"><code>NavierStokesProjection::create_triangulation_and_dofs</code></a>
        <li><a href="#codeNavierStokesProjectioninitializecode"> <code>NavierStokesProjection::initialize</code> </a><a href="#codeNavierStokesProjectioninitializecode"> <code>NavierStokesProjection::initialize</code> </a>
        <li><a href="#codeNavierStokesProjectioninitialize__matricescode"> <code>NavierStokesProjection::initialize_*_matrices</code> </a><a href="#codeNavierStokesProjectioninitialize__matricescode"> <code>NavierStokesProjection::initialize_*_matrices</code> </a>
        <li><a href="#codeNavierStokesProjectionruncode"> <code>NavierStokesProjection::run</code> </a><a href="#codeNavierStokesProjectionruncode"> <code>NavierStokesProjection::run</code> </a>
        <li><a href="#codeNavierStokesProjectiondiffusion_stepcode"><code>NavierStokesProjection::diffusion_step</code></a><a href="#codeNavierStokesProjectiondiffusion_stepcode"><code>NavierStokesProjection::diffusion_step</code></a>
        <li><a href="#codeNavierStokesProjectionassemble_advection_termcode"> <code>NavierStokesProjection::assemble_advection_term</code> </a><a href="#codeNavierStokesProjectionassemble_advection_termcode"> <code>NavierStokesProjection::assemble_advection_term</code> </a>
        <li><a href="#codeNavierStokesProjectionprojection_stepcode"><code>NavierStokesProjection::projection_step</code></a><a href="#codeNavierStokesProjectionprojection_stepcode"><code>NavierStokesProjection::projection_step</code></a>
        <li><a href="#codeNavierStokesProjectionupdate_pressurecode"> <code>NavierStokesProjection::update_pressure</code> </a><a href="#codeNavierStokesProjectionupdate_pressurecode"> <code>NavierStokesProjection::update_pressure</code> </a>
        <li><a href="#codeNavierStokesProjectionoutput_resultscode"> <code>NavierStokesProjection::output_results</code> </a><a href="#codeNavierStokesProjectionoutput_resultscode"> <code>NavierStokesProjection::output_results</code> </a>
      </ul>
...
@f]where   $n$   is the outer unit normal. The boundary conditions on   $\Gamma_2$   are oftenused to model outflow conditions.
* In previous tutorial programs (see for instance   step-20   and  step-22  ) we have seenhow to solve the time-independent Stokes equations using a Schur complement approach. For thetime-dependent case, after time discretization, we would arrive at a system like
* @f{align*}
  \frac1\tau u^k
* 
- \nu \Delta u^k + \nabla p^k = F^k, \\
  \nabla \cdot u^k = 0,
@f}
* where   $\tau$   is the time-step. Although the structure of this system is similar to the Stokes system and thusit could be solved using a Schur complement approach, it turns out that the condition number of theSchur complement is proportional to   $\tau^{-2}$  . This makes the system verydifficult to solve, and means that for the Navier-Stokes equations, this isnot a useful avenue to the solution.
* <a name="Projection"></a><a name="Projectionmethods"></a><h3> Projection methods </h3>
* 
...
* 
- \frac13 \phi^{k-1}.
  @f]    <li>   <b>Step 1</b>: Diffusion step. We find   $u^{k+1}$   that solves the single  linear equation  @f[
    \frac1{2\tau}\left( 3u^{k+1}
* 
- 4u^k + u^{k-1} \right)
...
*     <li>   <b>Step 2</b>: Projection. Find   $\phi^{k+1}$   that solves  @f[
  ...
  @f]    <li>   <b>Step 3</b>: Pressure correction. Here we have two options:      <ul>          <li>   <i>Incremental Method in Standard Form</i>. The pressure is updated by:      @f[
        p^{k+1} = p^k + \phi^{k+1}.
      @f]        <li>   <i>Incremental Method in Rotational Form</i>. In this case      @f[
        p^{k+1} = p^k + \phi^{k+1}
* 
- \nu \nabla \cdot u^{k+1}.
      @f]      </ul>    </ul>  
...
* <a name ="fullydiscrete"></a><a name="TheFullyDiscreteSetting"></a><h3> The Fully Discrete Setting </h3>
...
- \int_{\partial\Omega} \partial_n u \cdot v
                                    = \nu \int_\Omega \nabla u : \nabla v.
@f]One of the advantages of this formulation is that it fully decouples the components of the velocity. Moreover,they all share the same system matrix. This can be exploited in the program.
...
* 
-\int_\Omega p \nabla \cdot v,
@f]where the boundary integral in   $\Gamma_1$   equals zero given the boundary conditions for the velocity,and the one in   $\Gamma_2$   given the boundary conditions for the pressure.
...
* 
* <a name = "implementation"></a><a name="Implementation"></a><h3> Implementation </h3>
* 

* Our implementation of the projection methods follows <i>verbatim</i> the description given above. We must note,however, that as opposed to most other problems that have several solution components, we do not usevector-valued finite elements. Instead, we use separate finite elements the components of the velocityand the pressure, respectively, and use different   <code>DoFHandler</code>  's for those as well. The mainreason for doing this is that, as we see from the description of the scheme, the   <code>dim</code>   componentsof the velocity and the pressure are decoupled. As a consequence, the equations for all the velocity componentslook all the same, have the same system matrix, and can be solved in %parallel. Obviously, this approachhas also its disadvantages. For instance, we need to keep several   <code>DoFHandler</code>  s and iteratorssynchronized when assembling matrices and right hand sides; obtaining quantities that are inherent tovector-valued functions (e.g. divergences) becomes a little awkward, and others.
* <a name ="testcase"></a><a name="TheTestcase"></a><h3> The Testcase </h3>
* 
...
*   This includes, in particular, the formulation of the equation for the auxiliary variable   $\phi$  , for which we declare an   <code>enum</code>  . Next, we declare a class that is going to read and store all the parameters that our program needs to run.
* 
...
* 
 @endcode
* 
*   <a name="ThecodeNavierStokesProjectioncodeclass"></a>  <h3>The <code>NavierStokesProjection</code> class</h3>
* 
...
* 
 @endcode
* 
*   <a name="codeNavierStokesProjectionNavierStokesProjectioncode"></a>  <h4> <code>NavierStokesProjection::NavierStokesProjection</code> </h4>
* 

* 
*  In the constructor, we just read all the data from the   <code>Data_Storage</code>   object that is passed as an argument, verify that the data we read is reasonable and, finally, create the triangulation and load the initial data.
* 
...
* 
 ...
 @endcode
* 
*   <a name="codeNavierStokesProjectioncreate_triangulation_and_dofscode"></a>  <h4><code>NavierStokesProjection::create_triangulation_and_dofs</code></h4>
* 
...
* 
 ...
 @endcode
* 
*   <a name="codeNavierStokesProjectioninitializecode"></a>  <h4> <code>NavierStokesProjection::initialize</code> </h4>
* 
...
* 
 ...
 @endcode
* 
*   <a name="codeNavierStokesProjectioninitialize__matricescode"></a>  <h4> <code>NavierStokesProjection::initialize_*_matrices</code> </h4>
* 

* 
*  In this set of methods we initialize the sparsity patterns, the constraints (if any) and assemble the matrices that do not depend on the timestep   <code>dt</code>  . Note that for the Laplace and mass matrices, we can use functions in the library that do this. Because the expensive operations of this function
* 
...
* 
 ...
 @endcode
* 
*   <a name="codeNavierStokesProjectionruncode"></a>  <h4> <code>NavierStokesProjection::run</code> </h4>
* 

* 
*  This is the time marching function, which starting at   <code>t_0</code>   advances in time using the projection method with time step   <code>dt</code>   until   <code>T</code>  .   
*   Its second parameter,   <code>verbose</code>   indicates whether the function should output information what it is doing at any given moment: for example, it will say whether we are working on the diffusion, projection substep; updating preconditioners etc. Rather than implementing this output using code like   <div class=CodeFragmentInTutorialComment>
...
* 
 ...
 @endcode
* 
*   <a name="codeNavierStokesProjectiondiffusion_stepcode"></a>  <h4><code>NavierStokesProjection::diffusion_step</code></h4>
* 
...
* 
 ...
 @endcode
* 
*   <a name="codeNavierStokesProjectionassemble_advection_termcode"></a>  <h4> <code>NavierStokesProjection::assemble_advection_term</code> </h4>
* 
...
* 
 @endcode
* 
*   <a name="codeNavierStokesProjectionprojection_stepcode"></a>  <h4><code>NavierStokesProjection::projection_step</code></h4>
* 
...
* 
 ...
 @endcode
* 
*   <a name="codeNavierStokesProjectionupdate_pressurecode"></a>  <h4> <code>NavierStokesProjection::update_pressure</code> </h4>
* 
...
* 
 ...
 @endcode
* 
*   <a name="codeNavierStokesProjectionoutput_resultscode"></a>  <h4> <code>NavierStokesProjection::output_results</code> </h4>
* 
...
* We run the code with the following   <code>parameter-file.prm</code>  , which can be found in thesame directory as the source:
* @verbatim
...
@endverbatim
* 
* Since the   <code>verbose</code> parameter is set to <code>false</code>  ,we do not get any kind of output besides the number of the time stepthe program is currently working on.If we were to set it to   <code>true</code>   we would get information on what the program is doing andhow many steps each iterative process had to make to converge, etc.
* Let us plot the obtained results for   $t=1,5,12,20,25$   (i.e. time steps200, 1000, 2400, 4000, and 5000), where in the left column we show thevorticity and in the right the velocity field:
*   <table>
  <tr>
...
* We can change the Reynolds number,   $Re$  , in the parameter file to avalue of   $500$  . Doing so, and reducing the time step somewhat as well,yields the following images at times   $t=20,40$  :
*   <table>
  <tr>
...
*     <li>   Adaptive time-stepping: Although there apparently is currently no theory about  projection methods with variable time step,  practice shows that they perform very well.
*     <li>   High Reynolds %numbers: As we can see from the results, increasing the Reynolds number changes significantly  the behavior of the discretization scheme. Using well-known stabilization techniques we could be able to  compute the flow in this, or many other problems, when the Reynolds number is very large and where computational  costs demand spatial resolutions for which the flow is only marginally resolved, especially for 3D turbulent  flows.
*     <li>   Variable density incompressible flows: There are projection-like methods for the case of incompressible  flows with variable density. Such flows play a role if fluids of different  density mix, for example fresh water and salt water, or alcohol and water.
