...
* This is the first of a number of tutorial programs that will finallycover "real" time-dependent problems, not the slightly odd form of timedependence found in   step-18   or the DAE model of   step-21  . In particular, this program introducesthe wave equation in a bounded domain. Later,   step-24  will consider an example of absorbing boundary conditions, and   @ref
step_25   "  step-25  " a kind of nonlinear wave equation producingsolutions called solitons.
* The wave equation in its prototypical form reads as follows: find  $u(x,t), x\in\Omega, t\in[0,T]$   that satisfies@f{eqnarray*}
...
* For all these reasons, for the present program, we choose to use theRothe method for discretization, i.e. we first discretize in time andthen in space. We will not actually use adaptive meshes at all, sincethis involves a large amount of additional code, but we will commenton this some more in the <a href="#Results">results section below</a>.
* 
...
* For the first step, let us take a little detour first: in order todiscretize a second time derivative, we can either discretize itdirectly, or we can introduce an additional variable and transform thesystem into a first order system. In many cases, this turns out to beequivalent, but dealing with first order systems is often simpler. Tothis end, let us introduce@f[
	v = \frac{\partial u}{\partial t},
@f]and call this variable the <i>velocity</i> for obvious reasons. We canthen reformulate the original wave equation as follows:@f{eqnarray*}
...
\\
	...
	\textrm{in}\ \Omega.
@f}
* The advantage of this formulation is that it now only contains firsttime derivatives for both variables, for which it is simple to writedown time stepping schemes. Note that we do not have boundaryconditions for   $v$   at first. However, we could enforce   $v=\frac{\partial
g}{\partial t}$   on the boundary. It turns out in numerical examples that thisis actually necessary: without doing so the solution doesn't look particularlywrong, but the Crank-Nicolson scheme does not conserve energy if one doesn'tenforce these boundary conditions.
* With this formulation, let us introduce the following timediscretization where a superscript   $n$   indicates the number of a timestep and   $k=t_n-t_{n-1}$   is the length of the present time step:\f{eqnarray*}
  \frac{u^n
* 
- u^{n-1}}{k}
* 
...
\f}Note how we introduced a parameter   $\theta$   here. If we chose  $\theta=0$  , for example, the first equation would reduce to  $\frac{u^n
* 
- u^{n-1}}{k}
* 
...
* 
* 
- v^{n-1} = 0$  , which is well-known as theforward or explicit Euler method. On the other hand, if we set  $\theta=1$  , then we would get  $\frac{u^n
* 
- u^{n-1}}{k}
* 
...
* 
* 
- v^n = 0$  , which corresponds to thebackward or implicit Euler method. Both these methods are first orderaccurate methods. They are simple to implement, but they are notreally very accurate.
* The third case would be to choose   $\theta=\frac 12$  . The first of theequations above would then read   $\frac{u^n
* 
- u^{n-1}}{k}
* 
...
* In the program, we will leave   $\theta$   as a parameter, so that it willbe easy to play with it. The results section will show some numericalevidence comparing the different schemes.
* The equations above (called the <i>semidiscretized</i> equationsbecause we have only discretized the time, but not space), can besimplified a bit by eliminating   $v^n$   from the first equation andrearranging terms. We then get\f{eqnarray*}
...
\f}In this form, we see that if we are given the solution  $u^{n-1},v^{n-1}$   of the previous timestep, that we can then solve forthe variables   $u^n,v^n$   separately, i.e. one at a time. This isconvenient. In addition, we recognize that the operator in the firstequation is positive definite, and the second equation looksparticularly simple.
* 

* <a name="Spacediscretization"></a><h3>Space discretization</h3>
* 

* We have now derived equations that relate the approximate(semi-discrete) solution   $u^n(x)$   and its time derivative   $v^n(x)$   attime   $t_n$   with the solutions   $u^{n-1}(x),v^{n-1}(x)$   of the previoustime step at   $t_{n-1}$  . The next step is to also discretize thespatial variable using the usual finite element methodology. To thisend, we multiply each equation with a test function, integrate overthe entire domain, and integrate by parts where necessary. This leadsto\f{eqnarray*}
...
U_i^n\phi_i^n(x)$  , where   $\phi_i^n(x)$   are the shape functions usedfor the discretization of the   $n$  -th time step and   $U_i^n$   are theunknown nodal values of the solution. Similarly,   $v^n(x) \approx
v^n_h(x) = \sum_i V_i^n\phi_i^n(x)$  . Finally, we have the solutions ofthe previous time step,   $u^{n-1}(x) \approx u^{n-1}_h(x) = \sum_i
U_i^{n-1}\phi_i^{n-1}(x)$   and   $v^{n-1}(x) \approx v^{n-1}_h(x) = \sum_i
V_i^{n-1}\phi_i^{n-1}(x)$  . Note that since the solution of the previoustime step has already been computed by the time we get to time step  $n$  ,   $U^{n-1},V^{n-1}$   are known. Furthermore, note that the solutionsof the previous step may have been computed on a different mesh, sowe have to use shape functions   $\phi^{n-1}_i(x)$  .
* If we plug these expansions into above equations and test with thetest functions from the present mesh, we get the following linearsystem:\f{eqnarray*}
...
@f]By consequence, in absence of body forces and constant boundary values, we getthat@f[
	E(t) = \frac 12 \int_\Omega \left(\frac{\partial u}{\partial
	t}\right)^2 + (\nabla u)^2 \; dx
@f]is a conserved quantity, i.e. one that doesn't change with time. Wewill compute this quantity after each timestep. It is straightforward to see that if we replace   $u$   by its finiteelement approximation, and   $\frac{\partial u}{\partial t}$   by the finiteelement approximation of the velocity   $v$  , then@f[
	E(t_n) = \frac 12 \left<V^n, M^n V^n\right>
	+
	\frac 12 \left<U^n, A^n U^n\right>.
@f]As we will see in the results section, the Crank-Nicolson scheme does indeedconserve the energy, whereas neither the forward nor the backward Euler schemedo.
* 

* <a name="WhoareCourantFriedrichsandLewy"></a><h3>Who are Courant, Friedrichs, and Lewy?</h3>
* 

* One of the reasons why the wave equation is nasty to solve numerically is thatexplicit time discretizations are only stable if the time step is smallenough. In particular, it is coupled to the spatial mesh width   $h$  . For thelowest order discretization we use here, the relationship reads@f[
	k\le \frac hc
@f]where   $c$   is the wave speed, which in our formulation of the wave equation hasbeen normalized to one. Consequently, unless we use the implicit schemes with  $\theta>0$  , our solutions will not be numerically stable if we violate thisrestriction. Implicit schemes do not have this restriction for stability, butthey become inaccurate if the time step is too large.
* This condition was first recognized by Courant, Friedrichs, and Lewy &mdash;in 1928, long before computers became available for numericalcomputations! (This result appeared in the German language articleR. Courant, K. Friedrichs and H. Lewy: <i>&Uuml;ber die partiellen
Differenzengleichungen der mathematischen Physik</i>, MathematischeAnnalen, vol. 100, no. 1, pages 32-74, 1928.)This condition on the time step is most frequently just referredto as the <i>CFL</i> condition. Intuitively, the CFL condition saysthat the time step must not be larger than the time it takes a wave tocross a single cell.
* In the program, we will refine the square  $[-1,1]^2$   seven times uniformly, giving a mesh size of   $h=\frac 1{64}$  , whichis what we set the time step to. The fact that we set the time step and meshsize individually in two different places is error prone: it is too easy torefine the mesh once more but forget to also adjust the time step.   @ref
...
* Although the program has all the hooks to deal with nonzero initial andboundary conditions and body forces, we take a simple case where the domain isa square   $[-1,1]^2$   and@f{eqnarray*}
	f &=& 0,
	\\
	u_0 &=& 0,
	\\
...
*   Likewise, we need solution vectors for   $U^n,V^n$   as well as for the corresponding vectors at the previous time step,   $U^{n-1},V^{n-1}$  . The   <code>system_rhs</code>   will be used for whatever right hand side vector we have when solving one of the two linear systems in each time step. These will be solved in the two functions   <code>solve_u</code>   and   <code>solve_v</code>  .   
*   Finally, the variable   <code>theta</code>   is used to indicate the parameter   $\theta$   that is used to define which time stepping scheme to use, as explained in the introduction. The rest is self-explanatory.
* 
...
* 
 @endcode
* 
*  The next thing is to loop over all the time steps until we reach the end time (  $T=5$   in this case). In each time step, we first have to solve for   $U^n$  , using the equation   $(M^n + k^2\theta^2 A^n)U^n =$     $(M^{n,n-1}
* 
- k^2\theta(1-\theta) A^{n,n-1})U^{n-1} + kM^{n,n-1}V^{n-1}
 +$     $k\theta \left[k \theta F^n + k(1-\theta) F^{n-1} \right]$  . Note that we use the same mesh for all time steps, so that   $M^n=M^{n,n-1}=M$   and   $A^n=A^{n,n-1}=A$  . What we therefore have to do first is to add up   $MU^{n-1}
* 
- k^2\theta(1-\theta) AU^{n-1} + kMV^{n-1}$   and the forcing terms, and put the result into the   <code>system_rhs</code>   vector. (For these additions, we need a temporary vector that we declare before the loop to avoid repeated memory allocations in each time step.)     
...
* 
 ...
 @endcode
* 
*  The second step, i.e. solving for   $V^n$  , works similarly, except that this time the matrix on the left is the mass matrix (which we copy again in order to be able to apply boundary conditions, and the right hand side is   $MV^{n-1}
* 
- k\left[ \theta A U^n +
...
@endcode
* 
* What we see immediately is that the energy is a constant at least after  $t=\frac 12$   (until which the boundary source term   $g$   is nonzero, injectingenergy into the system).
...
* If you want to explore a bit, try out some of the following things:  <ul>      <li>  Varying   $\theta$  . This gives different time stepping schemes, some of  which are stable while others are not. Take a look at how the energy  evolves.
*     <li>  Different initial and boundary conditions, right hand sides.
*     <li>  More complicated domains or more refined meshes. Remember that the time  step needs to be bounded by the mesh width, so changing the mesh should  always involve also changing the time step. We will come back to this issue  in   step-24  .
*     <li>  Variable coefficients: In real media, the wave speed is often  variable. In particular, the "real" wave equation in realistic media would  read  @f[
...
-
     \nabla \cdot
     a(x) \nabla u = f,
  @f]  where   $\rho(x)$   is the density of the material, and   $a(x)$   is related to the  stiffness coefficient. The wave speed is then   $c=\sqrt{a/\rho}$  .
...
*   To make adaptively changing meshes possible, there are basically two routes.  The "correct" way would be to go back to the weak form we get using Rothe's  method. For example, the first of the two equations to be solved in each time  step looked like this:  \f{eqnarray*}
  (u^n,\varphi) + k^2\theta^2(\nabla u^n,\nabla \varphi) &=&
  (u^{n-1},\varphi)
* 
- k^2\theta(1-\theta)(\nabla u^{n-1},\nabla \varphi)
...
*   The second approach is the following: whenever we change the mesh,  we simply interpolate the solution from the last time step on the old  mesh to the new mesh, using the SolutionTransfer class. In other words,  instead of the equation above, we would solve  \f{eqnarray*}
  (u^n,\varphi) + k^2\theta^2(\nabla u^n,\nabla \varphi) &=&
  (I^n u^{n-1},\varphi)
* 
- k^2\theta(1-\theta)(\nabla I^n u^{n-1},\nabla \varphi)
