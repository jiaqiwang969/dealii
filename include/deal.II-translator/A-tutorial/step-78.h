...
@f}
* 
* The way we should interpret this equation is that it is a time-dependent partialdifferential equation of one "space" variable  $S$   as the price of the stock, and   $V(S,t)$   is the price of the option at time  $t$   if the stock price at that time were   $S$  .
* <a name="Particularitiesoftheequationsystem"></a><h3>Particularities of the equation system</h3>
* 

* There are a number of oddities in this equation that are worth discussing beforemoving on to its numerical solution. First, the "spatial" domain  $\Omega\subset\mathbb{R}$   is unbounded, and thus   $S$   can be unbounded in value.This is because there may be a practical upper bound for stock prices, but not aconceptual one. The boundary conditions   $V(S,t)\rightarrow S$   as  $S\rightarrow \infty$   can then be interpreted as follows: What is the value ofan option that allows me to buy a stock at price   $K$   if the stock price (todayor at time   $t=T$  ) is   $S\gg K$  ? One would expect that it is   $V\approx S-K$   plussome adjustment for inflation, or, if we really truly consider huge values of  $S$  , we can neglect   $K$   and arrive at the statement that the boundary values atthe infinite boundary should be of the form   $V\rightarrow S$   as stated above.
* In practice, for us to use a finite element method to solve this, we are goingto need to bound   $\Omega$  . Since this equation describes prices, and it doesn'tmake sense to talk about prices being negative, we will set the lower bound of  $\Omega$   to be 0. Then, for an upper bound, we will choose a very large number,one that   $S$   is not very likely to ever get to. We will call this   $S_\text{max}$  .So,   $\Omega=[0,S_\text{max}]$  .
* Second, after truncating the domain, we need to ask what boundary values weshould pose at this now finite boundary. To take care of this, we use "put-call"parity   @cite stoll1969relationship  . A "pull option" is one in which we areallowed, but not required, tosell* a stock at price   $K$   to someone at a futuretime   $T$  . This says
* @f{align*}{
    V(S,t)+Ke^{-r(T-t)}=P(S,t)+S
@f}
* where   $V(S,t)$   is the value of the call option, and   $P(S,t)$   is the value of theput option. Since we expect   $P(S,t) \rightarrow 0$   as   $S \rightarrow \infty$  ,this says
* @f{align*}{
    V(S,t) \rightarrow S-Ke^{-r(T-t)},
@f}
* and we can use this as a reasonable boundary condition at our finite point  $S_\text{max}$  .
* The second complication of the Block-Scholes equation is that we are given afinal condition, and not an initial condition. This is because we know what theoption is worth at time   $t=T$  : If the stock price at   $T$   is   $S<K$  , then we haveno incentive to use our option of buying a price   $K$   because we can buy that stockfor cheaper on the open market. So   $V(S,T)=0$   for   $S<K$  . On the other hand, ifat time   $T$   we have   $S>K$  , then we can buy the stock at price   $K$   via the optionand immediately sell it again on the market for price   $S$  , giving me a profit of  $S-K$  . In other words,   $V(S,T)=S-K$   for   $S>K$  . So, we only knowvalues for   $V$   at theend time* but not the initial time
* 
*  -  in fact, findingout what a fair price at the current time (conventionally taken to be   $t=0$  ) iswhat solving these equations is all about.
* This means that this is not an equation that is posed going forward intime, but in fact goingbackward* in time. Thus it makes sense to solve thisproblem in reverse by making the change of variables   $\tau=T-t$   where now   $\tau$  denotes the time before the strike time   $T$  .
...
* We will solve this problem using an IMEX method. In particular, we first discretizein time with the theta method and will later pick different values of theta forthe advective and diffusive terms.Let   $V^n(S)$   approximate   $V(S,\tau_n)$  :
* @f{align*}{
...
* As usual, (1) becomes   $-\textbf{M}V^n+\textbf{M}V^{n-1}$   and (4) becomes  $k_n\left[-r(1-\theta)\textbf{M}V^{n-1}
* 
- \theta r\textbf{M}V^n\right]$  , where  $\textbf{M}_{i,j}=\left(\phi_i(S),\phi_j(S)\right)$  , and where we have taken theliberty of denoting by   $V$   not only the function   $V(S)$   but also the vector ofnodal values after discretization.
...
-\sigma^2\textbf{B}V^n
* 
- \frac{1}{2}\sigma^2\textbf{D}V^n, \quad\quad \
    \textbf{B}_{i,j} = \left(\phi_i(S)S, \frac{d\phi_j(S)}{dS}\right),\
    \textbf{D}_{i,j} = \left(\frac{d\phi_i(S)}{dS}S^2,\frac{d\phi_j(S)}{dS}\right)
@f}
* 
* So, after adding in the constants and exchanging   $V^n$   for   $V^{n-1}$   whereapplicable, we arrive at the following for (2):
* @f{align*}{
    &k_n\int_0^{S_\text{max}}\phi_i(S)\left[\frac{\sigma^2S^2}{2}
        \left[(1-\theta)\
    \frac{d^2V^{n-1}(S)}{dS^2} + \
...
@f}
* But, because the matrix   $\textbf{B}$   involves an advective term, we will choose  $\theta=0$   there
* 
*  -  in other words, we use an explicit Euler method to treatadvection. Conversely, since the matrix   $\textbf{D}$   involves the diffusive term,we will choose   $\theta=1/2$   there
* 
...
* @f{align*}{
    ...
    &= \phi_i(S)rSV^n\Bigg|_0^{S_\text{max}}
* 
- \int_0^{S_\text{max}}
        \left[r\phi_i(S) \
...
-r\textbf{M}V^n
* 
-r\textbf{A}V^n, \quad\quad\
    \textbf{M}_{i,j} = \left(\phi_i(S), \phi_j(S)\right),\
    \textbf{A}_{i,j} = \left(S\frac{d\phi_i(S)}{dS}, \phi_j(S)\right)
@f}
* 
* So, again after adding in the constants and exchanging   $V^n$   for   $V^{n-1}$   whereapplicable, we arrive at the following for (3):
* @f{align*}{
    &k_n\int_0^{S_\text{max}}\phi_i(S)\left[rS\left[(1-\theta)
        \frac{dV^{n-1}(S)}{dS} +\
     \theta\frac{dV^{n}(S)}{dS}\right]\right]dS \\
    &= k_n\left[-(1-\theta)r\textbf{M}V^{n-1}
* 
-(1-\theta)r\textbf{A}V^{n-1}\
* 
...
-\theta r\textbf{M}V^n
* 
-\theta r\textbf{A}V^n\right]
@f}
* Just as before, we will use   $\theta=0$   for the matrix   $\textbf{A}$   and  $\theta=\frac{1}{2}$   for the matrix   $\textbf{M}$  . So, we arrive at thefollowing for (3):
* @f{align*}{
    k_n\left[-\frac{1}{2}r\textbf{M}V^{n-1}
* 
- \frac{1}{2}r\textbf{M}V^n \
* 
...
* 
* 
-r\frac{1}{2}\textbf{M}V^{n-1}
* 
- \frac{1}{2} r\textbf{M}V^n\right] \\
    &=
* 
-\textbf{M}V^n + \textbf{M}V^{n-1} +\
    k_n\left[- \frac{1}{4}\sigma^2\textbf{D}V^{n-1}
...
- r(-\tau^2
* 
- S^2 + 6)$  .This set-up now has right hand sides for the equation itself and for theboundary conditions at   $S=0$   that we did not have before, along with "final"conditions (or, with   $\tau$  -time "initial conditions") that do not match thereal situation. We will implement this in such a way in the code that it is easyto exchange
* 
...
* 
 @endcode
* 
*  Then the usual placing of all content of this program into a namespace and the importation of the deal.II namespace into the one we will work in. We also define an identifier to allow for the MMS code to be run when   <code>MMS</code>   is defined. Otherwise, the program solves the original problem:
* 
...
* 
 @endcode
* 
*   <a name="SolutionClass"></a>  <h3>Solution Class</h3>
* 

* 
*  This section creates a class for the known solution when testing using the MMS. Here we are using   $v(\tau,S) =
* 
-\tau^2
* 
-S^2 + 6$   for the solution. We need to include the solution equation and the gradient for the H1 seminorm calculation.
* 
...
* 
                                        ...
                                        const unsigned int component) const
   {
     ...
     return
* 
-Utilities::fixed_power<2, double>(p(component)) + 6;
 #else
     return std::max(p(component)
* 
- strike_price, 0.);
 #endif
   }
...
* 
 @endcode
* 
*   <a name="ThecodeBlackScholescodeClass"></a>  <h3>The <code>BlackScholes</code> Class</h3>
* 
...
* 
* 
*  -   <code>maximum_stock_price</code>  : The imposed upper bound on the spatial domain. This is the maximum allowed stock price.
* 

* 
* 
*  -   <code>maturity_time</code>  : The upper bound on the time domain. This is when the option expires.\n
* 
...
* 
* 
*  -   <code>strike_price</code>  : The agreed upon price that the buyer will have the option of purchasing  the stocks at the expiration time.   
*   Some slight differences between this program and   step-26   are the creation of the   <code>a_matrix</code> and the <code>b_matrix</code>  , which is described in the introduction. We then also need to store the current time, the size of the time step, and the number of the current time step. Next, we will store the output into a   <code>DataOutStack</code>   variable because we will be layering the solution at each time on top of one another to create the solution manifold. Then, we have a variable that stores the current cycle and number of cycles that we will run when calculating the solution. The cycle is one full solution calculation given a mesh. We refine the mesh once in between each cycle to exhibit the convergence properties of our program. Finally, we store the convergence data into a convergence table.   
*   As far as member functions are concerned, we have a function that calculates the convergence information for each cycle, called   <code>process_solution</code>  . This is just like what is done in   step-7  .
* 
...
* 
 @endcode
* 
*   <a name="ThecodeBlackScholescodeImplementation"></a>  <h3>The <code>BlackScholes</code> Implementation</h3>
* 
...
* 
 @endcode
* 
*   <a name="codeBlackScholessetup_systemcode"></a>  <h4><code>BlackScholes::setup_system</code></h4>
* 
...
* 
 @endcode
* 
*  Below is the code to create the Laplace matrix with non-constant coefficients. This corresponds to the matrix D in the introduction. This non-constant coefficient is represented in the   <code>current_coefficient</code>   variable.
* 
...
* 
 @endcode
* 
*  Now we will create the A matrix. Below is the code to create the matrix A as discussed in the introduction. The non constant coefficient is again represented in  the   <code>current_coefficient</code>   variable.
* 
...
* 
 @endcode
* 
*  Finally we will create the matrix B. Below is the code to create the matrix B as discussed in the introduction. The non constant coefficient is again represented in the   <code>current_coefficient</code>   variable.
* 
...
* 
 @endcode
* 
*   <a name="codeBlackScholessolve_time_stepcode"></a>  <h4><code>BlackScholes::solve_time_step</code></h4>
* 
...
* 
 @endcode
* 
*   <a name="codeBlackScholesadd_results_for_outputcode"></a>  <h4><code>BlackScholes::add_results_for_output</code></h4>
* 
...
* 
 @endcode
* 
*   <a name="codeBlackScholesrefine_gridcode"></a>  <h4><code>BlackScholes::refine_grid</code></h4>
* 
...
* 
 @endcode
* 
*   <a name="codeBlackScholesprocess_solutioncode"></a>  <h4><code>BlackScholes::process_solution</code></h4>
* 
...
* 
 @endcode
* 
*   <a name="codeBlackScholeswrite_convergence_tablecode"></a>  <h4><code>BlackScholes::write_convergence_table</code> </h4>
* 
...
* 
 @endcode
* 
*   <a name="codeBlackScholesruncode"></a>  <h4><code>BlackScholes::run</code></h4>
* 
...
* 
             system_rhs.add((-1) (1
* 
- theta) time_step interest_rate 2,
                            vmult_result);
* 
...
@endcode
* 
* What is more interesting is the output of the convergence tables. They areoutputted into the console, as well into a LaTeX file. The convergence tablesare shown above. Here, you can see that the the solution has a convergence rateof   $\mathcal{O}(h)$   with respect to the   $H^1$  -norm, and the solution has a convergence rateof   $\mathcal{O}(h^2)$   with respect to the   $L^2$  -norm.
* 
