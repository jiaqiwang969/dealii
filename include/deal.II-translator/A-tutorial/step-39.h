 ...
 * This tutorial depends on   step-12  b. @htmlonly <table class="tutorial"
 ...
 * <ul>
 * <li><a href="#Thelocalintegrators">The local integrators</a><a
 * href="#Thelocalintegrators">The local integrators</a>
 * <li><a href="#Themainclass">The main class</a><a href="#Themainclass">The
 * main class</a>
 * </ul>
 ...
 * <ul>
 * <li><a href="#Logfileoutput">Logfile output</a><a
 * href="#Logfileoutput">Logfile output</a>
 * <li><a href="#Postprocessingofthelogfile">Postprocessing of the
 * logfile</a><a href="#Postprocessingofthelogfile">Postprocessing of the
 * logfile</a>
 * </ul>
 * <li> <a href="#PlainProg" class=bold>The plain program</a><a
 * href="#PlainProg" class=bold>The plain program</a>
 * </ol> </td> </tr> </table>
 * @endhtmlonly <a name="Intro"></a> In this program, we use the interior
 * penalty method and Nitsche's weakboundary conditions to solve Poisson's
 * equation. We use multigridmethods on locally refined meshes, which are
 * generated using a bulkcriterion and a standard error estimator based on
 * cell and faceresiduals. All operators are implemented using the MeshWorker
 * interface.
 * Like in   step-12  , the discretization relies on finite element spaces,which are polynomial inside the mesh cells   $K\in \mathbb T_h$  , buthave no continuity between cells. Since such functions have two valueson each interior face   $F\in \mathbb F_h^i$  , one from each side, wedefine mean value and jump operators as follows: let<i>K</i><sub>1</sub> and <i>K</i><sub>2</sub> be the two cells sharinga face, and let the traces of functions <i>u<sub>i</sub></i> and theouter normal vectors <b>n</b><i><sub>i</sub></i> be labeledaccordingly. Then, on the face, we let@f[
 * \average{ u } = \frac{u_1 + u_2}2
 * @f]
 * Note, that if such an expression contains a normal vector, theaveraging operator turns into a jump. The interior penalty method for the problem@f[
 *
 *
 *
 *
 ...
 * @f]becomes@f{multline*}
 * \sum_{K\in \mathbb T_h} (\nabla u, \nabla v)_K \\ + \sum_{F \in F_h^i}
 * \biggl\{4\sigma_F (\average{ u \mathbf n}, \average{ v \mathbf n })_F
 *
 *
 *
 *
 ...
 *
 *
 * - 2 (\average{ \nabla v },\average{ u\mathbf n })_F \biggr\} \\ + \sum_{F
 * \in F_h^b} \biggl\{2\sigma_F (u, v)_F
 *
 *
 *
 *
 ...
 *
 *
 * - (\partial_n v,u)_F \biggr\} \\ = (f, v)_\Omega + \sum_{F \in F_h^b}
 * \biggl\{ 2\sigma_F (u^D, v)_F
 *
 * - (\partial_n v,u^D)_F \biggr\}. @f}
 *
 * Here,   $\sigma_F$   is the penalty parameter, which is chosen as follows:for a face <i>F</i> of a cell <i>K</i>, compute the value@f[
 * \sigma_{F,K} = p(p+1) \frac{|F|_{d-1}}{|K|_d},
 * @f]where <i>p</i> is the polynomial degree of the finite elementfunctions and   $|\cdot|_d$   and   $|\cdot|_{d-1}$   denote the   $d$   and   $d-1$  dimensional Hausdorff measure of the correspondingobject. If the face is at the boundary, choose   $\sigma_F = \sigma_{F,K}$  .For an interior face, we take the average of the two values at this face.
 * In our finite element program, we distinguish three differentintegrals,
 * corresponding to the sums over cells, interior faces andboundary faces
 * above. Since the   MeshWorker::loop   organizes the sumsfor us, we only
 * need to implement the integrals over each meshelement. The class
 * MatrixIntegrator below has these three functionsfor the left hand side of
 * the formula, the class RHSIntegrator for theright. As we will see below,
 * even the error estimate is of the samestructure, since it can be written as
 * @f{align*}
 ...
 * @f}
 *
 * Thus, the functions for assembling matrices, right hand side and
 * errorestimates below exhibit that these loops are all generic and can
 * beprogrammed in the same way. This program is related to   step-12  b, in
 * that it uses MeshWorker anddiscontinuous Galerkin methods. While there, we
 * solved an advectionproblem, here it is a diffusion problem. Here, we also
 * use multigridpreconditioning and a theoretically justified error estimator,
 * seeKarakashian and Pascal (2003). The multilevel scheme was discussed
 * indetail in Kanschat (2004). The adaptive iteration and its convergencehave
 * been discussed (for triangular meshes) in Hoppe, Kanschat, andWarburton
 * (2009).
 *
 *  <a name="CommProg"></a> <h1> The commented program</h1> The include files
 * for the linear algebra: A regular SparseMatrix, which in turn will include
 * the necessary files for SparsityPattern and Vector classes.
 *
 *
 * @code
 ...
 *
 *
 * MeshWorker separates local integration from the loops over cells and faces.
 * Thus, we have to write local integration classes for generating matrices,
 * the right hand side and the error estimator.
 *
 *
 * All these classes have the same three functions for integrating over cells,
 * boundary faces and interior faces, respectively. All the information needed
 * for the local integration is provided by
 ...
 * cell and face matrices. It is used to assemble the global matrix as well as
 * the level matrices.
 *
 *
 * @code
 ...
 * @endcode
 *
 * On each cell, we integrate the Dirichlet form. We use the library of ready
 * made integrals in LocalIntegrators to avoid writing these loops ourselves.
 * Similarly, we implement Nitsche boundary conditions and the interior
 * penalty fluxes between cells. The boundary and flux terms need a penalty
 * parameter, which should be adjusted to the cell size and the polynomial
 * degree. A safe choice of this parameter for constant coefficients can be
 * found in   LocalIntegrators::Laplace::compute_penalty()   and we use this
 * below.
 *
 *
 * @code
 ...
 * @endcode
 *
 * The second local integrator builds the right hand side. In our example, the
 * right hand side function is zero, such that only the boundary condition is
 * set here in weak form.
 *
 *
 * @code
 * template <int dim>
 ...
 * @endcode
 *
 * The cell contribution is the Laplacian of the discrete solution, since the
 ...
 * gradient inside the cells, but also the jump terms across faces and at the
 * boundary, we cannot just use   VectorTools::integrate_difference().
 * Instead, we use the MeshWorker interface to compute the error ourselves.
 *
 *
 *  There are several different ways to define this energy norm, but all of them are equivalent to each other uniformly with mesh size (some not uniformly with polynomial degree). Here, we choose @f[ \|u\|_{1,h} =
 ...
 * @endcode
 *
 * Here we have the integration on cells. There is currently no good interface
 * in MeshWorker that would allow us to access values of regular functions in
 * the quadrature points. Thus, we have to create the vectors for the exact
 * function's values and gradients inside the cell integrator. After that,
 * everything is as before and we just add up the squares of the differences.
 *
 *
 ...
 *
 *
 * This class does the main job, like in previous examples. For a description
 * of the functions declared here, please refer to the implementation below.
 *
 *
 * @code
 * template <int dim>
 ...
 * @endcode
 *
 * Finally, we have a group of sparsity patterns and sparse matrices related
 * to the multilevel preconditioner.  First, we have a level matrix and its
 * sparsity pattern.
 *
 *
 * @code
 ...
 * @endcode
 *
 * When we perform multigrid with local smoothing on locally refined meshes,
 * additional matrices are required; see Kanschat (2004). Here is the sparsity
 * pattern for these edge matrices. We only need one, because the pattern of
 * the up matrix is the transpose of that of the down matrix. Actually, we do
 * not care too much about these details, since the MeshWorker is filling
 * these matrices.
 *
 *
 * @code
 ...
 * @endcode
 *
 * The transpose of the flux matrix at the refinement edge, coupling coarse
 * level degrees of freedom to fine level.
 *
 *
 * @code
 ...
 * @endcode
 *
 * First, we use the finite element to distribute degrees of freedom over the
 * mesh and number them.
 *
 *
 * @code
 ...
 * @endcode
 *
 * Then, we already know the size of the vectors representing finite element
 * functions.
 *
 *
 * @code
 ...
 * @endcode
 *
 * Next, we set up the sparsity pattern for the global matrix. Since we do not
 * know the row sizes in advance, we first fill a temporary
 * DynamicSparsityPattern object and copy it to the regular SparsityPattern
 * once it is complete.
 *
 *
 * @code
 ...
 * @endcode
 *
 * The global system is set up, now we attend to the level matrices. We resize
 * all matrix objects to hold one matrix per level.
 *
 *
 * @code
 ...
 * @endcode
 *
 * It is important to update the sparsity patterns after <tt>clear()</tt> was
 * called for the level matrices, since the matrices lock the sparsity pattern
 * through the SmartPointer and Subscriptor mechanism.
 *
 *
 * @code
 ...
 * @endcode
 *
 * These are roughly the same lines as above for the global matrix, now for
 * each level.
 *
 *
 * @code
 ...
 * @endcode
 *
 * Additionally, we need to initialize the transfer matrices at the refinement
 * edge between levels. They are stored at the index referring to the finer of
 * the two indices, thus there is no such object on level 0.
 *
 *
 * @code
 *       if (level > 0)
 ...
 * indicate that this is the matrix of the discrete system we solve and it is
 ...
 * @endcode
 *
 * First, we need t set up the object providing the values we integrate. This
 * object contains all FEValues and FEFaceValues objects needed and also
 * maintains them automatically such that they always point to the current
 * cell. To this end, we need to tell it first, where and what to compute.
 * Since we are not doing anything fancy, we can rely on their standard choice
 * for quadrature rules. Since their default update flags are minimal, we add
 * what we need additionally, namely the values and gradients of shape
 * functions on all objects (cells, boundary and interior faces). Afterwards,
 * we are ready to initialize the container, which will create all necessary
 ...
 * @endcode
 *
 * This is the object into which we integrate local data. It is filled by the
 * local integration routines in MatrixIntegrator and then used by the
 ...
 * global matrix. These assembler objects have all the knowledge of the
 * structures of the target object, in this case a SparseMatrix, possible
 * constraints and the mesh structure.
 *
 *
 * @code
 ...
 * @endcode
 *
 * Now comes the part we coded ourselves, the local integrator. This is the
 * only part which is problem dependent.
 *
 *
 * @code
 ...
 * traverses all active cells of the mesh, computes cell and face matrices and
 * assembles them into the global matrix. We use the variable
 * <tt>dof_handler</tt> here in order to use the global numbering of degrees
 * of freedom.
 *
 *
 * @code
 ...
 * function looks like a twin of the previous one. Indeed, there are only two
 * minor differences.
 *
 *
 * @code
 ...
 * cells, not only the active ones. And we use functions ending on
 * <code>_mg</code>   since we need the degrees of freedom on each level, not
 ...
 * @endcode
 *
 * Since this assembler allows us to fill several vectors, the interface is a
 * little more complicated as above. The pointers to the vectors have to be
 * stored in an AnyData object. While this seems to cause two extra lines of
 * code here, it actually comes handy in more complex applications.
 *
 *
 * @code
 ...
 * @endcode
 *
 * Now we are setting up the components of the multilevel preconditioner.
 * First, we need transfer between grid levels. The object we are using here
 * generates sparse matrices for these transfers.
 *
 *
 * @code
 ...
 * flexibility is offered for the smoother. First, we choose Gauss-Seidel as
 ...
 * iteration below, here is a trick to make the multilevel preconditioner a
 * symmetric operator even for nonsymmetric smoothers.
 *
 *
 * @code
 ...
 * @endcode
 *
 * The smoother class optionally implements the variable V-cycle, which we do
 * not want here.
 *
 *
 * @code
 ...
 * @endcode
 *
 * Another clone of the assemble function. The big difference to the previous
 * ones is here that we also have an input vector.
 *
 *
 * @code
 * template <int dim>
 ...
 * cell. Since cells in deal.II are not numbered, we have to create our own
 * numbering in order to use this vector. For the assembler used below the
 * information in which component of a vector the result is stored is
 * transmitted by the user_index variable for each cell. We need to set this
 * numbering up here. On the other hand, somebody might have used the user
 * indices already. So, let's be good citizens and save them before tampering
 * with them.
 *
 *
 * @code
 ...
 * the second derivatives of this solution (to compute the Laplacian).
 * Therefore, the Boolean arguments selecting function values and first
 * derivatives a false, only the last one selecting second derivatives is
 * true.
 *
 *
 * @code
 ...
 * @endcode
 *
 * On interior and boundary faces, we need the function values and the first
 ...
 * @endcode
 *
 * And we continue as before, with the exception that the default update flags
 * are already adjusted to the values and derivatives we requested above.
 *
 *
 * @code
 ...
 * @endcode
 *
 * The assembler stores one number per cell, but else this is the same as in
 ...
 * @endcode
 *
 * Right before we return the result of the error estimate, we restore the old
 * user indices.
 *
 *
 * @code
 ...
 * itself. This function is a clone of the estimation function right above.
 *
 *
 * Since we compute the error in the energy and the <i>L<sup>2</sup></i>-norm,
 * respectively, our block vector needs two blocks here.
 *
 *
 * @code
 * template <int dim>
 ...
 * @endcode
 *
 * Create graphical output. We produce the filename by collating the name from
 * its various components, including the refinement cycle that we output with
 * two digits.
 *
 *
 * @code
 ...
 *
 *  <a name="Logfileoutput"></a><h3>Logfile output</h3> First, the program
 * produces the usual logfile here stored in <tt>deallog</tt>. It reads (with
 ...
 * @endcode
 *
 * This log for instance shows that the number of conjugate gradientiteration
 * steps is constant at approximately 15. <a
 * name="Postprocessingofthelogfile"></a><h3>Postprocessing of the
 * logfile</h3>
 *
 *  <img
 * src="https://www.dealii.org/images/steps/developer/step-39-convergence.svg"
 * alt="">  Using the perl script <tt>postprocess.pl</tt>, we extract
 * relevantdata into <tt>output.dat</tt>, which can be used to plot graphs
 * with<tt>gnuplot</tt>. The graph above for instance was produced using the
 * gnuplotscript <tt>plot_errors.gpl</tt> via
 * @code
 ...
 *
* <a name="PlainProg"></a><h1> The plain program</h1>  @include "step-39.cc"
 *
 */
