...
@f}
* In some sense, this equation is simpler than the ones we have discussed in thepreceding programs   step-23  ,   step-24  ,   step-25  , namely the wave equation. Thisis due to the fact that the heat equation smoothes out the solution over time,and is consequently more forgiving in many regards. For example, when usingimplicit time stepping methods, we can actually take large time steps, we haveless trouble with the small disturbances we introduce through adapting themesh every few time steps, etc.
* Our goal here will be to solve the equations above using the theta-scheme thatdiscretizes the equation in time using the following approach, where we wouldlike   $u^n(\mathbf x)$   to approximate   $u(\mathbf x, t_n)$   at some time   $t_n$  :
* @f{align*}
...
-
  ...
  \right].
@f}
* Here,   $k_n=t_n-t_{n-1}$   is the time step size. The theta-scheme generalizesthe explicit Euler (  $\theta=0$  ), implicit Euler (  $\theta=1$  ) andCrank-Nicolson (  $\theta=\frac 12$  ) time discretizations. Since the latter hasthe highest convergence order, we will choose   $\theta=\frac 12$   in the programbelow, but make it so that playing with this parameter remains simple. (If youare interested in playing with higher order methods, take a look at   step-52  .)
* Given this time discretization, space discretization happens as it alwaysdoes, by multiplying with test functions, integrating by parts, and thenrestricting everything to a finite dimensional subspace. This yields thefollowing set of fully discrete equations after multiplying through with  $k_n$  :
* @f{align*}
  M U^n-MU^{n-1}
  +
  k_n \left[
  ...
  \right],
@f}
* where   $M$   is the mass matrix and   $A$   is the stiffness matrix that results fromdiscretizing the Laplacian. Bringing all known quantities to the right handside yields the linear system we have to solve in every step:
* @f{align*}
  (M
  +
  k_n \theta A) U^n
...
* We can start the iteration above if we have the set of nodal coefficients  $U^0$   at the initial time. Here, we take the ones we get by interpolating theinitial values   $u_0(\mathbf x)$   onto the mesh used for the first time step. Wewill also need to choose a time step; we will here just choose it as fixed,but clearly advanced simulators will want to choose it adaptively. We willbriefly come back to this in the <a href="#Results">results section
below</a>.
* 
...
*   There is a similar problem in that we will choose a right hand side that  will switch on in different parts of the domain at different times. To avoid  being caught flat footed with too coarse a mesh in areas where we suddenly  need a finer mesh, we will also enforce in our program a <i>minimal</i> mesh  refinement level.
*     <li>  <i>Test functions from different meshes</i>: Let us consider again the  semi-discrete equations we have written down above: 
* @f{align*}
    \frac{u^n(\mathbf x)-u^{n-1}(\mathbf x)}{k_n}
* 
...
-
    ...
    \right],
  @f}
*   where   $I_h^n$   is the interpolation operator onto the finite element space  used in time step   $n$  . This is not the optimal approach since it introduces  an additional error besides time and space discretization, but it is a  pragmatic one that makes it feasible to do time adapting meshes.  </ul>  
* 
...
@f}
* (with integer constants   $n_x,n_y$  )if only the initial condition, right hand side and boundary values are allof the form   $\sin(n_x \pi x) \sin(n_y \pi y)$   as well. This is due to the factthat the function   $\sin(n_x \pi x) \sin(n_y \pi y)$   is an eigenfunction of theLaplace operator and allows us to compute things like the time factor   $a(t)$  analytically and, consequently, compare with what we get numerically.
...
@f}
* and due to the initial conditions,   $a(0)=1$  . This differential equation can beintegrated to yield
* @f{align*}
  a(t) =
* 
...
@f}
* and due to the initial conditions,   $a(0)=0$  . Integrating this equation in timeyields
* @f{align*}
...
@f}
* 
* Again, if you have the wrong factors of   $\theta$   or   $k$   in front of the righthand side terms you will either not get the right temporal behavior of thesolution, or it will converge to a maximum value other than  $\frac{1}{(n_x^2+n_y^2)\pi^2}$  .
...
* @f{align*}
  ...
  \right.
@f}
* In other words, in every period of length   $\tau$  , the right hand side firstflashes on in domain 1, then off completely, then on in domain 2, then offcompletely again. This pattern is probably best observed via the littleanimation of the solution shown in the <a href="#Results">results
section</a>.
...
* 
 ...
 @endcode
* 
*   <a name="ThecodeHeatEquationcodeclass"></a>  <h3>The <code>HeatEquation</code> class</h3>   
...
*   As far as member functions are concerned, the only possible surprise is that the   <code>refine_mesh</code>   function takes arguments for the minimal and maximal mesh refinement level. The purpose of this is discussed in the introduction.
* 
...
* 
*   @note   If you're an experienced programmer, you may be surprised that we use a   <code>goto</code>   statement in this piece of code!   <code>goto</code>   statements are not particularly well liked any more since Edsgar Dijkstra, one of the greats of computer science, wrote a letter in 1968 called "Go To Statement considered harmful" (see <a href="http://en.wikipedia.org/wiki/Considered_harmful">here</a>). The author of this code subscribes to this notion whole-heartedly:   <code>goto</code>   is hard to understand. In fact, deal.II contains virtually no occurrences: excluding code that was essentially transcribed from books and not counting duplicated code pieces, there are 3 locations in about 600,000 lines of code at the time this note is written; we also use it in 4 tutorial programs, in exactly the same context as here. Instead of trying to justify the occurrence here, let's first look at the code and we'll come back to the issue at the end of function.
* 
...
* 
 ...
 @endcode
* 
*  Now that you have seen what the function does, let us come back to the issue of the   <code>goto</code>  . In essence, what the code does is something like this:   <div class=CodeFragmentInTutorialComment>
...
@code
 ...
 @endcode
* 
 </div>   Here, the condition "happy with the result" is whether we'd like to keep the current mesh or would rather refine the mesh and start over on the new mesh. We could of course replace the use of the   <code>goto</code>   by the following:   <div class=CodeFragmentInTutorialComment>
...
* 
 ...
 @endcode
* 
 </div>   This has the advantage of getting rid of the   <code>goto</code>   but the disadvantage of having to duplicate the code that implements the "solve timestep" and "postprocess" operations in two different places. This could be countered by putting these parts of the code (sizable chunks in the actual implementation above) into their own functions, but a   <code>while(true)</code>   loop with a   <code>break</code>   statement is not really all that much easier to read or understand than a   <code>goto</code>  .
* 

* 
*  In the end, one might simply agree that <i>in general</i>   <code>goto</code>   statements are a bad idea but be pragmatic and state that there may be occasions where they can help avoid code duplication and awkward control flow. This may be one of these places, and it matches the position Steve McConnell takes in his excellent book "Code Complete"   @cite CodeComplete   about good programming practices (see the mention of this book in the introduction of   step-1  ) that spends a surprising ten pages on the question of   <code>goto</code>   in general.
* 
...
* To increase the accuracy and resolution of your simulation in time, onetypically decreases the time step size   $k_n$  . If you start playing aroundwith the time step in this particular example, you will notice that thesolution becomes partly negative, if   $k_n$   is below a certain threshold.This is not what we would expect to happen (in nature).
...
- a_{ij} u^{n}_j \right),
@f}
* where   $S_i$   is the set of degrees of freedom that DoF   $i$   couples with (i.e.,for which either the matrix   $A$   or matrix   $B$   has a nonzero entry at position  $(i,j)$  ). If all coefficientsfulfill the following conditions:
* @f{align*}
  a_{ii} &> 0, & b_{ii} &\geq 0, & a_{ij} &\leq 0, & b_{ij} &\geq 0,
  &
  \forall j &\in S_i,
@f}
* all solutions   $u^{n}$   keep their sign from the previous ones   $u^{n-1}$  , andconsequently from the initial values   $u^0$  . See e.g.<a href="http://bookstore.siam.org/cs14/">Kuzmin, H&auml;m&auml;l&auml;inen</a>for more information on positivity preservation.
* Depending on the PDE to solve and the time integration scheme used, one isable to deduce conditions for the time step   $k_n$  . For the heat equation withthe Crank-Nicolson scheme,<a href="https://doi.org/10.2478/cmam-2010-0025">Schatz et. al.</a> havetranslated it to the following ones:
* @f{align*}
  (1
* 
...
@f}
* where   $M = m_{ij}$   denotes the mass matrix and   $A = a_{ij}$   the stiffnessmatrix with   $a_{ij} \leq 0$   for   $j \neq i$  , respectively. With  $a_{ij} \leq 0$  , we can formulate bounds for the global time step   $k$   asfollows:
* @f{align*}
  k_{\text{max}} &= \frac{ 1 }{ 1
* 
- \theta }
...
and upper bound</i> in case of a Crank-Nicolson scheme. These bounds should beconsidered along with the CFL condition to ensure significance of the performedsimulations.
* Being unable to make the time step as small as we want to get moreaccuracy without losing the positivity property is annoying. It raisesthe question of whether we can at least <i>compute</i> the minimal time stepwe can choose  to ensure positivity preservation in this particular tutorial.Indeed, we can usethe SparseMatrix objects for both mass and stiffness that are created viathe MatrixCreator functions. Iterating through each entry via SparseMatrixIteratorslets us check for diagonal and off-diagonal entries to set a proper time stepdynamically. For quadratic matrices, the diagonal element is stored as thefirst member of a row (see SparseMatrix documentation). An exemplary codesnippet on how to grab the entries of interest from the   <code>mass_matrix</code>  is shown below.
* @code
