...
* This program deals with an example of a non-linear elliptic partialdifferential equation, the[minimal surface equation](https://en.wikipedia.org/wiki/Minimal_surface).You can imagine the solution of this equation to describethe surface spanned by a soap film that is enclosed by aclosed wire loop. We imagine the wire to not just be a planar loop, but infact curved. The surface tension of the soap film will then reduce the surfaceto have minimal surface. The solution of the minimal surface equationdescribes this shape with the wire's vertical displacement as a boundarycondition. For simplicity, we will here assume that the surface can be writtenas a graph   $u=u(x,y)$   although it is clear that it is not very hard toconstruct cases where the wire is bent in such a way that the surface can onlylocally be constructed as a graph but not globally.
...
* As described above, we solve this equation using Newton's method in which wecompute the   $n$  th approximate solution from the   $(n-1)$  th one, and usea damping parameter   $\alpha^n$   to get better global convergence behavior: 
* @f{align*}
...
* 
-\nabla \cdot \left( \frac{1}{\sqrt{1+|\nabla u|^{2}}}\nabla u \right)
  @f]and   $F'(u,\delta u)$   the derivative of F in direction of   $\delta u$  :@f[
  F'(u,\delta u)=\lim \limits_{\epsilon \rightarrow 0}{\frac{F(u+\epsilon \delta u)-
  F(u)}{\epsilon}}.
@f]
* Going through the motions to find out what   $F'(u,\delta u)$   is, we find thatwe have to solve a linear elliptic PDE in every Newton step, with   $\delta u^n$  as the solution of:
*   @f[
* 
...
- \nabla \cdot \left( \frac{1}{\left(1+|\nabla u^{n}|^{2}\right)^{\frac{1}{2}}}
  ...
  @f]
* In order to solve the minimal surface equation, we have to solve this equationrepeatedly, once per Newton step. To solve this, we have to take a look at theboundary condition of this problem. Assuming that   $u^{n}$   already has theright boundary values, the Newton update   $\delta u^{n}$   should have zeroboundary conditions, in order to have the right boundary condition afteradding both.  In the first Newton step, we are starting with the solution  $u^{0}\equiv 0$  , the Newton update still has to deliver the right boundarycondition to the solution   $u^{1}$  .
* 
...
*   @note   In some sense, one may argue that if the program already  implements   $F(u)$  , it is duplicative to also have to implement    $F'(u,\delta)$  . As always, duplication tempts bugs and we would like  to avoid it. While we do not explore this issue in this program, we  will come back to it at the end of the <a
  href="#extensions">Possibilities for extensions</a> section below,  and specifically in   step-72  .
* 

* <a name="Weakformulationoftheproblem"></a><h3>Weak formulation of the problem</h3>
* 

* Starting with the strong formulation above, we get the weak formulation by multiplyingboth sides of the PDE with a test function   $\varphi$   and integrating by parts on both sides:  @f[
...
-\left(\nabla \varphi , \frac{1}{\left(1+|\nabla u^{n}|^{2}\right)^{\frac{1}{2}}} \nabla u^{n}
   \right).
  @f]Here the solution   $\delta u^{n}$   is a function in   $H^{1}(\Omega)$  , subject tothe boundary conditions discussed above.Reducing this space to a finite dimensional space with basis   $\left\{
...
* The matrix that corresponds to the Newton step above can be reformulated toshow its structure a bit better. Rewriting it slightly, we get that it has theform@f[
  A_{ij}
  =
  \left(
    \nabla \varphi_i,
    ...
    \nabla \varphi_j
  \right),
@f]where the matrix   $B$   (of size   $d \times d$   in   $d$   space dimensions) is givenby the following expression:@f[
  B
  =
  a_n \left\{
   \mathbf I
...
-
  ...
  \frac{\nabla u_n}{\sqrt{1+|\nabla u^{n}|^{2}}}
  \right\}.
@f]From this expression, it is obvious that  $B$   is symmetric, and so   $A$   is symmetric as well.On the other hand,   $B$   is also positive definite, which confers the sameproperty onto   $A$  . This can be seen by noting that the vector   $v_1 =
\frac{\nabla u^n}{|\nabla u^n|}$   is an eigenvector of   $B$   with eigenvalue  $\lambda_1=a_n \left(1-\frac{|\nabla u^n|^2}{1+|\nabla u^n|^2}\right) > 0$   while all vectors   $v_2\ldots v_d$  that are perpendicular to   $v_1$   and each other are eigenvectors witheigenvalue   $a_n$  . Since all eigenvalues are positive,   $B$   is positive definiteand so is   $A$  . We can thus use the CG method for solving the Newton steps.(The fact that the matrix   $A$   is symmetric and positive definite should not comeas a surprise. It results from taking the derivative of an operator thatresults from taking the derivative of an energy functional: the minimalsurface equation simply minimizes some non-quadratic energy. Consequently,the Newton matrix, as the matrix of second derivatives of a scalar energy,must be symmetric since the derivative with regard to the   $i$  th and   $j$  thdegree of freedom should clearly commute. Likewise, if the energy functionalis convex, then the matrix of second derivatives must be positive definite,and the direct calculation above simply reaffirms this.)
* It is worth noting, however, that the positive definiteness degenerates forproblems where   $\nabla u$   becomes large. In other words, if we simply multiplyall boundary values by 2, then to first order   $u$   and   $\nabla u$   will also bemultiplied by two, but as a consequence the smallest eigenvalue of   $B$   willbecome smaller and the matrix will become more ill-conditioned. (Morespecifically, for   $|\nabla u^n|\rightarrow\infty$   we have that  $\lambda_1 \propto a_n \frac{1}{|\nabla u^n|^2}$   whereas  $\lambda_2\ldots \lambda_d=a_n$  ; thus, the condition number of   $B$  ,which is a multiplicative factor in the condition number of   $A$   growslike   ${\cal O}(|\nabla u^n|^2)$  .) It is simpleto verify with the current program that indeed multiplying the boundary valuesused in the current program by larger and larger values results in a problemthat will ultimately no longer be solvable using the simple preconditioned CGmethod we use here.
* 
...
\delta u^{n}$   with a step length   $0 < \alpha^n \le 1$  . It is a commonobservation that for strongly nonlinear models, Newton's method doesnot converge if we always choose   $\alpha^n=1$   unless one starts withan initial guess   $u^0$   that is sufficiently close to the solution   $u$  of the nonlinear problem. In practice, we don't always have such aninitial guess, and consequently taking full Newton steps (i.e., using  $\alpha=1$  ) does frequently not work.
* A common strategy therefore is to use a smaller step length for thefirst few steps while the iterate   $u^n$   is still far away from thesolution   $u$   and as we get closer use larger values for   $\alpha^n$  until we can finally start to use full steps   $\alpha^n=1$   as we areclose enough to the solution. The question is of course how to choose  $\alpha^n$  . There are basically two widely used approaches: linesearch and trust region methods.
* In this program, we simply always choose the step length equal to0.1. This makes sure that for the testcase at hand we do getconvergence although it is clear that by not eventually reverting tofull step lengths we forego the rapid, quadratic convergence thatmakes Newton's method so appealing. Obviously, this is a point oneeventually has to address if the program was made into one that ismeant to solve more realistic problems. We will comment on this issuesome more in the <a href="#Results">results section</a>, and use aneven better approach in   step-77  .
* 

* <a name="Summaryofthealgorithmandtestcase"></a><h3> Summary of the algorithm and testcase </h3>
* 

* Overall, the program we have here is not unlike   step-6   in many regards. Thelayout of the main class is essentially the same. On the other hand, thedriving algorithm in the   <code>run()</code>   function is different and works asfollows:  <ol>    <li>    Start with the function   $u^{0}\equiv 0$   and modify it in such a way  that the values of   $u^0$   along the boundary equal the correct  boundary values   $g$   (this happens in    <code>MinimalSurfaceProblem::set_boundary_values</code>  ). Set    $n=0$  .  </li>  
*   <li>    Compute the Newton update by solving the system   $A^{n}\;\delta
  U^{n}=b^{n}$    with boundary condition   $\delta u^{n}=0$   on   $\partial \Omega$  .  </li>  
*   <li>    Compute a step length   $\alpha^n$  . In this program, we always set    $\alpha^n=0.1$  . To make things easier to extend later on, this  happens in a function of its own, namely in    <code>MinimalSurfaceProblem::determine_step_length</code>  .  (The strategy of always choosing   $\alpha^n=0.1$   is of course not  optimal
* 
*  -  we should choose a step length that works for a given  search direction
* 
*  -  but it requires a bit of work to do that. In the  end, we leave these sorts of things to external packages:   step-77    does that.)  </li>  
*   <li>    The new approximation of the solution is given by    $u^{n+1}=u^{n}+\alpha^n \delta u^{n}$  .  </li>  
*   <li>    If   $n$   is a multiple of 5 then refine the mesh, transfer the  solution   $u^{n+1}$   to the new mesh and set the values of   $u^{n+1}$    in such a way that along the boundary we have    $u^{n+1}|_{\partial\Gamma}=g$   (again in    <code>MinimalSurfaceProblem::set_boundary_values</code>  ). Note that  this isn't automatically  guaranteed even though by construction we had that before mesh  refinement   $u^{n+1}|_{\partial\Gamma}=g$   because mesh refinement  adds new nodes to the mesh where we have to interpolate the old  solution to the new nodes upon bringing the solution from the old to  the new mesh. The values we choose by interpolation may be close to  the exact boundary conditions but are, in general, nonetheless not  the correct values.  </li>  
*   <li>    Set   $n\leftarrow n+1$   and go to step 2.  </li>    </ol>  
* The testcase we solve is chosen as follows: We seek to find the solution ofminimal surface over the unit disk   $\Omega=\{\mathbf x: \|\mathbf
x\|<1\}\subset {\mathbb R}^2$   where the surface attains the values  $u(x,y)|{\partial\Omega} = g(x,y) \dealcoloneq \sin(2 \pi (x+y))$   along theboundary.
* 
...
* 
* 
*  - The   <code>setup_system</code>   function takes an argument that denotes whether this is the first time it is called or not. The difference is that the first time around we need to distribute the degrees of freedom and set the solution vector for   $u^n$   to the correct size. The following times, the function is called after we have already done these steps as part of refining the mesh in   <code>refine_mesh</code>  .
* 

* 
* 
*  - We then also need new functions:   <code>set_boundary_values()</code>   takes care of setting the boundary values on the solution vector correctly, as discussed at the end of the introduction.   <code>compute_residual()</code>   is a function that computes the norm of the nonlinear (discrete) residual. We use this function to monitor convergence of the Newton iteration. The function takes a step length   $\alpha^n$   as argument to compute the residual of   $u^n + \alpha^n
 \; \delta u^n$  . This is something one typically needs for step length control, although we will not use this feature here. Finally,   <code>determine_step_length()</code>   computes the step length   $\alpha^n$   in each Newton iteration. As discussed in the introduction, we here use a fixed step length and leave implementing a better strategy as an exercise. (  step-77   does this differently: It simply uses an external package for the whole solution process, and a good line search strategy is part of what that package provides.)
* 
...
* 
 @endcode
* 
*  We end the function by updating all the remaining data structures, indicating to   <code>setup_dofs()</code>   that this is not the first go-around and that it needs to preserve the content of the solution vector:
* 
...
* 
 @endcode
* 
*  The actual computation is much as in   <code>assemble_system()</code>  . We first evaluate the gradients of   $u^n+\alpha^n\,\delta u^n$   at the quadrature points, then compute the coefficient   $a_n$  , and then plug it all into the formula for the residual:
* 
...
* 
*  - It may not converge from arbitrarily chosen starting points. Rather, a  starting point has to be close enough to the solution to guarantee  convergence. However, we can enlarge the area from which Newton's method  converges by damping the iteration using a <i>step length</i> 0<  $\alpha^n\le
  1$  .
* 
...
* A consequence of these two observations is that a successful strategy is tochoose   $\alpha^n<1$   for the initial iterations until the iterate has comeclose enough to allow for convergence with full step length, at which point wewant to switch to   $\alpha^n=1$  . The question is how to choose   $\alpha^n$   in anautomatic fashion that satisfies these criteria.
...
(2/3)^2, \ldots$   chosen above, for   $0<r<1$  . It is obvious where the term"backtracking" comes from: we try a long step, but if that doesn't work we trya shorter step, and ever shorter step, etc. The function  <code>determine_step_length()</code>   is written the way it is to supportexactly this kind of use case.
* Whether we accept a particular step length   $\alpha^n$   depends on how we define"substantially smaller". There are a number of ways to do so, but withoutgoing into detail let us just mention that the most common ones are to use theWolfe and Armijo-Goldstein conditions. For these, one can show the following:
* 
...
* As outlined in the introduction, when solving a nonlinear problem ofthe form  @f[
    F(u) \dealcoloneq
* 
...
-\nabla \cdot \left( \frac{1}{\sqrt{1+|\nabla u|^{2}}}\nabla u \right)
    = 0
  @f]we use a Newton iteration that requires us to repeatedly solve thelinear partial differential equation 
* @f{align*}
    F'(u^{n},\delta u^{n}) &=- F(u^{n})
  @f}
...
* @f{align*}
    u^{n+1}&=u^{n}+\alpha^n \delta u^{n}
  @f}
* with the solution   $\delta u^{n}$   of the Newton step. For the problemhere, we could compute the derivative   $F'(u,\delta u)$   by hand andobtained  @f[
  F'(u,\delta u)
  =
* 
...
- \nabla \cdot \left( \frac{1}{\left(1+|\nabla u|^{2}\right)^{\frac{1}{2}}}\nabla
  ...
  \right).
  @f]But this is already a sizable expression that is cumbersome both toderive and to implement. It is also, in some sense, duplicative: If weimplement what   $F(u)$   is somewhere in the code, then   $F'(u,\delta u)$  is not an independent piece of information but is something that, atleast in principle, a computer should be able to infer itself.Wouldn't it be nice if that could actually happen? That is, if wereally only had to implement   $F(u)$  , and   $F'(u,\delta u)$   was then somehowdone implicitly? That is in fact possible, and runs under the name"automatic differentiation".   step-71   discusses this veryconcept in general terms, and   step-72   illustrates how this can beapplied in practice for the very problem we are considering here.
* 
