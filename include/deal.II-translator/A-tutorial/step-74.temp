...
@f]subject to the boundary condition@f[
u = g_D \qquad \mbox{on } \partial \Omega.
@f]For simplicity, we assume that the diffusion coefficient   $\nu$   is constant here.Note that if   $\nu$   is discontinuous, we need to take this into account when computing jump termson cell faces.
* We denote the mesh by   ${\mathbb T}_h$  , and   $K\in{\mathbb T}_h$   is a mesh cell.The sets of interior and boundary faces are denoted by   ${\mathbb F}^i_h$   and   ${\mathbb F}^b_h$  respectively. Let   $K^0$   and   $K^1$   be the two cells sharing a face   $f\in F_h^i$  ,and   $\mathbf n$   be the outer normal vector of   $K^0$  . Then the jumpoperator is given by the "here minus there" formula,@f[
\jump{v} = v^0
* 
- v^1
@f]and the averaging operator as@f[
\average{v} = \frac{v^0 + v^1}{2}
@f]respectively. Note that when   $f\subset \partial \Omega$  , we define   $\jump{v} = v$   and  $\average{v}=v$  .The discretization using the SIPG is given by the following weak formula(more details can be found in   @cite di2011mathematical   and the references therein)
* @f{align*}
...
* The penalty parameter is defined as   $\sigma = \gamma/h_f$  , where   $h_f$   a local length scale associatedwith the cell face; here we choose an approximation of the length of the cell in the direction normal to the face:  $\frac 1{h_f} = \frac 12 \left(\frac 1{h_K} + \frac 1{h_{K'}}\right)$  ,where   $K,K'$   are the two cells adjacent to the face   $f$   and we wecompute   $h_K = \frac{|K|}{|f|}$  .
* In the formula above,   $\gamma$   is the penalization constant.To ensure the discrete coercivity, the penalization constant has to be large enough   @cite ainsworth2007posteriori  .People do not really have consensus on which of the formulas proposedin the literature should be used. (This is similar to the situationdiscussed in the "Results" section of   step-47  .)One can just pick a large constant, while other options could be the multiples of   $(p+1)^2$   or   $p(p+1)$  . In this code,we follow   step-39   and use   $\gamma = p(p+1)$  .
* 

* <a name="Aposteriorierrorestimator"></a><h3>A posteriori error estimator</h3>
* In this example, with a slight modification, we use the error estimator by Karakashian and Pascal   @cite karakashian2003posteriori  @f[
...
* 
 @endcode
* 
*   <a name="Equationdata"></a>  <h3>Equation data</h3> Here we define two test cases: convergence_rate for a smooth function and l_singularity for the   Functions::LSingularityFunction.  
* 
...
* 
 @endcode
* 
*   <a name="Auxiliaryfunctions"></a>  <h3>Auxiliary functions</h3> The following two auxiliary functions are used to compute jump terms for   $u_h$   and   $\nabla u_h$   on a face, respectively.
* 
...
* 
 @endcode
* 
*   <a name="TheSIPGLaplaceclass"></a>  <h3>The SIPGLaplace class</h3> After these preparations, we proceed with the main class of this program, called `SIPGLaplace`. The overall structure of the class is as in many of the other tutorial programs. Major differences will only come up in the implementation of the assemble functions, since we use FEInterfaceValues to assemble face terms.
* 
...
* 
 @endcode
* 
*   <a name="Theassemble_systemfunction"></a>  <h3>The assemble_system function</h3> The assemble function here is similar to that in   step-12   and   step-47  . Different from assembling by hand, we just need to focus on assembling on each cell, each boundary face, and each interior face. The loops over cells and faces are handled automatically by   MeshWorker::mesh_loop().     
...
* 
 @endcode
* 
*   <a name="Thesolveandoutput_resultsfunction"></a>  <h3>The solve() and output_results() function</h3> The following two functions are entirely standard and without difficulty.
* 
...
* 
 ...
 @endcode
* 
*   <a name="Thecompute_error_estimatefunction"></a>  <h3>The compute_error_estimate() function</h3> The assembly of the error estimator here is quite similar to that of the global matrix and right-had side and can be handled by the   MeshWorker::mesh_loop()   framework. To understand what each of the local (lambda) functions is doing, recall first that the local cell residual is defined as   $h_K^2 \left\| f + \nu \Delta u_h \right\|_K^2$  :
* 
...
* 
 @endcode
* 
*   <a name="Thecompute_energy_norm_errorfunction"></a>  <h3>The compute_energy_norm_error() function</h3> Next, we evaluate the accuracy in terms of the energy norm. This function is similar to the assembling of the error estimator above. Here we compute the square of the energy norm defined by @f[
 \|u \|_{1,h}^2 = \sum_{K \in \Gamma_h} \nu\|\nabla u \|_K^2 +
 \sum_{f \in F_i} \sigma \| [ u ] \|_f^2 +
 \sum_{f \in F_b} \sigma  \|u\|_f^2.
 @f] Therefore the corresponding error is @f[
 \|u
* 
...
* 
 @endcode
* 
*   <a name="Thecompute_errorsfunction"></a>  <h3>The compute_errors() function</h3> We compute three errors in the   $L_2$   norm,   $H_1$   seminorm, and the energy norm, respectively. These are then printed to screen, but also stored in a table that records how these errors decay with mesh refinement and which can be output in one step at the end of the program.
* 
...
* 
 @endcode
* 
*   <a name="Themainfunction"></a>  <h3>The main() function</h3> The following   <code>main</code>   function is similar to previous examples as well, and need not be commented on.
* 
...
@endcode
* 
* When using the smooth case with polynomial degree 3, the convergencetable will look like this:  <table align="center" class="doxtable">
  <tr>
    <th>cycle</th>
    <th>n_cellss</th>
    <th>n_dofs</th>
...
* Theoretically, for polynomial degree   $p$  , the order of convergence in   $L_2$  norm and   $H^1$   seminorm should be   $p+1$   and   $p$  , respectively. Our numericalresults are in good agreement with theory.
...
@endcode
* 
* The following figure provides a log-log plot of the errors versusthe number of degrees of freedom for this test case on the L-shapeddomain. In order to interpret it, let   $n$   be the number of degrees offreedom, then on uniformly refined meshes,   $h$   is of order  $1/\sqrt{n}$   in 2D. Combining the theoretical results in the previous case,we see that if the solution is sufficiently smooth,we can expect the error in the   $L_2$   norm to be of order   $O(n^{-\frac{p+1}{2}})$  and in   $H^1$   seminorm to be   $O(n^{-\frac{p}{2}})$  . It is not a prioriclear that one would get the same kind of behavior as a function of  $n$   on adaptively refined meshes like the ones we use for this secondtest case, but one can certainly hope. Indeed, from the figure, we seethat the SIPG with adaptive mesh refinement produces asymptoticallythe kinds of hoped-for results:
*   <img width="600px" src="https://www.dealii.org/images/steps/developer/step-74.log-log-plot.png" alt="">  
* In addition, we observe that the error estimator decreasesat almost the same rate as the errors in the energy norm and   $H^1$   seminorm,and one order lower than the   $L_2$   error. This suggestsits ability to predict regions with large errors.
* While this tutorial is focused on the implementation, the   step-59   tutorial program achieves an efficientlarge-scale solver in terms of computing time with matrix-free solution techniques.Note that the   step-59   tutorial does not work with meshes containing hanging nodes at this moment,because the multigrid interface matrices are not as easily determined,but that is merely the lack of some interfaces in deal.II, nothing fundamental.
* 
