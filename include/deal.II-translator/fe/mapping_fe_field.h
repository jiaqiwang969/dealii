 ...
 * only FE_Q. This class effectively decouples the topology from the geometry,
 * by relegating all geometrical information to some components of a
 * FiniteElement vector field. The components that are used for the geometry
 * can be arbitrarily selected at construction time. The idea is to consider
 * the Triangulation as a parameter configuration space, on which we construct
 * an arbitrary geometrical mapping, using the instruments of the deal.II
 * library: a vector of degrees of freedom, a DoFHandler associated to the
 * geometry of the problem and a ComponentMask that tells us which components
 * of the FiniteElement to use for the mapping. Typically, the DoFHandler
 * operates on a finite element that is constructed as a system element
 * (FESystem()) from continuous FE_Q() (for iso-parametric discretizations) or
 * FE_Bernstein() (for iso-geometric discretizations) objects. An example is
 * shown below:
 *
 *
 * @code
...
public:
   ...
   * configuration.     In general this class decouples geometry from
   * topology, allowing users to   define geometries which are only
   * topologically equivalent to the   underlying Triangulation, but which may
   * otherwise be arbitrary.   Differently from what happens in
   * MappingQEulerian, the FiniteElement   field which is passed to the
   * constructor is interpreted as an absolute   geometrical configuration,
   * therefore one has to make sure that the   euler_vector actually
   * represents a valid geometry (i.e., one with no   inverted cells, or with
   * no zero-volume cells).     If the underlying FiniteElement is a system of
   * FE_Q(), and euler_vector   is initialized using
   * VectorTools::get_position_vector(),   then this class   is in all
   * respects identical to MappingQ().     The optional ComponentMask argument
   * can be used to specify what   components of the FiniteElement to use for
   * the geometrical   transformation. If no mask is specified at construction
   * time, then a   default one is used, which makes this class works in the
   * same way of   MappingQEulerian(), i.e., the first spacedim components of
   * the   FiniteElement are assumed to represent the geometry of the problem.
   ...
   * will be used for the geometry.     If an incompatible mask is passed, an
   * exception is thrown.
   *
   */
   ...
   * each level, i.e., be of compatible length   DoFHandler::n_dofs(level).
   * A   prerequisite of this constructor is that
   * DoFHandler::distribute_mg_dofs()     has been called. Apart from the
   * level vectors, the same arguments as in   the other constructor need to
   * be provided.
   *
   */
  MappingFEField(const DoFHandler<dim, spacedim> &euler_dof_handler,
  ...
  /**
   * Constructor with MGLevelObject instead of   std::vector,   otherwise the
   * same   as above. It is required that `euler_vector.max_level()+1` equals
   * the   global number of levels in the triangulation. The minimum level may
   * be   zero or more &mdash; it only needs to be consistent between what is
   * set   here and later used for evaluation of the mapping.
   *
   */
  MappingFEField(const DoFHandler<dim, spacedim> &euler_dof_handler,
                 const MGLevelObject<VectorType> &euler_vector,
  ...
  /**
   * See the documentation of   Mapping::preserves_vertex_locations()     for
   * the purpose of this function. The implementation in this   class always
   * returns   @p false.
   *
   */
  virtual bool
  ...
  /**
   * Return the mapped vertices of a cell.     This mapping ignores the
   * vertices of the Triangulation it is associated to,   and constructs the
   * position of the vertices according to the   @p euler_vector     that was
...
public:
  /**
   * Storage for internal data of this mapping. See
   * Mapping::InternalDataBase     for an extensive description.     This
   * includes data that is computed once when the object is created (in
  ...
  public:
     ...
     * third_derivative.         Computed once.
     *
     */
     ...
     * fourth_derivative.         Computed once.
     *
     */
     ...
     * normal vectors.         This array has
     * `(dim-1)*GeometryInfo<dim>::%faces_per_cell`   entries. The     first
     ...
     * euler_dh.         If a mask is specified, then it has to match the
     * underlying     FiniteElement, and it has to have exactly spacedim
     * non-zero elements,     indicating the components (in order) of the
     * FiniteElement which will be     used for the euler vector and the euler
     * dof handler.
     *
     */
    ComponentMask mask;
...
private:
  /**
   * Transform a point   @p p   on the unit cell to the point   @p p_real   on
   * the   real cell   @p cell   and returns   @p p_real.       This function
   * is called by   @p transform_unit_to_real_cell   and multiple   times
   * (through the Newton iteration) by   @p
   * transform_real_to_unit_cell_internal.     Takes a reference to an   @p
   * InternalData   that must already include the   shape values at point   @p
   * p   and the mapping support points of the cell.     This   @p
   * InternalData   argument avoids multiple computations of the shape
   * values at point   @p p   and especially multiple computations of the
   ...
   * on   the unit cell   @p cell   by a Newton iteration.     Takes a
   * reference to an   @p InternalData   that is assumed to be previously
   * created by the   @p get_data   function with   @p UpdateFlags   including
   * @p     update_transformation_values and   @p
   * update_transformation_gradients   and a   one point Quadrature that
   * includes the given initial guess for the   transformation   @p
   * initial_p_unit.    Hence this function assumes that   @p     mdata
   * already includes the transformation shape values and gradients   computed
   * at   @p initial_p_unit.         @p mdata   will be changed by this
   * function.
   *
   */
  Point<dim>
