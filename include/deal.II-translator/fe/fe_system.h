 ...
 * $d$   of the domain) is discretized with   $Q_2$   elements and the
 * pressure with   $Q_1$   elements. Mathematically, the finite element space
 * for the coupled problem is then often written as   $V_h = Q_2^d \times Q_1$
 * where the exponentiation is understood to be the tensor product of spaces
 *
 *  -  i.e., in 2d, we have   $V_h=Q_2\times Q_2\times Q_1$
 *
 *  -  and tensor products lead to vectors where each component of the vector-valued function space corresponds to a scalar function in one of the   $Q_2$   or   $Q_1$   spaces. Using the FESystem class, this space is created using
 *
 * @code
 ...
 * product with the element in the second line. This kind of construction is
 * used, for example, in the   step-22   tutorial program. Similarly,   step-8
 * solves an elasticity equation where we need to solve for the displacement
 * of a solid object. The displacement again has   $d$   components if the
 * domain is   $d$  -dimensional, and so the combined finite element is
 * created using
 *
 * @code
 ...
 * $Q_1$   space. To the outside world, FESystem objects look just like a
 * usual finite element object, they just happen to be composed of several
 * other finite elements that are possibly of different type. These "base
 * elements" can themselves have multiple components and, in particular, could
 * also be vector-valued
 *
 *  -  for example, if one of the base elements is an FESystem itself (see also below). An example is given in the documentation of namespace   FETools::Compositing,   when using the "tensor product" strategy.
 * %Vector valued
 * elements are discussed in a number of tutorial programs, for example
 * step-8  ,   step-20  ,   step-21  ,   step-22  , and in particular in the
 * @ref vector_valued   module. @dealiiVideoLecture{19,20}
 *
 *  <h3>FESystem, components and blocks</h3>
 * An FESystem, except in the most trivial case, produces a vector-valued finite element with several components. The number of components n_components() corresponds to the dimension of the solution function in the PDE system, and correspondingly also to the number of equations your PDE system has. For example, the mixed Laplace system covered in   step-20   has   $d+1$   components in   $d$   space dimensions: the scalar pressure and the   $d$   components of the velocity vector. Similarly, the elasticity equation covered in   step-8   has   $d$   components in   $d$   space dimensions. In general, the number of components of a FESystem element is the accumulated number of components of all base elements times their multiplicities. A bit more on components is also given in the   @ref GlossComponent   "glossary entry on components".
 * While the concept of components is important from the viewpoint of a partial differential equation, the finite element side looks a bit different Since not only FESystem, but also vector-valued elements like FE_RaviartThomas, have several components. The concept needed here is a   @ref GlossBlock   "block".
 ...
 * information in   DoFHandler::block_info().   The number of blocks of a
 * FESystem object is simply the sum of all multiplicities of base elements
 * and is given by n_blocks(). For example, the FESystem for the Taylor-Hood
 * element for the three-dimensional Stokes problem can be built using the
 * code
 *
 * @code
 * const FE_Q<3> u(2);
 ...
 * @endcode
 *
 * This example creates an FESystem   @p sys1   with four components, three
 * for the velocity components and one for the pressure, and also four blocks
 * with the degrees of freedom of each of the velocity components and the
 * pressure in a separate block each. The number of blocks is four since the
 * first base element is repeated three times. On the other hand, a
 * Taylor-Hood element can also be constructed using
 *
 *
 * @code
 * FESystem<3> U(u,3);
 ...
 * contains the pressure degrees of freedom. Note that while   @p U   itself
 * has 3 blocks, the FESystem   @p sys2   does not attempt to split   @p U
 * into its base elements but considers it a block of its own. By blocking all
 * velocities into one system first as in   @p sys2,   we achieve the same
 * block structure that would be generated if instead of using a   $Q_2^3$
 * element for the velocities we had used vector-valued base elements, for
 * instance like using a mixed discretization of Darcy's law using
 *
 *
 * @code
 ...
 * blocks. In most cases, the composed element behaves as if it were a usual
 ...
 * velocity is only coupled to   @p u   at the vertices and the line on the
 * larger cell next to this vertex, but there is no interaction with   @p v
 * and   @p w   of this or the other cell.
 *
 *  <h3>Internal information on numbering of degrees of freedom</h3> The
 * overall numbering of degrees of freedom is as follows: for each subobject
 * (vertex, line, quad, or hex), the degrees of freedom are numbered such that
 * we run over all subelements first, before turning for the next dof on this
 ...
 * <ul>     <li>   First vertex: <tt>u0, v0, p0 = s0, s1, s2</tt>   <li>   Second vertex: <tt>u1, v1, p1 = s3, s4, s5</tt>   <li>   First component on the line: <tt>u2, u3 = s4, s5</tt>   <li>   Second component on the line: <tt>v2, v3 = s6, s7</tt>.   <li>   Third component on the line: <tt>p2 = s8</tt>.   </ul>   That said, you should not rely on this numbering in your application as these %internals might change in future. Rather use the functions system_to_component_index() and component_to_system_index().
...
public:
   ...
   * group together using this class.     The object   @p fe   is not actually
   * used for anything other than creating a   copy that will then be owned by
   * the current object. In other words, it is   completely fine to call this
   * constructor with a temporary object for the   finite element, as in this
   * code snippet:
   * @code
   * FESystem<dim> fe (FE_Q<dim>(2), 2);
   * @endcode
   * Here,   <code>FE_Q@<dim@>(2)</code>   constructs an unnamed, temporary object   that is passed to the FESystem constructor to create a finite element   that consists of two components, both of which are quadratic FE_Q   elements. The temporary is destroyed again at the end of the code that   corresponds to this line, but this does not matter because FESystem   creates its own copy of the FE_Q object.     This constructor (or its variants below) is used in essentially all   tutorial programs that deal with vector valued problems. See   step-8  ,     step-20  ,   step-22   and others for use cases. Also see the module on     @ref vector_valued   "Handling vector valued problems".
   * @dealiiVideoLecture{19,20}         @param[in]   fe The finite element
   * that will be used to represent the   components of this composed element.
   * @param[in]   n_elements An integer denoting how many copies of   @p fe
   * this   element should consist of.
   *
   */
  ...
  /**
   * Constructor for mixed discretizations with two base elements.     See the
   * other constructor above for an explanation of the general idea of
   * composing elements.
   *
   */
  ...
  /**
   * Constructor for mixed discretizations with three base elements.     See
   * the other constructor above for an explanation of the general idea of
   * composing elements.
   *
   */
  FESystem(const FiniteElement<dim, spacedim> &fe1,
  ...
  /**
   * Constructor for mixed discretizations with four base elements.     See
   * the first of the other constructors above for an explanation of the
  ...
  /**
   * Constructor for mixed discretizations with five base elements.     See
   * the first of the other constructors above for an explanation of the
   * general idea of composing elements.
   *
   */
   ...
   * constructor. The lengths of these vectors are assumed to be equal.     As
   * above, the finite element objects pointed to by the first argument are
   ...
   * constructor.     <h4>How to use this constructor</h4>     Using this
   * constructor is a bit awkward at times because you need to pass   two
   * vectors in a place where it may not be straightforward to construct
   * such a vector
   *
   ...
   * initializer list enclosed in braces   <code>{ ... }</code>  .     This
   * code has a problem: it creates four memory leaks because the first
   * vector above is created with pointers to elements that are allocated with
   * <code>new</code>   but never destroyed.     The solution to the second of
   * these problems is to create two static   member functions that can create
   ...
   * variable   <code>fe</code>  . They need to be static because they are
   * called   during the constructor of   <code>MySimulator</code>   at a time
   * when the     <code>*this</code>   object isn't fully constructed and,
   * consequently,   regular member functions cannot be called yet.     The
   * code above does not solve the problem with the memory leak yet,   though:
   * the   <code>create_fe_list()</code>   function creates a vector of
   ...
   * constructor of   <code>fe</code>   has finished) and destroys the
   * elements of   the temporary vector. Voila: not short nor elegant, but it
   * works!
   *
   */
...
#  if !defined(__INTEL_COMPILER) || __INTEL_COMPILER >= 1900
   ...
   * int></code>. In combination with   FiniteElement::operator^,   this
   * allows to   construct FESystem objects as follows:
   * @code
   * FiniteElementType1<dim,spacedim> fe_1;
   * FiniteElementType1<dim,spacedim> fe_2;
   * FESystem<dim,spacedim> fe_system ( fe_1^dim, fe_2 );
   * @endcode
   * The `fe_1` and `fe_2` objects are not actually used for anything other
   * than   creating a copy that will then be owned by the current object. In
   * other   words, it is completely fine to call this constructor with a
   * temporary   object for the finite element, as in this code snippet:
   * @code
   * FESystem<dim> fe (FE_Q<dim>(2)^2);
   * @endcode
   * Here,   <code>FE_Q@<dim@>(2)</code>   constructs an unnamed, temporary
   * object   that is passed to the FESystem constructor to create a finite
   * element   that consists of two components, both of which are quadratic
   * FE_Q   elements. The temporary is destroyed again at the end of the code
   * that   corresponds to this line, but this does not matter because
   * FESystem   creates its own copy of the FE_Q object.     As a shortcut,
   * this constructor also allows calling
   * @code
   ...
   * would have expected).       @warning   This feature is not available for
   * Intel compilers   prior to version 19.0. Defining this   constructor
   * leads to internal compiler errors for Intel compilers prior   to 18.0.
   *
   */
  template <
    class... FEPairs,
   ...
   * @endcode
   * @warning   This feature is not available for Intel compilers   prior to
   * version 19.0. The constructor is just not selected for overload
   * resolution.
   *
   */
  FESystem(
...
#  endif
  ...
  /**
   * Copy constructor. This constructor is deleted, i.e., copying   FESystem
   ...
   * <tt>FESystem<dim>[name1^m1-name2^m2-...-nameN^mN]</tt>, where   @p mi
   * are   the multiplicities of the basis elements. If a multiplicity is
   * equal to   one, then the superscript is omitted.
   *
   */
  virtual std::string
   ...
   * @p p   is   a point on the reference element. Since this finite element
   * is always   vector-valued, we return the value of the only non-zero
   ...
   * the term non-primitive),   then throw an exception of type   @p
   * ExcShapeFunctionNotPrimitive.       An   @p ExcUnitShapeValuesDoNotExist
   * is thrown if the shape values of the     @p FiniteElement
   * (corresponding to the   @p ith   shape function) depend on   the shape of
   * the cell in real space.
   *
   */
  virtual double
  ...
  /**
   * Return the value of the   @p componentth   vector component of the   @p
   * ith     shape function at the point   @p p.   See the FiniteElement base
   ...
   * Since   this finite element is always vector-valued, we return the value
   * of the   only non-zero component of the vector value of this shape
   * function. If   the shape function has more than one non-zero component
   * (which we refer   to with the term non-primitive), then throw an
   * exception of type   @p     ExcShapeFunctionNotPrimitive.     An   @p
   * ExcUnitShapeValuesDoNotExist   is thrown if the shape values of the
   * @p FiniteElement   (corresponding to the   @p ith   shape function)
   * depend on   the shape of the cell in real space.
   *
   */
  ...
  /**
   * Return the gradient of the   @p componentth   vector component of the
   * @p ith     shape function at the point   @p p.   See the FiniteElement
   * base class for   more information about the semantics of this function.
   * Since this element is vector valued in general, it relays the computation
   * of these values to the base elements.
   *
   */
   ...
   * the unit   cell with respect to unit cell coordinates. Since this finite
   * element is   always vector-valued, we return the value of the only
   ...
   * with the term non-   primitive), then throw an exception of type   @p
   * ExcShapeFunctionNotPrimitive.     An   @p ExcUnitShapeValuesDoNotExist
   * is thrown if the shape values of the     @p FiniteElement
   * (corresponding to the   @p ith   shape function) depend on   the shape of
   * the cell in real space.
  ...
  /**
   * Return the second derivatives of the   @p componentth   vector component
   * of   the   @p ith   shape function at the point   @p p.   See the
   * FiniteElement base   class for more information about the semantics of
   * this function.     Since this element is vector valued in general, it
   * relays the computation   of these values to the base elements.
   *
   */
  virtual Tensor<2, dim>
  ...
  /**
   * Return the third derivatives of the   @p componentth   vector component
   * of   the   @p ith   shape function at the point   @p p.   See the
   * FiniteElement base   class for more information about the semantics of
   * this function.     Since this element is vector valued in general, it
   * relays the computation   of these values to the base elements.
  ...
  /**
   * Return the fourth derivatives of the   @p componentth   vector component
   * of   the   @p ith   shape function at the point   @p p.   See the
   * FiniteElement base   class for more information about the semantics of
   * this function.     Since this element is vector valued in general, it
   * relays the computation   of these values to the base elements.
   ...
   * <tt>source.n_dofs_per_cell()</tt>.     These matrices are available if
   * source and destination element are both     @p FESystem   elements, have
   * the same number of base elements with same   element multiplicity, and if
   * these base elements also implement their   @p
   ...
   * (initialize when requested).     If this projection operator is
   * associated with a matrix   @p P,   then the   restriction of this matrix
   * @p P_i   to a single child cell is returned   here.     The matrix   @p P
   * is the concatenation or the sum of the cell matrices   @p     P_i,
   * depending on the value of   FiniteElement::restriction_is_additive().
   * This distinguishes interpolation (concatenation) and projection with
   * respect to scalar products (summation).     Row and column indices are
   * related to coarse grid and fine grid spaces,   respectively, consistent
   * with the definition of the associated operator.     If projection
   * matrices are not implemented in the derived finite element   class, this
   * function aborts with an exception of type
   ...
   * associated with a matrix   @p P.   The restriction of this matrix   @p
   * P_i   to a   single child cell is returned here.     The matrix   @p P
   * is the concatenation, not the sum of the cell matrices   @p     P_i. That
   * is, if the same non-zero entry <tt>j,k</tt> exists in two   different
   * child matrices   @p P_i,   the value should be the same in both
   * matrices and it is copied into the matrix   @p P   only once.     Row and
   * column indices are related to fine grid and coarse grid spaces,
   ...
   * are discarded and will not fill up the transfer matrix.     If
   * prolongation matrices are not implemented in one of the base finite
   * element classes, this function aborts with an exception of type
   * FiniteElement::ExcEmbeddingVoid.   You can check whether this would
   * happen   by first calling the prolongation_is_implemented() or the
   ...
   * This function returns   @p true   if and only if all its base elements
   * return     @p true   for this function.
   *
   */
  virtual bool
   ...
   * function.     This function simply calls
   * FiniteElement::convert_generalized_support_point_values_to_dof_values
   ...
   * values are filled with "signaling" NaNs instead.     The function fails
   * if none of the base elements of the FESystem are   interpolatory.
   *
   */
  virtual void
...
protected:
  ...
  /**
   * Do the work for the three <tt>fill_fe*_values</tt> functions.     Calls
   * (among other things) <tt>fill_fe_([sub]face)_values</tt> of the   base
   * elements. Calls   @p fill_fe_values   if
   ...
   * <tt>sub_no!=invalid_face_no</tt>; and calls   @p fill_fe_subface_values
   * if   <tt>face_no!=invalid_face_no</tt> and
...
private:
  ...
  /**
   * Pointers to underlying finite element objects.     This object contains a
   * pointer to each contributing element of a mixed   discretization and its
   * multiplicity. It is created by the constructor and   constant afterwards.
   *
   */
   ...
   * to the vector of generalized support points of the FE System.   It holds
   * true that
   * @code
   ...
   * several of them. It sets up the index table for the system as well as
   * @p     restriction and   @p prolongation   matrices.
   *
   */
  void
  ...
  /**
   * Usually: Fields of cell-independent data.     However, here, this class
   * does not itself store the data but only   pointers to   @p InternalData
  ...
  public:
    /**
     * Constructor. Is called by the   @p get_data   function. Sets the size
     * of     the   @p base_fe_datas   vector to   @p n_base_elements.
     *
     */
    InternalData(const unsigned int n_base_elements);

    /**
     * Destructor. Deletes all   @p InternalDatas   whose pointers are stored
     * by     the   @p base_fe_datas   vector.
     *
     */
  ...
  private:
     ...
     * They     are accessed to by the   @p set_   and   @p get_fe_data
     * functions.         The size of this vector is set to   @p
     * n_base_elements   by the     InternalData constructor.  It is filled by
     * the   @p get_data   function.     Note that since the data for each
     * instance of a base class is     necessarily the same, we only need as
     * many of these objects as there     are base elements, irrespective of
     * their multiplicity.
     *
     */
    typename std::vector<
     ...
     * functions on them.         The size of this vector is set to   @p
     * n_base_elements   by the     InternalData constructor.
     *
     */
    mutable std::vector<
      internal::FEValuesImplementation::FiniteElementRelatedData<dim, spacedim>>
