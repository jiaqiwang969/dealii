...
/**
 * Implementation of a scalar Lagrange finite element   @p Qp   that yields
 * the finite element space of continuous, piecewise polynomials of degree
 * @p p   in each coordinate direction. This class is realized using tensor
 * product polynomials based on 1D Lagrange polynomials with equidistant
 * (degree up to 2), Gauss-Lobatto (starting from degree 3), or given support
 * points. The standard constructor of this class takes the degree   @p p   of
 * this finite element. Alternatively, it can take a quadrature formula   @p
 * points   defining the support points of the Lagrange interpolation in one
 * coordinate direction. For more information about the <tt>spacedim</tt>
 * template parameter check the documentation of FiniteElement or the one of
 * Triangulation. <h3>Implementation</h3> The constructor creates a
 * TensorProductPolynomials object that includes the tensor product of   @p
 * LagrangeEquidistant   polynomials of degree   @p p.   This   @p
 * TensorProductPolynomials   object provides all values and derivatives of
 ...
 * are implemented only up to a certain degree and may not be available for
 * very high polynomial degree. <h3>Unit support point distribution and
 * conditioning of interpolation</h3> When constructing an FE_Q element at
 * polynomial degrees one or two, equidistant support points at 0 and 1
 * (linear case) or 0, 0.5, and 1 (quadratic case) are used. The unit support
 * or nodal points <i>x<sub>i</sub></i> are those points where the <i>j</i>th
 * Lagrange polynomial satisfies the   $\delta_{ij}$   property, i.e., where
 * one polynomial is one and all the others are zero.  For higher polynomial
 * degrees, the support points are non-equidistant by default, and chosen to
 * be the support points of the <tt>(degree+1)</tt>-order Gauss-Lobatto
 * quadrature rule. This point distribution yields well-conditioned Lagrange
 * interpolation at arbitrary polynomial degrees. By contrast, polynomials
 * based on equidistant points get increasingly ill-conditioned as the
 * polynomial degree increases. In interpolation, this effect is known as the
 * Runge phenomenon. For Galerkin methods, the Runge phenomenon is typically
 * not visible in the solution quality but rather in the condition number of
 * the associated system matrices. For example, the elemental mass matrix of
 * equidistant points at degree 10 has condition number 2.6e6, whereas the
 * condition number for Gauss-Lobatto points is around 400. The Gauss-Lobatto
 * points in 1D include the end points 0 and +1 of the unit interval. The
 * interior points are shifted towards the end points, which gives a denser
 * point distribution close to the element boundary. If combined with
 * Gauss-Lobatto quadrature, FE_Q based on the default support points gives
 * diagonal mass matrices. This case is demonstrated in   step-48  . However,
 * this element can be combined with arbitrary quadrature rules through the
 * usual FEValues approach, including full Gauss quadrature. In the general
 * case, the mass matrix is non-diagonal. <h3>Numbering of the degrees of
 * freedom (DoFs)</h3> The original ordering of the shape functions
 * represented by the TensorProductPolynomials is a tensor product numbering.
 * However, the shape functions on a cell are renumbered beginning with the
 * shape functions whose support points are at the vertices, then on the line,
 * on the quads, and finally (for 3d) on the hexes. To be explicit, these
 * numberings are listed in the following:
 * <h4>Q1 elements</h4>   <ul>     <li>   1D case:
 * @verbatim
 *    0-------1
 * @endverbatim
 *
 ...
 * @endverbatim
 *
 * The respective coordinate values of the support points of the shape functions are as follows:   <ul>     <li>   Shape function 0: <tt>[0, 0, 0]</tt>;   <li>   Shape function 1: <tt>[1, 0, 0]</tt>;   <li>   Shape function 2: <tt>[0, 1, 0]</tt>;   <li>   Shape function 3: <tt>[1, 1, 0]</tt>;   <li>   Shape function 4: <tt>[0, 0, 1]</tt>;   <li>   Shape function 5: <tt>[1, 0, 1]</tt>;   <li>   Shape function 6: <tt>[0, 1, 1]</tt>;   <li>   Shape function 7: <tt>[1, 1, 1]</tt>;   </ul>     </ul>
 ...
 *
 * <h4>Q2 elements</h4>   <ul>     <li>   1D case:
 * @verbatim
 *    0---2---1
 * @endverbatim
 *
 ...
 * The respective coordinate values of the support points of the shape functions are as follows:   <ul>     <li>   Shape function 0: <tt>[0, 0, 0]</tt>;   <li>   Shape function 1: <tt>[1, 0, 0]</tt>;   <li>   Shape function 2: <tt>[0, 1, 0]</tt>;   <li>   Shape function 3: <tt>[1, 1, 0]</tt>;   <li>   Shape function 4: <tt>[0, 0, 1]</tt>;   <li>   Shape function 5: <tt>[1, 0, 1]</tt>;   <li>   Shape function 6: <tt>[0, 1, 1]</tt>;   <li>   Shape function 7: <tt>[1, 1, 1]</tt>;   <li>   Shape function 8: <tt>[0, 1/2, 0]</tt>;   <li>   Shape function 9: <tt>[1, 1/2, 0]</tt>;   <li>   Shape function 10: <tt>[1/2, 0, 0]</tt>;   <li>   Shape function 11: <tt>[1/2, 1, 0]</tt>;   <li>   Shape function 12: <tt>[0, 1/2, 1]</tt>;   <li>   Shape function 13: <tt>[1, 1/2, 1]</tt>;   <li>   Shape function 14: <tt>[1/2, 0, 1]</tt>;   <li>   Shape function 15: <tt>[1/2, 1, 1]</tt>;   <li>   Shape function 16: <tt>[0, 0, 1/2]</tt>;   <li>   Shape function 17: <tt>[1, 0, 1/2]</tt>;   <li>   Shape function 18: <tt>[0, 1, 1/2]</tt>;   <li>   Shape function 19: <tt>[1, 1, 1/2]</tt>;   <li>   Shape function 20: <tt>[0, 1/2, 1/2]</tt>;   <li>   Shape function 21: <tt>[1, 1/2, 1/2]</tt>;   <li>   Shape function 22: <tt>[1/2, 0, 1/2]</tt>;   <li>   Shape function 23: <tt>[1/2, 1, 1/2]</tt>;   <li>   Shape function 24: <tt>[1/2, 1/2, 0]</tt>;   <li>   Shape function 25: <tt>[1/2, 1/2, 1]</tt>;   <li>   Shape function 26: <tt>[1/2, 1/2, 1/2]</tt>;   </ul>     </ul>
 *
 ...
 *
 * <h4>Q3 elements</h4>   <ul>     <li>   1D case:
 * @verbatim
 *    0--2--3--1
 * @endverbatim
 *
 ...
 *
 * <h4>Q4 elements</h4>   <ul>     <li>   1D case:
 * @verbatim
 *    0--2--3--4--1
 * @endverbatim
 *
...
public:
  ...
  /**
   * Constructor for tensor product polynomials with support points   @p
   * points     based on a one-dimensional quadrature formula. The degree of
   * the finite   element is <tt>points.size()-1</tt>. Note that the first
   * point has to be   0 and the last one 1. Constructing
   ...
   * Thespace* created by this constructor is the   same as if you had called
   * `FE_Q<dim>(points.size()-1)`, but whatis*
   * different are the basis functions of this space. This is useful in   contexts where one wants to achieve certain properties of the matrices   that result from integrating bilinear forms of these shape functions.   For example, when computing the mass matrix on cell   $K$  ,   @f[
   * M_{ij}^K = \int_K \varphi_i(\mathbf x) \varphi_j(\mathbf x) \; dx
   * @f]   one generally applies a quadrature formula and approximates the real   mass matrix by the following:   @f[
   * M_{ij}^K = \sum_q \varphi_i(\mathbf x_q) \varphi_j(\mathbf x_q) w_q,
   * @f]   where the locations of the quadrature points   $\mathbf x_q$   and the   weights   $w_q$   depend on the cell   $K$  . If the quadrature formula used   to define these points   $\mathbf x_q$   is the same as the one passed   to this constructor (or, in higher dimensions, is generated by a   tensor product of the one used to the constructor), then     $\varphi_i(\mathbf x_q) = \delta_{iq}$  , and the matrix reduces to   @f[
   * M_{ij}^K = \sum_q \delta_{iq} \delta_{jq} w_q = \delta_{ij} w_i, @f]
   ...
   * returns <tt>FE_Q<dim>(degree)</tt>, with   @p dim   and   @p degree
