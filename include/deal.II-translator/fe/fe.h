 ...
 *
 *
 *  - %Functions that describe the properties of individual shape functions, for example which   @ref GlossComponent   "vector components" of a   @ref vector_valued   "vector-valued finite element's" shape function is nonzero, or whether an element is   @ref GlossPrimitive   "primitive".
 *
 *
 *
 *  - For elements that are interpolatory, such as the common   $Q_p$   Lagrange elements, data that describes where their   @ref GlossSupport   "support points" are located.
 *
 *
 *
 ...
 * element space. As a general rule, there are three ways by which derived
 ...
 *
 *
 *  - Uncommon information, or information that depends on specific input arguments, and that needs to be implemented by derived classes. For these, this base class only declares abstract virtual member functions and derived classes then have to implement them. Examples of this category would include the functions that compute values and derivatives of shape functions on the reference cell for which it is not possible to tabulate values because there are infinitely many points at which one may want to evaluate them. In some cases, derived classes may choose to simply not implement <i>all</i> possible interfaces (or may not <i>yet</i> have a complete implementation); for uncommon functions, there is then often a member function derived classes can overload that describes whether a particular feature is implemented. An example is whether an element implements the information necessary to use it in the   $hp$   finite element context (see   @ref hp   "hp-finite element support").
 *
 *  <h3>Nomenclature</h3> Finite element classes have to define a large number
 * of different properties describing  a finite element space. The following
 * subsections describe some nomenclature that will be used in the
 * documentation below. <h4>Components and blocks</h4>
 ...
 * primitive". The   FiniteElement::get_nonzero_components()   can then be
 * used to determine which vector components of a shape function are nonzero.
 * The number of nonzero components of a shape function is returned by
 * FiniteElement::n_components().   Whether a shape function is non-primitive
 * can be queried by   FiniteElement::is_primitive().
 * Oftentimes, one may want to split linear system into blocks so that they reflect the structure of the underlying operator. This is typically not done based on vector components, but based on the use of   @ref GlossBlock   "blocks",
 ...
 * element can be determined by   FiniteElement::n_blocks(). To better
 * illustrate these concepts, let's consider the following example of the
 ...
 * FiniteElement::system_to_base_index()   one can get the following
 * information for each degree-of-freedom "i":
 *
 * @code
 ...
 * @endcode
 * which will result in: | DoF    | Component  | Base element | Shape function
 ...
 * 0 |                          7 |            1 | |     20 |          0 |
 * 0 |                          8 |            0 | |     21 |          1 |
 * 0 |                          8 |            1 | What we see is the
 * following: there are a total of 22 degrees-of-freedom on this element with
 * components ranging from 0 to 2. Each DoF corresponds to one of the two base
 * elements used to build FESystem :   $\mathbb Q_2$   or   $\mathbb Q_1$  .
 * Since FE_Q are primitive elements, we have a total of 9 distinct
 * scalar-valued shape functions for the quadratic element and 4 for the
 * linear element. Finally, for DoFs corresponding to the first base element
 * multiplicity is either zero or one, meaning that we use the same scalar
 * valued   $\mathbb Q_2$   for both   $x$   and   $y$   components of the
 * velocity field   $\mathbb Q_2 \otimes \mathbb Q_2$  . For DoFs
 * corresponding to the second base element multiplicity is zero. <h4>Support
 * points</h4> Finite elements are frequently defined by defining a polynomial
 * space and a set of dual functionals. If these functionals involve point
 * evaluations, then the element is "interpolatory" and it is possible to
 * interpolate an arbitrary (but sufficiently smooth) function onto the finite
 * element space by evaluating it at these points. We call these points
 * "support points".
 ...
 *
 *  <h3>Implementing finite element spaces in derived classes</h3> The
 * following sections provide some more guidance for implementing concrete
 ...
 * by a list of tools helping to generate information in concrete cases. It is
 * important to note that there is a number of intermediate classes that can
 * do a lot of what is necessary for a complete description of finite element
 * spaces. For example, the FE_Poly, FE_PolyTensor, and FE_PolyFace classes in
 * essence build a complete finite element space if you only provide them with
 * an abstract description of the polynomial space upon which you want to
 * build an element. Using these intermediate classes typically makes
 * implementing finite element descriptions vastly simpler.
 * As a
 * general rule, if you want to implement an element, you will likely want to
 * look at the implementation of other, similar elements first. Since many of
 * the more complicated pieces of a finite element interface have to do with
 * how they interact with mappings, quadrature, and the FEValues class, you
 * will also want to read through the   @ref FE_vs_Mapping_vs_FEValues
 * documentation module.
 *
 *  <h4>Interpolation matrices in one dimension</h4> In one space dimension
 * (i.e., for   <code>dim==1</code>   and any value of   <code>spacedim</code>
 * ), finite element classes implementing the interface of the current base
 * class need only set the #restriction and #prolongation matrices that
 * describe the interpolation of the finite element space on one cell to that
 * of its parent cell, and to that on its children, respectively. The
 * constructor of the current class in one dimension presets the
 ...
 * zero because there are no hanging nodes in 1d. <h4>Interpolation matrices
 * in two dimensions</h4> In addition to the fields discussed above for 1D, a
 * constraint matrix is needed to describe hanging node constraints if the
 * finite element has degrees of freedom located on edges or vertices. These
 * constraints are represented by an   $m\times n$  -matrix
 ...
 * thus a rectangular one. The   $m\times n$   size of the
 * #interface_constraints matrix can also be accessed through the
 * interface_constraints_size() function. The mapping of the dofs onto the
 * indices of the matrix on the unrefined side is as follows: let   $d_v$   be
 * the number of dofs on a vertex,   $d_l$   that on a line, then
 * $n=0...d_v-1$   refers to the dofs on vertex zero of the unrefined line,
 * $n=d_v...2d_v-1$   to those on vertex one,   $n=2d_v...2d_v+d_l-1$   to
 * those on the line. Similarly,   $m=0...d_v-1$   refers to the dofs on the
 * middle vertex of the refined side (vertex one of child line zero, vertex
 * zero of child line one),   $m=d_v...d_v+d_l-1$   refers to the dofs on
 * child line zero,   $m=d_v+d_l...d_v+2d_l-1$   refers to the dofs on child
 * line one.  Please note that we do not need to reserve space for the dofs on
 * the end vertices of the refined lines, since these must be mapped
 * one-to-one to the appropriate dofs of the vertices of the unrefined line.
 ...
 *
 *
 * @note   The hanging node constraints described by these matrices are only relevant to the case where the same finite element space is used on neighboring (but differently refined) cells. The case that the finite element spaces on different sides of a face are different, i.e., the   $hp$   case (see   @ref hp   "hp-finite element support")
 ...
 *
 *  <h4>Interpolation matrices in three dimensions</h4> For the interface
 * constraints, the 3d case is similar to the 2d case. The numbering for the
 * indices   $n$   on the mother face is obvious and keeps to the usual
 * numbering of degrees of freedom on quadrilaterals. The numbering of the
 * degrees of freedom on the interior of the refined faces for the index   $m$
 * is as follows: let   $d_v$   and   $d_l$   be as above, and   $d_q$   be
 * the number of degrees of freedom per quadrilateral (and therefore per
 * face), then   $m=0...d_v-1$   denote the dofs on the vertex at the center,
 * $m=d_v...5d_v-1$   for the dofs on the vertices at the center of the
 * bounding lines of the quadrilateral,   $m=5d_v..5d_v+4*d_l-1$   are for the
 * degrees of freedom on the four lines connecting the center vertex to the
 * outer boundary of the mother face,   $m=5d_v+4*d_l...5d_v+4*d_l+8*d_l-1$
 * for the degrees of freedom on the small lines surrounding the quad, and
 * $m=5d_v+12*d_l...5d_v+12*d_l+4*d_q-1$   for the dofs on the four child
 * faces. Note the direction of the lines at the boundary of the quads, as
 * shown below. The order of the twelve lines and the four child faces can be
 * extracted from the following sketch, where the overall order of the
 * different dof groups is depicted:
 *
 * @verbatim
 *
 *
 ...
 * respectively, depending on the face. The three-dimensional case has a few
 * pitfalls available for derived classes that want to implement constraint
 * matrices. Consider the following case:
 *
 * @verbatim
 *
 *
 ...
 * different local numbers. For the constraint matrix this means the
 * following: if a degree of freedom on one edge of a face is constrained by
 * some other nodes on the same edge with some weights, then the weights have
 * to be exactly the same as those for constrained nodes on the three other
 * edges with respect to the corresponding nodes on these edges. If this isn't
 * the case, you will get into trouble with the AffineConstraints class that
 * is the primary consumer of the constraint information: while that class is
 * able to handle constraints that are entered more than once (as is necessary
 * for the case above), it insists that the weights are exactly the same.
 * Using this
 * scheme, child face degrees of freedom are constrained against parent face
 * degrees of freedom that contain those on the edges of the parent face; it
 * is possible that some of them are in turn constrained themselves, leading
 * to longer chains of constraints that the AffineConstraints class will
 * eventually have to sort out. (The constraints described above are used by
 * the   DoFTools::make_hanging_node_constraints()   function that constructs
 * an AffineConstraints object.) However, this is of no concern for the
 * FiniteElement and derived classes since they only act locally on one cell
 * and its immediate neighbor, and do not see the bigger picture. The   @ref
 * hp_paper   details how such chains are handled in practice.
 *
 *  <h4>Helper functions</h4> Construction of a finite element and computation
 * of the matrices described above is often a tedious task, in particular if
 * it has to be performed for several dimensions. Most of this work can be
 * avoided by using the intermediate classes already mentioned above (e.g.,
 * FE_Poly, FE_PolyTensor, etc). Other tasks can be automated by some of the
 * functions in namespace FETools. <h5>Computing the correct basis from a set
 * of linearly independent functions</h5>
 * First, it may already be difficult to compute the basis of shape functions for arbitrary order and dimension. On the other hand, if the   @ref GlossNodes   "node values"
 ...
 * Using this matrix allows the construction of the basis of shape functions in two steps.   <ol>
 ...
 * <li>  Compute the basis <i>v<sub>j</sub></i> of the finite element shape function space by applying <i>M<sup>-1</sup></i> to the basis <i>w<sub>j</sub></i>.   </ol>
 * The matrix <i>M</i> may be computed with   FETools::compute_node_matrix().   This function relies on the existence of #generalized_support_points and   FiniteElement::convert_generalized_support_point_values_to_dof_values()   (see the   @ref GlossGeneralizedSupport   "glossary entry on generalized support points"
 * for more information). With this, one can then use the following piece of
 * code in the constructor of a class derived from FiniteElement to compute
 * the   $M$   matrix:
 *
 * @code
 ...
 * #generalized_support_points are initialized before this! <h5>Computing
 * prolongation matrices</h5> Once you have shape functions, you can define
 * matrices that transfer data from one cell to its children or the other way
 * around. This is a common operation in multigrid, of course, but is also
 * used when interpolating the solution from one mesh to another after mesh
 * refinement, as well as in the definition of some error estimators. To
 * define the prolongation matrices, i.e., those matrices that describe the
 ...
 * by hand, or can call   FETools::compute_embedding_matrices(). In the latter
 * case, all that is required is the following piece of code:
 *
 * @code
 ...
 * a function of   $\mathbf x\in{\mathbb R}^\text{spacedim}$  , the finite
 * element field on the child is the same as on the parent.
 *
 *  <h5>Computing restriction matrices</h5> The opposite operation,
 * restricting a finite element function defined on the children to the parent
 * cell is typically implemented by interpolating the finite element function
 * on the children to the nodal values of the parent cell. In deal.II, the
 * restriction operation is implemented as a loop over the children of a cell
 * that each apply a matrix to the vector of unknowns on that child cell
 * (these matrices are stored in #restriction and are accessed by
 * get_restriction_matrix()). The operation that then needs to be implemented
 * turns out to be surprisingly difficult to describe, but is instructive to
 * describe because it also defines the meaning of the
 * #restriction_is_additive_flags array (accessed via the
 * restriction_is_additive() function). To give a concrete example, assume we
 * use a   $Q_1$   element in 1d, and that on each of the parent and child
 * cells degrees of freedom are (locally and globally) numbered as follows:
 *
 * @code
 * meshes:
 *
 ...
 * @endcode
 *  Then we want the restriction operation to take the value of the zeroth DoF on child 0 as the value of the zeroth DoF on the parent, and take the value of the first DoF on child 1 as the value of the first DoF on the parent. Ideally, we would like to write this follows @f[
 * U^\text{coarse}|_\text{parent}
 * = \sum_{\text{child}=0}^1 R_\text{child} U^\text{fine}|_\text{child}
 * @f] where   $U^\text{fine}|_\text{child=0}=(U^\text{fine}_0,U^\text{fine}_1)^T$   and   $U^\text{fine}|_\text{child=1}=(U^\text{fine}_1,U^\text{fine}_2)^T$  . Writing the requested operation like this would here be possible by choosing @f[
 * R_0 = \left(\begin{matrix}1 & 0 \\ 0 & 0\end{matrix}\right), \qquad\qquad
 * R_1 = \left(\begin{matrix}0 & 0 \\ 0 & 1\end{matrix}\right). @f] However,
 * this approach already fails if we go to a   $Q_2$   element with the
 ...
 * work because we have to add nonzero values to   $U^\text{coarse}_2$
 * twice, once for each child.
 *  Consequently, restriction is typically implemented as a <i>concatenation</i> operation. I.e., we first compute the individual restrictions from each child, @f[
 * \tilde U^\text{coarse}_\text{child}
 * = R_\text{child} U^\text{fine}|_\text{child},
 * @f] and then compute the values of   $U^\text{coarse}|_\text{parent}$   with the following code:
 *
 * @code
 ...
 * @f] and the compatibility condition is the   $R_{0,21}=R_{1,20}$   because they both indicate that   $U^\text{coarse}|_\text{parent,2}$   should be set to one times   $U^\text{fine}|_\text{child=0,1}$   and   $U^\text{fine}|_\text{child=1,0}$  .
 ...
 *
 *
 *  <h5>Computing #interface_constraints</h5> Constraint matrices can be
 * computed semi-automatically using
...
public:
   ...
   * wish to store.     The class is used as follows: Whenever an FEValues (or
   * FEFaceValues or   FESubfaceValues) object is initialized, it requests
   * that the finite   element it is associated with creates an object of a
   * class derived from   the current one here. This is done via each derived
   * class's     FiniteElement::get_data()   function. This object is then
   * passed to the     FiniteElement::fill_fe_values(),
   * FiniteElement::fill_fe_face_values(),     and
   * FiniteElement::fill_fe_subface_values()   functions as a constant
   ...
   * at the beginning and reused later on.     Because only derived classes
   * can know what they can pre-compute, each   derived class that wants to
   * store information computed once at the   beginning, needs to derive its
   * own InternalData class from this class,   and return an object of the
   * derived type through its get_data() function.
   *
   */
  ...
  public:
    /**
     * Constructor. Sets update_flags to   @p update_default   and   @p
     * first_cell       to   @p true.
     *
     */
    InternalDataBase();
...
public:
   ...
   * contain more than one entry. (For   these two elements, all entries will
   * in fact be set, but this would not   be the case if you couple a
   * FE_RaviartThomas and a FE_Nedelec together   into a FESystem.)       @pre
   * <code>restriction_is_additive_flags.size() == dofs_per_cell</code>  ,
   * or   <code>restriction_is_additive_flags.size() == 1</code>  . In the
   * latter   case, the array is simply interpreted as having size
   * <code>dofs_per_cell</code>   where each element has the same value as the
   * single element given.       @pre     <code>nonzero_components.size() ==
   * dofs_per_cell</code>  , or     <code>nonzero_components.size() ==
   * 1</code>  . In the latter case, the   array is simply interpreted as
   * having size   <code>dofs_per_cell</code>     where each element equals
   * the component mask provided in the single   element given.
   *
   */
  ...
  /**
   * Creates information for creating a FESystem with this class as   base
   * element and with multiplicity   @p multiplicity.   In particular,   the
   * return type of this function can be used in the constructor   for a
   * FESystem object.   This function calls clone() and hence creates a copy
  ...
  /**
   * A sort of virtual copy constructor, this function returns a copy of   the
   * finite element object. Derived classes need to override the function
   * here in this base class and return an object of the same type as the
   * derived class.     Some places in the library, for   example the
   * constructors of FESystem as well as the   hp::FECollection     class,
   * need to make copies of finite elements without knowing their exact
   ...
   * for a cubic element in 2d.     Systems of elements have their own naming
   * convention, see the FESystem   class.
   *
   */
  virtual std::string
  get_name() const = 0;
   ...
   * doesn't offer a   <code>dofs_per_cell</code>   member variable: one first
   * has   to select which finite element to work on, which is done using the
   * operator[]. Fortunately,   <code>cell-@>active_fe_index()</code>   also
   * works   for non-hp-classes and simply returns zero in that case. The
   * present   operator[] accepts this zero argument, by returning the finite
   * element   with index zero within its collection (that, of course,
   * consists only of   the present finite element anyway).       @deprecated
   * With   DoFHandler::get_fe(int)   and the deprecation of the
  ...
  /**
   * Return the value of the   @p ith   shape function at the point   @p p.
   * @p p   is   a point on the reference element. If the finite element is
   ...
   * use the   shape_value_component() function.     Implementations of this
   * function should throw an exception of type   ExcUnitShapeValuesDoNotExist
   * if the shape functions of the FiniteElement   under consideration depend
   * on the shape of the cell in real space, i.e.,   if the shape functions
   * are not defined by mapping from the reference   cell. Some non-conforming
   * elements are defined this way, as is the   FE_DGPNonparametric class, to
   * name just one example.     The default implementation of this virtual
   * function does exactly this,   i.e., it simply throws an exception of type
  ...
  /**
   * Return the gradient of the   @p ith   shape function at the point   @p p.
   * @p p     is a point on the reference element, and likewise the gradient
   * is the   gradient on the unit cell with respect to unit cell coordinates.
   * If the   finite element is vector-valued, then return the value of the
   ...
   * to with the   term non-primitive), then derived classes implementing this
   * function   should throw an exception of type
   * ExcShapeFunctionNotPrimitive. In that   case, use the
   * shape_grad_component() function.     Implementations of this function
   * should throw an exception of type   ExcUnitShapeValuesDoNotExist if the
   * shape functions of the FiniteElement   under consideration depend on the
   * shape of the cell in real space, i.e.,   if the shape functions are not
   * defined by mapping from the reference   cell. Some non-conforming
   * elements are defined this way, as is the   FE_DGPNonparametric class, to
   * name just one example.     The default implementation of this virtual
   * function does exactly this,   i.e., it simply throws an exception of type
   * ExcUnitShapeValuesDoNotExist.
   ...
   * shape function has more than one non-zero vector component. In that case,
   * this function should return the gradient of the   @p component-th
  ...
  /**
   * Return the tensor of second derivatives of the   @p ith   shape function
   * at   point   @p p   on the unit cell. The derivatives are derivatives on
   * the unit   cell with respect to unit cell coordinates. If the finite
   * element is   vector-valued, then return the value of the only non-zero
   * component of   the vector value of this shape function. If the shape
   * function has more   than one non-zero component (which we refer to with
   * the term non-   primitive), then derived classes implementing this
   * function should throw   an exception of type
   * ExcShapeFunctionNotPrimitive. In that case, use the
   * shape_grad_grad_component() function.     Implementations of this
   * function should throw an exception of type   ExcUnitShapeValuesDoNotExist
   * if the shape functions of the FiniteElement   under consideration depend
   * on the shape of the cell in real space, i.e.,   if the shape functions
   * are not defined by mapping from the reference   cell. Some non-conforming
  ...
  /**
   * Return the tensor of third derivatives of the   @p ith   shape function
   * at   point   @p p   on the unit cell. The derivatives are derivatives on
   * the unit   cell with respect to unit cell coordinates. If the finite
   * element is   vector-valued, then return the value of the only non-zero
   * component of   the vector value of this shape function. If the shape
   ...
   * function should throw   an exception of type
   * ExcShapeFunctionNotPrimitive. In that case, use the
   * shape_3rd_derivative_component() function.     Implementations of this
   * function should throw an exception of type   ExcUnitShapeValuesDoNotExist
   * if the shape functions of the FiniteElement   under consideration depend
   * on the shape of the cell in real space, i.e.,   if the shape functions
   * are not defined by mapping from the reference   cell. Some non-conforming
   ...
   * that case, this function should return the gradient of the   @p
   * component-     th vector component of the   @p ith   shape function at
   * point   @p p.
   *
   */
  virtual Tensor<3, dim>
  ...
  /**
   * Return the tensor of fourth derivatives of the   @p ith   shape function
   * at   point   @p p   on the unit cell. The derivatives are derivatives on
   * the unit   cell with respect to unit cell coordinates. If the finite
   * element is   vector-valued, then return the value of the only non-zero
   * component of   the vector value of this shape function. If the shape
   ...
   * function should throw   an exception of type
   * ExcShapeFunctionNotPrimitive. In that case, use the
   * shape_4th_derivative_component() function.     Implementations of this
   * function should throw an exception of type   ExcUnitShapeValuesDoNotExist
   * if the shape functions of the FiniteElement   under consideration depend
   * on the shape of the cell in real space, i.e.,   if the shape functions
   * are not defined by mapping from the reference   cell. Some non-conforming
   ...
   * The   function is typically used to determine whether some matrix
   * elements   resulting from face integrals can be assumed to be zero and
   * may therefore   be omitted from integration.     A default implementation
   * is provided in this base class which always   returns   @p true.   This
   ...
   * the given   @p child   (as obtained by the given   @p refinement_case)
   * to the   parent cell. The interpretation of the returned matrix depends
   * on what   restriction_is_additive() returns for each shape function.
   ...
   * FiniteElement::ExcProjectionVoid.   You can check whether this would
   * happen   by first calling the restriction_is_implemented() or the
  ...
  /**
   * Prolongation/embedding matrix between grids.     The identity operator
   * from a coarse grid space into a fine grid space   (where both spaces are
   * identified as functions defined on the parent and   child cells) is
   * associated with a matrix   @p P   that maps the corresponding
   ...
   * here.     The matrix   @p P   is the concatenation, not the sum of the
   * cell matrices   @p     P_i. That is, if the same non-zero entry
   * <tt>j,k</tt> exists in two   different child matrices   @p P_i,   the
   * value should be the same in both   matrices and it is copied into the
   * matrix   @p P   only once.     Row and column indices are related to fine
   * grid and coarse grid spaces,   respectively, consistent with the
   * definition of the associated operator.     These matrices are used by
   * routines assembling the prolongation matrix   for multi-level methods.
   * Upon assembling the transfer matrix between   cells using this matrix
   * array, zero elements in the prolongation matrix   are discarded and will
   * not fill up the transfer matrix.     If prolongation matrices are not
   * implemented in the derived finite   element class, this function aborts
   * with an exception of type     FiniteElement::ExcEmbeddingVoid.   You can
   * check whether this would happen   by first calling the
   * prolongation_is_implemented() or the
   ...
   * isotropic_prolongation_is_implemented function instead.     This function
   * is mostly here in order to allow us to write more efficient   test
   * programs which we run on all kinds of weird elements, and for which   we
   * simply need to exclude certain tests in case something is not
   ...
   * for isotropic refinement only, use the
   * isotropic_restriction_is_implemented() function instead.     This
   * function is mostly here in order to allow us to write more efficient
   * test programs which we run on all kinds of weird elements, and for which
   * we simply need to exclude certain tests in case something is not
   * implemented. It will in general probably not be a great help in
   * applications, since there is not much one can do if one needs these
   ...
   * isotropic children. The return value also indicates whether a call to the
   * get_restriction_matrix() function will generate an error or not.     This
   * function is mostly here in order to allow us to write more efficient
   * test programs which we run on all kinds of weird elements, and for which
   * we simply need to exclude certain tests in case something is not
   * implemented. It will in general probably not be a great help in
   ...
   * information.     The index must be between zero and the number of shape
   ...
   * at the interface between a refined and an unrefined cell.     Some finite
   * elements do not (yet) implement hanging node constraints. If   this is
   * the case, then this function will generate an exception, since no
   ...
   * will generate an error or not.     This function is mostly here in order
   * to allow us to write more efficient   test programs which we run on all
   * kinds of weird elements, and for which   we simply need to exclude
   * certain tests in case hanging node constraints   are not implemented. It
   * will in general probably not be a great help in   applications, since
   * there is not much one can do if one needs hanging   node constraints and
   * they are not implemented. This function could be   used to check whether
   * a call to <tt>constraints()</tt> will succeed;   however, one then still
   * needs to cope with the lack of information this   just expresses.
   *
   */
  bool
   ...
   * error or not.     Currently the main purpose of this function is to allow
   * the   make_hanging_node_constraints method to decide whether the new
   ...
   * probably go away.     Derived classes should implement this function
   * accordingly. The default   assumption is that a finite element does not
   * provide hp-capable face   interpolation, and the default implementation
   * therefore returns   @p false.
   *
   */
  virtual bool
   ...
   * <tt>source.#dofs_per_cell</tt>.     Derived elements will have to
   * implement this function. They may only   provide interpolation matrices
   * for certain source finite elements, for   example those from the same
   * family. If they don't implement interpolation   from a given element,
   * then they must throw an exception of type
  ...
  /**
   * Comparison operator.     The implementation in the current class checks
   * for equality of the   following pieces of information between the current
   * object and the one   given as argument, in this order:
   *
   *
   *
   *
   ...
   *
   *
   *  - constraint matrices.     This covers most cases where elements can differ, but there are   cases of derived elements that are different and for which the   current function still returns   @p true.   For these cases, derived   classes should overload this function.
   ...
   * within this component is equal to the overall index.     This is the
   * opposite operation from the system_to_component_index()   function.
   *
   */
  unsigned int
   ...
   * 0..#dofs_per_face.     You will rarely need this function in application
   * programs, since almost   all application codes only need to deal with
   * cell indices, not face   indices. The function is mainly there for use
   * inside the library.
   *
   */
  std::pair<unsigned int, unsigned int>
   ...
   * shape functions. Given a local dof   @p index   on a quad, return the
   * local   index, if the face has non-standard face_orientation, face_flip
   * or   face_rotation. In 2D and 1D there is no need for permutation and
   ...
   * index of the same degree of freedom on the cell.     To explain the
   * concept, consider the case where we would like to know   whether a degree
   * of freedom on a face, for example as part of an FESystem   element, is
   * primitive. Unfortunately, the is_primitive() function in the
   ...
   * This function does that.     Code implementing this would then look like
   * this:
   * @code
   ...
   * @endcode
   * The function
   * takes additional arguments that account for the fact that   actual faces
   * can be in their standard ordering with respect to the cell   under
   * consideration, or can be flipped, oriented, etc.       @param
   * face_dof_index The index of the degree of freedom on a face. This   index
   * must be between zero and dofs_per_face.     @param   face The number of
   * the face this degree of freedom lives on. This   number must be between
   * zero and   GeometryInfo::faces_per_cell.       @param   face_orientation
   * One part of the description of the orientation of   the face. See
   * @ref GlossFaceOrientation  .     @param   face_flip One part of the
   * description of the orientation of the   face. See     @ref
   * GlossFaceOrientation  .     @param   face_rotation One part of the
   * description of the orientation of   the face. See     @ref
   * GlossFaceOrientation  .     @return   The index of this degree of freedom
   * within the set of degrees of   freedom on the entire cell. The returned
   * value will be between zero and   dofs_per_cell.
   ...
   * For this, consider the situation of a   $Q_3$   element in 2d. If
   * face_flip   is true, then we need to consider the two degrees of freedom
   * on the edge   in reverse order. On the other hand, if the element were a
   * $Q_1^2$  , then   because the two degrees of freedom on this edge belong
   * to different   vector components, they should not be considered in
   * reverse order. What   all of this shows is that the function can't work
   * if there are more than   one degree of freedom per line or quad, and that
   * in these cases the   function will throw an exception pointing out that
   * this functionality   will need to be provided by a derived class that
   * knows what degrees of   freedom actually represent.
   *
   */
   ...
   * functions. Given a local dof   @p index   on a line, return the local
   * index,   if the line has non-standard line_orientation. In 2D and 1D
   * there is no   need for permutation, so the given index is simply
   * returned.
   *
   */
  unsigned int
  ...
  /**
   * Return in
   * which of the vector components of this finite element the   @p     ith
   * shape function is non-zero. The length of the returned array is equal
   * to the number of vector components of this element.     For most finite
   * element spaces, the result of this function will be a   vector with
   * exactly one element being   @p true,   since for most spaces the
   ...
   * system_to_component_index() returns.     Only for those spaces that
   * couple the components, for example to make a   shape function divergence
   * free, will there be more than one   @p true     entry.  Elements for
   * which this is true are called non-primitive (see     @ref GlossPrimitive
   * ).
   *
   */
  const ComponentMask &
   ...
   * non-   zero. This value equals the number of entries equal to   @p true
   * in the   result of the get_nonzero_components() function.     For most
   * finite element spaces, the result will be equal to one. It is   not equal
   * to one only for those ansatz spaces for which vector-valued   shape
   * functions couple the individual components, for example in order to
   ...
   * then this is always the case.     Since this is an extremely common
   * operation, the result is cached and   returned by this function.
   *
   */
  bool
  is_primitive() const;
  ...
  /**
   * Return whether the   @p ith   shape function is primitive in the sense
   * that   the shape function is non-zero in only one vector component. Non-
   ...
   * coupled.     The result of the function is   @p true   if and only if the
   * result of   <tt>n_nonzero_components(i)</tt> is equal to one.
   *
   */
  bool
   ...
   * vector valued finite elements, the number of   components needs not
   * coincide with the number of base elements, since   they may be reused.
   * For example, if you create a FESystem with three   identical finite
   * element classes by using the constructor that takes one   finite element
   * and a multiplicity, then the number of base elements is   still one,
   * although the number of components of the finite element is   equal to the
   * multiplicity.
   *
   */
  unsigned int
   ...
   * components   selected by the given ComponentMask   @p mask.       For an
   * arbitrarily nested FESystem, this function returns the inner-most
   * FiniteElement that matches the given mask. The method fails if the   @p
   * mask     does not exactly match one of the contained finite elements. It
   * is most   useful if the current object is an FESystem, as the return
   * value can   only be   @p this   in all other cases.     Note that the
   * returned object can be an FESystem if the   mask matches it but not any
   * of the contained objects.     Let us illustrate the function with the an
   * FESystem   @p fe   with 7 components:
   * @code
  ...
  /**
   * Return a reference to a contained finite element that matches the
   * components     @p n_selected_components   components starting at
   * component with index     @p first_component.       See the other
   * get_sub_fe() function above for more details.
   *
   */
  virtual const FiniteElement<dim, spacedim> &
  ...
  /**
   * Return for shape function   @p index   the base element it belongs to,
   * the   number of the copy of this base element (which is between zero and
   * the   multiplicity of this element), and the index of this shape function
   * within this base element.     If the element is not composed of others,
   * then base and instance are   always zero, and the index is equal to the
   * number of the shape function.   If the element is composed of single
   * instances of other elements (i.e.   all with multiplicity one) all of
   * which are scalar, then base values and   dof indices within this element
   * are equal to the   #system_to_component_table. It differs only in case
   * the element is   composed of other elements and at least one of them is
   * vector-valued   itself.     See the class documentation above for an
   * example of how this function is   typically used.     This function
   * returns valid values also in the case of vector-valued   (i.e.
   ...
   * FESystem::base_element()   function.     If this is a scalar finite
   * element, then the return value is always equal   to a pair of zeros.
   *
   */
  std::pair<unsigned int, unsigned int>
  ...
  /**
   * Return a component mask with as many elements as this object has vector   components and of which exactly the one component is true that   corresponds to the given argument. See     @ref GlossComponentMask   "the glossary"
   * for more information.       @param   scalar An object that represents a
   * single scalar vector component   of this finite element.     @return   A
   * component mask that is false in all components except for the   one that
  ...
  /**
   * Return a component mask with as many elements as this object has vector   components and of which exactly the   <code>dim</code>   components are true   that correspond to the given argument. See     @ref GlossComponentMask   "the glossary"
   * for more information.       @param   vector An object that represents dim
   * vector components of this   finite element.     @return   A component
   * mask that is false in all components except for the   ones that
   * corresponds to the argument.
   *
   */
  ComponentMask
  ...
  /**
   * Return a component mask with as many elements as this object has vector   components and of which exactly the   <code>dim*(dim+1)/2</code>   components   are true that correspond to the given argument. See     @ref GlossComponentMask   "the glossary"
   * for more information.       @param   sym_tensor An object that represents
   * dim*(dim+1)/2 components of   this finite element that are jointly to be
   * interpreted as forming a   symmetric tensor.     @return   A component
   * mask that is false in all components except for the   ones that
   * corresponds to the argument.
   *
   */
  ...
  /**
   * Return a block mask with as many elements as this object has blocks and   of which exactly the one component is true that corresponds to the given   argument. See     @ref GlossBlockMask   "the glossary"
   ...
   * exception.       @param   scalar An object that represents a single
   * scalar vector component   of this finite element.     @return   A
   * component mask that is false in all components except for the   one that
   * corresponds to the argument.
   *
   */
  ...
  /**
   * Return a component mask with as many elements as this object has vector   components and of which exactly the   <code>dim</code>   components are true   that correspond to the given argument. See     @ref GlossBlockMask   "the glossary"
   * for more information.
   ...
   * full blocks and does not split blocks of this element.       @param
   * vector An object that represents dim vector components of this   finite
   * element.     @return   A component mask that is false in all components
  ...
  /**
   * Return a component mask with as many elements as this object has vector   components and of which exactly the   <code>dim*(dim+1)/2</code>   components   are true that correspond to the given argument. See     @ref GlossBlockMask   "the glossary"
   * for more information.
   * @note   The same caveat applies as to the version of the function above:
   * The extractor object passed as argument must be so that it corresponds to
   * full blocks and does not split blocks of this element.       @param
   * sym_tensor An object that represents dim*(dim+1)/2 components of   this
   * finite element that are jointly to be interpreted as forming a
   ...
   * produce an   exception.       @param   component_mask The mask that
   * selects individual components of the   finite element     @return   A
   * mask that selects those blocks corresponding to the selected   blocks of
   * the input argument.
   *
   */
  BlockMask
   ...
   * FE_Q_DG0.     In order to match the constant modes to the actual
   * components in the   element, the returned data structure also returns a
   * vector with as many   components as there are constant modes on the
   * element that contains the   component number.
   *
   */
   ...
   * case, the returned field is empty.     If the finite element defines
   * support points, then their number equals   the number of degrees of
   * freedom of the element.  The order of points in   the array matches that
   * returned by the <tt>cell->get_dof_indices</tt>   function.     See the
   * class documentation for details on support points.
   ...
   * @note   If this element implements support points, then it will return
   * one   such point per shape function. Since multiple shape functions may
   * be   defined at the same location, the support points returned here may
   * be   duplicated. An example would be an element of the kind
   ...
   * array.     The result may be false if an element is not defined by
   * interpolating   shape functions, for example by P-elements on
   * quadrilaterals. It will   usually only be true if the element constructs
   * its shape functions by the   requirement that they be one at a certain
   * point and zero at all the   points associated with the other shape
   * functions.     In composed elements (i.e. for the FESystem class), the
   * result will be   true if all the base elements have defined support
   * points. FE_Nothing   is a special case in FESystems, because it has 0
   * support points and   has_support_points() is false, but an FESystem
   * containing an FE_Nothing   among other elements will return true.
   *
   */
  bool
   ...
   * function. If it does not exist, raise an exception.     The default
   * implementation simply returns the respective element from the   array you
   * get from get_unit_support_points(), but derived elements may   overload
   * this function. In particular, note that the FESystem class   overloads it
   * so that it can return the support points of individual base   elements,
   * if not all the base elements define support points. In this   way, you
   * can still ask for certain support points, even if
   ...
   * hand, elements that define their degrees of freedom by, for example,
   * moments on faces, or as derivatives, don't have support points. In that
   * case, the returned field is empty     Note that elements that have
   * support points need not necessarily have   some on the faces, even if the
   * interpolation points are located   physically on a face. For example, the
   * discontinuous elements have   interpolation points on the vertices, and
   * for higher degree elements also   on the faces, but they are not defined
   * to be on faces since in that case   degrees of freedom from both sides of
   * a face (or from all adjacent   elements to a vertex) would be identified
   * with each other, which is not   what we would like to have). Logically,
   * these degrees of freedom are   therefore defined to belong to the cell,
   * rather than the face or vertex.   In that case, the returned element
   * would therefore have length zero.     If the finite element defines
   * support points, then their number equals   the number of degrees of
   * freedom on the face (#dofs_per_face). The order   of points in the array
   * matches that returned by the   <tt>cell->face(face)->get_dof_indices</tt>
   * function.     See the class documentation for details on support points.
   *
   */
   ...
   * yields a non-empty vector.     For more information, see the
   * documentation for the has_support_points()   function.
   *
   */
  bool
  ...
  /**
   * Return whether a finite element has defined generalized support   points. If the result is true, then a call to the   get_generalized_support_points() yields a non-empty vector.     See the     @ref GlossGeneralizedSupport   "glossary entry on generalized support points"
   * for more information.
   *
   */
  bool
   ...
   * with a vertex, line, quad or hex.     For instance, for continuous finite
   * elements this coincides with the   lowest dimensional object the support
   * point of the degree of freedom lies   on. To give an example, for   $Q_1$
   * elements in 3d, every degree of freedom   is defined by a shape function
   * that we get by interpolating using support   points that lie on the
   * vertices of the cell. The support of these points   of course extends to
   * all edges connected to this vertex, as well as the   adjacent faces and
   * the cell interior, but we say that logically the   degree of freedom is
   * associated with the vertex as this is the lowest-   dimensional object it
   * is associated with. Likewise, for   $Q_2$   elements in   3d, the degrees
   * of freedom with support points at edge midpoints would   yield a value of
   * GeometryPrimitive::line   from this function, whereas   those on the
   * centers of faces in 3d would return   GeometryPrimitive::quad.       To
   * make this more formal, the kind of object returned by this function
   ...
   * On the other hand, for discontinuous elements of type   $DGQ_2$  , a
   * degree   of freedom associated with an interpolation polynomial that has
   * its   support point physically located at a line bounding a cell, but is
   * nonzero only on one cell. Consequently, it is logically associated with
   * the interior of that cell (i.e., with a   GeometryPrimitive::quad   in 2d
   * and   a   GeometryPrimitive::hex   in 3d).       @param[in]
   * cell_dof_index The index of a shape function or degree of   freedom. This
   * index must be in the range   <code>[0,dofs_per_cell)</code>  .
  ...
  /**
   * Given the values of a function   $f(\mathbf x)$   at the (generalized)   support points of the reference cell, this function then computes what   the nodal values of the element are, i.e.,   $\Psi_i[f]$  , where   $\Psi_i$     are the node functionals of the element   (see also     @ref GlossNodes   "Node values or node functionals").
   * The values   $\Psi_i[f]$   are then the expansion coefficients   for the
   * shape functions of the finite element function that   <i>interpolates</i>
   * the given function   $f(x)$  , i.e.,     $ f_h(\mathbf x) = \sum_i
   * \Psi_i[f] \varphi_i(\mathbf x)
   * $   is the finite element interpolant of   $f$   with the current element.   The operation described here is used, for example, in the     FETools::compute_node_matrix()   function.     In more detail, let us assume that the generalized support points   (see     @ref GlossGeneralizedSupport   "this glossary entry"
   * ) of the current   element are   $\hat{\mathbf x}_i$   and that the node
   * functionals associated   with the current element are   $\Psi_i[\cdot]$
   * . Then, the fact that the   element is based on generalized support
   * points, implies that if we   apply   $\Psi_i$   to a (possibly
   * vector-valued) finite element function     $\varphi$  , the result must
   * have the form     $\Psi_i[\varphi] = f_i(\varphi(\hat{\mathbf x}_i))$
   *
   *  -  in other words,   the value of the node functional   $\Psi_i$   applied to   $\varphi$   <i>only</i>   depends on the <i>values of $\varphi$ at $\hat{\mathbf x}_i$</i> and not   on values anywhere else, or integrals of   $\varphi$  , or any other kind   of information.     The exact form of   $f_i$   depends on the element. For example, for scalar     @ref GlossLagrange   "Lagrange elements",   we have that in fact     $\Psi_i[\varphi] = \varphi(\hat{\mathbf x}_i)$  . If you combine multiple   scalar Lagrange elements via an FESystem object, then     $\Psi_i[\varphi] = \varphi(\hat{\mathbf x}_i)_{c(i)}$   where   $c(i)$     is the result of the   FiniteElement::system_to_component_index()     function's return value's first component. In these two cases,     $f_i$   is therefore simply the identity (in the scalar case) or a   function that selects a particular vector component of its argument.   On the other hand, for Raviart-Thomas elements, one would have that     $f_i(\mathbf y) = \mathbf y \cdot \mathbf n_i$   where   $\mathbf n_i$     is the normal vector of the face at which the shape function is   defined.     Given all of this, what this function does is the following: If you   input a list of values of a function   $\varphi$   at all generalized   support points (where each value is in fact a vector of values with   as many components as the element has), then this function returns   a vector of values obtained by applying the node functionals to   these values. In other words, if you pass in     $\{\varphi(\hat{\mathbf x}_i)\}_{i=0}^{N-1}$   then you   will get out a vector     $\{\Psi[\varphi]\}_{i=0}^{N-1}$   where   $N$   equals   @p dofs_per_cell.         @param[in]   support_point_values An array of size   @p dofs_per_cell       (which equals the number of points the get_generalized_support_points()     function will return) where each element is a vector with as many entries     as the element has vector components. This array should contain     the values of a function at the generalized support points of the     current element.     @param[out]   nodal_values An array of size   @p dofs_per_cell   that contains     the node functionals of the element applied to the given function.
   ...
   * points. Elements that do not have generalized support points
   *
   ...
   * implement these matrices.     Exception
   * @ingroup Exceptions
   *
   */
...
protected:
   ...
   * more detail on the actual vector sizes.       @param
   * isotropic_restriction_only only the restriction matrices required   for
   * isotropic refinement are reinited to the right size.     @param
   * isotropic_prolongation_only only the prolongation matrices   required for
   * isotropic refinement are reinited to the right size.
   *
   */
  void
   ...
   * changed by derived classes implementing them.     Note, that
   * <code>restriction[refinement_case-1][child]</code>   includes   the
   ...
   * which can be changed by derived classes implementing them.     Note, that
   * <code>prolongation[refinement_case-1][child]</code>   includes   the
   * prolongation matrix of child   <code>child</code>   for the
   ...
   * once.     For further details see the general description of the derived
   * class.     This field is obviously useless in one dimension and has there
   * a zero   size.
   *
   */
   ...
   * in some contents.     Finite elements that allow some kind of
   * interpolation operation usually   have support points. On the other hand,
   * elements that define their   degrees of freedom by, for example, moments
   * on faces, or as derivatives,   don't have support points. In that case,
   ...
   * (quads) have to be permuted in order to be combined with the correct
   * shape functions. Given a local dof   @p index   on a quad, return the
   * shift   in the local index, if the face has non-standard
   * face_orientation, i.e.     <code>old_index + shift = new_index</code>  .
   * In 2D and 1D there is no need   for permutation so the vector is empty.
   * In 3D it has the size of   <code> #dofs_per_quad 8 </code>  , where 8 is
   * the number of orientations, a face   can be in (all combinations of the
   * three bool flags face_orientation,   face_flip and face_rotation).
   * The constructor of this class fills this table with zeros, i.e.,   no
   * permutation at all. Derived finite element classes have to   fill this
   ...
   * For lines with non-standard line_orientation in 3D, the dofs on lines
   * have to be permuted in order to be combined with the correct shape
   * functions. Given a local dof   @p index   on a line, return the shift in
   * the   local index, if the line has non-standard line_orientation, i.e.
   * <code>old_index + shift = new_index</code>  . In 2D and 1D there is no
   * need   for permutation so the vector is empty. In 3D it has the size of
   * #dofs_per_line.     The constructor of this class fills this table with
   * zeros, i.e.,   no permutation at all. Derived finite element classes have
   * to   fill this vector with the correct values.
   *
   */
   ...
   * overwrite the information if necessary.     By component, we mean the
   * vector component, not the base element. The   information thus makes only
   * sense if a shape function is non-zero in only   one component.
   *
   */
   ...
   * is vector-valued itself.     This array has valid values also in the case
   * of vector-valued (i.e. non-   primitive) shape functions, in contrast to
   * the   #system_to_component_table.
   *
   */
  ...
  /**
   * The base element establishing a component.     For each component number
   * <tt>c</tt>, the entries have the following   meaning:   <dl>
   * <dt><tt>table[c].first.first</tt></dt>   <dd>  Number of the   base
   * element for <tt>c</tt>. This is the index you can pass to
   * base_element().  </dd>   <dt><tt>table[c].first.second</tt></dt>     <dd>
   * Component within the base element for <tt>c</tt>. This value is   between
   * 0 and the n_components() of this base element.  </dd>
   * <dt><tt>table[c].second</tt></dt>   <dd>  Index of the multiple of the
   * base   element that contains <tt>c</tt>. This value is between 0 and the
   * element_multiplicity() of this base element.  </dd>     </dl>       This
   * variable is set to the correct size by the constructor of this   class,
   * but needs to be initialized by derived classes, unless its size is   one
   * and the only entry is a zero, which is the case for scalar elements.   In
   * that case, the initialization by the base class is sufficient.
   * @note   This table is filled by
   ...
   * which component each of these shape functions is non-zero.     For
   * primitive elements, there is only one non-zero component.
   *
   */
  const std::vector<ComponentMask> nonzero_components;
   ...
   * dependent size of these matrices each time.     Note that some elements
   * do not implement the interface constraints for   certain polynomial
   * degrees. In this case, this function still returns the   size these
   * matrices should have when implemented, but the actual matrices   are
   * empty.
   *
   */
  TableIndices<2>
  ...
  /**
   * Given a
   * set of update flags, compute which other quantities <i>also</i>   need to
   * be computed in order to satisfy the request by the given flags.   Then
   * return the combination of the original set of flags and those just
   * computed.     As an example, if   @p update_flags   contains
   * update_gradients a finite   element class will typically require the
   * computation of the inverse of   the Jacobian matrix in order to rotate
   * the gradient of shape functions on   the reference cell to the real cell.
   * It would then return not just   update_gradients, but also
   * update_covariant_transformation, the flag that   makes the mapping class
   * produce the inverse of the Jacobian matrix.     An extensive discussion
   * of the interaction between this function and   FEValues can be found in
   * the     @ref FE_vs_Mapping_vs_FEValues     documentation module.
   * @see   UpdateFlags
   *
   */
  virtual UpdateFlags
  ...
  /**
   * Create an
   * internal data object and return a pointer to it of which the   caller of
   * this function then assumes ownership. This object will then be   passed
   * to the   FiniteElement::fill_fe_values()   every time the finite
   ...
   * values to a concrete cell).     This function is the first one called in
   * the process of initializing a   FEValues object for a given mapping and
   * finite element object. The   returned object will later be passed to
   * FiniteElement::fill_fe_values()     for a concrete cell, which will
   * itself place its output into an object of   type
   * internal::FEValuesImplementation::FiniteElementRelatedData.   Since
   * there may be data that can already be computed in its <i>final</i> form
   * on   the reference cell, this function also receives a reference to the
   * internal::FEValuesImplementation::FiniteElementRelatedData   object as
   * its   last argument. This output argument is guaranteed to always be the
   * same one   when used with the InternalDataBase object returned by this
   * function. In   other words, the subdivision of scratch data and final
   * data in the returned   object and the   @p output_data   object is as
   * follows: If data can be pre-   computed on the reference cell in the
   * exact form in which it will later   be needed on a concrete cell, then
   * this function should already emplace   it in the   @p output_data
   * object. An example are the values of shape   functions at quadrature
   * points for the usual Lagrange elements which on a   concrete cell are
   * identical to the ones on the reference cell. On the   other hand, if some
   * data can be pre-computed to make computations on a   concrete cell
   * <i>cheaper</i>, then it should be put into the returned   object for
   * later re-use in a derive class's implementation of
   ...
   * the object returned.     An extensive discussion of the interaction
   * between this function and   FEValues can be found in the     @ref
   * FE_vs_Mapping_vs_FEValues     documentation module. See also the
   * documentation of the InternalDataBase   class.       @param[in]
   * update_flags A set of UpdateFlags values that describe what   kind of
   * information the FEValues object requests the finite element to   compute.
   * This set of flags may also include information that the finite   element
   * can not compute, e.g., flags that pertain to data produced by the
   ...
   * compute     @param[in]   mapping A reference to the mapping used for
   * computing values   and derivatives of shape functions.     @param[in]
   * quadrature A reference to the object that describes where the   shape
   * functions should be evaluated.     @param[out]   output_data A reference
   * to the object that FEValues will use   in conjunction with the object
   * returned here and where an implementation   of
   * FiniteElement::fill_fe_values()   will place the requested information.
   ...
   * FiniteElement::fill_fe_face_values().   See the documentation of
   * get_data()   for more information.     The default implementation of this
   * function converts the face quadrature   into a cell quadrature with
   * appropriate quadrature point locations, and   with that calls the
   * get_data() function above that has to be implemented   in derived
   * classes.       @param[in]   update_flags A set of UpdateFlags values that
   * describe what   kind of information the FEValues object requests the
   * finite element to   compute. This set of flags may also include
   * information that the finite   element can not compute, e.g., flags that
   * pertain to data produced by the   mapping. An implementation of this
   * function needs to set up all data   fields in the returned object that
   * are necessary to produce the finite-   element related data specified by
   * these flags, and may already pre-   compute part of this information as
   * discussed above. Elements may want to   store these update flags (or a
   * subset of these flags) in     InternalDataBase::update_each   so they
   * know at the time when     FiniteElement::fill_fe_face_values()   is
   * called what they are supposed to   compute     @param[in]   mapping A
   * reference to the mapping used for computing values   and derivatives of
   * shape functions.     @param[in]   quadrature A reference to the object
   * that describes where the   shape functions should be evaluated.
   * @param[out]   output_data A reference to the object that FEValues will
   * use   in conjunction with the object returned here and where an
   * implementation   of   FiniteElement::fill_fe_face_values()   will place
   * the requested   information. This allows the current function to already
   * pre-compute   pieces of information that can be computed on the reference
   * cell, as   discussed above. FEValues guarantees that this output object
   * and the   object returned by the current function will always be used
   * together.     @return   A pointer to an object of a type derived from
   * InternalDataBase   and that derived classes can use to store scratch data
   * that can be pre-   computed, or for scratch arrays that then only need to
   * be allocated once.   The calling site assumes ownership of this object
   * and will delete it when   it is no longer necessary.
   *
   */
  virtual std::unique_ptr<InternalDataBase>
   ...
   * discussed above. Elements may want to   store these update flags (or a
   * subset of these flags) in     InternalDataBase::update_each   so they
   * know at the time when     FiniteElement::fill_fe_subface_values()   is
   * called what they are supposed   to compute     @param[in]   mapping A
   * reference to the mapping used for computing values   and derivatives of
   * shape functions.     @param[in]   quadrature A reference to the object
   * that describes where the   shape functions should be evaluated.
   * @param[out]   output_data A reference to the object that FEValues will
   * use   in conjunction with the object returned here and where an
   * implementation   of   FiniteElement::fill_fe_subface_values()   will
   * place the requested   information. This allows the current function to
   * already pre-compute   pieces of information that can be computed on the
   * reference cell, as   discussed above. FEValues guarantees that this
   * output object and the   object returned by the current function will
   * always be used together.     @return   A pointer to an object of a type
   * derived from InternalDataBase   and that derived classes can use to store
   * scratch data that can be pre-   computed, or for scratch arrays that then
   * only need to be allocated once.   The calling site assumes ownership of
   * this object and will delete it when   it is no longer necessary.
   *
   */
  virtual std::unique_ptr<InternalDataBase>
  get_subface_data(
  ...
  /**
   * Compute information
   * about the shape functions on the cell denoted by the   first argument.
   * Derived classes will have to implement this function   based on the kind
   * of element they represent. It is called by     FEValues::reinit().
   ...
   * from the   @p mapping_data   object that has been filled for the current
   * cell   before this function is called, or by calling the member functions
   * of a   Mapping object with the   @p mapping_internal   object that also
   * corresponds   to the current cell.     The information computed by this
   * function is used to fill the various   member variables of the output
   * argument of this function. Which of the   member variables of that
   * structure should be filled is determined by the   update flags stored in
   * the   FiniteElement::InternalDataBase::update_each     field of the
   * object passed to this function. These flags are typically   set by
   * FiniteElement::get_data(),     FiniteElement::get_face_date()   and
   * FiniteElement::get_subface_data()   (or, more specifically,
   * implementations   of these functions in derived classes).     An
   * extensive discussion of the interaction between this function and
   * FEValues can be found in the     @ref FE_vs_Mapping_vs_FEValues
   * documentation module.       @param[in]   cell The cell of the
   * triangulation for which this function is   to compute a mapping from the
   * reference cell to.     @param[in]   cell_similarity Whether or not the
   * cell given as first   argument is simply a translation, rotation, etc of
   * the cell for which   this function was called the most recent time. This
   * information is   computed simply by matching the vertices (as stored by
   * the Triangulation)   between the previous and the current cell. The value
   * passed here may be   modified by implementations of this function and
   * should then be returned   (see the discussion of the return value of this
   * function).     @param[in]   quadrature A reference to the quadrature
   * formula in use for   the current evaluation. This quadrature object is
   * the same as the one   used when creating the   @p internal_data   object.
   * The current object is   then responsible for evaluating shape functions
   * at the mapped locations   of the quadrature points represented by this
   * object.     @param[in]   mapping A reference to the mapping object used
   * to map from the   reference cell to the current cell. This object was
   * used to compute the   information in the   @p mapping_data   object
   * before the current function was   called. It is also the mapping object
   * that created the   @p     mapping_internal object via
   * Mapping::get_data().   You will need the   reference to this mapping
   * object most often to call   Mapping::transform()     to transform
   * gradients and higher derivatives from the reference to the   current
   * cell.     @param[in]   mapping_internal An object specific to the mapping
   * object.   What the mapping chooses to store in there is of no relevance
   * to the   current function, but you may have to pass a reference to this
   * object to   certain functions of the Mapping class (e.g.,
   * Mapping::transform())   if   you need to call them from the current
   * function.     @param[in]   mapping_data The output object into which the
   ...
   * as other information that   FEValues::reinit()   requested from the
   * mapping.     @param[in]   fe_internal A reference to an object previously
   * created by   get_data() and that may be used to store information the
   * mapping can   compute once on the reference cell. See the documentation
   * of the     FiniteElement::InternalDataBase   class for an extensive
   * description of the   purpose of these objects.     @param[out]
   * output_data A reference to an object whose member variables   should be
   * computed. Not all of the members of this argument need to be   filled;
   * which ones need to be filled is determined by the update flags   stored
   * inside the   @p fe_internal   object.
   * @note   FEValues ensures that this function is always called with the
   * same   pair of   @p fe_internal   and   @p output_data   objects. In
   * other words, if an   implementation of this function knows that it has
   * written a piece of data   into the output argument in a previous call,
   * then there is no need to   copy it there again in a later call if the
   * implementation knows that this   is the same value.
   *
   */
  virtual void
  fill_fe_values(
   ...
   * but   for faces of cells. See there for an extensive discussion of its
   * purpose.   It is called by   FEFaceValues::reinit().         @param[in]
   * cell The cell of the triangulation for which this function is   to
   * compute a mapping from the reference cell to.     @param[in]   face_no
   * The number of the face we are currently considering,   indexed among the
   * faces of the cell specified by the previous argument.     @param[in]
   * quadrature A reference to the quadrature formula in use for   the current
   * evaluation. This quadrature object is the same as the one   used when
   * creating the   @p internal_data   object. The current object is   then
   * responsible for evaluating shape functions at the mapped locations   of
   * the quadrature points represented by this object.     @param[in]
   * mapping A reference to the mapping object used to map from the
   * reference cell to the current cell. This object was used to compute the
   * information in the   @p mapping_data   object before the current function
   * was   called. It is also the mapping object that created the   @p
   * mapping_internal object via   Mapping::get_data().   You will need the
   * reference to this mapping object most often to call
   * Mapping::transform()     to transform gradients and higher derivatives
   * from the reference to the   current cell.     @param[in]
   * mapping_internal An object specific to the mapping object.   What the
   * mapping chooses to store in there is of no relevance to the   current
   * function, but you may have to pass a reference to this object to
   * certain functions of the Mapping class (e.g.,   Mapping::transform())
   * if   you need to call them from the current function.     @param[in]
   * mapping_data The output object into which the
   * Mapping::fill_fe_values()   function wrote the mapping information
   * corresponding to the current cell. This includes, for example, Jacobians
   * of the mapping that may be of relevance to the current function, as well
   * as other information that   FEValues::reinit()   requested from the
   ...
   * is   to compute a mapping from the reference cell to.     @param[in]
   * face_no The number of the face we are currently considering,   indexed
   * among the faces of the cell specified by the previous argument.
   ...
   * children of the face specified by the previous argument.     @param[in]
   * quadrature A reference to the quadrature formula in use for   the current
   * evaluation. This quadrature object is the same as the one   used when
   * creating the   @p internal_data   object. The current object is   then
   * responsible for evaluating shape functions at the mapped locations   of
