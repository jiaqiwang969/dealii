...
/**
 * A class that implements a polynomial mapping   $Q_p$   of degree   $p$   on
 * cells at the boundary of the domain (or, if requested in the constructor,
 * for all cells) and linear mappings for interior cells. The class is in fact
 * poorly named since (unless explicitly specified during the construction of
 * the object, see below), it does not actually use mappings of degree   $p$
 * <i>everywhere</i>, but only on cells at the boundary. This is in contrast
 * to the MappingQGeneric class which indeed does use a polynomial mapping
 * $Q_p$   of degree   $p$   everywhere. The point of the current class is
 * that in many situations, curved domains are only provided with information
 * about how exactly edges at the boundary are shaped, but we do not know
 * anything about internal edges. Thus, in the absence of other information,
 * we can only assume that internal edges are straight lines, and in that case
 * internal cells may as well be treated is bilinear quadrilaterals or
 * trilinear hexahedra. (An example of how such meshes look is shown in
 * step-1   already, but it is also discussed in the "Results" section of
 * step-6  .) Because bi-/trilinear mappings are significantly cheaper to
 * compute than higher order mappings, it is advantageous in such situations
 * to use the higher order mapping only on cells at the boundary of the
 * domain. This class implements exactly this behavior. There are a number of
 ...
 * <h4>Behavior along curved boundaries and with different manifolds</h4> For
 * the behavior of the mapping and convergence rates in case of mixing
...
public:
   ...
   * the   polynomials that are used to map cells boundary.     The second
   * argument determines whether the higher order mapping should   also be
   * used on interior cells. If its value is   <code>false</code>   (the
   ...
   * MappingQEulerian class is one such case.     The value of   @p
   * use_mapping_q_on_all_cells   is ignored if   @p dim   is not   equal to
   * @p spacedim,   i.e., if we are considering meshes on surfaces   embedded
  ...
  /**
   * Transform the point   @p p   on the unit cell to the point   @p p_real
  ...
  /**
   * Transform the point   @p p   on the real cell to the point   @p p_unit
   * on the   unit cell   @p cell   and returns   @p p_unit.       Uses Newton
   * iteration and the   @p transform_unit_to_real_cell   function.     In the
   * codimension one case, this function returns the normal projection   of
   * the real point   @p p   on the curve or surface identified by the   @p
   * cell.
   ...
   * determined by   checking whether the return reference coordinates lie
   * inside of outside   the reference cell (e.g., using
   * GeometryInfo::is_inside_unit_cell)   or   whether the exception mentioned
...
protected:
   ...
   * variables are marked as 'mutable'.     The current class uses essentially
   * the same fields for storage as the   MappingQGeneric class. Consequently,
   * it inherits from     MappingQGeneric::InternalData,   rather than from
   * Mapping::InternalDataBase.   The principal difference to
   * MappingQGeneric::InternalData   is that MappingQ switches between   $Q_1$
   * and     $Q_p$   mappings depending on the cell we are on, so the internal
   * data   object needs to also store a pointer to an InternalData object
   * that   pertains to a   $Q_1$   mapping.
   *
   */
...
protected:
  ...
  /**
   * If this flag is set   @p true   then   @p MappingQ   is used on all
   * cells, not   only on boundary cells.
   *
   */
