 ...
 * components continuous between mesh cells. These elements are based on an
 * article from Arnold, Boffi and Falk: Quadrilateral H(div) finite elements,
 * SIAM J. Numer. Anal. Vol.42, No.6, pp.2429-2451 In this article, the
 * authors demonstrate that the usual RT elements and also BDM and other
 * proposed finite dimensional subspaces of H(div) do not work properly on
 * arbitrary FE grids. I.e. the convergence rates deteriorate on these meshes.
 * As a solution the authors propose the ABF elements, which are implemented
 * in this module. This class is not implemented for the codimension one case
 * (<tt>spacedim != dim</tt>).
...
public:
   ...
   * returns <tt>FE_ABF<dim>(degree)</tt>, with   @p dim   and   @p degree
...
private:
  ...
  /**
   * Initialize the   @p generalized_support_points   field of the FiniteElement   class and fill the tables with interpolation weights (#boundary_weights   and #interior_weights). Called from the constructor.     See the     @ref GlossGeneralizedSupport   "glossary entry on generalized support points"
   * for more information.
   *
   */
  void
