...
/**
 * This class represents a mask that can be used to select individual vector components of a finite element (see also   @ref GlossComponentMask   "this glossary entry").
 ...
 *
 *
 * @note   A "mask" represents a data structure with   @p true   and   @p
 * false     entries that is generally used to enable or disable an operation
 * for a particular vector component. By this definition, disabled   vector
 * components still exist
 *
 *  -  they are simply not touched. As   a consequence, when you apply a component mask for interpolating   boundary values (to choose just one example) of a problem with     $C$   vector components, the input argument that describes the   boundary values will still have to provide   $C$   components even   if the mask says that we only want to interpolate a subset of   these components onto the finite element space. In other words,   a component mask does not represent a <i>reduction</i> operation;   it represents a <i>selection</i>.
 * Objects of
 * this kind are used in many places where one wants to restrict operations to
 * a certain subset of components, e.g. in
...
public:
   ...
   * specified by the argument.       @param   component_mask A vector of
   * <code>true/false</code>   entries that   determine which components of a
   * finite element are selected. If the   length of the given vector is zero,
   * then this interpreted as the case   where <i>every</i> component is
   * selected.
   *
   */
   ...
   * all true or false.       @param   n_components The number of elements of
   * this mask     @param   initializer The value each of these elements is
   ...
   * of the given argument.       @param   component_index The index for which
   * the function should return   whether the component is selected. If this
   * object represents a mask in   which all components are always selected
   * then any index is allowed here.   Otherwise, the given index needs to be
   * between zero and the number of   components that this mask represents.
   *
   */
   ...
   * <code>n</code>   components. This is true if either it was initialized
   * with   a vector with exactly   <code>n</code> entries of type
   * <code>bool</code>     (in this case,   @p n   must equal the result of
   * size()) or if it was   initialized with an empty vector (or using the
   * default constructor) in   which case it can represent a mask with an
   * arbitrary number of components   and will always say that a component is
   * selected.
   *
   */
  bool
  ...
  /**
   * Return the number of components that are selected by this mask.     Since
   * empty component masks represent a component mask that would return
   ...
   * that denotes the overall number of components.     If the object has been
   * initialized with a non-empty mask (i.e., if the   size() function returns
   * something greater than zero, or equivalently if
