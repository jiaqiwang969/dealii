...
namespace hp
{
  /**
   * This class
   * implements a collection of mapping objects in the same way as   the
   * hp::FECollection   implements a collection of finite element classes.
   * It implements the concepts stated in the     @ref hpcollection     module
   * described in the doxygen documentation.     Although it is recommended to
   * supply an appropriate mapping for each   finite element kind used in a
   * hp-computation, the MappingCollection class   implements a conversion
   * constructor from a single mapping.  Therefore it   is possible to offer
   * only a single mapping to the   hp::FEValues   class   instead of a
   ...
   * class   for the rules which mapping will be selected for a given cell.
  ...
  public:
     ...
     * more mapping objects passed to the constructor. For this     call to be
     * valid, all arguments need to be of types derived     from class
     ...
     * want to use the same order for mappings as for the elements of     the
     * hp::FECollection   object you use. However, the same     considerations
     * as discussed with the   hp::QCollection::push_back()       function
     * also apply in the current context.         This class creates a copy of
     * the given mapping object, i.e., you can     do things like
     * <tt>push_back(MappingQ<dim>(3));</tt>. The internal copy     is later
     * destroyed by this object upon destruction of the entire     collection.
     *
     */
    void
   ...
   * cases, the called function has to create a   $Q_1$   mapping object,
   * i.e., an   object of kind MappingQGeneric(1). This is costly. It would
   * also be   costly to create such objects as static objects in the affected
   ...
   * first time code runs through a particular function.     In order to avoid
   * creation of (static or dynamic)   $Q_1$   mapping objects   in these
   * contexts throughout the library, this class defines a static   collection
   * of mappings with a single   $Q_1$   mapping object. This   collection can
   * then be used in all of those places where such a   collection is needed.
   *
   */
  template <int dim, int spacedim = dim>
