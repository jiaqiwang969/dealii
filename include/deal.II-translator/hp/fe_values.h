...
namespace hp
{
   ...
   * can be bypassed by computing all objects in advance with the
   * corresponding   precalculate_fe_values() function.     The first template
   * parameter denotes the space dimension we are in, the   second the
   * dimensionality of the object that we integrate on, i.e. for   usual   @p
   * hp::FEValues   it is equal to the first one, while for face   integration
   * it is one less. The third template parameter indicates the   type of
   * underlying non-hp-FE*Values base type, i.e. it could either be
  ...
  public:
     ...
     * For   hp::FEFaceValues,   the ith entry of the quadrature collections
     * are     interpreted as the face quadrature rules to be applied the ith
     * face.
     *
     */
    FEValuesBase(
     ...
     * allocation as usual in this class.         This function precalculates
     * the FE*Values objects corresponding to the     provided parameters: The
     * total of all vector entries corresponding to the     same index
     * describes an FE*Values object similarly to select_fe_values().
     *
     */
    void
    ...
    /**
     * Same as above, geared to the most common use of   hp::FEValues
     * objects in     which FE, quadrature and mapping indices are similar on
     * each individual     cell.         FE*Values objects are created for
     * every FE in the FECollection, with     quadrature and mapping
     * corresponding to the same index from the     QuadratureCollection and
     * MappingCollection, respectively.         If QuadratureCollection or
     * MappingCollection contains only one object, it     is used for all
     * FE*Values objects.
     *
     */
    void
     ...
     * you called the   @p reinit   function of the   <tt>hp::FE*Values</tt>
     * class     the last time.
     *
     */
    const FEValuesType &
  ...
  protected:
    ...
    /**
     * Vector of quadrature collections. For   hp::FEFaceValues,   the ith
     * entry of     the quadrature collections are interpreted as the face
     * quadrature rules     to be applied the ith face.         The variable
     * q_collection collects the first quadrature rule of each     quadrature
     * collection of the vector.
     *
     */
  ...
  private:
     ...
     * within the q_collection.         Initially, all entries have zero
     * pointers, and we will allocate them     lazily as needed in
...
namespace hp
{
   ...
   * program for examples of use.     The idea of this class is as follows:
   * when one assembled matrices in the   hp-finite element method, there may
   * be different finite elements on   different cells, and consequently one
   * may also want to use different   quadrature formulas for different cells.
   * On the other hand, the   ::FEValues efficiently handles pre-evaluating
   * whatever information is   necessary for a single finite element and
   * quadrature object. This class   brings these concepts together: it
   * provides a "collection" of ::FEValues   objects.     Upon construction,
   * one passes not one finite element and quadrature   object (and possible a
   * mapping), but a whole collection of type     hp::FECollection   and
   * hp::QCollection.   Later on, when one sits on a   concrete cell, one
   * would call the reinit() function for this particular   cell, just as one
   * does for a regular ::FEValues object. The difference is   that this time,
   * the reinit() function looks up the active FE index of   that cell, if
   * necessary creates a ::FEValues object that matches the   finite element
   * and quadrature formulas with that particular index in   their
   * collections, and then re-initializes it for the current cell. The
   ...
   * with any ::FEValues object for non-hp-DoFHandler objects.     The
   * reinit() functions have additional arguments with default values. If
   ...
   * different index into the   hp::QCollection   class, one does not need to
   * sort   the quadrature objects in the quadrature collection so that they
   * match   one-to-one the order of finite element objects in the FE
   * collection (even   though choosing such an order is certainly
   * convenient).     Note that ::FEValues objects are created on the fly,
   * i.e. only as they   are needed. This ensures that we do not create
   * objects for every   combination of finite element, quadrature formula and
   * mapping, but only   those that will actually be needed. Alternatively if
   * desired, this   can be bypassed by computing all objects in advance with
   * the corresponding     hp::FEValuesBase::precalculate_fe_values()
   * function.     This class has not yet been implemented for the use in the
  ...
  public:
    ...
    /**
     * Reinitialize the object for the given cell.         After the call, you
     * can get an FEValues object using the     get_present_fe_values()
     * function that corresponds to the present cell.     For this FEValues
     * object, we use the additional arguments described     below to
     * determine which finite element, mapping, and quadrature     formula to
     * use. They are order in such a way that the arguments one may     want
     * to change most frequently come first. The rules for these     arguments
     * are as follows:         If the   @p fe_index   argument to this
     * function is left at its default     value, then we use that finite
     * element within the   hp::FECollection       passed to the constructor
     ...
     * <code>cell-@>active_fe_index()</code>  .         If the   @p q_index
     * argument is left at its default value, then we use     that quadrature
     * formula within the   hp::QCollection   passed to the     constructor of
     * this class with index given by
     ...
     * quadrature formula for each finite element in the   hp::FECollection.
     * As     a special case, if the quadrature collection contains only a
     * single     element (a frequent case if one wants to use the same
     * quadrature object     for all finite elements in an hp-discretization,
     * even if that may not     be the most efficient), then this single
     * quadrature is used unless a     different value for this argument is
     * specified. On the other hand, if a     value is given for this
     * argument, it overrides the choice of
     * <code>cell-@>active_fe_index()</code>   or the choice for the single
     * quadrature.         If the   @p mapping_index   argument is left at its
     * default value, then we     use that mapping object within the
     * hp::MappingCollection   passed to the     constructor of this class
     * with index given by       <code>cell-@>active_fe_index()</code>  , i.e.
     * the same index as that of     the finite element. As above, if the
     * mapping collection contains only a     single element (a frequent case
     * if one wants to use a   $Q_1$   mapping for     all finite elements in
     * an hp-discretization), then this single mapping     is used unless a
     * different value for this argument is specified.
     *
     */
    template <bool lda>
    void
    ...
    /**
     * Like the previous function, but for non-DoFHandler iterators. The
     * reason     this function exists is so that one can use   hp::FEValues
     * for     Triangulation objects too.         Since
     * <code>cell-@>active_fe_index()</code>   doesn't make sense for
   ...
   * ::FEValues.     The same comments apply as in the documentation of the
   * hp::FEValues     class. However, it is important to note that it is here
   * more common that   one would want to explicitly specify an index to a
   * particular quadrature   formula in the reinit() functions. This is
   * because the default index   corresponds to the finite element index on
   * the current function. On the   other hand, integration on faces will
   * typically have to happen with a   quadrature formula that is adjusted to
   * the finite elements used on both   sides of a face. If one sorts the
   * elements of the   hp::FECollection   with   ascending polynomial degree,
   * and matches these finite elements with   corresponding quadrature
   * formulas in the   hp::QCollection   passed to the   constructor, then the
   * quadrature index passed to the reinit() function   should typically be
   * something like   <code>std::max     (cell-  @>active_fe_index(),
   * neighbor-  @>active_fe_index()</code>   to ensure   that a quadrature
   * formula is chosen that is sufficiently accurate for     <em>  both  </em>
   * finite elements.
   * @ingroup hp hpcollection
   *
   */
  ...
  public:
    ...
    /**
     * Like the function above, but taking a vector of collection of
     * quadrature     rules. This allows to assign each face a different
     * quadrature rule: the     ith entry of a collection is used as the face
     * quadrature rule on the ith     face.         In the case that the
     * collections only contains a single face quadrature,     this quadrature
    ...
    /**
     * Reinitialize the object for the given cell and face.         After the
     * call, you can get an FEFaceValues object using the
     ...
     * arguments are as follows:         If the   @p fe_index   argument to
     * this function is left at its default     value, then we use that finite
     * element within the   hp::FECollection       passed to the constructor
     * of this class with index given by
     * <code>cell-@>active_fe_index()</code>  . Consequently, the
     * hp::FECollection   argument given to this object should really be the
    ...
    /**
     * Like the previous function, but for non-DoFHandler iterators. The
     * reason     this function exists is so that one can use this class for
     * Triangulation objects too.         Since
     * <code>cell-@>active_fe_index()</code>   doesn't make sense for
     * triangulation iterators, this function chooses the zero-th finite
     * element, mapping, and quadrature object from the relevant constructions
     * passed to the constructor of this object. The only exception is if you
  ...
  public:
     ...
     * for these arguments are as follows:         If the   @p q_index
     * argument is left at its default value, then we use     that quadrature
     * formula within the   hp::QCollection   passed to the     constructor of
     * this class with index given by
     * <code>cell-@>active_fe_index()</code>  , i.e. the same index as that of
