 ...
 * argument   @p dim   is ignored. This class exists because in some cases we
 * want to construct objects of type Tensor  @<spacedim-dim,dim,Number@>,
 * which should expand to scalars, vectors, matrices, etc, depending on the
 * values of the template arguments   @p dim   and   @p spacedim.   We
 * therefore need a class that acts as a scalar (i.e.   @p Number)   for all
 * purposes but is part of the Tensor template family.
 ...
 * @tparam   Number The data type in which the tensor elements are to be
 * stored. This will, in almost all cases, simply be the default   @p double,
 * but there are cases where one may want to store elements in a different
 * (and always scalar) type. It can be used to base tensors on   @p float   or
 * @p complex   numbers or any other data type that implements basic
 * arithmetic operations. Another example would be a type that allows for
 * Automatic Differentiation (see, for example, the Sacado type used in
 * step-33  ) and thereby can generate analytic (spatial) derivatives of a
 * function that takes a tensor as argument.
 *
 *
...
public:
   ...
   * as the template argument to this class. For   std::complex<number>,
   * this   corresponds to type number, and it is equal to Number for all
   * other   cases. See also the respective field in Vector<Number>.     This
   * alias is used to represent the return type of norms.
   *
   */
   ...
   * tensors are scalars, this is a natural operation.     This is the
   * non-const conversion operator that returns a writable   reference.
   * @note   This function can also be used in CUDA device code.
   *
   */
  ...
  /**
   * Return a reference to the encapsulated Number object. Since rank-0
   * tensors are scalars, this is a natural operation.     This is the const
   * conversion operator that returns a read-only reference.
   * @note   This function can also be used in CUDA device code.
   *
   */
 ...
 * The rank of a tensor specifies which types of physical quantities it can represent:   <ul>       <li>   A rank-0 tensor is a scalar that can store quantities such as     temperature or pressure. These scalar quantities are shown in this     documentation as simple lower-case Latin letters e.g.   $a, b, c, \dots$  .     </li>       <li>   A rank-1 tensor is a vector with   @p dim   components and it can     represent vector quantities such as velocity, displacement, electric     field, etc. They can also describe the gradient of a scalar field.     The notation used for rank-1 tensors is bold-faced lower-case Latin     letters e.g.   $\mathbf a, \mathbf b, \mathbf c, \dots$  .     The components of a rank-1 tensor such as   $\mathbf a$   are represented     as   $a_i$   where   $i$   is an index between 0 and <tt>dim-1</tt>.     </li>       <li>   A rank-2 tensor is a linear operator that can transform a vector     into another vector. These tensors are similar to matrices with       $\text{dim} \times \text{dim}$   components. There is a related class     SymmetricTensor<2,dim> for tensors of rank 2 whose elements are     symmetric. Rank-2 tensors are usually denoted by bold-faced upper-case     Latin letters such as   $\mathbf A, \mathbf B, \dots$   or bold-faced Greek     letters for example   $\boldsymbol{\varepsilon}, \boldsymbol{\sigma}$  .     The components of a rank 2 tensor such as   $\mathbf A$   are shown with     two indices   $(i,j)$   as   $A_{ij}$  . These tensors usually describe the     gradients of vector fields (deformation gradient, velocity gradient,     etc.) or Hessians of scalar fields. Additionally, mechanical stress     tensors are rank-2 tensors that map the unit normal vectors of internal     surfaces into local traction (force per unit area) vectors.     </li>       <li>   Tensors with ranks higher than 2 are similarly defined in a     consistent manner. They have   $\text{dim}^{\text{rank}}$   components and     the number of indices required to identify a component equals     <tt>rank</tt>. For rank-4 tensors, a symmetric variant called     SymmetricTensor<4,dim> exists.     </li>     </ul>
...
public:
   ...
   * Obviously, the ArrayView object must represent a stretch of   data of
   * size `dim`<sup>`rank`</sup>. The sequentially ordered elements   of the
   * argument `initializer` are interpreted as described by
   * unrolled_to_component_index().     This constructor obviously requires
   * that the   @p ElementType   type is   either equal to   @p Number,   or
   * is convertible to   @p Number.     Number.
   * @note   This function can also be used in CUDA device code.
   *
   */
  template <typename ElementType, typename MemorySpace>
   ...
   * This obviously requires that the   @p OtherNumber   type is convertible
   * to   @p     Number.
   * @note   This function can also be used in CUDA device code.
   *
   */
  ...
  /**
   * Fill a vector with all tensor elements.     This function unrolls all
   * tensor entries into a single, linearly numbered   vector. As usual in
   * C++, the rightmost index of the tensor marches   fastest.
   *
   */
  template <typename OtherNumber>
  void
...
private:
  ...
  /**
   * This constructor is for internal use. It provides a way   to create
   * constexpr constructors for Tensor<rank, dim, Number>
   * @note   This function can also be used in CUDA device code.
   *
   */
...
namespace internal
{
  ...
  /**
   * The structs below are needed to initialize nested Tensor objects.   Also
   * see numbers.h for another specialization.
   *
   */
...
/**
 * Scalar multiplication of two tensors of rank 0. This function unwraps the
 * underlying objects of type   @p Number   and   @p   OtherNumber that are
 * stored within the Tensor and multiplies them. It returns an unwrapped
 ...
 * right. Only multiplication with a scalar number type (i.e., a floating
 * point number, a complex floating point number, etc.) is allowed, see the
...
/**
 * Multiplication of a tensor of general rank with a scalar number from the
 * left. Only multiplication with a scalar number type (i.e., a floating point
 ...
 *  This multiplication is also called "Hadamard-product" (c.f. https://en.wikipedia.org/wiki/Hadamard_product_(matrices)), and generates a new tensor of size <rank, dim>: @f[
 * \text{result}_{i, j}
 * = \text{left}_{i, j}\circ
 *   \text{right}_{i, j}
 * @f]
 ...
 * Return a tensor of rank   $(\text{rank}_1 + \text{rank}_2
 *
 * - 2)$   that is the contraction of index   @p index_1   of a tensor   @p src1   of rank   @p rank_1   with the index   @p index_2   of a tensor   @p src2   of rank   @p rank_2:   @f[
 * \text{result}_{i_1,\ldots,i_{r1},j_1,\ldots,j_{r2}}
 * = \sum_{k}
 ...
 * <code>index_2==2</code>) of a tensor <code>t2</code>  , this function
 ...
 * rank: Return a tensor of rank   $(\text{rank}_1 + \text{rank}_2
 *
 * - 4)$   that is the contraction of index   @p index_1   with index   @p index_2,   and index   @p   index_3 with index   @p index_4   of a tensor   @p src1   of rank   @p rank_1   and a tensor   @p src2   of rank   @p rank_2:   @f[
 * \text{result}_{i_1,\ldots,i_{r1},j_1,\ldots,j_{r2}}
 * = \sum_{k, l}
 ...
 * <code>index_4==0</code>) of a tensor <code>t2</code>  , this function
 * should be invoked as
 *
 * @code
...
/**
 * The scalar product, or (generalized) Frobenius inner product of two tensors of equal rank: Return a scalar number that is the result of a full contraction of a tensor   @p left   and   @p right:   @f[
 * \sum_{i_1,\ldots,i_r}
...
/**
 * Full contraction of three tensors: Return a scalar number that is the result of a full contraction of a tensor   @p left   of rank   @p rank_1,   a tensor   @p middle   of rank   $(\text{rank}_1+\text{rank}_2)$   and a tensor   @p   right of rank   @p rank_2:   @f[
...
/**
 * The outer product of two tensors of   @p rank_1   and   @p rank_2:   Returns a tensor of rank   $(\text{rank}_1 + \text{rank}_2)$  : @f[
 * \text{result}_{i_1,\ldots,i_{r1},j_1,\ldots,j_{r2}}
...
/**
 * Return the adjugate of the given tensor of rank 2. The adjugate of a tensor   $\mathbf A$   is defined as @f[
 * \textrm{adj}\mathbf A
...
/**
 * Return the cofactor of the given tensor of rank 2. The cofactor of a tensor   $\mathbf A$   is defined as @f[
 * \textrm{cof}\mathbf A
...
/**
 * Return the nearest orthogonal matrix   $\hat {\mathbf A}=\mathbf U
 * \mathbf{V}^T$   by combining the products of the singular value
 * decomposition (SVD)   ${\mathbf A}=\mathbf U  \mathbf S \mathbf V^T$   for
 * a given input   ${\mathbf A}$  , effectively replacing   $\mathbf S$   with
 * the identity matrix. This is a (nonlinear) [projection
 * operation](https://en.wikipedia.org/wiki/Projection_(mathematics)) since
 * when applied twice, we have   $\hat{\hat{\mathbf A}}=\hat{\mathbf A}$   as
 * is easy to see. (That is because the SVD of   $\hat {\mathbf A}$   is
 * simply   $\mathbf U \mathbf I \mathbf{V}^T$  .) Furthermore,   $\hat
 * {\mathbf A}$   is really an orthogonal matrix because orthogonal matrices
 * have to satisfy   ${\hat {\mathbf A}}^T \hat {\mathbf A}={\mathbf I}$  ,
 ...
 * \mathbf I
 * @f}
 * due to the fact that the   $\mathbf U$   and   $\mathbf V$   factors that
 * come out of the SVD are themselves orthogonal matrices.
 * @param   A The tensor for which to find the closest orthogonal tensor.
 * @tparam   Number The type used to store the entries of the tensor.   Must
 * be either `float` or `double`.   @pre   In order to use this function, this
 * program must be linked with the   LAPACK library.   @pre     @p A   must
 * not be singular. This is because, conceptually, the problem   to be solved
 * here is trying to find a matrix   $\hat{\mathbf A}$   that   minimizes some
 * kind of distance from   $\mathbf A$   while satisfying the   quadratic
 * constraint     ${\hat {\mathbf A}}^T \hat {\mathbf A}={\mathbf I}$  . This
 * is not so   dissimilar to the kind of problem where one wants to find a
 * vector     $\hat{\mathbf x}\in{\mathbb R}^n$   that minimizes the quadratic
 * objective   function   $\|\hat {\mathbf x}
 *
 * - \mathbf x\|^2$   for a given   $\mathbf x$     subject to the constraint
 * $\|\mathbf x\|^2=1$
 *
 *  -  in other   words, we are seeking the point   $\hat{\mathbf x}$   on the unit sphere   that is closest to   $\mathbf x$  . This problem has a solution for all     $\mathbf x$   except if   $\mathbf x=0$  . The corresponding condition   for the problem we are considering here is that   $\mathbf A$   must not   have a zero eigenvalue.
 * @relatesalso   Tensor
 *
 *
 */
...
/**
 * Return the   $l_1$   norm of the given rank-2 tensor, where   $\|\mathbf
 * T\|_1 = \max_j \sum_i |T_{ij}|$   (maximum of the sums over columns).
 * @relatesalso   Tensor
 *
 *
 */
 ...
 * $\|\mathbf T\|_\infty = \max_i \sum_j |T_{ij}|$   (maximum of the sums over
 * rows).
 * @relatesalso   Tensor
 *
 *
