 ...
 * logical structure, and input files may include other files. The
 * ParameterHandler class is discussed in   step-29  ,   step-33  , and
 * step-34  . <h3>Declaring entries</h3> In order to use the facilities of a
 * ParameterHandler object, one first has to make known the different entries
 * the input file may or may not contain. This is done in the following way:
 *
 * @code
 *   ...
 ...
 * to   Patterns::Anything,   i.e. a pattern that matches every input string.
 * The fourth parameter can be used to document the intent or expected format
 * of an entry; its value is printed as a comment when writing all entries of
 * a ParameterHandler object using the print_parameters() function to allow
 * for easier understanding of a parameter file. It can be omitted as well, in
 * which case no such documentation will be printed. Entries may be located in
 * subsections which form a kind of input tree. For example input parameters
 * for linear solver routines should be classified in a subsection named
 * <tt>Linear solver</tt> or any other suitable name. This is accomplished in
 * the following way:
 *
 * @code
 * ...
 ...
 *
 *
 *  <h3>Input files and special characters</h3> For the first example above
 ...
 * recognized if in the declaration multiple whitespace is used. In entry
 * names and values the following characters are not allowed: <tt>\#</tt>,
 * <tt>{</tt>, <tt>}</tt>, <tt>|</tt>. Their use is reserved for the
 * MultipleParameterLoop class. Comments starting with \# are skipped.
 ...
 * preprocessor, all whitespace at the beginning of the line is ignored. We
 * propose to use the following scheme to name entries: start the first word
 * with a capital letter and use lowercase letters further on. The same
 * applies to the possible entry values to the right of the <tt>=</tt> sign.
 *
 *  <h3>Including other input files</h3> An input file can include other
 * include files using the syntax
 * @code
 *   ...
 ...
 * function).
 *
 *  <h3>Reading data from input sources</h3> In order to read input there are
 * three possibilities: reading from an   <tt>std::istream</tt>   object,
 * reading from a file of which the name is given and reading from a string in
 * memory in which the lines are separated by <tt>  @\n</tt>   characters.
 * These possibilities are used as follows:
 * @code
 *   ParameterHandler prm;
 *   ...
 ...
 * changed more than once will be overwritten every time they are used. You
 * should not try to declare entries using declare_entry() and
 * enter_subsection() with as yet unknown subsection names after using
 * parse_input(). The results in this case are unspecified. If an error occurs
 * upon reading the input, error messages are written to   <tt>std::cerr</tt>
 * and the reader function returns with a return value of   <code>false</code>
 * . This is opposed to almost all other functions in deal.II, which would
 * normally throw an exception if an error occurs; this difference in behavior
 * is a relic of the fact that this class predates deal.II and had previously
 * been written for a different project.
 *
 *  <h3>Using the %ParameterHandler Graphical User Interface</h3> An
 * alternative to using the hand-written input files shown above is to use the
 * graphical user interface (GUI) that accompanies this class. See <a
 * href="https://github.com/dealii/parameter_gui">the parameter_gui github
 * repository</a> for further details. <h3>Getting entry values out of a
 * %ParameterHandler object</h3> Each class gets its data out of a
 * ParameterHandler object by calling the get()  member functions like this:
 * @code
 ...
 * and leave subsections exactly as you did when declaring subsections. You
 * may choose the order in which to traverse the subsection tree. It is
 * possible to avoid calls to enter_subsection() and leave_subsection() by
 * supplying get() with a vector of strings representing the path from which
 * to get a value. For example, the following two versions of get_parameters()
 ...
 * expression is not stored. You can use get() to retrieve the parameter in
 * text form, get_integer() to get an integer or get_double() to get a double.
 * You can also use get_bool(). It will cause an internal error if the string
 * could not be converted to an integer, double or a bool. This should,
 * though, not happen if you correctly specified the regular expression for
 * this entry; you should not try to get out an integer or a double from an
 * entry for which no according regular expression was set. The internal error
 * is raised through the Assert() macro family which only works in debug mode.
 ...
 *
 *
 * <h3>Adding Actions to Parameters</h3> It is often convenient to have
 * something happen as soon as a parameter value is read. This could be a
 * check that it is valid
 *
 *  -  say, that a file that is listed in the parameter file exists
 *
 *  -  or to initiate something else in response, such as setting a variable outside the ParameterHandler (as in the example shown below). In almost all cases, this "action" could also be initiated once all parameters are read via parse_input(), but it is sometimes <i>convenient</i> to do it right away.
 * This is facilitated by the add_action() function that can be called after
 * declaring a parameter via declare_entry(). "Actions" are in essence
 ...
 * argument, and can then do whatever they want with it
 *
 *  -  e.g., save it somewhere outside the ParameterHandler object. (Exactly when the action is called is described in the documentation of the add_action() function.) Of course, in C++ one doesn't usually pass around the address of a function, but an action can be a function-like object (taking a string as argument) that results from calling such as a <a href="http://en.cppreference.com/w/cpp/language/lambda">lambda
 * function</a> that has the form
 *
 * @code
 * [] (const std::string &value) { ... do something with the value ... }
 * @endcode
 * and that is attached to a specific parameter. A typical example of such an
 * action would be as follows: let's assume that you have a program that
 * declares a parameter for the number of iterations it is going to run, say
 *
 * @code
 * class MyAlgorithm
 * {
 ...
 * };
 * @endcode
 * then one could obtain this parameter from a parameter file using a code
 * snippet in   @p run()   as follows:
 *
 * @code
 ...
 *
 *  -  is a bit cumbersome because one has to first declare <i>all</i> parameters and at a later time retrieve them from the ParameterHandler object. In large programs, these two things also often happen in different functions.
 * To avoid this, it would be nice if we could put both the declaration and
 * the retrieval into the same place. This can be done via actions, and the
 ...
 *   ... actual code doing something useful follows here...
 * @endcode
 * Here, the action consists of a lambda function that takes the value for
 * this parameter as a string, and then converts it to an integer to store in
 * the variable where it belongs. This action is executed inside the call to
 * <code>prm.parse_input()</code>  , and so there is now no longer a need to
 * extract the parameter's value at a later time. Furthermore, the code that
 * sets the member variable is located right next to the place where the
 * parameter is actually declared, so we no longer need to have two separate
 * parts of the code base that deal with input parameters. Of course, it is
 * possible to execute far more involved actions than just setting a member
 * variable as shown above, even though that is a typical case.
 *
 *  <h3>Style guide for data retrieval</h3> We propose that every class which
 * gets data out of a ParameterHandler object provides a function named
 * <tt>get_parameters</tt>. This should be declared <tt>virtual</tt>.
 * <tt>get_parameters</tt> functions in derived classes should call the
 * <tt>BaseClass::get_parameters</tt>   function.
 *
 *  <h3>Experience with large parameter lists</h3> Experience has shown that
 * in programs defining larger numbers of parameters (more than, say, fifty)
 * it is advantageous to define an additional class holding these parameters.
 * This class is more like a C-style structure, having a large number of
 * variables, usually public. It then has at least two functions, which
 * declare and parse the parameters. In the main program, the main class has
 * an object of this parameter class and delegates declaration and parsing of
 * parameters to this object. The advantage of this approach is that you can
 * keep out the technical details (declaration and parsing) out of the main
 * class and additionally don't clutter up your main class with dozens or more
 * variables denoting the parameters.
 *
 *
 * <h3>Worked Example</h3> This is the code:
 * @code
 ...
 *
 *
 * <h3>Representation of Parameters</h3> Here is some more internal
 * information about the representation of parameters: Logically, parameters
 * and the nested sections they are arranged in can be thought of as a
 * hierarchical directory structure, or a tree. Take, for example, the
 * following code declaring a set of parameters and sections they live in:
 * @code
 *   ParameterHandler prm;
 *
 ...
 *
 *
 *  -   <code>value</code>  : The content of this file is the current value of this parameter; initially, the content of the file equals the default value of the parameter.
 *
 *
 *
 *  -   <code>default_value</code>  : The content of this file is the default value of the parameter.
 *
 *
 *
 *  -   <code>pattern</code>  : A textual representation of the pattern that describes the parameter's possible values.
 *
 *
 *
 *  -   <code>pattern_index</code>  : A number that indexes the   Patterns::PatternBase   object that is used to describe the parameter.
 *
 *
 *
 *  -   <code>documentation</code>  : The content of this file is the documentation given for a parameter as the last argument of the   ParameterHandler::declare_entry   call. With the exception of the   <code>value</code>   file, the contents of files are never changed after declaration of a parameter.
 ...
 * subsections. Given this explanation, the code above will lead to a
 * hierarchical representation of data that looks like this (the content of
 * files is indicated at the right in a different font):
*    @image html parameter_handler.png
 * Once parameters have been read in, the contents of the   <code>value</code>
 * "files" may be different while the other files remain untouched. Using the
 ...
 * be only a single top-level construct in each file. The tree structure (and
 * its XML representation) is what the graphical user interface (see above)
 * uses to represent parameters like a directory/file collection.
 *
 *
 *
 * @ingroup input
...
public:
   ...
   * ParameterHandler::print_parameters().   The options can be categorized
   * into   two groups:
   *
   *
   *
  ...
  enum OutputStyle
  {
    ...
    /**
     * Write human readable output suitable to be read by
     * ParameterHandler::parse_input()   again.           @deprecated   Use
     * `PRM` instead of `Text`.
     *
     */
    Text = PRM,
     ...
     * href="http://en.wikipedia.org/wiki/XML">XML</a> file suitable to be
     * read     by   ParameterHandler::parse_input_from_xml()   again.
    ...
    /**
     * Write the content of ParameterHandler without comments or changed
     * default     values.
     *
     */
    ShortPRM = PRM | Short,
    ...
    /**
     * Write the content of ParameterHandler without comments or changed
     * default     values.           @deprecated   Use `ShortPRM` instead of
     * `ShortText`.
     *
     */
    ShortText = ShortPRM,
    ...
    /**
     * Write the content of ParameterHandler without comments or changed
     * default     values as a LaTeX file.
     *
     */
   ...
   * condition or error to provide values for known parameter fields. The
   * second   argument can be used to denote the name of the file (if that's
   * what the   input stream represents) we are reading from; this is only
   * used when   creating output for exceptions.     If non-empty   @p
   * last_line   is provided, the ParameterHandler object   will stop parsing
   * lines after encountering   @p last_line   .   This is handy when adding
   * extra data that shall be parsed manually.     If   @p skip_undefined   is
   * <code>true</code>  , the parameter handler   will skip undefined sections
   * and entries. This is useful for partially   parsing a parameter file, for
   * example to obtain only the spatial dimension   of the problem. By default
   * all entries and subsections are expected to be   declared.     The
   * function sets the value of all parameters it encounters in the   input
   * file to the provided value. Parameters not explicitly listed   in the
   * input file are left at the value they previously held, which   will be
   * the default value provided to declare_entry() unless one   has previously
   * read a different input file.     Each parameter value is matched against
   * the pattern for this   parameter that was provided to declare_entry(),
   * and for each parameter   all associated actions that may previously have
   * been set by   add_action() are executed. If a parameter does not satisfy
   * its   pattern, or if an associated action throws an exception, then the
   * value provided for the parameter is not set and the current   object
   * reverts to the subsection it was in before the current   function was
   * called. No further processing of the input stream   occurs, that is
   * everything that comes after the parameter whose   value does not satisfy
   ...
   * consistent with its name.     The parameter   @p last_line   will only be
   * used for parameter files of .prm type.   See the other parse_input
   * function for documentation.     The user can specify whether parameters
   * in the input file not added to the   parameter handler will be skipped by
   * @p skip_undefined   (enables partial   parsing), and whether the code
   * will assert that all parameters of the   parameter handler declared with
   * flag `has_to_be_set=true` are indeed found   in the input file.     If
   * the function is called with `skip_undefined=true`, it is recommended to
   ...
   * function in essence reads the entire file into a stream and   then calls
   * the other parse_input() function with that stream. See   there for more
   * information.
   *
   */
  virtual void
   ...
   * could be from a file originally written by the print_parameters()
   * function   using the XML output style and then modified by hand as
   * necessary, or from   a file written using this method and then modified
   * by the graphical   parameter GUI (see the general documentation of this
   * class).
   *
   */
  virtual void
   ...
   * could be from a file originally written by the print_parameters()
   * function   using the JSON output style and then modified by hand as
   * necessary, or from   a separate program that knows how to write JSON
   * format for ParameterHandler   input.
   *
   */
  virtual void
   ...
   * input has to match the <tt>pattern</tt> (default: any pattern).     The
   * function generates an exception of type ExcValueDoesNotMatchPattern   if
   * the default value doesn't match the given pattern, using the C++ throw
   ...
   * exception.     The parameter   @p documentation   defaulting to an empty
   * string is used   to add a documenting text to each entry which will be
   * printed as a comment   when this class is asked to write out all
   * declarations to a stream using   the print_parameters() function.     The
   * parameter   @p has_to_be_set   can be used in order to declare this
   * parameter as a parameter whose default value has to be overwritten by
   * one of the methods provided by this class. Whether a parameter has been
   * set   successfully can be queried by the functions
   * get_entries_wrongly_not_set()   and assert_that_entries_have_been_set().
   ...
   * documentation of this class for a longer description of actions, as well
   * as   examples.     The action is executed in three different
   * circumstances:
   *
   *
   *
   ...
   * it is attached to. In other words, it is not allowed to enter or    leave
   * sections of the current ParameterHandler object. It is, in    principle,
   * acceptable to call   ParameterHandler::get()   and related    functions
   * on other parameters in the current section, but since    there is no
   * guarantee about the order in which they will be read    from an input
   * file, you will not want to rely on the values these    functions would
   * return.
   * @note   Throwing an exception in an action is generally not a good
   * idea, but yields fundamentally the same result as if one tries to    read
   * a parameter from a file for which the value does not satisfy    the
   * pattern associated with the parameter. In other words, the    value just
   * read is discarded, and   ParameterHandler::parse_input()      stops to
   * read any further content from the file. See
  ...
  /**
   * Declare a new entry name   @p entry,   set its default value to the
   * content of   the variable   @p parameter,   and create an action that
   * will fill   @p     parameter with updated values when a file is parsed,
   * or the entry is set   to a new value.     By default, the pattern to use
   * is obtained by calling the function
   * Patterns::Tools::Convert<T>::to_pattern(),   but a custom one can be
   * used.     The parameter   @p has_to_be_set   can be used in order to
   * declare this   parameter as a parameter whose default value has to be
   * overwritten by   one of the methods provided by this class. Whether a
   * parameter has been set   successfully can be queried by the functions
   * get_entries_wrongly_not_set()   and assert_that_entries_have_been_set().
   *
   */
   ...
   * entry in the current section.     The primary purpose of this function is
   * to allow for a backward   compatible way of changing names in input files
   * of applications for which   backward compatibility is important. This can
   * be achieved by changing the   name of the parameter in the call to
   * declare_entry(), and then creating   an alias that maps the old name to
   * the new name. This way, old input   files can continue to refer to
   * parameters under the old name, and they   will automatically be mapped to
   * the new parameter name.     It is valid to set the same parameter
   * multiple times in an input file.   The value that will ultimately be
   * chosen in such cases is simply the last   value set. This rule also
   * applies to aliases, where the final value of a   parameter is the last
   * value set either through the current name of the   parameter or through
   * any of its possible multiple aliases. For example,   if you have an input
   ...
   * @endcode
   * then the final value for the parameter called   <code>parm1</code>   will
   * be   2, not 1.       @param   existing_entry_name The name of an existing
   * parameter in the   current section that the alias should refer to.
   * @param   alias_name An alternate name for the parameter referenced by the
   * first argument.     @param   alias_is_deprecated If true, mark the alias
   * as deprecated. This   will then be listed in the description of the alias
   * if you call   print_parameters(), and you will get a warning on the
   * screen when reading   an input file that contains this deprecated alias.
   * The purpose of this   argument is to be able to allow the use of an old
   * name for a parameter   (see above) but make it clear that this old name
   ...
   * subsection. The first string in   @p entry_subsection_path   must be the
   * name   of a subsection of the current section, and each next string must
   * be the   name of a subsection of the one before it.
   *
   */
  std::string
  ...
  /**
   * Return value of entry   @p entry_string   as   <code>long int</code>  .
   * (A long   int is chosen so that even very large unsigned values can be
   ...
   * Return value of entry   @p entry_string   as   <code>long int</code>  .
   * (A long   int is chosen so that even very large unsigned values can be
   * returned by   this function).   If   @p entry_subsection_path   is
   * non-empty, the value will be gotten   from the subsection represented by
   * that path instead of the current   subsection.
   *
   */
  long int
  ...
  /**
   * Return value of entry   @p entry_name   as   @p double.     If   @p
   * entry_subsection_path   is non-empty, the value will be gotten   from the
   * subsection represented by that path instead of the current   subsection.
   *
   */
  double
   ...
   * Return value of entry   @p entry_name   as   @p bool.   The entry may
   * be "true" or "yes" for   @p true,   "false" or "no" for   @p false
   * respectively.   If   @p entry_subsection_path   is non-empty, the value
   * will be gotten   from the subsection represented by that path instead of
   ...
   * given in the second argument.     The parameter must already exist in the
   * present subsection.     The function throws an exception of type
   * ExcValueDoesNotMatchPattern if   the new value does not conform to the
   ...
   * intended.     The function throws an exception of type
   * ExcValueDoesNotMatchPattern if   the new value does not conform to the
   * pattern for this entry.
   *
   */
   ...
   * Change the value presently stored for <tt>entry_name</tt> to the one
   * given in the second argument.     The parameter must already exist in the
   * present subsection.     For internal purposes, the new value needs to be
   * converted to a string.   This is done using 16 digits of accuracy, so the
   * set value and the one   you can get back out using get_double() may
   * differ in the 16th digit.     The function throws an exception of type
   * ExcValueDoesNotMatchPattern if   the new value does not conform to the
   * pattern for this entry.
   *
   */
   ...
   * alphabetically by default.   This behavior can be disabled setting the
   * optional parameter   @p style     to <tt>KeepDeclarationOrder</tt>: in
   * this case entries are printed in the   same order as they have been
   * declared.     In <tt>PRM</tt>, <tt>XML</tt>, and <tt>JSON</tt> format,
   * the output is   formatted in such a way that it is   possible to use it
   * for later input again. This is most useful to record   the parameters for
   * a specific run, since if you output the parameters   using this function
   * into a log file, you can always recover the results   by simply copying
   * the output to your input file.     Besides the name and value of each
   * entry, the output also contains the   default value of entries if it is
   * different from the actual value, as   well as the documenting string
   * given to the declare_entry() function if   available.     By using the
   * flag <tt>Short</tt> in combination with <tt>PRM</tt>,   <tt>XML</tt>,
   * <tt>JSON</tt>, or <tt>LaTeX</tt> (or by using the shortcuts
   * <tt>ShortPRM</tt>, <tt>ShortXML</tt>, <tt>ShortJSON</tt>, or
   * <tt>ShortLaTeX</tt>), a reduced output can be generated, only containing
   * the values and skipping the documentation.     In <tt>XML</tt> format,
   * the output starts with one root element   <tt>ParameterHandler</tt> in
   * order to get a valid XML document and all   subsections under it.     In
   * <tt>LaTeX</tt> format, the output contains the same information but in
   ...
   * are replaced by   <code>_XX</code>, where <code>XX</code>   is the
   * two-digit   ascii code of the character in hexadecimal encoding (so a
   * space becomes     <code>_20</code>   for example).     While this
   * function escapes special LaTeX-specific characters (backslash,
   ...
   * environments and other formatting in the description, but you need   to
   * escape quotes, backslashes, underscores, etc. yourself.     In addition,
   * all parameter names are listed with   <code>@\index</code>     statements
   * in two indices called   <code>prmindex</code>   (where the name of   each
   * parameter is listed in the index) and   <code>prmindexfull</code>
  ...
  /**
   * Print all parameters to the file given by   @p filename   with the given
   * output   style   @p style.       This function deduces the output format
   * from the extension of the specified   filename. Supported extensions are
   * `prm`, `xml`, `tex`, and `json`. Hence,   it is not necessary to specify
   * an output format via the   @p style   argument   as long as one of these
   * extensions is added to the filename. If an output   format is specified
   * in the   @p style   parameter nevertheless, the output   format has to be
   * consistent with the filename extension.     If no extension is specified
   * or the extension is not supported, the   output format is deduced from
   * the   @p style   argument.     If neither the extension is supported, nor
   * does the   @p style   parameter   contain a format specification, an
   * assertion is thrown.       @param   filename The output file name.
   ...
   * file style.     All current parameters and subsections are sorted
   * alphabetically by default.   This behavior can be disabled setting the
   * optional parameter   @p style     to <tt>KeepDeclarationOrder</tt>: in
   * this case entries are printed in the   same order as they have been
   * declared.
   * @note   All style settings in   @p style   not related to the ordering
   * are     ignored.
   *
   */
  void
   ...
   * leave_subsection() functions.     All current parameters and subsections
   * are sorted   alphabetically by default.   This behavior can be disabled
   * setting the optional parameter   @p style     to
   * <tt>KeepDeclarationOrder</tt>: in this case entries are printed in the
   * same order as they have been declared.
   * @note   All style settings in   @p style   not related to the ordering
   * are     ignored.     In most cases, you will not want to use this
   * function directly, but have   it called recursively by the previous
   * function.
   *
   */
  void
  ...
  /**
   * Return a set of parameter names (including subsection names)
   * corresponding   to those entries of the parameter handler that have not
   * been set by one of   the functions parsing parameters from an input file
   * or by an explicit call   to one of the set() functions, but that have
   * been declared as mandatory   parameters that must be set (through the
   * last argument of the   declare_entry() function or add_parameter()
   * function).
   *
   */
  std::set<std::string>
   ...
   * `has_to_be_set = true` have been set. An exception is invoked   if at
   * least one of these parameters has not been set.
   *
   */
  void
   ...
   * name, and a description of the pattern.       @deprecated   Use
   * ExcValueDoesNotMatchPattern instead of ExcInvalidEntryForPatternXML.
   *
   */
...
private:
  ...
  /**
   * A map that stores a pair of boolean variables for each entry   added to
   * the parameter handler. The first bool describes whether the   parameter
   * has to be set according to the last argument of the functions
  ...
  /**
   * A list of actions that are associated with parameters. These   are added
   * by the add_action() function. Nodes in the property   tree corresponding
   * to individual parameters   store indices into this array in order to
   ...
   * path_separator character.     This function simply calls
   * collate_path_string() with     @p subsection_path   as argument
   *
   */
  std::string
   ...
   * as a valid parameter file expression.     The function modifies its
   * argument, but also takes it by value, so the   caller's variable is not
   * changed.     If   @p skip_undefined   is   <code>true</code>  , the
   * parser   will skip undefined sections and entries. This is useful for
   * partially   parsing a parameter file, for example to obtain only the
   * spatial dimension   of the problem. By default all entries and
   * subsections are expected to be   declared.
   *
   */
  void
  ...
  /**
   * Print out the parameters of the subsection given by the     @p
   * target_subsection_path   argument, as well as all subsections   within it
   * recursively. This function is called from the   print_parameters()
   * function, and is implemented for all   @p style     arguments other than
   * XML and JSON (where we can output the   entire set of parameters via
   * BOOST functions). The   @p indent_level     argument indicates how many
   * spaces the output should be indented,   so that subsections properly nest
   * inside the output of higher   sections.
   *
   */
  void
 ...
 * parameters. Variant entry values are given like this:
 * @verbatim
 ...
 * each of the two solvers. In addition to variant entries, this class also
 * supports <i>array entries</i> that look like this:
 * @verbatim
 ...
 * the main loop, also the <i>n</i>th value of an array is returned. Since the
 * different variants are constructed in the order of declaration, not in the
 * order in which the variant entries appear in the input file, it may be
 * difficult to guess the mapping between the different variants and the
 * appropriate entry in an array. You will have to check the order of
 * declaration, or use only one variant entry. It is guaranteed that only
 * selections which match the regular expression (pattern) given upon
 * declaration of an entry are given back to the program. If a variant value
 * does not match the regular expression, the default value is stored and an
 * error is issued. Before the first run of the loop, all possible values are
 * checked for their conformance, so that the error is issued at the very
 * beginning of the program.
 *
 *  <h3>Usage</h3> The usage of this class is similar to the ParameterHandler
 * class. First the entries and subsections have to be declared, then a loop
 * is performed in which the different parameter sets are set, a new instance
 * of a user class is created which is then called. Taking the classes of the
 * example for the ParameterHandler class, the extended program would look
 * like this:
 * @code
 ...
 * from both the   MultipleParameterLoop::UserClass   and the problem class.
 * In any case, <tt>create_new</tt> has to provide a clean problem object
 * which is the problem in the second and third possibility. The derived class
 * also has to provide for member functions which declare the entries and
 * which run the program. Running the program includes getting the parameters
 * out of the ParameterHandler object. After defining an object of this helper
 * class and an object of the MultipleParameterLoop class, the entries have to
 * be declared in the same way as for the ParameterHandler class. Then the
 * input has to be read. Finally the loop is called. This executes the
 * following steps:
 * @code
 ...
 *
 *  <h3>Syntax for variant and array entry values</h3> Variant values are
 * specified like <tt>prefix{ v1 | v2 | v3 | ... }postfix</tt>. Whitespace to
 * the right of the opening brace <tt>{</tt> is ignored as well as to the left
 * of the closing brace <tt>}</tt> while whitespace on the respectively other
 * side is not ignored. Whitespace around the mid symbols <tt>|</tt> is also
 * ignored. The empty selection <tt>prefix{ v1 | }postfix</tt> is also allowed
 * and produces the strings <tt>prefixv1postfix</tt> and
 * <tt>prefixpostfix</tt>. The syntax for array values is equal, apart from
 * the double braces: <tt>prefix{{ v1 | v2 | v3 }}postfix</tt>.
 *
 *  <h3>Worked example</h3> Given the above extensions to the example program
 * for the ParameterHandler and the following input file
 * @verbatim
 *   set Equation 1 = Poisson
...
public:
   ...
   * from; this is only used when creating output for error messages.     If
   * non-empty   @p last_line   is provided, the ParameterHandler object
   * will stop parsing lines after encountering   @p last_line   .   This is
   * handy when adding extra data that shall be parsed manually.     If   @p
   * skip_undefined   is   <code>true</code>  , the parameter handler   will
   * skip undefined sections and entries. This is useful for partially
   * parsing a parameter file, for example to obtain only the spatial
   * dimension   of the problem. By default all entries and subsections are
   ...
   * ParameterHandler::parse_input   and then overriding the one we care
   * about.
   *
   */
