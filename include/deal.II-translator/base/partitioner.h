...
namespace Utilities
{
  namespace MPI
  {
    ...
    public:
       ...
       * @p local_size   and the number of ghost degrees of freedom   @p
       * ghost_size.               The local index range is translated to
       * global indices in an ascending       and one-to-one fashion, i.e.,
       * the indices of process   $p$   sit exactly       between the indices
       * of the processes   $p-1$   and   $p+1$  , respectively.
       * @note   Setting the   @p ghost_size   variable to an appropriate
       * value         provides memory space for the ghost data in a vector's
       * memory         allocation as and allows access to it via
       * local_element(). However,         the associated global indices must
       * be handled externally in this         case.
       *
       */
       ...
       * The optional parameter   @p larger_ghost_index_set   allows defining
       * an       indirect addressing into a larger set of ghost indices. This
       * setup is       useful if a distributed vector is based on that larger
       * ghost index       set but only a tighter subset should be
       * communicated according to         @p ghost_indices.
       *
       */
      void
      ...
      /**
       * Return the number of locally owned indices,       i.e.,
       * local_range().second minus local_range().first.       The returned
       * numbers need to add up to the total number of indices when
       * summed over all processes               @deprecated   Use the more
       * clearly named function locally_owned_size()       instead.
       *
       */
      DEAL_II_DEPRECATED
      unsigned int
       ...
       * exception is thrown.             Note that the returned local index
       * for locally owned indices will be       between 0 and
       * locally_owned_size()
       *
       ...
       * an IndexSet, but tailored to be iterated over.             In case
       * the partitioner did not take a second set of ghost indices       into
       * account, this subset is simply defined as the half-open interval
      ...
      /**
       * Return a vector of ranges of local indices that we are importing
       * during       compress(), i.e., others' ghosts that belong to the
       * local range.       Similar structure as in an IndexSet, but tailored
       * to be iterated over,       and some indices may be duplicated. The
       * returned pairs consists of the       index of the first element and
       * the index of the element one past the       last one in a range.
       *
       */
      const std::vector<std::pair<unsigned int, unsigned int>> &
       ...
       * of freedom imported from it during compress() operation (second
       * entry)       for all the processors that data is obtained from, i.e.,
       * locally owned       indices that are ghosts on other processors.
       * @note   The returned vector only contains those processor id's for
       * which       the second entry is non-zero.
       *
       */
      const std::vector<std::pair<unsigned int, unsigned int>> &
       ...
       * partitioner. Two partitioners are compatible if       they have the
       * same local sizes and the same ghost indices. They do not
       * necessarily need to correspond to the same data that is stored based
       * on these partioner objects. This is a local operation       only,
       * i.e., if only some processors decide that the partitioning is
       * not compatible, only these processors will return   @p false,
       * whereas       the other processors will return   @p true.
       *
       */
      bool
       ...
       * current partitioner. Two partitioners are compatible if       they
       * have the same local size and the same ghost indices. They do not
       * necessarily need to correspond to the same data that is stored based
       * on these partioner objects. As opposed to       is_compatible(), this
       * method checks for compatibility among all       processors and the
       * method only returns   @p true   if the partitioner is       the same
       * on all processors. In other words, it does a global       "and"
       * operation over the results returned by is_compatible() on all
       * involved processes.             This method performs global
       * communication, so make sure to use it       only in a context where
       * all processors call it the same number of       times.
       *
       */
      bool
       ...
       * is a valid value.               @param   locally_owned_array The
       * array of data from which the data is       extracted and sent to the
       * ghost entries on a remote processor.               @param
       * temporary_storage A temporary storage array of length
       ...
       * @param   requests The list of MPI requests for the ongoing
       * non-blocking       communication that will be finalized in the
       * export_to_ghosted_array_finish() call.             This functionality
       * is used in
       ...
       * range described by the ghost indices of this class.
       * @param   ghost_array The array that will receive the exported data
       * started in the   @p export_to_ghosted_array_start().   This must be
       * the       same array as passed to that function, otherwise the
       * behavior is       undefined.               @param   requests The list
       * of MPI requests for the ongoing non-blocking       communication that
       * were started in the       export_to_ghosted_array_start() call. This
       * must be the same array as       passed to that function, otherwise
       * MPI will likely throw an error.             This functionality is
       * used in
       * LinearAlgebra::distributed::Vector::update_ghost_values().
       *
       */
       ...
       * import_from_ghosted_array_finish().               @param
       * vector_operation Defines how the data sent to the owner should
       * be combined with the existing entries, e.g., added into.
       * @param   communication_channel Sets an offset to the MPI_Isend and
       * MPI_Irecv calls that avoids interference with other ongoing
       * import_from_ghosted_array_start() calls on different       entries.
       * Typically handled within the blocks of a block vector.       Any
       * value less than 200 is a valid value.               @param
       * ghost_array The array of ghost data that is sent to a remote
       ...
       * ghost_array to zero.               @param   temporary_storage A
       * temporary storage array of length       n_import_indices() that is
       * used to hold the packed data from MPI       communication that will
       * later be written into the locally owned       array. Note that this
       * array must not be touched until the respective
       * import_from_ghosted_array_finish() call has been made because the
       * model uses non-blocking communication.               @param
       * requests The list of MPI requests for the ongoing non-blocking
       * communication that will be finalized in the
       * export_to_ghosted_array_finish() call.             This functionality
       * is used in         LinearAlgebra::distributed::Vector::compress().
       *
       */
      template <typename Number, typename MemorySpaceType = MemorySpace::Host>
      void
       ...
       * @param   vector_operation Defines how the data sent to the owner
       * should       be combined with the existing entries, e.g., added into.
       ...
       * @p vector_operation.                 @param   ghost_array The array
       * of ghost data that is sent to a remote       owner of the respective
       * index in a vector. Its size must either be       n_ghost_indices() or
       * equal the number of ghost indices in the larger       index set that
       * was given as second argument to       set_ghost_indices(). This
       * function will set all data entries behind         @p ghost_array   to
       * zero for the implementation-dependent cases when it       was not
       * already done in the import_from_ghosted_array_start() call.
       * @param   locally_owned_storage The array of data where the resulting
       * data       sent by remote processes to the calling process will be
       * accumulated       into.               @param   requests The list of
       * MPI requests for the ongoing non-blocking       communication that
       * have been initiated in the       import_to_ghosted_array_finish()
       * call. This must be the same array as       passed to that function,
       * otherwise MPI will likely throw an error.             This
    ...
    private:
      ...
      /**
       * The set of (local) indices that we are importing during compress(),
       * i.e., others' ghosts that belong to the local range. The data stored
       * is       the same than in import_indices_data but the data is
       * expanded in plain       arrays. This variable is only used when using
       * CUDA-aware MPI.
       *
       */
      ...
      /**
       * A variable caching the number of ghost indices. It would be expensive
       * to compute it by iterating over the import indices and accumulate
       * them.
       *
       */
      ...
      /**
       * An array that caches the number of chunks in the import indices per
       * MPI       rank. The length is ghost_indices_subset_data.size()+1.
       *
       */
      ...
      /**
       * A variable storing whether the ghost indices have been explicitly
       * set.
       *
       */
