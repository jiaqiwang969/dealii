 ...
 * [0,1]x[0,1], etc. There are a number of derived classes, denoting concrete
 * integration formulae. Their names are prefixed by <tt>Q</tt>. Refer to the
 * list of derived classes for more details. The schemes for higher dimensions
 * are typically tensor products of the one- dimensional formulae, but refer
 * to the section on implementation detail below. In order to allow for
 * dimension independent programming, a quadrature formula of dimension zero
 * exists. Since an integral over zero dimensions is the evaluation at a
 * single point, any constructor of such a formula initializes to a single
 * quadrature point with weight one. Access to the weight is possible, while
 * access to the quadrature point is not permitted, since a Point of dimension
 * zero contains no information. The main purpose of these formulae is their
 * use in QProjector, which will create a useful formula of dimension one out
 * of them. <h3>Mathematical background</h3> For each quadrature formula we
 * denote by <tt>m</tt>, the maximal degree of polynomials integrated exactly.
 * This number is given in the documentation of each formula. The order of the
 * integration error is <tt>m+1</tt>, that is, the error is the size of the
 * cell to the <tt>m+1</tt> by the Bramble- Hilbert Lemma. The number
 * <tt>m</tt> is to be found in the documentation of each concrete formula.
 * For the optimal formulae QGauss we have   $m = 2N-1$  , where N is the
 * constructor parameter to QGauss. The tensor product formulae are exact on
 * tensor product polynomials of degree <tt>m</tt> in each space direction,
 * but they are still only of <tt>m+1</tt>st order. <h3>Implementation
 * details</h3> Most integration formulae in more than one space dimension are
 * tensor products of quadrature formulae in one space dimension, or more
 * generally the tensor product of a formula in <tt>(dim-1)</tt> dimensions
 * and one in one dimension. There is a special constructor to generate a
 * quadrature formula from two others.  For example, the QGauss  @<dim@>
 * formulae include <i>N<sup>dim</sup></i> quadrature points in <tt>dim</tt>
 * dimensions, where N is the constructor parameter of QGauss.
 *
 *
 * @note
 * Instantiations for this template are provided for dimensions 0, 1, 2, and 3
 * (see the section on   @ref Instantiations  ).
 *
 *
 */
template <int dim>
...
public:
  ...
  /**
   * Constructor.     This constructor is marked as explicit to avoid
   * involuntary accidents   like in   <code>hp::QCollection@<dim@>
   * q_collection(3)</code>   where     <code>hp::QCollection@<dim@>
   * q_collection(QGauss@<dim@>(3))</code>   was   meant.
   *
   */
   ...
   * This constructor assumes (and tests) that constant functions are
   * integrated   exactly, i.e. the sum of the quadrature weights is one.
   * <tt>SubQuadrature<dim>::type</tt>   expands to
   ...
   * a formula in one dimension.     Assuming that the points in the
   * one-dimensional rule are in ascending   order, the points of the
   * resulting rule are ordered lexicographically   with <i>x</i> running
   * fastest.     In order to avoid a conflict with the copy constructor in
   * 1d, we let the   argument be a 0d quadrature formula for dim==1, and a 1d
   * quadrature   formula for all other space dimensions.     This constructor
   * does not require that constant functions are integrated   exactly.
   * Therefore, it is appropriate if the one-dimensional formula   is defined
   * with respect to a weighting function.
   *
   */
   ...
   * returns the   @p dim   one-dimensional basis objects.   Otherwise,
   * calling this function is not allowed.     For   @p dim   equal to one, we
   * can not return the   std::array   as a const   reference and have to
   * return it by value. In this case, the array   will always contain a
   * single element (  @p this).
...
protected:
   ...
   * constructors taking a Quadrature<1>   (and possibly a Quadrature<dim-1>
   * object) is called. This implies   that the quadrature points are sorted
   * lexicographically.
   *
   */
  ...
  /**
   * Stores the one-dimensional tensor basis objects in case this object   can
   * be represented by a tensor product.
   *
   */
 ...
 * dimension. In one space dimension, the given base formula is copied and
 * scaled onto a given number of subintervals of length <tt>1/n_copies</tt>.
 * If the quadrature formula uses both end points of the unit interval, then
 * in the interior of the iterated quadrature formula there would be
 * quadrature points which are used twice; we merge them into one with a
 * weight which is the sum of the weights of the left- and the rightmost
 * quadrature point. Since all dimensions higher than one are built up by
 * tensor products of one dimensional and <tt>dim-1</tt> dimensional
 * quadrature formulae, the argument given to the constructor needs to be a
 * quadrature formula in one space dimension, rather than in <tt>dim</tt>
 * dimensions. The aim of this class is to provide a low order formula, where
 * the error constant can be tuned by increasing the number of quadrature
 * points. This is useful in integrating non-differentiable functions on
 * cells.
 *
 *
 */
