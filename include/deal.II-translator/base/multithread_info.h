...
/**
 * This class
 * provides information about the system which may be of use in multithreaded
 * programs.  At the moment this is just the number of CPUs. If deal.II is
 * compiled with multithreading support, some functions will use multiple
 * threads for their action. Currently the library supports both thread-based
 * and task-based parallelism.   @ref threads   describes the different uses
 * of each. The default number of threads used for task-based parallel methods
 * is selected automatically by the Threading Building Blocks library. See
 * @ref threads   for more information on this.  Thread-based parallel methods
 * need to explicitly create threads and may want to use a number of threads
 * that is related to the number of CPUs in your system. The recommended
 * number of threads can be queried using   MultithreadInfo::n_threads(),
 * while the number of cores in the system is returned by
...
public:
  /**
   * Constructor. This constructor is deleted because no instance of   this
   * class needs to be constructed (all members are static).
   *
   */
  MultithreadInfo() = delete;

  /**
   * The number of CPUs in the system.     This internally calls
   ...
   * system).     This routine is executed automatically with the default
   * argument before   your code in main() is running (using a static
   * constructor). It is also   executed by
   * Utilities::MPI::MPI_InitFinalize.   Use the appropriate   argument of the
   * constructor of   Utilities::MPI::MPI_InitFinalize   if you   have an MPI
   * based code.
   *
   */
  static void
...
#  ifdef DEAL_II_WITH_TASKFLOW
  /**
   * Return a reference to the global Executor from taskflow.     The Executor
   * is set to use n_threads() worker threads that you can   control using
   * set_thread_limit() and the DEAL_II_NUM_THREADS environment   variable.
   *
   */
  static tf::Executor &
