...
/**
 * A namespace whose main template function supports running multiple threads each of which operates on a subset of the given range of objects. The class uses the Intel Threading Building Blocks (TBB) to load balance the individual subranges onto the available threads. For a lengthy discussion of the rationale of this class, see the   @ref threads   "Parallel computing with multiple processors"
 * module. It is used in the tutorial first in   step-9  , and again in
 * step-13  ,   step-14  ,   step-32   and others. The class is built on the
 * following premise: One frequently has some work that needs to be done on a
 * sequence of objects; a prototypical example is assembling cell
 * contributions to a system matrix or right hand side. In many such examples,
 * part of the work can be done entirely independently and in parallel,
 * possibly using several processor cores on a machine with shared memory.
 * However, some other part of this work may need to be synchronized and be
 * done in order. In the example of assembling a matrix, the computation of
 * local contributions can be done entirely in parallel, but copying the local
 * contributions into the global matrix requires some care: First, several
 * threads can't write at the same time, but need to synchronize writing using
 * a mutex; secondly, we want the order in which local contributions are added
 * to the global matrix to be always the same because floating point addition
 * is not commutative and adding local contributions to the global matrix in
 * different orders leads to subtly different results that can affect the
 * number of iterations for iterative solvers as well as the round-off error
 * in the solution in random ways. Consequently, we want to ensure that only
 * one thread at a time writes into the global matrix, and that results are
 * copied in a stable and reproducible order. This class implements a
 * framework for this work model. It works with a stream of objects given by
 * an iterator range, runs a worker function in parallel on all of these
 * objects and then passes each object to a postprocessor function that runs
 * sequentially and gets objects in exactly the order in which they appear in
 * the input iterator range. None of the synchronization work is exposed to
 * the user of this class. Internally, the range given to the run() function
 * of this class is split into a sequence of "items", which are then
 * distributed according to some %internal algorithm onto the number of
 * available threads. An item is an element of the range of iterators on which
 * we are to operate; for example, for the purpose of assembling matrices or
 * evaluating error indicators, an item could be a cell. The TBB library
 * determines how many threads are created (typically as many as there are
 * processor cores), but the number of items that may be active at any given
 * time is specified by the argument to the constructor. It should be bigger
 * or equal to the number of processor cores
 *
 ...
 * the iterator range. Typically, worker functions need additional data, for
 * example FEValues objects, input data vectors, etc, some of which can not be
 * shared among threads. To this end, the run() function takes another
 * template argument, ScratchData, which designates a type objects of which
 * are stored with each item and which threads can use as private data without
 * having to share them with other threads. The run() function takes an
 * additional argument with an object of type ScratchData that is going to be
 * copied for the arguments passed to each of the worker functions. In
 * addition, worker functions store their results in objects of template type
 * CopyData. These are then handed off to a separate function, called copier,
 * that may use the stored results to transfer them into permanent storage.
 * For example, it may copy the results of local contributions to a matrix
 * computed by a worker function into the global matrix. In contrast to the
 * worker function, however, only one instance of the copier is run at any
 ...
 * same order as the items were created. Once an item is processed by the
 * copier, it is deleted and the ScratchData and CopyData objects that were
 * used in its computation are considered unused and may be re-used for the
 * next invocation of the worker function, on this or another thread. However,
 * the WorkStream functions make no attempt to reset these objects to any kind
 * of pristine state
 *
 ...
 * used with   LocalIntegrators::L2::weighted_mass_matrix()   to assemble the
 * local matrix could be resized to the corresponding number of quadrature
 * points of the current cell in DoFHandlers with hp-capabilities. Similarly,
 * local stiffness matrix in CopyData can be resized in accordance with the
 * number of local DoFs on the current cell.
 *
 *
...
#  ifdef DEAL_II_WITH_TBB
    /**
     * A namespace
     * for the implementation of details of the WorkStream pattern     and
     * function. This namespace holds classes that deal with the second
     * implementation described in the paper by Turcksin, Kronbichler and
     * Bangerth (see       @ref workstream_paper  ).     Here, no coloring is
     * provided, so copying is done sequentially using a     TBB filter.
    ...
    namespace tbb_no_coloring
    {
      ...
      private:
         ...
         * may start tasks itself and then call
         * Threads::TaskGroup::join_all()           or a similar function,
         * which the TBB scheduler may use to run         something else on
         * the current thread
         *
         *  -  for example another         instance of the worker function. Consequently, there would be two         instances of the worker function that use the same scratch object         if we only provided a single scratch object per thread. The         solution is to provide a list of scratch objects for each thread,         together with a flag indicating whether this scratch object is         currently used. If a thread needs a scratch object, it walks this         list until it finds an unused object, or, if there is none, creates         one itself. Note that we need not use synchronization primitives         for this process since the lists are thread-local and we are         guaranteed that only a single thread accesses them as long as we         have no yield point in between the accesses to the list.                 The pointers to scratch objects stored in each of these lists must         be so that they are deleted on all threads when the thread local         object is destroyed. This is achieved by using unique_ptr.                 Note that when a worker needs to create a scratch object, it         allocates it using sample_scratch_data to copy from. This has the         advantage of a first-touch initialization, i.e., the memory for the         scratch data object is allocated and initialized by the same thread         that will later use it.
         *
...
#  endif // DEAL_II_WITH_TBB
    ...
    /**
     * A reference implementation without using multithreading to be used if
     * we     don't have multithreading support or if the user requests to run
     * things     sequentially. This is more efficient than using TBB or
     * taskflow if we     only have a single thread.
     *
     */
...
#  ifdef DEAL_II_WITH_TBB
     ...
     * A namespace
     * for the implementation of details of the WorkStream pattern     and
     * function. This namespace holds classes that deal with the third
     * implementation described in the paper by Turcksin, Kronbichler and
...
#  endif // DEAL_II_WITH_TBB
   ...
   * to recognize that the empty function object created above isnot* the same
   * as a lambda function with an empty body,   `[](const CopyData &){}`
   *
   *  -  from the perspective of this function,   there is no way to recognize whether a lambda function provided as   a copier does something or does not do something in its body,   and so it needs to be copied. On the other hand, a default-constructed     `std::function`   objectcan* be recognized, and is then used to select   a more efficient algorithm.
   *
   */
  template <typename Worker,
            typename Copier,
  ...
  /**
   * This is
   * one of two main functions of the WorkStream concept, doing work   as
   ...
   * @ref workstream_paper  ).     This function that can be used for worker
   * and copier objects that are   either pointers to non-member functions or
   * objects that allow to be   called with an operator(), for example lambda
   * functions   or objects created by   std::bind.   If the copier is an
   * empty function, it is   ignored in the pipeline. (However, a lambda
   * function with an empty body is  not* equivalent to an empty
   * `std::function`   object and will, consequently,   not be ignored.
   * The argument passed as   @p end   must be convertible to the same type as
   * @p     begin, but doesn't have to be of the same type itself. This allows
   * to   write code like <code>WorkStream().run(dof_handler.begin_active(),
   * dof_handler.end(), ...</code> where the first is of type
   * DoFHandler::active_cell_iterator   whereas the second is of type
   * DoFHandler::raw_cell_iterator.       The two data types
   * <tt>ScratchData</tt> and <tt>CopyData</tt> need to   have a working copy
   * constructor. <tt>ScratchData</tt> is only used in the   <tt>worker</tt>
   * function, while <tt>CopyData</tt> is the object passed   from the
   * <tt>worker</tt> to the <tt>copier</tt>.     The   @p queue_length
   * argument indicates the number of items that can be   live at any given
   * time. Each item consists of   @p chunk_size   elements of   the input
   * stream that will be worked on by the worker and copier   functions one
   * after the other on the same thread.
   * @note   If your data objects are large, or their constructors are
   * expensive, it is helpful to keep in mind that <tt>queue_length</tt>
   * copies of the <tt>ScratchData</tt> object and
   * <tt>queue_length*chunk_size</tt> copies of the <tt>CopyData</tt> object
...
#  endif
   ...
   * functions   `IteratorRangeType::begin()`   and
   * `IteratorRangeType::end()`,     both of which return iterators to
   * elements that form the bounds of the   range.
   *
   */
  template <typename Worker,
            typename Copier,
...
#  endif
  ...
  /**
   * This is
   * a variant of one of the two main functions of the WorkStream   concept,
   * doing work as described in the introduction to this namespace.   It
   * corresponds to implementation 2 of the paper by Turcksin, Kronbichler
   * and Bangerth (see     @ref workstream_paper  ).     This is the function
   * that can be used for worker and copier functions   that are member
   * functions of a class. If the copier is an empty function,   it is ignored
   * in the pipeline.     The argument passed as   @p end   must be
   * convertible to the same type as   @p     begin, but doesn't have to be of
   * the same type itself. This allows to   write code like
   * <code>WorkStream().run(dof_handler.begin_active(),   dof_handler.end(),
   * ...</code> where the first is of type
   * DoFHandler::active_cell_iterator   whereas the second is of type
