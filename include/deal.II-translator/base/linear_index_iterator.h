 ...
 * SparseMatrix, store their data in contiguous buffers (though the   <em>
 * interpretation  </em>   of what the elements of these buffers represent
 ...
 * <code>AccessorType</code>   provides certain members (documented below)
 * that completely describe the state of the iterator. The intended use of
 * this class is for containers to define their own accessor classes and then
 * use the curiously recurring template pattern (CRTP) technique to define
 * their iterators. For example, here is a container that uses
 * LinearIndexIterator to define its own iterator classes:
 *
 *
 * @code
 * template <typename T>
 ...
 * correct type.
 * @tparam   AccessorType LinearIndexIterator assumes that the   <code>AccessorType</code>   template parameter has the following members which completely describe the current state of the iterator:   <ol>       <li>  A pointer named   <code>container</code>   to the original container (e.g.,   the relevant SparseMatrix). This should be a   <code>const</code>   pointer   for   <code>const</code>   iterators.  </li>       <li>  An array index named   <code>linear_index</code>   that stores the current   position in the container's storage buffer.   <code>linear_index</code>   does   not need to be an integer: it could be a class type (convertible to the   correct index type of the container) that implements     <code>operator+=</code>, <code>operator&lt;</code>  , and     <code>operator==</code>  . For example, one could implement a strided   iterator by implementing   <code>operator+=</code>   and     <code>operator-</code>   with multiplicative factors.  </li>     </ol>   In addition,   <code>AccessorType</code>   should declare the relevant LinearIndexIterator instantiation to be a   <code>friend</code>   and define a   <code>size_type</code>   type.
 *
 *
...
public:
  ...
  /**
   * Comparison operator. Returns   <code>true</code>   if both iterators
   * point to   the same entry in the same container.
   *
   */
   ...
   * checks for equality.     This function is only valid if both iterators
