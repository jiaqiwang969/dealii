 ...
 * certain format. <h3>Structure of the output data</h3> Data is not written
 * with the deal.II mesh structure. Instead, it relies on a set of
 * <tt>patches</tt> created by a derived class (for example the DataOut,
 * DataOutStack, DataOutFaces, DataOutRotation, or MatrixOut classes).  Each
 * Patch describes a single logical cell of a mesh, possibly subdivided a
 * number of times to represent higher order polynomials defined on this cell.
 * To this end, a patch consists of a <tt>dim</tt>-dimensional regular grid
 * with the same number of grid points in each direction. In the simplest case
 * it may consist of the corner points of a single mesh cell. For each point
 * of this local grid, the Patch contains an arbitrary number of data values,
 * though the number of data sets must be the same for each point on each
 * patch. By offering this interface to the different output formats, it is
 * simple to extend this class to new formats without depending on such things
 ...
 *
 *  <h4>Patches</h4> Grids can be thought of as a collection of cells; if you
 * want to write out data on such a grid, you can do so by writing them one
 * cell at a time. The functions in this class therefore take a list of
 * objects describing the data on one cell each. This data for each cell
 * usually consists of a list of vertices for this cell, and a list of data
 * values (for example solution data, error information, etc) at each of these
 * vertices. In some cases, this interface to a cell is too restricted,
 * however. For example, you may have higher order elements and printing the
 * values at the vertices only is not enough. For this reason, we not only
 * provide writing the data on the vertices only, but the data is organizes as
 * a tensor product grid on each cell. The parameter <tt>n_subdivisions</tt>,
 * which is given for each patch separately, denotes how often the cell is to
 * be divided for output; for example, <tt>n_subdivisions==1</tt> yields no
 ...
 * inside the Patch, as they cannot be easily recovered otherwise. Given these
 * comments, the actual data to be printed on this patch of points consists of
 * several data sets each of which has a value at each of the patch points.
 * For example with <tt>n_subdivisions==2</tt> in two space dimensions, each
 * data set has to provide nine values, and since the patch is to be printed
 * as a tensor product (or its transformation to the real space cell), its
 * values are to be ordered like <i>(x0,y0) (x0,y1) (x0,y2) (x1,y0) (x1,y1)
 * (x1,y2) (x2,y0) (x2,y1) (x2,y2)</i>, i.e. the z-coordinate runs fastest,
 * then the y-coordinate, then x (if there are that many space directions).
 *
 *  <h4>Generalized patches</h4> In general, the patches as explained above
 * might be too restricted. For example, one might want to draw only the outer
 * faces of a domain in a three-dimensional computation, if one is not
 * interested in what happens inside. Then, the objects that should be drawn
 * are two-dimensional in a three-dimensional world. The Patch class and
 * associated output functions handle these cases. The Patch class therefore
 * takes two template parameters, the first, named <tt>dim</tt> denoting the
 * dimension of the object (in the above example, this would be two), while
 * the second, named <tt>spacedim</tt>, denotes the dimension of the embedding
 * space (this would be three). The corner points of a patch have the
 * dimension of the space, while their number is determined by the dimension
 * of the patch. By default, the second template parameter has the same value
 * as the first, which would correspond to outputting a cell, rather than a
 * face or something else. <h3>DataOutBaseInterface</h3> The members of this
 * namespace are not usually called from user code directly. Rather, classes
 * that use the functions declared here are typically derived from
 * DataOutInterface. The interface of this class basically consists of the
 * declaration of a data type describing a patch and a bunch of functions
 * taking a list of patches and writing them in one format or other to the
 * stream. It is in the responsibility of the derived classes to provide this
 * list of patches. In addition to the list of patches, a name for each data
 * set may be given.
 *
 *  <h3>Querying interface</h3> This class also provides a few functions
 * (parse_output_format(), get_output_format_names(), default_suffix()) that
 * can be used to query which output formats this class supports. The provide
 * a list of names for all the formats we can output, parse a string and
 * return an enum indicating each format, and provide a way to convert a value
 * of this enum into the usual suffix used for files of that name. Using these
 * functions, one can entirely free applications from knowledge which formats
 * the library presently allows to output; several of the example programs
 * show how to do this. <h3>Output parameters</h3> All functions take a
 * parameter which is a structure of type <tt>XFlags</tt>, where <tt>X</tt> is
 * the name of the output format. To find out what flags are presently
 * supported, read the documentation of the different structures. Note that
 * usually the output formats used for scientific visualization programs have
 * no or very few parameters (apart from some compatibility flags) because
 * there the actual appearance of output is determined using the visualization
 * program and the files produced by this class store more or less only raw
 * data. The direct output formats, like Postscript or Povray need to be given
 * a lot more parameters, though, since there the output file has to contain
 * all details of the viewpoint, light source, etc. <h3>Writing backends</h3>
 ...
 * information for the grid cells and data fields. For each of these fields,
 * output functions are implemented, namely write_nodes(), write_cells() and
 * write_data(). In order to use these functions, a format specific output
 * stream must be written, following the examples of DXStream, GmvStream,
 * VtkStream and so on, implemented in the .cc file. In this framework, the
 * implementation of a new output format is reduced to writing the section
 * headers and the new output stream class for writing a single mesh object.
 * <h3>Credits</h3>   <ul>
 * <li>  EPS output based on an earlier implementation by Stefan Nauber for
 * the old DataOut class
...
namespace DataOutBase
{
  ...
  struct Patch
  {
    ...
    /**
     * Corner points of a patch.  Interior points are computed by a
     * multilinear     transformation of the unit cell to the cell specified
     * by these corner     points, if
     * <code>points_are_available==false</code>  .         On the other hand,
     * if   <code>points_are_available==true</code>  , then     the
     * coordinates of the points at which output is to be generated     is
     * attached in additional rows to the   <code>data</code>   table.
     ...
     * for the OpenDX format that requires neighbor     information for
     * advanced output.
     *
     */
     ...
     * <tt>(subdivisions+1)^{dim}</tt>. <tt>data.n_rows()</tt> equals the
     * number     of data vectors. For the current purpose, a data vector
     * equals one     scalar, even if multiple scalars may later be
     * interpreted as vectors.         Within each column, <tt>data(.,j)</tt>
     * are the data values at the     output point <tt>j</tt>, where
     * <tt>j</tt> denotes the usual     lexicographic ordering in deal.II.
     * This is also the order of points as     provided by the
     * <tt>QIterated</tt> class when used with the     <tt>QTrapezoid</tt>
     * class as subquadrature.         Since the number of data vectors is
     * usually the same for all patches to     be printed,
     * <tt>data.size()</tt> should yield the same value for all     patches
     * provided. The exception are patches for which     points_are_available
     * are set, where the actual coordinates of the point     are appended to
     * the 'data' field, see the documentation of the     points_are_available
     * flag.
     *
     */
    Table<2, float> data;
     ...
     * appended to the   @p data   table (  @p true)   or not (  @p false).
     * The latter     is the default and in this case the locations of the
     * points interior to     this patch are computed by (bi-, tri-)linear
     * interpolation from the     vertices of the patch.         This option
     * exists since patch points may be evaluated using a Mapping     (rather
     * than by a linear interpolation) and therefore have to be stored     in
     * the Patch structure.
     *
     */
   ...
   * in   @p spacedim   dimensional space.     The current class is compatible
   * with the general template to allow for   using the same functions
   * accessing patches of arbitrary dimensionality   in a generic way.
   * However, it makes some variables that are nonsensical   for
   * zero-dimensional patches into   @p static   variables that exist only
   * once in the entire program, as opposed to once per patch. Specifically,
   * this is the case for the   @p neighbors   array and the   @p
   * n_subdivisions     member variable that make no sense for
   * zero-dimensional patches because   points have no natural neighbors
   * across their non-existent faces, nor   can they reasonably be subdivided.
   *
   */
  ...
  struct Patch<0, spacedim>
  {
     ...
     * patches, there is of course only a single vertex.         If
     * <code>points_are_available==true</code>  , then     the coordinates of
     * the point at which output is to be generated     is attached as an
     * additional row to the   <code>data</code>   table.
     *
     */
     ...
     * Number of subdivisions with which this patch is to be written.
     * <tt>1</tt> means no subdivision, <tt>2</tt> means bisection, <tt>3</tt>
     * trisection, etc.         Since subdivision makes no sense for
     * zero-dimensional patches,     this variable is not used but exists only
     * to allow access     from general code in a generic fashion.
     *
     */
     ...
     * Data vectors. The format is as follows: <tt>data(i,.)</tt> denotes the
     * data belonging to the <tt>i</tt>th data vector. <tt>data.n_cols()</tt>
     * therefore equals the number of output points; this number is     of
     * course one for the current class, given that we produce output on
     * points. <tt>data.n_rows()</tt> equals the number of     data vectors.
     * For the current purpose, a data vector equals one scalar,     even if
     * multiple scalars may later be interpreted as vectors.         Within
     * each column, <tt>data(.,j)</tt> are the data values at the     output
     * point <tt>j</tt>; for the current class,   @p j   can only     be zero.
    ...
    /**
     * Default constructor. Sets #points_are_available     to false, and
     * #patch_index to #no_neighbor.
     *
     */
    Patch();
   ...
   * flags.     This is implemented with the "Curiously Recurring Template
   * Pattern";   derived classes use their own type to fill in the typename so
   * that   <tt>memory_consumption</tt> works correctly. See the Wikipedia
   * page on   the pattern for more information.
   * @ingroup output
   *
   */
  ...
  struct OutputFlagsBase
  {
     ...
     * in input files.         This method does nothing, but child classes may
     * override this method to     add fields to <tt>prm</tt>.
     *
     */
    static void
     ...
     * for this output format accordingly.         This method does nothing,
     * but child classes may override this method to     add fields to
     * <tt>prm</tt>.
     *
     */
    void
  ...
  struct DXFlags : public OutputFlagsBase<DXFlags>
  {
    ...
    /**
     * Read the parameters declared in declare_parameters() and set the flags
     * for this output format accordingly.         The flags thus obtained
     * overwrite all previous contents of this object.
     *
     */
    void
    parse_parameters(const ParameterHandler &prm);
  ...
  struct UcdFlags : public OutputFlagsBase<UcdFlags>
  {
     ...
     * using this flag.         Default:   <code>false</code>  .
     *
     */
    bool write_preamble;
  ...
  struct GnuplotFlags : public OutputFlagsBase<GnuplotFlags>
  {
     ...
     * # <x> <t>
     * @endverbatim
     * Any extra labels will be ignored.         If you specify these labels
     * yourself then there should be at least     <tt>spacedim</tt> labels,
     * where <tt>spacedim</tt> is the spatial     dimension of the output
     * data.
     *
     */
  ...
  struct PovrayFlags : public OutputFlagsBase<PovrayFlags>
  {
    ...
    /**
     * Use bicubic patches (b-splines) instead of triangles.         default =
     * false
     *
     */
    bool bicubic_patch;
  ...
  struct EpsFlags : public OutputFlagsBase<EpsFlags>
  {
     ...
     * is specified by the flag <tt>size_type</tt>.         Default is 300,
     * which represents a size of roughly 10 cm.
     *
     */
    unsigned int size;
     ...
     * negative y-axis.         Default is the Gnuplot-default of 30.  An
     * example of a Gnuplot-default     of 0 is the following:
     * @verbatim
     *
     *        3________7
     *        /       /|
     ...
     * the opposite case.         Default is <tt>1.0</tt>.
     *
     */
    double z_scaling;
     ...
     * of each patch) are to be plotted.         Default: <tt>true</tt>.
     *
     */
    bool draw_mesh;
     ...
     * foreground.         If this flag is <tt>false</tt> and #draw_mesh is
     * <tt>false</tt> as     well, nothing will be printed.         If this
     * flag is <tt>true</tt>, then the cells will be drawn either     colored
     * by one of the data sets (if #shade_cells is <tt>true</tt>), or     pure
     * white (if #shade_cells is false or if there are no data sets).
     ...
     * #color_function.         Default is <tt>true</tt>.
     *
     */
    bool shade_cells;
     ...
     * triple of color values in RGB values.         Besides the actual value
     * by which the color is to be computed, min and     max values of the
     * data to be colorized are given as well.
     *
     */
     ...
     * refer to the implementation.         This function was originally
     * written by Stefan Nauber.
     *
     */
    static RgbValues
    ...
    /**
     * Declare all flags with name and type as offered by this class, for use
     * in input files.         For coloring, only the color functions declared
     * in this class are     offered.
     *
     */
    static void
    declare_parameters(ParameterHandler &prm);
  ...
  struct VtkFlags : public OutputFlagsBase<VtkFlags>
  {
     ...
     * simulation.         The value of this variable is written into the
     * output file according to     the instructions provided in
     ...
     * a comment in the file's second line.         Default is <tt>true</tt>.
     *
     */
     ...
     * high-order Lagrange cell.         Default is <tt>false</tt>.
  ...
  struct DataOutFilterFlags
  {
     ...
     * flags for this output format accordingly.         The flags thus
     * obtained overwrite all previous contents of this object.
     *
     */
    void
    parse_parameters(const ParameterHandler &prm);
   ...
   * across faces.     This class is an attempt to rein in the amount of data
   * that is written.   If the fields that are written to files are indeed
   ...
   * writing multiple node   locations for the same vertex and defining data
   * at these nodes). However,   for fine meshes, one may not necessarily be
   * interested in an exact   representation of output fields that will likely
   * only have small   discontinuities. Rather, it may be sufficient to just
   * output one value   per vertex, which may be chosen arbitrarily from among
   * those that are   defined at this vertex from any of the adjacent cells.
   *
   */
  class DataOutFilter
  {
  ...
  private:
    ...
    struct Point3Comp
    {
      ...
      {
        /*         The return statement below is an optimized version of the following         code:                 for (unsigned int d=0; d<3; ++d)        {           if (one(d) < two(d))             return true;           else if (one(d) > two(d))             return false;         }         return false;        
* */
    ...
    /**
     * The number of space dimensions in which the vertices represented     by
     * the current object live. This corresponds to the usual       @p dim
     * argument, but since this class is not templated on the     dimension,
    ...
    /**
     * The number
     * of cells stored in   @ref filtered_cells  .
     *
     */
  ...
  enum OutputFormat
  {
     ...
     * format.           @deprecated   Using Tecplot binary output is
     * deprecated.
     *
     */
    tecplot_binary,
   ...
   * may want to change.)     This function only supports output for
   * two-dimensional domains (i.e.,   with dim==2), with values in the
   * vertical direction taken from a data   vector.     Basically, output
   * consists of the mesh and the cells in between them. You   can draw either
   * of these, or both, or none if you are really interested   in an empty
   * picture. If written, the mesh uses black lines. The cells in   between
   * the mesh are either not printed (this will result in a loss of   hidden
   * line removal, i.e.  you can "see through" the cells to lines   behind),
   * printed in white (which does nothing apart from the hidden line
   ...
   * cell is not used.     By default, the viewpoint is chosen like the
   * default viewpoint in   GNUPLOT, i.e.  with an angle of 60 degrees with
   * respect to the positive   z-axis and rotated 30 degrees in positive sense
   * (as seen from above) away   from the negative y-axis.  Of course you can
   * change these settings.     EPS output is written without a border around
   * the picture, i.e. the   bounding box is close to the output on all four
   * sides. Coordinates are   written using at most five digits, to keep
   * picture size at a reasonable   size.     All parameters along with their
   * default values are listed in the   documentation of the <tt>EpsFlags</tt>
   * member class of this class. See   there for more and detailed
   * information.
   *
   */
  template <int spacedim>
   ...
   * <b>n-2</b>.     The GNUPLOT format is not able to handle data on
   * unstructured grids   directly. Directly would mean that you only give the
   * vertices and the   solution values thereon and the program constructs its
   * own grid to   represent the data. This is only possible for a structured
   * tensor product   grid in two dimensions. However, it is possible to give
   ...
   * continuous at interfaces between   patches, so this method also works for
   * discontinuous elements. Note also,   that GNUPLOT can do hidden line
   * removal for patched data.     While this discussion applies to two
   * spatial dimensions, it is more   complicated in 3d. The reason is that we
   * could still use patches, but it   is difficult when trying to visualize
   * them, since if we use a cut through   the data (by, for example, using x-
   * and z-coordinates, a fixed y-value   and plot function values in
   * z-direction, then the patched data is not a   patch in the sense GNUPLOT
   * wants it any more. Therefore, we use another   approach, namely writing
   * the data on the 3d grid as a sequence of lines,   i.e. two points each
   * associated with one or more data sets.  There are   therefore 12 lines
   * for each subcells of a patch.     Given the lines as described above, a
   * cut through this data in Gnuplot   can then be achieved like this:
   * @verbatim
   * set data style lines
   ...
   * -1)
   * @endverbatim
   * This command plots data in   $x$  - and   $y$  -direction unbounded, but
   * in     $z$  -direction only those data points which are above the   $x$
   * -  $y$  -plane (we   assume here a positive solution, if it has negative
   * values, you might   want to decrease the lower bound). Furthermore, it
   * only takes the data   points with z-values (<tt>&3</tt>) equal to 0.5,
   * i.e. a cut through the   domain at <tt>z=0.5</tt>. For the data points on
   * this plane, the data   values of the first data set (<tt>&4</tt>) are
   * raised in z-direction   above the x-y-plane; all other points are denoted
   * the value <tt>-1</tt>   instead of the value of the data vector and are
   * not plotted due to the   lower bound in z plotting direction, given in
   * the third pair of brackets.     More complex cuts are possible, including
   * nonlinear ones. Note however,   that only those points which are actually
   * on the cut-surface are plotted.
   *
   */
  template <int dim, int spacedim>
  void
  ...
  /**
   * Write the given list of patches to the output stream for the Povray   raytracer.     Output in this format creates a povray source file, include standard   camera and light source definition for rendering with povray 3.1 At   present, this format only supports output for two-dimensional data, with   values in the third direction taken from a data vector.     The output uses two different povray-objects:       <ul>       <li>   <tt>BICUBIC_PATCH</tt> A <tt>bicubic_patch</tt> is a 3-dimensional   Bezier patch. It consists of 16 Points describing the surface. The 4   corner points are touched by the object, while the other 12 points pull   and stretch the patch into shape. One <tt>bicubic_patch</tt> is generated   on each patch. Therefore the number of subdivisions has to be 3 to provide   the patch with 16 points. A bicubic patch is not exact but generates very   smooth images.       <li>   <tt>MESH</tt> The mesh object is used to store large number of   triangles. Every square of the patch data is split into one upper-left   and one lower-right triangle. If the number of subdivisions is three, 32   triangle are generated for every patch.     Using the smooth flag povray interpolates the normals on the triangles,   imitating a curved surface     </ul>       All objects get one texture definition called Tex. This texture has to be   declared somewhere before the object data. This may be in an external   data file or at the beginning of the output file. Setting the   <tt>external_data</tt> flag to false, an standard camera, light and   texture (scaled to fit the scene) is added to the output file. Set to   true an include file "data.inc" is included. This file is not generated   by deal and has to include camera, light and the texture definition Tex.     You need povray (>=3.0) to render the scene. The minimum options for   povray are:
   * @verbatim
   ...
   * format (FEBLOCK).     For more information consult the Tecplot Users and
   * Reference manuals.
   *
   */
  template <int dim, int spacedim>
   ...
   * programs which understand the UCD format.     No use is made of the
   * possibility to give model data since these are not   supported by all UCD
   * aware programs. You may give cell data in derived   classes by setting
   * all values of a given data set on a patch to the same   value.
   *
   */
  template <int dim, int spacedim>
  void
   ...
   * format that write_vtu() produces.     The nonscalar_data_ranges argument
   * denotes ranges of components in the   output that are considered a
   * vector, rather than simply a collection of   scalar fields. The VTK
   * output format has special provisions that allow   these components to be
   * output by a single name rather than having to   group several scalar
   * fields into a vector later on in the visualization   program.
   ...
   * format that write_vtk() produces.     The nonscalar_data_ranges argument
   * denotes ranges of components in the   output that are considered a
   * vector, rather than simply a collection of   scalar fields. The VTK
   * output format has special provisions that allow   these components to be
   * output by a single name rather than having to   group several scalar
   * fields into a vector later on in the visualization   program.     Some
   * visualization programs, such as ParaView, can read several separate   VTU
   * files to parallelize visualization. In that case, you need a
   * <code>.pvtu</code>   file that describes which VTU files form a group.
   * The     DataOutInterface::write_pvtu_record()   function can generate
   ...
   * to look at   DataOutInterface::write_pvd_record()       The use of this
   * function is explained in   step-40  .
   *
   */
  template <int dim, int spacedim>
  void
   ...
   * used internally together with   DataOutInterface::write_vtu_footer()
   * and     DataOutInterface::write_vtu_main()   by
   ...
   * would have to provide to the current function by hand.     In any case,
   * whether this function is called directly or via
   ...
   * function   gets the names and types of fields through the third and
   * fourth   argument; you can determine these by hand, but in practice, this
   * function   is most easily called by calling
   ...
   * parallel set.
   * @note   Use   DataOutBase::write_vtu()   and
   * DataOutInterface::write_vtu()     for writing each piece. Also note that
   ...
   * contains the graphical data for this time instant.     A typical use
   * case, in program that computes a time dependent solution,   would be the
   * following (  <code>time</code> and <code>time_step</code>   are   member
   * variables of the class with types   <code>double</code>   and
   ...
   * @endcode
   *
   * @note   See   DataOutInterface::write_vtu,
   * DataOutInterface::write_pvtu_record,     and
   * DataOutInterface::write_vtu_in_parallel     for writing solutions at each
   * timestep.
   ...
   * this function.     This function is documented in the "Creating a master
   * file for parallel"   section (section 5.7) of the "Getting data into
   * VisIt" report that can be   found here:
   ...
   * each timestep. Here is an example of how the function would be   used:
   * @code
   * const unsigned int number_of_time_steps = 3;
   ...
   * third direction taken from a data vector.     For the output, each patch
   * is subdivided into four triangles which are   then written as polygons
   * and filled with a linear color gradient. The   arising coloring of the
   * patches visualizes the data values at the   vertices taken from the
   * specified data vector. A colorbar can be drawn to   encode the coloring.
   * @note   This function is so far only implemented for two dimensions with
   * an   additional dimension reserved for data information.
   *
   */
  template <int spacedim>
  void
   ...
   * present class to the final graphics format.     Note that the
   * intermediate format is what its name suggests: a direct   representation
   * of internal data. It isn't standardized and will change   whenever we
   * change our internal representation. You can only expect to   process
   * files written in this format using the same version of deal.II   that was
   * used for writing.     The reason why we offer to write out this
   * intermediate format is that it   can be read back into a deal.II program
   * using the DataOutReader class,   which is helpful in at least two
   * contexts: First, this can be used to   later generate graphical output in
   * any other graphics format presently   understood; this way, it is not
   * necessary to know at run-time which   output format is requested, or if
   * multiple output files in different   formats are needed. Secondly, in
   * contrast to almost all other graphics   formats, it is possible to merge
   * several files that contain intermediate   format data, and generate a
   * single output file from it, which may be   again in intermediate format
   * or any of the final formats. This latter   option is most helpful for
   * parallel programs: as demonstrated in the     step-17   example program,
   * it is possible to let only one processor   generate the graphical output
   * for the entire parallel program, but this   can become vastly inefficient
   * if many processors are involved, because   the load is no longer
   * balanced. The way out is to let each processor   generate intermediate
   * graphical output for its chunk of the domain, and   the later merge the
   * different files into one, which is an operation that   is much cheaper
   * than the generation of the intermediate data.     Intermediate format
   * deal.II data is usually stored in files with the   ending <tt>.d2</tt>.
   *
   */
  template <int dim, int spacedim>
  void
  ...
  /**
   * Write the data in   @p data_filter   to a single HDF5 file containing
   * both the   mesh and solution values.
   *
   */
  template <int dim, int spacedim>
  void
  ...
  /**
   * Write the data in   @p data_filter   to HDF5 file(s). If   @p
   * write_mesh_file   is   false, the mesh data will not be written and the
   * solution file will   contain only the solution values. If   @p
   * write_mesh_file   is true and the   filenames are the same, the resulting
   * file will contain both mesh data   and solution values.
   *
   */
  template <int dim, int spacedim>
  void
   ...
   * called, and return them as a pair of values.     Note that this function
   * eats a number of elements at the present position   of the stream, and
   * therefore alters it. In order to read from it using,   for example, the
   * DataOutReader class, you may wish to either reset the   stream to its
   ...
   * string does not match any known format, an exception is thrown.     The
   * main purpose of this function is to allow a program to use any
   * implemented output format without the need to extend the program's parser
   * each time a new format is implemented.     To get a list of presently
   * available format names, e.g. to give it to the   ParameterHandler class,
   * use the function get_output_format_names().
   *
   */
  OutputFormat
  ...
  /**
   * Provide a function which tells us which suffix a file with a given output   format usually has. At present the following formats are defined:     <ul>       <li>   <tt>dx</tt>: <tt>.dx</tt>     <li>   <tt>ucd</tt>: <tt>.inp</tt>     <li>   <tt>gnuplot</tt>: <tt>.gnuplot</tt>     <li>   <tt>povray</tt>: <tt>.pov</tt>     <li>   <tt>eps</tt>: <tt>.eps</tt>     <li>   <tt>gmv</tt>: <tt>.gmv</tt>     <li>   <tt>tecplot</tt>: <tt>.dat</tt>     <li>   <tt>tecplot_binary</tt>: <tt>.plt</tt>     <li>   <tt>vtk</tt>: <tt>.vtk</tt>     <li>   <tt>vtu</tt>: <tt>.vtu</tt>     <li>   <tt>svg</tt>: <tt>.svg</tt>     <li>   <tt>deal_II_intermediate</tt>: <tt>.d2</tt>.     </ul>         @deprecated   Using Tecplot binary output is deprecated.
   *
   */
  std::string
 ...
 * dispatch what output format to chose. This class is thought as a base class
 * to classes actually generating data for output. It has two abstract virtual
 * functions, get_patches() and get_dataset_names() produce the data which is
 * actually needed. These are the only functions that need to be overloaded by
 * a derived class. In addition to that, it has a function for each output
 * format supported by the underlying base class which gets the output data
 * using these two virtual functions and passes them to the raw output
 * functions. The purpose of this class is mainly two-fold: to support storing
 * flags by which the output in the different output formats are controlled,
 * and means to work with output in a way where output format, flags and other
 * things are determined at run time. In addition to that it offers the
 * abstract interface to derived classes briefly discussed above.
 *
 *  <h3>Output flags</h3> The way we treat flags in this class is very similar
 * to that used in the <tt>GridOut</tt> class. For detailed information on the
 * why's and how's, as well as an example of programming, we refer to the
 * documentation of that class. Basically, this class stores a set of flags
 * for each output format supported by the underlying <tt>DataOutBase</tt>
 * class. These are used whenever one of the <tt>write_*</tt> functions is
 * used. By default, the values of these flags are set to reasonable
 * start-ups, but in case you want to change them, you can create a structure
 * holding the flags for one of the output formats and set it using the
 * <tt>set_flags</tt> functions of this class to determine all future output
 * the object might produce by that output format. For information on what
 * parameters are supported by different output functions, please see the
 * documentation of the <tt>DataOutBase</tt> class and its member classes.
 *
 *  <h3>Run time selection of output parameters</h3> In the output flags
 * classes, described above, many flags are defined for output in the
 * different formats. In order to make them available to the input file
 * handler class <tt>ParameterHandler</tt>, each of these has a function
 * declaring these flags to the parameter handler and to read them back from
 * an actual input file. In order to avoid that in user programs these
 * functions have to be called for each available output format and the
 ...
 * them in the flags associated with this object (see above). Using these
 * functions, you do not have to track which formats are presently
 * implemented. Usage is as follows:
 *
 * @code
 ...
 * alike.
 *
 *  <h3>Run time selection of formats</h3> This class, much like the
 * <tt>GridOut</tt> class, has a set of functions providing a list of
 * supported output formats, an <tt>enum</tt> denoting all these and a
 * function to parse a string and return the respective <tt>enum</tt> value if
 * it is a valid output format's name (actually, these functions are inherited
 * from the base class). Finally, there is a function <tt>write</tt>, which
 * takes a value of this <tt>enum</tt> and dispatches to one of the actual
 * <tt>write_*</tt> functions depending on the output format selected by this
 * value. The functions offering the different output format names are,
 ...
 * a new format is implemented. Additionally, objects of this class have a
 * default format, which can be set by the parameter "Output format" of the
 * parameter file. Within a program, this can be changed by the member
 * function <tt>set_default_format</tt>. Using this default format, it is
 * possible to leave the format selection completely to the parameter file. A
 * suitable suffix for the output file name can be obtained by
 * <tt>default_suffix</tt> without arguments.
 *
 *
 * @ingroup output
 *
...
public:
   ...
   * (VTK's XML) format. See   DataOutBase::write_vtu.       Some
   * visualization programs, such as ParaView, can read several separate   VTU
   * files to parallelize visualization. In that case, you need a
   * <code>.pvtu</code>   file that describes which VTU files form a group.
   * The     DataOutInterface::write_pvtu_record()   function can generate
   * such a   centralized record. Likewise,
   * DataOutInterface::write_visit_record()   does   the same for older
   * versions of VisIt (although VisIt can also read     <code>pvtu</code>
   * records since version 2.5.1). Finally,
   ...
   * example, through the   DataOutInterface::write_vtu()   function) form a
   * group.   The current function can generate such a centralized record.
   * The central record file generated by this function   contains a list of
   * (scalar or vector) fields that describes which   fields can actually be
   * found in the individual files that comprise the set   of parallel VTU
   * files along with the names of these files. This function   gets the names
   * and types of fields through the get_dataset_names() and
   ...
   * DataOutInterface::write_pvtu_record().       For example, running
   ...
   * half.     A specified   @p directory   and a   @p
   * filename_without_extension     form the first part of the filename. The
   * filename is then extended with   a   @p counter   labeling the current
   * timestep/iteration/etc., the processor ID,   and finally the .vtu/.pvtu
   * ending. Since the number of timesteps to be   written depends on the
   * application, the number of digits to be reserved in   the filename can be
   * specified as parameter   @p n_digits_for_counter,   and the number   is
   * not padded with leading zeros if this parameter is left at its default
   ...
   * last identifier is used as   @p counter,   while all other identifiers
   * have to be   added to   @p filename_without_extension   when calling this
   * function.     In a   parallel setting, several files are typically
   * written per time step. The   number of files written in parallel depends
   * on the number of MPI processes   (see parameter   @p mpi_communicator),
   * and a   specified number of   @p n_groups   with default value 0. The
   * background is that   VTU file output supports grouping files from several
   * CPUs into a given   number of files using MPI I/O when writing on a
   * parallel filesystem. The   default value of   @p n_groups   is 0, meaning
   * that every MPI rank will write one   file. A value of 1 will generate one
   * big file containing the solution over   the whole domain, while a larger
   * value will create   @p n_groups   files (but not   more than there are
   * MPI ranks).     Note that only one processor needs to   generate the
   * .pvtu file, where processor zero is chosen to take over this   job.
   * The return value is the filename of the centralized file for the pvtu
   * record.
   * @note   The code simply combines the strings   @p directory   and     @p
   * filename_without_extension,   i.e., the user has to make sure that     @p
   * directory   contains a trailing character, e.g. "/", that separates the
  ...
  /**
   * Write the data in   @p data_filter   to a single HDF5 file containing
   * both the   mesh and solution values. Below is an example of how to use
   * this function   with the DataOutFilter:
   * @code
   * DataOutBase::DataOutFilterFlags flags(true, true);
   * DataOutBase::DataOutFilter data_filter(flags);
   * // Filter the data and store it in data_filter
   ...
   * format.     Some of the declared subsections may not contain entries, if
   * the   respective format does not export any flags.     Note that the
   * top-level parameters denoting the number of subdivisions   per patch and
   * the output format are not declared, since they are only   passed to
   * virtual functions and are not stored inside objects of this   type. You
   * have to declare them yourself.
   *
   */
  static void
  declare_parameters(ParameterHandler &prm);
  ...
  /**
   * Read the parameters declared in declare_parameters() and set the flags
   * for the output formats accordingly.     The flags thus obtained overwrite
   * all previous contents of the flag   objects as default-constructed or set
...
protected:
   ...
   * interpreted.     It returns a list of index pairs and corresponding name
   * and type indicating   which components of the output are to be considered
   * vector- or   tensor-valued rather than just a collection of scalar data.
   * The index pairs   are inclusive; for example, if we have a Stokes problem
   * in 2d with   components (u,v,p), then the corresponding vector data range
   * should be   (0,1), and the returned list would consist of only a single
   * element with a   tuple such as
   * (0,1,"velocity",component_is_part_of_vector).     Since some of the
   * derived classes do not know about non-scalar data, this   function has a
   * default implementation that simply returns an empty   string, meaning
   * that all data is to be considered a collection of scalar   fields.
   *
   */
  virtual std::vector<
    std::tuple<unsigned int,
 ...
 * graphics formats. This class has two main purposes: The first use of this
 * class is so that application programs can defer the decision of which
 * graphics format to use until after the program has been run. The data is
 * written in intermediate format into a file, and later on it can then be
 * converted into any graphics format you wish. This may be useful, for
 * example, if you want to convert it to gnuplot format to get a quick glimpse
 * and later on want to convert it to OpenDX format as well to get a high
 * quality version of the data. The present class allows to read this
 * intermediate format back into the program, and allows it to be written in
 * any other supported format using the relevant functions of the base class.
 ...
 * written out in any of the supported established graphics formats. This
 * second use scenario is explained in some detail in the   step-18   example
 * program. In order to read data back into this object, you have to know the
 * template parameters for the space dimension which were used when writing
 * the data. If this knowledge is available at compile time, then this is no
 ...
 * DataOutBase::determine_intermediate_format_dimensions()   function. Note
 * that the intermediate format is what its name suggests: a direct
 * representation of internal data. It isn't standardized and will change
 * whenever we change our internal representation. You can only expect to
...
public:
   ...
   * cells.     For this to work, the input files for the present object and
   * the given   argument need to have the same number of output vectors, and
   * they need to   use the same number of subdivisions per patch. The output
   * will probably   look rather funny if patches in both objects overlap in
   * space.     If you call read() for this object after merging in patches,
   * the previous   state is overwritten, and the merged-in patches are lost.
...
protected:
   ...
   * DataOutBase) to the actual output function.     It returns the patches as
   * read the last time a stream was given to the   read() function.
   *
   */
  ...
  /**
   * Abstract virtual function through which the names of data sets are
   * obtained by the output functions of the base class.     Return the names
   * of the variables as read the last time we read a file.
   *
   */
  virtual std::vector<std::string>
   ...
   * vector-valued rather than   just a collection of scalar data. The index
   * pairs are inclusive; for   example, if we have a Stokes problem in 2d
   * with components (u,v,p), then   the corresponding vector data range
   * should be (0,1), and the returned   list would consist of only a single
   * element with a tuple such as   (0,1,"velocity").     Since some of the
   * derived classes do not know about vector data, this   function has a
   * default implementation that simply returns an empty   string, meaning
   * that all data is to be considered a collection of scalar   fields.
   *
   */
...
/**
 * A class to store relevant data to use when writing a lightweight XDMF file.
 * The XDMF file in turn points to heavy data files (such as HDF5) where the
 * actual simulation data is stored. This allows flexibility in arranging the
 * data, and also allows the mesh to be separated from the point data.
 *
 *
 */
class XDMFEntry
...
public:
  ...
  /**
   * Simplified constructor that calls the complete constructor for   cases
   * where   <code>solution_filename == mesh_filename</code>  , and
  ...
  /**
   * Get the XDMF content associated with this entry.   If the entry is not
   * valid, this returns an empty string.       @deprecated   Use the overload
   * taking an `unsigned int` and a   `const ReferenceCell &` instead.
   *
   */
  DEAL_II_DEPRECATED
  std::string
  ...
  /**
   * Get the XDMF content associated with this entry.   If the entry is not
   * valid, this returns an empty string.
   *
   */
  std::string
...
private:
  ...
  /**
   * The dimension of the space the data lives in.   Note that dimension <=
   * space_dimension.
   *
   */
...
namespace DataOutBase
{
  ...
  /**
   * Output operator for an object of type   <tt>DataOutBase::Patch</tt>.
   * This   operator dumps the intermediate graphics format represented by the
   * patch   data structure. It may later be converted into regular formats
   * for a   number of graphics programs.
   *
   */
  template <int dim, int spacedim>
  std::ostream &
  ...
  /**
   * Input operator for an object of type   <tt>DataOutBase::Patch</tt>.
   * This   operator reads the intermediate graphics format represented by the
   * patch   data structure, using the format in which it was written using
   * the   operator<<.
   *
   */
  template <int dim, int spacedim>
