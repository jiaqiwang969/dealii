 ...
 * objects (of arbitrary rank). The rationale to implement such functionality
 * in a generic fashion in a separate namespace is
 *
 *
 *
 *
 ...
 * dealii::Tensor<rank,   dim, Number> and   dealii::SymmetricTensor<rank,
 * dim, Number> The methods and algorithms implemented in this namespace,
 * however, are fully generic. More precisely, it can operate on nested
 * c-style arrays, or on class types   <code>T</code>   with a minimal
 * interface that provides a local alias   <code>value_type</code>   and an
 * index operator   <code>operator[](unsigned int)</code>   that returns a
 * (const or non-const) reference of   <code>value_type</code>  :
 *
 * @code
 * template <...>
 * class T
...
namespace TensorAccessors
{
  ...
  /**
   * This class provides a local alias   @p value_type   denoting the
   ...
   *
   *
   *  -   <code>T::value_type</code>   if T is a tensorial class providing an   alias   <code>value_type</code>   and does not have a const qualifier.
   *
   *
   *
   *
   ...
   *
   *
   *  -   <code>const T::value_type</code>   if T is a tensorial class   providing an alias   <code>value_type</code>   and does have a const   qualifier.
   *
   *
   *
   *
   ...
   *
   *
   *  -   <code>const A</code> if T is of array type <code>A[...]</code>   and   does have a const qualifier.
   *
   */
  template <typename T>
  struct ValueType
   ...
   * rank     @p rank   in which the index   @p index   is shifted to the end.
   * As an example   consider a tensor of 5th order in dim=5 space dimensions
   * that can be   accessed through 5 recursive   <code>operator[]()</code>
   * invocations:
   * @code
   * Tensor<5, dim> tensor;
   * tensor[0][1][2][3][4] = 42.;
   ...
   * fairly general tensorial types   @p T.       The purpose of this
   * reordering facility is to be able to contract over an   arbitrary index
   * of two (or more) tensors:
   *
   *
   *
   *
   ...
   * @tparam   rank Rank of the tensorial object   @p t       @tparam   T A
   * tensorial object of rank   @p rank.     @p T   must provide a local
   ...
   * object   @p t   of type   @p T,   as described by an array type   @p
   * ArrayType     object   @p indices.   For example:
   * @code
   * Tensor<5, dim> tensor;
   ...
   * @tparam   T A tensorial object of rank   @p rank.     @p T   must provide
   * a local   alias   <code>value_type</code>   and an index operator
   * <code>operator[]()</code>   that returns a (const or non-const) reference
   * of   <code>value_type</code>  . Further, its tensorial rank must be equal
   * or   greater than   @p rank.         @tparam   ArrayType An array like
   * object, such as   std::array,   or     dealii::TableIndices    that
   * stores at least   @p rank   indices that can be   accessed via
   * operator[]().
   *
   */
  ...
  /**
   * This function contracts two tensorial objects   @p left   and   @p right   and   stores the result in   @p result.   The contraction is done over the _last_     @p no_contr   indices of both tensorial objects:     @f[
   ...
   * @f]     Calling this function is equivalent of writing the following low level   code:
   * @code
  ...
  /**
   * Full contraction of three tensorial objects:     @f[
   ...
   * @endcode
   *
   * @note   The Types   @p T2,     @p T3,   and   @p T4   must have rank
   * rank_1, rank_1 +   rank_2, and rank_3, respectively.   @p T1   must be a
   * scalar type.
   *
   */
  template <int rank_1,
  ...
  namespace internal
  {
     ...
     * function reordered_index_view(T &t).         The problem is that when
     * working with the actual tensorial types, we     have to return
     * subtensors by reference
     *
