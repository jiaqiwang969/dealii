 ...
 * provide the gradient function himself. The following example of an user
 * defined function overloads and implements only the value() function but not
 * the gradient() function. If the gradient() function is invoked then the
 * gradient function implemented by the AutoDerivativeFunction is called,
 ...
 * course, the users gradient function is called. Note, that the usage of the
 * value() and gradient() functions explained above, also applies to the
 * value_list() and gradient_list() functions as well as to the vector valued
 * versions of these functions, see e.g. vector_value(), vector_gradient(),
 * vector_value_list() and vector_gradient_list(). The gradient() and
 * gradient_list() functions make use of the   Function::value()   function.
 * The vector_gradient() and vector_gradient_list() make use of the
 * Function::vector_value()   function. Make sure that the user defined
 * function implements the value() function and the vector_value() function,
 * respectively. Furthermore note, that an object of this class does
 * <b>not</b> represent the derivative of a function, like FunctionDerivative,
 * that gives a directional derivative by calling the value() function. In
 * fact, this class (the AutoDerivativeFunction class) can substitute the
 * Function class as base class for user defined classes. This class
 * implements the gradient() functions for automatic computation of numerical
 * difference quotients and serves as intermediate class between the base
 * Function class and the user defined function class.
 *
 *
 * @ingroup functions
 *
...
public:
  ...
  {
    /**
     * The symmetric Euler formula of second order:     @f[
     * u'(t) \approx
     * \frac{u(t+h)
     *
     * -
    ...
    Euler,
    /**
     * The upwind Euler formula of first order:     @f[
     * u'(t) \approx
     * \frac{u(t)
     *
     * -
   ...
   * furthermore does not vary to much.     <tt>h</tt> can be changed later
   * using the set_h() function.     Sets DifferenceFormula <tt>formula</tt>
   * to the default <tt>Euler</tt>   formula of the set_formula() function.
   * Change this preset formula by   calling the set_formula() function.
   *
   */
   ...
   * given point.     Compute numerical difference quotients using the preset
   * #DifferenceFormula.
   *
   */
   ...
   * array.     Compute numerical difference quotients using the preset
   * #DifferenceFormula.
   *
   */
  virtual void
   ...
   * the function.     Compute numerical difference quotients using the preset
   * #DifferenceFormula.
   *
   */
  virtual void
