...
#  endif
   ...
   * that accesses the object.     This class offers ways so that every thread
   * that accesses it has its own   copy of an object of type T. In essence,
   * accessing this object can never   result in race conditions in
   * multithreaded programs since no other thread   than the current one can
   * ever access it.     <h3>Construction and destruction</h3>     Objects of
   * this class can either be default constructed or by providing   an
   * "exemplar", i.e. an object of type T so that every time we need to
   * create a T on a thread that doesn't already have such an object, it is
   * copied from the exemplar.     Upon destruction of objects of this class,
   * all T objects that correspond   to threads that have accessed this object
   * are destroyed. Note that this   may be before the time when a thread is
   * terminated.     <h3>Access</h3>     The T object stored by this object
   * can be accessed using the get()   function. It provides a reference to a
   * unique object when accessed from   different threads. Objects of type T
   * are created lazily, i.e. they are   only created whenever a thread
   * actually calls get().
   *
   */
  template <typename T>
  ...
  public:
     ...
     * thread this function is called on.         Note that there is no member
     * function get() that is const and returns a     const reference as one
     * would expect. The reason is that if such a     member function were
     * called on a thread for which no thread-local     object has been
     * created yet, then one has to create such an object     first which
     * would certainly be a non-constant operation. If you need to     call
     * the get() function for a member variable of a class from a const
     ...
     * ThreadLocalStorage object look     more like the object it represents
     * on the current thread.           @param   t The object to be copied
     * into the storage space used for the     current thread.
     ...
     * the current thread. Move assignment operator.           @param   t The
     * object to be copied into the storage space used for the     current
     * thread.           @return   The current object, after the changes have
     * been made
     *
     */
    ThreadLocalStorage<T> &
     ...
     * be created.         If deal.II has been configured to not use
     * multithreading, then this     function does not do anything at all.
     * Note that this of course has     different semantics as in the
     * multithreading context the objects are     deleted and created again
     * (possible by copying from a sample object, if     the appropriate
     * constructor of this class was called), whereas in the     multithreaded
     * context the object is simply not touched at all. At the     same time,
     * the purpose of this function is to release memory other     threads may
     * have allocated for their own thread local objects after     which every
     * use of this object will require some kind of     initialization. This
     * is necessary both in the multithreaded or non-     multithreaded case.
     *
     */
    void
    clear();
  ...
  private:
    ...
    /**
     * A mutex to guard insertion into the data object.         We use a
     * std::shared_timed_mutex   (or   std::shared_mutex   if available)
     * here to be able to use   std::unique_lock   and   std::shared_lock
     * for a     readers-writer lock
...
#    ifndef DOXYGEN
  namespace internal
  {
    /*     We have to make sure not to call "data.emplace(id,exemplar)" if     the corresponding element is not copy constructible. We use some     SFINAE magic to work around the fact that C++14 does not have     "if constexpr".    
* */
    template <typename T>
