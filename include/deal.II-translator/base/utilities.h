...
namespace Utilities
{
   ...
   * identifies the version of deal.II you are using. This information   is
   * also provided by the DEAL_II_PACKAGE_NAME and   DEAL_II_PACKAGE_VERSION
  ...
  /**
   * Assign to each point in   @p points   an index using the Hilbert space
   * filling curve.   To that end, a bounding box for   @p points   will be
   * determined, based on which their   integer coordinates are calculated.
   * The linear index is given as a dim-collection of bits, from high to low.
   * This is done in order to keep the maximum resolution in terms of bit
   * depth   along each axis. Note that this dim-integer index can still be
   * easily used   for sorting and ordering, for example using the
   * lexicographic ordering of   tuples of integers.     The depth of the
   * Hilbert curve (i.e. the number of bits per dimension) by   default is
   * equal to   <code>64</code>  .
   * @note   This function can also handle degenerate cases, e.g. when the
   * bounding   box has zero size in one of the dimensions.
   *
   */
  template <int dim, typename Number>
  ...
  /**
   * Pack the least significant   @p bits_per_dim   bits from each element of
   * @p index     (starting from last) into a single unsigned integer. The
   * last element   of   @p index   will be used to set the first   @p
   * bits_per_dim   bits in the   resulting integer, the second to last
   * element is used to set the next   @p bits_per_dim   bits,   etc.. To fit
   * all the data into the output, the following should hold
   * <code>bits\_per\_dim dim <= 64</code>  .     The function is useful in
   * debugging and visualization of indices returned   by
   * inverse_Hilbert_space_filling_curve().
   ...
   * compressed input.     If the library was not configured with ZLIB
   * enabled, the returned string   is identical to the input string.
   * @param[in]   input The string to compress       @return   A compressed
  ...
  /**
   * If the library is configured with ZLIB, then this function assumes that
   * the   input string has been compressed using the compress() function, and
   * returns   the original decompressed string.     If the library was not
   * configured with ZLIB enabled, the returned string   is identical to the
   * input string.       @param[in]   compressed_input A compressed string, as
   * returned by the   function compress()       @return   The original
   * uncompressed string.
   *
   */
  std::string
  ...
  /**
   * Encodes the binary input as a base64 string.     Base64 is a group of
   * binary-to-text encoding schemes that represent binary   data in an ASCII
   * string format by translating it into a radix-64   representation. Base64
   * is designed to carry data stored in binary formats   across channels that
   * only reliably support text content. It is used also   to store binary
   * formats in a machine independent way.       @param   binary_input A
   * vector of characters, representing your input as   binary data.
  ...
  /**
   * Decodes a base64 string into a binary output.     This is the inverse of
   * the encode_base64() function above.       @param   base64_input A string
   * that contains the input in base64 format.     @return   A vector of
   * characters that represents your input as binary data.
   *
   */
  ...
  /**
   * Convert a number   @p value   to a string, with as many digits as given
   * to   fill with leading zeros.     If the second parameter is left at its
   * default value, the number is not   padded with leading zeros. The result
   * is then the same as if the standard   C++   `std::to_string`   (or the
   * older C function `itoa()`) had been called.     This function takes an
   * `unsigned int` as argument. As a consequence,   if you call it with a
   * `signed int` (which is of course the same   type as `int`), the argument
   * is implicitly converted to   unsigned integers and negative numbers may
   * not be printed as you had   hoped. Similarly, if you call the function
   * with a `long int`, the   printed result might show the effects of an
   * overflow upon conversion   to `unsigned int`.
   ...
   * Therefore the total number of padding zeros is   @p digits   minus any
   * signs,   decimal points and digits of   @p value.       If the second
   * parameter is left at its default value, the number is not   padded with
   * leading zeros. The result is then the same as if the C++   function
   * `std::to_string()`   had been called (for integral types),   or if
   * `boost::lexical_cast()`   had been called (for all other types).
   *
   */
   ...
   * after   @p n_digits   of accuracy, i.e., after   @p n_digits   decimal
   * places   in scientific floating point notation. When interpreted as
   * rounding   operation, this function reduces the absolute value of a
   * floating point   number and always rounds towards zero, since decimal
   * places are simply   cut off.
   *
   */
  template <typename Number>
  Number
  ...
  /**
   * Given a string that contains text separated by a   @p delimiter,   split
   * it   into its components; for each component, remove leading and trailing
   * spaces. The default value of the delimiter is a comma, so that the
   * function splits comma separated lists of strings.     To make data input
   * from tables simpler, if the input string ends in a   delimiter (possibly
   * followed by an arbitrary amount of whitespace), then   this last
   * delimiter is ignored. For example,
   * @code
   * Utilities::split_string_list("abc; def; ghi; ", ';');
   * @endcode
   * yields the same 3-element list of output
   * <code>{"abc","def","ghi"}</code>     as you would get if the input had
   * been
   * @code
   * Utilities::split_string_list("abc; def; ghi", ';');
   * @endcode
   ...
   * single element is the empty string.     This function can digest the case
   * that the delimiter is a space. In this   case, it returns all words in
   * the string. Combined with the rules above,   this implies that
   * @code
   * Utilities::split_string_list("abc def ghi ", ' ');
   * @endcode
   * yields again the 3-element list of output
   * <code>{"abc","def","ghi"}</code>   from above despite the presence of
   * space   at the end of the string. Furthermore,
   * @code
   * Utilities::split_string_list("      ", ' ');
   * @endcode
   ...
   * breaking at positions marked by   @p delimiter   in the text. If this is
   * not   possible, return the shortest lines that are longer than   @p
   * width.    The   default value of the delimiter is a space character. If
   * original_text   contains newline characters (\n), the string is split at
   * these locations,   too.
   *
   */
  std::vector<std::string>
  ...
  /**
   * Read a (signed) integer starting at the position in   @p name   indicated
   * by   the second argument, and return this integer as a pair together with
   * how   many characters it takes up in the string.     If no integer can be
   * read at the indicated position, return
  ...
  /**
   * Return a string with all occurrences of   @p from   in   @p input
   * replaced by     @p to.
   *
   */
  std::string
   ...
   * distribution centered around   @p a   and with standard deviation   @p
   * sigma.     The returned number will be different every time the function
   * is called.     This function is reentrant, i.e., it can safely be called
   * from multiple   threads at the same time. In addition, each thread will
   * get the same   sequence of numbers every time. On the other hand, if you
   * run     Threads::Task   objects via the Threading Building Blocks, then
   * tasks will   be assigned to mostly random threads, and may get a
   * different sequence of   random numbers in different runs of the program,
   * since a previous task   may already have consumed the first few random
   * numbers generated for the   thread you're on. If this is a problem, you
   * need to create your own   random number generator objects every time you
   * want to start from a   defined point.
   ...
   * really verify statistical properties of a code. For `rand()`, you can
   * call   `srand()` to "seed" the random number generator to get different
   * sequences   of random numbers every time a program is called. However,
   * this function   does not allow seeding the random number generator. If
   * you need this, as   above, use one of the C++ or BOOST facilities.
   *
   */
  double
  ...
  /**
   * Return a string description of the type of the variable   @p t.       In
   * general, C++ uses mangled names to identify types. This function   uses
   * boost::core::demangle   to return a human readable string describing
   ...
   * <code>t^N</code> where <code>N</code>   is a known number at compile
   * time.     Use this function as in   <code>fixed_power@<dim@> (n)</code>
   * .
   *
   */
   ...
   * calculations for constant expression arguments. The   @p base   must   be
   * an integer type and the exponent   @p iexp   must not be negative.
   *
   */
  template <typename T>
  constexpr T
  ...
  /**
   * Optimized replacement for   <tt>std::lower_bound</tt>   for searching
   * within   the range of column indices. Slashes execution time by
   * approximately one   half for the present application, partly because the
   * binary search is   replaced by a linear search for small loop lengths.
   * Another reason for this function is rather obscure: when using the GCC
   * libstdc++ function   std::lower_bound,   complexity is O(log(N)) as
   * required.   However, when using the debug version of the GCC libstdc++ as
   * we do when   running the testsuite, then   std::lower_bound   tests
   * whether the sequence   is in fact partitioned with respect to the pivot
   * 'value' (i.e. in essence   that the sequence is sorted as required for
   * binary search to work).   However, verifying this means that the
   * complexity of   std::lower_bound     jumps to O(N); we call this function
   * O(N) times below, making the overall   complexity O(N**2). The
   * consequence is that a few tests with big meshes   completely run off the
   * wall time limit for tests and fail with the   libstdc++ debug mode
  ...
  /**
   * Given a permutation vector (i.e. a vector   $p_0\ldots p_{N-1}$   where
   * each     $p_i\in [0,N)$   and   $p_i\neq p_j$   for   $i\neq j$  ),
   * produce the reverse   permutation   $q_i=N-1-p_i$  .
   *
   */
   ...
   * Given a permutation vector (i.e. a vector   $p_0\ldots p_{N-1}$   where
   * each     $p_i\in [0,N)$   and   $p_i\neq p_j$   for   $i\neq j$  ),
   * produce the inverse   permutation   $q_0\ldots q_{N-1}$   so that
   * $q_{p_i}=p_{q_i}=i$  .
   *
   */
  template <typename Integer>
  ...
  /**
   * Given an arbitrary object of type T, use   boost::serialization
   * utilities   to pack the object into a vector of characters and append it
   * to the   given buffer. The number of elements that have been added to the
   * buffer   will be returned. The object can be unpacked using the
   * Utilities::unpack     function below.     If the library has been
   * compiled with ZLIB enabled, then the output buffer   can be compressed.
   * This can be triggered with the parameter     @p allow_compression,   and
   * is only of effect if ZLIB is enabled.     If many consecutive calls with
   * the same buffer are considered, it is   recommended for reasons of
   * performance to ensure that its capacity is   sufficient.
   *
   */
  template <typename T>
  size_t
   ...
   * above mentioned pack function.     If the library has been compiled with
   * ZLIB enabled, then the output buffer   can be compressed. This can be
   * triggered with the parameter     @p allow_compression,   and is only of
   * effect if ZLIB is enabled.
   *
   */
  template <typename T>
  std::vector<char>
   ...
   * Utilities::pack,   restore its content in an object of type T.     This
   * function uses   boost::serialization   utilities to unpack the object
   * from a vector of characters, and it is the inverse of the function
   * Utilities::pack().       The   @p allow_compression   parameter denotes
   * if the buffer to   read from could have been previously compressed with
   * ZLIB, and   is only of effect if ZLIB is enabled.
  ...
  /**
   * Same unpack function as above, but takes constant iterators on   (a
   * fraction of) a given packed buffer of type   std::vector<char>   instead.
   * The   @p allow_compression   parameter denotes if the buffer to   read
   * from could have been previously compressed with ZLIB, and   is only of
   * effect if ZLIB is enabled.
   *
   */
  template <typename T>
  ...
  /**
   * Given a vector of characters, obtained through a call to the function
   * Utilities::pack,   restore its content in an array of type T.     This
   * function uses   boost::serialization   utilities to unpack the object
   * from a vector of characters, and it is the inverse of the function
   * Utilities::pack().       The   @p allow_compression   parameter denotes
   * if the buffer to   read from could have been previously compressed with
   * ZLIB, and   is only of effect if ZLIB is enabled.
   * @note   This function exists due to a quirk of C++. Specifically,    if
   * you want to pack() or unpack() arrays of objects, then the    following
   * works:
   * @code
   *  double array[3] = {1,2,3};
   *  std::vector<char> buffer = Utilities::pack(array);
   ...
   *  array = Utilities::unpack<double[3]>(buffer);
   * @endcode
   * This is because C++ does not allow functions to return arrays.    The
   * current function therefore allows to write
   * @code
   *  Utilities::unpack(buffer, array);
   * @endcode
   * Note that unlike the other unpack() function, it is not necessary    to
   * explicitly specify the template arguments since they can be    deduced
   ...
   * @endcode
   *
   * @note   This function does not try to convert the `Deleter` objects
   * stored     by   `std::unique_ptr`   objects. The function therefore only
   * works if the     deleter objects are at their defaults, i.e., if they are
   * of type       `std::default_delete<To>`   and
   * `std::default_delete<From>`.
   *
   */
  ...
  namespace System
  {
     ...
     * list of possible return values is:           <table> <tr>
     * <td><tt>VECTORIZATION_LEVEL</tt></td> <td>Return Value</td> <td>Width
     * in bits</td> </tr> <tr> <td>0</td> <td>disabled</td> <td>64</td> </tr>
     * <tr> <td>1</td> <td>SSE2/AltiVec</td> <td>128</td> </tr> <tr>
     * <td>2</td> <td>AVX</td> <td>256</td> </tr> <tr> <td>3</td>
     * <td>AVX512</td> <td>512</td> </tr> </table>
     *
     */
    const std::string
    ...
    struct MemoryStats
    {
      ...
      /**
       * Peak resident memory size in kB. Also known as "high water mark"
       * (HWM).
       *
       */
     ...
     * call point to the allocated memory.       @param   alignment The
     * minimal alignment of the memory block, in bytes.       @param   size
  ...
  namespace Trilinos
  {
     ...
     * Epetra_Maps.         If deal.II has been configured to use a compiler
     * that does not support     MPI then the resulting communicator will be a
     * serial one. Otherwise,     the communicator will correspond to
     * MPI_COMM_WORLD, i.e. a communicator     that encompasses all processes
     * within this MPI universe.
     *
     */
    const Epetra_Comm &
    comm_world();
     ...
     * that does not support     MPI then the resulting communicator will be a
     * serial one. Otherwise,     the communicator will correspond to
     * MPI_COMM_SELF, i.e. a communicator     that comprises only this one
     * processor.
     *
     */
    const Epetra_Comm &
    ...
    /**
     * Return a   Teuchos::Comm   object needed for creation of
     * Tpetra::Maps.           If deal.II has been configured to use a
     * compiler that does not support     MPI then the resulting communicator
     * will be a serial one. Otherwise,     the communicator will correspond
     * to MPI_COMM_SELF, i.e. a communicator     that comprises only this one
     * processor.
     *
     */
     ...
     * destroyed using the destroy_communicator() function.         This
     * facility is used to separate streams of communication. For     example,
     * a program could simply use MPI_Comm_World for everything. But     it is
     * easy to come up with scenarios where sometimes not all processors
     * participate in a communication that is intended to be global
     *
     ...
     * comm_self().         It is necessary to call this function at the time
     * when the result of     duplicate_communicator() is no longer needed.
     * The reason is that in     that function, we first create a new MPI_Comm
     * object and then create an     Epetra_Comm around it. While we can take
     * care of destroying the latter,     it doesn't destroy the communicator
     * since it can only assume that it     may also be still used by other
     * objects in the program. Consequently,     we have to take care of
     * destroying it ourselves, explicitly.         This function does exactly
     * that. Because this has to happen while the     Epetra_Comm object is
     * still around, it first resets the latter and then     destroys the
     * communicator object.
     ...
     * processors in the same way, but using a separate channel.         This
     * function is typically used with a communicator that has been
