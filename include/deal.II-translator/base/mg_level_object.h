 ...
 * multilevel hierarchy, for example for use in the multigrid algorithms. In
 * contrast to just a generic   <code>std::vector</code>  , this class allows
 * to store objects only between some minimal and maximal index (=level), as
 * one often wants to run a multilevel algorithm only on a subset of the
 * levels of a mesh (e.g., because the second or third coarsest level is
 ...
 * for these "interesting" levels, the class allows indexing simply by level.
 * Internally, this is of course done by simply shifting the given index by
 * the minimum level we have stored. In a typical use case for this class, the
 * objects stored on each level are either matrices or vectors.
 *
 *
 * @ingroup mg
 *
...
public:
  /**
   * Constructor. Create a multilevel object with given minimal and   maximal
   * level, and allocate storage for objects on
   ...
   * an <i>inclusive</i> range of levels for which to allocate storage   for
   * level objects. Consequently, the defaults for the two arguments   will
   * create an array with one level object, rather than an empty   array.
   * @param[in]   minlevel The lowest level for which to provision memory
   * for level objects.     @param[in]   maxlevel The highest level for which
   * to provision memory     for level objects.     @param[in]   args Optional
   * arguments passed to the constructor of the     underlying object.
  ...
  /**
   * Access object on level   @p level.       This function can be called on a
   * @p const   object, and   consequently returns a   @p const   reference.
   *
   */
   ...
   * which to provision memory     for level objects.     @param[in]   args
   * Optional arguments passed to the constructor of the     underlying
   * object.       @pre   minlevel <= maxlevel
   *
   */
  template <class... Args>
  void
   ...
   * clearly requires that the objects stored on each level allow for   this
   * operation. This is, in particular, true for vectors and matrices   if
   * @p d   is zero, thereby zeroing out all vector or matrix entries.
   *
   */
   ...
   * is only implemented for some   @p Object   classes, e.g., matrix   types
   * or the PreconditionBlockSOR and similar classes. Using this   function
   * will fail with a compiler error if the   @p Object     template type to
   * this class does not provide a     <code>clear()</code>   member function.
   *
   */
  void
  clear_elements();
   ...
   * the syntax   <code>     action(const unsigned int level, Object &object);
   * </code>   This means this function can accept a lambda, a
   * std::function,   or a plain   function pointer.
   *
   */
