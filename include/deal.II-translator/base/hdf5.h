...
// clang-format off
/**
 * Namespace containing deal.II's HDF5 interface. The [Hierarchical Data
 * Format (HDF)](https://www.hdfgroup.org/) is a cross platform and a high I/O
 * performance format designed to store large amounts of data. It supports
 * serial and MPI I/O access. This set of classes provides an interface to the
 * [HDF5 library](https://www.hdfgroup.org/downloads/hdf5/). The tutorial
 * step-62   shows how to use deal.II's HDF5 interface. # Groups, Datasets and
 * attributes An HDF5 file is organized in
 ...
 * Groups can contain datasets and other groups. Datasets are objects composed
 * by a collection of data elements. Datasets are equivalent to tensors and
 * matrices. In addition, attributes can be attached to the root file, a group
 * or a dataset. An [HDF5
 ...
 * HDF5Object::set_attribute()   can be used to get and set attributes. An
 ...
 * @endcode
 *
 * # MPI I/O An HDF5 file can be opened/created with serial (one single
 * process) or MPI support (several processes access the same HDF5 file).
 * File::File(const     std::string   &, const FileAccessMode) opens/creates
 * an HDF5 file for serial operations.   File::File(const     std::string   &,
 * const FileAccessMode, const MPI_Comm &) creates or opens an HDF5 file in
 * parallel using MPI. The HDF5 calls that modify the structure of the file
 * are always collective, whereas writing and reading raw data in a dataset
 * can be done independently or collectively. [Collective access is usually
 * faster](https://www.hdfgroup.org/2015/08/parallel-io-with-hdf5/) since it
 * allows MPI to do optimizations. In the deal.II's HDF5 interface all the
 * calls are set to collective in order to maximize the performance. This
 * means that all the MPI processes have to contribute to every single call,
 * even if they don't have data to write. MPI HDF5 requires that deal.II and
 * HDF5 have been compiled with MPI support. ## Write a hyperslab in parallel
 ...
 * or blocks in a datataset. Hyperslabs are equivalent to python numpy and
 * h5py
 ...
 * and Data Transfer</a>  section in the HDF5 User's Guide. See as well the <a
 * href="https://support.hdfgroup.org/HDF5/doc1.8/RM/RM_H5S.html#Dataspace-SelectHyperslab">H5Sselect_hyperslab
 * definition</a>. The example below shows how to write a simple rectangular
 * hyperslab. The offset defines the origin of the hyperslab in the original
 * dataset. The dimensions of the hyperslab are `hyperslab_dimensions ={2,
 * 5}`. Note that each process can write a hyperslab with a different size. If
 * a process does not write any data at all, the process should call the
 * function   DataSet::write_none()   because the operation iscollective* and
 ...
 * @endcode
 *
 * The function   DataSet::write_hyperslab(const   Container &,const
 * std::vector<hsize_t>   &, const   std::vector<hsize_t>   &) is used to
 * write simple hyperslabs and the function   DataSet::write_hyperslab(const
 * Container &,const   std::vector<hsize_t>   &, const   std::vector<hsize_t>
 * &, const   std::vector<hsize_t>   &, const   std::vector<hsize_t>   &,
 * const   std::vector<hsize_t>   &) is used to write complex hyperslabs. ##
 * Write unordered data in parallel The example below shows how to write a
 * selection of data. Note that each process can write a different amount of
 * data. If a process does not write any data at all, the process should call
 * the function   DataSet::write_none()   because the operation iscollective*
 * and all the MPI processes have to contribute to the call, even if they
 * don't have data to write. A more detailed example can be found in   step-62
 * .
 *
 * @code
 * std::vector<hsize_t> dataset_dimensions = {50, 30};
 ...
 * @endcode
 *
 * ## Query the I/O mode that HDF5 used in the last parallel I/O call The
 * default access mode in the deal.II's HDF5 C++ interface  is collective
 ...
 * decide to do independent I/O instead of collective I/O, even if the user
 * asks for collective I/O. See the following
 * [article](https://www.hdfgroup.org/2015/08/parallel-io-with-hdf5/). In
 * cases where maximum performance is a requirement, it is important to make
 * sure that all MPI read/write operations are collective. The HDF5 library
 * provides API routines that can be used after the read/write I/O operations
 * to query the I/O mode. If   DataSet::query_io_mode   is True, then after
 ...
 * @endcode
 * See   DataSet::get_io_mode(),     DataSet::get_local_no_collective_cause()
 * and   DataSet::get_global_no_collective_cause()   for all the possible
 * return codes. # Rank of HDF5 datasets and hyperslabs The deal.II's HDF5
 * interface can be used to write/read data to datasets and hyperslabs of any
 * particular rank. `FullMatrix` can only be used to write/read data to
 * datasets and hyperslabs of rank 2. In the other hand,   `std::vector`   and
 * `Vector` can be used to write/read data to datasets and hyperslabs of rank
 * 1, 2, 3 and higher, the data is organized in [row-major
 * order](https://en.wikipedia.org/wiki/Row-_and_column-major_order) which is
 * commonly used in C and C++ matrices. We can re-write the code from the
 * previous section using   std::vector
 *
 * @code
 ...
 * @endcode
 *
 * # Datatypes Attribute datatypes can be float, `double`,
 * `std::complex<float>`,     `std::complex<double>`,   `int`, `unsigned int`,
 * `bool` and   `std::string`.     HDF5Object::get_attribute()   and
 * HDF5Object::set_attribute()   can be used with all of these datatypes.
 * Dataset datatypes can be `float`, `double`,   `std::complex<float>`,
 * `std::complex<double>`,   `int` and `unsigned int`.   DataSet::read(),
 * DataSet::write(),     DataSet::read_selection(),   etc. can be used with
 * all of these datatypes. Note that the dataset datatype can not be `bool`,
 * the reason is that it can not be assumed that   `std::vector<bool>`
 * stores the elements in a contiguous way.
 *
 *  ## Complex numbers and HDF5 There is no official HDF5 format to store
 * `std::complex`   numbers in a HDF5 file. But thede facto* standard is to
 * store the   `std::complex`   number in a compound type in which `r`
 * corresponds to the real part and `i` corresponds to the imaginary part. In
 * this interface we define two compound types one for
 * `std::complex<double>`   which corresponds to `(double,double)` and another
 * one for   `std::complex<float>`   which corresponds to `(float,float)`.
 * These two types correspond respectively to the types of python/numpy/h5py:
 ...
 * Data exchange with python scripts The HDF5 format can be used to exchange
 * data with python scripts. The strings are stored as HDF5 variable-length
 * UTF-8 strings and the complex numbers, as explained above, are stored as
 * HDF5 compound datatypes compatible with [h5py](https://www.h5py.org/) and
 * [numpy](http://www.numpy.org/). The following python script writes the
 * parameters for a deal.II simulation: ~~~~~~~~~~~~~{.py} h5_file =
 * h5py.File('simulation.hdf5','w') data = h5_file.create_group('data')
 * data.attrs['nb_frequency_points'] = 50 # int data.attrs['rho'] = 2300.5 #
 * double data.attrs['save_vtk_files'] = True # bool
 ...
 * @endcode
 *
 * Read the simulation results with python: ~~~~~~~~~~~~~{.py} h5_file =
 * h5py.File('simulation.hdf5','r+') data = h5_file['data'] displacement =
 * data['displacement'] # complex128 dtype active_cells =
 * data.attrs['degrees_of_freedom']) ~~~~~~~~~~~~~ # HDF5 and thread safety By
 * default HDF5 is not thread-safe. The HDF5 library can be configured to be
 * thread-safe, see [the HDF5
 * documentation](https://support.hdfgroup.org/HDF5/faq/threadsafe.html). The
 * thread-safe HDF5 version serializes the API but does not provide any level
 * of concurrency. To achieve high parallel performance with HDF5, we advice
 * to use HDF5 with MPI.
 *
 *
 */
// clang-format on
namespace HDF5
{
  ...
  protected:
    /**
     * Constructor.   @p string_name   is the name of the HDF5 Object. If   @p
     * mpi   is     True then MPI I/O is used.
     *
     */
  ...
  public:
    /**
     * Reads an attribute.   @p T   can be `float`, `double`,
     * `std::complex<float>`,         `std::complex<double>`,   `int`,
     * `unsigned int`, `bool` or   `std::string`.       Note that the encoding
     * of   `std::string`   is UTF8 in order to be compatible     with
     * python3.         Datatype conversion takes place at the time of a read
     * or write and is     automatic. See the <a
    ...
    /**
     * Writes an attribute.   @p T   can be `float`, `double`,
     * `std::complex<float>`,         `std::complex<double>`,   `int`,
     * `unsigned int`, `bool` or   `std::string`.       Note that the encoding
     * of   `std::string`   is UTF8 in order to be compatible     with
     * python3.         Datatype conversion takes place at the time of a read
     ...
     * to the file name. In the case of Group and DataSet, #name corresponds
     * to     the name of the object in the HDF5 file.
     *
     */
    std::string
    get_name() const;
  ...
  protected:
    /**
     * Name of the HDF5Oject. In the case of File,   @p name   corresponds to
     * the     file name. In the case of Group and DataSet   @p name
     ...
     * `std::shared_ptr<>`   pointer allows the object to be copied. For
     * example     several parts of the program can share and access the same
     * group; when     all the functions that access the group are closed, the
     * HDF5 resources of     the group will be automatically released.
     *
     */
  ...
  public:
    /**
     * Reads all the data of the dataset.         Datatype conversion takes
     * place at the time of the read operation and is     automatic. See the
     * <a
     * href="https://support.hdfgroup.org/HDF5/doc/UG/HDF5_Users_Guide-Responsive%20HTML5/index.html#t=HDF5_Users_Guide%2FDatatypes%2FHDF5_Datatypes.htm%23TOC_6_10_Data_Transferbc-26&rhtocid=6.5_2">Data
     * Transfer: Datatype Conversion and Selection</a>  section in the HDF5
     * User's Guide.         `Container` can be   `std::vector<float>`,
     ...
     * `FullMatrix<std::complex<float>>`   or
    ...
    /**
     * Reads data of a subset of the dataset.         Datatype conversion
     * takes place at the time of the read operation and is     automatic. See
     * the <a
     * href="https://support.hdfgroup.org/HDF5/doc/UG/HDF5_Users_Guide-Responsive%20HTML5/index.html#t=HDF5_Users_Guide%2FDatatypes%2FHDF5_Datatypes.htm%23TOC_6_10_Data_Transferbc-26&rhtocid=6.5_2">Data
     * Transfer: Datatype Conversion and Selection</a>  section in the HDF5
     * User's Guide.         The selected elements can be scattered and take
     * any shape in the dataset.     For example, in the case of a dataset
     * with rank 4 a selection of 3 points     will be described by a 3-by-4
     * array. Note the indexing is zero-based. To     select the points
     * (1,1,1,1), (14,6,12,18), and (8,22,30,22), the point     selection
     ...
     * HDF5 supports collective I/O on point selections.</a>         Datatype
     * conversion takes place at the time of the read operation and is
     * automatic. See the <a
     * href="https://support.hdfgroup.org/HDF5/doc/UG/HDF5_Users_Guide-Responsive%20HTML5/index.html#t=HDF5_Users_Guide%2FDatatypes%2FHDF5_Datatypes.htm%23TOC_6_10_Data_Transferbc-26&rhtocid=6.5_2">Data
     * Transfer: Datatype Conversion and Selection</a>  section in the HDF5
     * User's Guide.
     ...
     *
     *
     *  -   @p count:   The number of elements to select along each dimension.         When reading a hyperslab, HDF5 also allows to provide "stride" and     "block" parameters (see the [HDF5 documentation](https://support.hdfgroup.org/HDF5/doc1.8/RM/RM_H5S.html#Dataspace-SelectHyperslab)).     These are not used by the current function and set to `nullptr`. However     these parameters can be used with the function     read_hyperslab(const   std::vector<hsize_t>   &, const   std::vector<hsize_t>   &, const   std::vector<hsize_t>   &, const   std::vector<hsize_t>   &, const   std::vector<hsize_t>   &)         See the <a
     * href="https://support.hdfgroup.org/HDF5/doc/UG/HDF5_Users_Guide-Responsive%20HTML5/HDF5_Users_Guide/Dataspaces/HDF5_Dataspaces_and_Partial_I_O.htm?rhtocid=7.2#TOC_7_4_Dataspaces_and_Databc-6">Dataspaces
     * and Data Transfer</a>  section in the HDF5 User's Guide. See as well
     * the     <a
     * href="https://support.hdfgroup.org/HDF5/doc1.8/RM/RM_H5S.html#Dataspace-SelectHyperslab">H5Sselect_hyperslab
     * definition</a>.         Datatype conversion takes place at the time of
     * a read or write and is     automatic. See the <a
     * href="https://support.hdfgroup.org/HDF5/doc/UG/HDF5_Users_Guide-Responsive%20HTML5/index.html#t=HDF5_Users_Guide%2FDatatypes%2FHDF5_Datatypes.htm%23TOC_6_10_Data_Transferbc-26&rhtocid=6.5_2">Data
     * Transfer: Datatype Conversion and Selection</a>  section in the HDF5
     * User's Guide.         `Container` can be   `std::vector<float>`,
     * `std::vector<double>`,         `std::vector<std::complex<float>>`,
     ...
     *
     *
     *  -   @p stride:   The number of elements to separate each element or block to                   be selected.
     *
     *
     *
     *
     ...
     *
     *
     *  -   @p count:   The number of elements or blocks to select along each                  dimension.
     *
     *
     *
     *
     ...
     *
     *
     *  -   @p block:   The size of the block selected from the dataspace.         See the <a
     * href="https://support.hdfgroup.org/HDF5/doc/UG/HDF5_Users_Guide-Responsive%20HTML5/HDF5_Users_Guide/Dataspaces/HDF5_Dataspaces_and_Partial_I_O.htm?rhtocid=7.2#TOC_7_4_Dataspaces_and_Databc-6">Dataspaces
     * and Data Transfer</a>  section in the HDF5 User's Guide. See as well
     * the     <a
     * href="https://support.hdfgroup.org/HDF5/doc1.8/RM/RM_H5S.html#Dataspace-SelectHyperslab">H5Sselect_hyperslab
     ...
     * `std::complex<float>`,     `std::complex<double>`,   `int` or `unsigned
     * int`.         Datatype conversion takes place at the time of a read or
     * write and is     automatic. See the <a
     * href="https://support.hdfgroup.org/HDF5/doc/UG/HDF5_Users_Guide-Responsive%20HTML5/index.html#t=HDF5_Users_Guide%2FDatatypes%2FHDF5_Datatypes.htm%23TOC_6_10_Data_Transferbc-26&rhtocid=6.5_2">Data
     * Transfer: Datatype Conversion and Selection</a>  section in the HDF5
     * User's Guide.
     *
    ...
    /**
     * Writes data to a subset of the dataset.   @p number   can be `float`,
     * `double`,       `std::complex<float>`,     `std::complex<double>`,
     * `int` or `unsigned int`.         The selected elements can be scattered
     * and take any shape in the dataset.     For example, in the case of a
     * dataset with rank 4 a selection of 3 points     will be described by a
     * 3-by-4 array. Note the indexing is zero-based. To     select the points
     * (1,1,1,1), (14,6,12,18), and (8,22,30,22), the point     selection
     * array would be as follows:
     * @code
     *  0  0  0  0
     ...
     * href="https://support.hdfgroup.org/newsletters/newsletter140.html">Parallel
     * HDF5 supports collective I/O on point selections.</a>         Datatype
     * conversion takes place at the time of a read or write and is
     * automatic. See the <a
     * href="https://support.hdfgroup.org/HDF5/doc/UG/HDF5_Users_Guide-Responsive%20HTML5/index.html#t=HDF5_Users_Guide%2FDatatypes%2FHDF5_Datatypes.htm%23TOC_6_10_Data_Transferbc-26&rhtocid=6.5_2">Data
     * Transfer: Datatype Conversion and Selection</a>  section in the HDF5
     * User's Guide.
     ...
     *
     *
     *  -   @p count:   The number of elements to select along each dimension.         When writing a hyperslab, HDF5 also allows to provide "stride" and     "block" parameters (see the [HDF5 documentation](https://support.hdfgroup.org/HDF5/doc1.8/RM/RM_H5S.html#Dataspace-SelectHyperslab)).     These are not used by the current function and set to `nullptr`. However     these parameters can be used with the function     write_hyperslab(const Container &data, const   std::vector<hsize_t>   &data_dimensions, const   std::vector<hsize_t>   &offset, const   std::vector<hsize_t>   &stride, const   std::vector<hsize_t>   &count, const   std::vector<hsize_t>   &block).         See the <a
     * href="https://support.hdfgroup.org/HDF5/doc/UG/HDF5_Users_Guide-Responsive%20HTML5/HDF5_Users_Guide/Dataspaces/HDF5_Dataspaces_and_Partial_I_O.htm?rhtocid=7.2#TOC_7_4_Dataspaces_and_Databc-6">Dataspaces
     * and Data Transfer</a>  section in the HDF5 User's Guide. See as well
     * the     <a
     * href="https://support.hdfgroup.org/HDF5/doc1.8/RM/RM_H5S.html#Dataspace-SelectHyperslab">H5Sselect_hyperslab
     ...
     * this function because the operation iscollective* and all the MPI
     * processes have to contribute to the call, even if they don't have data
     * to write.   @p number   can be `float`, `double`,
     * `std::complex<float>`,         `std::complex<double>`,   `int` or
     * `unsigned int`.         Datatype conversion takes place at the time of
     * a read or write and is     automatic. See the <a
     * href="https://support.hdfgroup.org/HDF5/doc/UG/HDF5_Users_Guide-Responsive%20HTML5/index.html#t=HDF5_Users_Guide%2FDatatypes%2FHDF5_Datatypes.htm%23TOC_6_10_Data_Transferbc-26&rhtocid=6.5_2">Data
     * Transfer: Datatype Conversion and Selection</a>  section in the HDF5
     * User's Guide.         An example of how to use this function can be
     * found in   step-62  .
     *
     */
    template <typename number>
    ...
    /**
     * This function returns the boolean query_io_mode.         In cases where
     * maximum performance has to be achieved, it is important to     make
     * sure that all MPI read/write operations are collective. The HDF5
     ...
     * The return value is a   `std::string`   and can be     Value
     * | Meaning
     *
     *
     *
     ...
     * value returned by H5Pget_mpio_actual_io_mode.         The return value
     * can be     Value                          | Meaning
     *
     *
     *
     *
     ...
     * The return value is a string and can be     Value
     * | Meaning
     *
     *
     *
    ...
    /**
     * This function returns the global causes that broke collective I/O on
     * the     last parallel I/O call. See <a
     * href="https://support.hdfgroup.org/HDF5/doc/RM/RM_H5P.html#Property-GetMpioNoCollectiveCause">H5Pget_mpio_no_collective_cause</a>.
     * The return type is `uint32_t` and corresponds to the value returned by
     * H5Pget_mpio_no_collective_cause.         The return value can be
     * Value                                      | Meaning
     *
     *
     *
     ...
     * dimensions is a one-dimensional array of size rank specifying the size
     * of     each dimension of the dataset.
     *
     */
    std::vector<hsize_t>
  ...
  private:
    ...
    /**
     * Local causes that broke collective I/O on the     last parallel I/O
     * call. See <a
     * href="https://support.hdfgroup.org/HDF5/doc/RM/RM_H5P.html#Property-GetMpioNoCollectiveCause">H5Pget_mpio_no_collective_cause</a>.
     *
     */
    ...
    /**
     * Global causes that broke collective I/O on the     last parallel I/O
     * call. See <a
     * href="https://support.hdfgroup.org/HDF5/doc/RM/RM_H5P.html#Property-GetMpioNoCollectiveCause">H5Pget_mpio_no_collective_cause</a>.
     *
     */
  ...
  protected:
     ...
     * @p mode.   The group will be placed inside the group   @p parent_group.
     * The     parameter   @p mpi   defines if the I/O operations are serial
     * or     parallel. This is an internal constructor, the functions
     * open_group() and     create_group() of the current class should be used
     * to open or create a     group.
     *
     */
     ...
     * const members of HDF5Group:   @p name   and   @p mpi.   It does not
  ...
  public:
    ...
    /**
     * Create and write data to a dataset.   @p number   can be `float`,
     * `double`,       `std::complex<float>`,     `std::complex<double>`,
     * `int` or `unsigned int`.         Datatype conversion takes place at the
     * time of a read or write and is     automatic. See the <a
     * href="https://support.hdfgroup.org/HDF5/doc/UG/HDF5_Users_Guide-Responsive%20HTML5/index.html#t=HDF5_Users_Guide%2FDatatypes%2FHDF5_Datatypes.htm%23TOC_6_10_Data_Transferbc-26&rhtocid=6.5_2">Data
     * Transfer: Datatype Conversion and Selection</a>  section in the HDF5
     * User's Guide.         `Container` can be   `std::vector<float>`,
     * `std::vector<double>`,         `std::vector<std::complex<float>>`,
  ...
  private:
    /**
     * Delegation internal constructor.     File(const   std::string   &,
     * const MPI_Comm &, const Mode);     and     File(const   std::string
     * &, const Mode)     should be used to open or create HDF5 files.
     *
     */
    File(const std::string &  name,
         const FileAccessMode mode,
  ...
  namespace internal
  {
     ...
     * In the case of   std::complex   types the HDF5 handlers are
     * automatically     freed using the destructor of   `std::shared_ptr`.
     * `std::shared_ptr`   is     used instead of   `std::unique_ptr`
     * because the destructor of       `std::shared_ptr`   doesn't have to be
     * defined in the template argument. In     the other hand, the destructor
     * of   `std::unique`   has to be defined in the     template argument.
     * Native types such as `H5T_NATIVE_DOUBLE` do not     require a
     * destructor, but compound types such as   std::complex<double>
    ...
    /**
     * Return the dimensions of `data`. For a   std::vector   this function
     * returns       `std::vector<hsize_t>{vector_size}`.           Several
     * HDF5 functions such as H5Screate_simple() require a     one-dimensional
     * array that specifies the size of each dimension of the     container,
     * see:
    ...
    /**
     * This function returns the total size of the container. For a
     * std::vector       the function returns `int(vector_size)`.
     *
     */
    template <typename number>
    unsigned int
    ...
    /**
     * This function initializes and returns a container of type
     * std::vector,       Vector or FullMatrix. The function does not set the
     * values of the     elements of the container. The container can store
     * data of a HDF5 dataset     or a HDF5 selection. The dimensions
     * parameter holds the dimensions of the     HDF5 dataset or selection.
     * In the case of a   std::vector,   the size of the vector will be the
     * total     size given by dimensions. For example in the case of a
     * dataset of rank 3,     the dimensions are
     * `std::vector<hsize_t>{dim_0,dim_1,dim_2}`.   The size of     the
     * returned   std::vector   will be `dim_0*dim_1*dim_2`.         In the
     * case of a   dealii::Vector,   the size of the returned   dealii::Vector
     * will be as well `dim_0*dim_1*dim_2`.         A FullMatrix can store
     * only data of HDF5 datasets with rank 2. The size     of the FullMatrix
     * will be FullMatrix(dim_0,dim_2)
     *
     */
    template <typename Container>
...
* */
    ...
    /*     Write scalar attribute.     In most of the cases H5Awrite and H5Dwrite take a pointer to the data.     But in the particular case of a variable length string, H5Awrite takes     the address of the pointer of the string.    
* */
