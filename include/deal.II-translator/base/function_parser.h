 ...
 * Please refer to the muparser documentation for more information.  This
 * class is used in the   step-33   and   step-36   tutorial programs (the
 * latter being much simpler to understand). In addition to the built-in
 * functions of muparser, namely
 *
 * @code
 ...
 * all situations, this is not a problem except if the evaluation of   one of
 * the expressions throws a floating point exception in cases   where it will
 * later be discarded. (Assuming floating point exceptions   are switched on,
 * as is the default for deal.II in debug mode on most   systems.) An example
 * would be the expression `if(x>0, sqrt(x), 0)`   which is mathematically
 * well defined, but on systems where this is   enabled will abort the program
 * with a floating point exception when   evaluated with a negative `x`. This
 * is because the square root of   `x` is computed before the `if` statement's
 * condition is considered   to determine whether the result should be the
 * second or third   argument. If this kind of behavior is a problem, you can
 * resort to   the muparser built-in syntax `(condition ? then-value :
 * else-value)`,   using the ternary syntax familiar to C++ programmers. If
 * this   syntax is used, muparser uses lazy evaluation in which only one of
 * the   branches is evaluated, depending on whether the `condition` is   true
 * or false. The following examples shows how to use this class:
 *
 * @code
 * // set up problem:
 ...
 * the function as an argument (among other things). The syntax to describe a
 ...
 * https://beltoforion.de/en/muparser/. For a wrapper of the FunctionParser
 * class that supports ParameterHandler, see   Functions::ParsedFunction.
 ...
 * strings each defining one vector component. An example of time dependent
 * scalar function is the following:
 *
 * @code
...
public:
   ...
   * <code>time_dependent</code>   parameter to true.  An exception is thrown
   * if   the number of variables specified here is different from dim (if
   * this   function is not time-dependent) or from dim+1 (if it is
   * time-dependent).       @param[in]   expressions a list of strings
   * containing the expressions that   will be byte compiled by the internal
   * parser (muParser). Note that   the size of this vector must match exactly
   * the number of components of   the FunctionParser, as declared in the
   * constructor. If this is not the   case, an exception is thrown.
   * @param[in]   constants a map of constants used to pass any necessary
   ...
   * @param[in]   time_dependent If this is a time dependent function, then
   * the   last variable declared in   @p vars   is assumed to be the time
   * variable, and     FunctionTime::get_time()   is used to initialize it
   * when evaluating the   function. Naturally the number of variables parsed
   * by initialize() in   this case is dim+1. The value of this parameter
   ...
   * p.       <code>values</code>   shall have the right size beforehand, i.e.
   * #n_components.
   *
   */
  virtual void
