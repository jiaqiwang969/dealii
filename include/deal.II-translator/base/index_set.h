 ...
 * range   $[0,\mathrm{dof\_handler.n\_dofs()})$   that belongs to a
 * particular subdomain, or those among all degrees of freedom that are stored
 * on a particular processor in a distributed parallel computation. This class
 * can represent a collection of half-open ranges of indices as well as
 * individual elements. For practical purposes it also stores the overall
 * range these indices can assume. In other words, you need to specify the
 * size of the index space   $[0,\text{size})$   of which objects of this
 * class are a subset. There are two ways to iterate over the IndexSets:
 * First, begin() and end() allow iteration over individual indices in the
 * set. Second, begin_interval() and end_interval() allow iteration over the
 * half-open ranges as described above.
 * The data structures used in this class along with a rationale can be found in the   @ref distributed_paper   "Distributed Computing paper".
 *
 *
 */
class IndexSet
...
public:
   ...
   * indicating the type of the elements of the vector would then be   @p
   * bool.       On the other hand,   @p bool   has the disadvantage that it
   * is not a   numerical type that, for example, allows multiplication with a
   * @p double.     In other words, one can not easily use a vector of
   * booleans in a place   where other vectors are allowed. Consequently, we
   * declare the type of the   elements of such a vector as a signed integer.
   * This uses the fact that in   the C++ language, booleans are implicitly
   * convertible to integers. In   other words, declaring the type of the
   * elements of the vector as a signed   integer is only a small lie, but it
   * is a useful one.
   *
   */
   ...
   * of.     Note that the result is not equal to the number of indices within
   * this   set. The latter information is returned by n_elements().
   *
   */
  size_type
  size() const;
  ...
  /**
   * Add the half-open range   $[\text{begin},\text{end})$   to the set of
   * indices   represented by this class.     @param[in]   begin The first
   * element of the range to be added.     @param[in]   end The past-the-end
   * element of the range to be added.
   *
   */
  void
   ...
   * the iterator range   <code>[begin,end)</code>  .       @param[in]   begin
   * Iterator to the first element of range of indices to be   added
   * @param[in]   end The past-the-end iterator for the range of elements to
   * be   added.   @pre   The condition   <code>begin@<=end</code>   needs to
   * be satisfied.
   *
   */
   ...
   * union ofthis and   @p other.       If the   @p offset   argument is
   * nonzero, then every index in   @p other   is   shifted by   @p offset
   * before being added to the current index set. This   allows to construct,
   * for example, one index set from several others that   are supposed to
   * represent index sets corresponding to different ranges   (e.g., when
   * constructing the set of nonzero entries of a block vector   from the sets
   * of nonzero elements of the individual blocks of a vector).     This
   * function will generate an exception if any of the (possibly shifted)
   ...
   * This is similar, but faster than checking   <code>n_elements() ==
   * 0</code>  .
   *
   */
  bool
   ...
   * contiguous subset of indices, and the index set on process   $p+1$
   * starts   at the index one larger than the last one stored on process
   * $p$  .   In case there is only one MPI process, this just means that the
   * IndexSet   is complete.
   *
   */
  bool
   ...
   * stored in this index set.   @p local_index   obviously needs to be less
   * than   n_elements().
   *
   */
  size_type
  ...
  /**
   * Return the how-manyth element of this set (counted in ascending order)
   * @p     global_index is.   @p global_index   needs to be less than the
   * size(). This   function returns   numbers::invalid_dof_index   if the
   * index   @p global_index   is not actually   a member of this index set,
   * i.e. if is_element(global_index) is false.
   *
   */
  size_type
   ...
   * range.     In other words, the return value is nth_index_in_set(x), where
   * x is the   first index of the largest contiguous range of indices in the
   * IndexSet. The return value is therefore equal to the number of elements
   * in the set that come before the largest range.     This call assumes that
   * the IndexSet is nonempty.
   *
   */
  size_type
   ...
   * range <tt>[0, end-begin)</tt>.     In other words, the result of this
   * operation is the intersection of the   set represented by the current
   * object and the interval <tt>[begin,   end)</tt>, as seen <i>within the
   * interval <tt>[begin, end)</tt></i> by   shifting the result of the
   * intersection operation to the left by   <tt>begin</tt>. This corresponds
   * to the notion of a <i>view</i>: The   interval <tt>[begin, end)</tt> is a
   * <i>window</i> through which we see   the set represented by the current
   * object.
   *
   */
  IndexSet
  ...
  /**
   * Remove all elements contained in   @p other   from this set. In other
   * words,   if   $x$   is the current object and   $o$   the argument, then
   * we compute   $x \leftarrow x \backslash o$  .
   *
   */
  void
   ...
   * (n+1)*other.size())` of the   @p other   IndexSet.     The name results
   * from the perspective that one starts with an IndexSet and   takes the
   * tensor product with another IndexSet with `other.size()`   elements; this
   * results in a matrix of size `this->size()` times   `other.size()` that
   * has ones in exactly the rows for which this IndexSet   contained an index
   * and in the columns for which the   @p other   IndexSet   contained an
   * index. This matrix is then "unrolled" again by going through   each row
   * one by one and reindexing the entries of the matrix in consecutive
  ...
  /**
   * Remove and return the last element of the last range.   This function
  ...
  /**
   * Remove and return the first element of the first range.   This function
   * throws an exception if the IndexSet is empty.
   *
   */
  size_type
   ...
   * already have the correct size.     The given argument is filled with
   * integer values zero and one, using     <code>vector.operator[]</code>  .
   * Thus, any object that has such an   operator can be used as long as it
   * allows conversion of integers zero and   one to elements of the vector.
   * Specifically, this is the case for classes   Vector, BlockVector, but
   * also   std::vector@<bool@>,     std::vector@<int@>,     and
   * std::vector@<double@>.
   *
   */
   ...
   * MPI processes.     The last argument only plays a role if the
   * communicator is a parallel   one, distributing computations across
   * multiple processors. In that case,   if the last argument is false, then
   * it is assumed that the index sets   this function is called with on all
   * processors are mutually exclusive but   together enumerate each index
   * exactly once. In other words, if you call   this function on two
   * processors, then the index sets this function is   called with must
   * together have all possible indices from zero to   size()-1, and no index
   * must appear in both index sets. This corresponds,   for example, to the
   * case where we want to split the elements of vectors   into unique subsets
   * to be stored on different processors
   *
...
#  endif
   ...
   * this type. It allows access to a contiguous interval   $[a,b[$   (also
   * called   a range) of the IndexSet being iterated over.
   *
   */
  ...
  public:
     ...
     * range_idx       of the range to point to.
     *
     */
  ...
  public:
     ...
     * the expression   <code>it_left-it_right</code>   the distance is given
     * by     how many times one has to apply operator++ to the right operand
     * @p       it_right to get the left operand   @p it_left   (for a
     * positive return     value), or to   @p it_left   to get the   @p
     * it_right   (for a negative return     value).
     *
     */
    std::ptrdiff_t
...
private:
   ...
   * .     The nth_index_in_set denotes the how many-th index within this
   * IndexSet   the first element of the current range is. This information is
   * only   accurate if   IndexSet::compress()   has been called after the
   * last   insertion.
   *
   */
  struct Range
  {
     ...
     * @param   i1 Left end point of the interval.       @param   i2 First
     * index greater than the last index of the indicated     range.
     *
     */
   ...
   * set. This variable is always kept sorted.     The variable is marked
   * "mutable" so that it can be changed by compress(),   though this of
   * course doesn't change anything about the external   representation of
   * this index set.
   *
   */
   ...
   * indices.     The variable is marked "mutable" so that it can be changed
   * by compress(),   though this of course doesn't change anything about the
   * external   representation of this index set.
   *
   */
   ...
   * nth_index_in_set.     In many applications, the largest range contains
   * most elements (the   locally owned range), whereas there are only a few
   * other elements   (ghosts).
   *
   */
...
/**
 * Create and return an index set of size   $N$   that contains every single
 * index within this range. In essence, this function returns an index set
 * created by
 *
 * @code
 * IndexSet is (N);
