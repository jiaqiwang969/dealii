 ...
 * Fortin's <i>Mixed and Hybrid Finite Element Methods</i> (refer to pages 119
 *
 *  - 124).
 ...
 * part of   $(P_{k+1})^{n}$   (ie.   $(P_{k})^{n} \subset BDM_{k} \subset
 * (P_{k+1})^{n}$  ).  Furthermore,   $BDM_{k}$   elements are designed so
 * that   $\nabla \cdot q \in P_{k-1} (K)$   and   $q \cdot n |_{e_{i}} \in
 * P_{k}(e_{i})$  . More details of two and three dimensional   $BDM_{k}$
 * elements are given below.  <dl>     <dt> In 2D:     <dd>     $ BDM_{k} =
 * \{\mathbf{q} | \mathbf{q} = p_{k} (x,y) + r \; \text{curl} (x^{k+1}y) + s
 * \; \text{curl} (xy^{k+1}), p_{k} \in (P_{k})^{2} \}$  . Note: the curl of a
 * scalar function is given by   $\text{curl}(f(x,y)) = \begin{pmatrix}
 * f_{y}(x,y) \\
 *
 * -f_{x}(x,y) \end{pmatrix}$  . The basis used to construct the   $BDM_{1}$
 * shape functions is
 * @f{align*}{
 ...
 * @f}
 *
 * The dimension of the   $BDM_{k}$   space is   $(k+1)(k+2)+2$  , with
 * $k+1$   unknowns per edge and   $k(k-1)$   interior unknowns. <dt> In 3D:
 ...
 * \}$  . Note: the 3D description of   $BDM_{k}$   is not unique.  See
 * <i>Mixed and Hybrid Finite Element Methods</i> page 122 for an alternative
 * definition. The dimension of the   $BDM_{k}$   space is
 * $\dfrac{(k+1)(k+2)(k+3)}{2}+3(k+1)$  , with   $\dfrac{(k+1)(k+2)}{2}$
 * unknowns per face and   $\dfrac{(k-1)k(k+1)}{2}$   interior unknowns. </dl>
 *
 *
 *
 *
...
public:
   ...
   * degree.       @arg   k: the degree of the BDM-space, which is the degree
   * of the largest   complete polynomial space <i>P<sub>k</sub></i> contained
   * in the BDM-   space.
   *
   */
