...
/**
 * The enumerator NeighborType describes the neighboring relation between two
 * bounding boxes.
 *
 *
 */
enum class NeighborType
{
  ...
  /**
   * Simple neighbors: the boxes intersect with an intersection of dimension
   * at   most `spacedim
   *
  ...
  /**
   * Attached neighbors: neighbors with an intersection of   `dimension >
   * spacedim
   *
 ...
 * @f]
 * where   $(x_0^L , ..., x_{spacedim-1}^L) and $  (x_0^U , ...,
 * x_{spacedim-1}^U) denote the two vertices (bottom left and top right) which
 * are used to represent the box. Geometrically, a bounding box is thus:
 *
 *
 *
 ...
 * direction gives a box in one dimension lower: BoundingBox<spacedim
 *
 ...
 * (y, z)                 | |      y        |               (z, x)
 * | |      z        |               (x, y)                 | This is
 * according to the convention set by the function
...
public:
   ...
   * which describe the box: one for the bottom and one for the top   corner.
   *
   */
   ...
   * container.     The constructor supports any Container that provides
   * begin() and end()   iterators to Point<spacedim, Number> elements.
   *
   */
  ...
  /**
   * Check if the current object and   @p other_bbox   are neighbors, i.e. if
   * the boxes   have dimension spacedim, check if their intersection is non
   * empty.     Return an enumerator of type NeighborType.
   *
   */
  NeighborType
  ...
  /**
   * Enlarge the current object so that it contains   @p other_bbox   .   If
   * the current object already contains   @p other_bbox   then it is not
   ...
   * parameter   @p tolerance   is a factor by which the bounding box is
   * enlarged   relative to the dimensions of the bounding box in order to
   * determine in a   numerically robust way whether the point is inside.
   *
   */
  bool
  point_inside(
   ...
   * have each coordinate decreased by   @p amount,   and the upper right
   * corner   of the bounding box will have each coordinate increased by   @p
   * amount.       If you call this method with a negative number, and one of
   * the axes of the   original bounding box is smaller than amount/2, the
   * method will trigger   an assertion.
   *
   */
  void
  ...
  /**
   * Return the bounds of the box in   @p direction,   as a one-dimensional
   * box.
   *
   */
   ...
   * for a cell, so that   @p index     $\in [0, 2^{\text{dim}}
   *
   * - 1]$  .
   *
   */
  ...
  /**
   * Returns the indexth child of the box. Child is meant in the same way as
   * for   a cell.
   *
   */
  ...
  /**
   * Apply the affine transformation that transforms this BoundingBox to a
   * unit   BoundingBox object.     If   $B$   is this bounding box, and
   * $\hat{B}$   is the unit bounding box,   compute the affine mapping that
   * satisfies   $G(B) = \hat{B}$   and apply it to     @p point.
   *
   */
  Point<spacedim, Number>
  ...
  /**
   * Apply the affine transformation that transforms the unit BoundingBox
   * object   to this object.     If   $B$   is this bounding box, and
   * $\hat{B}$   is the unit bounding box,   compute the affine mapping that
   * satisfies   $F(\hat{B}) = B$   and apply it to     @p point.
   *
   */
  Point<spacedim, Number>
...
namespace internal
{
   ...
   * value.     The convention is the following: Starting from the locked
   * coordinate we   store the lower dimensional coordinates consecutively and
   * wrap around   when going over the dimension. This relationship is, in 2D,
   * | locked in 2D | 1D coordinate | 2D coordinate |
   * |:------------:|:-------------:|:-------------:|   |     x0       |
   * (a)      |   (x0,  a)    |   |     x1       |      (a)      |   (a , x1)
   * |     and, in 3D,     | locked in 3D | 2D coordinates | 3D coordinates |
   * |:-------------|:--------------:|:--------------:|   |     x0       |
   * (a, b)      | (x0,  a,  b)   |   |     x1       |    (a, b)      | ( b,
   * x1,  a)   |   |     x2       |    (a, b)      | ( a,  b, x2)   |
   * Given a locked coordinate, this function maps a coordinate index in dim
   * dimension to a coordinate index in dim + 1 dimensions.       @param
   * locked_coordinate should be in the range [0, dim+1).     @param
   * coordinate_in_dim should be in the range [0, dim).     @return   A
   * coordinate index in the range [0, dim+1)       @relates   BoundingBox
   *
   */
  template <int dim>
  inline int
