 ...
 * Access to the time goes through the following functions:   <ul>      <li>   <tt>get_time</tt>: return the present value of the time variable.    <li>   <tt>set_time</tt>: set the time value to a specific value.    <li>   <tt>advance_time</tt>: increase the time by a certain time step.   </ul>   The latter two functions are virtual, so that derived classes can perform computations which need only be done once for every new time. For example, if a time dependent function had a factor <tt>sin(t)</tt>, then it may be a reasonable choice to calculate this factor in a derived version of set_time(), store it in a member variable and use that one rather than computing it every time <tt>value()</tt>, <tt>value_list</tt> or one of the other functions of class Function is called.
 ...
 * for computations as sketched out above. The constructor of this class takes
 * an initial value for the time variable, which defaults to zero. Because a
 * default value is given, none of the derived classes needs to take an
 * initial value for the time variable if not needed.
 * @tparam   Number The data type in which time values are to be stored. This
 * will, in almost all cases, simply be the default   @p double,   but there
 * are cases where one may want to store the time in a different (and always
...
public:
  ...
  /**
   * The type this class is initialized with and that is used to represent
   * time.
   *
   */
