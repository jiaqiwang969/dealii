 ...
 * object?), but rather to aid in the search for memory bottlenecks. This
 * namespace has a single member function memory_consumption() and a lot of
 * specializations. Depending on the argument type of the function, there are
 * several modes of operation:
 * <ol>     <li>   If the argument is a fundamental C++ data type (such as <tt>bool</tt>, <tt>float</tt>, <tt>double</tt> or any of the integer types), then memory_consumption() just returns <tt>sizeof</tt> of its argument. The library also provides an estimate for the amount of memory occupied by a   <tt>std::string</tt>.
 ...
 * MemoryConsumption::memory_consumption()   for compounds of the class helps
 * to obtain this estimate. Most classes in the deal.II library have such a
 * member function.
 ...
 * </ol>
 * <h3>Extending this namespace</h3> The function in this namespace and the
 * functionality provided by it relies on the assumption that there is either
 * a function <tt>memory_consumption(T)</tt> in this namespace determining the
 * amount of memory used by objects of type <tt>T</tt> or that the class
 * <tt>T</tt> has a member function of that name. While the latter is true for
 * almost all classes in deal.II, we have only implemented the first kind of
 * functions for the most common data types, such as fundamental types,
 * strings, C++ vectors, C-style arrays, and C++ pairs. These functions
 * therefore do not cover, for example, C++ maps, lists, etc. If you need such
 * functions feel free to implement them and send them to us for inclusion.
 *
 *
...
namespace MemoryConsumption
{
  ...
  /**
   * Determine the amount of memory in bytes consumed by a
   * <tt>std::vector</tt>   of elements of type <tt>T</tt> by   calling
   * memory_consumption() for each entry.     This function loops over all
   * entries of the vector and determines their   sizes using
   * memory_consumption() for each <tt>v[i]</tt>. If the entries   are of
   * constant size, there might be another global function
   ...
   * expensive.     Using the algorithm, in particular the loop over all
   * elements, it is   possible to also compute the memory consumption of
   * vectors of vectors,   vectors of strings, etc, where the individual
   * elements may have vastly   different sizes.     Note that this algorithm
   * also takes into account the size of elements   that are allocated by this
   * vector but not currently used.     For the most commonly used vectors,
   * there are special functions that   compute the size without a loop. This
   * also applies for the special case   of vectors of bools.
   *
   */
  template <typename T>
  inline std::size_t
   ...
   * calling memory_consumption() for each entry.     This function loops over
   * all entries of the array and determines their   sizes using
   * memory_consumption() for each <tt>v[i]</tt>. If the entries   are of
   * constant size, there might be another global function
   * memory_consumption() for this data type or if there is a member function
   * of that class of that names that returns a constant value and the
   ...
   * expensive.     Using the algorithm, in particular the loop over all
   * elements, it is   possible to also compute the memory consumption of
   * arrays of vectors,   arrays of strings, etc, where the individual
   * elements may have vastly   different sizes.
   *
   */
   ...
   * like   for the   <tt>std::vector</tt>   arrays, but always use the loop
   * over all   elements.
   *
   */
   ...
   * vector, here for a vector of <tt>bool</tt>s.     This is a special case,
   * as the bools are not stored one-by-one, but as a   bit field.
   *
   */
  inline std::size_t
   ...
   * @note   This returns the size of the pointer, not of the object pointed
   * to.
   *
   */
  template <typename T>
