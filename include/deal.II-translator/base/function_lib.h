...
{
  /**
   * The distance to the origin squared.     This function returns the square
   * norm of the radius vector of a point.     Together with the function, its
   * derivatives and Laplacian are defined.
   * @ingroup functions
   *
   */
  template <int dim>
  ...
  /**
   * d-quadratic pillow on the unit hypercube.     This is a function for
   * testing the implementation. It has zero Dirichlet   boundary values on
   * the domain   $(-1,1)^d$  . In the inside, it is the   product of
   * $1-x_i^2$   over all space dimensions.     Providing a non-zero argument
   * to the constructor, the whole function can   be offset by a constant.
  ...
  /**
   * Gradient of the cosine-shaped pillow function.     This is a
   * vector-valued function with   @p dim   components, the gradient of
  ...
  /**
   * A function that solves the Laplace equation (with specific   boundary
   * values but zero right hand side) and that has a   singularity at the
   * center of the L-shaped domain in 2D (i.e.,   at the location of the
   * re-entrant corner of this non-convex   domain).     The function is given
   * in polar coordinates by   $r^{\frac{2}{3}}
   * \sin(\frac{2}{3} \phi)$   with a singularity at the origin and   should be used with   GridGenerator::hyper_L().   Here,   $\phi$   is   defined as theclockwise* angle against the positive   $x$  -axis.     This function is often used to illustrate that the solutions of the Laplace   equation   @f[
   *
   *
   *
   *
  ...
  /**
   * A jump in x-direction transported into some direction.     If the
   * advection is parallel to the y-axis, the function is
   * <tt>-atan(sx)</tt>, where <tt>s</tt> is the steepness parameter provided
   * in the constructor.     For different advection directions, this function
   * will be turned in the   parameter space.     Together with the function,
   * its derivatives and Laplacian are defined.
   * @ingroup functions
   *
   */
  template <int dim>
   ...
   * coefficient is given as a   $d$  -dimensional point   $k$   in Fourier
   * space, and   the function is then recovered as   $f(x) = \cos(\sum_i k_i
   * x_i) = Re(\exp(i k.x))$  .     The class has its name from the fact that
   * it resembles one component of a   Fourier cosine decomposition.
   * @ingroup functions
   *
   */
  template <int dim>
   ...
   * coefficient is given as a   $d$  -dimensional point   $k$   in Fourier
   * space, and   the function is then recovered as   $f(x) = \sin(\sum_i k_i
   * x_i) = Im(\exp(i k.x))$  .     The class has its name from the fact that
   * it resembles one component of a   Fourier sine decomposition.
   * @ingroup functions
   *
   */
  template <int dim>
   ...
   * point   $k$   in Fourier space, and the entire function is then recovered
   * as     $f(x) = \sum_j w_j sin(\sum_i k_i x_i) = Im(\sum_j w_j \exp(i
   * k.x))$  .
   * @ingroup functions
   *
   */
   ...
   * cosine functions. Each wavenumber coefficient is given as a     $d$
   * -dimensional point   $k$   in Fourier space, and the entire function is
   * then recovered as   $f(x) = \sum_j w_j cos(\sum_i k_i x_i) = Re(\sum_j
   * w_j \exp(i k.x))$  .
   * @ingroup functions
   *
   */
  ...
  public:
    ...
    /**
     * Constructor.           @param[in]   radius Radius of the ball
     * @param[in]   center Center of the ball       @param[in]   n_components
     * Number of components of this function object       @param[in]   select
     * If this is different from       CutOffFunctionBase<dim>::no_component,
     * then the function will be non-zero     for this component only
     * @param[in]   integrate_to_one Rescale the value of the function
     * whenever a     new radius is set, to guarantee that the integral is
     * equal to one       @param[in]   unitary_integral_value Value of the
     * integral when the radius     is equal to 1.0. Derived classes will need
     * to supply this value, to     guarantee that the rescaling is performed
     * correctly.
     *
     */
    CutOffFunctionBase(
  ...
  protected:
    ...
    /**
     * Flag that controls whether we rescale the value when the radius
     * changes.
     *
     */
  ...
  /**
   * Tensor product of CutOffFunctionBase objects.     Instead of using the
   * distance to compute the cut-off function, this class   performs a tensor
   * product of the same CutOffFunctionBase object in each   coordinate
   * direction.
   * @ingroup functions
   *
   */
  ...
  public:
    /**
     * Construct an empty CutOffFunctionTensorProduct object.         Before
     * you can use this class, you have to call the set_base() method     with
     * a class derived from the CutOffFunctionBase object.         If you try
     * to use this class before you call the set_base() method,     and
    ...
    /**
     * Initialize the class with an object of type       @tparam
     * CutOffFunctionBaseType<1>.
     *
     */
  ...
  public:
     ...
     * If an argument <tt>select</tt> is given and not
     *
   ...
   * $C^1$     (i.e., continuously differentiable). This is a cut-off function
   * that is   often used in the literature of the Immersed Boundary Method.
   * The expression of the function in radial coordinates is given by
   * $f(r)=1/2(cos(\pi r/s)+1)$   where   $r<s$   is the distance to the
   * center, and     $s$   is the radius of the sphere. If vector valued, it
   ...
   * <tt>center</tt>,   $f(r)=exp(1-1/(1-r**2/s**2))$  , where   $r$   is the
   ...
   * $x^\alpha$  , in 2-d the form   $x_1^{\alpha_1}x_2^{\alpha_2}$  , and in
   * 3-d     $x_1^{\alpha_1}x_2^{\alpha_2}x_3^{\alpha_3}$  . Monomials are
   * therefore   described by a   $dim$  -tuple of exponents. Consequently,
   * the class's   constructor takes a Tensor<1,dim> to describe the set of
   * exponents. Most   of the time these exponents will of course be integers,
   * but real   exponents are of course equally valid. Exponents can't be real
   ...
   * $y_0,\ldots,y_{L-1}$  ,   $z_1,\ldots,z_{M-1}$  , and data   $d_{klm}$
   * defined at   point   $(x_k,y_l,z_m)^T$  , then evaluating the function at
   * a point   $\mathbf x=(x,y,z)$   will find the box so that   $x_k\le x\le
   * x_{k+1}, y_l\le y\le y_{l+1}, z_m\le z\le z_{m+1}$  , and do a trilinear
   * interpolation of the   data on this cell. Similar operations are done in
   * lower dimensions.     This class is most often used for either evaluating
   * coefficients or right   hand sides that are provided experimentally at a
   * number of points inside   the domain, or for comparing outputs of a
   * solution on a finite element   mesh against previously obtained data
   * defined on a grid.
   * @note   If the points   $x_i$   are actually equally spaced on an
   * interval     $[x_0,x_1]$   and the same is true for the other data points
   * in higher   dimensions, you should use the InterpolatedUniformGridData
   * class instead.     If a point is requested outside the box defined by the
   * end points of the   coordinate arrays, then the function is assumed to
   * simply extend by   constant values beyond the last data point in each
   * coordinate direction.   (The class does not throw an error if a point
   * lies outside the box since   it frequently happens that a point lies just
   * outside the box by an amount   on the order of numerical roundoff.)
   * @note   The use of the related class InterpolatedUniformGridData is
   * discussed in   step-53  .       <h3>Dealing with large data sets</h3>
   * This class is often used to interpolate data provided by fairly   large
   * data tables that are expensive to read from disk, and that take   a large
   * amount of memory when replicated on every process of parallel   (MPI)
   * programs.     The Table class can help with amortizing this cost by using
   * shared memory to store the data only as often as necessary
   *
   ...
   * the table into its own member variable. Rather, it is necessary to   use
   * themove* constructor of this class to take over ownership of   the table
   * and its shared memory space. This can be achieved using   the following
   * extension of the code snippet shown in the   documentation of the
   * TableBase class:
   * @code
  ...
  public:
    /**
     * Constructor to initialize this class instance with the data given in
     * @p       data_values.           @param   coordinate_values An array of
     * dim arrays. Each of the inner     arrays contains the coordinate values
     * $x_0,\ldots, x_{K-1}$   and     similarly for the other coordinate
     * directions. These arrays need not     have the same size. Obviously, we
     * need dim such arrays for a dim-     dimensional function object. The
     * coordinate values within this array     are assumed to be strictly
     * ascending to allow for efficient lookup.           @param   data_values
     * A dim-dimensional table of data at each of the mesh     points defined
     * by the coordinate arrays above. The data passed in is     copied into
     * internal data structures. Note that the Table     class has a number of
     * conversion constructors that allow converting     other data types into
     * a table where you specify this argument.
     *
     */
     ...
     * references andmove*, instead ofcopy* the data. This is often useful
     ...
     * object     from which this object could copy its information, but it
     * might as well     take over ("move") the data.
     *
     */
    InterpolatedTensorProductGridData(
      std::array<std::vector<double>, dim> &&coordinate_values,
     ...
     * given data set.           @param   p The point at which the function is
     * to be evaluated.       @param   component The vector component. Since
     * this function is scalar,     only zero is a valid argument here.
     * @return   The interpolated value at this point. If the point lies
     * outside     the set of coordinates, the function is extended by a
     * constant.
     *
     */
    virtual double
     ...
     * of the given data set.           @param   p The point at which the
     * function gradient is to be evaluated.       @param   component The
     * vector component. Since this function is scalar,     only zero is a
     * valid argument here.       @return   The value of the gradient of the
     * interpolated function at this     point. If the point lies outside the
     * set of coordinates, the function     is extended by a constant and so
     * its gradient is extended by 0.
     *
     */
    virtual Tensor<1, dim>
    gradient(const Point<dim> & p,
   ...
   * dimensional case, let there be points   $x_0,\ldots, x_{K-1}$   that
   * result   from a uniform subdivision of the interval   $[x_0,x_{K-1}]$
   * into   $K-1$     sub-intervals of size   $\Delta x = (x_{K-1}-x_0)/(K-1)$
   * , and similarly     $y_0,\ldots,y_{L-1}$  ,   $z_1,\ldots,z_{M-1}$  .
   * Also consider data   $d_{klm}$     defined at point   $(x_k,y_l,z_m)^T$
   * , then evaluating the function at a   point   $\mathbf x=(x,y,z)$   will
   * find the box so that   $x_k\le x\le x_{k+1}, y_l\le y\le y_{l+1}, z_m\le
   * z\le z_{m+1}$  , and do a trilinear   interpolation of the data on this
   * cell. Similar operations are done in   lower dimensions.     This class
   * is most often used for either evaluating coefficients or right   hand
   * sides that are provided experimentally at a number of points inside   the
   * domain, or for comparing outputs of a solution on a finite element   mesh
   * against previously obtained data defined on a grid.
   * @note   If you have a problem where the points   $x_i$   are not equally
   * spaced   (e.g., they result from a computation on a graded mesh that is
   * denser   closer to one boundary), then use the
   * InterpolatedTensorProductGridData   class instead.     If a point is
   * requested outside the box defined by the end points of the   coordinate
   * arrays, then the function is assumed to simply extend by   constant
   * values beyond the last data point in each coordinate direction.   (The
   * class does not throw an error if a point lies outside the box since   it
   * frequently happens that a point lies just outside the box by an amount
   * on the order of numerical roundoff.)
   * @note   The use of this class is discussed in   step-53  .
   * <h3>Dealing with large data sets</h3>     This class supports the same
   * facilities for dealing with large data sets   as the
   * InterpolatedTensorProductGridData class. See there for more   information
   * and example codes.
   * @ingroup functions
   *
   */
  ...
  public:
    /**
     * Constructor       @param   interval_endpoints The left and right end
     * points of the     (uniformly subdivided) intervals in each of the
     * coordinate directions.       @param   n_subintervals The number of
     * subintervals in each coordinate     direction. A value of one for a
     * coordinate means that the interval is     considered as one subinterval
     * consisting of the entire range. A value     of two means that there are
     * two subintervals each with one half of the     range, etc.       @param
     * data_values A dim-dimensional table of data at each of the mesh
     ...
     * the     given data set.           @param   p The point at which the
     * function is to be evaluated.       @param   component The vector
     * component. Since this function is scalar,       only zero is a valid
     * argument here.       @return   The gradient of the interpolated
     * function at this point. If the       point lies outside the set of
     * coordinates, the function is extended       by a constant whose
   ...
   * monomials and   $\alpha_{i,d}$   are their exponents. The class's
   * constructor   takes a Table<2,double> to describe the set of exponents
  ...
  public:
     ...
     * $a_{i}\prod_{d=1}^{dim} x_{d}^{\alpha_{i,d}}$  . The i-th element of
     * the coefficients vector     contains the coefficient   $a_{i}$   for
     * the i-th monomial.
     *
     */
