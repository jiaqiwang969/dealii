...
namespace Utilities
{
  namespace MPI
  {
    ...
    public:
       ...
       * IndexSets arguments   @p indexset_locally_owned   and   @p
       * indexset_ghost   for the MPI       communicator   @p communicator.
       *
       */
      ...
      /**
       * Constructor. Same as above but for vectors of indices   @p
       * indices_locally_owned         and   @p indices_ghost.   This allows
       * the indices to not be sorted and the       values are read and
       * written automatically at the right position of       the vector
       * during update_values(), update_values_start(), and
       * update_values_finish(). It is allowed to include entries with the
       * value   numbers::invalid_dof_index   which do not take part of the
       * index       exchange but are present in the data vectors as padding.
       *
       */
      ...
      /**
       * Fill the vector   @p ghost_array   according to the precomputed
       * communication       pattern with values from   @p
       * locally_owned_array.                 @pre   The vectors only have to
       * provide a method begin(), which allows         to access their raw
       * data.               @pre   The size of both vectors must be at least
       * as large as the number         of entries in the index sets passed to
       * the constructors or the         reinit() functions.
       ...
       * Utilities::MPI::Partitioner::export_to_ghosted_array_finish.   In
       * this       function, the user can provide the temporary data
       * structures to be       used.               @pre   The size of the
       * @p temporary_storage   vector has to be at least
       * temporary_storage_size. The reason for this is that this vector is
       * used as buffer for both sending and receiving data.
       * @note   Any value less than 10 is a valid value of           @p
       ...
       * Utilities::MPI::Partitioner::export_to_ghosted_array_start,   the
       * user         does not pass a reference to the destination vector,
       * since the data         is received into a designated part of the
       * buffer   @p temporary_storage.   This         allows for padding and
       * other post-processing of the received data.               @pre   The
       * required size of the vectors are the same as in the functions
       * above.
       * @note   Any value less than 10 is a valid value of           @p
       * communication_channel.
       *
       ...
       * @note   In contrast to the function
       * Utilities::MPI::Partitioner::export_to_ghosted_array_finish,   the
       * user         also has to pass a reference to the buffer   @p
       * temporary_storage,           since the data has been received into
       * the buffer and not into the         destination vector.
       * @pre   The required size of the vectors are the same as in the
       * functions         above.
       *
       */
      template <typename Number>
    ...
    private:
      ...
      /**
       * Buffer containing the values sorted by rank for sending and
       * receiving.
       * @note   Only allocated if not provided externally by user.
       * @note   At this place we do not know the type of the data to be sent.
       * So         we use an arbitrary type of size 1 byte. The type is cast
       * to the         requested type in the relevant functions.
       *
       */
