...
/**
 *
 * @brief The ParsedConvergenceTable class This class simplifies the
 * construction of convergence tables, reading the options for the generation
 * of the table from a parameter file. It provides a series of methods that
 * can be used to compute the error given a reference exact solution, or the
 * difference between two numerical solutions, or any other custom computation
 * of the error, given via   std::function   objects. An example usage of this
 * class is given by
 *
 * @code
 ...
 * @endcode
 *
 * The above code constructs a ParsedConvergenceTable that works for scalar
 * problems, and will produce an error table with `H1_norm`, `L2_norm`, and
 * `Linfty_norm` norms of the error. Whenever a call to the methods
 * error_from_exact() or difference() is made, the instance of this class
 * inspects its parameters, computes all norms specified by the parameter
 * given at construction time, possibly modified via a parameter file,
 * computes all extra column entries specified using the method
 * add_extra_column(), and writes one row of the convergence table. Once you
 * have finished with the computations, a call to output_table() will generate
 * a formatted convergence table on the provided stream, and to the file (if
 * any) specified in the parameter file. With a small modification, the same
 * code can be used to estimate the errors of mixed or multi-physics problems,
 * e.g.:
 *
 * @code
 ...
 * component for the pressure field `p`, and will produce an error table with
 * `H1` and `L2` norm of the error in the velocity field (first two,
 * components) and `L2` error in the pressure field. You may also call
 * `table.output_table()` without arguments, to write the table only to the
 * file specified in the parameter file. By calling the method
 * add_parameters() passing a ParameterHandler object, the following options
 * will be defined in the given ParameterHandler object (in the current level
 * of the ParameterHandler object, i.e., whatever level you have entered with
 * the   ParamterHandler::enter_subsection()   method), and can be modified at
 * run time through a parameter file:
 *
 * @code
...
public:
  /**
   * Minimal constructor for ParsedConvergenceTable objects.     The number of
   * components must match the number of components of the   finite element
   * space that is used to compute the errors. If   a component name is
   * repeated, than it is interpreted as a vector field,   and the errors of
   * the repeated components are grouped together.     The size of the vector
   * @p list_of_error_norms   must match the number of   unique component
   * names, and may contain zero or more comma separated   identifiers for the
   * norm to compute for each component (see the   documentation of
   * VectorTools::NormType   for the available options).     For example, the
   ...
   * @endcode
   * would produce (if the parameter file is left untouched) a table similar
   * to
   * @code
   ...
   * @endcode
   * See the other constructor for a documentation of all the parameters you
   * can   change.       @param   component_names Specify the names of the
   * components;     @param   list_of_error_norms Specify what error norms to
   * compute for each   unique component name.
   *
   */
  ...
  /**
   * Full constructor for ParsedConvergenceTable.       @param
   * component_names Names of the components. Repeated consecutive   names are
   * interpreted as components of a vector valued field;     @param
   * list_of_error_norms Specify what error norms to compute for each   unique
   * component name;     @param   exponent The exponent to use in p-norms;
   * @param   extra_columns Extra columns to add. These may be "cells" or
   * "dofs";     @param   rate_key Specify the extra column by which we will
   * compute the   error rates. This key can either be one of "cells" or
   * "dofs", or, if you   add extra columns to the table via the method
   * add_extra_column(), it may   be one of the extra columns you added;
   * @param   rate_mode Specify the rate mode to use when computing error
   * rates.   This maybe either "reduction_rate", "reduction_rate_log2", or
   * "none". See   the documentation of   ConvergenceTable::RateMode   for an
   * explanation of how   each of this mode behaves;     @param
   * error_file_name Name of error output file (with extension txt,   gpl,
   * tex, or org). If different from the empty string, than   output_table()
   * also writes in this file in the format deduced from its   extension;
   * @param   precision How many digits to use when writing the error;
   * @param   compute_error Control whether the filling of the table is
   * enabled   or not. This flag may be used to disable at run time any error
   * computation;     The parameters you specify with this constructor can be
   * written to a   ParameterHandler object by calling the add_parameters()
   * method. Once you   call the add_parameters() method, the following
   * options will be defined in   the given ParameterHandler object, and the
   * parameters of the instance of   this class will follow the modification
   * you make to the ParameterHandler   object at run time:
   * @code
   ...
   * handler   @p prm.   Whenever the content of   @p prm   changes, the
   * parameters   of this class will be updated.
   *
   */
  void
  ...
  /**
   * Add a row to the error table, containing the error between   @p solution
   * and   the   @p exact   function, in the norm(s) specified in the
   * parameter file.     If you specify a   @p weight   function during this
   * call, then this is used   to compute weighted errors. The weight function
   * can be either a scalar   function (which will be used for all
   * components), or a vector function.   When it is a vector function, an
   * assertion is triggered if the number of   components does not coincide
   * with the number of components of the   underlying finite element space.
   *
   */
  ...
  /**
   * Add an additional column (with name   @p column_name)   to the table, by
   * invoking   the function   @p custom_function,   when calling
   * error_from_exact() or   difference().     You can call this method as
   * many times as you want. If   @p column_name   was   already used in a
   * previous call, then calling this method with the same   name will
   * overwrite whatever function you had previously specified. If   you use a
   * lambda function for this call, make sure that the variables used
   * internally in the lambda function remain valid until the call to
   * error_from_exact() or difference().     Make sure you add all extra
   * columns before the first call to   error_from_exact() or difference().
   * Adding additional columns to the   convergence table after you already
   * started filling the table will trigger   an exception.     This method
   * may be used, for example, to compute the error w.r.t. to   time step
   * increments in time, for example:
   * @code
   * using namespace VectorTools;
   ...
   * set Rate key                       = dt
   * @endcode
   * @param   column_name Name of the column to add;     @param
   * custom_function %Function that will be called to fill the given   entry.
   * You need to make sure that the scope of this function is valid   up to
   * the call to error_from_exact() or difference();     @param   compute_rate
   * If set to true, then this column will be included in   the list of
   * columns for which error rates are computed. You may want to set   this to
   * false if you want to compute error rates with respect to this   column.
   * In this case, you should also specify   @p column_name   as the rate
   ...
   * (possibly) to the file stream specified in the parameters (with the
   * format   deduced from the file name extension).
   *
   */
  void
...
private:
  ...
  /**
   * Reduction rate mode. See   ConvergenceTable::RateMode   for a
   * documentation.
   *
   */
