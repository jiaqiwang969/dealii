...
namespace Functions
{
  ...
  {
    /**
     * Signed-distance level set function of a sphere:       $\psi(x) = \| x
     *
     * - x^c \|
     *
     * - R$  .     Here,   $x^c$   is the center of the sphere and   $R$   is
     * its radius. This     function is thus zero on the sphere, negative
     * "inside" the ball having     the sphere as its boundary, and positive
     * in the rest of       $\mathbb{R}^{dim}$  .         This function has
     * gradient and Hessian equal to       $\partial_i \psi(x) = (x
     *
     * - x^c)/\| x
     *
     ...
     * - x_j^c)/\| x
     *
     * - x^c \|^3$  ,     where   $\delta_{ij}$   is the Kronecker delta
     * function.
     * @ingroup functions
     *
     */
     ...
     * $\psi(x) = n \cdot (x
     *
     * - x_p)$  .     Here,   $n$   is the plane normal and   $x_p$   is a
     * point in the plane.     Thus, with respect to the direction of the
     * normal, this function is     positive above the plane, zero in the
     * plane, and negative below the     plane. If the normal is normalized,
     * $\psi$   will be the signed distance to     the closest point in the
     * plane.
     * @ingroup functions
     *
     */
