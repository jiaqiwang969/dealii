 ...
 * the monomials   ${1,x,x^2,...}$  . I.e. in <tt>d</tt> dimensions it
 * constructs all polynomials of the form   $\prod_{i=1}^d x_i^{n_i}$  , where
 * $\sum_i n_i\leq p$  . The base polynomials are given a specific ordering,
 * e.g. in 2 dimensions:   ${1,x,y,xy,x^2,y^2,x^2y,xy^2,x^3,y^3,...}$  . The
 * ordering of the monomials in   $P_k1$   matches the ordering of the
 * monomials in   $P_k2$   for   $k2>k1$  .
 *
 *
 */
template <int dim>
...
public:
  ...
  /**
   * Constructor. Creates all basis functions of   $P_p$  .   @arg   p: the
   ...
   * Note, that this number is   <tt>PolynomialSpace::degree()-1</tt>,
  ...
  /**
   * For the <tt>n</tt>th polynomial   $p_n(x,y,z)=x^i y^j z^k$   this
   * function   gives the degrees i,j,k in the x,y,z directions.     In 1d and
   * 2d, obviously only i and i,j are returned.
   *
   */
