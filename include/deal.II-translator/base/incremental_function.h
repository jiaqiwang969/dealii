...
{
  /**
   * This class represents an incremental function. That is, given an
   * arbitrary   function   <code>f</code>  , this class will return
   * <code>f(t)
   *
   *  - f(t
   *
   *  - delta_t)</code>, where   <code>f(t)</code>   denotes the   function evaluated at time   <code>t</code>   and, likewise, <code>f(t
   *
   *  -   delta_t)</code> denotes the function evaluated at time <code>t
   *
   *  -   delta_t</code>. The decrement   <code>delta_t</code>   is set by the method   set_decrement(). The main application of this class is to transform a given   Dirichlet boundary condition function into incremental form, as is   required by some implementations of non-linear solution schemes.
   * @ingroup functions
   *
   */
  ...
  public:
     ...
     * and will call   <code>base.set_time()</code>   during evaluation     in
     * order to evaluate the   @p base   class at any arbitrary time.     It
     * is guaranteed that the temporal state of   @p base   is returned     to
     * its original settings after each function evaluation in this     class.
     *
     */
     ...
     * It is required that the   @p values   vector have the correct size
    ...
    /**
     * Set the time decrement.         It is expected that this value be
     * positive.
     *
     */
    void
