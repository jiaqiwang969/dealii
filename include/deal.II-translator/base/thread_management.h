...
namespace Threads
{
   ...
   * Mutexes are used to lock data structures to ensure that only a   single
   * thread of execution can access them at the same time.     This class is a
   * thin wrapper around   `std::mutex`.   The only difference   is that this
   * class is copyable when   `std::mutex`   is not.  Indeed, when   copied,
   * the receiving object does not copy any state from the object   being
   * copied, i.e. an entirely new mutex is created. These semantics   are
   * consistent with the common use case if a mutex is used as a member
   ...
   * copied-to object, not the members of both the copied-to and   copied-from
   * object. Since at the time when the class is copied, the   destination's
   * member variable is not used yet, its corresponding mutex   should also
...
namespace Threads
{
   ...
   * divisible by   <code>n_intervals</code>  . The type of the iterators has
   * to   fulfill the requirements of a forward iterator, i.e.
   * <code>operator++</code>   must be available, and of course it must be
   * assignable.     A list of subintervals is returned as a vector of pairs
   * of iterators,   where each pair denotes the range
   ...
   * (almost)   equal size. This function works mostly as the one before, with
   * the   difference that instead of iterators, now values are taken that
   * define   the whole interval.
   * @ingroup threads
   *
   */
  ...
  {
    /**
     * @internal           If in a sub-thread an exception is thrown, it is
     * not propagated to the     main thread. Therefore, the exception handler
     * that is provided by the     applications main function or some of its
     * other parts will not be able     to catch these exceptions. Therefore,
     * we have to provide an exception     handler in the top function of each
     * sub-thread that at least catches     the exception and prints some
     * information, rather than letting the     operating system to just kill
     * the program without a message. In each of     the functions we use as
     * entry points to new threads, we therefore     install a try-catch
     * block, and if an exception of type       <code>std::exception</code>
     * is caught, it passes over control to this     function, which will then
     * provide some output.
     *
     */
    [[noreturn]] void
    ...
    /**
     * @internal           Same as above, but the type of the exception is not
...
namespace Threads
{
  ...
  {
    /**
     * @internal           Given an arbitrary type RT, store an element of it
     * and grant     access to it through functions get() and set(). There are
     ...
     * an exception previously stored in the   `std::promise`).   On     the
     * other hand, this class makes the result available for     as many times
     * as desired. It also doesn't store any exceptions     (though they will
     * be forwarded by the classes using the current     class).
     *
     */
    ...
    /**
     * @internal           Given an arbitrary type RT, store an element of it
     * and grant access to     it through functions get() and set(). This is
     * the specialization for     reference types: since references cannot be
     * set after construction time,     we store a pointer instead, which
     * holds the address of the object being     referenced.         This
     * function is not dissimilar to the   `std::promise`/`std::future`
     * combination of classes. The difference is that a   `std::promise`
     * can only be read once via   `std::future::get()`   (presumably this
     * design is due to the fact that   `std::future::get()`   can throw
     * an exception previously stored in the   `std::promise`).   On     the
     ...
     * @internal           Given an arbitrary type RT, store an element of it
     * and grant access to     it through functions get() and set(). This is
     * the specialization for     type void: there is obviously nothing to
     * store, so no function set(),     and a function get() that returns
     * void.         This function is not dissimilar to the
     * `std::promise`/`std::future`       combination of classes. The
     * difference is that a   `std::promise`       can only be read once via
     * `std::future::get()`   (presumably this     design is due to the fact
     * that   `std::future::get()`   can throw     an exception previously
     * stored in the   `std::promise`).   On     the other hand, this class
     * makes the result available for     as many times as desired. It also
     * doesn't store any exceptions     (though they will be forwarded by the
  ...
  namespace internal
  {
     ...
     * of these objects
     *
     *  -  exactly one because it carries the returned value     of the function called on the thread.         While we have only one of these objects per thread, several       Threads::Thread   objects may refer to this descriptor. If all Thread     objects go out of scope the ThreadDescriptor will detach from the     thread before being destroyed.
     *
     */
    template <typename RT>
    struct ThreadDescriptor
    {
       ...
       * the thread.             The return value is stored in a shared_ptr
       * because we might abandon       the ThreadDescriptor.  This makes sure
       * the object stays alive       until the thread exits.
       *
       */
       ...
       * has been called.             We use this variable to make sure we can
       * call join() twice on the       same thread. For some reason, the C++
       * standard library throws a         std::system_error   exception if
       * one tries to call   std::thread::join         twice (and in fact,
       * before the second call,   std::thread::joinable         returns
       * false) but this is a somewhat desirable thing to do because       one
       * doesn't have to keep track whether join() has been called before.
       * Using this variable, whenever we have called join() before, the
       * variable is set to true and we can skip over calling
       * std::thread::join()   a second time. Access to this variable is
       * guarded       by the following mutex.
       ...
       * in the same function as we call   std::thread::join,   these actions
       * are       atomic and need no mutex. Of course, two threads may call
       * join() on       the same thread object at the same time, but this
       * action is undefined       anyway since they can not both join the
       * same thread. That said, more       recent C++ standards do not appear
       * to have the requirement any more       that the only thread that can
       * call join() is the one that created the       thread. Neither does
       * `pthread_join` appear to have this requirement any       more.
       * Consequently, we can in fact join from different threads and       we
       * test this in base/thread_validity_07.
   ...
   * value.     Threads can be abandoned, i.e. if you just call
   * Threads::new_thread   but   don't care about the returned object, or if
   * you assign the return     Threads::Thread   object to an object that
   * subsequently goes out of scope,   then the thread previously created will
   * still continue to do work. You   will simply not be able to access its
   * return value any more, and it may   also happen that your program
   * terminates before the thread has finished   its work.     The default
   * value of the template argument is   <code>void</code>  , so if   the
   * function you are calling on a new thread has no return value, you can
   * omit the template argument.
   * @ingroup threads         @deprecated   Use   std::thread   or
   * std::jthread   instead.
  ...
  public:
    ...
    /**
     * Get the return value of the function of the thread. Since it     is
     * only available once the thread finishes, this function     internally
     * also calls join(). You can call this function     multiple times as
     * long as the object refers to the same task,     and expect to get the
     * same return value every time. (With the     exception of the case where
     * the returned object has been moved;     see below.)
     * @note   The function returns a <i>non-@p const reference</i> to     the
     * returned object, instead of the returned object. This     allows
     ...
     * <code>std::unique_ptr</code>   objects can not be copied. In other
     * words,     to get the pointer out of the object returned from the
     * thread, it needs     to be moved, and in order to be moved, the current
     * function needs to     return a writable (non-  @p const)   reference.
     *
     */
  ...
  namespace internal
  {
     ...
     * type, and t otherwise.         The case that t is of reference type is
     * handled in a partial     specialization declared below.
     *
     */
    template <typename T>
     ...
     * A general template that returns   std::ref(t)   if t is of reference
     * type, and t otherwise.         The case that t is of reference type is
     * handled in this partial     specialization.
     *
     */
    template <typename T>
  ...
  /**
   * Overload of the new_thread() function for objects that can be called like
   * a   function object without arguments. In particular, this function
   * allows   calling   Threads::new_thread()   with either objects that
   * result from using     std::bind,   or using lambda functions. For
   * example, this function is called   when writing code such as
   * @code
   * Threads::Thread<int>
   ...
   *                                      });
   * @endcode
   * Here, we run the sequence of functions     <code>do_this()</code> and
   * <code>then_do_that()</code>   on   a separate thread, by making the
   * lambda function declared here the   function to execute on the thread.
   * The lambda function then returns   42 (which is a bit pointless here, but
   * it could of course be some   computed number), and this is going to be
   * the returned value you   can later retrieve via
   * <code>thread.return_value()</code>   once the   thread (i.e., the body of
   * the lambda function) has completed.
   * @note   Every lambda function (or whatever else it is you pass to     the
   * new_thread() function here, for example the result of a       std::bind()
   * expression) has a return type and consequently     returns an object of
   * this type. This type can be inferred     using the C++11
   * <code>decltype</code>   statement used in the     declaration of this
   * function, and it is then used as the template     argument of the
   * Threads::Thread   object returned by the current function.     In the
   * example above, because the lambda function returns 42     (which in C++
   * has data type   <code>int</code>  ), the inferred     type is
   * <code>int</code>   and the task object will have type
   * <code>Task@<int@></code>  . In other words, it is not <i>necessary</i>
   * to explicitly specify in user code what that return type     of the
   * lambda or   std::bind   expression will be, though it is     possible to
   * explicitly do so by (entirely equivalently) writing
   * @code
   * Threads::Thread<int>
   *   thread = Threads::new_thread ( [] ()
   *
   ...
   * new_thread() will of course typically be more complicated.     In
   * particular, they will likely <i>capture</i> variables     from the
   * surrounding context and use them within the lambda.     See
   * https://en.wikipedia.org/wiki/Anonymous_function#C.2B.2B_.28since_C.2B.2B11.29
   * for more on how lambda functions work.
   * @note   If you pass a lambda function as an argument to the     current
   * function that captures a variable <i>by reference</i>,     or if you use
   * a   std::bind   that binds a function argument to     a reference
   * variable using   std::ref()   or   std::cref(),   then     obviously you
   * can only do this if the variables you reference     or capture have a
   * lifetime that extends at least until the time     where the thread
   * finishes.
   * @ingroup CPP11
   *
   */
   ...
   * @ingroup threads         @deprecated   Use TaskGroup instead.
   *
   */
  template <typename RT = void>
  ...
  namespace internal
  {
     ...
     * action. This function is a specialization of the previous one     for
     * the case where the return type is `void`. Consequently, we     can't
     * set a value. But we do evaluate the function object and     call
     * `std::promise::set_value()`   without argument.
     *
     */
   ...
   * Threads::new_task().   The idea is that   Threads::new_task()   allows
   * one to run   a function whenever the C++ run-time system finds it
   * convenient
   *
   *  -    typically, when there is an idle processor available. This can be used to   run things in the background when there is no immediate need for the   result, or if there are other things that could well be done in parallel.   Whenever the result of that background task is needed, one can call either   join() to just wait for the task to finish, or return_value() to obtain the   value that was returned by the function that was run on that background   task.     This class is conceptually similar to the     [`std::future`](https://en.cppreference.com/w/cpp/thread/future)   class that   is returned by     [`std::async`](https://en.cppreference.com/w/cpp/thread/async)   (which is   itself similar to what   Threads::new_task()   does). The principal conceptual   difference is that one can only call   `std::future::get()`   once, whereas one   can call   Threads::Task::return_value()   as many times as desired. It is,   thus, comparable to the     [`std::shared_future`](https://en.cppreference.com/w/cpp/thread/shared_future)     class. However,   `std::shared_future`   can not be used for types that can not   be copied
   *
   *  -  a particular restriction for   `std::unique_ptr`,   for example.
   * @ingroup threads
   *
   */
  template <typename RT = void>
  ...
  public:
    /**
     * Construct a task object, given a function object to execute on     the
     * task, and then schedule this function for     execution. However, when
     * MultithreadInfo::n_threads()   returns     1, i.e., if the deal.II
     * runtime system has been configured to     only use one thread, then
     * just execute the given function     object.           @post   Using
     * this constructor automatically makes the task object     joinable().
     *
     */
     ...
     * by the   Threads::new_task()   functions.           @post   Using this
     * constructor leaves the object in an unjoinable state,     i.e.,
     * joinable() will return false.
     *
     */
    Task() = default;
     ...
     * to join will return immediately).         If the operation that was
     * executed on the task with which this     object was initialized throws
     * an exception instead of returning     regularly, then calling the
     * current join() function will first     wait for that task to finish,
     * and then in turn throw the     exception that the task operation had
     * thrown originally. This     allows for the propagation of exceptions
     * from tasks executed on     a separate thread to the calling thread.
     * (This behavior differs from that of
     * [`std::future`](https://en.cppreference.com/w/cpp/thread/future),
     * where the   `std::future::wait()`   function only waits for
     * completion of the operation, whereas the exception is     propagated
     * only once one calls   `std::future::get()`.   However,     this is
     * awkward when putting `void` functions onto separate     tasks because
     * these do not actually return anything;     consequently, it is more
     * natural to call   `std::task::wait()`       for such tasks than the
     * `std::task::get()`   function since the     latter does not, actually,
     * return anything that could be     gotten.)           @pre   You can't
     * call this function if you have used the default     constructor of this
     * class and have not assigned a task object to it. In     other words,
     * the function joinable() must return true.
     *
     */
    void
    join() const
     ...
     * false if the object has been default constructed.         A task can be
     * joined multiple times (while the first join() operation     may block
     * until the task has completed running, all successive attempts     to
     * join will return immediately). Consequently, if this function
    ...
    /**
     * Get the return value of the function of the task. Since it is     only
     * available once the thread finishes, this function     internally also
     * calls join(). You can call this function     multiple times as long as
     * the object refers to the same task,     and expect to get the same
     * return value every time. (With the     exception of the case where the
     * returned object has been moved;     see below.)
     * @note   The function returns a <i>non-@p const reference</i> to     the
     * returned object, instead of the returned object. This     allows
     * writing code such as
     * @code
     ...
     * the   <code>std::unique_ptr</code>   object) because
     * <code>std::unique_ptr</code>   objects can not be copied. In other
     * words,     to get the pointer out of the object returned from the task,
     * it needs     to be moved, and in order to be moved, the current
     * function needs to     return a writable (non-  @p const)   reference.
     * This function internally calls the join() member function. As a
     * consequence, and as explained there, if the packaged task     throws an
     * exception that is then re-thrown by the join()     function and
     * consequently also the current function if you have     not previously
     * called join().           @pre   You can't call this function if you
     * have used the default     constructor of this class and have not
     * assigned a task object to it. In     other words, the function
     * joinable() must return true.
     *
     */
    typename internal::return_value<RT>::reference_type
    return_value()
  ...
  private:
    /**
     * A data structure that holds a   std::future   into which the task
     * deposits     its return value. Since one can only call
     * std::future::get()   once,     we do so in the get() member function
     * and then move the returned object     into the `returned_object` member
     * variable from where we can read it     multiple times and from where it
     * can also be moved away if it is not     copyable.
     *
     */
    class TaskData
    {
    ...
    private:
      ...
      /**
       * The promise associated with the task that is represented by the
       * current       class.
       *
       */
       ...
       * efficient implementation) see [3].             [1]
       * https://en.cppreference.com/w/cpp/atomic/atomic       [2]
    ...
    /**
     * A pointer to a descriptor of the object that described the task     and
     * its return value.
     *
     */
   ...
   * deal.II runtime system has been configured to only use one     thread,
   * then this function just executes the given function     object
   * immediately and stores the return value in the Task     object returned
   * by this function.
   ...
   * calling   Threads::new_task()   with either objects that result from
   * using     std::bind,   or using lambda functions. For example, this
   * function is called   when writing code such as
   * @code
   * Threads::Task<int>
   ...
   * <code>do_this()</code> and <code>then_do_that()</code>   on   a separate
   * task, by making the lambda function declared here the   function to
   * execute on the task. The lambda function then returns   42 (which is a
   * bit pointless here, but it could of course be some   computed number),
   * and this is going to be the returned value you   can later retrieve via
   * <code>task.return_value()</code>   once the   task (i.e., the body of the
   * lambda function) has completed.
   * @note   When   MultithreadInfo::n_threads()   returns 1, i.e., if the
   * deal.II runtime system has been configured to only use one     thread,
   * then this function just executes the given function     object
   * immediately and stores the return value in the Task     object returned
   * by this function.
   * @note   Every lambda function (or whatever else it is you pass to     the
   * new_task() function here, for example the result of a       std::bind()
   * expression) has a return type and consequently     returns an object of
   * this type. This type can be inferred     using the C++11
   * <code>decltype</code>   statement used in the     declaration of this
   * function, and it is then used as the template     argument of the
   * Threads::Task   object returned by the current function.     In the
   * example above, because the lambda function returns 42     (which in C++
   * has data type   <code>int</code>  ), the inferred     type is
   * <code>int</code>   and the task object will have type
   * <code>Task@<int@></code>  . In other words, it is not <i>necessary</i>
   ...
   * @endcode
   *
   * @note   In practice, the lambda functions you will pass to     new_task()
   * will of course typically be more complicated.     In particular, they
   * will likely <i>capture</i> variables     from the surrounding context and
   * use them within the lambda.     See
   * https://en.wikipedia.org/wiki/Anonymous_function#C.2B.2B_.28since_C.2B.2B11.29
   * for more on how lambda functions work.
   * @note   If you pass a lambda function as an argument to the     current
   * function that captures a variable <i>by reference</i>,     or if you use
   ...
   * variable using   std::ref()   or   std::cref(),   then     obviously you
   * can only do this if the variables you reference     or capture have a
   * lifetime that extends at least until the time     where the task
   * finishes.
   * @note     Threads::new_task()   is, in essence, equivalent to calling
   * `std::async(std::launch::async,   ...)` in that it runs the given task
   * in the background. See https://en.cppreference.com/w/cpp/thread/async
  ...
  /**
   * Overload of the new_task function. See the other functions of same name
   * for   more information.
   * @ingroup threads
   *
   */
   ...
   * for the called function.     Note that the call to join_all() must be
   * executed on the same thread as   the calls that add subtasks. Otherwise,
   * there might be a deadlock. In   other words, a Task object should never
   * passed on to another task for   calling the join() method.
   * @ingroup tasks
   *
   */
  template <typename RT = void>
  ...
  public:
    ...
    /**
     * Return how many tasks have been put into this group. This     function
     * does not distinguish how many of these tasks have     already run and
     * have finished, are still waiting to be     scheduled to a CPU resource,
     * or are currently running. Tasks     that have been joined already are
     * also still counted.
     *
     */
    std::size_t
