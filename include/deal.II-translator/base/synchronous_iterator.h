 ...
 * where we have synchronous iterators marching through the containers
 * <code>a,b</code>  . If an object of this type represents the end of a
 * range, only the first element is considered (we only have
 * <code>a.end()</code>  , not   <code>b.end()</code>  ). An example of how
 * this class is used is given in   step-35  . The template argument of the
 * current class shall be of type   <code>std::tuple</code>   with arguments
 * equal to the iterator types. The individual iterators can be accessed using
 * <code>std::get<X>(synchronous_iterator.iterators)</code>   where X is the
 * number corresponding to the desired iterator. This type, and the helper
 * functions associated with it, are used as the Value concept for the
 * blocked_range type of the Threading Building Blocks.
 *
 *
 */
...
struct SynchronousIterators
{
  ...
  /**
   * Dereference operator. Returns a reference to the iterators   represented
