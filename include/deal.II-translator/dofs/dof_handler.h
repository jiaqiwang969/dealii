...
/**
 * Given a triangulation and a description of a finite element, this class enumerates degrees of freedom on all vertices, edges, faces, and cells of the triangulation. As a result, it also provides a <i>basis</i> for a discrete space   $V_h$   whose elements are finite element functions defined on each cell by a FiniteElement object. This class satisfies the   @ref ConceptMeshType   "MeshType concept"
 * requirements. It is first used in the   step-2   tutorial program. For each
 * vertex, line, quad, etc, this class stores a list of the indices of degrees
 ...
 * actual layout of storage of the indices is described in the
 ...
 * the triangulation on which this DoFHandler is constructed. The
 * <tt>spacedim</tt> parameter has to be used if one wants to solve problems
 * on surfaces. If not specified, this parameter takes the default value
 * <tt>=dim</tt> implying that we want to solve problems in a domain whose
 * dimension equals the dimension of the space in which it is embedded.
 *
 *  <h3>Distribution of indices for degrees of freedom</h3> The degrees of
 * freedom (`dofs') are distributed on the given triangulation by the function
 * distribute_dofs(). It gets passed a finite element object describing how
 * many degrees of freedom are located on vertices, lines, etc. It traverses
 * the triangulation cell by cell and numbers the dofs of that cell if not yet
 * numbered. For non-multigrid algorithms, only active cells are considered.
 * Active cells are defined to be those cells which have no children, i.e.
 * they are the most refined ones. Since the triangulation is traversed
 * starting with the cells of the coarsest active level and going to more
 * refined levels, the lowest numbers for dofs are given to the largest cells
 * as well as their bounding lines and vertices, with the dofs of more refined
 * cells getting higher numbers. This numbering implies very large bandwidths
 * of the resulting matrices and is thus vastly suboptimal for some solution
 * algorithms. For this reason, the DoFRenumbering class offers several
 * algorithms to reorder the dof numbering according. See there for a
 * discussion of the implemented algorithms.
 *
 *  <h3>Interaction with distributed meshes</h3>
 * Upon construction,
 * this class takes a reference to a triangulation object. In most cases, this
 * will be a reference to an object of type Triangulation, i.e. the class that
 * represents triangulations that entirely reside on a single processor.
 * However, it can also be of type   parallel::distributed::Triangulation
 * (see, for example,   step-32  ,   step-40   and in particular the   @ref
 * distributed   module) in which case the DoFHandler object will proceed to
 * only manage degrees of freedom on locally owned and ghost cells. This
 * process is entirely transparent to the used.
 *
 *  <h3>User defined renumbering schemes</h3> The DoFRenumbering class offers
 * a number of renumbering schemes like the Cuthill-McKee scheme. Basically,
 * the function sets up an array in which for each degree of freedom we store
 * the new index this DoF should have after renumbering. Using this array, the
 * renumber_dofs() function of the present class is called, which actually
 * performs the change from old DoF indices to the ones given in the array. In
 * some cases, however, a user may want to compute their own renumbering
 * order; in this case, one can allocate an array with one element per degree
 * of freedom and fill it with the number that the respective degree of
 * freedom shall be assigned. This number may, for example, be obtained by
 * sorting the support points of the degrees of freedom in downwind direction.
 * Then call the   <tt>renumber_dofs(vector<types::global_dof_index>)</tt>
 * function with the array, which converts old into new degree of freedom
 * indices.
 *
 *  <h3>Serializing (loading or storing) DoFHandler objects</h3> Like many
 * other classes in deal.II, the DoFHandler class can stream its contents to
 * an archive using BOOST's serialization facilities. The data so stored can
 * later be retrieved again from the archive to restore the contents of this
 * object. This facility is frequently used to save the state of a program to
 * disk for possible later resurrection, often in the context of
 * checkpoint/restart strategies for long running computations or on computers
 * that aren't very reliable (e.g. on very large clusters where individual
 * nodes occasionally fail and then bring down an entire MPI job). The model
 * for doing so is similar for the DoFHandler class as it is for the
 ...
 * saved; the latter can be achieved by calling
 * DoFHandler::distribute_dofs()   using the same kind of finite element
 * before re-loading data from the serialization archive.
 *
 *  <h3>hp-adaptive finite element methods</h3> Instead of only using one
 * particular FiniteElement on all cells, this class also allows for an
 * enumeration of degrees of freedom on different finite elements on every
 * cells. To this end, one assigns an   <code>active_fe_index</code>   to
 * every cell that indicates which element within a collection of finite
 * elements (represented by an object of type   hp::FECollection)   is the one
 * that lives on this cell. The class then enumerates the degree of freedom
 * associated with these finite elements on each cell of a triangulation and,
 * if possible, identifies degrees of freedom at the interfaces of cells if
 * they match. If neighboring cells have degrees of freedom along the common
 * interface that do not immediate match (for example, if you have   $Q_2$
 * and   $Q_3$   elements meeting at a common face), then one needs to compute
 * constraints to ensure that the resulting finite element space on the mesh
 * remains conforming.
 * The whole process of working with objects of this type is explained in   step-27  . Many of the algorithms this class implements are described in the   @ref hp_paper   "hp-paper".
 *
 *  <h3>Active FE indices and their behavior under mesh refinement</h3> The
 * typical workflow for using this class is to create a mesh, assign an active
 * FE index to every active cell, call   DoFHandler::distribute_dofs(),   and
 * then assemble a linear system and solve a problem on this finite element
 * space. Active FE indices will be automatically transferred during mesh
 * adaptation from the old to the new mesh. Future FE indices are meant to
 * determine the active FE index after mesh adaptation, and are used to
 * prepare data on the old mesh for the new one. If no future FE index is
 * specified, the finite element prevails. In particular, the following rules
 * apply during adaptation:
 *
 *
 *
 ...
 * Strategies for automatic hp-adaptation which will set future FE indices
 * based on criteria are available in the   hp::Refinement   namespace.
 *
 *  <h3>Active FE indices and parallel meshes</h3> When this class is used
 * with either a   parallel::shared::Triangulation   or a
 * parallel::distributed::Triangulation,   you can only set active FE indices
 * on cells that are locally owned, using a call such as
 * <code>cell-@>set_active_fe_index(...)</code>  . On the other hand, setting
 * the active FE index on ghost or artificial cells is not allowed. Ghost
 * cells do acquire the information what element is active on them, however:
 * whenever you call   DoFHandler::distribute_dofs(),   all processors that
 * participate in the parallel mesh exchange information in such a way that
 * the active FE index on ghost cells equals the active FE index that was set
 * on that processor that owned that particular ghost cell. Consequently, one
 * can <i>query</i> the   @p active_fe_index   on ghost cells, just not set it
 * by hand.
 ...
 * for more information. During refinement and coarsening, information about
 * the   @p active_fe_index   of each cell will be automatically transferred.
 ...
 * latter needs to be run after
 * parallel::distributed::Triangulation::load().   If further data will be
 * attached to the triangulation via the
...
public:
  /**
   * An alias that is used to identify cell iterators in DoFHandler objects.   The concept of iterators is discussed at length in the     @ref Iterators   "iterators documentation module".
   ...
   * the member functions of DoFAccessor, in addition to the ones   already
   * available through the TriaAccessor class.
   * @ingroup Iterators
   *
   */
   ...
   * TriaIterator<DoFAccessor>. The   TriaIterator class works like a pointer
   * to objects that when   you dereference it yields an object of type
   * DoFAccessor. DoFAccessor,   in turn, is a class that can be used to query
   * DoF indices on faces,   but it is also derived from TriaAccessor and
   * consequently can be used   to query geometric properties such as vertices
   * of faces, their area, etc.
   * @ingroup Iterators
   *
   */
  ...
  /**
   * Invalid active FE index which will be used as a default value to
   * determine   whether a future FE index has been set or not.
   *
   */
  ...
  /**
   * Copy constructor. DoFHandler objects are large and expensive.   They
   * should not be copied, in particular not by accident, but   rather
   * deliberately constructed. As a consequence, this constructor   is
   * explicitly removed from the interface of this class.
   *
   */
  ...
  /**
   * Copy operator. DoFHandler objects are large and expensive.   They should
   * not be copied, in particular not by accident, but   rather deliberately
   * constructed. As a consequence, this operator   is explicitly removed from
   ...
   * the distribution of degrees of freedom over the mesh.       @deprecated
   ...
   * DoFHandler::get_fe().   However, it is often more convenient to   keep a
   * named finite element object as a member variable in your   main class and
   * refer to it directly whenever you need to access   properties of the
   * finite element (such as     FiniteElementData::dofs_per_cell).   This is
   * what all tutorial programs do.       @warning   This function only sets a
   * FiniteElement. Degrees of freedom have   either not been distributed yet,
   * or are distributed using a previously set   element. In both cases,
   * accessing degrees of freedom will lead to invalid   results. To restore
   * consistency, call distribute_dofs().       @deprecated   Use
   * distribute_dofs() instead.
   *
   */
  DEAL_II_DEPRECATED
  void
  ...
  /**
   * Assign a Triangulation to the DoFHandler.     Remove all associations
   * with the previous Triangulation object and   establish connections with
   * the new one. All information about previous   degrees of freedom will be
   * removed. Activates hp-mode.
   *
   */
  void
  ...
  /**
   * Go through the triangulation and "distribute" the degrees of   freedom
   * needed for the given finite element. "Distributing"   degrees of freedom
   * involves allocating memory to store the   indices on all entities on
   * which degrees of freedom can be   located (e.g., vertices, edges, faces,
   * etc.) and to then enumerate   all degrees of freedom. In other words,
   * while the mesh and the   finite element object by themselves simply
   * define a finite   element space   $V_h$  , the process of distributing
   * degrees of   freedom makes sure that there is a basis for this space and
   * that   the shape functions of this basis are enumerated in an indexable,
   * predictable way.     The exact order in which degrees of freedom on a
   * mesh are   ordered, i.e., the order in which basis functions of the
   * finite   element space are enumerated, is something that deal.II treats
   * as   an implementation detail. By and large, degrees of freedom are
   * enumerated in the same order in which we traverse cells, but you   should
   * not rely on any specific numbering. In contrast, if you   want a
   * particular ordering, use the functions in namespace   DoFRenumbering.
   * This function is first discussed in the introduction to the     step-2
   * tutorial program.
   * @note   This function makes a copy of the finite element given as
   ...
   * FE_Nothing, for example).     If this object is based on a
   * parallel::distributed::Triangulation,   then   the current function
   * returns true if <i>any</i> partition of the parallel   DoFHandler object
   * has any degrees of freedom. In other words, the   function returns true
   * even if the Triangulation does not own any active   cells on the current
   * MPI process, but at least one process owns cells and   at least this one
   * process has any degrees of freedom associated with it.
   *
   */
  bool
   ...
   * of the degrees of freedom.     This function is called by the functions
   * in DoFRenumbering function after   computing a new ordering of the degree
   * of freedom indices. However, it   can of course also be called from user
   * code.       @arg   new_number This array must have a size equal to the
   * number of   degrees of freedom owned by the current processor, i.e. the
   * size must be   equal to what n_locally_owned_dofs() returns. If only one
   * processor   participates in storing the current mesh, then this equals
   * the total   number of degrees of freedom, i.e. the result of n_dofs().
   * The contents   of this array are the new global indices for each freedom
   * listed in the   IndexSet returned by locally_owned_dofs(). In the case of
   * a sequential   mesh this means that the array is a list of new indices
   * for each of the   degrees of freedom on the current mesh. In the case
   * that we have a     parallel::shared::Triangulation   or
   ...
   * returns the new global DoF index of   <code>i</code>  . Since the
   * IndexSet of   locally_owned_dofs() is complete in the sequential case,
   * the latter   convention for the content of the array reduces to the
   * former in the case   that only one processor participates in the mesh.
   * @note   While it follows from the above, it may be surprising to know that     the <i>number</i> of locally owned degrees of freedom in a parallel     computation is an invariant     under renumbering, even if the <i>indices</i> associated with these     locally owned degrees of freedom are not. At a fundamental level,     this invariant exists because the <i>decision</i> whether a degree of     freedom is locally owned or not has nothing to do with that     degree of freedom's (old or new) index. Indeed, degrees of freedom     are locally owned if they are on a locally owned cell and not on     an interface between cells where the neighboring cell has a lower       @ref GlossSubdomainId   "subdomain id".
   * Since both of these conditions     are independent of the index
   * associated with the DoF, a locally     owned degree of freedom will also
   * be locally owned after renumbering.     On the other hand, properties
   * such as whether the set of indices     of locally owned DoFs forms a
   * contiguous range or not     (i.e., whether the locally_owned_dofs()
   * returns an IndexSet object     for which   IndexSet::is_contiguous()
   * returns   @p true)   are of     course affected by the exact renumbering
   * performed here. For example,     while the initial numbering of DoF
   * indices done in distribute_dofs()     yields a contiguous numbering, the
   * renumbering performed by       DoFRenumbering::component_wise()   will,
   * in general, not yield     contiguous locally owned DoF indices.
   *
   */
  void
   ...
   * SparsityPattern object.     The returned number is not really the maximum
   * number but an estimate   based on the finite element and the maximum
   * number of cells meeting at a   vertex. The number holds for the
   * constrained matrix as well.     The determination of the number of
   * couplings can be done by simple   picture drawing. An example can be
   * found in the implementation of this   function.
   * @note
   * This function is most often used to determine the maximal row   length
   * for sparsity patterns. Unfortunately, while the estimates returned   by
   * this function are rather accurate in 1d and 2d, they are often
   ...
   * memory on demand. Refer to the   step-2   and   step-11   example
   * programs on how   to do this. The problem is also discussed in the
   * documentation of the   module on     @ref Sparsity  .
   *
   */
  unsigned int
   ...
   * dof on the boundary can couple with.     The number is the same as for
   * max_couplings_between_dofs() in one   dimension less.
   * @note
   * The same applies to this function as to max_couplings_per_dofs() as
...
* */
   ...
   * active_cell_iterators().       @return   The half open range
   * <code>[this->begin(), this->end())</code>
   * @ingroup CPP11
   *
   */
   ...
   * example in the documentation of active_cell_iterators().       @return
   * The half open range <code>[this->begin_mg(),   this->end_mg())</code>
   * @ingroup CPP11
   *
   */
   ...
   * <code>[this->begin_mg(level),   this->end_mg(level))</code>       @pre
...
* */
  ...
  /**
   * Return the
   * global number of degrees of freedom. If the current object   handles all
   * degrees of freedom itself (even if you may intend to solve   your linear
   * system in parallel, such as in   step-17   or   step-18  ), then this
   ...
   * believes that it owns every degree of freedom it knows about.     On the
   * other hand, if this object operates on a
   ...
   * numbers::invalid_dof_index.   Else returns the number of degrees of
   ...
   * set.   The reason that a   @p map   rather than a   @p set   is used is
   * the same as   described in the documentation of that variant of
   * DoFTools::make_boundary_sparsity_pattern()   that takes a map.     There
   * is, however, another overload of this function that takes   a   @p set
   * argument (see below).
   *
   */
  ...
  /**
   * Access to an object informing of the block structure of the dof handler.     If an FESystem is used in distribute_dofs(), degrees of freedom naturally   split into several     @ref GlossBlock   "blocks".
   * For each base element as many blocks appear as its multiplicity.     At
   * the end of distribute_dofs(), the number of degrees of freedom in each
   ...
   * by   n_dofs(). (Here, "sequential" means that either   the whole program
   * does not use MPI, or that it uses MPI   but only uses a single MPI
   * process, or that there are multiple MPI   processes but the Triangulation
   * on which this DoFHandler builds   works only on one MPI process.)   On
   * the other hand, if we are operating on a
   * parallel::distributed::Triangulation   or
   * parallel::shared::Triangulation,     then it includes only the degrees
  ...
  /**
   * Whenever serialization with a   parallel::distributed::Triangulation   as
   * the   underlying triangulation is considered, we also need to consider
   * storing   the active FE indices on all active cells as well.     This
   * function registers that these indices are to be stored whenever the
   ...
   * a different type is registered.       @see   The documentation of
   * parallel::distributed::SolutionTransfer   has further     information on
   * serialization.
   *
   */
  void
   ...
   * the   underlying triangulation is considered, we also need to consider
   * storing   the active FE indices on all active cells as well.     This
   * function deserializes and distributes the previously stored   active FE
   * indices on all active cells.
   * @note   Currently only implemented for triangulations of type
   * parallel::distributed::Triangulation.   An assertion will be triggered if
   * a different type is registered.       @see   The documentation of
   * parallel::distributed::SolutionTransfer   has further     information on
   ...
   * object.     This function is made virtual, since a dof handler object
   * might be   accessed through a pointers to this base class, although the
   * actual   object might be a derived class.
   *
   */
  virtual std::size_t
...
private:
  ...
  public:
    ...
    /**
     * Set the index of the   <code>dof_number</code>  th degree of freedom
     * for     the given level stored for the current vertex to
     * <code>index</code>  .
     *
     */
    void
  ...
  private:
     ...
     * on the various levels this vertex exists on.         The starting
     * offset of the DoFs that belong to a   @p level   are given by
     * <code>n_dofs_per_vertex() (level-coarsest_level)</code>  .   @p
     * n_dofs_per_vertex()       must therefore be passed as an argument to
     * the functions that set or     read an index.
     *
     */
   ...
   * degrees of freedom this object works on.     For most members of this
   * structure, there is an accessor function in this   class that returns its
   * value.
   *
   */
  ...
  /**
   * Cached indices of the degrees of freedom of all active cell.
   * Identification   of the appropriate position of a cell in the vectors is
   * done via   cell_dof_cache_ptr (CRS scheme).
   *
   */
  ...
  /**
   * Pointer to the first cached degree of freedom of a geometric object for
   * all   relevant active finite elements.
   * @note   In normal mode it is possible to access this data structure
   * directly.     In hp-mode, an indirection via
   * hp_object_fe_indices/hp_object_fe_ptr is   necessary.
   *
   */
  ...
  /**
   * Active FE indices of each geometric object. Identification   of the
   * appropriate position of a cell in the vectors is done via
  ...
  /**
   * We embed our data structure into a pointer to control that   all transfer
   * related data only exists during the actual transfer process.
   *
   */
  ...
  /**
   * Create default tables for the active and future fe_indices.     Active
   * indices are initialized with a zero indicator, meaning that fe[0] is
   * going to be used by default. Future indices are initialized with an
   * invalid   indicator, meaning that no p-adaptation is scheduled by
   * default.     This method is called upon construction and whenever the
   * underlying   triangulation gets created. This ensures that each cell has
   * a valid active   and future fe_index.
   *
   */
  void
  ...
  /**
   * Update tables for active and future fe_indices.     Whenever the
   * underlying triangulation changes (either by adaptation or
   * deserialization), active and future FE index tables will be adjusted to
   * the   current structure of the triangulation. Missing values of active
   * and future   indices will be initialized with their defaults (see
   * create_active_fe_table()).     This method is called post refinement and
   * post deserialization. This   ensures that each cell has a valid active
   * and future fe_index.
   *
   */
  void
  ...
  /**
   * A function that will be triggered through a triangulation   signal just
   * before the associated Triangulation or
   * parallel::shared::Triangulation   is modified.     The function that
   * stores the active FE indices of all cells that will   be refined or
   * coarsened before the refinement happens, so that   they can be set again
   * after refinement.
   *
   */
  void
  ...
  /**
   * A function that will be triggered through a triangulation   signal just
   * after the associated Triangulation or     parallel::shared::Triangulation
   * is modified.     The function that restores the active FE indices of all
   * cells that   were refined or coarsened.
   *
   */
  void
  ...
  /**
   * A function that will be triggered through a triangulation   signal just
   * before the associated   parallel::distributed::Triangulation   is
   * modified.     The function that stores all active FE indices on locally
   * owned cells for   distribution over all participating processors.
   *
   */
  void
  ...
  /**
   * A function that will be triggered through a triangulation   signal just
   * after the associated   parallel::distributed::Triangulation   is
   * modified.     The function that restores all active FE indices on locally
   * owned cells   that have been communicated.
   *
   */
  void
...
namespace internal
{
  namespace hp
  {
    ...
    {
      /**
       * Given a DoFHandler object in hp-mode, make sure that the       future
       * FE indices that a user has set for locally owned cells are
       * communicated to all other relevant cells as well.             For
       * parallel::shared::Triangulation   objects,       this information is
       * distributed on both ghost and artificial cells.             In case a
       * parallel::distributed::Triangulation   is used,       indices are
       * communicated only to ghost cells.
       *
       */
      template <int dim, int spacedim>
      void
       ...
       * hp::FECollection         that is dominated by those assigned as
       * future finite elements to the       children of   @p parent.
       ...
       * extend our search on the whole   hp::FECollection,   which is the
       * element       that describes the smallest finite element space that
       * includes all       future finite elements assigned to the children.
       * If the function is not       able to find a finite element at all, an
       * assertion will be triggered.             In this way, we determine
       * the finite element of the parent cell in case       of h-coarsening
       * in the hp-context.
       * @note   This function can only be called on direct parent cells,
       * i.e.,       non-active cells whose children are all active.
       * @note   On   parallel::shared::Triangulation   objects where sibling
       * cells       can be ghost cells, make sure that future FE indices have
       * been properly       communicated with communicate_future_fe_indices()
       * first. Otherwise,       results might differ on different processors.
       * There is no check for       consistency of future FE indices.
       *
       */
      template <int dim, int spacedim = dim>
      unsigned int
...
namespace internal
{
   ...
   * this is broken on Intel 13+.     Defined in dof_handler.cc.
   *
   */
  template <int dim, int spacedim>
  std::string
