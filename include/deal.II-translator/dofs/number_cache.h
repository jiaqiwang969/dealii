...
namespace internal
{
  namespace DoFHandlerImplementation
  {
    ...
    struct NumberCache
    {
       ...
       * degrees of freedom. (Here, "sequential" means that either       the
       * whole program does not use MPI, or that it uses MPI       but only
       * uses a single MPI process, or that there are multiple MPI
       ...
       * given argument, in which each processor stores the degrees       of
       * freedom indicated in the corresponding element of the       vector
       * passed as first argument. The second argument indicates       the
       * rank among all participating processors the current       processor
       * has, so that we can set the   @p locally_owned_dofs         and   @p
       * n_locally_owned_dofs   fields.             All other fields stored by
       * the current object can be and are computed       from the argument.
       *
       */
      ...
      /**
       * Copy operator. Simply copy all members of the referenced       object
      ...
      /**
       * Return a representation of   @p n_locally_owned_dofs_per_processor
       * both       in case it was set up (directly returning the array) or in
       * case we       need to accumulate some information over all
       * processors. The latter       case involves global communication and
       * is typically expensive to set       up because it invokes
       * MPI_Allgather.
       *
       */
      ...
      /**
       * Return a representation of   @p locally_owned_dofs_per_processor
       * both       in case it was set up (directly returning the array of
       * IndexSet       fields) or in case we need to accumulate some
       * information over all       processors. The latter case involves
       * global communication and is       typically expensive to set up
       * because it invokes MPI_Allgather.
       *
       */
      std::vector<IndexSet>
       ...
       * computation, then this equals n_global_dofs. (Here, "sequential"
       * means       that either the whole program does not use MPI, or that
       * it uses MPI but       only uses a single MPI process, or that there
       * are multiple MPI       processes but the Triangulation on which this
       * DoFHandler builds       works only on one MPI process.)
       *
       */
       ...
       * [0,n_global_dofs). (Here, "sequential" means that either       the
       * whole program does not use MPI, or that it uses MPI       but only
       * uses a single MPI process, or that there are multiple MPI
       * processes but the Triangulation on which this DoFHandler builds
       * works only on one MPI process.)
       ...
       * the whole program does not use MPI, or that it uses MPI       but
       ...
       * locally_owned_dofs. (Here, "sequential" means that either       the
       * whole program does not use MPI, or that it uses MPI       but only
       * uses a single MPI process, or that there are multiple MPI
       * processes but the Triangulation on which this DoFHandler builds
       * works only on one MPI process.)
