...
namespace internal
{
  ...
  {
    /**
     * This is a switch class which only declares an   @p alias.   It is meant
     * to     determine which class a DoFAccessor class is to be derived from.
     * By     default, <tt>DoFAccessor  @<structdim,dim,spacedim@></tt>
     * derives from     the alias in the general     <tt>Inheritance
     * @<structdim,dim,spacedim@></tt>   class, which is     <tt>TriaAccessor
     * @<structdim,dim,spacedim@></tt>,   but if     <tt>structdim==dim</tt>,
     * then the specialization     <tt>Inheritance  @<dim,dim,spacedim@></tt>
     * is used which declares its     local type to be <tt>CellAccessor
     * @<dim,spacedim@></tt>.   Therefore, the     inheritance is
     * automatically chosen to be from CellAccessor if the     object under
     * consideration has full dimension, i.e. constitutes a cell.
     * @ingroup dofs
     * @ingroup Accessors
     *
     */
...
/**
 * A class
 * that gives access to the degrees of freedom stored in a DoFHandler object.
 * Accessors are used to access the data that pertains to edges, faces, and
 * cells of a triangulation. The concept is explained in more detail in
 * connection to   @ref Iterators  . This class follows mainly the route laid
 * out by the accessor library declared in the triangulation library
 * (TriaAccessor). It enables the user to access the degrees of freedom on
 * lines, quads, or hexes. The first template argument of this class
 * determines the dimensionality of the object under consideration: 1 for
 * lines, 2 for quads, and 3 for hexes. The second argument denotes the type
 * of DoFHandler we should work on. From the second template argument we also
 * deduce the dimension of the Triangulation this object refers to as well as
 * the dimension of the space into which it is embedded. Finally, the template
 * argument   <code>level_dof_access</code>   governs the behavior of the
 * function get_active_or_mg_dof_indices(). See the section on Generic loops
 * below. <h3>Alias</h3> Usage is best to happen through the alias to the
 * various kinds of iterators provided by the DoFHandler class, since they are
 * more secure to changes in the class naming and template interface as well
 * as providing easier typing (much less complicated names!). <h3>Generic
 * loops and the third template argument</h3> Many loops look very similar,
 * whether they operate on the active dofs of the active cells of the
 * Triangulation or on the level dofs of a single level or the whole grid
 * hierarchy. In order to use polymorphism in such loops, they access degrees
 * of freedom through the function get_active_or_mg_dof_indices(), which
 * changes behavior according to the third template argument.  If the argument
 * is false, then the active dofs of active cells are accessed. If it is true,
 * the level dofs are used. DoFHandler has functions, for instance begin() and
 * begin_mg(), which return either type or the other. Additionally, they can
 * be cast into each other, in case this is needed, since they access the same
 * data. It is recommended to use the function get_active_or_mg_dof_indices()
 * in generic loops in lieu of get_dof_indices() or get_mg_dof_indices().
 * <h3>Inheritance</h3> If the structural dimension given by the first
 * template argument equals the dimension of the DoFHandler (given as the
 * second template argument), then we are obviously dealing with cells, rather
 * than lower-dimensional objects. In that case, inheritance is from
 * CellAccessor, to provide access to all the cell specific information
 * afforded by that class. Otherwise, i.e. for lower-dimensional objects,
 * inheritance is from TriaAccessor. There is a DoFCellAccessor class that
 * provides the equivalent to the CellAccessor class.
 ...
 * edges have   @p structdim   equal to one, etc.   @tparam   dim Dimension of
 * the underlying DoFHandler.   @tparam   spacedim Space dimension of the
 * underlying DoFHandler.   @tparam   level_dof_access If   @p false,   then
 * the accessor simply represents   a cell, face, or edge in a DoFHandler for
 * which degrees of freedom only   exist on the finest level. Some operations
 * are not allowed in this case,   such as asking for DoF indices on
 * non-active cells. On the other hand,   if this template argument is   @p
 * true,   then the accessor represents an   object in a multilevel hierarchy
 * of degrees of freedom. In this case,   accessing DoF indices of <i>any</i>
 * cell is possible, and will return   the <i>level</i> indices (which, for
 * active cells, may be different   from the <i>global</i> indices).
 *
 *
 * @ingroup dofs
 *
...
public:
   ...
   * face or edge in a DoFHandler.       @param   tria The triangulation into
   * which this accessor points.     @param   level The level within the mesh
   * hierarchy of the object pointed     to. For example, coarse mesh cells
   * will have level zero, their children     level one, and so on. This
   * argument is ignored for faces and edges     which do not have a level.
   * @param   index The index of the object pointed to within the specified
   * refinement level.     @param   dof_handler A pointer to the DoFHandler
   * object to which the     accessor shall refer. This DoFHandler object must
   * of course be built on     the same triangulation as the one specified in
   * the first argument.
   *
   */
   ...
   * on DoF handler objects.   Consequently, this operator is declared as
  ...
  /**
   * Return the <i>global</i> indices of the degrees of freedom located on   this object in the standard ordering defined by the finite element (i.e.,   dofs on vertex 0, dofs on vertex 1, etc, dofs on line 0, dofs on line 1,   etc, dofs on quad 0, etc.) This function is only available on   <i>active</i> objects (see     @ref GlossActive   "this glossary entry").
   * The cells needs to be an active cell (and not artificial in a parallel
   * distributed computation).     The vector has to have the right size
   * before being passed to this   function.     The last argument denotes the
   * finite element index. For the standard   ::DoFHandler class, this value
   * must be equal to its default value since   that class only supports the
   * same finite element on all cells anyway.     However, when the relevant
   * DoFHandler object has hp-capabilities enabled,   different finite element
   * objects may be used on different cells. On faces   between two cells, as
   * well as vertices, there may therefore be two sets   of degrees of
   * freedom, one for each of the finite elements used on the   adjacent
   * cells. In order to specify which set of degrees of freedom to   work on,
   * the last argument is used to disambiguate. Finally, if this   function is
   * called for a cell object, there can only be a single set of   degrees of
   * freedom, and fe_index has to match the result of   active_fe_index().
   ...
   * .   Consequently, the derived DoFCellAccessor class has an overloaded
   * version   of this function that calls the present function with
   ...
   * vertex of the present cell.     The last argument denotes the finite
   * element index. For the standard   ::DoFHandler class, this value must be
   * equal to its default value since   that class only supports the same
   * finite element on all cells anyway.     However, when hp-capabilities are
   * enabled, different finite element   objects may be used on different
   * cells. On faces between two cells, as   well as vertices, there may
   * therefore be two sets of degrees of freedom,   one for each of the finite
   * elements used on the adjacent cells.  In order   to specify which set of
   * degrees of freedom to work on, the last argument   is used to
   * disambiguate. Finally, if this function is called for a cell   object,
   * there can only be a single set of degrees of freedom, and   fe_index has
   * to match the result of active_fe_index().
   *
   */
   ...
   * associated with the   <code>vertex</code>  th vertex on level   @p level.
  ...
  /**
   * Index of the <i>i</i>th degree of freedom of this object.     The last
   * argument denotes the finite element index. For the standard
   ...
   * contains   the indices of all degrees of freedom that somehow live on
   * this object   (i.e. on the vertices, edges or interior of this object),
   * the current   dof_index() function only considers the DoFs that really
   * belong to this   particular object's interior. In other words, as an
   * example, if the   current object refers to a quad (a cell in 2d, a face
   * in 3d) and the   finite element associated with it is a bilinear one,
   * then the   get_dof_indices() will return an array of size 4 while
   * dof_index() will   produce an exception because no degrees are defined in
   ...
   * non-   hp-objects, there is only a single active FE index, so the
   * argument must   be equal to zero. For lower-dimensional hp-objects, there
   * are   n_active_fe_indices() active finite elements, and this function can
   * be   queried for their indices.
   *
   */
  unsigned int
  ...
  /**
   * Returns all active FE indices on this object.     The size of the
   * returned set equals the number of finite elements that   are active on
   * this object.
   *
   */
   ...
   * zero. For cells, it is the case if   @p fe_index   equals
   * active_fe_index()   of this cell. For faces and other lower- dimensional
   * objects, there may   be more than one   @p fe_index   that are active on
   * any given object (see   n_active_fe_indices()).
   *
   */
  bool
  ...
  /**
   * A function has been called for a cell which should be     @ref GlossActive   "active",
   * but is refined.
   * @ingroup Exceptions
   *
   */
...
public:
   ...
   * the same object.     The template parameters of this function allow for a
   * comparison of very   different objects. Therefore, some of them are
   * disabled. Namely, if the   dimension, or the dof handler of the two
   * objects differ, an exception is   generated. It can be expected that this
   * is an unwanted comparison.     The template parameter
   * <tt>level_dof_access2</tt> is ignored, such that   an iterator with level
   * access can be equal to one with access to the   active degrees of
   * freedom.
   *
   */
  template <int structdim2, int dim2, int spacedim2, bool level_dof_access2>
...
protected:
   ...
   * index.     The last argument denotes the finite element index. For the
   ...
   * cells anyway.     However, when the relevant DoFHandler has
   * hp-capabilities, different   finite element objects may be used on
   * different cells. On faces between   two cells, as well as vertices, there
   * may therefore be two sets of   degrees of freedom, one for each of the
   ...
   * vertex of   the present cell to   @p index.       The last argument
   * denotes the finite element index. For the standard   ::DoFHandler class,
   * this value must be equal to its default value since   that class only
   * supports the same finite element on all cells anyway.     However, when
   * the relevant DoFHandler has hp-capabilities, different   finite element
   * objects may be used on different cells. On faces between   two cells, as
   * well as vertices, there may therefore be two sets of   degrees of
   * freedom, one for each of the finite elements used on the   adjacent
   * cells.  In order to specify which set of degrees of freedom to   work on,
...
public:
   ...
   * dimensional triangulation, i.e. a face of the triangulation.     Since
   * there is no mapping from vertices to cells, an accessor object for   a
   * point has no way to figure out whether it is at the boundary of the
   * domain or not. Consequently, the second argument must be passed by the
   * object that generates this accessor
   *
  ...
  /**
   * Pointer to the   @p ith   line bounding this object.     Since meshes
   * with dimension 1 do not have quads this method just throws   an
   * exception.
   *
   */
  typename dealii::internal::DoFHandlerImplementation::
  ...
  /**
   * Pointer to the   @p ith   quad bounding this object.     Since meshes
   * with dimension 1 do not have quads this method just throws   an
   * exception.
   *
   */
  ...
  /**
   * Return the <i>global</i> indices of the degrees of freedom located on   this object in the standard ordering defined by the finite element. This   function is only available on <i>active</i> objects (see     @ref GlossActive   "this glossary entry").
   * The present vertex must belong to an active cell (and not artificial in a
   * parallel distributed computation).     The vector has to have the right
   * size before being passed to this   function.     The last argument
   * denotes the finite element index. For the standard   ::DoFHandler class,
   * this value must be equal to its default value since   that class only
   * supports the same finite element on all cells anyway.     However, when
   * the relevant DoFHandler has hp-capabilities, different   finite element
   ...
   * element index. For the standard   ::DoFHandler class, this value must be
   * equal to its default value since   that class only supports the same
   * finite element on all cells anyway.     However, when the relevant
   * DoFHandler has hp-capabilities, different   finite element objects may be
   * used on different cells. On faces between   two cells, as well as
   * vertices, there may therefore be two sets of   degrees of freedom, one
   * for each of the finite elements used on the   adjacent cells.  In order
   * to specify which set of degrees of freedom to   work on, the last
   * argument is used to disambiguate. Finally, if this   function is called
   * for a cell object, there can only be a single set of   degrees of
   * freedom, and fe_index has to match the result of   active_fe_index().
   *
   */
  types::global_dof_index
  ...
  /**
   * Return the   @p n-th   active FE index on this object.     Since vertices
   * do not store the information necessary for this to be   calculated, this
   * method just raises an exception and only exists to   enable
  ...
  /**
   * Return true if the finite element with given index is active on the
   * present object.     Since vertices do not store the information necessary
   * for this to be   calculated, this method just raises an exception and
   * only exists to   enable dimension-independent programming.
   *
   */
  bool
  fe_index_is_active(const unsigned int fe_index) const;
 ...
 * sense such as quad iterators in on 1d meshes.  This class can not be used
 * to create objects (it will in fact throw an exception if this should ever
 * be attempted but it sometimes allows code to be written in a simpler way in
 * a dimension independent way. For example, it allows to write code that
 * works on quad iterators that is dimension independent
 *
...
public:
   ...
   * @p index.   Since the current object doesn't point to anything useful,
   * like   all other functions in this class this function only throws an
   * exception.
   *
   */
  types::global_dof_index
...
/**
 * Grant access to the degrees of freedom on a cell. Note that since for the
 * class we derive from, i.e. <tt>DoFAccessor<dim></tt>, the two template
 * parameters are equal, the base class is actually derived from CellAccessor,
 * which makes the functions of this class available to the DoFCellAccessor
 * class as well.
 *
 *
 * @ingroup dofs
...
public:
   ...
   * hierarchy), an exception is generated.     This function is needed since
   * the parent function of the base class   CellAccessor returns a
   * triangulation cell accessor without access to the   DoF data.
   *
   */
  ...
  /**
   * Return the   @p ith   periodic neighbor as a DoF cell iterator. This
   * function   is needed since the neighbor function of the base class
  ...
  /**
   * Return the   @p ith   neighbor or periodic neighbor as a DoF cell
   * iterator.   This function is needed since the neighbor function of the
   * base class   returns a cell accessor without access to the DoF data.
   *
   */
  TriaIterator<DoFCellAccessor<dimension_, space_dimension_, level_dof_access>>
  ...
  /**
   * Return an iterator to the   @p ith   face of this cell.     This function
   * returns a DoFAccessor with   <code>structdim == 0</code>   in   1D, a
   * DoFAccessor::line   in 2D, and a   DoFAccessor::quad   in 3d.
   *
   */
  face_iterator
  ...
  /**
   * Return the result of the   @p periodic_neighbor_child_on_subface
   * function   of the base class, but convert it so that one can also access
   * the DoF   data (the function in the base class only returns an iterator
  ...
  /**
   * Collect the values of the given vector restricted to the dofs of this
   * cell   in the standard ordering: dofs on vertex 0, dofs on vertex 1, etc,
   * dofs   on line 0, dofs on line 1, etc, dofs on quad 0, etc. In other
   * words, this function implements a   [gather
   ...
   * function. This function is only callable for active cells.     The input
   * vector may be either a <tt>Vector<float></tt>, Vector<double>,   or a
   * BlockVector<double>, or a PETSc or Trilinos vector if deal.II is
   ...
   * compiled to support these libraries. It is in the responsibility of the
   * caller to assure that the types of the numbers stored in input and output
   * vectors are compatible and with similar accuracy. The   AffineConstraints
   * object passed as an argument to this function makes   sure that
   * constraints are correctly distributed when the dof values   are
   * calculated.
   *
   */
  template <class InputVector, typename ForwardIterator>
   ...
   * In other words, this function implements a   [scatter
   * operation](https://en.wikipedia.org/wiki/Gather-scatter_(vector_addressing)).
   * This function is only callable for active cells.     Note that for
   * continuous finite elements, calling this function affects   the dof
   * values on neighboring cells as well. It may also violate   continuity
   * requirements for hanging nodes, if neighboring cells are less   refined
   * than the present one. These requirements are not taken care of   and must
   * be enforced by the user afterwards.     The vector has to have the right
   * size before being passed to this   function.     The output vector may be
   * either a Vector<float>, Vector<double>, or a   BlockVector<double>, or a
   * PETSc vector if deal.II is compiled to support   these libraries. It is
   * in the responsibility of the caller to assure that   the types of the
   * numbers stored in input and output vectors are   compatible and with
   * similar accuracy.
   *
   */
   ...
   * compute the interpolation from the children to the present cell.     If
   * the cell is part of a DoFHandler with hp-capabilities, cells only have
   ...
   * finite element function from the   <code>values</code>   vector on this
   * cell   and interpolates it onto the space described by the
   * <code>fe_index</code>  th element of the   hp::FECollection   associated
   * with   the DoFHandler of which this cell is a part of. If the cell is not
   ...
   * keeping the function space the same.     It is assumed that both input
   * vectors already have the right size   beforehand.
   ...
   * children of the present cell and set on the terminal cells.     In
   * principle, it works as follows: if the cell pointed to by this object
   ...
   * called for each of them.     Using the get_interpolated_dof_values() and
   * this function, you can   compute the interpolation of a finite element
   * function to a coarser grid   by first getting the interpolated solution
   * on a cell of the coarse grid   and afterwards redistributing it using
   * this function.     Note that for continuous finite elements, calling this
   * function affects   the dof values on neighboring cells as well. It may
   * also violate   continuity requirements for hanging nodes, if neighboring
   * cells are less   refined than the present one, or if their children are
   * less refined than   the children of this cell. These requirements are not
   * taken care of and   must be enforced by the user afterward.     If the
   * cell is part of a DoFHandler with hp-capabilities, cells only have   an
   ...
   * element of the space described by the   <code>fe_index</code>  th element
   * of   the   hp::FECollection   associated with the DoFHandler of which
   * this   cell is a part of, and interpolates it into the space that is
   * associated   with this cell. On the other hand, if the cell is not
   * active, then we   first perform this interpolation from this cell to its
   * children using the   given   <code>fe_index</code>   until we end up on
   * an active cell, at which   point we follow the procedure outlined at the
   * beginning of the paragraph.     It is assumed that both vectors already
   * have the right size beforehand.   This function relies on the existence
   * of a natural interpolation property   of finite element spaces of a cell
   * to its children, denoted by the   prolongation matrices of finite element
   * classes. For some elements, the   spaces on coarse and fine grids are not
   * nested, in which case the   interpolation to a child is not the identity;
   * refer to the documentation   of the respective finite element class for a
   * description of what the   prolongation matrices represent in this case.
   * @note   Unlike the get_dof_values() function, this function is only
   * available on cells, rather than on lines, quads, and hexes, since
   * interpolation is presently only provided for cells by the finite element
   * classes.
   ...
   * implements a   [scatter
   * operation](https://en.wikipedia.org/wiki/Gather-scatter_(vector_addressing)).
   * The elements are   <em>  added  </em>   to the existing elements in the
   * global   vector, rather than just set, since this is usually what one
   * wants. You may   also want to take a look at the
   ...
   * one and entering the local values into the global vector.   In other
   ...
   * vector,   rather than just set, since this is usually what one wants.
   * Moreover, the   AffineConstraints object passed to this function makes
   * sure that also   constraints are eliminated in this process.
   *
   */
  template <typename ForwardIterator, typename OutputVector>
  void
   ...
   * get_mg_dof_indices(), else return get_dof_indices().     You will get a
   * level_cell_iterator when calling begin_mg() and a normal   one otherwise.
   ...
   * function is called) before being passed to this function.     This
   * function reimplements the same function in the base class. In   contrast
   * to the function in the base class, we do not need the
   * <code>fe_index</code>   here because there is always a unique finite
   * element index on cells.     This is a function which requires that the
   * cell is active.     Also see get_active_or_mg_dof_indices().
   ...
   * <code>fe.n_dofs_per_cell()</code>  ) but instead that the returned values
   * are   the <i>global</i> indices of those degrees of freedom that are
   * located   locally on the current cell.
   *
   */
  void
   ...
   * DoFHandler   objects with hp-capabilities (i.e., there is currently no
   * implementation   of multilevel such objects), it does not make sense to
   * query the finite   element on non-active cells since they do not have
   * finite element spaces   associated with them without having any degrees
   * of freedom. Consequently,   this function will produce an exception when
   * called on non-active cells.
   *
   */
  ...
  /**
   * Return the index inside the   hp::FECollection   of the FiniteElement
   * used   for this cell. This function is only useful if the DoFHandler
   * object   associated with the current cell has hp-capabilities enabled.
   * @note   Since degrees of freedom only exist on active cells for
   * DoFHandler   objects with hp-capabilities (i.e., there is currently no
   * implementation   of multilevel such objects), it does not make sense to
   * query the finite   element on non-active cells since they do not have
   ...
   * parallel::distributed::Triangulation     classes, it is only allowed to
   * call this function on locally   owned or ghost cells. No information is
   * available on artificial cells.   Furthermore,   @p active_fe_index
   * information is only exchanged from locally   owned cells on one processor
   * to other processors where they may be   ghost cells, during the call to
   * DoFHandler::set_fe()   and     DoFHandler::distribute_dofs().   Be aware
   * that if you call   set_active_fe_index() on a cell after calling one of
   * these functions, then   this information will not be propagated to other
   * processors who may have   this cell as a ghost cell. See the
   * documentation of DoFHandler for more   information.
   *
   */
  unsigned int
   ...
   * which element in an   hp::FECollection   to use. This function is only
   * useful   if the DoF handler object associated with the current cell has
   ...
   * the   triangulation gets refined and coarsened. If no future finite
   * element has   been specified for this cell via the set_future_fe_index()
   * function, the   active one will remain unchanged, in which case the
   * active finite element   will be returned.     For DoFHandlers without
   * hp-capabilities enabled, this is of course always   the same element,
   * independent of the cell we are presently on, but for hp-   DoFHandler
   * objects this may change from cell to cell.
   * @note   Since degrees of freedom only exist on active cells for
   * DoFHandler   objects with hp-capabilities (i.e., there is currently no
   * implementation   of multilevel such objects), it does not make sense to
   * query the finite   element on non-active cells since they do not have
   ...
   * parallel::shared::Triangulation   or
   * parallel::distributed::Triangulation     classes, it is only allowed to
   * call this function on locally owned cells.
   *
   */
  unsigned int
   ...
   * assigned future finite element will be overwritten.     See notes of
   * future_fe_index() for information about restrictions on this
   * functionality.
   *
   */
  void
  ...
  /**
   * Return whether a future finite element has been set.     See notes of
   * future_fe_index() for information about restrictions on this
   * functionality.
   *
   */
  ...
  /**
   * Revoke the future finite element assigned. Thus, the active finite
   * element   will remain unchanged next time the triangulation gets refined
   * and   coarsened.     See notes on future_fe_index() for information about
   * restrictions on this   functionality.
   *
   */
  void
