 ...
 * ubiquitous part of each finite element program. The workhorse of this
 * namespace is the loop() function, which implements a completely generic
 * loop over all mesh cells. Since the calls to loop() are error-prone due to
 * its generality, for many applications it is advisable to derive a class
 * from   MeshWorker::LocalIntegrator   and use the less general
 * integration_loop() instead. The loop() depends on certain objects handed to
 * it as arguments. These objects are of two types,   @p info   objects like
 * DoFInfo and IntegrationInfo and worker objects like LocalWorker and
 * IntegrationWorker. Worker objects usually do two different jobs: first,
 * they compute the local contribution of a cell or face to the global
 * operation. Second, they assemble this local contribution into the global
 * result, whether a functional, a form or a bilinear form. While the first
 * job is particular to the problem being solved, the second is generic and
 * only depends on the data structures. Therefore, base classes for workers
 * assembling into global data are provided in the namespace Assembler.
 * <h3>Template argument types</h3> The functions loop() and cell_action()
 * take some arguments which are template parameters. Let us list the minimum
 * requirements for these classes here and describe their properties.
 * <h4>ITERATOR</h4> Any object that has an <tt>operator++()</tt> and points
 * to a TriaAccessor or derived class. <h4>DOFINFO</h4> For an example
 * implementation, refer to the class template DoFInfo. In order to work with
 * cell_action() and loop(), DOFINFO needs to follow the following interface.
 *
 * @code
 * class DOFINFO
 * {
 ...
 * declaration at the end may be missing. Additionally, you will need at least
 * one public constructor. Furthermore DOFINFO is pretty useless yet:
 * functions to interface with INTEGRATIONINFO and ASSEMBLER are needed.
 ...
 * global data. <h4>INFOBOX</h4> This type is exemplified in
 * IntegrationInfoBox. It collects the input data for actions on cells and
 * faces in INFO objects (see below). It provides the following interface to
 * loop() and cell_action():
 *
 *
 * @code
 * class INFOBOX
 ...
 * these 5 objects with the names listed above. The two function templates are
 * call back functions called in cell_action(). The first is called before the
 * faces are worked on, the second after the faces. <h4>INFO</h4> See
 * IntegrationInfo for an example of these objects. They contain the temporary
 * data needed on each cell or face to compute the result. The MeshWorker only
 * uses the interface
 *
 *
 * @code
 ...
 * @endcode
 *
 * <h3>Simplified interfaces</h3> Since the loop() is fairly general, a
 * specialization integration_loop() is available, which is a wrapper around
 * loop() with a simplified interface. The integration_loop() function loop
 * takes most of the information that it needs to pass to loop() from an
 * IntegrationInfoBox object. Its use is explained in   step-12  , but in
 * short it requires functions that do the local integration on a cell,
 * interior or boundary face, and it needs an object (called "assembler") that
 * copies these local contributions into the global matrix and right hand side
 * objects. Before we can run the integration loop, we have to initialize
 * several data structures in our IntegrationWorker and assembler objects. For
 * instance, we have to decide on the quadrature rule or we may need more than
...
{
  /**
   * The class providing the scrapbook to fill with results of local   integration. Depending on the task the mesh worker loop is performing,   local results can be of different types: They can be scalars, vectors   of size equal to the number of degrees of freedom used in the integrals,   or square matrices of that same size. All of these have in common that they   are the result of local integration over a cell or face. Which kind of   object is the result of an operation is determined by the Assembler using   them. It is also the assembler that determines <i>how many</i> of each   kind of object are produced (for example, an assembler may create   both the local contributions to a mass and a stiffness matrix), and for   setting the arrays of local results to the sizes needed.     The interface of this class allows accessing all of this information   via the following functions:       <ol>       <li>   Scalars: n_values() returns the number of scalars stored by   an object of this class, and they are accessed via the value() function.       <li>   Vectors: n_vectors() returns the number of vectors stored by   an object of this class (each vector has length equal to the number of   degrees of freedom on this cell on which the integration happens).   The vectors are accessed by the vector() function.       <li>   Matrices: n_matrices() returns the number of matrices stored,   each of which is a square matrix of dimension equal to the number of   degrees of freedom per cell. The matrices are   accessed by matrix() with second argument <tt>false</tt>. These are   matrices coupling degrees of freedom in   the same cell. For fluxes across faces, there is an additional set of   matrices of the same size, with the dimension of these matrices being   according to the degrees of freedom on both cells. These are accessed   with matrix(), using the second argument <tt>true</tt>.     </ol>       The local matrices are initialized by reinit() of the   @p info   object and then   assembled into the global system by Assembler classes.
   * @ingroup MeshWorker
   *
   */
  ...
  public:
    /**
     * The number of scalar values stored by the current object.         This
     * number is set to a nonzero value by   Assembler::CellsAndFaces
     *
     */
    unsigned int
    n_values() const;
    ...
    /**
     * The number of vectors stored by the current object.         This number
     * is set to a nonzero value by   Assembler::ResidualSimple   and
    ...
    /**
     * Read-write access to the `i`th matrix stored by this class.         For
     * an explanation of the second argument, see the documentation     of the
    ...
    /**
     * Read access to the `i`th matrix stored by this class.         For an
     * explanation of the second argument, see the documentation     of the
     * current class itself.
     *
     */
  ...
  private:
     ...
     * functions on the other cell.         Only used on interior faces.
     *
     */
