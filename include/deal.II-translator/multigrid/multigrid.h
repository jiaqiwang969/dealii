...
{
  /**
   * A structure containing   boost::signal   objects for optional processing
   * in   multigrid solvers.     Each of these signals is called twice, once
   * before and once after   the action is performed. The two function calls
   * differ in the   boolean argument   @p before,   which is true the first
  ...
  {
    /**
     * This signal is triggered before (  @p before   is true) and after (  @p
     * before       is false) the call to   MGTransfer::copy_to_mg   which
     * transfers the vector     given to it to a multi-level vector.
     *
     */
    ...
    /**
     * This signal is triggered before (  @p before   is true) and after (  @p
     * before       is false) the call to   MGTransfer::copy_from_mg   which
     * transfers the     multi-level vector given to it to a normal vector.
     *
     */
    ...
    /**
     * This signal is triggered before (  @p before   is true) and after (  @p
     * before       is     false) the call to the coarse solver on   @p level.
    ...
    /**
     * This signal is triggered before (  @p before   is true) and after (  @p
     * before       is false) the call to   MGTransfer::restrict_and_add()
     * which restricts a     vector from   @p level   to the next coarser one
     * (  @p level
     *
    ...
    /**
     * This signal is triggered before (  @p before   is true) and after (  @p
     * before       is false) the call to   MGTransfer::prolongate()   which
     * prolongs a vector to       @p level   from the next coarser one (  @p
     * level
     *
     *  - 1).
     *
    ...
    /**
     * This signal is triggered before (  @p before   is true) and after (  @p
     * before       is false) the call to a pre-smoothing step via
     * MGPreSmoother::apply()   on       @p level.           The smoother
     * result will be stored in ``solution[level]`` and can be     inspected
     * by the user using this signal.
     *
     */
    boost::signals2::signal<void(const bool before, const unsigned int level)>
      pre_smoother_step;
    ...
    /**
     * This signal is triggered before (  @p before   is true) and after (  @p
     * before       is false) the call to a post-smoothing step via
     * MGPostSmoother::apply()       on       @p level.
     *
     */
    boost::signals2::signal<void(const bool before, const unsigned int level)>
      post_smoother_step;
...
/**
 * Implementation of the multigrid method. The implementation supports both continuous and discontinuous elements and follows the procedure described in the   @ref mg_paper   "multigrid paper by Janssen and Kanschat".
 ...
 * where the latter one can do different types of cycles. Using this class, it
 * is expected that the right hand side has been converted from a vector
 * living on the locally finest level to a multilevel vector. This is a
 * nontrivial operation, usually initiated automatically by the class
 * PreconditionMG and performed by the classes derived from MGTransferBase.
 *
 *
 * @note   The interface of this class is still very clumsy. In particular,
 * you will have to set up quite a few auxiliary objects before you can use
 * it. Unfortunately, it seems that this can be avoided only be restricting
 * the flexibility of this class in an unacceptable way.
 *
 *
 */
...
public:
   ...
   * valid matrices. By default the maxlevel is set to the maximal valid
   * level.     This function already initializes the vectors which will be
   * used later in   the course of the computations. You should therefore
   * create objects of   this type as late as possible.
   *
   */
   ...
   * use these vectors yourself.     The actual work for this function is done
   * in level_v_step().
   *
   */
  void
   ...
   * @note   While <tt>edge_out.vmult</tt> is used, for the second argument,
   * we   use <tt>edge_in.Tvmult</tt>. Thus, <tt>edge_in</tt> should be
   * assembled   in transposed form. This saves a second sparsity pattern for
   ...
   * @note   While <tt>edge_down.vmult</tt> is used, for the second argument,
   * we   use <tt>edge_up.Tvmult</tt>. Thus, <tt>edge_up</tt> should be
   * assembled   in transposed form. This saves a second sparsity pattern for
   ...
   * the current #maxlevel.     If <tt>relative</tt> is <tt>true</tt>, then
   * this function determines the   number of levels used, that is, it sets
   * #minlevel to   #maxlevel-<tt>level</tt>.
 ...
 * iterative methods. Furthermore, it needs functions <tt>void
 * copy_to_mg(const VectorType&)</tt> to store   @p src   in the right hand
 * side of the multi-level method and <tt>void copy_from_mg(VectorType&)</tt>
 * to store the result of the v-cycle in   @p dst. If VectorType is in fact a
 * block vector and the TRANSFER object supports use of a separate DoFHandler
 * for each block, this class also allows to be initialized with a separate
...
public:
  ...
  /**
   * Same as above in case every component of a block vector   uses its own
   * DoFHandler.
   *
   */
   ...
   * object passed to the constructor.     This is the operator used by LAC
   * iterative solvers.
   *
   */
  ...
  /**
   * Transposed preconditioning operator.     Not implemented, but the
...
private:
  ...
  /**
   * Storage for the pointers to the DoFHandler objects   without SmartPointer
   * wrapper.
   *
   */
