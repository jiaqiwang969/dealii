 ...
 * This is
 * the base class for MGTransfer objects for systems of equations where
 * multigrid is applied only to one or some blocks, where a   @ref GlossBlock
...
protected:
  ...
  /**
   * Flag of selected blocks.     The transfer operators only act on the
   * blocks having a <tt>true</tt>   entry here.
   *
   */
 ...
 * encounter problems, please contact Guido Kanschat. In addition to the
 * functionality of MGTransferPrebuilt, the operation may be restricted to
 * certain blocks of the vector. If the restricted mode is chosen, block
 * vectors used in the transfer routines may only have as many blocks as there
 * are   @p trues   in the selected-field. See MGTransferBase to find out
 * which of the transfer classes is best for your needs.
 *
 *
 */
...
public:
  ...
  /**
   * Build the prolongation matrices for each level.     This function is a
   * front-end for the same function in   MGTransferBlockBase.
   *
   */
  template <int dim, int spacedim>
  void
   ...
   * src.   All the   other level objects are left untouched.     The action
   * for discontinuous elements is as follows: on an active mesh   cell, the
   * global vector entries are simply copied to the corresponding   entries of
   * the level vector. Then, these values are restricted down to   the
   * coarsest level.
   *
   */
 ...
 * BlockVector. See MGTransferBase to find out which of the transfer classes
...
public:
  ...
  /**
   * Actually build the prolongation matrices for grouped blocks.     This
   * function is a front-end for the same function in   MGTransferBlockBase.
   * @param   dof_handler The DoFHandler to use.     @param   selected Number
   * of the block for which the transfer matrices   should be built.
   *
   */
  template <int dim, int spacedim>
  void
   ...
   * refined mesh only the finest level in   @p dst   is filled as a plain
   * copy of     @p src.   All the other level objects are left untouched.
   *
   */
  template <int dim, typename number2, int spacedim>
  void
   ...
   * Only the values for the active degrees of freedom of the block selected
   * are   transferred. In particular, for a globally refined mesh only the
   * finest   level in   @p dst   is filled as a plain copy of   @p src.   All
   * the other level   objects are left untouched.
   *
   */
  template <int dim, typename number2, int spacedim>
  void
