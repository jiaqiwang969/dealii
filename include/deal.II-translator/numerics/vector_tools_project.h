...
namespace VectorTools
{
  ...
  /**
   * Compute the projection of   @p function   to the finite element space. In
   * other   words, given a function   $f(\mathbf x)$  , the current function
   * computes a   finite element function   $f_h(\mathbf x)=\sum_j F_j
   * \varphi_j(\mathbf x)$     characterized by the (output) vector of nodal
   * values   $F$   that satisfies   the equation
   * @f{align*}{
   * (\varphi_i, f_h)_\Omega = (\varphi_i,f)_\Omega
   * @f}
   * for all test functions   $\varphi_i$  . This requires solving a linear
   * system   involving the mass matrix since the equation above is equivalent
   * to   the linear system
   * @f{align*}{
   * \sum_j (\varphi_i, \varphi_j)_\Omega F_j = (\varphi_i,f)_\Omega
   * @f}
   * which can also be written as   $MF = \Phi$   with     $M_{ij} =
   * (\varphi_i, \varphi_j)_\Omega$   and     $\Phi_i = (\varphi_i,f)_\Omega$
   ...
   * the boundary values of   $f$   onto the finite element space on the
   * boundary   of the mesh in a similar way to above, and then using these
   * values as the   imposed boundary values for   $f_h$  . The ordering of
   * arguments to this   function is such that you need not give a second
   * quadrature formula (of   type `Quadrature<dim-1>` and used for the
   * computation of the matrix and   right hand side for the projection of
   * boundary values) if you   don't want to project to the boundary first,
   * but that you must if you want   to do so.     A MatrixFree implementation
   * is used if the following conditions are met:
   *
   *
   *
   *
  ...
  /**
   * Call the project() function above, with   <tt>mapping=MappingQGeneric
   * @<dim@>(1)</tt>.
   *
   */
  template <int dim, typename VectorType, int spacedim>
  ...
  /**
   * The same as above for projection of scalar-valued quadrature data.   The
   * user provided function should return a value at the quadrature point
   ...
   * to assemble the right-hand-side.     This function can be used with
   * lambdas:
   * @code
   * VectorTools::project
   * (mapping,
   ...
   * data.   The user provided function   @p func   should return a
   * VectorizedArray value   at the quadrature point based on the cell number
   * and quadrature number and   should be consistent with the   @p
   * n_q_points_1d.       This function can be used with lambdas:
   * @code
   * VectorTools::project
   * (matrix_free_data,
   * constraints,
   ...
   * VectorizedArray<double> >, which stores quadrature point data.       @p
   * fe_component   allow to additionally specify which component of   @p data
