...
namespace VectorTools
{
   ...
   * for the whole domain.   Let   $f:\Omega \rightarrow \mathbb{R}^c$   be a
   * finite element function   with   $c$   components where component   $c$
   * is denoted by   $f_c$   and   $\hat{f}$     be the reference function
   * (the   @p fe_function   and   @p exact_solution     arguments to
   * integrate_difference()). Let   $e_c = \hat{f}_c
   *
   * - f_c$     be the difference or error between the two. Further,   let    $w:\Omega \rightarrow \mathbb{R}^c$   be the   @p weight   function of integrate_difference(), which is   assumed to be equal to one if not supplied. Finally, let   $p$   be the     @p exponent   argument (for   $L_p$  -norms).     In the following,we denote by   $E_K$   the local error computed by   integrate_difference() on cell   $K$  , whereas   $E$   is the global error   computed by compute_global_error(). Note that integrals are   approximated by quadrature in the usual way:   @f[
   * \int_A f(x) dx \approx \sum_q f(x_q) \omega_q.
   * @f]   Similarly for suprema over a cell   $T$  :   @f[
  ...
  {
    /**
     * The function or difference of functions is integrated on each cell   $K$  :     @f[
     * E_K
     * = \int_K \sum_c (\hat{f}_c
     *
     * - f_c) \, w_c
     * = \int_K \sum_c e_c \, w_c
     * @f]     and summed up to get     @f[
     * E = \sum_K E_K = \int_\Omega \sum_c (\hat{f}_c
     *
     * - f_c) \, w_c
     * @f]     or, for   $w \equiv 1$  :     @f[
     * E = \int_\Omega (\hat{f}
     *
     * - f) = \int_\Omega e. @f]         Note: This differs from what is
     * typically known as     the mean of a function by a factor of
     * $\frac{1}{|\Omega|}$  . To     compute the mean you can also use
     * compute_mean_value(). Finally,     pay attention to the sign: if
     * $\hat{f}=0$  , this will compute the     negative of the mean of   $f$
     * .
     *
     */
    mean,
    ...
    /**
     * The absolute value of the function is integrated:     @f[
    ...
    /**
     * The square of the function is integrated and the square root of the     result is computed on each cell:     @f[
    ...
    /**
     * The absolute value to the   $p$  -th power is integrated and the   $p$  -th     root is computed on each cell. The exponent   $p$   is the   @p       exponent argument of integrate_difference() and compute_mean_value():     @f[
    ...
    /**
     * The maximum absolute value of the function:     @f[
    ...
    /**
     * #L2_norm of the divergence of a vector field. The function   $f$   is     expected to have   $c \geq \text{dim}$   components and the first   @p dim       will be used to compute the divergence:     @f[
    ...
    /**
     * The square of this norm is the square of the #L2_norm plus the square     of the #H1_seminorm:     @f[
    ...
    /**
     * The same as the #H1_norm but using <i>L<sup>p</sup></i>:     @f[
    ...
    /**
     * The sum of #Linfty_norm and #W1infty_seminorm:     @f[
     * E_K = \sup_K \max_c |e_c| \, w_c + \sup_K \max_c |\nabla e_c| \, w_c.
     * @f]     The global norm is not implemented in compute_global_error(),     because it is impossible to compute the sum of the global     norms from the values   $E_K$  . As a work-around, you can compute the     global #Linfty_norm and #W1infty_seminorm separately and then add them     to get (with   $w \equiv 1$  ):     @f[
     * E = \| e \|_{L^\infty} + \| \nabla e \|_{L^\infty}. @f]
     *
     */
    W1infty_norm
