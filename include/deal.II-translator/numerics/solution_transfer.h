 ...
 * values. This class is used in the   step-15  ,   step-26  ,   step-31  ,
 * and   step-33   tutorial programs. A version of this class that works on
 * parallel triangulations is available as
 * parallel::distributed::SolutionTransfer. <h3>Usage</h3>
 * This class implements the algorithms in two different ways:   <ul>     <li>   If the grid will only be refined (i.e. no cells are coarsened) then use   @p SolutionTransfer   as follows:
 *
 * @code
 ...
 * important to note that the old solution(s) must be copied to one that also
 * provides access to the locally relevant DoF values (these values required
 ...
 * @endcode
 *
 * Multiple calls to the function <code>interpolate (const VectorType &in, VectorType &out)</code> are NOT allowed. Interpolating several functions can be performed in one step by using <tt>void interpolate (const vector<VectorType> &all_in, vector<VectorType> &all_out) const</tt>, and using the respective   @p   prepare_for_coarsening_and_refinement function taking several vectors as input before actually refining and coarsening the triangulation (see there).   </ul>
 * For deleting all stored data in   @p SolutionTransfer   and reinitializing
 * it use the <tt>clear()</tt> function. The template argument   @p VectorType
 * denotes the type of data container you want to transfer.
 *
 *  <h3>Interpolating in the presence of hanging nodes and boundary
 * values</h3> The interpolation onto the new mesh is a local operation, i.e.,
 * it interpolates onto the new mesh only. If that new mesh has hanging nodes,
 ...
 * and set into the vector   @p out   that is at the end the discrete function
 * @p   in interpolated on the refined mesh. The
 * <tt>refine_interpolate(in,out)</tt> function can be called multiple times
 * for arbitrary many discrete functions (solution vectors) on the original
 * grid.
 ...
 * that are not needed any more will be deleted from the Triangulation. For
 * the interpolation from the (to be coarsenend) children to their father the
 * children cells are needed. Hence this interpolation and the storing of the
 * interpolated values of each of the discrete functions that we want to
 ...
 * <tt>vector<VectorType> all_in</tt> includes all discrete functions to be
 * interpolated onto the new grid. As we need two different kinds of pointers
 * (<tt>vector<unsigned int></tt> for the Dof indices and
 * <tt>vector<VectorType></tt> for the interpolated DoF values) we use the
 * @p Pointerstruct   that includes both of these pointers and the pointer for
 * each cell points to these   @p   Pointerstructs. On each cell only one of
 * the two different pointers is used at one time hence we could use a
 ...
 * In <tt>interpolate(all_in, all_out)</tt> the refined cells are treated according to the solution transfer while pure refinement. Additionally, on each cell that is coarsened (hence previously was a father cell), the values of the discrete functions in   @p all_out   are set to the stored local interpolated values that are accessible due to the 'vector<VectorType>' pointer in   @p Pointerstruct   that is pointed to by the pointer of that cell. It is clear that <tt>interpolate(all_in, all_out)</tt> only can be called with the <tt>vector<VectorType> all_in</tt> that previously was the parameter of the <tt>prepare_for_coarsening_and_refinement(all_in)</tt> function. Hence <tt>interpolate(all_in, all_out)</tt> can (in contrast to <tt>refine_interpolate(in, out)</tt>) only be called once.   </ul>
 *
 *  <h3>Interaction with hanging nodes</h3> This class does its best to
 * represent on the new mesh the finite element function that existed on the
 * old mesh, but this may lead to situations where the function on the new
 * mesh is no longer conforming at hanging nodes. To this end, consider a
 * situation of a twice refined mesh that started with a single square cell
 * (i.e., we now have 16 cells). Consider also that we coarsen 4 of the cells
 * back to the first refinement level. In this case, we end up with a mesh
 * that will look as follows if we were to use a   $Q_1$   element:
 ...
 * constraints from hanging nodes</i>. Whether this is a problem you need to
 * worry about or not depends on your application. The situation is easily
 * corrected, of course, by applying   AffineConstraints::distribute()   to
 * your solution vector after transfer, using a constraints object computed on
 * the new DoFHandler object (you probably need to create this object anyway
 * if you have hanging nodes). This is also what is done, for example, in
 * step-15  .
 *
 *
 ...
 *
 *  <h3>Implementation in the context of hp-finite elements</h3> In the case
 * of DoFHandlers with hp-capabilities, nothing defines which of the finite
 * elements that are part of the   hp::FECollection   associated with the
 ...
 * non- active cells using   DoFAccessor::set_active_fe_index(). It is, thus,
 * not entirely natural what should happen if, for example, a few cells are
 * coarsened away. This class then implements the following algorithm:
 *
 *
 *
...
public:
  ...
  /**
   * Prepares the   @p SolutionTransfer   for pure refinement. It stores the
   * dof   indices of each cell. After calling this function only calling the
   * @p     refine_interpolate functions is allowed.
   *
   */
  void
   ...
   * includes all   vectors that are to be interpolated onto the new (refined
   * and/or   coarsenend) grid.
   *
   */
  void
  ...
  /**
   * This function interpolates the discrete function   @p in,   which is a
   * vector   on the grid before the refinement, to the function   @p out
   * which then is a   vector on the refined grid. It assumes the vectors
   * having the right sizes   (i.e. <tt>in.size()==n_dofs_old</tt>,
   * <tt>out.size()==n_dofs_refined</tt>)     Calling this function is allowed
   * only if   @p prepare_for_pure_refinement     is called and the refinement
   * is executed before. Multiple calling of this   function is allowed. e.g.
   * for interpolating several functions.
   *
   */
  void
   ...
   * no way of verifying this internally, so be careful here.     Calling this
   * function is allowed only if first
   ...
   * functions can be performed in one step.     The number of output vectors
   * is assumed to be the same as the number of   input vectors. Also, the
   * sizes of the output vectors are assumed to be of   the right size (  @p
   * n_dofs_refined).   Otherwise an assertion will be   thrown.
   *
   */
  void
...
private:
  ...
  /**
   * Declaration of   @p PreparationState   that denotes the three possible
   * states   of the   @p SolutionTransfer:   being prepared for 'pure
   * refinement',   prepared for 'coarsening and refinement' or not prepared.
   *
   */
   ...
   * from each cell. As each cell has got only one   @p user_pointer,
   * multiple   pointers to the data need to be packetized in a structure.
   * Note that in   our case on each cell either the <tt>vector<unsigned int>
   * indices</tt>   (if the cell will be refined) or the <tt>vector<double>
   * dof_values</tt>   (if the children of this cell will be deleted) is
   * needed, hence one   @p     user_pointer should be sufficient, but to
   * allow some error checks and to   preserve the user from making user
   * errors the   @p user_pointer   will be   'multiplied' by this structure.
   *
   */
  ...
  /**
   * Map mapping from level and index of cell to the   @p Pointerstructs
   * (cf.   there). This map makes it possible to keep all the information
   * needed to   transfer the solution inside this object rather than using
   * user pointers   of the Triangulation for this purpose.
   *
   */
  std::map<std::pair<unsigned int, unsigned int>, Pointerstruct> cell_map;

  /**
   * Is used for   @p prepare_for_refining_and_coarsening   The interpolated
