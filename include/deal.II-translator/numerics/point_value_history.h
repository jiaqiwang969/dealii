 ...
 * spreadsheets. The user can store extra variables which do not relate to
 * mesh location specifying n_independent_variables. The class then expects a
 * std::vector   of size n_independent_variables to be added during each step
 * using the method   @p push_back_independent.   This may be used for example
 * for recording external input, logging solver performance data such as time
 * taken to solve the step and solver steps before convergence, saving norms
 * calculated, or simply saving the time, number of time step, or number of
 * nonlinear iteration along with the data evaluated from the mesh.
 * The three "evaluate field" methods each have different strengths and weaknesses making each suitable for different contexts:   <ol>     <li>  Firstly, the   @p evaluate_field   version that does not take a   @p   DataPostprocessor object selects the nearest support point (see   @ref GlossSupport   "this entry in the glossary" ) to a given point to extract data from. This makes the code that needs to be run at each time step very short, since looping over the mesh to extract the needed dof_index can be done just once at the start. However, this method is not suitable for FiniteElement objects that do not assign dofs to actual mesh locations (i.e. FEs without   @ref GlossSupport   "support points" ) or if adaptive mesh refinement is used. The reason for the latter restriction is that the location of the closest support point to a given point may change upon mesh refinement. The class will throw an exception if any change to the triangulation is made (Although the nearest support point could be re- computed upon mesh refinement, the location of the support point will most likely change slightly, making the interpretation of the data difficult, hence this is not implemented currently.)
 * <li>   Secondly,   @p evaluate_field_at_requested_location   calls   @p
 * VectorTools::point_value   to compute values at the specific point
 * requested. This method is valid for any FE that is supported by   @p
 * VectorTools::point_value.   Specifically, this method can be called by
 * codes using adaptive mesh refinement.
 * <li>  Finally, the class offers a function   @p evaluate_field   that takes
 * a   @p   DataPostprocessor object. This method allows the deal.II data
 * postprocessor to be used to compute new quantities from the solution on the
 * fly. The values are located at the nearest quadrature point to the
 * requested point. If the mesh is refined between calls, this point will
 * change, so care must be taken when using this method in code using adaptive
 * refinement, but as the output will be meaningful (in the sense that the
 * quadrature point selected is guaranteed to remain in the same vicinity, the
 * class does not prevent the use of this method in adaptive codes. The class
 * provides warnings in the output files if the mesh has changed. Note that
 * one can reduce the error this procedure introduces by providing a
 * quadrature formula that has more points, at the expense of performing more
 * work since then the closest quadrature points is nearer to the point at
 * which the evaluation is really supposed to happen. (As a sidenote: Why not
 * do the evaluation at the requested point right away? The reason for this is
 * that it would require setting up a new quadrature point object on each cell
 * that has only a single point corresponding to the reference coordinates of
 * the point you really want; then initializing a FEValues object with it;
 * then evaluating the solution at this point; then handing the result to the
 * DataPostprocessor object. This sequence of things is expensive
 *
 * -  which is the reason why   VectorTools::point_value()   is expensive. Using the same quadrature formula on each cell on which we want to evaluate the solution and only having to initialize a FEValue object once is a much cheaper alternative, albeit of course at the expense of getting only an approximate result.)   </ol>
 ...
 * instead if desired. Following is a little code snippet that shows a common
 * usage of this class:
 *
 *
 * @code
...
public:
   ...
   * reads specific data from the   @p DoFHandler   and stores it internally
   * for   quick access (in particular dof indices of closest neighbors to
   * requested   points) the class is fairly intolerant to changes to the   @p
   * DoFHandler   if   data at support points is required. Mesh refinement and
   * @p DoFRenumbering     methods should be performed before the   @p
   * add_points   method is called and   adaptive grid refinement is only
   * supported by some methods.     The user can store extra variables which
   * do not relate to mesh location   by specifying the number required using
   * n_independent_variables and   making calls to   @p push_back_independent
   * as needed.  This may be used for   example for recording external input
   * or logging solver performance data.
   *
   */
   ...
   * stored for use when   @p evaluate_field   is called. If more than one
   * point   is required rather use the   @p add_points   method since this
   * minimizes   iterations over the mesh.
   *
   */
  void
   ...
   * stored for use when   @p evaluate_field   is called. If more than one
   * point   is required, rather call this method as it is more efficient than
   * the   add_point method since it minimizes iterations over the mesh. The
   * points   are added to the internal database in the order they appear in
   * the list   and there is always a one to one correspondence between the
   * requested   point and the added point, even if a point is requested
   * multiple times.
   *
   */
  void
   ...
   * are already in the class, so   @p add_field_name   and   @p add_points
   ...
   * add_field_name and   @p add_points   can be called in any order. This
   * method   generates a   std::vector   0, ..., n_components-1 and calls the
   * previous   function.
   *
   */
  void
  ...
  /**
   * Compute values using a   @p DataPostprocessor   object with the   @p
   * VectorType     supplied and add them to the new dataset in vector_name.
   * The   component_mask supplied when the field was added is used to select
   * components to extract from the   @p DataPostprocessor   return vector.
   * This   method takes a vector of field names to process and is preferred
   * if many   fields use the same   @p DataPostprocessor   object as each
   * cell is only   located once. The quadrature object supplied is used for
   * all components   of a vector field. Although this method will not throw
   * an exception if   the mesh has changed. (No internal data structures are
   * invalidated as the   quadrature points are repicked each time the
   * function is called.)   Nevertheless the user must be aware that if the
   * mesh changes the point   selected will also vary slightly, making
   * interpretation of the data more   difficult. If a   @p DoFHandler   is
   * used, one (and only one) evaluate_field   method must be called for each
   * dataset (time step, iteration, etc) for   each vector_name, otherwise a
   * @p ExcDataLostSync   error can occur.
   *
   */
  template <class VectorType>
  void
  ...
  /**
   * Construct a   std::vector     <std::string>   containing only vector_name
   * and   call the above function. The above function is more efficient if
   * multiple   fields use the same   @p DataPostprocessor   object.
   *
   */
  template <class VectorType>
  void
   ...
   * when the field was added is used to select components to extract. If a
   * @p     DoFHandler is used, one (and only one) evaluate_field method must
   * be   called for each dataset (time step, iteration, etc) for each
   * vector_name,   otherwise a   @p ExcDataLostSync   error can occur.
   *
   */
  template <class VectorType>
  void
   ...
   * points   selected and interpreting the data. If   @p n_indep   != 0 an
   * additional file   base_name + "_indep.gpl" containing key and independent
   * data. The file   name agrees with the order the points were added to the
   * class. The names   of the data columns can be supplied using the
   * functions   @p     add_component_names and   @p add_independent_names.
   * The support point   information is only meaningful if the dof_handler has
   * not been changed.   Therefore, if adaptive mesh refinement has been used
   * the support point   data should not be used. The optional parameter
   * postprocessor_locations   is used to add the postprocessor locations to
   * the output files. If this   is desired, the data should be obtained from
   * a call to   get_postprocessor_locations while the dof_handler is usable.
   * The default   parameter is an empty vector of strings, and will suppress
   * postprocessor   locations output.
   *
   */
  void
  ...
  /**
   * Return a   @p Vector   with the indices of selected points flagged with a
   * 1.   This method is mainly for testing and verifying that the class is
   * working   correctly. By passing this vector to a DataOut object, the user
   * can   verify that the positions returned by   @p get_support_locations
   * agree with   the positions that   @p DataOut   interprets from the   @p
   * Vector   returned. The   code snippet below demonstrates how this could
   * be done:
   * @code
   ...
   * requested, for example by using the   @p Point<dim>::distance   function.
   * For   convenience, location is resized to the correct number of points by
   * the   method.
   *
   */
  void
   ...
   * using the   @p Point<dim>::distance   function. Unlike the
   * support_locations,   these locations are computed every time the
   * evaluate_field method is   called with a postprocessor. This method uses
   * the same algorithm so can   will find the same points. For convenience,
   * location is resized to the   correct number of points by the method.
   *
   */
  void
   ...
   * @p PointValueHistory::get_support_locations   and   @p
   * PointValueHistory::status     do not require the class to be closed. If a
   * method that requires a class to   be open or close is called while in the
   * wrong state a   @p ExcInvalidState     exception is thrown.
   *
   */
  void
  close();
  ...
  /**
   * Delete the lock this object has to the   @p DoFHandler   used the last
   * time   the class was created.  This method should not normally need to be
   * called, but can be useful to ensure that the   @p DoFHandler   is
   * released   before it goes out of scope if the   @p PointValueHistory
   * class might live   longer than it. Once this method has been called, the
   * majority of methods   will throw a   @p ExcInvalidState   exception, so
   * if used this method should   be the last call to the class.
   *
   */
  void
  clear();
   ...
   * often used in   @p Assert   statements with the   @p ExcDataLostSync
   * exception.   If   @p strict   is   @p false   this method returns   @p
   * true   if all sizes are   within 1 of each other (needed to allow data to
   * be added), with   @p strict     =   @p true   they must be exactly equal.
   *
   */
...
private:
   ...
   * point_n-1_components][key]   This format facilitates scalar mnemonics in
   * a vector space, because   scalar mnemonics will only have one component
   * per point. Vector   components are strictly FE.n_components () long.
   *
   */
   ...
   * triangulation is modified.     It is currently used to check if the
   * triangulation has changed,   invalidating precomputed values.
   *
   */
  void
