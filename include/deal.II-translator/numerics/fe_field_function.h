...
namespace Functions
{
   ...
   * needs to find out where the points lie.     If you know in advance in
   * which cell your points lie, you can accelerate   things a bit, by calling
   * set_active_cell() before asking for values or   gradients of the
   * function. If you don't do this, and your points don't   lie in the cell
   * that is currently stored, the function
   * GridTools::find_active_cell_around_point   is called to find out where
   * the   point is.   You can specify an optional mapping to use when looking
   * for points in   the grid. If you don't do so, this function uses a Q1
   * mapping.     Once the FEFieldFunction knows where the points lie, it
   * creates a   quadrature formula for those points, and calls
   * FEValues::get_function_values   or   FEValues::get_function_gradients
   * with   the given quadrature points.     If you only need the quadrature
   * points but not the values of the finite   element function (you might
   * want this for the adjoint interpolation), you   can also use the function
   * compute_point_locations() alone.     An example of how to use this
   ...
   * of where the   points that will be computed actually are by using the
   ...
   * letting this class know.       <h3>Using FEFieldFunction with
   * parallel::distributed::Triangulation</h3>     When using this class with
   * a parallel distributed triangulation object   and evaluating the solution
   * at a particular point, not every processor   will own the cell at which
   * the solution is evaluated. Rather, it may be   that the cell in which
   * this point is found is in fact a ghost or   artificial cell (see     @ref
   * GlossArtificialCell     and     @ref GlossGhostCell  ).   The solution
   * can be evaluated on ghost cells, but for artificial cells   we have no
   * access to the solution there and   functions that evaluate the solution
   * at such a point will trigger an   exception of type
   * VectorTools::ExcPointNotAvailableHere.       To deal with this situation,
   * you will want to use code as follows when,   for example, evaluating the
   * solution at the origin (here using a parallel   TrilinosWrappers vector
  ...
  public:
     ...
     * cell (see       @ref GlossLocallyOwnedCell  ).     See the section in
     ...
     * function at the   @p points.   It is assumed that   @p values   already
     * has the     right size, i.e. the same size as the points array. This is
     * rather     efficient if all the points lie on the same cell. If this is
     * not the     case, things may slow down a bit.
     * @note
     * When using this function on a
     * parallel::distributed::Triangulation   you may get an exception when
     * trying to evaluate the solution at a point that lies on an artificial
     ...
     * points.   It     is assumed that   @p values   already has the right
     * size, i.e. the same     size as the points array. This is rather
     * efficient if all the points     lie on the same cell. If this is not
     * the case, things may slow down a     bit.
     * @note
     * When using this function on a
     * parallel::distributed::Triangulation   you may get an exception when
     * trying to evaluate the solution at a point that lies on an artificial
     ...
     * than one at a time, use the vector_value_list() function. For
     * efficiency     reasons, it is better if all the points lie on the same
     * cell. This is     not mandatory, however it does speed things up.
     * @note
     * When using this function on a
    ...
    /**
     * Given a set of points located in the domain (or, in the case of     a
     * parallel Triangulation, in the locally owned part of the domain     or
     * on the ghost cells for the current processor), sort these     points
     * into buckets for each of the cells on which at least     one of the
     * points is located.         This function fills three output vectors:
     * @p cells,     @p qpoints       and   @p maps.   The first is a list of
     * the cells that contain the     points, the second is a list of
     * quadrature points matching each     cell of the first list, and the
     * third contains the index of the     given quadrature points, i.e.,   @p
     * points[maps[3][4]]   ends up as     the 5th quadrature point in the 4th
     * cell.           @return   This function returns the number of cells
     * that       collectively contain the set of points give as   @p
     * points. This also equals the lengths of the output arrays.         This
     * function simply calls   GridTools::compute_point_locations   :
     * using the original function avoids computing a     new Cache at every
     * function call.
     *
     */
    unsigned int
