...
namespace VectorTools
{
  ...
  /**
   * Subtract the (algebraic) mean value from a vector.     This function is
   * most frequently used as a mean-value filter for Stokes:   The pressure in
   * Stokes' equations with only Dirichlet boundaries for the   velocities is
   * only determined up to a constant. This function allows to   subtract the
   * mean value of the pressure. It is usually called in a   preconditioner
   * and generates updates with mean value zero. The mean value   is computed
   * as the mean value of the degrees of freedom values as given   by the
   * input vector; they are not weighted by the area of cells, i.e. the   mean
   * is computed as   $\sum_i v_i$  , rather than as   $\int_\Omega v(x) =
   * \int_\Omega \sum_i
   * v_i \phi_i(x)$  . The latter can be obtained from the
   * VectorTools::compute_mean_function,   however.     Apart from the vector
   * @p v   to operate on, this function takes a boolean   mask   @p p_select
   * that has a true entry for every element of the vector   for which the
   * mean value shall be computed and later subtracted. The   argument is used
   * to denote which components of the solution vector   correspond to the
   * pressure, and avoid touching all other components of   the vector, such
   * as the velocity components. (Note, however, that the   mask is not a
   * @ref GlossComponentMask     operating on the vector components of the
   * finite element the solution   vector   @p v   may be associated with;
   * rather, it is a mask on the entire   vector, without reference to what
   * the vector elements mean.)     The boolean mask   @p p_select   has an
   * empty vector as default value, which   will be interpreted as selecting
   * all vector elements, hence, subtracting   the algebraic mean value on the
   * whole vector. This allows to call this   function without a boolean mask
   * if the whole vector should be processed.
   ...
   * not equal to   $(1,1,\ldots,1)^T$  . For such elements, a different
   * procedure   has to be used when subtracting the mean value.
  ...
  /**
   * Compute the mean value of one component of the solution.     This
   * function integrates the chosen component over the whole domain and
