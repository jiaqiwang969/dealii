 ...
 * documentation. We will explain the concept for a time dependent problem,
 * but instead of the time any parameter can be substituted. In our example, a
 * solution of an equation is computed for each discrete time level. This is
 * then added to an object of the present class and after all time levels are
 * added, a space- time plot will be written in any of the output formats
 * supported by the base class. Upon output, the (spatial) solution on each
 * time level is extended into the time direction by writing it twice, once
 * for the time level itself and once for a time equal to the time level minus
 * a given time step. These two copies are connected, to form a space-time
 * slab, with constant values in time. Due to the piecewise constant output in
 * time, the written solution will in general be discontinuous at discrete
 * time levels, but the output is still sufficient in most cases. More
 * sophisticated interpolations in time may be added in the future.
 *
 *  <h3>Example of Use</h3> The following little example shall illustrate the
 * different steps of use of this class. It is assumed that the finite element
 * used is composed of two components,   @p u   and   @p v,   that the
 * solution vector is named   @p solution   and that a vector   @p error   is
 * computed which contains an error indicator for each spatial cell. Note that
 * unlike for the DataOut class it is necessary to first declare data vectors
 * and the names of the components before first use. This is because on all
 * time levels the same data should be present to produce reasonable
 * time-space output. The output is generated with two subdivisions in each
 * space and time direction, which is suitable for quadratic finite elements
...
public:
   ...
   * parameter previously set by   @p new_parameter_value.       This has to
   * happen before adding data vectors for the present parameter   value.
   *
   */
  void
  ...
  /**
   * Declare a data vector. The   @p vector_type   argument determines whether
   * the   data vector will be considered as DoF or cell data.     This
   * version may be called if the finite element presently used by the
   ...
   * Declare a data vector. The   @p vector_type   argument determines whether
   * the   data vector will be considered as DoF or cell data.     This
   * version must be called if the finite element presently used by the
  ...
  /**
   * Add a data vector for the presently set value of the parameter.     This
   * version may be called if the finite element presently used by the
   * DoFHandler (and previously attached to this object) has only one
   * component and therefore only one name needs to be given.     If   @p vec
   * is a vector with multiple components this function will   generate
   * distinct names for all components by appending an underscore and   the
   * number of each component to   @p name       The data vector must have
   * been registered using the   @p     declare_data_vector function before
   * actually using it the first time.     Note that a copy of this vector is
   * stored until   @p finish_parameter_value     is called the next time, so
   * if you are short of memory you may want to   call this function only
   * after all computations involving large matrices   are already done.
   *
   */
   ...
   * you can also call this function with a
   * <tt>std::vector@<std::string@></tt>   containing only one element if the
   * finite element has only one component.     The data vector must have been
   * registered using the   @p     declare_data_vector function before
   * actually using it the first time.     Note that a copy of this vector is
   * stored until   @p finish_parameter_value     is called the next time, so
   * if you are short of memory you may want to   call this function only
   * after all computations involving large matrices   are already done.
  ...
  /**
   * Release all data that is no more needed once   @p build_patches   was
   * called   and all other transactions for a given parameter value are done.
