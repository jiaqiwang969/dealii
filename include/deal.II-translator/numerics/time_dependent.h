 ...
 * messing with my wave equation simulation for several months. The design of
 * this class is mostly tailored for the solution of time dependent partial
 * differential equations where the computational meshes may differ between
 * each two timesteps and where the computations on each time step take a
 * rather long time compared with the overhead of this class. Since no
 * reference to the class of problems is made within this class, it is not
 * restricted to PDEs, though, and it seems likely that a solver for large
 * ordinary matrix differential equations may successfully use the same setup
 * and therefore this class.
 *
 *  <h3>Overview</h3> The general structure of a time dependent problem solver
 * using a timestepping scheme is about the following: we have a collection of
 * time step objects on which we solve our problem subsequently. In order to
 * do so, we need knowledge of the data on zero or several previous timesteps
 * (when using single or multiple step methods, that is) and maybe also some
 * data of time steps ahead (for example the computational grid on these).
 * Depending on the problem in question, a second loop over all timesteps may
 * be done solving a dual problem, where the loop may run forward (one dual
 * problem for each time step) or backward (using a global dual problem).
 * Within one of these loops or using a separate loop, error estimators may be
 * computed and the grids may be refined. Each of these loops are initiated by
 * a call preparing each timestep object for the next loop, before actually
 * starting the loop itself. We will denote a complete set of all these loops
 * with the term "sweep". Since this library is mostly about adaptive methods,
 * it is likely that the last loop within a sweep will generate refined meshes
 * and that we will perform another sweep on these refined meshes. A total run
 * will therefore often be a sequence of several sweeps. The global setup
 ...
 * being needed for the solution of global dual problems, for example). Going
 * from the global overview to a more local viewpoint, we note that when a
 * loop visits one timestep (e.g. to solve the primal or dual problem, or to
 ...
 * latest time possible. In order to facilitate these principles, the concept
 * of waking up and letting sleep a time step object was developed. Assume we
 * have a time stepping scheme which needs to look ahead one time step and
 * needs the data of the last two time steps, the following pseudocode
 * describes what the central loop function of this class will do when we move
 ...
 * signal to the timestep where computations are done on. The calls to the
 * @p   wake_up and   @p sleep   functions with signal 0 could in principle be
 * absorbed into the function doing the computation; we use these redundant
 * signals, however, in order to separate computations and data management
 * from each other, allowing to put all stuff around grid management, data
 * reload and storage into one set of functions and computations into another.
 ...
 * this for you); timestep   @p n   builds matrices and sets solution vectors
 * to the right size, maybe using an initial guess; then it does the
 * computations; then it deletes the matrices since they are not needed by
 * subsequent timesteps; timestep   @p n-1   deletes those data vectors which
 * are only needed by one timestep ahead; timestep   @p n-2   deletes the
 * remaining vectors and deletes the computational grid, somewhere storing
 * information how to rebuild it eventually. From the given sketch above, it
 * is clear that each time step object sees the following sequence of events:
 *
 * @verbatim
 ...
 * sleep with signal 2
 * @endverbatim
 * This pattern is repeated for each loop in each sweep. For the different
 * loops within each sweep, the numbers of timesteps to look ahead (i.e. the
 * maximum signal number to the   @p wake_up   function) and the look-behind
 * (i.e. the maximum signal number to the   @p sleep   function) can be chosen
 * separately. For example, it is usually only needed to look one time step
 * behind when computing error estimation (in some cases, it may even be
 * possible to not look ahead or back at all, in which case only signals zero
 * will be sent), while one needs a look back of at least one for a
 * timestepping method. Finally, a note on the direction of look-ahead and
 * look-back is in place: look-ahead always refers to the direction the loop
 * is running in, i.e. for loops running forward,   @p wake_up   is called for
 * timestep objects with a greater time value than the one previously computed
 * on, while   @p sleep   is called for timesteps with a lower time. If the
 * loop runs in the opposite direction, e.g. when solving a global dual
 * problem, this order is reversed.
 *
 *  <h3>Implementation</h3> The main loop of a program using this class will
 * usually look like the following one, taken modified from an application
 * program that isn't distributed as part of the library:
 *
 * @code
 * template <int dim>
 ...
 * }
 * @endcode
 * Here,   @p timestep_manager   is an object of type TimeDependent_Wave<dim>,
 * which is a class derived from TimeDependent.   @p start_sweep,     @p
 * solve_primal_problem,   @p solve_dual_problem,     @p postprocess   and
 * @p   end_sweep are functions inherited from this class. They all do a loop
 * over all timesteps within this object and call the respective function on
 * each of these objects. For example, here are two of the functions as they
 ...
 * @p solve_primal_problem,     @p solve_dual_problem,     @p postprocess,
 * @p   refine_grids and   @p write_statistics   all have this form, where the
 * latter two give functions of the derived timestep class, rather than from
 * the base class). The function   TimeStepBase::init_for_primal_problem   and
 * the respective ones for the other operations defined by that class are only
 * used to store the type of operation which the loop presently performed will
 * do. As can be seen, most of the work is done by the   @p do_loop   function
 * of this class, which takes the addresses of two functions which are used to
 ...
 * run. Using lambda functions it is possible to do neat tricks, like the
 ...
 * @endcode
 * TimeStepBase_Wave<dim>::refine_grid   is a function taking an argument,
 * unlike all the other functions used above within the loops. However, in
 * this special case the parameter was the same for all timesteps and known
 * before the loop was started, so we fixed it and made a function object
 * which to the outside world does not take parameters. Since it is the
 * central function of this class, we finally present a stripped down version
 * of the   @p do_loop   method, which is shown in order to provide a better
 * understanding of the internals of this class. For brevity we have omitted
 * the parts that deal with backward running loops as well as the checks
 * whether wake-up and sleep operations act on timesteps outside
...
public:
  ...
  struct TimeSteppingData
  {
     ...
     * global in time.         However, it may be necessary to look ahead in
     * other circumstances, when     not wanting to access the data of the
     * next time step(s), but for     example to know the next grid, the
     * solution of a dual problem on the     next time level, etc.
     * Note that for a dual problem walking back in time, "looking ahead"
     * means looking towards smaller time values.         The value of this
     * number determines, how many time steps ahead the time     step manager
     * start to call the   @p wake_up   function for each time step.
     *
     */
     ...
     * in order to do the computations on the present time level.         For
     * one step schemes (e.g. the Euler schemes, or the Crank-Nicolson
     * scheme), this value will be one.         The value of this number
     * determines, how many time steps after having     done computations on a
     * time level the time step manager will call the   @p       sleep
   ...
   * sequence. If   @p position   is non-null, the new time step will be
   * inserted   before the respective element.     Note that by giving an
   * object to this function, the TimeDependent object   assumes ownership of
   * the object; it will therefore also take care of   deletion of the objects
   * its manages.     There is another function,   @p add_timestep,   which
   * inserts a time step at   the end of the list.     Note that this function
   * does not change the timestep numbers stored   within the other timestep
   * objects, nor does it set the timestep number of   this new timestep. This
   * is only done upon calling the   @p start_sweep     function. In not
   * changing the timestep numbers, it is simpler to operate   on a space-time
   * triangulation since one can always use the timestep   numbers that were
  ...
  /**
   * Just like   @p insert_timestep,   but insert at the end.     This
   * mechanism usually will result in a set-up loop like this
   * @code
   * for (int i=0; i<N; ++i)
   ...
   * object by the destructor is done through this function also.     Note
   * that this function does not change the timestep numbers stored   within
   * the other timestep objects. This is only done upon calling the   @p
   ...
   * and   @p solve_primal_problem   of the TimeStepBase class through the
   * @p     do_loop function of this class.     Look ahead and look back are
   * determined by the   @p     timestepping_data_primal object given to the
   * constructor.
   *
   */
  void
  ...
  /**
   * Solve the dual problem; uses the functions   @p init_for_dual_problem
   * and     @p solve_dual_problem   of the TimeStepBase class through the
   * @p do_loop     function of this class.     Look ahead and look back are
   * determined by the   @p timestepping_data_dual     object given to the
   * constructor.
   *
   */
  void
  ...
  /**
   * Do a postprocessing round; uses the functions   @p
   * init_for_postprocessing     and   @p postprocess   of the TimeStepBase
   * class through the   @p do_loop     function of this class.     Look ahead
   * and look back are determined by the   @p
  ...
  /**
   * Do a loop over all timesteps, call the   @p init_function   at the
   * beginning   and the   @p loop_function   of each time step. The   @p
   * timestepping_data     determine how many timesteps in front and behind
   * the present one the   @p     wake_up and   @p sleep   functions are
   * called.     To see how this function work, note that the function   @p
   ...
   * class, that is   @p static_castable   from a TimeStepBase. The function
   * may   be a virtual function (even a pure one) of that class, which should
   * help   if the actual class where it is implemented is one which is
   * derived   through virtual base classes and thus unreachable by   @p
   * static_cast   from   the TimeStepBase class.     Instead of using the
   * above form, you can equally well use   <tt>[args...](Xconst
   * x){x->unary_function(args...);}</tt>   which lets the   @p do_loop
   * function call the given function with the   specified parameters.
   *
   */
  template <typename InitFunctionObject, typename LoopFunctionObject>
  void
   ...
   * It also calls the   @p start_sweep   function of each time step object,
   * after   the numbers above are set.     This function is virtual, so you
   * may overload it. You should, however not   forget to call this function
   * as well from your overwritten version, at   best at the beginning of your
   * function since this is some kind of   "constructor-like" function, which
   * should be called bottom-up.     The default implementation of this
   * function calls   @p start_sweep   on all   time step objects.
   *
   */
  virtual void
  ...
  /**
   * Analogous to the above function, calling   @p end_sweep   of each time
   * step   object. The same applies with respect to the   @p virtualness   of
   * this   function as for the previous one.
   * @note   This function does not guarantee that   @p end_sweep   is called
   * for   successive time steps successively, rather the order of time step
   * objects   for which the function is called is arbitrary. You should
   * therefore not   assume that that function has been called for previous
   * time steps   already. If in multithread mode, the   @p end_sweep
   * function of several   time steps may be called at once, so you should use
   * synchronization   mechanisms if your program requires so.
   *
   */
  virtual void
  end_sweep();
...
protected:
  ...
  /**
   * Number of the present sweep. This is reset by the   @p start_sweep
   * function   called at the outset of each sweep.
   *
   */
  unsigned int sweep_no;

  /**
   * Some flags telling the   @p solve_primal_problem   function what to do.
   * See   the documentation of this struct for more information.
   *
   */
  const TimeSteppingData timestepping_data_primal;

  /**
   * Some flags telling the   @p solve_dual_problem   function what to do. See
 ...
 * functions (namely   @p sleep   and   @p wake_up),   the interface to
 * previous and following grids, and some functions to be called before a new
 * loop over all time steps is started.
 *
 *
 */
...
public:
   ...
   * vectors in case they have been stored to disk, etc.     The actual
   * implementation of this function does nothing.     Since this is an
   * important task, you should call this function from your   own function,
   * should you choose to overload it in your own class (which   likely is the
   * case), preferably at the beginning so that your function   can take
   * effect of the triangulation already existing.
   *
   */
  virtual void
  ...
  /**
   * This is the opposite function to   @p wake_up.   It is used to delete
   * data or   save it to disk after they are no more needed for the present
   * sweep.   Typical kinds of data for this are data vectors, degree of
   * freedom   handlers, triangulation objects, etc. which occupy large
   * amounts of   memory and may therefore be externalized.     By default,
   ...
   * files needed in the process of solving, etc.     At the time this
   * function is called, the values of   @p timestep_no,     @p     sweep_no
   * and the pointer to the previous and next time step object   already have
   * their correct value.     The default implementation of this function does
   * nothing.
   *
   */
  virtual void
   ...
   * default, this function sets the   @p next_action   variable of this
   * class.   You may overload this function, but you should call this
   ...
   * that, but would be a denormalized value such as   @p NaN.   However,
   * there is   not much difference in finding that the results of a
   * computation are all   denormalized values or in getting an exception; in
   * the latter case you at   least get the exact place where your problem
   * lies.)
   *
   */
  double
  ...
  /**
   * Determine an estimate for the memory consumption (in bytes) of this
   * object.     You will want to overload this function in derived classes to
   * compute the   amount memory used by the derived class, and add the result
   * of this   function to your result.
   *
   */
  virtual std::size_t
  memory_consumption() const;
...
private:
   ...
   * time level manager object.     This function is called at the set-up of
   * the manager object and whenever   a timestep is inserted or deleted.
   *
   */
  void
   ...
   * level manager object.     This function is called at the set-up of the
   * manager object and whenever   a timestep is inserted or deleted.
   *
   */
  void
...
namespace TimeStepBase_Tria_Flags
{
  ...
  struct Flags
  {
    ...
    /**
     * This flag determines whether the   @p sleep   and   @p wake_up
     * functions     shall delete and rebuild the triangulation.  While for
     * small problems,     this is not necessary, for large problems it is
     * indispensable to save     memory.  The reason for this is that there
     * may be several hundred time     levels in memory, each with its own
     * triangulation, which may require     large amounts if there are many
     * cells on each. Having a total of     100.000.000 cells on all time
     * levels taken together is not uncommon,     which makes this flag
     * understandable.
     *
     */
    ...
    /**
     * This number denotes the parameter to the   @p wake_up   function at
     * which     it shall rebuild the grid. Obviously, it shall be less than
     * or equal to     the   @p look_ahead   number passed to the time step
     * management object; if     it is equal, then the grid is rebuilt the
     * first time the   @p wake_up       function is called. If   @p
     * delete_and_rebuild_tria   is   @p false,   this     number has no
     * meaning.
     *
     */
    const unsigned int wakeup_level_to_build_grid;
  ...
  /**
   * This structure is used to tell the TimeStepBase_Tria() class how grids   should be refined. Before we explain all the different variables, fist   some terminology:     <ul>       <li>   Correction: after having flagged some cells of the triangulation for   following some given criterion, we may want to change the number of   flagged cells on this grid according to another criterion that the number   of cells may be only a certain fraction more or less then the number of   cells on the previous grid. This change of refinement flags will be   called "correction" in the sequel.     <li>   Adaption: in order to make the change between one grid and the next   not to large, we may want to flag some additional cells on one of the two   grids such that there are not too grave differences. This process will be   called "adaption".     </ul>         <h3>Description of flags</h3>       <ul>       <li>     @p max_refinement_level:   Cut the refinement of cells at a given   level. This flag does not influence the flagging of cells, so not more   cells on the coarser levels are flagged than usual. Rather, the flags are   all set, but when it comes to the actual refinement, the maximum   refinement level is truncated.     This option is only really useful when you want to compare global   refinement with adaptive refinement when you don't want the latter to   refine more than the global refinement.       <li>     @p first_sweep_with_correction:   When using cell number correction as   defined above, it may be worth while to start with this only in later   sweeps, not already in the first one. If this variable is zero, then   start with the first sweep, else with a higher one. The rationale for   only starting later is that we do not want to block the development of   grids at the beginning and only impose restrictions in the sweeps where   we start to be interested in the actual results of the computations.       <li>     @p min_cells_for_correction:   If we want a more free process of grid   development, we may want to impose less rules for grids with few cells   also. This variable sets a lower bound for the cell number of grids where   corrections are to be performed.       <li>     @p cell_number_corridor_top:   Fraction of the number of cells by   which the number of cells of one grid may be higher than that on the   previous grid. Common values are 10 per cent (i.e. 0.1). The naming of   the variable results from the goal to define a target corridor for the   number of cells after refinement has taken place.       <li>     @p cell_number_corridor_bottom:   Fraction of the number of cells by   which the number of cells of one grid may be lower than that on the   previous grid. Common values are 5 per cent (i.e. 0.05). Usually this   number will be smaller than   @p cell_number_corridor_top   since an increase   of the number of cells is not harmful (though it increases the numerical   amount of work needed to solve the problem) while a sharp decrease may   reduce the accuracy of the final result even if the time steps computed   before the decrease were computed to high accuracy.     Note however, that if you compute the dual problem as well, then the time   direction is reversed, so the two values defining the cell number   corridor should be about equal.       <li>     @p correction_relaxations:   This is a list of pairs of number with   the following meaning: just as for   @p min_cells_for_correction,   it may be   worth while to reduce the requirements upon grids if the have few cells.   The present variable stores a list of cell numbers along with some values   which tell us that the cell number corridor should be enlarged by a   certain factor. For example, if this list was <tt>((100 5) (200 3) (500   2))</tt>, this would mean that for grids with a cell number below 100,   the <tt>cell_number_corridor_*</tt> variables are to be multiplied by 5   before they are applied, for cell numbers below 200 they are to be   multiplied by 3, and so on.       @p correction_relaxations   is actually a vector of such list. Each entry   in this vector denotes the relaxation rules for one sweep. The last entry   defines the relaxation rules for all following sweeps. This scheme is   adopted to allow for stricter corrections in later sweeps while the   relaxations may be more generous in the first sweeps.     There is a static variable   @p default_correction_relaxations   which you   can use as a default value. It is an empty list and thus defines no   relaxations.       <li>     @p cell_number_correction_steps:   Usually, if you want the number of   cells to be corrected, the target corridor for the cell number is   computed and some additional cells are flagged or flags are removed. But   since the cell number resulting after flagging and deflagging can not be   easily computed, it will usually not be within the corridor. We therefore   need to iteratively get to our goal. Usually, three or four iterations   are needed, but using this variable, you can reduce the allowed number of   iterations; breaking the loop after two iterations yields good results   regularly. Setting the variable to zero will result in no correction   steps at all.       <li>     @p mirror_flags_to_previous_grid:   If a cell on the present grid is   flagged for refinement, also flag the corresponding cell on the previous   grid. This is useful if, for example, error indicators are computed for   space-time cells, but are stored for the second grid only. Now, since the   first grid has the same contributions to the indicators as the second, it   may be useful to flag both if necessary. This is done if the present   variable is set.       <li>     @p adapt_grids:   adapt the present grid to the previous one in the   sense defined above. What is actually done here is the following: if   going from the previous to the present grid would result in double   refinement or double coarsening of some cells, then we try to flag these   cells for refinement or coarsening such as to avoid the double step.   Obviously, more than double refinement of coarsening is also caught.     Grid adaption can try to avoid such changes between two grids, but it can   never promise that they don't occur. This is because the next grid may   change the present one, but then again there may be jumps in refinement   level between the present and the previous one; this could only be   avoided by looping iteratively through all grids, back and forth, until   nothing changes anymore, which is obviously impossible if there are many   time steps with very large grids.     </ul>
   *
   */
  template <int dim>
  struct RefinementFlags
  {
     ...
     * however.         Mirroring is done after cell number correction is
     * done, but before grid     adaption, so the cell number on this grid is
     * not noticeably influenced     by the cells flagged additionally on the
     * previous grid.
     *
     */
 ...
 * and   @p wake_up   functions declared in the base class. In addition to
 * that, it offers functions which do some rather hairy refinement rules for
 * time dependent problems, trying to avoid too much change in the grids
 * between subsequent time levels, while also trying to retain the freedom of
 * refining each grid separately. There are lots of flags and numbers
 * controlling this function, which might drastically change the behavior of
 * the function
 *
...
public:
   ...
   * coarse grid is longer than it is needed by this object.     You need to
   * give the general flags structure to this function since it is   needed
   * anyway; the refinement flags can be omitted if you do not intend   to
   * call the refinement function of this class.
   *
   */
  TimeStepBase_Tria(
    const double                   time,
   ...
   * to destroy it in the   @p sleep   function. It does so also the first
   * time we   hit this function and   @p wakeup_level   equals
   ...
   * right.)     Since this is an important task, you should call this
   * function from your   own function, should you choose to overload it in
   * your own class (which   likely is the case), preferably at the beginning
   * so that your function   can take effect of the triangulation already
   * existing.
   *
   */
  virtual void
   ...
   * sweep.   Typical kinds of data for this are data vectors, degree of
   * freedom   handlers, triangulation objects, etc. which occupy large
   * amounts of   memory and may therefore be externalized.     By default, if
   * the user specified so in the flags for this object, the   triangulation
   * is deleted and the refinement history is saved such that the   respective
   * @p wake_up   function can rebuild it. You should therefore call   this
   * function from your overloaded version, preferably at the end so that
   ...
   * class.     In fact, this function does not actually refine or coarsen the
...
protected:
   ...
   * the functions   @p sleep   and   @p wake_up   to save memory, if such a
   * behavior   is specified in the   @p flags   structure.
   *
   */
