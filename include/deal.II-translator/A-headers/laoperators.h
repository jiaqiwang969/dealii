 ...
 * deal.II includes support for describing linear transformations in a very general way. This is done with a LinearOperator class that, like   @ref ConceptMatrixType   "the MatrixType concept",
 ...
 * @endcode
 *
 * The big advantage of the LinearOperator class is that it provides syntactic
 * sugar for complex matrix-vector operations. As an example consider the
 * operation   $(A+k\,B)\,C$  , where   $A$  ,   $B$   and   $C$   denote
 ...
 * computation. The linear_operator() function can be used to wrap an ordinary
 * matrix or preconditioner object into a LinearOperator. A linear operator
 * can be transposed with transpose_operator(), or inverted by using the
 * inverse_operator() together with an iterative solver. For objects of type
 * LinearOperator, all vector space operations, i.e., addition and
 * subtraction, scalar multiplication and composition (of compatible linear
 ...
 * encapsulation of individual linear operators into blocked linear operator
 * variants. The   step-20   tutorial program has a detailed usage example of
 * the LinearOperator class.
 *
 *
 * @note   As explained below, when using LinearOperator as   <code>res =
 * op_a*x</code>   a PackagedOperation class instance is generated
 * behind-the-curtains. Consequently, the user program has to include header
 * files for both classes for compilation to be successful. In an attempt to
 * make easier the decision of which headers to include in what circumstances
 * and to prevent hidden templates-related compiler errors, all headers
 * relevant to LinearOperator are grouped in the
 * `<deal.II/lac/linear_operator_tools.h>` header file. <h3>Packaged
 * Operation</h3> An  application of a LinearOperator object to a vector via
 * <code>operator*</code>   yields a PackagedOperation object that stores this
 * computation. The PackagedOperation class allows lazy evaluation of
 * expressions involving vectors and linear operators. This is done by storing
 * the computational expression and only performing the computation when
 * either the object is implicitly converted to a vector object, or
 ...
 * results. As an example consider the addition of multiple vectors:
 *
 * @code
 * #include <deal.II/lac/linear_operator_tools.h>
 *
 ...
 *
 *  - op_a x</code> results again in an object of type PackagedOperation that stores the <i>sequence of
 ...
 *
 *
 * @note   Lazy evaluation of a computational expression necessarily involves
 ...
 * - op_a x;
 * @endcode
 * stores the computational expression of the residual with references to the
 * vector   <code>b</code> and matrix <code>A</code>  . It does not perform
 ...
 * right away, always explicitly use a vector type on the left side (and not
 * <code>auto</code>  ):
 *
 * @code
