 ...
 *
 * @ingroup dofs This module deals with constraints on degrees of freedom. The
 * central class to deal with constraints is the AffineConstraints class.
 ...
 * VectorTools::compute_no_normal_flux_constraints   function) or   prescribed
 * tangential components,   $\mathbf{n}\times\mathbf{u}=
 ...
 * function). For the former case, imagine for example that we are at   at
 * vertex where the normal vector has the form   $\frac 1{\sqrt{14}}
 * (1,2,3)^T$   and that the   $x$  -,   $y$  - and   $z$  -components of the
 * flow   field at this vertex are associated with degrees of freedom 12, 28,
 * and 40. Then the no-normal-flux condition means that we need to have   the
 * condition   $\frac 1{\sqrt{14}} (x_{12}+2x_{28}+3x_{40})=0$  .   The
 * prescribed tangential component leads to similar constraints   though there
 * is often something on the right hand side.
 *
 *
 *
 *  - If you have hanging node constraints, for example in a mesh like this:          @image html hanging_nodes.png ""     Let's assume the bottom right one of the two red degrees of freedom   is   $x_{12}$   and that the two yellow neighbors on its left and right   are   $x_{28}$   and   $x_{40}$  . Then, requiring that the finite element   function be continuous is equivalent to requiring that   $x_{12}=
   \frac 12 (x_{28}+x_{40})$  . A similar situation occurs in the   context of hp-adaptive finite element methods.   For example, when using Q1 and Q2 elements (i.e. using   FE_Q(1) and FE_Q(2)) on the two marked cells of the mesh         @image html hp-refinement-simple.png     there are three constraints: first   $x_2=\frac 12 x_0 + \frac 12 x_1$  ,   then   $x_4=\frac 14 x_0 + \frac 34 x_1$  , and finally the identity     $x_3=x_1$  . Similar constraints occur as hanging nodes even if all   cells used the same finite elements. In all of these cases, you   would use the   DoFTools::make_hanging_node_constraints   function to   compute such constraints.
 *
 *
 *
 ...
 * In all of these examples, constraints on degrees of freedom are linear and
 * possibly inhomogeneous. In other words, they always have the form
 * $x_{i_1} = \sum_{j=2}^M a_{i_j} x_{i_j} + b_i$  . The deal.II class that
 * deals with storing and using these constraints is AffineConstraints.
 *
 *  <h3>Eliminating constraints</h3> When building the global system matrix
 * and the right hand sides, one can build them without taking care of the
 * constraints, i.e. by simply looping over cells and adding the local
 * contributions to the global matrix and right hand side objects. In order to
 * do actual calculations, you have to 'condense' the linear system: eliminate
 ...
 * you eliminate (condense) away constrained nodes using the
 * AffineConstraints::condense()   functions, then you solve the remaining
 * system, and finally you compute the values of constrained nodes from the
 * values of the unconstrained ones using the
 * AffineConstraints::distribute()   function. Note that the
 * AffineConstraints::condense()   function is applied to matrix and right
 * hand side of the linear system, while the   AffineConstraints::distribute()
 * function is applied to the solution vector. This scheme of first building a
 * linear system and then eliminating constrained degrees of freedom is
 * inefficient, and a bottleneck if there are many constraints and matrices
 * are full, i.e. especially for 3d and/or higher order or hp-finite elements.
 * Furthermore, it is impossible to implement for %parallel computations where
 * a process may not have access to elements of the matrix. We therefore offer
 * a second way of building linear systems, using the
 ...
 * linear systems computed via either approach is the same, the linear systems
 * themselves do not necessarily have the same matrix and right hand side
 * vector entries. Specifically, the matrix diagonal and right hand side
 * entries corresponding to constrained degrees of freedom may be different as
 * a result of the way in which we compute them; they are, however, always
 * chosen in such a way that the solution to the linear system is the same.
 * <h4>Condensing matrices and sparsity patterns</h4> As mentioned above, the
 * first way of using constraints is to build linear systems without regards
 * to constraints and then "condensing" them away. Condensation of a matrix is
 * done in four steps:
 *
 *
 *
 ...
 * remain in place. In order not to disturb the solution process, these rows
 * and columns are filled with zeros and an appropriate positive value on the
 * main diagonal (we choose an average of the magnitudes of the other diagonal
 * elements, so as to make sure that the new diagonal entry has the same order
 * of magnitude as the other entries; this preserves the scaling properties of
 * the matrix). The corresponding value in the right hand sides is set to
 * zero. This way, the constrained node will always get the value zero upon
 * solution of the equation system and will not couple to other nodes any
 * more. Keeping the entries in the matrix has the advantage over creating a
 * new and smaller matrix, that only one matrix and sparsity pattern is needed
 * thus less memory is required. Additionally, the condensation process is
 ...
 * is larger, resulting in more memory consumption for those iterative
 * solution methods using a larger number of auxiliary vectors (e.g. methods
 * using explicit orthogonalization procedures). Nevertheless, this process is
 * more efficient due to its lower memory consumption. The condensation
 * functions exist for different argument types: SparsityPattern, SparseMatrix
 * and BlockSparseMatrix. Note that there are no versions for arguments of
 ...
 * matrices, and to modify them efficiently; this holds in particular, if the
 * matrix is actually distributed across a cluster of computers. If you want
 * to use PETSc/Trilinos matrices, you can either copy an already condensed
 * deal.II matrix, or assemble the PETSc/Trilinos matrix in the already
 * condensed form, see the discussion below.
 *
 *  <h4>Condensing vectors</h4> Condensing vectors works exactly as described
 * above for matrices. Note that condensation is an idempotent operation, i.e.
 * doing it more than once on a vector or matrix yields the same result as
 * doing it only once: once an object has been condensed, further condensation
 * operations don't change it any more. In contrast to the matrix condensation
 * functions, the vector condensation functions exist in variants for PETSc
 * and Trilinos vectors. However, using them is typically expensive, and
 * should be avoided. You should use the same techniques as mentioned above to
 * avoid their use.
 *
 *  <h4>Avoiding explicit condensation</h4> Sometimes, one wants to avoid
 * explicit condensation of a linear system after it has been built at all.
 * There are two main reasons for wanting to do so:
 * <ul>     <li>   Condensation is an expensive operation, in particular if there are many constraints and/or if the matrix has many nonzero entries. Both is typically the case for 3d, or high polynomial degree computations, as well as for hp-finite element methods, see for example the   @ref hp_paper   "hp-paper". This is the case discussed in the hp-tutorial program,   @ref   step_27 "  step-27  ", as well as in   step-22   and   @ref step_31   "  step-31  ".
 * <li>   There may not be an   AffineConstraints::condense()   function for the matrix you use (this is, for example, the case for the PETSc and Trilinos wrapper classes where we have no access to the underlying representation of the matrix, and therefore cannot efficiently implement the   AffineConstraints::condense()   operation). This is the case discussed in   step-17  ,   step-18  ,   step-31  , and   step-32  .   </ul>
 ...
 * condensed form at the time it is set up originally. The AffineConstraints
 * class offers support for these operations as well. For example, the
 * AffineConstraints::add_entries_local_to_global()   function adds nonzero
 * entries to a sparsity pattern object. It not only adds a given entry, but
 * also all entries that we will have to write to if the current entry
 * corresponds to a constrained degree of freedom that will later be
 ...
 * AffineConstraints::condense()   unnecessary. For examples of their use see
 * the tutorial programs referenced above. Note that, despite their name which
 * describes what the function really does, the
 * AffineConstraints::distribute_local_to_global()   functions has to be
 * applied to matrices and right hand side vectors, whereas the
 * AffineConstraints::distribute()   function discussed below is applied to
 * the solution vector after solving the linear system.
 *
 *  <h3>Distributing constraints</h3> After solving the condensed system of
 * equations, the solution vector has to be "distributed": the modification to
 * the original linear system that results from calling
 * AffineConstraints::condense()   leads to a linear system that solves
 * correctly for all degrees of freedom that are unconstrained but leaves the
 * values of constrained degrees of freedom undefined. To get the correct
 * values also for these degrees of freedom, you need to "distribute" the
 * unconstrained values also to their constrained colleagues. This is done by
 * the   AffineConstraints::distribute()   function. The operation of
 ...
 * constraint given the values of the unconstrained nodes plus possible
 * inhomogeneities.
 *
 *  <h3>Treatment of inhomogeneous constraints</h3> In case some constraint
 * lines have inhomogeneities (which is typically the case if the constraint
 * comes from implementation of inhomogeneous boundary conditions), the
 * situation is a bit more complicated than if the only constraints were due
 * to hanging nodes alone. This is because the elimination of the non-diagonal
 * values in the matrix generate contributions in the eliminated rows in the
 * vector. This means that inhomogeneities can only be handled with functions
 * that act simultaneously on a matrix and a vector. This means that all
 ...
 * before). The use of the AffineConstraints class for implementing Dirichlet
 ...
 * inhomogeneous constraints after creating an AffineConstraints object: First
 * approach:
 *
 *
 *
 ...
 * <code>use_inhomogeneities_for_rhs = false</code>   in
 * AffineConstraints::distribute_local_to_global()),   the linear system we
 ...
 * freedom and we need to call   AffineConstraints::distribute()   to give
 * these degrees of freedom their correct nonzero values. On the other hand,
 * in the second approach, the matrix diagonal element and corresponding right
 * hand side entry for inhomogeneously constrained degrees of freedom are so
 * that the solution of the linear system already has the correct value (e.g.,
 * if the constraint is that   $x_{13}=42$   then row   $13$   if the matrix
 * is empty with the exception of the diagonal entry, and
 ...
 * doing so. There remains the question of which of the approaches to take and
 * why we need to set to zero the values of the solution vector in the first
 ...
 * + x_2)$   where   $x_2$   is itself constrained by boundary values to
 * $x_2=1$  . In this case, the AffineConstraints container can of course not
 * figure out what the final value of   $x_3$   should be and, consequently,
 * can not set the solution vector's third component correctly. Thus, the
 * second approach will not work and you should take the first.
 *
 *  <h3>Dealing with conflicting constraints</h3>
*  There are situations where degrees of freedom are constrained in more than one way, and sometimes in conflicting ways. Consider, for example the following situation:       @image html conflicting_constraints.png ""   Here, degree of freedom   $x_0$   marked in blue is a hanging node. If we used trilinear finite elements, i.e. FE_Q(1), then it would carry the constraint   $x_0=\frac 12 (x_{1}+x_{2})$  . On the other hand, it is at the boundary, and if we have imposed boundary conditions   $u|_{\partial\Omega}=g$   then we will have the constraint   $x_0=g_0$   where   $g_0$   is the value of the boundary function   $g(\mathbf x)$   at the location of this degree of freedom.
 * So, which one will win? Or maybe: which one <i>should</i> win? There is no
 ...
 *
 *
 *  - If you want the hanging node constraints to win, then first build   these through the   DoFTools::make_hanging_node_constraints()   function.   Then interpolate the boundary values using     VectorTools::interpolate_boundary_values()   into the same   AffineConstraints object. If the latter function encounters a boundary   node that already is constrained, it will simply ignore the boundary   values at this node and leave the constraint untouched.
 *
 *
 *
 *  - If you want the boundary value constraint to win, build the hanging   node constraints as above and use these to assemble the matrix using   the   AffineConstraints::distribute_local_to_global()   function (or,   alternatively, assemble the matrix and then use     AffineConstraints::condense()   on it). In a second step, use the     VectorTools::interpolate_boundary_values()   function that returns   a   std::map   and use it as input for   MatrixTools::apply_boundary_values()     to set boundary nodes to their correct value.
 ...
 *
 *  <h3>Applying constraints indirectly with a LinearOperator</h3> Sometimes
 * it is either not desirable, or not possible to directly condense, or
 * eliminate constraints from a system of linear equations. In particular if
 * there is no underlying matrix object that could be condensed (or taken care
 * of constraints during assembly). This is usually the case if the system is
 * described by a LinearOperator.
 *  In this case we can solve the modified system @f[
 * (C^T A C + Id_c) \tilde x = C^T (b
 *
 * - A\,k)
 * @f] instead [1] (M. S. Shephard. Linear multipoint constraints applied via transformation as part of a direct stiffness assembly process. <i>International Journal for Numerical Methods in Engineering</i> 20(11):2107-2112, 1985).
 *  Here,   $A$   is a given (unconstrained) system matrix for which we only assume that we can apply it to a vector but can not necessarily access individual matrix entries.   $b$   is the corresponding right hand side of a system of linear equations   $A\,x=b$  . The matrix   $C$   describes the homogeneous part of the linear constraints stored in an AffineConstraints object and the vector   $k$   is the vector of corresponding inhomogeneities. More precisely, the   AffineConstraints::distribute()   operation applied on a vector   $x$   is the operation @f[
 *  x \leftarrow C\,x+k.
 * @f] And finally,   $Id_c$   denotes the identity on the subspace of constrained degrees of freedom.
 * The corresponding solution of   $A\,x=b$   that obeys these constraints is
 * then recovered by distributing constraints:   $x=C\tilde x+k$  . The whole
 * system can be set up and solved with the following snippet of code:
 *
 * @code
